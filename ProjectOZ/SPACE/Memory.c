...
NTSTATUS CleanCtx(ULONG xcpu, ULONG ctx)
{
    Space      *space;
    Domain     *domain;
    Mapping   **pmap;
    ...
    // free the domain structures, terminating the corresponding NT Process (and thus the CPU threads) make sure the handles get closed
    ...
    free(space);// free the space structure itself
    return STATUS_SUCCESS;
}


// insert the specified mapping for the given context and virtual page
NTSTATUS MapMemory(ULONG cpu, ULONG ctx, ULONG vpage, ULONG ppage, ULONG readmask, ULONG writemask)
{
    Space       **pspace;
    Mapping      *map;
    ULONG         bit_mask;
    ...
    // put this into the mapping tree for our space based on the changes, update the view in all domains within the space
