...
NTSTATUS Unsuspend(ULONG       cpu, SPACETOKEN  token)
{
    ...
    *prevlink = pcb->hash.link;// remove the pcb from the tokenhash
    ...
    cpuhardware[cpu].currentchain = pcb;// make the token chain the new current chain
...
NTSTATUS Resume(ULONG       cpu, ULONGLONG   result)
{
    ...
    if (!pcpu->pendinginterrupt) {
        ...
        s = NtReplyPort(spaceglobals.spaceport, (PPORT_MESSAGE)&spaces[pcb->oldctx]->domains[pcb->oldmode]->cpus[cpu]->ntreplymessage);
...
NTSTATUS DiscardToken(ULONG       cpu, SPACETOKEN  token)
{
    Hashentry   *ptr;
    Hashentry  **prevlink = NULL;
    PCB         *pcb;
...
ULONG CreatePortal(ULONG cpu, UCHAR type, UCHAR mode, UCHAR irql, ULONG ctx, ULONG handler, ULONG protmask)
{
...
NTSTATUS DestroyPortal(ULONG    cpu, ULONG    portalid)
{
    Hashentry  **prevlink = NULL;
    ...
    *prevlink = portal->hash.link;// remove the portal from the portalhash
