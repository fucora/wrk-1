...
NTSTATUS LoadKernel(PWCHAR kernelname, PULONG_PTR entrypoint, Section kernelsections[], int *nkernelsections)
{
    HANDLE                  kernelhandle;
    NTSTATUS                s;
    ...
    RtlGetFullPathName_U(kernelname, sizeof(kernelfullpath)-1, kernelfullpath, NULL) || BUGCHECK("RtlGetFullPathName... kernelname");
    ...
    NTHeaders.Signature == IMAGE_NT_SIGNATURE && pFileHeader->SizeOfOptionalHeader == sizeof(IMAGE_OPTIONAL_HEADER) ||
    ...
    while (n)
    {
        printf("%8.8s segment  virtual %08x  physical %08x  size %08x\n",
               phdr->Name,
               pOptionalHeader->ImageBase + phdr->VirtualAddress,
               physbasepage * NTSPACE_PAGESIZE,
               phdr->Misc.VirtualSize);
