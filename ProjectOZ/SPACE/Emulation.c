...
void UnexpectedMessage(PORT_MESSAGE *msg, EXCEPTION_MESSAGE *xmsg, BOOLEAN fatal)
{
    NTSTATUS s;

    printf("Unexpected message: type=%d ClientId=%d/%d (%x/%x)", msg->u2.s2.Type, msg->ClientId.UniqueProcess, msg->ClientId.UniqueThread, msg->ClientId.UniqueProcess, msg->ClientId.UniqueThread);
    if (xmsg)
    ...
    for (;;) {
        ...
        case LPC_EXCEPTION:
        {
            ...
            case 0: // DbgKmExceptionApi
                ...
                // Just ignore this message and when the interrupt resumes the instruction that caused this message will re-execute and cause it again.
                ...
                case STATUS_INVALID_LOCK_SEQUENCE:  // SPACE Hardware Emulation
                {
                    ...
                    EmulateTrap(cpu, xrec->ExceptionCode, trapdecode.iwhole, 0);// EmulateTrap for real illegal instruction
                }
...
void EmulateTrap(ULONG cpu, ULONG xcode, ULONG arg0, ULONG arg1)
{
    ...
    // assertion
    spaces[ctx] && spaces[ctx]->domains[mode] && spaces[ctx]->domains[mode]->cpus[cpu] && spaces[ctx]->domains[mode]->cpus[cpu]->ntcputhread || BUGCHECK("internal EmulateTrap");
    ...
    // find the NT thread for the current CPU in the domain we are returning to it won't exist if nothing has ever run there before
    ...
    free(temppcb);// free the temporary pcb
    //------- DEBUG
