...
//STRACE("[cpu,ctx,mode,eip]", cpu, pcb->oldctx, pcb->oldmode, pcb->ntcontext.Eip)
    ...
    if (domain->ntprocess == NULL) {
        ...
        s = RtlCreateUserProcess(&imgpath, 
                                 ...
                                 NULL,
                                 FALSE, 
                                 NULL, // in Windows XP we can provide the exception port here, and bypass the TCB privilege check
        ...
        s = NtCreateThread(&ntcputhread, THREAD_ALL_ACCESS, &obja, domain->ntprocess, &cid, &ntstartcontext, &teb, TRUE);
...
NTSTATUS StartCPU(ULONG xxcpu, ULONG boot_cpu, ULONG boot_ctx, ULONG boot_mode, ULONG_PTR boot_pc, ULONG_PTR boot_arg)
{
    CPUHardware    *pcpu;
    PCB            *pcb;
