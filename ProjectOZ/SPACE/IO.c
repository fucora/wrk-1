...
// Record this mapping, which the DMA routine may later use to perform DMA Changing while a DMA operation is in progress will cause an exception
NTSTATUS MapIO(ULONG cpu, UCHAR deviceid, UCHAR writable, ULONG iospacepage, ULONG physpage)
{
    ULONG        readmask;
    ULONG        writemask;
...
ULONGLONG AccessDevice(ULONG cpu, USHORT deviceid, UCHAR op, UCHAR deviceregister, ULONG value, ULONG valuex)
{
    Hashentry  *ptr;
    Device     *dev;
...
// Emulate a hardware operation to do DMA to/from the devicememory from/to physical memory.
struct DMAargs
{
    ULONG               cpu;
    ULONG               deviceid;
...
void DMAThread(struct DMAargs  *argp)
{
    PCHAR       vaddr;
    PCHAR       daddr;
    SIZE_T      count;
                  ...
                  PVOID callbackcontext)
{
    ...
    for (i = 0; i < npages; i++) {
        ...
        s = NtMapViewOfSection(spaceglobals.physmemsection,
                               NtCurrentProcess(),
                               (PVOID)&vaddr,
                               0,
                               NTSPACE_PAGESIZE,
