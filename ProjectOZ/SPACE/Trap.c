...
Trapvector * instantiatetrapvector()
{
...
RTL_GENERIC_COMPARE_RESULTS NTAPI VADCompare(PRTL_AVL_TABLE pVadTable, PVOID pFirst, PVOID pSecond)
{
...
PVOID NTAPI VADAlloc(struct _RTL_AVL_TABLE *Table, CLONG ByteSize)
{
...
VOID NTAPI VADFree(struct _RTL_AVL_TABLE *Table, PVOID Buffer)
{
    free(Buffer);
}
