...
)
{
    ...
    case raiseirql:
        if (irql < oirql)
            irql = oirql;     // will set irql to at least oirql
        break;
    case setirql:
        break;
    case lowerirql:
        if (irql > oirql)
            irql = oirql;     // will set irql to no more than oirql
        break;
    }

    pcpu->irql = irql;

    // if the IRQL did not go lower, then none of the pending interrupts will fire, and we can just return
    if (irql >= oirql)
        return oirql;

    // search for the highest-priority queued interrupt deepest in queue.
    // if we find one that can be interrupted we will prevent any code running before the interrupt is delivered
