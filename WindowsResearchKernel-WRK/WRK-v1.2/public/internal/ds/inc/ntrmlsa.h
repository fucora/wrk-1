...
// Memory type.  This defines the type of memory used for a record passed between the RM and LSA.

// SepRmLsaPortMemory - Memory allocated via RtlAllocateHeap() from the shared memory section associated with the Lsa command Port.
// SepRmLsaVirtualMemory - Memory allocated via ZwAllocateVirtualMemory()
// SepRmLsaUnreadableMemory - Memory not readable by the LSA.  This memory must be copied to another format before passage over the link.

// SepRmLsaLPCBufferMemory - Memory contained within the LPC buffer itself
...
// Reference Monitor Command Message Structure.
// This structure is used by the Local Security Authority to send commands to the Reference Monitor via the Reference Monitor Server Command LPC Port.
...
#define RM_COMMAND_MESSAGE_HEADER_SIZE                  (sizeof(PORT_MESSAGE) + sizeof(NTSTATUS) + sizeof(RM_COMMAND_NUMBER))


// Local Security Authority Command Message Structure.
// This structure is used by the Reference Monitor to send commands to the Local Security Authority via the LSA Server Command LPC Port.
...
#define POLICY_AUDIT_EVENT_TYPE_COUNT                                    ((ULONG) AuditEventMaxType - AuditEventMinType + 1)
#define LSARM_AUDIT_EVENT_OPTIONS_SIZE                                   (((ULONG)(POLICY_AUDIT_EVENT_TYPE_COUNT) * sizeof (POLICY_AUDIT_EVENT_OPTIONS)))
...
// The following symbol defines the value containing whether or not we're supposed to crash when an audit fails.
// It is used in the se and lsasrv directories.
