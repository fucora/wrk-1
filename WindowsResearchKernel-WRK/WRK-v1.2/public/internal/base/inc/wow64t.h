...
#define Wow64GetNumberOfX86Pages(NativePages)          (NativePages * (PAGE_SIZE >> PAGE_SHIFT_X86NT))

// Macro to round to the nearest page size
#define WOW64_ROUND_TO_PAGES(Size)          (((ULONG_PTR)(Size) + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1))

// Get number of native pages
#define WOW64_BYTES_TO_PAGES(Size)  (((ULONG)(Size) >> PAGE_SHIFT) + (((ULONG)(Size) & (PAGE_SIZE - 1)) != 0))
...
// Length in bytes of the new system directory, not counting a null terminator
...
#define Wow64GetSharedInformation(entry)         (USER_SHARED_DATA->Wow64SharedInformation [entry])
...
#if !_WIN64
__inline void * ULonglongToPtr(const ULONGLONG ull)
{
#pragma warning (push)
...
// NOTE : This structure shouldn't contain any pointer-dependent data, as it is viewed from 32-bit and 64-bit code.
...
#define WOW64_GET_TEB32_SAFE(teb64)         ((PTEB32) ((ULONGLONG)teb64 + WOW64_ROUND_TO_PAGES (sizeof (TEB))))

#define WOW64_GET_TEB32(teb64)         WOW64_GET_TEB32_SAFE(teb64)

// Update the first qword in the 64-bit TEB.
// The 32-bit rdteb instruction reads the TEB32 pointer value directly from this field.
#define WOW64_SET_TEB32(teb64, teb32)    (teb64)->NtTib.ExceptionList = (struct _EXCEPTION_REGISTRATION_RECORD *)(teb32);
...
BOOLEAN FORCEINLINE Wow64pIsExceptionRecordSource32 (PEXCEPTION_RECORD ExceptionRecord64)
{
...
VOID FORCEINLINE Wow64pMarkExceptionRecordSource32 (PEXCEPTION_RECORD ExceptionRecord64)
{
    PEXCEPTION_RECORD64 Exr64 = (PEXCEPTION_RECORD64) ExceptionRecord64;
...
VOID FORCEINLINE Wow64pClearExceptionRecordSource32 (PEXCEPTION_RECORD ExceptionRecord64)
{
    PEXCEPTION_RECORD64 Exr64 = (PEXCEPTION_RECORD64) ExceptionRecord64;
...
#if !defined(_WIN64)
#define Wow64EnableFilesystemRedirector()       NtCurrentTeb64()->TlsSlots[WOW64_TLS_FILESYSREDIR] = 0;
#define Wow64DisableFilesystemRedirector(filename)      NtCurrentTeb64()->TlsSlots[WOW64_TLS_FILESYSREDIR] = (ULONGLONG)PtrToUlong(filename);


FORCEINLINE PVOID Wow64SetFilesystemRedirectorEx (__in PVOID NewValue)
/*
Routine Description:
    This routine allows a thread running inside Wow64 to disable file-system redirection for all calls happening in the context of this thread.

    NOTE: This routine should only called from a wow64 process, and is only available when running on .NET server platforms and beyond.
          If you component will run on downlevel platforms (XP 2600 for example), you shouldn't use WOW64_FILE_SYSTEM_DISABLE_REDIRECT (see below).
...
Arguments:
    NewValue - New Wow64 file-system redirection value. This can either be:
               ...
               b- any of the following predefined values :
                  ...
                  * WOW64_FILE_SYSTEM_DISABLE_REDIRECT : Disables file-system redirection on all file I/O operations happening within the context of the current thread.
                  * WOW64_FILE_SYSTEM_DISABLE_REDIRECT_LEGACY: Use this only if you want to run on
                    download level platforms (for example XP 2600), as it will have no effect and prevents your program from malfunctioning.
...
#define Wow64GetSystemNativePageSize() ((PWOW64INFO)ULonglongToPtr((NtCurrentTeb64()->TlsSlots[WOW64_TLS_WOW64INFO])))->NativeSystemPageSize
...
FORCEINLINE VOID Wow64KiWrapApcProc (__inout PVOID *ApcContext, __inout PVOID *ApcRoutine)
/*
Routine Description:
    This routine is used by kernel mode callers to queue APCs to a thread running inside a Wow64 process.
    It wraps the original APC routine with a jacket routine inside Wow64. The target Apc routine must be inside 32-bit code.
    This routine must be executed in the context of the Wow64 thread where the target APC is going to run in.

    This routine, as it accesses a TEB-relative offset, has to be executed within a try/except block.
Environment:
    Kernel mode only.
Arguments:
...
#else
#define Wow64SetCpuSimulationFlag(NewValue)             Wow64TlsSetValue(WOW64_TLS_INCPUSIMULATION, (PVOID)NewValue)
#define Wow64GetCpuSimulationFlag()                     Wow64TlsGetValue(WOW64_TLS_INCPUSIMULATION)
#endif
