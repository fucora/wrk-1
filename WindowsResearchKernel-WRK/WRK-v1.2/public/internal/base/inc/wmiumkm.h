...
// This defines the guid under which the default WMI security descriptor is maintained.
...
// This defines the codes used to define what a request must do. These definitions must match the same in wmium.h
...
// This defines the data structure that is used to pass a handle from um to km.
// In 32bit code a handle has 32bits and in 64bit code a handle has 64 bits and both call into the kernel which is 64bits.
// In order to ensure that the data structures compile to the same size on 32 and 64
// bit systems we define the union with a dummy 64bit value so the field is forced to be 64 bits in all code.
// Note that the object manager always ignores the top 32bits of the handle in order to support 32 bit code that only maintains 32 bit handles
typedef union
{
    HANDLE  Handle;
    ULONG64 Handle64;
...
#ifdef _WIN64
#define WmipSetHandle3264(Handle3264, XHandle)     (Handle3264).Handle = XHandle
#else
#define WmipSetHandle3264(Handle3264, XHandle) { (Handle3264).Handle64 = 0; (Handle3264).Handle32 = (ULONG32)XHandle; }
#endif
...
#define IOCTL_WMI_READ_NOTIFICATIONS           CTL_CODE(FILE_DEVICE_UNKNOWN, WmiReadNotifications, METHOD_BUFFERED, FILE_READ_ACCESS)
