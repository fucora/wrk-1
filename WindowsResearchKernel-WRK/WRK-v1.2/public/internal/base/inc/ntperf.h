...
#define PERFINFO_CLEAR_GROUPMASK(pGroupMask)     RtlZeroMemory((pGroupMask), sizeof(PERFINFO_GROUPMASK))
#define PERFINFO_OR_GROUP_WITH_GROUPMASK(Group, pGroupMask)     (pGroupMask)->Masks[PERF_GET_MASK_INDEX(Group)] |= PERF_GET_MASK_GROUP(Group);

// Determines whether any group is on in a group mask
#define PerfIsAnyGroupOnInGroupMask(pGroupMask)     (pGroupMask != NULL)

// Determines whether a group is on in its set in a group mask

__forceinline BOOLEAN PerfIsGroupOnInGroupMask(ULONG Group,  PPERFINFO_GROUPMASK GroupMask)
/*
Routine Description:
    Determines whether any group is on in a group mask
Arguments:
...
#define PERFINFO_HOOK_HANDLE_TO_DATA(_HookHandle, _Type)     ((_Type) (&((_HookHandle).PerfTraceHeader)->Data[0]))
...
typedef struct _PERFINFO_DRIVER_COMPLETE_REQUEST {
    ...
    // Irp field and UniqMatchId is used to match COMPLETE_REQUEST and COMPLETE_REQUEST_RET logged for an IRP completion.
    PVOID Irp;
    ULONG UniqMatchId;
...
// This structure is logged when PopSetDevicesSystemState is called to propagate a system state to all devices.
...
// This structure is logged when PopNotifyDevice calls into a driver to set the power state of a device.
typedef struct _PERFINFO_PO_NOTIFY_DEVICE {
    PVOID Irp;// This field is used to match notification and completion log entries for a device.
    PVOID DriverStart;// Base address of the driver that owns this device.
    UCHAR OrderLevel;// Device node properties.
    ...
    // Length of the device name in characters excluding terminating NUL, and the device name itself.
    // Depending on how much fits into our stack buffer, this is the *last* part of the device name.
    ULONG DeviceNameLength;
    WCHAR DeviceName[1];
} PERFINFO_PO_NOTIFY_DEVICE, *PPERFINFO_PO_NOTIFY_DEVICE;

// This structure is logged when a PopNotifyDevice processing for a particular device completes.
typedef struct _PERFINFO_PO_NOTIFY_DEVICE_COMPLETE {
    PVOID Irp;// This field is used to match notification and completion log entries for a device.
    NTSTATUS Status;// Status with which the notify power IRP was completed.
