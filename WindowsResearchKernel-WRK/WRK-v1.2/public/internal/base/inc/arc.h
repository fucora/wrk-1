...
typedef struct _LOADER_PARAMETER_EXTENSION {
    ...
    PVOID TriageDumpBlock;// Pointer to the triage block, if present.

    ULONG LoaderPagesSpanned;   // Virtual Memory spanned by the loader that MM cannot recover the VA for.
    ...
    // If booting from the Network (PXE) then we will save the Network boot params in this loader block
    ...
    LIST_ENTRY  FirmwareDescriptorListHead;// Firmware Location

    // Pointer to the in-memory copy of override ACPI tables.
    // The override table file is a simple binary file with one or more ACPI tables laid out one after another.
    PVOID   AcpiTable;

    ULONG   AcpiTableSize;// Size of override ACPI tables in bytes.
