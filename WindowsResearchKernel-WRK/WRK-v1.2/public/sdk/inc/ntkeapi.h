...
Abstract:
    This module contains the include file for data types that are exported by kernel for general use.
*/
...
typedef struct _KSYSTEM_TIME {// Define system time structure.
    ULONG LowPart;
    LONG High1Time;
    LONG High2Time;
...
// N.B. The lock field of the spin lock queue structure contains the address of the associated kernel spin lock, an owner bit, and a lock bit.
//      Bit 0 of the spin lock address is the wait bit and bit 1 is the owner bit.
//      The use of this field is such that the bits can be set and cleared noninterlocked, however, the back pointer must be preserved.

//      The lock wait bit is set when a processor enqueues itself on the lock queue and it is not the only entry in the queue.
//      The processor will spin on this bit waiting for the lock to be granted.

//      The owner bit is set when the processor owns the respective lock.

//      The next field of the spin lock queue structure is used to line the queued lock structures together in fifo order.
//      It also can set set and cleared noninterlocked.
...
typedef enum _KPROFILE_SOURCE {// Profile source types
    ProfileTime,
...
NTSYSCALLAPI NTSTATUS NTAPI NtCallbackReturn (__in_bcount_opt(OutputLength) PVOID OutputBuffer, __in ULONG OutputLength, __in NTSTATUS Status);

NTSYSCALLAPI NTSTATUS NTAPI NtQueryDebugFilterState (__in ULONG ComponentId, __in ULONG Level);
NTSYSCALLAPI NTSTATUS NTAPI NtSetDebugFilterState (__in ULONG ComponentId, __in ULONG Level, __in BOOLEAN State);
