...
// This file contains io definitions that must be portable across operating systems (i.e. DOS)
...
// The FILE_READ_DATA and FILE_WRITE_DATA constants are also defined in devioctl.h as FILE_READ_ACCESS and FILE_WRITE_ACCESS.
// The values for these constants *MUST* always be in sync.
// The values are redefined in devioctl.h because they must be available to both DOS and NT.
...
// Note:  0x00000010 is reserved for use for the old DOS SUBDIRECTORY flag and is therefore not considered valid in NT.
//        This flag has been disassociated with file attributes since the other flags are protected with READ_ and WRITE_ATTRIBUTES access to the file.

// Note:  Note also that the order of these flags is set to allow both the
//        FAT and the Pinball File Systems to directly set the attributes flags in attributes words without having to pick each flag out individually.
//        The order of these flags should not be changed!
...
// Define the I/O status information return values for requests for oplocks via NtFsControlFile
#define FILE_OPLOCK_BROKEN_TO_LEVEL_2   0x00000007
#define FILE_OPLOCK_BROKEN_TO_NONE      0x00000008


// Define the I/O status information return values for NtCreateFile/NtOpenFile when the sharing access fails but a batch oplock break is in progress
...
// The FILE_EXPECT flags will only exist for WinXP.
// After that they will be ignored and an IRP will be sent in their place.
...
// flags specified here will be propagated up and down a device stack after FDO and all filter devices are added, but before the device stack is started
...
NTSYSCALLAPI NTSTATUS NTAPI NtCancelIoFile (__in HANDLE FileHandle, __out PIO_STATUS_BLOCK IoStatusBlock);
...
NTSYSCALLAPI NTSTATUS NTAPI NtDeleteFile (__in POBJECT_ATTRIBUTES ObjectAttributes);
NTSYSCALLAPI NTSTATUS NTAPI NtFlushBuffersFile (__in HANDLE FileHandle, __out PIO_STATUS_BLOCK IoStatusBlock);
...
NTSYSCALLAPI NTSTATUS NTAPI NtQueryAttributesFile (__in POBJECT_ATTRIBUTES ObjectAttributes, __out PFILE_BASIC_INFORMATION FileInformation);
NTSYSCALLAPI NTSTATUS NTAPI NtQueryFullAttributesFile(__in POBJECT_ATTRIBUTES ObjectAttributes, __out PFILE_NETWORK_OPEN_INFORMATION FileInformation);
...
NTSYSCALLAPI NTSTATUS NTAPI NtLoadDriver (__in PUNICODE_STRING DriverServiceName);
NTSYSCALLAPI NTSTATUS NTAPI NtUnloadDriver (__in PUNICODE_STRING DriverServiceName);
...
// These macros are defined in devioctl.h which contains the portable IO definitions (for use by both DOS and NT)
...
// General File System control codes - Note that these values are valid regardless of the actual file system type

//  IMPORTANT:  These values have been arranged in order of increasing control codes.  Do NOT breaks this!!  Add all new codes at end of list regardless of functionality type.

//  Note: FSCTL_QUERY_RETRIEVAL_POINTER and FSCTL_MARK_AS_SYSTEM_HIVE only work from Kernel mode on local paging files or the system hives.

// begin_winioctl
#ifndef _FILESYSTEMFSCTL_
#define _FILESYSTEMFSCTL_

// The following is a list of the native file system fsctls followed by additional network file system fsctls.
...
// The following long list of structs are associated with the preceding file system fsctls.
...
// The user must pass the basic buffer below.
// Ntfs will return as many fields as available in the extended buffer which follows immediately after the VOLUME_DATA_BUFFER.
...
//  In general, the MinorVersion may be changed if fields are added to this structure in such a way that the previous version of the software can still correctly the fields it knows about.
//  The MajorVersion should only be changed if the previous version of any software using this structure would incorrectly handle new records due to structure changes.

//  The first update to this will force the structure to version 2.0.
//  This will add the extended information about the source as well as indicate the file name offset within the structure.
...
//      USN_SOURCE_DATA_MANAGEMENT - Service is not modifying the external view of any part of the file.
//          Typical case is HSM moving data to and from external storage.
//      USN_SOURCE_AUXILIARY_DATA - Service is not modifying the external view of the file with regard to the application that created this file.
...
typedef struct _FILE_OBJECTID_BUFFER {
    UCHAR ObjectId[16];//  This is the portion of the object id that is indexed.
...
// Querying the allocated ranges requires an output buffer to store the allocated ranges and an input buffer to specify the range to query.
// The input buffer contains a single entry, the output buffer is an array of the following structure.
...
//  The input buffer to set encryption indicates whether we are to encrypt/decrypt a file or an individual stream.
...
//  The optional output buffer to set encryption indicates that the last encrypted stream in a file has been marked as decrypted.
...
//  Request Encrypted Data structure.
//  This is used to indicate
//  the range of the file to read.
//  It also describes the output buffer used to return the data.


typedef struct _REQUEST_RAW_ENCRYPTED_DATA {
    //  Requested file offset and requested length to read.
    //  The fsctl will round the starting offset down to a file system boundary.
    //  It will also round the length up to a file system boundary.
    LONGLONG FileOffset;
    ULONG Length;
} REQUEST_RAW_ENCRYPTED_DATA, *PREQUEST_RAW_ENCRYPTED_DATA;


//  Encrypted Data Information structure.
//  This structure is used to return raw encrypted data from a file in order to perform off-line recovery.
//  The data will be encrypted or encrypted and compressed.
//  The off-line service will need to use the encryption and compression format information to recover the file data.
//  In the event that the data is both encrypted and compressed then the decryption must occur before decompression.
//  All the data units below must be encrypted and compressed with the same format.

//  The data will be returned in units.
//  The data unit size will be fixed per request.
//  If the data is compressed then the data unit size will be the compression unit size.

//  This structure is at the beginning of the buffer used to return the encrypted data.
//  The actual raw bytes from the file will follow this buffer.
//  The offset of the raw bytes from the beginning of this structure is specified in the REQUEST_RAW_ENCRYPTED_DATA structure described above.

typedef struct _ENCRYPTED_DATA_INFO {
    //  This is the file offset for the first entry in the data block array.
    //  The file system will round the requested start offset down to a boundary that is consistent with the format of the file.
    ULONGLONG StartingFileOffset;

    //  Data offset in output buffer.
    //  The output buffer begins with an ENCRYPTED_DATA_INFO structure.
    //  The file system will then store the raw bytes from disk beginning at the following offset within the output buffer.
    ULONG OutputBufferOffset;

    //  The number of bytes being returned that are within the size of the file.
    //  If this value is less than (NumberOfDataBlocks << DataUnitShift), it means the end of the file occurs within this transfer.
    //  Any data beyond file size is invalid and was never passed to the encryption driver.
    ULONG BytesWithinFileSize;

    //  The number of bytes being returned that are below valid data length.
    //  If this value is less than (NumberOfDataBlocks << DataUnitShift), it means the end of the valid data occurs within this transfer.
    //  After decrypting the data from this transfer, any byte(s) beyond valid data length must be zeroed.
    ...
    //  The DataUnit is the granularity used to access the disk.
    //  It will be the same as the compression unit size for a compressed file.
    //  For an uncompressed file, it will be some cluster-aligned power of 2 that the file system deems convenient.
    //  A caller should not expect that successive calls will have the same data unit shift value as the previous call.

    //  Since chunks and compression units are expected to be powers of 2 in size, we express them log2.
    ...
    //  at least one cluster less in bytes than an uncompressed data block unit.
    UCHAR DataUnitShift;
    UCHAR ChunkShift;
    UCHAR ClusterShift;

    UCHAR EncryptionFormat;//  The format for the encryption.

    //  This is the number of entries in the data block size array.
    USHORT NumberOfDataBlocks;

    //  This is an array of sizes in the data block array.
    //  There must be one entry in this array for each data block read from disk.
    //  The size has a different meaning depending on whether the file is compressed.

    //  A size of zero always indicates that the final data consists entirely
    //  of zeroes.  There is no decryption or decompression to perform.

    //  If the file is compressed then the data block size indicates whether this block is compressed.
    //  A size equal to the block size indicates that the corresponding block did not compress.
    //  Any other non-zero size indicates the size of the compressed data which needs to be decrypted/decompressed.

    //  If the file is not compressed then the data block size indicates the amount of data within the block that needs to be decrypted.
    //  Any other non-zero size indicates that the remaining bytes in the data unit within the file consists of zeros.
    //  An example of this is when the the read spans the valid data length of the file.
    //  There is no data to decrypt past the valid data length.
...
//  Request Plex Read Data structure.
//  This is used to indicate the range of the file to read.
//  It also describes which plex to perform the read from.
typedef struct _PLEX_READ_DATA_REQUEST {
    //  Requested offset and length to read.
    //  The offset can be the virtual offset (vbo) in to a file, or a volume.
    //  In the case of a file offset, the fsd will round the starting offset down to a file system boundary.
    //  It will also round the length up to a file system boundary and enforce any other applicable limits.
...
// the beginning of FileNameBuffer, and the destination name immediately following.
// Length fields include terminating nulls.
...
// The reparse structure is used by layered drivers to store data in a reparse point.
...
// store data in a reparse point. For non-Microsoft tags,
// The GUID field cannot be GUID_NULL.
// The constraints on reparse tags are defined below.
...
// The reparse information structure is used to return information about a reparse point to the caller.
...
// The following constant represents the bits that are valid to use in reparse tags.
...
// The reparse tag 0x80000008 is reserved for Microsoft internal use (may be published in the future)
...
#define IO_REPARSE_TAG_FRS_GHOST                (0x80000012L)//      Tag allocated to Microsoft for FRS Ghosting support


// Non-Microsoft tags for reparse points
#define IO_REPARSE_TAG_IFSTEST_CONGRUENT        (0x00000009L)// Tag allocated to CONGRUENT, May 2000. Used by IFSTEST
#define IO_REPARSE_TAG_ARKIVIO                  (0x0000000CL)// Tag allocated to ARKIVIO
#define IO_REPARSE_TAG_SOLUTIONSOFT             (0x2000000DL)//  Tag allocated to SOLUTIONSOFT
#define IO_REPARSE_TAG_COMMVAULT                (0x0000000EL)//  Tag allocated to COMMVAULT
#define IO_REPARSE_TAG_OVERTONE                 (0x0000000FL)//      Tag allocated to Overtone Software
#define IO_REPARSE_TAG_ENIGMA_HSM               (0x00000011L)//      Tag allocated to Enigma Data for HSM
#define IO_REPARSE_TAG_INTERCOPE_HSM            (0x00000013L)//      Tag allocated to INTERCOPE for HSM
#define IO_REPARSE_TAG_KOM_NETWORKS_HSM         (0x00000014L)//      Tag allocated to KOM Networks for HSM
#define IO_REPARSE_TAG_MEMORY_TECH_HSM          (0x00000015L)//      Tag allocated to MEMORY_TECH for HSM

// The following three FSCTLs are placed in this file to facilitate sharing between the redirector and the IO subsystem

// This FSCTL is used to garner the link tracking information for a file.
// The data structures used for retrieving the information are LINK_TRACKING_INFORMATION defined further down in this file.
...
// This FSCTL is used to update the link tracking information on a server for an intra machine/ inter volume move on that server
...
// The following IOCTL is used in link tracking implementation.
// It determines if the two file objects passed in are on the same server.
...
// This is an extension to the client process info buffer containing the client computer name
...
// For links on DFS volumes the volume id and machine id are returned for link tracking
...
NTSYSCALLAPI NTSTATUS NTAPI NtOpenIoCompletion (__out PHANDLE IoCompletionHandle, __in ACCESS_MASK DesiredAccess, __in POBJECT_ATTRIBUTES ObjectAttributes);
...
// Defines that are used to access the registry, but are not registry specific.
