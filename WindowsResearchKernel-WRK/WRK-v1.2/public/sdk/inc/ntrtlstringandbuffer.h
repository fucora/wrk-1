...
Abstract:
    Broken out from nturtl and rtl so I can move it between them in separate trees without merge madness.
    To be integrated into ntrtl.h
*/
...
// odd but correct use of RTL_STRING_IS_PUT_AT_SAFE instead of RTL_STRING_IS_GET_AT_SAFE, we are reaching past the Length
#define RTL_STRING_IS_NUL_TERMINATED(s)    (RTL_STRING_IS_PUT_AT_SAFE(s, RTL_STRING_GET_LENGTH_CHARS(s), 0) && RTL_STRING_GET_AT_UNSAFE(s, RTL_STRING_GET_LENGTH_CHARS(s)) == 0)
...
#define RTL_STRING_GET_AT(s,n)         (ASSERT(RTL_STRING_IS_GET_AT_SAFE(s,n)),                                        RTL_STRING_GET_AT_UNSAFE(s,n))
#else
#define RTL_STRING_GET_AT(s,n)         (RTL_STRING_GET_AT_UNSAFE(s,n))
#endif

#if defined(RTL_STRING_RANGE_CHECKED)
#define RTL_STRING_PUT_AT(s,n,c)       (ASSERT(RTL_STRING_IS_PUT_AT_SAFE(s,n,c)),                                        RTL_STRING_PUT_AT_UNSAFE(s,n,c))
#else
...
// NTSTATUS RtlInitBuffer(OUT PRTL_BUFFER Buffer, IN PUCHAR StaticBuffer, IN SIZE_T StaticSize);
// Routine Description:
...
NTSYSAPI NTSTATUS NTAPI RtlpEnsureBufferSize(IN ULONG Flags, IN OUT PRTL_BUFFER Buffer, IN SIZE_T NewSizeBytes);


// NTSTATUS RtlEnsureBufferSize(IN OUT PRTL_BUFFER Buffer, IN SIZE_T NewSizeBytes);
// Routine Description:
// If Buffer is smaller than NewSize, grow it to NewSize, using the static buffer if it is large enough, else heap allocating
// Arguments:
//     Flags -
...
// VOID RtlFreeBuffer(IN OUT PRTL_BUFFER Buffer,);
// Routine Description:
// Free any heap allocated storage associated with Buffer.
// Notes:
// - RtlFreeBuffer returns a buffer to the state it was in after calling RtlInitBuffer, so you may reuse it.
...
// - You may RtlFreeBuffer an RTL_BUFFER that is all zeros. You do not need to track if you called RtlInitBuffer if you know you filled it with zeros.
...
// MAX_UNICODE_STRING_LENGTH is the maximum allowed value for UNICODE_STRING::Length, allowing room for a terminal nul.
...
// NTSTATUS RtlInitUnicodeStringBuffer(OUT PRTL_UNICODE_STRING_BUFFER Buffer, IN PUCHAR StaticBuffer, IN SIZE_T StaticSize);
// Arguments:
//     Buffer -
//     StaticBuffer - can be NULL, but generally is not
//     StaticSize - should be at least sizeof(WCHAR), but can be zero ought to be an even multiple of sizeof(WCHAR)
//                  gets rounded to down to an even multiple of sizeof(WCHAR)
//                  gets clamped to MAX_UNICODE_STRING_MAXLENGTH (64k - 2)
// RTL_UNICODE_STRING_BUFFER contains room for the terminal nul for the case of StaticBuffer == NULL or StaticSize < sizeof(WCHAR), or, more likely, for RtlTakeRemainingStaticBuffer leaving it with no static buffer.
// Return Value:
//     STATUS_SUCCESS
...
// NTSTATUS RtlSyncStringToBuffer(IN OUT PRTL_UNICODE_STRING_BUFFER UnicodeStringBuffer);
// Routine Description:
// After carefully modifying the underlying RTL_BUFFER, this updates dependent fields in the underlying UNICODE_STRING.
// For example, use this after you grow the buffer with RtlEnsureBufferSize, but that example you don't need, use RtlEnsureUnicodeStringBufferSizeChars or RtlEnsureStringBufferSizeBytes.
// Return Value:
//     STATUS_SUCCESS
...
// NTSTATUS RtlSyncBufferToString(IN OUT PRTL_UNICODE_STRING_BUFFER UnicodeStringBuffer);
// Routine Description:
// After carefully modifying the underlying UNICODE_STRING, this updates dependent fields in the underlying RTL_BUFFER.
// For example, use this after you the alloc the buffer with RtlAnsiStringToUnicodeString.
// This is possible because RTL_BUFFER deliberately uses the same memory allocator as RtlAnsiStringToUnicodeString.
// Return Value:
//     STATUS_SUCCESS
...
// NTSTATUS RtlEnsureUnicodeStringBufferSizeChars(IN OUT PRTL_BUFFER Buffer, IN USHORT NewSizeChars);
// NTSTATUS RtlEnsureUnicodeStringBufferSizeBytes(IN OUT PRTL_BUFFER Buffer, IN USHORT NewSizeBytes);
...
//     Buffer -
//     NewSizeChars -
//     NewSizeBytes - must be a multiple of sizeof(WCHAR), and we don't presently verify that.
...
// NTSTATUS RtlAppendUnicodeStringBuffer(OUT PRTL_UNICODE_STRING_BUFFER Destination, IN PCUNICODE_STRING Source);
// Return Value:
//     STATUS_SUCCESS
//     STATUS_NO_MEMORY
...
NTSYSAPI NTSTATUS NTAPI RtlMultiAppendUnicodeStringBuffer(OUT PRTL_UNICODE_STRING_BUFFER  Destination, IN  ULONG NumberOfSources, IN  const UNICODE_STRING* SourceArray);


// VOID RtlFreeUnicodeStringBuffer(OUT PRTL_UNICODE_STRING_BUFFER Buffer);
// Return Value:
//     none, unconditional success

// If Buffer is a local, the stores are generally "dead" (their result is never read) and the optimizer should "kill" them (not bother performing them).

// A buffer can be freed multiple times.
// A buffer that has been freed is in the same state as one that just been initialized.
// A buffer that is all zeros (RtlZeroMemory) can be freed. You do not need to track if you initialized it.
...
// NTSTATUS RtlAssignUnicodeStringBuffer(IN OUT PRTL_UNICODE_STRING_BUFFER Buffer, PCUNICODE_STRING String);
// Return Value:
//     STATUS_SUCCESS
//     STATUS_NO_MEMORY
#define RtlAssignUnicodeStringBuffer(Buff, Str)     (((Buff)->String.Length = 0), (RtlAppendUnicodeStringBuffer((Buff), (Str))))


// NTSTATUS RtlTakeRemainingStaticBuffer(IN OUT PRTL_BUFFER Buffer, OUT PUCHAR* RemainingStaticBuffer, OUT SIZE_T* RemainingStaticSize);

// Routine Description:
// This function makes it easy to share a static buffer among multiple buffers, as long as the buffers are actually initialized
// and "sealed" linearly/independently/one after another with no "overlap" (overlap in control and dataflow, not in actual addresses).

// Note that if a buffer is using exactly all of its static buffer, you will get back 0 and STATUS_SUCCESS.
// This is not an error condition.
// This is why RtlInitUnicodeStringBuffer can now accept zero sized static buffers.

// Note that even if you violate the conditions that make this function most useful, your code will still work, just less quickly.

// A pattern that should work is allocating one static buffer and moving it "through" multiple buffers.
// Even if you run out of static space, it should work to move the remaining zero size static buffer forward. Of course, you'll heap allocate.
...
// NTSTATUS RtlUnicodeStringBufferTakeValue(OUT NTSTATUS* Status, IN ULONG Flags, IN OUT RTL_UNICODE_STRING_BUFFER DestinationBuffer, IN OUT RTL_UNICODE_STRING_BUFFER SourceBuffer);
...
NTSTATUS NTAPI RtlInitAnsiStringBuffer(PRTL_ANSI_STRING_BUFFER StringBuffer, PUCHAR StaticBuffer, SIZE_T StaticSize);
VOID NTAPI RtlFreeAnsiStringBuffer(PRTL_ANSI_STRING_BUFFER AnsiStringBuffer);
NTSTATUS NTAPI RtlAssignAnsiStringBufferFromUnicodeString( PRTL_ANSI_STRING_BUFFER AnsiStringBuffer, PCUNICODE_STRING UnicodeString);
NTSTATUS NTAPI RtlAssignAnsiStringBufferFromUnicode(PRTL_ANSI_STRING_BUFFER AnsiStringBuffer, PCWSTR Unicode);
NTSTATUS NTAPI RtlUnicodeStringBufferEnsureTrailingNtPathSeparator(PRTL_UNICODE_STRING_BUFFER StringBuffer);
