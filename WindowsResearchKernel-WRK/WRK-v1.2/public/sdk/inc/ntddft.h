...
#define DISK_REGISTRY_KEY  "\\REGISTRY\\MACHINE\\SYSTEM\\DISK"// Define for registry information.
#define DISK_REGISTRY_VALUE "Information"// Define for value name for registry information.
...
// FT_STATE is an enumerated field that describes the state of the FT volume as a whole.
...
// State information for an FT set.  The partition state enum is used to specify the state for the complete set.
...
// When this state is set, FT assumes at boot time that the volume must be initialized and starts a system thread of execution to perform the initialization work.
...
typedef struct _FT_DESCRIPTION {
    ...
    USHORT   Type;
    ULONG    Reserved;
    FT_STATE FtVolumeState;// The volume state will indicate which member is being regenerated
    FT_MEMBER_DESCRIPTION FtMemberDescription[1];// Array for NumberOfMembers
...
typedef struct _FT_REGISTRY {
    ...
    USHORT ReservedShort;
    FT_DESCRIPTION FtDescription[1];// An array of NumberOfComponents
