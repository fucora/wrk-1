...
{
    ULONG Version;// The size of this structure is used for versioning
    DISK_CACHE_STATE State;// Indicates whether there are any issues with the disk cache
    BOOLEAN IsPowerProtected;// Indicates whether the disk cache is power protected or not
...
// Internal disk driver device controls to maintain the verify status bit for the device object.
...
// BOOLEAN IsRecognizedPartition(IN ULONG PartitionType)
// Routine Description:
//     This macro is used to determine to which partitions drive letters should be assigned.
// Arguments:
//     PartitionType - Supplies the type of the partition being examined.
// Return Value:
//     The return value is TRUE if the partition type is recognized, otherwise FALSE is returned.
...
// BOOLEAN IsContainerPartition(IN ULONG PartitionType)
// Routine Description:
//     This macro is used to determine to which partition types are actually containers for other partitions (ie, extended partitions).
// Arguments:
//     PartitionType - Supplies the type of the partition being examined.
// Return Value:
...
// BOOLEAN IsFTPartition(IN ULONG PartitionType)
// Routine Description:
//     This macro is used to determine if the given partition is an FT partition.
// Arguments:
//     PartitionType - Supplies the type of the partition being examined.
// Return Value:
...
// request and an array of them is returned on an IOCTL_DISK_GET_MEDIA_TYPES request.
...
// There are currently two ways a disk can be partitioned.
// With a traditional AT-style master boot record (PARTITION_STYLE_MBR) and with a new, GPT partition table (PARTITION_STYLE_GPT).
// RAW is for an unrecognizable partition style.
// There are a very limited number of things you can do with a RAW partition.
...
//  The following are GPT partition attributes applicable for any partition type. These attributes are not OS-specific
#define GPT_ATTRIBUTE_PLATFORM_REQUIRED             (0x0000000000000001)

// The following are GPT partition attributes applicable when the PartitionType is PARTITION_BASIC_DATA_GUID.
...
// The structure SET_PARTITION_INFO_EX is used with the ioctl IOCTL_SET_PARTITION_INFO_EX to set information about a specific partition.
// Note that for MBR partitions, you can only set the partition signature, whereas GPT partitions allow setting of all fields that you can get.
...
// The structure CREATE_DISK_GPT with the ioctl IOCTL_DISK_CREATE_DISK to initialize an virgin disk with an empty GPT partition table.
...
// The structure CREATE_DISK_MBR with the ioctl IOCTL_DISK_CREATE_DISK to initialize an virgin disk with an empty MBR partition table.
...
typedef struct _DISK_DETECTION_INFO {
        ...
        union {
                struct {
                        DISK_INT13_INFO Int13;// If DetectionType == DETECTION_INT13 then we have just the Int13 information.

                        // If DetectionType == DETECTION_EX_INT13, then we have the extended int 13 information.
...
// The Geometry structure is a variable length structure composed of a DISK_GEOMETRY_EX structure followed by a DISK_PARTITION_INFO structure followed by a DISK_DETECTION_DATA structure.
#define DiskGeometryGetPartition(Geometry) ((PDISK_PARTITION_INFO)((Geometry)->Data))

#define DiskGeometryGetDetect(Geometry)    ((PDISK_DETECTION_INFO)(((ULONG_PTR)DiskGeometryGetPartition(Geometry)+DiskGeometryGetPartition(Geometry)->SizeOfPartitionInfo)))
...
// IOCTL_DISK_SET_CACHE allows the caller to get or set the state of the disk read/write caches.

// If the structure is provided as the input buffer for the ioctl the read & write caches will be enabled or disabled depending on the parameters provided.

// If the structure is provided as an output buffer for the ioctl the state of the read & write caches will be returned.
// If both input and outut buffers are provided the output buffer will contain the cache state BEFORE any changes are made
typedef enum {
    EqualPriority,
    KeepPrefetchedData,
    KeepReadData
...
typedef struct _DISK_CACHE_INFORMATION {
    // on return indicates that the device is capable of saving any parameters in non-volatile storage.
    // On send indicates that the device should save the state in non-volatile storage.
    ...
    // Controls the likelyhood of data remaining in the cache depending on how it got there.
    // Data cached from a READ or WRITE operation may be given higher, lower or equal priority to data entered into the cache for other means (like prefetch)
    DISK_CACHE_RETENTION_PRIORITY ReadRetentionPriority;
    DISK_CACHE_RETENTION_PRIORITY WriteRetentionPriority;

    // Requests for a larger number of blocks than this may have prefetching disabled.
    // If this value is set to 0 prefetch will be disabled.
    USHORT DisablePrefetchTransferLength;

    // If TRUE then ScalarPrefetch (below) will be valid.
    // If FALSE then the minimum and maximum values should be treated as a block count (BlockPrefetch)
    BOOLEAN PrefetchScalar;

    // Contains the minimum and maximum amount of data which will be will be prefetched into the cache on a disk operation.
    // This value may either be a scalar multiplier of the transfer length of the request, or an absolute number of disk blocks.
    // PrefetchScalar (above) indicates which interpretation is used.
    union {
        struct {
            USHORT Minimum;
            USHORT Maximum;

            // The maximum number of blocks which will be prefetched - useful with the scalar limits to set definite upper limits.
...
// IOCTL_DISK_GROW_PARTITION will update the size of a partition by adding sectors to the length.
// The number of sectors must be predetermined by examining PARTITION_INFORMATION.
...
// The following structure is exchanged on an IOCTL_DISK_GET_PERFORMANCE request.
// This ioctl collects summary disk request statistics used in measuring performance.
...
// This structure defines the disk logging record.
// When disk logging is enabled, one of these is written to an internal buffer for each disk request.
...
// We keep it as flexible as possible, for instance, IO sizes are interesting in ranges like 0-4096, 4097-16384, 16385-65536, 65537+.
...
typedef struct _DRIVERSTATUS {
        UCHAR    bDriverError; // Error code from driver, or 0 if no error.
...
// The following device control code is for the SIMBAD simulated bad sector facility.
// See SIMBAD.H in this directory for related structures.
