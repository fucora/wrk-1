...
NTSYSAPI LOGICAL NTAPI RtlIsCriticalSectionLocked (IN PRTL_CRITICAL_SECTION CriticalSection);
NTSYSAPI LOGICAL NTAPI RtlIsCriticalSectionLockedByThread (IN PRTL_CRITICAL_SECTION CriticalSection);
NTSYSAPI ULONG NTAPI RtlGetCriticalSectionRecursionCount (IN PRTL_CRITICAL_SECTION CriticalSection);
NTSYSAPI LOGICAL NTAPI RtlTryEnterCriticalSection(PRTL_CRITICAL_SECTION CriticalSection);
...
NTSYSAPI VOID NTAPI RtlCheckHeldCriticalSections(IN HANDLE hThread, IN PRTL_CRITICAL_SECTION const *LocksHeld);
...
NTSYSAPI PVOID NTAPI RtlAddVectoredExceptionHandler (IN ULONG First, IN PVECTORED_EXCEPTION_HANDLER Handler);
NTSYSAPI ULONG NTAPI RtlRemoveVectoredExceptionHandler (IN PVOID Handle);
NTSYSAPI BOOLEAN NTAPI RtlCallVectoredContinueHandlers (IN PEXCEPTION_RECORD ExceptionRecord, IN PCONTEXT ContextRecord);
NTSYSAPI PVOID NTAPI RtlAddVectoredContinueHandler (IN ULONG First, IN PVECTORED_EXCEPTION_HANDLER Handler);
NTSYSAPI ULONG NTAPI RtlRemoveVectoredContinueHandler (IN PVOID Handle);
...
RTL_PATH_TYPE NTAPI RtlDetermineDosPathNameType_Ustr(IN PCUNICODE_STRING String);
NTSYSAPI ULONG NTAPI RtlIsDosDeviceName_U(PCWSTR DosFileName);
...
NTSYSAPI ULONG NTAPI RtlGetCurrentDirectory_U(__in ULONG nBufferLength, __out_bcount(nBufferLength) PWSTR lpBuffer);
NTSYSAPI NTSTATUS NTAPI RtlSetCurrentDirectory_U(IN PCUNICODE_STRING PathName);
NTSYSAPI ULONG NTAPI RtlGetLongestNtPathLength( VOID );
NTSYSAPI BOOLEAN NTAPI RtlDosPathNameToNtPathName_U(__in PCWSTR DosFileName, __out PUNICODE_STRING NtFileName, __out_opt PWSTR *FilePart, __reserved PVOID Reserved);
...
NTSYSAPI NTSTATUS NTAPI RtlComputePrivatizedDllName_U(IN PCUNICODE_STRING DllName,
...
// Routines for manipulating user mode handle tables.
// Used for Atoms and Local/Global memory allocations.
...
NTSYSAPI NTSTATUS NTAPI RtlSetThreadPoolStartFunc(PRTLP_START_THREAD StartFunc, PRTLP_EXIT_THREAD ExitFunc);
...
NTSYSAPI NTSTATUS NTAPI RtlSetIoCompletionCallback (IN  HANDLE  FileHandle, IN  APC_CALLBACK_FUNCTION  CompletionProc, IN  ULONG Flags) ;
...
/* returns E_NOTIMPL */
RTL_FILE_STREAM_API HRESULT STDMETHODCALLTYPE RtlRevertFileStream(PRTL_FILE_STREAM FileStream);
...
// Don't use NTSYSAPI directly so you can more easily
// statically link to these functions, independently of how you link to the rest of ntdll.
...
RTL_MEMORY_STREAM_API HRESULT STDMETHODCALLTYPE RtlCopyMemoryStreamTo(
    PRTL_MEMORY_STREAM_WITH_VTABLE MemoryStream,
    IStream*           AnotherStream,
    ULARGE_INTEGER     NumberOfBytesToCopy,
    ULARGE_INTEGER*    NumberOfBytesRead,
...
public:
    ...
    */
    ...
    {
        return RtlCopyMemoryStreamTo(*this, AnotherStream, NumberOfBytesToCopy, NumberOfBytesRead, NumberOfBytesWritten);
    }

    HRESULT STDMETHODCALLTYPE Commit(ULONG              Flags)
    {
...
public:
    ...
    HRESULT STDMETHODCALLTYPE QueryInterface(const IID& Interface, PVOID* Object)
    {
    ...
    HRESULT STDMETHODCALLTYPE Clone(IStream** NewStream)
    {
...
NTSYSAPI VOID NTAPI RtlFreeActivationContextStack(IN PACTIVATION_CONTEXT_STACK ActivationContextStackPointer);
...
NTSYSAPI VOID FASTCALL RtlActivateActivationContextUnsafeFast(IN PRTL_CALLER_ALLOCATED_ACTIVATION_CONTEXT_STACK_FRAME Frame, IN PACTIVATION_CONTEXT ActivationContext);
NTSYSAPI VOID FASTCALL RtlDeactivateActivationContextUnsafeFast(IN PRTL_CALLER_ALLOCATED_ACTIVATION_CONTEXT_STACK_FRAME Frame);
NTSYSAPI NTSTATUS NTAPI RtlAllocateActivationContextStack(IN OUT PACTIVATION_CONTEXT_STACK *ActivationContextStackPointer);
...
NTSYSAPI NTSTATUS RtlQueueApcWow64Thread(IN HANDLE ThreadHandle, IN PPS_APC_ROUTINE ApcRoutine, IN PVOID ApcArgument1, IN PVOID ApcArgument2, IN PVOID ApcArgument3);
NTSYSAPI NTSTATUS RtlWow64EnableFsRedirection (__in BOOLEAN Wow64FsEnableRedirection);
NTSYSAPI NTSTATUS RtlWow64EnableFsRedirectionEx (__in PVOID Wow64FsEnableRedirection, __out PVOID *OldFsRedirectionLevel);
NTSYSAPI NTSTATUS LdrGetKnownDllSectionHandle (IN PCWSTR DllName, IN BOOLEAN SearchKnownDlls32, OUT PHANDLE Section);

// ULONG RtlGetCurrentProcessId(VOID);
#define RtlGetCurrentProcessId() (HandleToUlong(NtCurrentTeb()->ClientId.UniqueProcess))

// ULONG RtlGetCurrentThreadId(VOID);
...
typedef LONG (NTAPI * PRTL_READ_MAPPED_MEMORY_EXCEPTION_FILTER)(IN PEXCEPTION_POINTERS  ExceptionPointer, IN PNTSTATUS ExceptionCode);
LONG NTAPI RtlReadMappedMemoryExceptionFilter(IN PEXCEPTION_POINTERS  ExceptionPointer, IN PNTSTATUS            ExceptionCode);
...
NTSYSAPI NTSTATUS NTAPI RtlSetThreadErrorMode(IN  ULONG  NewMode, OUT PULONG OldMode OPTIONAL);
NTSYSAPI NTSTATUS NTAPI RtlSetEnvironmentStrings(IN PWCHAR NewEnvironment, IN SIZE_T NewEnvironmentSize);
