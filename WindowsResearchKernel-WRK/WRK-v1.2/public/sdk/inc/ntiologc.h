...
// MessageText:
//  A conflict has been detected between two drivers which claimed two overlapping memory regions.
//  Driver %2, with device <%3>, claimed a memory range with starting address in data address 0x28 and 0x2c, and length in data address 0x30.
#define IO_ERR_MEMORY_CONFLICT_DETECTED  ((NTSTATUS)0xC0040015L)

// MessageId: IO_ERR_PORT_CONFLICT_DETECTED
// MessageText:
//  A conflict has been detected between two drivers which claimed two overlapping Io port regions.
//  Driver %2, with device <%3>, claimed an IO port range with starting address in data address 0x28 and 0x2c, and length in data address 0x30.
#define IO_ERR_PORT_CONFLICT_DETECTED    ((NTSTATUS)0xC0040016L)

// MessageId: IO_ERR_DMA_CONFLICT_DETECTED
// MessageText:
//  A conflict has been detected between two drivers which claimed equivalent DMA channels.
//  Driver %2, with device <%3>, claimed the DMA Channel in data address 0x28, with optional port in data address 0x2c.
...
//  with other devices.  The device DMA setting of '%2' could not be satisfied due to a conflict with Driver '%3'.
...
// MessageText:
//  The device could not allocate one or more required resources due to conflicts
//  with other devices.  The device interrupt setting of '%2' could not be satisfied due to a conflict with Driver '%3'.
...
// MessageText:
//  The device could not allocate one or more required resources due to conflicts
//  with other devices.  The device memory setting of '%2' could not be satisfied due to a conflict with Driver '%3'.
...
// MessageText:
//  The system has encountered an error rebuilding the user disk quota information on device %1 with label "%2".
...
//  To ensure accuracy, the file system quota information on the device %1 with label "%2" will be rebuilt.
...
//  option is set in the registry but a stop error occured before configuration completed
#define IO_DUMP_DUMPFILE_CONFLICT        ((NTSTATUS)0xC004002FL)

// MessageId: IO_DUMP_DIRECT_CONFIG_FAILED
// MessageText:
//  Direct dump configuration failed. Validate the filename and make sure the target device is not a Fault Tolerant set member, remote, or floppy device.
//  The failure may be because there is not enough room on the dump device to create the dump file.
...
//  file on the boot partition and that is large enough to contain all physical memory.
...
// MessageText:
//  Changing the disk signature of disk %2 because it is equal to the disk signature of disk %3.
...
// MessageText:
//  Machine Check Event reported is a CPU thermal throttling event reported from CPU %1. 
//  The CPU has exceeded the temperature limit and has been throttled down. %2 additional error(s) are contained within the record.
...
// MessageText:
//  Machine Check Event reported is a CPU thermal throttling event reported from CPU %1. 
//  The CPU has dropped below the temperature limit and throttling has been removed. %2 additional error(s) are contained within the record.
