...
//  Define an access token from a programmer's viewpoint.
//  The structure is completely opaque and the programer is only allowed to have pointers to tokens.
...
// If a new impersonation level is added or lowest value is changed from 0 to something else, fix the Windows CreateFile call.
...
NTSYSCALLAPI NTSTATUS NTAPI NtOpenProcessToken(__in HANDLE ProcessHandle, __in ACCESS_MASK DesiredAccess, __out PHANDLE TokenHandle);
NTSYSCALLAPI NTSTATUS NTAPI NtOpenProcessTokenEx(__in HANDLE ProcessHandle, __in ACCESS_MASK DesiredAccess, __in ULONG HandleAttributes, __out PHANDLE TokenHandle);
