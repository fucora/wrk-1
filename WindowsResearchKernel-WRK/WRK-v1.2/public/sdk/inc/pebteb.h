...
Abstract:
    ...
    defined three times, once "native" in ntpsapi.w, and twice, 32bit and 64bit in wow64t.w.
*/
...
typedef struct PEBTEB_STRUCT(_PEB) {
    ...
    // Where heap manager keeps track of all heaps created for a process Fields initialized by MmCreatePeb.
    // ProcessHeaps is initialized to point to the first free byte after the PEB and MaximumNumberOfHeaps
    // is computed from the page size used to hold the PEB, less the fixed size of this data structure.
    ULONG NumberOfHeaps;
    ...
    ULONG SessionId;// Id of the Hydra session in which this process is running
    ULARGE_INTEGER AppCompatFlags;// Filled in by LdrpInstallAppcompatBackend
    ULARGE_INTEGER AppCompatFlagsUser;// ntuser appcompat flags
    PEBTEB_POINTER(PVOID) pShimData;// Filled in by LdrpInstallAppcompatBackend
    PEBTEB_POINTER(PVOID) AppCompatInfo;// Filled in by LdrQueryImageFileExecutionOptions
    PEBTEB_STRUCT(UNICODE_STRING) CSDVersion;// Used by GetVersionExW as the szCSDVersion string
