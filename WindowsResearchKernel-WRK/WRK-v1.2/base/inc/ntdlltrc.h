...
Abstract:
    This file contains structures and functions definitions used in Ntdll events tracing
*/
...
#define IsCritSecLogging(CriticalSection) ((USER_SHARED_DATA->TraceLogging & ENABLECRITSECTRACE) \
    &&(bNtdllTrace || GlobalCounter != (USER_SHARED_DATA->TraceLogging >> 16)) &&((HandleToUlong(NtCurrentTeb()->EtwTraceData) & IN_TRACING) != IN_TRACING))
...
// When calling from deep inside heap allocation routines, we do not want to be initializing ETW process heap since that gets into recursive behaviour.
