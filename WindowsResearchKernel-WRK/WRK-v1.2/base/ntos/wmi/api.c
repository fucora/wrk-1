...
/*
Routine Description:
    This routine is the initialization routine for WMI and is called by IO within IoInitSystem on NT.
    This routine assumes that the IO system is initialized enough to call IoCreateDriver.
    The rest of the initialization occurs in the DriverEntry routine.
Arguments:
...
*/
{
    // We name the driver this so that any eventlogs fired will have the source name WMIxWDM and thus get the eventlog messages from ntiologc.mc
#define WMIDRIVERNAME L"\\Driver\\WMIxWDM"
    ...
    WmipRegisterMcaHandler(Phase);// Give MCA a chance to init during phase 0 and 1
#endif
...
NTSTATUS IoWMIRegistrationControl(__in PDEVICE_OBJECT DeviceObject, __in ULONG Action)
/*
Routine Description:
    This routine informs WMI of the existence and disappearance of a device object that would support WMI.
Arguments:
...
NTSTATUS IoWMIAllocateInstanceIds(__in GUID *Guid, __in ULONG InstanceCount, __out ULONG *FirstInstanceId)
/*
    Routine Description:
    This routine allocates a range of instance ids that are unique to the guid. This routine is to be called only at PASSIVE_LEVEL.
    Arguments:
...
*/
{
    ...
    while (InstIdChunk != NULL) {
        for (i = 0; i < INSTIDSPERCHUNK; i++) {
            ...
            if (InstId->BaseId == ~0) {
                // Since InstIds are always filled sequentially and are never freed, if we hit a free one then we know that
                // our guid is not in the list and we need to fill in a new entry.
                goto FillInstId;
            }
...
/*
    ...
    A driver is not bound to follow the instance name returned.

    Arguments:
    PhysicalDeviceObject - PDO of device for which a suggested instance name is being requested
    SymbolicLinkName - Symbolic link name returned from IoRegisterDeviceInterface.
    CombineNames - If TRUE then the suggested names arising from the PhysicalDeviceObject and the SymbolicLinkName are combined to create the resultant suggested name.
    SuggestedInstanceName - Supplies a pointer to an empty (i.e., Buffer
        field set to NULL) UNICODE_STRING structure which, upon success, will be set to a newly-allocated string buffer containing the suggested instance name.
        The caller is responsible for freeing SuggestedInstanceName->Buffer when it is no longer needed.
    Note:  If CombineNames is TRUE then both PhysicalDeviceObject and SymbolicLinkName must be specified.
           Otherwise only one of them must be specified.
    Return Value:
        Returns a status code
*/
{
    ...
    if (SymbolicLinkName != NULL) {
        ...
        if (NT_SUCCESS(Status)) {
            ...
            Status = ZwQueryValueKey(DeviceInstanceKey, &DefaultValue, KeyValueFullInformation, (PVOID)NULL, 0, &InfoSizeRequired);
...
*/
{
    ...
    if ((WnodeHeader->Flags & WNODE_FLAG_TRACED_GUID) || (WnodeHeader->Flags & WNODE_FLAG_LOG_WNODE)) {
        ...
        // If trace header, turn higher bit on and support only full header
        if (IsTrace) {
        ...
        if (LoggerId > 0 && LoggerId < MAXLOGGERS) {
            if (WmipLoggerContext[LoggerId] != NULL) {
                // NOTE: The rule here is that IoWMIWriteEvent is always called in kernel mode, and the buffer needs not be probed!
                Status = WmiTraceEvent(WnodeHeader, KernelMode);
            }
        }
        // NOTE: If it is a trace, we will not go any further
        // Otherwise, if it is a regular WMI event, it will still be processed by WMI.
        if (IsTrace) {
    ...
    if (WnodeHeader->BufferSize <= WmipMaxKmWnodeEventSize) {
        ...
        if (EventContext != NULL) {
            // Try to take a refcount on the regentry associated with the provider id in the event.
            // If we are successful then we set a flag in the wnode header saying so.
            // When processing the event in the work item we check the flag and if it is set we'll go looking for the regentry on the active and zombie lists and then use it.
            // At that time it will give up the ref count taken here so that if the regentry really is a zombie then it will go away peacefully.
            ...
            // If the queue was empty then there was no work item outstanding to move from non paged to paged memory.
            // So fire up a work item to do so.
...
NTSTATUS IoWMIOpenBlock(__in GUID *Guid, __in ULONG DesiredAccess, __out PVOID *DataBlockObject)
{
    ...
    if (NT_SUCCESS(Status)) {
        Status = ObReferenceObjectByHandle(DataBlockHandle, DesiredAccess, WmipGuidObjectType, KernelMode, DataBlockObject, NULL);
...
NTSTATUS IoWMIQueryAllDataMultiple(__in_ecount(ObjectCount) PVOID *DataBlockObjectList, __in ULONG ObjectCount, __inout ULONG *InOutBufferSize, __out_bcount_opt(*InOutBufferSize) PVOID OutBuffer)
{
    ...
    if (!ARGUMENT_PRESENT(DataBlockObjectList) || (ObjectCount == 0) || !ARGUMENT_PRESENT(InOutBufferSize)) {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
...
NTSTATUS IoWMIQuerySingleInstance(__in PVOID DataBlockObject, __in PUNICODE_STRING InstanceName, __inout ULONG *InOutBufferSize, __out_bcount_opt(*InOutBufferSize) PVOID OutBuffer)
{
    ...
    if (WnodeSI != NULL) {
        ...
        // Instance name takes a USHORT Length and then followed by a string.
