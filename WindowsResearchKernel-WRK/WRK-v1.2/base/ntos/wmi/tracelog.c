...
Abstract:
    This is the source file that implements the private routines for the performance event tracing and logging facility.
    The routines here work on a single event tracing session, the logging thread, and buffer synchronization within a session.
*/
...
NTSTATUS WmipCreateLogFile(IN PWMI_LOGGER_CONTEXT LoggerContext, IN ULONG SwitchFile, IN ULONG Append);
NTSTATUS WmipSwitchToNewFile(IN PWMI_LOGGER_CONTEXT LoggerContext);
NTSTATUS WmipRequestLogFile(IN PWMI_LOGGER_CONTEXT LoggerContext);
NTSTATUS WmipFinalizeHeader(IN HANDLE FileHandle, IN PWMI_LOGGER_CONTEXT LoggerContext);
NTSTATUS
...
PWMI_BUFFER_HEADER WmipGetFreeBuffer(IN PWMI_LOGGER_CONTEXT LoggerContext)
...
ULONG WmipAllocateFreeBuffers(IN PWMI_LOGGER_CONTEXT LoggerContext, IN ULONG NumberOfBuffers)
/*
Routine Description:
...
*/
{
    ...
    for (i = 0; i < NumberOfBuffers; i++) {
        ...
        if (TotalBuffers <= LoggerContext->MaximumBuffers) {
            Buffer = (PWMI_BUFFER_HEADER)ExAllocatePoolWithTag(LoggerContext->PoolType, LoggerContext->BufferSize, TRACEPOOLTAG);
            if (Buffer != NULL) {
                ...
                InterlockedPushEntrySList(&LoggerContext->FreeList, (PSLIST_ENTRY)&Buffer->SlistEntry);
                InterlockedPushEntrySList(&LoggerContext->GlobalList, (PSLIST_ENTRY)&Buffer->GlobalEntry);
            } else {
...
NTSTATUS WmipAdjustFreeBuffers(IN PWMI_LOGGER_CONTEXT LoggerContext)
/*
Routine Description:
    This routine does buffer management.  It checks the number of free buffers and will allocate additonal or free some based on the situation.
Arguments:
    LoggerContext - Logger Context
Return Value:
    Status
...

{
    ...
    Header = WmipReserveTraceBuffer(LoggerContext, AuxSize, BufferResource, &TimeStamp);
    if (Header != NULL) {
...
PPERFINFO_TRACE_HEADER FASTCALL WmiReserveWithPerfHeader(IN ULONG AuxSize, OUT PWMI_BUFFER_HEADER *BufferResource)
// It returns with LoggerContext locked, so caller must explicitly call
// WmipDereferenceLogger() after call WmipReleaseTraceBuffer()
{
    ...
    Header = WmipReserveTraceBuffer(WmipGetLoggerContext(LoggerId), AuxSize, BufferResource, &TimeStamp);
    if (Header != NULL) {
...
PVOID FASTCALL WmipReserveTraceBuffer(
    IN  PWMI_LOGGER_CONTEXT LoggerContext,
...
Routine Description:
    ...
    A local variable Offset is used to track the initial value when InterlockedExchangeAdd is taken.
    If there is enough space for this event (i.e., (Offset + RequiredSize) <= BufferSize), then we have successfully reserved the space.

    If there is not enough space left on this buffer, we will call WmipSwitchBuffer for a new buffer.
    In this case, CurrentOffset should be larger than the buffersize.
    Since other thread can still be trying to reserve space using their buffer, we saved the offset on SavedOffset the the logger thread knows the real offset to be written to disk.

    Note that, since the CurrentOffset if growing monotonically, only one thread is advancing the CurrentOffset from below BufferSize to beyond BufferSize.
...
*/
{
    ...
    *BufferResource = NULL;
    ...
    Processor = KeGetCurrentProcessorNumber();// Get processor number again here due to possible context switch
    Buffer = LoggerContext->ProcessorBuffers[Processor];// Get the processor specific buffer pool
    if (Buffer == NULL) {
        // Nothing in per process list, ask to get a new buffer
        Status = WmipSwitchBuffer(LoggerContext, &Buffer, &LoggerContext->ProcessorBuffers[Processor], Processor);
        if (!NT_SUCCESS(Status)) {
            goto LostEvent;// Nothing available
        }
...
TryFindSpace:
    InterlockedIncrement(&Buffer->ReferenceCount);// Increment refcount to buffer first to prevent it from going away
    Offset = (ULONG)InterlockedExchangeAdd((PLONG)&Buffer->CurrentOffset, RequiredSize);// Check if there is enough space in this buffer.

    // If many threads concurrently hit this line, the Offset+RequiredSize might arithmetically overflow.
    // Therefore we need to add an extra check to make sure the Offset is in range.
    if ((Offset + RequiredSize <= LoggerContext->BufferSize) && (Offset < LoggerContext->BufferSize)) {
        ...
        // There is not enough space left to log this event, Ask for buffer switch.
        // The WmipSwitchBuffer() will push this current buffer into Dirty list.

        // Before asking for buffer switch, check if I am the one that got overboard.
        // If yes, put the correct offset back.
        if (Offset <= LoggerContext->BufferSize) {
            Buffer->SavedOffset = Offset;
        }

        InterlockedDecrement((PLONG)&Buffer->ReferenceCount);// Also, dereference the buffer, so it can be freed.
...
FoundSpace:
    *BufferResource = Buffer;// notify the logger after critical section
...
/*
Routine Description:
    This routine is called to get a LogFileHandle for GlobalLogger or when a fileswitch is needed for NEWFILE mode. It will create the file
    and add logfileheader to it. It closes the oldfile by properly finalizing its header.
Arguments:
    LoggerContext - Logger Context
Return Value:
    Status
...
*/
{
    ...
    if (LoggerContext->LoggerMode & EVENT_TRACE_FILE_MODE_NEWFILE) {
        Status = WmipGenerateFileName(&LoggerContext->LogFilePattern, (PLONG)&LoggerContext->FileCounter, &NewFileName);
        if (!NT_SUCCESS(Status)) {
        ...
        if (!RtlCreateUnicodeString(&NewFileName, LoggerContext->LogFileName.Buffer)) {
    ...
    if (NT_SUCCESS(Status)) {
        NewHeaderBuffer = (PWMI_BUFFER_HEADER)ExAllocatePoolWithTag(LoggerContext->PoolType, LoggerContext->BufferSize, TRACEPOOLTAG);
        if (NewHeaderBuffer != NULL) {
            ...
            WmipResetBufferHeader(LoggerContext, NewHeaderBuffer, WMI_BUFFER_TYPE_RUNDOWN);
            ...
            Status = WmipPrepareHeader(LoggerContext, NewHeaderBuffer, WMI_BUFFER_TYPE_RUNDOWN);
            if (NT_SUCCESS(Status)) {
                Status = ZwWriteFile(NewHandle, NULL, NULL, NULL, &IoStatus, NewHeaderBuffer, BufferSize, NULL, NULL);
            }
            if (!NT_SUCCESS(Status)) {
...
/*
Routine Description:
    This routine switches logfiles for an active logger.
    This routine must be called only by the Logger Thread.
    It will close the previous logfile, if any, properly by terminating it with FLUSH_MARKER and Finalizing the LogHeader.
    ...
    The callers of this function are
        1. UpdateLogger: Sets the REQUEST_FLAG_NEW_FILE after presenting a user mode created logfile in NewLogFile.
        2. NT Kernel Logger session: Switches from DELAY_OPEN mode with a user mode created logfile
        3. FILE_MODE_NEWFILE: When current logfile reaches the FileLimit FlushBuffer requests a newfile.
        4. GlobalLogger: Started in DELAY_OPEN && ADD_HEADER mode needs to create the logfile when the FileSystem is ready.

    In all cases, when the switch is made the old logfile needs to be properly closed after Finalizing its header.
...
*/
{
    ...
    if ((LoggerContext->LogFileName.Buffer == NULL) && (LoggerContext->LogFilePattern.Buffer == NULL) && (LoggerContext->NewLogFileName.Buffer == NULL)) {
        return Status;
    }
    ...
    if (NT_SUCCESS(Status)) {
        if ((LoggerContext->LoggerMode & EVENT_TRACE_FILE_MODE_NEWFILE) || ((LoggerContext->LoggerMode & EVENT_TRACE_DELAY_OPEN_FILE_MODE) && (LoggerContext->LoggerMode & EVENT_TRACE_ADD_HEADER_MODE))) {
            Status = WmipSwitchToNewFile(LoggerContext);
        } else {
            // UpdateTrace case
            TraceDebug((3, "WmipLogger: New File\n"));
            Status = WmipCreateLogFile(LoggerContext, TRUE, EVENT_TRACE_FILE_MODE_APPEND);
            if (NT_SUCCESS(Status)) {
                LoggerContext->LoggerMode &= ~EVENT_TRACE_DELAY_OPEN_FILE_MODE;
            }
...
*/
{
    ...
    InitialStatus = InterlockedExchange(&LoggerContext->LoggerStatus, STATUS_PENDING);
    ...
    InitialStatus = InterlockedExchange(&LoggerContext->LoggerStatus, Status);
    ...
    while (LoggerContext->CollectionOn) {
        if (LoggerContext->LoggerMode & EVENT_TRACE_BUFFERING_MODE) {
            ...
            // flush list.
            Status = KeWaitForSingleObject(&LoggerContext->LoggerSemaphore, Executive, KernelMode, FALSE, &WmiOneSecond);
            ...
            if ((LoggerContext->RequestFlag & REQUEST_FLAG_NEW_FILE) || ((LoggerContext->LoggerMode & EVENT_TRACE_DELAY_OPEN_FILE_MODE) && (LoggerContext->LoggerMode & EVENT_TRACE_ADD_HEADER_MODE))) {
            ...
            if (FlushFlag) {
                ...
                if (LoggerContext->RequestFlag & REQUEST_FLAG_CIRCULAR_TRANSITION) {
                    if (LoggerContext->LogFileHandle != NULL) {
                        WmipFinalizeHeader(LoggerContext->LogFileHandle, LoggerContext);
                    }
    ...
    while ((LoggerContext->NumberOfBuffers > 0) && (LoggerContext->NumberOfBuffers > LoggerContext->BuffersAvailable)) {
        ...
        TraceDebug((2, "WmipLogger: Stop %d %d %d %d %d\n", LoggerContext->LoggerId, LoggerContext->BuffersAvailable, LoggerContext->BuffersInUse, LoggerContext->BuffersDirty, LoggerContext->NumberOfBuffers));
    }
...
NTSTATUS WmipFlushBuffer(IN PWMI_LOGGER_CONTEXT LoggerContext, IN PWMI_BUFFER_HEADER Buffer, IN USHORT BufferFlag)
/*
Routine Description:
    This function is responsible for flushing a filled buffer out to disk, or to a real time consumer.
Arguments:
...
*/
{
    ...
    NTSTATUS Status;
    ULONG BufferSize;
    ULONG BufferPersistenceData = LoggerContext->RequestFlag & (REQUEST_FLAG_CIRCULAR_PERSIST | REQUEST_FLAG_CIRCULAR_TRANSITION);
    ...
    TraceDebug((2, "WmipFlushBuffer: %p, Flushed %X %8x %8x %5d\n", Buffer, Buffer->ClientContext, Buffer->SavedOffset, Buffer->CurrentOffset, LoggerContext->BuffersWritten));
        ...
        if (LoggerContext->LogFileHandle != NULL) {
            if (LoggerContext->MaximumFileSize > 0) { // if quota given
                ...
                if (FileSize >= FileLimit) {
                    ...
                    if (LoggerMode == EVENT_TRACE_FILE_MODE_SEQUENTIAL) {
                    ...
                    } else if (LoggerMode == EVENT_TRACE_FILE_MODE_CIRCULAR || LoggerMode == EVENT_TRACE_FILE_MODE_CIRCULAR_PERSIST) {
                        if (BufferPersistenceData > 0) {
                            ...
                            LoggerContext->LastFlushedBuffer = (ULONG)(LoggerContext->FirstBufferOffset.QuadPart / LoggerContext->BufferSize);
                        }
                        ...
                        if ((LoggerContext->RequestFlag & REQUEST_FLAG_NEW_FILE) != REQUEST_FLAG_NEW_FILE) {
                            LoggerContext->RequestFlag |= REQUEST_FLAG_NEW_FILE;
                        }
                    }
                }
            ...
            if (NT_SUCCESS(Status)) {
                Status = ZwWriteFile(LoggerContext->LogFileHandle, NULL, NULL, NULL, &IoStatus, Buffer, BufferSize, &LoggerContext->ByteOffset, NULL);
                if (NT_SUCCESS(Status)) {
                    LoggerContext->ByteOffset.QuadPart += BufferSize;
                    ...
                    if ((BufferPersistenceData > 0) && (Buffer->BufferType == WMI_BUFFER_TYPE_RUNDOWN)) {
...
NTSTATUS WmipCreateLogFile(IN PWMI_LOGGER_CONTEXT LoggerContext, IN ULONG SwitchFile, IN ULONG Append)
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        Status = ZwReadFile(newHandle, NULL, NULL, NULL, &IoStatus, pFirstBuffer, LoggerContext->BufferSize, &ByteOffset, NULL);
        if (NT_SUCCESS(Status)) {
            ...
            LogfileHeaderFile = (PTRACE_LOGFILE_HEADER)(((PUCHAR)BufferFile) + sizeof(WMI_BUFFER_HEADER) + sizeof(SYSTEM_TRACE_HEADER));
        ...
        if (LoggerContext->LoggerMode & EVENT_TRACE_FILE_MODE_PREALLOCATE) {
            ...
            Status = ZwQueryInformationFile(newHandle, &IoStatus, &FileSize, sizeof(FILE_STANDARD_INFORMATION), FileStandardInformation);
            if (!NT_SUCCESS(Status)) {
                ZwClose(newHandle);
                goto Cleanup;
            }
        ...
        // Force to 1K alignment. In future, if disk alignment exceeds this, then use that
        ...
        if (!(LoggerContext->LoggerMode & EVENT_TRACE_FILE_MODE_PREALLOCATE)) {
            ...
            // the end of file to make sure that no one else has written garbage to it
...
ULONG FASTCALL WmipReleaseTraceBuffer(IN PWMI_BUFFER_HEADER BufferResource, IN PWMI_LOGGER_CONTEXT LoggerContext)
{
    ULONG BufRefCount;
    LONG  ReleaseQueue;
...
NTKERNELAPI ULONG FASTCALL WmiReleaseKernelBuffer(IN PWMI_BUFFER_HEADER BufferResource)
{
...
NTSTATUS WmipFlushBuffersWithMarker(
    IN PWMI_LOGGER_CONTEXT  LoggerContext,
    IN PSLIST_ENTRY         List,
    IN USHORT               BufferFlag
)
{
    ...
    while (LocalList != NULL) {
        ...
        // Wait until no one else is using the buffer before writing it out.
        ...
        while (Buffer->ReferenceCount) {
            ...
            KeDelayExecutionThread(KernelMode, FALSE, (PLARGE_INTEGER)&WmiShortTime);
            Retry++;
            if (Retry > 10) {
                ...
                ULONG BufferSize = LoggerContext->BufferSize;
                TmpBuffer = ExAllocatePoolWithTag(NonPagedPool, BufferSize, TRACEPOOLTAG);
                if (TmpBuffer) {
                    TraceDebug((1, "Buffer %p has ref count %3d, Tmporary buffer %p Allocated\n", Buffer, Buffer->ReferenceCount, TmpBuffer));
        ...
        if (TmpBuffer) {
            ...
            // The buffer still in use but allocation of temporary buffer failed.
        ...
        if (TmpBuffer != Buffer) {
            ...
            InterlockedPushEntrySList(&LoggerContext->WaitList, (PSLIST_ENTRY)&Buffer->SlistEntry);
        } else {
...
NTSTATUS WmipFlushActiveBuffers(IN PWMI_LOGGER_CONTEXT LoggerContext, IN ULONG FlushAll)
{
    PWMI_BUFFER_HEADER Buffer;
    ULONG i;
    NTSTATUS Status = STATUS_SUCCESS;
    ...
    if ((LoggerContext->LogFileHandle == NULL) && (!(LoggerMode & EVENT_TRACE_REAL_TIME_MODE)) && (LoggerContext->CollectionOn)) {
        return Status;
    }
    ...
    if (ExQueryDepthSList(&LoggerContext->WaitList) != 0) {
        ...
        while (LocalList != NULL) {
            ...
            Buffer = CONTAINING_RECORD(Entry, WMI_BUFFER_HEADER, SlistEntry);
            TraceDebug((1, "Wait List Buffer %p RefCount: %3d\n", Buffer, Buffer->ReferenceCount));
            if (Buffer->ReferenceCount) {
                InterlockedPushEntrySList(&LoggerContext->WaitList, (PSLIST_ENTRY)&Buffer->SlistEntry);// Still in use, put it back to WaitList
            } else {
                WmipPushFreeBuffer(LoggerContext, Buffer);// Push it to free list
            }
        }
    }
...
NTSTATUS WmipGenerateFileName(IN PUNICODE_STRING FilePattern, IN OUT PLONG FileCounter, OUT PUNICODE_STRING FileName)
{
    LONG FileCount, Size;
    PWCHAR Buffer = NULL;
    HRESULT hr;
...
NTSTATUS WmipPrepareHeader(IN PWMI_LOGGER_CONTEXT LoggerContext, IN OUT PWMI_BUFFER_HEADER Buffer, IN USHORT BufferFlag)
/*
Routine Description:
    This routine prepares the buffer header before writing the buffer out to Disk.

    If (SavedOffset > 0), it must be the case the we have overflown
    CurrentOffset during WmipReserveTraceBuffer.  SavedOffset should be the real offset of dirty buffer area.

    If SavedOffset is not set, it is either a ContextSwap buffer or a buffer flushed due to flush timer.  CurrentOffset should be used for writting it out.
...
NTKERNELAPI VOID WmiBootPhase1()
/*
Routine Description:
    NtInitializeRegistry to inform WMI that autochk is performed and it is OK now to write to disk.
Arguments:
    None
Return Value:
    None
...
NTSTATUS WmipFinalizeHeader(IN HANDLE FileHandle, IN PWMI_LOGGER_CONTEXT LoggerContext)
{
    ...
    if (LoggerContext->RequestFlag & REQUEST_FLAG_CIRCULAR_TRANSITION) {
        FileHeader->StartBuffers = (ULONG)(LoggerContext->FirstBufferOffset.QuadPart / LoggerContext->BufferSize);
    }
    ...
    if (LoggerContext->Wow && !LoggerContext->KernelTraceOn) {
        ...
        *((PULONG)((PUCHAR)(&FileHeader->BuffersLost) - 8)) = LoggerContext->LogBuffersLost;
    } else {
...
*/
{
    ...
    if (WmipTraceDebugLevel >= DebugPrintLevel) {
        ...
        DbgPrintEx(DPFLTR_WMILIB_ID, DPFLTR_INFO_LEVEL, "%u (%5u): %s", Clock.LowPart, Tid, TraceDebugBuffer);
    }
...
/*
Routine Description:
    This is a function which initializes a few buffer header values that are used by both WmipGetFreeBuffer and WmipPopFreeContextSwapBuffer
...
/*
Routine Description:
    This is a function which prepares a buffer's header and places it on a logger's flush list.
...
*/
{
    ...
    InterlockedPushEntrySList(&LoggerContext->FlushList, (PSLIST_ENTRY)&Buffer->SlistEntry);
...
/*
Routine Description:
    This is a function which prepares a buffer's header and places it on a logger's Free list.
    Note that this function manages a few logger context reference counts
Calling Functions:
    - WmipFlushActiveBuffers
    - WmipGetFreeBuffer
...
PWMI_BUFFER_HEADER FASTCALL WmipPopFreeContextSwapBuffer(UCHAR CurrentProcessor)
/*
Routine Description:
...
Routine Description:
    ...
    2. Get a new buffer and use InterlockedCompareExchangePointer to switch buffers only if the OldBuffer is still not switched.
    3. If the OldBuffer has been switched, ask the caller to try using the newly switched buffer for logging.
Assumptions:
...
*/
{
    ...
    if (!LoggerContext->CollectionOn) {
        ...
        if (NewBuffer) {
            ...
            CurrentBuffer = InterlockedCompareExchangePointer(BufferPointerLocation, NewBuffer, OldBuffer);
            ...
            if (OldBuffer != NULL && CurrentBuffer == NULL) {
                CurrentBuffer = InterlockedCompareExchangePointer(BufferPointerLocation, NewBuffer, NULL);
                ...
                if (CurrentBuffer != NULL) {
                    ...
                    // We will use the ProcessorBuffer and push our new buffer into FreeList.
                    InterlockedPushEntrySList(&LoggerContext->FreeList, (PSLIST_ENTRY)&NewBuffer->SlistEntry);
                    InterlockedIncrement((PLONG)&LoggerContext->BuffersAvailable);
                    InterlockedDecrement((PLONG)&LoggerContext->BuffersInUse);
                ...
                // Someone has switched the buffer, use this one and push the new allocated buffer back to free list.
                InterlockedPushEntrySList(&LoggerContext->FreeList, (PSLIST_ENTRY)&NewBuffer->SlistEntry);
                InterlockedIncrement((PLONG)&LoggerContext->BuffersAvailable);
                InterlockedDecrement((PLONG)&LoggerContext->BuffersInUse);
                ...
                // Successfully switched the buffer, push the current buffer into flush list
                WmipPushDirtyBuffer(LoggerContext, OldBuffer);

                if (!(LoggerContext->LoggerMode & EVENT_TRACE_BUFFERING_MODE)) {
                    if (KeGetCurrentIrql() <= DISPATCH_LEVEL) {
                        WmipNotifyLogger(LoggerContext);// Wake up the walker thread to write it out to disk.
                    } else {
                        InterlockedIncrement(&LoggerContext->ReleaseQueue);// Queue the item.
                    }
                }
            }
        } else {
            ...
            // may overflow.
            OldBuffer = *BufferPointer;
            CurrentBuffer = InterlockedCompareExchangePointer(BufferPointerLocation, NULL, OldBuffer);
