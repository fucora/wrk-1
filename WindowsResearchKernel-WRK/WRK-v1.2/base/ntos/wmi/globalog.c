...
// if the creation routines and logger routines can run at all while in boot path and being pageable
...
*/
{
    ...
    // We lock and unlock non-paged portion of ETW code so that we can keep global logger in the memory while it's alive.
    EtwpPageLockHandle = MmLockPageableCodeSection((PVOID)(ULONG_PTR)WmipReserveTraceBuffer);
    ...
    status = WmipGetGuidSecurityDescriptor(&DefaultTraceGuidName, &EtwpDefaultTraceSecurityDescriptor, NULL);
    ...
    GLog.LoggerInfo.LogFileMode = EVENT_TRACE_DELAY_OPEN_FILE_MODE | EVENT_TRACE_ADD_HEADER_MODE;
    ...
    if (NT_SUCCESS(status) && (StartRequested != 0)) {
        ...
        if (GLog.LoggerInfo.LogFileName.Buffer == NULL) {
            RtlCreateUnicodeString(&GLog.LoggerInfo.LogFileName, DEFAULT_GLOBAL_LOGFILE_ROOT); // Use ROOT as indicator
            if (GLog.LoggerInfo.LogFileName.Buffer == NULL)
    ...
    RtlWriteRegistryValue(RTL_REGISTRY_CONTROL, L"WMI\\GlobalLogger", L"Status", REG_DWORD, &Win32Error, sizeof(ULONG));
}
...
Routine Description:
    ...
    we pop a buffer from the free list, write the header, and flush the buffer right away.
Arguments:
...
*/
{
    ...
    // if this is a non-kernel logger started under wow64, we need to shrink the logfileheader
    if (LoggerContext->Wow && !LoggerContext->KernelTraceOn) {
        HeaderSize = sizeof(TRACE_LOGFILE_HEADER) + LoggerContext->LoggerName.Length + sizeof(UNICODE_NULL) + LoggerContext->LogFileName.Length + sizeof(UNICODE_NULL) - 8;
    } else {
        HeaderSize = sizeof(TRACE_LOGFILE_HEADER) + LoggerContext->LoggerName.Length + sizeof(UNICODE_NULL) + LoggerContext->LogFileName.Length + sizeof(UNICODE_NULL);
    }
    if (LoggerContext->BufferSize < (HeaderSize + sizeof(WMI_BUFFER_HEADER) + sizeof(SYSTEM_TRACE_HEADER))) {
    ...
    if (!BufferProvided) {
        ...
        if (Buffer == NULL) {
            Buffer = ExAllocatePoolWithTag(NonPagedPool, LoggerContext->BufferSize, TRACEPOOLTAG);
            if (Buffer == NULL) {// No buffer available.
                return STATUS_NO_MEMORY;
            }
            LocalBuffer = TRUE;
            Buffer->Flags = 1;
    ...
    if (LoggerContext->Wow && !LoggerContext->KernelTraceOn) {
        ...
        *((PLARGE_INTEGER)((PUCHAR)(&LogfileHeader->StartTime) - 8)) = LoggerContext->ReferenceSystemTime;
        *((PLARGE_INTEGER)((PUCHAR)(&LogfileHeader->BootTime) - 8)) = KeBootTime;
        LogfileHeader->PointerSize = sizeof(ULONG);

        // ReservedFlags to indicate using Perf Clock
        *((PULONG)((PUCHAR)(&LogfileHeader->ReservedFlags) - 8)) = LoggerContext->UsePerfClock;
        LoggerNamePtr64 = (PUCHAR)LogfileHeader + sizeof(TRACE_LOGFILE_HEADER) - 8;
        *((PULONG)(&LogfileHeader->LoggerName)) = PtrToUlong(LoggerNamePtr64);
        LogFileNamePtr64 = (PUCHAR)LogfileHeader + sizeof(TRACE_LOGFILE_HEADER) - 8 + LoggerContext->LoggerName.Length + sizeof(UNICODE_NULL);
        *((PULONG)((PUCHAR)(&LogfileHeader->LogFileName) - 4)) = PtrToUlong(LogFileNamePtr64);
        RtlCopyMemory(LoggerNamePtr64, LoggerContext->LoggerName.Buffer, LoggerContext->LoggerName.Length + sizeof(UNICODE_NULL));
        RtlCopyMemory(LogFileNamePtr64, LoggerContext->LogFileName.Buffer, LoggerContext->LogFileName.Length + sizeof(UNICODE_NULL));
        ...
        LogfileHeader->LogFileName = (PWCHAR)((PUCHAR)LogfileHeader->LoggerName + LoggerContext->LoggerName.Length + sizeof(UNICODE_NULL));
        RtlCopyMemory(LogfileHeader->LoggerName, LoggerContext->LoggerName.Buffer, LoggerContext->LoggerName.Length + sizeof(UNICODE_NULL));
        RtlCopyMemory(LogfileHeader->LogFileName, LoggerContext->LogFileName.Buffer, LoggerContext->LogFileName.Length + sizeof(UNICODE_NULL));
...
Arguments:
    ...
    FileName        The logfile name. If the default was chosen, we will returned the actual pathname in %systemroot%
*/
{
    PWCHAR Buffer;
    ...
    if (DefaultFile) {
        ...
        Length = (ULONG)(NtSystemRoot.Length + sizeof(WCHAR) * (wcslen(DEFAULT_GLOBAL_DIRECTORY) + wcslen(DEFAULT_GLOBAL_LOGFILE) + 1) + sizeof(UNICODE_NULL));
        strBuffer = (PWCHAR)ExAllocatePoolWithTag(PagedPool, Length, TRACEPOOLTAG);
        if (strBuffer == NULL)
            return STATUS_NO_MEMORY;

        hr = StringCbPrintfW(strBuffer, Length, L"%ws%ws\\%ws", NtSystemRoot.Buffer, DEFAULT_GLOBAL_DIRECTORY, DEFAULT_GLOBAL_LOGFILE);
        if (FAILED(hr)) {
...
NTSTATUS WmipCreateDirectoryFile(IN PWCHAR DirFileName, IN BOOLEAN IsDirectory, OUT PHANDLE FileHandle, IN ULONG Append)
{
    ...
    Status = ZwCreateFile(
        &DirHandle,
        FILE_GENERIC_READ | SYNCHRONIZE | (IsDirectory ? FILE_GENERIC_WRITE : FILE_WRITE_DATA),
        &ObjectAttributes,
        &IoStatus,
        NULL,
        ...
        FILE_SHARE_READ,
        CreateDisposition,
        FILE_SYNCHRONOUS_IO_NONALERT | (IsDirectory ? FILE_DIRECTORY_FILE : FILE_NO_INTERMEDIATE_BUFFERING),
        NULL,
        0);
...
NTSTATUS WmipCreateNtFileName(IN  PWCHAR strFileName, OUT PWCHAR * strNtFileName)
{
    PWCHAR   NtFileName;
    ULONG    lenFileName;
    HRESULT  hr;
    ...
    if ((strFileName[0] == L'\\') && (strFileName[1] == L'\\')) {
        hr = StringCbPrintfW(NtFileName, lenFileName, L"%ws%ws", UNCDEVICES, &(strFileName[1]));
    } else {
        hr = StringCbPrintfW(NtFileName, lenFileName, L"%ws%ws", DOSDEVICES, strFileName);
    }
    if (FAILED(hr)) {
