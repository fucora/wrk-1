...
Abstract:
    This is the source file that implements the published routines of the performance event tracing and logging facility.
    These routines are be declared in ntos\inc\wmi.h
*/
...
NTKERNELAPI NTSTATUS WmiStartTrace(IN OUT PWMI_LOGGER_INFORMATION LoggerInfo)
/*
Routine Description:
    ...
    if (!DelayOpen) {
        if (LoggerInfo->LogFileName.Buffer != NULL) { // && !delay_create
            ...
            if (!NT_SUCCESS(status))
                return status;
            status = WmipCreateDirectoryFile(LogFileName, FALSE, &FileHandle, LoggerInfo->LogFileMode & EVENT_TRACE_FILE_MODE_APPEND);
            if (LogFileName != NULL) {
...
NTKERNELAPI NTSTATUS WmiQueryTrace(IN OUT PWMI_LOGGER_INFORMATION LoggerInfo)
/*
Routine Description:
...
NTKERNELAPI NTSTATUS WmiStopTrace(IN PWMI_LOGGER_INFORMATION LoggerInfo)
/*
Routine Description:
    It is called by WmipIoControl in wmi.c, with IOCTL_WMI_STOP_LOGGER to stop an instance of the logger. If the logger is the kernel logger,
        ...
        if (LoggerInfo != NULL) {
            if (NT_SUCCESS(LoggerContext->LoggerStatus)) {
                ...
                while (Status == STATUS_TIMEOUT) {
                    ...
                    /*                    if (LoggerContext->NumberOfBuffers == LoggerContext->BuffersAvailable)
                                            break;
                                        else if (LoggerContext->BuffersAvailable == Buffers) {
                                            TraceDebug((1, "WmiStopTrace: Logger %d hung %d != %d\n", LoggerId, Buffers, LoggerContext->NumberOfBuffers));
                                            KeResetEvent(&LoggerContext->FlushEvent);
                    //                        break;
                                        }
                    */
...
NTKERNELAPI NTSTATUS WmiUpdateTrace(IN OUT PWMI_LOGGER_INFORMATION LoggerInfo)
/*
Routine Description:
    It is called by WmipIoControl in wmi.c, with IOCTL_WMI_UPDATE_LOGGER to update certain characteristics of a running logger.
Arguments:
    LoggerInfo      a pointer to the structure for the logger's control and status information
Return Value:
    The status of performing the action requested.
*/
{
    ...
    // see if Logger is running properly first. Error checking will be done in WmiQueryTrace
    if (LoggerInfo == NULL)
        return STATUS_INVALID_PARAMETER;
    ...
    // First, check to make sure that you cannot turn on certain modes in UpdateTrace()
    ...
    if (LoggerInfo->LogFileHandle != NULL) {
        ...
        bDelayOpenFlag = (LoggerContext->LogFileHandle == NULL && (LoggerContext->LoggerMode & EVENT_TRACE_DELAY_OPEN_FILE_MODE));

        if (LoggerInfo->LogFileName.Buffer == NULL || LoggerMode & EVENT_TRACE_FILE_MODE_NEWFILE || NewMode & EVENT_TRACE_FILE_MODE_NEWFILE) {
        ...
        // Obtain the security context here so we can use it later to impersonate the user,
        // which we will do if we cannot access the file as SYSTEM.
        // This usually occurs if the file is on a remote machine.
        ...
        if (LoggerInfo->Checksum != NULL) {
            if (LoggerContext->LoggerHeader == NULL) {
                LoggerContext->LoggerHeader = ExAllocatePoolWithTag(PagedPool, sizeof(WNODE_HEADER) + sizeof(TRACE_LOGFILE_HEADER), TRACEPOOLTAG);
            }
            ...
            if (LoggerContext->LoggerHeader != NULL) {
                RtlCopyMemory(LoggerContext->LoggerHeader, LoggerInfo->Checksum, sizeof(WNODE_HEADER) + FIELD_OFFSET(TRACE_LOGFILE_HEADER, LoggerName));
            }
        }
    ...
    if (LoggerContext->KernelTraceOn && LoggerId == WmipKernelLogger && IsEqualGUID(&LoggerInfo->Wnode.Guid, &SystemTraceControlGuid)) {
    ...
    if (LoggerInfo->MaximumBuffers > 0) {
        Max_Buffers = (LoggerContext->BufferSize > 0) ? (ULONG)(MmMaximumNonPagedPoolInBytes / TRACE_MAXIMUM_NP_POOL_USAGE / LoggerContext->BufferSize) : 0;
...
NTKERNELAPI NTSTATUS WmiFlushTrace(IN OUT PWMI_LOGGER_INFORMATION LoggerInfo)
/*
Routine Description:
    It is called by WmipIoControl in wmi.c, with IOCTL_WMI_FLUSH_LOGGER to flush all the buffers out of a particular logger
Arguments:
    LoggerInfo      a pointer to the structure for the logger's control and status information
Return Value:
    The status of performing the action requested.
...
// Trace Provider APIs
NTKERNELAPI NTSTATUS FASTCALL WmiGetClockType(IN TRACEHANDLE LoggerHandle, OUT WMI_CLOCK_TYPE  *ClockType)
/*
Routine Description:
    This is called by anyone internal to find the clock type that is in use with a logger specified by the LoggerHandle
Arguments:
...
#endif
            WmipDereferenceLogger(LoggerId);
        TraceDebug((4, "WmiGetClockType: Status=%X %d %d->%d\n", STATUS_WMI_INSTANCE_NOT_FOUND, LoggerId, RefCount + 1, RefCount));
...
NTKERNELAPI LONG64 FASTCALL WmiGetClock(IN WMI_CLOCK_TYPE ClockType, IN PVOID Context)
/*
Routine Description:
    This is called anyone internal to use a particular clock for sequencing events.
Arguments:
...
NTSYSCALLAPI NTSTATUS NTAPI NtTraceEvent(__in HANDLE TraceHandle, __in ULONG Flags, __in ULONG FieldSize, __in PVOID Fields)
/*
Routine Description:
...
NTKERNELAPI NTSTATUS FASTCALL WmiTraceEvent(IN PWNODE_HEADER Wnode, IN KPROCESSOR_MODE RequestorMode)
/*
Routine Description:
    This routine is used by WMI data providers to trace events.
    It expects the user to pass in the handle to the logger.
    Also, the user cannot ask to log something that is larger than the buffer size (minus buffer header).
...
*/
{
    ...
    try {
...
#endif
                WmipDereferenceLogger(LoggerId);
            TraceDebug((4, "WmiTraceEvent: Status1=%X %d %d->%d\n", STATUS_UNSUCCESSFUL, LoggerId, RefCount + 1, RefCount));
            return STATUS_UNSUCCESSFUL;
        }
        ...
        if ((LoggerContext->RequestFlag & REQUEST_FLAG_CIRCULAR_PERSIST) && (LoggerContext->LoggerMode & EVENT_TRACE_FILE_MODE_CIRCULAR)) {
            if (!(Flags & WNODE_FLAG_PERSIST_EVENT)) {
                ULONG RequestFlag = LoggerContext->RequestFlag & (~(REQUEST_FLAG_CIRCULAR_PERSIST | REQUEST_FLAG_CIRCULAR_TRANSITION));
                if (InterlockedCompareExchange((PLONG)&LoggerContext->RequestFlag, RequestFlag | REQUEST_FLAG_CIRCULAR_TRANSITION, RequestFlag | REQUEST_FLAG_CIRCULAR_PERSIST)) {
                    // All persistence events are fired in circular
                    // logfile, flush out all active buffers and flushlist
                    // buffers. Also mark the end of persistence event collection in circular logger.

                    // It is the provider's responsibility to ensure that
                    // no persist event fires after this point. If it did, that event may be  overwritten during wrap around.
        ...
        TraceRecord = (PEVENT_TRACE_HEADER)WmipReserveTraceBuffer(LoggerContext, Size, &BufferResource, &TimeStamp);
        if (TraceRecord == NULL) {
#if DBG
            RefCount =
#endif
...
/*
Routine Description:
    This routine is used by trace kernel events only.
    These events can be charged to the given thread instead of the running thread.
    Because it can be called by I/O events at DPC level, this routine cannot be pageable when tracing is on.
...
*/
{
    ...
    Header = (PSYSTEM_TRACE_HEADER)WmipReserveTraceBuffer(LoggerContext, Size, &BufferResource, &TimeStamp);
    if (Header == NULL) {
#if DBG
        RefCount =
#endif
...
NTKERNELAPI NTSTATUS FASTCALL WmiTraceFastEvent(IN PWNODE_HEADER Wnode)
/*
Routine Description:
...
*/
{
    ...
    if ((*Marker & 0xF0000000) == TRACE_HEADER_ULONG32_TIME) {
        ...
        Buffer = WmipReserveTraceBuffer(LoggerContext, Size, &BufferResource, &TimeStamp);
        if (Buffer == NULL) {
#if DBG
            RefCount =
#endif
...
NTKERNELAPI NTSTATUS WmiTraceMessage(
...
#endif
                            WmipDereferenceLogger(LoggerId);
                        TraceDebug((4, "WmiTraceMessage: elemBytes too big %x %d %d->%d\n", STATUS_INVALID_BUFFER_SIZE, LoggerId, RefCount + 1, RefCount));
                        return STATUS_INVALID_BUFFER_SIZE;
                    }
       ...
       // Figure the size of the message including the header
        size = (USHORT)((MessageFlags&TRACE_MESSAGE_SEQUENCE ? sizeof(ULONG) : 0) +
            (MessageFlags&TRACE_MESSAGE_GUID ? sizeof(GUID) : 0) +
                        ...
                        dataBytes);
        if (dataBytes > size) {
            // We can ONLY log 64K (USHORT) data for a message. If the message is going to be larger than we could log, fail it.
#if DBG
            RefCount =
#endif
                WmipDereferenceLogger(LoggerId);
        ...
        Header = (PMESSAGE_TRACE_HEADER)WmipReserveTraceBuffer(LoggerContext, size, &BufferResource, &TimeStamp);
        if (Header == NULL) {
#if DBG
            RefCount =
#endif
        ...
        // Fill in Header.
        Header->Size = size;
        Header->Packet.OptionFlags = ((USHORT)MessageFlags & (TRACE_MESSAGE_SEQUENCE | TRACE_MESSAGE_GUID | TRACE_MESSAGE_COMPONENTID | TRACE_MESSAGE_TIMESTAMP | TRACE_MESSAGE_PERFORMANCE_TIMESTAMP | TRACE_MESSAGE_SYSTEMINFO)) & TRACE_MESSAGE_FLAG_MASK;
        // Message Number
        ...
        // Note that the order in which these are added is critical New entries must be added at the end!
...
NTKERNELAPI NTSTATUS FASTCALL WmiTraceUserMessage(IN PMESSAGE_TRACE_USER pMessage, IN ULONG    MessageSize)
/*
Routine Description:
    This routine is used by trace User messages only. it is called via an IOCTL on the WMI Device.
Arguments:
...
*/
{
    ...
    try {
        ...
        return (WmiTraceMessage(pMessage->LoggerHandle, pMessage->MessageFlags, &pMessage->MessageGuid, pMessage->MessageHeader.Packet.MessageNumber, &pMessage->Data, pMessage->DataSize, NULL, 0));
    } except(EXCEPTION_EXECUTE_HANDLER)
    {
...
NTKERNELAPI NTSTATUS WmiSetMark(IN PWMI_SET_MARK_INFORMATION MarkInfo, IN ULONG InBufferLen)
/*
Routine Description:
...
NTKERNELAPI NTSTATUS WmiSetTraceBufferCallback(IN TRACEHANDLE  TraceHandle, IN WMI_TRACE_BUFFER_CALLBACK Callback, IN PVOID Context)
/*
Routine Description:
...
NTKERNELAPI NTSTATUS WmiQueryTraceInformation(
...
Routine Description:
    ...
    If RequiredLength is given, the needed size for the requested information is returned.
Arguments:
    ...
    NOTE: we do not consider NULL TraceInformation an error, In this case, we only update RequiredLength, if that is given.
Return Value:
    The status of performing the action requested.
*/
{
