...
Abstract:
    ...
    WMI security is guid based, that is each guid can be assigned a security descriptor.
    There is also a default security descriptor that applies to any guid that does not have its own specific security descriptor.
    Security is enforced by relying upon the object manager.
    We define the WmiGuid object type and require all WMI requests to have a handle to the WmiGuid object.
    In this way the guid is opened with a specific ACCESS_MASK and if the caller is permitted those rights (as specified in the specific or default security descriptor) then a handle is returned.
    When the caller wants to do an operation he must pass the handle and before the operation is performed we check that the handle has the allowed access.

    Guid security descriptors are serialized as REG_BINARY values under the registry key HKLM\CurrentControlSet\Control\Wmi\Security.
    If no specific or default security descriptor for a guid exists then the all access is available for anyone.
    For this reason this registry key must be protected.

    WMI implements its own security method for the WmiGuid object type to allow it to intercept any changes to an objects security descriptor.
    By doing this we allow the standard security apis (Get/SetKernelObjectSecurity) to query and set the WMI security descriptors.
...
/*
Routine Description:
    This is the WMI security method for objects.
    It is responsible for either retrieving, setting, and deleting the security descriptor of an object.
    ...
    It does not do anything special except serialize any SD that is being set for an object.
...
Arguments:
    ...
    CapturedLength - For the query operation this specifies the length, in bytes, of the security descriptor buffer,
        and upon return contains the number of bytes needed to store the descriptor.
    ...
    ObjectsSecurityDescriptor - For the Set operation this supplies the address of a pointer to the object's current security descriptor.
        This routine will either modify the security descriptor in place or allocate a new security descriptor and use this variable to indicate its new location.
        For the query operation it simply supplies the security descriptor being queried.
        The caller is responsible for freeing the old security descriptor.
    ...
    PoolType - For the set operation this specifies the pool type to use if a new security descriptor needs to be allocated.
        ...
        the mapping of generic to specific/standard access types for the object being accessed.
...
*/
{
    ...
    // If the object's security descriptor is null, then object is not one that has security information associated with it.  Return an error.
    ...
    case SetSecurityDescriptor:
    {
        ...
        Status = ObSetSecurityDescriptorInfo(Object, SecurityInformation, SecurityDescriptor, ObjectsSecurityDescriptor, PoolType, GenericMapping);
        if (NT_SUCCESS(Status)) {
            // Serialize the guid's new security descriptor in the registry.
            // But first we need to get a copy of it.
            ...
            do {
                ...
                Status2 = ObQuerySecurityDescriptorInfo(Object, &LocalSecInfo, SecurityDescriptorCopy, &SecurityDescriptorLength, ObjectsSecurityDescriptor);
            ...
            if (NT_SUCCESS(Status2)) {
                ...
                StringCbPrintf(GuidBuffer, sizeof(GuidBuffer), L"%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x",
    ...
    case QuerySecurityDescriptor:
    {
        ...
        return ObQuerySecurityDescriptorInfo(Object, SecurityInformation, SecurityDescriptor, CapturedLength, ObjectsSecurityDescriptor);
    }
    case DeleteSecurityDescriptor:
    {
        Status = ObDeassignSecurity(ObjectsSecurityDescriptor);//  call the default delete security method
        return(Status);
    }
    case AssignSecurityDescriptor:
        ObAssignObjectSecurityDescriptor(Object, SecurityDescriptor, PoolType);
        return(STATUS_SUCCESS);
    default://  Bugcheck on any other operation code,  We won't get here if the earlier asserts are still checked.        
...
/*
Routine Description:
    This routine will initialize WMI security subsystem.
    Basically we create the WMIGUID object type, obtain the SECURITY_SUBJECT_CONTEXT for
    the System process and establish a SD that allows all access that is used when no default or specific SD is assigned to a guid.
*/
{
    NTSTATUS Status;
    ...
    DefaultAccessDacl = (PACL)ExAllocatePoolWithTag(PagedPool, DaclLength, WMIPOOLTAG);
    if (DefaultAccessDacl == NULL) {
        return(STATUS_INSUFFICIENT_RESOURCES);
    }

    Status = RtlCreateAcl(DefaultAccessDacl, DaclLength, ACL_REVISION2);
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }

    Status = RtlAddAccessAllowedAce(DefaultAccessDacl, ACL_REVISION2, (STANDARD_RIGHTS_ALL | SPECIFIC_RIGHTS_ALL), SeLocalSystemSid);
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }

    Status = RtlAddAccessAllowedAce(DefaultAccessDacl, ACL_REVISION2, TRACELOG_REGISTER_GUIDS, SeAliasUsersSid);
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }

    Status = RtlAddAccessAllowedAce(DefaultAccessDacl, ACL_REVISION2, (STANDARD_RIGHTS_ALL | SPECIFIC_RIGHTS_ALL | ACCESS_SYSTEM_SECURITY), SeAliasAdminsSid);
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }

    Status = RtlAddAccessAllowedAce(DefaultAccessDacl, ACL_REVISION2, (STANDARD_RIGHTS_ALL | SPECIFIC_RIGHTS_ALL), SeExports->SeLocalServiceSid);
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }


    Status = RtlAddAccessAllowedAce(DefaultAccessDacl, ACL_REVISION2, (STANDARD_RIGHTS_ALL | SPECIFIC_RIGHTS_ALL), SeExports->SeNetworkServiceSid);
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }

    WmipDefaultAccessSd = &WmipDefaultAccessSecurityDescriptor;
    Status = RtlCreateSecurityDescriptor(WmipDefaultAccessSd, SECURITY_DESCRIPTOR_REVISION1);
    ...
    Status = RtlSetOwnerSecurityDescriptor(WmipDefaultAccessSd, SeAliasAdminsSid, FALSE);
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }

    Status = RtlSetGroupSecurityDescriptor(WmipDefaultAccessSd, SeAliasAdminsSid, FALSE);
    if (!NT_SUCCESS(Status)) {
    Cleanup:
    ...
    SeCaptureSubjectContext(&WmipSystemSubjectContext);// Remember System process subject context
    ...
    // All named objects may (must ?) have security descriptors attached to them.
    // If unnamed objects also must have security descriptors attached then this must be TRUE
    ObjectTypeInitializer.SecurityRequired = TRUE;

    ObjectTypeInitializer.MaintainHandleCount = FALSE;// Tracks # handles open for object within a process

    // Need to be in non paged pool since KEVENT contained within the object must be in non paged pool
    ObjectTypeInitializer.PoolType = NonPagedPool;
    ObjectTypeInitializer.DefaultNonPagedPoolCharge = sizeof(WMIGUIDOBJECT);

    // Use a custom security procedure so that we can serialize any changes to the security descriptor.
...
NTSTATUS WmipSDRegistryQueryRoutine(IN PWSTR ValueName, IN ULONG ValueType, IN PVOID ValueData, IN ULONG ValueLength, IN PVOID Context, IN PVOID EntryContext)
/*
Routine Description:
...
Arguments:
    ...
    Context - Not used
    EntryContext - Pointer to PSECURITTY_DESCRIPTOR to store a pointer to store the security descriptor read from the registry value
Return Value:
    NT Status code
*/
{
    ...
    if ((ValueType == REG_BINARY) && (ValueData != NULL)) {
        // If a SD is specified in the registry then verify that it is valid and if so then copy it
        if (SeValidSecurityDescriptor(ValueLength, (PSECURITY_DESCRIPTOR)ValueData)) {
...
/*
Routine Description:
    This routine will serialize the security descriptor associated with a guid.

    Security descriptors are maintained as REG_BINARY values named by the guid in the registry under
...
*/
{
    ...
    Status = RtlWriteRegistryValue(RTL_REGISTRY_CONTROL, L"WMI\\Security", GuidName->Buffer, REG_BINARY, SecurityDescriptor, SecurityDescriptorLength);
    return(Status);
}
...
/*
Routine Description:
    This routine will retrieve the security descriptor associated with a guid.
    First it looks for a security descriptor specifically for the guid and if not found then looks for the default security descriptor.
...
Arguments:
    ...
    *SecurityDescriptor returns the security descriptor for the guid.
    It must be freed back to pool unless it is the same value as that in WmipDefaultAccessSd which must NOT be freed.
Return Value:
    NT Status code
*/
{
    ...
    *SecurityDescriptor = NULL;
    if (NT_SUCCESS(Status)) {
        ...
        // default SD. Else we use the default SD unless that doesn't exist and so there is no security
    ...
    if (*SecurityDescriptor == NULL) {
        ...
        // Set the default security if none was found in the registry for the Guid
...
NTSTATUS WmipOpenGuidObject(
    ...
    /*
    Routine Description:
        This routine will open a handle to a WmiGuid object with the access rights specified.
        WmiGuid objects are temporary objects that are created on an as needed basis.
        We will always create a new unnamed guid object each time a guid is opened.
    Arguments:
        GuidString is the string representation for the guid that refers to the object to open.
            Note that this parameter has NOT been probed.
        ...
        *ObjectPtr returns containing a pointer to the object. This object will have a reference attached to it that must be dereferenced by the calling code.
    ...
    */
{
    ...
    // Validate guid object name passed by ensuring that it is in the correct object directory and the correct format for a uuid
    CapturedGuidString = CapturedObjectAttributes->ObjectName;
    if (RtlEqualMemory(CapturedGuidString->Buffer, WmiGuidObjectDirectory, (WmiGuidObjectDirectoryLength - 1) * sizeof(WCHAR)) == 0) {
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (!NT_SUCCESS(Status)) {
            // Remove extra ref count taken by ObInsertObject since we are returning an error
            ObDereferenceObject(GuidObject);
        }

        // Make sure to close handle obtained in creating object. We
        // attach to the system process since the handle was created in its handle table.
...
NTSTATUS WmipCreateGuidObject(IN OUT POBJECT_ATTRIBUTES ObjectAttributes, IN ACCESS_MASK DesiredAccess, IN LPGUID Guid, OUT PHANDLE CreatorHandle, OUT PWMIGUIDOBJECT *Object)
/*
Routine Description:
    This routine will create a new guid object for the guid passed.
    The handle returned is the handle issued to the creator of the object and should be closed after the object is opened.

    Guid Objects are created on the fly, but

Arguments:
    ObjectAttributes - Describes object being created. ObjectAttributes is modified in this call.
    Guid is the guid for which the object is being created
    *CreatorHandle returns a handle to the created guid object. This handle is in the system process handle table
...
*/
{
    ...
    Status = WmipGetGuidSecurityDescriptor(&UnicodeString, &SecurityDescriptor, NULL);
    if (NT_SUCCESS(Status)) {
        ...
        if (NT_SUCCESS(Status)) {
            ...
            *SubjectContext = WmipSystemSubjectContext;
            ...
            // Attach to system process so that the initial handle created by ObInsertObject is not available to user mode. This handle
            // allows full access to the object.
            KeAttachProcess(&PsInitialSystemProcess->Pcb);

            Status = ObCreateObject(KernelMode, WmipGuidObjectType, &UnnamedObjectAttributes, KernelMode, NULL, sizeof(WMIGUIDOBJECT), 0, 0, (PVOID *)Object);
            if (NT_SUCCESS(Status)) {
                ...
                // code. User mode can guess the handle and close it even before we return it.
...
VOID WmipCloseMethod(IN PEPROCESS Process OPTIONAL, IN PVOID Object, IN ACCESS_MASK GrantedAccess, IN ULONG_PTR ProcessHandleCount, IN ULONG_PTR SystemHandleCount)
/*
Routine Description:
    This routine is called whenever a guid object handle is closed.
    We only need to worry about this for reply object and then only when the last handle to it is closed.
Arguments:
    Process
    Object
    GrantedAccess
...
*/
{
    ...
    if (SystemHandleCount == 1) {
        ...
        if (ReplyObject->Flags & WMIGUID_FLAG_REPLY_OBJECT) {
            // When a reply object is closed we need to make sure that any reference to it by a request object is cleaned up
...
VOID WmipDeleteMethod(IN  PVOID   Object)
{
    ...
    if (GuidObject->Flags & WMIGUID_FLAG_REQUEST_OBJECT) {
        ...
        // First reply to all reply objects that are waiting for a reply
        WmipEnterSMCritSection();
        ...
        // sure to remove the pointer from the datasource to the regentry
        ...
        // guidentry list and finally remove the refcount on the guid entry held by the object
...
BOOLEAN WmipHexStringToDword(IN PWCHAR lpsz, OUT PULONG RetValue, IN ULONG cDigits, IN WCHAR chDelim)
{
    ULONG Count;
    ULONG Value;
...
NTSTATUS WmipUuidFromString(IN PWCHAR StringUuid, OUT LPGUID Uuid)
/*
Routine Description:
    We convert a UUID from its string representation into the binary representation.
    Parse UUID such as 00000000-0000-0000-0000-000000000000
Arguments:
    StringUuid -  supplies the string representation of the UUID. It is assumed that this parameter has been probed and captured
...
NTSTATUS WmipCheckGuidAccess(IN LPGUID Guid, IN ACCESS_MASK DesiredAccess, IN PSECURITY_DESCRIPTOR UserDefaultSecurity)
/*
Routine Description:
...
Arguments:
    ...
    DesiredAccess is the access that is desired by the user.
                  NOTE: This does not support GENERIC_* mappings or ASSIGN_SYSTEM_SECURITY
Return Value:
...
*/
{
    ...
    Status = WmipGetGuidSecurityDescriptor(&GuidString, &SecurityDescriptor, UserDefaultSecurity);
    if (NT_SUCCESS(Status)) {
        ...
        if ((SecurityDescriptor != WmipDefaultAccessSd) && (SecurityDescriptor != UserDefaultSecurity)) {
            ExFreePool(SecurityDescriptor);
        }
    }
...
NTSTATUS WmipCreateAdminSD(PSECURITY_DESCRIPTOR *Sd)
{
    ...
    DaclLength = (ULONG)sizeof(ACL) + (2 * ((ULONG)sizeof(ACCESS_ALLOWED_ACE))) + SeLengthSid(SeAliasAdminsSid) + SeLengthSid(SeLocalSystemSid) + 8; // The 8 is just for good measure

    AdminDeviceSd = (PSECURITY_DESCRIPTOR)ExAllocatePoolWithTag(PagedPool, DaclLength + sizeof(SECURITY_DESCRIPTOR), WMIPOOLTAG);
    if (AdminDeviceSd != NULL) {
        AdminDeviceDacl = (PACL)((PUCHAR)AdminDeviceSd + sizeof(SECURITY_DESCRIPTOR));
        Status = RtlCreateAcl(AdminDeviceDacl, DaclLength, ACL_REVISION2);
        if (NT_SUCCESS(Status)) {
            Status = RtlAddAccessAllowedAce(AdminDeviceDacl, ACL_REVISION2, FILE_ALL_ACCESS, SeAliasAdminsSid);
            if (NT_SUCCESS(Status)) {
                Status = RtlAddAccessAllowedAce(AdminDeviceDacl, ACL_REVISION2, FILE_ALL_ACCESS, SeLocalSystemSid);
                if (NT_SUCCESS(Status)) {
                    Status = RtlCreateSecurityDescriptor(AdminDeviceSd, SECURITY_DESCRIPTOR_REVISION1);
                    if (NT_SUCCESS(Status)) {
                        Status = RtlSetDaclSecurityDescriptor(
                            AdminDeviceSd,
                            TRUE,                       // DaclPresent
                        ...
                        if (NT_SUCCESS(Status)) {
                            // We need to make sure that there is an owner for the security descriptor since it is required when security is being checked when the device is being opened.
                            Status = RtlSetOwnerSecurityDescriptor(AdminDeviceSd, SeAliasAdminsSid, FALSE);
                        }
                    }
                }
            }
