...
NTSTATUS WmipAddProviderIdToPIList(PBINSTANCESET **PIPtrPtr, PULONG PICountPtr, PULONG PIMaxPtr, PBINSTANCESET *StaticPIPtr, PBINSTANCESET InstanceSet);
NTSTATUS WmipPrepareForWnodeAD(
    IN PWMIGUIDOBJECT GuidObject, OUT LPGUID Guid, IN OUT ULONG *ProviderIdCount, OUT PBINSTANCESET **ProviderIdList, OUT BOOLEAN *InternalGuid);
...
NTSTATUS WmipWriteMBToObject(IN PWMIGUIDOBJECT RequestObject, IN PWMIGUIDOBJECT ReplyObject, IN PUCHAR Message, IN ULONG MessageSize);
NTSTATUS WmipWriteMessageToGuid(IN PBGUIDENTRY GuidEntry, IN PWMIGUIDOBJECT ReplyObject, IN PUCHAR Message, IN ULONG MessageSize, OUT PULONG WrittenCount);
...
BOOLEAN WmipIsQuerySetGuid(PBGUIDENTRY GuidEntry)
{
    ...
    while (InstanceSetList != &GuidEntry->ISHead) {
        ...
        if ((InstanceSet->Flags & (IS_TRACED | IS_CONTROL_GUID | IS_EVENT_ONLY)) == 0) {
            // If there is at least one IS that isn't traced and isn't an event only then it is a queryset guid
...
NTSTATUS WmipOpenBlock(IN ULONG Ioctl, IN KPROCESSOR_MODE AccessMode, IN POBJECT_ATTRIBUTES CapturedObjectAttributes, IN ULONG DesiredAccess, OUT PHANDLE Handle)
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (Ioctl != IOCTL_WMI_OPEN_GUID) {
            ...
            {
                // Guid is being opened for query/set/method operations so we need to ensure that there is a guid entry and that
                // the guid entry has InstanceSets attached and it is has at least one instance set that is not a traced guid and is not an event only guid
                if ((GuidEntry == NULL) || (GuidEntry->ISCount == 0) || (!WmipIsQuerySetGuid(GuidEntry))) {
                    // Either we could not find a guidentry or there is no instance sets attached.
                    // We close the original handle and fail the IOCTL
                    ZwClose(*Handle);
            ...
            {
                // Since we can register to receive events before the event provider has been registered we'll need to create the guid entry if one does not exist
                ...
                if (GuidEntry == NULL) {
                    ...
                    if (GuidEntry != NULL) {
                        // Initialize the new GuidEntry and place it on the master GuidEntry list.
                ...
                // Now we need to see if we have to enable collection or events
                Status = WmipEnableCollectOrEvent(GuidEntry, Ioctl, &Object->EnableRequestSent, 0);
                if (!NT_SUCCESS(Status)) {
                    // For some reason enabling failed so just return the error
                    ZwClose(*Handle);
                }

                // Don't unref the guid entry as that ref count is taken by the object just placed on the list
                break;
            }
            default:
            {
            ...
            Object->Flags |= WMIGUID_FLAG_SECURITY_OBJECT;// Mark this as a security object
        }

        ObDereferenceObject(Object);// remove the ref taken when the object was created
    }
...
NTSTATUS WmipAddProviderIdToPIList(PBINSTANCESET **PIPtrPtr, PULONG PICountPtr, PULONG PIMaxPtr, PBINSTANCESET *StaticPIPtr, PBINSTANCESET InstanceSet)
{
    ...
    if (PICount == PIMax) {
        // We have overflowed the PI List so we need to reallocate a bigger buffer
        NewPIMax = PIMax * 2;
        ...
        if (NewPIPtr != NULL) {
            ...
            *PIMaxPtr = PIMax;
            ...
            // Bad break, we could not allocate more space unref any instance sets and return an error
            for (i = 0; i < PIMax; i++) {
...
NTSTATUS WmipPrepareForWnodeAD(IN PWMIGUIDOBJECT GuidObject, OUT LPGUID Guid, IN OUT ULONG *ProviderIdCount, OUT PBINSTANCESET **ProviderIdList, OUT BOOLEAN *InternalGuid)
{
        ...
        if (GuidEntry->Flags & GE_FLAG_INTERNAL) {
            ...
            *InternalGuid = FALSE;
            ...
            while ((InstanceSetList != &GuidEntry->ISHead) && NT_SUCCESS(Status)) {
                ...
                // Take a refcount on the instance set so that it won't go away until after we are done with our query
                // The refcount gets removed by the caller when it is done with the list or in WmipAddProviderIdTOLlist  if it returns an error
                if ((InstanceSet->Flags & (IS_TRACED | IS_CONTROL_GUID | IS_EVENT_ONLY)) == 0) {
                    // Only take those IS that are not traced or control guids and are not event only guids
...
Return Value:
    Size needed to place instance names in a WNODE_ALL_DATA plus 3.
    The extra 3 bytes are added in case the OffsetInstanceNameOffsets need to be padded since they must be on a 4 byte boundary.
*/
{
    SIZE_T NameSize;
    ULONG i;
    ...
    // Start with a name size of 3 in case the OffsetInstanceNameOffset will need to be padded so that it starts on a 4 byte boundary.
    NameSize = 3;
    if (InstanceSet->Flags & IS_INSTANCE_BASENAME) {
        // For static base names we assume that there will never be more than MAXBASENAMESUFFIXVALUE instances of a guid. So the size of each instance name
        // would be the size of the base name plus the size of the suffix plus a USHORT for the count (for counted string) plus a ULONG to hold the offset to the instance name
        WmipAssert((InstanceSet->IsBaseName->BaseIndex + InstanceSet->Count) < MAXBASENAMESUFFIXVALUE);
        NameSize += ((wcslen(InstanceSet->IsBaseName->BaseName) * sizeof(WCHAR)) + MAXBASENAMESUFFIXLENGTH * sizeof(WCHAR) + sizeof(USHORT) + sizeof(ULONG)) * InstanceSet->Count;
    } else if (InstanceSet->Flags & IS_INSTANCE_STATICNAMES) {
        // For a static name list we count up each size of the static instance names in the list and add a ULONG and a USHORT for the offset and count (for counted string)
...
/*
Routine Description:
    This routine will copy into the WNODE_ALL_DATA instance names for a static instance name set.
    If the Wnode_All_data is too small then it is converted to a WNODE_TOO_SMALL
Arguments:
...
*/
{
    ...
    // Pad out the size of the incoming wnode to a 4 byte boundary since the OffsetInstanceNameOffsets is being appended to the end of the wnode and it must be on a 4 byte boundary
    PaddedBufferSize = (Wnode->WnodeHeader.BufferSize + 3) & ~3;

    // Compute the complete size needed to rewrite the WNODE to include the instance names.
    //     Include the size that is needed to fill out
    Size = WmipStaticInstanceNameSize(InstanceSet);

    //     Include the space needed for the array of offsets to the instance names plus the size of the names plus the padded size of the wnode
    SizeNeeded = (InstanceCount * sizeof(ULONG)) + (ULONG)Size + PaddedBufferSize;
    if (SizeNeeded > MaxWnodeSize) {
        // If not enough space left in the buffer passed then build a WNODE_TOO_SMALL as the result to indicate how much buffer space is needed.
    ...
    if (InstanceSet->Flags & IS_INSTANCE_BASENAME) {
        // The instance name is based upon a basename with a trailing index number to provide uniqueness
        ...
        for (i = 0; i < InstanceCount; i++) {
            ...
            *NameOffsetPtr++ = (ULONG)((PUCHAR)NamePtr - (PUCHAR)Wnode);
            ...
            hr = StringCbCopy(NamePtr + 1, Size, InstanceSet->IsBaseName->BaseName);
            WmipAssert(hr == S_OK);

            // Format unique index number
            hr = StringCbPrintf(Index, sizeof(Index), BASENAMEFORMATSTRING, InstanceSet->IsBaseName->BaseIndex + i);
            WmipAssert(hr == S_OK);

            // Append unique index number to instance name
            hr = StringCbCat(NamePtr + 1, Size, Index);
            WmipAssert(hr == S_OK);
        ...
        for (i = 0; i < InstanceCount; i++) {
            ...
            *NamePtr++ = Len;
            ...
            hr = StringCbCopyEx(NamePtr, Size, StaticName, NULL, &Size, 0);
            WmipAssert(hr == S_OK);
...
// This defines how many provider ids will fit within the static block.
// If we need more than this, then we'll have to allocate memory for it
#if DBG
...
NTSTATUS WmipQueryAllData(IN PWMIGUIDOBJECT GuidObject, IN PIRP Irp, IN KPROCESSOR_MODE AccessMode, IN PWNODE_ALL_DATA Wnode, IN ULONG OutBufferLen, OUT PULONG RetSize)
{
    ...
    if (GuidObject != NULL) {
        ...
        Status = ObReferenceObjectByHandle(KernelHandle, WMIGUID_QUERY, WmipGuidObjectType, AccessMode, &GuidObject, NULL);
    }
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (NT_SUCCESS(Status)) {
            if (InternalGuid) {
                // This is an internal guid so we fill out the WNODE_ALL_DATA and mark it to be completed by the user mode code
                ...
                *RetSize = sizeof(WNODE_HEADER);
                ...
                // Get all of the information from the WNODE_HEADER so we can rebuild it
                ...
                for (i = 0; i < PICount; i++) {
                    ...
                    if ((IsBufferTooSmall) || (BufferLeft < sizeof(WNODE_ALL_DATA))) {
                        // If we have already determined that the buffer is too small then we use the static WNODE_ALL_DATA just to get the size needed
                        WnodeAD = &WnodeAllData;
                        BufferLeft = sizeof(WNODE_ALL_DATA);
                        IsBufferTooSmall = TRUE;
                    } else {// Otherwise we will append to the end of the buffer
                    ...
                    UsesStaticNames = ((InstanceSet->Flags & IS_INSTANCE_BASENAME) || (InstanceSet->Flags & IS_INSTANCE_STATICNAMES));
                    ...
                    if (Irp != NULL) {
                        Status = WmipForwardWmiIrp(Irp, IRP_MN_QUERY_ALL_DATA, WnodeHeader->ProviderId, &WnodeHeader->Guid, BufferLeft, WnodeAD);
                    } else {
                        Status = WmipSendWmiIrp(IRP_MN_QUERY_ALL_DATA, WnodeHeader->ProviderId, &WnodeHeader->Guid, BufferLeft, WnodeAD, &Iosb);
                    }

                    if (NT_SUCCESS(Status)) {
                        if (WnodeHeader->Flags & WNODE_FLAG_TOO_SMALL) {
                            // There was not enough space to write the WNODE so we keep track of how much was needed and then switch to the mode where we just query for size needed
                            WnodeTooSmall = (PWNODE_TOO_SMALL)WnodeAD;
                            SizeNeeded += WnodeTooSmall->SizeNeeded;
                            if (UsesStaticNames) {
                                SizeNeeded = (SizeNeeded + 3) &~3;
                                SizeNeeded += WmipStaticInstanceNameSize(InstanceSet) + (InstanceSet->Count * sizeof(ULONG));
                            }

                            SizeNeeded = (SizeNeeded + 7) & ~7;
                            IsBufferTooSmall = TRUE;
                        } else if (IsBufferTooSmall) {
                            // We passed a minimum sized buffer, but it is large enough for the driver. 
                            // Since we are just trying to get the size needed we get the size he needs and throw away his data
                            SizeNeeded += WnodeAD->WnodeHeader.BufferSize + WmipStaticInstanceNameSize(InstanceSet) + (InstanceSet->Count * sizeof(ULONG));
                            SizeNeeded = (SizeNeeded + 7) & ~7;
                        } else {
                            // The driver returned a completed WNODE_ALL_DATA so we need to link the previous WNODE_ALL_DATA to this one, 
                            // fill out any static instance names, and then update the buffer pointer and size
                            if (WnodeLast != NULL) {
                            ...
                            if (UsesStaticNames) {
                                ...
                                if (WnodeAD->WnodeHeader.Flags & WNODE_FLAG_TOO_SMALL) {
                                    // The static names caused us to run out of buffer so we switch to mode  where we query for the sizes
                                    WnodeTooSmall = (PWNODE_TOO_SMALL)WnodeAD;
                                    IsBufferTooSmall = TRUE;
                                    BufferUsed = WnodeTooSmall->SizeNeeded;
                                } else {
                                    // Static names fit so just pull out the updated wnode size
                                    BufferUsed = WnodeAD->WnodeHeader.BufferSize;
                                }
                            } else {
                                // Wnode has dynamic names so just add size returned by driver
                                BufferUsed = WnodeAD->WnodeHeader.BufferSize;
                            }

                            // Update size needed and advance to free space in output buffer
                            BufferUsed = (BufferUsed + 7) & ~7;
                            SizeNeeded += BufferUsed;

                            // Make sure that by adding in pad we don't run out of room in buffer
                        ...
                        // The driver failed the request, but that is no biggie as we just ignore it for now
                    }

                    // We are done with the instance set so remove our ref on it so it can go away if need be
                    WmipUnreferenceIS(InstanceSet);
                }

                if (SizeNeeded == 0) {
                    // No devices responded to the WMI Query All Data so we return an error
                    Status = STATUS_WMI_GUID_NOT_FOUND;
        ...
        ObDereferenceObject(GuidObject);// And remove ref on guid object
    }
...
)
{
    ...
    if (ObjectList == NULL) {
        ...
        // Copy the handle list out of the system buffer since it will be overwritten by the first query all data
    ...
    for (i = 0; i < Count; i++) {
        if ((Wnode == &WnodeAD) || (WnodeSize < sizeof(WNODE_ALL_DATA))) {
            // If there is no more room, we are just querying for the size that will be needed.
            Wnode = &WnodeAD;
        ...
        if (NT_SUCCESS(Status2)) {
            if (Wnode->WnodeHeader.Flags & WNODE_FLAG_INTERNAL) {
                ...
                // There is no enough room so just tally up the size that will be needed.
                WnodeTooSmall = (PWNODE_TOO_SMALL)Wnode;
                ...
                // Even though this succeeded, we still aren't going to be able to return any data so just count up how much size we need
                SizeNeeded += (RetSize + 7) & ~7;
                BufferOverFlow = TRUE;
                WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_API_INFO_LEVEL, "WMI: %x Large Enough but full %x needed, total %x\n", ObjectList ? ObjectList[i] : Handles[i], RetSize, SizeNeeded));
            } else {
                // We successfully got data. Link the previous wnode to this one
                if (WnodePrev != NULL) {
    ...
    OUT BOOLEAN *InternalGuid)
{
    ...
    *IsDynamic = TRUE;
    ...
    if ((GuidEntry != NULL) && (GuidEntry->ISCount > 0)) {
        // We were passed a valid guid handle, fill out the guid in WNODE_HEADER
        Status = STATUS_SUCCESS;
        ...
        if (GuidEntry->Flags & GE_FLAG_INTERNAL) {
            ...
            *InternalGuid = FALSE;
            ...
            if (InstanceName != NULL) {
                // Remember the static provider id list and assume that the request is going to be dynamic
                StaticPIPtr = *ProviderIdList;
                PIPtr = StaticPIPtr;
                PIMax = *ProviderIdCount;
                PICount = 0;

                // March down instance set list to see if we have a static name and build the list of dynamic provider ids
                Done = FALSE;
                WmipEnterSMCritSection();
                    ...
                    while ((InstanceSetList != &GuidEntry->ISHead) && !Done) {
                        ...
                        if ((InstanceSet->Flags & (IS_TRACED | IS_CONTROL_GUID | IS_EVENT_ONLY)) == 0) {
                            ...
                            if (InstanceSet->Flags & IS_INSTANCE_BASENAME) {
                                ...
                                if ((wcslen(InstanceName) > BaseNameLen) && (_wcsnicmp(InstanceName, BaseName, BaseNameLen) == 0)) {
                                    // The suffix matches the beginning of our instance name and our instance name is longer than the suffix.
                                    SuffixPtr = &InstanceName[BaseNameLen];
                                    Suffix = _wtoi(SuffixPtr);
                                    if ((WmipIsNumber(SuffixPtr) && (Suffix >= BaseIndex) && (Suffix < (BaseIndex + InstanceSet->Count)))) {
                                        // Our suffix is a number within the range for this instance set
                                        if (Suffix < MAXBASENAMESUFFIXVALUE) {
                                            StringCbPrintf(SuffixText, sizeof(SuffixText), BASENAMEFORMATSTRING, Suffix);
                                            if (_wcsicmp(SuffixText, SuffixPtr) == 0) {
                                                // Our instance name is part of the instance set so note the provider id and instance index
                                ...
                                // See if the passed instance name matches any of the static names for this instance set
                                PWCHAR *StaticNames;
                                StaticNames = InstanceSet->IsStaticNames->StaticNamePtr;
                                for (i = 0; i < InstanceSet->Count; i++) {
                                    if (_wcsicmp(StaticNames[i], InstanceName) == 0) {
                                        // We matched our instance name with a static instance name.
                                        // Remember provider id and instance index.
                                        Wnode->Flags |= WNODE_FLAG_STATIC_INSTANCE_NAMES;
                                        Wnode->ProviderId = InstanceSet->ProviderId;
                ...
                } else {// There are no instance sets registered for this guid
                    Status = STATUS_WMI_GUID_DISCONNECTED;
                }
            ...
            if (*IsDynamic) {
                ...
                *ProviderIdCount = PICount;
                *ProviderIdList = PIPtr;
            } else {// Static instance name so unref an dynamic instance sets
                if (PIPtr != NULL) {
                    for (i = 0; i < PICount; i++) {
                        WmipUnreferenceIS(PIPtr[i]);
                    }
    ...
    OUT PULONG RetSize)
{
    ...
    WmipAssert(((MinorFunction >= IRP_MN_QUERY_ALL_DATA) && (MinorFunction <= IRP_MN_CHANGE_SINGLE_ITEM)) || (MinorFunction == IRP_MN_EXECUTE_METHOD));

    // Check Security
    if (GuidObject != NULL) {
        Status = ObReferenceObjectByPointer(GuidObject, DesiredAccessForFunction[MinorFunction], WmipGuidObjectType, AccessMode);
    } else {
        KernelHandle = Wnode->KernelHandle;
        Status = ObReferenceObjectByHandle(KernelHandle, DesiredAccessForFunction[MinorFunction], WmipGuidObjectType, AccessMode, &GuidObject, NULL);
    }

    if (NT_SUCCESS(Status)) {
        PIList = StaticPIList;
        PICount = MANYPROVIDERIDS;
        Status = WmipPrepareWnodeSI(GuidObject, (PWNODE_SINGLE_INSTANCE)Wnode, &PICount, &PIList, &IsDynamic, &InternalGuid);
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_API_INFO_LEVEL, "WMI: QSI Prepare [%s - %s] %x with %x PI at %p\n",
                          IsDynamic ? "Dynamic" : "Static", InternalGuid ? "Internal" : "External", Wnode->KernelHandle, PICount, PIList));
        if (NT_SUCCESS(Status)) {
            if (InternalGuid) {
                // Internal guid query
                ...
                if (IsDynamic) {
                    // We need to loop over all dynamic instance names until one of them responds successfully and then we assume that they own the instance
                    ...
                    for (i = 0; i < PICount; i++) {
                        ...
                        if (Irp != NULL) {
                            ReturnStatus = WmipForwardWmiIrp(Irp, MinorFunction, Wnode->ProviderId, &Wnode->Guid, OutBufferSize, Wnode);
                            if (NT_SUCCESS(ReturnStatus)) {
                                *RetSize = (ULONG)Irp->IoStatus.Information;
                            }
                        } else {
                            ReturnStatus = WmipSendWmiIrp(MinorFunction, Wnode->ProviderId, &Wnode->Guid, OutBufferSize, Wnode, &Iosb);
                            if (NT_SUCCESS(ReturnStatus)) {
                                *RetSize = (ULONG)Iosb.Information;
                            }
                        }

                        // One of these status codes imply that the device does positively claim the instance name and so we break out and return the results
                        if ((NT_SUCCESS(ReturnStatus)) ||
                        ...
                        // If the device does not own the instance it can only return STATUS_WMI_INSTANCE_NOT_FOUND or STATUS_WMI_GUID_NOT_FOUND.
                        // Any other return code implies that the device owns the instance, but encountered an error.
                        if ((ReturnStatus != STATUS_WMI_INSTANCE_NOT_FOUND) && (ReturnStatus != STATUS_WMI_GUID_NOT_FOUND)) {
                            Status = ReturnStatus;
                        }
                    }
                    ...
                    // Since we have a static instance name we can target directly at the device that has our instance name
                    if (Irp != NULL) {
                        Status = WmipForwardWmiIrp(Irp, MinorFunction, Wnode->ProviderId, &Wnode->Guid, OutBufferSize, Wnode);
                        *RetSize = (ULONG)Irp->IoStatus.Information;
                    } else {
                        Status = WmipSendWmiIrp(MinorFunction, Wnode->ProviderId, &Wnode->Guid, OutBufferSize, Wnode, &Iosb);
                        *RetSize = (ULONG)Iosb.Information;
                    }
                }
            }
...
)
{
    ...
    // We are called by kernel mode and passed an object list and InstanceNames or we are called by user mode and passed a QsiMultiple instead
    WmipAssert(((AccessMode == KernelMode) && (QsiMultiple == NULL) && (ObjectList != NULL) && (InstanceNames != NULL)) ||
        ((AccessMode == UserMode) && (QsiMultiple != NULL) && (ObjectList == NULL) && (InstanceNames == NULL)));

    Status = STATUS_SUCCESS;
    if (ObjectList == NULL) {
        // if this is a user call then we need to copy out the QSIMULTIPLE information since it is in the system buffer and will get overwritten on the first query
    ...
    if (NT_SUCCESS(Status)) {
        ...
        for (i = 0; i < QueryCount; i++) {
            ...
            WnodeSizeNeeded = (FIELD_OFFSET(WNODE_SINGLE_INSTANCE, VariableData) + UString.Length + sizeof(USHORT) + 7) & ~7;
            if ((BufferFull) || (BufferSize < WnodeSizeNeeded)) {
                // If there is no more room, we are just querying for the size that will be needed.
                if (WnodeSizeNeeded > WnodeQSISize) {
                    // Our temporary buffer is too small so lets alloc a larger one
                    ...
                    if (WnodeQSIPtr == NULL) {
                        // We couldn't allocate a larger temporary buffer so we abort this call and try to exit gracefully
                        Status = STATUS_INSUFFICIENT_RESOURCES;
                        break;
                    }
                ...
                // Plenty of room so build wnode directly into the output buffer
            ...
            *InstanceName++ = UString.Length;
            ...
            Status2 = WmipQuerySetExecuteSI(Object, Irp, AccessMode, IRP_MN_QUERY_SINGLE_INSTANCE, (PWNODE_HEADER)Wnode, WnodeSize, &RetSize);
            if (NT_SUCCESS(Status2)) {
                if (Wnode->WnodeHeader.Flags & WNODE_FLAG_INTERNAL) {
                    // Skip any internal guid quesries
                } else if (Wnode->WnodeHeader.Flags & WNODE_FLAG_TOO_SMALL) {
                    ...
                    BufferFull = TRUE;
                    BufferOverFlow = TRUE;
                    WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_API_INFO_LEVEL, "WMI: QSIM %ws too small %x SizeNeeded %x\n",
                                      UString.Buffer, (WnodeTooSmall->SizeNeeded + 7) & ~7, SizeNeeded));
                } else if (BufferFull) {
                    // There was enough room, but the buffer was already filled so we just tally up the size needed
                    SizeNeeded += (RetSize + 7) & ~7;
                    BufferOverFlow = TRUE;
                    WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_API_INFO_LEVEL, "WMI: QSIM %ws big enough but full  %x SizeNeeded %x\n",
                                      UString.Buffer, (RetSize + 7) & ~7, SizeNeeded));
                } else {
                    // We successfully got data. Link the previous wnode to this one
                    if (WnodePrev != NULL) {
                        WnodePrev->Linkage = Linkage;
                    ...
                    WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_API_INFO_LEVEL, "WMI: QSIM %ws big enough %x SizeNeeded %x\n", UString.Buffer, SkipSize, SizeNeeded));
...
/*
Routine Description:
    This routine fills GuidInfo with the properties for the Guid represented by the GuidEntry.
    Note that this call is made holding the SMCritSection.
*/
{
    PLIST_ENTRY InstanceSetList;
    PBINSTANCESET InstanceSet;
    ...
    while (InstanceSetList != &GuidEntry->ISHead) {
        ...
        if (((InstanceSet->Flags & IS_ENABLE_EVENT) || (InstanceSet->Flags & IS_ENABLE_COLLECTION)) || (InstanceSet->Flags & IS_COLLECTING)) {
    ...
    if (GuidEntry->Flags & GE_NOTIFICATION_TRACE_FLAG) {
        if (GuidInfo->GuidType == WMI_GUIDTYPE_TRACECONTROL) {
            // If a NotificationEntry is found for a TraceControlGuid it means that it is enabled.
...
NTSTATUS WmipQueryGuidInfo(IN OUT PWMIQUERYGUIDINFO QueryGuidInfo)
{
    ...
    if (NT_SUCCESS(Status)) {
        GuidEntry = GuidObject->GuidEntry;
        if (GuidEntry != NULL) {
            // Assume that the guid is not expensive and then loop over all instances to see if one of them is expensive.
            ...
            while (InstanceSetList != &GuidEntry->ISHead) {
                ...
                if (InstanceSet->Flags & IS_EXPENSIVE) {
                    // The guid is expensive so remember that and break out of loop
            ...
            // The guid object exists, but there is not a corresponding guidentry which is an error.
            Status = STATUS_WMI_GUID_DISCONNECTED;
        }
...
// The head of the list that contains the guid objects associated with an irp is in the DriverContext  part of the irp
...
void WmipClearIrpObjectList(PIRP Irp)
{
    ...
    // Loop over all objects associated with this irp and reset the value for its associated irp since this irp is now going away
...
void WmipClearObjectFromThreadList(PWMIGUIDOBJECT Object)
{
    ...
    if (IsListEmpty(ThreadList)) {
        // if this is the last object on the thread list then we need to close the handle (in the system handle table) to the user mode process
...
)
{
    ...
    // 2. Once we find the earliest event we look ahead in that same event queue buffer assuming that it will be earlier than events in other buffers.
    //    This makes sense when only one queue has events left in it.

    PAGED_CODE();

    // This routine assumes that the output buffer has been checked and that it is large enough to accommodate all of the events.
    // This implies that this function is called while holding the critical section.
    ...
    for (i = 0; i < HandleCount; i++) {
        ...
        if (IsHiPriority) {
            if ((GuidObject->HiPriority.Buffer != NULL) && (GuidObject->HiPriority.NextOffset != 0)) {
                ...
                WmipSetHighWord(ObjectArray[i].NextWnode->Version, GuidObject->HiPriority.EventsLost);
            ...
            if ((GuidObject->LoPriority.Buffer != 0) && (GuidObject->LoPriority.NextOffset != 0)) {
                ...
                WmipSetHighWord(ObjectArray[i].NextWnode->Version, GuidObject->LoPriority.EventsLost);
    ...
    // loop until all events in all guid objects have been processed
    SizeUsed = 0;
    Earliest = 0;
    OutWnode = NULL;
    while (Earliest != 0xffffffff) {
        ...
        for (i = 0; i < HandleCount; i++) {
            ...
            if ((InWnode != NULL) && (InWnode->TimeStamp.QuadPart < Timestamp.QuadPart)) {
                // We found an event that is earlier than any previous one so we remember the new candidate for earliest event and also the previous early event
        ...
        if (Earliest != 0xffffffff) {
            // We found the earliest event so copy it into the output buffer
...
void WmipCompleteGuidIrpWithError(PWMIGUIDOBJECT GuidObject)
{
    ...
    // If this object is already being waited on by a different irp then we need to fail the original irp since we only allow a single irp to wait on a specific object
    ...
    if (IoSetCancelRoutine(OldIrp, NULL)) {
        // If there was a cancel routine then this means that the irp is still pending so we can go and complete it
...
NTSTATUS WmipMarkHandleAsClosed(HANDLE Handle)
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (GuidObject->Irp != NULL) {
            // If this object was is waiting in a pending irp then we need to complete the irp to keep the pump moving
...
#define MANY_NOTIFICATION_OBJECTS 16
    ...
    // First check that we all handles are entitled to receive notifications and that the object is not already associated with an irp.
    ...
    ObjectCount = 0;
    for (i = 0; (i < HandleCount); i++) {
        Status = ObReferenceObjectByHandle(HandleArray[i].Handle, WMIGUID_NOTIFICATION, WmipGuidObjectType, UserMode, &GuidObject, NULL);
        if (!NT_SUCCESS(Status)) {
            // If one handle is bad then it spoils the whole request

            // Now try with Trace flags and if succeeds, We need to make sure the object is a trace request object.
            Status = ObReferenceObjectByHandle(HandleArray[i].Handle, TRACELOG_REGISTER_GUIDS, WmipGuidObjectType, UserMode, &GuidObject, NULL);
            if (!NT_SUCCESS(Status)) {
                goto Cleanup;
            }
        ...
        for (j = 0; j < ObjectCount; j++) {
            if (GuidObject == ObjectArray[j].GuidObject) {
                WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_ERROR_LEVEL, "WMI: Duplicate object %p passed to WmiReceiveNotifciations\n", GuidObject));
                ObDereferenceObject(GuidObject);
        ...
        if (!DuplicateObject) {
            // See if there was an irp attached to the guid object already.
            // We'll need to cancel it if all guid objects are valid
            if (GuidObject->Irp != NULL) {
                ReplacingIrp = 1;
            }
    ...
    // If any of the guid objects already had an irp attached then we need to complete that irp with an error and move on
    if (ReplacingIrp == 1) {
    ...
    if ((IsHighPriorityEvent | IsLoPriorityEvent) != 0) {
        if (SizeNeeded <= MaxBufferSize) {
            // There are events waiting to be received so pull them all out, high priority ones first then low priority ones.            // events will show up first.
            ...
            // We need to set the linkage field for the last wnode in the list to 0 so it can mark the end of the list correctly
            if (LastWnode != NULL) {
                LastWnode->Linkage = 0;
            }

            // Compute the number of bytes used to fill the output buffer by subtracting the size left from the size passed in
            *OutBufferSize = MaxBufferSize - SizeLeft;
        } else {
            // Not enough room to return all of the event data so we return/ a WNODE_TOO_SMALL to indicate the size needed
        ...
        // There are no events waiting to be returned so we need to create our wait structures, pend the irp and return pending
        if (ReceiveNotification->Action == RECEIVE_ACTION_NONE) {
            ...
            // Pump has called us to tell us that it is shutting down so we need to establish a list linking the guid objects and stashing away the callback address
#if defined(_WIN64)
            // For native Win64 processes, ensure that the thread start address is aligned properly
            Wow64Process = _PsGetCurrentProcess()->Wow64Process;
            if ((Wow64Process == NULL) && (((ULONG_PTR)ReceiveNotification->UserModeCallback.Handle64 & 0x7) != 0)) {
                Status = STATUS_INVALID_PARAMETER;
                goto Cleanup;
            }
#endif

            // Make sure that the process handle we get is valid and has enough permissions to create the thread
            Status = ObReferenceObjectByHandle(ReceiveNotification->UserModeProcess.Handle,
                                               ...
                                               NULL);
            if (NT_SUCCESS(Status)) {
                // Create a handle for the process that lives in the system handle table so that it will be available in any thread context.
                // Note that one handle is created for each thread object list and the handle is closed when the last object is removed from the list
                Status = ObOpenObjectByPointer(UserProcessObject, OBJ_KERNEL_HANDLE, NULL, THREAD_ALL_ACCESS, NULL, KernelMode, &UserModeProcess);
                if (NT_SUCCESS(Status)) {
                    // Get the default stack size and commit for this process and store it away in the guid object so that the pump threads created from kernel will have appropriately sized stacks
                    try {
...
Cleanup:
    // Remove any object references that we took and free memory for the object array
    WmipLeaveSMCritSection();
...
NTSTATUS WmipCsrClientMessageServer(IN PVOID CsrPort, IN OUT PCSR_API_MSG m, IN CSR_API_NUMBER ApiNumber, IN ULONG ArgLength)
/*
Routine Description:
    This function sends an API datagram to the Windows Emulation Subsystem Server.
Arguments:
    CsrPort - pointer to LPC port object that is connected to CSR on behalf of this process
    m - Pointer to the API request message to send.
    ApiNumber - Small integer that is the number of the API being called.
    ArgLength - Length, in bytes, of the argument portion located at the end of the request message.  Used to calculate the length of the request message.
Return Value:
...
*/
{
    ...
    if (!NT_SUCCESS(Status))// Check for failed status and do something.
    {
    ...
    return(m->ReturnValue);// The value of this function is whatever the server function returned.
}
...
VOID WmipPumpThreadApc(IN PKAPC Apc, IN PKNORMAL_ROUTINE *NormalRoutine, IN PVOID *NormalContext, IN PVOID *SystemArgument1, IN PVOID *SystemArgument2)
/*
Routine Description:
...
*/
{
    ...
    ExFreePool(Apc);// Free memory used by APC

    // Get the ExceptionPort from the process object.
    // In a Win32 process this port is set by CSR to allow it to be notified when an exception occurs.
    // This code will also use it to register this thread with CSR.
    // Note that if the exception port is NULL then the process is not a Win32 process and it doesn't matter if the thread doesn't get registered.
    ...
    if (Process->ExceptionPort != NULL) {
        ...
        WmipCsrClientMessageServer(Process->ExceptionPort, (PCSR_API_MSG)&m, CSR_MAKE_API_NUMBER(BASESRV_SERVERDLL_INDEX, BasepRegisterThread), sizeof(*a));
    } else {
...
NTSTATUS WmipCreatePumpThread(PWMIGUIDOBJECT Object)
{
    ...
    // First off we need to create the pump thread suspended so we'll have a chance to queue a kernel apc before the thread starts running
    WmipEnterSMCritSection();
    if (Object->UserModeProcess != NULL) {
        Status = RtlCreateUserThread(Object->UserModeProcess,
                                     ...
                                     NULL);
        if (NT_SUCCESS(Status)) {
            // Queue a kernel mode apc that will call into CSR to register this newly created thread.
            // Note that if the APC cannot be run it is not fatal as we can allow the thread to run without being registered with CSR.
            // The APC is freed at the end of the APC routine
            Status = ObReferenceObjectByHandle(ThreadHandle, 0, NULL, KernelMode, &ThreadObj, NULL);
            if (NT_SUCCESS(Status)) {
                Apc = WmipAllocNP(sizeof(KAPC));
                if (Apc != NULL) {
                    KeInitializeApc(Apc, ThreadObj, OriginalApcEnvironment, WmipPumpThreadApc, NULL, NULL, KernelMode, NULL);
                ...
                Status = STATUS_SUCCESS;// Status is still successful since the pump thread was created, just not registered with CSR
            }

            // If we successfully created the pump thread then mark all of the related objects as not needing any thread creation anymore
...
/*
Routine Description:
    This routine is a worker routine that will create a user mode pump thread so that events can be delivered.
Arguments:
    Context is a pointer to a CREATETHREADWORKITEM struct. It is freed in this routine
*/
{
    ...
    if (ObReferenceObjectSafe(WorkItem->Object)) {
        ...
        if (!NT_SUCCESS(Status)) {
            WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_ERROR_LEVEL, "WMI: Delayed pump thread creation failed %x\n", Status));
        }
...
NTSTATUS WmipQueueNotification(PWMIGUIDOBJECT Object, PWMIEVENTQUEUE EventQueue, PWNODE_HEADER Wnode)
{
    ...
    // If there is not a buffer allocated to store the event then allocate one
    if (EventQueue->Buffer == NULL) {
        // If we get an event that is larger than the default max buffer size then we bump the buffer size up to 64K, 
        // unless it is larger than 64K where we bump up to the actual size of the event.
    ...
    if (NextOffset <= EventQueue->MaxBufferSize) {
        // Link the previous wnode to this one, copy in the new wnode and update the pointer to next free space
        ...
        // If consumer requested that we autostart a thread then we do that now
        if (Object->EventQueueAction == RECEIVE_ACTION_CREATE_THREAD) {
            if (KeIsAttachedProcess()) {
                // If the current thread is attached to a process then it is not safe to create a thread.
                // So we queue a work item and let the work item create it
                WorkItem = ExAllocatePoolWithTag(NonPagedPool, sizeof(CREATETHREADWORKITEM), WMIPCREATETHREADTAG);
                if (WorkItem != NULL) {
                    // Take reference on object. Reference released in worker routine
                    Status = ObReferenceObjectByPointer(Object, 0, NULL, KernelMode);
                    if (NT_SUCCESS(Status)) {
...
PWNODE_HEADER WmipDereferenceEvent(PWNODE_HEADER Wnode)
{
    ...
    WnodeTargetSize = WnodeRef->TargetDataBlockSize + FIELD_OFFSET(WNODE_SINGLE_INSTANCE, VariableData) + InstanceNameLen + 8;
    Retries = 0;
    do {
        ...
        if (WnodeTarget != NULL) {
            ...
            if (IsStaticInstanceNames != 0) {
                ...
                *Ptr++ = (USHORT)InstanceNameLen2;
                ...
                WnodeTarget->DataBlockOffset = ((FIELD_OFFSET(WNODE_SINGLE_INSTANCE, VariableData) + InstanceNameLen2 + sizeof(USHORT) + 7) & 0xfffffff8);
            }
            Status = WmipDeliverWnodeToDS(IRP_MN_QUERY_SINGLE_INSTANCE, DataSource, (PWNODE_HEADER)WnodeTarget, WnodeTargetSize);
    ...
    if (!NT_SUCCESS(Status)) {
        WmipReportEventLog(EVENT_WMI_CANT_GET_EVENT_DATA, EVENTLOG_WARNING_TYPE, 0, Wnode->BufferSize, Wnode, 0, NULL);
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_ERROR_LEVEL, "WMI: Query to dereference WNODE failed %d\n", Status));
        if (WnodeTarget != NULL) {
            WmipFree(WnodeTarget);
            WnodeTarget = NULL;
        }
    } else {
        WnodeTarget->WnodeHeader.Flags |= (WnodeRef->WnodeHeader.Flags &  WNODE_FLAG_SEVERITY_MASK) | WNODE_FLAG_EVENT_ITEM;
    }
...
PWNODE_HEADER WmipIncludeStaticNames(PWNODE_HEADER Wnode)
{
    ...
    if (GuidEntry != NULL) {
        // Loop over all instance sets to find the one that corresponds to our provider id
    ...
    if (TargetInstanceSet != NULL) {
        if ((TargetInstanceSet->Flags & (IS_INSTANCE_BASENAME | IS_INSTANCE_STATICNAMES)) != 0) {
            if (Wnode->Flags & WNODE_FLAG_ALL_DATA) {
                ...
                // Allocate a new buffer to hold all of the original wnode plus the instance names.
                // We need to add space for padding the wnode to 4 bytes plus space for the array of offsets to instance names plus space for the instance names
                ...
                if (WnodeFull != NULL) {
                    ...
                    WmipInsertStaticNames(WnodeAllData, (ULONG)WnodeFullSize, TargetInstanceSet);
                ...
                if (InstanceIndex < TargetInstanceSet->Count) {
                    if (TargetInstanceSet->Flags & IS_INSTANCE_STATICNAMES) {
                        ...
                        InstanceNameLen = (wcslen(InstanceName) + 2 + MAXBASENAMESUFFIXLENGTH) * sizeof(WCHAR);
                    }

                    // Allocate a new Wnode and fill in the instance name.
                    // Include space for padding the wnode to a 2 byte boundary and space for the instance name
                    ...
                    if (WnodeFull != NULL) {
                        ...
                        if (TargetInstanceSet->Flags & IS_INSTANCE_STATICNAMES) {
                            ...
                            *TargetInstanceName++ = (USHORT)InstanceNameLen;
                            StringCbCopy(TargetInstanceName, InstanceNameLen, InstanceName);
                        } else {
                            if (TargetInstanceSet->Flags & IS_PDO_INSTANCENAME) {
                                WnodeFull->Flags |= WNODE_FLAG_PDO_INSTANCE_NAMES;
                            }
                            StringCbPrintf(Index, sizeof(Index), BASENAMEFORMATSTRING, TargetInstanceSet->IsBaseName->BaseIndex + InstanceIndex);
                            StringCbCopy(TargetInstanceName + 1, InstanceNameLen, InstanceName);
                            StringCbCat(TargetInstanceName + 1, InstanceNameLen, Index);
    ...
    if (IsError) {
        // If we had an error resolving the instance name then report it and remove the instance name from the event.
        WmipReportEventLog(EVENT_WMI_CANT_RESOLVE_INSTANCE, EVENTLOG_WARNING_TYPE, 0, Wnode->BufferSize, Wnode, 0, NULL);
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_WARNING_LEVEL, "WMI: Static instance name in event, but error processing\n"));
        if (Wnode->Flags & WNODE_FLAG_ALL_DATA) {
...
/*
Routine Description:
    This routine will write a WNODE into the queue of events to be returned for a guid object.
    If there is an irp already waiting then it will be satisfied with the event otherwise it will be queued in the objects buffer.
    This routine assumes that the SM Critical section is held
Arguments:
    Object is the object to which to send the request
    Wnode is the Wnode with the event
    IsHighPriority is TRUE if the event should go into the high priority queue
Return Value:
...
*/
{
    ...
    // Someone has registered to receive this event so see if there is an irp waiting to be completed or if we should just queue it
    Irp = Object->Irp;
    if ((Irp != NULL) && (IoSetCancelRoutine(Irp, NULL))) {
        // There is an irp waiting for this event, copy out the event and complete the irp
        ...
        if (WnodeSize > OutBufferSize) {
            // There is not enough room to return the event so we return a WNODE_TOO_SMALL with the size needed and then go and queue the event
            ...
            // Plenty of room, copy the event into the irp buffer and complete the irp
            WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_EVENT_INFO_LEVEL, "WMI: Returning event to waiting irp for object %p\n", Object));
            RtlCopyMemory(OutBuffer, Wnode, WnodeSize);
            Status = STATUS_SUCCESS;
        }

        // Remove link from all objects associated with the irp since now the irp is going away.
        WmipClearIrpObjectList(Irp);
        Irp->IoStatus.Information = WnodeSize;
        Irp->IoStatus.Status = STATUS_SUCCESS;
        IoCompleteRequest(Irp, IO_NO_INCREMENT);
    } else {
        // There is no irp waiting to receive the event so we need to queue it if we can
...
NTSTATUS WmipProcessEvent(PWNODE_HEADER InWnode, BOOLEAN IsHighPriority, BOOLEAN FreeBuffer)
{
    ...
    // If the event references a guid that needs to be queried then go do the dereferencing here.
    ...
    // Be sure to use the guid of the referenced event, not the event that was originally fired.
    ...
    if (IsEqualGUID(EventGuid, &TraceErrorGuid)) {
        ...
        if (InWnode->BufferSize >= sizeof(WMI_TRACE_EVENT)) {
            // Logger thread terminating will result in DisableTrace through StopTrace. No need to call twice.
    ...
    if (GuidEntry != NULL) {
        // Yup, so check if there are any open objects to the guid and if anyone is interested in receiving events from them
        ...
        while (ObjectList != &GuidEntry->ObjectHead) {
            ...
            // ObRefSafe so that we can be sure that the object is not in the process of being deleted.
            // If this function returns FALSE then the object is being deleted and so we don't want to use it.
            // If TRUE then it is safe to use the object
            ObjectListNext = ObjectList->Flink;
            if (ObReferenceObjectSafe(Object)) {
                // Make sure the object has not been marked as one that should not receive any events since it is transitioning to a closed state
                if ((Object->Flags & WMIGUID_FLAG_RECEIVE_NO_EVENTS) == 0) {
                    if (Object->Flags & WMIGUID_FLAG_KERNEL_NOTIFICATION) {
                        ...
                        if (!NT_SUCCESS(Status)) {
                            // If any attempts to queue the event fail then we return an error
    ...
    if (FreeBuffer) {
        ExFreePool(InWnode);// Free buffer passed by driver containing event
    }

    if ((Wnode != InWnode) && (Wnode != WnodeTarget)) {
        WmipFree(Wnode);// If we inserted static names then free it
    }

    if (WnodeTarget != NULL) {
        WmipFree(WnodeTarget);// if we dereferenced then free it
    }
...
/*
Routine Description:
    This routine will register a set of user mode guids with WMI for use by tracelog.
    The following steps will occur:
        * A request object is created using the passed object attributes.
          Although the object created is unnamed, the object name passed is used to lookup a security descriptor to associate with the object.
        * The guids are registered in the system.
Arguments:
    ObjectAttribtes is a pointer to the passed object attributes used to create the request object
    Cookie is a unique id to associate with the request object so that when a request is delivered the UM code can understand the context via the cookie.
    RegInfo is the registration information passed
    RegInfoSize is the number of bytes of registration information passed
    *RequestHandle returns with a handle to the request object.
                   UM logger creation and tracelog enabled/disable requests are delivered to the object as WMI events.
...
*/
{
    ...
    Status = WmipProbeAndCaptureGuidObjectAttributes(&CapturedObjectAttributes, &CapturedGuidString, CapturedGuidBuffer, ObjectAttributes);
    if (NT_SUCCESS(Status)) {
        ...
        RegEntry = WmipAllocRegEntry(Callback,
                                     WMIREG_FLAG_CALLBACK | REGENTRY_FLAG_TRACED | REGENTRY_FLAG_NEWREGINFO | REGENTRY_FLAG_INUSE | REGENTRY_FLAG_REG_IN_PROGRESS);
        WmipLeaveSMCritSection();
        if (RegEntry != NULL) {
            // Build a request object for this data source so that any enable requests can be posted to it while processing the WmiRegInfo
            Status = WmipOpenGuidObject(&CapturedObjectAttributes, TRACELOG_REGISTER_GUIDS, UserMode, RequestHandle, &RequestObject);
            if (NT_SUCCESS(Status)) {
                ...
                if (NT_SUCCESS(Status)) {
                    ...
                    // Find out if this Guid is currently Enabled. If so find its LoggerContext
                    *LoggerContext = 0;
...
NTSTATUS WmipUnregisterGuids(PWMIUNREGGUIDS UnregGuids)
{
    ...
    // Check to see if this GUID got disabled in the middle of Unregister Call. If so, send the LoggerContext back
...
/*
Routine Description:
    This routine will build a WNODE out of a message and then write it into the Request object.
    If a reply object is specified then the reply object is linked into the request object so when the reply is written to the request object it can be routed to the reply object correctly,.
    This routine assumes that the SM Critical section is held
Arguments:
    RequestObject is the object to which to send the request
    ReplyObject is the object to which the request object should reply. This may be NULL in the case that no reply is needed.
...
*/
{
    ...
    if (Wnode != NULL) {
        ...
        // if this request requires a reply then update the lists for the request and reply objects
        if (ReplyObject != NULL) {
            // Find a free spot in the request object to link in the reply.
            Status = STATUS_INSUFFICIENT_RESOURCES;
            for (i = 0; i < MAXREQREPLYSLOTS; i++) {
                MBRequest = &RequestObject->MBRequests[i];
                if (MBRequest->ReplyObject == NULL) {
                    // We have a free slot so link request and reply objects together and send off the request.
                    // The request object takes a ref count on the reply object since it maintains a pointer to it.
                    // The refcount is released when the request object writes the reply back to the reply object.
                    Status = ObReferenceObjectByPointer(ReplyObject, 0, WmipGuidObjectType, KernelMode);
                    if (NT_SUCCESS(Status)) {
            ...
            // No reply required so we just write the message to the object and continue with our business
            Status = WmipWriteWnodeToObject(RequestObject, Wnode, TRUE);
        }
...
NTSTATUS WmipWriteMessageToGuid(IN PBGUIDENTRY GuidEntry,
    ...
    /*
    Routine Description:
        This routine will loop over all instance sets attached to a guid entry and if the data source for it is a user mode data source then it will get a request message sent to it.
        Note that if there are more than one providers to which a message is sent, then success is returned as long as writing to one of them is successful.
    Arguments:
    ...
    */
{
    ...
    *WrittenCount = 0;
    ...
    // Loop over all instances and send create logger request to all user mode data providers
    InstanceSetList = GuidEntry->ISHead.Flink;
    while (InstanceSetList != &GuidEntry->ISHead) {
        InstanceSet = CONTAINING_RECORD(InstanceSetList, INSTANCESET, GuidISList);
...
/*
Routine Description:
    This routine will send a request to create a UM logger.
    First it will find the providers associated with the control guid and then create a reply object which the providers will reply to when the UM logger is created.
    Note that the reply object is created as an unnamed object,
    but that the guid passed in the object name is used to look up the security descriptor for the reply object.
...
*/
{
    ...
    Status = WmipProbeAndCaptureGuidObjectAttributes(&CapturedObjectAttributes, &CapturedGuidString, CapturedGuidBuffer, CreateInfo->ObjectAttributes);
    if (NT_SUCCESS(Status)) {
        GuidEntry = WmipFindGEByGuid(&CreateInfo->ControlGuid, FALSE);
        if (GuidEntry != NULL) {
            // Control guid is registered so create a reply object that the provider will write to.
            if (WmipIsControlGuid(GuidEntry)) {
                // Create the reply object
                Status = WmipOpenGuidObject(&CapturedObjectAttributes,
                                            TRACELOG_CREATE_INPROC | TRACELOG_GUID_ENABLE | WMIGUID_NOTIFICATION,
                                            UserMode,
                                            &ReplyHandle,
                                            &ReplyObject);
                if (NT_SUCCESS(Status)) {
                    // Send request to all providers who registered for control guid
                    ...
                    if (NT_SUCCESS(Status)) {
                        // Create logger requests delivered ok so return handle to object that will receive the replies.
                        CreateInfo->ReplyHandle.Handle = ReplyHandle;
                        CreateInfo->ReplyCount = ReplyCount;
                    } else {
                        // We were not able to deliver the requests so we do not need to keep the reply object open
                        ZwClose(ReplyHandle);
                    }

                    ObDereferenceObject(ReplyObject);// remove the ref taken when the object was created
                }
            }

            WmipUnreferenceGE(GuidEntry);
        } else {
            Status = STATUS_WMI_INSTANCE_NOT_FOUND;// Control guid is not registered so return an error
        }
    }
...
/*
Routine Description:
    This routine will write a MB reply message to the appropriate reply object and unlink the reply object from the request object;
Arguments:
...
*/
{
    ...
    Status = ObReferenceObjectByHandle(RequestHandle, TRACELOG_REGISTER_GUIDS, WmipGuidObjectType, UserMode, &RequestObject, NULL);
    if (NT_SUCCESS(Status)) {
        if (ReplyIndex < MAXREQREPLYSLOTS) {
            ...
            if (ReplyObject != NULL) {
                // We have figured out who we need to reply to so clear out the link between the reply object and this request object
                RemoveEntryList(&MBRequest->RequestListEntry);
                MBRequest->ReplyObject = NULL;
                ObDereferenceObject(ReplyObject);
                Status = WmipWriteMBToObject(ReplyObject, NULL, Message, MessageSize);
                if (!NT_SUCCESS(Status)) {
                    WmipDebugPrintEx((DPFLTR_WMICORE_ID,
                                      DPFLTR_ERROR_LEVEL,
