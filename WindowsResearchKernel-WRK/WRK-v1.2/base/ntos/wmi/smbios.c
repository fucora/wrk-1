...
NTSTATUS WmipSMBiosDataRegQueryRoutine(IN PWSTR ValueName,
                                       IN ULONG ValueType,
                                       IN PVOID ValueData,
...
NTSTATUS WmipSMBiosIdentifierRegQueryRoutine(IN PWSTR ValueName,
                                             IN ULONG ValueType,
                                             IN PVOID ValueData,
                                             IN ULONG ValueLength,
                                             IN PVOID Context,
...
NTSTATUS WmipParseSysIdTable(PHYSICAL_ADDRESS PhysicalAddress,
                             ULONG NumberEntries,
...
*/
{
    ...
    // Scan the bios for the two anchor strings that that signal the SMBIOS table.
    ...
    while (SMBiosVirtualAddress < SearchEnd) {
        ...
        if (CheckSum == 0) {
            ...
            if (CheckLength == sizeof(DMIBIOS_EPS_HEADER)) {// We only had got a DMI header so copy that
                RtlCopyMemory(&EPSHeader->Signature2[0], DMIBiosEPSHeader, sizeof(DMIBIOS_EPS_HEADER));
            } else {// We got the full SMBIOS header so copy that
...
// On X86 we look at the hardware device description keys to find the one that contains the SMBIOS data.
// The key is created by NTDETECT in the case that the machine only supports the 2.0 calling mechanism
...
NTSTATUS WmipSMBiosIdentifierRegQueryRoutine(IN PWSTR ValueName,
    ...
    Arguments:
        ...
        Context - Not used
        EntryContext - Pointer to PUCHAR to store a pointer to store the SMBIOS data read from the registry value.
            If this is NULL then the caller is not interested in the SMBIOS data
    Return Value:
        NT Status code -
    ...
    */
{
    ...
    if ((ValueType == REG_FULL_RESOURCE_DESCRIPTOR) && (ValueData != NULL)) {
        ...
        if (PartialResourceList->Count > 1) {
            ...
            // may have 0 bytes in the case that SMBIOS data was not collected by NTDETECT.
            PartialDescriptor = &PartialResourceList->PartialDescriptors[0];
            Buffer = (PUCHAR)PartialDescriptor + sizeof(CM_PARTIAL_RESOURCE_DESCRIPTOR) + PartialDescriptor->u.DeviceSpecificData.DataSize;
...
BOOLEAN WmipIsSMBiosKey(HANDLE ParentKeyHandle, PWCHAR KeyName, PUCHAR *SMBiosTableVirtualAddress, PULONG SMBiosTableLength)
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        Status = RtlQueryRegistryValues(RTL_REGISTRY_HANDLE | RTL_REGISTRY_ABSOLUTE, (PWCHAR)KeyHandle, QueryTable, NULL, NULL);
        if (NT_SUCCESS(Status)) {
...
BOOLEAN WmipFindSMBiosTable(PPHYSICAL_ADDRESS SMBiosTablePhysicalAddress,
    ...
    Arguments:
        SMBiosTablePhysicalAddress points to a variable to return the physical address of the SMBIOS 2.1 table.
            If table is not available then it returns with 0.
        SMBiosTableVirtualAddress points to a variable to return the virtual address of the SMBIOS 2.0 table as collected by NTDETECT.
            If the SMBIOS 2.0 data was not collected by NTDETECT it returns with 0.
    ...
    */
{
    ...
    *SMBiosTableLength = 0;
    ...
    if (NT_SUCCESS(Status)) {
        ...
        while (NT_SUCCESS(Status)) {
            ...
            if (NT_SUCCESS(Status)) {
                ...
                if (WmipIsSMBiosKey(KeyHandle, KeyBasic.Info.Name, SMBiosTableVirtualAddress, SMBiosTableLength)) {
    ...
    if (SearchForHeader) {
        ...
        BiosVirtualAddress = MmMapIoSpace(BiosPhysicalAddress, SMBIOS_EPS_SEARCH_SIZE, MmNonCached);
    ...
    if (HaveEPSHeader) {
        ...
        if (DMIBiosEPSHeader->StructureTableLength) {
            ...
            *SMBiosTableLength = DMIBiosEPSHeader->StructureTableLength;
            ...
            WmipDebugPrintEx((DPFLTR_WMICORE_ID,
                              DPFLTR_INFO_LEVEL,
                              "WMI: SMBIOS 2.1 data at (%x%x) size %x \n",
...
*/
{
    ...
    if (!WmipSMBiosChecked) {
        ...
        WmipFindSMBiosTable(&WmipSMBiosTablePhysicalAddress, &WmipSMBiosTableVirtualAddress, &WmipSMBiosTableLength, &WmipSMBiosVersionInfo);
    ...
    if (*BufferSize >= WmipSMBiosTableLength) {
        if (WmipSMBiosTableLength == 0) {
            ...
            SMBiosDataVirtualAddress = MmMapIoSpace(WmipSMBiosTablePhysicalAddress, WmipSMBiosTableLength, MmNonCached);
...
#define WmipUnmapSMBiosStructure(Address, Size)     if ((Address) != NULL) MmUnmapIoSpace((Address), (Size));
...
Routine Description:
    ...
    WmipUnmapSNVuisStructure should be called if this function returns successfully.
Arguments:
...
*/
{
    ...
    // Make sure SMBIOS table has been obtained. Note we already hold the critical section
    if (!WmipSMBiosChecked) {
        // See if there is any SMBIOS information and if so register
        Found = WmipFindSMBiosTable(&WmipSMBiosTablePhysicalAddress, &WmipSMBiosTableVirtualAddress, &WmipSMBiosTableLength, &WmipSMBiosVersionInfo);
        WmipSMBiosChecked = TRUE;
    } else {
    ...
    */
{
    ...
    if (VirtualAddress != NULL) {
        ...
        for (i = 0; i < NumberEntries; i++) {
            // Make sure we have not moved beyond the end of the mapped memory.
            if (BytesLeft >= sizeof(SYSID_TABLE_ENTRY)) {
                ...
                } else if ((RtlCompareMemory(&SysId->Type, SYSID_TYPE_1394, 6) == 6) && (Length == sizeof(SYSID_1394_ENTRY))) {
                    SysidType = SYSID_1394_TYPE;
                } else {// unknown type SYSID
                    WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_INFO_LEVEL, "WMI: Unknown SYSID type %c%c%c%c%c%c found at %p\n",
                                      SysId->Type[0], SysId->Type[1], SysId->Type[2], SysId->Type[3], SysId->Type[4], SysId->Type[5], SysId));
                    Status = STATUS_UNSUCCESSFUL;
                    break;
                }
                ...
                if (BytesLeft >= Length) {
                    ...
                    WmipDebugPrintEx((DPFLTR_WMICORE_ID,
                                      DPFLTR_INFO_LEVEL,
                                      "WMI: SYSID Table at %p is larger at %p than expected",
                                      VirtualAddress,
                                      SysId));
                    Status = STATUS_UNSUCCESSFUL;
                    break;
    ...
    } else {// Lets hope that the failure to map is a temporary condition
        Status = STATUS_INSUFFICIENT_RESOURCES;
    }
...
/*
Routine Description:
    This routine will obtain the 1394 and UUID sysids from the bios.
    First we look for a specific memory signature that contains a list of 1394 and UUID sysids.
    If we do not find that we then look at the SMBIOS information structure SYSTEM INFORMATION (type 1) which may have it embedded within it.
...
Arguments:
    ...
    *BufferSize on entry has the size of the buffer that can receive the eventlog data, on return it has the number of bytes used
        by the smbios eventlog data or the number of bytes needed for the smbios eventlog data.
Return Value:
    NT Status code -
...
*/
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (SystemEventlog->Length >= SMBIOS_SYSTEM_EVENTLOG_LENGTH) {
            ...
            if (SystemEventlog->Length != (LogTypeDescLength + FIELD_OFFSET(SMBIOS_SYSTEM_EVENTLOG_STRUCT, LogTypeDescriptor))) {
                // The SMBIOS spec says that the Length of the structure is the length of the base part of the structures plus
                // the length of the type descriptors. Since this is not the case we may have run into a buggy bios
                WmipDebugPrintEx((DPFLTR_WMICORE_ID,
                                  DPFLTR_INFO_LEVEL,
                                  "WMI: SMBIOS System Eventlog structure %p size is %x, but expecting %x\n",
                                  SystemEventlog,
        ...
        if (*BufferSize >= SizeNeeded) {
            ...
            if (LogHeaderDescExists == 1) {
                // if log header descriptors exist (smbios 2.1+) then copy rest of smbios header plus log type descriptors
                ...
                RtlCopyMemory(&EventlogInfo->LogAreaLength,
                              &SystemEventlog->LogAreaLength,
                              FIELD_OFFSET(SMBIOS_EVENTLOG_INFO, LogHeaderFormat) - FIELD_OFFSET(SMBIOS_EVENTLOG_INFO, LogAreaLength));
...
NTSTATUS WmipDockUndockEventCallback(IN PVOID NotificationStructure, IN PVOID Context)
{
    PAGED_CODE();
    ...
    // if SMBIOS data is obtained via the table in the bios, then reset the flag to indicate that we need to rescan for the table.
    // It is possible that a dock or undock could have changed the data.
    // If we obtained the data from ntdetect then there is nothing we can do since we cannot call the bios.
    if (WmipSMBiosTablePhysicalAddress.QuadPart != 0) {
        WmipEnterSMCritSection();
