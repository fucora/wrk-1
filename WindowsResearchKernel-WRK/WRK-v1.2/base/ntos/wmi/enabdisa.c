...
BOOLEAN WmipIsISFlagsSet(PBGUIDENTRY GuidEntry, ULONG Flags);
...
/*
Routine Description:
    This routine determines whether any of the instance sets associated with the GuidEntry has ALL of the flags set
Arguments:
...
NTSTATUS WmipSendEnableDisableRequest(UCHAR ActionCode, PBGUIDENTRY GuidEntry, BOOLEAN IsEvent, BOOLEAN IsTraceLog, ULONG64 LoggerContext)
/*
Routine Description:
    This routine will deliver an event or collection WNODE to all data providers of a guid.
    This routine assumes that it is called with the SM critical section held.
    The routine does not hold the critical section for the duration of the call.
Arguments:
    ActionCode is WMI_ENABLE_EVENTS, WMI_DISABLE_EVENTS, WMI_ENABLE_COLLECTION or WMI_DISABLE_COLLECTION
    GuidEntry is the guid entry for the guid that is being enabled/disable or collected/stop collected
    IsEvent is TRUE then ActionCode is to enable or disable events. If FALSE then ActionCode is to enable or disbale collecton
    IsTraceLog is TRUE then enable is only sent to those guids registered as being a tracelog guid
    LoggerContext is a logger context handle that should be placed in the HistoricalContext field of the WNODE_HEADER if IsTraceLog is TRUE.
Return Value:
...
#define AVGISPERGUID 64
    ...
    // First we make a list of all of the DataSources that need to be called while we have the critical section and take a reference on them so
    // they don't go away after we release them. Note that the DataSource structure will stay, but the actual data provider may in fact go away.
    ...
    while ((InstanceSetList != &GuidEntry->ISHead) && (DSCount < GuidEntry->ISCount)) {
        ...
        // We send requests to those data providers that are not inprocs when it is an event being enabled or it is collection being enabled
        // and they are defined to be expensive (collection needs to be enabled)
        if (((IsTraceLog && (InstanceSet->Flags & IS_TRACED)) ||
            (!IsTraceLog && (!(InstanceSet->Flags & IS_TRACED)) && (IsEvent || (InstanceSet->Flags & IS_EXPENSIVE))))) {
            if ((!IsEnable && (InstanceSet->Flags & IsFlags)) || ((IsEnable && !(InstanceSet->Flags & IsFlags)) || (IsUpdate && IsTraceLog))) {
    ...
    // data providers. Any new data providers who register after we made our list will be enabled by the registration code.
    if (DSCount > 0) {
        ...
        if (IsTraceLog) {
            ...
            // If this GUID is already enabled then this must an update call. So mark it so.
...
void WmipWaitForCollectionEnabled(PBGUIDENTRY GuidEntry)
{
    ...
    // Collection Enable/Disable is in progress so we cannot return just yet. Right now there could be a
    // disable request being processed and if we didn't wait, we might get back to this caller before that disable request
    // got around to realizing that it needs to send and enable request (needed by this thread's caller). So we'd have a
    // situation where a thread though that collection was enabled but in reality it wasn't yet enabled.
...
/*
Routine Description:
    This routine will send an enable collection or notification request to all of the data providers that have registered the guid being enabled.
    This routine will manage any race conditions that might occur when multiple threads are enabling and disabling the notification simultaneously.

    This routine is called while the SM critical section is being held and will increment the appropriate reference count. if the ref count
    transitions from 0 to 1 then the enable request will need to be forwarded to the data providers otherwise the routine is all done and returns.
    Before sending the enable request the routine checks to see if any enable or disable requests are currently in progress and if not then sets
    the in progress flag, releases the critical section and sends the enable request. If there was a request in progress then the routine does not
    send a request, but just returns. When the other thread that was sending the request returns from processing the request it will recheck the
    refcount and notice that it is greater than 0 and then send the enable request.
Arguments:
    GuidEntry is the guid entry that describes the guid being enabled. For a notification it may be NULL.
    NotificationContext is the notification context to use if enabling events
    IsEvent is TRUE if notifications are being enables else FALSE if collection is being enabled
...
*/
{
    ...
    // If the guid is transitioning from a refcount of 0 to 1 and there is not currently a request in progress, then we need to set the
    // request in progress flag, release the critical section and send an enable request. If there is a request in progress we can't
    // do another request. Whenever the current request finishes  it will notice the ref count change and send the enable request on our behalf.
    if ((RefCount == 0) && !(GuidEntry->Flags & InProgressFlag)) {
        // Take an extra ref count so that even if this gets disabled
        // while the enable request is in progress the GuidEntry will stay valid.
    ...
    EnableNotification:
        ...
        if (RefCount == 0) {
            ...
            // was disabled. So leave the in progress flag set and send the disable.
            ...
            RefCount = IsEvent ? GuidEntry->EventRefCount : GuidEntry->CollectRefCount;
            if (RefCount > 0) {
                // We have hit a pathological case. One thread called to enable and while the enable request was being processed
                // another thread called to disable, but was postponed since the enable was in progress. So once the enable
                // completed we realized that the ref count reached 0 and so we need to disable and sent the disable request.
                // But while the disable request was being processed an enable request came in so now we need to enable the notification.
        ...
        // the enable/disable work completed, we close the event handle to release them from their wait.
        if (!IsEvent) {
            WmipReleaseCollectionEnabled(GuidEntry);
        }

        // Get rid of extra ref count we took above. Note that the
        // GuidEntry could be going away here if there was a disable while the enable was in progress.
        WmipUnreferenceGE(GuidEntry);
    } else if (IsTraceLog && (GuidEntry->Flags & GE_NOTIFICATION_TRACE_UPDATE)) {
        // If it's a tracelog and we have a trace Update enable call, ignore the refcount and send it through.
        WmipReferenceGE(GuidEntry);
...
ULONG WmipDoDisableRequest(PBGUIDENTRY GuidEntry, BOOLEAN IsEvent, BOOLEAN IsTraceLog, ULONG64 LoggerContext, ULONG InProgressFlag)
{
    ULONG RefCount;
    ULONG Status;
...
DisableNotification:
    ...
    RefCount = IsEvent ? GuidEntry->EventRefCount : GuidEntry->CollectRefCount;
    if (RefCount > 0) {
        // While we were processing the disable request an enable request arrived. Since the in progress
        // flag was set the enable request was not sent so now we need to do that.
        ...
        RefCount = IsEvent ? GuidEntry->EventRefCount : GuidEntry->CollectRefCount;
        if (RefCount == 0) {
            // While processing the enable request above the notification was disabled and since a request
            // was in progress the disable request was not forwarded. Now it is time to forward the request.
...
/*
Routine Description:
    This routine will send an disable collection or notification request to all of the data providers that have registered the guid being disabled.
    This routine will manage any race conditions that might occur when multiple threads are enabling and disabling the notification simultaneously.

    This routine is called while the SM critical section is being held and will increment the appropriate reference count. if the ref count
    transitions from 1 to 0 then the disable request will need to be forwarded to the data providers otherwise the routine is all done and returns.
    Before sending the disable request the routine checks to see if any enable or disable requests are currently in progress and if not then sets
    the in progress flag, releases the critical section and sends the disable request. If there was a request in progress then the routine does not
    send a request, but just returns. When the other thread that was sending the request returns from processing the request it will recheck the
    refcount and notice that it is  0 and then send the disable request.
Arguments:
    GuidEntry is the Notification entry that describes the guid being enabled.
    GuidEntry is the guid entry that describes the guid being enabled. For a notification it may be NULL.
    NotificationContext is the notification context to use if enabling events
    IsEvent is TRUE if notifications are being enables else FALSE if collection is being enabled
    IsTraceLog is TRUE if enable is for a trace log guid
...
*/
{
    ...
    if (IsEvent) {
        ...
        RefCount = GuidEntry->EventRefCount;
        if (RefCount == 0) {
            // A bad data consumer is disabling his event more than once. Just ignore it
            return(STATUS_SUCCESS);
        }
    ...
    if ((RefCount == 0) && !(GuidEntry->Flags & InProgressFlag)) {
        // Take an extra ref count so that even if this gets disabled while the disable request is in progress the GuidEntry will stay valid.
        GuidEntry->Flags |= InProgressFlag;
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_INFO_LEVEL, "WMI: %p.%p NE %p flags -> %x at %d\n",
                          PsGetCurrentProcessId(), PsGetCurrentThreadId(), GuidEntry, GuidEntry->Flags, __LINE__));
...
NTSTATUS WmipEnableCollectOrEvent(PBGUIDENTRY GuidEntry, ULONG Ioctl, BOOLEAN *RequestSent, ULONG64 LoggerContext)
{
    ...
    *RequestSent = FALSE;
    ...
    {
        // See if the guid requires an enable collection. Loop over all instance sets that are not for tracelog or events.
        DoEnable = FALSE;
        IsTracelog = FALSE;
        IsEvent = FALSE;
        WmipEnterSMCritSection();
        ...
        while (InstanceSetList != &GuidEntry->ISHead) {
            ...
            if (!((InstanceSet->Flags & IS_TRACED) || ((InstanceSet->Flags & IS_EVENT_ONLY) && DoEnable))) {
                // Only those guids not Traced guids, event only guids and unresolved references are not available for queries
    ...
    case IOCTL_WMI_OPEN_GUID_FOR_EVENTS:
    {
        ...
        // then it will get enabled for tracelog as well as for wmi events.
        break;
    }
...
*/
{
    ...
    Status = WmipCheckGuidAccess(Guid, TRACELOG_GUID_ENABLE, EtwpDefaultTraceSecurityDescriptor);
    if (NT_SUCCESS(Status)) {
        ...
        if (IsEqualGUID(&HeapGuid, Guid)) {
            if (IsEnable) {
                ...
                // increment counter. The counter is composed of first two bytes
        ...
        if (GuidEntry == NULL) {
            ...
            if (IsEnable) {
                // If the NtdllTraceGuid is not in list then we do not want to enable it the NtdllTraceGuid will make an entry only to call starttrace
                if (IsEqualGUID(&NtdllTraceGuid, Guid)) {
                    ...
                    if (GuidEntry != NULL) {
                        ...
                        // from creation. When tracelog enables we take a ref count and when it disables we release it
                        GuidEntry->Guid = *Guid;
            ...
            // The control guid is already registered so lets go and enabled or disable it
            if (WmipIsControlGuid(GuidEntry)) {
                if (IsEnable) {
                    GuidEntry->LoggerContext = LoggerContext;
                    if (GuidEntry->Flags & GE_NOTIFICATION_TRACE_FLAG) {
                        // We are trying to enable a trace guid that is already registered
                        GuidEntry->Flags |= GE_NOTIFICATION_TRACE_UPDATE;
                        ...
                        Status = WmipEnableCollectOrEvent(GuidEntry, Ioctl, &RequestSent, LoggerContext);
                        if (!NT_SUCCESS(Status) && (GuidEntry->Flags & GE_NOTIFICATION_TRACE_FLAG)) {
                            // We failed to enable the trace event, and our
                            // flag is still set. Remove the flag and deref the guid entry.

                            // When we call WmipEnableCollectOrEvent above, we actually drop the SM lock for a little bit.
                            // That means it's possible for a disable call to come through at the same time, which would
                    ...
                    if (GuidEntry->Flags & GE_NOTIFICATION_TRACE_FLAG) {
                        // Send the disable collection call and then remove the refcount that was taken when we enabled
                        GuidEntry->Flags &= ~GE_NOTIFICATION_TRACE_FLAG;
                        Status = WmipDisableCollectOrEvent(GuidEntry, Ioctl, LoggerContext);

                        // Whether the Disable request succeeds or not
                        // we will remove the extra refcount since we reset the NOTIFICATION_FLAG
                ...
                // If this GUID is not a control GUID, check to see if there are no instance sets for this GUID. If so,
                // it is getting disabled before any instances registered it. Disable the GUID and clean up the GE.
                if (GuidEntry->Flags & GE_NOTIFICATION_TRACE_FLAG) {
                    GuidEntry->Flags &= ~GE_NOTIFICATION_TRACE_FLAG;
                    GuidEntry->LoggerContext = 0;
                    WmipUnreferenceGE(GuidEntry);
...
// When a Logger is shutdown, all providers logging to this logger are notified to stop logging first.


NTSTATUS WmipDisableTraceProviders(ULONG StopLoggerId)
{
    PBGUIDENTRY GuidEntry;
    ...
    // Find all the providers that are logging to this logger and disable them automatically.
CheckAgain:
    ...
    while (GuidEntryList != WmipGEHeadPtr) {
        ...
        if (GuidEntry->Flags & GE_NOTIFICATION_TRACE_FLAG) {
            ...
            if (LoggerId == StopLoggerId) {
                ...
                // we will take out the extra ref count whether the SendDisableRequest was successful or not.
                GuidEntry->LoggerContext = 0;
                WmipUnreferenceGE(GuidEntry);

                // We need to remove the refcount that was taken when this Guid was Enabled.
                WmipUnreferenceGE(GuidEntry);

                WmipLeaveSMCritSection();

                // We have to jump out and restart the loop because we let go of the critsect during SendDisableRequest call
                goto CheckAgain;
            }
        }
