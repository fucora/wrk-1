...
// All structures that rely upon the chunk allocator must be defined so that their members match that of ENTRYHEADER.
// These include DATASOURCE, GUIDENTRY, INSTANCESET, DCENTRY, NOTIFICATIONENTRY, MOFCLASS, MOFRESOURCE
...
#define WmipReferenceEntry(Entry)     InterlockedIncrement((PLONG)&((PENTRYHEADER)(Entry))->RefCount)
...
// An INSTANCESET contains the information a set of instances that is provided by a single data source.
// An instance set is part of two lists. One list is the set of instance sets for a particular guid.
// The other list is the list of instance sets supported by a data source.

// Instance names for an instance set registered with a base name and count are stored in a ISBASENAME structure.
// This structure is tracked by PDFISBASENAME in wmicore.idl.
typedef struct
{
...
// This defines the maximum number of characters that can be part of a suffix to a basename.
// The current value of 6 will allow up to 999999 instances of a guid with a static base name
#define MAXBASENAMESUFFIXLENGTH  6
#define MAXBASENAMESUFFIXVALUE   999999
#define BASENAMEFORMATSTRING     L"%d"


// Instance names for an instance set registered with a set of static names are kept in a ISSTATICNAMES structure.
// This structure is tracked by PDFISSTATICNAMES defined in wmicore.idl
typedef struct
{
...
typedef struct tagInstanceSet
{
    union
    {
        LIST_ENTRY GuidISList;// Entry in list of instances within a guid
        LIST_ENTRY FreeISList;// Entry in main list of free instances
    };
    PCHUNKHEADER Chunk;            // Chunk in which entry is located
    ULONG Flags;

    ULONG RefCount;// Reference count of number of guids using this instance set
    ULONG Signature;// Signature to identify entry
    LIST_ENTRY DSISList;// Entry in list of instances within a data source
    PBGUIDENTRY GuidEntry;// Back link to guid that this instance set is a member
    struct tagDATASOURCE *DataSource;// Back link to data source that this instance set is a member
    ULONG Count;// Count of instances in instance set
    ULONG WADInstanceNameSize;// Size needed to place all instance names in a WNODE_ALL_DATA
    ULONG ProviderId;// ProviderId for the DS associated with this IS

    // If IS_INSTANCE_BASENAME is set then IsBaseName pointed at instance base name structure.
    // Else if IS_INSTANCE_STATICNAME is set then
...
// Only those Guids that are Unregistered while a logger session is in progress is kept in this list.
// It is also used as a placeholder for InstanceIds.
// Trace Guid Registration calls return a handle to a GUIDMAPENTRY which maintains the map and the Instance Ids.
...
#define IS_TRACED                   0x00001000// Any traced guids are used for trace logging and not querying
#define IS_ENABLE_EVENT             0x00002000// Set when events are enabled for instance set
#define IS_ENABLE_COLLECTION        0x00004000// Set when events are enabled for instance set
#define IS_EVENT_ONLY               0x00008000// Set if guid is used only for firing events and not querying
#define IS_ANSI_INSTANCENAMES       0x00010000// Set if data provider for instance set is expecting ansi instsance names
#define IS_PDO_INSTANCENAME         0x00020000// Set if instance names are originated from a PDO
#define IS_CONTROL_GUID             0x00080000// Set if a Traced Guid is also a Trace Control Guid
...
typedef struct tagGUIDENTRY
{
    union
    {
        LIST_ENTRY MainGEList;// Entry in list of all guids registered with WMI
        LIST_ENTRY FreeGEList;// Entry in list of free guid entry blocks
    };
    PCHUNKHEADER Chunk;            // Chunk in which entry is located
    ULONG Flags;

    ULONG RefCount;// Count of number of data sources using this guid
    ULONG Signature;// Signature to identify entry
    LIST_ENTRY ObjectHead;// Head of list of open objects to this guid
    ULONG ISCount;// Count of InstanceSets headed by this guid
    LIST_ENTRY ISHead;// Head of list of all instances for guid
    GUID Guid;// Guid that represents data block
...
#define GE_FLAG_INTERNAL    0x00000001// When set this guid is an internally defined guid that has no data source attached to it.
#define GE_FLAG_NOTIFICATION_IN_PROGRESS 0x00000002// Set when a notification request is being processed by the data providers
#define GE_FLAG_COLLECTION_IN_PROGRESS 0x00000004// Set when a collection request is being processed by the data providers
#define GE_FLAG_TRACEDISABLE_IN_PROGRESS 0x00000008// Set when a trace disable is being processed by a worker thread
#define GE_FLAG_WAIT_ENABLED 0x00000010// Set when there is a wait in progress for collect/event enable/disable
#define GE_FLAG_COLLECTION_ENABLED 0x00000020// Set when the guid has had an enable collection sent to it
#define GE_FLAG_NOTIFICATIONS_ENABLED 0x00000040// Set when the guid has had an enable notifications sent to it
#define GE_NOTIFICATION_TRACE_FLAG 0x00000080
#define GE_NOTIFICATION_TRACE_UPDATE 0x00000100// Set when an enabled guid receives another enable notification

typedef struct
{
    union
    {
        LIST_ENTRY MainMRList;// Entry in list of all DS
        LIST_ENTRY FreeMRList;// Entry in list of free DS
    };
...
typedef struct tagDATASOURCE
{
    union
    {
        LIST_ENTRY MainDSList;// Entry in list of all DS
        LIST_ENTRY FreeDSList;// Entry in list of free DS
    };
    PCHUNKHEADER Chunk;            // Chunk in which entry is located
    ULONG Flags;
    ULONG RefCount;
    ...
    LIST_ENTRY ISHead;// Head of list of instances for this DS
    ULONG ProviderId;// Provider id of kernel mode driver
    PTCHAR RegistryPath;// Path to registry holding ACLs
    ...
    struct _WMIGUIDOBJECT *RequestObject;// Guid object if this is a UM provider
};
