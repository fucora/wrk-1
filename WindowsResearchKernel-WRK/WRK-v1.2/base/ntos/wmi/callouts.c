...
Abstract:
    This is the source file that contains all the callout routines from the kernel itself.
    The only exception is TraceIo for DiskPerf.
*/
...
VOID WmipTraceVolMgr(IN PIRP ParentIrp, IN PIRP ChildIrp);
VOID WmipTraceFile(IN PVOID TraceFileContext);
VOID WmipTraceLoadImage(IN PUNICODE_STRING ImageName, IN HANDLE ProcessId, IN PIMAGE_INFO ImageInfo);
VOID WmipTraceRegistry(
    IN NTSTATUS         Status,
    IN PVOID            Kcb,
...
/*
Routine Description:
    This is called by WmipStartLogger in tracelog.c.
    Its purpose is to set up all the kernel notification routines that can produce event traces for capacity planning.
Arguments:
...
*/
{
    ...
    DiskFound = 0;// Since we cannot do anything, we will have to count the number of entries we need to create first, and add some buffer
    ...
    if (enableDisk || enableNetwork || enableVolMgr) {
        // Setting the callouts will cause new PDO registration to be enabled from here on.
        if (enableDisk) {
        ...
        // Now, we will go through what's already in the list and enable trace notification routine.
        // Devices who registered while after we've set the callout will get another Irp to enable, but that's alright
        ...
        while (--Index >= 0 && irp != NULL) {
            if (device->DeviceObject != NULL) {
                ...
                } else if ((device->TraceClass == WMIREG_NOTIFY_VOLMGR_IO) && enableVolMgr) {
                ...
                do {
                    ...
                    status = WmipForwardWmiIrp(irp, IRP_MN_SET_TRACE_NOTIFY, IoWMIDeviceObjectToProviderId(device->DeviceObject), NULL, sizeof(notifyRoutine), &notifyRoutine);
    ...
    if (EnableFlags & EVENT_TRACE_FLAG_DISK_FILE_IO) {
        // NOTE: We assume StartLogger will always reserve space for FileTable already
...
/*
Routine Description:
    This is called by WmipStopLogger in tracelog.c.
    Its purpose of the disable all the kernel notification routines that was defined by WmipEnableKernelTrace
Arguments:
...
*/
{
    ...
    // Note. Since this is in the middle is StopLogger, it is not possible StartLogger will prevent kernel tracing from being enabled, hence
    // we need not worry about WmipEnableKernelTrace() being called while this is in progress.
    ...
    // To disable we do not need to worry about TraceClass, since we simply set all callouts to NULL
    WmipEnterSMCritSection();
    RegEntryList = WmipInUseRegEntryHead.Flink;
    while (RegEntryList != &WmipInUseRegEntryHead) {
        RegEntry = CONTAINING_RECORD(RegEntryList, REGENTRY, InUseEntryList);
    ...
    device = (PTRACE_DEVICE)deviceList;        // start from first element
    while (--Index >= 0 && irp != NULL) {
        if ((device->DeviceObject != NULL) && ((device->TraceClass == WMIREG_NOTIFY_TDI_IO) || (device->TraceClass == WMIREG_NOTIFY_DISK_IO) || (device->TraceClass == WMIREG_NOTIFY_VOLMGR_IO))) {
            do {
                IoInitializeIrp(irp, IoSizeOfIrp(stackSize), stackSize);
                IoSetNextIrpStackLocation(irp);
                irpStack = IoGetCurrentIrpStackLocation(irp);
                irpStack->DeviceObject = WmipServiceDeviceObject;
                irp->Tail.Overlay.Thread = PsGetCurrentThread();
                status = WmipForwardWmiIrp(irp, IRP_MN_SET_TRACE_NOTIFY, IoWMIDeviceObjectToProviderId(device->DeviceObject), NULL, sizeof(NullPtr), &NullPtr);
                if (status == STATUS_WMI_TRY_AGAIN) {
                    IoFreeIrp(irp);
                    stackSize = WmipServiceDeviceObject->StackSize;
                    irp = IoAllocateIrp(stackSize, FALSE);
...
VOID WmipSetTraceNotify(IN PDEVICE_OBJECT DeviceObject, IN ULONG TraceClass, IN ULONG Enable)
{
    ...
    do {
        ...
        irpStack = IoGetCurrentIrpStackLocation(irp);
        irpStack->DeviceObject = WmipServiceDeviceObject;
        status = WmipForwardWmiIrp(irp, IRP_MN_SET_TRACE_NOTIFY, IoWMIDeviceObjectToProviderId(DeviceObject), NULL, sizeof(NotifyRoutine), &NotifyRoutine);
        IoFreeIrp(irp);
    } while (status == STATUS_WMI_TRY_AGAIN);
}


// All the following routines are callout or notification routines for generating kernel event traces


NTKERNELAPI VOID FASTCALL WmiTraceProcess(IN PEPROCESS Process, IN BOOLEAN Create)
/*
Routine Description:
...
*/
{
    ...
    Status = SeLocateProcessImageName(Process, &pImageFileName);// Get image name not limited to 16 chars.
    if (NT_SUCCESS(Status)) {
            ...
            if (LoggerContext->EnableFlags & EVENT_TRACE_FLAG_PROCESS) {
                ...
                if (Header) {
                    ...
                    if (ImageLength != 0) {
                        Dst = AuxPtr;
                        if (LongImageName) {
                            // ImageOnlyLength is from SeLocateProcessImageName(), so we can trust it.
...
NTKERNELAPI VOID WmiTraceThread(IN PETHREAD Thread, IN PINITIAL_TEB InitialTeb OPTIONAL, IN BOOLEAN Create)
/*
Routine Description:
    This callout routine is called from ps\create.c and ps\psdelete.c.
        ...
        if (LoggerContext != NULL) {
            if (LoggerContext->EnableFlags & EVENT_TRACE_FLAG_THREAD) {
                if (Create) {
                    ...
                    Header = (PSYSTEM_TRACE_HEADER)WmiReserveWithSystemHeader(LoggerId, sizeof(WMI_EXTENDED_THREAD_INFORMATION), NULL, &BufferResource);
                    if (Header) {
                    ...
                    Header = (PSYSTEM_TRACE_HEADER)WmiReserveWithSystemHeader(LoggerId, sizeof(WMI_THREAD_INFORMATION), NULL, &BufferResource);
                    if (Header) {
...
*/
{
    ...
    Header = (PSYSTEM_TRACE_HEADER)WmiReserveWithSystemHeader(WmipKernelLogger, 2 * sizeof(PVOID), NULL, &BufferResource);
    if (Header == NULL)
        return;
...
Arguments:
    ...
    EventInfo       a pointer to contiguous memory containing information to be attached to event trace
...
*/
{
    ...
    Header = (PSYSTEM_TRACE_HEADER)WmiReserveWithSystemHeader(WmipKernelLogger, Size, Irp->Tail.Overlay.Thread, &BufferResource);
    if (Header == NULL) {
        return;
    }
    ...
    if (FileTraceOn) {
        ...
        // We are done with the IO Hook. Release the Buffer but take a refcount on the logger context so that the fileTable does not go away.
        ...
        // If found, return else queue work item after checking max work item limit.
        ...
        // Cache Miss: Simply kick out the next item based on global index while ensuring that the WmipFileIndex is always in range.
        ...
        TraceFileWorkQueueItem = ExAllocatePoolWithTag(NonPagedPool, MAX_FILENAME_TO_LOG, TRACEPOOLTAG);
        ...
        Status = ObReferenceObjectByPointer(fileObject, 0L, IoFileObjectType, KernelMode);
        if (!NT_SUCCESS(Status)) {
        ...
        ExInitializeWorkItem(&TraceFileWorkQueueItem->WorkItem, WmipTraceFile, TraceFileWorkQueueItem);
        ...
        RefCount =
            ...
            WmipDereferenceLogger(LoggerId);
        TraceDebug((4, "WmiTraceFile: %d %d->%d\n", LoggerId, RefCount + 1, RefCount));
        ExQueueWorkItem(&TraceFileWorkQueueItem->WorkItem, DelayedWorkQueue);
    } else {
        WmipReleaseTraceBuffer(BufferResource, LoggerContext);
    }
}
...
/*
Routine Description:
    This routine traces the connection between IRPs entering the volume manager and the child IRPs they spawn
*/
{
    PSYSTEM_TRACE_HEADER Header;
    PVOID BufferResource;
...
VOID WmipTraceFile(IN PVOID TraceFileContext)
{
    ...
    FileNameInfo = (POBJECT_NAME_INFORMATION)((PUCHAR)TraceFileContext + FileNameInfoOffset);
    fileObject = WorkItem->FileObject;
    ASSERT(fileObject != NULL);
    ASSERT(WorkItem->BufferSize > FileNameInfoOffset);
    Status = ObQueryNameString(fileObject, FileNameInfo, WorkItem->BufferSize - FileNameInfoOffset, &ReturnLen);
    ...
    for (LoggerId = 0; LoggerId < MAXLOGGERS; LoggerId++) {
        ...
        if (LoggerContext != NULL) {
            if (LoggerContext->EnableFlags & EVENT_TRACE_FLAG_IMAGE_LOAD) {
                ...
                Header = WmiReserveWithSystemHeader(LoggerId, FIELD_OFFSET(WMI_IMAGELOAD_INFORMATION, FileName) + Length + sizeof(WCHAR), NULL, &BufferResource);
                if (Header != NULL) {
...
*/
{
    ...
    try {
        if (KeyName && KeyName->Buffer && KeyName->Length) {
            ...
            if ((len == 0) || (KeyName->Buffer[len / sizeof(WCHAR) - 1] != 0)) {
                len += sizeof(WCHAR);// make room for NULL terminator
            }
        } else {
            len += sizeof(WCHAR);
        }
    ...
    {
        return;// KeyName buffer is from user. AV can happen.
    }
    ...
    *((ULONG *)EventInfo) = Index;
    ...
    {
        RtlZeroMemory(EventInfo, len - sizeof(WCHAR));// Cleanup just in case
    }
...
/*
Routine Description:
    This routine is called to trace context swap operations.
    It is called directly from the context swap procedure while the context swap lock is being held, so it is critical that this routine not take any locks.

    Assumptions:
    - This routine will only be called from the ContextSwap routine
    - This routine will always be called at IRQL >= DISPATCH_LEVEL
    - This routine will only be called when the PPerfGlobalGroupMask is not equal to null,
      and the context swap flag is set within the structure to which PPerfGlobalGroupMask points to, and the kernel's WMI_LOGGER_CONTEXT struct has been fully initialized.
    - The Wmi kernel WMI_LOGGER_CONTEXT object, as well as all buffers it allocates are allocated from nonpaged pool.
      All Wmi globals that we access are also in nonpaged memory.
    - This code has been locked into paged memory when the logger started
    - The logger context reference count has been incremented via the InterlockedIncrement() operation in WmipReferenceLogger(WmipKernelLogger) by our start code.
Arguments:
    OldThread - ptr to ETHREAD object of thread being swapped out
    NewThread - ptr to ETHREAD object of thread being swapped in
*/
{
    ...
    CurrentProcessor = (UCHAR)KeGetCurrentProcessorNumber();// Figure out which processor we are running on

    // If we currently have no context swap buffer for this processor then we need to grab one from the ETW Free list.
    ...
    if (Buffer == NULL) {
        ...
        WmipContextSwapProcessorBuffers[CurrentProcessor] = Buffer;// We have a legitimate buffer, so now we set it as this processor's current cxtswap buffer
    }

    if (Buffer->Offset <= Buffer->CurrentOffset) {
        // Due to an rare unfortunate timing issue with buffer recycle, buffer CurrentOffset is corrupt.
        // We should not write over buffer boundary.
        WmipPushDirtyContextSwapBuffer(CurrentProcessor, Buffer);

        // Zero out the processor buffer pointer so that when we next come into the trace code, we know to grab another one.
        WmipContextSwapProcessorBuffers[CurrentProcessor] = NULL;
        return;
    }

    // Compute the pointers to our event structures within the buffer
    // At this point, we will always have enough space in the buffer for this event.
    // We check for a full buffer after we fill out the event
    ...
    ASSERT(EventSize % WMI_CTXSWAP_EVENTSIZE_ALIGNMENT == 0);// Assert that the event size is at aligned correctly
    ...
    Buffer->CurrentOffset += (ULONG)EventSize;// Increment the offset.  Don't need synchronization here because IRQL >= DISPATCH_LEVEL.

    // Check if the buffer is full by taking the difference between the buffer's maximum offset and the current offset.
    if ((Buffer->Offset - Buffer->CurrentOffset) <= EventSize) {
        WmipPushDirtyContextSwapBuffer(CurrentProcessor, Buffer);// Push the full buffer onto the FlushList.
        WmipContextSwapProcessorBuffers[CurrentProcessor] = NULL;// Zero out the processor buffer pointer so that when we next come into the trace code, we know to grab another one.
    }
}
...
/*
Routine Description:
    Allocates the memory to track the per-processor buffers used by context swap tracing.
    "locks" the logger by incrementing the logger context reference count by one.

    Assumptions:
    - This function will not run at DISPATCH or higher
    - The kernel logger context mutex has been acquired before entering this function.

    Calling Functions:
    - PerfInfoStartLog
*/
{
    PAGED_CODE();// Only used in checked builds - asserts if this code is called with Irql > APC_LEVEL.

    // Set the pointers to our buffers to NULL, indicating to the trace event code that a buffer needs to be acquired.
...
Routine Description:
    ...
    Once a context swap has occured on a processor after the context swap tracing flag has been disabled, we are guaranteed that the buffer associated with that processor is not in use.
    It is then safe to place that buffer on the flush list.

    Assumptions:
    - This function will not run at DISPATCH
    - The kernel logger context mutex was acquired before this function was called.
...
Return Value:
    None;
    if we fail here there's nothing we can do anyway.
*/
{
    PKTHREAD            ThisThread;
    ...
    // If we can't get the logger context, then we have nowhere to flush buffers and we might as well stop here.
    LoggerContext = WmipLoggerContext[WmipKernelLogger];
    ...
    // This would probably break if the number of processors were decreased in the middle of the trace.
    for (i = 0; i < KeNumberProcessors; i++) {
        ...
        // Check to make sure this processor even has a buffer, if it doesn't, then next loop
...
PWMI_LOGGER_CONTEXT FASTCALL WmipIsLoggerOn(IN ULONG LoggerId)
{
    PWMI_LOGGER_CONTEXT LoggerContext;
