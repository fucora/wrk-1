...
Abstract:
    This module registers all of the firmware providers supported by the kernel.
*/
...
NTSTATUS __cdecl WmipRawSMBiosTableHandler(PSYSTEM_FIRMWARE_TABLE_INFORMATION TableInfo);
NTSTATUS __cdecl WmipFirmwareTableHandler(PSYSTEM_FIRMWARE_TABLE_INFORMATION TableInfo);


VOID WmipRegisterFirmwareProviders()
/*
Routine Description:
    This routine is called to register the correct set of data providers with the Executive part of the kernel.
...
// This array defines the legal ranges of memory that can be returned by the FirmwareTableHandler. The format is starting address and length.
...
Routine Description:
    ...
    Currently, the only requested range is the E0000-FFFFF range, but we also support handing back the C0000-DFFFF range.
Arguments:
...
*/
{
    ...
    // Make sure that the length field is correct and that the ID requested matches one of the supported starting addresses.
    ...
    if (i == TableCount) {// If we failed to find a match, than abort.
        return STATUS_INVALID_PARAMETER_1;
    }
    ...
    RtlCopyMemory(TableInfo->TableBuffer, Firmware, TableSize);// Copy the buffer.
    MmUnmapIoSpace(Firmware, TableSize);// Unamp the firmware pointer.
    return STATUS_SUCCESS;
}
...
--*/
{
    ...
    if (TableInfo->Action == SystemFirmwareTable_Enumerate) {
        ...
        // The "TableID" of the only table is '0'. Make sure that we indicate that only one table exists.
    ...
    // Setup all the pointers that we will need for getting a real copy of the tables.
