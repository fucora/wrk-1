...
NTSTATUS WmipOpenCloseCleanup(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp);
NTSTATUS WmipIoControl(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp);
NTSTATUS WmipObjectToPDO(PFILE_OBJECT FileObject, PDEVICE_OBJECT DeviceObject, PDEVICE_OBJECT *PDO);
BOOLEAN WmipFastIoDeviceControl(
...
NTSTATUS WmipDriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
/*
Routine Description:
    This is the callback function when we call IoCreateDriver to create a WMI Driver Object.
    In this function, we need to remember the DriverObject,
    create a device object and then create a Win32 visible symbolic link name so that the WMI user mode component can access us.
Arguments:
...
*/
{
    ...
    // Since Io does not pass a registry path for this device we need to make up one
    ...
    // It will allow only Administrators access to the device and no access to anyone else.
    ...
    RtlInitUnicodeString(&AdminSymbolicLinkName, WMIAdminSymbolicLinkName);
    Status = IoCreateSymbolicLink(&AdminSymbolicLinkName, &DeviceName);
    if (!NT_SUCCESS(Status)) {
        IoDeleteSymbolicLink(&ServiceSymbolicLinkName);
        IoDeleteDevice(WmipServiceDeviceObject);
        IoDeleteDevice(WmipAdminDeviceObject);
    ...
    // We only do this for the kernel dll since the IO manager does it when WMI loads as a normal device.
...
/*
Routine Description:
    This routine will update the stack size that is specified in the WMI device's device object.
    This needs to be protected since it can be updated when a device registers and whenever an irp is forwarded to a device.
    WMI needs to maintain a stack size one greater than the stack size of the largest device stack to which it forwards irps to.
    Consider a bottom driver that registers with WMI and has a stack size of 1.
    If 2 device attach on top of it then WMI will forward to the topmost in the stack which would need a stack size of 3,
    so the original WMI irp (ie the one created by the IOCTL to the WMI device) would need a stack size of 4.
Arguments:
...
NTSTATUS WmipIoControl(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    ...
    case IOCTL_WMI_QUERY_ALL_DATA:
    {
        if (OutBufferLen < sizeof(WNODE_ALL_DATA)) {
            // WMI will not send any request whose output buffer is not at least the size of a WNODE_ALL_DATA.
    ...
    case IOCTL_WMI_QAD_MULTIPLE:
    {
        ...
        // Check that the input/output sizes make sense and that we have a valid HandleCount.
        if ((OutBufferLen >= sizeof(WNODE_TOO_SMALL)) &&
            ...
            Status = WmipQueryAllDataMultiple(0, NULL, Irp, UserMode, Buffer, OutBufferLen, QadMultiple, &OutBufferLen);
        } else {
    ...
    {
        if (OutBufferLen < sizeof(WNODE_TOO_SMALL)) {
            // WMI will not send any request whose output buffer is not at least the size of a WNODE_TOO_SMALL.
            Status = STATUS_BUFFER_TOO_SMALL;
            break;
        }
        ...
        if (NT_SUCCESS(Status)) {
            Status = WmipQuerySetExecuteSI(NULL, Irp, UserMode, IRP_MN_QUERY_SINGLE_INSTANCE, Wnode, OutBufferLen, &OutBufferLen);
            if (NT_SUCCESS(Status)) {
    ...
    case IOCTL_WMI_QSI_MULTIPLE:
    {
        ...
        // Check that the input/output sizes make sense and that we have a valid QueryCount.
        if ((OutBufferLen >= sizeof(WNODE_TOO_SMALL)) &&
    ...
    case IOCTL_WMI_SET_SINGLE_INSTANCE:
    {
        ...
        if (NT_SUCCESS(Status)) {
            Status = WmipQuerySetExecuteSI(NULL, Irp, UserMode, IRP_MN_CHANGE_SINGLE_INSTANCE, Wnode, InBufferLen, &OutBufferLen);
            OutBufferLen = 0;
        } else {
    ...
    case IOCTL_WMI_SET_SINGLE_ITEM:
    {
        ...
        if (NT_SUCCESS(Status)) {
            Status = WmipQuerySetExecuteSI(NULL, Irp, UserMode, IRP_MN_CHANGE_SINGLE_ITEM, Wnode, InBufferLen, &OutBufferLen);
            OutBufferLen = 0;
        } else {
    ...
    {
        // The buffer passed is the InputWnode directly followed by the method wnode.
        // This is so that the driver can fill in the output WNODE directly on top of the input wnode.
        ...
        if (NT_SUCCESS(Status)) {
            Status = WmipQuerySetExecuteSI(NULL, Irp, UserMode, IRP_MN_EXECUTE_METHOD, Wnode, OutBufferLen, &OutBufferLen);
            if (NT_SUCCESS(Status)) {
                WmipAssert(Irp->IoStatus.Information <= OutBufferLen);
            }
        }
    ...
    case IOCTL_WMI_RECEIVE_NOTIFICATIONS:
    {
        ...
        if ((InBufferLen >= sizeof(WMIRECEIVENOTIFICATION)) && (OutBufferLen >= sizeof(WNODE_TOO_SMALL))) {
            ...
            // Input and or output buffers not large enough which is an error
            Status = STATUS_INVALID_DEVICE_REQUEST;
        }
        break;
    }
    ...
    case IOCTL_WMI_NOTIFY_LANGUAGE_CHANGE:
    {
        ...
        if (DeviceObject == WmipAdminDeviceObject) {
            // Only allow this ioctl to be executed on the admin device object
    ...
    case IOCTL_WMI_REGISTER_GUIDS:
    {
        ...
        Status = WmipProbeWmiRegRequest(DeviceObject, Buffer, InBufferLen, OutBufferLen, &MofIgnored);
        if (NT_SUCCESS(Status)) {
            ...
            // For WOW64, WMIREGINFOW and WMIREGGUIDW structures both need to be thunked here because of padding and ULONG_PTR in them.
#if defined(_WIN64)
            if (IoIs32bitProcess(NULL)) {
                ...
                SizeNeeded = sizeof(WMIREGINFOW) + GuidCount * sizeof(WMIREGGUIDW) + ImageNameLength + ResourceNameLength;
                ...
                for (i = 0; i < GuidCount; i++) {
                    ...
                    // The InstanceCount checks are done here because the source may not be aligned.
    ...
    { // NOTE: This relies on WmiTraceEvent to probe the buffer!
        ...
        Status = WmiTraceEvent((PWNODE_HEADER)irpStack->Parameters.DeviceIoControl.Type3InputBuffer, KeGetPreviousMode());
        break;
    }
    ...
    { // NOTE: This relies on WmiTraceUserMessage to probe the buffer!
        ...
        Status = WmiTraceUserMessage((PMESSAGE_TRACE_USER)irpStack->Parameters.DeviceIoControl.Type3InputBuffer, InBufferLen);
        break;
    }
...
/*
Routine Description:
    WMI forwarded IRP completion routine.
    Set an event and return STATUS_MORE_PROCESSING_REQUIRED.
    WmipForwardWmiIrp will wait on this event and then re-complete the irp after cleaning up.
Arguments:
...
Routine Description:
    ...
    The mechanism is to build a IRP_MJ_PNP irp with IRP_MN_QUERY_RELATIONS and query for TargetDeviceRelation.
    This irp is supposed to be passed down a device stack until it hits the PDO which will fill in its device object and return.
    Note that some drivers may not support this.
Arguments:
    FileObject is the file object for device that is being queried
    DeviceObject is the device object that is being queried
    *PDO returns with the PDO that is targeted by the file object. When the caller has finished using the PDO it must ObDereferenceObject it.
Return Value:
    NT status code
*/
{
...
NTSTATUS WmipForwardWmiIrp(PIRP Irp, UCHAR MinorFunction, ULONG ProviderId, PVOID DataPath, ULONG BufferLength, PVOID Buffer)
/*
Routine Description:
    If the provider is a driver then this routine will allocate a new irp with the correct stack size and send it to the driver.
    If the provider is a callback then it is called directly.
...
Arguments:
    ...
    WmiRegistrationId is the id passed by the user mode code. This routine will look it up to determine the device object pointer.
...
*/
{
    ...
    // For non-file handle based requests we get the registration entry to validate the target and check for a callback
    ...
    // Get the top of the device stack for our target WMI device.
    // Note that IoGetAttachedDeviceReference also takes an object reference which we get rid of after the the irp is completed by the data provider driver.
    TargetDeviceObject = IoGetAttachedDeviceReference(DeviceObject);
    DeviceStackSize = TargetDeviceObject->StackSize + 1;

    // Check that there are enough stack locations in our irp so that we can forward it to the top of the device stack.
    // We must also check if our target device is the WMI data or service device otherwise the number of stack locations for it will keep incrementing until the machine crashes
    if ((DeviceStackSize <= WmipServiceDeviceObject->StackSize) || (TargetDeviceObject == WmipServiceDeviceObject)) {
        // There are enough stack locations in the WMI irp to forward Remember some context information in our irp stack and use it as our completion context value
        ...
        // Initialize irp status to STATUS_NOT_SUPPORTED so that we can detect the case where no data provider responded to the irp
        Irp->IoStatus.Status = STATUS_NOT_SUPPORTED;
        ...
        // Check if the status code is still STATUS_NOT_SUPPORTED.
        // If this is the case then most likely no data provider responded to the irp.
        // So we want to change the status code to something more relevant to WMI like STATUS_WMI_GUID_NOT_FOUND
        ...
        // If this was a registration request then we need to see if there are any PDOs that need to be translated into static instance names.
        if (((MinorFunction == IRP_MN_REGINFO) || (MinorFunction == IRP_MN_REGINFO_EX)) &&
            (NT_SUCCESS(Status)) &&
        ...
        // We bump the stack count for the WMI device and return an error asking to try the irp again.
        WmipUnreferenceRegEntry(RegEntry);
        WmipDecrementIrpCount(RegEntry);
    ...
    // Dereference the target device which was the top of the stack to which we forwarded the irp.
...
Routine Description:
    ...
    The routine handles the case where the stack size is too small and will retry the irp.
Arguments:
...
/*
Routine Description:
    This routine will translate a file handle or device object into the device instance name for the target PDO of the device object pointed to by the file handle.
Arguments:
    FhToInstanceName passes in the file handle and returns the device instance name.
Return Value:
    NT status code
*/
{
                ...
                if (NT_SUCCESS(Status)) {
                    ...
                    if (NT_SUCCESS(Status)) {
                        if (FhToInstanceName != NULL) {
                            ...
                            SizeNeeded = DeviceInstanceName.Length + 2 * sizeof(WCHAR) + FIELD_OFFSET(WMIFHTOINSTANCENAME, InstanceNames);
                            ...
                            InstanceName = ExAllocatePoolWithTag(PagedPool, InstanceNameString->MaximumLength, WmipInstanceNameTag);
                            if (InstanceName != NULL) {
                                StringCbPrintf(InstanceName, InstanceNameString->MaximumLength, L"%ws_%d", DeviceInstanceName.Buffer, BaseIndex);
...
Routine Description:
    ...
    5. For inbound Wnode->DataBlockOffset must be 0 (implying no data) or Wnode->DataBlockOffset must be <= incoming buffer size and >= sizeof(WNODE_SINGLE_INSTANCE),
       that is the data block must start in the incoming buffer, but after the WNODE_SINGLE_INSTANCE header.
    6. Wnode and Wnode->DataBlockOffset must be aligned on an 8 byte boundray.
    7. For inbound data (SetSingleInstance) (Wnode->DataBlockOffset + Wnode->DataBlockSize) < incoming buffer length.
       That is the entire data block must fit within the incoming buffer.
    8. For outbound data (QuerySingleInstance) Wnode->DataBlockOffset must be <= outgoing buffer length.
       That is the start of the outgoing data block must fit within the outgoing data buffer.
       Note that it is the provider's responsibility to determine if there will be enough space in the outgoing buffer to write the returned data.
    10. Wnode->OffsetInstanceNames must be aligned on a 2 byte boundary
    11. Wnode->OffsetInstanceNames must be <= (incoming buffer size) + sizeof(USHORT),
        that is it must start within the incoming buffer and the USHORT that specifies the length must be within the incoming buffer.
...
Arguments:
    ...
    CheckOutBound - If TRUE, WNODE needs to be validated for provider to return data.
    CheckInBound - If TRUE WNODE needs to be validated for provider to receive data
Return Value:
    NT status code
*/
{
    ...
    if (InstanceNameOffset != 0) {
        ...
        // Validate InstanceName is aligned properly.
        // This is left in the free build since alphas may have alignment requiremnts in handling USHORTs and WCHARs

        // Validate that USHORT holding instance name length is within WNODE
        if ((!WmipIsAligned(InstanceNameOffset, 2)) || (InstanceNameOffset > InBufferLen - sizeof(USHORT))) {
            return(STATUS_UNSUCCESSFUL);
        }

        // Validate Dynamic Instance Name text is fully within input buffer and output buffer for outbound WNODEs
        ...
        // If data block is specified then it must be placed after the end of the instance name
    ...
    if (CheckOutBound) {
        // For outgoing WNODE make sure there is enough room to write the WNODE header

        // For outgoing WNODE make sure the data block offset is within the bounds of the output buffer
        if ((OutBufferLen < MinWnodeSize) || (DataBlockOffset > OutBufferLen)) {
            return(STATUS_UNSUCCESSFUL);
        }

        // Make sure that the data block offset is specified so provider can know where to write data
...
*/
{
    ...
    // Make sure that enough of the WNODE_ALL_DATA was passed so that we can look at it and the drivers can fill it in
    if (OutBufferLen < sizeof(WNODE_ALL_DATA)) {
        return(STATUS_UNSUCCESSFUL);
    }
...
/*
Routine Description:
    Probe the incoming WNODE_SINGLE_INSTANCE to ensure that any offsets in the header point to memory that is valid within the buffer.
    Also validate that the WNODE_SINGLE_INSTANCE is properly formed.

    This routine MUST succeed before any fields in the WNODE_SINGLE_INSTANCE can be used by any  kernel components when passed in from user mode.
    Note that we can trust that the input and output buffer are properly sized since the WMI IOCTLs are METHOD_BUFFERED and the IO manager does that for us.

    WNODE_SINGLE_INSTANCE Rules:
    1. The incoming buffer must be at least as large as sizeof(WNODE_SINGLE_INSTANCE)
    2. The outgoing buffer must be at least as large as sizeof(WNODE_SINGLE_INSTANCE)
    3. WnodeHeader->ProviderId must be non null, Actual value validated when irp is forwarded.
    4. WnodeHeader->BufferSize must equal incoming buffer size
    5. Wnode->DataBlockOffset must be 0 (implying no data) or Wnode->DataBlockOffset must be <= incoming buffer size and >= sizeof(WNODE_SINGLE_INSTANCE),
       that is the data block must start in the incoming buffer, but after the WNODE_SINGLE_INSTANCE header.
    ...
    10. Wnode->OffsetInstanceNames must be aligned on a 2 byte boundary
    11. Wnode->OffsetInstanceNames + sizeof(USHORT) must be <= incoming buffer size,
        that is it must start within the incoming buffer and the USHORT that specifies the length must be within the incoming buffer.
    12. The entire instance name string must fit with the incoming buffer
    13. For outbound data (QuerySingleInstance) the entire instance name must start and fit within the output buffer.
    14. Wnode->DataBlockOffset must be placed after any instance name and not overlap the instance name.
Arguments:
    ...
    InBufferLen - Size of the incoming buffer
    OutBufferLen - Size of the outgoing buffer
    OutBound - If FALSE, WNODE_SINGLE_INSTANCE has inbound data that must be validated to be within the input buffer.
               If FALSE, WNODE_SINGLE_INSTANCE is expected to be filled with data by the driver so ensure that data buffer is validated to be within the output buffer.
Return Value:
    NT status code
*/
{
...
/*
Routine Description:
    Probe the incoming WNODE_SINGLE_ITEM to ensure that any offsets in the header point to memory that is valid within the buffer.
    Also validate that the WNODE_SINGLE_ITEM is properly formed.

    This routine MUST succeed before any fields in the WNODE_SINGLE_INSTANCE can be used by any  kernel components when passed in from user mode.
    Note that we can trust that the input and output buffer are properly sized since the WMI IOCTLs are METHOD_BUFFERED and the IO manager does that for us.
WNODE_SINGLE_ITEM rules:
    1. The incoming buffer must be at least as large as sizeof(WNODE_SINGLE_ITEM)
    2. The outgoing buffer must be at least as large as sizeof(WNODE_SINGLE_ITEM)
    3. WnodeHeader->ProviderId must be non null, Actual value validated when irp is forwarded.
    4. WnodeHeader->BufferSize must equal incoming buffer size
    5. Wnode->DataBlockOffset must be 0 (implying no data) or Wnode->DataBlockOffset must be <= incoming buffer size and >= sizeof(WNODE_SINGLE_ITEM), that is
       the data block must start in the incoming buffer, but after the WNODE_SINGLE_ITEM header.
    6. Wnode and Wnode->DataBlockOffset must be aligned on an 8 byte boundary.
    7. (Wnode->DataBlockOffset + Wnode->SizeDataItem) < incoming buffer length. That is the entire data block must fit within the incoming buffer.
    8. Wnode->DataItemId must not be 0
    9. Wnode->OffsetInstanceNames must be aligned on a 2 byte boundary
    10. Wnode->OffsetInstanceNames must be <= (incoming buffer size) + sizeof(USHORT),
        that is it must start within the incoming buffer and the USHORT that specifies the length must be within the incoming buffer.
    11. The entire instance name string must fit with the incoming buffer
    12. Wnode->DataBlockOffset must be placed after any instance name and not overlap the instance name.
Arguments:
...
*/
{
    ...
    // Make sure that enough of the WNODE_SINGLE_ITEM was passed so that we can look at it
    if (InBufferLen < FIELD_OFFSET(WNODE_SINGLE_ITEM, VariableData)) {
        return(STATUS_UNSUCCESSFUL);
    }

    // We don't use sizeof(WNODE_SINGLE_ITEM), but rather use the offset to the variable data since in the case of WNODE_SINGLE_ITEM they are different.
    // The C compiler will round up the packing to 8 bytes so the former is 48 and the later is 44.
    Status = WmipProbeWnodeWorker(WnodeHeader,
...
/*
Routine Description:
    Probe the incoming WNODE_METHOD_ITEM to ensure that any offsets in the header point to memory that is valid within the buffer.
    Also validate that the WNODE_METHOD_ITEM is properly formed.

    This routine MUST succeed before any fields in the WNODE_METHOD_INSTANCE can be used by any  kernel components when passed in from user mode.
    Note that we can trust that the input and output buffer are properly sized since the WMI IOCTLs are METHOD_BUFFERED and the IO manager does that for us.

    WNODE_METHOD_ITEM Rules:
    1. The incoming buffer must be at least as large as sizeof(WNODE_METHOD_ITEM)
    2. The outgoing buffer must be at least as large as sizeof(WNODE_METHOD_ITEM)
    3. WnodeHeader->ProviderId must be non null, Actual value validated when irp is forwarded and Wnode->MethodId must not be 0
    4. WnodeHeader->BufferSize must equal incoming buffer size
    5. Wnode->DataBlockOffset must be 0 (implying no data) or Wnode->DataBlockOffset must be <= incoming buffer size and >= sizeof(WNODE_METHOD_ITEM), that is
       the data block must start in the incoming buffer, but after the WNODE_METHOD_ITEM header.
    6. Wnode and Wnode->DataBlockOffset must be aligned on an 8 byte boundary.
    7. For inbound data (Wnode->DataBlockOffset + Wnode->DataBlockSize) < incoming buffer length. That is the entire data block must fit within the incoming buffer.
    8. For outbound data Wnode->DataBlockOffset must be <= outgoing buffer length.
       That is the start of the outgoing data block must fit within the outgoing data buffer.
       Note that it is the provider's responsibility to determine if there will be enough space in the outgoing buffer to write the returned data.
    ...
    11. Wnode->OffsetInstanceNames must be <= (incoming buffer size) + sizeof(USHORT),
        that is it must start within the incoming buffer and the USHORT that specifies the length must be within the incoming buffer.
    12. The entire instance name string must fit with the incoming buffer
    13. For outbound data the entire instance name must start and fit within the output buffer.
    14. Wnode->DataBlockOffset must be placed after any instance name and not overlap the instance name.
Arguments:
...
*/
{
    ...
    // Make sure that enough of the WNODE_METHOD_ITEM was passed so that we can look at it
...
/*
Routine Description:
    Probe the incoming WMIREGREQUEST to ensure that any offsets in the header point to memory that is valid within the buffer.
    Also validate that the WMIREGINFO is properly formed.

    This routine MUST succeed before any fields in the WMI_REG_INFO can be used by any  kernel components when passed in from user mode.
    Note that we can trust that the input and output buffer are properly sized since the WMI IOCTLs are METHOD_BUFFERED and the IO manager does that for us.
    ...
    *pMofIgnored = FALSE;
    if (InBufferLen >= (sizeof(WMIREGREQUEST) + sizeof(WMIREGINFO))) {
        ...
        if (WmiRegInfoSize <= (InBufferLen - sizeof(WMIREGREQUEST))) {
            ...
            // If the Registration call came through Admin device, we are okay to send the BinaryMof through.
            // If it came through the DataDevice, then we need to disable the Binary MOF.
            // We do that by simply zapping the MofResourceName and RegistryPath
            if (DeviceObject != WmipAdminDeviceObject) {
                if (WmiRegInfo->MofResourceName > 0) {
                    *pMofIgnored = TRUE;
                }
            ...
            // Make sure the RegistryPath and MofResourceName offsets are within the REGINFO buffer.
            ...
            // Note: If the ImagePath and MofResource trample over each other but stayed within BufferSize , we will not catch it.

#if defined(_WIN64)
            if (IoIs32bitProcess(NULL)) {
                // Check to make sure the 32 bit structure sizes passed in by the caller is comparable to the 64-bit counterparts
                ...
                // InstanceCount and InstanceNameList in WMIREGGUIDW structure must be zero.
                // This check is done after thunking gor WOW64.
                SizeNeeded = WmiRegRequest->WmiRegInfo32Size + GuidCount * WmiRegRequest->WmiRegGuid32Size + ImageNameLength + ResourceNameLength;
...
#endif
            {
                ...
                // Check to see if the InstanceCount and InstanceNameList in WMIREGGUIDW structure is zero
            ...
            return STATUS_SUCCESS;// All tests passed. Return SUCCESS
        }
    }
