...
/*
Routine Description:
    This routine attempts to build an MCA error log event from the information in the supplied MCA_EXCEPTION.
Arguments:
    McaException - Pointer to a MCA_EXCEPTION that encapsulates the machine check event.
    ...
    RawPtr       - Address into which a pointer to the raw MCA data is to be copied.
Return Value:
...
*/
{
    ...
    WmipAssert(sizeof(MSMCAEvent_MemoryError) >= sizeof(MSMCAEvent_MemoryHierarchyError));
    WmipAssert(sizeof(MSMCAEvent_MemoryError) >= sizeof(MSMCAEvent_TLBError));
    WmipAssert(sizeof(MSMCAEvent_MemoryError) >= sizeof(MSMCAEvent_BusError));
    ...
    if (mcaCode == MCA_CODE_MICROCODE_ROM_PARITY_ERROR ||
        ...
        *RawPtr = invalidEvent->RawRecord;// Copy the address of the raw record into the supplied address.
        invalidEvent->Size = ErrorLogSize;// Indicate the size of the entire record.
        ...
        Wnode->SizeDataBlock = FIELD_OFFSET(MSMCAEvent_BusError, RawRecord) + ErrorLogSize;
        ...
        *RawPtr = busEvent->RawRecord;
        ...
        Wnode->SizeDataBlock = FIELD_OFFSET(MSMCAEvent_MemoryHierarchyError, RawRecord) + ErrorLogSize;
        ...
        *RawPtr = memoryEvent->RawRecord;
        ...
        Wnode->SizeDataBlock = FIELD_OFFSET(MSMCAEvent_TLBError, RawRecord) + ErrorLogSize;
