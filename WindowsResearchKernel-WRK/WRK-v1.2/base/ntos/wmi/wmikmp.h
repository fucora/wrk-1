...
// Define this to cause no security descriptor to be placed on the service device.
// This should never be defined for released builds; it is only needed when debugging the service startup code.
...
// Tracelog Critical section is to serialize the Enabling and disabling of trace control guids.
// Since we want to allow Updates (enable a a Guid with a different set of flags and levels), we serialize this operation to reduce complexity of code.
...
Arguments:
    ...
    Buffer on return is filled with the returned data blocks. Note that each
        instance of the data block must be aligned on a 8 byte boundary. If this is NULL then there was not enough space in the output buffer
        to fulfill the request so the irp should be completed with the buffer needed.
...
/*
Routine Description:
    This routine is a callback into the driver to query for the contents of a data block.
...
/*
Routine Description:
    This routine is a callback into the driver to execute a method.
    When the driver has finished filling the data block it must call IoWMICompleteRequest to complete the irp.
    The driver can return STATUS_PENDING if the irp cannot be completed immediately.
...
Arguments:
    ...
    InstanceIndex is the index that denotes which instance of the data block is being called.
    ...
    Buffer on entry has the input data block and on return has the output output data block.
...
Routine Description:
    ...
    generation or data block collection. A device should only expect a single enable when the first event or data consumer enables events or
    data collection and a single disable when the last event or data consumer disables events or data collection. Data blocks will only
    receive collection enable/disable if they were registered as requiring it.
...
void WmipTranslatePDOInstanceNames(IN OUT PIRP Irp, IN UCHAR MinorFunction, IN ULONG BufferSize, IN OUT PREGENTRY RegEntry);
...
NTSTATUS FASTCALL WmiTraceUserMessage(IN PMESSAGE_TRACE_USER pMessage, IN ULONG MessageSize);
