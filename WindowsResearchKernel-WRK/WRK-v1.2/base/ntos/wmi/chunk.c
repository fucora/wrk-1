...
Return Value:
    Pointer to structure or NULL if one cannot be allocated.
    Entry returns with its refcount set to 1
*/
{
    ...
    // There are no more free entries in any of the chunks. Allocate a new chunk if we can
    ...
    if (Chunk != NULL) {
        // Initialize the chunk by building the free list of entries within it while also initializing each entry.
        ...
        // EntryPtr now points to the last entry in the chunk which has not been placed on the free list.
        // This will be the entry returned to the caller.
        Entry = (PENTRYHEADER)EntryPtr;
        Entry->Chunk = Chunk;
        Entry->RefCount = 1;
        Entry->Flags = ChunkInfo->InitialFlags;
...
*/
{
    ...
    if ((--Chunk->EntriesInUse == 0) && (ChunkInfo->ChunkHead.Blink != &Chunk->ChunkList)) {
        // We return the chunks memory back to the heap if there are no more entries within the chunk in use and the chunk was not the first chunk to be allocated.
        RemoveEntryList(&Chunk->ChunkList);
        WmipLeaveSMCritSection();
        ExFreePoolWithTag(Chunk, ChunkInfo->Signature);
    } else {
        // Otherwise just mark the entry as free and put it back on the chunks free list.
#if DBG
...
*/
{
    ...
    if (RefCount == 0) {
        // Entry has reached a ref count of 0 so mark it as invalid and remove it from its active list.
