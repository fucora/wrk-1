...
{
    // This is the pnp id guid which is registered by wmi into other device objects' registration info. And requests to the innocent devices
    // are hijacked by wmi so that wmi can complete the request for it.
    // We have the WMIREG_FLAG_REMOVE_GUID set so that the guid is not registered for the wmi device which does not support it.
...
NTSTATUS WmipQueryWmiDataBlock(
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP Irp,
    IN ULONG GuidIndex,
    IN ULONG InstanceIndex,
...
/*
Routine Description:
    This routine is a callback into the driver to query for the contents of all instances of a data block.
    When the driver has finished filling the data block it must call IoWMICompleteRequest to complete the irp.
    The driver can return STATUS_PENDING if the irp cannot be completed immediately.
Arguments:

    DeviceObject is the device whose data block is being queried.
    In the case of the PnPId guid this is the device object of the device on whose behalf the request is being processed.
    ...
    InstanceLengthArray is a pointer to an array of ULONG that returns the lengths of each instance of the data block.
        If this is NULL then there was not enough space in the output buffer to fulfill the request so the irp should be completed with the buffer needed.

    BufferAvail on entry has the maximum size available to write the data blocks.

    Buffer on return is filled with the returned data blocks. Note that each instance of the data block must be aligned on a 8 byte boundary.
*/
{
    ...
    case PnPIdInstanceNamesGuidIndex:
    {
        ...
        if (regEntry != NULL) {
            ...
            if (pDO != NULL) {
                status = WmipPDOToDeviceInstanceName(pDO, &instancePath);
                if (NT_SUCCESS(status)) {
                    sizeNeeded = sizeof(ULONG) + instancePath.Length + 2 * sizeof(WCHAR) +
                        sizeof(USHORT);
    ...
    case MSAcpiInfoGuidIndex:
    {
        ...
        status = RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, L"\\Registry\\Machine\\Hardware\\Description\\System", queryTable, NULL, NULL);
        if (NT_SUCCESS(status)) {
    ...
    {
        sizeNeeded = BufferAvail;
        status = WmipGetRawMCAInfo(Buffer, &sizeNeeded);
        if (NT_SUCCESS(status)) {
            *InstanceLengthArray = sizeNeeded;
        }
        break;
    ...
    status = IoWMICompleteRequest((PWMILIB_INFO)&WmipWmiLibInfo, DeviceObject, Irp, status, sizeNeeded, IO_NO_INCREMENT);
    return(status);
}


BOOLEAN WmipFindGuid(IN PGUIDREGINFO GuidList, IN ULONG GuidCount, IN LPGUID Guid, OUT PULONG GuidIndex, OUT PULONG InstanceCount)
/*
Routine Description:
    This routine will search the list of guids registered and return the index for the one that was registered.
Arguments:
...
/*
Routine Description:
    Dispatch routine for IRP_MJ_SYSTEM_CONTROL.
    This routine will process all wmi requests received,
    forwarding them if they are not for this driver or determining if the guid is valid and
    if so passing it to the driver specific function for handing out wmi requests.
Arguments:
...
*/
{
    ...
    if ((minorFunction != IRP_MN_REGINFO) &&
        ...
        if (NT_SUCCESS(status) &&
            ...
            if (((((PWNODE_HEADER)buffer)->Flags) & WNODE_FLAG_STATIC_INSTANCE_NAMES) == 0) {
    ...
    case IRP_MN_QUERY_ALL_DATA:
    {
        ...
        if (bufferSize < FIELD_OFFSET(WNODE_ALL_DATA, OffsetInstanceDataAndLength)) {
            // The buffer should never be smaller than the size of WNODE_ALL_DATA, 
            // however if it is then return with an error requesting the minimum sized buffer.
            WmipAssert(FALSE);
        ...
        // If this is the pnp id guid then we need to get the instance count from the regentry for the device and switch the device object.
        ...
        if (dataBlockOffset <= bufferSize) {
            ...
            // There is not enough room in the WNODE to complete the query
...
/*
Routine Description:
    This routine will do the work of completing a WMI irp.
    Depending upon the the WMI request this routine will fixup the returned WNODE appropriately.
Arguments:
    WmiLibInfo has the WMI information control block
    DeviceObject - Supplies a pointer to the device object for this request.
    Irp - Supplies the Irp making the request.
    Status has the return status code for the IRP
    BufferUsed has the number of bytes needed by the device to return the data requested in any query.
       In the case that the buffer passed to the device is too small this has the number of bytes needed for the return data.
       If the buffer passed is large enough then this has the number of bytes actually used by the device.
