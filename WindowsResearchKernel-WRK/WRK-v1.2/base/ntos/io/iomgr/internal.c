...
*/
{
    ...
    while (deviceObject) {
        if (deviceObject->ReferenceCount || deviceObject->AttachedDevice || deviceObject->DeviceObjectExtension->ExtensionFlags & (DOE_DELETE_PENDING | DOE_REMOVE_PENDING)) {
            unload = FALSE;
            break;
        }
...
#define SynchronousIo( Irp, FileObject ) (  \
    ...
    if (!NT_ERROR(irp->IoStatus.Status) || (NT_ERROR(irp->IoStatus.Status) && irp->PendingReturned && !SynchronousIo(irp, fileObject))) {
        PVOID port = NULL;
        PVOID key = NULL;
...
#define MESSAGE_SIZE    ( (2 * sizeof( FILE_VOLUMEID_WITH_TYPE )) + \
    ...
    if (!IopLinkTrackingServiceObject) {
        ...
        if (KeReadStateEvent(IopLinkTrackingServiceEvent)) {
            ...
            status = NtConnectPort(&serviceHandle, &portName, &dynamicQos, (PPORT_VIEW)NULL, (PREMOTE_PORT_VIEW)NULL, &maxMessageLength, (PVOID)NULL, (PULONG)NULL);
...
*/
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        // The driver node does not specify an object name, so determine what the default name for the driver object should be based on the information in the key.
...
*/
{
    ...
    do {
        ...
        if (ExReadyForErrors) {
            (VOID)ExRaiseHardError(hardErrorPacket->ErrorStatus, parameterPresent, parameterPresent, parameterPresent ? &errorParameter : NULL, OptionOkNoWait, &errorResponse);
        }
...
*/
{
    ...
    if (fastIoDispatch && fastIoDispatch->FastIoDeviceControl) {
        if (fastIoDispatch->FastIoDeviceControl(
            SourceFile,
            TRUE,
            (PVOID)&target,
            sizeof(target),
...
NTSTATUS IopLoadDriver(IN HANDLE KeyHandle, IN BOOLEAN CheckForSafeBoot, IN BOOLEAN IsFilter, OUT NTSTATUS *DriverEntryStatus)
/*
Routine Description:
    This routine is invoked to load a device or file system driver, either during system initialization, or dynamically while the system is running.
Arguments:
    KeyHandle - Supplies a handle to the driver service node in the registry that describes the driver to be loaded.
...
*/
{
    ...
    *DriverEntryStatus = STATUS_SUCCESS;
    ...
    if (!NT_SUCCESS(status)) {
...
#if defined(_WIN64)
            // If this is a driver meant for another architecture, then block this driver and continue with loading the rest of the drivers stack.
        ...
        status = ObReferenceObjectByHandle(driverHandle, 0, IoDriverObjectType, KeGetPreviousMode(), (PVOID *)&driverObject, (POBJECT_HANDLE_INFORMATION)NULL);
    ...
    *DriverEntryStatus = status;
    ...
    if (dtime > 50) {
        DbgPrint("IOLOAD: Driver %wZ took %d.%ds to %s\n", &driverName, dtime / 10, dtime % 10, NT_SUCCESS(status) ? "initialize" : "fail initialization");
    }
#endif
...
*/
{
    ...
    DeviceObject->ReferenceCount--;
    if (!DeviceObject->ReferenceCount && (AlwaysUnload || DeviceObject->DeviceObjectExtension->ExtensionFlags & (DOE_DELETE_PENDING | DOE_UNLOAD_PENDING | DOE_REMOVE_PENDING))) {
...
*/
{
    ...
    // Set the final status of the load or unload operation, and indicate to the caller that the operation is now complete.
...
*/
{
    ...
    if (!DeviceLockAlreadyHeld) {
        status = KeWaitForSingleObject(&DeviceObject->DeviceLock, Executive, KeGetPreviousModeByThread(&CurrentThread->Tcb), Alertable, (PLARGE_INTEGER)NULL);
    ...
    if ((DeviceObject->Vpb->Flags & (VPB_MOUNTED | VPB_REMOVE_PENDING)) == 0) {
        ...
        for (entry = queueHeader->Flink; entry != queueHeader && !NT_SUCCESS(status); entry = entry->Flink) {
            ...
            // If this is the final entry (Raw file system), and it is also not the first entry, and a raw mount is not permitted, then break out of the loop at this point, as this volume cannot be mounted for the caller's purposes.
            ...
            // The number of stack locations that must be allocated in the IRP must include one for the file system itself, and then one for each driver that is attached to it.
            ...
            if (NT_SUCCESS(ioStatus.Status)) {
                ...
                *Vpb = IopMountInitializeVpb(DeviceObject, attachedDevice, rawMountOnly);
                ...
                // If the error wasn't STATUS_UNRECOGNIZED_VOLUME, and this request is not going to the Raw file system, then there is no reason to continue looping.
...
*/
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        irpSp = IoGetNextIrpStackLocation(Irp);
        if (irpSp->Parameters.SetFile.FileInformationClass == FileLinkInformation && !renameBuffer->ReplaceIfExists && ioStatus.Information == FILE_EXISTS) {
    ...
    */
{
    ...
    if (synchronousIo) {
        if (status == STATUS_PENDING) {
            status = KeWaitForSingleObject(&FileObject->Event, Executive, Mode, (BOOLEAN)((FileObject->Flags & FO_ALERTABLE_IO) != 0), (PLARGE_INTEGER)NULL);
...
*/
{
    ...
    // Start by determining the size of the DeviceName, and allocate enough storage for both the ObjectName structure and the string ObQueryNameString( realDeviceObject, NULL, 0, &length );
    ...
    if (ExReadyForErrors) {
        ...
        status = ExRaiseHardError(irp->IoStatus.Status, numberOfParameters, parameterMask, parameters, OptionCancelTryContinue, &response);
        if (attached) {
            KeDetachProcess();
        }
    } else {
...
*/
{
    ...
    irql = KeAcquireQueuedSpinLock(LockQueueIoDatabaseLock);
    if (DriverObject->Flags & DRVO_UNLOAD_INVOKED || !deviceObject || !(deviceObject->DeviceObjectExtension->ExtensionFlags & DOE_UNLOAD_PENDING)) {
        KeReleaseQueuedSpinLock(LockQueueIoDatabaseLock, irql);
...
*/
{
    ...
    status = ObReferenceObjectByHandle(FileHandle, SetEa ? FILE_WRITE_EA : FILE_WRITE_DATA, IoFileObjectType, requestorMode, (PVOID *)&fileObject, NULL);
    if (!NT_SUCCESS(status)) {
        return status;
    }
...
*/
{
    ...
    if (RequestorMode != KernelMode) {
        try {
            ...
            if (!trackingInfo->DestinationFile || ((Length - FIELD_OFFSET(FILE_TRACKING_INFORMATION, ObjectInformation)) < trackingInfo->ObjectInformationLength)) {
    ...
    if (trackingInfo->DestinationFile) {
        status = ObReferenceObjectByHandle(trackingInfo->DestinationFile, FILE_WRITE_DATA, IoFileObjectType, RequestorMode, (PVOID *)&dstFileObject, NULL);
        if (!NT_SUCCESS(status)) {
            if (RequestorMode != KernelMode) {
                ExFreePool(trackingInfo);
            }
            ...
            if (trackingInfo->DestinationFile) {
                if (IsFileLocal(dstFileObject)) {
                    ...
                    if (status == STATUS_DUPLICATE_NAME || status == STATUS_OBJECT_NAME_COLLISION) {
                        ...
                        if (NT_SUCCESS(status)) {
                            // Write the birth ID
                            status = IopGetSetObjectId(dstFileObject, &CrossVolumeObjectId.ExtendedInfo[0], sizeof(CrossVolumeObjectId.ExtendedInfo), FSCTL_SET_OBJECT_ID_EXTENDED);
                        }
                    }
                    ...
                    if (!NT_SUCCESS(status)) {
                        ...
                        if (IdSetOnTarget) {
                            if (RtlEqualMemory(&TargetObjectId.ObjectId, &SourceObjectId.ObjectId, sizeof(TargetObjectId.ObjectId))) {
                                ...
                                statusT = IopGetSetObjectId(dstFileObject, &TargetObjectId.ExtendedInfo[0], sizeof(TargetObjectId.ExtendedInfo), FSCTL_SET_OBJECT_ID_EXTENDED);
                            }
                        }
                    ...
                    IopMarshalIds(&trackingBuffer, &TargetVolumeId, &TargetObjectId, trackingInfo);
                    status = IopTrackLink(FileObject, IoStatusBlock, &trackingBuffer.TrackingInformation, FIELD_OFFSET(FILE_TRACKING_INFORMATION, ObjectInformation) + trackingBuffer.TrackingInformation.ObjectInformationLength, Event, KernelMode);
                    if (!NT_SUCCESS(status)) {
                        leave;
                    }
...
Caution:
    ...
    Should this code ever need to directly access any of the other parameters other than Apc, this routine will need to be split into two separate routines.
...
Arguments:
    ...
    InputBuffer - Optionally supplies an input buffer to be passed to the driver.  Whether or not the buffer is actually optional is dependent on the IoControlCode.
    InputBufferLength - Length of the InputBuffer in bytes.
    OutputBuffer - Optionally supplies an output buffer to receive information from the driver.  Whether or not the buffer is actually optional is dependent on the IoControlCode.
...
*/
{
    ...
    if (DeviceIoControl) {
        ...
        if (fastIoDispatch && fastIoDispatch->FastIoDeviceControl) {
            ...
            if (fastIoDispatch->FastIoDeviceControl(fileObject, TRUE, InputBuffer, InputBufferLength, OutputBuffer, OutputBufferLength, IoControlCode, &localIoStatus, deviceObject)) {
                PVOID port;
                PVOID key;
    ...
    case METHOD_OUT_DIRECT:
        ...
        try {
            ...
            if (OutputBufferLength != 0) {
                ...
                IopProbeAndLockPages(irp->MdlAddress, requestorMode, (LOCK_OPERATION)((method == 1) ? IoReadAccess : IoWriteAccess), deviceObject, *majorFunction);
            }
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
...
Arguments:
    ...
    Buffer - Supplies a pointer to a unicode character buffer that will receive the bus name.
        Since this buffer is used in an intermediate step to retrieve a KEY_VALUE_FULL_INFORMATION structure, it must be large enough to contain this structure (including the longest value name & data length under KeyHandle).
    Length - Supplies the length, in bytes, of the Buffer.
...
*/
{
    ...
    sprintf(AnsiTimeBuffer, "%2d %2d %4d %02d:%02d:%02d.%03d\r\n",
            TimeFields.Month, TimeFields.Day, TimeFields.Year, TimeFields.Hour, TimeFields.Minute, TimeFields.Second, TimeFields.Milliseconds);
...
/*
Routine Description:
    This routine processes the boot driver reinitialization list.  It calls each entry and then removes it from the list.
Returns:
...
*/
{
    ...
    // Walk the list reinitialization list in case this driver, or some other driver, has requested to be invoked at a re-initialization entry point.
...
*/
{
    ...
    if (fastIoDispatch && fastIoDispatch->FastIoQueryBasicInfo) {
        queryResult = fastIoDispatch->FastIoQueryBasicInfo(FileObject, (FileObject->Flags & FO_SYNCHRONOUS_IO) ? TRUE : FALSE, BasicInformationBuffer, &localIoStatus, deviceObject);
        if (queryResult) {
            return (localIoStatus.Status);
        }
    }
    ...
    status = IopGetFileInformation(FileObject, sizeof(FILE_BASIC_INFORMATION), FileBasicInformation, BasicInformationBuffer, &lengthNeeded);
    return status;
}
