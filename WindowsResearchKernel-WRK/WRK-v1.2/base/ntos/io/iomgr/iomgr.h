...
#define IopProfileIrpStackCount(StackSize)  ((StackSize < MAX_LOOKASIDE_IRP_STACK_COUNT) ? IopIrpStackProfiler.Profile[StackSize]++ : 0)
...
#define IopIrpProfileStackCountEnabled()     ((IopIrpStackProfiler.Flags & (IOP_PROFILE_STACK_COUNT|IOP_ENABLE_AUTO_SIZING)) == (IOP_PROFILE_STACK_COUNT|IOP_ENABLE_AUTO_SIZING))
...
NTSTATUS IopSendMessageToTrackService(IN PLINK_TRACKING_INFORMATION SourceVolumeId, IN PFILE_OBJECTID_BUFFER SourceObjectId, IN PFILE_TRACKING_INFORMATION TargetObjectInformation);
#endif
...
*/
{
    ...
    irpSp->DeviceObject = DeviceObject;// Save a pointer to the device object for this request so that it can be used later in completion.
...
PDEVICE_OBJECT IopAttachDeviceToDeviceStackSafe( IN PDEVICE_OBJECT SourceDevice, IN PDEVICE_OBJECT TargetDevice, OUT PDEVICE_OBJECT *AttachedToDeviceObject OPTIONAL);
