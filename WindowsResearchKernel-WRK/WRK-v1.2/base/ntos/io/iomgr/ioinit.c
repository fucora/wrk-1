...
// Define the default number of IRP that can be in progress and allocated from a lookaside list.
...
VOID IopInitializeData(VOID);// Define external procedures not in common header files
...
NTSTATUS IopCreateArcNamesDisk(IN  PLOADER_PARAMETER_BLOCK LoaderBlock, IN  BOOLEAN SingleBiosDiskFound, OUT BOOLEAN *BootDiskFound);
NTSTATUS IopCreateArcNamesCd(IN  PLOADER_PARAMETER_BLOCK LoaderBlock);
...
*/
{
    ...
    // Initialize the I/O database resource, lock, and the file system and network file system queue headers.
    // Also allocate the cancel spin lock.
    ntDeviceName.Buffer = ExAllocatePoolWithTag(NonPagedPool, DEVICENAME_BUFFER_LENGTH + VALUE_BUFFER_LENGTH, 'oI');
    ...
    IopSetIoRoutines();
    // Initialize the unique device object number counter used by IoCreateDevice when automatically generating a device object name.
    IopUniqueDeviceObjectNumber = 0;

    // Initialize the large I/O Request Packet (IRP) lookaside list head and the mutex which guards the list.
    ...
    ExInitializeSystemLookasideList(&IopLargeIrpLookasideList, NonPagedPool, largePacketSize, 'lprI', largeIrpZoneSize, &ExSystemLookasideListHead);

    // Initialize the system small IRP lookaside list.
    smallPacketSize = (ULONG)(sizeof(IRP) + sizeof(IO_STACK_LOCATION));
    ExInitializeSystemLookasideList(&IopSmallIrpLookasideList, NonPagedPool, smallPacketSize, 'sprI', smallIrpZoneSize, &ExSystemLookasideListHead);

    // Initialize the system MDL lookaside list.
    mdlPacketSize = (ULONG)(sizeof(MDL) + (IOP_FIXED_SIZE_MDL_PFNS * sizeof(PFN_NUMBER)));
    ExInitializeSystemLookasideList(&IopMdlLookasideList, NonPagedPool, mdlPacketSize, ' ldM', mdlZoneSize, &ExSystemLookasideListHead);

    lookasideIrpLimit /= KeNumberProcessors;// Compute the lookaside IRP float credits per processor.

    // Initialize the per processor nonpaged lookaside lists and descriptors.

    // N.B. All the I/O related lookaside list structures are allocated at one time to make sure they are aligned, if possible, and to avoid pool overhead.
    ...
    for (Index = 0; Index < (ULONG)KeNumberProcessors; Index += 1) {
        ...
        prcb->LookasideIrpFloat = lookasideIrpLimit;// Set the per processor IRP float credits.
        ...
        if (lookaside != NULL) {
            ExInitializeSystemLookasideList(lookaside, NonPagedPool, sizeof(IOP_MINI_COMPLETION_PACKET), 'PpcI', completionZoneSize, &ExSystemLookasideListHead);
        ...
        if (lookaside != NULL) {
            ExInitializeSystemLookasideList(lookaside, NonPagedPool, largePacketSize, 'LprI', largeIrpZoneSize, &ExSystemLookasideListHead);
        ...
        if (lookaside != NULL) {
            ExInitializeSystemLookasideList(lookaside, NonPagedPool, smallPacketSize, 'SprI', smallIrpZoneSize, &ExSystemLookasideListHead);
        ...
        if (lookaside != NULL) {
            ExInitializeSystemLookasideList(lookaside, NonPagedPool, mdlPacketSize, 'PldM', mdlZoneSize, &ExSystemLookasideListHead);
    ...
    if (NT_SUCCESS(status)) {
        ...
        status = NtQueryValueKey(handle, &LocalStrings.startTypeName, KeyValuePartialInformation, valueBuffer, VALUE_BUFFER_LENGTH, &Index);
    ...
    // Initialize the timer database and start the timer DPC routine firing so that drivers can use it during initialization.
    ...
    InitializeObjectAttributes(&objectAttributes, &LocalStrings.eventName, OBJ_PERMANENT | OBJ_KERNEL_HANDLE, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);
    ...
    PoInitDriverServices(0);// Call Power manager to initialize for drivers
    ...
    // Save this for use during PnP enumeration -- we NULL it out later before LoaderBlock is reused.
    ...
    // Once we have initialized the boot drivers, we don't need the copy of the pointer to the loader block any more.
    IopLoaderBlock = NULL;
    ...
    // Do last known good boot processing. If this is a last known good boot, we will copy over the last known good drivers and files.
    // Otherwise we will ensure this boot doesn't taint our last good info (in case we crash before the boot is marked good).
    // Note that loading of the correct boot drivers was handled by the boot loader, who chose an LKG boot in the first place.
    PpLastGoodDoBootProcessing();

    // Save the current value of the NT Global Flags and enable kernel debugger symbol loading while drivers are being loaded so that systems can be debugged regardless of whether they are free or checked builds.
    ...
    CcPfBeginBootPhase(PfSystemDriverInitPhase);// Notify the boot prefetcher of boot progress.
    ...
    status = RtlUnicodeStringToAnsiString(&LocalStrings.ansiString, &NtSystemRoot, FALSE);
    if (!NT_SUCCESS(status)) {
    ...
    IoAssignDriveLetters(LoaderBlock, &ntDeviceName, (PUCHAR)LocalStrings.ansiString.Buffer, &LocalStrings.ansiString);
    status = RtlAnsiStringToUnicodeString(&NtSystemRoot, &LocalStrings.ansiString, FALSE);
    if (!NT_SUCCESS(status)) {
    ...
    NtGlobalFlag = oldNtGlobalFlag;// Also restore the NT Global Flags to their original state.
    WMIInitialize(1, NULL);// Let WMI have a second chance to initialize, now that all drivers are started and should be ready to get WMI irps
    PoInitDriverServices(1);// Call Power manager to initialize for post-boot drivers
    return TRUE;// Indicate that the I/O system successfully initialized itself.
}
...
NTSTATUS IopFetchConfigurationInformation(IN OUT PWSTR *ListTarget, IN GUID ClassGuid, IN ULONG ExpectedNumber, OUT ULONG *NumberFound)
{
    NTSTATUS    status;
    ULONG       count = 0;
    wchar_t *   pNameList;
    ...
    if (!NT_SUCCESS(status)) {
        ...
        // if the disk returned by PNP are not all the disks in the system it means that some legacy driver has generated a disk device object/link.
        // In that case we need to enumerate all pnp disks and then using the legacy for-loop also enumerate the non-pnp disks
        *NumberFound = count;
...
Return Value:
    STATUS_INSUFFICIENT_RESOURCES if we can't allocate memory, otherwise SUCCESS.
*/
{
    ...
    status = IopFetchConfigurationInformation(&diskList, DiskClassGuid, totalDriverDisksFound, &totalPnpDisksFound);
    if (!NT_SUCCESS(status)) {
    ...
    // In order to deal with the case of disk dissappearing before we get to this point (due to a failed start on one of many disks present in the system) we ask PNP for a list of all the currenttly active disks in the system.
    // If the number of disks returned is less than the IoGetConfigurationInformation()->DiskCount, then we have legacy disks that we need to enumerate in the for loop.
    // In the legacy case, the ending condition for the loop is NOT the total disk on the system but an arbitrary number of the max total legacy disks expected in the system..
    ...
    if (useLegacyEnumerationDisk && (totalPnpDisksFound == 0)) {
        totalDriverDisksFound += 20;// search up to a maximum arbitrary number of legacy disks
    }
    ...
    for (diskNumber = 0; diskNumber < totalDriverDisksFound; diskNumber++) {
        ...
        if (pDiskNameList && (*pDiskNameList != L'\0')) {
            ...
            if (NT_SUCCESS(status)) {
                // since PNP gave us just a sym link we have to retrieve the actual disk number through an IOCTL call to the disk stack.
            ...
            if (useLegacyEnumerationDisk && (*pDiskNameList == L'\0')) {
                // end of pnp disks if there are any legacy disks following we need to update the total disk found number to cover the maximum disk number a legacy disk could be at. (in a sparse name space)
            ...
            pnpDiskDeviceNumber.DeviceNumber = 0xFFFFFFFF;// set the pnpDiskNumber value so its not used.
        }
        ...
        status = IoReadPartitionTableEx(deviceObject, &driveLayout);// Get partition information for this disk.
        if (!NT_SUCCESS(status)) {
            ObDereferenceObject(fileObject);
            continue;
        }
        ...
        // Check to see if EZ Drive is out there on this disk.
        // If it is then zero out the signature in the drive layout since this will never be written by anyone AND change to offset to actually read sector 1 rather than 0 cause that's what the loader actually did.
        offset.QuadPart = 0;
        ...
        buffer = ExAllocatePool(NonPagedPoolCacheAligned, diskGeometry.BytesPerSector);
        if (buffer) {
            ...
            if ((SingleBiosDiskFound &&
                ...
                for (partitionNumber = 0; partitionNumber < driveLayout->PartitionCount; partitionNumber++) {
                    ...
                    if (RtlEqualString(&arcNameString, &arcSystemDeviceString, TRUE)) {
                        // We've found the system partition--store it away in the registry to later be transferred to a application-friendly location.
                    ...
                    IoCreateSymbolicLink(&arcNameUnicodeString, &deviceNameUnicodeString);// Create symbolic link between NT device name and ARC name.
                    RtlFreeUnicodeString(&arcNameUnicodeString);
                    RtlFreeUnicodeString(&deviceNameUnicodeString);
                }
            } else {
                ...
                if (diskBlock->Signature == diskSignature && (diskBlock->CheckSum + checkSum) != 0 && diskBlock->ValidPartitionTable) {
...
*/
{
    ...
    // ask PNP to give us a list with all the currently active disks
    status = IopFetchConfigurationInformation(&cdRomList, CdRomClassGuid, totalDriverCdRomsFound, &totalPnpCdRomsFound);
    if (!NT_SUCCESS(status)) {
    ...
    if (diskBlock) {
        // This could be a CdRom boot.
        // Search all of the NT CdRoms to locate a checksum match on the diskBlock found.
        // If there is a match, assign the ARC name to the CdRom.
        irp = NULL;
        buffer = ExAllocatePool(NonPagedPoolCacheAligned, 2048);
        if (buffer) {
            // Construct the NT names for CdRoms and search each one for a checksum match.
            // If found, create the ARC Name symbolic link.
            totalDriverCdRomsFound = max(totalPnpCdRomsFound, totalDriverCdRomsFound);
            currentDiskNumber = 0;
            if (useLegacyEnumerationCdRom && (totalPnpCdRomsFound == 0)) {
                totalDriverCdRomsFound += 5;// search up to a maximum arbitrary number of legacy CdRoms
            }

            for (diskNumber = 0; diskNumber < totalDriverCdRomsFound; diskNumber++) {
                ...
                if (pCdRomNameList && (*pCdRomNameList != L'\0')) {
                    ...
                    sprintf(deviceNameBuffer, "\\Device\\CdRom%d", pnpDiskDeviceNumber.DeviceNumber);
                    RtlInitAnsiString(&deviceNameString, deviceNameBuffer);
                    status = RtlAnsiStringToUnicodeString(&deviceNameUnicodeString, &deviceNameString, TRUE);
                    if (!NT_SUCCESS(status)) {
                        if (cdRomList) {
                ...
                } else {// Fall back to legacy devices if we have no more PNP
                    sprintf(deviceNameBuffer, "\\Device\\CdRom%d", currentDiskNumber);
                    currentDiskNumber += 1;
                    RtlInitAnsiString(&deviceNameString, deviceNameBuffer);
                    status = RtlAnsiStringToUnicodeString(&deviceNameUnicodeString, &deviceNameString, TRUE);
                    if (NT_SUCCESS(status)) {
                ...
                if (!(diskBlock->CheckSum + checkSum)) {
                    // This is the boot CdRom.  Create the symlink for the ARC name from the loader block.
...
NTSTATUS IopCreateArcNames(IN PLOADER_PARAMETER_BLOCK LoaderBlock)
/*
Routine Description:
    The loader block contains a table of disk signatures and corresponding ARC names.
    Each device that the loader can access will appear in the table.
    This routine opens each disk device in the system, reads the signature and compares it to the table.
    For each match, it creates a symbolic link between the nt device name and the ARC name.

    The checksum value provided by the loader is the ULONG sum of all elements in the checksum, inverted, plus 1:
    checksum = ~sum + 1;
    This way the sum of all of the elements can be calculated here and added to the checksum in the loader block.
...
*/
{
    ...
    status = IopCreateArcNamesDisk(LoaderBlock, singleBiosDiskFound, &bootDiskFound);
...
/*
Routine Description:
    This routine is invoked to create the object manager directory objects to contain the various device and file system driver objects.
Return Value:
    The function value is a BOOLEAN indicating whether or not the directory objects were successfully created.
*/
{
    HANDLE handle;
    OBJECT_ATTRIBUTES objectAttributes;
    ...
    RtlInitUnicodeString(&nameString, L"\\Driver");
    InitializeObjectAttributes(&objectAttributes, &nameString, OBJ_PERMANENT | OBJ_KERNEL_HANDLE, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);
...
*/
{
    ...
    InitializeObjectAttributes(ObjectAttributes, ObjectName, OBJ_PERMANENT | OBJ_CASE_INSENSITIVE, (HANDLE)NULL, (PSECURITY_DESCRIPTOR)NULL);
...
NTSTATUS IopInitializeBuiltinDriver(IN PUNICODE_STRING DriverName,
    ...
    Return Value:
        The function returns a pointer to a DRIVER_OBJECT if the built-in driver successfully initialized.  Otherwise, a value of NULL is returned.
    */
{
    HANDLE handle;
    PDRIVER_OBJECT driverObject;
    ...
    *Result = NULL;
    ...
    HeadlessKernelAddLogEntry(HEADLESS_LOG_LOADING_FILENAME, DriverName);// Log the file name
    ...
    status = ObReferenceObjectByHandle(handle, 0, IoDriverObjectType, KernelMode, (PVOID *)&tmpDriverObject, (POBJECT_HANDLE_INFORMATION)NULL);
    ASSERT(status == STATUS_SUCCESS);

    // Fill in the DriverSection so the image will be automatically unloaded on failures. We should use the entry from the PsModuleList.
    ...
    InbvIndicateProgress();// The boot process takes a while loading drivers.   Indicate that progress is being made.
    ...
    // Save the name of the driver so that it can be easily located by functions such as error logging.
    ...
    // Save the name of the service key so that it can be easily located by PnP mamager.
...
#if DBG
    // If DriverInit took longer than 5 seconds or the driver did not load, print a message.
...
/*
Routine Description:
    This routine is invoked to locate and mark the boot partition device object as a boot device so that subsequent operations can fail more cleanly and 
    with a better explanation of why the system failed to boot and run properly.
Arguments:
    LoaderBlock - Supplies a pointer to the loader parameter block created by the OS Loader during the boot process.
                    This structure contains the various system partition and boot device names and paths.
Return Value:
...
*/
{
    ...
    // Open the ARC boot device object. The boot device driver should have created the object.
    ...
    InitializeObjectAttributes(&objectAttributes, &deviceNameUnicodeString, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
    ...
    fileObject->DeviceObject->Flags |= DO_SYSTEM_BOOT_PARTITION;// Mark the device object represented by the file object.

    // Save away the characteristics of boot device object for later use in WinPE mode
    if (InitIsWinPEMode) {
...
/*
Routine Description:
    This routine is invoked to reassign \SystemRoot from being an ARC path name to its NT path name equivalent.
    This is done by looking up the ARC device name as a symbolic link and determining which NT device object is referred to by it.
    The link is then replaced with the new name.
Arguments:
    LoaderBlock - Supplies a pointer to the loader parameter block created by the OS Loader during the boot process.
                    This structure contains the various system partition and boot device names and paths.
...
*/
{
    ...
    // Open the ARC boot device symbolic link object. The boot device driver should have created the object.
    sprintf(deviceNameBuffer, ArcNameFmt, LoaderBlock->ArcBootDeviceName);
    RtlInitAnsiString(&deviceNameString, deviceNameBuffer);
    status = RtlAnsiStringToUnicodeString(&deviceNameUnicodeString, &deviceNameString, TRUE);
    if (!NT_SUCCESS(status)) {
    ...
    InitializeObjectAttributes(&objectAttributes, &linkUnicodeString, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
    status = NtOpenSymbolicLinkObject(&linkHandle, SYMBOLIC_LINK_ALL_ACCESS, &objectAttributes);
    if (!NT_SUCCESS(status)) {
        return FALSE;
    }
    ...
    RtlInitAnsiString(&deviceNameString, deviceNameBuffer);
    InitializeObjectAttributes(&objectAttributes, &linkUnicodeString, OBJ_CASE_INSENSITIVE | OBJ_PERMANENT | OBJ_KERNEL_HANDLE, NULL, NULL);
...
Arguments:
    NtSystemPartitionDeviceName - supplies the NT device name of the system partition.
        This is the \Device\Harddisk<n>\Partition<m> name, which used to be the actual device name, but now is a symbolic link to a name of the form \Device\Volume<x>.
    ...
    OsLoaderPathName - supplies the path (on the partition specified in the 1st parameter) where the OS loader is located.
        Upon return, this path will have had its trailing backslash removed (if present, and path isn't root).
*/
{
    NTSTATUS status;
    HANDLE linkHandle;
    ...
    InitializeObjectAttributes(&objectAttributes, NtSystemPartitionDeviceName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
    ...
    ObCloseHandle(linkHandle, KernelMode);// We don't need the handle to the symbolic link any longer.
    if (!NT_SUCCESS(status)) {
#if DBG
    ...
    volumeNameString.Buffer[volumeNameString.Length / sizeof(WCHAR)] = L'\0';// Make sure the volume name string is null-terminated.
    status = IopOpenRegistryKeyEx(&systemHandle, NULL, &CmRegistryMachineSystemName, KEY_ALL_ACCESS);// Open HKLM\SYSTEM key.
    if (!NT_SUCCESS(status)) {
#if DBG
...
Arguments:
    SequenceNumber - A value that is unique to an IRP over the life of the irp in this driver. - 0 generally means an error not associated with an IRP
    UniqueErrorValue - A unique long word that identifies the particular call to this function.
    FinalStatus - The final status given to the irp that was associated with this error.
                    If this log entry is being made during one of the retries this value will be STATUS_SUCCESS.
    SpecificIOStatus - The IO status for a particular error.
    LengthOfInsert1 - The length in bytes (including the terminating NULL) of the first insertion string.
    Insert1 - The first insertion string.
    LengthOfInsert2 - The length in bytes (including the terminating NULL) of the second insertion string.
                        NOTE, there must be a first insertion string for their to be a second insertion string.
...
Arguments:
    Allocator - Pointer to the reserve IRP allocator structure. created by the OS Loader.
Return Value:
    The function value is a BOOLEAN indicating whether or not the reserver allocator was successfully initialized.
*/
{
