...
// The library exposes everything with the name "Wdmlib".
// This ensures drivers using the backward compatible Cancel DDI Lib won't opportunistically pick up the kernel exports just because they were built using the XP DDK.
#if CSQLIB
...
Routine Description:
    ...
    It's expected that this routine will be called from a timer or DPC or other threads which complete an IO.
    Note that the IRP associated with this context could already have been freed.
Arguments:
    Csq - Pointer to the cancel queue.
    Context - Context associated with Irp.
Return Value:
    Returns the IRP associated with the context.
    If the value is not NULL, the IRP was successfully retrieved and can be used safely.
    If the value is NULL, the IRP was already canceled.
*/
{
    KIRQL   irql;
    ...
    if (irpContext->Type == IO_TYPE_CSQ_IRP_CONTEXT) {
        cfq = irpContext->Csq;
    } else if ((irpContext->Type == IO_TYPE_CSQ) || (irpContext->Type == IO_TYPE_CSQ_EX)) {
...
NTSTATUS CSQLIB_DDI(IoCsqInitialize)(
    IN PIO_CSQ                          Csq,
...
NTSTATUS CSQLIB_DDI(IoCsqInsertIrpEx)(
    IN  PIO_CSQ             Csq,
    IN  PIRP                Irp,
    ...
    Routine Description:
        ...
        It's ok to pass a NULL context if the driver assumes that it will always use IoCsqRemoveNextIrp to remove an IRP.
    Arguments:
        Csq - Pointer to the cancel queue.
    ...
    Return Value:
        NTSTATUS - Status returned by the driver's insert IRP routine.
        If the driver's insert IRP routine returns an error status, the IRP is not inserted and the same status is returned by this API.
        This allows drivers to implement startIo kind of functionality.
    */
{
    KIRQL           irql;
    ...
    if (Irp->Cancel) {
        ...
        if (cancelRoutine) {
            ...
            Csq->CsqReleaseLock(Csq, irql);// The cancel routine beat us to it.
        }
    } else {
        Csq->CsqReleaseLock(Csq, irql);
    }
...
/*
Routine Description:
    This routine removes the next IRP from the queue.
    This routine will enumerate the queue and return an IRP that's not canceled.
    If an IRP in the queue is canceled it goes to the next IRP.
    If no IRP is available it returns a NULL.
    The IRP returned is safe and cannot be canceled.
Arguments:
    Csq - Pointer to the cancel queue.
Return Value:
...
*/
{
    ...
    while (1) {
        // This routine will return a pointer to the next IRP in the queue adjacent to the irp passed as a parameter.
        // If the irp is NULL, it returns the IRP at the head of the queue.
        if (!irp) {
            Csq->CsqReleaseLock(Csq, irql);
            return NULL;
        }
