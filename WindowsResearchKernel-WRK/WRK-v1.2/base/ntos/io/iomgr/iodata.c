...
// The following lock is used to guard access to the CancelRoutine address in IRPs.
// It must be locked to set the address of a routine, clear the address of a routine, when a cancel routine is invoked, or when
// manipulating any structure that will set a cancel routine address in a packet.


// extern KSPIN_LOCK IopCancelSpinLock;


// The following lock is used to guard access to VPB data structures.
// It must be held each time the reference count, mount flag, or device object fields of a VPB are manipulated.


// extern KSPIN_LOCK IopVpbSpinLock;


// The following lock is used to guard access to the I/O system database for unloading drivers.
// It must be locked to increment or decrement device reference counts and to set the unload pending flag in a device object.
// The lock is allocated by the I/O system during phase 1 initialization.

// This lock is also used to decrement the count of Associated IRPs for a given Master IRP.


// extern KSPIN_LOCK IopDatabaseLock;

// The following resource is used to control access to the I/O system's database.
// It allows exclusive access to the file system queue for registering a file system as well as shared access to the same when
// searching for a file system to mount a volume on some media.
// The resource is initialized by the I/O system initialization code during phase 1 initialization.

ERESOURCE IopDatabaseResource;

// The following resource is used to control access while loading a driver to ensure once a driver image has been verified as loaded the driver
// object will be created without another thread failing in the same code.
// The resource is initialized by the I/O system initialization code during phase 1 initialization.
ERESOURCE IopDriverLoadResource;

// The following resource is used to control access to security descriptors on devices.
// It allows multiple readers to perform security checks and queries on device security, but only a single writer to modify the security on a device at a time.
ERESOURCE IopSecurityResource;

// The following queue header contains the list of disk file systems currently loaded into the system.
// The list actually contains the device objects for each of the file systems in the system.
// Access to this queue is protected using the IopDatabaseResource for exclusive (write) or shared (read) access locks.
LIST_ENTRY IopDiskFileSystemQueueHead;

// The following queue header contains the list of CD ROM file systems currently loaded into the system.
// The list actually contains the device objects for each of the file systems in the system.
// Access to this queue is protected using the IopDatabaseResource for exclusive (write) or shared (read) access locks.
LIST_ENTRY IopCdRomFileSystemQueueHead;


// The following queue header contains the list of network file systems (redirectors) currently loaded into the system.
// The list actually contains the device objects for each of the network file systems in the system.
// Access to this queue is protected using the IopDatabaseResource for exclusive (write) or shared (read) access locks.
LIST_ENTRY IopNetworkFileSystemQueueHead;

// The following queue header contains the list of tape file systems currently loaded into the system.
// The list actually contains the device objects for each of the file systems in the system.
// Access to this queue is protected using the IopDatabaseResource for exclusive (write) or shared (read) access locks.
LIST_ENTRY IopTapeFileSystemQueueHead;

// The following queue header contains the list of boot drivers that have registered for a call back once all devices have been enumerated.
LIST_ENTRY IopBootDriverReinitializeQueueHead;

// The following queue header contains the list of drivers that have registered reinitialization routines.
LIST_ENTRY IopDriverReinitializeQueueHead;

// The following queue headers contain the lists of the drivers that have registered shutdown notification routines.
LIST_ENTRY IopNotifyShutdownQueueHead;
LIST_ENTRY IopNotifyLastChanceShutdownQueueHead;

// The following queue header contains the list of the driver that have registered to be notified when a file system registers or unregisters itself as an active file system.
LIST_ENTRY IopFsNotifyChangeQueueHead;

// The following are the lookaside lists used to keep track of the two I/O Request Packet (IRP), the Memory Descriptor List (MDL) Lookaside list, and the I/O Completion List (ICP) Lookaside list.

// The "large" IRP contains 4 stack locations, the maximum in the SDK, and the "small" IRP contains a single entry, the most common case for devices other than disks and network devices.
...
// The following spinlock is used to control access to the I/O system's error log database.
// It is initialized by the I/O system initialization code when the system is being initialized.
// This lock must be owned in order to insert or remove entries from either the free or entry queue.


// extern KSPIN_LOCK IopErrorLogLock;


// The following is the list head for all error log entries in the system which have not yet been sent to the error log process.
// Entries are written placed onto the list by the IoWriteElEntry procedure.
LIST_ENTRY IopErrorLogListHead;


// The following is used to track how much memory is allocated to I/O error log packets.
// The spinlock is used to protect this variable.


LONG IopErrorLogAllocation;
// extern KSPIN_LOCK IopErrorLogAllocationLock;

// The following spinlock is used by the I/O system to synchronize examining the thread field of an I/O Request Packet so that the request can be queued as a special kernel APC to the thread.
// The reason that the spinlock must be used is for cases when the request times out, and so the thread has been permitted to possibly exit.

// extern KSPIN_LOCK IopCompletionLock;

// The following global contains the queue of informational hard error pop-ups.
IOP_HARD_ERROR_QUEUE IopHardError;

// The following global is non-null when there is a pop-up on the screen waiting for user action.  It points to that packet.
PIOP_HARD_ERROR_PACKET IopCurrentHardError;


// The following are used to implement the I/O system's one second timer.
// The lock protects access to the queue, the queue contains an entry for each driver that needs to be invoked, and the timer and DPC data structures are used to actually get the internal timer routine invoked once every second.
// The count is used to maintain the number of timer entries that actually indicate that the driver is to be invoked.
...
// The following are the global pointers for the Object Type Descriptors that are created when each of the I/O specific object types are created.
...
// The following is a global lock and counters for I/O operations requested on a system-wide basis.
// The first three counters simply track the number of read, write, and other types of operations that have been requested.
// The latter three counters track the actual number of bytes that have been transferred throughout the system.
...
// The following is the base pointer for the crash dump control block that is used to control dumping all of physical memory to the paging file after a system crash.
// And, the checksum for the dump control block is also declared.
PDUMP_CONTROL_BLOCK IopDumpControlBlock;
ULONG IopDumpControlBlockChecksum;

// The following are the spin lock and event that allow the I/O system to implement fast file object locks.
KEVENT IopFastLockEvent;


// The following is a monotonically increasing number (retrieved via InterlockedIncrement) that is used by IoCreateDevice to automatically generate a device object name when the FILE_AUTOGENERATED_DEVICE_NAME device characteristic is specified.
...
IOP_RESERVE_IRP_ALLOCATOR  IopReserveIrpAllocator;// Reserve IRP allocator for paging reads.

#if defined(REMOTE_BOOT)
// The following indicates whether or not the Client Side Caching subsystem was successfully initialized.
...
// Note:  All of the following data is potentially pageable, depending on the target platform.
...
// The following array specifies the minimum length of the FileInformation buffer for an NtQueryInformationFile service.

// WARNING:  This array depends on the order of the values in the FileInformationClass enumerated type.
//           Note that the enumerated type is one-based and the array is zero-based.
...
// The following array specifies the minimum length of the FileInformation buffer for an NtSetInformationFile service.
...
// The following array specifies the alignment requirement of both all query and set operations, including directory operations, but not FS operations.
...
// The following array specifies the required access mask for the caller to access information in an NtQueryXxxFile service.
...
// The following array specifies the required access mask for the caller to access information in an NtSetXxxFile service.

// WARNING:  This array depends on the order of the values in the FILE_INFORMATION_CLASS enumerated type.
//           Note that the enumerated type is one-based and the array is zero-based.
...
// The following array specifies the minimum length of the FsInformation buffer for an NtQueryVolumeInformation service.

// WARNING:  This array depends on the order of the values in the FS_INFORMATION_CLASS enumerated type.
//           Note that the enumerated type is one-based and the array is zero-based.
...
// The following array specifies the minimum length of the FsInformation buffer for an NtSetVolumeInformation service.
...
// The following array specifies the required access mask for the caller to access information in an NtQueryVolumeInformation service.
...
// The following array specifies the required access mask for the caller to access information in an NtSetVolumeInformation service.
