...
*/
{
    ...
    InitializeListHead(&Port->MsgQueue.ReceiveHead);//  Initialize the port msg queue to be empty
...
Routine Description:
    ...
    After running this message will either be empty (like it was just initialized) or completely gone (needs to be initialized)
Arguments:
...
/*
Routine Description:
    This routine is used to disconnect an LPC port so no more messages can be sent and 
    anybody waiting for a message is woken up with an error.
Arguments:
...
VOID FASTCALL LpcpFreeToPortZone(IN PLPCP_MESSAGE Msg, IN ULONG MutexFlags)
{
    ...
    if ((MutexFlags & LPCP_MUTEX_OWNED) == 0) {
        LpcpAcquireLpcpLock();//  Acquire the global lock if necessary
    }
    ...
    if ((Msg->Request.u2.s2.Type & ~LPC_KERNELMODE_MESSAGE) == LPC_CONNECTION_REQUEST) {
        ...
        if (ConnectMsg->ClientPort) {
            //  Capture a pointer to the client port then null it out so that no one else can use it, 
            //  then release lpcp lock before we dereference the client port
