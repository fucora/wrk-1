...
ULONG LpcpEventCounts = 0;//  The current number of events registered
...
Routine Description:
    ...
    If the MapInfoOffset field of the reply message is non-zero, 
    then the PORT_MAP_INFORMATION structure it points to will be processed and the
    ...
    If the target thread is in fact waiting for this reply message, 
    then the reply message is copied into the thread's message buffer and the thread's wait is satisfied.

    If the thread is not waiting for a reply or is waiting for a reply to some other MessageId, 
    then the message is placed in the message queue of
    the port that is connected to the communication port specified by the PortHandle parameter and the 
    Type field of the message is set to LPC_LOST_REPLY.
Arguments:
...
*/
{
    ...
    if ((WakeupThread->LpcReplyMessageId != CapturedReplyMessage.MessageId)
        ...
        LpcpPrint(("failed.  MessageId == %u  Client Id: %x.%x\n", CapturedReplyMessage.MessageId, CapturedReplyMessage.ClientId.UniqueProcess, CapturedReplyMessage.ClientId.UniqueThread));
    ...
    //  Do this before we actually fiddle with the wakeup threads fields.  Otherwise we could mess up its state
    try {
    ...
    if ((CurrentThread->LpcReceivedMsgIdValid) && (CurrentThread->LpcReceivedMessageId == CapturedReplyMessage.MessageId)) {
        CurrentThread->LpcReceivedMessageId = 0;
        CurrentThread->LpcReceivedMsgIdValid = FALSE;
    }

    //  Locate and free the message from the port.  
    //  This call use to test for (CapturedReplyMessage.u2.s2.DataInfoOffset != 0) as a prerequisite for doing the call.
    LpcpFreeDataInfoMessage(PortObject, CapturedReplyMessage.MessageId, CapturedReplyMessage.CallbackId, CapturedReplyMessage.ClientId);
    LpcpReleaseLpcpLock();

    //  Wake up the thread that is waiting for an answer to its request inside of NtRequestWaitReplyPort or NtReplyWaitReplyPort.  
    //  That will dereference itself when it wakes up.
...
Routine Description:
    ...
    This service works the same as NtReplyPort, except that after delivering the reply message, 
    it blocks waiting for a reply to a previous message.
    When the reply is received, it will be placed in the location specified by the ReplyMessage parameter.
Arguments:
    PortHandle - Specifies the handle of the communication port that the original message was received from.
...
*/
{
    ...
    //  Acquire the mutex that guards the LpcReplyMessage field of the thread and 
    //  get the pointer to the message that the thread is waiting for a reply to.
    Msg = (PLPCP_MESSAGE)LpcpAllocateFromPortZone(CapturedReplyMessage.u1.s1.TotalLength);
    if (Msg == NULL) {
        LpcpTraceError(STATUS_NO_MEMORY, CurrentThread->Cid, &CapturedReplyMessage);
        ObDereferenceObject(WakeupThread);
    ...
    //  Save the port context in the current thread, because it waits a reply from the same message
    ...
    if (Status == STATUS_SUCCESS) {
        //  Acquire the mutex that guards the request message queue.
        //  Remove the request message from the list of messages being processed and free the message back to the queue's zone.
        //  If the zone's free list was zero before freeing this message then pulse the free event after free the message so that threads waiting to allocate a request message buffer will wake up.
...
Arguments:
    ...
    DataEntryIndex - Supplies the index of the port data entry in the preceding message that we are reading
...
Arguments:
    ...
    DataEntryIndex - Supplies the index of the port data entry in the preceding message that we are writing
    Buffer - Supplies the location into which the data is to be written
    BufferSize - Supplies the size, in bytes, of the preceding buffer
    NumberOfBytesWritten - Optionally returns the number of bytes written from the buffer
Return Value:
    NTSTATUS - An appropriate status value
*/
{
...
Arguments:
    ...
    DataEntryIndex - Supplies the index of the port data entry in the preceding message that we are transferring
    Buffer - Supplies the location into which the data is to be transferred
    BufferSize - Supplies the size, in bytes, of the preceding buffer
    NumberOfBytesRead - Optionally returns the number of bytes transferred from the buffer
Return Value:
    NTSTATUS - An appropriate status value
*/
{
    ...
    !LpcpValidateClientPort(ClientThread, PortObject, LPCP_VALIDATE_REASON_WRONG_DATA)) {
        ...
        Msg = LpcpFindDataInfoMessage(PortObject, CapturedMessage.MessageId, CapturedMessage.CallbackId, CapturedMessage.ClientId);
        if (Msg != NULL) {
...
BOOLEAN FASTCALL LpcpValidateClientPort(IN PETHREAD Thread, IN PLPCP_PORT_OBJECT ReplyPort, IN ULONG Reason)
/*
Routine Description:
    This routine validates whether the reply for a request come from an appropriate port
Arguments:
...
*/
{
    ...
    //  and we can have a client sending a request from a server connection port and csrss can reply with a server communication port
    if ((ReplyPort == PortThread->ConnectionPort) || (ReplyPort == PortThread->ConnectedPort) || (ReplyPort == PortThread)
