...
//  Global macrodefinitions to acquire and release the LPC_MUTEX in order to track the owner and allow recursive calls
...
#if defined(_AMD64_)
VOID FORCEINLINE LpcpMoveMessage (OUT PPORT_MESSAGE DstMsg, 
...
IN PCLIENT_ID ClientId OPTIONAL)
{
    *DstMsg = *SrcMsg;
    ...
    __movsd( (PULONG)(DstMsg+1), (PULONG)SrcMsgData, ((USHORT)SrcMsg->u1.s1.DataLength + sizeof(ULONG) - 1) / sizeof(ULONG) );
}
...
#define LpcpGetThreadAttributes(T)     (((ULONG_PTR)(T)->LpcWaitingOnPort) & LPCP_THREAD_ATTRIBUTES)
#define LpcpSetThreadAttributes(T,A)     (T)->LpcWaitingOnPort = (PVOID)(((ULONG_PTR)(T)->LpcWaitingOnPort) | (A));
#define LpcpSetPortToThread(T,P)     (T)->LpcWaitingOnPort = (PVOID)(((ULONG_PTR)P) | LPCP_PORT_BIT);
...
#define LpcpPrint( _x_ ) {                              \
    DbgPrint( "LPC[ %02x.%02x ]: ", PsGetCurrentThread()->Cid.UniqueProcess, PsGetCurrentThread()->Cid.UniqueThread ); \
    DbgPrint _x_ ;                                      \
}
...
__forceinline PLPCP_MESSAGE LpcpAllocateFromPortZone (ULONG Size)
{
    ...
    if (Msg != NULL) {
        ...
        //  Clear the message type field. In some failure paths this message get freed w/o having it initialized.
