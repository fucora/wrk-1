...
Abstract:
    Local Inter-Process Communication close procedures that are called when a connection port or a communications port is closed.
*/
...
*/
{
    ...
    if ((Port->Flags & PORT_TYPE) == SERVER_CONNECTION_PORT) {
        //  If this is a server communication port without any system handles then we can completely destroy the communication queue for the port
...
*/
{
    ...
    //  If the port is a server communication port then make sure that if there is a dangling client thread that we get rid of it.
    //  This handles the case of someone calling NtAcceptConnectPort and not calling NtCompleteConnectPort
    ...
    //  If connected, disconnect the port, and then scan the message queue for this port and dereference any messages in the queue.
    LpcpDestroyPortQueue(Port, TRUE);

    //  If we had mapped sections into the server or client communication ports, we need to unmap them in the context of that process.
    ...
    if (ConnectionPort) {
        ...
        while (Next != Head) {
            ...
            if (Port == ConnectionPort) {
                ...
                //  Another thread might free the LPC message captured above in Next. We need to restart the search at the list head.
                Next = Head->Flink;
            } else if ((Msg->Request.ClientId.UniqueProcess == CurrentProcessId) && 
                ((Msg->SenderPort == Port) || (Msg->SenderPort == Port->ConnectedPort) || (Msg->SenderPort == ConnectionPort)))
            {
                //  Test whether the message come from the same port and process
                LpcpTrace(("%s Freeing DataInfo Message %lx (%u.%u)  Port: %lx\n", 
                           PsGetCurrentProcess()->ImageFileName, Msg, Msg->Request.MessageId, Msg->Request.CallbackId, ConnectionPort));
                RemoveEntryList(&Msg->Entry);
                InitializeListHead(&Msg->Entry);
                LpcpFreeToPortZone(Msg, LPCP_MUTEX_OWNED);
...
*/
{
    ...
    //  If we need to reply to a message then if the thread that we need to reply to is still around we want to dereference the thread and free the message
    ...
    if (Msg != NULL) {
        ...
        LpcpReleaseLpcpLock();//  Free the global lpc mutex.
    }
