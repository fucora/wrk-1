...
Routine Description:
    ...
    If the PortHandle parameter specifies a connection port, 
    then the receive will return whenever a message is sent to a server communication port that
    does not have its own receive queue and the message is therefore queued to the receive queue of the connection port.

    If the PortHandle parameter specifies a server communication port that does not have a receive queue,
    then behaves as if the associated connection port handle was specified.
    Otherwise the receive will return whenever message is placed in the receive queue associated with the server communication port.

    The received message will be returned in the variable specified by the ReceiveMessage parameter.
    If the MapInfoOffset field of the reply message is non-zero, 
    then the PORT_MAP_INFORMATION structure it points to will be processed and the relevant pages will be mapped into the caller's address space.
    The service returns an error if there is not enough room in the caller's address space to accommodate the mappings.
Arguments:
    PortHandle - Specifies the handle of the connection or communication port to do the receive from.
    PortContext - Specifies an optional pointer to a variable that is to receive the context value associated with the communication port that
        the message is being received from.  This context variable was specified on the call to the NtAcceptConnectPort service.
    ReplyMessage - This optional parameter specifies the address of a reply message to be sent.  
        The ClientId and MessageId fields determine which
        thread will get the reply.  See description of NtReplyPort for how the reply is sent.  
...
*/
{
    ...
    if (ARGUMENT_PRESENT(ReplyMessage)) {
        ...
        if (CapturedReplyMessage.MessageId == 0) {//  Make sure the user didn't give us a bogus reply message id
            return STATUS_INVALID_PARAMETER;
        }
    }
    ...
    if (ARGUMENT_PRESENT(ReplyMessage)) {//  Validate the message length
    ...
    //  The receive port we use is either the connection port for the port object we were given if we were given a client communication port then we expect to receive the reply on the communication port itself.
    ...
    if (ARGUMENT_PRESENT(ReplyMessage)) {//  If ReplyMessage argument present, then send reply
        ...
        //  Acquire the global Lpc mutex that guards the LpcReplyMessage field of the thread and get the pointer to the message that the thread is waiting for a reply to.
        ...
        //  Locate and free the message from the port. This call may free an existing message and release the LPC lock. 
        //  It was moved down to keep checking for the message ID and set the reply message atomically
    ...
    //  The timeout on this wait and the next wait appear to be the only substantial difference between NtReplyWaitReceivePort and NtReplyWaitReceivePortEx
    ...
    if (Status == STATUS_SUCCESS) {
        ...
        try {
            ...
            if ((Msg->Request.u2.s2.Type & ~LPC_KERNELMODE_MESSAGE) == LPC_CONNECTION_REQUEST) {
                ...
                //  Don't free message until NtAcceptConnectPort called, 
                //  and if it's never called then we'll keep the message until the client exits.
                TempMsg = Msg;
                Msg = NULL;
            ...
            } else if ((Msg->Request.u2.s2.Type & ~LPC_KERNELMODE_MESSAGE) != LPC_REPLY) {//  Check if the message is not a reply
                LpcpMoveMessage(ReceiveMessage, &Msg->Request, (&Msg->Request) + 1, 0, NULL);
                if (ARGUMENT_PRESENT(PortContext)) {
                    *PortContext = Msg->PortContext;
                }

                //  If message contains DataInfo for access via NtRead/WriteRequestData then put the message on a list in the communication port and don't free it.  
            ...
            } else {//  Otherwise this is a reply message we just received
