...
Routine Description:
    ...
    The message pointed to by the RequestMessage parameter is placed in the message queue of the port connected to the communication port specified by the PortHandle parameter.
    This service returns an error if PortHandle is invalid or if the MessageId field of the PortMessage structure is non-zero.
Arguments:
...
*/
{
    ...
    } else {//  This is a kernel mode caller
        CapturedRequestMessage = *RequestMessage;
    ...
    //  Acquire the global Lpc mutex that guards the LpcReplyMessage field of the thread and the request message queue.
    //  Stamp the request message with a serial number, 
    //  insert the message at the tail of the request message queue and remember the address of the message in the LpcReplyMessage field for the current thread.
    LpcpAcquireLpcpLockByThread(CurrentThread);

    //  Based on what type of port the caller gave us we'll need to get the port to actually queue the message off to.
    if ((PortObject->Flags & PORT_TYPE) != SERVER_CONNECTION_PORT) {
        ...
        if (QueuePort != NULL) {
            ...
            } else if ((PortObject->Flags & PORT_TYPE) != SERVER_COMMUNICATION_PORT) {//In the case we don't have a CLIENT_COMMUNICATION_PORT nor SERVER_COMMUNICATION_PORT we'll use the connection port to queue messages.
                ConnectionPort = QueuePort = PortObject->ConnectionPort;
                if (ConnectionPort == NULL) {
                    LpcpFreeToPortZone(Msg, LPCP_MUTEX_OWNED | LPCP_MUTEX_RELEASE_ON_RETURN);
                    ObDereferenceObject(PortObject);
    ...
    } else {//  The caller supplied a server connection port so that is the port we queue off to
        QueuePort = PortObject;
    }

    //  At this point we have an LPC message ready to send and if queue port is not null then we have a port to actually send the message off to
    if (QueuePort != NULL) {
        //  Reference the QueuePort to prevent this port evaporating under us
        //  Test if the QueuePort isn't in the process of going away (i.e. we need to have at least 2 references for this object when ObReferenceObject returns).
        //  Note the LPC lock is still held.
        if (ObReferenceObjectSafe(QueuePort)) {
            ...
            //  Release the mutex, increment the request message queue semaphore by one for the newly inserted request message then exit the critical region.

            //  Disable APCs to prevent this thread from being suspended before being able to release the semaphore.
            KeEnterCriticalRegionThread(&CurrentThread->Tcb);
            LpcpReleaseLpcpLock();
            KeReleaseSemaphore(QueuePort->MsgQueue.Semaphore, LPC_RELEASE_WAIT_INCREMENT, 1L, FALSE);

            //  If this is a waitable port then we'll need to set the event for anyone that was waiting on the port
            ...
            return Status;//  And return to our caller.  This is the only successful way out of this routine
        }
    }

    //  At this point we have a message but not a valid port to queue it off to so we'll free up the port object and release the unused message.
    LpcpFreeToPortZone(Msg, LPCP_MUTEX_OWNED | LPCP_MUTEX_RELEASE_ON_RETURN);
    ObDereferenceObject(PortObject);
    if (ConnectionPort) {
        ObDereferenceObject(ConnectionPort);
...
Routine Description:
    ...
    If the Type field of the RequestMessage structure is euqal to LPC_REQUEST, then this is identified as a callback request.
    ...
    Otherwise the Type field of the message must be zero and it will be set to LPC_REQUEST by the service.
    The message pointed to by the RequestMessage parameter is placed in the message queue of the port connected to the communication port specified by the PortHandle parameter.
    This service returns an error if PortHandle is invalid.
    The calling thread then blocks waiting for a reply.

    The reply message is stored in the location pointed to by the ReplyMessage parameter.
    The ClientId, MessageId and message type fields will be filled in by the service.
Arguments:
    PortHandle - Specifies the handle of the communication port to send the request message to.
    RequestMessage - Specifies a pointer to a request message to send.
    ReplyMessage - Specifies the address of a variable that will receive the reply message.
                   This parameter may point to the same buffer as the RequestMessage parameter.
Return Value:
    NTSTATUS - A status code that indicates whether or not the operation was successful.
*/
{
    ...
    if (PreviousMode != KernelMode) {
        try {
            ...
            //  Make sure that if this message has a data info offset that the port data information actually fits in the message.

            //  We first check that the DataInfoOffset doesn't put us beyond the end of the message.

            //  Then we capture the data info record and compute a pointer to the first unused data entry based on the supplied count.
            //  If the start of the message plus its total length doesn't come up to the first unused data entry then the last valid data entry doesn't fit in the message buffer.
            //  Also if the data entry pointer that we compute is less than the data info pointer then we must have wrapped.
            if (CapturedRequestMessage.u2.s2.DataInfoOffset != 0) {
                ...
                if (((PUCHAR)DataEntry < (PUCHAR)DataInfo) || ((((PUCHAR)RequestMessage) + CapturedRequestMessage.u1.s1.TotalLength) < (PUCHAR)DataEntry)) {
                    return STATUS_INVALID_PARAMETER;
                }
            }
        } except(EXCEPTION_EXECUTE_HANDLER)
    ...
    //  Otherwise if it not an lpc request and it is not a kernel mode message then it is an illegal parameter.
    //  A third case is if the type is a kernel mode message in which case we make it look like an lpc request but without the callback.
    ...
    if (CallbackRequest) {
        ...
        //  See if the thread is waiting for a reply to the messagespecified on this call.
        ...
        if ((WakeupThread->LpcReplyMessageId != CapturedRequestMessage.MessageId)
            || ((LpcpGetThreadMessage(WakeupThread) != NULL) && (LpcpGetThreadMessage(WakeupThread)->Request.u2.s2.Type & ~LPC_KERNELMODE_MESSAGE) != LPC_REQUEST)
            || (!LpcpValidateClientPort(WakeupThread, PortObject, LPCP_VALIDATE_REASON_REPLY))) {
            LpcpPrint(("%s Attempted CallBack Request to Thread %lx (%s)\n", PsGetCurrentProcess()->ImageFileName, WakeupThread, THREAD_TO_PROCESS(WakeupThread)->ImageFileName));
            ...
            LpcpPrint(("         Thread MessageId == %u  Client Id: %x.%x\n", WakeupThread->LpcReplyMessageId, WakeupThread->Cid.UniqueProcess, WakeupThread->Cid.UniqueThread));
        ...
        *((PULONG)(Msg + 1) + 0), *((PULONG)(Msg + 1) + 1),
        *((PULONG)(Msg + 1) + 2), *((PULONG)(Msg + 1) + 3), 
        WakeupThread,
        ...
        //  Wake up the thread that is waiting for an answer to its request inside of NtRequestWaitReplyPort or NtReplyWaitReplyPort
        ReleaseSemaphore = &WakeupThread->LpcReplySemaphore;
    } else {//  A callback is not required, so continue setting up the lpc message
        try {
            LpcpMoveMessage(&Msg->Request, &CapturedRequestMessage, (RequestMessage + 1), MsgType, &CurrentThread->Cid);
            if (CapturedRequestMessage.u2.s2.DataInfoOffset != 0) {
                PPORT_DATA_INFORMATION DataInfo;
    ...
    if (Status == STATUS_SUCCESS) {
        if (Msg != NULL) {
            ...
            if (((Msg->Request.u2.s2.Type & ~LPC_KERNELMODE_MESSAGE) == LPC_REQUEST) && (Msg->Request.u2.s2.DataInfoOffset != 0)) {
    ...
    } else {//  Wait failed, free the message if any.
...
NTSTATUS LpcRequestPort(__in PVOID PortAddress, __in PPORT_MESSAGE RequestMessage)
/*
Routine Description:
...
*/
{
    ...
    if (RequestMessage->u2.s2.Type != 0) {
        ...
        //  If previous mode is kernel, allow the LPC_KERNELMODE_MESSAGE bit to be passed in message type field.
    ...
    if (QueuePort != NULL) {
        ...
        //  Release the mutex, increment the request message queue semaphore by one for the newly inserted request message, 
    ...
    //  At this point we have a message but not a valid port to queue it off to so we'll release the unused message and release our mutex.
    LpcpFreeToPortZone(Msg, LPCP_MUTEX_OWNED | LPCP_MUTEX_RELEASE_ON_RETURN);
    if (ConnectionPort) {
        ObDereferenceObject(ConnectionPort);
    }
...
*/
{
    ...
    //  Allow the LPC_KERNELMODE_MESSAGE bit to be passed in message type field. Don't check the previous mode !!!
    ...
    if (CallbackRequest) {//  Check for a valid request message id
        ...
        LpcpTrace(("%s CallBack Request (%s) Msg %lx (%u.%u) [%08x %08x %08x %08x] to Thread %lx (%s)\n",
                   ...
                   LpcpMessageTypeName[Msg->Request.u2.s2.Type & ~LPC_KERNELMODE_MESSAGE],
                   Msg, 
                   Msg->Request.MessageId, Msg->Request.CallbackId,
                   *((PULONG)(Msg + 1) + 0),
                   *((PULONG)(Msg + 1) + 1),
                   *((PULONG)(Msg + 1) + 2),
                   *((PULONG)(Msg + 1) + 3),
        ...
        //  insert the message at the tail of the request message queue and remember the address of the message in the LpcReplyMessage field for the current thread.
        LpcpAcquireLpcpLockByThread(CurrentThread);
        if ((CurrentThread->LpcReplyMessage != NULL) || (CurrentThread->LpcReplyMessageId != 0) || (CurrentThread->KeyedEventInUse)) {
            LpcpFreeToPortZone(Msg, LPCP_MUTEX_OWNED | LPCP_MUTEX_RELEASE_ON_RETURN);
...
NTSTATUS LpcRequestWaitReplyPort(__in PVOID PortAddress, __in PPORT_MESSAGE RequestMessage, __out PPORT_MESSAGE ReplyMessage)
{
    return LpcpRequestWaitReplyPort(PortAddress, RequestMessage, ReplyMessage, KernelMode);
}


NTSTATUS LpcRequestWaitReplyPortEx(IN PVOID PortAddress, IN PPORT_MESSAGE RequestMessage, OUT PPORT_MESSAGE ReplyMessage)
{
