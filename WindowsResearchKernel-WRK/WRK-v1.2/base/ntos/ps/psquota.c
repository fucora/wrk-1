...
SIZE_T FORCEINLINE PspInterlockedExchangeQuota(IN PSIZE_T pQuota, IN SIZE_T NewQuota)
/*
Routine Description:
...
SIZE_T FORCEINLINE PspInterlockedCompareExchangeQuota(IN PSIZE_T pQuota, IN SIZE_T NewQuota, IN SIZE_T OldQuota)
/*
Routine Description:
...
SIZE_T PspReleaseReturnedQuota(IN PS_QUOTA_TYPE QuotaType)
/*
Routine Description:
    This function walks the list of system quota blocks and returns any non-returned quota.
    This function is called when we are about to fail a quota charge and we want to try and free some resources up.
Arguments:
...
*/
{
    ...
    while (1) {
        ...
        // If no more processes are associated with this block then trim its limit back.
        // This block can only have quota returned at this point.
...
*/
{
    ...
    while (1) {
        ...
        // This looks strange because we don't care if the exchanged succeeded.
        // We only care that the quota is greater than our new quota.
        Quota = PspInterlockedCompareExchangeQuota(pQuota, NewQuota, Quota);
    }
}


SIZE_T FORCEINLINE PspInterlockedAddQuota(IN PSIZE_T pQuota, IN SIZE_T Amount)
/*
Routine Description:
...
SIZE_T FORCEINLINE PspInterlockedSubtractQuota(IN PSIZE_T pUsage, IN SIZE_T Amount)
/*
Routine Description:
...
BOOLEAN PspExpandQuota(IN PS_QUOTA_TYPE QuotaType, IN PEPROCESS_QUOTA_ENTRY QE, IN SIZE_T Usage, IN SIZE_T Amount, OUT SIZE_T *pLimit)
/*
Routine Description:
...
*/
{
    ...
    // We don't want to do too many expansions.
    // If somebody else did it then we want to use theirs if possible.
    ExAcquireSpinLock(&PspQuotaLock, &OldIrql);
    ...
    // If this fails then scavenge any returns from all the quota blocks in the system and try again.
    if (((QuotaType == PsNonPagedPool) ? PspDefaultNonPagedLimit : PspDefaultPagedLimit) == 0) {
        if (MmRaisePoolQuota(QuotaType, Limit, &NewLimit) || (PspReleaseReturnedQuota(QuotaType) > 0 && MmRaisePoolQuota(QuotaType, Limit, &NewLimit))) {
...
NTSTATUS FORCEINLINE PspChargeQuota(IN PEPROCESS_QUOTA_BLOCK QuotaBlock, IN PEPROCESS Process, IN PS_QUOTA_TYPE QuotaType, IN SIZE_T Amount)
/*
Routine Description:
    This function charges the specified quota to a process quota block
Arguments:
...
*/
{
    ...
    // It is very likely that the quota entry will be modified, so bring it in to the cache as exclusive now.
    Usage = ReadForWriteAccess(&QE->Usage);

    // This memory barrier is important.
    // In order not to have to recheck the limit after we charge the quota we only ever reduce the limit by the same amount we are about to reduce the usage by.
    // Using an out of data limit will only allow us to over charge by an amount another thread is just about to release.
    KeMemoryBarrier();
    Limit = QE->Limit;
    while (1) {
        ...
        if (NewUsage <= Limit) {
            ...
            if (tUsage == Usage) {
                ...
                if (Process != NULL) {
                    ...
                    PspInterlockedMaxQuota(&Process->QuotaPeak[QuotaType], NewUsage);// Update the peak value
                }
                return STATUS_SUCCESS;
            }

            // The usage has changed under us.
            // We have a new usage from the exchange but must refetch the limit to preserve the ordering we established above this loop.
            // We don't need a memory barrier as we obtained the new value via an interlocked operation and they contain barriers.
            Usage = tUsage;
            KeMemoryBarrier();
            Limit = QE->Limit;
            continue;
        ...
        if (QuotaType == PsPageFile) {
            ...
            if (Extra > 0) {
                // We had some returns so add this to the limit.
                // We can retry the acquire with the new limit.
                // We refetch the limit here but that doesn't violate the state we set up at the top of the loop.
...
VOID FORCEINLINE PspReturnQuota(IN PEPROCESS_QUOTA_BLOCK QuotaBlock, IN PEPROCESS Process, IN PS_QUOTA_TYPE QuotaType, IN SIZE_T Amount)
/*
Routine Description:
...
#define PSMINGIVEBACK ((MMPAGED_QUOTA_INCREASE > MMNONPAGED_QUOTA_INCREASE)?MMNONPAGED_QUOTA_INCREASE:MMPAGED_QUOTA_INCREASE)
    ...
    // The charge might have been split across the default quota block and a new quota block.
    // We have to handle this case here by first returning quota to the specified quota block then skipping to the default.
    rAmount = Amount;
    while (1) {
        if (rAmount > Usage) {
            tAmount = Usage;
...
/*
Routine Description:
    This function charges shared pool quota of the specified pool type to the specified process's pooled quota block.
    If the quota charge would exceed the limits allowed to the process, then an exception is raised and quota is not charged.
Arguments:
...
/*
Routine Description:
    This function returns pool quota of the specified pool type to the specified process.
Arguments:
...
VOID PsChargePoolQuota(__in PEPROCESS Process, __in POOL_TYPE PoolType, __in ULONG_PTR Amount)
/*
Routine Description:
    This function charges pool quota of the specified pool type to the specified process.
    If the quota charge would exceed the limits allowed to the process, then an exception is raised and quota is not charged.
Arguments:
    Process - Supplies the process to charge quota to.
...
Return Value:
    Raises STATUS_QUOTA_EXCEEDED if the quota charge would exceed the limits allowed to the process.
*/
{
    NTSTATUS Status;
...
NTSTATUS PsChargeProcessPoolQuota(__in PEPROCESS Process, __in POOL_TYPE PoolType, __in ULONG_PTR Amount)
/*
Routine Description:
    This function charges pool quota of the specified pool type to the specified process.
    If the quota charge would exceed the limits allowed to the process, then an exception is raised and quota is not charged.
...
VOID PsReturnPoolQuota(__in PEPROCESS Process, __in POOL_TYPE PoolType, __in ULONG_PTR Amount)
/*
Routine Description:
    This function returns pool quota of the specified pool type to the specified process.
Arguments:
...
NTSTATUS PsChargeProcessNonPagedPoolQuota(__in PEPROCESS Process, __in SIZE_T Amount)
/*
Routine Description:
...
VOID PsReturnProcessNonPagedPoolQuota(__in PEPROCESS Process, __in SIZE_T Amount)
/*
Routine Description:
...
NTSTATUS PsChargeProcessPagedPoolQuota(__in PEPROCESS Process, __in SIZE_T Amount)
/*
Routine Description:
...
VOID PsReturnProcessPagedPoolQuota(__in PEPROCESS Process, __in SIZE_T Amount)
/*
Routine Description:
