...
*/
{
    ...
    if (!VdmIoHandler) {
        try {
            VdmIoHandler = ExAllocatePoolWithQuotaTag(PagedPool, sizeof(VDM_IO_HANDLER), 'HVsP');
    ...
    if (pVdmObjects->VdmIoListHead == NULL) {
        ...
        if (pVdmObjects->VdmIoListHead == NULL) {
            ...
            try {
                ...
                HandlerListHead = ExAllocatePoolWithQuotaTag(NonPagedPool, sizeof(VDM_IO_LISTHEAD), 'LVsP');
            } except(EXCEPTION_EXECUTE_HANDLER)
            {
                Status = GetExceptionCode();
                if (HandlerListHead) {
