...
const GENERIC_MAPPING PspProcessMapping = {
    ...
    STANDARD_RIGHTS_WRITE | PROCESS_CREATE_PROCESS | PROCESS_CREATE_THREAD | PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_DUP_HANDLE |
...
*/
{
    ...
    PspShutdownThread = NULL;// Initialize the shutdown thread pointer
    ...
    // N.B. The CID handle table is removed from the handle table list so it will not be enumerated for object handle queries.
...
#if defined(i386)
    if (!NT_SUCCESS(PspLdtInitialize())) {// Ldt Initialization
        return FALSE;
    }
    if (!NT_SUCCESS(PspVdmInitialize())) {// Vdm support Initialization
        return FALSE;
    }
#endif
    ...
    if (!NT_SUCCESS(PspCreateProcess(&PspInitialSystemProcessHandle, PROCESS_ALL_ACCESS, &ObjectAttributes, NULL, 0, NULL, NULL, NULL, 0))) {
        return FALSE;
    }
    ...
    if (!NT_SUCCESS(ObReferenceObjectByHandle(ThreadHandle, 0L, PsThreadType, KernelMode, &Thread, NULL))) {
        return FALSE;
    }
...
*/
{
    ...
    if (st == STATUS_IMAGE_CHECKSUM_MISMATCH || st == STATUS_INVALID_IMAGE_PROTECT) {
        ...
        NtRaiseHardError(st, 1, 1, &ErrorParameters, OptionOk, &ErrorResponse);// Hard error time. A driver is corrupt.
        return st;
    }
    ...
    if (ReplaceExisting) {
        ...
        PspSystemDll.Section = NtDllSection;
        st = PsMapSystemDll(PsGetCurrentProcess(), &PspSystemDll.DllBase, FALSE);// Map the system dll into the user part of the address space
...
UseLargePages - Attempt the map using large pages. If this fails, the map is automatically performed using small pages.
*/
{
    NTSTATUS st;
    PVOID ViewBase;
    ...
    st = MmMapViewOfSection(CapturedSection, Process, &ViewBase, 0L, 0L, &SectionOffset, &ViewSize, ViewShare, (UseLargePages ? MEM_LARGE_PAGES : 0L), PAGE_READWRITE);
...
*/
{
    ...
    if (PsSystemDllDllBase == NULL) {// If we skipped dll load because we are kernel only then exit now.
        return STATUS_SUCCESS;
    }

    dll_entrypoint = "LdrInitializeThunk";
    st = PspLookupSystemDllEntryPoint(dll_entrypoint, (PVOID)&PspSystemDll.LoaderInitRoutine);// Locate the important system dll entrypoints
    if (!NT_SUCCESS(st)) {
#if DBG
...
VOID PsChangeQuantumTable(BOOLEAN ModifyActiveProcesses, ULONG PrioritySeparation)
{
    ...
    // Job Scheduling classes are ONLY meaningful if long fixed quantums are selected. In practice, this means stock NTS configurations
    ...
    if (ModifyActiveProcesses) {
        ...
        while (NextProcess != &PsActiveProcessHead) {
            ...
            if (Process->PriorityClass != PROCESS_PRIORITY_CLASS_IDLE) {
                // If the process is contained within a JOB, AND we are running Fixed, Long Quantums, use the quantum associated with the Job's scheduling class
                Job = Process->Job;
