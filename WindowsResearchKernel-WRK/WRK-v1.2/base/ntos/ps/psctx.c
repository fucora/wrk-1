...
NTSYSAPI NTSTATUS NTAPI NtQueueApcThread(__in HANDLE ThreadHandle, __in PPS_APC_ROUTINE ApcRoutine, __in_opt PVOID ApcArgument1,
...
NTSTATUS PsGetContextThread(__in PETHREAD Thread, __inout PCONTEXT ThreadContext, __in KPROCESSOR_MODE Mode)
/*
Routine Description:
...
*/
{
    ...
    CurrentThread = PsGetCurrentThread();// Get previous mode and reference specified thread.
    ...
    try {
        ...
        if (Mode != KernelMode) {
            ProbeForReadSmallStructure(ThreadContext, FIELD_OFFSET(CONTEXT, ContextFlags) + sizeof(ThreadContext->ContextFlags), CONTEXT_ALIGN);
        }

        ContextFlags = ThreadContext->ContextFlags;

        // We don't need to re-probe here so long as the structure is smaller than the guard region
        ContextLength = sizeof(CONTEXT);
        ASSERT(ContextLength < 0x10000);

#if defined(_X86_)
        // CONTEXT_EXTENDED_REGISTERS is SET, then we want sizeof(CONTEXT) set above otherwise (not set) we only want the old part of the context record.
    ...
    if (Thread == CurrentThread) {
        ...
        try {// Move context to specfied context record. If an exception occurs, then return the error.
        ...
        KeInitializeApc(&ContextFrame.Apc, &Thread->Tcb, OriginalApcEnvironment, PspGetSetContextSpecialApc, NULL, NULL, KernelMode, NULL);
        if (!KeInsertQueueApc(&ContextFrame.Apc, NULL, Thread, 2)) {
            ...
            try {// Move context to specfied context record. If an exception occurs, then silently handle it and return success.
                RtlCopyMemory(ThreadContext, &ContextFrame.Context, ContextLength);
            } except(EXCEPTION_EXECUTE_HANDLER)
            {
                Status = GetExceptionCode();
...
NTSTATUS PsSetContextThread(__in PETHREAD Thread, __in PCONTEXT ThreadContext, __in KPROCESSOR_MODE Mode)
/*
Routine Description:
...
*/
{
    ...
    // Attempt to get the context of the specified thread.
    try {
        // Capture the context flags, and set the default size of the context record.
        if (Mode != KernelMode) {
            ProbeForReadSmallStructure(ThreadContext, FIELD_OFFSET(CONTEXT, ContextFlags) + sizeof(ThreadContext->ContextFlags), CONTEXT_ALIGN);
        }

        // We don't need to re-probe here so long as the structure is small enough not to cross the guard region.
        ContextFlags = ThreadContext->ContextFlags;
        ContextLength = sizeof(CONTEXT);
        ASSERT(ContextLength < 0x10000);
...
NTSTATUS PsWrapApcWow64Thread(__inout PVOID *ApcContext, __inout PVOID *ApcRoutine)
/*
Routine Description:
    This routine is used by kernel mode callers to queue APCs to a 32-bit thread running inside a Wow64 process.
    It wraps the original APC routine with a wrapper routine inside Wow64.
    The target Apc routine must be inside 32-bit code.
    This routine must be executed in the context of the Wow64 thread where the target APC is going to run in.
    The resulting ApcContext and ApcRoutine from this routine are used when initializing, using KeInitializeApc, to be queued later to a Wow64 thread.
