...
*/
{
    ...
    Status = PspLookupSystemDllEntryPoint(EntryName, &KeUserPopEntrySListEnd);
    if (!NT_SUCCESS(Status)) {
    ...
    Status = PspLookupSystemDllEntryPoint(EntryName, &KeUserPopEntrySListFault);
    if (!NT_SUCCESS(Status)) {
    ...
    Status = PspLookupSystemDllEntryPoint(EntryName, &KeUserPopEntrySListResume);
    if (!NT_SUCCESS(Status)) {
    ...
    if (KeFeatureBits & KF_FAST_SYSCALL) {
        ...
        Status = PspLookupSystemDllEntryPoint(EntryName, (PVOID *)&SharedUserData->SystemCall);// Get the addresses of the fast system call code.
        if (!NT_SUCCESS(Status)) {
        ...
        Status = PspLookupSystemDllEntryPoint(EntryName, (PVOID *)&SharedUserData->SystemCallReturn);// Get the addresses of the fast system call return code.
        if (!NT_SUCCESS(Status)) {
    ...
    SharedUserData->TestRetInstruction = 0xc3;// Ret instruction to test no execute state of shared user data.
#endif
