...
*/
{
    ...
    try {
        ...
        if (CapturedInitialTeb.OldInitialTeb.OldStackBase == NULL && CapturedInitialTeb.OldInitialTeb.OldStackLimit == NULL) {
            CapturedInitialTeb = *InitialTeb;// Since the structure size here is less than 64k we don't need to reprobe
        }
    ...
    Status = PspCreateThread(ThreadHandle, DesiredAccess, ObjectAttributes, ProcessHandle, NULL, ClientId, ThreadContext, &CapturedInitialTeb, CreateSuspended, NULL, NULL);
    return Status;
}
...
*/
{
    ...
    Status = PspCreateThread(ThreadHandle, DesiredAccess, ObjectAttributes, SystemProcess, ProcessPointer, ClientId, NULL, NULL, FALSE, StartRoutine, StartContext);
    return Status;
}
...
Arguments:
    ...
    InitialTeb - Supplies the contents of certain fields for the new threads TEB. This parameter is only examined if both a trap and exception frame were specified.
...
*/
{
    ...
    if (ProcessHandle != NULL) {
        ...
        Status = ObReferenceObjectByHandle(ProcessHandle, PROCESS_CREATE_THREAD, PsProcessType, PreviousMode, &Process, NULL);
    } else {
        if (StartRoutine != NULL) {
    ...
    Status = ObCreateObject(PreviousMode, PsThreadType, ObjectAttributes, PreviousMode, NULL, sizeof(ETHREAD), 0, 0, &Thread);
    if (!NT_SUCCESS(Status)) {
        ObDereferenceObject(Process);
        return Status;
    }
    ...
    ExInitializeRundownProtection(&Thread->RundownProtect);// Initialize rundown protection for cross thread TEB refs etc.
    Thread->ThreadsProcess = Process;// Assign this thread to the process so that from now on we don't have to dereference in error paths.
    ...
    PspInitializeThreadLock(Thread);// Initialize the thread lock
    ...
    if (ARGUMENT_PRESENT(ThreadContext)) {
        Status = MmCreateTeb(Process, InitialTeb, &Thread->Cid, &Teb);// User-mode thread. Create TEB etc
        if (!NT_SUCCESS(Status)) {
...
#error "no target architecture"
        ...
        if (NT_SUCCESS(Status)) {
            Status = KeInitThread(&Thread->Tcb, NULL, PspUserThreadStartup, (PKSTART_ROUTINE)NULL, Thread->StartAddress, ThreadContext, Teb, &Process->Pcb);
        }
    } else {
        Teb = NULL;
        PS_SET_BITS(&Thread->CrossThreadFlags, PS_CROSS_THREAD_FLAGS_SYSTEM);// Set the system thread bit thats kept for all time

        // Initialize kernel thread object for kernel mode thread.
        Thread->StartAddress = (PKSTART_ROUTINE)StartRoutine;
        Status = KeInitThread(&Thread->Tcb, NULL, PspSystemThreadStartup, StartRoutine, StartContext, NULL, NULL, &Process->Pcb);
    }
    ...
    // We check the calling threads termination status so we abort any thread creates while ExitProcess is being called --
    // but the call is blocked only if the new thread would be created in the terminating thread's process.
    if ((Process->Flags&PS_PROCESS_FLAGS_PROCESS_DELETE) != 0 || (((CurrentThread->CrossThreadFlags&PS_CROSS_THREAD_FLAGS_TERMINATED) != 0) &&
        (ThreadContext != NULL) && (THREAD_TO_PROCESS(CurrentThread) == Process))) {
    ...
    Job = Process->Job;
    if (Job != NULL && Job->CompletionPort && !(Process->JobStatus & (PS_JOB_STATUS_NOT_REALLY_ACTIVE | PS_JOB_STATUS_NEW_PROCESS_REPORTED))) {
    ...
    if (PspCreateThreadNotifyRoutineCount != 0) {// Notify registered callout routines of thread creation.
        ULONG i;
        PEX_CALLBACK_ROUTINE_BLOCK CallBack;
    ...
    ObReferenceObjectEx(Thread, 2); // Reference count of thread is biased once for itself and once for the handle if we create it.

    if (CreateSuspended) {
        try {
            KeSuspendThread(&Thread->Tcb);
        } except((GetExceptionCode() == STATUS_SUSPEND_COUNT_EXCEEDED) ? EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH)
        {
        }

        if (Thread->CrossThreadFlags&PS_CROSS_THREAD_FLAGS_TERMINATED) {// If deletion was started after we suspended then wake up the thread
    ...
    if (!NT_SUCCESS(Status)) {// The insert failed. Terminate the thread.
        // This trick is used so that Dbgk doesn't report events for dead threads
        PS_SET_BITS(&Thread->CrossThreadFlags, PS_CROSS_THREAD_FLAGS_DEADTHREAD);
        if (CreateSuspended) {
    ...
    __in_opt HANDLE ExceptionPort)
{
    ...
    return NtCreateProcessEx(ProcessHandle, DesiredAccess, ObjectAttributes OPTIONAL, ParentProcess, Flags, SectionHandle, DebugPort, ExceptionPort, 0);
}
...
*/
{
    ...
    if (ARGUMENT_PRESENT(ParentProcess)) {
        Status = PspCreateProcess(ProcessHandle, DesiredAccess, ObjectAttributes, ParentProcess, Flags, SectionHandle, DebugPort, ExceptionPort, JobMemberLevel);
    } else {
...
NTSTATUS PsCreateSystemProcess(__out PHANDLE ProcessHandle, __in ULONG DesiredAccess, __in_opt POBJECT_ATTRIBUTES ObjectAttributes)
/*
Routine Description:
    This routine creates a system process object. A system process has an address space that is initialized to an empty address space that maps the system.
    The process inherits its access token and other attributes from the initial system process. The process is created with an empty handle table.
Arguments:
    ProcessHandle - Returns the handle for the new process.
    DesiredAccess - Supplies the desired access modes to the new process.
    ObjectAttributes - Supplies the object attributes of the new process.
*/
{
    ...
    Status = PspCreateProcess(ProcessHandle, DesiredAccess, ObjectAttributes, PspInitialSystemProcessHandle, 0, NULL, NULL, NULL, 0);
    return Status;
}
    ...
    */
{
    ...
    // Parent
    if (ARGUMENT_PRESENT(ParentProcess)) {
        Status = ObReferenceObjectByHandle(ParentProcess, PROCESS_CREATE_PROCESS, PsProcessType, PreviousMode, &Parent, NULL);
        if (!NT_SUCCESS(Status)) {
            return Status;
        }
    ...
    if (ARGUMENT_PRESENT(SectionHandle)) {
        Status = ObReferenceObjectByHandle(SectionHandle, SECTION_MAP_EXECUTE, MmSectionObjectType, PreviousMode, &SectionObject, NULL);
        if (!NT_SUCCESS(Status)) {
            goto exit_and_deref;
        }
    } else {
        ...
        if (Parent != PsInitialSystemProcess) {
            ...
            // Since the section pointer is removed at last thread exit we need to protect against process exit here to be safe.
    ...
    if (ARGUMENT_PRESENT(DebugPort)) {
        Status = ObReferenceObjectByHandle(DebugPort, DEBUG_PROCESS_ASSIGN, DbgkDebugObjectType, PreviousMode, &DebugPortObject, NULL);
        if (!NT_SUCCESS(Status)) {
            goto exit_and_deref;
        }
    ...
    if (ARGUMENT_PRESENT(ExceptionPort)) {
        Status = ObReferenceObjectByHandle(ExceptionPort, 0, LpcPortObjectType, PreviousMode, &ExceptionPortObject, NULL);
        if (!NT_SUCCESS(Status)) {
            goto exit_and_deref;
        }
    ...
    if (Parent != NULL) {
        ...
        // Normally, the call to MmInitializeAddressSpace initializes the working set mutex, however, in this case, we have already initialized
    ...
    //      2 - System Process. Address space is a virgin address space that only maps system space. Process is same as PspInitialSystemProcess.
    ...
    if (SectionHandle != NULL) {
        // User Process (New Image Address Space). Don't specify Process to clone, just SectionObject.

        // Passing in the 4th parameter as below lets the EPROCESS struct contain its image file name, provided that appropriate audit settings are enabled.
        // Memory is allocated inside of MmInitializeProcessAddressSpace and pointed to by ImageFileName, so that must be freed in the process deletion routine (PspDeleteProcess())
        Status = MmInitializeProcessAddressSpace(Process, NULL, SectionObject, &Flags, &(Process->SeAuditProcessCreationInfo.ImageFileName));
        if (!NT_SUCCESS(Status)) {
            goto exit_and_deref;
        }
        ...
        if (Parent != PsInitialSystemProcess) {
            ...
            Status = MmInitializeProcessAddressSpace(Process, Parent, NULL, &Flags, NULL);
            if (!NT_SUCCESS(Status)) {
                goto exit_and_deref;
            }
            ...
            // A cloned process isn't started from an image file, so we give it the name of the process of which it is a clone, provided the original has a name.
            if (Parent->SeAuditProcessCreationInfo.ImageFileName != NULL) {
                ImageFileNameSize = sizeof(OBJECT_NAME_INFORMATION) + Parent->SeAuditProcessCreationInfo.ImageFileName->Name.MaximumLength;
                Process->SeAuditProcessCreationInfo.ImageFileName = ExAllocatePoolWithTag(PagedPool, ImageFileNameSize, 'aPeS');
                if (Process->SeAuditProcessCreationInfo.ImageFileName != NULL) {
                    RtlCopyMemory(Process->SeAuditProcessCreationInfo.ImageFileName, Parent->SeAuditProcessCreationInfo.ImageFileName, ImageFileNameSize);

                    // The UNICODE_STRING in the process is self contained, so calculate the offset for the buffer.
            ...
            // In case the image file name of this system process is ever queried, we give a zero length UNICODE_STRING.
            Process->SeAuditProcessCreationInfo.ImageFileName = ExAllocatePoolWithTag(PagedPool, sizeof(OBJECT_NAME_INFORMATION), 'aPeS');
            if (Process->SeAuditProcessCreationInfo.ImageFileName != NULL) {
    ...
    if (SeDetailedAuditingWithToken(NULL)) {// Audit the process creation.
        ...
        if (Job != NULL && !(Job->LimitFlags & JOB_OBJECT_LIMIT_SILENT_BREAKAWAY_OK)) {
            if (Flags&PROCESS_CREATE_FLAGS_BREAKAWAY) {
                ...
                if (NT_SUCCESS(Status)) {
                    ...
                    Token = Job->Token;
                    if (Token != NULL) {
                        Status = SeSubProcessToken(Token, &NewToken, FALSE, Job->SessionId);
                        if (!NT_SUCCESS(Status)) {
                            goto exit_and_deref;
                        }
    ...
    if (Parent && CreatePeb) {
        // For processes created w/ a section, a new "virgin" PEB is created.
        // Otherwise, for forked processes, uses inherited PEB with an updated mutant.
        ...
        if (SectionHandle != NULL) {
            ...
            MmCopyVirtualMemory(CurrentProcess, &InitialPeb, Process, Process->Peb, sizeof(INITIAL_PEB), KernelMode, &BytesCopied);
            ...
            if (Process->Wow64Process != NULL) {
                ...
                MmCopyVirtualMemory(CurrentProcess, &InitialPeb32, Process, Process->Wow64Process->Wow64, sizeof(INITIAL_PEB32), KernelMode, &BytesCopied);
            }
#endif
        }
    }
    ...
    if (!PsUseImpersonationToken) {
        ...
        Status = SeCreateAccessStateEx(NULL,
            (Parent == NULL || Parent != PsInitialSystemProcess) ? PsGetCurrentProcessByThread(CurrentThread) : PsInitialSystemProcess,
                                       AccessState,
                                       &AuxData,
                                       DesiredAccess,
    ...
    // Insert the object. Once we do this is reachable from the outside world via open by name.
    // Open by ID is still disabled. Since its reachable somebody might create a thread in the process and cause rundown.
    Status = ObInsertObject(Process, AccessState, DesiredAccess, 1,     // bias the refcnt by one for future process manipulations
    ...
    // Compute the base priority and quantum reset values for the process and set the memory priority.
    ASSERT(IsListEmpty(&Process->ThreadListHead) == TRUE);
    BasePriority = PspComputeQuantumAndPriority(Process, PsProcessPriorityBackground, &QuantumReset);
    Process->Pcb.BasePriority = (SCHAR)BasePriority;
    Process->Pcb.QuantumReset = QuantumReset;

    // As soon as a handle to the process is accessible, allow the process to be deleted.
    ...
    if (Parent && Parent != PsInitialSystemProcess) {
        ...
        // Note: Changes to this set of bits should be reflected in psquery.c code, in PspSetPrimaryToken.
        Process->GrantedAccess |= (PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_QUERY_INFORMATION | PROCESS_TERMINATE | PROCESS_CREATE_THREAD |
                                   PROCESS_DUP_HANDLE | PROCESS_CREATE_PROCESS | PROCESS_SET_INFORMATION | STANDARD_RIGHTS_ALL | PROCESS_SET_QUOTA);
    } else {
...
*/
{
    ...
    if (Remove) {
        for (i = 0; i < PSP_MAX_CREATE_PROCESS_NOTIFY; i++) {
            CallBack = ExReferenceCallBackBlock(&PspCreateProcessNotifyRoutine[i]);// Reference the callback so we can check its routine address.
            if (CallBack != NULL) {
                ...
                if ((PCREATE_PROCESS_NOTIFY_ROUTINE)ExGetCallBackBlockRoutine(CallBack) == NotifyRoutine) {
                    if (ExCompareExchangeCallBack(&PspCreateProcessNotifyRoutine[i], NULL, CallBack)) {
                        ...
                        ExDereferenceCallBackBlock(&PspCreateProcessNotifyRoutine[i], CallBack);
                        ExWaitForCallBacks(CallBack);// Wait for any active callbacks to finish and free the block.
        ...
        CallBack = ExAllocateCallBack((PEX_CALLBACK_FUNCTION)NotifyRoutine, NULL);// Allocate a new callback block.
        if (CallBack == NULL) {
            return STATUS_INSUFFICIENT_RESOURCES;
        }

        for (i = 0; i < PSP_MAX_CREATE_PROCESS_NOTIFY; i++) {
            if (ExCompareExchangeCallBack(&PspCreateProcessNotifyRoutine[i], CallBack, NULL)) {// Try and swap a null entry for the new block.
...
*/
{
    ...
    for (i = 0; i < PSP_MAX_CREATE_THREAD_NOTIFY; i += 1) {
        if (ExCompareExchangeCallBack(&PspCreateThreadNotifyRoutine[i], CallBack, NULL)) {// Try and swap a null entry for the new block.
            InterlockedIncrement((PLONG)&PspCreateThreadNotifyRoutineCount);
            return STATUS_SUCCESS;
        }
    }

    ExFreeCallBack(CallBack);// No slots left. Free the block and return.
    return STATUS_INSUFFICIENT_RESOURCES;
}


NTSTATUS PsRemoveCreateThreadNotifyRoutine(__in PCREATE_THREAD_NOTIFY_ROUTINE NotifyRoutine)
/*
Routine Description:
    This function allows an installable file system to unhook from thread
    creation and deletion.
Arguments:
    NotifyRoutine - Supplies the address of the routine which was previously registered with PsSetCreateThreadNotifyRoutine
Return Value:
...
*/
{
    ...
    for (i = 0; i < PSP_MAX_CREATE_THREAD_NOTIFY; i += 1) {
        CallBack = ExReferenceCallBackBlock(&PspCreateThreadNotifyRoutine[i]);// Reference the callback so we can check its routine address.
        if (CallBack != NULL) {
            if ((PCREATE_THREAD_NOTIFY_ROUTINE)ExGetCallBackBlockRoutine(CallBack) == NotifyRoutine) {// See if the routine matches our target
...
*/
{
    ...
    Teb = NtCurrentTeb();// Note: Initializing the TEB here is just to satisfy the compiler.
    ...
    if (KillThread) {
        ...
        if (CCPF_IS_PREFETCHER_ENABLED()) {
            ...
            // prefetch the pages likely to be used when initializing the application into the system cache.
            if ((Process->Flags & PS_PROCESS_FLAGS_LAUNCH_PREFETCHED) == 0) {
                ...
                if ((OldFlags & PS_PROCESS_FLAGS_LAUNCH_PREFETCHED) == 0) {
                    if (Process->SectionObject) {
                        CcPfBeginAppLaunch(Process, Process->SectionObject);// Notify cache manager of this application launch.
                    }
                }
            }
        }
        ...
        KiInitializeUserApc(PspGetBaseExceptionFrame(Thread), PspGetBaseTrapFrame(Thread), PspSystemDll.LoaderInitRoutine, NULL, PspSystemDll.DllBase, NULL);
        KeLowerIrql(PASSIVE_LEVEL);
    }
...
// PspUnhandledExceptionInSystemThread doesn't return, and the compiler sometimes gives 'unreachable code' warnings as a result.
#pragma warning(push)
...
BOOLEAN PsGetVersion(__out_opt PULONG MajorVersion, __out_opt PULONG MinorVersion, __out_opt PULONG BuildNumber, __out_opt PUNICODE_STRING CSDVersion)
{
...
*/
{
    ...
    for (i = 0; i < PSP_MAX_LOAD_IMAGE_NOTIFY; i++) {        
        if (ExCompareExchangeCallBack(&PspLoadImageNotifyRoutine[i], CallBack, NULL)) {// Try and swap a null entry for the new block.
...
NTSTATUS PsRemoveLoadImageNotifyRoutine(__in PLOAD_IMAGE_NOTIFY_ROUTINE NotifyRoutine)
/*
Routine Description:
    This function allows an installable file system to unhook from image load notification.
Arguments:
    NotifyRoutine - Supplies the address of the routine which was previously registered with PsSetLoadImageNotifyRoutine
Return Value:
    STATUS_SUCCESS if successful, and STATUS_PROCEDURE_NOT_FOUND if not.
*/
{
    ...
    for (i = 0; i < PSP_MAX_LOAD_IMAGE_NOTIFY; i++) {
        CallBack = ExReferenceCallBackBlock(&PspLoadImageNotifyRoutine[i]);// Reference the callback so we can check its routine address.
        if (CallBack != NULL) {
            if ((PLOAD_IMAGE_NOTIFY_ROUTINE)ExGetCallBackBlockRoutine(CallBack) == NotifyRoutine) {// See if the routine matches our target
