...
#define CONTEXT_EXCEPTION_FLAGS (CONTEXT_EXCEPTION_ACTIVE | CONTEXT_SERVICE_ACTIVE)// Define context exception flags.

// Define forward referenced functions.
PXMM_SAVE_AREA32 PspGetSetContextInternal(IN PKAPC Apc, IN PVOID OperationType, OUT PKEVENT *Event);
...
Arguments:
    ...
    ContextRecord - Supplies the address of a context record.
    ProcessorMode - Supplies the processor mode to use when sanitizing the PSR and FSR.
*/
{
    ULONG ContextFlags;
    ...
    if ((ContextFlags & CONTEXT_FLOATING_POINT) == CONTEXT_FLOATING_POINT) {
        ...
        *ContextPointers->Xmm15 = ContextRecord->Xmm15;
        ...
        TrapFrame->MxCsr = SANITIZE_MXCSR(ContextRecord->MxCsr);// Clear all reserved bits in MXCSR.
...
/*
Routine Description:
    This function either captures the state of the current thread, or sets the state of the current thread.
Arguments:
...
*/
{
    ...
    // Get the address of the context block and compute the address of the system entry trap frame.
    ...
    RtlCaptureContext(&ContextRecord);// Capture the current thread context and set the initial control PC value.
    ...
    // Start with the frame specified by the context record and virtually unwind call frames until the system entry trap frame is encountered.
    do {
    ...
    XmmSaveArea = NULL;
    if (OperationType != NULL) {// Set context.
        // If the context mode is user and floating state is being set, then set the address of the legacy floating information.
        ContextFlags = ContextBlock->Context.ContextFlags;
        if ((ContextBlock->Mode == UserMode) && ((ContextFlags & CONTEXT_FLOATING_POINT) == CONTEXT_FLOATING_POINT)) {
            XmmSaveArea = &ContextBlock->Context.FltSave;
        }

        PspSetContext((PKTRAP_FRAME)TrapFrame, ContextPointers, &ContextBlock->Context, ContextBlock->Mode);// Set context.
    } else {// Get context.         
        if (ContextBlock->Mode == UserMode) {// If the context mode is user, then save the legacy floating state.
            KeSaveLegacyFloatingPointState(&ContextBlock->Context.FltSave);
        }

        PspGetContext((PKTRAP_FRAME)TrapFrame, ContextPointers, &ContextBlock->Context);
    }
