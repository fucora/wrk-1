...
PVOID PsGetCurrentThreadStackBase(VOID)
{
    return KeGetCurrentThread()->StackBase;
}


PVOID PsGetCurrentThreadStackLimit(VOID)
{
    return KeGetCurrentThread()->StackLimit;
}


CCHAR PsGetCurrentThreadPreviousMode(VOID)
{
    return KeGetPreviousMode();
}


PERESOURCE PsGetJobLock(__in PEJOB Job)
{
    return &Job->JobLock;
}


ULONG PsGetJobSessionId(__in PEJOB Job)
{
    return Job->SessionId;
}


ULONG PsGetJobUIRestrictionsClass(__in PEJOB Job)
{
    return Job->UIRestrictionsClass;
}


LONGLONG PsGetProcessCreateTimeQuadPart(__in PEPROCESS Process)
{
    return Process->CreateTime.QuadPart;
}


PVOID PsGetProcessDebugPort(__in PEPROCESS Process)
{
    return Process->DebugPort;
}


BOOLEAN PsIsProcessBeingDebugged(__in PEPROCESS Process)
{
...
BOOLEAN PsGetProcessExitProcessCalled(__in PEPROCESS Process)
{
    return (BOOLEAN)((Process->Flags&PS_PROCESS_FLAGS_PROCESS_EXITING) != 0);
}


NTSTATUS PsGetProcessExitStatus(__in PEPROCESS Process)
{
...
ULONG PsGetProcessSessionId(__in PEPROCESS Process)
{
    return MmGetSessionId(Process);
}


ULONG PsGetProcessSessionIdEx(__in PEPROCESS Process)
{
...
UCHAR PsGetProcessPriorityClass(__in PEPROCESS Process)
{
    return Process->PriorityClass;
}


HANDLE PsGetProcessWin32WindowStation(__in PEPROCESS Process)
{
    return Process->Win32WindowStation;
}


PVOID PsGetProcessWin32Process(__in PEPROCESS Process)
{
    return Process->Win32Process;
}


PVOID PsGetCurrentProcessWin32Process(VOID)
{
...
HANDLE PsGetThreadId(__in PETHREAD Thread)
{
    return Thread->Cid.UniqueThread;
}


CCHAR PsGetThreadFreezeCount(__in PETHREAD Thread)
{
    return Thread->Tcb.FreezeCount;
}


BOOLEAN PsGetThreadHardErrorsAreDisabled(__in PETHREAD Thread)
{
    return (BOOLEAN)(Thread->CrossThreadFlags&PS_CROSS_THREAD_FLAGS_HARD_ERRORS_DISABLED) != 0;
}


PEPROCESS PsGetThreadProcess(__in PETHREAD Thread)
{
    return THREAD_TO_PROCESS(Thread);
}


PEPROCESS PsGetCurrentThreadProcess(VOID)
{
    return THREAD_TO_PROCESS(_PsGetCurrentThread());
}


HANDLE PsGetThreadProcessId(__in PETHREAD Thread)
{
    return Thread->Cid.UniqueProcess;
}


HANDLE PsGetCurrentThreadProcessId(VOID)
{
    return _PsGetCurrentThread()->Cid.UniqueProcess;
}


ULONG PsGetThreadSessionId(__in PETHREAD Thread)
{
...
PVOID PsGetThreadWin32Thread(__in PETHREAD Thread)
{
...
PVOID PsGetCurrentThreadWin32ThreadAndEnterCriticalRegion(__out PHANDLE ProcessId)
{
...
BOOLEAN PsIsSystemThread(__in PETHREAD Thread)
{
    return (BOOLEAN)(IS_SYSTEM_THREAD(Thread));
}


BOOLEAN PsIsSystemProcess(__in PEPROCESS Process)
{
    return (BOOLEAN)(Process == PsInitialSystemProcess);
}


VOID PsSetJobUIRestrictionsClass(__out PEJOB Job, __in ULONG UIRestrictionsClass)
{
    Job->UIRestrictionsClass = UIRestrictionsClass;
}


VOID PsSetProcessPriorityClass(__out PEPROCESS Process, __in UCHAR PriorityClass)
{
    Process->PriorityClass = PriorityClass;
}


NTSTATUS PsSetProcessWin32Process(__in PEPROCESS Process, __in PVOID Win32Process, __in PVOID PrevWin32Process)
{
    NTSTATUS Status;
...
VOID PsSetProcessWindowStation(__out PEPROCESS Process, __in HANDLE Win32WindowStation)
{
    Process->Win32WindowStation = Win32WindowStation;
}


VOID PsSetThreadHardErrorsAreDisabled(__in PETHREAD Thread, __in BOOLEAN HardErrorsAreDisabled)
{
...
VOID PsSetThreadWin32Thread(__inout PETHREAD Thread, __in PVOID Win32Thread, __in PVOID PrevWin32Thread)
{
    if (Win32Thread != NULL) {
...
PVOID PsGetProcessSecurityPort(__in PEPROCESS Process)
{
    return Process->SecurityPort;
}


NTSTATUS PsSetProcessSecurityPort(__out PEPROCESS Process, __in PVOID Port)
{
...
BOOLEAN PsIsThreadImpersonating(__in PETHREAD Thread)
/*
Routine Description:
