...
NTSTATUS NTAPI NtCreateJobObject(__out PHANDLE JobHandle, __in ACCESS_MASK DesiredAccess, __in_opt POBJECT_ATTRIBUTES ObjectAttributes)
{
    PEJOB Job;
    HANDLE Handle;
    ...
    // Establish an exception handler, probe the output handle address, and attempt to create a job object.
    ...
    try {
        if (PreviousMode != KernelMode) {
            ProbeForWriteHandle(JobHandle);// Probe output handle address if necessary.
        }
        *JobHandle = NULL;
    ...
    if (NT_SUCCESS(Status)) {// If the job object was successfully allocated, then initialize it and attempt to insert the job object in the current process' handle table.
        ...
        // We will use this sessionid to restrict the processes that can be added to a job.
        ...
        if (NT_SUCCESS(Status)) {// If the job object was successfully inserted in the current process' handle table, then attempt to write the job object handle value.
            try {
                *JobHandle = Handle;
            } except(ExSystemExceptionFilter())
            {
...
NTSTATUS NTAPI NtOpenJobObject(__out PHANDLE JobHandle, __in ACCESS_MASK DesiredAccess, __in POBJECT_ATTRIBUTES ObjectAttributes)
{
    HANDLE Handle;
    KPROCESSOR_MODE PreviousMode;
    NTSTATUS Status;
    ...
    // Establish an exception handler, probe the output handle address, and attempt to open the job object.
    // If the probe fails, then return the exception code as the service status.
    ...
    if (PreviousMode != KernelMode) {
        ...
        {
            return GetExceptionCode();// If an exception occurs during the probe of the output job handle, then always handle the exception and return the exception code as the status value.
        }
    }

    Status = ObOpenObjectByName(ObjectAttributes, PsJobType, PreviousMode, NULL, DesiredAccess, NULL, &Handle);// Open handle to the event object with the specified desired access.

    // If the open was successful, then attempt to write the job object handle value.
    // If the write attempt fails then just report an error.
...
NTSTATUS NTAPI NtAssignProcessToJobObject(__in HANDLE JobHandle, __in HANDLE ProcessHandle)
{
    PEJOB Job;
    PEPROCESS Process;
    PETHREAD CurrentThread;
    ...
    Status = ObReferenceObjectByHandle(
        ProcessHandle,
        PROCESS_SET_QUOTA | PROCESS_TERMINATE | ((JobToken != NULL) ? PROCESS_SET_INFORMATION : 0),
        PsProcessType,
        PreviousMode,
        &Process,
        NULL);
    ...
    // We only allow a process that is running in the Job creator's hydra session to be assigned to the job.
    ...
    // Security Rules:  If the job has no-admin set, and it is running as admin, that's not allowed
    ...
    if (JobToken != NULL) {
        Status = SeSubProcessToken(JobToken, &NewToken, FALSE, SessionId);
        if (!NT_SUCCESS(Status)) {
            goto deref_and_return_status;
        }
    }
    ...
    // If the job has a token filter established, use it to filter the
    ExReleaseRundownProtection(&Process->RundownProtect);
    ...
    if (JobToken != NULL) {
        ...
        ObDereferenceObject(NewToken);
        ASSERT(NT_SUCCESS(Status1));// Only bad callers should fail here.
    }
...
NTSTATUS PspAddProcessToJob(PEJOB Job, PEPROCESS Process)
{
    ...
    if ((Job->LimitFlags & JOB_OBJECT_LIMIT_ACTIVE_PROCESS) && Job->ActiveProcesses > Job->ActiveProcessLimit) {
        PS_SET_CLEAR_BITS(&Process->JobStatus, PS_JOB_STATUS_NOT_REALLY_ACTIVE | PS_JOB_STATUS_ACCOUNTING_FOLDED, PS_JOB_STATUS_LAST_REPORT_MEMORY);
        if (Job->CompletionPort != NULL) {
            IoSetIoCompletion(Job->CompletionPort, Job->CompletionKey, NULL, STATUS_SUCCESS, JOB_OBJECT_MSG_ACTIVE_PROCESS_LIMIT, TRUE);
        }
    ...
    // If the last handle to the job has been closed and the kill on close option is set we don't let new processes enter the job. This is to make cleanup solid.
    ...
    if (NT_SUCCESS(Status)) {
        // We can't attach with APCs diabled so we have to unlock attach and then check working set limits.
...
VOID PspJobDelete(IN PVOID Object)
{
    ...
    // If we are part of a jobset then we must be the pinning job. We must pass on the pin to the next job in the chain.
    ...
    // Removing the pin from the job set can cause a cascade of deletes that would cause a stack overflow as we recursed at this point.
    // We break recursion by forcing the deferred delete path here.
    if (tJob != NULL) {
...
VOID PspJobClose(IN PEPROCESS Process, IN PVOID Object, IN ACCESS_MASK GrantedAccess, IN ULONG_PTR ProcessHandleCount, IN ULONG_PTR SystemHandleCount)
/*
Routine Description:
...
*/
{
    ...
    // This is used to prevent new processes entering a job marked as terminate on close and also prevents a completion port being set on a torn down job.
    // Completion ports are removed on last handle close.
...
)
{
    ...
    if (JobObjectInformationLength != RequiredLength) {
        // BasicProcessIdList is variable length, so make sure header is ok. Can not enforce an exact match !  Security Limits can be as well, due to the token groups and privs
        if ((JobObjectInformationClass == JobObjectBasicProcessIdList) || (JobObjectInformationClass == JobObjectSecurityLimitInformation)) {
            if (JobObjectInformationLength < RequiredLength) {
    ...
    if (PreviousMode != KernelMode) {
        try {
            // Since these functions don't change any state thats not reversible in the error paths we only probe the output buffer for write access.
            // This improves performance by not touching the buffer multiple times And only writing the portions of the buffer that change.
    ...
    if (ARGUMENT_PRESENT(JobHandle)) {
        st = ObReferenceObjectByHandle(JobHandle, JOB_OBJECT_QUERY, PsJobType, PreviousMode, &Job, NULL);
        if (!NT_SUCCESS(st)) {
            return st;
        }
    } else {
        // if the current process has a job, NULL means the job of the current process. Query is always allowed for this case
    ...
    case JobObjectBasicAndIoAccountingInformation:
        // These two cases are identical, EXCEPT that with AndIo, IO information is returned as well,
        // but the first part of the local is identical to basic, and the shorter return'd data length chops what we return.
    ...
    case JobObjectBasicLimitInformation:
        ...
        if (JobObjectInformationClass == JobObjectExtendedLimitInformation) {
            ...
            ExReleaseResourceLite(&Job->JobLock);
            KeLeaveGuardedRegionThread(&CurrentThread->Tcb);
            RtlZeroMemory(&ExtendedLimitInfo.IoInfo, sizeof(ExtendedLimitInfo.IoInfo));// Zero un-used I/O counters
    ...
    __in ULONG JobObjectInformationLength)
{
    ...
    st = ObReferenceObjectByHandle(JobHandle, RequiredAccess, PsJobType, PreviousMode, &Job, NULL);
    if (!NT_SUCCESS(st)) {
        return st;
    }
    ...
    case JobObjectBasicLimitInformation:
        ...
        if (NT_SUCCESS(st)) {
            ...
            if (ExtendedLimitInfo.BasicLimitInformation.LimitFlags & ~ValidFlags) {
                ...
                LocalJob.LimitFlags = Job->LimitFlags;// Deal with each of the various limit flags

                // ACTIVE PROCESS LIMIT
                if (LimitFlags & JOB_OBJECT_LIMIT_ACTIVE_PROCESS) {
                    // Active Process Limit is NOT retroactive.
                    // New processes are denied, but existing ones are not killed just because the limit is reduced.
                ...
                if (LimitFlags & JOB_OBJECT_LIMIT_PRIORITY_CLASS) {
                    if (ExtendedLimitInfo.BasicLimitInformation.PriorityClass > PROCESS_PRIORITY_CLASS_ABOVE_NORMAL) {
                        ...
                        if (ExtendedLimitInfo.BasicLimitInformation.PriorityClass == PROCESS_PRIORITY_CLASS_HIGH ||
                            ...
                            // Increasing the base priority of a process is a privileged operation.  Check for the privilege here.
                ...
                if (LimitFlags & JOB_OBJECT_LIMIT_JOB_TIME) {
                    ...
                    if (LimitFlags & JOB_OBJECT_LIMIT_PRESERVE_JOB_TIME) {
                        // If we are supposed to preserve existing job time limits, then preserve them !
                ...
                if (LimitFlags & JOB_OBJECT_LIMIT_WORKINGSET) {
                    // the only issue with this check is that when we enforce through the processes,
                    // we may find a process that can not handle the new working set limit because it will make the process's working set not fluid
                ...
                if (NT_SUCCESS(st)) {
                    ...
                    if (LimitFlags & JOB_OBJECT_LIMIT_JOB_TIME) {
                        // Take any signaled processes and fold their accounting into the job.
                        // This way a process that exited clean but still is open won't impact the next period
                        Next = Job->ProcessListHead.Flink;
                        while (Next != &Job->ProcessListHead) {
                            Process = (PEPROCESS)(CONTAINING_RECORD(Next, EPROCESS, JobLinks));

                            // see if process has been signaled.
                            // This indicates that the process has exited.
                            // We can't do this in the exit path because of the lock order problem
                            // between the process lock and the job lock since in exit we hold the process lock for a long time and can't drop it until thread termination
                            if (KeReadStateProcess(&Process->Pcb)) {
                                ...
                                // running processes have their current runtime added to the programmed limit.
                                // This way, you can set a limit on a job with processes in the job and not have previous runtimes count against the limit
                                if (!(Process->JobStatus & PS_JOB_STATUS_ACCOUNTING_FOLDED)) {
    ...
    case JobObjectSecurityLimitInformation:
        ...
        if (NT_SUCCESS(st)) {
            if (SecurityLimitInfo.SecurityLimitFlags & (~JOB_OBJECT_SECURITY_VALID_FLAGS)) {
                ...
                // Deal with specific options.  Basic rules:  Once a flag is on, it is always on (so even with a handle to the job, a process could not lift the security restrictions).
                ...
                // The forcible token is a little more interesting.
                // It cannot be reset, so if there is a pointer there already, fail the call.
                // If a filter is already in place, this is not allowed, either.
                // If no-admin is set, it is checked at the end, once the token has been ref'd.
                if (SecurityLimitInfo.SecurityLimitFlags & JOB_OBJECT_SECURITY_ONLY_TOKEN) {
                    if (Job->Token ||
                        ...
                        if (NT_SUCCESS(st)) {
                            // If the token supplied is not a restricted token based on the caller's ID,
                            // then they must have assign primary privilege in order to associate the token with the job.
                            if (!IsChild) {
                                HasPrivilege = SeCheckPrivilegedObject(SeAssignPrimaryTokenPrivilege, JobHandle, JOB_OBJECT_SET_SECURITY_ATTRIBUTES, PreviousMode);
                                if (!HasPrivilege) {
                                    st = STATUS_PRIVILEGE_NOT_HELD;
                                }
                            }
                            ...
                            if (NT_SUCCESS(st)) {
                                // Not surprisingly, specifying no-admin and supplying an admin token is a no-no.
                                if ((Job->SecurityLimitFlags & JOB_OBJECT_SECURITY_NO_ADMIN) && SeTokenIsAdmin(LocalToken)) {
                                    st = STATUS_INVALID_PARAMETER;
                                    ObDereferenceObject(LocalToken);
                                } else {
                                    // Grab a reference to the token into the job object
                                    KeMemoryBarrier();
                                    Job->Token = LocalToken;
                                    Job->SecurityLimitFlags |= JOB_OBJECT_SECURITY_ONLY_TOKEN;
                                }
                            } else {
                                // This is the token was a child or otherwise ok, but assign primary was not held, so the request was rejected.
                                ObDereferenceObject(LocalToken);
                            }
                        }
                    }
    ...
    case JobObjectAssociateCompletionPortInformation:
        ...
        if (NT_SUCCESS(st)) {
            if (Job->CompletionPort || AssociateInfo.CompletionPort == NULL) {
                ...
                st = ObReferenceObjectByHandle(AssociateInfo.CompletionPort, IO_COMPLETION_MODIFY_STATE, IoCompletionObjectType, PreviousMode, &IoCompletion, NULL);
                if (NT_SUCCESS(st)) {
                    ExAcquireResourceExclusiveLite(&Job->JobLock, TRUE);

                    // If the job already has a completion port or if the job has been rundown then reject the request.
                    if (Job->CompletionPort != NULL || (Job->JobFlags&PS_JOB_FLAGS_CLOSE_DONE) != 0) {
                        ...
                        // Now whip through ALL existing processes in the job and send notification messages
                        Next = Job->ProcessListHead.Flink;
                        while (Next != &Job->ProcessListHead) {
                            Process = (PEPROCESS)(CONTAINING_RECORD(Next, EPROCESS, JobLinks));

                            // If the process is really considered part of the job, has been assigned its id, and has not yet checked in, do it now
    ...
    if (ProcessWorkingSetHead) {
        ...
        while (!IsListEmpty(&PspWorkingSetChangeHead.Links)) {
            ...
            MmEnforceWorkingSetLimit(WsChangeRecord->Process, MM_WORKING_SET_MAX_HARD_ENABLE);// call MM to Enable hard workingset
            KeUnstackDetachProcess(&ApcState);
        }
        PspUnlockWorkingSetChangeExclusiveUnsafe();
    ...
    KeLeaveGuardedRegionThread(&CurrentThread->Tcb);
    ObDereferenceObject(Job);// Finish Up
    return st;
}
...
VOID PspApplyJobLimitsToProcess(PEJOB Job, PEPROCESS Process)
{
    ...
    if (!(Job->LimitFlags & JOB_OBJECT_LIMIT_WORKINGSET)) {
        MmEnforceWorkingSetLimit(Process, MM_WORKING_SET_MAX_HARD_DISABLE);// call MM to disable hard workingset
    }
    ...
    // If the process is NOT IDLE Priority Class, and long fixed quantums are in use, use the scheduling class stored in the job object for this process
    if (Process->PriorityClass != PROCESS_PRIORITY_CLASS_IDLE) {
        if (PspUseJobSchedulingClasses) {
            KeSetQuantumProcess(&Process->Pcb, PspJobSchedulingClasses[Job->SchedulingClass]);
        }

        // if the scheduling class is PSP_NUMBER_OF_SCHEDULING_CLASSES-1, then give this process non-preemptive scheduling
...
VOID PsEnforceExecutionTimeLimits(VOID)
{
    ...
    for (Job = PsGetNextJob(NULL); Job != NULL; Job = PsGetNextJob(Job)) {
        if (Job->LimitFlags & (JOB_OBJECT_LIMIT_PROCESS_TIME | JOB_OBJECT_LIMIT_JOB_TIME)) {
            for (Process = PsGetNextJobProcess(Job, NULL); Process != NULL; Process = PsGetNextJobProcess(Job, Process)) {
                // Job looks like a candidate for time enforcing.
                // Need to get the job lock to be sure, but we don't want to hang waiting for the job lock, so skip the job until next time around if we need to
                KeEnterCriticalRegionThread(&CurrentThread->Tcb);
                    ...
                    if (Job->LimitFlags & (JOB_OBJECT_LIMIT_PROCESS_TIME | JOB_OBJECT_LIMIT_JOB_TIME)) {
                        ...
                        if (Job->LimitFlags & JOB_OBJECT_LIMIT_PROCESS_TIME) {
                            if (ProcessTime.QuadPart > Job->PerProcessUserTimeLimit.QuadPart) {
                                ...
                                // Reference the process. Assert that it is not in its delete routine.
                                // If all is OK, then delete and dereference the process
                                if (!(Process->JobStatus & PS_JOB_STATUS_NOT_REALLY_ACTIVE)) {
                        ...
                        if (RunningJobTime.QuadPart > Job->PerJobUserTimeLimit.QuadPart) {
                            ...
                            case JOB_OBJECT_POST_AT_END_OF_JOB:
                                if (Job->CompletionPort) {
                                    Status = IoSetIoCompletion(Job->CompletionPort, Job->CompletionKey, NULL, STATUS_SUCCESS, JOB_OBJECT_MSG_END_OF_JOB_TIME, FALSE);
                                    if (NT_SUCCESS(Status)) {
...
BOOLEAN PspTerminateAllProcessesInJob(PEJOB Job, NTSTATUS Status, BOOLEAN IncCounter)
{
    PEPROCESS Process;
...
VOID PspFoldProcessAccountingIntoJob(PEJOB Job, PEPROCESS Process)
{
    ...
    if (!(Process->JobStatus & PS_JOB_STATUS_ACCOUNTING_FOLDED)) {
        ...
        if (Job->CompletionPort && Job->ActiveProcesses == 0) {
            IoSetIoCompletion(Job->CompletionPort, Job->CompletionKey, NULL, STATUS_SUCCESS, JOB_OBJECT_MSG_ACTIVE_PROCESS_ZERO, FALSE);
        }
    }
}
...
BOOLEAN PsChangeJobMemoryUsage(IN ULONG Flags, IN SSIZE_T Amount)
{
    ...
    if ((Flags & PS_JOB_STATUS_REPORT_COMMIT_CHANGES) == 0) {
        // Calls with PS_JOB_STATUS_REPORT_PHYSICAL_PAGE_CHANGES are already happening, but we don't have handling for them yet.
        return TRUE;
    }
    ...
    Job = Process->Job;
    if (Job) {
        // This routine can be called while holding the process lock (during teb deletion... So instead of using the job lock, we must use the memory limits lock.
        // The lock order is always (job lock followed by process lock.
        // The memory limits lock never nests or calls other code while held.
        // It can be grapped while holding the job lock, or the process lock.
        PspLockJobLimitsShared(Job, CurrentThread);
        ...
        if (Job->LimitFlags & JOB_OBJECT_LIMIT_JOB_MEMORY && CurrentJobMemoryUsed > Job->JobMemoryLimit) {
            ...
            // Tell the job port that commit has been exceeded, and process id x was the one that hit it.
            if (Job->CompletionPort && Process->UniqueProcessId && (Process->JobStatus & PS_JOB_STATUS_NEW_PROCESS_REPORTED) && (Process->JobStatus & PS_JOB_STATUS_LAST_REPORT_MEMORY) == 0) {
                ...
                IoSetIoCompletion(Job->CompletionPort, Job->CompletionKey, (PVOID)Process->UniqueProcessId, STATUS_SUCCESS, JOB_OBJECT_MSG_JOB_MEMORY_LIMIT, TRUE);
            }
        }
...
VOID PsReportProcessMemoryLimitViolation(VOID)
{
    ...
    if (Job != NULL && (Job->LimitFlags & JOB_OBJECT_LIMIT_PROCESS_MEMORY)) {
        ...
        if (Job->CompletionPort &&
            ...
            PS_SET_BITS(&Process->JobStatus, PS_JOB_STATUS_LAST_REPORT_MEMORY);
            IoSetIoCompletion(Job->CompletionPort, Job->CompletionKey, (PVOID)Process->UniqueProcessId, STATUS_SUCCESS, JOB_OBJECT_MSG_PROCESS_MEMORY_LIMIT, TRUE);
        }
...
VOID PspJobTimeLimitsDpcRoutine(IN PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2)
{
...
{
    // Wait until Phase1 executive initialization completes (ie: the worker queues must be initialized) before setting off our DPC timer (which queues work items!).
    KeSetTimer(&PspJobTimeLimitsTimer, PspJobTimeLimitsInterval, &PspJobTimeLimitsDpc);
}
...
*/
{
    ...
    if (ProcessHandle != NtCurrentProcess()) {
        Status = ObReferenceObjectByHandle(ProcessHandle, PROCESS_QUERY_INFORMATION, PsProcessType, PreviousMode, &Process, NULL);
        if (!NT_SUCCESS(Status)) {
            return Status;
        }
    } else {
...
/*
Routine Description:
    The function selects the job a process will run in.
    Either the same job as the parent or a job in the same job set as the parent but with a JobMemberLevel >= to the parents level/
Arguments:
...
*/
{
    ...
    for (JobsProcessed = 0; JobsProcessed < NumJob; JobsProcessed++) {
        ...
        Status = ObReferenceObjectByHandle(JobSet[JobsProcessed].JobHandle, JOB_OBJECT_QUERY, PsJobType, PreviousMode, &Job, NULL);
        if (!NT_SUCCESS(Status)) {
            break;
        }
    ...
    // Dereference all the objects in the error path. If we succeeded then pin all but the first object by leaving the reference there.
    if (!NT_SUCCESS(Status)) {
        for (JobsProcessed = 0; JobsProcessed < NumJob; JobsProcessed++) {
            Job = JobSet[JobsProcessed].JobHandle;
            ObDereferenceObject(Job);
...
*/
{
    ...
    if ((Process->Flags & PS_PROCESS_FLAGS_IN_SESSION) && (SessionId == MmGetSessionId(Process))) {
        (CalloutRoutine)(Parameters);// If the call is from a user mode process, and we are asked to call the current session, call directly.
        Status = STATUS_SUCCESS;
    } else {
        OpaqueSession = MmGetSessionById(SessionId);// Reference the session object for the specified session.
        if (OpaqueSession == NULL) {
            return STATUS_NOT_FOUND;
        }

        Status = MmAttachSession(OpaqueSession, &ApcState);// Attach to the specified session.
        if (!NT_SUCCESS(Status)) {
            KdPrintEx((DPFLTR_SYSTEM_ID, DPFLTR_WARNING_LEVEL,
                       "PspWin32SessionCallout: could not attach to 0x%p, session %d for registered notification callout @ 0x%p\n", OpaqueSession, SessionId, CalloutRoutine));
            MmQuitNextSession(OpaqueSession);
            return Status;
        }

        (CalloutRoutine)(Parameters);// Dispatch notification to the callout routine.

        Status = MmDetachSession(OpaqueSession, &ApcState);// Detach from the session.
        ASSERT(NT_SUCCESS(Status));

        Status = MmQuitNextSession(OpaqueSession);// Dereference the session object.
        ASSERT(NT_SUCCESS(Status));
    }
