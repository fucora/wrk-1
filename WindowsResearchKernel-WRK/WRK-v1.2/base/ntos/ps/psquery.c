...
NTSTATUS PspQueryWorkingSetWatch(IN HANDLE ProcessHandle, IN PROCESSINFOCLASS ProcessInformationClass, OUT PVOID ProcessInformation, IN ULONG ProcessInformationLength, OUT PULONG ReturnLength OPTIONAL, IN KPROCESSOR_MODE PreviousMode)
{
    ...
    if (WorkingSetCatcher->CurrentIndex) {
        WorkingSetCatcher->WatchInfo[WorkingSetCatcher->CurrentIndex].FaultingPc = NULL;// Null Terminate the first empty entry in the buffer
                             ...
                             IN KPROCESSOR_MODE PreviousMode)
{
    ...
    Status = MmQueryWorkingSetInformation(&PeakWorkingSetSize, &WorkingSetSize, &QuotaLimits.MinimumWorkingSetSize, &QuotaLimits.MaximumWorkingSetSize, &HardEnforcement);
                                   ...
                                   IN KPROCESSOR_MODE PreviousMode)
{
    ...
    st = ObReferenceObjectByHandle(ProcessHandle, PROCESS_QUERY_INFORMATION, PsProcessType, PreviousMode, &Process, NULL);
    if (!NT_SUCCESS(st)) {
        return st;
    }
...
NTSTATUS PspSetPrimaryToken(IN HANDLE ProcessHandle OPTIONAL, IN PEPROCESS ProcessPointer OPTIONAL, IN HANDLE TokenHandle OPTIONAL, IN PACCESS_TOKEN TokenPointer OPTIONAL, IN BOOLEAN PrivilegeChecked)
/*
    Sets the primary token for a process.
    The token and process supplied can be either by handle or by pointer.
*/
{
    NTSTATUS Status;
    ...
    // Check to see if the supplied token is a child of the caller's token. If so, we don't need to do the privilege check.
    PreviousMode = KeGetPreviousMode();
    if (TokenPointer == NULL) {
        // Reference the specified token, and make sure it can be assigned as a primary token.
        Status = ObReferenceObjectByHandle(TokenHandle, TOKEN_ASSIGN_PRIMARY, SeTokenObjectType, PreviousMode, &Token, NULL);
        if (!NT_SUCCESS(Status)) {
            return Status;
        }
    } else {
    ...
    // If the privilege check has already been done (when the token was assign to a job for example). We don't want to do it here.
    if (!PrivilegeChecked) {
        Status = SeIsChildTokenByPointer(Token, &IsChildToken);
        if (!NT_SUCCESS(Status)) {
            goto exit_and_deref_token;
        }

        if (!IsChildToken) {
            Status = SeIsSiblingTokenByPointer(Token, &IsSiblingToken);
            if (!NT_SUCCESS(Status)) {
                goto exit_and_deref_token;
            }
    ...
    if (ProcessPointer == NULL) {
        Status = ObReferenceObjectByHandle(ProcessHandle, PROCESS_SET_INFORMATION, PsProcessType, PreviousMode, &Process, NULL);
        if (!NT_SUCCESS(Status)) {
            goto exit_and_deref_token;
        }
    } else {
    ...
    if (NT_SUCCESS(Status)) {// Recompute the process's access to itself for use with the CurrentProcess() pseudo handle.
        NTSTATUS accesst;
        BOOLEAN AccessCheck;
        ...
        if (NT_SUCCESS(Status)) {
            ...
            // To keep consistency with process creation,
            // grant these bits otherwise CreateProcessAsUser messes up really badly for restricted tokens and
            // we end up with a process that has no access to itself when new token is set on the suspended process.
            GrantedAccess |= (PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_QUERY_INFORMATION | PROCESS_TERMINATE |
                              PROCESS_CREATE_THREAD | PROCESS_DUP_HANDLE | PROCESS_CREATE_PROCESS | PROCESS_SET_INFORMATION | STANDARD_RIGHTS_ALL | PROCESS_SET_QUOTA);

            Process->GrantedAccess = GrantedAccess;
        }

        // Since the process token is being set, Set the device map for process to NULL.
        // During the next reference to the process' device map, the object manager will set the device map for the process
...
NTSTATUS NtQueryInformationProcess(__in HANDLE ProcessHandle,
                                   ...
                                   __out_opt PULONG ReturnLength)
{
    ...
    case ProcessImageFileName:
    {
        ...
        // SeLocateProcessImageName will allocate space for a UNICODE_STRING and point pTempNameInfo at that string.  This memory will be freed later in the routine.
    ...
    case ProcessBasicInformation:
        ...
        // Either of these may cause an access violation.
        // The exception handler will return access violation as status code. No further cleanup needs to be done.
        try {
    ...
    case ProcessVmCounters:
        ...
        KeStackAttachProcess(&Process->Pcb, &ApcState);
        st = MmQueryWorkingSetInformation(&VmCounters.PeakWorkingSetSize, &VmCounters.WorkingSetSize, &MinimumWorkingSetSize, &MaximumWorkingSetSize, &HardEnforcement);
        KeUnstackDetachProcess(&ApcState);
    ...
    case ProcessTimes:
        ...
        SysUserTime.KernelTime.QuadPart = UInt32x32To64(TotalKernel, KeMaximumIncrement);
        SysUserTime.UserTime.QuadPart = UInt32x32To64(TotalUser, KeMaximumIncrement);
    ...
    case ProcessLdtInformation:
        st = ObReferenceObjectByHandle(ProcessHandle, PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, PsProcessType, PreviousMode, &Process, NULL);
        if (!NT_SUCCESS(st)) {
            return st;
        }
    ...
    case ProcessWx86Information:
        ...
        // The returned flags is used as a BOOLEAN to indicate whether the ProcessHandle specifies a NtVdm Process.  In another words, the caller can simply do a
    ...
    case ProcessWow64Information:
        ...
        // Acquire process rundown protection as we are about to look at process structures torn down at process exit.
    ...
    case ProcessHandleTracing:
    {
        ...
        StacksLeft = (ProcessInformationLength - FIELD_OFFSET(PROCESS_HANDLE_TRACING_QUERY, HandleTrace)) / sizeof(Pht->HandleTrace[0]);
                    ...
                    for (i = 0; i < DebugInfo->TableSize; i++) {
                        ...
                        if ((Pht->Handle == Trace.Handle || Pht->Handle == 0) && Trace.Type != 0) {
                            ...
                            if (StacksLeft > 0) {
                                ...
                                RtlCopyMemory(NextTrace->Stacks, Trace.StackTrace, min(sizeof(NextTrace->Stacks), sizeof(Trace.StackTrace)));
                                NextTrace++;
                            } else {
                                st = STATUS_INFO_LENGTH_MISMATCH;
                            }
    ...
    case ProcessImageInformation:
        // This code queries the section image information for the current process.
...
/*
Routine Description:
    This function tests whether a debug port or an exception port is attached to the current process and returns a corresponding value.
    This function is used to bypass raising an exception through the system when no associated ports are present.
...
NTSTATUS PspSetQuotaLimits(IN HANDLE ProcessHandle, IN PROCESSINFOCLASS ProcessInformationClass, IN PVOID ProcessInformation, IN ULONG ProcessInformationLength, IN KPROCESSOR_MODE PreviousMode)
{
    ...
    try {
        if (ProcessInformationLength == sizeof(QUOTA_LIMITS)) {
            RtlCopyMemory(&RequestedLimits, ProcessInformation, sizeof(QUOTA_LIMITS));
    ...
    if (RequestedLimits.Flags & ~(QUOTA_LIMITS_HARDWS_MAX_ENABLE | QUOTA_LIMITS_HARDWS_MAX_DISABLE | QUOTA_LIMITS_HARDWS_MIN_ENABLE | QUOTA_LIMITS_HARDWS_MIN_DISABLE)) {
        return STATUS_INVALID_PARAMETER;
    }

    // Disallow both enable and disable bits set at the same time.
    if (PS_TEST_ALL_BITS_SET(RequestedLimits.Flags,
                             QUOTA_LIMITS_HARDWS_MIN_ENABLE | QUOTA_LIMITS_HARDWS_MIN_DISABLE) || PS_TEST_ALL_BITS_SET(RequestedLimits.Flags, QUOTA_LIMITS_HARDWS_MAX_ENABLE | QUOTA_LIMITS_HARDWS_MAX_DISABLE)) {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (RequestedLimits.Reserved1 != 0 || RequestedLimits.Reserved2 != 0 || RequestedLimits.Reserved3 != 0 || RequestedLimits.Reserved4 != 0 || RequestedLimits.Reserved5 != 0) {
        return STATUS_INVALID_PARAMETER;
    }

    st = ObReferenceObjectByHandle(ProcessHandle, PROCESS_SET_QUOTA, PsProcessType, PreviousMode, &Process, NULL);
    if (!NT_SUCCESS(st)) {
        return st;
    }
    ...
    // If the process has no quota block, all that can be done is a quota set operation.
    ...
    if ((Process->QuotaBlock == &PspDefaultQuotaBlock) && (RequestedLimits.MinimumWorkingSetSize == 0 || RequestedLimits.MaximumWorkingSetSize == 0)) {
        ...
        // Otherwise punt.
        if (InterlockedCompareExchangePointer(&Process->QuotaBlock, NewQuotaBlock, &PspDefaultQuotaBlock) != &PspDefaultQuotaBlock) {
            ...
            do {
                ...
                if (Job != NULL) {
                    ...
                    if (Job->LimitFlags & JOB_OBJECT_LIMIT_WORKINGSET) {
                        // Don't let a process in a job change if job limits are applied except purge requests which can always be done.
                        EnableHardLimits = MM_WORKING_SET_MAX_HARD_ENABLE;
                        OkToIncrease = TRUE;
                ...
                ReturnStatus = MmAdjustWorkingSetSizeEx(RequestedLimits.MinimumWorkingSetSize, RequestedLimits.MaximumWorkingSetSize, FALSE, OkToIncrease, EnableHardLimits, &IncreasePerformed);
                if (!NT_SUCCESS(ReturnStatus) && IgnoreError) {
                    MmEnforceWorkingSetLimit(Process, EnableHardLimits);
                }
                ...
                // We loop here in case this process was added to a job after we checked but before we set the limits
            } while (Process->Job != Job);

            if (FreeCtx) {
                PspSinglePrivCheckAudit(PrivilegeUsed, &PrivCtx);
            }
        }
    }
    ...
    */
{
    ...
    {
        PPAGEFAULT_HISTORY WorkingSetCatcher;
        st = ObReferenceObjectByHandle(ProcessHandle, PROCESS_SET_INFORMATION, PsProcessType, PreviousMode, &Process, NULL);
        if (!NT_SUCCESS(st)) {
            return st;
        }
    ...
    {
        // This code is used to boost the priority of all threads within a process.
        // It cannot be used to change a thread into a realtime class, or to lower the priority of a thread.
        // The argument is a boost value that is added to the base priority of the specified process.
        if (ProcessInformationLength != sizeof(ULONG)) {
            return STATUS_INFO_LENGTH_MISMATCH;
        }
    ...
    case ProcessLdtInformation:
        st = ObReferenceObjectByHandle(ProcessHandle, PROCESS_SET_INFORMATION | PROCESS_VM_WRITE, PsProcessType, PreviousMode, &Process, NULL);
        if (!NT_SUCCESS(st)) {
            return st;
        }
    ...
    case ProcessPriorityBoost:
        ...
        // Acquire rundown protection to give back the correct error if the process has or is being terminated.
        if (!ExAcquireRundownProtection(&Process->RundownProtect)) {
            st = STATUS_PROCESS_IS_TERMINATING;
        } else {
            PLIST_ENTRY Next;
    ...
    case ProcessDeviceMap:
        ...
        // The devmap fields here are synchronized using a private ob spinlock. We don't need to protect with a lock at this level.
    ...
    case ProcessSessionInformation:
        ...
        st = ObReferenceObjectByHandle(ProcessHandle, PROCESS_SET_INFORMATION | PROCESS_SET_SESSIONID, PsProcessType, PreviousMode, &Process, NULL);
        if (!NT_SUCCESS(st)) {
            return st;
        }
    ...
    case ProcessHandleTracing:
    {
        ...
        if (ProcessInformationLength != 0) {
            if (ProcessInformationLength != sizeof(PROCESS_HANDLE_TRACING_ENABLE) && ProcessInformationLength != sizeof(PROCESS_HANDLE_TRACING_ENABLE_EX)) {
                return STATUS_INFO_LENGTH_MISMATCH;
            }
    ...
    case ProcessExecuteFlags:
        // This code is used to enable execution flags for user thread stacks within a process.
...
NTSTATUS NtQueryInformationThread(__in HANDLE ThreadHandle,
    ...
    */
{
    ...
    case ThreadQuerySetWin32StartAddress:
        ...
        return st;
    case ThreadPerformanceCount:// Query thread cycle counter.
    ...
    case ThreadIsIoPending:
        ...
        // Since the result is worthless the second its fetched this isn't a problem.
    ...
    */
{
    ...
    case ThreadPriority:
        ...
        if (Priority >= LOW_REALTIME_PRIORITY) {
            // Increasing the priority of a thread beyond LOW_REALTIME_PRIORITY is a privileged operation.
    ...
    case ThreadBasePriority:
        ...
        if (BasePriority > THREAD_BASE_PRIORITY_MAX || BasePriority < THREAD_BASE_PRIORITY_MIN) {
            if (BasePriority == THREAD_BASE_PRIORITY_LOWRT + 1 || BasePriority == THREAD_BASE_PRIORITY_IDLE - 1) {
                ...
                if (PsGetCurrentProcessByThread(CurrentThread) == ExpDefaultErrorPortProcess || Process->PriorityClass == PROCESS_PRIORITY_CLASS_REALTIME) {
        ...
        // If the thread is running within a job object, and the job object has a priority class limit,
        // do not allow priority adjustments that raise the thread's priority, unless the priority class is realtime
        Job = Process->Job;
    ...
    case ThreadImpersonationToken:
        ...
        st = ObReferenceObjectByHandle(ThreadHandle, THREAD_SET_THREAD_TOKEN, PsThreadType, PreviousMode, &Thread, NULL);
        if (!NT_SUCCESS(st)) {
            return st;
        }

        // Check for proper access to (and type of) the token, and assign it as the thread's impersonation token.
    ...
    case ThreadZeroTlsCell:
        ...
        // This code isn't 100% correct since threads have a conversion state where they are changing from 64 to 32 and they don't have a TEB32 yet.
        // Fortunately, the slots will be zero when the thread is created so no damage is done by not clearing it here.

        // Note that the test for the process type is inside the inner loop.
        // This is bad programming, but this function is hardly time constrained and fixing this with complex macros would not be worth it due to the loss of clarity.
        for (Thread = PsGetNextProcessThread(Process, NULL); Thread != NULL; Thread = PsGetNextProcessThread(Process, Thread)) {
                    ...
                    try {
                        ...
                        if (TlsIndex > TLS_MINIMUM_AVAILABLE - 1) {
                            if (TlsIndex < (TLS_MINIMUM_AVAILABLE + TLS_EXPANSION_SLOTS) - 1) {
                                // This is an expansion slot, so see if the thread has an expansion cell
#if defined(_WIN64)
                                if (Process->Wow64Process) { //Wow64 process.
                                    if (Teb32) {
...
/*
Routine Description:
    This function collects data about page faults and stores information about the page fault in the current process's data structure.
Arguments:
...
*/
{
    ...
    // Both transition and demand zero faults count as soft faults. Only disk reads count as hard faults.
    ...
    // Store the Pc and Va values in the buffer. Use the least sig. bit of the Va to store whether it was a soft or hard fault
...
VOID PsSetProcessPriorityByClass(__inout PEPROCESS Process, __in PSPROCESSPRIORITYMODE PriorityMode)
{
    KPRIORITY BasePriority;
    SCHAR QuantumReset;
    ...
    BasePriority = PspComputeQuantumAndPriority(Process, PriorityMode, &QuantumReset);
    KeSetPriorityAndQuantumProcess(&Process->Pcb, BasePriority, QuantumReset);
}


KPRIORITY PspComputeQuantumAndPriority(__inout PEPROCESS Process, __in PSPROCESSPRIORITYMODE PriorityMode, __out PSCHAR QuantumReset)
/*
Routine Description:
    This function computes the base priority and quantum reset values for the specified process and sets the memory priority of the process.
Arguments:
    Process - Supplies a pointer to a executive process object.
    PriorityMode - Supplies the priority mode.
    QuantumReset - Supplies a pointer to a variable that receives the computed quantum reset value.
Return Value:
...
Environment:
    On x86 this function needs to build an EBP frame.
    The function KeSwitchKernelStack depends on this fact.
    The '#pragma optimize ("y",off)' below disables frame pointer omission for all builds.
*/
{
    PVOID NewStack;
    PVOID OldStack;
    ...
    // If the thread is using the shadow service table, then an attempt is being made to convert a thread that has already been converted,
    // or a limit violation has occured on the Win32k system service table.
#if defined(_AMD64_)
    ...
    if (!Thread->Tcb.LargeStack) {
        ...
        NewStack = MmCreateKernelStack(TRUE, Node->NodeNumber);
        if (!NewStack) {
            try {
        ...
        // Switching kernel stacks will copy the base trap frame. This needs to be protected from context changes by disabling kernel APC's.
    ...
    // Switch the thread to use the shadow system service table which will enable it to execute Win32k services.
#if defined(_AMD64_)
