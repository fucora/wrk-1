...
Routine Description:
    ...
    The first process (if Process is NULL) or subsequent process (if process not NULL) is returned on each call.
    If process is not NULL then this process must have previously been obtained by a call to PsGetNextProcess.
    Enumeration may be terminated early by calling PsQuitNextProcess on the last non-NULL process returned by PsGetNextProcess.
...
*/
{
    ...
    ListEntry = ListEntry->Flink) {
        ...
        // Processes are removed from this list during process objected deletion (object reference count goes to zero).
        // To prevent double deletion of the process we need to do a safe reference here.
        if (ObReferenceObjectSafe(NewProcess)) {
            break;
        }
        NewProcess = NULL;
...
Arguments:
    ...
    Thread  - Thread to start enumeration from.
              This must have been obtained from previous call to PsGetNextProcessThread. 
              If NULL enumeration starts at the first non-terminating thread in the process.
Return Value:
    PETHREAD - Pointer to a non-terminated process thread or a NULL if there are non. 
               This thread must be passed either to another call to PsGetNextProcessThread or PsQuitNextProcessThread.
*/
{
    PLIST_ENTRY ListEntry;
    ...
    for (ListEntry = (Thread == NULL) ? Process->ThreadListHead.Flink : Thread->ThreadListEntry.Flink; ; ListEntry = ListEntry->Flink) {
...
Routine Description:
    ...
    The first job (if Job is NULL) or subsequent jobs (if Job not NULL) is returned on each call.
    If Job is not NULL then this job must have previously been obtained by a call to PsGetNextJob.
    Enumeration may be terminated early by calling PsQuitNextJob on the last non-NULL job returned by PsGetNextJob.
...
*/
{
    ...
    for (ListEntry = (Job == NULL) ? PspJobList.Flink : Job->JobLinks.Flink; ListEntry != &PspJobList; ListEntry = ListEntry->Flink) {
        NewJob = CONTAINING_RECORD(ListEntry, EJOB, JobLinks);

        // Jobs are removed from this list during job objected deletion (object reference count goes to zero). 
        // To prevent double deletion of the job we need to do a safe reference here.
...
Arguments:
    ...
    Process - Process to start enumeration from. This must have been obtained from previous call to PsGetNextJobProcess.
              If NULL enumeration starts at the first non-terminating process in the Job.
Return Value:
    PEPROCESS - Pointer to a non-terminated process or a NULL if there are non. 
                This process must be passed either to another call to PsGetNextJobProcess or PsQuitNextJobProcess.
*/
{
    PLIST_ENTRY ListEntry;
    ...
    for (ListEntry = (Process == NULL) ? Job->ProcessListHead.Flink : Process->JobLinks.Flink; ; ListEntry = ListEntry->Flink) {
...
NTSTATUS NtGetNextProcess(__in HANDLE ProcessHandle,
...
*/
{
    ...
    while (1) {
        if (NewProcess->GrantedAccess != 0) {
            Status = ObOpenObjectByPointer(NewProcess, HandleAttributes, &AccessState, 0, PsProcessType, PreviousMode, &Handle);
            if (NT_SUCCESS(Status)) {
                try {
...
NTSTATUS NtGetNextThread(__in HANDLE ProcessHandle, 
...
*/
{
    ...
    if (ThreadHandle == NULL) {
        ...
        Status = ObReferenceObjectByHandle(ThreadHandle, 0, PsProcessType, PreviousMode, &Thread, NULL);
        if (!NT_SUCCESS(Status)) {
            ObDereferenceObject(Process);
            return Status;
        }
    ...
    while (1) {
        if (NewThread->GrantedAccess != 0) {
            Status = ObOpenObjectByPointer(NewThread, HandleAttributes, &AccessState, 0, PsThreadType, PreviousMode, &Handle);
            if (NT_SUCCESS(Status)) {
                try {
