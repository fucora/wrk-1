...
Abstract:
    This module contains the public data structures and procedure prototypes for the Kernel Debugger sub-component of NTOS.
*/
...
BOOLEAN KdInitSystem(IN ULONG Phase, IN PLOADER_PARAMETER_BLOCK LoaderBlock);
...
NTSTATUS KdEnableDebuggerWithLock(IN BOOLEAN TakeLock);
NTSTATUS KdDisableDebuggerWithLock(IN BOOLEAN TakeLock);
...
NTKERNELAPI BOOLEAN KdPollBreakIn (VOID);
...
// Data structure for passing information to KdpReportLoadSymbolsStateChange function via the debug trap
...
// signals the event when the system time has slipped due to debugger activity.
...
NTKERNELAPI NTSTATUS KdDisableDebugger(VOID);
NTKERNELAPI NTSTATUS KdEnableDebugger(VOID);

// KdRefreshDebuggerPresent attempts to communicate with the debugger host machine to refresh the state of KdDebuggerNotPresent.
// It returns the state of KdDebuggerNotPresent while the kd locks are held.
// KdDebuggerNotPresent may immediately change state after the kd locks are released so it may not match the return value.
...
NTKERNELAPI NTSTATUS KdPowerTransition(IN DEVICE_POWER_STATE newDeviceState);

// DbgPrint strings will always be logged to a circular buffer. This
// function may be called directly by the debugger service trap handler even when the debugger is not enabled.
...
NTSTATUS KdSetDbgPrintBufferSize(IN ULONG Size);
...
Routine Description:
    ...
    This function should be called by routines that run at an IRQL above clock level that want to be broken in by CTRL-C requests from the debugger.
    Crashdump and hiber, for example, run at HIGH_LEVEL and explicitly need to poll for breaking.
*/
{
...
// Allow raw tracing data to be exported to the host over the kd protocol.
...
NTSTATUS KdReadRemoteFile(IN HANDLE Handle, IN ULONG64 Offset, OUT PVOID Buffer, IN ULONG Length, OUT PULONG Completed);
NTSTATUS KdWriteRemoteFile(IN HANDLE Handle, IN ULONG64 Offset, IN PVOID Buffer, IN ULONG Length, OUT PULONG Completed);
NTSTATUS KdCloseRemoteFile(IN HANDLE Handle);
NTSTATUS KdPullRemoteFile(IN PUNICODE_STRING FileName, IN ULONG FileAttributes, IN ULONG CreateDisposition, IN ULONG CreateOptions);
NTSTATUS KdPushRemoteFile(IN PUNICODE_STRING FileName, IN ULONG FileAttributes, IN ULONG CreateDisposition, IN ULONG CreateOptions);
#endif  // _KD_