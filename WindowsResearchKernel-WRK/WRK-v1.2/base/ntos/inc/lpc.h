...
// begin_ntosp
NTKERNELAPI NTSTATUS LpcRequestPort(__in PVOID PortAddress, __in PPORT_MESSAGE RequestMessage);
NTKERNELAPI NTSTATUS LpcRequestWaitReplyPort(__in PVOID PortAddress, __in PPORT_MESSAGE RequestMessage, __out PPORT_MESSAGE ReplyMessage);
NTKERNELAPI NTSTATUS LpcRequestWaitReplyPortEx (IN PVOID PortAddress, IN PPORT_MESSAGE RequestMessage, OUT PPORT_MESSAGE ReplyMessage);
NTKERNELAPI NTSTATUS LpcDisconnectPort (IN PVOID Port);
...
typedef struct _LPCP_PORT_ZONE {
    KEVENT FreeEvent;           // Autoclearing event that is whenever the zone free list goes from empty to non-empty
    ULONG MaxPoolUsage;
    ULONG GrowSize;
    ZONE_HEADER Zone;
...
typedef struct _LPCP_MESSAGE {
    ...
    PVOID SenderPort;
    PETHREAD RepliedToThread;               // Filled in when reply is sent so recipient of reply can dereference it.
