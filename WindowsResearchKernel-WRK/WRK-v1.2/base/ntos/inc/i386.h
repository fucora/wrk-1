...
//          won't get context switched between the call to it and the variable reference, OR, were we don't care, (ie TEB pointer)

//  NOTE - we must not macro out things we export
//      Things like KeFlushIcache and KeFlushDcache cannot be macro-ed
//      out because external code (like drivers) will want to import them by name.
//      Therefore, the defines below that turn them into nothing are inappropriate.
...
//              or ring-3.  Ring-1 or Ring-2 support will require changing this value and all of the code that refers to it.
...
FORCEINLINE VOID KiFlushSingleTb (IN PVOID Virtual)
{
    __asm {
        mov eax, Virtual
        invlpg [eax]
...
FORCEINLINE VOID KiFlushProcessTb (VOID)
{
    __asm {
        mov eax, cr3
        mov cr3, eax
...
// The interrupt direction bitmap is used on Pentium to allow the processor to emulate V86 mode software interrupts for us.
// There is one for each IOPM.  It is located by subtracting 32 from the IOPM base in the Tss.
...
// dumps the processor state to the processor state frame for debugger to examine.
...
//      last eight bytes of a cache line. This makes the dispatcher lock and the context swap lock lie in separate cache lines.
...
// ISR timing data.
    ...
    FX_SAVE_AREA NpxSaveArea;// Npx save area - 16-byte aligned.
...
// purpose). Therefore, these fields are overlaid with other data to get better cache locality.
...
FORCEINLINE ULONG KeGetContextSwitches (PKPRCB Prcb)
{
    PKPCR Pcr;
...
BOOLEAN KeDisableInterrupts (VOID);
...
#else
FORCEINLINE LONG FASTCALL InterlockedExchange(IN OUT LONG volatile *Target, IN LONG Value)
{
    __asm {
        mov     eax, Value
        mov     ecx, Target
...
#else
FORCEINLINE LONG FASTCALL InterlockedExchangeAdd(IN OUT LONG volatile *Addend, IN LONG Increment)
{
    __asm {
        mov     eax, Increment
        mov     ecx, Addend
...
FORCEINLINE LONG FASTCALL InterlockedCompareExchange(IN OUT LONG volatile *Destination, IN LONG Exchange, IN LONG Comperand)
{
    __asm {
        mov     eax, Comperand
        mov     ecx, Destination
...
FORCEINLINE LONG InterlockedAnd (IN OUT LONG volatile *Target, LONG Set)
{
    LONG i;
    LONG j;
...
FORCEINLINE LONG InterlockedOr (IN OUT LONG volatile *Target, IN LONG Set)
{
    LONG i;
    LONG j;
...
FORCEINLINE PKPCR NTAPI KeGetPcr(VOID)
{
#if NT_UP
...
FORCEINLINE ULONG KeGetCurrentNodeShiftedColor (VOID)
{
...
FORCEINLINE PKPRCB NTAPI KeGetCurrentPrcb (VOID)
{
#if (_MSC_FULL_VER >= 13012035)
...
FORCEINLINE ULONG NTAPI KeGetCurrentProcessorNumber(VOID)
{
#if (_MSC_FULL_VER >= 13012035)
...
// WARNING: This inline macro can not be used for device drivers or HALs they must call the kernel function KeGetCurrentThread.
// WARNING: This inline macro is always MP enabled because filesystems utilize it
FORCEINLINE struct _KTHREAD * NTAPI KeGetCurrentThread (VOID)
{
#if (_MSC_FULL_VER >= 13012035)
...
// WARNING: This inline macro is always MP enabled because filesystems utilize it
FORCEINLINE ULONG NTAPI KeIsExecutingDpc(VOID)
{
#if (_MSC_FULL_VER >= 13012035)
...
#define KiReturnHandlerAddressFromIDT(Vector)  MAKEULONG(KiPcr()->IDT[HalVectorToIDTEntry(Vector)].ExtendedOffset, KiPcr()->IDT[HalVectorToIDTEntry(Vector)].Offset)
...
#if defined(NT_UP)
// On UP x86 systems, FP state is lazy saved and loaded.
// If this thread owns the current FP context, clear the ownership field so we will not try to save to this thread after it has been terminated.
...
ULONG Ki386DispatchOpcode (IN PKTRAP_FRAME TrapFrame);
NTSTATUS NTAPI Ke386SetVdmInterruptHandler (IN struct _KPROCESS *Process, IN ULONG Interrupt, IN USHORT Selector, IN ULONG  Offset, IN BOOLEAN Gate32);
#endif //_X86_
// end_ntosp
