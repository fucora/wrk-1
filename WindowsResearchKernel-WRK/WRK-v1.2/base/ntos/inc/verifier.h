...
NTSTATUS VfGetScatterGatherList (
...
// VfFailSystemBIOS(major, minor, VFFAILURE_FAIL_LOGO, &perMinorFlags, "Driver at %Routine returned %Ulong", "%Ulong%Routine", value, routine);
VOID
VfFailSystemBIOS(
    IN      ULONG               BugCheckMajorCode,
    IN      ULONG               BugCheckMinorCode,
...
typedef enum {
    VFOBJTYPE_DRIVER = 0,// Driver object
    VFOBJTYPE_DEVICE,// Physical Device Object pointing to hardware
    VFOBJTYPE_SYSTEM_BIOS// System BIOS (no object)
} VF_OBJECT_TYPE;
...
VERIFIER_MODE VfVerifierRunningMode (VOID);
LOGICAL VfIsVerifierEnabled (VOID);
#endif