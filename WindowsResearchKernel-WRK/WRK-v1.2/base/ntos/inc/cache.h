...
typedef VOID (*PRELEASE_FROM_LAZY_WRITE) (__in PVOID Context);

// This routine is called by the Lazy Writer prior to doing a readahead.
typedef BOOLEAN (*PACQUIRE_FOR_READ_AHEAD) (__in PVOID Context, __in BOOLEAN Wait);

// This routine releases the Context acquired above.
typedef VOID (*PRELEASE_FROM_READ_AHEAD) (__in PVOID Context);
...
typedef VOID (*PFLUSH_TO_LSN) (__in PVOID LogHandle, __in LARGE_INTEGER Lsn);
...
NTKERNELAPI BOOLEAN CcUninitializeCacheMap (__in PFILE_OBJECT FileObject, __in_opt PLARGE_INTEGER TruncateSize, __in_opt PCACHE_UNINITIALIZE_EVENT UninitializeEvent);
NTKERNELAPI VOID CcSetFileSizes (__in PFILE_OBJECT FileObject, __in PCC_FILE_SIZES FileSizes);

//  VOID CcFastIoSetFileSizes (IN PFILE_OBJECT FileObject, IN PCC_FILE_SIZES FileSizes);
...
NTKERNELAPI VOID CcSetDirtyPageThreshold (__in PFILE_OBJECT FileObject, __in ULONG DirtyPageThreshold);
NTKERNELAPI VOID CcFlushCache (__in PSECTION_OBJECT_POINTERS SectionObjectPointer, __in_opt PLARGE_INTEGER FileOffset, __in ULONG Length, __out_opt PIO_STATUS_BLOCK IoStatus);
NTKERNELAPI LARGE_INTEGER CcGetFlushedValidData (__in PSECTION_OBJECT_POINTERS SectionObjectPointer, __in BOOLEAN BcbListHeld);

// end_ntifs
NTKERNELAPI VOID CcZeroEndOfLastPage (IN PFILE_OBJECT FileObject);

// begin_ntifs
NTKERNELAPI BOOLEAN CcZeroData (__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER StartOffset, __in PLARGE_INTEGER EndOffset, __in BOOLEAN Wait);

NTKERNELAPI PVOID CcRemapBcb (__in PVOID Bcb);
NTKERNELAPI VOID CcRepinBcb (__in PVOID Bcb);
NTKERNELAPI VOID CcUnpinRepinnedBcb (__in PVOID Bcb, __in BOOLEAN WriteThrough, __out PIO_STATUS_BLOCK IoStatus);
NTKERNELAPI PFILE_OBJECT CcGetFileObjectFromSectionPtrs (__in PSECTION_OBJECT_POINTERS SectionObjectPointer);
NTKERNELAPI PFILE_OBJECT CcGetFileObjectFromBcb (__in PVOID Bcb);
...
NTKERNELAPI VOID CcMdlReadComplete (__in PFILE_OBJECT FileObject, __in PMDL MdlChain);
...
NTKERNELAPI VOID CcMdlWriteComplete (__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER FileOffset, __in PMDL MdlChain);
NTKERNELAPI VOID CcMdlWriteAbort (__in PFILE_OBJECT FileObject, __in PMDL MdlChain);
...
NTKERNELAPI VOID CcScheduleReadAhead (__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER FileOffset, __in ULONG Length);

//  The following routine allows a caller to wait for the next batch of lazy writer work to complete.
//  In particular, this provides a mechanism for a caller to be sure that all available lazy closes at the time of this call have issued.
NTKERNELAPI NTSTATUS CcWaitForCurrentLazyWriterActivity (VOID);

// This routine changes the read ahead granularity for a file, which is PAGE_SIZE by default.
NTKERNELAPI VOID CcSetReadAheadGranularity (__in PFILE_OBJECT FileObject, __in ULONG Granularity);
...
NTKERNELAPI VOID CcSetDirtyPinnedData (__in PVOID BcbVoid, __in_opt PLARGE_INTEGER Lsn);
NTKERNELAPI VOID CcUnpinData (__in PVOID Bcb);
NTKERNELAPI VOID CcSetBcbOwnerPointer (__in PVOID Bcb, __in PVOID OwnerPointer);
NTKERNELAPI VOID CcUnpinDataForThread (__in PVOID Bcb, __in ERESOURCE_THREAD ResourceThreadId);

// end_ntifs

// The following routines are in logsup.c, and provide special Cache Manager support for storing Lsns with dirty file pages, and performing subsequent operations based on them.
...
NTKERNELAPI LARGE_INTEGER CcGetLsnForFileObject(__in PFILE_OBJECT FileObject, __out_opt PLARGE_INTEGER OldestLsn);
