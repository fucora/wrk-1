...
Abstract:
    This module contains the public (external) header file for the kernel that must be included after all other header files.
...
NTKERNELAPI VOID KiCheckForKernelApcDelivery (VOID);
VOID FASTCALL KiAcquireGuardedMutex (IN PKGUARDED_MUTEX Mutex);


FORCEINLINE VOID KeEnterGuardedRegionThread (IN PKTHREAD Thread)
/*
Routine Description:
    ...
    N.B. The following code does not require any interlocks.
         ...
         1) On an MP system, the thread cannot be running on two processors as once,
         and 2) if the thread is
         is interrupted to deliver a kernel mode APC which also calls this routine, the values read and stored will stack and unstack properly.
Arguments:
...
FORCEINLINE VOID KeEnterGuardedRegion (VOID)
/*
Routine Description:
    This function disables special kernel APC's for the current thread.
    N.B. The following code does not require any interlocks.
...
FORCEINLINE VOID KeLeaveGuardedRegionThread (IN PKTHREAD Thread)
/*
Routine Description:
...
FORCEINLINE VOID KeLeaveGuardedRegion (VOID)
/*
Routine Description:
    This function enables special kernel APC's.
    N.B. The following code does not require any interlocks.
...
FORCEINLINE VOID KeEnterCriticalRegionThread (PKTHREAD Thread)
/*
Routine Description:
    This function disables kernel APC's for the current thread.
    N.B. The following code does not require any interlocks.
         There are two cases of interest:
         1) On an MP system, the thread cannot
...
FORCEINLINE VOID KeLeaveCriticalRegionThread (IN PKTHREAD Thread)
/*
Routine Description:
...
Routine Description:
    This function enables normal kernel APC's for the current thread.
    N.B. The following code does not require any interlocks. There are
         two cases of interest: 1) On an MP system, the thread cannot be running on two processors as once,
         and 2) if the thread is is interrupted to deliver a kernel mode APC which also calls this routine,
         the values read and stored will stack and unstack properly.
*/
{
...
FORCEINLINE BOOLEAN KeAreApcsDisabled (VOID)
/*
Routine description:
...
Arguments:
    None.
Return Value:
    If either the kernel or special APC disable count is nonzero, then a value of TRUE is returned. Otherwise, a value of FALSE is returned.
*/
{
...
FORCEINLINE BOOLEAN KeAreAllApcsDisabled (VOID)
/*
Routine description:
...
Arguments:
    None.
Return Value:
    If either the special APC disable count is nonzero or the IRQL is greater than or equal to APC_LEVEL, then a value of TRUE is returned.
    Otherwise, a value of FALSE is returned.
*/
{
    return (BOOLEAN)((KeGetCurrentThread()->SpecialApcDisable != 0) || (KeGetCurrentIrql() >= APC_LEVEL));
}


FORCEINLINE VOID FASTCALL KeInitializeGuardedMutex (IN PKGUARDED_MUTEX Mutex)
/*
Routine Description:
...
FORCEINLINE VOID FASTCALL KeAcquireGuardedMutex (IN PKGUARDED_MUTEX Mutex)
/*
Routine Description:
    This function enters a guarded region and acquires ownership of a guarded mutex.
Arguments:
    Mutex  - Supplies a pointer to a guarded mutex.
Return Value:
    None.
*/
{
    ...
    // Enter a guarded region and attempt to acquire ownership of the guarded mutex.
...
FORCEINLINE VOID FASTCALL KeReleaseGuardedMutex (IN PKGUARDED_MUTEX Mutex)
/*
Routine Description:
    This function releases ownership of a guarded mutex and leaves a guarded region.
Arguments:
    Mutex - Supplies a pointer to a guarded mutex.
Return Value:
    None.
*/
{
    ...
    // If there are no waiters or a waiter has already been woken, then release the mutex. Otherwise, attempt to wake a waiter.
    if ((OldValue != 0) && ((OldValue & GM_LOCK_WAITER_WOKEN) == 0)) {
...
FORCEINLINE BOOLEAN FASTCALL KeTryToAcquireGuardedMutex (IN PKGUARDED_MUTEX Mutex)
/*
Routine Description:
    This function attempts to acquire ownership of a guarded mutex, and if successful, enters a guarded region.
Arguments:
    Mutex  - Supplies a pointer to a guarded mutex.
Return Value:
    If the guarded mutex was successfully acquired, then a value of TRUE is returned as the function value. Otherwise, a value of FALSE is returned.
*/
{
    PKTHREAD Thread;
    ...
    KeEnterGuardedRegionThread(Thread);
    if (!InterlockedBitTestAndReset(&Mutex->Count, GM_LOCK_BIT_V)) {
        // The guarded mutex is owned - leave the guarded region and return FALSE.
        KeLeaveGuardedRegionThread(Thread);
        KeYieldProcessor();
        return FALSE;
    } else {
        // Grant ownership of the guarded mutex to the current thread and return TRUE.
        Mutex->Owner = Thread;
...
FORCEINLINE VOID FASTCALL KeAcquireGuardedMutexUnsafe (IN PKGUARDED_MUTEX Mutex)
/*
Routine Description:
    This function acquires ownership of a guarded mutex, but does enter a guarded region.
Arguments:
    Mutex - Supplies a pointer to a guarded mutex.
Return Value:
    None.
...
FORCEINLINE VOID FASTCALL KeReleaseGuardedMutexUnsafe (IN PKGUARDED_MUTEX Mutex)
/*
Routine Description:
    This function releases ownership of a guarded mutex, and does not leave a guarded region.
Arguments:
    Mutex - Supplies a pointer to a guarded mutex.
Return Value:
    None.
...
FORCEINLINE PKTHREAD FASTCALL KeGetOwnerGuardedMutex (IN PKGUARDED_MUTEX Mutex)
/*
Routine Description:
...
Arguments:
    Mutex - Supplies a pointer to a guarded mutex.
Return Value:
    If the guarded mutex is owned, then a pointer to the owner thread is returned. Otherwise, NULL is returned.
*/
{
    return Mutex->Owner;
}
...
FORCEINLINE BOOLEAN FASTCALL KeIsGuardedMutexOwned (IN PKGUARDED_MUTEX Mutex)
/*
Routine Description:
...
Arguments:
    Mutex - Supplies a pointer to a guarded mutex.
Return Value:
    A value of TRUE is returned if the guarded mutex is owned. Otherwise, a value of FALSE is returned.
*/
{
