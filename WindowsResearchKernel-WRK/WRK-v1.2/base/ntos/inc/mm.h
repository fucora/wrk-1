...
extern PBOOLEAN Mm64BitPhysicalAddress;//  Indicates the system may do I/O to physical addresses above 4 GB.

// end_ntddk end_wdm end_nthal end_ntifs

#else
extern BOOLEAN Mm64BitPhysicalAddress;//  Indicates the system may do I/O to physical addresses above 4 GB.
#endif
...
extern ULONG MmSecondaryColorMask;// Mask for isolating secondary color from physical page number.
...
extern PFN_NUMBER MmAvailablePages;// Number of available pages.
extern PFN_COUNT MmNumberOfPhysicalPages;// Number of physical pages.
extern PFN_NUMBER MmLowestPhysicalPage;// Lowest physical page number on the system.
extern PFN_NUMBER MmHighestPhysicalPage;// Highest physical page number on the system.
...
ULONG MmGetNumberOfFreeSystemPtes (VOID);
...
extern ULONG_PTR MmSizeOfSystemCacheInPages;// Virtual size of system cache in pages.
...
PPHYSICAL_MEMORY_DESCRIPTOR MmInitializeMemoryLimits (IN PLOADER_PARAMETER_BLOCK LoaderBlock, IN PBOOLEAN IncludedType, IN OUT PPHYSICAL_MEMORY_DESCRIPTOR Memory OPTIONAL);
VOID MmFreeLoaderBlock (IN PLOADER_PARAMETER_BLOCK LoaderBlock);
VOID MmEnablePAT (VOID);
PVOID MmAllocateIndependentPages (IN SIZE_T NumberOfBytes, IN ULONG NodeNumber);
NTKERNELAPI BOOLEAN MmSetPageProtection (__in_bcount(NumberOfBytes) PVOID VirtualAddress, __in SIZE_T NumberOfBytes, __in ULONG NewProtect);
...
BOOLEAN MmShutdownSystem (IN ULONG);
...
// LOGICAL MmIsSessionLeaderProcess (__in PEPROCESS Process);
// Routine Description:
// This macro checks whether or not the process is the session leader on the system. Mm ensures that there is only ever one such process.
// Arguments:
//     Process - The EPROCESS object to query.
// Return Value:
//     TRUE if the passed process object is the session leader.
#define MmIsSessionLeaderProcess(Process)       ((Process)->Vm.Flags.SessionLeader == 1)
...
// MmMemoryIsLow is not for general consumption, this is only used by the balance set manager.
LOGICAL MmMemoryIsLow (VOID);
...
NTSTATUS MmInitializeHandBuiltProcess (IN PEPROCESS Process, OUT PULONG_PTR DirectoryTableBase);
NTSTATUS MmInitializeHandBuiltProcess2 (IN PEPROCESS Process);
...
NTKERNELAPI NTSTATUS MmGrowKernelStack (__in PVOID CurrentStack);
NTKERNELAPI NTSTATUS MmGrowKernelStackEx (__in PVOID CurrentStack, __in SIZE_T CommitSize);
...
// begin_ntosp
NTKERNELAPI NTSTATUS MmAdjustWorkingSetSize (__in SIZE_T WorkingSetMinimumInBytes, __in SIZE_T WorkingSetMaximumInBytes, __in ULONG SystemCache, __in BOOLEAN IncreaseOkay);
// end_ntosp

NTSTATUS MmAdjustWorkingSetSizeEx (IN SIZE_T WorkingSetMinimum, IN SIZE_T WorkingSetMaximum, IN ULONG SystemCache, IN BOOLEAN IncreaseOkay, IN ULONG Flags, OUT PBOOLEAN IncreaseRequested);
...
BOOLEAN MmCheckForSafeExecution (IN PVOID InstructionPointer, IN PVOID StackPointer, IN PVOID BranchTarget, IN BOOLEAN PermitStackExecution);
...
#define MmEnoughMemoryForWrite()                         \
            ((MmAvailablePages > MmThrottleTop) || (((MmModifiedPageListHead.Total < 1000)) && (MmAvailablePages > MmThrottleBottom)))
...
NTKERNELAPI NTSTATUS MmUnmapViewOfSection (__in PEPROCESS Process, __in PVOID BaseAddress);

// end_ntosp begin_ntifs

NTKERNELAPI BOOLEAN MmForceSectionClosed (__in PSECTION_OBJECT_POINTERS SectionObjectPointer, __in BOOLEAN DelayClose);

// end_ntifs

NTSTATUS MmGetFileNameForSection (IN PVOID SectionObject, OUT POBJECT_NAME_INFORMATION *FileNameInfo);
NTSTATUS MmGetFileNameForAddress (IN PVOID ProcessVa, OUT PUNICODE_STRING FileName);
NTSTATUS MmRemoveVerifierEntry (IN PUNICODE_STRING ImageFileName);

// begin_ntddk begin_wdm begin_ntifs begin_ntosp

NTKERNELAPI NTSTATUS MmIsVerifierEnabled (__out PULONG VerifierFlags);
NTKERNELAPI NTSTATUS MmAddVerifierThunks (__in_bcount(ThunkBufferSize) PVOID ThunkBuffer, __in ULONG ThunkBufferSize);

// end_ntddk end_wdm end_ntifs end_ntosp

NTSTATUS MmAddVerifierEntry (IN PUNICODE_STRING ImageFileName);
...
NTSTATUS MmFlushSection (IN PSECTION_OBJECT_POINTERS SectionObjectPointer, IN PLARGE_INTEGER Offset OPTIONAL, IN SIZE_T RegionSize, OUT PIO_STATUS_BLOCK IoStatus, IN ULONG Flags);
...
NTKERNELAPI BOOLEAN MmFlushImageSection (__in PSECTION_OBJECT_POINTERS SectionPointer, __in MMFLUSH_TYPE FlushType);
NTKERNELAPI BOOLEAN MmCanFileBeTruncated (__in PSECTION_OBJECT_POINTERS SectionPointer, __in_opt PLARGE_INTEGER NewFileSize);


// end_ntifs

ULONG MmDoesFileHaveUserWritableReferences (IN PSECTION_OBJECT_POINTERS SectionPointer);
NTKERNELAPI BOOLEAN MmDisableModifiedWriteOfSection (__in PSECTION_OBJECT_POINTERS SectionObjectPointer);
...
NTSTATUS MmCopyToCachedPage (IN PVOID Address, IN PVOID UserBuffer, IN ULONG Offset, IN SIZE_T CountInBytes, IN LOGICAL ExposeZeroPageOk);
...
NTSTATUS MmDbgCopyMemory (IN ULONG64 UntrustedAddress, IN PVOID Buffer, IN ULONG Size, IN ULONG Flags);
LOGICAL MmDbgIsLowMemOk (IN PFN_NUMBER PageFrameIndex, OUT PPFN_NUMBER NextPageFrameIndex, IN OUT PULONG CorruptionOffset);
LOGICAL MmCanThreadFault (VOID);
...
NTKERNELAPI LOGICAL MmIsIoSpaceActive (__in PHYSICAL_ADDRESS StartAddress, __in SIZE_T NumberOfBytes);
...
NTKERNELAPI NTSTATUS MmMapUserAddressesToPage (__in_bcount(NumberOfBytes) PVOID BaseAddress, __in SIZE_T NumberOfBytes, __in PVOID PageAddress);
...
// LowPagePriority should be used when it is acceptable to the driver for the mapping request to fail if the system is low on resources.
...
// NormalPagePriority should be used when it is acceptable to the driver for the mapping request to fail if the system is very low on resources.
...
NTKERNELAPI PVOID MmMapLockedPagesSpecifyCache (__in PMDL MemoryDescriptorList,
...
NTKERNELAPI VOID MmUnmapLockedPages (__in PVOID BaseAddress, __in PMDL MemoryDescriptorList);
NTKERNELAPI __bcount(NumberOfBytes) PVOID MmAllocateMappingAddress (__in SIZE_T NumberOfBytes, __in ULONG PoolTag);
NTKERNELAPI VOID MmFreeMappingAddress (__in PVOID BaseAddress, __in ULONG PoolTag);
NTKERNELAPI PVOID MmMapLockedPagesWithReservedMapping (__in PVOID MappingAddress, __in ULONG PoolTag, __in PMDL MemoryDescriptorList, __in MEMORY_CACHING_TYPE CacheType);
NTKERNELAPI VOID MmUnmapReservedMapping (IN PVOID BaseAddress, IN ULONG PoolTag, IN PMDL MemoryDescriptorList);
...
NTKERNELAPI NTSTATUS MmAddPhysicalMemory (__in PPHYSICAL_ADDRESS StartAddress, __inout PLARGE_INTEGER NumberOfBytes);

// end_ntddk end_nthal end_ntifs
NTKERNELAPI NTSTATUS MmAddPhysicalMemoryEx (IN PPHYSICAL_ADDRESS StartAddress, IN OUT PLARGE_INTEGER NumberOfBytes, IN ULONG Flags);
// begin_ntddk begin_nthal begin_ntifs

NTKERNELAPI NTSTATUS MmRemovePhysicalMemory (__in PPHYSICAL_ADDRESS StartAddress, __inout PLARGE_INTEGER NumberOfBytes);

// end_ntddk end_nthal end_ntifs
NTKERNELAPI NTSTATUS MmRemovePhysicalMemoryEx (IN PPHYSICAL_ADDRESS StartAddress, IN OUT PLARGE_INTEGER NumberOfBytes, IN ULONG Flags);
// begin_ntddk begin_nthal begin_ntifs
...
NTKERNELAPI NTSTATUS MmMarkPhysicalMemoryAsGood (__in PPHYSICAL_ADDRESS StartAddress, __inout PLARGE_INTEGER NumberOfBytes);
NTKERNELAPI NTSTATUS MmMarkPhysicalMemoryAsBad (__in PPHYSICAL_ADDRESS StartAddress, __inout PLARGE_INTEGER NumberOfBytes);
...
NTKERNELAPI PMDL MmAllocatePagesForMdl (__in PHYSICAL_ADDRESS LowAddress, __in PHYSICAL_ADDRESS HighAddress, __in PHYSICAL_ADDRESS SkipBytes, __in SIZE_T TotalBytes);
NTKERNELAPI VOID MmFreePagesFromMdl (__in PMDL MemoryDescriptorList);
NTKERNELAPI __out_bcount(NumberOfBytes) PVOID MmMapIoSpace (__in PHYSICAL_ADDRESS PhysicalAddress, __in SIZE_T NumberOfBytes, __in MEMORY_CACHING_TYPE CacheType);
NTKERNELAPI VOID MmUnmapIoSpace (__in_bcount(NumberOfBytes) PVOID BaseAddress, __in SIZE_T NumberOfBytes);

// end_wdm end_ntddk end_ntifs end_ntosp

NTKERNELAPI VOID MmProbeAndLockSelectedPages (__inout PMDL MemoryDescriptorList, __in PFILE_SEGMENT_ELEMENT PagedSegmentArray, __in KPROCESSOR_MODE AccessMode, __in LOCK_OPERATION Operation);

// begin_ntddk begin_ntifs begin_ntosp

NTKERNELAPI __out_bcount(NumberOfBytes) PVOID MmMapVideoDisplay (__in PHYSICAL_ADDRESS PhysicalAddress, __in SIZE_T NumberOfBytes, __in MEMORY_CACHING_TYPE CacheType);
NTKERNELAPI VOID MmUnmapVideoDisplay (__in_bcount(NumberOfBytes) PVOID BaseAddress, __in SIZE_T NumberOfBytes);
NTKERNELAPI PHYSICAL_ADDRESS MmGetPhysicalAddress (__in PVOID BaseAddress);
NTKERNELAPI PVOID MmGetVirtualForPhysical (__in PHYSICAL_ADDRESS PhysicalAddress);
NTKERNELAPI __bcount(NumberOfBytes) PVOID MmAllocateContiguousMemory (__in SIZE_T NumberOfBytes, __in PHYSICAL_ADDRESS HighestAcceptableAddress);
NTKERNELAPI __bcount(NumberOfBytes) PVOID MmAllocateContiguousMemorySpecifyCache (
...
NTKERNELAPI VOID MmFreeContiguousMemory (__in PVOID BaseAddress);
NTKERNELAPI VOID MmFreeContiguousMemorySpecifyCache (__in_bcount(NumberOfBytes) PVOID BaseAddress, __in SIZE_T NumberOfBytes, __in MEMORY_CACHING_TYPE CacheType);
...
NTKERNELAPI __out_bcount(NumberOfBytes) PVOID MmAllocateNonCachedMemory (__in SIZE_T NumberOfBytes);
NTKERNELAPI VOID MmFreeNonCachedMemory (__in_bcount(NumberOfBytes) PVOID BaseAddress, __in SIZE_T NumberOfBytes);
NTKERNELAPI BOOLEAN MmIsAddressValid (__in PVOID VirtualAddress);
DECLSPEC_DEPRECATED_DDK NTKERNELAPI BOOLEAN MmIsNonPagedSystemAddressValid (__in PVOID VirtualAddress);
...
NTKERNELAPI VOID MmLockPageableSectionByHandle (__in PVOID ImageSectionHandle);
...
NTKERNELAPI VOID MmResetDriverPaging (__in PVOID AddressWithinSection);
NTKERNELAPI PVOID MmPageEntireDriver (__in PVOID AddressWithinSection);
NTKERNELAPI VOID MmUnlockPageableImageSection(__in PVOID ImageSectionHandle);

// end_wdm end_ntosp

// begin_ntosp

// Note that even though this function prototype says "HANDLE", MmSecureVirtualMemory does NOT return anything resembling a Win32-style handle.
// The return value from this function can ONLY be used with MmUnsecureVirtualMemory.
NTKERNELAPI HANDLE MmSecureVirtualMemory (__in_bcount(Size) PVOID Address, __in SIZE_T Size, __in ULONG ProbeMode);
NTKERNELAPI VOID MmUnsecureVirtualMemory (__in HANDLE SecureHandle);

// end_ntosp

NTKERNELAPI NTSTATUS MmMapViewInSystemSpace (__in PVOID Section, __deref_inout_bcount(*ViewSize) PVOID *MappedBase, __inout PSIZE_T ViewSize);
NTKERNELAPI NTSTATUS MmUnmapViewInSystemSpace (__in PVOID MappedBase);

// begin_ntosp
NTKERNELAPI NTSTATUS MmMapViewInSessionSpace (__in PVOID Section, __deref_inout_bcount(*ViewSize) PVOID *MappedBase, __inout PSIZE_T ViewSize);

// end_ntddk end_ntifs
NTKERNELAPI NTSTATUS MmCommitSessionMappedView (__in_bcount(ViewSize) PVOID MappedAddress, __in SIZE_T ViewSize);
// begin_ntddk begin_ntifs

NTKERNELAPI NTSTATUS MmUnmapViewInSessionSpace (__in PVOID MappedBase);
// end_ntosp
...
// Routine Description:
//     This routine returns the mapped address of an MDL.
//     If the Mdl is not already mapped or a system address, it is mapped.
// Arguments:
...
//     This macro is not usable by WDM 1.0 drivers as 1.0 did not include MmMapLockedPagesSpecifyCache.
//     The solution for WDM 1.0 drivers is to provide synchronization and set/reset the MDL_MAPPING_CAN_FAIL bit.
...
//     (((MDL)->MdlFlags & (MDL_MAPPED_TO_SYSTEM_VA)) ?
//                             ((MDL)->MappedSystemVa) :
//                ((((MDL)->MdlFlags & (MDL_SOURCE_IS_NONPAGED_POOL)) ? ((PVOID)((ULONG)(MDL)->StartVa | (MDL)->ByteOffset)) : (MmMapLockedPages((MDL),KernelMode)))))
...
NTKERNELAPI NTSTATUS MmCreateMirror (VOID);
...
LOGICAL MmIsSpecialPoolAddress (IN PVOID VirtualAddress);
LOGICAL MmUseSpecialPool (IN SIZE_T NumberOfBytes, IN ULONG Tag);
...
NTKERNELAPI LOGICAL MmIsDriverVerifying (__in struct _DRIVER_OBJECT *DriverObject);

// end_ntddk end_wdm end_ntosp

NTKERNELAPI LOGICAL MmTrimAllSystemPageableMemory (__in LOGICAL PurgeTransition);
...
NTKERNELAPI VOID MmMapMemoryDumpMdl (__inout PMDL MemoryDumpMdl);
...
NTKERNELAPI NTSTATUS MmSetBankedSection (
...
// Crash dump only
// Called to initialize the kernel memory for a kernel memory dump.

typedef NTSTATUS (*PMM_SET_DUMP_RANGE) (IN struct _MM_KERNEL_DUMP_CONTEXT* Context, IN PVOID StartVa, IN PFN_NUMBER Pages, IN ULONG AddressFlags);
typedef NTSTATUS (*PMM_FREE_DUMP_RANGE) (IN struct _MM_KERNEL_DUMP_CONTEXT* Context, IN PVOID StartVa, IN PFN_NUMBER Pages, IN ULONG AddressFlags);
...
NTKERNELAPI NTSTATUS MmPrefetchPages (__in ULONG NumberOfLists, __in_ecount(NumberOfLists) PREAD_LIST *ReadLists);

// end_ntifs

NTSTATUS MmPrefetchPagesIntoLockedMdl (IN PFILE_OBJECT FileObject, IN PLARGE_INTEGER FileOffset, IN SIZE_T Length, OUT PMDL *MdlOut);
LOGICAL MmPrefetchForCacheManager (IN PFILE_OBJECT FileObject, IN LARGE_INTEGER SectionOffset, IN SIZE_T NumberOfBytes);
