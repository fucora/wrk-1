...
Abstract:
    This header file describes public data structures and functions that make up the kernel mode portion of the Dbg subsystem.
*/
...
typedef struct _DEBUG_OBJECT {
    KEVENT EventsPresent;// Event thats set when the EventList is populated.
    FAST_MUTEX Mutex;// Mutex to protect the structure
    LIST_ENTRY EventList;// Queue of events waiting for debugger intervention
    ULONG Flags;// Flags for the object
...
NTSTATUS DbgkOpenProcessDebugPort (IN PEPROCESS TargetProcess, IN KPROCESSOR_MODE PreviousMode, OUT HANDLE *pHandle);
