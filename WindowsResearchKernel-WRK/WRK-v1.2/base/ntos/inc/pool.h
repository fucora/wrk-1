...
// WARNING: POOL_QUOTA_MASK is overloaded by POOL_QUOTA_FAIL_INSTEAD_OF_RAISE which is exported from ex.h.

// WARNING: POOL_RAISE_IF_ALLOCATION_FAILURE is exported from ex.h with a value of 16.

// These definitions are used to control the raising of an exception as the result of quota and allocation failures.

#define POOL_QUOTA_MASK 8
#define POOL_TYPE_MASK (3)

// Size of a pool page.
#define POOL_PAGE_SIZE  PAGE_SIZE// This must be greater than or equal to the page size.
...
#define PAGE_END(Address) (((ULONG_PTR)(Address) & (PAGE_SIZE - 1)) == 0)// Define page end macro.

typedef struct _POOL_DESCRIPTOR {// Define pool descriptor structure.
    POOL_TYPE PoolType;
    ULONG PoolIndex;
    ULONG RunningAllocs;
...
//              The pool header must be QWORD (8 byte) aligned in size.
//              If it is not, the pool allocation code will trash the allocated buffer.
...
// N.B. The size fields of the pool header are expressed in units of the smallest pool block size.
// N.B. AMD64 has a 16 byte allocation size and 4096 byte page size. Therefore, byte fields in the pool header are adequate.
...
#define POOL_OVERHEAD               ((LONG)sizeof(POOL_HEADER))// Define size of pool block overhead.
#define POOL_FREE_BLOCK_OVERHEAD    (POOL_OVERHEAD + sizeof (LIST_ENTRY))// Define size of pool block overhead when the block is on a freelist.

typedef struct _POOL_BLOCK {// Define dummy type so computation of pointers is simplified.
    UCHAR Fill[1 << POOL_BLOCK_SHIFT];
} POOL_BLOCK, *PPOOL_BLOCK;

#define POOL_SMALLEST_BLOCK (sizeof(POOL_BLOCK))// Define size of smallest pool block.
#define POOL_BACKTRACEINDEX_PRESENT 0x8000// Define pool tracking information.
...
//SIZE_T EX_REAL_POOL_USAGE (IN SIZE_T SizeInBytes);
// Routine Description:
...
// N.B. The last entry of the pool tracker table is used for all overflow table entries.
