...
#define ROUND_TRIP_DECREMENT_COUNT 16// Define the round trip decrement count.
...
// KiMask32Array - This is an array of 32-bit masks that have one bit set in each mask.
...
// The following AMD64 code reads an unaligned quadword value.
// The quadword value, however, is guaranteed to be within a cache line, and therefore, the value will be read atomically.
#if defined(_AMD64_)
#define KiQuerySystemTime(CurrentTime)     (CurrentTime)->QuadPart = *((LONG64 volatile *)(&SharedUserData->SystemTime))
#else
...
//  N.B. There are really two types of event objects; NotificationEvent and SynchronizationEvent.
//       The type value for a notification event is 0, and that for a synchronization event 1.
//  N.B. There are two types of new timer objects; NotificationTimer and SynchronizationTimer.
//       The type value for a notification timer is 8, and that for a synchronization timer is 9.
//       These values are very carefully chosen so that the dispatcher object type AND'ed with 0x7 yields 0 or 1 for event objects and the timer objects.
...
NTKERNELAPI BOOLEAN KeIsWaitListEmpty (__in PVOID Object);

// end_ntosp

typedef ULONG KPAGE_FRAME;// Page frame
...
#define KWAIT_BLOCK_OFFSET_TO_BYTE0                                              (FIELD_OFFSET(KWAIT_BLOCK, SpareByte) + sizeof(KWAIT_BLOCK) * 0)
#define KWAIT_BLOCK_OFFSET_TO_BYTE1                                              (FIELD_OFFSET(KWAIT_BLOCK, SpareByte) + sizeof(KWAIT_BLOCK) * 1)
#define KWAIT_BLOCK_OFFSET_TO_BYTE2                                              (FIELD_OFFSET(KWAIT_BLOCK, SpareByte) + sizeof(KWAIT_BLOCK) * 2)
#define KWAIT_BLOCK_OFFSET_TO_BYTE3                                              (FIELD_OFFSET(KWAIT_BLOCK, SpareByte) + sizeof(KWAIT_BLOCK) * 3)

#if defined(_AMD64_)
#define KWAIT_BLOCK_OFFSET_TO_LONG0                                              (FIELD_OFFSET(KWAIT_BLOCK, SpareLong) + sizeof(KWAIT_BLOCK) * 0)
#define KWAIT_BLOCK_OFFSET_TO_LONG1                                              (FIELD_OFFSET(KWAIT_BLOCK, SpareLong) + sizeof(KWAIT_BLOCK) * 1)
#define KWAIT_BLOCK_OFFSET_TO_LONG2                                              (FIELD_OFFSET(KWAIT_BLOCK, SpareLong) + sizeof(KWAIT_BLOCK) * 2)
#define KWAIT_BLOCK_OFFSET_TO_LONG3                                              (FIELD_OFFSET(KWAIT_BLOCK, SpareLong) + sizeof(KWAIT_BLOCK) * 3)
#endif
...
// N.B. Descriptor table entries must be a power of 2 in size. Currently this is 16 bytes on a 32-bit system and 32 bytes on a 64-bit system.
...
#if defined(_AMD64_)
LONG KiKernelDpcFilter (IN PKDPC Dpc, IN PEXCEPTION_POINTERS Information);
#endif
...
typedef struct _KINTERRUPT {// Interrupt object
    CSHORT Type;
    CSHORT Size;
...
//      The AMD64 SLIST_HEADER is 16-byte aligned and contains quadword header - the region field is not used.
//      The below packing for AMD64 allows the NUMA node structure to fit in a single cache line.
...
typedef struct _KPROCESS {
    // The dispatch header and profile listhead are fairly infrequently referenced.
    DISPATCHER_HEADER Header;
    ...
    // N.B. The following bit number definitions must match the following bit field.
...
// Define the number of times a user mode SLIST pop fault is permitted to be retried before raising an exception.
#define KI_SLIST_FAULT_COUNT_MAXIMUM 1024

typedef struct _KTHREAD {// Thread object
    // The dispatcher header and mutant listhead are fairly infrequently referenced.
    DISPATCHER_HEADER Header;
    LIST_ENTRY MutantListHead;

    // The following fields are referenced during context switches and wait operatings. They have been carefully laid out to get the best cache hit ratios.
    PVOID InitialStack;
    PVOID StackLimit;
    PVOID KernelStack;
...
#define KTHREAD_DISABLE_BOOST_BIT 1
    ...
    // The following fields are referenced during ready thread and wait completion.
    UCHAR ApcStateIndex;
...
// DPC object
NTKERNELAPI VOID KeInitializeDpc (__out PRKDPC Dpc, __in PKDEFERRED_ROUTINE DeferredRoutine, __in_opt PVOID DeferredContext);

// end_ntddk end_wdm end_nthal end_ntifs

NTKERNELAPI VOID KeInitializeThreadedDpc (__out PRKDPC Dpc, __in PKDEFERRED_ROUTINE DeferredRoutine, __in_opt PVOID DeferredContext);

// begin_ntddk begin_wdm begin_nthal begin_ntifs

NTKERNELAPI BOOLEAN KeInsertQueueDpc (__inout PRKDPC Dpc, __in_opt PVOID SystemArgument1, __in_opt PVOID SystemArgument2);
NTKERNELAPI BOOLEAN KeRemoveQueueDpc (__inout PRKDPC Dpc);
...
NTKERNELAPI VOID KeSetImportanceDpc (__inout PRKDPC Dpc, __in KDPC_IMPORTANCE Importance);
NTKERNELAPI VOID KeSetTargetProcessorDpc (__inout PRKDPC Dpc, __in CCHAR Number);

// end_ntddk end_ntifs end_nthal

#else
FORCEINLINE VOID KeSetImportanceDpc (__inout PRKDPC Dpc, __in KDPC_IMPORTANCE Importance)
/*
Routine Description:
...
*/
{
    ...
    Dpc->Importance = (UCHAR)Importance;// Set the importance of the DPC.
}


FORCEINLINE VOID KeSetTargetProcessorDpc (__inout PRKDPC Dpc, __in CCHAR Number)
/*
Routine Description:
...
*/
{
    ...
    // The target processor number is biased by the maximum number of processors that are supported.
...
// Device queue object
NTKERNELAPI VOID KeInitializeDeviceQueue (__out PKDEVICE_QUEUE DeviceQueue);
NTKERNELAPI BOOLEAN KeInsertDeviceQueue (__inout PKDEVICE_QUEUE DeviceQueue, __inout PKDEVICE_QUEUE_ENTRY DeviceQueueEntry);
NTKERNELAPI BOOLEAN KeInsertByKeyDeviceQueue (__inout PKDEVICE_QUEUE DeviceQueue, __inout PKDEVICE_QUEUE_ENTRY DeviceQueueEntry, __in ULONG SortKey);
NTKERNELAPI PKDEVICE_QUEUE_ENTRY KeRemoveDeviceQueue (__inout PKDEVICE_QUEUE DeviceQueue);
NTKERNELAPI PKDEVICE_QUEUE_ENTRY KeRemoveByKeyDeviceQueue (__inout PKDEVICE_QUEUE DeviceQueue, __in ULONG SortKey);
NTKERNELAPI PKDEVICE_QUEUE_ENTRY KeRemoveByKeyDeviceQueueIfBusy (__inout PKDEVICE_QUEUE DeviceQueue, __in ULONG SortKey);
NTKERNELAPI BOOLEAN KeRemoveEntryDeviceQueue (__inout PKDEVICE_QUEUE DeviceQueue, __inout PKDEVICE_QUEUE_ENTRY DeviceQueueEntry);

// end_ntddk end_wdm end_ntifs end_ntosp

// Interrupt object
NTKERNELAPI VOID KeInitializeInterrupt (
...
NTKERNELAPI BOOLEAN KeConnectInterrupt (__inout PKINTERRUPT Interrupt);

// end_nthal

NTKERNELAPI BOOLEAN KeDisconnectInterrupt (__inout PKINTERRUPT Interrupt);

// begin_ntddk begin_wdm begin_nthal begin_ntosp

NTKERNELAPI BOOLEAN KeSynchronizeExecution (__inout PKINTERRUPT Interrupt, __in PKSYNCHRONIZE_ROUTINE SynchronizeRoutine, __in_opt PVOID SynchronizeContext);
NTKERNELAPI KIRQL KeAcquireInterruptSpinLock (__inout PKINTERRUPT Interrupt);
NTKERNELAPI VOID KeReleaseInterruptSpinLock (__inout PKINTERRUPT Interrupt, __in KIRQL OldIrql);
...
BOOLEAN KeStartProfile (__inout PKPROFILE Profile, __out_opt PULONG Buffer);
BOOLEAN KeStopProfile (__inout PKPROFILE Profile);
VOID KeSetIntervalProfile (__in ULONG Interval, __in KPROFILE_SOURCE Source);
ULONG KeQueryIntervalProfile (__in KPROFILE_SOURCE Source);
...
NTKERNELAPI VOID KeInitializeEvent (__out PRKEVENT Event, __in EVENT_TYPE Type, __in BOOLEAN State);
NTKERNELAPI VOID KeClearEvent (__inout PRKEVENT Event);
...
NTKERNELAPI LONG KePulseEvent (__inout PRKEVENT Event, __in KPRIORITY Increment, __in BOOLEAN Wait);

// end_ntddk end_ntifs end_ntosp

// begin_ntddk begin_wdm begin_nthal begin_ntifs begin_ntosp

NTKERNELAPI LONG KeReadStateEvent (__in PRKEVENT Event);
NTKERNELAPI LONG KeResetEvent (__inout PRKEVENT Event);
NTKERNELAPI LONG KeSetEvent (__inout PRKEVENT Event, __in KPRIORITY Increment, __in BOOLEAN Wait);

// end_ntddk end_wdm end_nthal end_ntifs end_ntosp

VOID KeSetEventBoostPriority (__inout PRKEVENT Event, __in_opt PRKTHREAD *Thread);
VOID KeInitializeEventPair (__inout PKEVENT_PAIR EventPair);
...
NTKERNELAPI VOID KeInitializeMutant (__out PRKMUTANT Mutant, __in BOOLEAN InitialOwner);
LONG KeReadStateMutant (__in PRKMUTANT Mutant);
NTKERNELAPI LONG KeReleaseMutant (__inout PRKMUTANT Mutant, __in KPRIORITY Increment, __in BOOLEAN Abandoned, __in BOOLEAN Wait);

// begin_ntddk begin_wdm begin_nthal begin_ntosp

// Mutex object
NTKERNELAPI VOID KeInitializeMutex (__out PRKMUTEX Mutex, __in ULONG Level);
NTKERNELAPI LONG KeReadStateMutex (__in PRKMUTEX Mutex);
NTKERNELAPI LONG KeReleaseMutex (__inout PRKMUTEX Mutex, __in BOOLEAN Wait);

// end_ntddk end_wdm

// Queue Object.
NTKERNELAPI VOID KeInitializeQueue (__out PRKQUEUE Queue, __in ULONG Count);
NTKERNELAPI LONG KeReadStateQueue (__in PRKQUEUE Queue);
NTKERNELAPI LONG KeInsertQueue (__inout PRKQUEUE Queue, __inout PLIST_ENTRY Entry);
NTKERNELAPI LONG KeInsertHeadQueue (__inout PRKQUEUE Queue, __inout PLIST_ENTRY Entry);
NTKERNELAPI PLIST_ENTRY KeRemoveQueue (__inout PRKQUEUE Queue, __in KPROCESSOR_MODE WaitMode, __in_opt PLARGE_INTEGER Timeout);
NTKERNELAPI PLIST_ENTRY KeRundownQueue (__inout PRKQUEUE Queue);

// begin_ntddk begin_wdm

// Semaphore object
NTKERNELAPI VOID KeInitializeSemaphore (__out PRKSEMAPHORE Semaphore, __in LONG Count, __in LONG Limit);
NTKERNELAPI LONG KeReadStateSemaphore (__in PRKSEMAPHORE Semaphore);
NTKERNELAPI LONG KeReleaseSemaphore (__inout PRKSEMAPHORE Semaphore, __in KPRIORITY Increment, __in LONG Adjustment, __in BOOLEAN Wait);

// end_ntddk end_wdm end_nthal end_ntifs end_ntosp

// Process object
VOID KeInitializeProcess (__out PRKPROCESS Process, __in KPRIORITY Priority, __in KAFFINITY Affinity, __in ULONG_PTR DirectoryTableBase[2], __in BOOLEAN Enable);
LOGICAL KeForceAttachProcess (__inout PKPROCESS Process);

// begin_ntifs begin_ntosp
NTKERNELAPI VOID KeAttachProcess (__inout PRKPROCESS Process);
NTKERNELAPI VOID KeDetachProcess (VOID);
NTKERNELAPI VOID KeStackAttachProcess (__inout PRKPROCESS PROCESS, __out PRKAPC_STATE ApcState);
...
ULONG KeQueryRuntimeProcess (__in PKPROCESS Process, __out PULONG UserTime);
...
VOID KeQueryValuesProcess (__in PKPROCESS Process, __out PKPROCESS_VALUES Values);
LONG KeReadStateProcess (__in PKPROCESS Process);
LOGICAL KeSetAutoAlignmentProcess (__inout PKPROCESS Process, __in LOGICAL Enable);
LONG KeSetProcess (__inout PKPROCESS Process, __in KPRIORITY Increment, __in BOOLEAN Wait);
KAFFINITY KeSetAffinityProcess (__inout PKPROCESS Process, __in KAFFINITY Affinity);
KPRIORITY KeSetPriorityAndQuantumProcess (__inout PKPROCESS Process, __in KPRIORITY BasePriority, __in SCHAR QuantumReset);
VOID KeSetQuantumProcess (__inout PKPROCESS Process, __in SCHAR QuantumReset);
LOGICAL KeSetDisableBoostProcess (__inout PKPROCESS Process, __in LOGICAL Disable);
LOGICAL KeSetDisableQuantumProcess (__inout PKPROCESS Process, __in LOGICAL Disable);
...
BOOLEAN KeAlertThread (__inout PKTHREAD Thread, __in KPROCESSOR_MODE ProcessorMode);
...
ULONG_PTR KeGetCurrentStackPointer (VOID);
VOID KeCheckIfStackExpandCalloutActive (VOID);

FORCEINLINE VOID KeGetActualStackLimits (__out PULONG64 LowLimit, __out PULONG64 HighLimit)
/*
Routine Description:
...
Arguments:
    LowLimit - Supplies a pointer to a variable that receives the low stack limit.
    HighLimit - Supplies a pointer to a variable that receives the high stack limit.
*/
{
...
FORCEINLINE PKERNEL_STACK_SEGMENT KeGetFirstKernelStackSegment (__in PKTHREAD Thread)
/*
Routine Description:
    This function returns a pointer to the first kernel stack segment for an in-memory thread.
Arguments:
    Thread - Supplies a pointer to a kernel thread object.
Return Value:
    A pointer to a kernel stack segment structure is returned as the function value.
*/
{
    PKERNEL_STACK_CONTROL StackControl;
...
FORCEINLINE PKERNEL_STACK_SEGMENT KeGetNextKernelStackSegment (__in PKERNEL_STACK_SEGMENT StackSegment)
/*
Routine Description:
    This function returns a pointer to the next kernel stack segment for an in-memory thread.
Arguments:
    StackStack - Supplies a pointer to a kernel stack segment structure that was either returned as the first or subsequent kernel stack segment structure address.
Return Value:
    If another kernel stack segment is required to describe the kernel stack of the specified thread,
    then a pointer to the next kernel stack segment structure is returned as the function value.
...
FORCEINLINE BOOLEAN KeIsKernelStackTrimable (__in PKTHREAD Thread)
/*
Routine Description:
    This function determines whether the kernel stack for the specified thread is trimable.
Arguments:
    Thread - Supplies a pointer to a kernel thread object.
Return Value:
    A value of TRUE is returned if the kernel stack of the specified thread is trimable.
...
BOOLEAN KeQueryCurrentStackInformation (__out PKERNEL_STACK_LIMITS Type, __out PULONG64 LowLimit, __out PULONG64 HighLimit);
...
typedef VOID (*PEXPAND_STACK_CALLOUT) (__in_opt PVOID Parameter);
NTKERNELAPI NTSTATUS KeExpandKernelStackAndCallout (__in PEXPAND_STACK_CALLOUT Callout, __in_opt PVOID Parameter, __in SIZE_T Size);
#endif
...
// begin_nthal begin_ntosp
NTKERNELAPI VOID KeSetSystemAffinityThread (__in KAFFINITY Affinity);
// end_nthal end_ntosp
...
// begin_ntifs
NTKERNELAPI UCHAR KeSetIdealProcessorThread (__inout PKTHREAD Thread, __in UCHAR Processor);
// begin_ntosp

NTKERNELAPI BOOLEAN KeSetKernelStackSwapEnable (__in BOOLEAN Enable);
...
NTKERNELAPI VOID KeTerminateThread (__in KPRIORITY Increment);
BOOLEAN KeTestAlertThread (__in KPROCESSOR_MODE);
...
NTKERNELAPI VOID KeEnterGuardedRegion (VOID);
NTKERNELAPI VOID KeLeaveGuardedRegion (VOID);
...
// Timer object
NTKERNELAPI VOID KeInitializeTimer (__out PKTIMER Timer);
NTKERNELAPI VOID KeInitializeTimerEx (__out PKTIMER Timer, __in TIMER_TYPE Type);
NTKERNELAPI BOOLEAN KeCancelTimer (__inout PKTIMER);
NTKERNELAPI BOOLEAN KeReadStateTimer (__in PKTIMER Timer);
NTKERNELAPI BOOLEAN KeSetTimer (__inout PKTIMER Timer, __in LARGE_INTEGER DueTime, __in_opt PKDPC Dpc);
NTKERNELAPI BOOLEAN KeSetTimerEx (__inout PKTIMER Timer, __in LARGE_INTEGER DueTime, __in LONG Period, __in_opt PKDPC Dpc);
...
FORCEINLINE BOOLEAN KeIsSMTSetIdle (__in PKPRCB Prcb)
/*
Routine Description:
...
/*
KPROCESSOR_MODE KeGetPReviousModeByThread (__in PKTHREAD xxCurrentThread)
Routine Description:
    This function gets the threads previous mode from the trap frame.
Arguments:
...
VOID KeCheckForTimer(__in_bcount(BlockSize) PVOID BlockStart, __in SIZE_T BlockSize);
VOID KeClearTimer (__inout PKTIMER Timer);
ULONGLONG KeQueryTimerDueTime (__in PKTIMER Timer);


// Wait functions
NTSTATUS KiSetServerWaitClientEvent (__inout PKEVENT SeverEvent, __inout PKEVENT ClientEvent, __in ULONG WaitMode);
...
FORCEINLINE VOID KeWaitForContextSwap (__in PKTHREAD Thread)
/*
Routine Description:
...
FORCEINLINE LOGICAL KeTestForWaitersQueuedSpinLock (__in KSPIN_LOCK_QUEUE_NUMBER Number)
{
...
VOID FASTCALL KeAcquireQueuedSpinLockAtDpcLevel (__inout PKSPIN_LOCK_QUEUE LockQueue);
VOID FASTCALL KeReleaseQueuedSpinLockFromDpcLevel (__inout PKSPIN_LOCK_QUEUE LockQueue);
LOGICAL FASTCALL KeTryToAcquireQueuedSpinLockAtRaisedIrql (__inout PKSPIN_LOCK_QUEUE QueuedLock);

// begin_ntifs begin_ntosp

_DECL_HAL_KE_IMPORT KIRQL FASTCALL KeAcquireQueuedSpinLock (__in KSPIN_LOCK_QUEUE_NUMBER Number);
_DECL_HAL_KE_IMPORT VOID FASTCALL KeReleaseQueuedSpinLock (__in KSPIN_LOCK_QUEUE_NUMBER Number, __in KIRQL OldIrql);
_DECL_HAL_KE_IMPORT LOGICAL FASTCALL KeTryToAcquireQueuedSpinLock (__in KSPIN_LOCK_QUEUE_NUMBER Number, __out PKIRQL OldIrql);

// end_ntifs end_ntosp

_DECL_HAL_KE_IMPORT KIRQL FASTCALL KeAcquireQueuedSpinLockRaiseToSynch (__in KSPIN_LOCK_QUEUE_NUMBER Number);
_DECL_HAL_KE_IMPORT LOGICAL FASTCALL KeTryToAcquireQueuedSpinLockRaiseToSynch (__in KSPIN_LOCK_QUEUE_NUMBER Number, __out PKIRQL OldIrql);
...
// On Uni-processor systems there is no real Dispatcher Database Lock so raising to SYNCH won't help get the lock released any sooner.

#if defined(NT_UP)

#if defined(_X86_)
#define KiLockDispatcherDatabase(OldIrql)         *(OldIrql) = KeRaiseIrqlToDpcLevel()
#define KiTryToLockDispatcherDatabase(OldIrql)    *(OldIrql) = KeRaiseIrqlToDpcLevel(), TRUE
#else
#define KiLockDispatcherDatabase(OldIrql)         *(OldIrql) = KeRaiseIrqlToSynchLevel()
#define KiTryToLockDispatcherDatabase(OldIrql)    *(OldIrql) = KeRaiseIrqlToSynchLevel(), TRUE
#endif

#else   // NT_UP

#if defined(_AMD64_)
KIRQL KiAcquireDispatcherLockRaiseToSynch (VOID);
LOGICAL KiTryToAcquireDispatcherLockRaiseToSynch (__out PKIRQL OldIrql);
#define KiLockDispatcherDatabase(OldIrql)       *(OldIrql) = KiAcquireDispatcherLockRaiseToSynch()
#define KiTryToLockDispatcherDatabase(OldIrql)  KiTryToAcquireDispatcherLockRaiseToSynch(OldIrql)
#else
#define KiLockDispatcherDatabase(OldIrql)       *(OldIrql) = KeAcquireQueuedSpinLockRaiseToSynch(LockQueueDispatcherLock)
#define KiTryToLockDispatcherDatabase(OldIrql)  KeTryToAcquireQueuedSpinLockRaiseToSynch(LockQueueDispatcherLock,OldIrql)
#endif
...
#if defined(_AMD64_)
VOID KiAcquireDispatcherLockAtSynchLevel (VOID);
VOID KiReleaseDispatcherLockFromSynchLevel (VOID);
#define KiLockDispatcherDatabaseAtSynchLevel()       KiAcquireDispatcherLockAtSynchLevel()
#define KiUnlockDispatcherDatabaseFromSynchLevel()   KiReleaseDispatcherLockFromSynchLevel()
#else
#define KiLockDispatcherDatabaseAtSynchLevel()       KeAcquireQueuedSpinLockAtDpcLevel(&KeGetCurrentPrcb()->LockQueue[LockQueueDispatcherLock])
#define KiUnlockDispatcherDatabaseFromSynchLevel()   KeReleaseQueuedSpinLockFromDpcLevel(&KeGetCurrentPrcb()->LockQueue[LockQueueDispatcherLock])
#endif

#endif

VOID FASTCALL KiSetPriorityThread (__inout PRKTHREAD Thread, __in KPRIORITY Priority);

// begin_ntddk begin_wdm begin_nthal begin_ntifs begin_ntndis begin_ntosp

// spin lock functions
#if defined(_X86_) && (defined(_WDMDDK_) || defined(_NTDDK_) || defined _NTIFS_ || defined(WIN9X_COMPAT_SPINLOCK))
NTKERNELAPI VOID NTAPI KeInitializeSpinLock (__out PKSPIN_LOCK SpinLock);
#else
FORCEINLINE VOID NTAPI KeInitializeSpinLock (__out PKSPIN_LOCK SpinLock)
{
    *SpinLock = 0;
}
#endif
...
NTKERNELAPI VOID FASTCALL KefAcquireSpinLockAtDpcLevel (__inout PKSPIN_LOCK SpinLock);
NTKERNELAPI VOID FASTCALL KefReleaseSpinLockFromDpcLevel (__inout PKSPIN_LOCK SpinLock);

#define KeAcquireSpinLockAtDpcLevel(a) KefAcquireSpinLockAtDpcLevel(a)
#define KeReleaseSpinLockFromDpcLevel(a) KefReleaseSpinLockFromDpcLevel(a)

_DECL_HAL_KE_IMPORT KIRQL FASTCALL KfAcquireSpinLock (__inout PKSPIN_LOCK SpinLock);
_DECL_HAL_KE_IMPORT VOID FASTCALL KfReleaseSpinLock (__inout PKSPIN_LOCK SpinLock, __in KIRQL NewIrql);

// end_wdm end_ntddk

_DECL_HAL_KE_IMPORT KIRQL FASTCALL KeAcquireSpinLockRaiseToSynch (__inout PKSPIN_LOCK SpinLock);
...
NTKERNELAPI BOOLEAN FASTCALL KeTestSpinLock (__in PKSPIN_LOCK SpinLock);
NTKERNELAPI BOOLEAN FASTCALL KeTryToAcquireSpinLockAtDpcLevel (__inout PKSPIN_LOCK SpinLock);
...
NTKERNELAPI KIRQL FASTCALL KeAcquireSpinLockRaiseToSynch (__inout PKSPIN_LOCK SpinLock);

// begin_wdm begin_ntddk

NTKERNELAPI VOID KeAcquireSpinLockAtDpcLevel (__inout PKSPIN_LOCK SpinLock);
NTKERNELAPI KIRQL KeAcquireSpinLockRaiseToDpc (__inout PKSPIN_LOCK SpinLock);
NTKERNELAPI VOID KeReleaseSpinLock (__inout PKSPIN_LOCK SpinLock, __in KIRQL NewIrql);
NTKERNELAPI VOID KeReleaseSpinLockFromDpcLevel (__inout PKSPIN_LOCK SpinLock);
NTKERNELAPI BOOLEAN FASTCALL KeTestSpinLock (__in PKSPIN_LOCK SpinLock);
NTKERNELAPI BOOLEAN FASTCALL KeTryToAcquireSpinLockAtDpcLevel (__inout PKSPIN_LOCK SpinLock);
...
NTKERNELAPI KIRQL FASTCALL KeAcquireSpinLockForDpc (__inout PKSPIN_LOCK SpinLock);
NTKERNELAPI VOID FASTCALL KeReleaseSpinLockForDpc (__inout PKSPIN_LOCK SpinLock, __in KIRQL OldIrql);

// end_ntndis end_ntosp

#if !defined(_AMD64_)
BOOLEAN KeTryToAcquireSpinLock (__inout PKSPIN_LOCK SpinLock, __out PKIRQL OldIrql);
#endif

// Enable interrupts.

#if !defined(USER_MODE_CODE)
FORCEINLINE VOID KeEnableInterrupts (__in BOOLEAN Enable)
/*
Routine Description:
    This function enables interrupts based on the specified enable state.
Arguments:
    Enable - Supplies a boolean value that determines whether interrupts are to be enabled.
*/
{
    if (Enable != FALSE) {
        _enable();
...
_DECL_HAL_KE_IMPORT VOID FASTCALL KfLowerIrql (__in KIRQL NewIrql);
_DECL_HAL_KE_IMPORT KIRQL FASTCALL KfRaiseIrql (__in KIRQL NewIrql);
...
KIRQL FORCEINLINE KeGetCurrentIrql (VOID)
{
    ULONG tprValue;
    KIRQL currentIrql;
...
VOID FORCEINLINE KfLowerIrql (__in KIRQL NewIrql)
{
    ULONG tprValue;
...
KIRQL FORCEINLINE KfRaiseIrql (__in KIRQL NewIrql)
{
    KIRQL oldIrql;
    ULONG tprValue;
...
KIRQL FORCEINLINE KeRaiseIrqlToDpcLevel (VOID)
{
    return KfRaiseIrql(DISPATCH_LEVEL);
}

KIRQL FORCEINLINE KeRaiseIrqlToSynchLevel (VOID)
{
...
// The following three functions RAISE and LOWER IRQL when a queued in stack spin lock is acquired or released using these routines.

_DECL_HAL_KE_IMPORT VOID FASTCALL KeAcquireInStackQueuedSpinLock (__inout PKSPIN_LOCK SpinLock, __out PKLOCK_QUEUE_HANDLE LockHandle);

// end_ntddk end_nthal end_ntifs end_ntosp

_DECL_HAL_KE_IMPORT VOID FASTCALL KeAcquireInStackQueuedSpinLockRaiseToSynch (__inout PKSPIN_LOCK SpinLock, __out PKLOCK_QUEUE_HANDLE LockHandle);

// begin_ntddk begin_nthal begin_ntifs begin_ntosp

_DECL_HAL_KE_IMPORT VOID FASTCALL KeReleaseInStackQueuedSpinLock (__in PKLOCK_QUEUE_HANDLE LockHandle);


// The following two functions do NOT raise or lower IRQL when a queued in stack spin lock is acquired or released using these functions.
NTKERNELAPI VOID FASTCALL KeAcquireInStackQueuedSpinLockAtDpcLevel (__inout PKSPIN_LOCK SpinLock, __out PKLOCK_QUEUE_HANDLE LockHandle);
NTKERNELAPI VOID FASTCALL KeReleaseInStackQueuedSpinLockFromDpcLevel (__in PKLOCK_QUEUE_HANDLE LockHandle);

// end_ntddk end_nthal end_ntifs

// The following two functions conditionally raise or lower IRQL when a queued in-stack spin lock is acquired or released using these functions.
NTKERNELAPI VOID FASTCALL KeAcquireInStackQueuedSpinLockForDpc (__inout PKSPIN_LOCK SpinLock, __out PKLOCK_QUEUE_HANDLE LockHandle);
NTKERNELAPI VOID FASTCALL KeReleaseInStackQueuedSpinLockForDpc (__in PKLOCK_QUEUE_HANDLE LockHandle);
...
NTKERNELAPI BOOLEAN KeDeregisterBugCheckCallback (__inout PKBUGCHECK_CALLBACK_RECORD CallbackRecord);
...
NTKERNELAPI BOOLEAN KeDeregisterBugCheckReasonCallback (__inout PKBUGCHECK_REASON_CALLBACK_RECORD CallbackRecord);
...
NTKERNELAPI PVOID KeRegisterNmiCallback (__in PNMI_CALLBACK CallbackRoutine, __in_opt PVOID Context);
NTSTATUS KeDeregisterNmiCallback (__in PVOID Handle);

// end_wdm

NTKERNELAPI DECLSPEC_NORETURN VOID NTAPI KeBugCheck (__in ULONG BugCheckCode);

// end_ntddk end_nthal end_ntifs end_ntosp

#if defined(_AMD64_)
NTKERNELAPI DECLSPEC_NORETURN VOID NTAPI KiBugCheck3 (
...
BOOLEAN KeGetBugMessageText (__in ULONG MessageId, __out_opt PANSI_STRING ReturnedString OPTIONAL);
...
NTKERNELAPI VOID __cdecl KeSaveStateForHibernate (__out PKPROCESSOR_STATE ProcessorState);

// end_nthal

BOOLEAN FASTCALL KeInvalidAccessAllowed (__in_opt PVOID TrapInformation);

typedef VOID (*PGDI_BATCHFLUSH_ROUTINE) (VOID);//  GDI TEB Batch Flush routine
...
// KiFindFirstSetLeft - This is an array tha this used to lookup the left most bit in a byte.
...
#else
FORCEINLINE ULONG KiFindFirstSetLeftMemberInt (__in ULONG Set)
{
    __asm {
        bsr eax, Set
    }
...
FORCEINLINE void KeFindFirstSetLeftMember (__in ULONG Set, __out PULONG Member)
{
    *Member = KiFindFirstSetLeftMemberInt (Set);
}
#endif

ULONG KeFindNextRightSetAffinity (__in ULONG Number, __in KAFFINITY Set);


// Find first set right in 32-bit set.

// KiFindFirstSetRight - This is an array that this used to lookup the right most bit in a byte.
...
#if defined(_X86_)
FORCEINLINE ULONG KeFindFirstSetRightMember (__in ULONG Set)
{
    __asm {
        bsf eax, Set
    }
...
VOID KxFlushEntireTb (VOID);

FORCEINLINE VOID KeFlushEntireTb (__in BOOLEAN Invalid, __in BOOLEAN AllProcessors)
{
...
FORCEINLINE VOID FASTCALL KeFlushSingleTb (__in PVOID Virtual, __in BOOLEAN AllProcesors)
{
...
{
    // While the TB flush time stamp counter is being updated the low order bit of the time stamp value is set (MP only).
    // Otherwise, the bit is clear.

    // N.B. Memory ordering is required so modifications to page tables are completed before reading the timestamp.
    KeMemoryBarrier();
    return KiTbFlushTimeStamp;
}

FORCEINLINE VOID KeLoopTbFlushTimeStampUnlocked (VOID)
{
...
#if defined(_AMD64_)
NTKERNELAPI KAFFINITY KeQueryMultiThreadProcessorSet (ULONG Number);
#endif
...
NTKERNELAPI PCONFIGURATION_COMPONENT_DATA KeFindConfigurationEntry (
...
NTKERNELAPI PCONFIGURATION_COMPONENT_DATA KeFindConfigurationNextEntry (
    IN PCONFIGURATION_COMPONENT_DATA Child,
    IN CONFIGURATION_CLASS Class,
    IN CONFIGURATION_TYPE Type,
    IN PULONG Key OPTIONAL,
...
// These are defined as calls through a function pointer which is set to point at the optimal routine for this processor implementation.
#if defined(_X86_)
...
VOID KeZeroSinglePage (IN PVOID PageBase);
#endif

// Routine for copying a page.
#if defined(_AMD64_)
VOID KeCopyPage (IN PVOID Destination, IN PVOID Source);
#else
...
NTKERNELAPI BOOLEAN KeAreAllApcsDisabled (VOID);
NTKERNELAPI VOID FASTCALL KeInitializeGuardedMutex (IN PKGUARDED_MUTEX Mutex);
NTKERNELAPI VOID FASTCALL KeAcquireGuardedMutex (IN PKGUARDED_MUTEX Mutex);
NTKERNELAPI VOID FASTCALL KeReleaseGuardedMutex (IN PKGUARDED_MUTEX Mutex);
NTKERNELAPI BOOLEAN FASTCALL KeTryToAcquireGuardedMutex (IN PKGUARDED_MUTEX Mutex);
NTKERNELAPI VOID FASTCALL KeAcquireGuardedMutexUnsafe (IN PKGUARDED_MUTEX FastMutex);
NTKERNELAPI VOID FASTCALL KeReleaseGuardedMutexUnsafe (IN PKGUARDED_MUTEX FastMutex);
...
ULARGE_INTEGER KeComputeReciprocal (IN LONG Divisor, OUT PCCHAR Shift);
ULONG KeComputeReciprocal32 (IN LONG Divisor, OUT PCCHAR Shift);
#endif // _KE_