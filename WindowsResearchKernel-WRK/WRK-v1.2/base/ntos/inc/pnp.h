...
// Define structure returned in response to IRP_MN_QUERY_BUS_INFORMATION by a PDO indicating the type of bus the device exists on.
...
// Define structure returned in response to IRP_MN_QUERY_LEGACY_BUS_INFORMATION by an FDO indicating the type of bus it is.
// This is normally the same bus type as the device's children (i.e., as retrieved from the child PDO's via
// IRP_MN_QUERY_BUS_INFORMATION) except for cases like CardBus, which can support both 16-bit (PCMCIABus) and 32-bit (PCIBus) cards.
...
NTKERNELAPI NTSTATUS NTAPI IoRegisterDeviceInterface(
...
typedef struct _PNP_LOCATION_INTERFACE {
    ...
    PGET_LOCATION_STRING GetLocationString;// interface specific entry
...
typedef struct _ARBITER_CONFLICT_INFO {
    PDEVICE_OBJECT OwningObject;// The device object owning the device that is causing the conflict
    ULONGLONG Start;// The start of the conflicting range
    ULONGLONG End;// The end of the conflicting range
...
typedef struct _ARBITER_PARAMETERS {
    union {
        struct {
            IN OUT PLIST_ENTRY ArbitrationList; // Doubly linked list of ARBITER_LIST_ENTRY's
            IN ULONG AllocateFromCount;// The size of the AllocateFrom array
            IN PCM_PARTIAL_RESOURCE_DESCRIPTOR AllocateFrom;// Array of resource descriptors describing the resources available to the arbiter for it to arbitrate
        } TestAllocation;
        ...
        struct {
            OUT PCM_PARTIAL_RESOURCE_LIST *AllocatedResources;// The resources that are currently allocated
        } QueryAllocatedResources;

        struct {
            IN PDEVICE_OBJECT PhysicalDeviceObject;// This is the device we are trying to find a conflict for
            IN PIO_RESOURCE_DESCRIPTOR ConflictingResource;// This is the resource to find the conflict for
            OUT PULONG ConflictCount;// Number of devices conflicting on the resource
            OUT PARBITER_CONFLICT_INFO *Conflicts;// Pointer to array describing the conflicting device objects and ranges
        } QueryConflict;

        struct {
            IN PLIST_ENTRY ArbitrationList;// Doubly linked list of ARBITER_LIST_ENTRY's - should have only one entry
        } QueryArbitrate;

        struct {
            PDEVICE_OBJECT ReserveDevice;// Indicates the device whose resources are to be marked as reserved
        } AddReserved;
    } Parameters;
...
typedef struct _ARBITER_LIST_ENTRY {
    ...
    LIST_ENTRY ListEntry;// This is a doubly linked list of entries for easy sorting

    ULONG AlternativeCount;// The number of alternative allocation

    PIO_RESOURCE_DESCRIPTOR Alternatives;// Pointer to an array of resource descriptors for the possible allocations

    PDEVICE_OBJECT PhysicalDeviceObject;// The device object of the device requesting these resources.

    ARBITER_REQUEST_SOURCE RequestSource;// Indicates where the request came from

    ULONG Flags;// Flags these indicate a variety of things (use ARBITER_FLAG_*)

    // Space to aid the arbiter in processing the list it is initialized to 0 when the entry is created.
    // The system will not attempt to interpret it.
    LONG_PTR WorkSpace;

    // Interface Type, Slot Number and Bus Number from Resource Requirements list, used only for reverse identification.
    INTERFACE_TYPE InterfaceType;
    ULONG SlotNumber;
    ULONG BusNumber;

    // A pointer to a descriptor to indicate the resource that was allocated.
    // This is allocated by the system and filled in by the arbiter in response to an ArbiterActionTestAllocation.
...
typedef struct _ARBITER_INTERFACE {
    ...
    PARBITER_HANDLER ArbiterHandler;// Entry point to the arbiter
    ULONG Flags;// Other information about the arbiter, use ARBITER_* flags
...
NTSTATUS PpSetHwProfileChangeEvent(
