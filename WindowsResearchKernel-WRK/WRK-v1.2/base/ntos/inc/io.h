...
// Define I/O system data structure type codes.
// Each major data structure in the I/O system has a type code  The type field in each structure is at the same offset.
// The following values can be used to determine which type of data structure a pointer refers to.
...
// Define the minor function codes for IRPs.
// The lower 128 codes, from 0x00 to 0x7f are reserved to Microsoft.
// The upper 128 codes, from 0x80 to 0xff, are reserved to customers of Microsoft.
...
// Note that "user request" is assumed to be zero by both the I/O system and file systems.
...
// Read and Write minor function codes for file systems supporting Lan Manager software.
// All of these subfunction codes are invalid if the file has been opened with FO_NO_INTERMEDIATE_BUFFERING.
// They are also invalid in combi- nation with synchronous calls (Irp Flag or file open option).

// Note that "normal" is assumed to be zero by both the I/O system and file systems.
// Do not change this value.
...
// Device Control Request minor function codes for SCSI support.
// Note that user requests are assumed to be zero.
...
// Define option flags for IoCreateFile.
// Note that these values must be exactly the same as the SL_... flags for a create function.
// Note also that there are flags that may be passed to IoCreateFile that are not placed in the stack location for the create IRP.
// These flags start in the next byte.
...
// Define Information fields for whether or not a REPARSE or a REMOUNT has occurred in the file system.
...
//  FileHandles created using IoCreateFileSpecifyDeviceObjectHint with this flag set will bypass ShareAccess checks on this file.
...
typedef NTSTATUS(*PDUMP_DRIVER_WRITE_PENDING) (IN LONG Action, IN PLARGE_INTEGER DiskByteOffset, IN PMDL Mdl, IN PVOID LocalData);
...
NTSTATUS IoConfigureCrashDump(CRASHDUMP_CONFIGURATION Config);
BOOLEAN IoInitializeCrashDump(IN HANDLE Pagefile);
...
BOOLEAN IoIsTriageDumpEnabled(VOID);
BOOLEAN IoAddTriageDumpDataBlock(IN PVOID Address, IN ULONG Length);
...
typedef VOID(*PIO_DPC_ROUTINE) (IN PKDPC Dpc, IN struct _DEVICE_OBJECT *DeviceObject, IN struct _IRP *Irp, IN PVOID Context);// Define the I/O version of a DPC routine.
typedef VOID(*PIO_TIMER_ROUTINE) (IN struct _DEVICE_OBJECT *DeviceObject, IN PVOID Context);// Define driver timer routine type.
typedef NTSTATUS(*PDRIVER_INITIALIZE) (IN struct _DRIVER_OBJECT *DriverObject, IN PUNICODE_STRING RegistryPath);// Define driver initialization routine type.

// end_wdm

typedef VOID(*PDRIVER_REINITIALIZE) (IN struct _DRIVER_OBJECT *DriverObject, IN PVOID Context, IN ULONG Count);// Define driver reinitialization routine type.

// begin_wdm begin_ntndis


typedef VOID(*PDRIVER_CANCEL) (IN struct _DEVICE_OBJECT *DeviceObject, IN struct _IRP *Irp);// Define driver cancel routine type.
typedef NTSTATUS(*PDRIVER_DISPATCH) (IN struct _DEVICE_OBJECT *DeviceObject, IN struct _IRP *Irp);// Define driver dispatch routine type.
typedef VOID(*PDRIVER_STARTIO) (IN struct _DEVICE_OBJECT *DeviceObject, IN struct _IRP *Irp);// Define driver start I/O routine type.
typedef VOID(*PDRIVER_UNLOAD) (IN struct _DRIVER_OBJECT *DriverObject);// Define driver unload routine type.
typedef NTSTATUS(*PDRIVER_ADD_DEVICE) (IN struct _DRIVER_OBJECT *DriverObject, IN struct _DEVICE_OBJECT *PhysicalDeviceObject);// Define driver AddDevice routine type.

// end_ntddk end_wdm end_nthal end_ntndis end_ntosp

typedef VOID(*PDRIVER_FS_NOTIFICATION) (IN struct _DEVICE_OBJECT *DeviceObject, IN BOOLEAN FsActive);// Define driver FS notification change routine type.
...
// Define callbacks for NtCreateSection to synchronize correctly with the file system.
// It pre-acquires the resources that will be needed when calling to query and set file/allocation size in the file system.
...
// Define callback for drivers that have device objects attached to lower- level drivers' device objects.
// This callback is made when the lower-level driver is deleting its device object.
typedef VOID(*PFAST_IO_DETACH_DEVICE) (IN struct _DEVICE_OBJECT *SourceDevice, IN struct _DEVICE_OBJECT *TargetDevice);


// This structure is used by the server to quickly get the information needed to service a server open call.
// It is takes what would be two fast io calls one for basic information and the other for standard information and makes it into one call.
typedef
BOOLEAN
...
//  Parameters union for the operations that are exposed to the filters through the FsFilterCallbacks registration mechanism.
...
//  These are the valid values for the Operation field of the FS_FILTER_CALLBACK_DATA structure.
...
//  Prototype for the callbacks received before an operation is passed to the base file system.

//  A filter can fail this operation, but consistant failure will halt system progress.
typedef NTSTATUS(*PFS_FILTER_CALLBACK) (IN PFS_FILTER_CALLBACK_DATA Data, OUT PVOID *CompletionContext);


//  Prototype for the completion callback received after an operation is completed.
typedef VOID(*PFS_FILTER_COMPLETION_CALLBACK) (IN PFS_FILTER_CALLBACK_DATA Data, IN NTSTATUS OperationStatus, IN PVOID CompletionContext);


//  This is the structure that the file system filter fills in to receive notifications for these locking operations.

//  A filter should set the field to NULL for any notification callback it doesn't wish to receive.
...
// Define the I/O system's structure for a connected interrupt.
// This is useful for connecting an ISR to several different processors.
...
// end_ntddk end_nthal end_ntifs end_wdm end_ntosp
    ...
    ULONG ExtensionFlags;// Device object extension flags.  Protected by the IopDatabaseLock.
    PVOID           DeviceNode;// PnP manager fields

    // AttachedTo is a pointer to the device object that this device object is attached to.
    // The attachment chain is now doubly linked: this pointer and DeviceObject->AttachedDevice provide the linkage.
    PDEVICE_OBJECT  AttachedTo;

    // The next two fields are used to prevent recursion in IoStartNextPacket interfaces.
...
{
    struct _DRIVER_OBJECT *DriverObject;// Back pointer to Driver Object
...
// end_ntddk end_wdm end_nthal end_ntifs end_ntosp
    ...
    // The client driver object extension field is used by class driver to store per driver information.
    PIO_CLIENT_EXTENSION ClientDriverExtension;

    //  The file system filter callback extension field is used to safely notify filters of system operations that were previously not shown to file system filters.
...
typedef struct _DRIVER_OBJECT
{
    ...
    // The following section describes where the driver is loaded.
    ...
    // The following section contains the optional pointer to an array of alternate entry points to a driver for "fast I/O" support.
    // Fast I/O is performed by invoking the driver routine directly with separate parameters, rather than using the standard IRP call mechanism.
    // Note that these functions may only be used for synchronous I/O, and when the file is cached.
    PFAST_IO_DISPATCH FastIoDispatch;

    // The following section describes the entry points to this particular driver.
    // Note that the major function dispatch table must be the last field in the object so that it remains extensible.
...
// There is one of these objects per PnP device.
// This object is given to the device driver as a PVOID and is used by the driver to refer to a particular device.
typedef struct _DEVICE_HANDLER_OBJECT
{
    CSHORT Type;
    USHORT Size;

    struct _BUS_HANDLER *BusHandler;// Identifies which bus extender this device handler object is associated with
    ULONG SlotNumber;// The associated SlotNumber for this device handler
...
typedef struct DECLSPEC_ALIGN(MEMORY_ALLOCATION_ALIGNMENT) _IRP
{
    ...
    // Define a pointer to the Memory Descriptor List (MDL) for this I/O request.
    // This field is only used if the I/O is "direct I/O".
    PMDL MdlAddress;

    ULONG Flags;// Flags word - used to remember various flags.
    ...
    LIST_ENTRY ThreadListEntry;// Thread list entry - allows queueing the IRP to the thread pending I/O request packet list.
    IO_STATUS_BLOCK IoStatus;// I/O status - final status of operation.
    KPROCESSOR_MODE RequestorMode;// Requestor mode - mode of the original requestor of this operation.
    BOOLEAN PendingReturned;// Pending returned - TRUE if pending was initially returned as the status for this packet.

    // Stack state information.
    CHAR StackCount;
    CHAR CurrentLocation;

    BOOLEAN Cancel;// Cancel - packet has been canceled.
    KIRQL CancelIrql;// Cancel Irql - Irql at which the cancel spinlock was acquired.
    CCHAR ApcEnvironment;// ApcEnvironment - Used to save the APC environment at the time that the packet was initialized.
    UCHAR AllocationFlags;// Allocation control flags.
    ...
    // Note that the UserBuffer parameter is outside of the stack so that I/O completion can copy data back into the user's address space without having to know exactly which service was being invoked.
    // The length of the copy is stored in the second half of the I/O status block.
    // If the UserBuffer field is NULL, then no copy is performed.
    PVOID UserBuffer;

    // Kernel structures

    // The following section contains kernel structures which the IRP needs in order to place various work information in kernel controller system queues.
    // Because the size and alignment cannot be controlled, they are placed here at the end so they just hang off and do not affect the alignment of other fields in the IRP.
    union
    {
        struct
        {
            ...
            PETHREAD Thread;// Thread - pointer to caller's Thread Control Block.
            ...
            // The following unnamed structure must be exactly identical to the unnamed structure used in the minipacket header used for completion queue entries.
            struct
            {
            ...
            // Original file object - pointer to the original file object that was used to open the file.
            // This field is owned by the I/O system and should not be used by any other drivers.
            PFILE_OBJECT OriginalFileObject;
        } Overlay;

        // APC - This APC control block is used for the special kernel APC as well as for the caller's APC, if one was specified in the original argument list.
        // If so, then the APC is reused for the normal APC for whatever mode the caller was in and the "special" routine that is invoked before the APC gets control simply deallocates the IRP.
        KAPC Apc;

        PVOID CompletionKey;// CompletionKey - This is the key that is used to distinguish individual I/O operations initiated on a single file handle.
    } Tail;
} IRP, *PIRP;
...
// The following flags must exactly match those in the IoCreateFile call's options.
// The case sensitive flag is added in later, by the parse routine, and is not an actual option to open.
// Rather, it is part of the object manager's attributes structure.
...
// workaround overloaded definition (rpc generated headers all define INTERFACE to match the class name).
#undef INTERFACE
...
typedef struct _IO_STACK_LOCATION
{
    ...
    // The following user parameters are based on the service that is being invoked.
    // Drivers and file systems can determine which set to use based on the above major and minor function codes.
    union
    {
        ...
        // end_ntddk end_wdm end_nthal end_ntifs end_ntosp
                ...
                // Notice that the fields in the following parameter structure must match those for the create structure other than the last longword.
                // This is so that no distinctions need be made by the I/O system's parse routine other than for the last longword.
        struct
        {
            PIO_SECURITY_CONTEXT SecurityContext;
            ULONG Options;
        ...
        // begin_ntosp
                ...
                // Note that the user's output buffer is stored in the UserBuffer field and the user's input buffer is stored in the SystemBuffer field.
        struct
        {
    ...
    PDEVICE_OBJECT DeviceObject;// Save a pointer to this device driver's device object for this request so it can be passed to the completion routine if needed.
    PFILE_OBJECT FileObject;// The following location contains a pointer to the file object for this
    PIO_COMPLETION_ROUTINE CompletionRoutine;// The following routine is invoked depending on the flags in the above flags field.
    PVOID Context;// The following is used to store the address of the context parameter that should be passed to the CompletionRoutine.
...
// The following structure is used by drivers that are initializing to determine the number of devices of a particular type that have already been initialized.
// It is also used to track whether or not the AtDisk address range has already been claimed.
// Finally, it is used by the NtQuerySystemInformation system service to return device type counts.
typedef struct _CONFIGURATION_INFORMATION
{
    // This field indicates the total number of disks in the system.
    // This number should be used by the driver to determine the name of new disks.
    // This field should be updated by the driver as it finds new disks.
    ...
    // These next two fields indicate ownership of one of the two IO address spaces that are used by WD1003-compatible disk controllers.
    ...
    // This field will be updated by the drivers as it determines that new devices have been found and will be supported.
...
// Routine Description:
//     This routine is invoked by drivers of bootable media to create a symbolic link between the ARC name of their device and its NT name.
//     This allows the system to determine which device in the system was actually booted from since the ARC firmware only deals in ARC names, and NT only deals in NT names.
// Arguments:
...
// This value should be returned from completion routines to continue completing the IRP upwards.
// Otherwise, STATUS_MORE_PROCESSING_REQUIRED should be returned.
...
// Routine Description:
//     This routine is invoked by drivers to deassign an ARC name that they created to a device.
//     This is generally only called if the driver is deleting the device object, which means that the driver is probably unloading.
// Arguments:
...
NTSTATUS IoGetDeviceInstanceName(IN PDEVICE_OBJECT PhysicalDeviceObject, OUT PUNICODE_STRING InstanceName);
...
// Routine Description:
//     This routine is invoked to return a pointer to the current stack location in an I/O Request Packet (IRP).
// Arguments:
...
FORCEINLINE ULONG_PTR IoGetRemainingStackSize(VOID)
{
    ULONG_PTR Top;
    ULONG_PTR Bottom;
...
// end_ntddk end_wdm end_nthal
// VOID IoInitializeTheadedDpcRequest(IN PDEVICE_OBJECT DeviceObject, IN PIO_DPC_ROUTINE DpcRoutine)

// begin_ntddk begin_wdm begin_nthal
// Routine Description:
//     This routine is invoked to initialize the DPC in a device object for a device driver during its initialization routine.
//     The DPC is used later when the driver interrupt service routine requests that a DPC routine be queued for later execution.
// Arguments:
...
#define IoInitializeThreadedDpcRequest( DeviceObject, DpcRoutine ) (\
    KeInitializeThreadedDpc( &(DeviceObject)->Dpc, (PKDEFERRED_ROUTINE) (DpcRoutine), (DeviceObject) ) )
// begin_ntddk begin_wdm begin_nthal
...
// Routine Description:
//     This routine is invoked to determine if an error was as a result of user actions.
//     Typically these error are related to removable media and will result in a pop-up.
// Arguments:
//     Status - The status value to check.
// Return Value:
//     The function value is TRUE if the user induced the error, otherwise FALSE is returned.
...
// Routine Description:
//     This routine is invoked to determine whether the file open represented by the specified file object is opened exclusively.
// Arguments:
//     FileObject - Pointer to the file object that represents the open instance of the target file to be tested for exclusive access.
// Return Value:
//     The function value is TRUE if the open instance of the file is exclusive;
//     otherwise FALSE is returned.
#define IoIsFileOpenedExclusively( FileObject ) ((BOOLEAN) !((FileObject)->SharedRead || (FileObject)->SharedWrite || (FileObject)->SharedDelete))
...
// Routine Description:
//     This routine marks the specified I/O Request Packet (IRP) to indicate that an initial status of STATUS_PENDING was returned to the caller.
//     This is used so that I/O completion can determine whether or not to fully complete the I/O operation requested by the packet.
// Arguments:
...
NTKERNELAPI NTSTATUS IoEnumerateRegisteredFiltersList(IN  PDRIVER_OBJECT *DriverObjectList,
...
NTKERNELAPI NTSTATUS IoReportHalResourceUsage(
...
// Routine Description:
//     This routine is invoked by the device driver's interrupt service routine to request that a DPC routine be queued for later execution at a lower IRQL.
// Arguments:
...
// Routine Description:
//     This routine is invoked to set the address of a cancel routine which is to be invoked when an I/O packet has been canceled.
// Arguments:
...
//     If the caller wishes to call the next driver in a stack, and does not wish to change the arguments, nor does he wish to set a completion
...
//    While the lock is held, the caller can assume that no pending pnp REMOVE requests will be completed.

//    The lock should be acquired immediately upon entering a dispatch routine.
//    It should also be acquired before creating any new reference to the device object if there's a chance of releasing the reference before the new one is done,
//    in addition to references to the driver code itself, which is removed from memory when the last device object goes.

//    Arguments:
//    RemoveLock - A pointer to an initialized REMOVE_LOCK structure.
//    Tag - Used for tracking lock allocation and release.
//          The same tag specified when acquiring the lock must be used to release the lock.
...
//    If successful the caller should continue with work calling IoReleaseRemoveLock when finished.

//    If not successful the lock was not obtained.  The caller should abort the work but not call IoReleaseRemoveLock.
#define IoReleaseRemoveLock(RemoveLock, Tag)         IoReleaseRemoveLockEx(RemoveLock, Tag, sizeof (IO_REMOVE_LOCK))

NTKERNELAPI VOID NTAPI IoReleaseRemoveLockEx(
    IN PIO_REMOVE_LOCK RemoveLock,
...
//    It must be called when finished using a previously locked reference to the device object.
//    If an Tag was specified when acquiring the lock then the same Tag must be specified when releasing the lock.

//    When the lock count reduces to zero, this routine will signal the waiting event to release the waiting thread deleting the device object protected by this lock.
// Arguments:
//    DeviceObject - the device object to lock
//    Tag - The TAG (if any) specified when acquiring the lock.  This is used for lock tracking purposes
#define IoReleaseRemoveLockAndWait(RemoveLock, Tag)         IoReleaseRemoveLockAndWaitEx(RemoveLock, Tag, sizeof (IO_REMOVE_LOCK))

NTKERNELAPI VOID NTAPI IoReleaseRemoveLockAndWaitEx(
    IN PIO_REMOVE_LOCK RemoveLock,
    IN PVOID            Tag,
    IN ULONG            RemlockSize // are we checked or free
);
// Routine Description:
//    This routine is called when the client would like to delete the remove-locked resource.
//    This routine will block until all the remove locks have released.
...
// The following is set for a KM provider who is considered private to kernel tracing
...
NTKERNELAPI NTSTATUS IoWMIAllocateInstanceIds(__in GUID *Guid, __in ULONG InstanceCount, __out ULONG *FirstInstanceId);
...
NTKERNELAPI NTSTATUS IoWMIWriteEvent(__in PVOID WnodeEventItem);

#if defined(_WIN64)
NTKERNELAPI ULONG IoWMIDeviceObjectToProviderId(__in PDEVICE_OBJECT DeviceObject);
#else
...
NTKERNELAPI BOOLEAN WMIInitialize(ULONG Phase, PVOID LoaderBlock);

// IoRemoteBootClient indicates whether the system was booted as a remote boot client.
...
NTSTATUS IoShutdownPnpDevices(VOID);
...
NTSTATUS FASTCALL IovCallDriver(IN PDEVICE_OBJECT  DeviceObject, IN OUT  PIRP Irp, IN PVOID ReturnAddress);
VOID FASTCALL IovCompleteRequest(IN PIRP Irp, IN CCHAR PriorityBoost);
PIRP IovBuildAsynchronousFsdRequest(
    IN ULONG MajorFunction,
    IN PDEVICE_OBJECT DeviceObject,
    IN OUT PVOID Buffer OPTIONAL,
    IN ULONG Length OPTIONAL,
...
PIRP IovBuildDeviceIoControlRequest(
    IN ULONG IoControlCode,
    IN PDEVICE_OBJECT DeviceObject,
    IN PVOID InputBuffer OPTIONAL,
    IN ULONG InputBufferLength,
...
#if defined(_WIN64)
NTKERNELAPI BOOLEAN IoIs32bitProcess(IN PIRP Irp);
#endif
...
NTKERNELAPI NTSTATUS IoCreateDisk(IN PDEVICE_OBJECT DeviceObject, IN struct _CREATE_DISK* Disk);
NTKERNELAPI NTSTATUS IoReadPartitionTableEx(IN PDEVICE_OBJECT DeviceObject, IN struct _DRIVE_LAYOUT_INFORMATION_EX** DriveLayout);
NTKERNELAPI NTSTATUS IoWritePartitionTableEx(IN PDEVICE_OBJECT DeviceObject, IN struct _DRIVE_LAYOUT_INFORMATION_EX* DriveLayout);
NTKERNELAPI NTSTATUS IoSetPartitionInformationEx(IN PDEVICE_OBJECT DeviceObject, IN ULONG PartitionNumber, IN struct _SET_PARTITION_INFORMATION_EX* PartitionInfo);
NTKERNELAPI NTSTATUS IoUpdateDiskGeometry(IN PDEVICE_OBJECT DeviceObject, IN struct _DISK_GEOMETRY_EX* OldDiskGeometry, IN struct _DISK_GEOMETRY_EX* NewDiskGeometry);
NTKERNELAPI NTSTATUS IoVerifyPartitionTable(IN PDEVICE_OBJECT DeviceObject, IN BOOLEAN FixErrors);
...
NTKERNELAPI NTSTATUS IoReadDiskSignature(IN PDEVICE_OBJECT DeviceObject, IN ULONG BytesPerSector, OUT PDISK_SIGNATURE Signature);
...
NTKERNELAPI NTSTATUS IoVolumeDeviceToDosName(IN PVOID VolumeDeviceObject, OUT PUNICODE_STRING DosName);
...
NTKERNELAPI PDEVICE_OBJECT IoGetLowerDeviceObject(IN  PDEVICE_OBJECT  DeviceObject);
NTKERNELAPI PDEVICE_OBJECT IoGetDeviceAttachmentBaseRef(IN PDEVICE_OBJECT DeviceObject);
NTKERNELAPI NTSTATUS IoGetDiskDeviceObject(IN  PDEVICE_OBJECT  FileSystemDeviceObject, OUT PDEVICE_OBJECT  *DiskDeviceObject);

// end_ntosp end_ntifs

// begin_ntosp begin_ntifs begin_ntddk

NTKERNELAPI NTSTATUS IoSetSystemPartition(PUNICODE_STRING VolumeNameString);

// begin_wdm

NTKERNELAPI VOID IoFreeErrorLogEntry(PVOID ElEntry);

// Cancel SAFE API set start

// The following APIs are to help ease the pain of writing queue packages that handle the cancellation race well.
// The idea of this set of APIs is to not force a single queue data structure but allow the cancel logic to be hidden from the drivers.
...
// Then in the dispatch routine when the driver wants to insert an IRP into the queue it calls IoCsqInsertIrp.
// When the driver wants to remove something from the queue it calls IoCsqRemoveIrp.
// Note that Insert can fail if the IRP was cancelled in the meantime.
// Remove can also fail if the IRP was already cancelled.

// There are typically two modes where drivers queue IRPs.
// These two modes are covered by the cancel safe queue API set.

// Mode 1:
// One is where the driver queues the IRP and at some later point in time dequeues an IRP and issues the IO request.
// For this mode the driver should use IoCsqInsertIrp and IoCsqRemoveNextIrp.
// The driver in this case is expected to pass NULL to the irp context parameter in IoInsertIrp.

// Mode 2:
// In this the driver queues the IRP, issues the IO request (like issuing a DMA request or writing to a register) and
// when the IO request completes (either using a DPC or timer) the driver dequeues the IRP and completes it.
// For this mode the driver should use IoCsqInsertIrp and IoCsqRemoveIrp.
// In this case the driver should allocate an IRP context and pass it in to IoCsqInsertIrp.
// The cancel API code creates an association between the IRP and the context and thus ensures that when the time comes to remove the IRP it can ascertain correctly.

// Note that the cancel API set assumes that the field DriverContext[3] is always available for use and that the driver does not use it.


// Bookkeeping structure. This should be opaque to drivers.
// Drivers typically include this as part of their queue headers.
// Given a CSQ pointer the driver should be able to get its queue header using CONTAINING_RECORD macro
...
typedef NTSTATUS(*PIO_CSQ_INSERT_IRP_EX)(IN struct _IO_CSQ *Csq, IN PIRP Irp, IN OUT PVOID InsertContext);
...
// This should be opaque to drivers.
// Drivers typically include this as part of their queue headers.
// Given a CSQ pointer the driver should be able to get its queue header using CONTAINING_RECORD macro
...
NTKERNELAPI VOID IoCsqInsertIrp(IN PIO_CSQ Csq, IN PIRP Irp, IN PIO_CSQ_IRP_CONTEXT Context);
NTKERNELAPI NTSTATUS IoCsqInsertIrpEx(IN PIO_CSQ Csq, IN PIRP Irp, IN PIO_CSQ_IRP_CONTEXT Context, IN PVOID InsertContext);


// Returns an IRP if one can be found. NULL otherwise.
NTKERNELAPI PIRP IoCsqRemoveNextIrp(IN  PIO_CSQ Csq, IN PVOID PeekContext);
...
NTKERNELAPI PIRP IoCsqRemoveIrp(IN  PIO_CSQ Csq, IN  PIO_CSQ_IRP_CONTEXT Context);
...
NTKERNELAPI BOOLEAN IoIsFileOriginRemote(IN PFILE_OBJECT FileObject);
NTKERNELAPI NTSTATUS IoSetFileOrigin(IN PFILE_OBJECT FileObject, IN BOOLEAN Remote);
...
BOOLEAN IoIsDeviceEjectable(IN  PDEVICE_OBJECT);
NTSTATUS IoComputeDesiredAccessFileObject(IN PFILE_OBJECT FileObject, OUT PNTSTATUS DesiredAccess);

// begin_ntosp begin_ntifs begin_ntddk
// begin_wdm

NTKERNELAPI NTSTATUS IoValidateDeviceIoControlAccess(IN  PIRP Irp, IN ULONG RequiredAccess);

// end_wdm

NTKERNELAPI IO_PAGING_PRIORITY FASTCALL IoGetPagingIoPriority(IN PIRP IRP);
