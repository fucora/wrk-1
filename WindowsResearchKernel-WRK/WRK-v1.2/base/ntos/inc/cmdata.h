...
// we create NameBlock structure so KCB's of same names can share the NameBlock.
// NameBlock is compressed.
...
// 2. CM_KCB_SYM_LINK_FOUND: This bit is only for symbolic keys.
//                           It indicates that the symbolic link has been resolved and the KCB for the link is pointed to by ValueCache.RealKcb.
//                           In this case, the Value Index of this key is no longer available in the KCB.
//                           (We hardly query the value of a symbolic link key other than finding the path of the real key anyway).
...
//                           key should not be kept in delay close when the reference count goes to zero.
//                           This is for the case when a key has no open handles but still has subkeys in the cache.
//                           When its last subkey is kicked out of cache, we do not want to keep this key around.
//                           This is done so CmpSearchForOpenSubKeysInCachen can clean up the cache properly before a key can be unloaded.
...
// NOTE: the KCB size is 56 bytes now, plus the pool header of 8 bytes, it fits into a 64byte allocation.
//       Think carefully if you want to enlarge the data structure.
//       Also, watch it if the pool allocation code changes.
...
//      NOTE:   CHILD_LIST structures are normally referred to with HCELL_INDEX, not PCHILD_LIST vars.
...
// for the HCELL_INDEX and one dword to hold the first four characters of the subkey name.
//If one of the first four characters in the subkey name is a unicode character where the high byte is non-zero, the actual subkey must be examined to determine the name.

// Hive version 1 & 2 do not support the fast index. Version 3 adds support for the
// fast index. All hives that are newly created on a V3-capable system are therefore unreadable on V1 & 2 systems.
...
// Allow index to grow to size that will cause allocation of exactly one logical block.
// Works out to be 1013 entries.
#define CM_MAX_INDEX                                                        \
 ( (HBLOCK_SIZE-                                                             \
...
// Allow index to grow to size that will cause allocation of exactly one logical block.
// Works out to be approx. 500 entries.
...
#define KEY_VOLATILE        0x0001      // This key (and all its children) is volatile.
#define KEY_HIVE_EXIT       0x0002      // This key marks a boundary to another hive (sort of a link).
                                        // The null value entry contains the hive and hive index of the root of the child hive.
#define KEY_HIVE_ENTRY      0x0004      // This key is the root of a particular hive.
...
#define KEY_COMP_NAME       0x0020      // The name for this key is stored in a compressed form.
...
#define KEY_USER_FLAGS_VALID_MASK   0x000F  // we only allow 4 bits for the user defined flags
                                            // (this is just for the time being) - we may extend this as we see fit)
...
typedef struct _CM_KEY_NODE {
    ...
    ULONG       WorkVar;                // WARNING: This DWORD is used by the system at run time, do attempt to store user data in it.
...
typedef struct _CM_KEY_SECURITY_CACHE_ENTRY {
    HCELL_INDEX             Cell;                   // security cellindex (inside the hive) -
                                                    // this is to avoid touching the Security pages at lookup operations
...
typedef struct _CELL_DATA {
    union _u {
        ...
        CM_BIG_DATA      ValueData;      // This is only for big cells; a list of cells all of the length CM_KEY_VALUE_BIG
...
#define CM_KCB_SUBKEY_ONE       0x0002      // This key has only one subkey and the first 4 char
...
typedef struct _CM_KEY_CONTROL_BLOCK {
    ...
    struct {
        ...
        ULONG                   DelayedCloseIndex   : 12; // 00000000 00?????? ??????00 00000000  CmpDelayedCloseSize  means it is not in the delay close table
    ...
    union {                                                         // The hint is always stored in uppercase.
        ...
        ULONG                   SubKeyCount;                        // when none of the  CM_KCB_NO_SUBKEY | CM_KCB_SUBKEY_ONE | CM_KCB_SUBKEY_HINT is set in ExtFlags,
                                                                    // we cache here the number of subkeys (Node->SubKeyCounts[Stable] + Node->SubKeyCounts[Volatile])
    };
