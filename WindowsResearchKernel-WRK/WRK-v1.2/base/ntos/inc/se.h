...
//  Data structure used to capture subject security context for access validations and auditing.
...
//    ClientToken - optionally points to a token object being used by the subject's thread to impersonate a client.
//        If the subject's thread is not impersonating a client, this field is set to null.
//        The token's reference count is incremented to count this field as an outstanding reference.

//    ImpersonationLevel - Contains the impersonation level of the subject's thread.
//        This field is only meaningful if the ClientToken field is not null.
//        This field over-rides any higher impersonation level value that might be in the client's token.
...
//        reference count of the process object is not incremented to count it as an outstanding reference.
...
//  Initial Privilege Set - Room for three privileges, which should be enough for most applications.
//  This structure exists so that it can be embedded in an ACCESS_STATE structure.
//  Use PRIVILEGE_SET for all other references to Privilege sets.
...
where:
    ...
    OperationID - an LUID to identify the operation being performed.  This
        ID will be put in the audit log to allow non-contiguous operations on the same object to be associated with one another.

    SecurityEvaluated - a marker to be set by Parse Methods to indicate that security access checking and audit logging has been performed.

    Flags - Holds misc flags for reference during the access attempt.

    AuditHandleCreation - a flag set by SeOpenObjectAuditAlarm to indicate that auditing is to take place when the handle for the object is allocated.

    RemainingDesiredAccess - Access mask containing the access types that have not yet been granted.

    PreviouslyGrantedAccess - Access mask containing the access types that have been granted,
        one way or another (for example, a given access may be granted as a result of owning a privilege rather than being in an ACL.
        A routine can check the privilege and mark the access as granted without doing a formal access check).

    SubjectSecurityContext - The subject's captured security context

    PrivilegesAllocated - Flag to indicate whether we have allocated space for the privilege set from pool memory, so it can be freed.

    SecurityDescriptor - Temporarily contains the security descriptor for the object being created between the time the user's
       security descriptor is captured and the time the security descriptor is passed to SeAssignSecurity.
       NO ONE BUT SEASSIGNSECURITY SHOULD EVER LOOK IN THIS FIELD FOR AN OBJECT'S SECURITY DESCRIPTOR.

    AuxData - points to an auxiliary data structure to be used for future expansion of the access state in an upwardly compatible way.
        This field replaces the PrivilegesUsed pointer, which was for internal use only.

    Privileges - A set of privileges, some of which may have the UsedForAccess bit set.
        If the pre-allocated number of privileges is not enough, we will allocate space from pool memory to allow for growth.
*/

//  Since the AccessState structure is publically exposed to driver writers, this structure contains additional data added after NT 3.51.       *
//                                                                              *
//  Its contents must be accessed only through Se level interfaces, never directly by name.
//                                                                              *
...
/*
where:
    PrivilegesUsed - Points to the set of privileges used during the access validation.

    GenericMapping - Points to the generic mapping for the object being accessed.
        Normally this would be filled in with the generic mapping passed to SeCreateAccessState,
        but in the case of the IO system (which does not know the type of object being accessed until it parses the name),
        it must be filled in later.  See the discussion of the GenericMapping parameter in SeCreateAccessState for more details.

    AccessToAudit - Used as a temporary holding area for the access mask to put into the audit record.
        This field is necessary because the access being put into the newly created handle may not be the ones we want to audit.
        This occurs when a file is opened for read-only transacted mode, where a read only file is opened for write access.
        We don't want to audit the fact that we granted write access, since we really didn't, and customers would be confused to see the extra
        bit in the audit record.

   MaximumAuditMask - Stores the audit mask that will be stored into the new handle structure to support operation based audits.
*/
...
//  registered of logon sessions being terminated, so they can cleanup state associated with this logon session
...
//  ACCESS_MASK SeComputeDeniedAccesses(IN ACCESS_MASK GrantedAccess, IN ACCESS_MASK DesiredAccess);
//  Routine Description:
//      This routine generates an access mask containing those accesses requested by DesiredAccess that aren't granted by GrantedAccess.
//      The result of this routine may be compared to 0 to determine
//      if a DesiredAccess mask contains any accesses that have not been granted.
...
//  Return Value:
//      An ACCESS_MASK containing the desired accesses that have not been granted.
#define SeComputeDeniedAccesses( GrantedAccess, DesiredAccess )     ((~(GrantedAccess)) & (DesiredAccess) )


//  BOOLEAN SeComputeGrantedAccesses(IN ACCESS_MASK GrantedAccess, IN ACCESS_MASK DesiredAccess);
//  Routine Description:
//      This routine generates an access mask containing accesses requested by DesiredAccess that are granted by GrantedAccess.
//      The result of this routine may be compared to 0 to determine
//      if any desired accesses have been granted.

//      If the result IS NON-ZERO, then at least one desired accesses has been granted.

//  Arguments:
//      GrantedAccess - Specifies the granted access mask.
//      DesiredAccess - Specifies the desired access mask.
//  Return Value:
//      This routine returns TRUE if the DesiredAccess mask does specifies any bits that are set in the GrantedAccess mask.
#define SeComputeGrantedAccesses( GrantedAccess, DesiredAccess )     ((GrantedAccess) & (DesiredAccess) )


// begin_ntifs
//  ULONG SeLengthSid(IN PSID Sid);
//  Routine Description:
...
//  BOOLEAN SeSameToken (IN PTOKEN_CONTROL TokenControl1, IN PTOKEN_CONTROL TokenControl2)
//  Routine Description:
//      This routine returns a boolean value indicating whether the two token control values represent the same token.  The token may
...
// begin_ntifs
// VOID SeDeleteClientSecurity(IN PSECURITY_CLIENT_CONTEXT ClientContext)
// Routine Description:
//    This service deletes a client security context block,
//    performing whatever cleanup might be necessary to do so.  In particular, reference to any client token is removed.
// Arguments:
//    ClientContext - Points to the client security context block to be deleted.
...
// VOID SeStopImpersonatingClient()
// Routine Description:
//    This service is used to stop impersonating a client using an
//    impersonation token.  This service must be called in the context of the server thread which wishes to stop impersonating its client.
...
/*
Routine Description:
    This routine asserts that the given AccessMask does not contain any generic access types.
Arguments:
...
/*
Routine Description:
    This macro computes the amount of quota to charge for security information.

    The current algorithm is to use the larger of twice the size of the Group + Dacl information being applied and the default as specified by SE_DEFAULT_SECURITY_QUOTA.
Arguments:
    Size - The size in bytes of the Group + Dacl information being applied to the object.
Return Value:
...
//  PACCESS_TOKEN SeQuerySubjectContextToken(IN PSECURITY_SUBJECT_CONTEXT SubjectContext);
//  Routine Description:
//      This routine returns the effective token from the subject context, either the client token, if present, or the process token.
//  Arguments:
...
VOID SeSetSecurityAccessMask(__in SECURITY_INFORMATION SecurityInformation, __out PACCESS_MASK DesiredAccess);
VOID SeQuerySecurityAccessMask(__in SECURITY_INFORMATION SecurityInformation, __out PACCESS_MASK DesiredAccess);
NTSTATUS SeDefaultObjectMethod (
    __in PVOID Object,
...
// begin_ntosp
NTKERNELAPI NTSTATUS SeCaptureSecurityDescriptor (
...
NTKERNELAPI VOID SeReleaseSecurityDescriptor (__in PSECURITY_DESCRIPTOR CapturedSecurityDescriptor, __in KPROCESSOR_MODE RequestorMode, __in BOOLEAN ForceCapture);

// begin_ntifs

NTKERNELAPI VOID SeCaptureSubjectContext (__out PSECURITY_SUBJECT_CONTEXT SubjectContext);
NTKERNELAPI VOID SeLockSubjectContext(__in PSECURITY_SUBJECT_CONTEXT SubjectContext);
NTKERNELAPI VOID SeUnlockSubjectContext(__in PSECURITY_SUBJECT_CONTEXT SubjectContext);
NTKERNELAPI VOID SeReleaseSubjectContext (__inout PSECURITY_SUBJECT_CONTEXT SubjectContext);
NTSTATUS SeCaptureAuditPolicy(
...
VOID SeReleaseAuditPolicy (__in PTOKEN_AUDIT_POLICY CapturedPolicy, __in KPROCESSOR_MODE RequestorMode, __in BOOLEAN ForceCapture);
...
VOID SeReleaseAcl (__in PACL CapturedAcl, __in KPROCESSOR_MODE RequestorMode, __in BOOLEAN ForceCapture);


NTSTATUS SeCaptureLuidAndAttributesArray (
...
NTSTATUS SeCaptureSidAndAttributesArray (
...
NTKERNELAPI NTSTATUS SeDeassignSecurity (__deref_inout PSECURITY_DESCRIPTOR *SecurityDescriptor);
...
VOID SeGetWorldRights (__in PSECURITY_DESCRIPTOR SecurityDescriptor, __in PGENERIC_MAPPING GenericMapping, __out PACCESS_MASK GrantedAccess);
#endif
...
NTKERNELAPI BOOLEAN SePrivilegeCheck(__inout PPRIVILEGE_SET RequiredPrivileges, __in PSECURITY_SUBJECT_CONTEXT SubjectSecurityContext, __in KPROCESSOR_MODE AccessMode);
NTKERNELAPI VOID SeFreePrivileges(__in PPRIVILEGE_SET Privileges);

// end_ntifs end_ntosp

NTSTATUS SePrivilegePolicyCheck(
...
NTKERNELAPI VOID SeDeleteObjectAuditAlarm(__in PVOID Object, __in HANDLE Handle);

// end_ntifs

VOID SeCloseObjectAuditAlarm(__in PVOID Object, __in HANDLE Handle, __in BOOLEAN GenerateOnClose);

VOID SeCreateInstanceAuditAlarm(
...
// begin_ntosp
NTKERNELAPI VOID SePrivilegeObjectAuditAlarm(
    __in HANDLE Handle,
    __in PSECURITY_SUBJECT_CONTEXT SubjectSecurityContext,
    __in ACCESS_MASK DesiredAccess,
...
NTKERNELAPI BOOLEAN SeValidSecurityDescriptor(__in ULONG Length, __in_bcount(Length) PSECURITY_DESCRIPTOR SecurityDescriptor);

// end_ntddk end_wdm end_ntifs


// VOID SeImplicitObjectAuditAlarm(
//    IN PLUID OperationID OPTIONAL,
//    IN PVOID Object,
//    IN PSECURITY_DESCRIPTOR SecurityDescriptor,
//    IN PSECURITY_SUBJECT_CONTEXT SubjectSecurityContext,
...
VOID SeAuditHandleCreation(__in PACCESS_STATE AccessState, __in HANDLE Handle);
PACCESS_TOKEN SeMakeSystemToken (VOID);
...
SECURITY_IMPERSONATION_LEVEL SeTokenImpersonationLevel(__in PACCESS_TOKEN Token);
...
VOID SeAssignPrimaryToken(__in PEPROCESS Process, __in PACCESS_TOKEN Token);
VOID SeDeassignPrimaryToken(__in PEPROCESS Process);
...
NTSTATUS SeCopyClientToken(__in PACCESS_TOKEN ClientToken, __in SECURITY_IMPERSONATION_LEVEL ImpersonationLevel, __in KPROCESSOR_MODE RequestorMode, __deref_out PACCESS_TOKEN *DuplicateToken);
...
// begin_ntosp
NTKERNELAPI NTSTATUS SeQueryAuthenticationIdToken(__in PACCESS_TOKEN Token, __out PLUID AuthenticationId);

// end_ntosp
NTKERNELAPI NTSTATUS SeQuerySessionIdToken(__in PACCESS_TOKEN Token, __out PULONG SessionId);
NTKERNELAPI NTSTATUS SeSetSessionIdToken(__in PACCESS_TOKEN Token, __in ULONG SessionId);

// begin_ntosp
NTKERNELAPI NTSTATUS SeCreateClientSecurity (__in PETHREAD ClientThread, __in PSECURITY_QUALITY_OF_SERVICE ClientSecurityQos, __in BOOLEAN RemoteSession, __out PSECURITY_CLIENT_CONTEXT ClientContext);
// end_ntosp

NTKERNELAPI VOID SeImpersonateClient(__in PSECURITY_CLIENT_CONTEXT ClientContext, __in_opt PETHREAD ServerThread);

// begin_ntosp
NTKERNELAPI NTSTATUS SeImpersonateClientEx(__in PSECURITY_CLIENT_CONTEXT ClientContext, __in_opt PETHREAD ServerThread);
// end_ntosp
...
// does not take the AuxData parameter and export that.

// begin_ntosp
NTKERNELAPI NTSTATUS SeCreateAccessState(__out PACCESS_STATE AccessState, __out PAUX_ACCESS_DATA AuxData, __in ACCESS_MASK DesiredAccess, __in PGENERIC_MAPPING GenericMapping);
NTKERNELAPI VOID SeDeleteAccessState(__in PACCESS_STATE AccessState);
// end_ntosp

NTSTATUS SeCreateAccessStateEx(
...
NTSTATUS SeLocateProcessImageName(__in PEPROCESS Process, __deref_out PUNICODE_STRING *pImageFileName);
VOID SeAuditSystemTimeChange(__in LARGE_INTEGER OldTime, __in LARGE_INTEGER NewTime);
...
NTKERNELAPI NTSTATUS SeAppendPrivileges(__inout PACCESS_STATE AccessState, __in PPRIVILEGE_SET Privileges);

// end_ntifs end_ntosp

NTSTATUS SeComputeQuotaInformationSize(__in PSECURITY_DESCRIPTOR SecurityDescriptor, __out PULONG Size);
VOID SePrivilegedServiceAuditAlarm (__in_opt PUNICODE_STRING ServiceName, __in PSECURITY_SUBJECT_CONTEXT SubjectSecurityContext, __in PPRIVILEGE_SET Privileges, __in BOOLEAN AccessGranted);
...
BOOLEAN SeCheckAuditPrivilege (__in PSECURITY_SUBJECT_CONTEXT SubjectSecurityContext, __in KPROCESSOR_MODE PreviousMode);
...
BOOLEAN SeFastTraverseCheck(__in PSECURITY_DESCRIPTOR SecurityDescriptor, __in_opt PACCESS_STATE AccessState, __in ACCESS_MASK TraverseAccess, __in KPROCESSOR_MODE AccessMode);

// begin_ntifs

NTKERNELAPI BOOLEAN SeAuditingFileEvents(__in BOOLEAN AccessGranted, __in PSECURITY_DESCRIPTOR SecurityDescriptor);
NTKERNELAPI BOOLEAN SeAuditingFileEventsWithContext(__in BOOLEAN AccessGranted, __in PSECURITY_DESCRIPTOR SecurityDescriptor, __in_opt PSECURITY_SUBJECT_CONTEXT SubjectSecurityContext);
NTKERNELAPI BOOLEAN SeAuditingHardLinkEvents(__in BOOLEAN AccessGranted, __in PSECURITY_DESCRIPTOR SecurityDescriptor);
NTKERNELAPI BOOLEAN SeAuditingHardLinkEventsWithContext(__in BOOLEAN AccessGranted, __in PSECURITY_DESCRIPTOR SecurityDescriptor, __in_opt PSECURITY_SUBJECT_CONTEXT SubjectSecurityContext);
NTKERNELAPI BOOLEAN SeAuditingFileOrGlobalEvents(__in BOOLEAN AccessGranted, __in PSECURITY_DESCRIPTOR SecurityDescriptor, __in PSECURITY_SUBJECT_CONTEXT SubjectSecurityContext);
NTKERNELAPI BOOLEAN FASTCALL SeDetailedAuditingWithToken(__in_opt PACCESS_TOKEN AccessToken);

// end_ntifs

VOID SeAuditProcessCreation(__in PEPROCESS Process);
VOID SeAuditProcessExit(__in PEPROCESS Process);
...
VOID SeAuditLPCInvalidUse(__in PUNICODE_STRING LpcCallName, __in PUNICODE_STRING LpcServerPort);
...
VOID SeSetAccessStateGenericMapping (__inout PACCESS_STATE AccessState, __in PGENERIC_MAPPING GenericMapping);
...
NTKERNELAPI NTSTATUS SeQueryInformationToken (__in PACCESS_TOKEN Token, __in TOKEN_INFORMATION_CLASS TokenInformationClass, __deref_out PVOID *TokenInformation);

// end_ntifs end_ntosp

NTSTATUS SeIsChildToken(__in HANDLE Token, __out PBOOLEAN IsChild);
NTSTATUS SeIsChildTokenByPointer(__in PACCESS_TOKEN Token, __out PBOOLEAN IsChild);
NTSTATUS SeIsSiblingToken(__in HANDLE Token, __out PBOOLEAN IsSibling);
NTSTATUS SeIsSiblingTokenByPointer(__in PACCESS_TOKEN Token, __out PBOOLEAN IsSibling);
...
//  If you wish to include this file in an NT driver and use SeExports structure defined above, you need to call:
...
extern NTKERNELAPI PSE_EXPORTS SeExports;//  Grants access to SeExports structure
