...
// A pair of macros to deal with the packing of parent & balance in the MMADDRESS_NODE.
...
#define MI_MAKE_PARENT(ParentNode,ExistingBalance)                 (PMMADDRESS_NODE)((ULONG_PTR)(ParentNode) | ((ExistingBalance) & 0x3))
...
// Audit Information structure: this is a member of the EPROCESS structure and currently contains only the name of the exec'ed image file.
...
// If you remove a field from this structure, please also remove the reference to it from within the kernel debugger
// (nt\private\sdktools\ntsd\ntkext.c)
...
typedef struct _EPROCESS {
    ...
    // Structure to allow lock free cross process access to the process handle table, process section and address space.
    // Acquire rundown protection with this if you do cross process handle table, process section or address space references.
    EX_RUNDOWN_REF RundownProtect;

    HANDLE UniqueProcessId;

    // Global list of all processes in the system. Processes are removed from this list in the object deletion routine.
    // References to processes in this list must be done with ObReferenceObjectSafe because of this.
    ...
    EX_FAST_REF PrefetchTrace;// Pointer to the prefetches trace block.
    ...
    // Process flags. Use interlocked operations with PS_SET_BITS, etc to modify these.
    ...
    union {
        ...
        // Fields can only be set by the PS_SET_BITS and other interlocked macros.
        // Reading fields is best done via the bit definitions so references are easy to locate.
        struct {
...
typedef struct _TERMINATION_PORT {// Thread termination port
...
#define PS_SET_THREAD_CREATE_TIME(Thread, InputCreateTime)             ((Thread)->CreateTime.QuadPart = (InputCreateTime.QuadPart))
...
typedef struct _ETHREAD {
    ...
    LIST_ENTRY PostBlockList;// Registry

    // Single linked list of termination blocks
    union {
        PTERMINATION_PORT TerminationPort;// List of termination ports
        struct _ETHREAD *ReaperLink;// List of threads to be reaped. Only used at thread exit
        PVOID KeyedWaitValue;// Keyvalue being waited for
    };
    ...
    PPS_IMPERSONATION_INFORMATION ImpersonationInfo;//    Client - If non null, indicates the thread is impersonating a client.

    LIST_ENTRY IrpList;// Io
    ...
    EX_RUNDOWN_REF RundownProtect;// Rundown protection structure. Acquire this to do cross thread TEB, TEB32 or stack references.

    EX_PUSH_LOCK ThreadLock;// Lock to protect thread impersonation information
    ULONG LpcReplyMessageId;    // MessageId this thread is waiting for reply to
    ULONG ReadClusterSize;

    ACCESS_MASK GrantedAccess;// Client/server

    // Flags for cross thread access. Use interlocked operations via PS_SET_BITS etc.
    #define PS_CROSS_THREAD_FLAGS_TERMINATED           0x00000001UL// Used to signify that the delete APC has been queued or the thread has called PspExitThread itself.
    #define PS_CROSS_THREAD_FLAGS_DEADTHREAD           0x00000002UL// Thread create failed
    #define PS_CROSS_THREAD_FLAGS_HIDEFROMDBG          0x00000004UL// Debugger isn't shown this thread
    #define PS_CROSS_THREAD_FLAGS_IMPERSONATING        0x00000008UL// Thread is impersonating
    #define PS_CROSS_THREAD_FLAGS_SYSTEM               0x00000010UL// This is a system thread
    #define PS_CROSS_THREAD_FLAGS_HARD_ERRORS_DISABLED 0x00000020UL// Hard errors are disabled for this thread
    #define PS_CROSS_THREAD_FLAGS_BREAK_ON_TERMINATION 0x00000040UL// We should break in when this thread is terminated
    #define PS_CROSS_THREAD_FLAGS_SKIP_CREATION_MSG    0x00000080UL// This thread should skip sending its create thread message
    #define PS_CROSS_THREAD_FLAGS_SKIP_TERMINATION_MSG 0x00000100UL// This thread should skip sending its final thread termination message
    ...
    // Flags to be accessed in this thread's context only at PASSIVE level -- no need to use interlocked operations.
    union {
        ULONG SameThreadPassiveFlags;

        struct {
            // This thread is an active Ex worker thread; it should not terminate.
            ULONG ActiveExWorker : 1;
            ULONG ExWorkerCanWaitUser : 1;
            ULONG MemoryMaker : 1;

            ULONG KeyedEventInUse : 1;// Thread is active in the keyed event code. LPC should not run above this in an APC.
        };
    };
    ...
    union {
        ...
        struct {
            // The stored thread's MSGID is valid. This is only accessed while the LPC mutex is held so it's an APC_LEVEL flag.
...
// The following two inline functions allow a thread or process object to be converted into a kernel thread or process, respectively, without
// having to expose the ETHREAD and EPROCESS definitions to the world.

// These functions take advantage of the fact that the kernel structures appear as the first element in the respective object structures.

// begin_ntosp

PKTHREAD FORCEINLINE PsGetKernelThread(IN PETHREAD ThreadObject)
{
    return (PKTHREAD)ThreadObject;
}

PKPROCESS FORCEINLINE PsGetKernelProcess(IN PEPROCESS ProcessObject)
{
    return (PKPROCESS)ProcessObject;
}

NTKERNELAPI NTSTATUS PsGetContextThread(__in PETHREAD Thread, __inout PCONTEXT ThreadContext, __in KPROCESSOR_MODE Mode);
NTKERNELAPI NTSTATUS PsSetContextThread(__in PETHREAD Thread, __in PCONTEXT ThreadContext, __in KPROCESSOR_MODE Mode);
...
typedef struct _EJOB {
    ...
    // All processes within this job. Processes are removed from this list at last dereference.
    // Safe object referencing needs to be done.
    ...
    // List of jobs in a job set. Processes within a job in a job set can create processes in the same or higher members of the jobset.
    // Protected by the global lock PspJobListLock
    LIST_ENTRY JobSetLinks;

    ULONG MemberLevel;// Member level for this job in the jobset.
...
BOOLEAN PsChangeJobMemoryUsage(IN ULONG Flags, IN SSIZE_T Amount);
...
NTSTATUS PsMapSystemDll (IN PEPROCESS Process, OUT PVOID *DllBase OPTIONAL, IN LOGICAL UseLargePages);
...
// N.B. The kernel thread object is architecturally defined as being at offset zero of the executive thread object.
//      This assumption has been exported from ntddk.h for some time.
C_ASSERT(FIELD_OFFSET(ETHREAD, Tcb) == 0);
...
NTKERNELAPI PEPROCESS PsGetCurrentProcess(VOID);
...
NTKERNELAPI NTSTATUS PsTerminateSystemThread(__in NTSTATUS ExitStatus);
NTKERNELAPI NTSTATUS PsWrapApcWow64Thread (__inout PVOID *ApcContext, __inout PVOID *ApcRoutine);


// end_ntddk end_wdm end_nthal end_ntifs end_ntosp

NTKERNELAPI NTSTATUS PsCreateSystemProcess(__out PHANDLE ProcessHandle, __in ULONG DesiredAccess, __in_opt POBJECT_ATTRIBUTES ObjectAttributes);
typedef VOID (*PBBT_NOTIFY_ROUTINE)(PKTHREAD Thread);
NTKERNELAPI ULONG PsSetBBTNotifyRoutine(__in PBBT_NOTIFY_ROUTINE BBTNotifyRoutine);
...
NTKERNELAPI NTSTATUS PsSetCreateProcessNotifyRoutine(__in PCREATE_PROCESS_NOTIFY_ROUTINE NotifyRoutine, __in BOOLEAN Remove);
typedef VOID (*PCREATE_THREAD_NOTIFY_ROUTINE)(IN HANDLE ProcessId, IN HANDLE ThreadId, IN BOOLEAN Create);
NTKERNELAPI NTSTATUS PsSetCreateThreadNotifyRoutine(__in PCREATE_THREAD_NOTIFY_ROUTINE NotifyRoutine);
NTKERNELAPI NTSTATUS PsRemoveCreateThreadNotifyRoutine (__in PCREATE_THREAD_NOTIFY_ROUTINE NotifyRoutine);
...
NTKERNELAPI NTSTATUS PsSetLoadImageNotifyRoutine(__in PLOAD_IMAGE_NOTIFY_ROUTINE NotifyRoutine);
NTKERNELAPI NTSTATUS PsRemoveLoadImageNotifyRoutine(__in PLOAD_IMAGE_NOTIFY_ROUTINE NotifyRoutine);
...
NTKERNELAPI NTSTATUS PsAssignImpersonationToken(__in PETHREAD Thread, __in HANDLE Token);

// begin_ntosp

NTKERNELAPI PACCESS_TOKEN PsReferencePrimaryToken(__inout PEPROCESS Process);
NTKERNELAPI VOID PsDereferencePrimaryToken(__in PACCESS_TOKEN PrimaryToken);
NTKERNELAPI VOID PsDereferenceImpersonationToken(__in PACCESS_TOKEN ImpersonationToken);
...
NTKERNELAPI PACCESS_TOKEN PsReferenceImpersonationToken(__inout PETHREAD Thread, __out PBOOLEAN CopyOnOpen, __out PBOOLEAN EffectiveOnly, __out PSECURITY_IMPERSONATION_LEVEL ImpersonationLevel);
...
NTKERNELAPI LARGE_INTEGER PsGetProcessExitTime(VOID);
...
NTKERNELAPI BOOLEAN PsIsThreadTerminating(__in PETHREAD Thread);
...
NTKERNELAPI BOOLEAN PsDisableImpersonation(__inout PETHREAD Thread, __inout PSE_IMPERSONATION_STATE ImpersonationState);
NTKERNELAPI VOID PsRestoreImpersonation(__inout PETHREAD Thread, __in PSE_IMPERSONATION_STATE ImpersonationState);
...
NTKERNELAPI VOID PsRevertThreadToSelf(__inout PETHREAD Thread);
...
NTKERNELAPI NTSTATUS PsLookupProcessThreadByCid(__in PCLIENT_ID Cid, __deref_opt_out PEPROCESS *Process, __deref_out PETHREAD *Thread);

// begin_ntosp

NTKERNELAPI NTSTATUS PsLookupProcessByProcessId(__in HANDLE ProcessId, __deref_out PEPROCESS *Process);
NTKERNELAPI NTSTATUS PsLookupThreadByThreadId(__in HANDLE ThreadId, __deref_out PETHREAD *Thread);

// begin_ntifs

// Quota Operations
NTKERNELAPI VOID PsChargePoolQuota(__in PEPROCESS Process, __in POOL_TYPE PoolType, __in ULONG_PTR Amount);
NTKERNELAPI NTSTATUS PsChargeProcessPoolQuota(__in PEPROCESS Process, __in POOL_TYPE PoolType, __in ULONG_PTR Amount);
NTKERNELAPI VOID PsReturnPoolQuota(__in PEPROCESS Process, __in POOL_TYPE PoolType, __in ULONG_PTR Amount);

// end_ntifs
// end_ntosp

NTSTATUS PsChargeProcessQuota (IN PEPROCESS Process, IN PS_QUOTA_TYPE QuotaType, IN SIZE_T Amount);
VOID PsReturnProcessQuota (IN PEPROCESS Process, IN PS_QUOTA_TYPE QuotaType, IN SIZE_T Amount);
NTKERNELAPI NTSTATUS PsChargeProcessNonPagedPoolQuota(__in PEPROCESS Process, __in SIZE_T Amount);
NTKERNELAPI VOID PsReturnProcessNonPagedPoolQuota(__in PEPROCESS Process, __in SIZE_T Amount);
NTKERNELAPI NTSTATUS PsChargeProcessPagedPoolQuota(__in PEPROCESS Process, __in SIZE_T Amount);
NTKERNELAPI VOID PsReturnProcessPagedPoolQuota(__in PEPROCESS Process, __in SIZE_T Amount);
NTSTATUS PsChargeProcessPageFileQuota(IN PEPROCESS Process, IN SIZE_T Amount);
...
NTKERNELAPI VOID PsEstablishWin32Callouts(__in PKWIN32_CALLOUTS_FPNS pWin32Callouts);
...
NTKERNELAPI VOID PsSetProcessPriorityByClass(__inout PEPROCESS Process, __in PSPROCESSPRIORITYMODE PriorityMode);
...
NTKERNELAPI HANDLE PsGetCurrentThreadId(VOID);

// end_ntosp

NTKERNELAPI BOOLEAN PsGetVersion(__out_opt PULONG MajorVersion, __out_opt PULONG MinorVersion, __out_opt PULONG BuildNumber, __out_opt PUNICODE_STRING CSDVersion);
...
NTKERNELAPI PVOID PsGetCurrentThreadStackLimit(VOID);
NTKERNELAPI PVOID PsGetCurrentThreadStackBase(VOID);
NTKERNELAPI CCHAR PsGetCurrentThreadPreviousMode(VOID);
NTKERNELAPI PERESOURCE PsGetJobLock(__in PEJOB Job);
NTKERNELAPI ULONG PsGetJobSessionId(__in PEJOB Job);
NTKERNELAPI ULONG PsGetJobUIRestrictionsClass(__in PEJOB Job);
NTKERNELAPI LONGLONG PsGetProcessCreateTimeQuadPart(__in PEPROCESS Process);
NTKERNELAPI PVOID PsGetProcessDebugPort(__in PEPROCESS Process);
NTKERNELAPI BOOLEAN PsIsProcessBeingDebugged(__in PEPROCESS Process);
NTKERNELAPI BOOLEAN PsGetProcessExitProcessCalled(__in PEPROCESS Process);
NTKERNELAPI NTSTATUS PsGetProcessExitStatus(__in PEPROCESS Process);
...
NTKERNELAPI PEJOB PsGetProcessJob(__in PEPROCESS Process);
NTKERNELAPI ULONG PsGetProcessSessionId(__in PEPROCESS Process);
NTKERNELAPI ULONG PsGetProcessSessionIdEx(__in PEPROCESS Process);
NTKERNELAPI PVOID PsGetProcessSectionBaseAddress(__in PEPROCESS Process);

#define PsGetProcessPcb(Process) ((PKPROCESS)(Process))

NTKERNELAPI PPEB PsGetProcessPeb(__in PEPROCESS Process);
NTKERNELAPI UCHAR PsGetProcessPriorityClass(__in PEPROCESS Process);
NTKERNELAPI HANDLE PsGetProcessWin32WindowStation(__in PEPROCESS Process);

#define PsGetCurrentProcessWin32WindowStation() PsGetProcessWin32WindowStation(PsGetCurrentProcess())

NTKERNELAPI PVOID PsGetProcessWin32Process(__in PEPROCESS Process);
NTKERNELAPI PVOID PsGetCurrentProcessWin32Process(VOID);

#if defined(_WIN64)
NTKERNELAPI PVOID PsGetProcessWow64Process(__in PEPROCESS Process);
NTKERNELAPI PVOID PsGetCurrentProcessWow64Process(VOID);
#endif

NTKERNELAPI HANDLE PsGetThreadId(__in PETHREAD Thread);
NTKERNELAPI CCHAR PsGetThreadFreezeCount(__in PETHREAD Thread);
NTKERNELAPI BOOLEAN PsGetThreadHardErrorsAreDisabled(__in PETHREAD Thread);
NTKERNELAPI PEPROCESS PsGetThreadProcess(__in PETHREAD Thread);
NTKERNELAPI PEPROCESS PsGetCurrentThreadProcess(VOID);
NTKERNELAPI HANDLE PsGetThreadProcessId(__in PETHREAD Thread);
NTKERNELAPI HANDLE PsGetCurrentThreadProcessId(VOID);
NTKERNELAPI ULONG PsGetThreadSessionId(__in PETHREAD Thread);

#define  PsGetThreadTcb(Thread) ((PKTHREAD)(Thread))

NTKERNELAPI PVOID PsGetThreadTeb(__in PETHREAD Thread);
NTKERNELAPI PVOID PsGetCurrentThreadTeb(VOID);
NTKERNELAPI PVOID PsGetThreadWin32Thread(__in PETHREAD Thread);
NTKERNELAPI PVOID PsGetCurrentThreadWin32Thread(VOID);
NTKERNELAPI PVOID PsGetCurrentThreadWin32ThreadAndEnterCriticalRegion(__out PHANDLE ProcessId);
...
NTKERNELAPI BOOLEAN PsIsSystemProcess(__in PEPROCESS Process);
NTKERNELAPI BOOLEAN PsIsThreadImpersonating (__in PETHREAD Thread);
NTSTATUS PsReferenceProcessFilePointer (IN PEPROCESS Process, OUT PVOID *pFilePointer);
NTKERNELAPI VOID PsSetJobUIRestrictionsClass(__out PEJOB Job, __in ULONG UIRestrictionsClass);
NTKERNELAPI VOID PsSetProcessPriorityClass(__out PEPROCESS Process, __in UCHAR PriorityClass);
NTKERNELAPI NTSTATUS PsSetProcessWin32Process(__in PEPROCESS Process, __in PVOID Win32Process, __in PVOID PrevWin32Process);
NTKERNELAPI VOID PsSetProcessWindowStation(__out PEPROCESS Process, __in HANDLE Win32WindowStation);
NTKERNELAPI VOID PsSetThreadHardErrorsAreDisabled(__in PETHREAD Thread, __in BOOLEAN HardErrorsAreDisabled);
NTKERNELAPI VOID PsSetThreadWin32Thread(__inout PETHREAD Thread, __in PVOID Win32Thread, __in PVOID PrevWin32Thread);
NTKERNELAPI PVOID PsGetProcessSecurityPort(__in PEPROCESS Process);
NTKERNELAPI NTSTATUS PsSetProcessSecurityPort(__out PEPROCESS Process, __in PVOID Port);
...
NTSTATUS PsEnumProcesses ( IN PROCESS_ENUM_ROUTINE CallBack, IN PVOID Context);
NTSTATUS PsEnumProcessThreads (IN PEPROCESS Process, IN THREAD_ENUM_ROUTINE CallBack, IN PVOID Context);
PEPROCESS PsGetNextProcess (IN PEPROCESS Process);
PETHREAD PsGetNextProcessThread (IN PEPROCESS Process, IN PETHREAD Thread);
VOID PsQuitNextProcess (IN PEPROCESS Process);
VOID PsQuitNextProcessThread (IN PETHREAD Thread);
PEJOB PsGetNextJob (IN PEJOB Job);
PEPROCESS PsGetNextJobProcess (IN PEJOB Job, IN PEPROCESS Process);
...
#ifndef _WIN64
NTSTATUS PsSetLdtEntries (IN ULONG Selector0, IN ULONG Entry0Low, IN ULONG Entry0Hi, IN ULONG Selector1, IN ULONG Entry1Low, IN ULONG Entry1Hi);
