...
Arguments:
    Hive - supplies a pointer to hive control structure for hive to free.
            this structure itself will NOT be freed, but everything it points to will.
*/
{
    PHMAP_DIRECTORY Dir;
    PHMAP_ENTRY     Me;
            ...
            do {
                ...
                if (Me->BinAddress & HMAP_DISCARDABLE) {
                    ...
                    if (FreeBin->Flags & FREE_HBIN_DISCARDABLE) {
                        ...
                        // Since the hive is being freed, the quota must be returned here.
...
/*
Routine Description:
    Free the memory and associated maps for the end of a hive starting at Start.
    The baseblock, hive, etc will not be touched.
Arguments:
    ...
    do {
        ...
        if (Me->BinAddress & HMAP_DISCARDABLE) {
            ...
            if (FreeBin->Flags & FREE_HBIN_DISCARDABLE) {
                ...
                // The bin has been freed, but quota is still charged.
                // Since the file will now shrink, the quota must be returned here.
                if (Me->BinAddress & HMAP_INPAGEDPOOL) {
