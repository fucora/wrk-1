...
PCM_VIEW_OF_FILE CmpAllocateCmView(IN  PCMHIVE CmHive);
...
BOOLEAN CmIsFileLoadedAsHive(PFILE_OBJECT FileObject);
...
// this controls how many views we allow per each hive (basically how many address space we allow per hive). We use this to optimize boot time.
...
Arguments:
    ...
    MoveToEnd - moves the view to the end of the LRUList after unmapping
                This is normally TRUE, unless we want to be able to iterate through the entire list and unmap views in the same time
*/
{
    ULONG           Offset;
    ULONG_PTR       Address;
    ...
    if (MapIsValid == TRUE) {
        ...
        while ((Address < AddressEnd) && (Offset < CmHive->Hive.Storage[Stable].Length)) {
            ...
            if (Me->BinAddress & HMAP_INPAGEDPOOL) {
                ...
                ASSERT((Me->CmView = NULL) == NULL);// we don't need to set it - just for debug purposes
            }
    ...
    if (MoveToEnd == TRUE) {
        RemoveEntryList(&(CmView->LRUViewList));// remove the view from the LRU list
        InsertTailList(&(CmHive->LRUViewListHead), &(CmView->LRUViewList));// add it to the end of LRU list
    }
}
...
Routine Description:
    ...
    This function is to be called from HvpGetCellPaged, every time a view is touched.
Arguments:
    Hive - Hive containing the section
    CmView - pointer to the view to operate on
*/
{
    ...
    if (IsListEmpty(&(CmView->PinViewList)) == FALSE) {
        // the view is pinned; don't mess with it as it is guaranteed that it'll be in memory until the next flush
        return;
    }
    ...
    RemoveEntryList(&(CmView->LRUViewList));// remove the view from the LRU list
    InsertHeadList(&(CmHive->LRUViewListHead), &(CmView->LRUViewList));// add it on top of LRU list
}
            ...
            if (TempCmView == NULL) {
                ...
                if ((*CmView)->UseCount != 0) {
                    ...
                    while ((*CmView)->LRUViewList.Blink != &CmHive->LRUViewListHead) {
                        *CmView = CONTAINING_RECORD((*CmView)->LRUViewList.Blink, CM_VIEW_OF_FILE, LRUViewList);
                        if ((*CmView)->UseCount == 0) {
                    ...
                    CmpUnmapCmView(CmHive, (*CmView), TRUE, TRUE);// unmap it!
                }
            } else {
...
RetryToMap:
    try {
        ...
        if (!CcMapData(CmHive->FileObject, (PLARGE_INTEGER)&SectionOffset, (*CmView)->Size, MAP_WAIT | MAP_NO_READ, (PVOID *)(&((*CmView)->Bcb)), (PVOID *)(&((*CmView)->ViewAddress)))) {
...
*/
{
    ...
    InitializeListHead(&(CmView->PinViewList));// add it to the list(s)
...
*/
{
    ...
    while (IsListEmpty(&(CmHive->PinViewListHead)) == FALSE) {
        ...
        InitializeListHead(&(CmView->PinViewList));// empty the LRUList for this view

        // update the counters
        CmHive->PinnedViews--;
        CmHive->MappedViews++;

        InsertTailList(&(CmHive->LRUViewListHead), &(CmView->LRUViewList));// add it at the tail of LRU list for this hive
    }
    ...
    while (IsListEmpty(&(CmHive->LRUViewListHead)) == FALSE) {
        ...
        CmHive->MappedViews--;// update the counter
        RemoveEntryList(&(CmView->LRUViewList));// remove the view from the LRU list
...
/*
Routine Description:
    Drops the extra reference kept on the file object (if any) and frees the name
Arguments:
    CmHive
*/
{
...
*/
{
    ...
    Status = ObReferenceObjectByHandle(FileHandle, FILE_READ_DATA | FILE_WRITE_DATA, IoFileObjectType, KernelMode, (PVOID *)FileObject, NULL);
    if (!NT_SUCCESS(Status)) {
        ...
        if (NT_SUCCESS(Status2)) {
            ...
            CmHive->FileFullPath.Buffer = ExAllocatePoolWithTag(PagedPool, (FileNameLength + 1) * sizeof(WCHAR), CM_NAME_TAG | PROTECTED_POOL);
...
*/
{
    ...
    ASSERT_VIEW_LOCK_OWNED(CmHive);// ViewLock must be held
...
*/
{
    ...
    ASSERT_VIEW_MAPPED(CmView);// We only pin mapped views
    ASSERT_VIEW_LOCK_OWNED(CmHive);
    ASSERT(CmHive->Hive.ReadOnly == FALSE);// sanity check; we shouldn't get here for a read-only hive

    SaveBcb = CmView->Bcb;// we may need this later
    ...
    if (NT_SUCCESS(Status)) {
        ...
        InitializeListHead(&(CmView->LRUViewList));// empty the LRUList for this view
        InsertTailList(&(CmHive->PinViewListHead), &(CmView->PinViewList));// add it at the tail of pinned list for this hive
...
*/
{
    ...
    CmLockHiveViews(CmHive);// Grab the viewLock, to protect the viewlist

    ASSERT_VIEW_PINNED(CmView);// We only pin mapped views
    ASSERT(CmHive->Hive.ReadOnly == FALSE);// sanity check; we shouldn't get here for a read-only hive
    ...
    if (SetClean == TRUE) {
        ...
        try {
            if (!CcMapData(CmHive->FileObject, (PLARGE_INTEGER)&FileOffset, CmView->Size, MAP_WAIT | MAP_NO_READ, (PVOID *)(&NewBcb), (PVOID *)(&NewViewAddress))) {
                Status = STATUS_CANT_WAIT;
            }
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            CcUnpinData(CmView->Bcb);// unpin old data
            ...
            for (LRUListEntry = CmHive->LRUViewListHead.Blink; LRUListEntry != &CmHive->LRUViewListHead; LRUListEntry = LRUListEntry->Blink) {
                CmView = CONTAINING_RECORD(LRUListEntry, CM_VIEW_OF_FILE, LRUViewList);
                if (CmView->UseCount == 0) {
                    // this one is free go ahead and use it !
                    // first unmap, then signal that we found it
    ...
    CmUnlockHiveViews(CmHive);// at last, release the view lock
}
...
Routine Description:
    ...
    In a perfect world this should never be called;
    If we get here, somewhere inside a cm function we took an exception and never had a chance to release all used cells.
    We fix that here, and as we hold the reglock exclusive, we are safe to do so.
...
*/
{
    ...
    while (CmHive->MappedViews >= CmMaxViewsPerHive) {
        ...
        CmpUnmapCmView(CmHive, CmCurrentView, TRUE, FALSE);// unmap it; no need to move it at the end as we shall free it anyway
        RemoveEntryList(&(CmCurrentView->LRUViewList));// remove it from LRU list
#if DBG
        // do this to signal that LRUViewList is empty.
    ...
    CmHive->UseCount = 0;// finally, fix hive use count
}
...
Routine Description:
    ...
    We're using MmPrefetchPages fast routine;
    Pages will be put in the transition state, and they'll be used by the hive load worker while mapping data
Arguments:
...
Routine Description:
    ...
    The purpose of this is to allow a retry in case CcMapData failed because of the system having to many mapped views.
...
*/
{
    ...
    while (p != &CmpHiveListHead) {
        ...
        if (CmCurrentHive < CmHive) {// we need to be the only ones operating on this list
            CmLockHiveViews(CmCurrentHive);
        } else if (CmCurrentHive == CmHive) {// we already have the mutex owned
            NOTHING;
        } else {// can't lock this one as we may deadlock with another thread racing us here
            goto Ignore;
        }
        ...
        for (NrViews = CmCurrentHive->MappedViews; NrViews; NrViews--) {
            ...
            if ((CmView->ViewAddress != 0) && (CmView->UseCount == 0)) {
                ...
                CmpUnmapCmView(CmCurrentHive, CmView, TRUE, FALSE);// unmap it without altering its position in the list
            }
...
NTSTATUS CmPrefetchHivePages(__in PUNICODE_STRING FullHivePath, __inout PREAD_LIST ReadList)
/*
Routine Description:
...
*/
{
    ...
    if (CmView && CmHive->Hive.ReleaseCellRoutine) {
        CmView->UseCount++;// up the view use count if any
    }
}
...
*/
{
    ...
    CmpReferenceHiveView(CmHive, CmView);// call the unsafe routine
...
*/
{
    ...
    CmLockHiveViews(CmHive);
    CmpDereferenceHiveView(CmHive, CmView);// call the unsafe routine
    CmUnlockHiveViews(CmHive);
}