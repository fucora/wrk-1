...
Abstract:
    This module contains parse routines for the configuration manager, particularly the registry.
*/
...
PCM_KEY_CONTROL_BLOCK CmpAddInfoAfterParseFailure(
    PHHIVE          Hive,
    HCELL_INDEX     Cell,
    PCM_KEY_NODE    Node,
...
BOOLEAN CmpGetSymbolicLink(IN PHHIVE Hive, IN OUT PUNICODE_STRING ObjectName, IN OUT PCM_KEY_CONTROL_BLOCK SymbolicKcb, IN PUNICODE_STRING RemainingName);
NTSTATUS CmpDoOpen(
    IN PHHIVE                       Hive,
    IN HCELL_INDEX                  Cell,
...
BOOLEAN CmpOKToFollowLink(IN PCMHIVE  OrigHive, IN PCMHIVE  DestHive);
NTSTATUS
...
PULONG CmpBuildAndLockKcbArray(
    IN PCM_HASH_ENTRY HashStack,
...
ULONG CmpUnLockKcbArray(IN PULONG LockedKcbs, IN ULONG  Exempt);
VOID CmpReLockKcbArray(IN PULONG LockedKcbs, IN BOOLEAN LockExclusive);
...
RetryHash:
    ...
    *TotalRemainingSubkeys = CmpComputeHashValue(HashStack, TotalSubkeys, HashKeyCopy, Current);
    // we now lock it shared as 85% of the create calls are in fact opens the lock will be acquired exclusively in CmpDoCreate/CmpCreateLinkNode

    // We only lock the registry here, in the parse routine to reduce contention on the registry lock (NO reason to wait on OB)
    if (!RegLocked) {
        CmpLockRegistry();
        RegLocked = TRUE;
    }
    ...
    // Because of the way the parse paths work, this will always be defined. 
    // (ObOpenObjectByName had to bounce off of a KeyObject or KeyRootObject to get here)
    *Hive = (*kcb)->KeyHive;
    *Cell = (*kcb)->KeyCell;

    // Look up from the cache.
    // kcb will be changed if we find a partial or exact match PCmpCacheEntry, the entry found, will be moved to the front of the Cache.
    status = CmpCacheLookup(HashStack, *TotalRemainingSubkeys, MatchRemainSubkeyLevel, kcb, Current, Hive, Cell, OuterStackArray);
    ...
    if (NT_SUCCESS(status)) {
        if (((*TotalRemainingSubkeys) == (*MatchRemainSubkeyLevel)) && (OuterStackArray[0] == 1)) {
            ...
            *LockedKcbs = CmpBuildAndLockKcbArray(HashStack, *TotalRemainingSubkeys, *MatchRemainSubkeyLevel, *kcb, OuterStackArray, TRUE);
            if (*LockedKcbs == NULL) {
...
Routine Description:
    ...
    It is invoked when the object system is given the name of an entity to create or open and a Key or KeyRoot is encountered in the path.
    In practice this means that this routine is called for all objects whose names are of the form \REGISTRY\...

    This routine will create a Key object, which is effectively an open instance to a registry key node, and return its address (for the success case.)
Arguments:
    ...
    Context - if create or hive root open, points to a CM_PARSE_CONTEXT structure, if open, is NULL.
    SecurityQos - Optional security quality of service indicator.
    Object - The address of a variable to receive the created key object, if any.
Return Value:
    The function return value is one of the following:
        a)  Success - This indicates that the function succeeded and the object parameter contains the address of the created key object.
        b)  STATUS_REPARSE - This indicates that a symbolic link key was found, and the path should be reparsed.
        c)  Error - This indicates that the file was not found or created and no file object was created.
*/
{
    ...
    while ((RemainingName->Length > 0) && (RemainingName->Buffer[(RemainingName->Length / sizeof(WCHAR)) - 1] == OBJ_NAME_PATH_SEPARATOR)) {
    ...
    } else {// keep the old behavior (open == parse without context)        
        lcontext = NULL;
    }
    ...
    if (CmAreCallbacksRegistered()) {
        ...
        if (ARGUMENT_PRESENT(lcontext)) {// NtCreateKey
    ...
    status = CmpBuildHashStackAndLookupCache(
        ParseObject,
        &kcb,
        &Current,
        &Hive,
    ...
    if (TotalRemainingSubkeys == 0) {
        ...
        // in this case (this could only happen if we called with the lpSubkey = NULL )
    ...
    // 1. This cached key is a fake key (CM_KCB_KEY_NON_EXIST), then either let it be created or return STATUS_OBJECT_NAME_NOT_FOUND.
    // 2. The cached key is not the destination and it has no subkey (CM_KCB_NO_SUBKEY).
    // 3. The cached key is not the destination and it has the first four characters of its subkeys.
    //    If the flag is CM_KCB_SUBKEY_ONE, there is only one subkey and the four char is embedded in the KCB.
    //    If the flag is CM_KCB_SUBKEY_INFO, then there is an allocation for these info.

    // We do need to lock KCB tree to protect the KCB being modified.
    // Currently there is not lock contention problem on KCBs, We can change KCB lock to a read-write lock if this becomes a problem.
    // We already have the lock on the kcb tree and we need it until we finish work on the cache table.
    if (kcb->Delete) {// kcb has been deleted while playing with the lock        
        ...
        if (MatchRemainSubkeyLevel == TotalRemainingSubkeys) {
            ...
            if (kcb->ExtFlags & CM_KCB_KEY_NON_EXIST) {
                ...
                if (ARGUMENT_PRESENT(lcontext)) {
                    ...
                    // Temporarily increase the RefCount of the ParentKcb so it's not removed while removing the fake and creating the real KCB.
            ...
            // When NoMatch is FALSE, it can we either we found a match or there is not enough information.
            // Either case, we need to continue the parse.
            TmpNodeName = Current;
            ...
            for (CmpCount = 0; CmpCount < HintCounts; CmpCount++) {
                ...
                if (NextHashKey == TempHashKey[CmpCount]) {
                    NoMatch = FALSE;// There is a match.
                    break;
                }
            }
    ...
    if (MatchRemainSubkeyLevel) {
        // Found something, update the information to start the search from the new BaseName
        if (MatchRemainSubkeyLevel == TotalSubkeys) {
            // The complete key has been found in the cache, go directly to the CmpDoOpen.
    ...
    if (kcb->Flags & KEY_SYM_LINK) {
        // The given key was a symbolic link.  Find the name of its link, and return STATUS_REPARSE to the Object Manager.
    ...
    while (TRUE) {
        ...
        if ((NextName.Length > 0) && (rc == TRUE)) {
            ...
            // Always use the information in kcb to avoid touching registry data.
            if (!(kcb->Flags & KEY_SYM_LINK)) {
                // Got a legal name component, see if we can find a sub key that actually has such a name.
                ...
                if (NextCell != HCELL_NIL) {
                    ...
                    Found:
                        // We will open the key regardless of whether the call was open or create, so step through exit portholes here.
                        if (CompleteKeyCached == TRUE) {
                            // If the key found is already cached, do not need to StepThroughExit (no kcb is created using exit node).
                            ...
                            if (Node == NULL) {// we couldn't map view for this cell                                
                                status = STATUS_INSUFFICIENT_RESOURCES;
                                break;
                            }
                        }
                        ...
                        if (status == STATUS_REPARSE) {
                            // The given key was a symbolic link. 
                            // Find the name of its link, and return STATUS_REPARSE to the Object Manager.
                    ...
                    // else
                    //   Not at end, so we'll simply iterate and consume the next component.

                    // Step through exit portholes here.
                    // This ensures that no KCB is created using the Exit node.
                    CmpStepThroughExit(Hive, Cell, Node, HiveToRelease, CellToRelease);
                    if (Node == NULL) {// we couldn't map view for this cell                        
                        status = STATUS_INSUFFICIENT_RESOURCES;
                        break;
                    ...
                    kcb = CmpCreateKeyControlBlock(Hive, Cell, Node, ParentKcb, CMP_CREATE_KCB_KCB_LOCKED, &NextName);
                    if (kcb == NULL) {
                        status = STATUS_INSUFFICIENT_RESOURCES;
                        goto FreeAndReturn;
                    ...
                    // Now we have created a kcb for the next level, the kcb in the previous level is no longer needed.
                    // Dereference the parent kcb.
                    CmpDereferenceKeyControlBlockWithLock(ParentKcb, FALSE);

                    ParentKcb = kcb;
                } else {
                    // We did not find a key matching the name, but no unexpected error occured
                    if ((Last == TRUE) && (ARGUMENT_PRESENT(lcontext))) {
                    CreateChild:
                        // Only unfound component is last one, and operation is a create, so perform the create.

                        // There are two possibilities here. 
                        // The normal one is that we are simply creating a new node.

                        // The abnormal one is that we are creating a root node that is linked to the main hive. 
                        // In this case, we must create the link. 
                        // Once the link is created, we can check to see if the root node exists, then either create it or open it as necessary.

                        // CmpCreateLinkNode creates the link, and calls back to CmpDoCreate or CmpDoOpen to create or open the root node as appropriate.

                        // either one of this will drop the reglock and reaquire it exclusive; we need not to hurt ourselves, so release all cells here
                        CmpReleasePreviousAndHookNew(NULL, HCELL_NIL, HiveToRelease, CellToRelease);

                        if (lcontext->CreateLink) {
                            status = CmpCreateLinkNode(Hive, Cell, AccessState, NextName, AccessMode, Attributes, lcontext, ParentKcb, LockedKcbs, Object);
                        } else {
                            if ((Hive == &(CmpMasterHive->Hive)) && (CmpNoMasterCreates == TRUE)) {
                                // attempting to create a cell in the master hive, and not a link, so blow out of here, since it wouldn't work anyway.
                                status = STATUS_INVALID_PARAMETER;
                                break;
                            }

                            status = CmpDoCreate(Hive, Cell, AccessState, &NextName, AccessMode, lcontext, ParentKcb, CmpParseGetOriginatingPoint(Context), Object);
                        }

                        if (status == STATUS_REPARSE) {
                            // somebody else created the key in between;
                            // let the Object Manager work for us !!!
                            // now we have the lock exclusive, so nothing can happen in between next iteration will find the key very quick
                            break;
                        }
                        lcontext->Disposition = REG_CREATED_NEW_KEY;
                        break;
                    } else {
                        // Did not find a key to match the component, and are not at the end of the path.
                        // Thus, open must fail because the whole path dosn't exist, create must fail because more than 1 component doesn't exist.

                        // We have a lookup failure here, 
                        // so having additional information about this kcb may help us not to go through all the code just to fail again.
            ...
            // Or some strange remaining name that messes up the lookup.
            CmpStepThroughExit(Hive, Cell, Node, HiveToRelease, CellToRelease);
            if (Node == NULL) {// we couldn't map view for this cell                
                status = STATUS_INSUFFICIENT_RESOURCES;
                break;
            ...
            // We have found the entire path, so we want to open it (for both Open and Create calls).
            // Hive,Cell -> the key we are supposed to open.
            status = CmpDoOpen(Hive,
                               Cell,
                               Node,
...
FreeAndReturn:
    // Now we have to free the last kcb that still has one extra reference count to protect it from being freed.
    if (LockedKcbs != NULL) {
        CmpUnLockKcbArray(LockedKcbs, CmpHashTableSize);
        if (LockedKcbs != LocalFastKcbArray) {
            ExFreePool(LockedKcbs);
...
/*
Routine Description:
    This routine parses off the next component of a registry path, returning all of the interesting state about it, including whether it's legal.
Arguments:
    ...
    NextName - supplies pointer to a unicode_string, which will be set up to point into the parse string.
    Last - supplies a pointer to a boolean - set to TRUE if this is the last component of the name being parse, FALSE otherwise.
Return Value:
    TRUE if all is well.
    FALSE if illegal name (too long component, bad character, etc.) (if false, all out parameter values are bogus.)
*/
{
    BOOLEAN rc = TRUE;
...
Arguments:
    ...
    CachedKcb - If the completekey is cached, this is the kcb for the destination. If not, this is the parent kcb.
    KeyName - Relative name (to BaseName)
    Object - The address of a variable to receive the created key object, if any.
    NeedDeref - if specified, keep reference in the fake create kcb (link case). Caller is responsible to release the fake kcb after it finishes with it.
Return Value:
    NTSTATUS
*/
{
    ...
    if (ARGUMENT_PRESENT(Context)) {
        if (Context->CreateLink) {// It's a create of some sort            
            return STATUS_ACCESS_DENIED;// The node already exists as a regular key, so it cannot be turned into a link node.
        } else if (Context->CreateOptions & REG_OPTION_CREATE_LINK) {
            return STATUS_OBJECT_NAME_COLLISION;// Attempt to create a symbolic link has hit an existing key so return an error
        } else {// Operation is an open, so set Disposition            
    ...
    if (ControlFlags&CMP_DO_OPEN_COMPLETE_KEY_CACHED) {// Check for symbolic link and caller does not want to open a link
        ASSERT_KCB_LOCKED(*CachedKcb);

        if ((*CachedKcb)->Flags & KEY_SYM_LINK && !(Attributes & OBJ_OPENLINK)) {// The complete key is cached.
            ...
            kcb = *CachedKcb;// Not a symbolic link, increase the reference Count of Kcb.
        }

        if (!CmpReferenceKeyControlBlock(kcb)) {// common path instead of repeating code
            return STATUS_INSUFFICIENT_RESOURCES;
        }
    } else {// The key is not in cache, the CachedKcb is the parentkcb of this key to be opened.            
        ...
        if (Node->Flags & KEY_SYM_LINK && !(Attributes & OBJ_OPENLINK)) {
            ...
            ASSERT_KCB_LOCKED_EXCLUSIVE(kcb);
            if (ARGUMENT_PRESENT(NeedDeref)) {
                *NeedDeref = TRUE;// caller will perform deref.
            } else {
                CmpDereferenceKeyControlBlockWithLock(kcb, FALSE);
            }
            *CachedKcb = kcb;
...
#if DBG
    if (kcb->ExtFlags & CM_KCB_KEY_NON_EXIST) {
        DbgBreakPoint();// we shouldn't fall into this
        return STATUS_OBJECT_NAME_NOT_FOUND;
    }
#endif // DBG

    if (!CmpOKToFollowLink(OriginatingHive, (PCMHIVE)Hive)) {
        status = STATUS_ACCESS_DENIED;// about to cross class of trust boundary
    } else {
        status = ObCreateObject(AccessMode, CmpKeyObjectType, NULL, AccessMode, NULL, sizeof(CM_KEY_BODY), 0, 0, Object);// Allocate the object.
    }
    ...
    } else {// Failed to create object, so undo key control block work
#if DBG
    ...
    if (BackupRestore == TRUE) {
        // this is an open to support a backup or restore operation, do the special case work
        ...
        if (AccessState->PreviouslyGrantedAccess == 0) {
            // relevant privileges not asserted/possessed, so deref (which will cause CmpDeleteKeyObject to clean up) and return an error.
        ...
        AllowAccess = ObCheckObjectAccess(*Object, AccessState, TRUE,         // Type mutex already locked
                                          AccessMode, &status);
        pbody->KeyControlBlock = (PCM_KEY_CONTROL_BLOCK)((ULONG_PTR)pbody->KeyControlBlock ^ 1);
        if (!AllowAccess) {// Access denied, so deref object, will cause CmpDeleteKeyObject to be called, it will clean up.            
...
/*
Routine Description:
    Perform the creation of a link node.
    Allocate all components, and attach to parent key.
    Calls CmpDoCreate or CmpDoOpen to create or open the root node of the hive as appropriate.

    Note that you can only create link nodes in the master hive.
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive
    Cell - supplies index of node to create child under
    Name - supplies pointer to a UNICODE string which is the name of the child to be created.
    AccessMode - Access mode of the original caller.
    Attributes - Attributes to be applied to the object.
    Context - pointer to CM_PARSE_CONTEXT structure passed through the object manager
...
*/
{
    ...
    *Object = NULL;
    ...
    // this is a create, so we need exclusive access on the registry first get the time stamp to see if somebody messed with this key
    // this might be more easier if we decide to cache the LastWriteTime in the KCB ; now it IS !!!
    if (CmIsKcbReadOnly(ParentKcb)) {// key is protected        
    ...
    if (ParentKcb->Delete) {// key was deleted in between        
        Status = STATUS_OBJECT_NAME_NOT_FOUND;
        goto Exit;
    }
    ...
    // Link nodes are always in the master hive, so their storage type is mostly irrelevant.
    ...
    if (KeyCell != HCELL_NIL) {
        ChildCell = KeyCell;// This hive already exists, so we just need to open the root node.

        // The root cell in the hive does not has the Name buffer space reserved. 
        // This is why we need to pass in the Name for creating KCB instead of using the name in the keynode.
        CellData = HvGetCell(Context->ChildHive.KeyHive, ChildCell);
        if (CellData == NULL) {// we couldn't map the bin containing this cell            
        ...
        if (TempNode == NULL) {// we couldn't map the bin containing this cell            
            HvFreeCell(Hive, LinkCell);
            Status = STATUS_INSUFFICIENT_RESOURCES;
            goto Exit;
        }
        ...
        // This is a newly created hive, so we must allocate and initialize the root node.
        Status = CmpDoCreateChild(Context->ChildHive.KeyHive,
                                  ...
                                  Object);
        if (NT_SUCCESS(Status)) {
            Context->ChildHive.KeyHive->BaseBlock->RootCell = ChildCell;// Initialize hive root cell pointer.
        }
    }

    if (NT_SUCCESS(Status)) {
        // Initialize parent and flags. 
        // Note that we do this whether the root has been created or opened, 
        // because we are not guaranteed that the link node is always the same cell in the master hive.
        ...
        HvReleaseCell(Context->ChildHive.KeyHive, ChildCell);// release the cell right here as the view is already pinned
        ...
        if (Parent == NULL) {// we couldn't map the bin containing this cell            
            HvFreeCell(Hive, LinkCell);
            Status = STATUS_INSUFFICIENT_RESOURCES;
            goto Cleanup;
        }
    ...
    Cleanup:
        if (!NT_SUCCESS(Status)) {
            ASSERT((*Object) != NULL);
            // mark the kcb as "no-delay-close" so it gets kicked out of cache when refcount goes down to 0
            KeyBody = (PCM_KEY_BODY)(*Object);
...
BOOLEAN CmpGetSymbolicLink(IN PHHIVE Hive, IN OUT PUNICODE_STRING ObjectName, IN OUT PCM_KEY_CONTROL_BLOCK SymbolicKcb, IN PUNICODE_STRING RemainingName OPTIONAL)
/*
Routine Description:
    This routine extracts the symbolic link name from a key, if it is marked as a symbolic link.
Arguments:
    Hive - Supplies the hive of the key.
    ObjectName - Supplies the current ObjectName.
                 Returns the new ObjectName.
                 If the new name is longer than the maximum length of the current ObjectName, the old buffer will be freed and a new buffer allocated.
    RemainingName - Supplies the remaining path.  If present, this will be concatenated with the symbolic link to form the new objectname.
Return Value:
            ...
            if ((ConvKey == KeyHash->ConvKey) && (TotalLevels == RealKcb->TotalLevels) && (!(RealKcb->ExtFlags & CM_KCB_KEY_NON_EXIST))) {
                ...
                if (ConstructedName) {
                    ...
                    if (ConstructedName->Length == ValueLength) {
                        ...
                        if (KcbFound) {
                            // Now the RealKcb is also pointed to by its symbolic link Kcb, Increase the reference count.
                            ...
                            if (CmpReferenceKeyControlBlock(RealKcb)) {
                                if (CmpOKToFollowLink((((PCMHIVE)(SymbolicKcb->KeyHive))->Flags&CM_CMHIVE_FLAG_UNTRUSTED) ?
                                    (PCMHIVE)(SymbolicKcb->KeyHive) :
                                                      NULL, (PCMHIVE)(RealKcb->KeyHive))) {
                                ...
                                } else {// let go of the extra ref and break                                    
...
#pragma prefast(suppress:12005, "overflow test already done above")
        ...
        *ObjectName = NewObjectName;
        ...
        if (ARGUMENT_PRESENT(RemainingName)) {
            RtlMoveMemory(&ObjectName->Buffer[(ValueLength / sizeof(WCHAR)) + 1], RemainingName->Buffer, RemainingName->Length);
...
/*
Routine Description:
    This routine parses the complete path of a request registry key and calculate the hash value at each level.
Arguments:
...
Arguments:
    ...
    MatchRemainSubkeyLevel - Number of Levels in RemaingName that matches. (0 if not found)
...
*/
{
    ...
    BaseKcb = *Kcb;
    // try shared first
    LockedKcbs = CmpBuildAndLockKcbArray(HashStack, TotalRemainingSubkeys, 0, BaseKcb, OuterStackArray, FALSE);
    if (LockedKcbs == NULL) {
        return STATUS_INSUFFICIENT_RESOURCES;
    }
                    ...
                    if (BaseKcb == ParentKcb) {
                        ...
                        if (CurrentKcb->ParentKcb->Delete || CurrentKcb->Delete) {
                            if (!LockedExclusive) {
                                ...
                                LockedKcbs = CmpBuildAndLockKcbArray(HashStack, TotalRemainingSubkeys, 0, BaseKcb, OuterStackArray, TRUE);
                                if (LockedKcbs == NULL) {
...
/*
Routine Description:
    This routine builds up further information in the cache when parse fails.  The additional information can be
    ...
    3. If lookup failed even we have index hint cached, then create a fake key so
       ...
       We need to monitor this periodly and work out a way to work around if we do create too many fake keys.
       One solution is to use hash value for index hint (We can do it in the cache only if we need to be backward comparable).
...
*/
{
    ...
    if (CreateFakeKcb && (CmpCacheOnFlag & CM_CACHE_FAKE_KEY)) {
        ...
        kcb = CmpCreateKeyControlBlock(Hive, Cell, Node, ParentKcb, CMP_CREATE_KCB_FAKE | CMP_CREATE_KCB_KCB_LOCKED, NodeName);
        if (kcb) {
            ASSERT_KCB_LOCKED_EXCLUSIVE(ParentKcb);
            CmpDereferenceKeyControlBlockWithLock(ParentKcb, FALSE);
            ParentKcb = kcb;
...
*/
{
    ...
    if (OrigHive == NULL) {
        return TRUE;// OK to follow links from trusted to anywhere
    }
    if (OrigHive == DestHive) {
        return TRUE;// OK to follow links inside the same hive
    }
    ...
    ASSERT(DestHive->Flags & CM_CMHIVE_FLAG_UNTRUSTED);// both untrusted; see if they are in the same class of trust
    ...
    while (TmpHive != (PCMHIVE)AnchorAddr) {
        TmpHive = CONTAINING_RECORD(TmpHive, CMHIVE, TrustClassEntry);
        if (TmpHive == OrigHive) {
        ...
        TmpHive = (PCMHIVE)(TmpHive->TrustClassEntry.Flink);// skip to the next element
    }
    ...
    */
{
    ...
    if ((TotalLevels == RemainingLevel) && (kcb->ParentKcb != NULL)) {
        Count++;// need to lock the parent also
    }
    ...
    if (Current == 1) {
        ...
        ASSERT(Current == 0);
        InputArray[Current++] = HashIndex;// simply store it
    }
