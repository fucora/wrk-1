...
Abstract:
    ...
    This routine is intended to be called at system shutdown in order to detect memory leaks.
    It is supposed to free all registry data that is not freed by CmShutdownSystem.
*/
...
*/
{
    ...
    while (IsListEmpty(&CmpHiveListHead) == FALSE) {
        ...
        CmpFreeMutex(CmHive->ViewLock);// free the hive lock  and view lock
#if DBG
        ...
        CmpDestroySecurityCache(CmHive);// free security cache
        HvFreeHive(&(CmHive->Hive));// free the hv level structure
        CmpFree(CmHive, sizeof(CMHIVE));// free the cm level structure
    }
    ...
    // should be cleaned up (freed) at the last handle closure time the related handles are closed
