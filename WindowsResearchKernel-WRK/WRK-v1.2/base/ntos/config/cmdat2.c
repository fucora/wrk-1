...
Abstract:
    This module contains data strings that describes the registry space and that are exported to the rest of the system.
*/
...
SIZE_T  CmpGlobalQuotaUsed = 0;// GQ actually in use

BOOLEAN CmpProfileLoaded = FALSE;// State flag to remember when to turn it on
...
// when a local hive cannot be loaded, set this to it's index and the load hive worker thread responsible for it will be held of
