...
Abstract:
    ...
    The worker thread (actually an executive worker thread is used) is required for operations that must take place in the context of the system process.
    (particularly file I/O)
*/
...
// LAZY_FLUSH_INTERVAL_IN_SECONDS controls how many seconds will elapse between when the hive is marked dirty and when the lazy flush worker thread is queued to write the data to disk.
...
// LAZY_FLUSH_TIMEOUT_IN_SECONDS controls how long the lazy flush worker thread will wait for the registry lock before giving up and queueing the lazy flush timer again.
...
VOID CmpEnableLazyFlushDpcRoutine(IN PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2);
...
*/
{
    ...
    oldFlag = IoSetThreadHardErrorMode(FALSE); // disable hard error popups, to workaround ObAttachProcessStack
    ASSERT_CM_LOCK_OWNED_EXCLUSIVE_OR_HIVE_LOADING(CmHive);// Close the files associated with this hive.
    ...
    for (i = 0; i < HFILE_TYPE_MAX; i++) {
        if (CmHive->FileHandles[i] != NULL) {
            ...
            if (i == HFILE_TYPE_PRIMARY) {
                if (NT_SUCCESS(ZwQueryInformationFile(CmHive->FileHandles[i], &IoStatusBlock, &BasicInfo, sizeof(BasicInfo), FileBasicInformation))) {
                    ...
                    ZwSetInformationFile(CmHive->FileHandles[i], &IoStatusBlock, &BasicInfo, sizeof(BasicInfo), FileBasicInformation);
                }
...
*/
{
    ...
    KeInitializeDpc(&CmpEnableLazyFlushDpc, CmpEnableLazyFlushDpcRoutine, NULL);
    KeInitializeTimer(&CmpEnableLazyFlushTimer);
    DueTime.QuadPart = Int32x32To64(ENABLE_LAZY_FLUSH_INTERVAL_IN_SECONDS, -SECOND_MULT);
...
*/
{
    ...
    Status = ZwSetInformationFile(Handle, &IoStatusBlock, (PVOID)RenameInfo, sizeof(FILE_RENAME_INFORMATION) + NewName->Length, FileRenameInformation);
...
*/
{
    ...
    Status = CmpInitHiveFromFile(FileName, 0, NewHive, Allocate, CheckFlags);
    // NT Servers will return STATUS_ACCESS_DENIED. Netware 3.1x
    // servers could return any of the other error codes if the GUEST account is disabled.
    if (((Status == STATUS_ACCESS_DENIED) ||
        ...
        // Impersonate the caller and try it again.  This lets us open hives on a remote machine.
...
/*
Routine Description:
    This routine resets the registry timer to go off at a specified interval in the future (LAZY_FLUSH_INTERVAL_IN_SECONDS).
*/
{
    LARGE_INTEGER DueTime;
    ...
    if ((!CmpNoWrite) && (!CmpHoldLazyFlush)) {
        DueTime.QuadPart = Int32x32To64(CmpLazyFlushIntervalInSeconds, -SECOND_MULT);
...
/*
Routine Description:
    This is the DPC routine triggered by the lazy flush timer.
    All it does is queue a work item to an executive worker thread.
    The work item will do the actual lazy flush to disk.
Arguments:
...
VOID CmSetLazyFlushState(__in BOOLEAN Enable)
/*
Routine Description:
    Enables/Disables the lazy flusher; Designed for the standby/resume case,
    where we we don't want the lazy flush to fire off, blocking registry writers until the disk wakes up.
Arguments:
