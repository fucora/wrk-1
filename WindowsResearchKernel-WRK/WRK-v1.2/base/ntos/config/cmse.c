...
PCM_KEY_SECURITY CmpGetKeySecurity(IN PHHIVE Hive, IN PCM_KEY_NODE Key, OUT PHCELL_INDEX SecurityCell);
...
// This macro takes a PSECURITY_DESCRIPTOR and returns the size of the hive cell required to contain the entire security descriptor.
...
*/
{
    ...
    // This is a request from the base test team; no dbg should be hit on the free builds at the client; after RC2 is shipped we should enable this on free builds too.
#if DBG
    try {
        DbgBreakPoint();
...
NTSTATUS CmpAssignSecurityDescriptorWrapper(IN PVOID Object, IN OUT PSECURITY_DESCRIPTOR SecurityDescriptor)
{
    ...
    if (TempNode == NULL) {
        return STATUS_INSUFFICIENT_RESOURCES;// we couldn't map the bin containing this cell
    }

    try {
        // Set the SecurityDescriptor field in the object's header to NULL.
        // This indicates that our security method needs to be called for any security descriptor operations.
        ...
        if (NT_SUCCESS(Status)) {
            CmpAssignSecurityToKcb(kcb, TempNode->Security, TRUE);// Security has been changed, update the cache.
        }
...
/*
Routine Description:
    This is the security method for registry objects.
    It is responsible for retrieving, setting, and deleting the security descriptor of a registry object.
    It is not used to assign the original security descriptor to an object (use SeAssignSecurity for that purpose).

    IT IS ASSUMED THAT THE OBJECT MANAGER HAS ALREADY DONE THE ACCESS VALIDATIONS NECESSARY TO ALLOW THE REQUESTED OPERATIONS TO BE PERFORMED.
Arguments:

    Object - Supplies a pointer to the object being used.

    OperationCode - Indicates if the operation is for setting, querying, or deleting the object's security descriptor.

    SecurityInformation - Indicates which security information is being queried or set.  This argument is ignored for the delete operation.

    SecurityDescriptor - The meaning of this parameter depends on the OperationCode:

        QuerySecurityDescriptor - For the query operation this supplies the buffer to copy the descriptor into.
            The security descriptor is assumed to have been probed up to the size passed in in Length.
            Since it still points into user space, it must always be accessed in a try clause in case it should suddenly disappear.

        SetSecurityDescriptor - For a set operation this supplies the security descriptor to copy into the object.
            The security descriptor must be captured before this routine is called.

        DeleteSecurityDescriptor - It is ignored when deleting a security descriptor.

        AssignSecurityDescriptor - For assign operations this is the security descriptor that will be assigned to the object.
            It is assumed to be in kernel space, and is therefore not probed or captured.

    CapturedLength - For the query operation this specifies the length, in bytes, of the security descriptor buffer, and upon return contains
        the number of bytes needed to store the descriptor.
    ...
    ObjectsSecurityDescriptor - For the Set operation this supplies the address of a pointer to the object's current security descriptor.
        This routine will either modify the security descriptor in place or deallocate/allocate a new security descriptor and use this variable to indicate its new location.
        For the query operation it simply supplies the security descriptor being queried.

    PoolType - For the set operation this specifies the pool type to use if a new security descriptor needs to be allocated.
        It is ignored in the query and delete operation.

    GenericMapping - Passed only for the set operation, this argument provides the mapping of generic to specific/standard access types for the object being accessed.
        This mapping structure is expected to be safe to access (i.e., captured if necessary) prior to be passed to this routine.

Return Value:
    NTSTATUS - STATUS_SUCCESS if the operation is successful and an appropriate error status otherwise.
*/
{
    PCM_KEY_CONTROL_BLOCK   kcb;
    NTSTATUS                Status = STATUS_UNSUCCESSFUL;
    ...
    ASSERT((OperationCode == SetSecurityDescriptor) || (OperationCode == QuerySecurityDescriptor) || (OperationCode == AssignSecurityDescriptor) || (OperationCode == DeleteSecurityDescriptor));

    kcb = ((PCM_KEY_BODY)Object)->KeyControlBlock;

    // Lock hive for shared or exclusive, depending on what we need to do.
    CmpLockRegistry();
    if (OperationCode == QuerySecurityDescriptor) {
        ...
        if ((ULONG_PTR)kcb & 1) {
            ...
            CmpLockKCBShared(kcb);// serialize access to this key.
        }
    } else {
    ...
    if (kcb->Delete) {
        // Key has been deleted, performing security operations on it is Not Allowed.
        if (UnlockKcb) {
            CmpUnlockKCB(kcb);
        }
        CmpUnlockRegistry();
    ...
    if (OperationCode != QuerySecurityDescriptor) {
        CmpLockHiveFlusherShared((PCMHIVE)kcb->KeyHive);// no flush from this point on        
        CmLockHiveSecurityExclusive((PCMHIVE)kcb->KeyHive);// we will be changing the security for this hive.
        UnlockSecurity = TRUE;
    }

    try {
        //  This routine simply cases off of the operation code to decide which support routine to call
        switch (OperationCode) {
        case SetSecurityDescriptor:
            //  check the rest of our input and call the set security method
            ASSERT((PoolType == PagedPool) || (PoolType == NonPagedPool));
            ASSERT_KCB_LOCKED(kcb);
            Status = CmpSetSecurityDescriptorInfo(kcb, SecurityInformation, SecurityDescriptor, ObjectsSecurityDescriptor, PoolType, GenericMapping);

            // this is the one and only path on which a user could change a security descriptor, therefore, report such changes for notification here.
            if (NT_SUCCESS(Status)) {
                ASSERT(UnlockSecurity);
                CmUnlockHiveSecurity((PCMHIVE)kcb->KeyHive);
                UnlockSecurity = FALSE;

                CmpReportNotify(kcb, kcb->KeyHive, kcb->KeyCell, REG_NOTIFY_CHANGE_ATTRIBUTES | REG_NOTIFY_CHANGE_SECURITY);
            }

            break;
        case QuerySecurityDescriptor:
            //  check the rest of our input and call the default query security method
            ASSERT(CapturedLength != NULL);
            Status = CmpQuerySecurityDescriptorInfo(kcb, SecurityInformation, SecurityDescriptor, CapturedLength, ObjectsSecurityDescriptor);
            break;
        case DeleteSecurityDescriptor:
            // Nobody should ever call the delete method.
            // When the key is freed, the security descriptor associated with it is explicitly freed (CmpFreeSecurityDescriptor)
            ASSERT(FALSE);
            break;
        case AssignSecurityDescriptor:
            ...
            break;
        default:
            //  Bugcheck on any other operation code,  We won't get here if the earlier asserts are still checked.
...
/*
Routine Description:
    This routine will set a node's security descriptor.
    The input security descriptor must be previously captured.
Arguments:
    Key - Supplies a pointer to the KEY_CONTROL_BLOCK for the node whose security descriptor will be set.
    SecurityInformation - Indicates which security information is to be applied to the object.
        The value(s) to be assigned are passed in the SecurityDescriptor parameter.
...
*/
{
    ...
    // Map in the hive cell for the security descriptor before we make the call to SeSetSecurityDescriptorInfo.
    // This prevents us from changing its security descriptor and then being unable to bring the hive cell into memory for updating.
    Security = CmpGetKeySecurity(Key->KeyHive, Node, &SecurityCell);
    if (Security == NULL) {// couldn't map view inside        
    ...
    // SeSetSecurityDescriptorInfo takes a pointer to the original descriptor.
    // This pointer is not freed, but a new pointer will be returned.
    DescriptorCopy = &Security->Descriptor;
    Status = SeSetSecurityDescriptorInfo(NULL, SecurityInformation, ModificationDescriptor, &DescriptorCopy, PoolType, GenericMapping);
    if (!NT_SUCCESS(Status)) {
        goto ErrorExit;
    }

    // Set Security operation succeeded, so we update the security descriptor in the hive.
    ...
    if (CmpFindMatchingDescriptorCell((PCMHIVE)Hive, DescriptorCopy, Type, &MatchSecurityCell, &CachedSecurity)) {
        ...
        if (MatchSecurityCell == SecurityCell) {
            ExFreePool(DescriptorCopy);// what we want to set is already here, so bail out

            // Update the LastWriteTime of the key.
            KeQuerySystemTime(&SystemTime);
            Node->LastWriteTime = SystemTime;

            Key->KcbLastWriteTime = SystemTime;// update the time in kcb too, to keep the cache in sync
            ...
            if (Security->ReferenceCount == 1) {
                ...
                if (!(HvMarkCellDirty(Hive, Security->Flink, FALSE) && HvMarkCellDirty(Hive, Security->Blink, FALSE))) {
                    ExFreePool(DescriptorCopy);
                    Status = STATUS_NO_LOG_SPACE;
                    goto ErrorExit;
                }
                ...
                Security->ReferenceCount -= 1;// Just decrement the count on the old security cell
            }
        ...
        if (Security->ReferenceCount > 1) {
            // We can't change the existing security cell, since it is shared by multiple keys.
            // Allocate a new cell and decrement the existing one's reference count.
            ...
            if (NewSecurity == NULL) {// we couldn't map the bin containing this cell                
                ExFreePool(DescriptorCopy);
                Status = STATUS_INSUFFICIENT_RESOURCES;
                goto ErrorExit;
            }
            ...
            if (FlinkSecurity == NULL) {// we couldn't map the bin containing this cell                
                ExFreePool(DescriptorCopy);
                Status = STATUS_INSUFFICIENT_RESOURCES;
                goto ErrorExit;
            }
            ...
            RtlCopyMemory(&(Security->Descriptor), DescriptorCopy, DescriptorLength);// copy the descriptor
            ...
            if (!NT_SUCCESS(CmpAddSecurityCellToCache((PCMHIVE)Key->KeyHive, NewCell, FALSE, NULL))) {
                // we couldn't map the bin containing this cell this shouldn't happen as we just allocated (marked dirty) the cell
                ASSERT(FALSE);
                ExFreePool(DescriptorCopy);
                Status = STATUS_INSUFFICIENT_RESOURCES;
                goto ErrorExit;
            ...
            Node->Security = NewCell;// Update the pointer in the node cell.
        } else {
            // when this is FALSE, the new cell is ADDED to cache;
            // Otherwise (the cell index and size did not change), the new sd is copied over the one in cache
            BOOLEAN UpdateCache;

            if (DescriptorLength != Security->DescriptorLength) {
                // The security descriptor's size has changed, and it is not shared by any other cells, so reallocate the cell.
                if (!(HvMarkCellDirty(Key->KeyHive, Security->Flink, FALSE) && HvMarkCellDirty(Key->KeyHive, Security->Blink, FALSE))) {
                    ExFreePool(DescriptorCopy);
                    Status = STATUS_INSUFFICIENT_RESOURCES;
                    goto ErrorExit;
                }
                ...
                Node->Security = SecurityCell;// Update the Node's security data.
                ...
                // Size hasn't changed, and it's not shared by any other cells, so we can just write the new bits over the old bits.

                // new bits should be copied over the cached security descriptor too, to keep cache consistency
            ...
            if (UpdateCache == TRUE) {
                ...
                InsertTailList(&(((PCMHIVE)Hive)->SecurityHash[CachedSecurity->ConvKey % CmpSecHashTableSize]), &(CachedSecurity->List));
            } else {
...
/*
Routine Description:
    This routine assigns the given security descriptor to the specified node in the configuration tree.
Arguments:
    Hive - Supplies a pointer to the Hive for the node whose security descriptor will be assigned.
    Cell - Supplies the HCELL_INDEX of the node whose security descriptor will be assigned.
    Node - Supplies a pointer to the node whose security descriptor will be assigned.
    SecurityDescriptor - Supplies a pointer to the security descriptor to be assigned to the node.
    PoolType - Supplies the type of pool the SecurityDescriptor was a allocated from.
Return Value:
    NTSTATUS - STATUS_SUCCESS if successful and an appropriate error value otherwise
*/
{
    ...
    // This is a CreateKey, so the registry node has just been created and the security descriptor we have been passed needs to be associated
    // with the new registry node and inserted into the hive.

    // Try to find an existing security descriptor that matches this one.
    // If successful, then we don't need to allocate a new cell, we can just point to the existing one and increment its reference count.
    ...
    if (!CmpFindMatchingDescriptorCell((PCMHIVE)Hive, SecurityDescriptor, Type, &SecurityCell, NULL)) {
        ...
        if (Security == NULL) {
            // we couldn't map the bin containing this cell this shouldn't happen as we just allocated this cell
            // (i.e. it should be PINNED into memory at this point)
            ASSERT(FALSE);
            HvFreeCell(Hive, SecurityCell);
            return STATUS_INSUFFICIENT_RESOURCES;
        ...
        HvReleaseCell(Hive, SecurityCell);// release the cell right here as the view is pinned        
        DescriptorLength = RtlLengthSecurityDescriptor(SecurityDescriptor);// Initialize the security cell
        ...
        // Insert the new security descriptor into the list of security cells; takes care of cache too
        if (!CmpInsertSecurityCellList(Hive, Cell, SecurityCell)) {
            HvFreeCell(Hive, SecurityCell);
            return STATUS_NO_LOG_SPACE;
        }
    } else {
        // Found identical descriptor already existing.  Map it in and increment its reference count.
        if (!HvMarkCellDirty(Hive, SecurityCell, FALSE)) {
            return STATUS_NO_LOG_SPACE;
        }
        Security = (PCM_KEY_SECURITY)HvGetCell(Hive, SecurityCell);
        if (Security == NULL) {
            // we couldn't map the bin containing this cell this shouldn't happen as we just marked the cell dirty (dirty means PIN !)
            ASSERT(FALSE);
            return STATUS_INSUFFICIENT_RESOURCES;
        }

        HvReleaseCell(Hive, SecurityCell);// release the cell right here as the cell is dirty
        Security->ReferenceCount += 1;
    }

    Node->Security = SecurityCell;// Initialize the reference in the node cell
...
/*
Routine Description:
    This routine will extract the desired information from the passed security descriptor and return the information in the passed buffer as a security descriptor in absolute format.
Arguments:
    Key - Supplies a pointer to the CM_KEY_REFERENCE for the node whose security descriptor will be deleted.
...
/*
Routine Description:
    This routine checks to see if we are allowed to create a sub-key in the given key, and performs auditing as appropriate.
Arguments:
    RelativeName - Supplies the relative name of the key being created.
    Descriptor - Supplies the security descriptor of the key in which the sub-key is to be created.
    ...
    AdditionalAccess - access rights in addition to KEY_CREATE_SUB_KEY that are required.  (e.g. KEY_CREATE_LINK)
...
/*
Routine Description:
    Check whether the subject process/thread/user specified by the security data in the NotifyBlock has required access to the key specified by Hive.Cell.
Arguments:
    NotifyBlock - pointer to structure that describes the notify operation, including the identity of the subject that opened the notify.
    Hive - Supplies pointer to hive containing Node.
    Node - Supplies pointer to key of interest.
Return Value:
    TRUE if RequiredAccess is in fact possessed by the subject, else FALSE.
Note:
...
*/
{
    ...
    if (Security == NULL) {// we couldn't map the bin containing this cell        
        return NULL;
    }
    ASSERT_SECURITY(Security);
...
/*
Routine Description:
    This routine allocates and initializes the default security descriptor for a system-created registry key.
    The caller is responsible for freeing the allocated security descriptor when he is done with it.
Return Value:
...
*/
{
    ...
    *(RtlSubAuthoritySid(AdminSid, 1)) = DOMAIN_ALIAS_RID_ADMINS;
    ...
    AceLength = (SeLengthSid(WorldSid) - sizeof(ULONG) + sizeof(ACCESS_ALLOWED_ACE)) + (SeLengthSid(RestrictedSid) - sizeof(ULONG) + sizeof(ACCESS_ALLOWED_ACE))
        + (SeLengthSid(SystemSid) - sizeof(ULONG) + sizeof(ACCESS_ALLOWED_ACE)) + (SeLengthSid(AdminSid) - sizeof(ULONG) + sizeof(ACCESS_ALLOWED_ACE));
    ...
    // Allocate enough space to hold both the security descriptor and the ACL.
    // This allows us to free the whole thing at once when we are done with it.
...
/*
Routine Description:
    Frees the security descriptor associated with a particular node.
    This can only happen when the node is actually being deleted from the registry.
...
*/
{
    ...
    if (Security->u.KeySecurity.ReferenceCount == 1) {
        // This is the only cell that references this security descriptor, so it is ok to free it now.
        CmpRemoveSecurityCellList(Hive, SecurityCell);
        HvFreeCell(Hive, SecurityCell);
        CmKdPrintEx((DPFLTR_CONFIG_ID, CML_SEC, "CmpFreeSecurityDescriptor: freeing security cell\n"));
    } else {
        // More than one node references this security descriptor, so just decrement the reference count.
    ...
    Node->u.KeyNode.Security = HCELL_NIL;// Zero out the pointer to the security descriptdr in the main cell
    // release the cells
    HvReleaseCell(Hive, Cell);
...
/*
Routine Description:
    Inserts a newly-created security cell into the per-hive linked list of security cells.
    NOTE:   Assumes that NodeCell and SecurityCell have already been marked dirty.
Arguments:
...
*/
{
    ...
    CM_PAGED_CODE();
    // If the new cell's storage type is Volatile, simply make it the anchor of it's own list.
    //  (Volatile security entries will disappear at reboot, restore, etc, so we don't need the list to hunt them down at those times.)

    // Else, the storage type is Stable.
    //   Map in the node that owns the new security cell.
    //   If it is a root cell, then we are creating the hive for the first time, so this is the only security cell in the list.
    //   If it is not a root cell, then we simply find its parent's security cell and stick the new security cell into the list immediately after it.

    ASSERT_HIVE_SECURITY_LOCK_OWNED((PCMHIVE)Hive);// we have the lock exclusive or nobody is operating inside this hive

    Cell = (PCM_KEY_SECURITY)HvGetCell(Hive, SecurityCell);
    if (Cell == NULL) {// we couldn't map the bin containing this cell        
        return FALSE;
    }
    ...
    if (HvGetCellType(SecurityCell) == Volatile) {
        ...
        if (Node == NULL) {// we couldn't map the bin containing this cell            
            goto ErrorExit;
        }
        ...
        if (Node->Flags & KEY_HIVE_ENTRY) {
            // This must be the hive creation, so this cell becomes the anchor for the list.
            ...
            // Map in the node's parent's security cell, so we can hook into the list there.
            ParentNode = (PCM_KEY_NODE)HvGetCell(Hive, Node->Parent);
            if (ParentNode == NULL) {// we couldn't map the bin containing this cell                
                goto ErrorExit;
            }
            ...
            if (BlinkCell == NULL) {// we couldn't map the bin containing this cell                
                goto ErrorExit;
            }
            ...
            if (FlinkCell == NULL) {// we couldn't map the bin containing this cell                
                goto ErrorExit;
            }
            ...
            if (!(HvMarkCellDirty(Hive, ParentNode->Security, FALSE) && HvMarkCellDirty(Hive, BlinkCell->Flink, FALSE))) {
                goto ErrorExit;
            }
...
Arguments:
    Hive - Supplies a pointer to the hive control structure
    SecurityCell - Supplies the cell index of the security cell to be removed
*/
{
    PCM_KEY_SECURITY FlinkCell;
    PCM_KEY_SECURITY BlinkCell;
    ...
    Cell = (PCM_KEY_SECURITY)HvGetCell(Hive, SecurityCell);
    if (Cell == NULL) {// we couldn't map the bin containing one of these cells        
        return;
    }

    FlinkCell = (PCM_KEY_SECURITY)HvGetCell(Hive, Cell->Flink);
    if (FlinkCell == NULL) {// we couldn't map the bin containing one of these cells        
        HvReleaseCell(Hive, SecurityCell);
        return;
    }

    BlinkCell = (PCM_KEY_SECURITY)HvGetCell(Hive, Cell->Blink);
    if (BlinkCell == NULL) {// we couldn't map the bin containing one of these cells        
        HvReleaseCell(Hive, SecurityCell);
    ...
    CmpRemoveFromSecurityCache((PCMHIVE)Hive, SecurityCell);// finally, remove the security cell from cache, as it'll be freed
...
NTSTATUS CmpCheckKeyAccess(IN PHHIVE Hive, IN HCELL_INDEX NodeCell, IN KPROCESSOR_MODE PreviousMode, IN ACCESS_MASK DesiredAccess)
/*
Routine Description:
    Checks if the specified access is granted on this key by looking at the hive storage.
    SD as stored in the key needs to be converted to relative first.
...
*/
{
    ...
    while (StackIndex >= 0) {
        ...
        if (CheckStack[StackIndex].ChildIndex < (Node->SubKeyCounts[Stable] + Node->SubKeyCounts[Volatile])) {
            ...
            CheckStack[StackIndex].ChildIndex++;// next iteration will check the next child
