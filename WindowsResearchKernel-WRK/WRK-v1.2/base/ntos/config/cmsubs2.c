...
Abstract:
    ...
    The routines in this module are independent enough to be linked into any other program.
    The routines in cmsubs.c are not.
*/
...
Routine Description:
    ...
    Works only for the information cached into kcb. I.e. KeyBasicInformation and KeyCachedInfo
Arguments:
...
*/
{
    ...
    // we cannot afford to return the kcb NameBlock as the key name for KeyBasicInformation as there are lots of callers expecting
...
*/
{
        ...
        if (Length < minimumlength) {
            *status = STATUS_BUFFER_TOO_SMALL;
            ...
            if (pcell->u.KeyValue.Flags & VALUE_COMP_NAME) {
                CmpCopyCompressedName(pbuffer->KeyValueBasicInformation.Name, requiredlength, pcell->u.KeyValue.Name, pcell->u.KeyValue.NameLength);
            } else {
                RtlCopyMemory(&(pbuffer->KeyValueBasicInformation.Name[0]), &(pcell->u.KeyValue.Name[0]), requiredlength);
            }
        }
    ...
    case KeyValueFullInformationAlign64:
        ...
        if (realsize > 0) {
            ...
            // Some clients will have passed in a structure that they "know" will be exactly the right size.
            // The fact that alignment has changed on NT64 may cause these clients to have problems.

            // The solution is to fix the client, but print out some debug spew here if it looks like this is the case.
            // This problem isn't particularly easy to spot from the client end.
            if ((KeyValueInformationClass == KeyValueFullInformation) &&
                (Length != minimumlength) &&
                (requiredlength > Length) &&
                ((requiredlength - Length) <= (ALIGN_OFFSET64(base) - ALIGN_OFFSET(base)))) {
        ...
        *status = STATUS_SUCCESS;
        if (Length < minimumlength) {
            *status = STATUS_BUFFER_TOO_SMALL;
            ...
            if (pcell->u.KeyValue.Flags & VALUE_COMP_NAME) {
                CmpCopyCompressedName(pbuffer->KeyValueFullInformation.Name, requiredlength, pcell->u.KeyValue.Name, pcell->u.KeyValue.NameLength);
            } else {
                RtlCopyMemory(&(pbuffer->KeyValueFullInformation.Name[0]), &(pcell->u.KeyValue.Name[0]), requiredlength);
            }
        ...
        if (Length < minimumlength) {
            *status = STATUS_BUFFER_TOO_SMALL;
            ...
            if (realsize > 0) {
                ...
                if (datapointer != NULL) {
                    try {
                        RtlCopyMemory((PUCHAR)&(pbuffer->KeyValuePartialInformation.Data[0]), datapointer, requiredlength);
                    } finally{
                        if (BufferAllocated == TRUE) {
                            ExFreePool(datapointer);
                        }
        ...
        if (Length < minimumlength) {
            *status = STATUS_BUFFER_TOO_SMALL;
            ...
            if (realsize > 0) {
                ...
                if (datapointer != NULL) {
                    try {
                        RtlCopyMemory((PUCHAR)&(pbuffer->KeyValuePartialInformationAlign64.Data[0]), datapointer, requiredlength);
                    } finally{
                        if (BufferAllocated == TRUE) {
                            ExFreePool(datapointer);
                        }
