...
VOID CmpReportNotifyHelper(PCM_KEY_CONTROL_BLOCK KeyControlBlock, IN PHHIVE SearchHive, IN PHHIVE Hive, IN HCELL_INDEX Cell, IN ULONG Filter);
...
/*
Routine Description:
    This routine is called when a notifiable event occurs.
    It will apply CmpReportNotifyHelper to the hive the event occured in, and the master hive if different.
Arguments:
...
*/
{
    ...
    // If the operation was create or delete, treat it as a change to the parent.
...
*/
{
    ...
    if (IsListEmpty(&(NotifyBlock->PostList)) == FALSE) {
        // check if it is a kernel notify. Look at the first post block to see that.
        // If is a kernel post-block, then all posts in the list should be kernel notifies
...
#if DBG
            // DEBUG only code: All post blocks should be of the same type (kernel/user)
...
/*
Routine Description:
    Scan the list of active notifies for the specified hive.
    For any with scope including KeyControlBlock and filter matching Filter, and with proper security access, post the notify.
Arguments:
    Name - canonical path name (as in a key control block) of the key at which the event occured.  (This is the name for reporting purposes.)
            ...
            if (kcb == NotifyBlock->KeyControlBlock) {
                ...
                if ((NotifyBlock->Filter & Filter) && ((NotifyBlock->WatchTree == TRUE) || (Cell == kcb->KeyCell))) {
                    ...
                    if (CmpNotifyTriggerCheck(NotifyBlock, Hive, Node)) {
                        // Notify block has KEY_NOTIFY access to the node the event occured at.
                        // It is relevant.  Therefore, it gets to see this event.
                        // Post and be done.

                        // we specify that we want no key body dereferenciation during the CmpPostNotify call.
                        // This is to prevent the deletion of the current notify block
                        CmpPostNotify(NotifyBlock, NULL, Filter, STATUS_NOTIFY_ENUM_DIR, FALSE, &DelayedDeref
#if DBG
                                      , (PCMHIVE)CmSearchHive
#endif
                        );
...
/*
Routine Description:
    Actually report the notify event by signaling events, enqueing APCs, and so forth.
    When Status is STATUS_NOTIFY_CLEANUP:
      - if the post block is a slave one, just cancel it.
      - if the post block is a master one, cancel all slave post blocks and trigger event on the master block.
Comments:
    This routine is using a "delayed dereferencing" technique to prevent deadlocks that may appear when a keybody is dereferenced while holding the post block lock.
    As for this, a list with keybodies that have to be dereferenced is constructed while walking the list of postblocks attached to the current notify block and the related (slave or master) post blocks.
    The list is built by tricking postblocks. For all postblock about to be freed the PostKeyBody member is added to the local list and then set to NULL on the postblock.
    This will avoid the key body dereferencing in CmpFreePostBlock.
    Instead, after the postblock lock is released, the local list is iterated and the keybodies are dereferenced and the storage for associated CM_POST_KEY_BODY objects is freed.
Arguments:
    NotifyBlock - pointer to structure that describes the notify operation.  (Where to post to)
    Name - name of key at which event occurred.
    Filter - nature of event
    Status - completion status to report
    ExternalKeyDeref - this parameter (when not NULL) specifies that the caller doesn't want any keybody to be dereferenced while in this routine
*/
{
    PCM_POST_BLOCK      PostBlock;
    ...
    while (IsListEmpty(&(NotifyBlock->PostList)) == FALSE) {
        ...
        if ((Status == STATUS_NOTIFY_CLEANUP) && !IsMasterPostBlock(PostBlock)) {
            ...
            if (PostBlock->NotifyType != PostSynchronous) {
                CmpAddToDelayedDeref(PostBlock, DelayedDeref);// add to the deref list and clean the post block                
                CmpFreePostBlock(PostBlock);// Front-end routine will do self cleanup for synchronous notifications
            }
        ...
        // Doing that will ensure the right memory deallocation when the master (from now on this block) will be freed.
        if (!IsMasterPostBlock(PostBlock)) {
            ...
            do {
                ...
                ClearMasterPostBlockFlag(SlavePostBlock);// reset the "master flag" if set
            } while (SlavePostBlock != PostBlock);

            SetMasterPostBlockFlag(PostBlock);// Make this post block the master one
        }
        ...
        case PostSynchronous:
            // This is a SYNC notify call. 
            // There will be no user event, and no user apc routine. 
            // Quick exit here, just fill in the Status and poke the event.

            // Holder of the systemevent will wake up and free the postblock. 
            // If we free it here, we get a race & bugcheck.
        ...
        case PostAsyncKernel:
            ...
            CmpFreePostBlock(PostBlock);// it was already added to delayed deref.
            break;
        }
    }
    ...
    // At this point we have a list of keybody elements that have to be dereferenciated and the associated storage for the covering objects freed.
    // The keybodies in this list have only one reference count on them (they were referenced only in NtNotifyChangeMultipleKeys), dereferencing them here should free the object
    if (ARGUMENT_PRESENT(ExternalKeyDeref)) {
        // do nothing; the caller wants to handle the dereferenciation by himself!
    } else {
        CmpDelayedDerefKeys(DelayedDeref);// dereferenciate all keybodies in the delayed list
    }
}
...
Routine Description:
    ...
    It is called for all notifies, regardless of what form of notification the caller requested.

    We compute the postblock address from the apc object address.
    IoStatus is set.
    SystemEvent and UserEvent will be signaled as appropriate.
    If the user requested an APC, then NormalRoutine will be set at entry and executed when we exit.
    The PostBlock is freed here.
Arguments:
    Apc - pointer to apc object
    NormalRoutine - Will be called when we return
    NormalContext - will be 1st argument to normal routine, ApcContext passed in when NtNotifyChangeKey was called
    SystemArgument1 -  IN: Status value for IoStatusBlock
                      OUT: Ptr to IoStatusBlock (2nd arg to user apc routine)
    SystemArgument2 - Pointer to the PostBlock
*/
{
    ...
    // IMPLEMENTATION NOTE:
    //      If we ever want to actually implement the code that returns names of things that changed, this is the place to copy the buffer into the caller's buffer.

    //  Win64 only: Use a 32bit IO_STATUS_BLOCK if the caller is 32bit.

    try {
        CmpCheckIoStatusPointer(PostBlock->u->AsyncUser);
        CmpSetIoStatus(PostBlock->u->AsyncUser.IoStatusBlock, *((ULONG *)SystemArgument1), 0L, PsGetCurrentProcess()->Wow64Process != NULL);
        CmpCheckIoStatusPointer(PostBlock->u->AsyncUser);
    ...
    *SystemArgument1 = PostBlock->u->AsyncUser.IoStatusBlock;
    ...
    // This is an Async notify, do all work here, including cleaning up the post block
    ...
    CmpRemoveEntryList(&(PostBlock->ThreadList));// Use Cmp variant to protect for multiple deletion of the same object    
    CmpCheckPostBlock(PostBlock);// debug only checks	
    CmpFreeSlavePost(PostBlock);// Free the slave post block to avoid "dangling" postblocks	
    CmpFreePostBlock(PostBlock);// free this post block
}
...
/*
Routine Description:
    This routine is called to clear away apcs in the apc queue of a thread that has been terminated.

    Since the apc is in the apc queue, we know that it is NOT in any NotifyBlock's post list.
    It is, however, in the threads's PostBlockList.

    Therefore, poke any user events so that waiters are not stuck, drop the references so the event can be cleaned up, delist the PostBlock and free it.
    Since we are cleaning up the thread, SystemEvents are not interesting.
    Since the apc is in the apc queue, we know that if there were any other notifications related to this one, they are cleaned up by the CmPostNotify routine
Arguments:
    Apc - pointer to apc object
*/
{
    ...
    try {
        CmpCheckIoStatusPointer(PostBlock->u->AsyncUser);
        CmpSetIoStatus(PostBlock->u->AsyncUser.IoStatusBlock, STATUS_NOTIFY_CLEANUP, 0L, PsGetCurrentProcess()->Wow64Process != NULL);
        CmpCheckIoStatusPointer(PostBlock->u->AsyncUser);
    } except(EXCEPTION_EXECUTE_HANDLER)
    {
        NOTHING;
    ...
    // part of the global registry pool computation, but are instead part of NonPagedPool with Quota.
...
// Cleanup procedure
VOID CmNotifyRunDown(__in PETHREAD    Thread)
/*
Routine Description:
    This routine is called from PspExitThread to clean up any pending notify requests.

    It will traverse the thread's PostBlockList, for each PostBlock it finds, it will:
        1.  Remove it from the relevant NotifyBlock.  This requires that we hold the Registry mutex.
        2.  Remove it from the thread's PostBlockList.  This requires that we run at APC level.
        3.  By the time this procedure runs, user apcs are not interesting and neither are SystemEvents, so do not bother processing them.

            UserEvents and IoStatusBlocks could be referred to by other threads in the same process, or even a different process,
            so process them so those threads know what happened, use status code of STATUS_NOTIFY_CLEANUP.
...
Arguments:
    Thread - pointer to the executive thread object for the thread we wish to do rundown on.
*/
{
    PCM_POST_BLOCK      PostBlock;
    KIRQL               OldIrql;
    ...
    while (IsListEmpty(&(Thread->PostBlockList)) == FALSE) {
        ...
        CmpClearListEntry(&(PostBlock->ThreadList));// Protect for multiple deletion of the same object
        ...
        // Canceling a master notification implies canceling all the slave notifications from the CancelPostList
        if (IsMasterPostBlock(PostBlock)) {
#if DBG
            if (PostBlock->TraceIntoDebugger) {
            ...
            if (PostBlockType(PostBlock) == PostAsyncUser) {
                // report status and wake up any threads that might otherwise be stuck. 
                // also drop any event references we hold
                ...
                // Cancel the APC. Otherwise the rundown routine will also free the post block if the APC happens to be queued at
                // this point. If the APC is queued, then the post block has already been removed from the notify list, so don't remove it again.
            ...
            CmpFreeSlavePost(PostBlock);// Free the slave Post blocks too
            // Free the post block.  Use Ex call because PostBlocks are NOT part of the global registry pool computation, but are instead part of NonPagedPool with Quota.
            CmpFreePostBlock(PostBlock);
        } else {
#if DBG
            if (PostBlock->TraceIntoDebugger) {
...
/*
Routine Description:
    Clean up notifyblock when a handle is closed or the key it refers to is deleted.
Arguments:
    KeyBody - supplies pointer to key object body for handle we are cleaning up.
*/
{
    PCM_NOTIFY_BLOCK    NotifyBlock;
    PCMHIVE             Hive;
    ...
    // Lock the hive exclusively to prevent multiple threads from whacking on the list.
    Hive = CONTAINING_RECORD(KeyBody->KeyControlBlock->KeyHive, CMHIVE, Hive);
    if (!LockHeld) {
        CmLockHive(Hive);
    } else {
        ASSERT_CM_LOCK_OWNED_EXCLUSIVE();// we should be holding the reglock exclusive
    }
    ...
    if (IsListEmpty(&(NotifyBlock->PostList)) == FALSE) {
        CmpPostNotify(NotifyBlock, NULL, 0, STATUS_NOTIFY_CLEANUP, FALSE, NULL
#if DBG
                      , Hive
#endif
        );
    ...
    SeReleaseSubjectContext(&NotifyBlock->SubjectContext);// Release the subject context

    // IMPLEMENTATION NOTE:
    //      If we ever do code to report names and types of events, this is the place to free the buffer.
    ...
    CmpClearListEntry(&(NotifyBlock->HiveList));// Protect for multiple deletion of the same object
    ...
    ExFreePool(NotifyBlock);// Free the block, clean up the KeyBody
}
...
/*
Routine Description:
    This routine sets up the NotifyBlock, and attaches the PostBlock to it.
    When it returns, the Notify is visible to the system, and will receive event reports.

    If there is already an event report pending, then the notify call will be satisfied at once.
Arguments:
    KeyBody - pointer to key object that handle refers to, allows access to key control block, notify block, etc.
    PostBlock - pointer to structure that describes how/where the caller is to be notified.
                WARNING:    PostBlock must come from Pool, THIS routine will keep it, back side will free it.
                            This routine WILL free it in case of error.
    CompletionFilter - what types of events the caller wants to see
    WatchTree - TRUE to watch whole subtree, FALSE to watch only immediate key the notify is applied to
    Buffer - pointer to area to receive notify data
    BufferSize - size of buffer, also size user would like to allocate for internal buffer
    MasterPostBlock - the post block of the master notification. Used to insert the PostBlock into the CancelPostList list.
OBS:
...
*/
{
    ...
    if (NotifyBlock == NULL) {
        ...
        // IMPLEMENTATION NOTE:
        //      If we ever want to actually return the buffers full of data, the buffer should be allocated and its address stored in the notify block here.

        SeCaptureSubjectContext(&NotifyBlock->SubjectContext);// Capture the subject context so we can do checking once the notify goes off.
    ...
    InsertHeadList(&(NotifyBlock->PostList), &(PostBlock->NotifyList));// Add post block to front of notify block's list, and add it to thread list.

    if (IsMasterPostBlock(PostBlock)) {
        ASSERT(PostBlock == MasterPostBlock);// Protect against outrageous calls        
        InitializeListHead(&(PostBlock->CancelPostList));// When the notification is a master one, initialize the CancelPostList list
    } else {
        InsertTailList(&(MasterPostBlock->CancelPostList), &(PostBlock->CancelPostList));// Add PostBlock at the end of the CancelPostList list from the master post
    }
    ...
    // If there is a notify pending (will not be if we just created the notify block) then post it at once. 
    // Note that this call ALWAYS returns STATUS_PENDING unless it fails. 
    // Caller must ALWAYS look in IoStatusBlock to see what happened.
    if (NotifyBlock->NotifyPending == TRUE) {
        CmpPostNotify(NotifyBlock, NULL, 0, STATUS_NOTIFY_ENUM_DIR, TRUE, NULL
#if DBG
                      , Hive
#endif
        );

        return STATUS_SUCCESS;// return STATUS_SUCCESS to signal to the caller the the notify already been triggered
    }

    return STATUS_PENDING;// return STATUS_PENDING to signal to the caller the the notify has not been triggered yet
}
...
*/
{
    ...
    ASSERT(!IsMasterPostBlock(SlavePostBlock));// This should be true !

    // Unchain from the Master CancelPostList

    CmpRemoveEntryList(&(SlavePostBlock->CancelPostList));// Use Cmp variant to protect for multiple deletion of the same object
    ...
    CmpFreePostBlock(SlavePostBlock);// Free the post block.    
    ASSERT(IsListEmpty(&(MasterPostBlock->CancelPostList)));// Result validation. was it the only slave?
}
...
Arguments:
    ...
    DelayedDeref - pointer to list of delayed deref keybodies. If this parameter is not NULL,
                the keybody for the slave is not cleared before calling CmpFreePostBlock, and instead is added to the list
*/
{
    PCM_POST_BLOCK  SlavePostBlock;
    ...
    if (DelayedDeref) {
        CmpAddToDelayedDeref(SlavePostBlock, DelayedDeref);// the caller wants to handle key body dereferenciation by himself
    }
}
...
Routine Description:
    ...
    Cleans the post block KeyBody member, so it will not be dereferenced when the post block is freed.
Arguments:
...
*/
{
    ...
    if (PostBlock->PostKeyBody) {
        // If the post block has a keybody attached, add it to delayed deref list and clear the post block member.
        // The key body will be deref'd prior after postblock lock is released.
...
*/
{
    ...
    while (IsListEmpty(DelayedDeref) == FALSE) {
        ...
        ObDereferenceObjectDeferDelete(PostKeyBody->KeyBody);// at last ..... dereference the key object        
