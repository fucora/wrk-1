...
Notes:
    This function doesn't support big cells;
    It is intended to be called just by the loader, which doesn't store large data.
    It'll bugcheck if big cell is queried.
*/
{
    PCELL_DATA  Buffer;
...
*/
{
    ...
    if (count > 1) {
        ...
        if (count < CM_MAX_REASONABLE_VALUES) {
            // A reasonable number of values, allocate just enough space.
            AllocateSize = count * sizeof(HCELL_INDEX);
        } else {
            // An excessive number of values, pad the allocation out to avoid fragmentation. (if there's this many values, there'll probably be more pretty soon)
    ...
    if (CmpIsHKeyValueSmall(realsize, DataLength) == TRUE) {
        ...
        if (CmpIsHKeyValueBig(Hive, realsize) == TRUE) {
            ...
            if (BigData == NULL) {
                ...
                // This shouldn't happen as this cell is marked dirty by this time (i.e. its view is pinned in memory)
                ASSERT(FALSE);
                return FALSE;
            }
            ...
            if (BigData->List != HCELL_NIL) {
                ...
                HvFreeCell(Hive, BigData->List);// mark the list as dirty
            }

            HvReleaseCell(Hive, DataCell);// fall through to free the cell data itself
        }

        HvFreeCell(Hive, DataCell);// normal case free the Data cell
    }
...
*/
{
    ...
    HvReleaseCell(Hive, Cell);
    HvFreeCell(Hive, Cell);// free the cell itself
    return TRUE;
}
...
*/
{
    ...
    if (CmpIsHKeyValueBig(Hive, DataSize) == TRUE) {
        ...
        *DataCell = HvAllocateCell(Hive, sizeof(CM_BIG_DATA), StorageType, ValueCell);
        ...
        Count = (USHORT)((DataSize + CM_KEY_VALUE_BIG - 1) / CM_KEY_VALUE_BIG);// Compute the number of cells needed
...
*/
{
    ...
    Plist = (PHCELL_INDEX)HvGetCell(Hive, BigData->List);
    if (Plist == NULL) {
        // cannot map view containing the cell; bail out this shouldn't happen as we just marked it as dirty
        // (i.e. its view should be pinned in memory)
        ASSERT(FALSE);
        return STATUS_INSUFFICIENT_RESOURCES;
    }
