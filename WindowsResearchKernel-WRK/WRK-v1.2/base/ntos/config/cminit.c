...
Routine Description:
    ...
    If extension is .log, open secondary for buffered I/O, else, open for non-buffered I/O.  Primary always uses non-buffered I/O.

    If primary is newly created, supersede secondary.
    If secondary does not exist, simply create (other code will complain if Log is needed but does not exist.)
...
Return Value:
    status - if status is success, Primary succeeded, check Secondary value to see if it succeeded.
*/
{
    ...
    // Do not fail to open the file if this fails, we don't want to prevent people from booting just because their disk is full.
    // Although they will not be able to update their registry, they will at lease be able to delete some files.
    CompressionState = 0;
    ASSERT_PASSIVE_LEVEL();
    status = ZwFsControlFile(*Primary, hEvent, NULL, NULL, &FsctlIoStatus, FSCTL_SET_COMPRESSION, &CompressionState, sizeof(CompressionState), NULL, 0);
    if (status == STATUS_PENDING) {
        KeWaitForSingleObject(pEvent, Executive, KernelMode, FALSE, NULL);
    }
    ...
    *PrimaryDisposition = (ULONG)IoStatus.Information;
    ...
    if (*PrimaryDisposition != FILE_CREATED) {
        ...
        status2 = ZwQueryInformationFile(*Primary, &IoStatus, (PVOID)&FileInformation, sizeof(FileInformation), FileStandardInformation);
    ...
    if (status == STATUS_ACCESS_DENIED) {
        // This means some person has put a read-only attribute on one of the critical system hive files.
        // Remove it so they don't hurt themselves.
        status = CmpOpenFileWithExtremePrejudice(Secondary, &ObjectAttributes, IoFlags, AttributeFlags);
    }

    if (!CmpShareSystemHives && (MarkAsSystemHive) && (NT_SUCCESS(status))) {
        ASSERT_PASSIVE_LEVEL();
        status = ZwFsControlFile(*Secondary, hEvent, NULL, NULL, &FsctlIoStatus, FSCTL_MARK_AS_SYSTEM_HIVE, NULL, 0, NULL, 0);
        if (status == STATUS_PENDING) {
            KeWaitForSingleObject(pEvent, Executive, KernelMode, FALSE, NULL);
            status = FsctlIoStatus.Status;
        }
    ...
    if (!NT_SUCCESS(status)) {
        ...
        *SecondaryDisposition = (ULONG)IoStatus.Information;
        ...
        // Do not fail to open the file if this fails, we don't want to prevent people from booting just because their disk is full.
        // Although they will not be able to update their registry, they will at least be able to delete some files.
        CompressionState = 0;

        ASSERT_PASSIVE_LEVEL();
        status = ZwFsControlFile(*Secondary, hEvent, NULL, NULL, &FsctlIoStatus, FSCTL_SET_COMPRESSION, &CompressionState, sizeof(CompressionState), NULL, 0);
        if (status == STATUS_PENDING) {
            KeWaitForSingleObject(pEvent, Executive, KernelMode, FALSE, NULL);
        }
    }
...
*/
{
    ...
    if ((OperationType == HINIT_FILE) || (OperationType == HINIT_MAPFILE) || (OperationType == HINIT_MEMORY) || (OperationType == HINIT_MEMORY_INPLACE)) {
...
/*
Routine Description:
    This routine opens a hive file that some person has put a read-only attribute on.
    It is used to prevent people from hurting themselves by making the critical system hive files read-only.
Arguments:
...
*/
{
    ...
    // Open the file
    Status = ZwOpenFile(&Handle, FILE_WRITE_ATTRIBUTES, Obja, &IoStatusBlock, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN_FOR_BACKUP_INTENT);
    if (!NT_SUCCESS(Status)) {
        return(Status);
    }
    ...
    if (NT_SUCCESS(Status)) {
        ...
        Status = ZwCreateFile(Primary, FILE_READ_DATA | FILE_WRITE_DATA, Obja, &IoStatusBlock, NULL, AttributeFlags, 0, FILE_OPEN, IoFlags, NULL, 0);
    }
#if DBG
    else {
