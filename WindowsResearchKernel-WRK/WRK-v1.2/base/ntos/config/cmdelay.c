...
Abstract:
    ...
    This module implements the new algorithm (LRU style) for the Delayed Close KCB table.

    Functions in this module are thread safe protected by the kcb lock.
    When kcb lock is converted to a resource, we should assert (enforce) exclusivity of that resource here !!!

Note:
    We might want to convert these functions to macros after enough testing provides that they work well
*/
...
VOID CmpDelayCloseDpcRoutine(IN PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2);
VOID CmpDelayDerefKCBDpcRoutine(IN PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2);
...
Routine Description:
      The fun part.
      We need to walk the cache and look for kcbs with refcount == 0
...
*/
{
    ...
    DueTime.QuadPart = Int32x32To64(CmpDelayCloseIntervalInSeconds, -SECOND_MULT);
    KeSetTimer(&CmpDelayCloseTimer, DueTime, &CmpDelayCloseDpc);// Indicate relative time
}
...
// not on delay close; don't try to remove it.
    ...
    if (DelayedEntry != NULL) {
        ...
        }
        {
            ...
            NewRefCount = 0;
            if (InterlockedCompareExchange((PLONG)&kcb->InDelayClose, NewRefCount, OldRefCount) != OldRefCount) {
                ASSERT(FALSE);
            }
        }
#endif //DBG
...
VOID CmpRunDownDelayDerefKCBEngine(PCM_KEY_CONTROL_BLOCK KeyControlBlock, BOOLEAN RegLockHeldEx)
{
    ...
    while (!CmpIsListEmpty(&CmpDelayDerefKCBListHead)) {
        ...
        DelayItem = CONTAINING_RECORD(DelayItem, CM_DELAY_DEREF_KCB_ITEM, ListEntry);
...
*/
{
    ...
    if ((NewRefCount & 0xffff) > 0 &&
        InterlockedCompareExchange((PLONG)&KeyControlBlock->RefCount, NewRefCount, OldRefCount) == OldRefCount) {
    ...
    if (!CmpDelayDerefKCBWorkItemActive) {
        ...
        // queue a timer for 5 secs later to do the actual delay close
        DueTime.QuadPart = Int32x32To64(CmpDelayDerefKCBIntervalInSeconds, -SECOND_MULT);
        KeSetTimer(&CmpDelayDerefKCBTimer, DueTime, &CmpDelayDerefKCBDpc);// Indicate relative time
    }
    UNLOCK_KCB_DELAY_DEREF_LIST();
}