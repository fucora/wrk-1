...
NTSTATUS CmpSetValueKeyExisting(
    IN PHHIVE  Hive,
...
#if DBG
ULONG CmpUnloadKeyWorker(PCM_KEY_CONTROL_BLOCK Current, PVOID Context1, PVOID Context2);
#endif
ULONG CmpCompressKeyWorker(PCM_KEY_CONTROL_BLOCK Current, PVOID Context1, PVOID Context2);
NTSTATUS CmpDuplicateKey(PHHIVE Hive, HCELL_INDEX OldKeyCell, PHCELL_INDEX NewKeyCell);
VOID CmpDestroyTemporaryHive(PCMHIVE CmHive);
VALUE_SEARCH_RETURN_TYPE CmpCompareNewValueDataAgainstKCBCache(PCM_KEY_CONTROL_BLOCK KeyControlBlock, PUNICODE_STRING ValueName, ULONG Type, PVOID Data, ULONG DataSize);
BOOLEAN CmpIsHiveAlreadyLoaded(IN HANDLE KeyHandle, IN POBJECT_ATTRIBUTES SourceFile, OUT PCMHIVE *CmHive);
BOOLEAN CmpDoFlushNextHive(BOOLEAN ForceFlush, PBOOLEAN PostWarning, PULONG DirtyCount);
BOOLEAN CmpGetHiveName(PCMHIVE CmHive, PUNICODE_STRING HiveName);
VOID CmpDoQueueLateUnloadWorker(IN PCMHIVE CmHive);
...
*/
{
    ...
    CmpLockKCBExclusive(KeyControlBlock);// serialize access to this key.
    ...
    if (KeyControlBlock->Delete) {
        ...
        CmpUnlockRegistry();
        CmpMarkAllBinsReadOnly(Hive);// Mark the hive as read only
    ...
    try {
        ...
        if (plist->Count != 0) {
            // The parent has at least one value, map in the list of values and call CmpFindChildInList
            ...
            if (ChildCell != HCELL_NIL) {
                ...
                CmpReportNotify(KeyControlBlock, KeyControlBlock->KeyHive, KeyControlBlock->KeyCell, REG_NOTIFY_CHANGE_LAST_SET);
...
Routine Description:
    ...
    CmEnumerateKey returns the name of the Index'th sub key of the open key specified.
    The value STATUS_NO_MORE_ENTRIES will be returned if value of Index is larger than the number of sub keys.

    Note that Index is simply a way to select among child keys.
    Two calls to CmEnumerateKey with the same Index are NOT guaranteed to return the same results.
...
*/
{
    ...
    CmpLockKCBShared(KeyControlBlock);// this should not be changed from under us
    ...
    HvReleaseCell(Hive, Cell);// release this cell here as we don't need this Node anymore
    ...
    if ((Node == NULL) || !HvTrackCellRef(&CellRef, Hive, childcell)) {// we couldn't map a view for the bin containing this cell
        CmpMarkAllBinsReadOnly(Hive);
        CmpUnlockKCB(KeyControlBlock);
        CmpUnlockRegistry();
        return STATUS_INSUFFICIENT_RESOURCES;
    ...
    try {// call a worker to perform data transfer
...
Routine Description:
    ...
    CmEnumerateValueKey returns the name of the Index'th value entry of the open key specified by KeyHandle.
    The value STATUS_NO_MORE_ENTRIES will be returned if value of Index is larger than the number of sub keys.

    Note that Index is simply a way to select among value entries.
    Two calls to NtEnumerateValueKey with the same Index are NOT guaranteed to return the same results.
...
RetryExclusive:
    ...
    if (KeyControlBlock->ExtFlags & CM_KCB_SYM_LINK_FOUND) {
        ...
        if ((CmpIsKCBLockedExclusive(KeyControlBlock) == FALSE) && (CmpTryConvertKCBLockSharedToExclusive(KeyControlBlock) == FALSE)) {
...
NTSTATUS CmFlushKey(IN PCM_KEY_CONTROL_BLOCK Kcb, IN BOOLEAN  RegistryLockOwnedExclusive)
/*
Routine Description:
    Forces changes made to a key to disk.

    CmFlushKey will not return to its caller until any changed data associated with the key has been written out.

    WARNING: CmFlushKey will flush the entire registry tree, and thus will burn cycles and I/O.
Arguments:
...
*/
{
    ...
    // If writes are not working, lie and say we succeeded, will clean up in a short time.
    // Only early system init code will ever know the difference.
    if (CmpNoWrite) {
    ...
    // Don't flush the master hive.
    // If somebody asks for a flushkey on the master hive, do a CmpDoFlushAll instead.
    // CmpDoFlushAll flushes every hive except the master hive, which is what they REALLY want.
    if (CmHive == CmpMasterHive) {
        ...
        if (HvHiveWillShrink(&(CmHive->Hive))) {
            ...
            CmUnlockHiveViews(CmHive);// release the views
        }
...
/*
Routine Description:
    Data about the class of a key, and the numbers and sizes of its children and value entries may be queried with CmQueryKey.
        ...
        if (KeyInformationClass == KeyNameInformation) {
            ...
            *ResultLength = requiredlength;
            ...
            if (KeyInformationClass == KeyCachedInformation) {
                ...
                status = CmpQueryKeyDataFromCache(KeyControlBlock, KeyInformationClass, KeyInformation, Length, ResultLength);
            } else {
                ...
                if ((Node == NULL) || !HvTrackCellRef(&CellRef, KeyControlBlock->KeyHive, KeyControlBlock->KeyCell)) {
                    ...
                    status = CmpQueryKeyData(KeyControlBlock->KeyHive, Node, KeyInformationClass, KeyInformation, Length, ResultLength);
                }
            }
        }
    } finally{
...
Routine Description:
    ...
    The ValueName, TitleIndex, Type, and Data for any one of a key's value entries may be queried with CmQueryValueKey.
...
RetryExclusive:
    ...
    SearchValue = CmpFindValueByNameFromCache(KeyControlBlock, &ValueName, &ContainingList, &Index, &ValueData, &ValueCached, &ValueDataCellToRelease);
    if (SearchValue == SearchNeedExclusiveLock) {
        // retry with exclusive lock, since we need to update the cache
        ASSERT(ValueDataCellToRelease == HCELL_NIL);
        ASSERT(ValueData == NULL);
    ...
    if (SearchValue == SearchSuccess) {
        ...
        try {
            // call a worker to perform data transfer; we are touching user-mode address; do it in a try/except
            SearchValue = CmpQueryKeyValueData(KeyControlBlock, ContainingList, ValueData, ValueCached, KeyValueInformationClass, KeyValueInformation, Length, ResultLength, &status);
            if (SearchValue == SearchNeedExclusiveLock) {
                // retry with exclusive lock, since we need to update the cache
                if (ValueDataCellToRelease != HCELL_NIL) {
...
Arguments:
    ...
    ResultLength - if present, Returns the length in bytes of the ValueBuffer array required to return the requested values of this key.
*/
{
    PHHIVE          Hive;
    NTSTATUS        Status;
    ...
    try {
        for (i = 0; i < EntryCount; i++) {
            ...
            if (ValueCell != HCELL_NIL) {
                ...
                // fill it in now. Otherwise, mark the buffer as full. We must keep iterating through the values in order to determine the RequiredLength.
                if ((UsedLength + DataLength <= *BufferLength) && (UsedLength + DataLength >= UsedLength) && (!BufferFull)) {
                    PCELL_DATA  Buffer;
...
Routine Description:
    ...
    If no such value entry exists, a new one is created.
    NULL is a legal Value ID.
...
*/
{
    ...
    while (TRUE) {
    TryAgain:
        // Check that we are not being asked to add a value to a key that has been deleted
        ...
        // Check to see if this is a symbolic link node.  If so caller is only allowed to create/change the SymbolicLinkValue value name
        if (KeyControlBlock->Flags & KEY_SYM_LINK &&
            (((Type != REG_LINK)) || ValueName == NULL || !RtlEqualUnicodeString(&CmSymbolicLinkValueName, ValueName, TRUE))) {
    ...
    if (found) {
        ...
        // An existing value entry of the specified name exists, set our data into it.
        ...
        // specified is not in the list.  In either case, create and fill a new one, and add it to the list
...
/*
Routine Description:
    Helper for CmSetValueKey, implements the case where the value entry being set already exists.
Arguments:
    Hive - hive of interest
    OldChild - hcell_index of the value entry body to which we are to set new data
    Type - The integer type number of the value entry.
    Data - Pointer to buffer with actual data for the value entry.
    DataSize - Size of Data buffer.
...
*/
{
    ...
    OldDataCell = Value->Data;// store it to be freed if the allocation succeeds
    ...
    if (NewSizeType == 1) {
        if ((OldSizeType == 1) && (OldRealSize > 0)) {
            ...
            if (DataSize <= (ULONG)(HvGetCellSize(Hive, pdata))) {
                ...
                NewCell = DataCell;// we'll keep this cell
            } else {
...
/*
Routine Description:
    Helper for CmSetValueKey, implements the case where the value entry being set does not exist.  Will create new value entry and data, place in list (which may be created)
Arguments:
    Hive - hive of interest
...
*/
{
    ...
    HvReleaseCell(Hive, ValueCell);// release it right here, as the view is PINNED    
    ASSERT_CELL_DIRTY(Hive, ValueCell);// sanity
...
/*
Routine Description:
    The LastWriteTime associated with a key node can be set with CmSetLastWriteTimeKey
Arguments:
    KeyControlBlock - pointer to kcb for the key to operate on
...
*/
{
    ...
    parent = (PCM_KEY_NODE)HvGetCell(Hive, Cell);
    if (parent == NULL) {// we couldn't map a view for the bin containing this cell
        status = STATUS_INSUFFICIENT_RESOURCES;
        goto Exit2;
    }
...
/*
Routine Description:
    Sets the user defined flags for the key; At this point there are only 4 bits reserved for user defined flags. kcb and knode must be kept in sync.
Arguments:
    KeyControlBlock - pointer to kcb for the key to operate on
...
Arguments:
    KeyHandle - should be the root of a hive. We'll query the name of the primary file and compare it against the name of SourceFile
...
Routine Description:
    ...
    N.B.  This routine assumes that the object attributes for the file to be opened have been captured into kernel space so that
          they can safely be passed to the worker thread to open the file and do the actual I/O.
...
*/
{
    ...
    if (KeyBody != NULL) {
        ...
        if (!(OtherHive->Flags & CM_CMHIVE_FLAG_UNTRUSTED)) {
            return STATUS_INVALID_PARAMETER;// deny attempts to join the TRUSTED class of trust
        }
    }

    // Obtain the security context here so we can use it later to impersonate the user, which we will do
    // if we cannot access the file as SYSTEM.  This usually occurs if the file is on a remote machine.
    ...
    Status = SeCreateClientSecurity(CONTAINING_RECORD(KeGetCurrentThread(), ETHREAD, Tcb), &ServiceQos, FALSE, &ClientSecurityContext);
    if (!NT_SUCCESS(Status)) {
        return(Status);
    }
    ...
    Status = ObOpenObjectByName(TargetKey, CmpKeyObjectType, KernelMode, NULL, KEY_READ, (PVOID)&ParseContext, &KeyHandle);
    if (!NT_SUCCESS(Status)) {
        KeyHandle = NULL;
    }
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        CmpLockRegistry();// we only need shared
    }

    LOCK_HIVE_LOAD();// only one hive loading at a time (I can imagine getting rid of that, if needed)
    ...
    if (OtherHive != NULL) {
        CmpJoinClassOfTrust(NewHive, OtherHive);// join the same class of trust with the otherhive
    }
    ...
    Status = CmpLinkHiveToMaster(TargetKey->ObjectName, TargetKey->RootDirectory, NewHive, Allocate, TargetKey->SecurityDescriptor);
    if (NT_SUCCESS(Status)) {
        CmpAddToHiveFileList(NewHive);// add new hive to hivelist

        // flush the hive right here if just created; this is to avoid situations where
        // the lazy flusher doesn't get a chance to flush the hive, or it can't (because the hive is a no_lazy_flush hive and it is never explicitly flushed)
        if (Allocate == TRUE) {
        ...
        CmpCmdHiveClose(NewHive);// Close the hive files
        CmpFreeMutex(NewHive->ViewLock);// free the cm level structure
#if DBG
...
/*
Routine Description:
    Unlinks a hive from its location in the registry, closes its file handles, and deallocates all its memory.
    There must be no key control blocks currently referencing the hive to be unloaded.
Arguments:
    Hive - Supplies a pointer to the hive control structure for the hive to be unloaded
    Kcb - Supplies the key control block
    Flags - REG_FORCE_UNLOAD will first mark open handles as invalid and then unload the hive.
...
*/
{
    ...
    if (InterlockedCompareExchangePointer(&(CmHive->UnloadWorkItem), (PVOID)1, NULL) != NULL) {
        return STATUS_TOO_LATE;// work item has already been queued
    }

    Hive->HiveFlags |= HIVE_IS_UNLOADING;// mark the hive as HIVE_IS_UNLOADING so no new kcbs are created within this hive.

    // Make sure there are no open references to key control blocks for this hive. 
    // If there are none, then we can unload the hive.
    if (Kcb->RefCount != 1) {
        if (Flags == REG_FORCE_UNLOAD) {
            // This will mark open handles as invalid. However, it may fail, so we account for that possibility here.
    ...
    CmFlushKey(Kcb, TRUE);// Flush any dirty data to disk. If this fails, too bad.
    ...
    if (Success) {
        // signal the user event (if any), then do the cleanup (i.e. deref the event and the artificial refcount we set on the root kcb)
        ...
        if (Remove) {
            CmpRemoveFromHiveFileList(&EntryName);// Remove the hive from the HiveFileList
        }
        ...
        HvFreeHive(Hive);
        CmpCmdHiveClose(CmHive);// Close the hive files
        CmpFreeMutex(CmHive->ViewLock);// free the cm level structure
#if DBG
...
NTSTATUS CmUnloadKeyEx(IN PCM_KEY_CONTROL_BLOCK kcb, IN PKEVENT UserEvent)
/*
Routine Description:
    First tries to unlink the hive, by calling the sync version
    If the hive cannot be unloaded (there are open handles inside it), reference the root of the hive (i.e. kcb) and freeze the hive.
Arguments:
    Kcb - Supplies the key control block
    UserEvent - the event to be signaled after the hive was unloaded (only if late - unload is needed)
Return Value:
    STATUS_PENDING - the hive was frozen and it'll be unloaded later
    STATUS_SUCCESS - the hive was successfully sync-unloaded (no need to signal for UserEvent)
...
*/
{
    ...
    if (IsHiveFrozen(CmHive)) {
        return STATUS_TOO_LATE;// don't let them hurt themselves by calling it twice
    }

    // first, try out he sync routine; this may or may not unload the hive, but at least will kick kcbs with refcount = 0 out of cache
    Status = CmUnloadKey(kcb, 0, CM_UNLOAD_REG_LOCKED_EX);
    if (Status != STATUS_CANNOT_DELETE) {
        return Status;// the hive was either unloaded, or some bad thing happened
    }
...
Routine Description:
    ...
    NOTE: Hives which are marked as HV_NOLAZYFLUSH are *NOT* flushed by this call.
          You must call HvSyncHive explicitly to flush a hive marked as HV_NOLAZYFLUSH.

Arguments:
    ForceFlush - used as a contingency plan when a prior exception left some hive in a used state.
                When set to TRUE, assumes the registry is locked exclusive.
                It also repairs the broken hives.

               - When FALSE saves only the hives with UseCount == 0.
Notes:
    If any of the hives is about to shrink CmpForceForceFlush is set to TRUE, otherwise, it is set to FALSE
*/
{
    NTSTATUS    Status;
    PLIST_ENTRY p;
    ...
    while (p != &CmpHiveListHead) {
        ...
        if (!(h->Hive.HiveFlags & HIVE_NOLAZYFLUSH)) {
            ...
            //            or shutdown flush did not work.  we don't know why. 
            //            there is no one to report an error to, so continue on and hope for the best.
            //            (in theory, worst that can happen is user changes are lost.)
        }
...
*/
{
    ...
    Node = (PCM_KEY_NODE)HvGetCell(Hive, Cell);
    if (Node == NULL) {// cannot map view        
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    HvReleaseCell(Hive, Cell);// release the cell right here, as the registry is locked exclusively, so we don't care
    ...
    // We need to check we have KEY_CREATE_SUB_KEY on the parent and DELETE on the entire subtree underneath us.
    Status = CmpCheckKeyAccess(KeyControlBlock->ParentKcb->KeyHive, KeyControlBlock->ParentKcb->KeyCell, PreviousMode, KEY_CREATE_SUB_KEY);
    if (NT_SUCCESS(Status)) {
        Status = CmpDoAccessCheckOnSubtree(Hive, Cell, PreviousMode, DELETE, TRUE);
    }
    ...
    if (ParentNode == NULL) {// cannot map view        
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    HvReleaseCell(Hive, Node->Parent);// release the cell right here, as the registry is locked exclusively, so we don't care
    ...
    try {
        ...
        NodeSize = CmpHKeyNodeSize(Hive, &NewKeyName);// since we are in try-except, compute the new node size
    } except(EXCEPTION_EXECUTE_HANDLER)
    {
    ...
    // 1. Allocate the new knode cell and copy the data from the old one, updating the name.
    ...
    if (Index == NULL) {// we just marked this dirty        
        ASSERT(FALSE);
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    HvReleaseCell(Hive, OldSubKeyList);// release the cell right here, as the registry is locked exclusively, so we don't care
    ...
    HvReleaseCell(Hive, NewKeyCell);// release the cell right here, as the registry is locked exclusively, so we don't care
    ...
    // update the NCB in the kcb; at the end of this function, the kcbs underneath this will eventually get rehashed
    ...
    if (Index->Signature == CM_KEY_INDEX_ROOT) {
        ...
        // should be a leaf
        ASSERT((Index->Signature == CM_KEY_INDEX_LEAF) || (Index->Signature == CM_KEY_FAST_LEAF) || (Index->Signature == CM_KEY_HASH_LEAF));
        ASSERT(Index->Count != 0);
    }

    HvFreeCell(Hive, OldSubKeyList);
    HvFreeCell(Hive, Cell);// free old cell
    ...
    CmpCleanUpSubKeyInfo(KeyControlBlock->ParentKcb);// and one last "little" thing: update parent's maxnamelen and reset parents cache
    ...
    if (ConvKey != KeyControlBlock->ConvKey) {
        // rehash the kcb by removing it from hash, and then inserting it again with th new ConvKey
        CmpRemoveKeyHash(&(KeyControlBlock->KeyHash));
        KeyControlBlock->ConvKey = ConvKey;
        CmpInsertKeyHash(&(KeyControlBlock->KeyHash), FALSE);
    }

    CmpSearchForOpenSubKeys(KeyControlBlock, SearchAndRehash, TRUE, NULL);// Additional work: take care of the kcb subtree; this cannot fail, punt
...
ErrorExit:
    if (OldSubKeyList != HCELL_NIL) {
        ...
        if (ParentNode->SubKeyLists[StorageType] != HCELL_NIL) {
            ...
            if (Index == NULL) {
                ASSERT(FALSE);// could not map view;this shouldn't happen as we just allocated this cell
            } else {
                HvReleaseCell(Hive, ParentNode->SubKeyLists[StorageType]);// release the cell right here, as the registry is locked exclusively, so we don't care
        ...
        ParentNode->SubKeyLists[StorageType] = OldSubKeyList;// restore the parent's index
    }
...
*/
{
    ...
    if (StorageType != Stable) {// nop the volatiles        
        return STATUS_SUCCESS;
    }

    if (OldKeyCell == Hive->BaseBlock->RootCell) {
        return STATUS_INVALID_PARAMETER;// this works only for stable keys.
    }
    ...
    if (OldKeyNode == NULL) {// cannot map view        
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    HvReleaseCell(Hive, OldKeyCell);// release the cell right here, as the registry is locked exclusively, and the key_cell is marked as dirty

    if (OldKeyNode->Flags & KEY_SYM_LINK) {// we do not compact links        
        return STATUS_INVALID_PARAMETER;
    }
    if (OldKeyNode->SubKeyLists[Stable] != HCELL_NIL) {
        ...
        if (OldIndex == NULL) {// we couldn't map the bin containing this cell            
            return STATUS_INSUFFICIENT_RESOURCES;
        }
    ...
    if (ParentKeyNode == NULL) {// cannot map view        
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    if (ParentIndex == NULL) {// we couldn't map the bin containing this cell        
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    if (ParentIndex->Signature == CM_KEY_INDEX_ROOT) {
        ...
        for (i = 0; i < ParentIndex->Count; i++) {
            ...
            if (Leaf == NULL) {// we couldn't map the bin containing this cell                
                return STATUS_INSUFFICIENT_RESOURCES;
            }
            ...
            if ((Leaf->Signature == CM_KEY_FAST_LEAF) || (Leaf->Signature == CM_KEY_HASH_LEAF)) {
                ...
                for (j = 0; j < Leaf->Count; j++) {
                    if (Leaf->List[j] == OldKeyCell) {// found it! remember the locations we want to update later and break the loop
                        if (!HvMarkCellDirty(Hive, LeafCell, FALSE)) {
                            return STATUS_NO_LOG_SPACE;
                        }
        ...
        for (j = 0; j < FastIndex->Count; j++) {
            if (FastIndex->List[j].Cell == OldKeyCell) {// found it! remember the locations we want to update later and break the loop
        ...
        for (j = 0; j < ParentIndex->Count; j++) {
            if (ParentIndex->List[j] == OldKeyCell) {// found it! remember the locations we want to update later and break the loop
                if (!HvMarkCellDirty(Hive, ParentKeyNode->SubKeyLists[Stable], FALSE)) {
                    return STATUS_NO_LOG_SPACE;
                }
    ...
    ASSERT(ParentIndexLocation != NULL);// we should've find it !!!
    ...
    // 4. replace the new Key_cell in the parent's subkeylist From now on, WE CANNOT fails.
    // we have everything marked dirty we just update some fields. no resources required !
...
*/
{
    ...
    // while copying the trees; So, we just set the release routines to NULL and restore after the copy is complete; this saves some pain
    ...
    *NewKeyCell = CmpCopyKeyPartial(Hive, OldKeyCell, Hive, OldKeyNode->Parent, TRUE);
    ...
    NewKeyNode = (PCM_KEY_NODE)HvGetCell(Hive, *NewKeyCell);
    if (NewKeyNode == NULL) {// cannot map view        
        HvReleaseCell(Hive, OldKeyCell);
        CmpFreeKeyByCell(Hive, *NewKeyCell, FALSE);
        return STATUS_INSUFFICIENT_RESOURCES;
    }
...
Routine Description:
    ...
    1. iterate through the kcb tree and make a list of all the kcbs that need to be changed (their keycell will change during the process)
    2. iterate through the cache and compute an array of security cells.
    We'll need it to map security cells into the new hive.
    3. Save the hive into a temporary hive, preserving the volatile info in keynodes and updating the cell mappings.
    ...
    6. Switch hive data from the compressed one to the existing one and update the kcb KeyCell and security mapping
...
*/
{
    ...
    // this will kick all kcb with refcount == 0 out of cache, so we can use CmpSearchKeyControlBlockTree for recording referenced kcbs
    ...
    // 6. Switch hive data from the compressed one to the existing one and update the kcb KeyCell and security mapping
    // This should better NOT fail!!! If it does, we are doomed, as we have partial data => bugcheck
...
*/
{
    ...
    ASSERT(((KeyControlBlock->ExtFlags&CM_KCB_READ_ONLY_KEY) && (KeyControlBlock->ExtFlags&CM_KCB_NO_DELAY_CLOSE)) || (!(KeyControlBlock->ExtFlags&CM_KCB_READ_ONLY_KEY)));

    KeyControlBlock->ExtFlags |= (CM_KCB_READ_ONLY_KEY | CM_KCB_NO_DELAY_CLOSE);//tag the kcb as read-only; also make it no-delay close so it can revert to the normal state after all handles are closed.
    InterlockedIncrement((PLONG)&KeyControlBlock->RefCount);//add an artificial refcount on this kcb. This will keep the kcb (and the read only flag set in memory for as long as the system is up)
    CmpUnlockKCB(KeyControlBlock);
    return STATUS_SUCCESS;
}
...
/*
Routine Description:
    Most of the SetValue calls are noops (i.e. they are setting the same value name to the same value data).
    By comparing against the data already in the kcb cache (i.e. faulted in) we can save page faults.
Arguments:
    KeyControlBlock - pointer to kcb for the key to operate on
    ValueName - The unique (relative to the containing key) name of the value entry.  May be NULL.
    Type - The integer type number of the value entry.
...
*/
{
    ...
    if (KeyControlBlock->Flags & KEY_SYM_LINK) {// need to rebuild the value cache, so we could runt the same code
        PCM_KEY_NODE    Node;
        if ((CmpIsKCBLockedExclusive(KeyControlBlock) == FALSE) && (CmpTryConvertKCBLockSharedToExclusive(KeyControlBlock) == FALSE)) {
            return SearchNeedExclusiveLock;// need to upgrade lock to exclusive
        }

        Node = (PCM_KEY_NODE)HvGetCell(KeyControlBlock->KeyHive, KeyControlBlock->KeyCell);
        if (Node == NULL) {
            return SearchFail;// we couldn't map the bin containing this cell
        }
    ...
    SearchValue = CmpFindValueByNameFromCache(KeyControlBlock, ValueName, &ContainingList, &Index, &Value, &ValueCached, &ValueDataCellToRelease);
    if (SearchValue == SearchNeedExclusiveLock) {
        // retry with exclusive lock, since we need to update the cache
    ...
    if (SearchValue == SearchSuccess) {
        ...
        if ((Type == Value->Type) && (DataSize == (Value->DataLength & ~CM_KEY_VALUE_SPECIAL_SIZE))) {
            ...
            if (small == TRUE) {
                ...
                SearchValue = CmpGetValueDataFromCache(KeyControlBlock, ContainingList, (PCELL_DATA)Value, ValueCached, &datapointer, &BufferAllocated, &CellToRelease);
                if (SearchValue != SearchSuccess) {
