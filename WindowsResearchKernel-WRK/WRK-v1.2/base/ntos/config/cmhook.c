...
Abstract:
    ...
    Callbacks are to be used by the virus filter drivers and cluster replication engine.
*/
...
BOOLEAN CmpCheckRecursionAndRecordThreadInfo(PCM_CALLBACK_CONTEXT_BLOCK CallbackBlock, PCM_ACTIVE_NOTIFY_THREAD ActiveThreadInfo);
...
*/
{
    ...
    CmCallbackContext = (PCM_CALLBACK_CONTEXT_BLOCK)ExAllocatePoolWithTag(PagedPool, sizeof(CM_CALLBACK_CONTEXT_BLOCK), 'bcMC');
...
NTSTATUS CmUnRegisterCallback(__in LARGE_INTEGER  Cookie)
/*
Routine Description:
...
*/
{
    ...
    for (; (i >= 0) && (i < CM_MAX_CALLBACKS); i += Direction) {
        ...
        if (RoutineBlock != NULL) {
            ...
            // record thread on a stack struct, so we don't need to allocate pool for it.
            // We unlink it from our lists prior to this function exit, so we are on the safe side.
            CM_ACTIVE_NOTIFY_THREAD ActiveThreadInfo;

            CmCallbackContext = (PCM_CALLBACK_CONTEXT_BLOCK)ExGetCallBackBlockContext(RoutineBlock);// get context info
            ...
            if (CmpCheckRecursionAndRecordThreadInfo(CmCallbackContext, &ActiveThreadInfo)) {
                ...
                } else {// regular callback call                    
                    Status = ExGetCallBackBlockRoutine(RoutineBlock)(CmCallbackContext->CallerContext, (PVOID)(ULONG_PTR)Type, Argument);
                    if (Wind == FALSE) {
                        Status = STATUS_SUCCESS;// always ignore post return calls so they all get a chance to run
                    }
                }
