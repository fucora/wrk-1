...
Revision History:
    Requests for cells bigger than 1K are doubled.
    This way we avoid fragmentation and we make the value-growing process more flexible.
...
// Private procedures
HCELL_INDEX HvpDoAllocateCell(PHHIVE Hive, ULONG NewSize, HSTORAGE_TYPE Type, HCELL_INDEX Vicinity);
...
//  CmpSetValueKeyExisting  always allocates a bigger data value cell  exactly the required size.
//  This creates problems when somebody  slowly grows a value one DWORD at a time to  some enormous size.
//  An easy fix for this would be to set a  certain threshold (like 1K).
//  Once a value size  crosses that threshold, allocate a new cell that is twice  the old size.
//  So the actual allocated size  would grow to 1k, then 2k, 4k, 8k, 16k, 32k,etc.
...
PHCELL HvpGetHCell(PHHIVE      Hive, HCELL_INDEX Cell)
/*
Routine Description:
...
/*
Routine Description:
    This routine should never be called directly, always call it via the HvReleaseCell() macro.
    This routine is intended to work with mapped hives. It is intended to prevent views that are still in use to get unmapped
Arguments:
    ...
    */
{
    ...
    if (Map->BinAddress & HMAP_INPAGEDPOOL) {
        ...
        if ((Map->BinAddress & HMAP_INVIEW) == 0) {
            ...
            if (!NT_SUCCESS(CmpMapCmView((PCMHIVE)Hive, Cell/*+HBLOCK_SIZE*/, &CmView, TRUE))) {
                // caller of HvGetCell should raise an STATUS_INSUFFICIENT_RESOURCES error as a result of this.!!!!
                CmUnlockHiveViews((PCMHIVE)Hive);
                return NULL;
            }
        ...
        CmpTouchView((PCMHIVE)Hive, CmView, (ULONG)Cell);// touch the view
    ...
    // we need to make sure all the cell's data is faulted in inside a try/except block, as the IO to fault the data in can throw exceptions
...
struct _CELL_DATA * HvpGetCellPaged(PHHIVE      Hive, HCELL_INDEX Cell)
    /*
    Routine Description:
        Returns the memory address for the specified Cell.
        Will never return failure, but may assert.
        Use HvIsCellAllocated to check validity of Cell.
...
Arguments:
    ...
    CoalesceForward - indicates whether we can coalesce forward or not.
        For the case where we have not finished scanning the hive and enlisting free cells, we do not want to coalesce forward.
*/
{
    PHMAP_ENTRY Map;
    PHCELL      pcell;
    ...
    // the HvpGetHCell call below touches the view containing the cell, and makes sure the CM_VIEW_SIZE window is mapped in the system cache
    ...
    if ((pcell == (PHCELL)(Bin + 1)) && (Size == Bin->Size - sizeof(HBIN))) {
        // We have a bin that is entirely free. 
        // But we cannot do anything with it unless the memalloc that contains the bin is entirely free. 
        ...
        while (HvpGetBinMemAlloc(Hive, FirstBin, Type) == 0) {
            ...
            if ((ULONG)(FirstCell->Size) != FirstBin->Size - sizeof(HBIN)) {
                // The first cell in the bin is either allocated, or not the only cell in the HBIN. 
                // We cannot free any HBINs.
                goto Done;
            }
        }
        ...
        while (LastBin->FileOffset + LastBin->Size < FirstBin->FileOffset + HvpGetBinMemAlloc(Hive, FirstBin, Type)) {
            if (!CoalesceForward) {
                // We are at the end of what's been built up.
                // Just return and this will get freed up when the next HBIN is added.
                goto Done;
            }
        ...
        if (FirstBin->Size != HvpGetBinMemAlloc(Hive, FirstBin, Type)) {
            ...
            if (!HvMarkDirty(Hive, FirstBin->FileOffset + (Type * HCELL_TYPE_MASK), sizeof(HBIN) + sizeof(HCELL), FALSE)) {
                goto Done;
            }
        }
...
*/
{
    ...
    if (pcell == NULL) {
        // we couldn't map view for this cell
        // this shouldn't happen as the cell here is already marked dirty or it's entire bin is mapped
        ASSERT(FALSE);
        return;
    }
...
HCELL_INDEX HvAllocateCell(PHHIVE Hive, ULONG NewSize, HSTORAGE_TYPE Type, HCELL_INDEX Vicinity)
/*
Routine Description:
...
Routine Description:
    ...
    If Vicinity is not NIL, it defines the "window" where the new cell to be allocated (if one free is found).
    The window is ensured by looking for a free cell of the desired size:

    1st - in the same CM_VIEW_SIZE window with the Vicinity cell.

Abstract:
    This first version allocates a new bin if a cell free cell big enough cannot be found in the specified window.
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive of interest
    NewSize - size in bytes of the cell to allocate
    Type - indicates whether Stable or Volatile storage is desired.
    Vicinity - the starting cell which defines the vicinity of the new allocated cell.
Return Value:
...
*/
{
    ...
    if (cell != HCELL_NIL) {
        ...
        HvReleaseCell(Hive, cell);// we are safe to release the cell here as the view is pinned
        ...
        // Either there is no large enough cell, or we have no free cells left at all. 
        // In either case, allocate a new bin, with a new free cell certain to be large enough in it, and use that cell.
...
#if DBG
    if (USE_OLD_CELL(Hive)) {
        RtlFillMemory(&(pcell->u.OldCell.u.UserData), (pcell->Size - FIELD_OFFSET(HCELL, u.OldCell.u.UserData)), HCELL_ALLOCATE_FILL);
    } else {
        RtlFillMemory(&(pcell->u.NewCell.u.UserData), (pcell->Size - FIELD_OFFSET(HCELL, u.NewCell.u.UserData)), HCELL_ALLOCATE_FILL);
    }
#endif
    pcell->Size *= -1;
...
// Procedure used for checking only  (used in production systems, so must always be here.)
...
*/
{
    ...
    if (Map->BinAddress & HMAP_DISCARDABLE) {
        ...
        while (Entry != &Hive->Storage[Type].FreeBins) {
            ...
            Entry = Entry->Flink;// advance to the new bin
        }

        CM_BUGCHECK(REGISTRY_ERROR, BAD_FREE_BINS_LIST, 1, (ULONG)Cell, (ULONG_PTR)Map);// we shouldn't get here
        return;
    }

    // as for the new way of dealing with free cells, all we have to do is to clear the bits in the FreeDisplay
...
struct _CELL_DATA * HvpGetCellFlat(PHHIVE      Hive, HCELL_INDEX Cell)
    ...
    Routine Description:
        ...
        This routine provides GetCell support for read only hives with single allocation flat images.
        Such hives do not have cell maps ("page tables"), instead, we compute addresses by arithmetic against the base image address.
...
/*
Routine Description:
    Returns the size of the specified Cell, based on its MEMORY ADDRESS.
    Must always call HvGetCell first to get that address.
...
Routine Description:
    ...
    NOTE:   CALLER is expected to mark relevant data dirty, so as to allow this call to always succeed.
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive of interest
...
*/
{
    ...
    ASSERT_HIVE_FLUSHER_LOCKED((PCMHIVE)Hive);// we have the lock exclusive or nobody is operating inside this hive

    // We should hit this if there is any bogus code path where data is modified
    // but not marked as dirty; We could run into a lot of problems if this ASSERT ever fires !!!
    ASSERT_CELL_DIRTY(Hive, Cell);

    CmpLockHiveWriter((PCMHIVE)Hive);// no two guys in here at the same time for the same hive
    ...
    HvReleaseCell(Hive, Cell);// release the cell right here as the view is pinned
    ...
    // Look for free neighbors and coalesce them. 
    // We will never travel around this loop more than twice.
    ...
    // freebase now points to the biggest free cell we could make, none of which is on the free list.  So put it on the list.
    ...
    if (USE_OLD_CELL(Hive)) {
        RtlFillMemory(&(freebase->u.OldCell.u.UserData), (freebase->Size - FIELD_OFFSET(HCELL, u.OldCell.u.UserData)), HCELL_FREE_FILL);
    } else {
        RtlFillMemory(&(freebase->u.NewCell.u.UserData), (freebase->Size - FIELD_OFFSET(HCELL, u.NewCell.u.UserData)), HCELL_FREE_FILL);
    }
#endif
...
/*
Routine Description:
    Reports on whether FreeCell has at least one free neighbor and if so where.
    Free neighbor will be cut out of the free list.
Arguments:
        ...
        while (ptcell < FreeCell) {
            ...
            if (ptcell->Size > 0) {
                if ((PHCELL)((PUCHAR)ptcell + ptcell->Size) == FreeCell) {
                    *FreeNeighbor = ptcell;
                    // Try and mark it dirty, since we will be changing the size field.
                    // If this fails, ignore the free neighbor, we will not fail the free just because we couldn't mark the cell dirty so it could be coalesced.

                    // Note we only bother doing this for new hives, for old format hives we always mark the whole bin dirty.
                    if ((Type == Volatile) || (HvMarkCellDirty(Hive, (ULONG)((ULONG_PTR)ptcell - (ULONG_PTR)Bin) + Bin->FileOffset, TRUE))) {
                        goto FoundNeighbor;
                    } else {
                        return(FALSE);
                    }
...
*/
{
    ...
    HvpAdjustCellSize(NewSize);// Adjust the size (an easy fix for granularity)
    ...
    if (NewSize == oldalloc) {
        NewCell = Cell;// This is a noop, return the same cell
    } else if (NewSize < oldalloc) {
        NewCell = Cell;// This is a shrink.
    } else {
        // This is a grow.
        ...
        if (newaddress == NULL) {
            // we couldn't map a view for this cell this shouldn't happen as we just allocated this cell (i.e. it's containing bin should be PINNED into memory)
            ASSERT(FALSE);
            HvReleaseCell(Hive, Cell);
            return HCELL_NIL;
        }

        RtlMoveMemory(newaddress, oldaddress, oldsize);// oldaddress points to the old data block for the cell, newaddress points to the new data block, copy the data
        HvReleaseCell(Hive, NewCell);
        HvFreeCell(Hive, Cell);// Free the old block of memory
    }
...
HCELL_INDEX HvDuplicateCell(PHHIVE Hive, HCELL_INDEX Cell, HSTORAGE_TYPE   Type, BOOLEAN         CopyData)
/*
Routine Description:
...
*/
{
    ...
    if (CellAddress == NULL) {
        return HCELL_NIL;// we couldn't map a view for this cell
    }

    HvReleaseCell(Hive, Cell);// release the cell here as we are holding the reglock exclusive
    ...
    HvReleaseCell(Hive, NewCell);// release the cell here as we are holding the reglock exclusive
...
*/
{
    ...
    if (IsListEmpty(&(Hive->Storage[Stable].FreeBins))) {
        return FALSE;// no free bins; no worth bothering
    }
    ...
    while (FreeBin != (PFREE_HBIN)AnchorAddr) {
        ...
        FreeBin = (PFREE_HBIN)(FreeBin->ListEntry.Flink);// skip to the next element
    }
    CompressLevel = CM_HIVE_COMPRESS_LEVEL * (Hive->Storage[Stable].Length / 100);

    if (FreeSpace < CompressLevel) {
        return FALSE;// disable temporary so we can test the system hive.
    }
...
HCELL_INDEX HvShiftCell(PHHIVE Hive, HCELL_INDEX Cell)
{
    PHMAP_ENTRY t;
    PHBIN       Bin;
...
BOOLEAN HvTrackCellRef(PHV_TRACK_CELL_REF   CellRef, PHHIVE               Hive, HCELL_INDEX          Cell)
/*
Routine Description:
    helper to track cell references; autogrowing array to be called after a successful HvGetCell
Return Value:
    TRUE/FALSE
*/
{
...
VOID HvReleaseFreeCellRefArray(PHV_TRACK_CELL_REF   CellRef)
{
    USHORT  i;
