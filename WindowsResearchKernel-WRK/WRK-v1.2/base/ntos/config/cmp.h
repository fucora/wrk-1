...
PVOID CmpAllocateTag(ULONG   Size, BOOLEAN UseForIo, ULONG   Tag);
#else
...
#define ASSERT_CM_LOCK_OWNED_EXCLUSIVE()     ASSERT((CmpSpecialBootCondition == TRUE) || (CmpTestRegistryLockExclusive() == TRUE) )
#define ASSERT_CM_EXCLUSIVE_HIVE_ACCESS(Hive)     ASSERT((CmpSpecialBootCondition == TRUE) || (CmpTestRegistryLockExclusive() == TRUE) || (Hive->ReleaseCellRoutine == NULL) )
#define ASSERT_CM_LOCK_OWNED_OR_HIVE_LOADING(_CmHive_)     ASSERT( (CmpSpecialBootCondition == TRUE) || (((PCMHIVE)(_CmHive_))->HiveIsLoading) || (CmpTestRegistryLock() == TRUE) )
#define ASSERT_CM_LOCK_OWNED_EXCLUSIVE_OR_HIVE_LOADING(_CmHive_)     ASSERT( (CmpSpecialBootCondition == TRUE) || (((PCMHIVE)(_CmHive_))->HiveIsLoading) || (CmpTestRegistryLockExclusive() == TRUE) )
#define ASSERT_CM_LOCK_NOT_OWNED()     ASSERT( (CmpTestRegistryLock() == FALSE)  && (CmpTestRegistryLockExclusive() == FALSE) )
#else
...
// Determines whether the Current Control Set used during booting is cloned in order to fully preserve it for being saved as the LKG Control Set.
...
//  Same structure used for KEY_ROOT and KEY objects.  This is the Cm defined part of the object.
...
// A post block can have attached a keybody which has to be dereferenced when the post block goes out of scope.
// This structure allows the implementation of keybody "delayed dereferencing". (see CmpPostNotify for comments)
...
//  Any one open instance (CM_KEY_BODY) will refer to at most one notify block.
//  A given key control block may have as many notify blocks referring to it as there are CM_KEY_BODYs referring to it.
...
//  Each time an event is posted against the notify, the waiter described by the post block is signaled.  (i.e. APC enqueued, event signaled, etc.)
...
NTSTATUS CmUnloadKey(IN PCM_KEY_CONTROL_BLOCK    Kcb, IN ULONG Flags, IN ULONG ControlFlags);
...
NTSTATUS CmpDoAccessCheckOnSubtree(PHHIVE HiveToCheck, HCELL_INDEX Cell, KPROCESSOR_MODE PreviousMode, ACCESS_MASK DesiredAccess, BOOLEAN CheckRoot);
...
#if DBG
BOOLEAN CmpTestRegistryLock(VOID);
BOOLEAN CmpTestRegistryLockExclusive(VOID);
#endif
...
// BOOLEAN CmpCopyTree(PHHIVE      SourceHive, HCELL_INDEX SourceCell, PHHIVE      TargetHive, HCELL_INDEX TargetCell);
...
// BOOLEAN CmpMergeTrees(PHHIVE      SourceHive, HCELL_INDEX SourceCell, PHHIVE      TargetHive, HCELL_INDEX TargetCell);
...
NTSTATUS CmpInitializeRegistryNode(
...
PCM_POST_BLOCK CmpAllocatePostBlock(
...
// structure definitions shared with the boot loader to select the hardware profile.
...
HCELL_INDEX CmpFindProfileOption(
    IN PHHIVE Hive,
    IN HCELL_INDEX ControlSet,
...
VOID CmpRemoveFromHiveFileList(PUNICODE_STRING HiveName);
NTSTATUS CmpInitHiveFromFile(
...
NTSTATUS CmpCreateHwProfileFriendlyName (
    IN HANDLE IDConfigDB,
...
// Note that the cell index for value list is stored in the cached allocation.
// It is not used now but may be in further performance optimization.
...
/* - macro
VOID CmpSetUpKcbValueCache(PCM_KEY_CONTROL_BLOCK   KeyControlBlock, ULONG                   Count, ULONG_PTR               ValueList)
*/
...
VALUE_SEARCH_RETURN_TYPE CmpGetValueKeyFromCache(
    IN PCM_KEY_CONTROL_BLOCK KeyControlBlock,
    IN PCELL_DATA           List,
    IN ULONG                Index,
...
VALUE_SEARCH_RETURN_TYPE CmpFindValueByNameFromCache(
    IN PCM_KEY_CONTROL_BLOCK    KeyControlBlock,
    IN PUNICODE_STRING          Name,
    OUT PPCM_CACHED_VALUE       *ContainingList,
    OUT ULONG                   *Index,
...
VALUE_SEARCH_RETURN_TYPE CmpGetValueDataFromCache(
    IN PCM_KEY_CONTROL_BLOCK KeyControlBlock,
...
VALUE_SEARCH_RETURN_TYPE CmpQueryKeyValueData(
    PCM_KEY_CONTROL_BLOCK KeyControlBlock,
...
NTSTATUS CmpAcquireFileObjectForFile(IN  PCMHIVE CmHive, IN HANDLE FileHandle, OUT PFILE_OBJECT *FileObject);
...
NTSTATUS CmpPinCmView(IN  PCMHIVE CmHive, PCM_VIEW_OF_FILE CmView);
VOID CmpUnPinCmView(IN  PCMHIVE CmHive, IN  PCM_VIEW_OF_FILE CmView, IN  BOOLEAN SetClean, IN  BOOLEAN MapIsValid);
NTSTATUS CmpMapThisBin(PCMHIVE CmHive, HCELL_INDEX Cell, BOOLEAN Touch);
...
VOID CmpAddToDelayedClose(IN PCM_KEY_CONTROL_BLOCK    kcb, IN BOOLEAN                  RegLockHeldEx);
...
VOID CmpAssignSecurityToKcb(IN PCM_KEY_CONTROL_BLOCK Kcb, IN HCELL_INDEX SecurityCell, IN BOOLEAN SecurityLocked);
...
NTSTATUS CmpCmdHiveOpen(POBJECT_ATTRIBUTES FileAttributes, PSECURITY_CLIENT_CONTEXT ImpersonationContext, PBOOLEAN Allocate, PCMHIVE *NewHive, ULONG CheckFlags);
...
NTSTATUS CmRenameKey(IN PCM_KEY_CONTROL_BLOCK KeyControlBlock, IN UNICODE_STRING NewKeyName, IN KPROCESSOR_MODE PreviousMode);
...
NTSTATUS CmpCallCallBacks (IN REG_NOTIFY_CLASS Type, IN PVOID Argument, IN BOOLEAN Wind, IN REG_NOTIFY_CLASS PostType, IN PVOID Object);
...
#define CmUpcaseUnicodeChar(c)          ( ((c) < 'a') ? (c) : ( ((c) > 'z') ? RtlUpcaseUnicodeChar(c) : ((c) - ('a'-'A')) ) )
...
#define CmpLockKCBExclusive(Kcb)            ExAcquirePushLockExclusive(&(GET_KCB_HASH_ENTRY_LOCK((Kcb))));GET_KCB_HASH_ENTRY_OWNER(Kcb) = KeGetCurrentThread()
#define CmpUnlockKCB(Kcb)                   GET_KCB_HASH_ENTRY_OWNER(Kcb) = NULL;ExReleasePushLock(&(GET_KCB_HASH_ENTRY_LOCK((Kcb))))
...
#define CmpUnlockHashEntry(ConvKey)         GET_HASH_ENTRY(CmpCacheTable,(ConvKey)).Owner = NULL;ExReleasePushLock(&(GET_HASH_ENTRY(CmpCacheTable,(ConvKey)).Lock))
...
BOOLEAN CmpKCBLockForceAcquireAllowed(ULONG Index1, ULONG Index2, ULONG NewIndex);
VOID CmpLockTwoHashEntriesExclusive(ULONG   ConvKey1, ULONG   ConvKey2);
VOID CmpLockTwoHashEntriesShared(ULONG   ConvKey1, ULONG   ConvKey2);
VOID CmpUnlockTwoHashEntries(ULONG   ConvKey1, ULONG   ConvKey2);
// DelayDerefKCB engine
VOID CmpInitDelayDerefKCBEngine();
VOID CmpRunDownDelayDerefKCBEngine(PCM_KEY_CONTROL_BLOCK    KeyControlBlock, BOOLEAN                  RegLockHeldEx);
