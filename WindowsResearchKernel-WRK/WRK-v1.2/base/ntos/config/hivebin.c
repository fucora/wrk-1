...
/*
Routine Description:
    Grows either the Stable or Volatile storage of a hive by adding a new bin.
    Bin will be allocated space in Stable store (e.g. file) if Type == Stable.
    Memory image will be allocated and initialized.
...
Arguments:
    ...
    NewSize - size of the object caller wishes to put in the hive.  New bin will be at least large enough to hold this.
...
*/
{
    ...
    //  Round size up to account for bin overhead.  Caller should have accounted for cell overhead.
    NewSize += sizeof(HBIN);
    ...
    //  (it is not illegal to have bins smaller than the page size, but it is less efficient)
...
Retry:
    ...
    while (Entry != &Hive->Storage[Type].FreeBins) {
        ...
        if ((FreeBin->Size >= NewSize) && ((CmHive->GrowOnlyMode == FALSE) || (Type == Volatile))) {
            ...
            if (FreeBin->Flags & FREE_HBIN_DISCARDABLE) {
                // HBIN is still in memory, don't need any more allocs, just fill in the block addresses.
                Me = NULL;
    ...
    if ((Entry == &Hive->Storage[Type].FreeBins) && (TotalDiscardedSize >= NewSize)) {
        // No sufficiently large discarded bin was found, but the total discarded space is large enough.
        // Attempt to coalesce adjacent discarded bins into a larger bin and retry.
    ...
    if (Entry != &Hive->Storage[Type].FreeBins) {
        if (Type == Volatile) {
            ...
            // Note we use ExAllocatePool directly here to avoid charging quota for this bin again.
            // When a bin is discarded, its quota is not returned.
            // This prevents sparse hives from requiring more quota after a reboot than on a running system.
            ...
            // for Stable, map the view containing the bin in memory and fix the map
    ...
    if (Entry != &Hive->Storage[Type].FreeBins) {
        // found a discarded HBIN we can use, just fill in the map and we are done.
        for (i = 0; i < NewSize; i += HBLOCK_SIZE) {
            Me = HvpGetCellMap(Hive, FreeBin->FileOffset + i + (Type*HCELL_TYPE_MASK));
            VALIDATE_CELL_MAP(__LINE__, Me, Hive, FreeBin->FileOffset + i + (Type*HCELL_TYPE_MASK));
    ...
    if ((HvpCheckViewBoundary(CheckLength, CheckLength + NewSize - 1) == FALSE) &&
        ...
        // of this CM_VIEW_SIZE window. Allocate it from the next CM_VIEW_SIZE window and add the remaining of this to the free bin list
...
/*
Routine Description:
    Walks through the list of discarded bins and attempts to coalesce adjacent discarded bins into one larger bin in order to satisfy an allocation request.

    It doesn't coalesce bins over the CM_VIEW_SIZE boundary.

    It doesn't coalesce bins from paged pool with bins mapped in system cache views.
Arguments:
