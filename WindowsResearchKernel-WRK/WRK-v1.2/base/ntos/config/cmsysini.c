...
Revision History:
     Added CmpSaveBootControlSet & CmpDeleteCloneTree in order to perform some of the LKG work that has been moved into the kernel.
     Modified system initialization to permit operation and LKG control set saves without a CurrentControlSet clone.
*/
...
// Key objects (CmpKeyObjectType) represent open instances of keys in the registry.
// They do not have object names, rather, their names are defined by the registry backing store.

// Master Hive

//  The KEY_NODEs for \REGISTRY, \REGISTRY\MACHINE, and \REGISTRY\USER are stored in a small memory only hive called the Master Hive.
//  All other hives have link nodes in this hive which point to them.
extern   PCMHIVE CmpMasterHive;
extern   BOOLEAN CmpNoMasterCreates;    // Init False, Set TRUE after we're done to prevent random creates in the master hive, which is not backed by a file.

extern   LIST_ENTRY  CmpHiveListHead;   // List of CMHIVEs

extern   POBJECT_TYPE CmpKeyObjectType;// Addresses of object type descriptors:
...
// Write-Control:
//  CmpNoWrite is initially true.
//  When set this way write and flush do nothing, simply returning success.
//  When cleared to FALSE, I/O is enabled.
//  This change is made after the I/O system is started AND autocheck (chkdsk) has done its thing.
...
NTSTATUS CmpCreateObjectTypes(VOID);
...
NTSTATUS CmpSetupPrivateWrite(PCMHIVE             CmHive);
NTSTATUS CmpSetSystemValues(IN PLOADER_PARAMETER_BLOCK LoaderBlock);
NTSTATUS CmpSetNetworkValue(IN PNETWORK_LOADER_BLOCK NetworkLoaderBlock);
...
BOOLEAN CmInitSystem1(__in PLOADER_PARAMETER_BLOCK LoaderBlock)
/*
Routine Description:
    This function is called as part of phase1 init, after the object manager has been initialized, but before IoInit.
    It's purpose is to set up basic registry object operations, 
    and transform data captured during boot into registry format
    (whether it was read from the SYSTEM hive file by the osloader or computed by recognizers.)
    After this call, Nt*Key calls work, but only part of the name space is available and any changes written must be held in memory.

    CmpMachineHiveList entries marked CM_PHASE_1 are available after return from this call, but writes must be held in memory.
    ...
    NOTE:   We do NOT free allocated pool in failure case.
            This is because our caller is going to bugcheck anyway, and having the memory object to look at is useful.

Arguments:
    LoaderBlock - supplies the LoaderBlock passed in from the OSLoader.
        By looking through the memory descriptor list we can find the SYSTEM hive which the OSLoader has placed in memory for us.
...
*/
{
    ...
    // Set the mini NT flag if we are booting into Mini NT environment
    if (InitIsWinPEMode) {
        CmpMiniNTBoot = InitIsWinPEMode;

        // On Remote boot client share the system hives

        // NOTE : We can't assume exclusive access to WinPE remote boot clients.
        // We don't flush anything to system hives in WinPE. All the system hives are loaded in memory in scratch mode
    ...
    // This simply initializes unicode strings so we don't have to bother with it later.
    // This can not fail.
    ...
    ExInitializeResourceLite(&CmpRegistryLock);// Initialize the global registry resource
    ExInitializePushLock(&CmpPostLock);// Initialize the PostList mutex
    ExInitializePushLock(&CmpStashBufferLock); // Initialize the Stash Buffer mutex
    CmpInitializeCache();// Initialize the cache

    // Initialize private allocator
    CmpInitCmPrivateAlloc();
    CmpInitCmPrivateDelayAlloc();

    CmpInitDelayDerefKCBEngine();// delay deref kcb
    CmpInitCallback();// Initialize callback module
    ...
    // try to allocate a stash buffer.
    // if we can't get 1 page this early on, we're in deep trouble, so punt.
    ...
    if (!NT_SUCCESS(status = NtCreateKey(&key1, KEY_READ | KEY_WRITE, &ObjectAttributes, 0, (PUNICODE_STRING)&nullclass, 0, NULL))) {
    ...
    InitializeObjectAttributes(&ObjectAttributes, &CmRegistryUserName, OBJ_CASE_INSENSITIVE, (HANDLE)NULL, SecurityDescriptor);
    if (!NT_SUCCESS(status = NtCreateKey(&key1, KEY_READ | KEY_WRITE, &ObjectAttributes, 0, (PUNICODE_STRING)&nullclass, 0, NULL))) {
        ExFreePool(SecurityDescriptor);
...
/*
Routine Description:
    This function is called to map hive files to hives.  It both maps existing hives to files, and creates new hives from files.

    It operates on files in "\SYSTEMROOT\CONFIG".

    NOTE:   MUST run in the context of the process that the CmpWorker thread runs in.  Caller is expected to arrange this.
...
#define MAX_NAME    128
    ...
    for (i = 0; i < CM_NUMBER_OF_MACHINE_HIVES; i++) {
        ...
        Status = PsCreateSystemThread(&Thread, THREAD_ALL_ACCESS, NULL, 0, NULL, CmpLoadHiveThread, (PVOID)(ULONG_PTR)(ULONG)i);
        if (NT_SUCCESS(Status)) {
            ZwClose(Thread);
        } else {
    ...
    for (i = 0; i < CM_NUMBER_OF_MACHINE_HIVES; i++) {
        ...
        if (CmpMachineHiveList[i].CmHive == NULL) {
            ...
            // Compute the name of the file, and the name to link to in the registry.
            ...
            Status = CmpLinkHiveToMaster(&RegName, NULL, CmpMachineHiveList[i].CmHive2, CmpMachineHiveList[i].Allocate, SecurityDescriptor);
            if (Status != STATUS_SUCCESS) {
...
*/
{
    ...
    // Structure that describes the mapping of generic access rights to object specific access rights for registry key objects.
    ...
    RtlInitUnicodeString(&TypeName, L"Key");// Initialize string descriptor.
...
/*
Routine Description:
    Manually create \REGISTRY in the master hive, create a key object to refer to it, 
    and insert the key object into the root (\) of the object space.
Return Value:
...
*/
{
    ...
    // WARNING: \\REGISTRY is not in pool, so if anybody ever tries to free it, we are in deep trouble.
    //          On the other hand, this implies somebody has removed \\REGISTRY from the root, so we're in trouble anyway.
    SecurityDescriptor = CmpHiveRootSecurityDescriptor();
    InitializeObjectAttributes(&ObjectAttributes, &CmRegistryRootName, OBJ_CASE_INSENSITIVE, (HANDLE)NULL, SecurityDescriptor);
    ...
    if (TempNode == NULL) {
        return FALSE;// we couldn't map the bin containing this cell
    }

    kcb = CmpCreateKeyControlBlock(&(CmpMasterHive->Hive), RootCellIndex, TempNode, NULL, 0, &CmRegistryRootName);// Create the key control block
    if (kcb == NULL) {
        return(FALSE);
    }
    ...
    Status = ObInsertObject(Object, NULL, (ACCESS_MASK)0, 0, NULL, &CmpRegistryRootHandle);// Put the object in the root directory
    if (!NT_SUCCESS(Status)) {
        CmKdPrintEx((DPFLTR_CONFIG_ID, CML_BUGCHECK, "CmpCreateRegistryRoot: "));
    ...
    // We cannot make the root permanent because registry objects in general are not allowed to be. 
    // (They're stable via virtue of being stored in the registry, not the object manager.) 
    // But we never ever want the root to go away.  So reference it.
    if (!NT_SUCCESS(Status = ObReferenceObjectByHandle(CmpRegistryRootHandle, KEY_READ, NULL, KernelMode, &ObjectPointer, NULL))) {
        CmKdPrintEx((DPFLTR_CONFIG_ID, CML_BUGCHECK, "CmpCreateRegistryRoot: "));
...
Arguments:
    ...
    RootCellIndex - supplies pointer to a variable to receive the cell index of the created node.
Return Value:
    TRUE == success, FALSE == failure
*/
{
...
/*
Routine Description:
    The existing, "free floating" hive CmHive describes is linked into the name space at the node named by LinkName.
    The node will be created.
    The hive is assumed to already have an appropriate root node.
Arguments:
    LinkName - supplies a pointer to a unicode string which describes where in the registry name space the hive is to be linked.
    ...
    SecurityDescriptor - supplies a pointer to the security descriptor to be placed on the hive root.
Return Value:
    TRUE == success, FALSE == failure
*/
{
    ...
    // Fill in special ParseContext to indicate that we are creating a link node and opening or creating a root node.
    ...
    if (Allocate) {
        ParseContext.ChildHive.KeyCell = HCELL_NIL;// Creating a new root node
    } else {
        ParseContext.ChildHive.KeyCell = CmHive->Hive.BaseBlock->RootCell;// Opening an existing root node
    }

    // Create a path to the hive
    InitializeObjectAttributes(&ObjectAttributes, LinkName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, (HANDLE)RootDirectory, SecurityDescriptor);
    Status = ObOpenObjectByName(&ObjectAttributes, CmpKeyObjectType, KernelMode, NULL, KEY_READ | KEY_WRITE, (PVOID)&ParseContext, &KeyHandle);
    if (!NT_SUCCESS(Status)) {
    ...
    CmHive->Hive.DirtyFlag = FALSE;// mark hive as "clean"
    ...
    if (NT_SUCCESS(Status)) {
        CmpReportNotify(KeyBody->KeyControlBlock, KeyBody->KeyControlBlock->KeyHive, KeyBody->KeyControlBlock->KeyCell, REG_NOTIFY_CHANGE_NAME);
...
/*
Routine Description:
    This routine guards calling the InterlockedFunction in the passed RegistryValueKey.

    The RegistryValueKey will record the status of the first call to the InterlockedFunction.
    If the system crashes during this call then ValueKey will be left in a state where the InterlockedFunction will not be called on subsequent attempts.
Arguments:
...
*/
{
    ...
    // If the value is a 0, then we haven't tried calling this interlocked function, set the value to a 1 and try it.
    // If the value is a 1, then we crased during an execution of the interlocked function last time, don't try it again.
    // If the value is a 2, then we called the interlocked function before and it worked.  Call it again this time.
    if (Value != 1) {
        if (Value != 2) {
            // This interlocked function is not known to work. 
            // Write a 1 to this value so we can detect if we crash during this call.
            Value = 1;
        ...
        if (Value != 2) {
            // The worker function didn't crash - update the value for this interlocked function to 2.
            Value = 2;
            NtSetValueKey(hSession, &Name, 0L, REG_DWORD, &Value, sizeof(Value));
        }
    } else {
...
*/
{
    ...
    KeRevertToUserAffinityThread();// Restore threads affinity
#endif
}
...
/*
Routine Description:
    Initializes the SYSTEM hive based on the raw hive image passed in from the OS Loader.
Arguments:
    LoaderBlock - Supplies a pointer to the Loader Block passed in by the OS Loader.
Return Value:
    TRUE - it worked
    FALSE - it failed
*/
{
    ...
    RtlInitAnsiString(&TempString, LoaderBlock->LoadOptions);// capture tail of boot.ini line (load options, portable)
    ...
    HiveImageBase = LoaderBlock->RegistryBase;// move the loaded registry into the real registry

    // We need to initialize the system hive as NO_LAZY_FLUSH
    //  - this is just temporary, until we get a chance to open the primary file for the hive.
    // Failure to do so, will result in loss of data on the LazyFlush worker (see CmpFileWrite, the
    //          if (FileHandle == NULL) {
    //              return TRUE;
    //          }
    // test. This might be a problem in 5.0 too, if system crashes between the
    // LazyFlush reported the hive as saved and the moment we actually open the file and save it again
    if (HiveImageBase == NULL) {
        // No memory descriptor for the hive, so we must recreate it.
        Status = CmpInitializeHive(&SystemHive, HINIT_CREATE, HIVE_NOLAZYFLUSH, HFILE_TYPE_LOG, NULL, NULL, NULL, NULL, &CmpSystemFileName, 0);
        if (!NT_SUCCESS(Status)) {
        ...
        // Mark the system hive as volatile, while in MiniNT boot case
        if (CmpShareSystemHives) {
    ...
    SecurityDescriptor = CmpHiveRootSecurityDescriptor();// Create the link node
...
/*
Routine Description:
    Traverses the current SERVICES subtree and creates the list of drivers to be loaded during Phase 1 initialization.
Return Value:
    A pointer to an array of handles, each of which refers to a key in the \Services section of the control set.
    The caller will traverse this array and load and initialize the drivers described by the keys.

    The last key will be NULL.  The array is allocated in Pool and should be freed by the caller.
*/
{
    OBJECT_ATTRIBUTES ObjectAttributes;
    HANDLE SystemHandle;
    ...
    Status = ObReferenceObjectByHandle(SystemHandle, KEY_QUERY_VALUE, CmpKeyObjectType, KernelMode, (PVOID *)(&KeyBody), NULL);
    if (!NT_SUCCESS(Status)) {
    ...
    // Now we have found out the PHHIVE and HCELL_INDEX of the root of the SYSTEM hive, we can use all the same code that the OS Loader does.
    ...
    // We now have a fully sorted and ordered list of drivers to be loaded by IoInit.
    ...
    // Walk the list, opening each registry key and adding it to the table of handles.
    Current = DriverList.Flink;
    DriverCount = 0;
    while (Current != &DriverList) {
        DriverEntry = CONTAINING_RECORD(Current, BOOT_DRIVER_LIST_ENTRY, Link);
        InitializeObjectAttributes(&ObjectAttributes, &DriverEntry->RegistryPath, OBJ_CASE_INSENSITIVE, (HANDLE)NULL, NULL);
...
Arguments:
    ...
    DriverList - Supplies a pointer to the head of the Driver List. 
                 Note that the head of the list is not actually freed, only all the entries in the list.
*/
{
    PLIST_ENTRY         Next;
    PLIST_ENTRY         Current;
...
NTSTATUS CmpInitHiveFromFile(IN PUNICODE_STRING FileName, IN ULONG HiveFlags, OUT PCMHIVE *CmHive, IN OUT PBOOLEAN Allocate, IN  ULONG CheckFlags)
/*
Routine Description:
...
RetryNoBuffering:
    *CmHive = NULL;
    Status = CmpOpenHiveFiles(FileName, L".LOG", &PrimaryHandle, &LogHandle, &Disposition, &SecondaryDisposition, *Allocate, FALSE, NoBuffering, NULL);
    if (!NT_SUCCESS(Status)) {
        return(Status);
    }
    ...
    Status = CmpInitializeHive(&NewHive, Operation, HiveFlags, FileType, HiveData, PrimaryHandle, LogHandle, NULL, FileName, CheckFlags);
    if (!NT_SUCCESS(Status)) {
        ...
        *CmHive = NewHive;
        ...
        (*CmHive)->FileUserName.Buffer = ExAllocatePoolWithTag(PagedPool, FileName->Length, CM_NAME_TAG | PROTECTED_POOL);
...
/*
Routine Description:
    Write DockID SerialNumber DockState and Capabilities into the given registry key.
*/
{
 ...
 *
Routine Description:
    Create an alias entry in the IDConfigDB database for the given hardware profile.
...
*/
{
    ...
    CmpAddDockingInfo(aliasEntry, ProfileBlock);// Write the standard goo
...
Routine Description:
    This routine sets up the symbolic links from
        \Registry\Machine\System\CurrentControlSet to \Registry\Machine\System\ControlSetNNN
        \Registry\Machine\System\CurrentControlSet\Hardware Profiles\Current to \Registry\Machine\System\ControlSetNNN\Hardware Profiles\NNNN

    based on the value of \Registry\Machine\System\Select:Current. and \Registry\Machine\System\ControlSetNNN\Control\IDConfigDB:CurrentConfig
Arguments:
    None
*/
{
    ...
    // Check to make sure that the key was created, not just opened. 
    // Since this key is always created volatile, it should never be present in the hive when we boot.
    ...
    // If aliasable we need to manufacture another alias entry in the alias table.

    // If the profile information is there and not failed then we should mark the Docking state information:
    ...
    if (NULL != LoaderBlock->Extension) {
        ...
        case HW_PROFILE_STATUS_ALIAS_MATCH:
            Status = CmpAddAliasEntry(IDConfigDB, &extension->Profile, HWProfile);// Create the alias entry for this profile.
            // Fall through
        case HW_PROFILE_STATUS_TRUE_MATCH:
            // Write DockID, SerialNumber, DockState, and Caps into the current Hardware profile.
    ...
    if (signalAcpiEvent) {
        ...
        // This is interesting because our buddies in PnP cannot tell us when we are booting without a dock. 
        // They can only tell us when they see a hot undock.

        // Therefore in the interest of matching a boot undocked with a hot undock, we need to simulate an acpi undock event.
...
/*
Routine Description:
    First, create a new hive, \registry\machine\clone, which will be HIVE_VOLATILE.
    Second, link \Registry\Machine\System\Clone to it.
    Third, tree copy \Registry\Machine\System\CurrentControlSet into \Registry\Machine\System\Clone (and thus into the clone hive.)
    When the service controller is done with the clone hive, it can simply NtUnloadKey it to free its storage.
Arguments:
...
*/
{
    ...
    // Get the security descriptor from the key so we can create the clone tree with the correct ACL.
    ...
    if (Status == STATUS_BUFFER_TOO_SMALL) {
        ...
        if (Security != NULL) {
            Status = NtQuerySecurityObject(CurrentHandle, DACL_SECURITY_INFORMATION, Security, SecurityLength, &SecurityLength);
            if (!NT_SUCCESS(Status)) {
    ...
    if (Disposition != REG_CREATED_NEW_KEY) {
        ...
        // WARNNOTE:
        //      If somebody somehow managed to create a key in our way, they'll thwart last known good.  Tough luck.  Claim it worked and go on.
        Status = STATUS_SUCCESS;
        goto Exit;
    }

    Status = ObReferenceObjectByHandle(CurrentHandle, KEY_READ, CmpKeyObjectType, KernelMode, (PVOID *)(&CurrentKey), NULL);
    if (!NT_SUCCESS(Status)) {
        CmKdPrintEx((DPFLTR_CONFIG_ID, CML_BUGCHECK, "CM: CmpCloneControlSet: couldn't reference CurrentHandle %08lx\n", Status));
        goto Exit;
    }

    Status = ObReferenceObjectByHandle(CloneHandle, KEY_WRITE, CmpKeyObjectType, KernelMode, (PVOID *)(&CloneKey), NULL);
    if (!NT_SUCCESS(Status)) {
        CmKdPrintEx((DPFLTR_CONFIG_ID, CML_BUGCHECK, "CM: CmpCloneControlSet: couldn't reference CurrentHandle %08lx\n", Status));
...
/*
Routine Description:
   This routine is responsible for saving the control set used to accomplish the latest boot into a different control set
   (presumably so that the different control set may be marked as the LKG control set).

   This routine is called from NtInitializeRegistry when a boot is accepted via that routine.
Arguments:
   ControlSetNum - The number of the control set that will be used to save the boot control set.
Return Value:
   NTSTATUS result code from call, among the following:

      STATUS_SUCCESS - everything worked perfectly
      STATUS_REGISTRY_CORRUPT - could not save the boot control set, 
                                it is likely that the copy or sync operation used for this save failed and some part of the boot control set was not saved.
*/
{
    ...
    InitializeObjectAttributes(&Attributes, &CmRegistryMachineSystemCurrentControlSet, OBJ_CASE_INSENSITIVE, NULL, NULL);// Figure out where the boot control set is   
    Status = NtOpenKey(&BootHandle, KEY_READ, &Attributes);// Open the boot control set
    if (!NT_SUCCESS(Status)) return(Status);

    // We may be saving the boot control set into a brand new tree that we will create.
    // If this is true, then we will need to create the root node of this tree below and give it the right security descriptor.
    // So, we fish the security descriptor out of the root node of the boot control set tree.
    Status = NtQuerySecurityObject(BootHandle, DACL_SECURITY_INFORMATION, NULL, 0, &SecurityLength);
    if (Status == STATUS_BUFFER_TOO_SMALL) {
        Security = ExAllocatePool(PagedPool, SecurityLength);
        if (Security != NULL) {
            Status = NtQuerySecurityObject(BootHandle, DACL_SECURITY_INFORMATION, Security, SecurityLength, &SecurityLength);
            if (!NT_SUCCESS(Status)) {
                ExFreePool(Security);
                Security = NULL;
            }
    ...
    Status = ObReferenceObjectByHandle(SavedBootHandle, KEY_WRITE, CmpKeyObjectType, KernelMode, (PVOID *)(&SavedBootKey), NULL);
    if (!NT_SUCCESS(Status)) {
        ObDereferenceObject((PVOID)BootKey);
        goto Exit;
    }

    CmpLockRegistryExclusive();// Lock the registry and do the actual saving

    if (Disposition == REG_CREATED_NEW_KEY) {
        PCM_KEY_NODE Node;

        // If we are saving to a control set that we have just created, 
        // it is most efficient to just copy the boot control set tree into the new control set.

        // N.B. We copy the volatile keys only if we are using a clone and thus our boot control set tree is composed only of volatile keys.
        CopyRet = CmpCopyTreeEx(
            BootKey->KeyControlBlock->KeyHive,
        ...
        // If we are saving to a control set that already exists then its likely that this control set is nearly identical to the boot control set (control sets don't change much between boots).

        // Furthermore, the control set we are saving to must be old and hence has not been modified at all since it ceased being a current control set.

        // Thus, it is most efficient for us to simply synchronize the target control set with the boot control set.

        // N.B. We sync the volatile keys only if we are using a clone for the same reasons as stated above.
    ...
    // Check if the Copy/Sync succeeded and adjust our return code accordingly.
    if (CopyRet) {
        Status = STATUS_SUCCESS;
    } else {
        Status = STATUS_REGISTRY_CORRUPT;
...
VOID CmBootLastKnownGood(__in ULONG ErrorLevel)
/*
Routine Description:
    ...
    The actual result is based on the value of ErrorLevel:
        ...
        SEVERE - If not booting LastKnownGood, will switch to LastKnownGood and reboot the system.
                 If already booting LastKnownGood, will return.  Boot should proceed.
        CRITICAL - If not booting LastKnownGood, will switch to LastKnownGood and reboot the system.
                 If already booting LastKnownGood, will bugcheck.
Arguments:
    ErrorLevel - Supplies the severity level of the failure
Return Value:
    None.  If it returns, boot should proceed.  May cause the system to reboot.
*/
{
    ARC_STATUS Status;
...
BOOLEAN CmIsLastKnownGoodBoot(VOID)
/*
Routine Description:
    Determines whether the current system boot is a LastKnownGood boot or not.
    It does this by comparing the following two values:

        \registry\machine\system\select:Current
        \registry\machine\system\select:LastKnownGood

    If both of these values refer to the same control set, and this control set is different from:
...
*/
{
    ...
    CmpCreatePredefined(Perflib, L"009", HKEY_PERFORMANCE_TEXT);// Always create the predefined keys for the english language

    // If the default language is not english, create a predefined key for that, too.
...
/*
Routine Description:
    Creates a special key that will always return the given predefined handle instead of a real handle.
Arguments:
    Root - supplies the handle the keyname is relative to
    KeyName - supplies the name of the key.
    PredefinedHandle - supplies the predefined handle to be returned when this key is opened.
*/
{
    OBJECT_ATTRIBUTES ObjectAttributes;
    CM_PARSE_CONTEXT ParseContext;
    ...
    Status = ObOpenObjectByName(&ObjectAttributes, CmpKeyObjectType, KernelMode, NULL, KEY_READ, (PVOID)&ParseContext, &Handle);
...
*/
{
    ...
    // Acquire the file object for the primary; This should be called AFTER the cache has been initialized.
...
*/
{
    ...
    CmpMachineHiveList[i].ThreadStarted = TRUE;// signal that we have started
    ...
    if (CmpMachineHiveList[i].CmHive == NULL) {
        ...
        if ((!NT_SUCCESS(Status)) || (!CmpShareSystemHives && (CmHive->FileHandles[HFILE_TYPE_LOG] == NULL))) {
            ...
            ExRaiseHardError(STATUS_CANNOT_LOAD_REGISTRY_FILE, 1, 1, (PULONG_PTR)&ErrorParameters, OptionOk, &ErrorResponse);
        }
        ...
        if (!(CmHive->Hive.HiveFlags & HIVE_VOLATILE)) {
            ...
            // First, open the files (Primary and Alternate) that back the hive.
            // Stuff their handles into the CmHive object.
            // Force the size of the files to match the in memory images.
            // Call HvSyncHive to write changes out to disk.
        ...
        retryNoBufering:
            ...
            fatal:
                ...
                // WARNNOTE
                // We've just told the user that something essential, like the SYSTEM hive, is hosed. 
                // Don't try to run, we just risk destroying user data.  Punt.
            ...
            // When an in-memory hive is opened with no backing file, ClusterSize is assumed to be 1.
            // When the file is opened later (for the SYSTEM hive) we need to update this field in the hive if we are
            ...
            if (CmHive->Hive.Cluster != ClusterSize) {
                ...
                // Since a cluster in the dirty vector must be either completely dirty or completely clean, go through the
                // dirty vector and mark all clusters that contain a dirty logical sector as completely dirty.
                PRTL_BITMAP  BitMap;
                ULONG        Index;
            ...
            if (CmHive->Hive.BaseBlock->BootRecover != 0) {
                ...
                HvSyncHive((PHHIVE)CmHive);// we only need to flush the hive when the loader has recovered it
            }
            ...
            if (CmpCannotWriteConfiguration) {
                CmpDiskFullWarning();// The system disk is full; Give user a chance to log-on and make room
            }
    ...
    if (LocalWorkerIncrement == CM_NUMBER_OF_MACHINE_HIVES) {
        KeSetEvent(&CmpLoadWorkerEvent, 0, FALSE);// this was the last thread (the lazyest); signal the main thread
    }
    ...
    if ((LocalWorkerIncrement == (CM_NUMBER_OF_MACHINE_HIVES - 1)) && // there is one more thread
        ...
        KeSetEvent(&CmpLoadWorkerDebugEvent, 0, FALSE);// wake up the thread to be debugged
    }
...
/*
Routine Description:
    This function will save the information in the Network Loader Block to the registry.
Arguments:
    NetworkLoaderBlock - Supplies a pointer to the network loader block that was created by the OS Loader.
*/
{
    NTSTATUS status;
    OBJECT_ATTRIBUTES objectAttributes;
    ...
    status = NtSetValueKey(handle, &string, 0, REG_BINARY, NetworkLoaderBlock->DHCPServerACK, NetworkLoaderBlock->DHCPServerACKLength);
    if (!NT_SUCCESS(status)) {
    ...
    status = NtSetValueKey(handle, &string, 0, REG_BINARY, NetworkLoaderBlock->BootServerReplyPacket, NetworkLoaderBlock->BootServerReplyPacketLength);
    if (!NT_SUCCESS(status)) {
...
/*
Routine Description:
    This function will save the system start information to the registry.
Arguments:
...
VOID CmpMarkCurrentValueDirty(IN PHHIVE SystemHive, IN HCELL_INDEX RootCell)
{
    ...
    if (Node == NULL) {
        return;// we couldn't map a view for the bin containing this cell
    }
