...
Abstract:
    This module contains the delete object method (used to delete key control blocks  when last handle to a key is closed,
    and to delete keys marked for deletetion when last reference to them goes away.)
*/
...
*/
{
    ...
    if (KeyBody->Type == KEY_BODY_TYPE) {
        ...
        if (KeyControlBlock != NULL) {
            CmpFlushNotify(KeyBody, FALSE);// Clean up any outstanding notifies attached to the KeyBody
            ...
            // If the key was deleted, it has already been removed from the hash table, and relevant notifications posted then as well.
            ...
            DelistKeyBodyFromKCB(KeyBody, FALSE);// Replace this with the definition so we avoid dropping and reacquiring the lock
            ...
            if (!KeyControlBlock->Delete) {
                ...
                if (IsHiveFrozen(CmHive)) {
                    DoUnloadCheck = TRUE;// unload is pending for this hive;
                }
            }
        ...
        if (KeyControlBlock != NULL) {
            CmHive = (PCMHIVE)CONTAINING_RECORD(KeyControlBlock->KeyHive, CMHIVE, Hive);
            if (IsHiveFrozen(CmHive)) {
                DoUnloadCheck = TRUE;// unload is pending for this hive; we shouldn't put the kcb in the delay close table
            }
            CmpDereferenceKeyControlBlock(KeyControlBlock);
        }
    }

    if (DoUnloadCheck == TRUE) {// if a handle inside a frozen hive has been closed, we may need to unload the hive
    ...
    CmPostCallbackNotification(RegNtPostKeyHandleClose, NULL, STATUS_SUCCESS);// just a notification; disregard the return status
}
...
VOID CmpDoQueueLateUnloadWorker(IN PCMHIVE CmHive)
{
    ...
    if ((CmHive->RootKcb->RefCount == 1) && (CmHive->UnloadWorkItem == NULL)) {
        // the only reference on the rookcb is the one that we artificially created queue a work item to late unload the hive
...
/*
Routine Description:
    "Late" unloads the hive; If nothing goes badly wrong (i.e. insufficient resources), this function should succeed
Arguments:
...
*/
{
    ...
    // hive is the parameter to this worker; make sure we free the work item allocated by CmpDeleteKeyObject
    CmHive = (PCMHIVE)Hive;
    ...
    if (CmHive->Frozen == FALSE) {
        // another thread mounted the exact same hive in the exact same place,
        // hence unfreezing the hive we've done the cleanup part (free the workitem) nothing more to do.
    ...
    if (RootKcb->RefCount > 1) {
        // somebody else must've gotten in between dropping/reacquiring the reglock and opened a handle inside this hive; bad luck, we can't unload
        CmpUnlockRegistry();
        return;
    }
