...
Abstract:
    ...
    The routines in this module are not independent enough to be linked into any other program.  The routines in cmsubs2.c are.
*/
...
VOID CmpDumpOneKeyBody(IN PCM_KEY_CONTROL_BLOCK kcb, IN PCM_KEY_BODY KeyBody, IN PUNICODE_STRING Name, IN PVOID Context);
VOID CmpDumpKeyBodyList(IN PCM_KEY_CONTROL_BLOCK kcb, IN PULONG Count, IN PVOID Context);
BOOLEAN CmpRehashKcbSubtree(PCM_KEY_CONTROL_BLOCK   Start, PCM_KEY_CONTROL_BLOCK   End);
...
VOID CmpDumpOneKeyBody(IN PCM_KEY_CONTROL_BLOCK kcb, IN PCM_KEY_BODY KeyBody, IN PUNICODE_STRING Name, IN PVOID Context)
{
    ASSERT_KEY_OBJECT(KeyBody);// sanity check: this should be a KEY_BODY
    ...
    if (!Context) {
        ...
        // NtQueryOpenSubKeysEx: build up the return buffer; make sure we only touch it inside a try except as it's user-mode buffer
        ...
        if (QueryContext->KeyBodyToIgnore != KeyBody) {
            ...
            if (NT_SUCCESS(QueryContext->StatusCode)) {
                ...
                if ((QueryContext->UsedLength + SizeNeeded) > QueryContext->BufferLength) {
                    QueryContext->StatusCode = STATUS_BUFFER_OVERFLOW;// buffer not big enough;
                } else {
                    // we have established we have enough room; create/add a new entry to the key array and build up unicode name buffer. copy key name to it.
                    // array elements are at the beginning of the user buffer, while name buffers start at the end and continue backwards, as long as there is enough room.
                    try {
                        ...
                        RtlCopyMemory(SubkeysInfo->KeyArray[SubkeysInfo->Count].KeyName.Buffer, Name->Buffer, Name->Length);
...
VOID CmpDumpKeyBodyList(IN PCM_KEY_CONTROL_BLOCK kcb, IN PULONG Count, IN PVOID Context)
{
    ...
    if (kcb->RefCount == 0) {// this kcb is in the delay close, ignore it.        
    ...
    if ((IsListEmpty(&(kcb->KeyBodyListHead)) == TRUE) && (KeyBodyArrayEmpty == TRUE)) {
        return;// Nobody has this subkey open, but for sure some subkey must be open. nicely return.
    }
    ...
    while (KeyBody != (PCM_KEY_BODY)(&(kcb->KeyBodyListHead))) {
        ...
        CmpDumpOneKeyBody(kcb, KeyBody, Name, Context);// dump it.        
        (*Count)++;// count it
    ...
    for (i = 0; i < CMP_LOCK_FREE_KEY_BODY_ARRAY_SIZE; i++) {
        ...
        if ((KeyBody != NULL) && ((ULONG_PTR)KeyBody != CMP_FAST_KEY_BODY_ARRAY_DUMP) && ((ULONG_PTR)KeyBody != CMP_FAST_KEY_BODY_ARRAY_FLUSH)) {
            // avoid races
            if (InterlockedCompareExchangePointer(&(kcb->KeyBodyArray[i]), (PVOID)CMP_FAST_KEY_BODY_ARRAY_DUMP, KeyBody) == KeyBody) {
                CmpDumpOneKeyBody(kcb, KeyBody, Name, Context);// dump it.                
                (*Count)++;// count it                
                InterlockedCompareExchangePointer(&(kcb->KeyBodyArray[i]), KeyBody, (PVOID)CMP_FAST_KEY_BODY_ARRAY_DUMP);// set it back
            }
        }
    }
...
VOID CmpFlushNotifiesOnKeyBodyList(IN PCM_KEY_CONTROL_BLOCK kcb, IN BOOLEAN LockHeld)
/*
Routine Description:
      Flushes notifications on all key_bodies linked to this kcb and sets the key object flag.
*/
{
    PCM_KEY_BODY    KeyBody;
    ULONG           i;
...
Again:
    ...
    for (i = 0; i < CMP_LOCK_FREE_KEY_BODY_ARRAY_SIZE; i++) {
        ...
        if ((KeyBody != NULL) && ((ULONG_PTR)KeyBody != CMP_FAST_KEY_BODY_ARRAY_DUMP) && ((ULONG_PTR)KeyBody != CMP_FAST_KEY_BODY_ARRAY_FLUSH)) {
            // avoid races
            if (InterlockedCompareExchangePointer(&(kcb->KeyBodyArray[i]), (PVOID)CMP_FAST_KEY_BODY_ARRAY_FLUSH, KeyBody) == KeyBody) {
                ASSERT_KEY_OBJECT(KeyBody);// sanity check: this should be a KEY_BODY

                // flush any notifies that might be set on it
                if (KeyBody->NotifyBlock) {
                    // if we hold exclusive registry lock, we are safe; defer delete can't race with us otherwise add an extra reference on the key body so it won't go away.
                    if (LockHeld) {
                        CmpFlushNotify(KeyBody, LockHeld);
                        ASSERT(KeyBody->NotifyBlock == NULL);
                    } else if (ObReferenceObjectSafe(KeyBody)) {
                ...
                InterlockedCompareExchangePointer(&(kcb->KeyBodyArray[i]), KeyBody, (PVOID)CMP_FAST_KEY_BODY_ARRAY_FLUSH);
            }
        }
    }
}
...
VOID CmpCleanUpKCBCacheTable(PCM_KEY_CONTROL_BLOCK KeyControlBlock, BOOLEAN RegLockHeldEx)
/*
Routine Description:
...
*/
{
    ...
    ASSERT((KeyControlBlock && CmpIsKCBLockedExclusive(KeyControlBlock) && ((KeyControlBlock->ParentKcb == NULL) || (CmpIsKCBLockedExclusive(KeyControlBlock->ParentKcb))))
...
TryAgain:
    ...
    for (i = 0; i < CmpHashTableSize; i++) {
        ...
        while (*Current) {
            ...
            if ((kcb->RefCount == 0) && ((KeyControlBlock == NULL) || (KeyControlBlock->KeyHive == kcb->KeyHive))) { // only interested in kcbs below this hive's root
...
ULONG CmpSearchForOpenSubKeys(PCM_KEY_CONTROL_BLOCK KeyControlBlock, SUBKEY_SEARCH_TYPE SearchType, BOOLEAN RegLockHeldEx, PVOID SearchContext)
/*
Routine Description:
...
Arguments:
    ...
    SearchType - the type of the search
        ...
        SearchAndDeref - Forces the keys underneath the Key referenced KeyControlBlock to be marked as not referenced (see the REG_FORCE_RESTORE flag in CmRestoreKey);
                return value indicates number of keys that could NOT be dereferenced, and thus are still open.
        SearchAndCount - Counts all open subkeys - returns the number of them
        SearchAndTagNoDelayClose - Flag the subkey so as not to place it in the delayed closed table.
...
*/
{
    ...
    ASSERT((CmpIsKCBLockedExclusive(KeyControlBlock) && ((KeyControlBlock->ParentKcb == NULL) || CmpIsKCBLockedExclusive(KeyControlBlock->ParentKcb))) ||
        (CmpTestRegistryLockExclusive() == TRUE));
                ...
                if (kcb->TotalLevels > KeyControlBlock->TotalLevels) {
                    ...
                    if (Parent == KeyControlBlock) {
                        ...
                        if (SearchType == SearchIfExist) {
                            ...
                            // don't mess with read only kcbs; this prevents a potential hack when trying to FORCE_RESTORE over WPA keys.
...
ULONG CmpComputeKcbConvKey(PCM_KEY_CONTROL_BLOCK   KeyControlBlock)
/*
Routine Description:
    Computes the convkey for this kcb based on the NCB and its parent ConvKey
Arguments:
    KeyControlBlock - Supplies the key control block for the key for which the ConvKey is to be calculated
Return Value:
    The new ConvKey
Notes:
...
/*
Routine Description:
    Walks the path between End and Start and rehashed all kcbs that need rehashing;
    ...
    (we use the ParentKcb member in the kcb structure for that). I.e. we build a forward path from Start to End
    2.Walks the forward path built at 1, rehashes kcbs whos need rehashing and restores the parent relationship.
...
#endif
    // Step 1: walk the path backwards (using the parentkcb link) and revert it, until we reach Start. It is assumed that Start is an
...
BOOLEAN CmpReferenceKeyControlBlock(PCM_KEY_CONTROL_BLOCK   KeyControlBlock)
{
    ...
    if ((KeyControlBlock->RefCount == 0) && (CmpIsKCBLockedExclusive(KeyControlBlock) == FALSE) && (CmpTryConvertKCBLockSharedToExclusive(KeyControlBlock) == FALSE)) {
        ...
        if (KeyControlBlock->DelayedCloseIndex == 1) {
            ...
            ASSERT((KeyControlBlock->DelayedCloseIndex == CmpDelayedCloseSize) || (KeyControlBlock->DelayedCloseIndex == 0));
        }
    }
...
PCM_NAME_CONTROL_BLOCK CmpGetNameControlBlock(PUNICODE_STRING NodeName, PBOOLEAN        NameUpCase)
{
...
/*
Routine Description:
    rebuilds all the kcb cache values from knode; this routine is intended to be called after a tree sync/copy
Arguments:
...
VOID CmpCleanUpKcbCacheWithLock(PCM_KEY_CONTROL_BLOCK   KeyControlBlock, BOOLEAN                 RegLockHeldEx)
/*
Routine Description:
...
*/
{
    ...
    if (Kcb->RefCount == 0) {
        CmpCleanUpKcbValueCache(Kcb);// First, free allocations for Value/data.        
        CmpDereferenceNameControlBlockWithLock(Kcb->NameBlock);// Free the kcb and dereference parentkcb and nameblock.

        if (Kcb->ExtFlags & CM_KCB_SUBKEY_HINT) {
            ExFreePoolWithTag(Kcb->IndexHint, CM_CACHE_INDEX_TAG | PROTECTED_POOL);// Now free the HintIndex allocation
        }

        ParentKcb = Kcb->ParentKcb;// Save the ParentKcb before we free the Kcb
...
*/
{
    ...
    while (TmpKcb) {
        ...
        Length += sizeof(WCHAR);// Add the space for OBJ_NAME_PATH_SEPARATOR;
        TmpKcb = TmpKcb->ParentKcb;
    }
    ...
    if (FullName) {
        ...
        while (TmpKcb) {
            if ((TmpKcb->KeyHive == NULL) || (TmpKcb->ExtFlags & CM_KCB_KEY_NON_EXIST) || ((!TmpKcb->Delete) && (TmpKcb->KeyCell == HCELL_NIL))) {
            ...
            if (TmpKcb->RealKeyName == NULL) {
                ...
                if (!(TmpKcb->Delete || DeletedPath)) {
                    ...
                    if (KeyNode == NULL) {// could not allocate view                        
                        ExFreePoolWithTag(FullName, CM_NAME_TAG | PROTECTED_POOL);
                        FullName = NULL;
                        break;
                    }
            ...
            if (KeyNode && (!(TmpKcb->Flags & (KEY_HIVE_ENTRY | KEY_HIVE_EXIT)))) {
                ...
                ASSERT(((KeyNode->Flags&KEY_COMP_NAME) && (TmpKcb->NameBlock->Compressed)) || ((!(KeyNode->Flags&KEY_COMP_NAME)) && (!(TmpKcb->NameBlock->Compressed))));
            }
#endif //DBG
...
PCM_KEY_CONTROL_BLOCK CmpCreateKeyControlBlock(
    PHHIVE          Hive,
    HCELL_INDEX     Cell,
    PCM_KEY_NODE    Node,
    PCM_KEY_CONTROL_BLOCK ParentKcb,
...
Routine Description:
    ...
    Full path will be BaseName + '\' + KeyName, unless BaseName NULL, in which case the full path is simply KeyName.
...
*/
{
    ...
    ASSERT(NodeName.Length > 0);// Manually compute the hash to use.
    ...
    if (kcbmatch != NULL) {
        ...
        if (!CmpReferenceKeyControlBlock(kcb)) {
            ...
            // Just fail the open, they've got enough handles already.
            ...
            if (!(kcb->ExtFlags & (CM_KCB_NO_SUBKEY | CM_KCB_SUBKEY_ONE | CM_KCB_SUBKEY_HINT))) {
                kcb->SubKeyCount = Node->SubKeyCounts[Stable] + Node->SubKeyCounts[Volatile];// SubKeyCount                
                kcb->ExtFlags &= ~CM_KCB_INVALID_CACHED_INFO;// clean up the invalid flag (if any)
            }
        ...
        if (kcb) {
            CmpAssignSecurityToKcb(kcb, Node->Security, FALSE);// Cache the security cells in the kcb
            ...
            if (kcb->NameBlock) {
                ...
                if (FakeKey) {// The KCb to be created is a fake one;                    
...
#endif
            } else {// We have maxed out the ref count on the Name.                
                CmpDereferenceKeyControlBlockWithLock(ParentKcb, FALSE);// First dereference the parent KCB.
                CmpRemoveKeyControlBlock(kcb);
    ...
    if (!(ControlFlags & CMP_CREATE_KCB_KCB_LOCKED)) {
        if (ParentKcb) {// need to lock them both atomically            
            CmpUnlockTwoHashEntries(ConvKey, ParentKcb->ConvKey);
        } else {// need to use ConvKey here since the kcb might not always be available            
...
/*
Routine Description:
    Traverse the kcb tree.
    We will visit all nodes unless WorkerRoutine tells us to stop part way through.
...
*/
{
    ...
    if ((NewRefCount & 0xffff) > 0 && InterlockedCompareExchange((PLONG)&KeyControlBlock->RefCount, NewRefCount, OldRefCount) == OldRefCount) {
        LogKCBReference(KeyControlBlock, 2);
        return;
    }

    ConvKey = KeyControlBlock->ConvKey;// kcb may be freed from under us !
...
VOID CmpDereferenceKeyControlBlockWithLock(PCM_KEY_CONTROL_BLOCK KeyControlBlock, BOOLEAN RegLockHeldEx)
{
    ...
    if ((InterlockedDecrement((PLONG)&KeyControlBlock->RefCount) & 0xffff) == 0) {
        ...
        if ((CmpHoldLazyFlush && (!(KeyControlBlock->ExtFlags & CM_KCB_SYM_LINK_FOUND)) && (!(KeyControlBlock->Flags & KEY_SYM_LINK))) || (KeyControlBlock->ExtFlags & CM_KCB_NO_DELAY_CLOSE)) {
            // Free storage directly so we can clean up junk quickly.            
            CmpCleanUpKcbCacheWithLock(KeyControlBlock, RegLockHeldEx);// Need to free all cached Index List, Index Leaf, Value, etc.
        } else if (!KeyControlBlock->Delete) {
            CmpAddToDelayedClose(KeyControlBlock, RegLockHeldEx);// Put this kcb on our delayed close list.
        } else {// Free storage directly as there is no point in putting this on our delayed close list.            
            CmpCleanUpKcbCacheWithLock(KeyControlBlock, RegLockHeldEx);// Need to free all cached Index List, Index Leaf, Value, etc.
        }
    }
}
...
*/
{
    ...
    CmpRemoveKeyHash(&KeyControlBlock->KeyHash);// Remove the KCB from the hash table
}
...
*/
{
    ...
    HvFreeCell(Hive, Cell);// unmap the cell itself and free it
    return TRUE;
}
...
*/
{
    ...
    while (Current) {
        ...
        if ((KeyHash->ConvKey == Current->ConvKey) && (KeyHash->KeyCell == Current->KeyCell) && (KeyHash->KeyHive == Current->KeyHive)) {
            // Found a match
...
VOID EnlistKeyBodyWithKCB(IN PCM_KEY_BODY   KeyBody, IN ULONG          ControlFlags)
{
    ULONG   i;
    ...
    for (i = 0; i < CMP_LOCK_FREE_KEY_BODY_ARRAY_SIZE; i++) {
        if (InterlockedCompareExchangePointer(&(KeyBody->KeyControlBlock->KeyBodyArray[i]), KeyBody, NULL) == NULL) {
            return;// we're lucky, it worked.
        }
    }
...
VOID DelistKeyBodyFromKCB(IN PCM_KEY_BODY   KeyBody, IN BOOLEAN        LockHeld)
{
    ULONG       i;
    ULONG_PTR   Value;
...
Retry:
    // try the fast path first; no locks required here.
    for (i = 0; i < CMP_LOCK_FREE_KEY_BODY_ARRAY_SIZE; i++) {
        Value = (ULONG_PTR)InterlockedCompareExchangePointer(&(KeyBody->KeyControlBlock->KeyBodyArray[i]), NULL, KeyBody);
        if (Value == (ULONG_PTR)KeyBody) {
            return;// we're lucky, it worked.
        }
        if ((Value == CMP_FAST_KEY_BODY_ARRAY_DUMP) || (Value == CMP_FAST_KEY_BODY_ARRAY_FLUSH)) {
            goto Retry;// other routine is performing work on this array; retry until they are done
        }
    }
