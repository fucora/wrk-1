...
/*
Routine Description:
    Performs the first step in the creation of a registry key.
    This routine checks to make sure the caller has the proper access to create a key here, and allocates space for the child in the parent cell.
    It then calls CmpDoCreateChild to initialize the key and create the key object.

    This two phase creation allows us to share the child creation code with the creation of link nodes.
Arguments:
...
*/
{
    ...
    if (ARGUMENT_PRESENT(Context)) {
        if (Context->CreateOptions & REG_OPTION_BACKUP_RESTORE) {
            BackupRestore = TRUE;// allow backup operators to create new keys
        }

        Context->Disposition = REG_CREATED_NEW_KEY;// Operation is a create, so set Disposition
    }
    ...
    if (BackupRestore == TRUE) {
        // this is a create to support a backup or restore operation, do the special case work
        ...
        if (AccessState->PreviouslyGrantedAccess == 0) {
            ...
            status = STATUS_ACCESS_DENIED;
            CreateAccess = FALSE;// this is not a backup-restore operator; deny the create
        } else {// allow backup operators to create new keys            
            status = STATUS_SUCCESS;
            CreateAccess = TRUE;
        }
    } else {
        // The FullName is not used in the routine CmpCheckCreateAccess,
        CreateAccess = CmpCheckCreateAccess(NULL, SecurityDescriptor, AccessState, AccessMode, AdditionalAccess, &status);
    }

    if (CreateAccess) {
        // Security check passed, so we can go ahead and create the sub-key.
        ...
        status = CmpDoCreateChild(Hive, Cell, SecurityDescriptor, AccessState, Name, AccessMode, Context, ParentKcb, 0, &KeyCell, Object);
        if (NT_SUCCESS(status)) {
            PCM_KEY_NODE KeyNode;

            KeyBody = (PCM_KEY_BODY)(*Object);

            if (!CmpAddSubKey(Hive, Cell, KeyCell)) {// Child successfully created, add to parent's list.
                CmpFreeKeyByCell(Hive, KeyCell, FALSE);// Unable to add child, so free it
            ...
            if (KeyNode == NULL) {
                // we couldn't map the bin containing this cell this shouldn't happen as we successfully marked the cell as dirty
                ASSERT(FALSE);
...
/*
Routine Description:
    Creates a new sub-key.  This is called by CmpDoCreate to create child sub-keys and CmpCreateLinkNode to create root sub-keys.
Arguments:
    Hive - supplies a pointer to the hive control structure for the hive
    ParentCell - supplies cell index of parent cell
    ParentDescriptor - Supplies security descriptor of parent key, for use in inheriting ACLs.
    AccessState - Running security access state information for operation.
    Name - Supplies pointer to a UNICODE string which is the name of the child to be created.
    AccessMode - Access mode of the original caller.
    Context - Supplies pointer to CM_PARSE_CONTEXT structure passed through the object manager.
    BaseName - Name of object create is relative to
    KeyName - Relative name (to BaseName)
...
Return Value:
    STATUS_SUCCESS - sub-key successfully created.  New object is returned in Object, and the new cell's cell index is returned in KeyCell.
...
*/
{
    ...
    StorageType = Stable;// Get allocation type
    ...
    try {
        ...
        *KeyCell = HvAllocateCell(Hive, CmpHKeyNodeSize(Hive, Name), StorageType, HCELL_NIL);// Allocate child cell
        ...
        HvReleaseCell(Hive, *KeyCell);// release the cell right here as the view is pinned
        ...
        if (NT_SUCCESS(Status)) {
            ...
            // We have managed to allocate all of the objects we need to, so initialize them
            ...
            if (Context->Class.Length > 0) {
                ...
                HvReleaseCell(Hive, ClassCell);// release the cell right here as the view is pinned (cell is dirty).
            ...
            // Assign a security descriptor to the object. 
            // Note that since registry keys are container objects, and ObAssignSecurity assumes that the only container object in the world is the ObpDirectoryObjectType,
            // we have to call SeAssignSecurity directly in order to get the right inheritance.
            ...
            SeDeassignSecurity(&NewDescriptor);// Since the security descriptor now lives in the hive, free the in-memory copy

            if (!NT_SUCCESS(Status)) {
                // Note that the dereference will clean up the kcb, so make sure and decrement the allocation count here.

                // Also mark the kcb as deleted so it does not get inappropriately cached.
                kcb->Delete = TRUE;
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            switch (alloc) {
            case 3:
                // Mark KCB as deleted so it does not get inadvertently added to the delayed close list.
                // That would have fairly disastrous effects as the KCB points to storage we are about to free.
                kcb->Delete = TRUE;
                CmpRemoveKeyControlBlock(kcb);
