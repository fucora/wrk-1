...
PCM_KEY_SECURITY_CACHE CmpFindReusableCellFromCache(IN PCMHIVE CmHive, IN HCELL_INDEX SecurityCell, IN ULONG PreviousCount);
...
Routine Description:
    ...
    For speed in the hash, we consider the security descriptor as an array of ULONGs.
    The fragment at the end that is ignored should not affect the collision nature of this hash.
Arguments:
...
/*
Routine Description:
    This routine adds the specified security cell to the cache of the specified hive.
    It takes care of cache allocation (grow) as well.
    At build up time, cache size grows with a PAGE_SIZE, to avoid memory fragmentation.
    After the table is built, it's size is adjusted (most of the hives never add new security cells).
    Then, at run-time, the size grows with 16 entries at a time (same reason)
    The cache is ordered by the cell's index, so we can do a binary search on cells retrieval.
Arguments:
...
Return Value:
    NTSTATUS - STATUS_SUCCESS if the operation is successful and an appropriate error status otherwise (i.e STATUS_INSUFFICIENT_RESOURCES).
Note:
...
*/
{
    ...
    if (CmHive->SecurityCount == CmHive->SecurityCacheSize) {
        ...
        // OBS: this takes care of the first allocation too, as SecurityCount and SecurityCacheSize are both initialized with 0
        ...
        if (BuildUp == TRUE) {
            ...
            // normal case (running time); a new security cell is added; grow the table with a fixed number of increments (to avoid fragmentation, in case of an Office install :-) )
            CmHive->SecurityCacheSize += SECURITY_CACHE_GROW_INCREMENTS;
        }
        CmRetryExAllocatePoolWithTag(PagedPool, CmHive->SecurityCacheSize * sizeof(CM_KEY_SECURITY_CACHE_ENTRY), CM_SECCACHE_TAG | PROTECTED_POOL, CmHive->SecurityCache);
...
Routine Description:
    ...
    Returns the index of the cache entry where the cell is cached or it should be added
Arguments:
    CmHive - the hive to which the security cell belongs
...
/*
Routine Description:
    Adjust the scusrity cache size for the specified hive.
    This function should be called after all the security cells for the hive were cached, in order to give back extra memory used in the process.
Arguments:
    CmHive - the hive to which the security cell belongs
Return Value:
    TRUE - success
...
*/
{
    ...
    if (CmHive->SecurityCount < CmHive->SecurityCacheSize) {
        // cache size is bigger than what we need; there is a good chance nobody will ever add new security cells to this hive, so go on and free the extra space

        // allocate a new buffer with the exact size we need
        CmRetryExAllocatePoolWithTag(PagedPool, CmHive->SecurityCount * sizeof(CM_KEY_SECURITY_CACHE_ENTRY), CM_SECCACHE_TAG | PROTECTED_POOL, Buffer);
        if (Buffer == NULL) {
...
Routine Description:
    ...
    (only if present !)
    For performance (and memory fragmentation) reasons, it does not change (shrink) the cache size.
Arguments:
    CmHive - the hive to which the security cell belongs
...
Routine Description:
    ...
    Then moves it at the end and return a pointer to it. Shifts the array towards the end as we are going to extend the cache

    Security doesn't change too often, so chances are that we will be able to reuse the cells 90% of the time.

    If one cannot be found, the last cell in the array is freed. A new one will be allocated inside CmpAddSecurityCellToCache
Arguments:
    CmHive - the hive to which the security cell belongs
...
*/
{
    ...
    for (i = CmHive->SecurityCount; i < PreviousCount; i++) {
        ...
        Found:
            // we have found one matching the exact size; move it at the end shift to the end one entry as we are going to extend the cache
            //
        ...
        if (SecurityCached->DescriptorLength > Size) {
            if ((SmallestSize == 0) || (SmallestSize > SecurityCached->DescriptorLength)) {
...
/*
Routine Description:
    Rebuilds the security cache by reiterating all security cells and adding them to the cache; this routine is intended for hive refresh operations
Arguments:
    CmHive - the hive to which the security cell belongs
Return Value:
    TRUE or FALSE
*/
{
    ...
    // be smart and reuse the cache; For each cell, we'll iterate through the cache find an entry big enough to accommodate the current cell, move it at the end and then reuse it.
...
BOOLEAN CmpFindMatchingDescriptorCell(
    IN PCMHIVE CmHive,
    IN PSECURITY_DESCRIPTOR SecurityDescriptor,
    IN ULONG Type,
    OUT PHCELL_INDEX MatchingCell,
...
/*
Routine Description:
    This routine attempts to find a security descriptor in the hive that is identical to the one passed in.
    If it finds one, it returns its cell index.

    Obsolete:
    Currently, this routine checks the security descriptors of the parent and siblings of the node to find a match.

    New:
    It looks for the sd in the security cache for this hive.
    This will eliminate duplicates and make the search process faster.
...
Arguments:
    ...
    SecurityDescriptor - Supplies the cooked security descriptor which should be searched for.

    Type - Indicates whether the Security Descriptor that matches must be in Stable or Volatile store
...
Return Value:
    TRUE - Matching security descriptor found.  MatchingCell returns the cell index of the matching security descriptor.
...
*/
{
    ...
    // iterate through the list of colisions for this convkey start with teh first element in list
...
/*
Routine Description:
    Establishes the connection between the KCB and the cached security descriptor.

    As most of the time this is called after the security cell has been linked to the Key Node, and because the binary search starts with the last cell looked up, we will not hit a performance impact here.
Arguments:
...
/*
Routine Description:
    Iterates through the security cache for the specified hive and build the array of mappings.
Arguments:
