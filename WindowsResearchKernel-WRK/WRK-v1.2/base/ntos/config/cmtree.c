...
/*
Routine Description:
    Get the Valve Index Array.
    Check if it is already cached, if not, cache it and return the cached entry.
Arguments:
...
Routine Description:
    Get the Valve Node.
    Check if it is already cached, if not but the index is cached, cache and return the value node.
Arguments:
    Hive - pointer to hive control structure for hive of interest.
...
/*
Routine Description:
    Find a value node given a value list array and a value name.
    It sequentially walk through each value node to look for a match.
    If the array and value nodes touched are not already cached, cache them.
Arguments:
    Hive - pointer to hive control structure for hive of interest
    ChildList - pointer/index to the Value Index array
    ...
    *Value = NULL;
    ...
    if (ChildList->Count != 0) {
        ...
        if (ret != SearchSuccess) {
            // retry with exclusive lock, since we need to update the cache or fail altogether
        ...
        while (TRUE) {
            ...
            if (IndexCached && (*ValueCached) && (HashKey != ((PCM_CACHED_VALUE)((CMP_GET_CACHED_ADDRESS(**ContainingList))))->HashKey)) {
                ...
                try {
                    ...
                    if ((*Value)->Flags & VALUE_COMP_NAME) {
                        Result = CmpCompareCompressedName(Name, (*Value)->Name, (*Value)->NameLength, 0);
                    } else {
...
Note:
    The caller is responsible for freeing the DataPointer when we signal it to him by setting Allocated on TRUE;
    ...
    if (ValueCached) {
        ...
        if (OldEntry->DataCacheType == CM_CACHE_DATA_CACHED) {
            ...
            *DataPointer = (PUCHAR)((ULONG_PTR)ValueKey + OldEntry->ValueKeySize);
            ...
            if ((OldEntry->DataCacheType == CM_CACHE_DATA_TOO_BIG) || (ValueKey->u.KeyValue.DataLength > MAXIMUM_CACHED_DATA)) {
                ...
                *CellToRelease = ValueKey->u.KeyValue.Data;
                ...
                if (NewEntry) {
                    ...
                    RtlCopyMemory((PVOID)&(NewEntry->KeyValue), (PVOID)&(OldEntry->KeyValue), NewEntry->ValueKeySize);
                    ...
                    CmpMakeSpecialPoolReadWrite(OldEntry);// Trying to catch the BAD guy who writes over our pool.
                    *ContainingList = (PCM_CACHED_VALUE)CMP_MARK_CELL_CACHED(NewEntry);
                    CmpMakeSpecialPoolReadOnly(NewEntry);// Trying to catch the BAD guy who writes over our pool.
                    ExFreePool(OldEntry);// Free the old entry
                }
            }
        }
    } else {
