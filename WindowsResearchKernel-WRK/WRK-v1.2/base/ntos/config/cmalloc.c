...
*/
{
    ...
    KeInitializeGuardedMutex(&CmpAllocBucketLock);// init the bucket lock	
    CmpAllocInited = TRUE;
}
...
Routine Description:
    ...
    If it doesn't work (we have maxed out our number of allocs or private allocator is not initialized) try from paged pool
Return Value:
    The  new kcb
*/
{
...
SearchFreeKcb:
    ...
    if (IsListEmpty(&CmpFreeKCBListHead) == FALSE) {
        ...
        AllocPage->FreeCount--;
        ASSERT(kcb->PrivateAlloc == 1);// set when page was allocated
        UNLOCK_ALLOC_BUCKET();
        return kcb;
    }
    ...
    AllocPage->FreeCount = CM_KCBS_PER_PAGE;// set up the page
...
*/
{
    ...
    InsertTailList(&CmpFreeKCBListHead, &(kcb->FreeListEntry));// add kcb to freelist	
    AllocPage = (PCM_ALLOC_PAGE)KCB_TO_ALLOC_PAGE(kcb);// get the page	
    ASSERT(AllocPage->FreeCount != CM_KCBS_PER_PAGE);// not all are free
    AllocPage->FreeCount++;
...
*/
{
    ...
    KeInitializeGuardedMutex(&CmpDelayAllocBucketLock);// init the bucket lock
}
...
SearchFreeItem:
    ...
    if (CmpIsListEmpty(&CmpFreeDelayItemsListHead) == FALSE) {
        ...
        DelayItem = CONTAINING_RECORD(DelayItem, CM_DELAY_ALLOC, ListEntry);
    ...
    for (j = 0; j < CM_DELAYS_PER_PAGE; j++) {
        ...
        InsertTailList(&CmpFreeDelayItemsListHead, &(DelayItem->ListEntry));
        DelayItem->Kcb = NULL;
    }
...
*/
{
    ...
    LOCK_DELAY_ALLOC_BUCKET();
    InsertTailList(&CmpFreeDelayItemsListHead, &(DelayItem->ListEntry));// add kcb to freelist	
    AllocPage = (PCM_ALLOC_PAGE)DELAY_ALLOC_TO_ALLOC_PAGE(DelayItem);// get the page	
    ASSERT(AllocPage->FreeCount != CM_DELAYS_PER_PAGE);// not all are free
    AllocPage->FreeCount++;
