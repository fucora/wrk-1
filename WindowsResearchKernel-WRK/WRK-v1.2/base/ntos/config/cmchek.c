...
Arguments:
    CmHive - supplies a pointer to the CM hive control structure for the hive of interest.
    ...
    HiveCheck - If TRUE, performs hive consistency check too (i.e. checks the bins)
...
Routine Description:
    Check consistency of the registry, from a particular cell on down.
        .   Check that the cell's value list, child key list, class, security are OK.
        .   Check that each value entry IN the list is OK.
        .   Apply self to each child key list.
    This version uses a stack in order to parse the tree "in-depth", but not to touch any key_node.
Arguments:
    Cell - HCELL_INDEX of subkey to work on.
    ParentCell - expected value of parent cell for Cell, unless HCELL_NIL, in which case ignore.
Return Value:
    0 if Hive is OK.  Error return indicator if not.
    RANGE:  4000 - 4999
*/
...
*/
{
    ...
    RemoveThisValue:
        if (CmDoSelfHeal()) {
            ...
            if (HvMarkCellDirty(Hive, KeyCell, FALSE) && HvMarkCellDirty(Hive, Node->ValueList.List, FALSE)) {
    ...
    for (; i < Count; i++) {
        ...
        if (DataLength < CM_KEY_VALUE_SPECIAL_SIZE) {
            ...
            if (CmpIsHKeyValueBig(Hive, DataLength) == TRUE) {
                ...
                if ((BigData->Signature != CM_BIG_DATA_SIGNATURE) || (BigData->Count == 0) || (BigData->List == HCELL_NIL)) {
...
BOOLEAN CmpCheckLexicographicalOrder(IN PHHIVE HiveToCheck, IN HCELL_INDEX  PriorSibling, IN HCELL_INDEX  Current)
{
    ...
    if (CurrentNode->Flags & KEY_COMP_NAME) {
        if (PriorNode->Flags & KEY_COMP_NAME) {
            // most usual case
            if (CmpCompareTwoCompressedNames(PriorNode->Name, PriorNode->NameLength, CurrentNode->Name, CurrentNode->NameLength) >= 0) {
                return FALSE;
            }
        } else {
            ...
            if (CmpCompareCompressedName(&PriorKeyName, CurrentNode->Name, CurrentNode->NameLength, 0) >= 0) {
                return FALSE;
            }
        }
    } else {
        if (PriorNode->Flags & KEY_COMP_NAME) {
            ...
            if (CmpCompareCompressedName(&CurrentKeyName, PriorNode->Name, PriorNode->NameLength, 0) <= 0) {
                return FALSE;
            }
        } else {
            ...
            CurrentKeyName.Length = CurrentNode->NameLength;
            CurrentKeyName.MaximumLength = CurrentKeyName.Length;
            if (RtlCompareUnicodeString(&PriorKeyName, &CurrentKeyName, TRUE) >= 0) {
                return FALSE;
            }
        }
    }
