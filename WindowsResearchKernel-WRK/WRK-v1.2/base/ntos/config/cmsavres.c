...
BOOLEAN CmpShiftAllCells2(PHHIVE Hive, PCMHIVE OldHive, HCELL_INDEX Cell, HCELL_INDEX ParentCell);
BOOLEAN CmpShiftAllCells(PHHIVE NewHive, PCMHIVE OldHive);
...
*/
{
    ...
    status = CmpInitializeHive(&TmpCmHive, HINIT_FILE, 0, HFILE_TYPE_PRIMARY, NULL, FileHandle, NULL, NULL, NULL, CM_CHECK_REGISTRY_CHECK_CLEAN);
...
*/
{
    ...
    status = CmpInitializeHive(&TempHive, HINIT_FILE, 0, HFILE_TYPE_PRIMARY, NULL, FileHandle, NULL, NULL, NULL, CM_CHECK_REGISTRY_CHECK_CLEAN);
    if (!NT_SUCCESS(status)) {
        CmpUnlockRegistry();
        return(status);
    }
...
Routine Description:
    ...
    It is supposed to work fast, Works only when KeyControlBlock is the root of the hive
Arguments:
...
*/
{
    ...
    if ((Hive->HiveFlags & HIVE_NOLAZYFLUSH) && (Hive->DirtyCount != 0) && (CmHive->FileHandles[HFILE_TYPE_PRIMARY] != NULL)) {
        // we really need the lock exclusive in this case as we can't afford somebody else to alter the file
        CmpUnlockKCB(KeyControlBlock);
        CmpUnlockRegistry();
        CmpLockRegistryExclusive();
        if (KeyControlBlock->Delete) {
        ...
        // Note that this will generate weird results if the key being saved is not the root of the hive, since the
        // resulting file will always be a copy of the entire hive, not just the subtree they asked for.
    ...
    CmpLockHiveFlusherExclusive(CmHive);
    newroot = CmpCopyKeyPartial(Hive, Cell, &(TmpCmHive->Hive), HCELL_NIL,          // will force KEY_HIVE_ENTRY set
                                TRUE);
    if (newroot == HCELL_NIL) {
...
*/
{
    ...
    // Merge the two trees. A Merge operation is a sync that obeys the following additional rules:
    //      1. keys the exist in the target tree and does not exist in the source tree remain as they are (don't get deleted)
...
*/
{
    ...
    // Attempt to allocate large buffer for copying stuff around.  If we can't get one, just use the stash buffer.
...
/*
Routine Description:
    Switches relevant storage between the hives.
    Then, rebuilds the kcb mapping according with the mapping stored inside OldHive
Arguments:
    OldHive - Hive to be updated; the one that is currently linked in the registry tree
...
*/
{
    ...
    // now the security cache; we preserve the security cache; only that we go through it and shift cells accordingly
...
/*
Routine Description:
    Parses the logical structure of the registry tree and remaps all cells inside, according to the Spare filed in each bin.
    Updates kcb and security mapping also.
Arguments:
