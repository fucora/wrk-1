...
*/
{
    ...
    *ReturnLength = l + sizeof(OBJECT_NAME_INFORMATION);
    ...
    // The ObjectNameInfo buffer is a usermode buffer, 
    // so make sure we have an exception handler in case a malicious app changes the protection out from under us.

    // Note the object manager is responsible for probing the buffer and ensuring that a top-level exception handler returns the correct error code.
    // We just need to make sure we drop our lock.
    try {
        RtlCopyMemory(t, s, l);
