...
/* -
The Structure:
    Use a 1 or 2 level tree.  Leaf nodes are arrays of pointers to cells, sorted.
    Binary search to find cell of interest.
    Directory node (can be only one) is an array of pointers to leaf blocks.
    ...
    Each block must fix in on HBLOCK_SIZE Cell.
    Allows about 1000 entries.
    Max of 1 million total, best case.
    Worst case something like 1/4 of that.
*/
...
*/
{
    ...
    // Try first the Stable, then the Volatile store.  Assumes that all Volatile refs in Stable space are zeroed out at boot.
    for (i = 0; i < Hive->StorageTypeCount; i++) {
        if (Parent->SubKeyCounts[i] != 0) {
            ...
            ASSERT((IndexRoot->Signature == CM_KEY_INDEX_LEAF) || (IndexRoot->Signature == CM_KEY_FAST_LEAF) || (IndexRoot->Signature == CM_KEY_HASH_LEAF));
...
Arguments:
    ...
    Child - pointer to variable to receive hcell_index of found leaf index block, HCELL_NIL if none.
            Non nil does not necessarily mean the key is present, call FindSubKeyInLeaf to decide that.
Return Value:
    Index in List of last Leaf Cell entry examined.
    If Child != HCELL_NIL, Index is entry that matched, else, index is for last entry we looked at.
    (Target Leaf will be this value plus or minus 1)

    If an error appears while searching the subkey (i.e. a cell cannot be mapped into memory) INVALID_INDEX is returned.
*/
{
    ULONG           High;
    ULONG           Low;
    ...
    if (Result == 0) {
        ...
        *Child = LeafCell;
        ...
        // So regardless of whether it's below the start of this leaf, it would be in this leaf if it were where, so report this leaf.
        *Child = LeafCell;
        ReturnIndex = High;
        goto JustReturn;
    }
...
Routine Description:
    ...
    The supplied index may be either a fast index or a slow one.
Arguments:
    Hive - pointer to hive control structure for hive of interest
...
Return Value:
    Index in List of last cell.  If Child != HCELL_NIL, is offset in list at which Child was found.
    Else, is offset of last place we looked.
...
Arguments:
    ...
    Index - Supplies pointer to either a CM_KEY_INDEX or a CM_KEY_FAST_INDEX. This routine will determine which type of index it is passed.
    Child - pointer to variable to receive hcell_index of found key HCELL_NIL if result != 0
Return Value:
    ...
    if ((Index->Signature == CM_KEY_FAST_LEAF) || (Index->Signature == CM_KEY_HASH_LEAF)) {
        ...
        if (Index->Signature == CM_KEY_FAST_LEAF) {
            ...
            for (i = 0; i < ValidChars; i++) {
                ...
                Result = (LONG)CmUpcaseUnicodeChar(c1) - (LONG)CmUpcaseUnicodeChar(c2);
                if (Result != 0) {
...
/*
Routine Description:
    Helper for CmpFindSubKeyByNumber, Find the Number'th entry in the index, starting from 0.
Arguments:
    Hive - pointer to hive control structure for hive of interest
            ...
            for (j = 0; j < Leaf->Count; j++) {
                if ((Leaf->Signature == CM_KEY_FAST_LEAF) || (Leaf->Signature == CM_KEY_HASH_LEAF)) {
                    ...
                    if (FastIndex->List[j].Cell == Child) {
                        ...
                        FastIndex->Count--;
                        RtlMoveMemory((PVOID)&(FastIndex->List[j]), (PVOID)&(FastIndex->List[j + 1]), (FastIndex->Count - j) * sizeof(CM_INDEX));
                        goto DirtyParent;
                    }
                } else {
                    if (Leaf->List[j] == Child) {
                        ...
                        HvMarkCellDirty(Hive, LeafCell, FALSE);
                        Leaf->Count--;
                        RtlMoveMemory((PVOID)&(Leaf->List[j]), (PVOID)&(Leaf->List[j + 1]), (Leaf->Count - j) * sizeof(HCELL_INDEX));
                        goto DirtyParent;
                    }
                }
            }
        ...
        for (j = 0; j < Index->Count; j++) {
            if ((Index->Signature == CM_KEY_FAST_LEAF) || (Index->Signature == CM_KEY_HASH_LEAF)) {
                ...
                if (Index->List[j] == Child) {
                    // found it!
                    RtlMoveMemory((PVOID)&(Index->List[j]), (PVOID)&(Index->List[j + 1]), (Index->Count - j) * sizeof(HCELL_INDEX));
                    HvMarkCellDirty(Hive, Node->SubKeyLists[Stable], FALSE);
                    Index->Count--;
                    goto DirtyParent;
                }
...
/*
Routine Description:
    Add a new child subkey to the subkey index for a cell.  The child MUST NOT already be present (bugcheck if so.)
...
*/
{
    ...
    if (pcell->SubKeyCounts[Type] == 0) {
        ...
        if (Index == NULL) {
            // we couldn't map the bin containing this cell this shouldn't happen 'cause we just allocated this cell (i.e. bin is PINNED in memory ! )
            ASSERT(FALSE);
            goto ErrorExit;
        }
        ...
        if ((Index->Signature == CM_KEY_FAST_LEAF) && (Index->Count >= (CM_MAX_FAST_INDEX))) {
            // We must change fast index to a slow index to accommodate growth.
            ...
            Index->Signature = CM_KEY_INDEX_LEAF;
        } else if (((Index->Signature == CM_KEY_INDEX_LEAF) || (Index->Signature == CM_KEY_HASH_LEAF)) && (Index->Count >= (CM_MAX_INDEX - 1))) {
            // We must change flat entry to a root/leaf tree
            WorkCell = HvAllocateCell(Hive, sizeof(CM_KEY_INDEX) + sizeof(HCELL_INDEX), // allow for 2
...
Routine Description:
    ...
    Supports both fast and slow leaf indexes and will determine which sort of index the given leaf is.
...
Return Value:
    ...
    Else - cell of Leaf index when are done, caller is expected to set this into Root index or Key body.
*/
{
    PCM_KEY_INDEX   Leaf;
    ...
    Leaf = (PCM_KEY_INDEX)HvGetCell(Hive, LeafCell);
    if (Leaf == NULL) {
        // we couldn't map the bin containing this cell this shouldn't happen as marking dirty means PINNING the view into memory
        ASSERT(FALSE);
        return HCELL_NIL;
    }
    ...
    if (freecount < 1) {
        ...
        if (Leaf == NULL) {
            // we couldn't map the bin containing this cell
            // this shouldn't happen 'cause we just allocated this cell (i.e. bin is PINNED in memory)
            ASSERT(FALSE);
            return HCELL_NIL;
        }
        // release the cell here; as the view is pinned
    ...
    // Select is the index in List of the entry nearest where the new entry should go.
    // Decide whether the new entry goes before or after Offset entry, and then ripple copy and set.
    ...
    if (Select != Leaf->Count) {
        ...
        if (Select != Leaf->Count) {
            ...
            if (FastLeaf != NULL) {
                RtlMoveMemory((PVOID)&(FastLeaf->List[Select + 1]), (PVOID)&(FastLeaf->List[Select]), sizeof(CM_INDEX)*(FastLeaf->Count - Select));
            } else {
                RtlMoveMemory((PVOID)&(Leaf->List[Select + 1]), (PVOID)&(Leaf->List[Select]), sizeof(HCELL_INDEX)*(Leaf->Count - Select));
            }
        }
    }
    if (FastLeaf != NULL) {
...
/*
Routine Description:
    This routine is only called if the subkey index for a cell is NOT simply a single Leaf index block.
    It selects the Leaf index block to which a new entry is to be added.
    It may create this block by splitting an existing Leaf block.
Arguments:
    Hive - pointer to hive control structure for hive of interest
    ...
    RootPointer - pointer to variable to receive address of HCELL_INDEX that points to Leaf block returned as function argument.
                Used for updates.
Return Value:
...
*/
{
    ...
    // Force root to always be dirty, since we'll either grow it or edit it, and it needs to be marked dirty for BOTH cases.  (Edit may not occur until after we leave
    ...
    while (TRUE) {
        ...
        if (LeafCell == HCELL_NIL) {
            ...
            if (Result < 0) {
                ...
                // since new key is not in a Leaf, and is not off the left end of the ResultSelect Leaf, it must be off the right end.
                LeafCell = Index->List[RootSelect];
                Leaf = (PCM_KEY_INDEX)HvGetCell(Hive, LeafCell);
                if (Leaf == NULL) {
                    // we couldn't map the bin containing this cell
                ...
                // No space, see if there's a leaf to the rigth and if it has space
            ...
            // Since newkey cannot already be in tree, it must be greater than the bottom of Leaf and less than the top, therefore it must go in Leaf.  If no space, split it.
            Leaf = (PCM_KEY_INDEX)HvGetCell(Hive, LeafCell);
            if (Leaf == NULL) {
                // we couldn't map the bin containing this cell
                goto ErrorExit;
...
/*
Routine Description:
    Split the Leaf index block specified by RootSelect, causing both of the split out Leaf blocks to appear in the Root index block specified by RootCell.
...
*/
{
    ...
    if (RootSelect < (ULONG)(Root->Count - 1)) {
        RtlMoveMemory((PVOID)&(Root->List[RootSelect + 2]), (PVOID)&(Root->List[RootSelect + 1]), (Root->Count - (RootSelect + 1)) * sizeof(HCELL_INDEX));
    }
...
/*
Routine Description:
    Mark as dirty relevant cells of a subkey index.
    The Leaf that points to TargetKey, and the Root index block, if applicable, will be marked dirty.
    This call assumes we are setting up for a subkey delete.
Arguments:
    Hive - pointer to hive control structure for hive of interest
...
Routine Description:
    ...
    NOTE:   Assumes that caller has marked relevant cells dirty, see CmpMarkIndexDirty.
Arguments:
    Hive - pointer to hive control structure for hive of interest
    ParentKey - key from whose subkey list delete is to be performed
    TargetKey - key being deleted
    ...
    if (Leaf->Count == 0) {
        ...
        if (Root != NULL) {
            ...
            if (Root->Count == 0) {
                ...
                RtlMoveMemory((PVOID)&(Root->List[RootSelect]), (PVOID)&(Root->List[RootSelect + 1]), (Root->Count - RootSelect) * sizeof(HCELL_INDEX));
            }
        ...
        if (Leaf->Signature == CM_KEY_INDEX_LEAF) {
            RtlMoveMemory((PVOID)&(Leaf->List[LeafSelect]), (PVOID)&(Leaf->List[LeafSelect + 1]), (Leaf->Count - LeafSelect) * sizeof(HCELL_INDEX));
        } else {
            FastIndex = (PCM_KEY_FAST_INDEX)Leaf;
            RtlMoveMemory((PVOID)&(FastIndex->List[LeafSelect]), (PVOID)&(FastIndex->List[LeafSelect + 1]), (FastIndex->Count - LeafSelect) * sizeof(CM_INDEX));
        }
    }
...
BOOLEAN CmpUpdateParentForEachSon(PHHIVE          Hive, HCELL_INDEX     Parent)
/*
Routine Description:
    Walks the child's list (both stable and volatile and marks updates the parent link to Parent.
...
*/
{
    ...
    // grab the parent node; this was already marked as dirty, we shouldn't have any problem here;
    ParentNode = (PCM_KEY_NODE)HvGetCell(Hive, Parent);
    if (ParentNode == NULL) {
        // cannot map view; this shouldn't happen as we just allocated this cell (i.e. it should be dirty/pinned into memory)
        ASSERT(FALSE);
        return FALSE;
    }

    // iterate through the child list (both stable and volatile), marking every child dirty; this will pin the cell into memory and we will have no problems changing the parent later on
...
ULONG CmpComputeHashKeyForCompressedName(IN ULONG    HashStart, IN PWCHAR   Source, IN ULONG    SourceLength)
{
    ULONG   i;
...
HCELL_INDEX CmpFindSubKeyByHash(PHHIVE                  Hive, PCM_KEY_FAST_INDEX      FastIndex, PUNICODE_STRING         SearchName)
/*
Routine Description:
    Find the child cell (either subkey or value) specified by name.
