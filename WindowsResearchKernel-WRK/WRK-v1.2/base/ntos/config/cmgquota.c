...
Abstract:
    ...
    Global Quota has little to do with NT's standard per-process/user quota system.
    Global Quota is waying of controlling the aggregate resource usage of the entire registry.
    It is used to manage space consumption by objects which user apps create,
    but which are persistent and therefore cannot be assigned to the quota of a user app.

    Global Quota prevents the registry from consuming all of paged pool, and indirectly controls how much disk it can consume.
    Like the release 1 file systems, a single app can fill all the space in the registry, but at least it cannot kill the system.

    Memory objects used for known short times and protected by serialization,
    or billable as quota objects, are not counted in the global quota.
*/
...
// Percent of used registry quota that triggers a hard error warning popup.
#define CM_REGISTRY_WARNING_LEVEL   (95)

// System hive hard quota limit

// For an x86 3GB system we set the limit at 12MB for now. Needs some MM changes before we bump this up.
...
// Set to largest positive number for use in boot.
// Will be set down based on pool and explicit registry values.
...
VOID CmQueryRegistryQuotaInformation(__inout PSYSTEM_REGISTRY_QUOTA_INFORMATION RegistryQuotaInformation)
/*
Routine Description:
    Returns the registry quota information
Arguments:
    RegistryQuotaInformation - Supplies pointer to buffer that will return the registry quota information.
*/
{
...
VOID CmSetRegistryQuotaInformation(__in PSYSTEM_REGISTRY_QUOTA_INFORMATION RegistryQuotaInformation)
/*
Routine Description:
    Sets the registry quota information.  The caller is assumed to have completed the necessary security checks already.
Arguments:
    RegistryQuotaInformation - Supplies pointer to buffer that provides the new registry quota information.
*/
{
...
/*
Routine Description:
    Displays hard error popup that indicates the registry quota is running out.
Arguments:
...
/*
Routine Description:
    If CmpGlobalQuotaUsed + Size >= CmpGlobalQuotaAllowed, return false.
    Otherwise, increment CmpGlobalQuotaUsed, in effect claiming the requested GlobalQuota.
Arguments:
...
*/
{
    ...
    if ((CmRegistrySizeLimitLength != 4) ||
        ...
        // If no value at all, or value of wrong type, or set to zero, use internally computed default
...
Routine Description:
    ...
    NOTE:   Do NOT put this in init segment, we call it after that code has been freed!
*/
{
    CmpGlobalQuotaAllowed = CmpGlobalQuota;
}
...
/*
Routine Description:
    Checks if the system hive is allowed to grow with the specified amount of data (using the hard quota limit on the system hive)
*/
{
    PCMHIVE             CmHive;
...
/*
Routine Description:
    Displays hard error popup that indicates the hard quota limit on the system hive is running out.
Arguments:
    WorkItem - Supplies pointer to the work item. This routine will free the work item.
*/
{
...
ULONG CmRegisterSystemHiveLimitCallback(__in ULONG Low, __in ULONG High, __in PVOID Ref, __in PCM_HYSTERESIS_CALLBACK Callback)
/*
Routine Description:
...
VOID CmpRaiseSelfHealWarningWorker(IN PVOID Arg)
{
    ...
    ExRaiseHardError(STATUS_REGISTRY_HIVE_RECOVERED, 1, 1, (PULONG_PTR)&ErrorParameters, OptionOk, &ErrorResponse);
    ...
    if (IsListEmpty(&CmpSelfHealQueueListHead) == FALSE) {
        ...
        Param = CONTAINING_RECORD(Param, CM_SELF_HEAL_WORK_ITEM_PARAMETER, SelfHealQueueListEntry);
...
/*
Routine Description:
    Raise a hard error informing the use the specified hive has been self healed and it might not be entirely consitent
Arguments:
...
*/
{
    ...
    if (Param) {
        ...
        if (Param->WorkItem != NULL) {
            ...
            LOCK_SELF_HEAL_QUEUE();
            if (!CmpSelfHealWorkerActive) {// no work item currently; ok to queue one.				
                ExQueueWorkItem(Param->WorkItem, DelayedWorkQueue);
                CmpSelfHealWorkerActive = TRUE;
            } else {// add it to the end of the list. It'll be picked up when the current work item completes				
                InsertTailList(&CmpSelfHealQueueListHead, &(Param->SelfHealQueueListEntry));
            }
            UNLOCK_SELF_HEAL_QUEUE();
        } else {
            ExFreePool(Param);
