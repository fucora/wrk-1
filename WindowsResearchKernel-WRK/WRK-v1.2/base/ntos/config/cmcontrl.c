...
VOID CmGetSystemControlValues(__in PVOID SystemHiveBuffer, __inout PCM_SYSTEM_CONTROL_VECTOR  ControlVector)
/*
Routine Description:
    Look for registry values in current control set, as specified by entries in ControlVector.
    Report data for value entries (if any) to variables ControlVector points to.
Arguments:
    SystemHiveBuffer - pointer to flat image of the system hive
    ControlVector - pointer to structure that describes what values to pull out and store
*/
{
    NTSTATUS        status;
    PHHIVE          SystemHive;
    ...
    status = HvInitializeHive(SystemHive, HINIT_FLAT, HIVE_VOLATILE, HFILE_TYPE_PRIMARY, SystemHiveBuffer, NULL, NULL, NULL, NULL, NULL, NULL, 1, NULL);
    ...
    // Set the default thread locale to the default system locale for now.
    // This will get changed as soon as somebody logs in.
