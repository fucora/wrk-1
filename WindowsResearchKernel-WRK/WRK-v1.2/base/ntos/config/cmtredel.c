...
/*
Routine Description:
    Delete all child subkeys, recursively, of Hive.Cell.  Delete all value entries of Hive.Cell.  Do NOT delete Hive.Cell itself.

    NOTE:   If this call fails part way through, it will NOT undo any successfully completed deletes.
    NOTE:   This algorithm can deal with a hive of any depth, at the cost of some "redundent" scaning and mapping.
Arguments:
...
Return Value:
    BOOLEAN - Result code from call, among the following:
        TRUE - it worked
        FALSE - the tree delete was not completed (though more than 0 keys may have been deleted)
*/
{
    ULONG  count;
    HCELL_INDEX ptr1;
    ...
    while (TRUE) {
        ...
        if (count > 0) {                // ptr1->count > 0?
            ptr2 = CmpFindSubKeyByNumber(Hive, Node, 0);// subkeys exist, find and delete them
            HvReleaseCell(Hive, ptr1);// release the cell here as we are overriding node
            if (ptr2 == HCELL_NIL) {
...
/*
Routine Description:
    Actually free the storage for the specified cell.
    We will first remove it from its parent's child key list, then free all of its values, then the key body itself.
Arguments:
    Hive - pointer to hive control structure for hive of interest
    Cell - index for cell to free storage for (the target)
    Unlink - if TRUE, target cell will be removed from parent cell's subkeylist, if FALSE, it will NOT be.
Return Value:
    NONE.
*/
{
...
Routine Description:
    ...
    Includes the parent, the parent's child list, the key body, class, security, all value entry bodies, and all of their data cells.
Arguments:
    Hive - pointer to hive control structure for hive of interest
...
*/
{
    ...
    if (ptarget->u.KeyNode.Flags & KEY_HIVE_EXIT) {
        // If this is a link node, we are done.
        // Link nodes never have classes, values, subkeys, or security descriptors.
        // Since they always reside in the master hive, they're always volatile.
        HvReleaseCell(Hive, Cell);
        return(TRUE);
    }
    ...
    if (!(ptarget->u.KeyNode.Flags & KEY_PREDEF_HANDLE) && (ptarget->u.KeyNode.ValueList.Count > 0)) {
        ...
        for (i = 0; i < ptarget->u.KeyNode.ValueList.Count; i++) {
            ...
            HvReleaseCell(Hive, plist->u.KeyList[i]);// we can safely release it here, as it is now dirty/pinned
    ...
    if (ptarget->u.KeyNode.Flags & KEY_HIVE_ENTRY) {
        // if this is an entry node, we are done.
        // our parent will be in the master hive (and thus volatile)
        return TRUE;
    }
