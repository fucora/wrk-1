...
// stack used for directing nesting of tree copy.  gets us off the kernel stack and thus allows for VERY deep nesting
...
Routine Description:
    ...
    a tree copy or a tree synchronization (sync).
    Which task is performed is determined by the TreeSync parameter.
    ...
    NOTE:   Volatile keys are only copied/synced if the CopyVolatile parameter is set to true.

    For a tree copy:
    ----------------

    A tree is copied from source to destination.
    The subkeys of the source root key and the full trees under those subkeys will be copied to a new tree at target root key.

    NOTE:   If this call fails part way through, it will NOT undo any successfully completed key copies, thus a partial tree copy CAN occur.

    For a tree sync:
    ----------------

    The target tree is synchronized with the source tree.
    It is assumed that for a certain period of the time the target tree has remained unmodified while modifications may have been made to the source tree.
    During a sync, any such modifications to the source tree are made to the target tree.
    Thus, at the end of a successful sync, the target tree is identical to the source tree.

    Since only things that have changed in the source tree are modified in the target tree, a sync operation is far more efficient than the delete/copy operations necessary to accomplish the same results.

    NOTE: It is assumed that no open handles are held on any target tree keys. Registry in-memory data structures may be corrupted if this is not true.
...
Arguments:
    ...
    CopyVolatile - indicates whether volatile keys should be copied/synced.
    CopyType - indicates the type of the copy operation:
                ...
                Merge - A merge is requested i.e.:
                    1. the target nodes that are not present on the source tree are not deleted.
                    2. the target nodes that are present in the source tree gets overrided no matter what the LastWriteTime value is.
Return Value:
    BOOLEAN - Result code from call, among the following:
        TRUE - it worked
        FALSE - the tree copy/sync was not completed (though more than 0 keys may have been copied/synced)
*/
{
    BOOLEAN result;
    PCMP_COPY_STACK_ENTRY   CmpCopyStack;
...
/*
Routine Description:
   This is a helper routine for CmpCopySyncTree.
   It accomplishes the functionality described by that routine in a "virtually" recursive manner which frees this routine from the limitations of the Kernel stack.
...
Inner: while (TRUE) {
    ...
    if ((SourceChild == HCELL_NIL) || (!CopyVolatile && (HvGetCellType(SourceChild) == Volatile))) {
        ...
        if (TreeSync && (CopyType != Merge)) {
            // If we are here during a sync, that means most of sync operations applied to the current SourceCell have been completed.
            // That is, we have:
            //   1) Synchronized SourceCell's values with its counterpart in the target tree.
            //   2) Synchronized any new SourceCell subkeys (subkeys present in SourceCell but not its counterpart) by creating and copying them to the proper place in the target tree.

            // What this means is that SourceCell's counterpart in the target tree (TargetCell) now has at least as many subkeys as SourceCell.

            // This implies that if TargetCell now has more subkeys that SourceCell than some subkeys of TargetCell are not present in the source tree
            // (probably because those keys were deleted from the source tree during the period between the previous sync and now).

            // If such keys exist, then they must be delete them from TargetCell in order to complete the sync. We do this below.
            ...
            if ((TargetCell->SubKeyCounts[Stable] + TargetCell->SubKeyCounts[Volatile]) > (SourceCell->SubKeyCounts[Stable] +
                                                                                           // We only count the volatile keys if we are actually syncing them.
                                                                                           // Note, however, that we always use the volatile counts in TargetCell since we may be syncing to a volatile tree where all keys are volatile.
            ...
            HvReleaseCell(CmpTargetHive, Frame->TargetCell);// release target cell as we don't need it anymore
        }
        HvReleaseCell(CmpSourceHive, Frame->SourceCell);// release the source cell
        break;
    } else {
        HvReleaseCell(CmpSourceHive, Frame->SourceCell);// release the source cell
    }
    ...
    if (TreeSync) {
        ...
        // Try to find the current child's counterpart in in the target tree using the child's name.
        ...
        HvReleaseCell(CmpTargetHive, Frame->TargetCell);// release the temporary node
        if (NewSubKey != HCELL_NIL) {
            // Found it, the current child (subkey) has a counterpart in the target tree. Thus, we just need to check if
            // the counterpart's values are out of date and should be updated.
            ...
            // Check if the current subkey has been modified more recently than its target tree counterpart.
            // When we are doing a tree merge, always override the target.
            if ((CopyType == Merge) || ((TargetChildCell->LastWriteTime.QuadPart) < (SourceChildCell->LastWriteTime.QuadPart))) {
                // The counterpart is out of date. Its values must be synchronized with the current subkey.

                // Sync up the key's values, sd, & class
                if (CopyType == Merge) {
                    if (!CmpMergeKeyValues(CmpSourceHive, SourceChild, SourceChildCell, CmpTargetHive, NewSubKey, TargetChildCell)) {
                        HvReleaseCell(CmpSourceHive, SourceChild);
                        HvReleaseCell(CmpTargetHive, NewSubKey);
                        goto CopyEnd;
                    }
                } else {
                    if (!CmpSyncKeyValues(CmpSourceHive, SourceChild, SourceChildCell, CmpTargetHive, NewSubKey, TargetChildCell)) {
                        HvReleaseCell(CmpSourceHive, SourceChild);
                        HvReleaseCell(CmpTargetHive, NewSubKey);
                        goto CopyEnd;
                    }
            ...
            // If we are here, then the current subkey's target tree counterpart has been synchronized (or did not need
            // to be). Transfer control to the code that will apply this function "recursively" to the current subkey in order to continue the sync.
            HvReleaseCell(CmpSourceHive, SourceChild);
            HvReleaseCell(CmpTargetHive, NewSubKey);
            goto NewKeyCreated;
        }
    ...
    // Check if the sync operation determined that this subtree should be copied
    if (TreeSync && SyncNeedsTreeCopy) {
        // We have just created a new key in the target tree with the above code.
        // However, since this is a sync, the parent of that new key has not been created by our
        // code and thus may not have been modified at all before the creation of the new key.
        // But this parent now has a new child, and must therefore be marked as dirty.
        if (!CmpMarkKeyParentDirty(CmpTargetHive, NewSubKey)) {
            goto CopyEnd;
        }

        // Record the stack level where we start the copy (and temporarily abandon the sync)
        // so that we can return to the sync operation when this stack level is reached again (i.e. when the tree under the current subkey is fully copied)
...
NewKeyCreated:
    // We succeeded in copying/syncing the subkey, apply ourselves to it
    CmpCopyStackTop++;
    if (CmpCopyStackTop >= CmpCopyStackSize) {
        // if we're here, it means that the tree we're trying to copy is more than 1024 COMPONENTS deep (from 2048 to 256k bytes)
        // we could grow the stack, but this is pretty severe, so return FALSE and fail the copy
        goto CopyEnd;
    }
    ...
    goto Outer;
       ...
       // This is a good time to check if we need to resume a temporarily suspended sync operation.
...
/*
Routine Description:
    Copy a key body and all of its values, but NOT its subkeylist or subkey entries.  SubKeyList.Count will be set to 0.
Arguments:
    SourceHive - pointer to hive control structure for source
...
Return Value:
    HCELL_INDEX - Cell of body of new key entry, or HCELL_NIL if some error.
*/
{
    NTSTATUS                status;
    ...
    // this cell was just allocated so it should be pinned; it's OK to release it here as the view is not going anywhere
...
/*
Routine Description:
    Copy a value entry.  Copies the body of a value entry and the data.  Returns cell of new value entry.
Arguments:
    SourceHive - pointer to hive control structure for source
...
Return Value:
    HCELL_INDEX - Cell of body of new value entry, or HCELL_NIL if some error.
*/
{
    HCELL_INDEX newvalue;
    HCELL_INDEX newdata;
    ...
    if (datalength > 0) {
        if (datalength > CM_KEY_VALUE_SMALL) {
            if ((CmpIsHKeyValueBig(SourceHive, datalength) == TRUE) || (CmpIsHKeyValueBig(TargetHive, datalength) == TRUE)) {
                PCELL_DATA  Buffer;
                ...
                // allocate a new value data in the target hive (regardless of the size) and copy the data onto it.
                ...
                // there's data, normal size, or none of the hives support bigdata cells, so do standard copy
...
/*
Routine Description:
   Free the cells associated with the value entries, the security descriptor, and the class of a particular key.
Arguments:
...
Routine Description:
    ...
    2. Only values from the source that are not present in the target are taken into account by this routine. They are added to the target node value list "as they are".
Arguments:
...
*/
{
    ...
    if (count == 0) {
        ...
        // The type of the new cells will be the same as that of the target cell.
        Type = HvGetCellType(TargetKeyCell);

        // Reallocate the value list for target to fit the new size
        // Worst case: all values from the source node will be added to the target node
...
/*
Routine Description:
    Synchronizes the value entries, security descriptor, and class of a target key with that of a source key - ensuring that the keys are identical with respect to the synchronized information.
Arguments:
   SourceHive     - Hive of the source key
   SourceKeyCell  - The source key's cell
   SourceKeyNode  - The source key's body
...
*/
{
    ...
    // Associate the new class with the target key and prepare and security descriptor assignment.
    ...
    if ((TargetKeyNode->Flags & KEY_HIVE_ENTRY) && (NewSecurity != OldSecurity)) {
        ...
        if (!HvMarkCellDirty(TargetHive, OldSecurity, FALSE) || !HvMarkCellDirty(TargetHive, LastSecCell, FALSE)) {
            // no log space
    ...
    // we need to play it safe here, to make sure we never end up having a key with a NIL security cell
    ...
    if (count == 0) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if ((pvalue == NULL) || !HvTrackCellRef(&CellRef, TargetHive, newvalue)) {
                // we couldn't map the bin containing this cell this shouldn't happen as we just allocated the cell
                // (i.e. the bin containing it should be PINNED into memory by now )
                ASSERT(FALSE);
                // for cleanup purposes
                newlist = TargetKeyNode->ValueList.List;
...
Routine Description:
   ...
   N.B. The initialized string's buffer is not meant to be modified.
Arguments:
   Cell       - The body of the key in question
   KeyName    - The UNICODE_STRING to initialize
   NameBuffer - A buffer REG_MAX_KEY_NAME_LENGTH bytes in size that will possibly be used as the UNICODE_STRING's buffer.
*/
{
    ...
    if (Cell->Flags & KEY_COMP_NAME) {
        ...
        // Name is not compressed. Just use the name string from the key buffer as the string buffer.
...
*/
{
    ...
    if (Cell->Flags & VALUE_COMP_NAME) {
        ...
        // Name is not compressed. Just use the name string from the ValueName buffer as the string buffer.
...
/*
Routine Description:
   This routine makes sure that any subkeys present in the target key but not present in the source key are deleted from the target key along with any trees under those subkeys.

   This routine is useful for synchronizing key deletion changes in a source cell with a target cell. It is used in this way from CmpCopySyncTree.
   NOTE: It is assumed that no open handles are held for the keys being deleted. If this is not so, registry in-memory data structures may become corrupted.
Arguments:
...
*/
{
    ...
    while ((TargetSubKey = CmpFindSubKeyByNumber(TargetHive, TargetCell, i)) != HCELL_NIL) {
        // Check if the current subkey has a counterpart subkey of the source cell.
        // (Note that we use similar techniques as in the code of CmpCopySyncTree2)
        ...
        if (SourceSubKey == HCELL_NIL) {
            // The current subkey has no counterpart, it must therefore be deleted from the target cell.
            ...
            // The subkey we are deleting is now a leaf (or has always been one), just delete it.
            if (!NT_SUCCESS(CmpFreeKeyByCell(TargetHive, TargetSubKey, TRUE))) {
                return FALSE;
            }

            // We have deleted a subkey, so *i* does not need to get incremented here because it now refers to the next subkey.
        } else {
...
/*
Routine Description:
   Marks the cells associated with a key's value entries, security descriptor, and class information as dirty.
Arguments:
   Hive     - The hive of the key in question
   Cell     - The cell of the key in question
...
*/
{
    ...
    if (Node->Security != HCELL_NIL) {
        ...
        if (!(HvMarkCellDirty(Hive, security->u.KeySecurity.Flink, FALSE) && HvMarkCellDirty(Hive, security->u.KeySecurity.Blink, FALSE))) {
...
*/
{
    ...
    if (ptarget->u.KeyNode.Flags & KEY_HIVE_ENTRY) {
        // if this is an entry node, we are done.  our parent will be in the master hive (and thus volatile)
        return TRUE;
    }
