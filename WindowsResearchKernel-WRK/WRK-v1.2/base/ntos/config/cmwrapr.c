...
Abstract:
    This module contains the source for wrapper routines called by the hive code, which in turn call the appropriate NT routines.
*/
...
// never read more than 64k, neither the filesystem nor some disk drivers like it much.
...
Arguments:
    ...
    UseForIo - TRUE if object allocated will be target of I/O, FALSE if not.
Return Value:
...
/*
Routine Description:
    This routine sets the size of a file.  It must not return until the size is guaranteed.
    It is environment specific.
...
Arguments:
    ...
    OldFileSize - old file size, in order to determine if this is a shrink;
                - ignored if file type is not primary, or hive doesn't use the mapped views technique
Return Value:
    FALSE if failure
    TRUE if success
*/
{
    ...
    oldFlag = IoSetThreadHardErrorMode(FALSE);// disable hard error popups, to avoid self deadlock on bogus devices
    ...
    Status = ZwSetInformationFile(FileHandle, &IoStatus, (PVOID)&FileInfo, sizeof(FILE_END_OF_FILE_INFORMATION), FileEndOfFileInformation);
    ...
    IoSetThreadHardErrorMode(oldFlag);// restore hard error popups mode

    // purge
    if (HiveWritesThroughCache(Hive, FileType) && (OldFileSize > FileSize)) {
        // first we have to unmap any possible mapped views in the last 256K window to avoid deadlock on CcWaitOnActiveCount inside CcPurgeCacheSection call below
        ULONG   Offset = FileSize & (~(_256K - 1));
        ASSERT_HIVE_WRITER_LOCK_OWNED((PCMHIVE)Hive);// we are not allowed to shrink in shared mode.
        ...
        CcPurgeCacheSection(((PCMHIVE)Hive)->FileObject->SectionObjectPointer, (PLARGE_INTEGER)(((ULONG_PTR)(&FileOffset)) + 1), OldFileSize - FileSize, FALSE);

        // Flush out this view to clear out the Cc dirty hints
        CcFlushCache(((PCMHIVE)Hive)->FileObject->SectionObjectPointer, (PLARGE_INTEGER)(((ULONG_PTR)(&FileOffset)) + 1),/*we are private writers*/OldFileSize - FileSize, NULL);
    }
...
Routine Description:
    ...
    NOTE:   We assume the handle is opened for asynchronous access, and that we, and not the IO system, are keeping the offset pointer.
...
Arguments:
    Hive - Hive we are doing I/O for
    FileType - which supporting file to use
    FileOffset - pointer to variable providing 32bit offset on input, and receiving new 32bit offset on output.
...
*/
{
    ...
    // We'd really like to just call the filesystems and have them do the right thing.
    // But the filesystem will attempt to lock our entire buffer into memory, and that may fail for large requests.
    // So we split our reads into 64k chunks and call the filesystem for each one.
    ASSERT_PASSIVE_LEVEL();
    while (DataLength > 0) {
...
Routine Description:
    ...
    The benefit is that writes don't go through Cc Lazy Writer, so there is no danger to be throttled or deferred.

    It also flushes the cache for the written ranges, guaranteeing that the data was commited to the disk upon return.
Arguments:
    Hive - Hive we are doing I/O for
    FileType - which supporting file to use
    offsetArray - array of structures where each structure holds a 32bit offset into the Hive file and pointer the a buffer written to that file offset.
    offsetArrayCount - number of elements in the offsetArray.
Return Value:
    FALSE if failure
    TRUE if success
Note:
    This routine is intended to deal only with paged bins (i.e. bins crossing the CM_VIEW_SIZE boundary or bins that were added after the last sync)
Assumption:
    We work on the assumption that the data to be written at one iteration never spans over the CM_VIEW_SIZE boundary. HvpFindNextDirtyBlock takes care of that !!!
*/
{
    ULONG           i;
    PVOID           DataBuffer;
    ...
    for (i = 0; i < offsetArrayCount; i++) {
        ...
        try {
            ...
            // copy data to pinned view; we need to do it inside try except, to protect against devices/volumes dismounting from under us.
        ...
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            ...
            // We want to catch this and treat as a  "not enough resources" problem, rather than letting it to surface the kernel call
...
*/
{
    ...
    // decide whether we wait for IOs to complete or just issue them and rely on the CcFlushCache to do the job

    // Bring pages being written into memory first to allow disk to write buffer contiguously.
    try {
        for (idx = 0; (ULONG)idx < offsetArrayCount; idx++) {
            ...
            while (start < end) {
                // perftouchbuffer globally declared so that compiler won't try to remove it and this loop (if its smart enough?).
        ...
        while ((arrayCount < offsetArrayCount) && (WaitBufferCount < MAXIMUM_WAIT_OBJECTS)) {
            ...
            while ((DataLength > 0) && (WaitBufferCount < MAXIMUM_WAIT_OBJECTS)) {
                ...
                if (WriteBlock->EventHandles[WaitBufferCount] == NULL) {
                    status = CmpCreateEvent(SynchronizationEvent, &(WriteBlock->EventHandles[WaitBufferCount]), &(WriteBlock->EventObjects[WaitBufferCount]));
                    if (!NT_SUCCESS(status)) {
...
Error_Exit:
    ...
    // if WaitBufferCount > 0 then we have successfully issued some I/Os, but not all of them.
    // This is an error, but we cannot return from this routine until all the successfully issued I/Os have completed.
    if (WaitBufferCount > 0) {
        ...
        status = KeWaitForMultipleObjects(WaitBufferCount, WriteBlock->EventObjects, WaitAll, Executive, KernelMode, FALSE, NULL, WriteBlock->WaitBlockArray);
    }
...
Return Value:
    ...
    TRUE if success
Note:
    FileOffset and Length are only taken into account when FileType == HFILE_TYPE_PRIMARY and the hive uses the mapped-views method.
*/
{
    NTSTATUS        status;
    IO_STATUS_BLOCK IoStatus;
