...
Abstract:
    This provides routines for determining driver load lists from the registry.
    The relevant drivers are extracted from the registry, sorted by groups, and then dependencies are resolved.

    This module is used by IoInitSystem for determining the drivers to be loaded in Phase 1 Initialization (CmGetSystemDriverList)
*/
...
/*
Routine Description:
    Traverses a particular control set and determines the filenames for the NLS data files that need to be loaded.
Arguments:
    ...
    CaseTableFileName - Returns the name of the Unicode upper/lowercase table for the language (l_intl.nls)
...
/*
Routine Description:
    Traverses a particular control set and creates a list of boot drivers to be loaded.
    This list is unordered, but complete.
Arguments:
    Hive - Supplies the hive control structure for the SYSTEM hive.
    ControlSet - Supplies the HCELL_INDEX of the root of the control set.
    LoadType - Supplies the type of drivers to be loaded (BootLoad, SystemLoad, AutoLoad, etc)
    BootFileSystem - If present, supplies the base name of the boot filesystem, which is explicitly added to the driver list.
    DriverListHead - Supplies a pointer to the head of the (empty) list of boot drivers to load.
Return Value:
...
/*
Routine Description:
    Determines if the driver is of a specified LoadType, based on its node values.
Arguments:
    Hive - Supplies a pointer to the hive control structure for the system hive.
...
*/
{
    ...
    // no mapped hives at this point. don't bother releasing cells
    ASSERT(Hive->ReleaseCellRoutine == NULL);
    // Must have a Start=BootLoad value in order to be a boot driver, so look for that first.
...
Routine Description:
    ...
    It initializes it with the registry path, filename, group name, and dependency list.
    Finally, it inserts the new node into the boot driver list.

    Note that this routine allocates memory by calling the Hive's memory allocation procedure.
Arguments:
    ...
    RegistryPath - Supplies the full registry path to the SERVICES node of the current control set.
...
*/
{
    ...
    // Check for ImagePath value, which will override the default name if it is present.
    ...
    // this defaults to 0xffffffff, so the driver is loaded last in the group.
...
/*
Routine Description:
    Sorts the list of boot drivers by their groups based on the group ordering in <control_set>\CONTROL\SERVICE_GROUP_ORDER:list

    Does NOT do dependency ordering.
Arguments:
    Hive - Supplies the hive control structure for the SYSTEM hive.
    ControlSet - Supplies the HCELL_INDEX of the root of the control set.
    DriverListHead - Supplies a pointer to the head of the list of boot drivers to be sorted.
Return Value:
...
*/
{
    ...
    // Dependency list is now pointed to by DependList->Buffer.  We need to sort the driver entry list.
...
Routine Description:
    ...
    Start with the last entry in the group order list and work towards the beginning.
    For each group entry, move all driver entries that are members of the group to the front of the list.
    Driver entries with no groups, or with a group that does not match any in the group list will be shoved to the end of the list.
Arguments:
    DriverListHead - Supplies a pointer to the head of the list of boot drivers to be sorted.
...
Routine Description:
    ...
    It removes any drivers that have circular dependencies from the list.
Arguments:
    DriverListHead - Supplies a pointer to the head of the list of boot drivers to be sorted.
Return Value:
...
*/
{
    ...
    while (CurrentEntry != DriverListHead) {
        ...
        // GroupStart now points to the first driver node in the group, and GroupEnd points to the last driver node in the group.
...
*/
{
    ...
    do {
        ...
        // If not, then remove the driver from the list and scan backwards until we find a driver with
        // a tag that is <= the current tag, or we reach the beginning of the list.
        Previous = Current;
        ...
        if (Previous->Tag > Current->Tag) {
            // Remove the Current driver from the list, and search backwards until we find a tag that is <= the current
...
/*
Routine Description:
    This routines parses the SYSTEM hive and "Select" node and verifies the following values:
    Current
    Default
    Failed
    LastKnownGood

    If any of these is missing the the loader will put the corrupt system hive message

    This routine is to be called by the loader just after it loads the system hive.
    It's purpose is to ensure a uniform and consistent way to treat missing values in this area.
Arguments:
...
/*
Routine Description:
    This routines parses the SYSTEM hive and "Select" node to locate the control set to be used for booting.

    Note that this routines also updates the value of Current to reflect the control set that was just found.
    This is what we want to do when this is called during boot.
    During I/O initialization, this is irrelevant, since we're just changing it to what it already is.
Arguments:
    SystemHive - Supplies the hive control structure for the SYSTEM hive.
    RootCell - Supplies the HCELL_INDEX of the root cell of the hive.
    SelectName - Supplies the name of the Select value to be used in determining the control set.  This should be one of "Current" "Default" or "LastKnownGood"
    AutoSelect - Returns the value of the AutoSelect value under the Select node.
Return Value:
...
*/
{
    ...
    // Control set was successfully found, so update the value in "Current" to reflect the control set we are going to use.
    Node = (PCM_KEY_NODE)HvGetCell(SystemHive, Select);
    if (Node == NULL) {
        // we couldn't map a view for the bin containing this cell
        return HCELL_NIL;
...
/*
Routine Description:
    Edits the in-memory copy of the registry to reflect the hardware profile that the system is booting from.
Arguments:
    Hive - Supplies a pointer to the hive control structure
...
/*
Routine Description:
    This routines parses the SYSTEM hive and locates the "CurrentControlSet\Control\IDConfigDB" node to determine the hardware profile configuration settings.
Arguments:
    SystemHive - Supplies the hive control structure for the SYSTEM hive.
    ControlSet - Supplies the HCELL_INDEX of the root cell of the hive.
    ProfileList - Returns the list of available hardware profiles sorted by preference.
                  Will be allocated by this routine if NULL is passed in, or a pointer to a CM_HARDWARE_PROFILE_LIST structure that is too small is passed in.
...
*/
{
    ...
    if (ARGUMENT_PRESENT(ReturnedProfileList)) {
        ...
        // Therefore, when the list is complete, the default hardware profile is at the head of the list.
        ...
        if (ProfileCell == HCELL_NIL) {
            ...
            if ((ProfileList == NULL) || (ProfileList->MaxProfileCount < ProfileCount)) {
                ...
                ProfileList = (SystemHive->Allocate)(sizeof(CM_HARDWARE_PROFILE_LIST) + (ProfileCount - 1) * sizeof(CM_HARDWARE_PROFILE), FALSE, CM_FIND_LEAK_TAG5);
                if (ProfileList == NULL) {
                    return(HCELL_NIL);
                }
            ...
            for (i = 0; i < ProfileCount; i++) {
                ...
                // ID for a hardware profile to possess, then we know that this must be a pristine profile.
                if (0 == TempProfile.Id) {
                    TempProfile.Flags = CM_HP_FLAGS_PRISTINE;
                    // NO other flags set.
                ...
                // Insert this new profile into the appropriate spot in the profile array. Entries are sorted by preference order.
    ...
    if (ARGUMENT_PRESENT(ReturnedAliasList)) {
        ...
        // Enumerate the keys under IDConfigDB\Alias and build the list of available hardware profiles aliases.
        // So that if we know our docking state we can find it in the alias table.
        ...
        if (AliasCell == HCELL_NIL) {
            ...
            if ((AliasList == NULL) || (AliasList->MaxAliasCount < AliasCount)) {
                ...
                AliasList = (SystemHive->Allocate)(sizeof(CM_HARDWARE_PROFILE_LIST) + (AliasCount - 1) * sizeof(CM_HARDWARE_PROFILE), FALSE, CM_FIND_LEAK_TAG6);
                if (AliasList == NULL) {
                    return(HCELL_NIL);
                }
...
/*
Routine Description:
    Calculates the tag index for a driver based on its tag value and the GroupOrderList entry for its group.
Arguments:
    Hive - Supplies the hive control structure for the driver.
    TagCell - Supplies the cell index of the driver's tag value cell.
    GroupOrderCell - Supplies the cell index for the control set's GroupOrderList:
            \Registry\Machine\System\CurrentControlSet\Control\GroupOrderList
    GroupName - Supplies the name of the group the driver belongs to.
            Note that if a driver's group does not have an entry under GroupOrderList, its tags will be ignored.
            Also note that if a driver belongs to no group (GroupName is NULL) its tags will be ignored.
Return Value:
...
*/
{
    ...
    for (CurrentTag = 1; CurrentTag <= OrderVector[0]; CurrentTag++) {
        if (OrderVector[CurrentTag] == *DriverTag) {
            // We have found a matching tag in the OrderVector, so return its index.
            if (BufferAllocated) {
...
BOOLEAN CmpGetBiosinfoFileNameFromRegistry(IN PHHIVE Hive, IN HCELL_INDEX ControlSet, OUT PUNICODE_STRING InfName)
{
    ...
    InfName->Length = 0;
    while ((InfName->Length < InfName->MaximumLength) && (InfName->Buffer[InfName->Length / sizeof(WCHAR)] != UNICODE_NULL)) {
