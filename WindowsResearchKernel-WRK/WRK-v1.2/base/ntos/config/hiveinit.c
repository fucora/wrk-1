...
*/
{
    ...
    if (Hive->Storage[Stable].Map) {
        for (i = 0; i <= Tables; i++) {// iterate through the directory
            Tab = Hive->Storage[Stable].Map->Directory[i];
            ASSERT(Tab);
            for (j = 0; j < HTABLE_SLOTS; j++) {// iterate through the slots in the directory
                Me = &(Tab->Table[j]);
                if (Me->BinAddress) {// BinAddress non-zero means allocated bin
...
Routine Description:
    ...
    Caller is expected to create/open files and store file handles in a way that can be derived from the hive pointer.

    Three kinds of initialization can be done, selected by OperationType:

        HINIT_CREATE
            Create a new hive from scratch.  Will have 0 storage.
            [Used to do things like create HARDWARE hive and for parts of SaveKey and RestoreKey]
        HINIT_MEMORY_INPLACE
            Build a hive control structure which allows read only access to a contiguous in-memory image of a hive.
            No part of the image will be copied, but a map will be made.
            [Used by osloader.]
        HINIT_FLAT
            Support very limited (read-only, no checking code) operation against a hive image.
        HINIT_MEMORY
            Create a new hive, using a hive image already in memory, at address supplied by pointer HiveData.
            The data will be copied.  Caller is expected to free HiveData.
            [Used for SYSTEM hive]
        HINIT_FILE
            Create a hive, reading its data from a file.
            Recovery processing  via log file will be done if a log is available.
            If a log is recovered, flush and clear operation will proceed.
        HINIT_MAPFILE
            Create a hive, reading its data from a file.  Data reading is done by mapping views of the file in the system cache.

  NOTE:   The HHive is not a completely opaque structure, because it is really only used by a limited set of code.
            Do not assume that only this routine sets all of these values.

Arguments:

    Hive - supplies a pointer to hive control structure to be initialized to describe this hive.
    ...
    HiveData - if present, supplies a pointer to an in memory image of from which to init the hive.
            Only useful when OperationType is set to HINIT_MEMORY.
    AllocateRoutine - supplies a pointer to routine called to allocate memory.  WILL be called before this routine returns.
    FreeRoutine - supplies a pointer to routine called to free memory.
                   CAN be called before this routine returns.
    FileSetSizeRoutine - supplies a pointer to a routine used to set the size of a file.
                         CAN be called before this routine returns.
    FileWriteRoutine - supplies a pointer to routine called to write memory to a file.
    FileReadRoutine - supplies a pointer to routine called to read from a file into memory.
                        CAN be called before this routine returns.
    FileFlushRoutine - supplies a pointer to routine called to flush a file.

    Cluster - clustering factor in HSECTOR_SIZE units.
            (i.e.  Size of physical sector in media / HSECTOR_SIZE.  1 for 512 byte physical sectors (or smaller), 2 for 1024, 4 for 2048, etc.
            (Numbers greater than 8 won't work.)

    FileName - some path like "...\system32\config\system", last 32 or so characters will be copied into baseblock (and thus to disk) as a debugging aid.  May be null.
...
*/
{
    ...
    if ((!ARGUMENT_PRESENT(HiveData)) && ((OperationType == HINIT_MEMORY) || (OperationType == HINIT_FLAT) || (OperationType == HINIT_MEMORY_INPLACE))) {
    ...
    if (OperationType == HINIT_CREATE) {// new create case
    ...
    if (OperationType == HINIT_FLAT) {// flat image case
    ...
    if (OperationType == HINIT_MEMORY_INPLACE) {// readonly image case
    ...
    if (OperationType == HINIT_MEMORY) {// memory copy case
        BaseBlock = (PHBASE_BLOCK)HiveData;
    ...
    if (OperationType == HINIT_FILE) {// file read case
        ...
        if (Status == STATUS_REGISTRY_RECOVERED) {
            ...
            // If we get here, we have recovered the hive, and written it out to disk correctly.
            // So we clear the log here.
        ...
        HvpFillFileName(Hive->BaseBlock, FileName);// slam debug name data into base block
        return STATUS_SUCCESS;
    }

    if (OperationType == HINIT_MAPFILE) {// file map case
...
Routine Description:
    ...
    If FileName is not NULL, copy last 64 bytes into name tail field of base block
Arguments:
