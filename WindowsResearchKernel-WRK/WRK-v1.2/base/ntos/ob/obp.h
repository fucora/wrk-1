...
//  We can no longer use an individual lock in the device map itself because that wasn't sufficient to protect the device map itself.

#ifdef OBP_PAGEDPOOL_NAMESPACE
#define OB_NAMESPACE_POOL_TYPE PagedPool
KGUARDED_MUTEX ObpDeviceMapLock;
#define OBP_DECLARE_OLDIRQL
#define ObpLockDeviceMap() KeAcquireGuardedMutex( &ObpDeviceMapLock )
#define ObpUnlockDeviceMap() KeReleaseGuardedMutex( &ObpDeviceMapLock )
...
#define ObpGetObjectTable() (PsGetCurrentProcess()->ObjectTable)//  A Macro to return the object table for the current process

//Macro to test whether or not the object manager is responsible for an object's security descriptor,
//or if the object has its own security management routines.
...
//  This is some special purpose code to keep a table of access masks correlated with back traces.
//  If used these routines replace the GrantedAccess mask in the preceding object table entry with a granted access index and a call back index.
#if i386
...
//  The three low order bits of the object table entry are used for handle attributes.

extern ULONG ObpAccessProtectCloseBit;//  We moved the PROTECT_CLOSE in the granted access mask
...
//  When a handle to an object with security is created, audit routines will be called to perform any auditing that may be required.
//  The audit routines will return a boolean indicating whether or not audits should be generated on close.
#define OBJ_AUDIT_OBJECT_CLOSE 0x00000004L


//  The following three bits are available for handle attributes in the Object field of an ObjectTableEntry.
...
#define SECURITY_DESCRIPTOR_CACHE_ENTRIES    257//  Number of minor hash entries
...
extern GENERAL_LOOKASIDE ObpCreateInfoLookasideList;//  Define date structures for the object creation information region.
...
PHANDLE_TABLE ObpKernelHandleTable;//  There is one global kernel handle table accessed via negative handle and only in kernel mode

//  The following macros are used to test and manipulate special kernel handles.
//  A kernel handle is just a regular handle with its sign bit set.
//  But must exclude -1 and -2 values which are the current process and current thread constants.
...
#define ObpReleaseObjectCreateInformation(_ObjectCreateInfo) {               \
    if ((_ObjectCreateInfo)->SecurityDescriptor != NULL) {                   \
        SeReleaseSecurityDescriptor((_ObjectCreateInfo)->SecurityDescriptor, (_ObjectCreateInfo)->ProbeMode, TRUE); \
        (_ObjectCreateInfo)->SecurityDescriptor = NULL;                      \
    }                                                                        \
}

NTSTATUS ObpCaptureObjectCreateInformation(IN POBJECT_TYPE ObjectType OPTIONAL, IN KPROCESSOR_MODE ProbeMode, IN KPROCESSOR_MODE CreatorMode,
    IN POBJECT_ATTRIBUTES ObjectAttributes, OUT PUNICODE_STRING CapturedObjectName, IN POBJECT_CREATE_INFORMATION ObjectCreateInfo, IN LOGICAL UseLookaside);
...
Arguments:
    None.
Return Value:
    If the allocation is successful, then the address of the allocated create information buffer is is returned as the function value.
    Otherwise, a value of NULL is returned.
*/
#define ObpAllocateObjectCreateInfoBuffer()             (POBJECT_CREATE_INFORMATION)ExAllocateFromPPLookasideList(LookasideCreateInfoList)
...
Return Value:
    None.
*/
#define ObpFreeObjectCreateInfoBuffer(ObjectCreateInfo)     ExFreeToPPLookasideList(LookasideCreateInfoList, ObjectCreateInfo)
...
BOOLEAN ObpCheckObjectReference(
    IN PVOID Object,
    IN OUT PACCESS_STATE AccessState,
    IN BOOLEAN TypeMutexLocked,
    IN KPROCESSOR_MODE AccessMode,
...
ULONG ObpHashSecurityDescriptor(PSECURITY_DESCRIPTOR SecurityDescriptor, ULONG Length);
ULONG ObpHashBuffer(PVOID Data, ULONG Length);
PSECURITY_DESCRIPTOR_HEADER ObpCreateCacheEntry(PSECURITY_DESCRIPTOR InputSecurityDescriptor, ULONG Length, ULONG FullHash, ULONG RefBias);
...
BOOLEAN ObpCompareSecurityDescriptors(IN PSECURITY_DESCRIPTOR SD1, ULONG Length, IN PSECURITY_DESCRIPTOR SD2);
...
//  Inline functions
FORCEINLINE BOOLEAN ObpSafeInterlockedIncrement(IN OUT LONG_PTR volatile *lpValue)
/*
Routine Description:
...
Return Value:
    Returns FALSE if the current value is 0 (so it cannot increment to 1). TRUE means the LONG value was incremented
*/
{
    LONG_PTR PointerCount, NewPointerCount;

    // If the object is being deleted then the reference count is zero.
    // So the idea here is to reference the long value but avoid the 0 -> 1 transition that would cause a double delete.
    PointerCount = ReadForWriteAccess(lpValue);
    do {
        if (PointerCount == 0) {
            return FALSE;
        }
        NewPointerCount = ObpInterlockedCompareExchange(lpValue, PointerCount + 1, PointerCount);

        if (NewPointerCount == PointerCount) {// If the exchange compare completed ok then we did a reference so return true.
            return TRUE;
        }

        // We failed because somebody else got in and changed the refence count on us. Use the new value to prime the exchange again.
...
FORCEINLINE BOOLEAN ObpSafeInterlockedIncrementLong(IN OUT LONG volatile *lpValue)
/*
Routine Description:
    This function increments the LONG value passed in.
    Unlike the InterlockedIncrement function, this will not increment from 0 to 1.
...
/*
Routine Description:
    This function references the name information structure.
    This is a substitute for the previous global locking mechanism that used the RootDirectoryMutex to protect the fields inside the NAME_INFO as well.
    If the function returns a non-NULL name info, the name buffer and Directory will not go away until the ObpDereferenceNameInfo call.
Arguments:
...
*/
{
    ...
    if ((NameInfo != NULL) && ObpSafeInterlockedIncrementLong((PLONG)&NameInfo->QueryReferences)) {
...
/*
Routine Description:
    This function dereferences the name information structure.
    If the number of references drops to 0, the name is freed and the directory dereferenced
Arguments:
    NameInfo - The pointer to the name info structure, as returned by ObpReferenceNameInfo.
    (NULL value is allowed)
*/
{
    if ((NameInfo != NULL) && (InterlockedDecrement((PLONG)&NameInfo->QueryReferences) == 0)) {
        PVOID DirObject;
...
FORCEINLINE LOGICAL ObpIsKernelExclusiveObject(IN POBJECT_HEADER ObjectHeader)
/*
Routine Description:
...
/*
Routine Description:
    This Function locks exclusively the Directory. It is used for write access to the directory structure.
Arguments:
...
/*
Routine Description:
    This Function locks shared the Directory. It is used to read fields inside the directory structure.
Arguments:
    Directory - The directory being locked
*/
{
...
/*
Routine Description:
    This function locks a lookup context.
    The directory is exclusively owned after this call and it's safe to access the directory in write mode.
    This function is intended to be used in insertion / deletion into/from the specified directory.
...
Routine Description:
    ...
    N.B. If ObpLookupDirectoryEntry is called several times in a loop, each call will undo the references done at the previous call.
    ObpReleaseLookupContext should be called only ones at the end.
Arguments:
...
VOID FORCEINLINE ObpLockAllObjects(IN POBJECT_TYPE ObjectType)
{
    LONG i;
...
VOID FORCEINLINE ObpUnlockAllObjects(IN POBJECT_TYPE ObjectType)
{
    LONG i;
