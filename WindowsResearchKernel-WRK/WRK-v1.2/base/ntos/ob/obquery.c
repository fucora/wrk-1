...
//  The following structure is used to pass the call back routine "ObpSetHandleAttributes" the captured object information and the processor mode of the caller.
    ...
    */
{
    ...
    TempReturnLength = 0;//  Initialize our local variables
    ...
    if (PreviousMode != KernelMode) {
        try {
            ...
            //  We'll use a local temp return length variable to pass through to the later ob query calls which will increment its value.
            //  We can't pass the users return length directly because the user might also be altering its value behind our back.
    ...
    //  If the query is not for types information then we will have to get the object in question.
    //  Otherwise for types information there really isn't an object to grab.
    ...
    //  Now process the particular information class being requested
    switch (ObjectInformationClass) {
    case ObjectBasicInformation:
        //  Make sure the output buffer is long enough and then fill in the appropriate fields into our local copy of basic information.
        ...
        //  Compute the size of the object name string by taking its name plus separators and traversing up to the root adding each directories name length plus separators
        ...
        //  Now that we've packaged up our local copy of basic info we need to copy it into the output buffer and set the return length
        try {
        ...
        break;
    case ObjectNameInformation://  Call a local worker routine
        Status = ObpQueryNameString(Object, (POBJECT_NAME_INFORMATION)ObjectInformation, ObjectInformationLength, &TempReturnLength, PreviousMode);
        break;
    case ObjectTypeInformation://  Call a local worker routine
        Status = ObQueryTypeInfo(ObjectType, (POBJECT_TYPE_INFORMATION)ObjectInformation, ObjectInformationLength, &TempReturnLength);
        break;
    case ObjectTypesInformation:
        try {
            //  The first thing we do is set the return length to cover the types info record.
            //  Later in each call to query type info this value will be updated as necessary
            TempReturnLength = sizeof(OBJECT_TYPES_INFORMATION);

            //  Make sure there is enough room to hold the types info record and if so then compute the number of defined types there are
            ...
            //  For each defined type we will query the type info for the object type and adjust the TypeInfo pointer to the next free spot
            ...
            for (i = 0; i < OBP_MAX_DEFINED_OBJECT_TYPES; i++) {
                ...
                if (NT_SUCCESS(Status)) {
                    TypeInfo = (POBJECT_TYPE_INFORMATION)((PCHAR)(TypeInfo + 1) + ALIGN_UP(TypeInfo->TypeName.MaximumLength, ULONG_PTR));
                }
            }
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
    ...
    case ObjectHandleFlagInformation:
        try {
            ...
            //  Make sure we have enough room for the query, and if so we'll set the output based on the flags stored in the handle
        ...
        break;
    default:
        //  To get to this point we must have had an object and the information class is not defined,
        //  so we should dereference the object and return to our user the bad status
        ObDereferenceObject(Object);
        return(STATUS_INVALID_INFO_CLASS);
    }

    //  Now if the caller asked for a return length we'll set it from our local copy
    try {
        if (ARGUMENT_PRESENT(ReturnLength)) {
    ...
    if (Object != NULL) {//  In the end we can free the object if there was one and return to our caller
        ObDereferenceObject(Object);
    }
...
NTSTATUS ObSetHandleAttributes(__in HANDLE Handle, __in POBJECT_HANDLE_FLAG_INFORMATION HandleFlags, __in KPROCESSOR_MODE PreviousMode)
{
    ...
    //  Or get the system handle table if this is a kernel handle and we are in kernel mode
    if (IsKernelHandle(Handle, PreviousMode)) {
        ObjectHandle = DecodeKernelHandle(Handle);//  Make the handle look like a regular handle
        ObjectTable = ObpKernelHandleTable;//  The global kernel handle table

        if (PsGetCurrentProcess() != PsInitialSystemProcess) {//  Go to the system process
    ...
    //	The callback routine will do the actual change
...
NTSTATUS NTAPI NtSetInformationObject(
    ...
    Arguments:
        ...
        ObjectInformationClass - Specifies the class of information being modified.  The only accepted value is ObjectHandleFlagInformation
        ObjectInformation - Supplies the buffer containing the handle flag information structure
        ObjectInformationLength - Specifies the length, in bytes, of the object information buffer
    Return Value:
        An appropriate status value
    */
{
    ...
    case ObjectHandleFlagInformation:
    {
        ...
        //  Get previous processor mode and probe and capture the input buffer
        PreviousMode = KeGetPreviousMode();
        try {
            if (PreviousMode != KernelMode) {
    ...
    case ObjectSessionInformation:
    {
        ...
        if (!SeSinglePrivilegeCheck(SeTcbPrivilege, PreviousMode)) {
            ...
            Status = ObReferenceObjectByHandle(Handle, 0, ObpDirectoryObjectType, PreviousMode, &Object, &HandleInformation);
            if (NT_SUCCESS(Status)) {
...
NTSTATUS ObQueryNameString(__in PVOID Object, __out_bcount(Length) POBJECT_NAME_INFORMATION ObjectNameInfo, __in ULONG Length, __out PULONG ReturnLength)
/*
Routine description:
    This routine processes a query of an object's name information
Arguments:
    Object - Supplies the object being queried
    ObjectNameInfo - Supplies the buffer to store the name string information
    Length - Specifies the length, in bytes, of the original object name info buffer.
    ReturnLength - Contains the number of bytes already used up in the object name info.
        On return this receives an updated byte count.
        (Length minus ReturnLength) is really now many bytes are left in the output buffer.
        The buffer supplied to this call may actually be offset within the original users buffer
Return Value:
    An appropriate status value
*/
{
    return ObpQueryNameString(Object, ObjectNameInfo, Length, ReturnLength, KernelMode);
}


NTSTATUS ObpQueryNameString(IN PVOID Object, OUT POBJECT_NAME_INFORMATION ObjectNameInfo, IN ULONG Length, OUT PULONG ReturnLength, IN KPROCESSOR_MODE Mode)
/*
Routine description:
    This routine processes a query of an object's name information
Arguments:
    ...
    ObjectNameInfo - Supplies the buffer to store the name string information
    Length - Specifies the length, in bytes, of the original object name info buffer.
    ReturnLength - Contains the number of bytes already used up in the object name info. On return this receives an updated byte count.
...
*/
{
    ...
    //  Otherwise, the object type does not specify a query name procedure so we get to do the work.
    //  The first thing to check is if the object doesn't even have a name.
    //  If object doesn't have a name then we'll return an empty name info structure.
RETRY:
    ...
    try {
        //  The object does have a name but now see if this is just the root directory object in which case the name size is only the "\" character
        if (Object == ObpRootDirectoryObject) {
            NameSize = sizeof(OBJ_NAME_PATH_SEPARATOR);
        } else {
            //  The named object is not the root so for every directory working out way up we'll add its size to the name keeping track of "\" characters inbetween each component.
            //  We first start with the object name itself and then move on to the directories
            ObjectDirectory = NameInfo->Directory;
            if (ObjectDirectory) {
                ObfReferenceObject(ObjectDirectory);
            ...
            while ((ObjectDirectory != ObpRootDirectoryObject) && (ObjectDirectory)) {
                ...
                NameInfo = ObpReferenceNameInfo(ObjectDirectoryHeader);
                if ((NameInfo != NULL) && (NameInfo->Directory != NULL)) {
                    //  This directory has a name so add it to the accumulated size and move up the tree
                    NameSize += sizeof(OBJ_NAME_PATH_SEPARATOR) + NameInfo->Name.Length;
                    ObjectDirectory = NameInfo->Directory;
                    if (ObjectDirectory) {
                        ObfReferenceObject(ObjectDirectory);
        ...
        //  At this point NameSize is the number of bytes we need to store the name of the object from the root down.
        //  The total buffer size we are going to need will include this size, plus object name information structure, plus an ending null character
    ...
    NameInfo = ObpReferenceNameInfo(ObjectHeader);
    if (!NameInfo) {//  Check whether someone else removed the name meanwhile
        //  The name is gone, we need to jump to the code path that handles empty object name
        goto RETRY;
    }

    //  Set the String buffer to point to the byte right after the last byte in the output string.
    //  This following logic actually fills in the buffer backwards working from the name back to the root
    ...
    try {
        ...
        *--StringBuffer = UNICODE_NULL;
        ...
        if (Object != ObpRootDirectoryObject) {
            ...
            ObjectDirectory = NameInfo->Directory;
            if (ObjectDirectory) {
                //  Reference the directory for this object to make sure it's valid while looking up
                ObfReferenceObject(ObjectDirectory);
                ReferencedObject = ObjectDirectory;
            }
            ...
            while ((ObjectDirectory != ObpRootDirectoryObject) && (ObjectDirectory)) {
                ...
                *--StringBuffer = OBJ_NAME_PATH_SEPARATOR;//  Tack on the "\" between the last name we added and this new name
                ...
                if ((NameInfo != NULL) && (NameInfo->Directory != NULL)) {
                    ...
                    //  We'll return \..  if the original directory was 1 char length, \..\ for 2 char length
                    if (StringBuffer < OriginalBuffer) {
                    ...
                    //  Test if we are in the case commented above.
                    //  If yes, we need to move the current pointer to the next char,
                    //  so the final assignment for \ a few lines below will take effect on the start of the block.
                    if (StringBuffer == OriginalBuffer) {
                        StringBuffer++;
                    }
        ...
        //  Tack on the "\" for the root directory and then set the output unicode string variable to have the right size and point to the right spot.
        *--StringBuffer = OBJ_NAME_PATH_SEPARATOR;
        ...
        //  If one of the parent directories disappeared, the final length will be smaller than we estimated before.
        //  We need to move the string to the beginning and adjust the returned size.
...
//  However, the caller we'll get an exception at the first access of the ObjectNameInfo
    }
    ...
    if (ForceRetry) {//  The query failed maybe because the object name changed during the query
        NameInfo = ObpReferenceNameInfo(ObjectHeader);
        ForceRetry = FALSE;
        goto RETRY;
    }
...
Arguments:
    Object - Supplies the object being queried
    ObjectTypeName - Supplies the buffer to store the type name string information
    Length - Specifies the length, in bytes, of the object type name buffer
    ReturnLength - Contains the number of bytes already used up in the object type name buffer. On return this receives an updated byte count
...
*/
{
    ...
    //  From the object get its object type and from that get the size of the object type name.
    //  The total size for we need for the output buffer must fit the name, a terminating null, and a preceding unicode string structure
    ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);
    ObjectType = ObjectHeader->Type;
    NameSize = ObjectType->Name.Length;
    TypeNameSize = NameSize + sizeof(UNICODE_NULL) + sizeof(UNICODE_STRING);

    //  Update the number of bytes we need and make sure the output buffer is large enough
    try {
    ...
    //  Set string buffer to point to the one byte beyond the buffer that we're going to fill in
    StringBuffer = (PWCH)ObjectTypeName;
    StringBuffer = (PWCH)((PCH)StringBuffer + TypeNameSize);
    String = &ObjectType->Name;
    try {
        //  Tack on the terminating null character and copy over the type name
        *--StringBuffer = UNICODE_NULL;
        StringBuffer = (PWCH)((PCH)StringBuffer - String->Length);
        RtlCopyMemory(StringBuffer, String->Buffer, String->Length);

        //  Now set the preceding unicode string to have the right lengths and to point to this buffer
...
Arguments:
    ...
    Length - Specifies the length, in bytes, of the object type information buffer
    ReturnLength - Contains the number of bytes already used up in the object type information buffer. On return this receives an updated byte count
...
*/
{
    ...
    try {
        //  The total number of bytes needed for this query includes the object type information structure plus the name of the type rounded up to a ulong boundary
        *ReturnLength += sizeof(*ObjectTypeInfo) + ALIGN_UP(ObjectType->Name.MaximumLength, ULONG);

        //  Make sure the buffer is large enough for this information and then fill in the record
        if (Length < *ReturnLength) {
            ...
            //  The type name goes right after this structure.
            //  We cannot use rtl routine like RtlCopyUnicodeString that might use the local memory to keep state,
            //  because this is the user buffer and it could be changing by user
...
NTSTATUS ObQueryObjectAuditingByHandle(__in HANDLE Handle, __out PBOOLEAN GenerateOnClose)
/*
Routine description:
    This routine tells the caller if the indicated handle will generate an audit if it is closed
Arguments:
    Handle - Supplies the handle being queried
    GenerateOnClose - Receives TRUE if the handle will generate an audit if closed and FALSE otherwise
Return Value:
    An appropriate status value
*/
{
    ...
    //  For the current process we'll grab its object table and then get the object table entry
    ...
    if (ObjectTableEntry != NULL) {//If we were given a valid handle we'll look at the attributes stored in the object table entry to decide if we generate an audit on close
...
Arguments:
    Object - Supplies the object being queried
Return Value:
    The address of the unicode string that stores the object name if available and NULL otherwise
*/
{
    POBJECT_HEADER ObjectHeader;
    POBJECT_HEADER_NAME_INFO NameInfo;
    ...
    //  If the object has a name then return the address of the name otherwise return null
...
/*
Routine description:
    This is the call back routine for the ExChangeHandle from NtSetInformationObject
Arguments:
    ObjectTableEntry - Supplies a pointer to the object table entry being modified
    Parameter - Supplies a pointer to the OBJECT_HANDLE_FLAG_INFORMATION structure to set into the table entry
Return Value:
...
*/
{
    ...
    //  Get a pointer to the object type via the object header and if the caller has asked for inherit but the object type says that inherit is an invalid flag then return false
    ...
    //  For each piece of information (inherit and protect from close) that is in the object information buffer we'll set or clear the bits in the object table entry.
    //  The bits modified are the low order bits of used to store the pointer to the object header.
