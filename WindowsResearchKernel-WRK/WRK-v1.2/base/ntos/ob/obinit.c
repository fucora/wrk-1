...
GENERAL_LOOKASIDE ObpCreateInfoLookasideList;//  Define date structures for the object creation information region.
#define OBJECT_NAME_BUFFER_SIZE 248//  Define data structures for the object name buffer lookaside list.
...
BOOLEAN ObpShutdownCloseHandleProcedure(IN PHANDLE_TABLE_ENTRY ObjectTableEntry, IN HANDLE HandleId, IN PVOID EnumParameter);

#ifdef ALLOC_PRAGMA
BOOLEAN ObDupHandleProcedure(PEPROCESS Process, PHANDLE_TABLE OldObjectTable, PHANDLE_TABLE_ENTRY OldObjectTableEntry, PHANDLE_TABLE_ENTRY ObjectTableEntry);
...
BOOLEAN ObpCloseHandleProcedure(IN PHANDLE_TABLE_ENTRY HandleTableEntry, IN HANDLE Handle, IN PVOID EnumParameter);
...
ULONG ObpAccessProtectCloseBit = MAXIMUM_ALLOWED;//  The default quota block is setup by obinitsystem
...
//  MmNumberOfPagingFiles is used in shutdown, to make sure we're not leaking any kernel handles.
...
//  A ULONGLONG aligned global variable for use by ObpLookupObjectName for quick comparisons.
...
/*
Routine Description:
    This function performs the system initialization for the object manager.
    The object manager data structures are self describing with the exception of the root directory, the type object type and the directory object type.
    The initialization code then constructs these objects by hand to get the ball rolling.
Return Value:
...
*/
{
    ...
    //  Determine the the size of the object creation and the name buffer lookaside lists.
    ...
    if (InitializationPhase == 0) {//  PHASE 0 Initialization
        //  Initialize the object creation lookaside list.
        ExInitializeSystemLookasideList(&ObpCreateInfoLookasideList, NonPagedPool, sizeof(OBJECT_CREATE_INFORMATION), 'iCbO', CreateInfoMaxDepth, &ExSystemLookasideListHead);
        ...
        ExInitializeSystemLookasideList(&ObpNameBufferLookasideList,
                                        ...
                                        PagedPool,
#endif
                                        OBJECT_NAME_BUFFER_SIZE, 'mNbO', NameBufferMaxDepth, &ExSystemLookasideListHead);

        //  Initialize the system create info and name buffer lookaside lists for the current processor.

        // N.B. Temporarily during the initialization of the system both lookaside list pointers in the processor block point to the same lookaside list structure.
        //      Later in initialization another lookaside list structure is allocated and filled for the per processor list.
        ...
        ObpRemoveObjectList = NULL;//  Initialize the object removal queue listhead.
        ObpInitSecurityDescriptorCache();//  Initialize security descriptor cache
        ...
        PsInitializeQuotaSystem();//  Initialize the quota system

        //Initialize the handle table for the system process and also the global kernel handle table
        ObpKernelHandleTable = PsGetCurrentProcess()->ObjectTable = ExCreateHandleTable(NULL);

#if DBG
        ExSetHandleTableStrictFIFO(ObpKernelHandleTable);// On checked make handle reuse take much longer
#endif

        ExInitializeWorkItem(&ObpRemoveObjectWorkItem, ObpProcessRemoveObjectQueue, NULL);// Initialize the deferred delete work item.

        //  Create an object type for the "Type" object.  This is the start of of the object types and goes in the ObpTypeDirectoryObject.
        ...
        ObCreateObjectType(&DirectoryTypeName, &ObjectTypeInitializer, (PSECURITY_DESCRIPTOR)NULL, &ObpDirectoryObjectType);

        //  Clear SYNCHRONIZE from the access mask to not allow synchronization on directory objects
        ...
        ObCreateObjectType(&SymbolicLinkTypeName, &ObjectTypeInitializer, (PSECURITY_DESCRIPTOR)NULL, &ObpSymbolicLinkObjectType);

        //  Clear SYNCHRONIZE from the access mask to not allow synchronization on symbolic link objects objects
        ObpSymbolicLinkObjectType->TypeInfo.ValidAccessMask &= ~SYNCHRONIZE;

#ifdef POOL_TAGGING
        ObpInitStackTrace();// Initialize the ref/deref object-tracing mechanism
#endif //POOL_TAGGING

#if i386
        //  Initialize the cached granted access structure.  These variables are used in place of the access mask in the object table entry.
    ...
    if (InitializationPhase == 1) {//  PHASE 1 Initialization
        ...
        for (Index = 0; Index < (ULONG)KeNumberProcessors; Index += 1) {
            ...
            if (Lookaside != NULL) {
                ExInitializeSystemLookasideList(Lookaside, NonPagedPool, sizeof(OBJECT_CREATE_INFORMATION), 'ICbO', CreateInfoMaxDepth, &ExSystemLookasideListHead);
            } else {
        ...
        if ((ObpAuditBaseDirectories != 0) || (ObpAuditBaseObjects != 0)) {
            ...
            Status = RtlAddAuditAccessAce(AuditAllAcl, ACL_REVISION2, GENERIC_ALL, SeWorldSid, TRUE, TRUE); //Audit success and failure
            ASSERT(NT_SUCCESS(Status));
            ...
            //  Now create a security descriptor that looks just like the public default, but has auditing in it as well.
        ...
        //  We only need to use the EffectiveSd on the root.  The SACL will be inherited by all other objects.

        //  Create a directory object for the root directory
        RtlInitUnicodeString(&RootDirectoryName, L"\\");
        InitializeObjectAttributes(&ObjectAttributes, &RootDirectoryName, OBJ_CASE_INSENSITIVE | OBJ_PERMANENT, NULL, EffectiveSd);
        Status = NtCreateDirectoryObject(&RootDirectoryHandle, DIRECTORY_ALL_ACCESS, &ObjectAttributes);
        if (!NT_SUCCESS(Status)) {
            return(FALSE);
        }

        Status = ObReferenceObjectByHandle(RootDirectoryHandle, 0, ObpDirectoryObjectType, KernelMode, (PVOID *)&ObpRootDirectoryObject, NULL);
        if (!NT_SUCCESS(Status)) {
            return(FALSE);
        }
        ...
        Status = RtlCreateAcl(Dacl, DaclLength, ACL_REVISION);// Create the SD for the the well-known directories
        if (!NT_SUCCESS(Status)) {
            ExFreePool(Dacl);
            return(FALSE);
        }

        Status = RtlAddAccessAllowedAce(Dacl, ACL_REVISION, DIRECTORY_QUERY | DIRECTORY_TRAVERSE | READ_CONTROL, SeWorldSid);
        if (!NT_SUCCESS(Status)) {
            ExFreePool(Dacl);
            return(FALSE);
        }
        ...
        InitializeObjectAttributes(&ObjectAttributes, &TypeDirectoryName, OBJ_CASE_INSENSITIVE | OBJ_PERMANENT, NULL, &SecurityDescriptor);
        ...
        InitializeObjectAttributes(&ObjectAttributes, &TypeDirectoryName, OBJ_CASE_INSENSITIVE | OBJ_PERMANENT, NULL, NULL);
        Status = NtCreateDirectoryObject(&TypeDirectoryHandle, DIRECTORY_ALL_ACCESS, &ObjectAttributes);
        if (!NT_SUCCESS(Status)) {
            return(FALSE);
        }

        Status = ObReferenceObjectByHandle(TypeDirectoryHandle, 0, ObpDirectoryObjectType, KernelMode, (PVOID *)&ObpTypeDirectoryObject, NULL);
        if (!NT_SUCCESS(Status)) {
            return(FALSE);
        }
        ...
        //  For every object type that has already been created we will insert it in the type directory.
        //  We do this looking down the linked list of type objects and for every one that has a name and isn't already in a directory we'll look the name up
        //  and then put it in the directory.
        //  Be sure to skip the first entry in the type object types lists.
        ObpInitializeLookupContext(&LookupContext);
        ObpLockLookupContext(&LookupContext, ObpTypeDirectoryObject);
        Head = &ObpTypeObjectType->TypeList;
        Next = Head->Flink;
        while (Next != Head) {
            //  Right after the creator info is the object header.  Get the object header and then see if there is a name.
            ...
            if ((NameInfo != NULL) && (NameInfo->Directory == NULL)) {//  Check if we have a name and we're not in a directory
                if (!ObpLookupDirectoryEntry(ObpTypeDirectoryObject, &NameInfo->Name, OBJ_CASE_INSENSITIVE, FALSE, &LookupContext)) {
        ...
        Status = ObpCreateDosDevicesDirectory();//  Create \DosDevices object directory for drive letters and Win32 device names
        if (!NT_SUCCESS(Status)) {
            return FALSE;
        }
    }
...
/*
Routine Description:
    This is the worker routine for ExDupHandleTable and is invoked via ObInitProcess.
Arguments:
    Process - Supplies a pointer to the new process
    HandleTable - Old handle table we are duplicating
    ObjectTableEntry - Supplies a pointer to the newly created handle table entry
Return Value:
    TRUE if the item can be inserted in the new table and FALSE otherwise
*/
{
    NTSTATUS Status;
    ...
    //  If we are tracing the call stacks for cached security indices then we have a translation to do.
    //  Otherwise the table entry contains straight away the granted access mask.
#if i386
    if (NtGlobalFlag & FLG_KERNEL_STACK_TRACE_DB) {
    ...
    //  Increment the handle count on the object because we've just added another handle to it.
...
/*
Routine Description:
    ExEnumHandleTable worker routine to generate audits when handles are inherited.
    An audit is generated if the handle attributes indicate that the handle is to be audited on close.
Arguments:
...
*/
{
    ...
    if (!(ObjectTableEntry->ObAttributes & OBJ_AUDIT_OBJECT_CLOSE)) {//  Check if we have to do an audit
        return(FALSE);
    }
...
/*
Routine Description:
    This function initializes a process object table.
    If the ParentProcess is specified, then all object handles with the OBJ_INHERIT attribute are copied from the parent object table to the new process' object table.
    The HandleCount field of each object copied is incremented by one.
    Both object table mutexes remained locked for the duration of the copy operation.
Arguments:
    ParentProcess - optional pointer to a process object that is the parent process to inherit object handles from.
...
*/
{
    ...
    //  If we have a parent process then we need to lock it down check that it is not going away and then make a copy of its handle table.
    //  If there isn't a parent then we'll start with an empty handle table.
    if (ARGUMENT_PRESENT(ParentProcess)) {
        ...
        NewObjectTable = ExDupHandleTable(NewProcess, OldObjectTable, ObDupHandleProcedure, OBJ_INHERIT);
    } else {
        OldObjectTable = NULL;
        NewObjectTable = ExCreateHandleTable(NewProcess);
    }

    //  Check that we really have a new handle table otherwise we must be out of resources
    if (NewObjectTable) {
        //  Set the new processes object table and if we are auditing then enumerate the new table calling the audit procedure
        ...
        //  Free the old table if it exists and then return to our caller
        if (OldObjectTable) {
            ObDereferenceProcessHandleTable(ParentProcess);
        }
        return (STATUS_SUCCESS);
    } else {
        //  We're out of resources to null out the new object table field, unlock the old object table, and tell our caller that this didn't work
...
/*
Routine Description:
    This function is called after an image file has been mapped into the address space of a newly created process.
    Allows the object manager to set LIFO/FIFO ordering for handle allocation based on the SubSystemVersion number in the image.
Arguments:
    NewProcess - pointer to the process object being initialized.
*/
{
...
/*
Routine Description:
    This function is used to close a handle but takes as input a handle table index that it first translates to an handle before calling close.
    Note that the handle index is really just the offset within the handle table entries.
Arguments:
...
/*
Routine Description:
    This function is called whenever a process is destroyed.
    It loops over the process' object table and closes all the handles.
Arguments:
...
*/
{
    ...
    ExWaitForRundownProtectionRelease(&Process->RundownProtect);// Wait for any cross process references to finish
    ExRundownCompleted(&Process->RundownProtect);// This routine gets recalled multiple times for the same object so just mark the object so future waits work ok.

    ObjectTable = Process->ObjectTable;
    if (ObjectTable != NULL) {//  If the process does NOT have an object table, return
        PreviousIOHardError = IoSetThreadHardErrorMode(FALSE);

        //  For each valid entry in the object table, close the handle that points to that entry.
...
//  The following structure is only used by the enumeration routine and the callback.  It provides context for the comparison of the objects.
...
/*
Routine Description:
    Call back routine when enumerating an object table to find a handle for a particular object
Arguments:
    HandleTableEntry - Supplies a pointer to the handle table entry being examined.
    HandleId - Supplies the actual handle value for the preceding entry
    EnumParameter - Supplies context for the matching.
Return Value:
    Returns TRUE if a match is found and the enumeration should stop.  Returns FALSE otherwise, so the enumeration will continue.
*/
{
    POBJECT_HEADER ObjectHeader;
    ...
    //  Get the object header from the table entry and see if object types and headers match if specified.
    ObjectHeader = (POBJECT_HEADER)((ULONG_PTR)ObjectTableEntry->Object & ~OBJ_HANDLE_ATTRIBUTES);
    ...
    if (ARGUMENT_PRESENT(MatchCriteria->HandleInformation)) {
        ...
        //  Get the handle attributes from table entry and see if the fields match.
        //  If they do not match we will return false to continue the search.
        HandleAttributes = ObpGetHandleAttributes(ObjectTableEntry);
        if (MatchCriteria->HandleInformation->HandleAttributes != HandleAttributes || MatchCriteria->HandleInformation->GrantedAccess != GrantedAccess) {
            return FALSE;
        }
    }

    return TRUE;//  We found something that matches our criteria so return true to our caller to stop the enumeration
}


BOOLEAN ObFindHandleForObject(
    ...
    /*
    Routine Description:
        This routine searches the handle table for the specified process, looking for a handle table entry that matches the passed parameters.
        If an an Object pointer is specified it must match.
        If an ObjectType is specified it must match.
        If HandleInformation is specified, then both the HandleAttributes and GrantedAccess mask must match.
        If all three match parameters are NULL, then will match the first allocated handle for the specified process that matches the specified object pointer.
    Arguments:
        ...
        Handle - Specifies the location to receive the handle value whose handle entry matches the supplied object pointer and optional match criteria.
    Return Value:
    ...
    */
{
    ...
    ObjectTable = ObReferenceProcessHandleTable(Process);//  Lock the object object name space
    if (ObjectTable != NULL) {//  We only do the work if the process has an object table meaning it isn't going away
        ...
        //  Call the routine the enumerate the object table, this will return true if we get match.
        //  The enumeration routine really returns a index into the object table entries we need to translate it to a real handle before returning.
...
/*
Routine Description:
    This routine creates the directory object for the dos devices and sets the device map for the system process.
Return Value:
...
*/
{
    ...
    //  Create a device map that will control this directory.
    //  It will be stored in the each EPROCESS for use by ObpLookupObjectName when translating names that begin with \??\
    //  With LUID device maps, the device map is stored in the each EPROCESS upon the first reference to the device map,
    //  and the EPROCESS device map field is cleared when the EPROCESS access token is set.
    ...
    Status = NtCreateSymbolicLinkObject(&SymbolicLinkHandle, SYMBOLIC_LINK_ALL_ACCESS, &ObjectAttributes, &TargetString);
    if (NT_SUCCESS(Status)) {
        NtClose(SymbolicLinkHandle);
    }

    //  Create a symbolic link, \??\Global, that points to the global \??
    //  Drivers loaded dynamically create the symbolic link in the global DosDevices directory.
    //  User mode components need some way to access this symbolic link in the global dosdevices directory.
    //  The Global symbolic link is used to escape out of the current sessions's DosDevices directory and use the global dosdevices directory. e.g CreateFile("\\\\.\\Global\\NMDev"..);
    RtlInitUnicodeString(&NameString, L"Global");
    InitializeObjectAttributes(&ObjectAttributes, &NameString, OBJ_PERMANENT, DirectoryHandle, &DosDevicesSD);
    Status = NtCreateSymbolicLinkObject(&SymbolicLinkHandle, SYMBOLIC_LINK_ALL_ACCESS, &ObjectAttributes, &RootNameString);
    if (NT_SUCCESS(Status)) {
        NtClose(SymbolicLinkHandle);
    }
    ...
    //  for backwards compatibility with old drivers that use the old name.
...
/*
Routine Description:
    This routine builds a security descriptor for use in creating the \DosDevices object directory.
    The protection of \DosDevices must establish inheritable protection which will dictate how dos devices created via the
    DefineDosDevice() and IoCreateUnprotectedSymbolicLink() apis can be managed.

    The protection assigned is dependent upon an administrable registry key:
    ...
    If this value is 0x1, then
            ...
            Anyone may create new Dos devices (such as net drives or additional printers),
            Anyone may use any Dos device,
            The creator of a Dos device may delete it.
            Note that this protects system-defined LPTs and COMs so that only administrators may redirect them.
            However, anyone may add additional printers and direct them to wherever they would like.
    ...
    If this value is 0x0, or not present, then
            ...
            Anyone may create new Dos devices (such as net drives or additional printers),
            Anyone may use any Dos device,
            Anyone may delete Dos devices created with either DefineDosDevice() or IoCreateUnprotectedSymbolicLink().
            This is how network drives and LPTs are created (but not COMs).
...
Arguments:
    SecurityDescriptor - The address of a security descriptor to be initialized and filled in.
    When this security descriptor is no longer needed, you should call ObpFreeDosDevicesProtection() to free the protection information.
Return Value:
...
Arguments:
    SecurityDescriptor - The address of a security descriptor initialized by ObpGetDosDevicesProtection().
*/
{
    NTSTATUS Status;
    PACL Dacl;
...
/*
Routine Description:
    ExEnumHandleTable worker routine will call this routine for each valid handle into the kernel table at shutdown
Arguments:
    ObjectTableEntry - Points to the handle table entry of interest.
    HandleId - Supplies the handle.
    EnumParameter - Supplies information about the source and target processes.
...
*/
{
    ...
    case 0:
    {
        ...
        // The starting depth is completely arbitrary, but not using zero as a valid depth lets us use it as a sentinel to
        // ensure we haven't over-decremented it, and as a check at the end (where it should be one less than its starting value).
        Depth = 1;
        SymlinkHitDepth = 1;
        ...
        restart_dir_walk:
            ...
            for (Bucket = 0; Bucket < NUMBER_HASH_BUCKETS; Bucket++) {
                ...
                while (*DirectoryEntryPtr) {
                    ...
                    if (DescentDirectory) {
                        // We're recovering from a descent; we want to iterate forward until we're past the directory which we were just processing.
                        if (Object == DescentDirectory) {
                            DescentDirectory = NULL;
                            if (SymlinkHitDepth > Depth) {
                                // We hit a symlink in that descent, which potentially rearranged the buckets in this chain; we need to rescan the entire chain.
                                DirectoryEntryPtr = Directory->HashBuckets + Bucket;
                        ...
                        // Either we haven't found the descent dir yet, or we did (and set it to NULL, so we'll stop skipping things),
                        // and don't have to deal with a symlink readjustment -- either way, march forward.
                        DirectoryEntryPtr = &(*DirectoryEntryPtr)->ChainLink;
                        continue;
                    }

                    if (ObjectType == ObpTypeObjectType) {// We'll clean these up later Keep going down the chain
                        DirectoryEntryPtr = &(*DirectoryEntryPtr)->ChainLink;
                        continue;
                    } else if (ObjectType == ObpDirectoryObjectType) {// Iteratively descend
                        Directory = Object;
                        Depth++;
                        goto restart_dir_walk;
                    } else {
                        // It's an object not related to Ob object management; mark it non-permanent,
                        // and if it doesn't have any handles, remove it from the directory (delete the name and dereference it once).
                        ...
                        if (ObjectHeader->HandleCount == 0) {
                            ...
                            *DirectoryEntryPtr = OldDirectoryEntry->ChainLink;
                            ...
                            //  If this is a symbolic link object then we also need to delete the symbolic link
                            if (ObjectType == ObpSymbolicLinkObjectType) {
                                SymlinkHitDepth = Depth;
                                ObpDeleteSymbolicLinkName((POBJECT_SYMBOLIC_LINK)Object);
                                // Since ObpDeleteSymbolicLinkName may potentially rearrange our buckets, we need to rescan from the beginning of this hash chain.
                                DirectoryEntryPtr = Directory->HashBuckets + Bucket;
                            }
                            ...
                            DirectoryEntryPtr = &(*DirectoryEntryPtr)->ChainLink;// Keep going down the chain
                        }
                    }
                } // while *DirectoryObjectPtr
            } // loop over buckets

            // Well -- we're done with this directory.
            // We might have been processing it as a child directory, though -- so if it has a parent, we need to go back up to it, and reset our iteration.
            Depth--;
            ObjectHeader = OBJECT_TO_OBJECT_HEADER(Directory);
            NameInfo = OBJECT_HEADER_TO_NAME_INFO(ObjectHeader);

            // We always assign DescentDirectory and Directory here;
            // if the current directory does not have a parent (i.e. it's the root), this will terminate the iteration.
    ...
    case 1:
    {
        ...
        ExEnumHandleTable(PsInitialSystemProcess->ObjectTable, ObpShutdownCloseHandleProcedure, &NumberOfOpenSystemHandles, NULL);
    ...
    //  Remove all types from the object type directory
        ...
        while (Next != Head) {
            ...
            //  Right after the creator info is the object header.
            //  Get the object header and then see if there is a name
            CreatorInfo = CONTAINING_RECORD(Next, OBJECT_HEADER_CREATOR_INFO, TypeList);
            ObjectTypeHeader = (POBJECT_HEADER)(CreatorInfo + 1);
        ...
        Status = ObReferenceObjectByName(&RootNameString, OBJ_CASE_INSENSITIVE, 0L, 0, ObpSymbolicLinkObjectType, KernelMode, NULL, &Object);
        if (NT_SUCCESS(Status)) {
            ObMakeTemporaryObject(Object);
            ObDereferenceObject(Object);
        }
        ...
        ObDereferenceObject(ObpRootDirectoryObject);//  Destroy the root directory
        ObDereferenceObject(ObpDirectoryObjectType);//  Destroy the ObpDirectoryObjectType object
        ObDereferenceObject(ObpSymbolicLinkObjectType);//  Destroy the ObpSymbolicLinkObjectType
        ObDereferenceObject(ObpTypeDirectoryObject);//  Destroy the type directory object
        ObDereferenceObject(ObpTypeObjectType);//  Destroy the ObpTypeObjectType
#if i386
        if (ObpCachedGrantedAccesses) {
            ExFreePool(ObpCachedGrantedAccesses);//  Free the ObpCachedGrantedAccesses pool
        }
#endif // i386
    } // default Phase (2)
    } // switch (Phase)
