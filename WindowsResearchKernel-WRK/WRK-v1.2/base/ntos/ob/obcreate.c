...
BOOLEAN ObpShowAllocAndFree;//  The following variable is only used on a checked build to control echoing out the allocs and frees of objects
#else
...
/*
Routine Description:
    This functions allocates space for an NT Object from either Paged or NonPaged pool.
    It captures the optional name and SECURITY_DESCRIPTOR parameters for later use when the object is inserted into an object table.
    No quota is charged at this time.
    That occurs when the object is inserted into an object table.
Arguments:
    ProbeMode - The processor mode to consider when doing a probe of the input parameters
    ObjectType - A pointer of the type returned by ObCreateObjectType that gives the type of object being created.
    ObjectAttributes - Optionally supplies the attributes of the object being created (such as its name)
    OwnershipMode - The processor mode of who is going to own the object
    ParseContext - Ignored
    ObjectBodySize - Number of bytes to allocate for the object body.
                     The object body immediately follows the object header in memory and are part of a single allocation.
    PagedPoolCharge - Supplies the amount of paged pool to charge for the object.
                      If zero is specified then the default charge for the object type is used.
    NonPagedPoolCharge - Supplies the amount of nonpaged pool to charge for the object.
                         If zero is specified then the default charge for the object type is used.
...
*/
{
    ...
    ObjectCreateInfo = ObpAllocateObjectCreateInfoBuffer();//  Allocate a buffer to capture the object creation information.
    if (ObjectCreateInfo == NULL) {
        Status = STATUS_INSUFFICIENT_RESOURCES;
    } else {
        //  Capture the object attributes, quality of service, and object name, if specified.
        //  Otherwise, initialize the captured object name, the security quality of service, and the create attributes to default values.
        Status = ObpCaptureObjectCreateInformation(ObjectType, ProbeMode, OwnershipMode, ObjectAttributes, &CapturedObjectName, ObjectCreateInfo, FALSE);
        if (NT_SUCCESS(Status)) {
            //  If the creation attributes are invalid, then return an error status.
            if (ObjectType->TypeInfo.InvalidAttributes & ObjectCreateInfo->Attributes) {
                Status = STATUS_INVALID_PARAMETER;
            } else {
                //  Set the paged and nonpaged pool quota charges for the object allocation.
                ...
                Status = ObpAllocateObject(ObjectCreateInfo, OwnershipMode, ObjectType, &CapturedObjectName, ObjectBodySize, &ObjectHeader);
                if (NT_SUCCESS(Status)) {
                    //  If a permanent object is being created, then check if the caller has the appropriate privilege.
                    *Object = &ObjectHeader->Body;
                    ...
                    if (ObpTraceEnabled && NT_SUCCESS(Status)) {
                        //  Register the object and push stack information for the first reference
                        ObpRegisterObject(ObjectHeader);
                        ObpPushStackInfo(ObjectHeader, TRUE);
                    }
#endif //POOL_TAGGING

                    //  Here is the only successful path out of this module but this path can also return privilege not held.
                    //  In the error case, all the resources have already been freed by ObpFreeObject.
                    return Status;
                }
            }
        ...
        ObpFreeObjectCreateInfoBuffer(ObjectCreateInfo);//  An error path, free object creation information buffer.
    }

    return Status;//  An error path
}
...
/*
Routine Description:
    This function captures the object creation information and stuff it into the input variable ObjectCreateInfo
Arguments:
    ObjectType - Specifies the type of object we expect to capture, currently ignored.
    ProbeMode - Specifies the processor mode for doing our parameter probes
    CreatorMode - Specifies the mode the object is being created for
    ObjectAttributes - Supplies the object attributes we are trying to capture
    CapturedObjectName - Receives the name of the object being created
    ObjectCreateInfo - Receives the create information for the object like its root, attributes, and security information
    UseLookaside - Specifies if we are to allocate the captured name buffer from the lookaside list or from straight pool.
Return Value:
...
*/
{
    ...
    //  Capture the object attributes, the security quality of service, if specified, and object name, if specified.
    ...
    try {
        if (ARGUMENT_PRESENT(ObjectAttributes)) {
            ...
            if (ARGUMENT_PRESENT(SecurityDescriptor)) {
                Status = SeCaptureSecurityDescriptor(SecurityDescriptor, ProbeMode, PagedPool, TRUE, &ObjectCreateInfo->SecurityDescriptor);
                if (!NT_SUCCESS(Status)) {
                    KdPrint(("OB: Failed to capture security descriptor at %08x - Status == %08x\n", SecurityDescriptor, Status));

                    //  The cleanup routine depends on this being NULL if it isn't allocated. 
                    //  SeCaptureSecurityDescriptor may modify this parameter even if it fails.
    ...
    //  Otherwise, initialize the object name descriptor and check for an incorrectly specified root directory.
    ...
    //  If the completion status is not successful, and a security quality of service parameter was specified, then free the security quality of service memory.
failureExit:
    if (!NT_SUCCESS(Status)) {
        ObpReleaseObjectCreateInformation(ObjectCreateInfo);
    }
...
/*
Routine Description:
    This function captures the object name but first verifies that it is at least properly sized.
Arguments:
    ProbeMode - Supplies the processor mode to use when probing the object name
    ObjectName - Supplies the caller's version of the object name
    CapturedObjectName - Receives the captured verified version of the object name
    UseLookaside - Indicates if the captured name buffer should be allocated from the lookaside list or from straight pool
Return Value:
    An appropriate status value
*/
{
    ...
    try {
        ...
        if (InputObjectName.Length != 0) {
            ...
            if (((Length & (sizeof(WCHAR) - 1)) != 0) || (Length == (MAXUSHORT - sizeof(WCHAR) + 1))) {
                ...
                //  N.B. The name buffer allocation routine adds one UNICODE character to the length and initializes the string descriptor.
                FreeBuffer = ObpAllocateObjectNameBuffer(Length, UseLookaside, CapturedObjectName);
                if (FreeBuffer == NULL) {
                    Status = STATUS_INSUFFICIENT_RESOURCES;
                } else {
                    RtlCopyMemory(FreeBuffer, InputObjectName.Buffer, Length); //  Copy the specified name string to the destination buffer.
                    FreeBuffer[Length / sizeof(WCHAR)] = UNICODE_NULL;//  Zero terminate the name string and initialize the string descriptor.
                }
            }
        }
    } except(ExSystemExceptionFilter())
...
Arguments:
    ...
    UseLookaside - Supplies a logical variable that determines whether an attempt is made to allocate the name buffer from the lookaside list.
    ObjectName - Supplies a pointer to a name buffer string descriptor.
Return Value:
    If the allocation is successful, then name buffer string descriptor is initialized and the address of the name buffer is returned as the function value.
...
*/
{
    ...
    //  If allocation from the lookaside lists is specified and the buffer size is less than the size of lookaside list entries, then attempt to allocate the name buffer from the lookaside lists.
    //  Otherwise, attempt to allocate the name buffer from nonpaged pool.
    Maximum = Length + sizeof(WCHAR);
    if ((UseLookaside == FALSE) || (Maximum > OBJECT_NAME_BUFFER_SIZE)) {
        Buffer = ExAllocatePoolWithTag(OB_NAMESPACE_POOL_TYPE, Maximum, 'mNbO');//  Attempt to allocate the buffer from nonpaged pool.
    } else {
        //  Attempt to allocate the name buffer from the lookaside list. If the allocation attempt fails, then attempt to allocate the name buffer from pool.
...
*/
{
    ...
    //  If the size of the buffer is not equal to the size of lookaside list entries, 
    //  then  free the buffer to pool. Otherwise, free the buffer to the lookaside list.
...
NTKERNELAPI VOID ObDeleteCapturedInsertInfo(__in PVOID Object)
/*
Routine Description:
    This function frees the creation information that could be pointed at by the object header.
Arguments:
...
*/
{
    ...
    //  Get the address of the object header and free the object create information object if the object is being created.
...
NTSTATUS ObpAllocateObject(
    ...
    /*
    Routine Description:
        This routine allocates a new object including the object header and body from pool and fill in the appropriate fields.
    Arguments:
        ObjectCreateInfo - Supplies the create information for the new object
        OwnershipMode - Supplies the processor mode of who is going to own the object
        ObjectType - Optionally supplies the object type of the object being created. If the object create info not null then this field must be supplied.
        ObjectName - Supplies the name of the object being created
        ObjectBodySize - Specifies the size, in bytes, of the body of the object being created
        ReturnedObjectHeader - Receives a pointer to the object header for the newly created objet.
    Return Value:
        An appropriate status value.
    */
{
    ...
    if (ObjectCreateInfo == NULL) {
        ...
        if (((ObjectCreateInfo->PagedPoolCharge != ObjectType->TypeInfo.DefaultPagedPoolCharge ||
              ...
              ObjectCreateInfo->SecurityDescriptorCharge > SE_DEFAULT_SECURITY_QUOTA) && PsGetCurrentProcess() != PsInitialSystemProcess) ||
    ...
    //  If the object type is not specified or specifies nonpaged pool, then allocate the object from nonpaged pool.
    ...
    //  Now based on if we are to put in the quota, handle, name, or creator info we will do the extra work. 
    //  This order is very important because we rely on it to free the object.
    if (QuotaInfoSize != 0) {
    ...
    //  N.B. The initialization of the object header is done field by field rather than zeroing the memory and then initializing the pertinent fields.

    //  N.B. It is assumed that the caller will initialize the object attributes, object ownership, and parse context.
...
*/
{
    ...
    //  Now from the header determine the start of the allocation.
    //  We need to backup based on what precedes the header.
    //  The order is very important and must be the inverse of that used by ObpAllocateObject
    ...
    //  Check where we were in the object initialization phase.
    //  This flag really only tests if we have charged quota for this object.
    //  This is because the object create info and the quota block charged are unioned together.
    if (ObjectHeader->Flags & OB_FLAG_NEW_OBJECT) {
        if (ObjectHeader->ObjectCreateInfo != NULL) {
            ObpFreeObjectCreateInformation(ObjectHeader->ObjectCreateInfo);
            ObjectHeader->ObjectCreateInfo = NULL;
    ...
    //  Trash type field so we don't get far if we attempt to use a stale object pointer to this object.
