...
//  The following flags enable or disable various diagnostic capabilities within OB code.
//  These flags are set in ObGlobalFlag (only available within a DBG system).
...
NTSTATUS ObLogSecurityDescriptor(__in PSECURITY_DESCRIPTOR InputSecurityDescriptor, __out PSECURITY_DESCRIPTOR *OutputSecurityDescriptor, __in ULONG RefBias)
/*
Routine Description:
    Takes a passed security descriptor and registers it into the security descriptor database.
Arguments:
    InputSecurityDescriptor - The new security descriptor to be logged into the database. On a successful return this memory will have been freed back to pool.
...
*/
{
    ...
    do {
        ...
        for (Front = Chain->Head.Flink; Front != &Chain->Head; Front = Front->Flink) {
            ...
            // The list is ordered by full hash value and is maintained this way by virtue of the fact that we use the 'Back' variable for the insert.
            if (Header->FullHash > FullHash) {
                break;
            }

            if (Header->FullHash == FullHash) {
                Match = ObpCompareSecurityDescriptors(InputSecurityDescriptor, Length, &Header->SecurityDescriptor);
                if (Match) {
                    break;
                }
        ...
        //  If we have a match then we'll get the caller to use the old cached descriptor, 
        //  but bumping its ref count, freeing what the caller supplied and returning the old one to our caller
        if (Match) {
        ...
        if (NewDescriptor == NULL) {
            ...
            //  Can't use an existing one, create a new entry and insert it into the list.
            NewDescriptor = ObpCreateCacheEntry(InputSecurityDescriptor, Length, FullHash, RefBias);
            if (NewDescriptor == NULL) {
    ...
    // Insert the entry before the 'Front' entry. If there is no 'Front' entry then this is just inserting at the head
...
PSECURITY_DESCRIPTOR_HEADER ObpCreateCacheEntry(PSECURITY_DESCRIPTOR InputSecurityDescriptor, ULONG SecurityDescriptorLength, ULONG FullHash, ULONG RefBias)
/*
Routine Description:
...
*/
{
    ...
    //  Compute the size that we'll need to allocate.  
    //  We need space for the security descriptor cache minus the funny quad at the end and the security descriptor itself.
    ...
    //  Fill the header, copy over the descriptor data, and return to our caller
...
*/
{
    ...
    InterlockedExchangeAdd((PLONG)&SecurityDescriptorHeader->RefCount, Count);//  Increment the reference count
}
...
*/
{
    ...
    // See if we can fast reference this security descriptor. 
    // Return NULL if there wasn't one and go the slow way if there are no more cached references left.
    Unused = ExFastRefGetUnusedReferences(OldRef);
    if (Unused >= 1 || SecurityDescriptor == NULL) {
        if (Unused == 1) {
            // If we took the counter to zero then attempt to make life easier for the next referencer by resetting the counter to its max.
            // Since we now have a reference to the security descriptor we can do this.
            RefsToAdd = ExFastRefGetAdditionalReferenceCount();
            SecurityDescriptorHeader = SD_TO_SD_HEADER(SecurityDescriptor);
            InterlockedExchangeAdd((PLONG)&SecurityDescriptorHeader->RefCount, RefsToAdd);

            // Try to add the added references to the cache. If we fail then just release them. 
            // This dereference can not take the reference count to zero.
...
*/
{
    ...
    //  NULL out the SecurityDescriptor in the object's header so we don't try to free it again.
...
VOID ObDereferenceSecurityDescriptor(__in PSECURITY_DESCRIPTOR SecurityDescriptor, __in ULONG Count)
/*
Routine Description:
...
*/
{
    ...
    //  Lock the security descriptor cache and get a pointer to the security descriptor header
    ...
    //  Decrement the ref count and if it is now zero then we can completely remove this entry from the cache
...
*/
{
    ...
    return Header;//  Now return the cached descriptor to our caller to free
}


BOOLEAN ObpCompareSecurityDescriptors(IN PSECURITY_DESCRIPTOR SD1, IN ULONG Length1, IN PSECURITY_DESCRIPTOR SD2)
/*
Routine Description:
...
*/
{
    ...
    //  Calculating the length is pretty fast, see if we can get away with doing only that.
