...
extern POBJECT_TYPE IoDeviceObjectType;//  This is the object type for device objects.
extern ULONG ObpLUIDDeviceMapsEnabled;//  Global that enables/disables LUID device maps
...
/*
Routine Description:
    This function creates a symbolic link object, sets it initial value to value specified in the LinkTarget parameter,
...
*/
{
    ...
    if (CapturedLinkTarget.MaximumLength % sizeof(WCHAR)) {//  Check if there is an odd MaximumLength
    ...
    //  Error if link target name length is odd, the length is greater than the maximum length, or zero and creating.
...
*/
{
    ...
    //  Get previous processor mode and probe output arguments if necessary.
    //  The object attributes does not need to be probed because the ObOpenObjectByName does the probe for us
    PreviousMode = KeGetPreviousMode();
    if (PreviousMode != KernelMode) {
        try {
            ProbeForWriteHandle(LinkHandle);
    ...
    //  Open handle to the symbolic link object with the specified desired access, set symbolic link handle value, and return service completion status.
...
*/
{
    ...
    //  Reference symbolic link object by handle, read current state, deference symbolic link object, fill in target name structure and return service status.
    ...
    if (NT_SUCCESS(Status)) {
        ...
        //  If the caller wants a return length and what we found can easily fit in the output buffer then we copy into the output buffer all the bytes from the link.

        //  If the caller did not want a return length and we found can still easily fit in the output buffer then copy over the bytes that just make up the string and nothing extra
        if ((ARGUMENT_PRESENT(ReturnedLength) && (SymbolicLink->LinkTarget.MaximumLength <= CapturedLinkTarget.MaximumLength)) ||
            ...
            try {
                RtlCopyMemory(
                    ...
                    ARGUMENT_PRESENT(ReturnedLength) ? SymbolicLink->LinkTarget.MaximumLength : SymbolicLink->LinkTarget.Length);
            ...
            //  The output buffer is just too small for the link target, but we'll tell the user how much is needed if they asked for that return value
            if (ARGUMENT_PRESENT(ReturnedLength)) {
                try {
                    *ReturnedLength = SymbolicLink->LinkTarget.MaximumLength;
                } except(EXCEPTION_EXECUTE_HANDLER)
...
Return Value:
    ...
    STATUS_REPARSE if the parse object has not been snapped.  In this case the Complete name has been modified with the link target name added in front of the remaining name.
                   The parameters remaining name and object must now be ignored by the caller
...
*/
{
    ...
    //  This routine needs to be synchonized with the delete symbolic link operation.  Which uses the root directory mutex.
    *Object = NULL;

    //  If there isn't any name remaining and the caller gave us an object type then we'll reference the parse object.
    //  If this is successful then that's the object we return.
    //  Otherwise if the status is anything but a type mismatch then we'll return that error status
    if (RemainingName->Length == 0) {
        ...
        //  If the remaining name does not start with a "\" then its is malformed and we'll call it a type mismatch
    } else if (*(RemainingName->Buffer) != OBJ_NAME_PATH_SEPARATOR) {
        return STATUS_OBJECT_TYPE_MISMATCH;
    }

    //  A symbolic link has been encountered. See if this link has been snapped to a particular object.
    SymbolicLink = (POBJECT_SYMBOLIC_LINK)ParseObject;
    if (SymbolicLink->LinkTargetObject != NULL) {
        //  This is a snapped link.  Get the remaining portion of the symbolic link target, if any.
        LinkTargetName = &SymbolicLink->LinkTargetRemaining;
        if (LinkTargetName->Length == 0) {
            //  Remaining link target string is zero, so return to caller quickly with snapped object pointer and remaining object name which we haven't touched yet.
            *Object = SymbolicLink->LinkTargetObject;
            return STATUS_REPARSE_OBJECT;
        }

        //  We have a snapped symbolic link that has additional text.
        //  Insert that in front of the current remaining name, preserving and text between CompleteName and RemainingName
        InsertAmount = LinkTargetName->Length;
        if ((LinkTargetName->Buffer[(InsertAmount / sizeof(WCHAR)) - 1] == OBJ_NAME_PATH_SEPARATOR) && (*(RemainingName->Buffer) == OBJ_NAME_PATH_SEPARATOR)) {
            //  Both the link target name ends in a "\" and the remaining starts with a "\" but we only need one when we're done
            InsertAmount -= sizeof(WCHAR);
        }

        //  We need to bias the difference between two pointers with * sizeof(wchar) because the difference is in wchar and we need the length in bytes
        ...
        //  Now check if the new computed length is too big for the input buffer containing the complete name
        if (CompleteName->MaximumLength <= Length) {
            //  The new concatenated name is larger than the buffer supplied for the complete name.  Allocate space for this new string
            ...
            //  Calculate the pointer within this buffer for the remaining name.  This value has not been biased by the new link target name
            ...
            //  If we have some remaining names then those over at the the location offset to hold the link target name
            if (RemainingName->Length != 0) {
                RtlCopyMemory((PVOID)((PUCHAR)NewRemainingName + InsertAmount), RemainingName->Buffer, RemainingName->Length);
            }

            RtlCopyMemory(NewRemainingName, LinkTargetName->Buffer, InsertAmount);//  Now insert the link target name
            ExFreePool(CompleteName->Buffer);//  Free the old complete name buffer and reset the input strings to use the new buffer
            ...
            //  Insert extra text associated with this symbolic link name before existing remaining name, if any.

            //  First shove over the remaining name to make a hole for the link target name
            if (RemainingName->Length != 0) {
                RtlMoveMemory((PVOID)((PUCHAR)RemainingName->Buffer + InsertAmount), RemainingName->Buffer, RemainingName->Length);
            }

            RtlCopyMemory(RemainingName->Buffer, LinkTargetName->Buffer, InsertAmount);//  Now insert the link target name
            CompleteName->Length = (USHORT)(CompleteName->Length + LinkTargetName->Length);//  Adjust input strings to account for this inserted text
        ...
        //  Return the object address associated with snapped symbolic link and the reparse object status code.
        *Object = SymbolicLink->LinkTargetObject;
        return STATUS_REPARSE_OBJECT;
    }
    ...
    //  Compute the size of the new name and check if the name will fit in the existing complete name buffer.
    ...
    Length = (USHORT)NewLength;
    if (CompleteName->MaximumLength <= Length) {
        //  The new concatenated name is larger than the buffer supplied for the complete name.
        MaximumLength = Length + sizeof(UNICODE_NULL);
        NewName = ExAllocatePoolWithTag(OB_NAMESPACE_POOL_TYPE, MaximumLength, 'mNbO');
        if (NewName == NULL) {
            return STATUS_INSUFFICIENT_RESOURCES;
    ...
    //  Concatenate the symbolic link name with the remaining name, if any.
    //  What this does is overwrite the front of the complete name up to the remaining name with the links target name
    if (RemainingName->Length != 0) {
    ...
    //  If a new name buffer was allocated, then free the original complete name buffer.
    if ((NewName != CompleteName->Buffer) && (CompleteName->Buffer != NULL)) {
        ExFreePool(CompleteName->Buffer);
    }

    //  Set the new complete name buffer parameters and return a reparse status.
    CompleteName->Buffer = NewName;
    CompleteName->Length = Length;
    CompleteName->MaximumLength = MaximumLength;
...
*/
{
    ...
    //  The only cleaning up we need to do is to free the link target buffer
    if (SymbolicLink->LinkTarget.Buffer != NULL) {
...
Routine Description:
    ...
    1.  Determine if the name of the symbolic link is a drive letter.
        If so, then we will need to update the drive type in the associated device map object.
    2.  Process the link target, trying to resolve it into a pointer to an object other than a object directory object.
        All object directories traversed must grant world traverse access other wise we bail out.
        If we successfully find a non object directory object, then reference the object pointer and store it in the symbolic link object,
        along with a remaining string if any.
        ObpLookupObjectName will used this cache object pointer to short circuit the name lookup directly to the cached object's parse routine.
        For any object directory objects traversed along the way, increment their symbolic link SymbolicLinkUsageCount field.
        This field is used whenever an object directory is deleted or its security is changed such that it no longer grants world traverse access.
        In either case, if the field is non-zero we walk all the symbolic links and resnap them.
Arguments:
...
*/
{
    ...
    //  Now see if this symbolic link is being created in an object directory controlled by a device map object.
    //  Since we are only called from NtCreateSymbolicLinkObject, after the handle to this symbolic link
    //  has been created but before it is returned to the caller the handle can't be closed while we are executing, unless via a random close,
    ...
    //  Here if we are creating a symbolic link in an object directory controlled by a device map object.
    //  See if this is a drive letter definition.
    //  If so calculate the drive letter index and remember in the symbolic link object.
...
Routine Description:
    ...
    For creates, it attempts to snap the symbolic link to a non-object directory object.
    It does this by walking the symbolic link target string, until it sees a non-directory object or a directory object that does NOT allow World traverse access.
    It stores a referenced pointer to this object in the symbolic link object.
    It also increments a count in each of the object directory objects that it walked over.
    This count is used to disallow any attempt to remove World traverse access from a directory object after it has participated in a snapped symbolic link.

    For deletes, it repeats the walk of the target string, decrementing the count associated with each directory object walked over.
    It also dereferences the snapped object pointer.
Arguments:
...
*/
{
    ...
    //  Check if we are creating a symbolic link or if the link has already been snapped
    if ((Action == CREATE_SYMBOLIC_LINK) || (SymbolicLink->LinkTargetObject != NULL)) {
        ...
        // If LUID device maps are enabled, then use the Object Manager's pointer to the global device map
        // With LUID device maps enabled, the process' device map pointer may be NULL
        if (ObpLUIDDeviceMapsEnabled != 0) {
            DeviceMap = ObSystemDeviceMap;
        } else {
            DeviceMap = PsGetCurrentProcess()->DeviceMap;// use the device map associated with the process
        }

    ReCalcDeviceMap:
        if (DeviceMap) {
            if (!((ULONG_PTR)(RemainingName.Buffer) & (sizeof(ULONGLONG) - 1)) && (DeviceMap->DosDevicesDirectory != NULL)) {
                //  Check if the object name is actually equal to the global dos devices short name prefix "\??\"
                if ((RemainingName.Length >= ObpDosDevicesShortName.Length) && (*(PULONGLONG)(RemainingName.Buffer) == ObpDosDevicesShortNamePrefix.Alignment.QuadPart)) {
        ...
        //  The following loop will dissect the link target checking that each directory exists and that we have access to the directory.
        //  When we pop out the local directories array will contain a list of directories that we need to traverse to process this action.
        while (TRUE) {
            ...
            //  And dissect the name into its first component and any remaining part
            ...
            //  Look this component name up in this directory.  If not found, then bail.

            //  If we are searching the same directory that contains the sym link we have already the directory exclusively locked.
            //  We need to adjust the lookupcontext state and avoid recursive locking
            ...
            if (ObjectHeader->Type == ObpDirectoryObjectType) {
                ...
                // To prevent Denial of Service attacks from parsing symbolic links infinitely.
                ...
                //  Found a symbolic link to another symbolic link that is not already snapped.
                //  So switch to its target string so we can chase down the real device object
                ParentDirectory = NULL;
                Directory = ObpRootDirectoryObject;

                if (RemainingTarget.Length == 0) {
                    RemainingTarget = RemainingName;//  Save the remaining name
                }

                RemainingName = ((POBJECT_SYMBOLIC_LINK)Object)->LinkTarget;
                goto ReCalcDeviceMap;
            } else {
                //  Not an object directory object, or a symbolic link to an unsnapped symbolic link, so all done.  Exit the loop
                break;
            }
        }
    }

    //  Done processing symbolic link target path.  Update symbolic link object as appropriate for passed in reason

    //  If this is a drive letter symbolic link,
    //  get the address of the device map object that is controlling the containing object directory so we can update the drive type in the device map.
    DeviceMap = NULL;
    ...
    if (Action == CREATE_SYMBOLIC_LINK) {
        ...
        if (Object != NULL) {
            // We only want to do snapping for console session.
            // When we create a remote session all the symbolic links stored in the console dos devices directory (\??) are copied into the per session DosDevices object directory (\Session\<id>\DosDevices).
            // We don't want to do snapping for the copied symbolic links since for each copy we will increment the ref count on the target object.
            // All these counts have to go to zero before the device can be deleted.

            // Disable snapping until we come up with a delete scheme for it
            if (FALSE /*( PsGetCurrentProcess()->SessionId == 0) || (DeviceMapUsed)*/) {
                //  Create action.  Store a referenced pointer to the snapped object along with the description of any remaining name string.
                //  Also, for Dos drive letters, update the drive type in the appropriate device map object.
            ...
            if (SymbolicLink->DosDeviceDriveIndex != 0) {
                //  Default is to calculate the drive type in user mode if we are unable to snap the symbolic link or it does not resolve to a DEVICE_OBJECT we know about.
                ObjectHeader = OBJECT_TO_OBJECT_HEADER(Object);
                if (ObjectHeader->Type == IoDeviceObjectType) {
                    ...
                    case FILE_DEVICE_NETWORK_FILE_SYSTEM:
                        ...
                        //  drive is a redirected drive, but needs to look like a local drive.
        ...
        //  Deleting the symbolic link.  Dereference the snapped object pointer if any and zero out the snapped object fields.
        RtlInitUnicodeString(&SymbolicLink->LinkTargetRemaining, NULL);
        Object = SymbolicLink->LinkTargetObject;
        if (Object != NULL) {
            SymbolicLink->LinkTargetObject = NULL;
        ...
        //  If this is a drive letter symbolic link, set the drive type to unknown and clear the bit in the drive letter bit map.
        if (DeviceMap != NULL) {
            ObpLockDeviceMap();
            DeviceMap->DriveMap &= ~(1 << (SymbolicLink->DosDeviceDriveIndex - 1));
        ...
        //  N.B. The original code freed here the target buffer.
        //  This is illegal because the parse routine for symbolic links reads the buffer unsynchronized
