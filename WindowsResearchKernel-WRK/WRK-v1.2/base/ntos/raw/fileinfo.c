...
Abstract:
    This module implements the File Information routines for Raw called by the dispatch driver.
*/
...
NTSTATUS RawQueryInformation(IN PVCB Vcb, IN PIRP Irp, PIO_STACK_LOCATION IrpSp)
/*
Routine Description:
    This is the routine for querying file information, though only query current file position is supported.
Arguments:
...
*/
{
    ...
    if (FileInformationClass == FilePositionInformation) {
        ...
        if (*Length < sizeof(FILE_POSITION_INFORMATION)) {
            ...
            Buffer->CurrentByteOffset = IrpSp->FileObject->CurrentByteOffset;//  Get the current position found in the file object.
...
NTSTATUS RawSetInformation(IN PVCB Vcb, IN PIRP Irp, PIO_STACK_LOCATION IrpSp)
/*
Routine Description:
    This is the routine for setting file information, though only setting current file position is supported.
Arguments:
    Vcb - Supplies the volume being queried.
    Irp - Supplies the Irp being processed.
...
*/
{
    ...
    if (FileInformationClass == FilePositionInformation) {
        //  Check that the new position we're supplied is aligned properly for the device.
