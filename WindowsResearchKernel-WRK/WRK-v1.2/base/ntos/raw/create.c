...
Abstract:
    This module implements the File Create routine for Raw called by the dispatch driver.
*/
...
NTSTATUS RawCreate(IN PVCB Vcb, IN PIRP Irp, IN PIO_STACK_LOCATION IrpSp)
/*
Routine Description:
    Open the volume.
Arguments:
...
*/
{
    ...
    //  This is an open/create request.  The only valid operation that is supported by the RAW file system is if the caller:
    ...
    // Don't allow any relative opens as well as opens with a filename. These opens have only been checked for traverse access by the I/O manager.
    if (((IrpSp->FileObject == NULL) || 
        ...
        if (FlagOn(Vcb->VcbState, VCB_STATE_FLAG_LOCKED)) {
            ...
            //  If the volume is already opened by someone then we need to check the share access
            USHORT ShareAccess;
            ACCESS_MASK DesiredAccess;

            ShareAccess = IrpSp->Parameters.Create.ShareAccess;
            DesiredAccess = IrpSp->Parameters.Create.SecurityContext->DesiredAccess;
            if ((Vcb->OpenCount > 0) && !NT_SUCCESS(Status = IoCheckShareAccess(DesiredAccess, ShareAccess, IrpSp->FileObject, &Vcb->ShareAccess, TRUE))) {
                Irp->IoStatus.Information = 0;
            } else {
                //  This is a valid create.  Increment the "OpenCount" and stuff the Vpb into the file object.
                if (Vcb->OpenCount == 0) {
        ...
        //  Fail this I/O request since one of the above conditions was not met.
        Status = STATUS_INVALID_PARAMETER;
        Irp->IoStatus.Information = 0;
    }

    //  If this was not successful and this was the first open on the volume, we must implicitly dis-mount the volume.
