...
*/
{
    ...
    //  Each of the procedures that we're calling fills up the output buffer if possible and returns true if it successfully filled the buffer and false if it couldn't wait for any I/O to complete.
    ...
    //  Set the information field to the number of bytes actually filled in, and complete the request.
...
Arguments:
    ...
    Length - Supplies the length of the buffer in byte.  This variable upon return receives the remaining bytes free in the buffer
Return Value:
...
*/
{
    ...
    *Length -= FIELD_OFFSET(FILE_FS_VOLUME_INFORMATION, VolumeLabel[0]);
    return STATUS_SUCCESS;//  Set our status and return to our caller
}
...
*/
{
    ...
    //  Prepare for our device control below.
    //  The device drivers only have to copy geometry and partition info from in-memory structures, so it is OK to make these calls even when we can't wait.
    ...
    //  If this call didn't succeed, the drive hasn't even been low-level formatted, and thus geometry information is undefined.
    if (!NT_SUCCESS(Status)) {
        *Length = 0;
        return Status;
    }

    //  See if we have to check the partition information (floppy disks are the only type that can't have partitions )
    if (FlagOn(RealDevice->Characteristics, FILE_FLOPPY_DISKETTE)) {
        ...
        KeResetEvent(&Event);
        Irp = IoBuildDeviceIoControlRequest(IOCTL_DISK_GET_LENGTH_INFO, RealDevice, NULL, 0, &GetLengthInformation, sizeof(GET_LENGTH_INFORMATION), FALSE, &Event, &Iosb);
        if (Irp == NULL) {
            return STATUS_INSUFFICIENT_RESOURCES;
        }
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            KeResetEvent(&Event);
            Irp = IoBuildDeviceIoControlRequest(IOCTL_DISK_GET_PARTITION_INFO, RealDevice, NULL, 0, &PartitionInformation, sizeof(PARTITION_INFORMATION), FALSE, &Event, &Iosb);
            if (Irp == NULL) {
                return STATUS_INSUFFICIENT_RESOURCES;
            }
    ...
    //  Now, based on whether the disk is partitioned, compute the total number of sectors on this disk.
    ...
    *Length -= sizeof(FILE_FS_SIZE_INFORMATION);//  Adjust the length variable
    return STATUS_SUCCESS;//  And return success to our caller
}
...
*/
{
    ...
    *Length -= sizeof(FILE_FS_DEVICE_INFORMATION);//  Adjust the length variable
    return STATUS_SUCCESS;//  And return success to our caller
}
...
*/
{
    ...
    *Length -= LengthUsed;//  Adjust the length variable
    return STATUS_SUCCESS;//  And return success to our caller
}