...
NTSTATUS RawShutdown(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp);
...
/*
Routine Description:
    This is the initialization routine for the Raw file system device driver.
    This routine creates the device object for the FileSystem device and performs all other driver initialization.
Arguments:
...
*/
{
    ...
    Status = IoCreateDevice(DriverObject, 0L, &NameString, FILE_DEVICE_DISK_FILE_SYSTEM, 0, FALSE, &RawDeviceDiskObject);
    ...
    Status = IoCreateDevice(DriverObject, 0L, &NameString, FILE_DEVICE_CD_ROM_FILE_SYSTEM, 0, FALSE, &RawDeviceCdRomObject);
    if (!NT_SUCCESS(Status)) {
    ...
    Status = IoCreateDevice(DriverObject, 0L, &NameString, FILE_DEVICE_TAPE_FILE_SYSTEM, 0, FALSE, &RawDeviceTapeObject);
    if (!NT_SUCCESS(Status)) {
    ...
    //  Note that only a limited capability is supported by the raw file system.
    DriverObject->MajorFunction[IRP_MJ_CREATE] =
        ...
        DriverObject->MajorFunction[IRP_MJ_PNP] = (PDRIVER_DISPATCH)RawDispatch;
