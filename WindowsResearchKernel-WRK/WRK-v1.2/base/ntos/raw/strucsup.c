...
Abstract:
    This module implements the Raw in-memory data structure manipulation routines
*/
...
NTSTATUS RawInitializeVcb (IN OUT PVCB Vcb, IN PDEVICE_OBJECT TargetDeviceObject, IN PVPB Vpb)
/*
Routine Description:
    This routine initializes and inserts a new Vcb record into the in-memory data structure.
    The Vcb record "hangs" off the end of the Volume device object and must be allocated by our caller.
Arguments:
...
*/
{
    ...
    //  We start by first zeroing out all of the VCB, this will guarantee that any stale data is wiped clean
    ...
    //  No need to take a extra reference on the Target Device object as IopMountVolume already has taken a reference.
...
BOOLEAN RawCheckForDismount (PVCB Vcb, BOOLEAN CalledFromCreate)
/*
Routine Description:
    This routine determines if a volume is ready for deletion.
    It correctly synchronizes with creates en-route to the file system.
...
*/
{
    ...
    ReferenceCount = Vcb->Vpb->ReferenceCount;
    {
        ...
        //  If a create is in progress on this volume, don't delete it.
        if ( ReferenceCount != (ULONG)(CalledFromCreate ? 1 : 0) ) {
            //  Cleanup the vpb on a forced dismount even if we can't delete the vcb if we haven't already done so
    ...
    if (DeleteVolume) {
        ...
        //  Free the spare vpb if we didn't use it or the original one if we did use it and there are no more reference counts.
        //  Otherwise i/o subsystem still has a ref and will free the vpb itself
        if (Vcb->SpareVpb) {
