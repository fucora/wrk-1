...
NTSTATUS RawMountVolume(IN PIO_STACK_LOCATION IrpSp);
NTSTATUS RawVerifyVolume(IN PIO_STACK_LOCATION IrpSp, IN PVCB Vcb);
NTSTATUS RawUserFsCtrl(IN PIO_STACK_LOCATION IrpSp, IN PVCB Vcb);
...
NTSTATUS RawFileSystemControl(IN PVCB Vcb, IN PIRP Irp, IN PIO_STACK_LOCATION IrpSp)
/*
Routine Description:
...
*/
{
    ...
    //  We know this is a file system control so we'll case on the minor function, and call an internal worker routine.
...
*/
{
    ...
    // A mount operation has been requested.  Create a new device object to represent this volume.
    ...
    //  Our alignment requirement is the larger of the processor alignment requirement already in the volume device object and that in the DeviceObjectWeTalkTo
    ...
    if (!NT_SUCCESS(Status)) {
        //  Unlike the other points of teardown we do not need to deref the target device a iosubsys will automatically do that for a failed mount
        IoDeleteDevice((PDEVICE_OBJECT)VolumeDeviceObject);
        return Status;
    }

    //  Finally, make it look as if the volume has been mounted.
    //  This includes storing the address of this file system's device object (the one that was created to handle this volume) in the VPB so all requests are directed to this file system from now until the volume is initialized with a real file structure.
    ...
    VolumeDeviceObject->DeviceObject.StackSize = (UCHAR)(DeviceObjectWeTalkTo->StackSize + 1);
    {
        ...
        //  We need to bump the count up 2 now so that the close we do in a few lines doesn't make the Vcb go away now.
        VolumeDeviceObject->Vcb.OpenCount += 2;
        FsRtlNotifyVolumeEvent(VolumeFileObject, FSRTL_VOLUME_MOUNT);
        ObDereferenceObject(VolumeFileObject);

        //  Okay, the close is over, now we can safely decrement the open count again (back to 0) so the Vcb can go away when we're really done with it.
        VolumeDeviceObject->Vcb.OpenCount -= 2;
    }
...
*/
{
    ...
    //  If the volume is somehow stale, dismount.
    //  We must synchronize our inspection of the close count so we don't rip the volume up while racing with a close, for instance.
    //  The VPB refcount drops *before* the close comes into the filesystem.

    // By this time its possible that the volume has been dismounted by RawClose.
    // So check if its mounted. If so, take a reference on the VPB The reference on the VPB will prevent close from deleting the device.
    ...
    //  Since we ignore all verify errors from the disk driver itself, this request must have originated from a file system, thus since we weren't the originators, we're going to say this isn't our volume, and if the open count is zero, dismount the volume.
    IoAcquireVpbSpinLock(&Irql);
    vpb->ReferenceCount--;
    IoReleaseVpbSpinLock(Irql);
...
*/
{
    ...
    //  Do pre-notification before entering the volume mutex so that we can be reentered by good threads cleaning up their resources.
    ...
    case FSCTL_UNLOCK_VOLUME:
        ...
        break;
    case FSCTL_DISMOUNT_VOLUME:
        //  Right now the logic in cleanup.c assumes that there can only be one handle on the volume if locked.  The code there needs to be fixed if forced dismounts are allowed.
        if (FlagOn(Vcb->VcbState, VCB_STATE_FLAG_LOCKED)) {
