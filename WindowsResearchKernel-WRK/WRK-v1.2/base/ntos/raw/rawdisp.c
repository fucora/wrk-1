...
Abstract:
    ...
    It is responsible for dispatching the request to the appropriate routine.
*/
...
Routine Description:
    ...
    It is the worker function's responsibility to appropriately complete the IRP.
Arguments:
...
*/
{
    ...
    //  This location contains the function codes and parameters for this particular request.
    IrpSp = IoGetCurrentIrpStackLocation( Irp );

    //  Check for operations associated with our FileSystemDeviceObjects as opposed to our VolumeDeviceObjects.
    //  Only mount is allowed to continue through the normal dispatch in this case.
    ...
    //  Get a pointer to the Vcb. 
    //  Note that is we are mount a volume this pointer will not have meaning, but that is OK since we will not use it in that case.
    Vcb = &VolumeDeviceObject->Vcb;

    //  Case on the function that is being performed by the requestor.
    //  We should only see expected requests since we filled the dispatch table by hand.
    try {
...
//  Completion routine for read, write, and device control to deal with verify issues.  Implemented in RawDisp.c
NTSTATUS RawCompletionRoutine(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
    PIO_STACK_LOCATION IrpSp = IoGetCurrentIrpStackLocation( Irp );

    //  Simply update the file pointer context in the file object if we were successful and this was a synrchonous read or write.
    if (((IrpSp->MajorFunction == IRP_MJ_READ) || (IrpSp->MajorFunction == IRP_MJ_WRITE)) && 
         (IrpSp->FileObject != NULL) && 
        FlagOn(IrpSp->FileObject->Flags, FO_SYNCHRONOUS_IO) && NT_SUCCESS(Irp->IoStatus.Status))
    {
        IrpSp->FileObject->CurrentByteOffset.QuadPart = IrpSp->FileObject->CurrentByteOffset.QuadPart + Irp->IoStatus.Information;
    }

    if ( Irp->PendingReturned ) {//  If IoCallDriver returned PENDING, mark our stack location with pending.
