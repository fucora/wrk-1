...
Abstract:
    This module implements the support routines for the Virtual Address Control Block support for the Cache Manager.
    These routines are used to manage a large number of relatively small address windows to map file data for all forms of cache access.
*/

#include "cc.h"
#include "ex.h"

#define me 0x000000040//  Define our debug constant
...
_inline VOID ReferenceVacbLevel(
...
)
{
    ...
    ASSERT(Amount > 0 || (!Special && VacbReference->Reference >= (0 - Amount)) || (Special && VacbReference->SpecialReference >= (0 - Amount)));
...
#define CcMoveVacbToReuseFree(V)        RemoveEntryList( &(V)->LruList );   InsertHeadList( &CcVacbFreeList, &(V)->LruList );
#define CcMoveVacbToReuseTail(V)        RemoveEntryList( &(V)->LruList );   InsertTailList( &CcVacbLru, &(V)->LruList );


//  If the HighPart is nonzero, then we will go to a multi-level structure anyway, which is most easily triggered by returning MAXULONG.
...
//  When the VACB array size is extended to make room for the BCB list heads, we need to make sure that the space allocated for these list heads will be
//  sizeof( LIST_ENTRY ) aligned, since BCB list heads are LIST_ENTRY
//  structures.  The VACB array size should already be PVOID aligned, so at most, we will need to increase the size by sizeof( PVOID ).
...
/*
Routine Description:
    This routine must be called during Cache Manager initialization to initialize the Virtual Address Control Block structures.
*/
{
    SIZE_T VacbBytes;
    PVACB NextVacb;
...
PVOID CcGetVirtualAddressIfMapped(
    IN PSHARED_CACHE_MAP SharedCacheMap,
...
/*
Routine Description:
    This routine returns a virtual address for the specified FileOffset, iff it is mapped.
    Otherwise, it informs the caller that the specified virtual address was not mapped.
    In the latter case, it still returns a ReceivedLength, which may be used to advance to the next view boundary.
Arguments:
    ...
    Vach - Returns a Vacb pointer which must be supplied later to free this virtual address, or NULL if not mapped.
    ReceivedLength - Returns the number of bytes to the next view boundary, whether the desired file offset is mapped or not.
Return Value:
    The virtual address at which the desired data is mapped, or NULL if it is not mapped.
*/
{
    KIRQL OldIrql;
    ...
    *ReceivedLength = VACB_MAPPING_GRANULARITY - VacbOffset;
    ...
    //  The VacbPushLock in the SharedCacheMap was introduced to provide mutually exclusive synchronization between threads testing to see if
    //  views are mapped for this stream and threads unmapping views for this stream from the system cache.  We need to provide synchronization
    //  across both Cc and Mm's data structure setup in these cases to avoid the race described below.

    //  VacbLock must be released during the call to CcUnmapVacb() because it contains a call to MmUnmapViewInSystemCache() (this routine cannot be
    //  called with a spinlock held).  It is this MM call that is responsible for copying the dirty bit from the PTEs back to the PFN.

    //  During this time the worker thread may call CcFlushCache() on the Vacb being unmapped.  CcGetVirtualAddressIfMapped() is used to determine
    //  if the Vacb's memory is mapped and will correctly report that the address is not mapped.  If the address is not mapped, CcFlushCache() will not
    //  call MmSetAddressRangeModified() to propagate the dirty bit from the PTE to the PFN, but CcFlushCache() will proceed to call MmFlushSection().

    //  This is where we have synchronization problems.  If MmUnmapViewInSystemCache() is not finished propagating the dirty PTE information back to the
    //  PFN when MmFlushSection() executes, the Mm doesn't think there is any dirty data to flush.  But, Cc has cleared its dirty hint and seen a
    //  successful flush, so Cc thinks the data has been flushed.  We are left with dirty data sitting in memory until:
    //   * the mapped page writer comes along and writes the data,
    //   * the data is dirtied again via the system cache so Cc will try to flush it again,
    //   * or, the dirty data says in memory forever because the page has
    //     modified writing disabled (only Cc can flush it) and it never gets dirtied again through the system cache.

    //  The solution to this problem is to introduce the SharedCacheMap->VacbPushLock.  This allows for multiple threads to unmap
    //  views as needed, but we synchronize to ensure that no thread is unmapping views for this stream (the CcGetVacbMiss() and
    //  CcUnmapVacbArray() paths) while another thread is checking to see if that view is still mapped (the CcGetVirtualAddressIfMapped() path).
    ...
    if ((*Vacb = GetVacb(SharedCacheMap, *(PLARGE_INTEGER)&FileOffset)) != NULL) {
        ...
        //  Move this range away from the front to avoid wasting cycles looking at it for reuse.
...
/*
Routine Description:
    This is the main routine for Vacb management.
    It may be called to acquire a virtual address for a given file offset.
    If the desired file offset is already mapped, this routine does very little work before returning with
    the desired virtual address and Vacb pointer (which must be supplied to free the mapping).

    If the desired virtual address is not currently mapped, then this routine claims a Vacb from the tail of the Vacb LRU to reuse its mapping.
    This Vacb is then unmapped if necessary (normally not required), and mapped to the desired address.
Arguments:
    SharedCacheMap - Supplies a pointer to the Shared Cache Map for the file.
    FileOffset - Supplies the desired FileOffset within the file.
    Vacb - Returns a Vacb pointer which must be supplied later to free this virtual address.
    ReceivedLength - Returns the number of bytes which are contiguously mapped starting at the virtual address returned.
Return Value:
...
*/
{
    ...
    //  might unmap a Vacb.  See CcGetVirtualAddressIfMapped() for more details.
    ExAcquirePushLockShared(&SharedCacheMap->VacbPushLock);

    //  If this file has modified writing disabled, it may be large enough to to use BCB list heads for BCB management.  If this file uses BCB list
    //  heads for BCB management, mapping a new view may cause more BCB list heads to be added to the BcbList.  In this case, we must acquire both
    //  the SharedCacheMap->BcbSpinLock and the VacbLock to synchronize this correctly.  Otherwise, the VACB lock is sufficient for synchronizing
    //  the VACB lookup.
    ...
    *ReceivedLength = VACB_MAPPING_GRANULARITY - VacbOffset;
    ...
    //  PREfix wants to know this cannot be NULL, otherwise it will complain about users of this function.
...
Routine Description:
    This is the main routine for Vacb management.
    It may be called to acquire a virtual address for a given file offset.
    If the desired file offset is already mapped,
    this routine does very little work before returning with the desired virtual address and Vacb pointer (which must be supplied to free the mapping).
...
Arguments:
    ...
    HasBcbListHeads - TRUE if the stream is large enough and of the type to use Bcb list head markers, FALSE otherwise.
Return Value:
    The Vacb.
*/
{
    ...
    //  For files that are not open for random access, we assume sequential access and periodically unmap unused views behind us as we go, to
    //  keep from hogging memory.

    //  We used to only do this for pure FO_SEQUENTIAL_ONLY access.  The sequential flags still has an effect (to put the pages at the front
    //  of the standby lists) but we intend for the majority of the file cache to live on the standby and are willing to take transition
    //  faults to bring it back.  Granted, this exacerbates the problem that it is hard to figure out how big the filecache really is since even
    //  less of it is going to be mapped at any given time.  It may also promote the synchronization bottlenecks in view mapping (MmPfnLock)
    //  to the forefront when significant view thrashing occurs.

    //  This isn't as bad as it seems.  When we see access take a view miss, it is really likely that it is a result of sequential access.  As long
    //  as the pages go onto the back of the standby, they'll live for a while.
    //  The problem we're dealing with here is that the cache can be filled at high speed, but the working set manager can't possibly trim it as fast,
    //  intelligently, while we have a pretty good guess where the candidate pages should come from.  We can't let the filecache size make large
    //  excursions, or we'll kick out a lot of valuable pages in the process.

    if (!FlagOn(SharedCacheMap->Flags, RANDOM_ACCESS_SEEN) && ((NormalOffset.LowPart & (SEQUENTIAL_MAP_LIMIT - 1)) == 0) && (NormalOffset.QuadPart >= (SEQUENTIAL_MAP_LIMIT * 2))) {
        //  Use MappedLength as a scratch variable to form the offset to start unmapping.  We are not synchronized with these past
        //  views, so it is possible that CcUnmapVacbArray will kick out early when it sees an active view.  That is why we go back
        //  twice the distance, and effectively try to unmap everything twice.  The second time should normally do it.  If the file
        //  is truly sequential only, then the only collision expected might be the previous view if we are being called from readahead,
        //  or there is a small chance that we can collide with the Lazy Writer during the small window where he briefly maps
    ...
    if (!IsListEmpty(&CcVacbFreeList)) {
        ...
        while (TRUE) {
            ...
            //  it is an Active Vacb, delete it now, because the reader may be idle and we want to clean up.
            OldSharedCacheMap = Vacb->SharedCacheMap;
            if ((Vacb->Overlay.ActiveCount == 0) || ((ActiveVacb == NULL) && (OldSharedCacheMap != NULL) && (OldSharedCacheMap->ActiveVacb == Vacb))) {
                //  The normal case is that the Vacb is no longer mapped and we can just get out and use it, however, here we
                ...
                if (Vacb->BaseAddress != NULL) {
                    ...
                    if (Vacb->Overlay.ActiveCount != 0) {
                        ...
                        //  is still mapped we can now safely increment the open count.
                    } else {
                        //  Note that if the SharedCacheMap is currently being deleted, we need to skip over
                        //  it, otherwise we will become the second deleter.  CcDeleteSharedCacheMap clears the pointer in the SectionObjectPointer.
            ...
            //  Advance to the next guy if we haven't scanned the entire list.
            if (Vacb->LruList.Flink != &CcVacbLru) {
                Vacb = CONTAINING_RECORD(Vacb->LruList.Flink, VACB, LruList);
            } else {
                CcReleaseBcbSpinLockAndVacbLock(HasBcbListHeads, LockHandle);

                //  If we found an active vacb, then free it and go back and try again.  Else it's time to bail.
                if (ActiveVacb != NULL) {
                    CcFreeActiveVacb(ActiveVacb->SharedCacheMap, ActiveVacb, ActivePage, PageIsDirty);
                    ActiveVacb = NULL;

                    //  Reacquire spinlocks to loop back and position ourselves at the head of the LRU for the next pass.
                    CcAcquireBcbSpinLockAndVacbLock(HasBcbListHeads, SharedCacheMap, LockHandle);
                    Vacb = CONTAINING_RECORD(CcVacbLru.Flink, VACB, LruList);
                } else {
                    ExReleasePushLockShared(&SharedCacheMap->VacbPushLock);
    ...
    //  Unlink it from the other SharedCacheMap, so the other guy will not try to use it when we free the spin lock.
    ...
    //  Mark it in use so no one else will muck with it after we release the spin lock.
    Vacb->Overlay.ActiveCount = 1;
    SharedCacheMap->VacbActiveCount += 1;
    CcReleaseBcbSpinLockAndVacbLock(HasBcbListHeads, LockHandle);
    ...
    if (Vacb->BaseAddress != NULL) {
        ...
        //  Now we can decrement the open count as we normally do, possibly deleting the guy.
        CcAcquireMasterLock(&LockHandle->OldIrql);

        //  Now release our open count.
        CcDecrementOpenCount(OldSharedCacheMap, 'mvGF');
        if ((OldSharedCacheMap->OpenCount == 0) && !FlagOn(OldSharedCacheMap->Flags, WRITE_QUEUED) && (OldSharedCacheMap->DirtyPages == 0)) {
    ...
    //  Assume we are mapping to the end of the section, but reduce to our normal mapping granularity if the section is too large.
    ...
    try {
        ...
        Status = MmMapViewInSystemCache(SharedCacheMap->Section, &Vacb->BaseAddress, &NormalOffset, &MappedLength.LowPart);
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            //  Vacb->Overlay.ActiveCount == 1 ensures that we are the only folks accessing this Vacb right now as we set it up so we can
        ...
        if (SharedCacheMap->SectionSize.QuadPart > VACB_SIZE_OF_FIRST_LEVEL) {
            ...
            if (!CcPrefillVacbLevelZone(CcMaxVacbLevelsSeen - 1,
                                        ...
                                        HasBcbListHeads,
                                        SharedCacheMap)) {
                //  We can't setup the Vacb levels, so we will raise the error here and the finally clause will do the proper cleanup.

                //  Since the Vacb->BaseAddress is non-NULL we will do the proper unmapping work in the finally.
                ExRaiseStatus(STATUS_INSUFFICIENT_RESOURCES);
            }
        ...
        if (AbnormalTermination()) {
            ...
            //  If there is someone waiting for this count to go to zero, wake them here.
        ...
        //  the Shared Cache Map.  (We have to rewrite the ActiveCount since it is overlaid.)  To do this we must reacquire the
        //  spin lock one more time.  Note we have to check for the unusual
        //  case that someone beat us to mapping this view, since we had to drop the spin lock.
        if ((TempVacb = GetVacb(SharedCacheMap, NormalOffset)) == NULL) {
            ...
            //  This is the unlucky case where we collided with someone else trying to map the same view.  He can get in because we dropped
            //  the spin lock above.  Rather than allocating events and making someone wait, considering this case is fairly unlikely, we just
            //  dump this one at the head of the LRU and use the one from the guy who beat us.
        } else {
            //  Now we have to increment all of the counts for the one that was already there, then ditch the one we had.
            if (TempVacb->Overlay.ActiveCount == 0) {
                SharedCacheMap->VacbActiveCount += 1;
            }
            ...
            //  On this path we have to release the spinlock to do the unmap, and then reacquire the spinlock before cleaning up.
            CcReleaseBcbSpinLockAndVacbLock(HasBcbListHeads, LockHandle);
            CcUnmapVacb(Vacb, SharedCacheMap, FALSE);
            CcAcquireBcbSpinLockAndVacbLock(HasBcbListHeads, SharedCacheMap, LockHandle);
            CheckedDec(Vacb->Overlay.ActiveCount);
...
/*
Routine Description:
    This routine must be called once for each call to CcGetVirtualAddress to free that virtual address.
Arguments:
...
*/
{
    ...
    //  If the count goes to zero, then we want to decrement the global Active count.
    if (Vacb->Overlay.ActiveCount == 0) {
        //  If the SharedCacheMap address is not NULL, then this one is in use by a shared cache map, and we have to decrement his
        ...
        if (SharedCacheMap != NULL) {
            ...
            CcMoveVacbToReuseTail(Vacb);//  Go to the back of the LRU to save this range for a bit
        } else {
        ...
        //  This range is still in use, so move it away from the front so that it doesn't consume cycles being checked.
        CcMoveVacbToReuseTail(Vacb);
    }
...
/*
Routine Description:
    This is a special form of reference that ensures that the multi-level Vacb structures are expanded to cover a given file offset.
Arguments:
    SharedCacheMap - Supplies a pointer to the Shared Cache Map for the file.
    FileOffset - Supplies the desired FileOffset within the file.
*/
...
/*
Routine Description:
    This routine must be called once for each call to CcReferenceFileOffset to remove the reference.
Arguments:
    SharedCacheMap - Supplies a pointer to the Shared Cache Map for the file.
    FileOffset - Supplies the desired FileOffset within the file.
*/
...
/*
Routine Description:
    This routine may be called to wait for outstanding mappings for a given SharedCacheMap to go inactive.
    It is intended to be called from CcUninitializeCacheMap, which is called by the file systems during cleanup processing.
    In that case this routine only has to wait if the user closed a handle without waiting for all I/Os on the handle to complete.

    This routine returns each time the active count is decremented.
    The caller must recheck his wait conditions on return, either waiting for the ActiveCount to go to 0,
    or for specific views to go inactive (CcPurgeCacheSection case).
Arguments:
    SharedCacheMap - Supplies the Shared Cache Map on whose VacbActiveCount we wish to wait.
*/
{
    KIRQL OldIrql;
    PKEVENT Event;
    ...
    //  on, we can wait here.  The caller must test the count for nonzero before calling this routine.

    //  Since we are being called from cleanup, we cannot afford to fail here.
    CcAcquireVacbLock(&OldIrql);

    //  It is possible that the count went to zero before we acquired the spinlock, so we must handle two cases here.
    if (SharedCacheMap->VacbActiveCount != 0) {
        Event = SharedCacheMap->WaitOnActiveCount;
        if (Event == NULL) {
            //  Take the event.  We avoid dispatcher lock overhead for every single zero transition by only picking up the event when we actually need it.
...
/*
Routine Description:
    This routine may be called to unmap a previously mapped Vacb, and clear its BaseAddress field.
Arguments:
    Vacb - Supplies the Vacb which was returned from CcGetVirtualAddress.
    UnmapBehind - If this is a result of our unmap behind logic (the only case in which we pay attention to sequential hints)
*/
{
    ...
    MmUnmapViewInSystemCache(Vacb->BaseAddress, SharedCacheMap->Section, UnmapBehind && FlagOn(SharedCacheMap->Flags, ONLY_SEQUENTIAL_ONLY_SEEN));
    Vacb->BaseAddress = NULL;
}


NTSTATUS FASTCALL CcCreateVacbArray(IN PSHARED_CACHE_MAP SharedCacheMap, IN LARGE_INTEGER NewSectionSize)
/*
Routine Description:
    This routine must be called when a SharedCacheMap is created to create and initialize the initial Vacb array.
Arguments:
    SharedCacheMap - Supplies the shared cache map for which the array is to be created.
    NewSectionSize - Supplies the current size of the section which must be covered by the Vacb array.
Return Value:
    NTSTATUS.
*/
{
    ...
    //  The following limit is greater than the MM limit (i.e., MM actually only supports even smaller sections).
    //  We have to reject the sign bit, and testing the high byte for nonzero will surely only catch errors.
    ...
    if (NewSize == (PREALLOCATED_VACBS * sizeof(PVACB))) {
        ...
        //  For large metadata streams, double the size to allocate an array of Bcb listheads.  Each two Vacb pointers also
        //  gets its own Bcb listhead, thus requiring double the size.
        ASSERT(SIZE_PER_BCB_LIST == (VACB_MAPPING_GRANULARITY * 2));

        //  If this stream is larger than the size for multi-level Vacbs, then fix the size to allocate the root.
        if (NewSize > VACB_LEVEL_BLOCK_SIZE) {
            ...
            //  Loop to calculate how many levels we have and how much we have to shift to index into the first level.
            do {
                Level += 1;
            ...
            if (FlagOn(SharedCacheMap->Flags, MODIFIED_WRITE_DISABLED) && (NewSectionSize.QuadPart > BEGIN_BCB_LIST_ARRAY)) {
                //  Grow the size we need to allocation sufficiently that it can accommodate the BCB list heads.  This is basically
                //  doubling the size of the array since we have a BCB list head (i.e., LIST_ENTRY) for every 2 VACB pointers.
                GrowArrayForBcbListHeads(SizeToAllocate);
                CreateBcbListHeads = TRUE;
            }

            //  Handle the boundary case by giving the proto-level a reference count.  This will allow us to simply push it
            //  in the expansion case.  In practice, due to pool granularity this will not change the amount of space allocated
            if (NewSize == VACB_LEVEL_BLOCK_SIZE) {
                SizeToAllocate += sizeof(VACB_LEVEL_REFERENCE);
                CreateReference = TRUE;
            }
...
NTSTATUS CcExtendVacbArray(IN PSHARED_CACHE_MAP SharedCacheMap, IN LARGE_INTEGER NewSectionSize)
/*
Routine Description:
    This routine must be called any time the section for a shared cache map is extended, in order to extend the Vacb array (if necessary).
Arguments:
    SharedCacheMap - Supplies the shared cache map for which the array is to be created.
    NewSectionSize - Supplies the new size of the section which must be covered by the Vacb array.
*/
{
    KLOCK_QUEUE_HANDLE LockHandle;
    PVACB *OldAddresses;
    ...
    //  See if we will be growing the Bcb ListHeads, so we can take out the BCB spin lock.
    if (FlagOn(SharedCacheMap->Flags, MODIFIED_WRITE_DISABLED) && (NewSectionSize.QuadPart > BEGIN_BCB_LIST_ARRAY)) {
    ...
    if (NewSectionSize.QuadPart > SharedCacheMap->SectionSize.QuadPart) {
        ...
        if (SharedCacheMap->SectionSize.QuadPart < VACB_SIZE_OF_FIRST_LEVEL) {
            ...
            //  pointer block.  We must adjust for Bcb listheads and the multilevel reference count.
            ...
            if (NewSize > OldSize) {
                ...
                //  See if we will be growing the Bcb ListHeads, if so we need both the BcbSpinLock to sychronize adding the list heads
                //  to the BCB list and the VacbLock to synchronize with anyone who could be unmapping a view at this time.
                CcAcquireBcbSpinLockAndVacbLock(GrowingBcbListHeads, SharedCacheMap, &LockHandle);
                ...
                if (GrowingBcbListHeads) {
                    ...
                    if ((SharedCacheMap->SectionSize.QuadPart > BEGIN_BCB_LIST_ARRAY) && (OldAddresses != NULL)) {
                        ...
                        //  Loop to remove each old listhead and insert the new one in its place.
                        do {
                        ...
                        //  Otherwise, handle the case where we are adding Bcb Listheads.
                    } else {
                        ...
                        while (TempEntry != &SharedCacheMap->BcbList) {
                            //  Sit on this Bcb until we have inserted all listheads that go before it.
                    ...
                    //  Now insert the rest of the new listhead entries that were not finished in either loop above.
            ...
            //  without a spinlock, so long as either Vacbs was not changed, or it was changed together with SectionSize under the spinlock(s) above.
            SharedCacheMap->SectionSize = NextLevelSize;
        }
        ...
        //  we just have to push the root one or more levels linked through the first pointer in the new root(s).
        if (NewSectionSize.QuadPart > SharedCacheMap->SectionSize.QuadPart) {
            PVACB *NextVacbArray;
            ULONG NewLevel;
            ULONG Level = 1;
            ...
            if (NewLevel > Level) {
                ...
                //  Raise if we cannot preallocate enough buffers.
                if (!CcPrefillVacbLevelZone(NewLevel - Level, &LockHandle, FALSE, GrowingBcbListHeads, SharedCacheMap)) {
                    return STATUS_INSUFFICIENT_RESOURCES;
                }

                //  Now if the current Level of the file is 1, we have not been maintaining a reference count, so we have to calculate it before pushing. 
                //  In the boundary case we have made sure that the reference space is available.
                if (Level == 1) {
                    //  We know this is always a leaf-like level right now.
                    CcCalculateVacbLevelLockCount(SharedCacheMap, SharedCacheMap->Vacbs, 0);
                }

                //  Finally, if there are any active pointers in the first level, then we have to create new levels by adding a new root enough times to create
                //  additional levels.  On the other hand, if the pointer count in the top 
                //  level is zero, then we must not do any pushes, because we never allow empty leaves!
                if (IsVacbLevelReferenced(SharedCacheMap, SharedCacheMap->Vacbs, Level - 1)) {
                    ...
                    //  We are now possessed of the additional problem that this level has no references but may have Bcb listheads due to the boundary case where
                    //  we have expanded up to the multilevel Vacbs above.  This level can't remain at the root and needs to be destroyed.  What we need to do is
                    //  replace it with one of our prefilled (non Bcb) levels and unlink the Bcb listheads in the old one.
                ...
                //  These two fields (Vacbs and SectionSize) must be changed while still holding the spinlock.
                SharedCacheMap->SectionSize = NewSectionSize;
                CcReleaseBcbSpinLockAndVacbLock(GrowingBcbListHeads, &LockHandle);
            }
...
BOOLEAN FASTCALL CcUnmapVacbArray(
    IN PSHARED_CACHE_MAP SharedCacheMap,
...
/*
Routine Description:
    This routine must be called to do any unmapping and associated cleanup for a shared cache map, just before it is deleted.
Arguments:
    SharedCacheMap - Supplies a pointer to the shared cache map which is about to be deleted.
    FileOffset - If supplied, only unmap the specified offset and length
    Length - Completes range to unmap if FileOffset specified.  If FileOffset is specified, Length of 0 means unmap to the end of the section.
...
*/
{
    ...
    //  See if a range was specified. Align it to the VACB boundaries so it works in the loop below
    ...
    //  If this file uses BCB list heads for BCB management, mapping a new view may cause more BCB list heads to be added to the BcbList.  In this case,
    //  we must acquire both the SharedCacheMap->BcbSpinLock and the VacbLock to synchronize this correctly.  Otherwise, the VACB lock is sufficient
    //  for synchronizing the VACB manipulations.
    if ((SharedCacheMap->SectionSize.QuadPart > BEGIN_BCB_LIST_ARRAY) && FlagOn(SharedCacheMap->Flags, MODIFIED_WRITE_DISABLED)) {
        HasBcbListHeads = TRUE;
    }

    //  Acquire the SCM's VacbPushLock to synchronize with threads checking for views to be mapped (see large comment in CcGetVirtualAddressIfMapped).
    //  Acquire VACB spin lock to sychronizes our view management.
    ExAcquirePushLockShared(&SharedCacheMap->VacbPushLock);

    //  Acquire the appropriate spin locks to synchronize the VACB lookup and unmapping.
    CcAcquireBcbSpinLockAndVacbLock(HasBcbListHeads, SharedCacheMap, &LockHandle);
    while (StartingFileOffset.QuadPart < EndingFileOffset.QuadPart) {
        //  Note that the caller with an explicit range may be off the end of the section (example CcPurgeCacheSection for cache
        //  coherency).  That is the reason for the first part of the test below.

        //  Check the next cell once without the spin lock, it probably will not change, but we will handle it if it does not.
        if ((StartingFileOffset.QuadPart < SharedCacheMap->SectionSize.QuadPart) && ((Vacb = GetVacb(SharedCacheMap, StartingFileOffset)) != NULL)) {
            //  Return here if we are unlucky and see an active Vacb.  It could be Purge calling, and the Lazy Writer
            ...
            //  Increment the open count so that no one else will try to unmap or reuse until we are done.
            Vacb->Overlay.ActiveCount += 1;
...
/*
Routine Description:
    This routine may be called to prefill the VacbLevelZone with the number of entries required, and return with CcVacbSpinLock acquired.
    This approach is taken so that the pool allocations and RtlZeroMemory calls can occur without
    holding any spinlock, yet the caller may proceed to perform a single indivisible
    operation without error handling, since there is a guaranteed minimum number of entries in the zone.
Arguments:
    NumberNeeded - Number of VacbLevel entries needed, not counting the possible one with Bcb listheads.
    LockHandle - supplies a pointer to the lock handle which should be used to
                 acquire the BCB spin lock or store the OldIrql if only the VACB lock is acquired.
    ...
    AcquireBcbSpinLock - True if the BCB spin lock must be acquired,
        SharedCacheMap must not be NULL.  False if only the VACB spin lock should be acquired.
...
*/
{
    ...
    CcAcquireBcbSpinLockAndVacbLock(AcquireBcbSpinLock, SharedCacheMap, LockHandle);
    ...
    while ((NumberNeeded > CcVacbLevelEntries) || (NeedBcbListHeads && (CcVacbLevelWithBcbsFreeList == NULL))) {
        ...
        if (NeedBcbListHeads && (CcVacbLevelWithBcbsFreeList == NULL)) {
            //  Allocate and initialize the Vacb block for this level, and store its pointer back into our parent.  We do not zero the listhead area.
            NextVacbArray = (PVACB *)ExAllocatePoolWithTag(NonPagedPool, (VACB_LEVEL_BLOCK_SIZE * 2) + sizeof(VACB_LEVEL_REFERENCE), 'lVcC');
            ...
            //  Allocate and initialize the Vacb block for this level, and store its pointer back into our parent.
            NextVacbArray = (PVACB *)ExAllocatePoolWithTag(NonPagedPool, VACB_LEVEL_BLOCK_SIZE + sizeof(VACB_LEVEL_REFERENCE), 'lVcC');
            if (NextVacbArray == NULL) {
                return FALSE;
            }
...
/*
Routine Description:
    This routine should be called any time some entries have been deallocated to the VacbLevel zone,
    and we want to ensure the zone is returned to a normal level.
Environment:
    No spinlocks should be held upon entry.
*/
{
    ...
    //  This is an unsafe loop to see if it looks like there is stuff to clean up.
...
PLIST_ENTRY CcGetBcbListHead(IN PSHARED_CACHE_MAP SharedCacheMap, IN LONGLONG FileOffset, IN BOOLEAN FailToSuccessor)
/*
Routine Description:
    This routine is called to return the Bcb listhead for the specified FileOffset, regardless of the size of the file.
Arguments:
    SharedCacheMap - Supplies the pointer to the SharedCacheMap for which the listhead is desired.
    FileOffset - Supplies the fileOffset corresponding to the desired listhead.
    FailToSuccessor - Instructs whether not finding the exact listhead should cause us to return the predecessor or successor Bcb listhead.
Return Value:
    Returns the desired Listhead pointer.
    If the desired listhead does not actually exist yet, then it returns the appropriate listhead.
Environment:
...
*/
{
    ...
    if ((SharedCacheMap->SectionSize.QuadPart > BEGIN_BCB_LIST_ARRAY) && FlagOn(SharedCacheMap->Flags, MODIFIED_WRITE_DISABLED)) {
        //  We've got BCB list head markers for this stream, so now do the work to find the appropriate list head for this offset.
        if (SharedCacheMap->SectionSize.QuadPart > VACB_SIZE_OF_FIRST_LEVEL) {
            //  We are using the multi-level VACB representation for this stream, so we need to walk the levels to find our BCB list head.
            BcbListHead = CcGetBcbListHeadLargeOffset(SharedCacheMap, FileOffset, FailToSuccessor);
        } else {
            //  We have a single array to represent the VACBs for this
            //  stream.  The BCB list heads are allocated contiguously *behind* the VACBs.
            if (FileOffset >= SharedCacheMap->SectionSize.QuadPart) {
                //  This offset is beyond the section size, so always return the list head so we know to stop looking.
                ...
                //  VacbArray begins, then index into that portion of the array to find the right BCB list head.

                //  Each BCB list head represents SIZE_PER_BCB_LIST amount of
                //  data.  (Dividing by SIZE_PER_BCB_LIST is the same as shifting right by BCB_LIST_SHIFT.)
                PLIST_ENTRY BcbListHeadArray;
                BcbListHeadArray = Add2Ptr(SharedCacheMap->Vacbs, SizeOfVacbArray(SharedCacheMap->SectionSize));
                BcbListHead = &BcbListHeadArray[FileOffset >> BCB_LIST_SHIFT];
            }
        }
    } else {
        //  We don't have BCB list heads for this stream, so just return the
        //  shared cache map's BCB list head and let the caller do a linear search.
        BcbListHead = &SharedCacheMap->BcbList;
    }
...
Arguments:
    ...
    FailToSuccessor - Instructs whether not finding the exact listhead should cause us to return the predecessor or successor Bcb listhead.
Return Value:
    Returns the desired Listhead pointer.  If the desired listhead does not actually exist yet, then it returns the appropriate listhead.
Environment:
    The BcbSpinlock should be held on entry.
*/
{
    ...
    //  Caller must have verified that we have a hierarchy, otherwise this routine would fail.
    ...
    do {
        ...
        if (NextVacbArray == NULL) {
            //  Back up to look for the highest guy earlier in this tree, i.e., the predecessor listhead.
            while (TRUE) {
                ...
                if (FailToSuccessor) {
                    if (Index != VACB_LAST_INDEX_FOR_LEVEL) {
                        ...
                        //  If we found a non-null index, get out and try to return the listhead.
                ...
                //  If there are no saved levels yet, then there is no predecessor or successor - it is the main listhead.
                if (SavedLevels == 0) {
                    return &SharedCacheMap->BcbList;
                }

                //  Otherwise, we can pop up a level in the tree and start scanning from that guy for a path to the right listhead.
                Level += 1;
        ...
        //  We save Index and VacbArray at each level, for the case that we have to walk back up the tree to find a predecessor.
        SavedIndexes[SavedLevels] = Index;
        SavedVacbArrays[SavedLevels] = VacbArray;
        SavedLevels += 1;

        //  Now make this one our current pointer, and mask away the extraneous high-order FileOffset bits for this level.
    ...
    //  the low bit gets you to the even-indexed Vacb pointer which is one block size below the two-pointer listhead for the Bcbs for that .5MB range...)
    Index = (ULONG)(FileOffset >> Shift);
...
/*
Routine Description:
    This routine may be called to adjust the lock count of the bottom Vacb level when Bcbs are inserted or deleted.
    If the count goes to zero, the level will be eliminated.
    The bottom level must exist, or we crash!
Arguments:
    SharedCacheMap - Supplies the pointer to the SharedCacheMap for which the Vacb is desired.
...
*/
{
    ...
    //  upping count and resetting to NULL.  Then smash OriginalFileOffset to be the first entry so we do not recalculate!
...
/*
Routine Description:
    This routine may be called to calculate or recalculate the lock count on a given Vacb level array.
    It is called, for example, when we are extending a section up to the point where we activate multilevel logic and want to start keeping the count.
Arguments:
    SharedCacheMap - Supplies the pointer to the SharedCacheMap for which the Vacb is desired.
...
*/
{
    ...
    //  If this is a metadata stream, we also have to count the Bcbs in the corresponding listheads.
    if (FlagOn(SharedCacheMap->Flags, MODIFIED_WRITE_DISABLED) && (Level == 0)) {
        ...
        //  Now loop through the list.  For each Bcb we see, increment the count, and for each listhead, increment Index.
        //  We are done when we hit the last listhead, which is actually the next listhead past the ones in this block.
        do {
...
Routine Description:
    ...
    For non-null Vacb, intermediate Vacb levels will be added as necessary, and if the lowest level has Bcb listheads, these will also be added.
    For this case the caller must acquire the spinlock by calling CcPrefillVacbLevelZone specifying the worst-case number of levels required.

    For a null Vacb pointer, the tree is pruned of all Vacb levels that go empty.
    If the lowest level has Bcb listheads, then they are removed.
    The caller should subsequently call CcDrainVacbLevelZone once the spinlock is release to actually free some of this zone to the pool.
Arguments:
    SharedCacheMap - Supplies the pointer to the SharedCacheMap for which the Vacb is desired.
    FileOffset - Supplies the fileOffset corresponding to the desired Vacb.
Return Value:
...
*/
{
    ...
    Shift -= VACB_LEVEL_SHIFT;
    do {
        Level -= 1;//  Decrement back to the level that describes the size we are within.

        //  Calculate the index into the Vacb block for this level.
        Index = (ULONG)(FileOffset >> Shift);
        ASSERT(Index <= VACB_LAST_INDEX_FOR_LEVEL);

        //  We save Index and VacbArray at each level, for the case that we are collapsing and deallocating blocks below.
        SavedIndexes[SavedLevels] = Index;
        SavedVacbArrays[SavedLevels] = VacbArray;
        SavedLevels += 1;

        NextVacbArray = (PVACB *)VacbArray[Index];//  Get block address for next level.
        if (NextVacbArray == NULL) {//  If it is NULL then we have to allocate the next level to fill it in.
            //  We better not be thinking we're dereferencing a level if the level doesn't currently exist.
            ...
            if (AllocatingBcbListHeads) {
                ...
                //  If he is followed by any Bcbs, they "belong" to him, and we have to skip over them.
                while (((PBCB)CONTAINING_RECORD(PredecessorListHead->Blink, BCB, BcbLinks))->NodeTypeCode == CACHE_NTC_BCB) {
            ...
            //  Increment the reference count.  Note that Level right now properly indicates what level NextVacbArray is at, not VacbArray.
    ...
    if (Vacb != NULL) {
        ...
        while (TRUE) {
            ...
            //  the only one.  The rest, as we tear up the tree, are regular (calculable) references.
            Special = FALSE;

            //  Now, if we have an empty block (other than the top one), then we should free the block and keep looping.
            if (!IsVacbLevelReferenced(SharedCacheMap, VacbArray, Level) && (SavedLevels != 0)) {
                SavedLevels -= 1;

                //  First see if we have Bcb Listheads to delete and if so, we have to unlink the whole block first.
                ...
                //  Free the unused block and then pick up the saved parent pointer array and index and erase the pointer to this block.
...
Routine Description:
    ...
    Originally, this routine is a macro.
    To reduce the nonpaged footprint of the system we want to page as much as possible,
    and it turns out this was the only reason a substantial part of the cache manager wasn't.
Arguments:
    SharedCacheMap - Supplies the pointer to the SharedCacheMap for which the active Vacb is desired.
...
*/
{
    ...
    //  When setting dirty, when we set ACTIVE_PAGE_IS_DIRTY the first time, we increment the dirty counts, and they never get decremented until
    //  CcFreeActiveVacb.  If we are trying to set and there is already an active Vacb *or* we are trying to set a clean one and the flag above
    //  is set, we do not allow it, and we just free the vacb (we only want to handle the clean transition in one place).

    //  MP & UP cases are separately defined, because I do not trust the compiler to otherwise generate the optimal UP code.
    //  In the MP case, we test if we are setting the page dirty, because then we must acquire CcMasterSpinLock to diddle CcDirtyPages.
    //  In the UP case, any FastLock will do, so we just use the ActiveVacb lock, and do not explicitly acquire CcMasterSpinLock.
