...
VOID CcMdlRead(__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER FileOffset, __in ULONG Length, __out PMDL *MdlChain, __out PIO_STATUS_BLOCK IoStatus)
/*
Routine Description:
    ...
    As each call returns, the pages described by the Mdl are locked in memory, but not mapped in system space.
    If the caller needs the pages mapped in system space, then it must map them.

    Note that each call is a "single shot" which should be followed by a call to CcMdlReadComplete.
    To resume an Mdl-based transfer, the caller must form one or more subsequent calls to CcMdlRead with appropriately adjusted parameters.
Arguments:
    ...
    MdlChain - On output it returns a pointer to an Mdl chain describing the desired data.
               Note that even if FALSE is returned, one or more Mdls may have been allocated, as may be ascertained by the IoStatus.Information field (see below).
    IoStatus - Pointer to standard I/O status block to receive the status for the transfer.
               ...
               The I/O Information Field indicates how many bytes have been successfully locked down in the Mdl Chain.
Raises:
...
*/
{
    ...
    MmSavePageFaultReadAhead(Thread, &SavedState);//  Save the current readahead hints.

    //  Get pointer to SharedCacheMap.
    SharedCacheMap = FileObject->SectionObjectPointer->SharedCacheMap;
    PrivateCacheMap = FileObject->PrivateCacheMap;

    GetActiveVacb(SharedCacheMap, OldIrql, ActiveVacb, ActivePage, PageIsDirty);//  See if we have an active Vacb, that we need to free.

    //  If there is an end of a page to be zeroed, then free that page now, so we don't send Greg the uninitialized data...
    ...
    CcMdlReadWait += 1;//  Increment performance counters

    //  This is not an exact solution, but when IoPageRead gets a miss, it cannot tell whether it was CcCopyRead or CcMdlRead, but since the miss should occur very soon,
    ...
    try {
        ...
        while (Length != 0) {
            ...
            //  Set to see if the miss counter changes in order to detect when we should turn on read ahead.
            ...
            Information += ReceivedLength;//  Update number of bytes transferred.
            Length -= ReceivedLength;//  Calculate length left to transfer.
        }
    } finally{
    ...
    if (AbnormalTermination()) {
        //  We may have failed to allocate an Mdl while still having data mapped.
        if (Vacb != NULL) {
            CcFreeVirtualAddress(Vacb);
        }
        ...
        if (!FlagOn(FileObject->Flags, FO_RANDOM_ACCESS) && !PrivateCacheMap->Flags.ReadAheadEnabled && (SavedMissCounter != 0)) {
...
VOID CcMdlReadComplete(__in PFILE_OBJECT FileObject, __in PMDL MdlChain)
//  First we have the old routine which checks for an entry in the FastIo vector.
//  This routine becomes obsolete for every component that compiles with the new definition of FsRtlMdlReadComplete in fsrtl.h.
{
...
/*
Routine Description:
    This routine must be called at IPL0 after a call to CcMdlRead.
    The caller must simply supply the address of the MdlChain returned in CcMdlRead.
...
VOID CcPrepareMdlWrite(__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER FileOffset, __in ULONG Length, __out PMDL *MdlChain, __out PIO_STATUS_BLOCK IoStatus)
/*
Routine Description:
    This routine attempts to lock the specified file data in the cache and return a description of it in an Mdl along with the correct I/O status.
    ...
    When this call returns, the caller may immediately begin to transfer data into the buffers via the Mdl.

    When the call returns with TRUE, the pages described by the Mdl are locked in memory, but not mapped in system space.
    If the caller needs the pages mapped in system space, then it must map them.
    On the subsequent call to CcMdlWriteComplete the pages will be unmapped if they were mapped, and in any case unlocked and the Mdl deallocated.
Arguments:
    FileObject - Pointer to the file object for a file which was opened with NO_INTERMEDIATE_BUFFERING clear, i.e., for which CcInitializeCacheMap was called by the file system.
    FileOffset - Byte offset in file for desired data.
    Length - Length of desired data in bytes.
    MdlChain - On output it returns a pointer to an Mdl chain describing the desired data.
               Note that even if FALSE is returned, one or more Mdls may have been allocated, as may be ascertained by the IoStatus.Information field (see below).
    IoStatus - Pointer to standard I/O status block to receive the status for the in-transfer of the data.
               (STATUS_SUCCESS guaranteed for cache hits, otherwise the actual I/O status is returned.)
               The I/O Information Field indicates how many bytes have been successfully locked down in the Mdl Chain.
*/
{
    ...
    //  Get pointer to SharedCacheMap.
    SharedCacheMap = FileObject->SectionObjectPointer->SharedCacheMap;
    if (!FlagOn(FileObject->Flags, FO_WRITE_THROUGH) && CcForceWriteThrough(FileObject, Length, SharedCacheMap, FALSE)) {
        ExRaiseStatus(STATUS_INSUFFICIENT_RESOURCES);
    }
    ...
    try {
        ...
        while (Length != 0) {
            ...
            //  Map and see how much we could potentially access at this FileOffset, then cut it down if it is more than we need.
            CacheBuffer = CcGetVirtualAddress(SharedCacheMap, FOffset, &Vacb, &ReceivedLength);
            if (ReceivedLength > Length) {
                ReceivedLength = Length;
            }
            ...
            //  See if the entire transfer is beyond valid data length, or at least starting from the second page.
            Temp = FOffset;
            ...
            //  Now that some data (maybe zeros) is locked in memory and set dirty, it is safe, and necessary for us to advance valid data goal, so that we will not subsequently ask for a zero page.
            //  Note if we are extending valid data, our caller has the file exclusive.
            KeAcquireInStackQueuedSpinLock(&SharedCacheMap->BcbSpinLock, &LockHandle);
        ...
        if (AbnormalTermination()) {
            ...
            while (*MdlChain != NULL) {
                ...
                //  Extract the File Offset for this part of the transfer, and tell the lazy writer to write these pages, since we have marked them dirty.
                //  Ignore the only exception (allocation error), and console ourselves for having tried.
            ...
            //  Make sure the SharedCacheMap does not go away while the Mdl write is in progress.
            //  We decrment below.
...
VOID CcMdlWriteComplete(__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER FileOffset, __in PMDL MdlChain)
//  First we have the old routine which checks for an entry in the FastIo vector.
//  This routine becomes obsolete for every component that compiles with the new definition of FsRtlMdlWriteComplete in fsrtl.h.
{
    PDEVICE_OBJECT DeviceObject;
    PFAST_IO_DISPATCH FastIoDispatch;
...
Routine Description:
    ...
    The caller supplies the ActualLength of data that it actually wrote into the buffer, which may be less than or equal to the Length specified in CcPrepareMdlWrite.
...
*/
{
    ...
    while (Mdl != NULL) {
        ...
        //  Now clear the dirty bits in the Pte and set them in the Pfn.
        //  The Mdls will not be locked on repeated completion attempts.
        if (First) {
            MmUnlockPages(Mdl);
        }
        ...
        if (FlagOn(FileObject->Flags, FO_WRITE_THROUGH)) {
            MmFlushSection(FileObject->SectionObjectPointer, &FOffset, Mdl->ByteCount, &IoStatus, MM_FLUSH_ACQUIRE_FILE);
    ...
    //  Remove our open count and check to see if this makes the shared cache map eligible for lazy close.

    //  We do this now so, on failure, old filesystems which did not expect writethrough to raise continue to work.
    //  They will be within exception handling with the Mdl still in the IRP.

    //  Note that non-writethrough is the only one that needs the cache map, and it'll always work.
    //  Removing the open count for writethrough could be a minor win.
    if (First) {
    ...
    //  Note that we have not free'd the Mdl yet so the owning filesystem can retry the completion.
...
VOID CcMdlWriteAbort(__in PFILE_OBJECT FileObject, __in PMDL MdlChain)
/*
Routine Description:
    This routine must be called at IPL0 after a call to CcPrepareMdlWrite.
    This call does the following:
        Unmaps the pages (if mapped), unlocks them and deletes the MdlChain unlike the CcMdlWriteComplete this is only used to do teardown in a non success case where we didn't actually write anything
Arguments:
    FileObject - Pointer to the file object for a file which was opened with NO_INTERMEDIATE_BUFFERING clear, i.e., for which CcInitializeCacheMap was called by the file system.
    MdlChain - same as returned from corresponding call to CcPrepareMdlWrite.
*/
{
    ...
    //  If this already went through completion, the opencount is already dropped.
    if (First) {
        CcAcquireMasterLock(&OldIrql);
        CcDecrementOpenCount(SharedCacheMap, 'AdmC');

        //  Check for a possible deletion, this Mdl write may have been the last reference.
        if ((SharedCacheMap->OpenCount == 0) && !FlagOn(SharedCacheMap->Flags, WRITE_QUEUED) && (SharedCacheMap->DirtyPages == 0)) {
            //  Move to the dirty list.
            RemoveEntryList(&SharedCacheMap->SharedCacheMapLinks);
            InsertTailList(&CcDirtySharedCacheMapList.SharedCacheMapLinks, &SharedCacheMap->SharedCacheMapLinks);
