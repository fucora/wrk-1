...
*/
{
    PerfInfoProfileSourceActive = ProfileMaximum;   // Invalid value stops us from collecting more samples
    KeStopProfile(&PerfInfoProfileObject);
    PerfInfoFlushProfileCache();
}


NTSTATUS PerfInfoStartLog(PERFINFO_GROUPMASK *PGroupMask, PERFINFO_START_LOG_LOCATION StartLogLocation)
/*
Routine Description:
    This routine is called by WMI as part of kernel logger initialization.
Arguments:
    GroupMask - Masks for what to log.  This pointer point to an allocated area in WMI LoggerContext.
    StartLogLocation - Indication of whether we're starting logging at boot time or while the system is running.
                If we're starting at boot time,
                we don't need to snapshot the open files because there aren't any and we would crash if we tried to find the list.
*/
{
    ...
    if (PerfIsGroupOnInGroupMask(PERF_PROFILE, PGroupMask)) {
        if ((KeGetPreviousMode() == KernelMode) || (SeSinglePrivilegeCheck(SeSystemProfilePrivilege, UserMode))) {
            PerfInfoProfileInit();
...
Finish:
    if (!NT_SUCCESS(Status)) {// Failed to turn on trace, clean up now.
...
*/
{
    ...
    PERFINFO_CLEAR_GROUPMASK(PPerfGlobalGroupMask);// Reset the PPerfGlobalGroupMask.
