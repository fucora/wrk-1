...
/*
Routine description:
    Flushes the profile cache to the log buffer.
    To make sure it get's valid data we read the 2 separate version numbers (1 before and 1 after) to check if it's been changed.
    If so, we just read again.  If that fails often, then we disable the cache.
    Once the cache is read, we clear it.
    This may cause samples to be lost but that's ok as this is statistical and it won't matter.
Arguments:
    CheckVersion - If FALSE, the version is not checked. This used when the profile interrupt code flushes the cache.
*/
{
    ...
    if (PreviousInProgress != 1) {
        InterlockedDecrement(&PerfInfoSampledProfileFlushInProgress);// A flush is already in progress so just return.
        return;
    }
...
/*
Routine description:
    Implements instruction profiling.  If the source is not the one we're sampling on, we return.
    If caching is off, we write any samples coming from the immediately to the log.
    If caching is on, wrap the cache update with writes to the two versions so that the flush routine can know if it has a valid buffer.
Arguments:
...
*/
{
    ...
    if (!PERFINFO_IS_GROUP_ON(PERF_PROFILE) && (Source != PerfInfoProfileSourceActive)) {
        return;// We don't handle multple sources.
    }

    ThreadId = HandleToUlong(PsGetCurrentThread()->Cid.UniqueThread);
    if (!PerfInfoSampledProfileCaching || PerfInfoSampledProfileFlushInProgress != 0) {// No caching. Log and return.
...
#ifdef _X86_
    // Clear the low two bits to have more cache hits for loops.  Don't waste cycles on other architectures.
    ...
    for (i = 0; i < PerfProfileCache.Entries; i++) {
        ...
        (((ULONG_PTR)PerfProfileCache.Sample[i].InstructionPointer & ~3) == TwiddledIP)
            ...
            PerfProfileCache.Sample[i].Count++;// If we find the instruction pointer in the cache, bump the count
            return;
        }
    }
    if (PerfProfileCache.Entries < PERFINFO_SAMPLED_PROFILE_CACHE_MAX) {// If we find an empty spot in the cache, use it for this instruction pointer
    ...
    // Flush the cache
    PerfInfoLogBytes(PERFINFO_LOG_TYPE_SAMPLED_PROFILE_CACHE, &PerfProfileCache, sizeof(PERFINFO_SAMPLED_PROFILE_CACHE));
...
VOID FASTCALL PerfInfoLogInterrupt(IN PVOID ServiceRoutine, IN ULONG RetVal, IN ULONGLONG InitialTime)
/*
Routine Description:
    This callout routine is called from ntoskrnl.exe (ke\intsup.asm) to log an interrupt.
Arguments:
    ServiceRoutine - Address of routine that serviced the interrupt.
    RetVal - Value returned from ServiceRoutine.
    InitialTime - Timestamp before ISR was called.  The timestamp in the event is used as the end time.
*/
{
...
NTSTATUS PerfInfoLogBytesAndUnicodeString(USHORT HookId, PVOID SourceData, ULONG SourceByteCount, PUNICODE_STRING String)
/*
Routine description:
...
*/
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (StringBytes != 0) {
            RtlCopyMemory(PERFINFO_APPLY_OFFSET_GIVING_TYPE(pvTemp, SourceByteCount, PVOID), String->Buffer, StringBytes);
        }
...
ULONG PerfInfoCalcHashValue(PVOID Key, ULONG Len)
/*
Routine Description:
...
BOOLEAN PerfInfoAddToFileHash(PPERFINFO_ENTRY_TABLE HashTable, PFILE_OBJECT ObjectPointer)
/*
Routine Description:
    This routine add a FileObject into the specified hash table if it is not already there.
Arguments:
...
*/
{
    ...
    // Get the hashed index into the table where the entry ideally should be at.
    HashIndex = PerfInfoCalcHashValue((PVOID)&ObjectPointer, sizeof(ObjectPointer)) % TableSize;
    for (i = 0; i < TableSize; i++) {
        if (Table[HashIndex] == NULL) {// Found a empty slot. Reference the object and insert it into the table.
            ObReferenceObject(ObjectPointer);
            Table[HashIndex] = ObjectPointer;
            Result = TRUE;
            break;
        } else if (Table[HashIndex] == ObjectPointer) {// Found a slot. Reference the object and insert it into the table.
            Result = TRUE;
            break;
        }

        HashIndex = (HashIndex + 1) % TableSize;// Try next slot.
    }
...
*/
{
    ...
    AllocateBytes = PAGE_SIZE + sizeof(PVOID) * IoFileObjectType->TotalNumberOfObjects;// First create a temporary hash table to build the list of files to walk through
    ...
    FileNameInfo = ExAllocatePoolWithTag(NonPagedPool, MAX_FILENAME_TO_LOG, PERFPOOLTAG);// Allocate Buffers for FileNames
    if (FileNameInfo == NULL) {
        ExFreePool(HashTable.Table);
        return STATUS_NO_MEMORY;
    }

    CcPerfFileRunDown(&HashTable);// Walk through the Cc SharedCacheMapList

    // Now, walk through each process
    for (Process = PsGetNextProcess(NULL); Process != NULL; Process = PsGetNextProcess(Process)) {
        FileObjects = MmPerfVadTreeWalk(Process);// First Walk the VAD tree
        if (FileObjects != NULL) {
            File = FileObjects;
            while (*File != NULL) {
        ...
        ObPerfHandleTableWalk(Process, &HashTable);// Next, walk the handle Table
    }

    ObPerfHandleTableWalk(NULL, &HashTable);// Walk through the kernel handle table;
    FileObjects = MmPerfUnusedSegmentsEnumerate();// Walk through the MmUnusedSegmentList;
    if (FileObjects != NULL) {
        File = FileObjects;
        while (*File != NULL) {
            PerfInfoAddToFileHash(&HashTable, *File);
    ...
    for (i = 0; i < HashTable.NumberOfEntries; i++) {
        if (HashTable.Table[i]) {
            ...
            Status = ObQueryNameString(FileObject, FileNameInfo, MAX_FILENAME_TO_LOG, &ReturnLen);
            if (NT_SUCCESS(Status)) {
...
Routine Description:
    ...
    Since this routine is called only by global logger (i.e., trace from boot), no Sid info is collected.
*/
{
    NTSTATUS Status;
...
retry:
    ...
    Status = ZwQuerySystemInformation(SystemExtendedProcessInformation, Buffer, BufferSize, &ReturnLength);
    ...
    if (NT_SUCCESS(Status)) {
        ...
        while (TRUE) {
            ...
            for (i = 0; i < ProcessInfo->NumberOfThreads; i++) {
                Status = PerfInfoReserveBytes(&Hook, WMI_LOG_TYPE_THREAD_DC_START, sizeof(WMI_EXTENDED_THREAD_INFORMATION));
                if (NT_SUCCESS(Status)) {
