...
Abstract:
    This module implements machine independent functions involved with emulating known code sequences in NX regions.
*/
...
/*
Routine Description:
    This routine is called to determine whether the 32-bit X86 IStream contains a recognized ATL thunk sequence and if so, performs the emulation.
Arguments:
    InstructionPointer - Supplies a pointer to the value of the 32-bit instruction pointer at the time of the fault.
    StackPointer - Supplies a pointer to the value of the 32-bit stack pointer at the time of the fault.
...
#pragma pack()
    ...
    // If thunk emulation is disabled, then do not attempt to emulate any thunks.
    if (KiQueryNxThunkEmulationState() != 0) {
        return FALSE;
    }

    // Carefully examine the instruction stream.  If it matches a known ATL thunk template, then emulate it.
    try {
        ...
        if ((thunk1->Mov == 0x042444C7) && (thunk1->Jmp == 0xe9)) {
            ...
            // emulate: jmp imm32
            imm32 = thunk1->JmpImmediate + PtrToUlong(rip) + sizeof(*thunk1);

            // Determine if it is safe to emulate this code stream.
            if ((MmCheckForSafeExecution(rip, rsp, UlongToPtr(imm32), TRUE) == FALSE) || (safeThunkCall == FALSE)) {
                goto Done;
            }
            ...
            *InstructionPointer = imm32;
            ...
            // emulate: jmp imm32
            imm32 = thunk2->JmpImmediate + PtrToUlong(rip) + sizeof(*thunk2);
            ...
            *InstructionPointer = imm32;
            validThunk = TRUE;
        } else if ((thunk3->MovEdx == 0xba) && (thunk3->MovEcx == 0xb9) && (thunk3->JmpEcx == 0xe1ff)) {
            // Type 3 thunk.
            ...
            // Determine if it is safe to emulate this code stream.
            if (MmCheckForSafeExecution(rip, rsp, UlongToPtr(imm32), FALSE) == FALSE) {
                goto Done;
            }
            ...
            *InstructionPointer = imm32;
            validThunk = TRUE;
        } else if ((thunk4->MovEcx == 0xb9) && (thunk4->MovEax == 0xb8) && (thunk4->JmpEax == 0xe0ff)) {
            // Type 4 thunk
            ...
            *InstructionPointer = imm32;
            ...
            // Determine if it is safe to emulate this code stream.
            if (MmCheckForSafeExecution(rip, rsp, UlongToPtr(branchTarget), FALSE) == FALSE) {
                goto Done;
            }
