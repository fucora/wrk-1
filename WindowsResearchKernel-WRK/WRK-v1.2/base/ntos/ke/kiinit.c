...
// Put all code for kernel initialization in the INIT section.
// It will be deallocated by memory management when phase 1 initialization is completed.
...
/*
Routine Description:
    This function initializes executive structures implemented by the kernel.
    N.B. This function is only called during phase 1 initialization.
Return Value:
    A value of TRUE is returned if initialization is successful.
...
*/
{
    ...
    // If threaded DPCs are enabled for the host system, then create a DPC thread for each processor.
    if (KeThreadDpcEnable != FALSE) {
        ...
        do {
            ...
            Status = PsCreateSystemThread(&Handle, THREAD_ALL_ACCESS, &ObjectAttributes, NULL, NULL, KiExecuteDpc, Prcb);
    ...
    return KiInitMachineDependent();// Perform platform dependent initialization.
}
...
/*
Routine Description:
    This function initializes the spinlock structures in the per processor PRCB.
    This function is called once for each processor.
Arguments:
...
*/
{
    ...
    // Initialize dispatcher ready queue list heads, the ready summary, and the deferred ready list head.
    Prcb->QueueIndex = 1;
    Prcb->ReadySummary = 0;
    ...
    // Initialize the generic call DPC structure, set the target processor number, and set the DPC importance.
    ...
    InitializeListHead(&Prcb->WaitListHead);// Initialize wait list.

#if defined(_AMD64_)
    KeGetPcr()->LockArray = &Prcb->LockQueue[0];// Initialize general numbered queued spinlock structures.
#endif
    ...
    KeInitializeSpinLock(&Prcb->PrcbLock);// Initialize processor control block lock.

    // If this is processor zero, then also initialize the queued spin lock home address.
    if (Number == 0) {
...
*/
{
    ...
    // Initialize the timer expiration DPC object and set the destination processor to processor zero.
    ...
    // N.B. Each entry in the timer table due time table is set to an infinite absolute due time.
    ...
    // Initialize the swap event, the process inswap listhead, the process outswap listhead, and the kernel stack inswap listhead.
    ...
    // Copy the system service descriptor table to the shadow table which is used to record the Win32 system services.
...
*/
{
    ...
    Status = HalQuerySystemInformation(HalNumaTopologyInterface, sizeof(HAL_NUMA_TOPOLOGY_INTERFACE), &NumaInformation, &Length);

    // If the query was successful and the number of nodes in the host system is greater than one, 
    // then set the number of nodes and the address of the page to node and query processor functions.
    if (NT_SUCCESS(Status)) {
