...
PKPRCB KeQueryPrcbAddress (__in ULONG Number)
/*
Routine Description:
    This function returns the processor block address for the specified processor number.
Arguments:
...
/*
Routine Description:
    Restore processor specific features.
    This routine is called when processors have been restored to a powered on state to restore those things which are not part of the processor's "normal" context which may have been lost.
    For example, this routine is called when a system is resumed from hibernate or suspend.
*/
{
    CPU_INFO InformationExtended;

    KiSetPageAttributesTable();// Restore the page attribute table for the current processor.
...
VOID KeSaveStateForHibernate (__out PKPROCESSOR_STATE ProcessorState)
/*
Routine Description:
...
VOID KiProcessNMI (IN PKTRAP_FRAME TrapFrame, IN PKEXCEPTION_FRAME ExceptionFrame)
/*
Routine Description:
...
*/
{
    ...
    // First check for a pending freeze execution request from another processor.
#if !defined(NT_UP)
...
/*
Routine Description:
    Frozen processors are resumed to this routine in the event that a .reboot is being processed.
Return Value:
    Never returns.
*/
{
