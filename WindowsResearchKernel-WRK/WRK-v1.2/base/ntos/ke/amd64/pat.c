...
/*
Routine Description:
    This function initializes the page attribute table for the current processor.
    The page attribute table is set up to provide write back, write combining, uncacheable/stronly order, and uncacheable/weakly ordered.
    ...
    N.B. The caller must have the PAGELK code locked before calling this function.
*/
{
    ...
    // Invalidate the cache on the current processor, write the page attributes table, and invalidate the cache a second time.
    WritebackInvalidate();
