...
Abstract:
    ...
    and passes a complete process state structure to the hal to obtain a new processor.
*/
...
// N.B. This function address is filled in during kernel initialization if the host system is a multinode system.
...
/*
Routine Description:
    This function is called during phase 1 initialization on the master boot processor to start all of the other registered processors.
*/
{
#if !defined(NT_UP)
    ...
    // Ensure that prefetch instructions in the IPI path are patched out if necessary before starting other processors.
    OldIrql = KeRaiseIrqlToSynchLevel();
    KiIpiSendRequest(1, 0, 0, IPI_FLUSH_SINGLE);
    KeLowerIrql(OldIrql);

    // Do not start additional processors if the relocate physical loader switch has been specified.
    ...
    // If this a multinode system and processor zero is not on node zero, then move it to the appropriate node.
    if (KeNumberNodes > 1) {
    ...
    // Save the offset of the GDT in the allocation structure and add in the size of the GDT.
    GdtOffset = AllocationSize;
    AllocationSize += CurrentPcr->Prcb.ProcessorState.SpecialRegisters.Gdtr.Limit + 1;

    // Save the offset of the IDT in the allocation structure and add in the size of the IDT.
    IdtOffset = AllocationSize;
    AllocationSize += CurrentPcr->Prcb.ProcessorState.SpecialRegisters.Idtr.Limit + 1;

    // If the registered number of processors is greater than the maximum number of processors supported, then only allow the maximum number of supported processors.
    if (KeRegisteredProcessors > MAXIMUM_PROCESSORS) {
        KeRegisteredProcessors = MAXIMUM_PROCESSORS;
    }

    // Set barrier that will prevent any other processor from entering the idle loop until all processors have been started.
    KiBarrierWait = 1;

    // Initialize the fixed part of the processor state that will be used to start processors.
    // Each processor starts in the system initialization code with address of the loader parameter block as an argument.
    ...
    // Check to determine if hyper-threading is really enabled.
    // Intel chips claim to be hyper-threaded with the number of logical processors greater than one even when hyper-threading is disabled in the BIOS.
    LogicalProcessors = KiLogicalProcessors;
    if (HalIsHyperThreadingEnabled() == FALSE) {
        LogicalProcessors = 1;
    }

    // If the total number of logical processors has not been set with the /NUMPROC loader option,
    // then set the maximum number of logical processors to the number of registered processors times the number of logical processors per registered processor.

    // N.B. The number of logical processors is never allowed to exceed the number of registered processors times the number of logical processors per physical processor.
    MaximumProcessors = KeNumprocSpecified;
    if (MaximumProcessors == 0) {
        MaximumProcessors = KeRegisteredProcessors * LogicalProcessors;
    }

    // Loop trying to start a new processors until a new processor can't be started or an allocation failure occurs.

    // N.B. The below processor start code relies on the fact a physical processor is started followed by all its logical processors.
    //      The HAL guarantees this by sorting the ACPI processor table by APIC id.
    Index = 0;
    Number = 0;
    while ((Index < (MAXIMUM_PROCESSORS - 1)) && ((ULONG)KeNumberProcessors < MaximumProcessors) && ((ULONG)KeNumberProcessors / LogicalProcessors) < KeRegisteredProcessors) {

        // If this is a multinode system and current processor does not exist on any node, then skip it.
        Index += 1;
        if (KeNumberNodes > 1) {
        ...
        Number += 1;// Increment the processor number.

        // Allocate memory for the new processor specific data. If the allocation fails, then stop starting processors.
        ...
        KiCopyDescriptorMemory(&CurrentPcr->Prcb.ProcessorState.SpecialRegisters.Gdtr, &ProcessorState.SpecialRegisters.Gdtr, Base + GdtOffset);

        GdtBase = (PKGDTENTRY64)ProcessorState.SpecialRegisters.Gdtr.Base;

        // Encode the processor number in the upper 6 bits of the compatibility mode TEB descriptor.
        ...
        KiCopyDescriptorMemory(&CurrentPcr->Prcb.ProcessorState.SpecialRegisters.Idtr, &ProcessorState.SpecialRegisters.Idtr, Base + IdtOffset);

        // Set the PCR base address for the next processor, set the processor number, and set the processor speed.

        // N.B. The PCR address is passed to the next processor by computing the containing address with respect to the PRCB.
        PcrBase = (PKPCR)Base;
        ...
        // N.B. System startup must be called with a stack pointer that is 8 mod 16.
        SysTssBase->Rsp0 = (ULONG64)KernelStack - sizeof(PVOID) * 4;
        ProcessorState.ContextFrame.Rsp = (ULONG64)KernelStack - 8;

        // If this is the first processor on this node, then use the space already allocated for the node. Otherwise, the space allocated is not used.
        ...
        PcrBase->Prcb.ParentNode = Node;// Set the parent node address.

        // Adjust the loader block so it has the next processor state. Ensure that the kernel stack has space for home registers for up to four parameters.
        ...
        // Attempt to start the next processor. If a processor cannot be started, then deallocate memory and stop starting processors.
        if (HalStartNextProcessor(KeLoaderBlock, &ProcessorState) == 0) {
            // Restore the old node address in the node address array before freeing the allocated data block (the node structure lies within the data block).
            *OldNode = *Node;
    ...
    // All processors have been started. If this is a multinode system, then allocate any missing node structures.
    if (KeNumberNodes > 1) {
    ...
    KiPrefetchRetry &= ~0x80;// Reset the initialization bit in prefetch retry.

    // Reset and synchronize the performance counters of all processors, by applying a null adjustment to the interrupt time.
    KeAdjustInterruptTime(0);
    KiBarrierWait = 0;// Allow all processors that were started to enter the idle loop and begin execution.
#endif

    return;

    // The failure to allocate memory or a unsuccessful status was returned during the attempt to start processors.
    // This is considered fatal since something is very wrong.
#if !defined(NT_UP)
    StartFailure:
