...
*/
{
    ...
    // register state from the processor context structure to the specified trap and exception frames, and restore the processor control state.
    ...
    KeContextToKframes(TrapFrame, ExceptionFrame, &Prcb->ProcessorState.ContextFrame, CONTEXT_FULL, PreviousMode);
...
*/
{
    ...
    // Get the address of the current processor block, move the specified
    // register state from specified trap and exception frames to the current processor context structure, and save the processor control state.
    Prcb = KeGetCurrentPrcb();
...
DECLSPEC_NOINLINE VOID KiIpiProcessRequests(VOID)
/*
Routine Description:
...
*/
{
    ...
    while (TargetSet != 0) {
        ...
        do {
            ...
            // If the request type is flush multiple immediate, flush process, flush single, or flush all, then packet done can be signaled before processing the request.
            // Otherwise, the request type must be a packet request, a cache invalidate, or a flush multiple
            if (Request.IpiRequest <= IPI_FLUSH_ALL) {
                // If the synchronization type is target set, then the IPI was only between two processors and target set should be used for synchronization.
                // Otherwise, packet barrier is used for synchronization.
                if (Request.IpiSynchType == 0) {
                    if (SetMember == InterlockedXor64((PLONG64)&Source->TargetSet, SetMember)) {
                ...
                // If the request type is packet ready, then call the worker function. Otherwise, the request must be either a flush multiple or a cache invalidate.
                if (Request.IpiRequest == IPI_PACKET_READY) {
                    ...
                    (RequestPacket->WorkerRoutine)((PKIPI_CONTEXT)Packet, RequestPacket->CurrentPacket[0], RequestPacket->CurrentPacket[1], RequestPacket->CurrentPacket[2]);
                } else {
        ...
        TargetSet = InterlockedExchangeAdd64((LONG64 volatile *)&Prcb->SenderSummary, -(LONG64)TargetSet) - TargetSet;// Clear target set in sender summary.
    }
#endif
}


DECLSPEC_NOINLINE PKAFFINITY KiIpiSendRequest(IN KAFFINITY TargetSet, IN ULONG64 Parameter, IN ULONG64 Count, IN ULONG64 RequestType)
/*
Routine Description:
    This routine executes the specified immediate request on the specified set of processors.
...
Arguments:
   ...
   Parameter - Supplies the parameter data that will be packed into the request summary.
...
Return Value:
    The address of the appropriate request barrier is returned as the function value.
*/
{
#if !defined(NT_UP)
    PKAFFINITY Barrier;
    ...
    // If the target set contains one and only one processor, then use the target set for signal done synchronization.
    // Otherwise, use packet barrier for signal done synchronization.
    ...
    // If a target processor is already processing a request, then remove that processor from the target set of processor that will be sent an interprocessor interrupt.

    // N.B. It is guaranteed that there is at least one bit set in the target set.
    Number = Prcb->Number;
    SetMember = Prcb->SetMember;
    SummarySet = TargetSet;
    ...
    do {
        ...
        // Make sure that processing of the last IPI is complete before sending another IPI to the same processor.
        while ((Destination->SenderSummary & SetMember) != 0) {
            KeYieldProcessor();
        }

        // If the request type is flush multiple and the flush entries will fit in the mailbox,
        // then copy the virtual address array to the destination mailbox and change the request type to flush immediate.

        // If the request type is packet ready, then copy the packet to the destination mailbox.
        if (RequestType == IPI_FLUSH_MULTIPLE) {
            Virtual = &RequestMailbox->Virtual[0];
            Start = (PVOID *)Parameter;
            switch (Count) {
                // Copy of up to seven virtual addresses and a conversion of the request type to flush multiple immediate.
            case 7:
                Virtual[6] = Start[6];
            case 6:
                Virtual[5] = Start[5];
        ...
        if (InterlockedExchangeAdd64((LONG64 volatile *)&Destination->SenderSummary, SetMember) != 0) {
    ...
    // Request interprocessor interrupts on the remaining target set of processors.

    // N.B. For packet sends, there exists a potential deadlock situation unless an IPI is sent to the original set of target processors.
    //      The deadlock arises from the fact that the targets will spin in their IPI routines.
...
*/
{
    ...
    KiIpiSendRequest(TargetSet, (ULONG64)&RequestPacket, 0, IPI_PACKET_READY);// Send request.
#else
    UNREFERENCED_PARAMETER(TargetSet);
...
*/
{
    ...
    // If the low bit of signal is set, then use target set to notify the sender that the operation is complete on the current processor.
    // Otherwise, use packet barrier to notify the sender that the operation is complete on the current processor.
    if (((ULONG64)SignalDone & 1) == 0) {
        SetMember = KeGetCurrentPrcb()->SetMember;
        TargetPrcb = (PKPRCB)SignalDone;
        if (SetMember == InterlockedXor64((PLONG64)&TargetPrcb->TargetSet, SetMember)) {
