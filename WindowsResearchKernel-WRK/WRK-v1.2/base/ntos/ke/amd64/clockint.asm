...
; Routine Description:
;   This routine is called as the result of an interrupt generated by the interval timer.
;   Its function is to update the interrupt time, update the system time, and check to determine if a timer has expired.

;   N.B. This routine is executed on a single processor in a multiprocess
;        system. The remainder of the processors only execute the quantum end and runtime update code.
; Arguments:
;   TrapFrame (rcx) - Supplies the address of a trap frame.
;   Increment (rdx) - Supplies the time increment value in 100 nanosecond units.
...
; Set the timer hand value in the current processor block if it is not already set.
...
;   at CLOCK_LEVEL. Its function is to provide clock interrupt service on secondary processors.
