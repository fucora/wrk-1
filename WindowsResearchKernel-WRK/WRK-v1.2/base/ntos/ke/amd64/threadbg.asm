...
; Abstract:
;    This module implements the code necessary to startup a thread in kernel mode.
...
; Routine Description:
;   This routine is called to start a system thread.
;   This function calls the initial thread procedure after having extracted the startup parameters from the specified start frame.
;   If control returns from the initial thread procedure, then a bugcheck will occur.

; Implicit Arguments:

;   N.B. This function begins execution at its alternate entry point with a start frame on the stack.
;        This frame contains the start context, the start routine, and the system routine.
...
; Routine Description:
;   This routine is called to start a user thread.
;   This function calls the initial thread procedure after having extracted the startup parameters from the specified exception frame.
;   If control returns from the initial thread routine, then the user mode context is restored and control is transferred to the exception exit code.

; Implicit Arguments:

;   N.B. This function begins execution with a trap frame and an exception frame on the stack that represents the user mode context.
;        The start context, start routine, and the system routine parameters are stored in the exception record.
; Return Value:
;    None.
