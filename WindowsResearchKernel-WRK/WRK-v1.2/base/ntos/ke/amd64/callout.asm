...
; Arguments:
;   OutputBuffer (rcx) - Supplies a pointer to the variable that receives the address of the output buffer.
;   OutputLength (rdx) - Supplies a pointer to a variable that receives the length of the output buffer.
; Return Value:
...
;        caller is executed when a callback return system service is executed.
...
; Save argument registers in frame and allocate a legacy floating point save area.
...
; Save the callback stack address and the initial stack address in the current frame.
...
; Initialize the current and previous kernel stack segment descriptors in the kernel stack control area.
; These descriptors are used to control kernel stack expansion from drivers.
...
; Save the current trap frame address and establish a new initial kernel stack address;
...
; N.B. Interrupts are not enabled throughout the remainder of the system service exit.
...
; N.B. The below code uses an unusual sequence to transfer control.
;      This instruction sequence is required to avoid detection as an epilogue.
...
; Routine Description:
;   This routine is a stub routine which is called by the system service dispatcher to convert the current thread to a GUI thread. The process
;   of converting to a GUI mode thread involves allocating a large stack, switching to the large stack, and then calling the win32k subsystem
;   to record state. In order to switch the kernel stack the frame pointer used in the system service dispatch must be relocated.

;   N.B. The address of the pushed rbp in this routine is located from the trap frame address in switch kernel stack.
;   N.B. This routine diverges from the calling standard in that the caller (KiSystemCall64) expects the parameter home addresses to be preserved.
; Arguments:
;   None.
; Implicit arguments:
;   rbp - Supplies a pointer to the trap frame.
; Return Value:
;   The status returned by the real convert to GUI thread is returned as the function status.
...
;   N.B. The address of the frame pointer used in the system service dispatcher is located using the trap frame.
; Arguments:
;   StackBase (rcx) - Supplies a pointer to the base of the new kernel stack.
;   StackLimit (rdx) - Supplies a pointer to the limit of the new kernel stack.
; Return Value:
...
; Save the address of the new stack and copy the current stack to the new stack.
...
; stack segment descriptors in the kernel stack control area, and return the address of the old kernel stack.
...
;   kernel stack has been expanded and a callout is active. This is a fatal condition and causes a bugcheck.
; Arguments:
...
; VOID KiSwitchKernelStackAndCallout (IN PVOID Parameter, IN PEXPAND_STACK_CALLOUT Callout, IN PVOID LargeStack, IN SIZE_T CommitSize)
; Routine Description:
;   This function initializes the kernel stack control region of the new stack, sets the new stack parameters for the current thread, calls
;   the specified function with the specified argument, and reverses the process on return from the called function.
; Arguments:
...
; Save the current thread stack parameters in the previous section of the kernel stack control area.
...
; Initialize the current thread parameters in the current section of the kernel stack control area.
...
; Initialize a dummy machine frame and parameter home address area on the new stack.
...
; This is a dummy function to effect a call out on an expanded stack with the proper unwind information.
...
; N.B. The TSS initial stack and the PRCB initial stack only needs to be reloaded after the call out and just before switching back to the
;      previous stack. This is in contrast to the user mode call code which reloads these two values in both directions. The reason this
;      is possible is because the thread can not return to user mode until after the call out is complete and any exceptions or interrupts
...
; NTSTATUS NtCallbackReturn (__in __opt PVOID OutputBuffer, __in ULONG OutputLength, __in NTSTATUS Status)
; Routine Description:
;   This function returns from a user mode callout to the kernel mode caller of the user mode callback function.
;   N.B. This function returns to the function that called out to user mode using the call user mode function.
...
;   Status (r8d) - Supplies the status value returned to the caller of the callback function.
; Return Value:
;   If the callback return cannot be executed, then an error status is returned.
;   Otherwise, the specified callback status is returned to the caller of the callback function.
...
; If debug registers are active, then copy the debug registers from the current trap frame to the previous trap frame.
