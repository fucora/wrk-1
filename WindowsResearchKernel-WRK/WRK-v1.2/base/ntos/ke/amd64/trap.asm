...
; Abstract:
;   This module implements the code necessary to field and process AMD64 trap conditions.
...
; N.B. This will only work if all functions in this module are declared with these macros.
...
; Routine Description:
;   This routine is entered as the result of an attempted division by zero or the result of an attempted division does not fit in the destination
;   operand (i.e., the largest negative number divided by minus one).

;   N.B. The two possible conditions that can cause this exception are not separated and the exception is reported as a divide by zero.
...
; Disposition:
;   A standard trap frame is constructed on the kernel stack, the exception parameters are loaded into registers, and the exception is dispatched via common code.
...
; Routine Description:
;   This routine is entered as the result of a debug trap or fault.
;   The following conditions cause entry to this routine:
...
; Routine Description:
;   This routine is entered as the result of a nonmaskable interrupt.
;   A switch to the panic stack occurs before the exception frame is pushed on the stack.
...
;   There is no error code for this exception.
; Disposition:
;   A standard trap frame is constructed on the kernel stack and the NMI is processed.
;   If a return to this routine occurs, then the NMI was handled.
...
; Routine Description:
;   This routine is entered as the result of the execution of an int 3 instruction.
; Arguments:
;   The standard exception frame is pushed by hardware on the kernel stack.
;   There is no error code for this exception.
; Disposition:
...
; Routine Description:
;   This routine is entered as the result of the execution of an into instruction when the OF flag is set.
; Arguments:
;   The standard exception frame is pushed by hardware on the kernel stack.
;   There is no error code for this exception.
; Disposition:
...
; Routine Description:
;   This routine is entered as the result of the execution of a bound instruction and when the bound range is exceeded.
; Arguments:
;   The standard exception frame is pushed by hardware on the kernel stack.
;   There is no error code for this exception.
; Disposition:
...
; Routine Description:
;   This routine is entered as the result of the execution of an invalid instruction.
; Arguments:
;   The standard exception frame is pushed by hardware on the kernel stack.
;   There is no error code for this exception.
; Disposition:
...
; Routine Description:
;   This routine is entered as the result of the numeric coprocessor not being available for one of the following conditions:
;   1. A floating point instruction was executed and EM is set in CR0 - this condition should never happen since EM will never be set.
;   2. A floating point instruction was executed and the TS flag is set in CR0 - this condition should never happen since TS will never be set.
;   3. A WAIT of FWAIT instruction was executed and the MP and TS flags are set in CR0 - this condition should never occur since neither TS nor MP will ever be set.
...
;   There is no error code for this exception.
; Disposition:
;   A standard trap frame is constructed on the kernel stack and bugcheck is called.
...
; Routine Description:
;   This routine is entered as the result of the generation of a second exception while another exception is being generated.
;   A switch to the panic stack occurs before the exception frame is pushed on the stack.
...
; Routine Description:
;   This routine is entered as the result of a hardware failure since this vector is obsolete.
; Arguments:
;   The standard exception frame is pushed by hardware on the new stack.
;   There is no error code for this exception.
; Disposition:
;   This trap should never occur and the system is shutdown via a call to bugcheck.
...
; Routine Description:
;   This routine is entered as the result of a hardware or software failure since there is no task switching in 64-bit mode and 32-bit code does not have any task state segments.
; Arguments:
;   The standard exception frame is pushed by hardware on the new stack.
;   The segment selector index for the segment descriptor that caused the violation is pushed as the error code.
; Disposition:
;   A standard trap frame is constructed on the kernel stack and bugcheck is called.
...
;   parameters are loaded into registers, and the exception is dispatched via common code.
...
; Routine Description:
;   This routine is entered as the result of a page fault which can occur because of the following reasons:
...
;   A special format error code is pushed which specifies the cause of the page fault as not present, read/write access denied, from user/kernel mode, and attempting to set reserved bits.
; Disposition:
;   A standard trap frame is constructed on the kernel stack and memory management is called to resolve the page fault.
;   If memory management successfully resolves the page fault, then working set information is recorded, owed breakpoints are inserted, and execution is continued.

;   If the page fault occurred at an IRQL greater than APC_LEVEL, then the system is shut down via a call to bugcheck.
...
; The registers eax and rcx are loaded with the error code and the virtual address of the fault respectively when the trap frame is generated.

        shr     eax, 1                  ; isolate load/store and i/d indicators
        and     eax, 09h                ;

; Save the load/store indicator and the faulting virtual address in the exception record in case an exception is raised.
...
; If the debugger has any breakpoints that should be inserted, then attempt to insert them now.
...
; Check if a 32-bit user mode program reloaded the segment register GS and wiped out the GS base address.
...
; Check if the 32-bit program attempted a reference outside the 32-bit address space.
...
;       page fault at IRQL greater than APC level. This status causes a bugcheck.
...
; Set virtual address, load/store and i/d indicators, exception address, and dispatch the exception.
...
; A page fault occurred at an IRQL that was greater than APC_LEVEL. Set bugcheck parameters and join common code.
...
; Make sure that IRQL is greater than passive level to block a set context operations.
...
; Routine Description:
;   This routine is entered as the result of an attempted access to unaligned data.
...
;   the machine check stack occurs before the exception frame is pushed on the stack.
...
; This routine generates an exception frame, then calls the HAL to process the machine check.
...
; Routine Description:
;   This routine is entered as the result of the execution of an int 2c instruction.
; Arguments:
;   None.
; Disposition:
;   A standard trap frame is constructed on the kernel stack, the exception arguments are loaded into registers, and the exception is dispatched via common code.
...
; Routine Description:
;   This routine is entered as the result of the execution of an int 2d instruction.
; Arguments:
;   The standard exception frame is pushed by hardware on the kernel stack.
;   There is no error code for this exception.
; Disposition:
...
; EXCEPTION_DISPOSITION KiSystemServiceHandler (IN PEXCEPTION_RECORD ExceptionRecord, IN PVOID EstablisherFrame, IN OUT PCONTEXT ContextRecord, IN OUT PDISPATCHER_CONTEXT DispatcherContext)
...
;   the target of the unwind, then an exception occured while attempting to copy the user's in-memory argument list.
;   Control is transferred to the system service exit by return a continue execution disposition value.

;   If an unwind is being performed and the previous mode is user, then
;   bugcheck is called to crash the system. It is not valid to unwind out of a system service into user mode.

;   If an unwind is being performed and the previous mode is kernel, then
;   the previous mode field from the trap frame is restored to the thread object.

;   If an exception is being raised and the exception PC is the address
;   of the system service dispatcher in-memory argument copy code, then an unwind to the system service exit code is initiated.

;   If an exception is being raised and the exception PC is not within
;   the range of the system service dispatcher, and the previous mode is not user, then a continue search disposition value is returned.
;   Otherwise, a system service has failed to handle an exception and bugcheck is
;   called. It is invalid for a system service not to handle all exceptions that can be raised in the service.
...
;   the user in-memory argument list, then there is no return from this routine, and unwind is called.
;   Otherwise, ExceptionContinueSearch is returned as the function value.
...
; the exception PC is the address of the in memory argument copy code for the system service dispatcher.
; If the exception PC is within the range of
; the in memory copy code, then call unwind to transfer control to the system service exit code.
...
; code or the argument copy code. Unwind to the system service exit with the exception status code as the return value.
...
; If the previous mode was kernel mode, then the continue the search for an exception handler. Otherwise, bugcheck the system.
...
; Previous mode is kernel mode - restore previous mode and continue unwind operation.
...
; Routine Description:
;   This routine gains control when a system call instruction is executed
;   from 64-bit mode. The specified system service is executed by locating its routine address in system service dispatch table and calling the specified function.
...
; The following code is very carefully optimized so there is exactly 8 bytes of code for each argument move.
...
; Routine Description:
;   This routine allocates an exception frame on stack, saves nonvolatile machine state, and calls the system exception dispatcher.

;   N.B. It is the responsibility of the caller to initialize the exception record.
...
; VOID KiServiceLinkage (VOID)
; Routine Description:
;   This is a dummay function that only exists to make trace back through a kernel mode to kernel mode system call work.
