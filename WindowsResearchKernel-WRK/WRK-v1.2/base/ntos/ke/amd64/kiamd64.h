...
Abstract:
    This module contains the private (internal) platform specific header file for the kernel.
*/
...
FORCEINLINE ULONG64 KiGetActualStackLimit (IN PKTHREAD Thread)
{
...
VOID KiSaveInitialProcessorControlState (IN PKPROCESSOR_STATE ProcessorState);
...
ULONG KiDivide6432 (ULONG64 Dividend, ULONG Divisor);
VOID KiDpcInterrupt (VOID);
VOID KiFreezeInterrupt (VOID);
VOID KiIpiInterrupt (VOID);
VOID KiIpiProcessRequests (VOID);
PKAFFINITY KiIpiSendRequest (IN KAFFINITY TargetSet, IN ULONG64 Parameter, IN ULONG64 Count, IN ULONG64 RequestType);


__forceinline VOID KiIpiWaitForRequestBarrier (KAFFINITY volatile *Barrier)
/*
Routine Description:
...
VOID KiSecondaryClockInterrupt (VOID);
VOID KiSystemCall32 (VOID);
VOID KiSystemCall64 (VOID);
VOID KiInterruptDispatchLBControl (VOID);
VOID KiInterruptDispatchNoLock (VOID);
VOID KiInterruptDispatchNoEOI (VOID);
VOID KiWaitForReboot (VOID);

__forceinline BOOLEAN KiSwapProcess (IN PKPROCESS NewProcess, IN PKPROCESS OldProcess)
/*
Routine Description:
...
FORCEINLINE BOOLEAN KiCheckForFreezeExecution (IN PKTRAP_FRAME TrapFrame, IN PKEXCEPTION_FRAME ExceptionFrame)
/*
Routine Description:
    This function checks for a pending freeze execution request from another processor.
    If such a request is pending then execution is frozen.
Arguments:
