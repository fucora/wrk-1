...
; Abstract:
;   This module implements routines to save and restore processor control state.
...
; Routine Description:
;   This routine calls extended bugcheck to crash the system in a controlled manner.
; Arguments:
...
; VOID KeBugCheck3 (__in ULONG BugCheckCode, __in ULONG_PTR P1, __in ULONG_PTR P2, __in ULONG_PTR P3)
; Routine Description:
;   This routine calls extended bugcheck to crash the system in a controlled manner.
;   This routine differs from KeBugCheckEx in that the fourth bugcheck parameter is not used and presumed zero.
; Arguments:
;   BugCheckCode (rcx) - BugCheck code.
...
; Routine Description:
;   This routine restores the context and control state of the current processor and passes control to KeBugCheck2.
; Arguments:
;   BugCheckCode (rcx) - BugCheck code.
...
; Update register values in captured context to their state at the entry of BugCheck function.
...
; VOID KeContextToKframes (IN OUT PKTRAP_FRAME TrapFrame, IN OUT PKEXCEPTION_FRAME ExceptionFrame, IN PCONTEXT ContextRecord, IN ULONG ContextFlags, IN KPROCESSOR_MODE PreviousMode)
; Routine Description:
;    This function saves the current non-volatile XMM state, performs a context to kernel frames operation, then restores the non-volatile XMM state.
; Arguments:
;    TrapFrame (rcx) - Supplies a pointer to a trap frame that receives the volatile context from the context record.
;    ExceptionFrame (rdx) - Supplies a pointer to an exception frame that receives the nonvolatile context from the context record.
;    ContextRecord (r8) - Supplies a pointer to a context frame that contains the context that is to be copied into the trap and exception frames.
;    ContextFlags (r9) - Supplies the set of flags that specify which parts of the context frame are to be copied into the trap and exception frames.
;    PreviousMode (32[rsp]) - Supplies the processor mode for which the exception and trap frames are being built.
...
; KiSaveInitialProcessorControlState (PKPROCESSOR_STATE ProcessorState);
; Routine Description:
...
; KiRestoreProcessorControlState (VOID);
; Routine Description:
...
; Force the TSS descriptor into a non-busy state so no fault will occur when TR is loaded.
...
; KiSaveProcessorControlState (PKPROCESSOR_STATE ProcessorState);
; Routine Description:
...
; VOID KiRestoreDebugRegisterState (VOID);
; Routine Description:
;    This routine is executed on a transition from kernel mode to user mode and restores the debug register state.
;    N.B. This routine is used for both trap/interrupt and system service exit.
; Implicit Arguments:
...
;    when user debug registers are active. It saves the user debug registers and loads the kernel debug register state.
; Implicit Arguments:
;    rbp - Supplies a pointer to a trap frame.
...
; ULONG64 KeGetCurrentStackPointer (VOID);
; Routine Description:
