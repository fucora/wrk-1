...
;   Its function is to transfer information from the specified context record into the trap frame that was built when the system service was executed, and then exit the system as if an exception had occurred.
; Arguments:
;   ContextRecord (rcx) - Supplies a pointer to a context record.
;   TestAlert (dl) - Supplies a boolean value that specifies whether alert should be tested for the previous processor mode.
; Implicit Arguments:
;   rbp - Supplies the address of a trap frame.
; Return Value:
;   Normally there is no return from this routine.
;   However, if the specified context record is misaligned or is not accessible, then the appropriate status code is returned.
...
; N.B. The context record and test alert arguments are in the same registers.
...
; If the return status is less than or equal to zero, then return via the system service dispatcher. Otherwise, return via exception exit.
...
; N.B. If the legacy state is restored, then xmm6-xmm15 are also restored with potentially incorrect values.
;      Fortunately, exception exit will restore these registers with their proper value before returning to the user.
...
;   FirstChance (r8b) - Supplies a boolean value that specifies whether this is the first (TRUE) or second chance (FALSE) for dispatching the exception.
...
; Record the address of the caller in a place that won't be overwritten by the following context to k-frames.
        mov     rax, TrRip[rbp]         ; copy fault address to preserved entry
        mov     TrFaultAddress[rbp], rax ;

; Call the raise exception kernel routine which will marshall the arguments and then call the exception dispatcher.
...
; If the kernel raise exception routine returns success, then exit via the exception exit code. Otherwise, return to the system service dispatcher.
...
; The context or exception record is misaligned or not accessible, or the exception was not handled.
...
; ULONG KiDivide6432 (IN ULONG64 Dividend, IN ULONG Divisor);
; Routine Description:
;   This routine performs a divide operation with a dividend of exactly 64 bits, a divisor of exactly 32 bits, and a quotient of exactly 32 bits.
; Arguments:
