...
Routine Description:
    ...
    N.B. This function does not check if context record is accessible.
         It is assumed the the caller of this routine is either prepared to handle access violations or has probed and copied the context record as appropriate.
Arguments:
    ...
    StartContext - Supplies an optional pointer to a data structure that will be passed to the StartRoutine as a parameter.
                   This parameter is specified if the thread is a system thread and will execute totally in kernel mode.
    ContextRecord - Supplies an optional pointer a context record which contains the initial user mode state of the thread.
                    This parameter is specified if the thread will execute in user mode.
*/
{
    ...
    // Allocate a legacy floating point save area at the base of the thread stack and record the initial stack as this address.
    // All threads have a legacy floating point save are to avoid special cases in the context switch code.
    ...
    // Initialize the current kernel stack segment descriptor in the kernel stack control area.
    // This descriptor is used to control kernel stack expansion from drivers.
    ...
    // If a context record is specified, then initialize a trap frame, and an exception frame with the specified user mode context.

    // N.B. The initial context of a thread cannot set the debug or floating state.
    if (ARGUMENT_PRESENT(ContextRecord)) {
        ...
        // Allocate a trap frame, an exception frame, and a context switch frame.
        ...
        // The main entry point for the user thread will be jumped to via a continue operation from the user APC dispatcher.
        // Therefore, the user stack must be initialized to an 8 mod 16 boundary.

        // In addition, we must have room for the home addresses for the first four parameters.
        ContextRecord->Rsp = (ContextRecord->Rsp & ~STACK_ROUND) - ((4 * 8) + 8);

        // Zero the exception and trap frames and copy information from the specified context frame to the trap and exception frames.

        // N.B. The function that performs the context to kernel frames does not initialize the legacy floating context.
        RtlZeroMemory(ExFrame, sizeof(KEXCEPTION_FRAME));
        RtlZeroMemory(TrFrame, sizeof(KTRAP_FRAME));
        KxContextToKframes(TrFrame, ExFrame, ContextRecord, ContextRecord->ContextFlags, UserMode);
        ...
        SwFrame->Return = (ULONG64)KiStartUserThread;// Initialize start address.

        // Set the initial legacy floating point control/tag word state and the XMM control/status state.
        NpxFrame->ControlWord = INITIAL_FPCSR;
        NpxFrame->MxCsr = INITIAL_MXCSR;
        TrFrame->MxCsr = INITIAL_MXCSR;

        Thread->NpxState = LEGACY_STATE_SWITCH;// Set legacy floating point state to switch.
        Thread->PreviousMode = UserMode;// Set the saved previous processor mode in the thread object.
    } else {
        ...
        SwFrame->Return = (ULONG64)KiStartSystemThread;// Initialize start address.
        Thread->NpxState = LEGACY_STATE_UNUSED;// Set legacy floating point state to unused.
        Thread->PreviousMode = KernelMode;// Set the previous mode in thread object to kernel.
    }
