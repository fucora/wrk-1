...
; Abstract:
;   This module implements the code necessary to initially startup the NT system on an AMD64 system.
...
; Routine Description:
;   This routine is called at system startup to perform early initialization and to initialize the kernel debugger.
;   This allows breaking into the kernel debugger very early during system startup.
;   After kernel debugger initialization, kernel initialization is performed.
;   On return from kernel initialization the idle loop is entered.
;   The idle loop begins execution and immediately finds the system startup (phase 1) thread ready to run.
;   Phase 1 initialization is performed and all other processors are started.
;   As each process starts it also passes through the system startup code, but it does not initialization the kernel debugger.
; Arguments:
...
;   The GDT and IDT address and limits are contained in the gdtr and idtr registers.
;   The address of the TSS must be extraced from the appropriate GDT entry and stored in the PCR.
;   The stack register (RSP) is loaded with the idle thread stack and the kernel stack field of the loader block contains the address of the DPC stack.
; Return Value:
...
; Initialize kernel special registers and the address of the GDT, TSS, and IDT in the PRCB and PCR.
...
; N.B. This reservation ensures that the initialization of the thread stack does not overwrite any information on the current stack which is the same stack.

        sub     rsp, TotalFrameLength   ; allocate stack

; Initialize kernel.

; N.B. Kernel initialization is called with interupts disabled at IRQL HIGH_LEVEL and returns with interrupt enabled at IRQL DISPATCH_LEVEL.
...
; In a multiprocessor system the boot processor proceeds directly into the idle loop.
; As other processors start executing, however, they do not enter
