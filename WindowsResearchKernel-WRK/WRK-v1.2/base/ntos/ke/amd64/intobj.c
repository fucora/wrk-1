...
Arguments:
    ...
    SpinLock - Supplies a pointer to an executive spin lock.  There are two distinguished values recognized for SpinLock:
        NO_INTERRUPT_SPINLOCK - The kernel does not manage a spinlock associated with this interrupt.
        NO_END_OF_INTERRUPT - The interrupt represents a spurious interrupt vector, which is handled with a special, non-EOI interrupt routine.
    Vector - Supplies the HAL-generated interrupt vector.  Note that this is not be directly used as an index into the Interrupt Dispatch Table.
    Irql - Supplies the request priority of the interrupting source.
    SynchronizeIrql - Supplies the request priority that the interrupt should be synchronized with.
    ...
    FloatingSave - Supplies a boolean value that determines whether the floating point registers are to be saved before calling the service routine function.
                   N.B. This argument is ignored.
*/
{
    LONG64 Index;
    ...
    // The AMD64 HAL expects this to be set here.
    // Other clients will overwrite this value as appropriate via KeConnectInterrupt().
    ...
    Interrupt->Connected = FALSE;// Set the connected state of the interrupt object to FALSE.
}
    ...
    if (((IdtIndex > MAXIMUM_PRIMARY_VECTOR) ||
        ...
        if (Interrupt->Connected == FALSE) {
            ...
            if (Unexpected == Dispatch) {
                ...
                // Raise IRQL to high level in order to prevent a pending interrupt from firing before the IDT is set up.
                ...
                if ((Interrupt->Mode == Interruptx->Mode) && 
                    ...
                    // If the chained dispatch routine is not being used, then switch to chained dispatch.
...
*/
{
    ...
    if (Disconnected != FALSE) {
        ...
        // If the specified interrupt vector is not connected to the chained interrupt dispatcher, 
        // then disconnect it by setting its dispatch address to the unexpected interrupt routine.
        ...
        if (Interruptx->DispatchAddress == &KiChainedDispatch) {
            ...
            // The interrupt object is not connected to the chained interrupt dispatcher.
