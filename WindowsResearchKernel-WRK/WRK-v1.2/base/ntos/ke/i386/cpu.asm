...
; Abstract:
;    This module implements the assembly code necessary to determine cpu type and stepping information.
...
; USHORT KiSetProcessorType (VOID)
; Routine Description:
;    This function determines type of processor (80486, 80386), and it's corresponding stepping.
;    The results are saved in the current processor's PRCB.
; Return Value:
;    Prcb->CpuType
...
; BOOLEAN CpuIdTrap6 (VOID)
; Routine Description:
;    Temporary int 6 handler - assumes the cause of the exception was the attempted CPUID instruction.
; Return Value:
;    none.
...
; BOOLEAN Check486AStepping (VOID)
; Routine Description:
;    This routine checks for 486 A Stepping.

;    It takes advantage of the fact that on the A-step of the i486 processor, the ET bit in CR0 could be set or cleared by software, but was not used by the hardware.
;    On B or C -step, ET bit in CR0 is now hardwired to a "1" to force usage of the 386 math coprocessor protocol.
; Return Value:
...
; BOOLEAN Check486BStepping (VOID)
; Routine Description:
;    This routine checks for 486 B Stepping.

;    On the i486 processor, the "mov to/from DR4/5" instructions were aliased to "mov to/from DR6/7" instructions.
;    However, the i486 B or earlier steps generate an Invalid opcode exception when DR4/5 are used with "mov to/from special register" instruction.
; Return Value:
...
; BOOLEAN Temporary486Int6 (VOID)
; Routine Description:
;    Temporary int 6 handler - assumes the cause of the exception was the attempted execution of an mov to/from DR4/5 instruction.
; Return Value:
;    none.
...
; BOOLEAN Check486CStepping (VOID)
; Routine Description:
;    This routine checks for 486 C Stepping.

;    This routine takes advantage of the fact that FSCALE produces wrong result with Denormal or Pseudo-denormal operand on 486 C and earlier steps.

;    If the value contained in ST(1), second location in the floating point stack, is between 1 and 11, and the value in ST, top of the
;    floating point stack, is either a pseudo-denormal number or a
;    denormal number with the underflow exception unmasked, the FSCALE instruction produces an incorrect result.
; Return Value:
...
;                Under C and earlier steps, 486 returns the original value in ST as the result.
;                The correct returned value should be original significand and an exponent of 0...01.
...
; BOOLEAN Check386B0 (VOID)
; Routine Description:
;    This routine checks for 386 B0 or earlier stepping.

;    It takes advantage of the fact that the bit INSERT and EXTRACT instructions that existed in B0 and earlier versions of the 386 were removed in the B1 stepping.
;    When executed on the B1, INSERT and EXTRACT cause an int 6 (invalid opcode) exception.
;    This routine can therefore discriminate between B1/later 386s and B0/earlier 386s.
;    It is intended to be used in sequence with other checks to determine processor stepping by exercising specific bugs found in specific steppings of the 386.
; Return Value:
...
; Attempt execution of Extract Bit String instruction.
; Execution on B0 or earlier with length (CL) = 0 will return 0 into the destination (CX in this case).
; Execution on B1 or later will fail either due to
; taking the invalid opcode trap, or if the opcode is valid, we don't expect CX will be zeroed by any new instruction supported by newer steppings.
; The dummy int 6 handler will clears the Carry Flag and returns execution to the appropriate label.
; If the instruction actually executes, CX will *probably* remain unchanged in any new stepping that uses the opcode for something else.
; The nops are meant to handle newer steppings with an unknown instruction length.
        xor     eax,eax
        mov     edx,eax
...
; BOOLEAN TemporaryInt6 (VOID)
; Routine Description:
;    Temporary int 6 handler - assumes the cause of the exception was the attempted execution of an XTBS instruction.
; Return Value:
;    none.
TemporaryInt6    proc
...
; BOOLEAN Check386D1 (VOID)
; Routine Description:
;    This routine checks for 386 D1 Stepping.

;    It takes advantage of the fact that on pre-D1 386, if a REPeated MOVS instruction is executed when single-stepping is enabled,
;    a single step trap is taken every TWO moves steps, but should occur each move step.

;    NOTE: This routine cannot distinguish between a D0 stepping and a D1 stepping.
;    If a need arises to make this distinction, this routine will need modification.  D0 steppings will be recognized as D1.
; Return Value:
...
; Execution on B1 or earlier with length (CX) > 1 will trace over two iterations before accepting the trace trap.
; Execution on D1 or later will accept the trace trap after a single iteration.
; The dummy int 1 handler will return execution to the instruction following the movsb instruction.
; Examination of (CX) will reveal the stepping.
...
; BOOLEAN TemporaryInt1 (VOID)
; Routine Description:
;    Temporary int 1 handler - assumes the cause of the exception was trace trap at the above rep movs instruction.
; Arguments:
...
; BOOLEAN MultiplyTest (VOID)
; Routine Description:
;    This routine checks the 386 32-bit multiply instruction.
;    The reason for this check is because some of the i386 fail to perform this instruction.
; Return Value:
...
; BOOLEAN Multiply (VOID)
; Routine Description:
;    This routine performs 32-bit multiplication test which is known to fail on bad 386s.
...
; Routine Description:
;     This routine determines if there is any Numeric coprocessor present.
