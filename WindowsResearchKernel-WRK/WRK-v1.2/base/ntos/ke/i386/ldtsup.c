...
VOID Ki386LoadTargetLdtr(PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2);
VOID Ki386FlushTargetDescriptors(PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2);
...
/*
Routine Description:
    The specified LDT (which may be null) will be made the active Ldt of the specified process,
    for all threads thereof, on whichever processors they are running.
    The change will take effect before the call returns.
    ...
    While a single Ldt structure can be shared among processes,
    any edits to the Ldt of one of those processes will only be synchronized for that process.
    Thus, processes other than the one the change is applied to may not see the change correctly.
...
*/
{
    ...
    if ((Ldt == NULL) || (Limit == 0)) {
        ...
        // N.B.  If this is not done, random values appear in the high portion of the Ldt limit.
        LdtDescriptor.HighWord.Bytes.Flags1 = 0;
        LdtDescriptor.HighWord.Bytes.Flags2 = 0;
        ...
        LdtDescriptor.HighWord.Bits.Pres = 1;// Make it present
    }

    LdtInfo.Process = Process;
    LdtInfo.LdtDescriptor = LdtDescriptor;

    KeGenericCallDpc(Ki386LoadTargetLdtr, &LdtInfo);
}
...
*/
{
    ...
    // Make sure all DPC's are running so a load of the process LdtDescriptor field can't be torn
    if (KeSignalCallDpcSynchronize(SystemArgument2)) {
        LdtInfo->Process->LdtDescriptor = LdtInfo->LdtDescriptor;// Set the Ldt fields in the process object.
    }

    KeSignalCallDpcSynchronize(SystemArgument2);// Make sure the field has been updated before we continue
    KiLoadLdtr();// Reload the LDTR register from currently active process object
...
/*
Routine Description:
    The specified LdtEntry (which could be 0, not present, etc) will be edited into the specified Offset in the Ldt of the specified Process.
    This will be synchronized across all the processors executing the process.
    The edit will take affect on all processors before the call returns.

    N.B.

    Editing an Ldt descriptor requires stalling all processors active for the process,
    to prevent accidental loading of descriptors in an inconsistent state.
Arguments:
...
*/
{
    ...
    KeGenericCallDpc(Ki386FlushTargetDescriptors, &LdtInfo);
}
...
*/
{
    ...
    // This call removes all possible references to the LDT from the segment registers.
    KiFlushDescriptors();

    // Make sure all DPC's are running so a load of the process LdtDescriptor field can't be torn
    if (KeSignalCallDpcSynchronize(SystemArgument2)) {
        LdtInfo->Ldt[LdtInfo->Offset] = LdtInfo->LdtEntry;// Update the LDT entry
    }
