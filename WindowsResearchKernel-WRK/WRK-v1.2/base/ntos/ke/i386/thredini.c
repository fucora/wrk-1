...
VOID KiInitializeContextThread(IN PKTHREAD Thread,
    ...
    Arguments:
        ...
        StartRoutine - Supplies an optional pointer to a function that is to be
            called after the system has finished initializing the thread. This parameter is specified if the thread is a system thread and will execute totally in kernel mode.
    ...
    */
{
    ...
    // If a context frame is specified, then initialize a trap frame and and an exception frame with the specified user mode context.
    if (ARGUMENT_PRESENT(ContextFrame)) {
        ...
        RtlZeroMemory(TrFrame, KTRAP_FRAME_LENGTH + sizeof(FX_SAVE_AREA));// Zero out the trap frame and save area
        ...
        if (KeI386NpxPresent) {
            ...
            // Threads NPX state is not in the coprocessor.
            // In the emulator case, do not set the CR0_EM bit as their emulators may not want exceptions on FWAIT instructions.
            Thread->NpxState = NPX_STATE_NOT_LOADED & ~CR0_MP;
        }

        // Force debug registers off.
        // They won't work anyway from an initial frame, debuggers must set a hard breakpoint in the target
        ContextFrame2->ContextFlags &= ~CONTEXT_DEBUG_REGISTERS;

        //  Space for arguments to KiThreadStartup.
        //  Order is important, Since args are passed on stack through KiThreadStartup to PStartRoutine with PStartContext as an argument.
        ...
        // Copy information from the specified context frame to the trap and exception frames.
...
#endif
        *PUserContextFlag = 1;// Tell KiThreadStartup that a user context is present.
        TrFrame->ExceptionList = EXCEPTION_CHAIN_END;// Initialize the kernel mode ExceptionList pointer
        TrFrame->PreviousPreviousMode = UserMode;// Initialize the saved previous processor mode.
        Thread->PreviousMode = UserMode;// Set the previous mode in thread object to user.
    } else {
        // Dummy floating save area.
        // Kernel threads don't have or use the floating point - the dummy save area is make the stacks consistent.
        NpxFrame = (PFX_SAVE_AREA)(((ULONG)(Thread->InitialStack) - sizeof(FX_SAVE_AREA)));
        ...
        Thread->NpxState = NPX_STATE_NOT_LOADED;// Threads NPX state is not in the coprocessor.

        //  Space for arguments to KiThreadStartup.
        //  Order of fields in the switchframe is important,
        //  Since args are passed on stack through KiThreadStartup to PStartRoutine with PStartContext as an argument.
        ...
        *PUserContextFlag = 0;// Tell KiThreadStartup that a user context is NOT present.
    ...
    *PSystemRoutine = (ULONG)SystemRoutine;
    ...
    //  Each thread starts with these same flags set, regardless of whether the hardware exists or not.
...
#if DBG
    //  On checked builds add a check field so context swap can break early on bad context swaps (corrupted stacks for example).
    //  We place this below the stack pointer so the kernel debugger doesn't need knowledge of this.
    ((PULONG)SwitchFrame)[-1] = (ULONG)(ULONG_PTR)Thread;
#endif

    Thread->KernelStack = (PVOID)SwitchFrame;// Set the initial kernel stack pointer.
}