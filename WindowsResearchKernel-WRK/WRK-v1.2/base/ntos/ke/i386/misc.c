...
// Allocate Pool returns a pointer which is 8 byte aligned. 
// The floating point save area needs to be 16 byte aligned. 
// When allocating the save area we add the difference and adjust if needed.
...
*/
{
    ...
    // If the system is using floating point emulation, then return an error
    if (!KeI386NpxPresent) {
    ...
    if (FloatSave->Flags & FLOAT_SAVE_COMPLETE_CONTEXT) {
        ...
        // Save Cr0NpxState, but clear CR0_TS as there's not non-volatile pending fp exceptions
...
*/
{
    ...
    if (Thread != KeGetCurrentThread()) {
        ...
        // belongs to is not the current thread (or the saved thread field is trash).
    ...
    if ((FloatSave->Flags & FLOAT_SAVE_FREE_CONTEXT_HEAP) != 0) {
        // If FXSAVE area was adjusted for alignment after allocation, undo that adjustment before freeing.
...
/*
Routine Description:
    The fast syscall/return feature cannot be used until certain processor specific registers have been initialized.
    This routine is called when the system is switching to a state where not all processors are powered on.

    This routine adjusts the exit path for system calls to use the iretd instruction instead of the faster sysexit
    instruction, it accomplishes this by adjusting the offset of a branch.
Arguments:
    None.
Return Value:
    None.
...
Routine Description:
    The fast syscall/return feature cannot be used until certain processor specific registers have been initialized.
    This routine is called once the registers are known to have been set on all processors.

    This routine adjusts the exit path for system calls to use the appropriate sequence for the processor, it does
...
*/
{
    ...
    // It can never be otherwise, but, make sure we aren't going to adjust it out of range.

    // The following is a workaround for the fact that in resume
    // from hibernate the kernel is read only.   Basically, we
    // won't try to do it again, we also don't undo it when hibernating/suspending.
...
/*
Routine Description:
    Load MSRs used to support Fast Syscall/return.  This routine is run on all processors.
Arguments:
    None.
Return Value:
    None.
...
VOID KiRestoreFastSyscallReturnState(VOID)
{
    ...
    if (KeFeatureBits & KF_FAST_SYSCALL) {
        ...
        KeIpiGenericCall((PKIPI_BROADCAST_WORKER)KiLoadFastSyscallMachineSpecificRegisters, Void);
    }

    // Set the appropriate code for system call into the system call area of the shared user data area.
...
Routine Description:
    ...
    routine is called when a system is resumed from hibernate or suspend.
Arguments:
    None.
Return Value:
    None.
...
Routine Description:
    ...
    Note: This routine must be called with current IRQL at or above dispatch lever, or interrupts disabled.
Arguments:
    QueuedLock  Supplies the address of the queued spinlock.
    TrapFrame   Supplies the address of the trap frame to pass to KiFreezeTargetExecution.
Return Value:
    None.
*/
{
    ...
    if (Previous == NULL) {
        ...
        *LockPointer |= LOCK_QUEUE_OWNER;
        ...
        // previous queue entry to point to this new waiter and wait until the lock is granted.

        // The following loop is careful not to write ANYTHING while waiting unless a freeze execution has been
        // requested.   This includes any stack variables or return addresses.
        ...
        *LockPointer |= LOCK_QUEUE_WAIT;
        ...
        while (*LockPointer & LOCK_QUEUE_WAIT) {
            if (Prcb->RequestSummary & IPI_FREEZE) {
                ...
                // get it next time around (unless the IPI has been handled).
                if (Summary == OldSummary) {
                    // IPI_FREEZE cleared in RequestSummary.   Now freeze as requested.
                    KiFreezeTargetExecution(TrapFrame, NULL);
                }
            }

            KeYieldProcessor();// Don't be a hog.
        }
    }
...
ULONG FASTCALL KiRDTSC(PULONGLONG Time);
...
KIRQL FASTCALL KiQueueStatAcquireQueuedLock(IN KSPIN_LOCK_QUEUE_NUMBER Number)
{
    KIRQL PreviousIrql;
...
KIRQL FASTCALL KiQueueStatAcquireQueuedLockRTS(IN KSPIN_LOCK_QUEUE_NUMBER Number)
{
    KIRQL PreviousIrql;
...
LOGICAL FASTCALL KiQueueStatTryAcquire(IN KSPIN_LOCK_QUEUE_NUMBER Number, IN PKIRQL OldIrql, IN KIRQL NewIrql)
{
    KIRQL PreviousIrql;
...
UCHAR FASTCALL KiQueuedLockDepth(IN PKSPIN_LOCK_QUEUE QueuedLock)
{
    ...
    // Get address of last acquirer in queue (stip the status bits out of the address).
...
// VOID FASTCALL KeAcquireQueuedSpinLockAtDpcLevel(IN PKSPIN_LOCK_QUEUE QueuedLock)
// {
//     KiAcquireQueuedLock(QueuedLock);
// }


// VOID FASTCALL KeReleaseQueuedSpinLockFromDpcLevel (IN PKSPIN_LOCK_QUEUE QueuedLock)
// {
...
Routine Description:
    ...
    If success, logs the same data as KiQueueStatAcquire except the wait time is 0.
...
Routine Description:
    ...
    purpose it to record wait time, acquisition time and who acquired the lock.
Arguments:
    Argument points to an array of ULONG data.
...
*/
{
    ...
    // lying around for the next pair if we happen to throw away this particular data point.
    ...
    for (Entry = KiQueuedSpinLockLog[LockNumber]; TRUE; Entry++) {
        ...
        if (Entry->Key == Key) {
            ...
            // to know about as it indicates contention on this lock.
...
BOOLEAN FASTCALL KiRecordDr7(IN OUT PULONG Dr7Ptr, IN OUT PUCHAR Mask OPTIONAL)
/*
Routine Description:
    This updates Dr7 (located in the trap frame) to reflect the status of both Dr7 and the active debug mask.

    N.B. This routine should be called after Dr7 has been written to
        with the user value and the debug mask accurately reflects the state of the remaining debug registers.

    N.B. Note that the Dr7 override technique enables the Bx bits in Dr6, associated with the appropriate Drs. In the previous model,
        the Dr registers were not save/restored unless Drs were enabled, and thus a dependency on these bits in that case would have
        been questionable. Additionally, the processor may clear these Dr6 bits on certain debug exceptions, making them unreliable.
...
Arguments:
    ...
    Mask - Optional pointer to the currently active debug mask. If
        absent, this routine will directly update the current thread's debugging state as needed.
...
*/
{
    ...
    // (i.e., whether it has valid contents or contents used only for bookkeeping).
    ...
    if (*Dr7Ptr == 0) {
        ...
        if ((NewMask & DR_REG_MASK) != 0) {
            ...
            *Dr7Ptr |= DR7_OVERRIDE_MASK;
            ...
            // The override bit only occurs in conjunction with bits from DR_REG_MASK.
            ASSERT(NewMask == 0);
        }
    } else {
...
BOOLEAN FASTCALL KiProcessDebugRegister(IN OUT PKTRAP_FRAME TrapFrame, IN ULONG Register)
/*
Routine Description:
    This routine processes the indicated debug register, and updates the trap frame and the thread's active debug mask accordingly.
    A caller should invoke the routine only after the register has been written with the sanitized value.

    N.B. This routine need only be called when the debug registers are updated individually (e.g., via the VDM).
    N.B. Dr7 should already contain the user specified value.
Arguments:
    TrapFrame - The target trap frame housing both the register being processed and the value of Dr7.
...
*/
{
    ...
    ASSERT((NewMask & (DR_MASK(7) | DR_MASK(DR7_OVERRIDE_V))) != (DR_MASK(7) | DR_MASK(DR7_OVERRIDE_V)));
...
VOID __cdecl KeSaveStateForHibernate(__out PKPROCESSOR_STATE ProcessorState)
/*
Routine Description:
    ...
    N.B. #pragma surrounding this function is required in order
         to create the frame pointer than RtlCaptureContext relies on.
    N.B. _CRTAPI1 (__cdecl) decoration is also required so that RtlCaptureContext can compute the correct ESP.
Arguments:
