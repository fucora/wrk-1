...
;  Abstract:
;     This module implements the routines for acquiring and releasing spin locks.
...
;  VOID KefAcquireSpinLockAtDpcLevel (IN PKSPIN_LOCK SpinLock)
;  Routine Description:
...
;  VOID KefReleaseSpinLockFromDpcLevel (IN PKSPIN_LOCK SpinLock)
;  Routine Description:
...
;  BOOLEAN FASTCALL KeTryToAcquireSpinLockAtDpcLevel (IN PKSPIN_LOCK SpinLock)
;  Routine Description:
;     This function attempts acquires a kernel spin lock.  If the
;     spinlock is busy, it is not acquire and FALSE is returned.
;  Arguments:
...
;     IRQL, only raising the IRQL when there is a reasonable hope of acquiring the lock.
;  Arguments:
...
; VOID KeAcquireQueuedSpinLockAtDpcLevel (IN PKSPIN_LOCK_QUEUE QueuedLock)
; Routine Description:
...
;    a couple of instructions as the address can be calculated at compile time.
...
;    LockQueueEntry (ecx) - Supplies the address of the queued spinlock entry in this processor's PRCB.

; Return Value:
;    None.

;    N.B. ecx is preserved, assembly code callers can take advantage
;    of this by avoiding setting up ecx for the call to release if the caller can preserve the lock that long.
...
if DBG
        ...
        ; KeBugCheckEx(SPIN_LOCK_ALREADY_OWNED, actual lock address, my context, previous acquirer, 2);
...
; Routine Description:
;    This function releases a queued spinlock and preserves the current IRQL.
; Arguments:
...
; VOID KeReleaseQueuedSpinLockFromDpcLevel (IN PKSPIN_LOCK_QUEUE QueuedLock)
; Routine Description:
...
; LOGICAL KeTryToAcquireQueuedSpinLockAtRaisedIrql (IN PKSPIN_LOCK_QUEUE QueuedLock)
; Routine Description:
