...
/*
Routine Description:
    This function gains control when the x86 processor generates a divide by zero trap.
    The x86 design generates such a trap on divide by zero and on division overflows.
    In order to determine which exception code to dispatch, the divisor of the "div" or "idiv" instruction needs to be inspected.
...
*/
{
    ...
    if (UserFrame->SegCs == KGDT_R0_CODE && (UserFrame->EFlags & EFLAGS_V86_MASK) == 0) {
        ...
        // Bugcheck NOW so we can figure out what went wrong.
        // If we try and proceed, then we'll likely fault in reading the top of user space, and then double fault (page fault in the div zero handler.) -- This is a debugging consideration.
        // You can't put breakpoints on the trap labels so this is hard to debug.
    ...
    try {
        ...
        // accum now contains the instruction operand, see if the operand was really a zero
        if (accum & operandmask) {
            status = STATUS_INTEGER_OVERFLOW;// operand was non-zero, must be an overflow
        }
...
/*
Routine Description:
    Reads the next byte from the istream pointed to by the UserFrame, and advances the EIP.
...
/*
Routine Description:
    This function gains control from the Trap07 handler.
    It examines the user mode instruction to see if it's a NoWait NPX instruction.
    Such instructions do not generate floating point exceptions - this check needs to be done due to the way 80386/80387 systems are implemented.
    Such machines will generate a floating point exception interrupt when the kernel performs an FRSTOR to reload the thread's NPX context.
    If the thread's next instruction is a NoWait style instruction, then we clear the exception or emulate the instruction.

    AND... due to a different 80386/80387 "feature" the kernel needs to use FWAIT at times which can causes 80487's to generate delayed exceptions that can lead to the same problem described above.

    Note: If the CR0_NE = 0 NPX mode is deprecated, this routine may be unnecessary, as the no-wait style instructions should not generate a pending FP exception.
...
Return Value:
    ...
    Note that this function toggles interrupts on/off, possibly affecting the caller's NPX state. Interrupts remain disabled on exit.
*/
{
    ...
    // Enable interrupts since the code will touch the instruction stream. Note that the trap frame integrity is protected by the elevated IRQL.
    _asm {
        sti
    }
    ...
    try {
        ...
        for (i = 0; NoWaitNpxInstructions[i].Opcode1; i++) {
            if (NoWaitNpxInstructions[i].Opcode1 == ibyte1) {// first opcode byte matched - check second part of opcode
                if (NoWaitNpxInstructions[i].ModRm) {// modrm only applies for opcode in range 0-0xbf
                    if (((ibyte2 & 0xc0) != 0xc0) &&
                        (NoWaitNpxInstructions[i].Opcode2 == inmodrm)) {
                        status = NoWaitNpxInstructions[i].type;// This is a no-wait NPX instruction
                        break;
                    }
                } else {
    ...
    // Disable interrupts as we are done touching the instruction stream, and may return to the caller.
    _asm {
        cli
    }
    ...
    if (status == 1) {
        // Ignore pending exception, user mode instruction does not trap on pending execptions and it will clear/mask the pending exceptions

        // The thread's NPX state may no longer be resident. Avoid executing with another thread's NPX state.
    ...
    // This is either FNSTSW or FNSTCW.
    // Both of these instructions get a value from the coprocessor without effecting the pending exception state.
    // To do this we emulate the instructions.

    // Read the coprocessors Status & Control word state, then re-enable interrupts.

    // NOTE: The new compiler is generating a FWAIT at the entry to the try/except block if it sees inline fp instructions, even if they are only control word accesses.
    ...
    try {
        ...
        if (rm != Mod->RmDisplaceOnly) {
            if (rm == Mod->RmSib) {
                ...
                accum = GETREG(UserFrame, RM32[rm]);// get register's value
            }
        }
        ...
        if (Mod->RmDisplace & (1 << rm)) {
            if (Mod->Disp == 4) {
                i = (KiNextIStreamByte(UserFrame, &istream) << 0) | (KiNextIStreamByte(UserFrame, &istream) << 8) | (KiNextIStreamByte(UserFrame, &istream) << 16) | (KiNextIStreamByte(UserFrame, &istream) << 24);
            } else {
                ibyte1 = KiNextIStreamByte(UserFrame, &istream);
    ...
    } except(KiCopyInformation(&ExceptionRecord, (GetExceptionInformation())->ExceptionRecord))
    {
        ...
        KiDispatchException(&ExceptionRecord, NULL,                // ExceptionFrame
...
// Code description is above.
// We do this here to stop the compiler from putting fwait in the try/except block
...
/*
Routine Description:
    This function is called from the APC and DPC interrupt code to check if the specified EIP lies within the SLIST pop code.
    If the specified EIP lies within the SLIST code, then RIP is reset to the SLIST resume address.
Arguments:
...
*/
{
    ...
    if (SegCs == KGDT_R0_CODE) {
        if ((Eip >= (ULONG)&ExpInterlockedPopEntrySListResume) && (Eip <= (ULONG)&ExpInterlockedPopEntrySListEnd)) {
            TrapFrame->Eip = (ULONG)&ExpInterlockedPopEntrySListResume;
        }
    } else if (SegCs == (KGDT_R3_CODE | RPL_MASK)) {
        if ((Eip >= (ULONG)KeUserPopEntrySListResume) && (Eip <= (ULONG)KeUserPopEntrySListEnd)) {
...
*/
{
    ...
    case KGDT_R0_CODE:// Fault occured in kernel mode
        slistFaultIP = (ULONG)&ExpInterlockedPopEntrySListFault;
        break;
    case KGDT_R3_CODE | RPL_MASK:// Fault occured in native usermode
