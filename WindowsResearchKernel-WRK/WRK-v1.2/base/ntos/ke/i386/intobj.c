...
Abstract:
    This module implements the kernel interrupt object. Functions are provided to initialize, connect, and disconnect interrupt objects.
*/

#include "ki.h"

// Data used for interrupt timing, aka, long ISR trapping.
// The initial value for KiIsrTscLimit is to prevent the trap from firing until a reasonable value is determined.
ULONGLONG KiIsrTscLimit = 0xFFFFFFFFFFFFFFFF;
ULONG KiTimeLimitDpcMicroseconds;
ULONG KiTimeLimitIsrMicroseconds;

//  Externs from trap.asm used to compute and set handlers for unexpected hardware interrupts.
...
Routine Description:
    ...
    The service routine, service context, spin lock, vector, IRQL, SynchronizeIrql, and floating context save flag are initialized.
Arguments:
...
*/
{
    ...
    // Initialize the address of the service routine, the service context, the address of the spin lock, the vector
    // number, the IRQL of the interrupting source, the Irql used for synchronize execution, the interrupt mode, the processor
    ...
    // to -1 so that the first time through the interrupt dispatch they will be reset correctly.
    ...
    // and edit the machine code stored in the structure (please see _KiInterruptTemplate in intsup.asm.)  Finally, flush the dcache
    // on all processors that the current thread can run on to ensure that the code is actually in memory.
    ...
    // The following two instructions set the address of current interrupt object the the NORMAL dispatching code.
...
BOOLEAN KeConnectInterrupt(__inout PKINTERRUPT Interrupt)
/*
Routine Description:
    This function connects an interrupt object to the interrupt vector specified by the interrupt object.
    If the interrupt object is already connected, or an attempt is made to connect to an interrupt that cannot
    be connected, then a value of FALSE is returned. Else the specified interrupt object is connected to the interrupt vector, the connected
    state is set to TRUE, and TRUE is returned as the function value.
Arguments:
    Interrupt - Supplies a pointer to a control object of type interrupt.
Return Value:
    If the interrupt object is already connected or an attempt is made to connect to an interrupt vector that cannot be connected,
    then a value of FALSE is returned. Else a value of TRUE is returned.
*/
{
    ...
    // that cannot be connected, the interrupt request level is invalid, or the processor number is invalid, then do not connect the interrupt
    // object. Else connect interrupt object to the specified vector and establish the proper interrupt dispatcher.
    Connected = FALSE;
    ConnectError = FALSE;
    Irql = Interrupt->Irql;
    ...
    if (!((Irql > HIGH_LEVEL) ||
        ...
        if (!Interrupt->Connected) {
            ...
            if (DispatchInfo.Type == NoConnect) {
                ...
                // Vector is already connected as sharable.  New vector is sharable and modes match.  Chain new vector.
                Connected = TRUE;
                Interrupt->Connected = TRUE;
...
BOOLEAN KeDisconnectInterrupt(__inout PKINTERRUPT Interrupt)
/*
Routine Description:
    This function disconnects an interrupt object from the interrupt vector specified by the interrupt object. If the interrupt object is not
    connected, then a value of FALSE is returned. Else the specified interrupt object is disconnected from the interrupt vector,
    the connected state is set to FALSE, and TRUE is returned as the function value.
Arguments:
    Interrupt - Supplies a pointer to a control object of type interrupt.
Return Value:
...
*/
{
    ...
    if (Connected) {
        ...
        if (DispatchInfo.Type == ChainConnect) {
            ...
            // If there's only one interrupt object left on this vector, determine proper interrupt dispatcher
            Interrupty = CONTAINING_RECORD(DispatchInfo.Interrupt->InterruptListEntry.Flink, KINTERRUPT, InterruptListEntry);

            if (DispatchInfo.Interrupt == Interrupty) {
                KiConnectVectorAndInterruptObject(Interrupty, NormalConnect);
            }
        } else {
            // Removing last interrupt object from the vector.  Disable the vector, and set it to unconnected
...
)
{
    ...
    case 0:
        ...
        DispatchInfo->NoDispatch = (PKINTERRUPT_ROUTINE)(((ULONG)&KiStartUnexpectedRange) + (IDTEntry - PRIMARY_VECTOR_BASE) * KiUnexpectedEntrySize);
    ...
    case 1:
        ...
        DispatchInfo->Interrupt = (PKINTERRUPT)((PUCHAR)CurrentDispatch - (PUCHAR)KiInterruptTemplate + (PUCHAR)&KiInterruptTemplate2ndDispatch);
        break;
    default:
        ...
        *pl = (ULONG)DispatchAddress - (ULONG)((PUCHAR)pl + 4);
        ...
        if (DispatchInfo.FlatDispatch) {
            ...
            DispatchAddress = (PKINTERRUPT_ROUTINE)(ULONG_PTR)
                ((PUCHAR) &(Interrupt->DispatchCode[0]) + ((PUCHAR)&KiInterruptTemplate2ndDispatch - (PUCHAR)KiInterruptTemplate));
        } else {
...
VOID FASTCALL KiTimedChainedDispatch2ndLvl(PKINTERRUPT Interrupt)
/*
Routine Description:
    This function performs the same function as KiChainedDispatch2ndLvl except that it is written in C instead of assembly code and includes code for timing ISRs.

    I'd be interested in seeing some benchmarks to show if the assembly code is actually faster.
    The Acquire/Release spinlock could be inlined fairly easily.
Arguments:
    Interrupt - Supplies a pointer to a control object of type interrupt.
Return Value:
    None.
*/
{
    ...
    do {
        // If the current IRQL (IRQL raised to by nature of taking this interrupt) is not equal to the Synchronization IRQL required
        ...
        // ElapsedTime is time since we started looking at this element on the chain.  (ie the current interrupt object).
        ElapsedTime = RDTSC() - StartTime;

        // TimeHigher is the amount Prcb->IsrTime has increased since we begin servicing this interrupt object, ie the amount of time spent in higher level ISRs.
        TimeHigher = Prcb->IsrTime - StartTimeHigher;

        // Adjust ElapsedTime to time spent on this interrupt object, excluding higher level ISRs.
        ...
        // (Though a simple atomic add would do, it's per processor and at IRQL > DISPATCH_LEVEL so it doesn't need to be locked).
        ...
        if ((Handled != FALSE) && (Interrupt->Mode == LevelSensitive)) {
            // The interrupt has been handled.
            return;
        }

        // If this is the last entry on the chain, get out, otherwise advance to the next entry.
        if (Interrupt->InterruptListEntry.Flink == ListEnd) {
            ...
            //     no ISR handled the request.   This is a system fatal condition, or,
            // (b) the chain has edge triggered interrupts in which case
            //     we must run the chain repeatedly until no ISR services the request.

            // Question:  Do we actually have chained edge triggered interrupts anymore?
            if (Handled == FALSE) {
                break;
            }
        }
        Interrupt = CONTAINING_RECORD(Interrupt->InterruptListEntry.Flink, KINTERRUPT, InterruptListEntry);
    } while (TRUE);
}


VOID FASTCALL KiTimedInterruptDispatch(PKINTERRUPT Interrupt)
/*
Routine Description:
    This function is a wrapper for the guts of KiDispatchInterrupt.
    It is called when the system has been patched to time interrupts.
Arguments:
    Interrupt - Supplies a pointer to a control object of type interrupt.
Return Value:
    None.
*/
{
    ...
    // TimeHigher is the amount Prcb->IsrTime has increased since we entered this routine, ie the amount of time spent in higher level ISRs.
    TimeHigher = Prcb->IsrTime - StartTimeHigher;

    // Adjust ElapsedTime to time spent in this routine, excluding higher level ISRs.
    ElapsedTime -= TimeHigher;
    if (ElapsedTime > KiIsrTscLimit) {
        // If there is a debugger attached, breakin.   Otherwise do nothing.
        // N.B. bugchecking is another possibility.
...
// KiInitializeInterruptTimers but not KiInitializeInterruptTimersDpc should be in the INIT section.
...
VOID KiInitializeInterruptTimersDpc(IN PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2)
/*
Routine Description:
    This DPC is run twice on timer expiration.
    The time between runs us used to determine the frequency of the processor's Time Stamp Counter (TSC) in order to calculate a TSC delta
...
*/
{
    ...
    if (KiIsrTscLimit == 0xFFFFFFFFFFFFFFFF) {
        ...
        // timer controlling this DPC and free the memory associated with the timer and the DPC.
        Delta = RDTSC() - KiIsrTimerInit->InitialTime;

        KeCancelTimer(&KiIsrTimerInit->SampleTimer);
        ExFreePool(KiIsrTimerInit);

        // Delta is now the number of TSC clock ticks that occured in 10 seconds.  We choose such a large number to minimize error.

        // Calculate the number of TSC clock ticks in KiTimeLimitIsrMicroseconds microseconds.
...
VOID KiInitializeInterruptTimers(VOID)
{
    ...
    if (KiTimeLimitIsrMicroseconds == 0) {// If not timing ISRs, nothing to do.
        return;
    }

    // The kernel is initialized.   Use a timer to determine the amount
    // the Time Stamp Counter advances by in 10 seconds, then use that result to set the ISR time limit.
    if ((KeFeatureBits & KF_RDTSC) == 0) {
        return;// Processor doesn't support the RDTSC instruction, don't attempt to time ISRs.
    }

    KiIsrTimerInit = ExAllocatePoolWithTag(NonPagedPool, sizeof(*KiIsrTimerInit), '  eK');
    if (KiIsrTimerInit == NULL) {
        return;// Couldn't allocate memory for timer?  Skip ISR timing.
    }
