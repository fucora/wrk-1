...
NTSTATUS KiLoadMTRR(PVOID Context);
...
#define AMDK6_REGION_UNUSED     0xFFFFFFFF// Value to set base address to for unused indication.
#define AMDK6_REGION_FLAGS_BIOS 0x00000001// Flag to indicate that this region was set up by the BIOS.
#define AMDK6_MAX_MTRR        2// Usage count for hardware MTRR registers.

// AMD Function Prototypes.
VOID KiAmdK6InitializeMTRR(VOID);
NTSTATUS KiAmdK6RestoreMTRR();
NTSTATUS KiAmdK6MtrrSetMemoryType(ULONG BaseAddress, ULONG Size, MEMORY_CACHING_TYPE Type);
BOOLEAN KiAmdK6AddRegion(ULONG BaseAddress, ULONG Size, MEMORY_CACHING_TYPE Type, ULONG Flags);
NTSTATUS KiAmdK6MtrrCommitChanges(VOID);
NTSTATUS KiAmdK6HandleWcRegionRequest(ULONG BaseAddress, ULONG Size);
VOID KiAmdK6MTRRAddRegionFromHW(AMDK6_MTRR RegImage);
PAMDK6_MTRR_REGION KiAmdK6FindFreeRegion(MEMORY_CACHING_TYPE Type);
...
ULONG AmdMtrrHwUsageCount;// Usage counter for hardware MTRRs.
AMDK6_MTRR_MSR_IMAGE    KiAmdK6Mtrr;// Global variable image of MTRR MSR.
...
VOID KiAmdK6InitializeMTRR(VOID)
{
    ...
    // N.B. Normally this is done by KiInitializeMTRR but that routine is not called in the AMD K6 case.
    KeInitializeSpinLock(&KiRangeLock);

    // Read the MTRR registers to see if the BIOS has set them up.
    // If so, add entries to the region table and adjust the usage count.  
    //Serialize the region table.
    KeAcquireSpinLock(&KiRangeLock, &OldIrql);

    KiAmdK6Mtrr.u.QuadPart = RDMSR(AMDK6_MTRR_MSR);
    KiAmdK6MTRRAddRegionFromHW(KiAmdK6Mtrr.u.hw.mtrr0);// Check MTRR0 first.
    KiAmdK6MTRRAddRegionFromHW(KiAmdK6Mtrr.u.hw.mtrr1);// Now check MTRR1.

    KeReleaseSpinLock(&KiRangeLock, OldIrql);// Release the locks.
}
...
VOID KiAmdK6MTRRAddRegionFromHW(AMDK6_MTRR RegImage)
{
    ...
    if (RegImage.type != AMDK6_MTRR_TYPE_DISABLED) {
        // If this is a write combined region then add an entry to the region table.
        if ((RegImage.type & AMDK6_MTRR_TYPE_UC) == 0) {
            BaseAddress = RegImage.base << 17;// Create a new resion table entry.
            TempMask = RegImage.mask;// Calculate the size base on the mask value.
            ASSERT(TempMask != 0);// There should never be 4GB WC region!
            Size = 0x00020000;// Start with 128 size and search upward.
            ...
            KiAmdK6AddRegion(BaseAddress, Size, MmWriteCombined, AMDK6_REGION_FLAGS_BIOS);// Add the region to the table.
            AmdMtrrHwUsageCount++;
        }
    }
}
...
NTSTATUS KiAmdK6MtrrSetMemoryType(ULONG BaseAddress, ULONG Size, MEMORY_CACHING_TYPE Type)
{
    ...
    case MmWriteCombined:
        // H/W needs updating, lock down the code required to effect the change.
        if (KeGetCurrentIrql() >= DISPATCH_LEVEL) {
            // Code can not be locked down.  
            // Supplying a new range type requires that the caller calls at irql < dispatch_level.
            DBGMSG("KeAmdK6SetPhysicalCacheTypeRange failed due to calling IRQL == DISPATCH_LEVEL\n");
            return STATUS_UNSUCCESSFUL;
        }

        MmLockPageableSectionByHandle(ExPageLockHandle);// Lock the code.
    ...
    // Don't need to add these to the region table. 
    // Non-cached regions are accessed using a non-caching virtual pointer set up in the page tables.
        break;
    case MmCached:
...
NTSTATUS KiAmdK6HandleWcRegionRequest(ULONG BaseAddress, ULONG Size)
{
    ...
    // Try and find a region that overlaps or is adjacent to the new one and check to see if the combined region would be a legal mapping.
    for (i = 0; i < AmdK6RegionCount; i++) {
        pRegion = &AmdK6Regions[i];
        if ((pRegion->BaseAddress != AMDK6_REGION_UNUSED) && (pRegion->RegionType == MmWriteCombined)) {
            // Does the new start address overlap or adjoin an existing WC region?
            if (((pRegion->BaseAddress >= BaseAddress) && (pRegion->BaseAddress <= (BaseAddress + Size))) ||
                ...
                // Find the smallest legal size that is equal to the requested range. 
                // Scan all ranges from 128k - 2G. (Start at 2G and work down).
                ...
                if (bCanCombine) {
                    // If the resized range is OK, record the change in the region table and commit the changes to hardware.
    ...
    // Find the smallest legal size that is less than or equal to the requested range. 
    // Scan all ranges from 128k - 2G. (Start at 2G and work down).
    ...
    if (!bValidRange) {// Couldn't find a legal region that fit.
    ...
    return KiAmdK6MtrrCommitChanges();// Commit the changes to hardware.
}
...
PAMDK6_MTRR_REGION KiAmdK6FindFreeRegion(MEMORY_CACHING_TYPE Type)
{
    ...
    // If this is a MmWriteCombined request, limit the number of regions to match the actual hardware support.
    if (Type == MmWriteCombined) {
        if (AmdMtrrHwUsageCount >= AMDK6_MAX_MTRR) {
            // Search the table to see if there are any BIOS entries we can replace.
            for (i = 0; i < AmdK6RegionCount; i++) {
            ...
            return FALSE;// No free HW MTRRs and no reusable entries.
        }
    }
...
*/
{
    ...
    for (i = 0; i < AmdK6RegionCount; i++) {
        ...
        if ((AmdK6Regions[i].BaseAddress != AMDK6_REGION_UNUSED) && (AmdK6Regions[i].RegionType == MmWriteCombined)) {
            // Calculate the correct mask for this range size. 
            // The BaseAddress and size were validated and adjusted in AmdK6MtrrSetMemoryType().

            // Start with 128K and scan for all legal range values and build the appropriate range mask at the same time.
            ...
            if (dwWcRangeCount == 0) {
                ...
                // Should never happen!  This should have been caught in the calling routine.
    ...
    KiLoadMTRR(NULL);// Commit the changes to hardware.
