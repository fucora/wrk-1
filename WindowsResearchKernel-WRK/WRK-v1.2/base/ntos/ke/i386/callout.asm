...
; Arguments:
;    OutputBuffer - Supplies a pointer to the variable that receives the address of the output buffer.
...
;    N.B. This function does not return to its caller. A return to the caller is executed when a NtCallbackReturn system service is executed.
...
; and callback stack addresses in the current frame. Also save the new callback stack address in the thread object.
...
;    StackLimit (esp + 8) - Supplies a pointer to the limit of the new kernel stack.
; Return Value:
...
; current trap frame, restore the trap frame and callback stack addresses, store the output buffer address and length, and set the service status.
