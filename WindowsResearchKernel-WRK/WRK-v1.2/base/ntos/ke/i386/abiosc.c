...
// ABIOS specific drivers.  This number should be the same across all the processors.
...
Routine Description:
    ...
    N.B. To speed the search, this routine ASSUMES that the LIDs with the same Device ID always appear consecutively in the
...
*/
{
    ...
    // If yes, we proceed to check the device id. 
    // Otherwise, we skip the Lid.
...
/*
Routine Description:
    This function releases a logical Id.
    This routine is called at ABIOS device driver destallation or termination.
Arguments:
...
/*
Routine Description:
    This function calls an ABIOS service routine on behave of device driver using Operating System Transfer Convention.
Arguments:
...
Return Value:
    ...
    STATUS_ABIOS_SELECTOR_NOT_AVAILABLE - if system can not allocate the number of selectors requested.
*/
{
    ...
    if (KiNumberFreeSelectors >= NumberOfSelectors) {
        ...
        // Because the 'selector' is an offset to the beginning of GDT and it should be the same across all the processors.
