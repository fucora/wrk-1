...
/*
    ...
    If the region of memory passed in includes memory that lies above 4G, then a new buffer is allocated below 4G.
Arguments:
    IdentityMap - Pointer to the structure which will be filled with the newly created top-level directory address.  
                  It also provides storage for the pointers used in allocating and freeing the memory.
    StartVa - Pointer to the first byte of the region of memory that is to be memory mapped.
    EndVa - Pointer to the byte immediately after the last byte of the region that is to be memory mapped.
Return Value:
    TRUE if the function succeeds, FALSE otherwise.

    Note - Ki386ClearIdentityMap() should be called even on FALSE return to free any memory allocated.
*/
    ...
    if( identityAddress.QuadPart == 0) {
        // The input buffer was not contiguous or not below 4G, and a suitable buffer could not be allocated.
        return FALSE;
    }
    ...
    do {
        ...
        // Advance both the Va and identityAddress pointers in anticipation of mapping in another page.
        currentVa += PAGE_SIZE;
...
#if defined(_X86PAE_)
    // This PAE-only outer loop walks the page directory pointer table entries and processes each valid page directory referenced.

    pageDirectoryPointerTable = IdentityMap->TopLevelDirectory;
    for (pageDirectoryPointerTableIndex = 0; pageDirectoryPointerTableIndex < (1 << PPI_BITS); pageDirectoryPointerTableIndex++) {
...
/*
    ...
    The allocation is recorded in the IdentityMap structure, so that it can be freed on cleanup.
Arguments:
...
*/
{
    ...
    if (Low4Meg != FALSE) {
        // The caller has specified that a page must reside physically below 4 MB.
    ...
    if (page != NULL) {
        // Record that this page was allocated so that it can be freed when the IdentityMap structure is cleared.
...
BOOLEAN Ki386IdentityMapMakeValid(
    IN OUT PIDENTITY_MAP IdentityMap,
    IN     PHARDWARE_PTE PageTableEntry,
    OUT    PVOID *Page OPTIONAL
    )
/*
    If the page table has the valid bit set, this function merely returns the address referenced by the page table entry.

    If the page table does not have the valid bit set, then another page
    is allocated and inserted into the page table entry and the entry is marked valid.

    NOTE: At this point, PTE frames are virtual.  After the entire mapping
          is built, we go through and convert all virtual frames to physical ones.
Arguments:
    IdentityMap - Context pointer for this identity mapping.
    PageTableEntry - Pointer to the page table entry.
    Page - Virtual address now referenced by the PTE, whether it was valid before or not.
Return Value:
...
*/
{
    ...
    if (PageTableEntry->Valid != 0) {
        ...
        //       the tables and convert all of the virtual page numbers to physical page numbers.
...
*/
{
    ...
    if (IdentityMap->TopLevelDirectory == NULL) {
        // Allocate a top-level directory structure, either a page directory or a page directory pointer table.
    ...
    // not valid, allocate a new page table and mark the page directory entry valid and writeable.
...
{
/*
    This function checks to see if the physical memory backing a virtual buffer is physically contiguous and lies completely below 4G.
    If these requirements are met, then the physical address of StartVa is returned.
    If not, then a physically contiguous buffer is allocated, the contents of the region is copied in, and its address is returned.
Arguments:
    IdentityMap - Pointer to the identity map building structure.
    StartVa - Virtual address of the start of the region for which a physically contiguous copy is desired.
    Length - Length of the region for which a physically contiguous copy is desired.
*/
    ...
    // Count the number of pages in the buffer, and record the physical address of the start of the buffer.
    ...
    while (TRUE) {
        ...
        if (pagesRemaining == 0) {
            ...
            // been found to meet the critera.  Return the physical address of the start of the buffer.
    ...
    // The buffer does not meet the criteria and so its contents must be copied to a buffer that does.
    identityBuffer = Ki386AllocateContiguousMemory( IdentityMap, pagesToMap, TRUE );
    if (identityBuffer == 0) {
        // A contiguous region of the appropriate size could not be located below 4G physical.
...
{
/*
    This function just frees the page directory and page tables created in Ki386CreateIdentityMap().
*/
...
{
/*
    This function just passes info on to the assembly routine Ki386EnableLargePage().
*/
