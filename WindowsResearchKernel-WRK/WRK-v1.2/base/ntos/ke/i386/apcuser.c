    ...
    */
{
    ...
    // APCs are not defined for V86 mode; however, it is possible a thread is trying to set it's context to V86 mode - this isn't going to work,
    // but we don't want to crash the system so we check for the possibility before hand.
    ...
    try {
        ...
        // Make sure to include space for a double-word aligned exception registration record as well.
        // For compatibility the exception registration must follow the context record.
        TopOfStack = (ContextFrame.Esp & ~(__alignof(EXCEPTION_REGISTRATION_RECORD) - 1));
        Length = CONTEXT_ALIGNED_SIZE + sizeof(KAPC_RECORD);
        UserStack = ((TopOfStack - sizeof(EXCEPTION_REGISTRATION_RECORD)) & ~CONTEXT_ROUND) - Length;

        // Probe user stack area for writability and then transfer the context record to the user stack.
        // Note the minimum alignment is used as the above code ensures the proper alignment of the addresses.
