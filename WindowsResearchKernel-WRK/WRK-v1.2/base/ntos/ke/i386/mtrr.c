...
// N.B. During system initialization these variables may be changed for 40-bits of physical address support.
...
//               and contains the current range database of how physical addresses have been set
...
/*
Routine Description:
    Called to incrementally initialize the physical range database feature.
    First processor's MTRR set is read into the physical range database.
Arguments:
...
*/
{
    ...
    if (KeGetPcr()->Number == 0) {
        ...
        if (!KiRangeInfo.Default.u.hw.MtrrEnabled || KiRangeInfo.Capabilities.u.hw.VarCnt == 0 || KiRangeInfo.Default.u.hw.Type != MTRR_TYPE_UC) {
            DBGMSG("MTRR feature disabled.\n");
            Status = FALSE;
        } else {
            // If USWC type is supported by hardware, but the MTRR feature is not set in KeFeatureBits, it is because
            // the HAL indicated USWC should not be used on this machine.  (Possibly due to shared memory clusters).
            ...
            // Don't allocate a new range on reinitialization from hibernate.
            if (KiRangeInfo.Ranges == NULL) {
                KiRangeInfo.Ranges = ExAllocatePoolWithTag(NonPagedPool, sizeof(ONE_RANGE) * KiRangeInfo.MaxRange, '  eK');
            }
            if (KiRangeInfo.Ranges != NULL) {
    ...
    // - if the request for setting a variable MTRR specifies an address which is not 4M aligned or length is not
    // a multiple of 4M then possible problem for INVLPG inst.
    // Detect if workaround is required

    Prcb = KeGetCurrentPrcb();
    if (Prcb->CpuType == 6 && (Prcb->CpuStep == 0x0101 || Prcb->CpuStep == 0x0102 || Prcb->CpuStep == 0x0106 || Prcb->CpuStep == 0x0107)) {
        if (strcmp((PCHAR)Prcb->VendorString, "GenuineIntel") == 0) {
            // Only do this if it's an Intel part, other manufacturers may have the same stepping numbers but no bug.
    ...
    if (Status && (KeGetPcr()->Number == 0)) {
        ...
        for (Index = 0; Index < Capabilities.u.hw.VarCnt; Index++) {
            ...
            // spanned by the variable MTRR. Thus just checking the Valid Bit should be sufficient for identifying a valid MTRR.
...
/*
Routine Description:
    This function reloads the MTRR registers to be the current known values.
    This is used on a system wakeup to ensure the registers are sane.
...
*/
{
    ...
    // If the processor is a AMD K6 with MTRR support then perform processor specific implementation.
...
Routine Description:
    ...
    If the system does not support setting cache policies based on physical ranges, no action is taken.
Arguments:
    PhysicalAddress - The starting address of the range being set
    NumberOfBytes   - The length, in bytes, of the range being set
    CacheType       - The caching type for which the physical range is to be set to.

                     NonCached:
                        Setting ranges to be NonCached is done for book keeping reasons.
                        A return of SUCCESS when setting a range NonCached does not mean it has been physically set to as NonCached.
                        The caller must use a cache-disabled virtual pointer for any NonCached range.

                     Cached:
                        A successful return indicates that the physical range has been set to cached.
                        This mode requires the caller to be at irql < dispatch_level.

                     FrameBuffer:
                        A successful return indicates that the physical range has been set to be framebuffer cached.
                        This mode requires the caller to be at irql < dispatch_level.

                     USWCCached:
                        This type is to be satisfied only via PAT and fails for the MTRR interface.

Return Value:

    STATUS_SUCCESS - if success, the cache attributes of the physical range have been set.

    STATUS_NOT_SUPPORTED - either feature not supported or not yet initialized, or MmWriteCombined type not supported and is
                           requested, or input range does not match restrictions imposed by workarounds for current processor stepping
                           or is below 1M (in the fixed MTRR range), or not yet initialized.

    STATUS_UNSUCCESSFUL - Unable to satisfy request due to
                        - Unable to map software image into limited # of hardware MTRRs.
...
*/
{
    ...
    // Addresses above 4GB, below 1MB or not page aligned and page length are not supported.
    ...
    // If processor doesn't have the memory type range feature return not supported.
    if (!KiRangeInfo.RangesValid) {
        return STATUS_NOT_SUPPORTED;
    }
    ...
    // an address which is not 4M aligned or length is not a multiple of 4M then return status not supported
    ...
    case MmNonCached:
        ...
        // NonCached ranges do not need to be reflected into the h/w state as all non-cached ranges are mapped with cache-disabled pointers.
        // This also means that cache-disabled ranges do not need to be put into mtrrs, or held in the range, regardless of the default range type.
    ...
    // The default type is UC thus the range is still mapped using a Cache Disabled VirtualPointer and hence it need not be added.

    // If h/w needs updated, lock down the code required to effect the change
    if (EffectRangeChange) {
        if (KeGetCurrentIrql() >= DISPATCH_LEVEL) {
            // Code can not be locked down.   Supplying a new range type requires that the caller calls at irql < dispatch_level.
    ...
    if (NT_SUCCESS(NewRange.Status)) {
        // If the new range is NonCached, then don't remove standard memory caching types
        memset(RemoveThisType, TRUE, MTRR_TYPE_MAX);
        if (NewRange.Type != MTRR_TYPE_UC) {
            // If the requested type is uncached then the physical memory region is mapped using a cache disabled virtual pointer.
            // The effective memory type for that region will be the lowest common denominator of the MTRR type and the cache type in the
            // PTE.  Therefore for a request of type UC, the effective type will be UC irrespective of the MTRR settings in that range.
            // Hence it is not necessary to remove the existing MTRR settings (if any) for that range.
...
/*
Routine Description:
    This function removes any range overlapping with the passed range, of type supplied in RemoveThisType from the global range database.
Arguments:
    ...
    RemoveThisType  - A TRUE flag for each type which can not overlap the target range
Return Value:
    TRUE  - if the range database was altered such that it may no longer be sorted.
*/
{
    ULONG       i;
    PONE_RANGE  Range;
...
/*
Routine Description:
    This functions sets up the context information required to track & later effect a range change in hardware
Arguments:
    NewRange        - Context information
Return Value:
    None
...
/*
Routine Description:
    This functions commits the range database to hardware, or backs out the current changes to it.
Arguments:
    NewRange        - Context information
Return Value:
    None
*/
{
    ...
    do {
        ...
        // base address. Scan range database combining adjacent and overlapping ranges of the same type
        for (Index = 0; Index < (ULONG)KiRangeInfo.NoRange - 1; Index++) {
            Range = &KiRangeInfo.Ranges[Index];

            // Scan the range database. If ranges are adjacent/overlap and are of the same type, combine them.
            for (Index2 = Index + 1; Index2 < (ULONG)KiRangeInfo.NoRange; Index2++) {
                ...
                if (l >= KiRangeInfo.Ranges[Index2].Base  && Range[0].Type == KiRangeInfo.Ranges[Index2].Type) {
                    ...
                    if (Index2 < (ULONG)KiRangeInfo.NoRange - 1) {
                        // Copy everything from Index2 till end of range list. # Entries to copy is
                        // (KiRangeInfo.NoRange -1) - (Index2+1) + 1
                        RtlCopyMemory(&(KiRangeInfo.Ranges[Index2]), &(KiRangeInfo.Ranges[Index2 + 1]), sizeof(ONE_RANGE) * (KiRangeInfo.NoRange - Index2 - 1));
                    }
        ...
        // At this point the range database is sorted on base address and adjacent/overlapping ranges of the same
        ...
        for (Index = 0; Index < (ULONG)KiRangeInfo.NoRange - 1 && !Restart; Index++) {
            ...
            // If ranges overlap and are not of same type, and if the overlap is not legal then carve them to the best cache type available.
            for (Index2 = Index + 1; Index2 < (ULONG)KiRangeInfo.NoRange && !Restart; Index2++) {
                if (l > KiRangeInfo.Ranges[Index2].Base) {
                    if (Range[0].Type == MTRR_TYPE_UC || KiRangeInfo.Ranges[Index2].Type == MTRR_TYPE_UC) {
                        // Overlap of a UC type with a range of any other type is legal
                        ...
                        // Overlap of WT and WB range is legal. The overlap range will be WT.
                    } else {
                        // This is an illegal overlap and we need to carve the ranges to remove the overlap.
                        // Pick range which has the cache type which should be used for the overlapped area
                        ...
                        Restart = KiRemoveRange(
                            ...
                            (Range[0].Limit < KiRangeInfo.Ranges[Index2].Limit ? Range[0].Limit : KiRangeInfo.Ranges[Index2].Limit),
                            RemoveThisType
                        );
                    }
                }
    ...
    for (Index = 0; NT_SUCCESS(NewRange->Status) && Index < KiRangeInfo.NoRange; Index++) {
        ...
        while (Length) {
            ...
            // If there are too many MTRRs, and currently setting a Non-USWC range try to remove a USWC MTRR.
            ...
            if (NoMTRR > (ULONG)KiRangeInfo.Capabilities.u.hw.VarCnt) {
                if (Range->Type != MTRR_TYPE_USWC) {
                    ...
                    // Default type should always be UC unless BIOS changes it. Still ASSERT!
                    ...
                    for (Index2 = 0; Index2 < KiRangeInfo.Capabilities.u.hw.VarCnt; Index2++) {
                        if (MTRR[Index2].Base.u.hw.Type == MTRR_TYPE_USWC) {
                            CurrLength = KiMaskToLength(MTRR[Index2].Mask.u.QuadPart & KiMtrrMaskMask);
    ...
    if (NT_SUCCESS(NewRange->Status)) {
        ...
        // Previously enabled MTRRs with index > NoMTRR which could conflict with existing setting should be disabled
        // This is taken care of by setting NewRange->NoMTRR to total number of variable MTRRs.
        ...
        if (TargetProcessors != 0) {
            ...
            // ensure this processor doesn't enter the debugger due to some interrupt service routine.
            KeRaiseIrql(HIGH_LEVEL, &OldIrql2);

            // There's no reason for any debug events now, so signal the other processors that they can all disable interrupts and begin the MTRR update
            Prcb->ReverseStall += 1;
        }
#endif
...
/*
Routine Description:
    This functions returns a weighting of the passed in range's cache type.
    When two or more regions collide within the same h/w region the types are weighted and that cache type of the higher weight is used for the collision area.
Arguments:
...
/*
Routine Description:
    This function returns the length specified by a particular mtrr variable register mask.
*/
{
    if (Mask == 0) {
...
/*
Routine Description:
    This function constructs the mask corresponding to the input length to be set in a variable MTRR register.
    The length is assumed to be a multiple of 4K.
*/
{
...
Routine Description:
    ...
    bit set in the passed ULONGLONG parameter. Passed parameter must be non-zero.
*/
{
    ULONG   bitno;
...
/*
Routine Description:
    This function returns a bit position of the most significant bit set in the passed ULONGLONG parameter.
    Passed parameter must be non-zero.
*/
{
    ULONG   bitno;
...
*/
{
    ...
    // Wait for all processors to reach the same place, restore interrupts and return.
    if (!(KeFeatureBits & KF_AMDK6MTRR)) {
        KiLockStepExecution(&Context->Synchronize);
    }
...
VOID KiLockStepExecution(IN PPROCESSOR_LOCKSTEP  Context)
{
    ...
    if (Prcb->Number == (CCHAR)Context->Processor) {
        ...
        Context->TargetCount = 0;// Reset count for next time        
        InterlockedIncrement(TargetPhase);// Let waiting processor go to next synchronization point
    } else {
        CurrentPhase = *TargetPhase;// Get current phase        
        InterlockedIncrement((PLONG)&Context->TargetCount);// Signal that we have completed the current phase
