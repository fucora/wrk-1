...
; Abstract:
;    This module implements the code necessary to field and process i386 trap conditions.
...
;    Note, all the memory write operations in this macro are done in such a way that if a page fault occurs the memory will still be in a consistent state.
...
; Macro Description:
;    This macro is called before returning to user mode.
;    It dispatches any pending user mode APCs.
; Arguments:
;    TFrame - TrapFrame
;    interrupts disabled
...
; Routine Description:
;    This routine is called after an exception being detected during a 16 bit stack.
;    The system will switch 16 stack to 32 bit stack and bugcheck.
; Arguments:
;    None.
; Return value:
;    system stopped.
...
; Macro Description:
;    This macro allocates exception record on stack, sets up exception record using specified parameters and finally sets up arguments and calls _KiDispatchException.
...
; Set up exception record for raising Integer_Divided_by_zero exception and call _KiDispatchException
...
; Routine Description:
;    This routine is called as a system service to continue execution after an exception has occurred.
;    Its function is to transfer information from the specified context record into the trap frame that was built when the
;    system service was executed, and then exit the system as if an exception had occurred.

;   WARNING - Do not call this routine directly, always call it as ZwContinue!!!  This is required because it needs the trapframe built by KiSystemService.

; Arguments:
;    KTrapFrame (ebp+0: after setup) -> base of KTrapFrame
;    ContextRecord (ebp+8: after setup) = Supplies a pointer to a context rec.
;    TestAlert (esp+12: after setup) = Supplies a boolean value that specifies whether alert should be tested for the previous processor mode.
...
;    Its function is to transfer information from the specified context record into the trap frame that was built when the system service was executed.
;    The exception may be raised as a first or second chance exception.

;   WARNING - Do not call this routine directly, always call it as ZwRaiseException!!!  This is required because it needs the trapframe built by KiSystemService.

;   NOTE - KiSystemService will terminate the ExceptionList, which is not what we want for this case, so we will fish it out of the trap frame and restore it.
...
;    FirstChance (ebp+16: after setup) -> Supplies a boolean value that specifies whether the exception is to be raised as a first (TRUE) or second chance (FALSE) exception.
; Return Value:
;    None.
...
;   If the exception was not handled, exit via KiServiceExit so the return status is propagated back to the caller.
...
;   Routine Description:
;       Local stub which reflects an exception to a VDM using Ki386VdmReflectException,
;   Arguments:
;       ebp -> Trap frame
