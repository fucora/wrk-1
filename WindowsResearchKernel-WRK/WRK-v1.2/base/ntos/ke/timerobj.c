...
// Flags controlling the nature of timer checks. 
// This checks are triggered when driver verifier is active and default behavior is to check for active timers involved in pool free and driver unload operations.
...
VOID KeInitializeTimerEx(__out PKTIMER Timer, __in TIMER_TYPE Type)
/*
Routine Description:
...
BOOLEAN KeCancelTimer(__inout PKTIMER Timer)
/*
Routine Description:
...
*/
{
    ...
    // Raise IRQL to dispatcher level, lock the dispatcher database, and capture the timer inserted status. If the timer is currently set, then remove it from the timer list.
...
BOOLEAN KeReadStateTimer(__in PKTIMER Timer)
/*
Routine Description:
...
BOOLEAN KeSetTimerEx(__inout PKTIMER Timer, __in LARGE_INTEGER DueTime, __in LONG Period, __in_opt PKDPC Dpc)
/*
Routine Description:
    This function sets a timer to expire at a specified time.
    If the timer is already set, then it is implicitly canceled before it is set to expire at the specified time.
...
Arguments:
    ...
    Dpc - Supplies an optional pointer to a control object of type DPC.
Return Value:
    A boolean value of TRUE is returned if the the specified timer was currently set. Otherwise, a value of FALSE is returned.
*/
{
    ULONG Hand;
    BOOLEAN Inserted;
    ...
    // If the timer has already expired, then signal the timer.
    // Otherwise, set the signal state to false and attempt to insert the timer in the timer table.

    // N.B. The signal state must be cleared before it is inserted in the timer table in case the period is not zero.
    Timer->Dpc = Dpc;
    Timer->Period = Period;
    ...
    return Inserted;// Return boolean value that signifies whether the timer was set or not.
}
...
/*
Routine Description:
    This function returns the interrupt time at which the specified timer is due to expire. If the timer is not pending, then zero is returned.
...
*/
{
    ...
    // Raise IRQL to dispatcher level and lock dispatcher database.
    // If the timer is currently pending, then return the due time. Otherwise, return zero.
    DueTime = 0;
    KiLockDispatcherDatabase(&OldIrql);
    if (Timer->Header.Inserted) {
        DueTime = Timer->DueTime.QuadPart;
    }
    KiUnlockDispatcherDatabase(OldIrql);// Unlock the dispatcher database, lower IRQL to its previous value, and return the due time.
...
/*
Routine Description:
    This function checks to detemine if the specified block of memory overlaps the range of an active timer.
Arguments:
...
*/
{
    ...
    // Raise IRQL to dispatcher level and lock dispatcher database.
    // Scan the timer database and check for any timers in the specified memory block.
    KiLockDispatcherDatabase(&OldIrql);
    Index = 0;
    do {
        ...
        while (NextEntry != ListHead) {
            ...
            if ((Address > (Start - sizeof(KTIMER))) && (Address < End)) {// Check that the timer object is not in the range.
                KeBugCheckEx(TIMER_OR_DPC_INVALID, 0x0, (ULONG_PTR)Address, (ULONG_PTR)Start, (ULONG_PTR)End);
            }

            if (Timer->Dpc) {
                Address = (PUCHAR)Timer->Dpc;
                if ((Address > (Start - sizeof(KDPC))) && (Address < End)) {// Check that the timer DPC object is not in the range.
                ...
                if (Address >= Start && Address < End) {// Check that the timer DPC routine is not in the range.
