...
/*
Routine Description:
    This function completes a timer that could not be inserted in the timer table because its due time has already passed.
    N.B. This function must be called with the corresponding timer table entry locked at raised IRQL.
...
*/
{
    ...
    // Remove the timer from the specified timer table list and insert the timer in a dummy list in case the timer is cancelled while releasing the timer table lock and acquiring the dispatcher lock.
    ...
    if (ListHead.Flink != &ListHead) {// If the timer has not been removed from the dummy list, then signal the timer.
        RequestInterrupt = KiSignalTimer(Timer);
    }

    KiUnlockDispatcherDatabaseFromSynchLevel();// Release the dispatcher lock and request a DPC interrupt if required.
...
Return Value:
    If the timer has expired, than a value of TRUE is returned. Otherwise, a value of FALSE is returned.
*/
{
    ULONG64 DueTime;
    LOGICAL Expired;
    ...
    // If the timer is due before the first entry in the computed list or the computed list is empty, then insert the timer at the front of the list and check if the timer has already expired.
    // Otherwise, insert then timer in the sorted order of the list searching from the back of the list forward.

    // N.B. The sequence of operations below is critical to avoid the race condition that exists between this code and the clock interrupt code that examines the timer table lists to detemine when timers expire.
    ...
    if (NextEntry == ListHead) {
        // The computed list is empty or the timer is due to expire before the first entry in the list.

        // Make sure the writes for the update of the due time table are done before reading the interrupt time.
...
/*
Routine Description:
    This function signals a timer that could not be inserted in the timer table because its due time has already passed.
    N.B. This function must be called with the dispatcher database locked at at raised IRQL.
Arguments:
    Timer - Supplies a pointer to a dispatcher object of type timer.
Return Value:
...
*/
{
    ...
    // If a DPC is specified, 
    // then insert it in the target processor's DPC queue or capture the parameters in the DPC table for subsequent execution on the current processor.

    // N.B. A dispatch interrupt must be forced on the current processor, however, this will occur very infrequently.
    if (Dpc != NULL) {
