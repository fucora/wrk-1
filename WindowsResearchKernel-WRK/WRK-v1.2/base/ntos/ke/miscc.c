...
/*
Routine Description:
    This function returns the time increment value in 100ns units. This is the value that is added to the system time at each interval clock interrupt.
Return Value:
...
Routine Description:
    ...
    N.B. There are only two valid values for the profile IRQL which are PROFILE_LEVEL and HIGH_LEVEL.
Arguments:
...
*/
{
    ...
    *((volatile ULONG64 *)&SharedUserData->SystemTime) = NewTime->QuadPart;
    ...
    KeLowerIrql(OldIrql2);// Lower IRQL to dispatch level and if needed adjust the physical system interrupt time.
    if (AdjustInterruptTime != FALSE) {
        AdjustInterruptTime = KeAdjustInterruptTime(TimeDelta.QuadPart);
    }

    // If the physical interrupt time of the system was not adjusted, then recompute any absolute timers in the system for the new system time.
    if (AdjustInterruptTime == FALSE) {
        // Acquire the timer table lock, remove all absolute timers from the timer queue so their due time can be recomputed, and release the timer table lock.
        ...
        // Recompute the due time and reinsert all absolute timers in the timer tree. If a timer has already expired, then insert the timer in the expired timer list.
...
/*
Routine Description:
    This function moves the physical interrupt time of the system forward by the specified time delta after a system wake has occurred.
Arguments:
    TimeDelta - Supplies the time delta to be added to the interrupt time, tick count and the performance counter in 100ns units.
*/
{
    ...
    if (TimeDelta < 0) {
        ...
        Adjust.Barrier = 1;
        KeIpiGenericCall((PKIPI_BROADCAST_WORKER)KiCalibrateTimeAdjustment, (ULONG_PTR)(&Adjust));
        return TRUE;
    }
}
...
*/
{
    ...
    if (InterlockedDecrement((PLONG)&Adjust->KiNumber)) {
        ...
        while (Adjust->Barrier);// Wait to perform the time set.
    } else {
        // Set timer expiration dpc to scan the timer queues once for any expired timers.
        KeRemoveQueueDpc(&KiTimerExpireDpc);
        KeInsertQueueDpc(&KiTimerExpireDpc, ULongToPtr(KiQueryLowTickCount() - TIMER_TABLE_SIZE), NULL);
        ...
        // Apply changes to interrupt time, tick count, tick offset, and the performance counter.
        ...
        *((volatile ULONG64 *)&SharedUserData->InterruptTime) = InterruptTime.QuadPart;
        ...
        Adjust->Barrier = 0;// Apply the performance counter change.
    }
    ...
    // Log an event that the performance counter has been calibrated properly and indicate the new performance counter value.
    if (PERFINFO_IS_GROUP_ON(PERF_POWER)) {
        LogEntry.PerformanceCounter = KeQueryPerformanceCounter(NULL);
        PerfInfoLogBytes(PERFINFO_LOG_TYPE_PO_CALIBRATED_PERFCOUNTER, &LogEntry, sizeof(LogEntry));
    }
...
*/
{
    ...
    if ((Index > NUMBER_SERVICE_TABLES - 1) || (KeServiceDescriptorTable[Index].Base != NULL) || (KeServiceDescriptorTableShadow[Index].Base != NULL)) {
        ...
        // Otherwise, both the shadow and static system service tables are updated.
...
NTSTATUS KeQueryLogicalProcessorInformation(OUT PVOID SystemInformation, IN ULONG SystemInformationLength, OUT PULONG ReturnedLength)
/*
Routine Description:
    This function returns information about the physical processors and nodes in the host system.
    Information is returned for each physical processor in the host system that describes any associated logical processors if present.
    Information is returned for each node in the host system that describes the processors associated with the node.
    N.B. It assumed that specified buffer is either accessible or access is protected by an outer try/except block.
Arguments:
    SystemInformation - Supplies a pointer to a buffer which receives the specified information.
    SystemInformationLength - Supplies the length of the output buffer in bytes.
    ReturnLength - Supples a pointer to a variable which receives the number of bytes necessary to return all of the information records available.
*/
{
    ...
    do {
        ...
        if ((ActiveProcessors & 1) != 0) {
            ...
            // Skip logical processors that are not the master of their thread set.
#if defined(NT_SMT)
            if (Prcb != Prcb->MultiThreadSetMaster) {
                continue;
            }

            // If this physical processor has more than one logical processor, then mark it as an SMT relationship.
            ...
            // Add a record for each cache level associated with the physical processor.
#if defined(_AMD64_)
    ...
    // Return the length of the buffer required to hold the entire set of information.
...
#if defined(_AMD64_)
KAFFINITY KeQueryMultiThreadProcessorSet(ULONG Number)
/*
Routine Description:
    This function queries the master multithread set for the specified processor.
Arguments:
...
Routine Description:
    ...
    This value should be used as a recommended alignment / granularity for shared data.
Return Value:
...
/*
Routine Description:
    This routine is called to process the list of registered Non-Maskable-Interrupt (NMI) handlers in the system.
...
PVOID KeRegisterNmiCallback(__in PNMI_CALLBACK CallbackRoutine, __in_opt PVOID Context)
/*
Routine Description:
...
Return Value:
    Returns an arbitary handle that must be passed to KeDeregisterNmiCallback or NULL if registration was unsuccessful.
*/
{
    PKNMI_HANDLER_CALLBACK Handler;
    ...
    Handler = ExAllocatePoolWithTag(NonPagedPool, sizeof(KNMI_HANDLER_CALLBACK), 'IMNK');
    if (Handler == NULL) {
        return Handler;
    }
    ...
    // Because the lock is held, the following can't fail but is needed to ensure the compiler doesn't store KiNmiCallbackList before
    // storing Handler->Next because the NMI handler may run down this list and does not (can not) take the lock.
    Next = InterlockedCompareExchangePointer(&KiNmiCallbackListHead, Handler, Handler->Next);
    ASSERT(Next == Handler->Next);
    KeReleaseSpinLock(&KiNmiCallbackListLock, OldIrql);

    return Handler->Handle;// Return the address of this handler as an opaque handle.
}
...
*/
{
    ...
    // N.B. In the current implementation, the handle is the address of the handler however this code is designed for a more opaque handle.
    PreviousNext = &KiNmiCallbackListHead;
    for (Handler = *PreviousNext; Handler; PreviousNext = &Handler->Next, Handler = Handler->Next) {
    ...
    *PreviousNext = Handler->Next;
    ...
    // Cycle through each processor in the system to ensure that any NMI which has begun execution on another processor has completed execution before releasing the memory for the NMI callback object.
#if !defined(NT_UP)
    ActiveProcessors = KeActiveProcessors;
