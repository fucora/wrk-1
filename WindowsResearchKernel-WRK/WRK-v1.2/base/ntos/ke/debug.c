...
// KiDebugRoutine - This is the address of the kernel debugger.
//      Initially this is filled with the address of a routine that just returns.
//      If the system debugger is present in the system, then it sets this location to the address of the system debugger's routine.
...
/*
Routine Description:
    This function freezes the execution of all other processors in the host configuration and then returns to the caller.
Arguments:
...
*/
{
    ...
    if (FrozenState(KeGetCurrentPrcb()->IpiFrozen) == FREEZE_OWNER) {
        ...
        // Return without trying to re-acquire lock or without trying to IPI the other processors again
        return Enable;
    }
    ...
    UNREFERENCED_PARAMETER(TrapFrame);
    UNREFERENCED_PARAMETER(ExceptionFrame);
    KeAcquireSpinLockAtDpcLevel(&KiFreezeExecutionLock);// Acquire the KiFreezeExecutionLock.  No need to check for FREEZE IPIs as these come in as NMIs anyway.
#else
    // Try to acquire the KiFreezeExecutionLock before sending the request.
    // To prevent deadlock from occurring, we need to accept and process incoming FreezeExecution requests while we are waiting to acquire the FreezeExecutionFlag.
    while (KeTryToAcquireSpinLockAtDpcLevel(&KiFreezeExecutionLock) == FALSE) {
        // FreezeExecutionLock is busy.  Another processor may be trying to IPI us - go service any IPI.
    ...
    // After acquiring the lock flag, we send Freeze request to each processor in the system (other than us) and wait for it to become frozen.
    ...
    KiOldIrql = OldIrql;// Save the old IRQL.
#else
    KiOldIrql = KeGetCurrentIrql();// Save the current IRQL.
#endif      // !defined(NT_UP)

    return Enable;// Return whether interrupts were previous enabled.
}
...
Routine Description:
    ...
    If a trapframe is supplied to current state is saved into the prcb for the debugger.
Arguments:
    TrapFrame - Supplies a pointer to the trap frame that describes the trap.
    ExceptionFrame - Supplies a pointer to the exception frame that describes the trap.
*/
{
#if !defined(NT_UP)
    KIRQL OldIrql;
    ...
    if ((KiFreezeExecutionLock == 0) && (KiFreezeLockBackup == 0) && (KeBugCheckActive == FALSE)) {
#if DBG
        DbgBreakPoint();
#endif
        return;
    ...
    // Sweep the data cache in case this is a system crash and the bugcheck code is attempting to write a crash dump file.
    KeSweepCurrentDcache();

    //  Wait for person requesting us to freeze to clear our frozen flag
    while (FrozenState(Prcb->IpiFrozen) == TARGET_FROZEN) {
        if (Prcb->IpiFrozen & FREEZE_ACTIVE) {
            ...
            if (TrapFrame) {
                ...
                ExceptionRecord.ExceptionAddress = (PVOID)CONTEXT_TO_PROGRAM_COUNTER(&Prcb->ProcessorState.ContextFrame);
                Status = (KiDebugSwitchRoutine)(&ExceptionRecord, &Prcb->ProcessorState.ContextFrame, FALSE);
            } else {
                Status = ContinueError;
            }

            // If status is anything other then, continue with next processor then reselect master
...
KCONTINUE_STATUS KeSwitchFrozenProcessor(IN ULONG ProcessorNumber)
{
    ...
    // This processor must be FREEZE_OWNER, wait to be reselected as the active processor
    ...
    return ContinueProcessorReselected;// Reselect this processor
}
...
/*
Routine Description:
    This function thaws the execution of all other processors in the host configuration and then returns to the caller.
    It is intended for use by the kernel debugger.
Arguments:
    Enable - Supplies the previous interrupt enable that is to be restored after having thawed the execution of all other processors.
*/
{
#if !defined(NT_UP)
...
/*
Routine Description:
    This routine is called from code that is spinning with interrupts disabled,
    waiting for something to happen, when there is some (possibly extremely small) chance that that thing will not happen because a system freeze has been initiated.
