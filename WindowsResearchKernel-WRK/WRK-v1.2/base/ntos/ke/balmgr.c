...
Abstract:
    ...
    The balance set manager performs the following operations:
        ...
        2. Removes processes from the balance set when memory gets tight and brings processes back into the balance set when there is more memory available.
        3. Makes the kernel stack resident for threads whose wait has been completed, but whose stack is nonresident.
        4. Arbitrarily boosts the priority of a selected set of threads to prevent priority inversion in variable priority levels.

    In general, the balance set manager only is active during periods when memory is tight.
*/
...
#define MAXIMUM_THREAD_STACKS 5// Define maximum number of thread stacks that can be out swapped in a single time period.
#define PERIODIC_INTERVAL (1 * 1000 * 1000 * 10)// Define periodic wait interval value.
#define READY_WITHOUT_RUNNING  (4 * 75)// Define amount of time a thread can be in the ready state without having is priority boosted (approximately 4 seconds).


// Define kernel stack protect time. For small systems the protect time is 3 seconds. For all other systems, the protect time is 5x seconds.
...
VOID KiScanReadyQueues(IN PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2);


LONG KiStackOutSwapRequest = FALSE;// Define swap request flag.


VOID KeBalanceSetManager(IN PVOID Context)
/*
Routine Description:
    This function is the startup code for the balance set manager.
    The balance set manager thread is created during system initialization and begins execution in this function.
Arguments:
...
*/
{
    ...
    KeSetPriorityThread(KeGetCurrentThread(), LOW_REALTIME_PRIORITY);// Raise the thread priority to the lowest realtime level.

    // Initialize the periodic timer, initialize the ready queue scan DPC, and set the periodic timer it to expire one period from now.
    ...
    KeSetTimerEx(&PeriodTimer, DueTime, PERIODIC_INTERVAL / (10 * 1000), &ScanDpc);

    // Compute the stack protect and scan period time based on the system size.
    ...
    do {
        ...
        {
        case TimerExpiration:// Periodic timer expiration.
#if !defined(NT_UP)
            KiAdjustIrpCredits();// Adjust I/O lookaside credits.
#endif
            ExAdjustLookasideDepth();// Adjust the depth of lookaside lists.
            MmWorkingSetManager();// Execute the virtual memory working set manager.
            ...
            break;
        case WorkingSetManagerEvent:// Working set manager event.
            MmWorkingSetManager();// Call the working set manager to trim working sets.
            break;
        default:// Illegal return status.
...
/*
Routine Description:
    This thread controls the swapping of processes and kernel stacks.
...
*/
{
    ...
    // Set address of swap thread object and raise the thread priority to the lowest realtime level + 7 (i.e., priority 23).
    ...
    do {
        ...
        // The following events are processed one after the other.
        // If another event of a particular type arrives after having processed the respective event type, 
        // them the swap event will have been set and the above wait will immediately be satisfied.
        // Check to determine if there is a kernel stack out swap scan request pending.
...
/*
Routine Description:
    This function adjusts the lookaside IRP float credits for two processors during each one second scan interval.
    IRP credits are adjusted by using a moving average of two processors.
    It is possible for the IRP credits for a processor to become negative, but this condition will be self correcting.
*/
{
    LONG Average;
    LONG Adjust;
    ...
    if (Number > 1) {
        // Compute the target average value by averaging the IRP credits across all processors.
        Index = 0;
        Average = 0;
        do {
...
*/
{
    ...
    while ((NextEntry != &Prcb->WaitListHead) && (NumberOfThreads < MAXIMUM_THREAD_STACKS)) {
        Thread = CONTAINING_RECORD(NextEntry, KTHREAD, WaitListEntry);
        ASSERT(Thread->WaitMode == UserMode);
        NextEntry = NextEntry->Flink;

        // Threads are inserted at the end of the wait list in very nearly reverse time order, 
        // i.e., the longest waiting thread is at the beginning of the list followed by the next oldest, etc.
        // Thus when a thread is encountered which still has a protected stack it is known that all further threads in the wait also have protected stacks, 
        // and therefore, the scan can be terminated.

        // N.B. It is possible due to a race condition in wait that a high priority thread was placed in the wait list.
        //      If this occurs, then the thread is removed from the wait list without swapping the stack.
    ...
    KiLastProcessor += 1;// Increment the last processor number.
    ...
    while (NumberOfThreads > 0) {
        ...
        // Wait until the context has been swapped for the thread and outswap the thread stack.
...
*/
{
    ...
    do {
        ...
        if (NextEntry != &Process->ReadyListHead) {
            ...
            // While the process was being outswapped there may have been one or more threads that attached to the process.
            // If the process ready list is not empty, then in swap the process.
            // Otherwise, mark the process as out of memory.
            KiLockDispatcherDatabase(&OldIrql);
            NextEntry = Process->ReadyListHead.Flink;
            if (NextEntry != &Process->ReadyListHead) {
...
Routine Description:
    ...
    N.B. This function is executed as a DPC from the periodic timer that drives the balance set manager.
Arguments:
...
*/
{
    ...
    if (Summary != 0) {
        do {
            ...
            if (Summary & PRIORITY_MASK(Index)) {
                ...
                do {
                    // If the thread has been waiting for an extended period, then boost the priority of the selected.
    ...
    // Release the PRCB lock, unlock the dispatcher database, and save the last ready queue index for the next scan.
    ...
    ScanIndex += 1;// Increment the processor number.
...
#if defined(_AMD64_)
NTSTATUS KeExpandKernelStackAndCallout(__in PEXPAND_STACK_CALLOUT Callout, __in_opt PVOID Parameter, __in SIZE_T Size)
/*
Routine Description:
    This function checks to determine if there is enough space in the current stack to execute the specified function.
    If enough space is not available, then the stack is expanded as necessary.
Arguments:
    Callout - Supplies the address of a function to call with an expanded stack.
    Parameter - Supplies a parameter to be passed to the callout function.
    Size - Supplies the size of the kernel stack needed to execute the callout function.
ReturnValue:
    If the stack allocation is successful and the callout is performed, then STATUS_SUCCESS is returned.
    Otherwise, STATUS_INVALID_PARAMETER_3 or STATUS_NO_MEMORY is returned.
*/
{
    ...
    // If the current IRQL is not less than or equal to APC_LEVEL, then bugcheck.
    if ((OldIrql = KeGetCurrentIrql()) > APC_LEVEL) {
        KeBugCheckEx(IRQL_NOT_LESS_OR_EQUAL, APC_LEVEL, OldIrql, 0, 0);
    }

    // If the stack request is for more than a large kernel stack, then return invalid parameter.
    if (Size > MAXIMUM_EXPANSION_SIZE) {
        return STATUS_INVALID_PARAMETER_3;
    }

    // If enough stack space is already available, then execute the callout function using the current stack.
    // Otherwise, allocate a new stack segment and execute the callout function using the new stack segment.

    // There are three cases to consider:
    //    1. There is committed space in the current stack for the specified allocation.
    //    2. There is uncommitted space in the current stack that can be committed for the specified allocation.
    //    3. There is not enough committed or uncommitted space in the current stack for the specified allocation.
    Thread = KeGetCurrentThread();
    ...
    if ((CurrentStack - ActualLimit) >= Size) {
        if ((CurrentStack - CurrentLimit) < Size) {
            // There is uncommitted space in the current stack that can be committed for the specified allocation.
    ...
    } else {// There is not enough committed or uncommitted space in the current stack for the specfied allocation.
        ...
        CalloutActive = Thread->CalloutActive;
        Thread->CalloutActive = TRUE;
        Status = KiSwitchKernelStackAndCallout(Parameter, Callout, LargeStack, Size);
