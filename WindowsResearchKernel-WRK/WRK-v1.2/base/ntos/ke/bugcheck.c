...
// Define max buffer size for the file - this value is based on the previous max value used in this file
...
VOID KiDisplayBlueScreen(IN ULONG PssMessage, IN BOOLEAN HardErrorCalled, IN PCHAR HardErrorCaption, IN PCHAR HardErrorMessage, IN PCHAR StateString);
...
LONG KeBugCheckCount = 1;// Define bugcheck recursion counter.
ULONG KeBugCheckOwnerRecursionCount;// Define an owner recursion counter is used to count the number of recursive bugchecks taken on the bugcheck owner processor.


// Define the bugcheck owner flag.
// This flag is used to track the processor that "owns" an active bugcheck operation by virtue of the fact that it decremented the bugcheck recursion counter to zero.
#if !defined(NT_UP)
...
BOOLEAN KeBugCheckActive = FALSE;// Define bugcheck active indicator.


#if !defined(_AMD64_)
VOID KeBugCheck(__in ULONG BugCheckCode)
{
    KeBugCheck2(BugCheckCode, 0, 0, 0, 0, NULL);
}


VOID KeBugCheckEx(__in ULONG BugCheckCode, __in ULONG_PTR P1, __in ULONG_PTR P2, __in ULONG_PTR P3, __in ULONG_PTR P4)
{
...
BOOLEAN KeGetBugMessageText(__in ULONG MessageId, __out_opt PANSI_STRING ReturnedString)
{
    SIZE_T  i;
    PCHAR   s;
...
VOID KiBugCheckDebugBreak(IN ULONG BreakStatus)
{
    do {
        try {
            DbgBreakPointWithStatus(BreakStatus);// Issue a breakpoint
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            ...
            // Failed to issue the breakpoint, must be no debugger.
            // Now, give the headless terminal a chance to reboot the system, if there is one.
...
Arguments:
    ...
    DataTableEntry - Supplies a pointer to a variable that receives the address of the data table entry that describes the image.
    DriversOnly - Supplies TRUE if the kernel and HAL should be skipped.
    InKernelOrHal - Set to TRUE if the PcValue is in the kernel or the HAL. This only has meaning if DriversOnly is FALSE.
Return Value:
    NULL - No image was found that contains the PcValue.
    NON-NULL - Returns the base address of the image that contains the PcValue.
*/
{
    ULONG i;
    ...
    // If the module list has been initialized, then scan the list to locate the appropriate entry.
...
*/
{
...
#if !defined(_AMD64_)
    // Save the current IRQL in the Prcb so the debugger can extract it later on for debugging purposes.
    ...
    // This is necessary on machines where the virtual unwind that happens during KeDumpMachineState() destroys the context record.
    ...
    case IRQL_NOT_LESS_OR_EQUAL:
        ...
        if (ExecutionAddress >= ExPoolCodeStart && ExecutionAddress < ExPoolCodeEnd) {
            ...
            if (InKernelOrHal == TRUE) {
                // The kernel faulted at raised IRQL.
                // Quite often this is a driver that has unloaded without deleting its lookaside lists or other resources.
                // Or its resources are marked pageable and shouldn't be.
                // Detect both cases here and identify the offending driver whenever possible.
    ...
    case KERNEL_MODE_EXCEPTION_NOT_HANDLED:
        ...
        // Extract the execution address from the trap frame to identify the component.
        if ((TrapFrame != NULL) && (BugCheckCode != KERNEL_MODE_EXCEPTION_NOT_HANDLED)) {
    ...
    case PAGE_FAULT_IN_NONPAGED_AREA:
        ...
        // Extract the execution address from the trap frame to identify component.
        // Note a passed trap frame takes precedence over the the third bugcheck parameter.
        if ((TrapFrame == NULL) && (BugCheckParameter3 != (ULONG_PTR)NULL)) {
            TrapFrame = (PKTRAP_FRAME)BugCheckParameter3;
        }
        ...
        if (TrapFrame != NULL) {
            ...
            InKernelOrHal = TRUE;// No trap frame, so no execution address either.
        }

        VirtualAddress = (PVOID)BugCheckParameter1;
        if (MmIsSpecialPoolAddress(VirtualAddress) == TRUE) {
            // Update the bugcheck number so the administrator gets useful feedback that enabling special pool has enabled the system to locate the corruptor.
    ...
    // If the user manually crashed the machine, skips the DbgPrints and go to the crashdump.
    // Trying to do DbgPrint causes us to reeeter the debugger which causes some problems.
    ...
    if (InterlockedDecrement(&KeBugCheckCount) == 0) {
        // Set the bugcheck owner flag so that we'll be able to distinguish between recursive bugchecks and bugchecks occurring on different processors.
#if !defined(NT_UP)
        KeBugCheckOwner = CurrentProcessor;

        // Attempt to get the other processors frozen now, but don't wait for them to freeze (in case someone is stuck).
        TargetSet = KeActiveProcessors & ~KeGetCurrentPrcb()->SetMember;
        if (TargetSet != 0) {
            KiSendFreeze(&TargetSet, FALSE);
            KeBugCheckActive = 2;// Indicate to KeFreezeExecution() that KiSendFreeze() must not wait for targets to be in a RUNNING state before transitioning to FREEZE.

            // Give the other processors one second to flush their data caches.
            // N.B. This cannot be synchronized since the reason for the bugcheck may be one of the other processors failed.
            KeStallExecutionProcessor(1000 * 1000);
        }
#endif

        KiDisplayBlueScreen(PssMessage, HardErrorCalled, HardErrorCaption, HardErrorMessage, AnsiBuffer);// Display the blue screen.
        KiInvokeBugCheckEntryCallbacks();// Invoke bugcheck callbacks.
        ...
        KeGetCurrentPrcb()->ProcessorState.ContextFrame = ContextSave;// Restore the original Context frame

        // For some bugchecks we want to change the thread and context before it is written to the dump file IFF it is a minidump.
        // Look at the original bugcheck data, not the processed data from above
#define MINIDUMP_BUGCHECK 0x10000000
        ...
        if (IoIsTriageDumpEnabled()) {
            ...
            if (TrapFrame != NULL) {
                ...
                ContextSave.ContextFlags = CONTEXT_CONTROL | CONTEXT_SEGMENTS | CONTEXT_INTEGER;
...
#endif
            {
                switch (BugCheckCode) {
                    ...
                    // use that.
                    // Also save the context record in case someone needs to look at it.
                case SYSTEM_THREAD_EXCEPTION_NOT_HANDLED:
...
#if defined (_X86_)
                case THREAD_STUCK_IN_DEVICE_DRIVER:
                    // Extract the address of the spinning code from the thread object, so the dump is based off this thread.
                    Thread = (PKTHREAD)BugCheckParameter1;
                    if (Thread->State == Running) {
                        // If the thread was running, the thread is now in a frozen state and the registers are in the PRCB context
                        ...
                        // This should be a uniproc machine, and the thread should be suspended.
                        // Just get the data off the switch frame.
                ...
                case UNEXPECTED_KERNEL_MODE_TRAP:
                    ...
                    if (BugCheckParameter1 == 0x8) {
                        ...
                        // Second parameter is the TSS.
                        // If we have a TSS, convert the context and mark the bugcheck as converted.
            ...
            // Check out if we need to save additional pages for special pool situations.
            {
                ULONG_PTR CrashCode;

                CrashCode = KiBugCheckData[0] & ~MINIDUMP_BUGCHECK;
                if (CrashCode == PAGE_FAULT_BEYOND_END_OF_ALLOCATION || CrashCode == DRIVER_PAGE_FAULT_BEYOND_END_OF_ALLOCATION) {
        ...
        // If we reach this point then the bugcheck recursion count had already been decremented by the time we decremented it above.
        // This means that we're either bugchecking on multiple processors simultaneously or that we took a recursive fault causing us to reenter the bugcheck routine.

        // In the multiprocessor kernel, start by waiting for the bugcheck owner (that is, the agent that decremented the bugcheck recursion count to 0) to move the bugcheck owner flag away from its initial distinguished value.
        // N.B. There is a very small window where the owner could take a recursive bugcheck after decrementing the bugcheck recursion counter and before setting the bugcheck owner flag.
        //      If we time out waiting for the flag to change, then assume we've hit this window and try to enter the kernel debugger.
#if !defined(NT_UP)
        WaitCount = 0;
        ...
        // Let the first recursive bugcheck fall through to the remainder of the bugcheck processing code.
        // If this generates a second recursive bugcheck, then try to halt the processor, breaking into the debugger if possible.
        // If this generates a third recursive bugcheck, then halt the processor here.
    ...
    // Invoke bugcheck callbacks after crashdump, so the callbacks will not prevent us from crashdumping.
    KiScanBugCheckCallbackList();
    ...
    if (Reboot) {
        ...
        // Unfreeze any frozen processors.  This is needed for the two functions below working properly.
    ...
    KiBugCheckDebugBreak(DBG_STATUS_BUGCHECK_SECOND);// Attempt to enter the kernel debugger.
}
...
/*
Routine Description:
    This function crashes the system in a controlled manner attempting to invoke the kernel debugger.
*/
{
#if !defined(i386)
    KIRQL OldIrql;
...
*/
{
    ...
    // Raise IRQL to HIGH_LEVEL and acquire the bugcheck callback list spinlock.
    KeRaiseIrql(HIGH_LEVEL, &OldIrql);
    KiAcquireSpinLock(&KeBugCheckCallbackLock);

    // If the specified callback record is currently registered, then deregister the callback record.
    Deregister = FALSE;
...
/*
Routine Description:
    This function registers a bugcheck callback record.
    If the system crashes, then the specified function will be called during bugcheck processing so it may dump additional state in the specified bugcheck buffer.
    N.B. Bugcheck callback routines are called in reverse order of registration, i.e., in LIFO order.
Arguments:
    ...
    Component - Supplies a pointer to a zero terminated component identifier.
Return Value:
    If the specified bugcheck callback record is successfully registered, then a value of TRUE is returned.
...
*/
{
    ...
    // If the specified callback record is currently not registered, then register the callback record.
    Inserted = FALSE;
        ...
        while (NextEntry != ListHead) {
            ...
            if (((ULONG_PTR)NextEntry & (sizeof(ULONG_PTR) - 1)) != 0) {
                ...
                if ((CallbackRecord->State == BufferInserted) && (CallbackRecord->Checksum == Checksum)) {
                    // Call the specified bugcheck callback routine and handle any exceptions that occur.
...
Arguments:
    CallbackRecord - Supplies a pointer to a callback record.
    CallbackRoutine - Supplies a pointer to the callback routine.
    Reason - Specifies the conditions under which the callback should be called.
    Component - Supplies a pointer to a zero terminated component identifier.
Return Value:
    If the specified bugcheck callback record is successfully registered, then a value of TRUE is returned. Otherwise, a value of FALSE is returned.
*/
{
    BOOLEAN Inserted;
    KIRQL OldIrql;
...
/*
Routine Description:
    This function scans the bugcheck reason callback list and calls each bugcheck entry callback routine.
