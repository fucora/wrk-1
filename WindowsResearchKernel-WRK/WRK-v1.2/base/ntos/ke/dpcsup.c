...
Abstract:
    ...
    Functions are provided to process quantum end, the power notification queue, and timer expiration.
...
// Define maximum number of timers that can be examined or processed before dropping the dispatcher database lock.
...
*/
{
    ...
    do {
        ...
        // If the thread DPC list is empty, then wait until the DPC event for the current processor is set.
        if (Prcb->DpcData[DPC_THREADED].DpcQueueDepth == 0) {
            KeWaitForSingleObject(&Prcb->DpcEvent, Suspended, KernelMode, FALSE, NULL);
        }
    } while (TRUE);
}
...
*/
{
    ...
    // Raise IRQL to SYNCH level, acquire the thread lock, and acquire the PRCB lock.

    // If the quantum has expired for the current thread, then update its quantum and priority.
    ...
    if (Thread->Quantum <= 0) {
        ...
        if ((Process->DisableQuantum != FALSE) && (Thread->Priority >= LOW_REALTIME_PRIORITY)) {
            ...
            // Compute the new thread priority and attempt to reschedule the current processor.
...
VOID KiCheckTimerTable(IN ULARGE_INTEGER CurrentTime)
{
    ...
    // Raise IRQL to highest level and scan timer table for timers that have expired.
    KeRaiseIrql(HIGH_LEVEL, &OldIrql);
    Index = 0;
    do {
    ...
    KeLowerIrql(OldIrql);// Lower IRQL to the previous level.
}
#endif


FORCEINLINE VOID KiProcessTimerDpcTable(IN PULARGE_INTEGER SystemTime, IN PDPC_ENTRY DpcTable, IN ULONG Count)
/**++
Routine Description:
    This function processes the time DPC table which is a array of DPCs that are to be called on the current processor.
...
*/
{
    ...
    KiUnlockDispatcherDatabase(DISPATCH_LEVEL);// Unlock the dispatcher database and lower IRQL to dispatch level.
...
/*
Routine Description:
    This function is called when the clock interupt routine discovers that a timer has expired.
    N.B. This function executes on the same processor that receives clock interrupts.
Arguments:
    TimerDpc - Not used.
    DeferredContext - Not used.
    SystemArgument1 - Supplies the starting timer table index value to use for the timer table scan.
...
*/
{
    ...
    // Capture the timer expiration time, the current interrupt time, and the low tick count.

    // N.B. Interrupts are disabled to ensure that interrupt activity on the current processor does not cause the values read to be skewed.
    _disable();
    ...
    // N.B. This later condition exists when DPC processing is blocked for a period longer than one round trip throught the timer table.
    // N.B. The current instance of the timer expiration execution will only process the timer queue entries specified by the computed index and hand limit.
    //      If another timer expires while the current scan is in progress, then another scan will occur when the current one is finished.
    ...
    // Acquire the dispatcher database lock and read the current interrupt time to determine which timers have expired.
    DpcCount = 0;
    ...
    do {
        ...
        while (ListHead != ListHead->Flink) {
            ...
            if ((NextEntry != ListHead) &&
                ...
                // Remove the entry from the timer tree and set the signal state of the timer.
                TimersProcessed -= 1;
                ...
                // Once wait test is called, the timer must not be touched again unless it is periodic.
                // The reason for this is that a thread may allocate a timer on its local stack and wait on it.
                // Wait test can cause that thread to immediately start running on another processor on an MP system.
                ...
                if ((TimersProcessed == 0) || (TimersExamined == 0)) {
                    ...
                    // Initialize the DPC count, the scan counters, and acquire the dispatcher database lock.
                ...
                // If the timer table list is not empty, then set the due time to the first entry in the respective timer table entry.
                // N.B. On the x86 the write of the due time is not atomic.
                //      Therefore, interrupts must be disabled to synchronize with the clock interrupt so the clock interupt code does not observe a partial update of the due time.
                ...
                // If the maximum number of timers have been scanned, then drop the dispatcher lock and process the DPC table.
                if (TimersExamined == 0) {
                    KiProcessTimerDpcTable(&SystemTime, &DpcTable[0], DpcCount);
                    // Initialize the DPC count, the scan counters, and acquire the dispatcher database lock.
                    // N.B. Control is returned with the dispatcher database unlocked.
...
Routine Description:
    ...
    N.B. This function is called with the dispatcher database locked and returns with the dispatcher database unlocked.
Arguments:
    ExpiredListHead - Supplies a pointer to a list of timers that have expired.
