...
Abstract:
    This module implements the kernel DPC object.
    Functions are provided to initialize, insert, and remove DPC objects.
*/
...
FORCEINLINE PKDPC_DATA KiSelectDpcData(IN PKPRCB Prcb, IN PKDPC Dpc)
/*
Routine Description:
    This function selects the appropriate DPC data structure in the specified processor control block based on the type of DPC and whether threaded DPCs are enabled.
Arguments:
...
*/
{
    // If the DPC is a threaded DPC and thread DPCs are enabled, then set the address of the threaded DPC data. 
    // Otherwise, set the address of the normal DPC structure.
    if ((Dpc->Type == (UCHAR)ThreadedDpcObject) && (Prcb->ThreadDpcEnable != FALSE)) {
...
VOID KiInitializeDpc(IN PRKDPC Dpc, IN PKDEFERRED_ROUTINE DeferredRoutine, IN PVOID DeferredContext, IN KOBJECTS DpcType)
/*
Routine Description:
    This function initializes a kernel DPC object.
    The deferred routine, context parameter, and object type are stored in the DPC object.
Arguments:
    Dpc - Supplies a pointer to a control object of type DPC.
    DeferredRoutine - Supplies a pointer to a function that is called when the DPC object is removed from the current processor's DPC queue.
    DeferredContext - Supplies a pointer to an arbitrary data structure which is to be passed to the function specified by the DeferredRoutine parameter.
...
VOID KeInitializeThreadedDpc(__out PRKDPC Dpc, __in PKDEFERRED_ROUTINE DeferredRoutine, __in_opt PVOID DeferredContext)
/*
Routine Description:
...
BOOLEAN KeInsertQueueDpc(__inout PRKDPC Dpc, __in_opt PVOID SystemArgument1, __in_opt PVOID SystemArgument2)
/*
Routine Description:
    This function inserts a DPC object into the DPC queue.
    If the DPC object is already in the DPC queue, then no operation is performed.
    Otherwise, the DPC object is inserted in the DPC queue and a dispatch interrupt is requested.
Arguments:
    Dpc - Supplies a pointer to a control object of type DPC.
    SystemArgument1, SystemArgument2  - Supply a set of two arguments that contain untyped data provided by the executive.
Return Value:
    If the DPC object is already in a DPC queue, then a value of FALSE is returned.
...
*/
{
    ...
    // Disable interrupts and acquire the DPC queue lock for the specified target processor.
    // N.B. Disable interrupt cannot be used here since it causes the software interrupt request code to get confused on some platforms.
    ...
    // If the DPC object is not in a DPC queue, then store the system arguments, 
    // insert the DPC object in the DPC queue, increment the number of DPCs queued to the target processor,
    // increment the DPC queue depth, set the address of the DPC target DPC spinlock, and request a dispatch interrupt if appropriate.
    Inserted = FALSE;
    if (InterlockedCompareExchangePointer(&Dpc->DpcData, DpcData, NULL) == NULL) {
        ...
        if (DpcData == &TargetPrcb->DpcData[DPC_THREADED]) {
            ...
            if ((TargetPrcb->DpcThreadActive == FALSE) && (TargetPrcb->DpcThreadRequested == FALSE)) {
                ...
                if (CurrentPrcb != TargetPrcb) {
                    if (((KiIdleSummary & AFFINITY_MASK(Number)) == 0) || (KeIsIdleHaltSet(TargetPrcb, Number) != FALSE)) {
                        RequestInterrupt = TRUE;
                    }
                } else {
                    RequestInterrupt = TRUE;
            ...
            // If a DPC routine is not active on the target processor and an interrupt has not been requested, 
            // then request a dispatch interrupt on the target processor if appropriate.
            if ((TargetPrcb->DpcRoutineActive == FALSE) && (TargetPrcb->DpcInterruptRequested == FALSE)) {
                // Request a dispatch interrupt on the current processor if the DPC is not of low importance, the length of the DPC
                // queue has exceeded the maximum threshold, or if the DPC request rate is below the minimum threshold.
#if defined(NT_UP)
...
BOOLEAN KeRemoveQueueDpc(__inout PRKDPC Dpc)
/*
Routine Description:
    This function removes a DPC object from the DPC queue.
    If the DPC object is not in the DPC queue, then no operation is performed.
    Otherwise, the DPC object is removed from the DPC queue and its inserted state is set FALSE.
Arguments:
    Dpc - Supplies a pointer to a control object of type DPC.
Return Value:
    If the DPC object is not in the DPC queue, then a value of FALSE is returned.
    Otherwise a value of TRUE is returned.
*/
{
    PKDPC_DATA DpcData;
    ...
    // If the DPC object is in the DPC queue, then remove it from the queue and set its inserted state to FALSE.
    Enable = KeDisableInterrupts();
    DpcData = Dpc->DpcData;
    if (DpcData != NULL) {
#if !defined(NT_UP)
        KiAcquireSpinLock(&DpcData->DpcLock);// Acquire the DPC lock of the target processor.
#endif
...
#if !defined(NT_UP)
        KiReleaseSpinLock(&DpcData->DpcLock);// Release the DPC lock of the target processor.
#endif
    }

    KeEnableInterrupts(Enable);// Enable interrupts and return whether the DPC was removed from a DPC queue.
...
Routine Description:
    ...
    This function is used at driver unload to make sure all driver DPC processing has exited the driver image before the code and data is deleted.
*/
{
#if !defined(NT_UP)
...
#if defined(NT_UP)
    CurrentPrcb = KeGetCurrentPrcb();
    if ((CurrentPrcb->DpcData[DPC_NORMAL].DpcQueueDepth > 0) || (CurrentPrcb->DpcData[DPC_THREADED].DpcQueueDepth > 0)) {
        KiRequestSoftwareInterrupt(DISPATCH_LEVEL);
    }
#else
    ...
    // Set the priority of this thread high so it will run quickly on the target processor.
    ...
    // Clear the current processor from the affinity set and switch to the remaining processors in the affinity set so it can be guaranteed that respective DPCs have been completed processed.
    while (1) {
        CurrentPrcb = KeGetCurrentPrcb();
        ...
        if ((SentDpcMask & CurrentProcessorMask) == 0 && (CurrentPrcb->DpcData[DPC_NORMAL].DpcQueueDepth > 0) || (CurrentPrcb->DpcData[DPC_THREADED].DpcQueueDepth > 0)) {
            ...
            // If processors have not been swapped, then the DPCs must have run to completion. 
            // If processors have been swapped, then just repeat for the current processor.
    ...
    // Restore the affinity of the current thread if it was changed and restore the original thread priority.
    if (SetAffinity) {
...
/*
Routine Description:
    This function acquires the DPC call lock,
    initializes a processor specific DPC for each process with the specified deferred routine and context,
    and queues the DPC for execution.
    When all DPCs routines have executed, the DPC call lock is released.
Arguments:
...
*/
{
    ...
    ExAcquireFastMutex(&KiGenericCallDpcMutex);// Acquire generic call DPC mutex.
#endif

    KeRaiseIrql(DISPATCH_LEVEL, &OldIrql);

    // Loop through all the target processors, initialize the deferred routine address, 
    // context parameter, barrier parameter, and queue the DPC to the target processor.
#if !defined(NT_UP)
    Number = KeGetCurrentProcessorNumber();
    do {
        // If the target processor is not the current processor, then initialize and queue the generic call DPC.
        if (Number != Index) {
    ...
    (Routine)(&KeGetCurrentPrcb()->CallDpc, Context, &Barrier, &ReverseBarrier);// Call deferred routine on current processor.
    ...
    KeLowerIrql(OldIrql);// Release generic all DPC mutex and lower IRQL to previous level.
...
/*
Routine Description:
    This function decrements the generic DPC call barrier whose address was passed to the deferred DPC function as the first system argument.
Arguments:
    SystemArgument1 - Supplies the address of the call barrier.
    N.B. This must be the system argument value that is passed to the target deferred routine.
*/
{
...
/*
Routine Description:
    This function decrements the generic DPC call reverse barrier whose address was passed to the deferred DPC function as the second system argument.
Arguments:
    SystemArgument2 - Supplies the address of the call barrier.
    N.B. This must be the second system argument value that is passed to the target deferred routine.
Return Value:
    LOGICAL - Tie breaker value. One processor receives the value TRUE, all others receive FALSE.
*/
{
#if !defined(NT_UP)
    ...
    // Signal other processors that the synchronization has occurred. If this is the last processor, then reset the barrier.
