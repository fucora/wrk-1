...
/*
Routine Description:
    This function is called to copy the specified context record when the previous mode is user.
    Its only purpose is to save stack space in the caller.
...
*/
{
    ...
    if (OldIrql < APC_LEVEL) {// If the current IRQL is less than APC_LEVEL, then raise IRQL to APC level.
        KfRaiseIrql(APC_LEVEL);
    }

    Status = STATUS_SUCCESS;
    if (KeGetPreviousMode() != KernelMode) {// If the previous mode was not kernel mode, then use wrapper function to copy context to kernel frames.
        try {
    ...
    } else {// Otherwise, copy context to kernel frames directly.
        KeContextToKframes(TrapFrame, ExceptionFrame, ContextRecord, ContextRecord->ContextFlags, KernelMode);
    }

    if (OldIrql < APC_LEVEL) {// If the old IRQL was less than APC level, then lower the IRQL to its previous value.
        KeLowerIrql(OldIrql);
    }
    ...
    */
{
    ...
    PreviousMode = KeGetPreviousMode();// Get the previous processor mode and probe the specified exception and context records if necessary.
    if (PreviousMode != KernelMode) {
        try {
            ProbeForReadSmallStructure(ContextRecord, sizeof(CONTEXT), sizeof(UCHAR));
            Parameters = ProbeAndReadUlong(&ExceptionRecord->NumberParameters);
            if (Parameters > EXCEPTION_MAXIMUM_PARAMETERS) {
                return STATUS_INVALID_PARAMETER;
            }

            // The exception record structure is defined unlike others with trailing information as being its maximum size rather than just a single trailing element.
            Length = FIELD_OFFSET(EXCEPTION_RECORD, ExceptionInformation[Parameters]);
            __assume(Length != 0);
            ProbeForRead(ExceptionRecord, Length, sizeof(UCHAR));

            // Copy the exception and context record to local storage so an access violation cannot occur during exception dispatching.
            RtlCopyMemory(&ContextRecord2, ContextRecord, sizeof(CONTEXT));
            RtlCopyMemory(&ExceptionRecord2, ExceptionRecord, Length);
            ContextRecord = &ContextRecord2;
            ExceptionRecord = &ExceptionRecord2;
            ExceptionRecord->NumberParameters = Parameters; // Make sure the number of parameter is correct in the copied exception record.
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
    ...
    KeContextToKframes(TrapFrame, ExceptionFrame, ContextRecord, ContextRecord->ContextFlags, PreviousMode);// Move information from the context record to the exception and trap frames.

    // Make sure the reserved bit is clear in the exception code and perform exception dispatching.
    // N.B. The reserved bit is used to differentiate internally generated codes from codes generated by application programs.
