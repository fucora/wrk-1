...
Abstract:
   This module contains the variables used to implement the run-time reference monitor database.
*/
...
//  Resource Locks  - These locks protect access to the modifiable fields of the reference monitor database.
//                    There is one lock for a set of hash buckets.
ERESOURCE SepRmDbLock[SEP_LOGON_TRACK_LOCK_ARRAY_SIZE] = {0};


//  Resource Lock  - This mutex protects access to the file systems notify routines list
...
SEP_RM_STATE SepRmState = {0};// State of the reference monitor



// The following array is used as a hash bucket for tracking logon sessions.
// The sequence number of logon LUIDs is ANDed with 0x0F and then used as an index into this array.
// This entry in the array serves as a listhead of logon session reference count records.
...
*/
{
    ...
    SepLogonSessions = ExAllocatePoolWithTag( PagedPool, sizeof( PSEP_LOGON_SESSION_REFERENCES ) * SEP_LOGON_TRACK_ARRAY_SIZE, 'SLeS');
    ...
    // The correct RM state will be set when the local security policy information is retrieved (by the LSA) and subsequently passed to the reference monitor later on in initialization.
    // For now, initialize the state to something that will work for the remainder of system initialization.
