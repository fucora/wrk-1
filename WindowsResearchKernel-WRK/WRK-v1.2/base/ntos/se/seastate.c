...
// The PRIVILEGE_SET data structure includes an array including ANYSIZE_ARRAY elements.
// This definition provides the size of an empty PRIVILEGE_SET (i.e., one with no privileges in it).
...
Routine Description:
    ...
    - mapping generic access types in the passed DesiredAccess and putting it into the structure
    - "capturing" the Subject Context, which must be held for the duration of the access attempt (at least until auditing is performed).
    - Allocating an Operation ID, which is an LUID that will be used to associate different parts of the access attempt in the audit log.
Arguments:
    AccessState - a pointer to the structure to be initialized.
    AuxData - Supplies a buffer big enough for an AuxData structure so we don't have to allocate one.
    DesiredAccess - Access mask containing the desired access
    GenericMapping - Optionally supplies a pointer to a generic mapping that may be used to map any generic access requests that may have been passed in the DesiredAccess parameter.
        Note that if this parameter is not supplied, it must be filled in at some later point.  The IO system does this in IopParseDevice.
Return Value:
    Error if the attempt to allocate an LUID fails.

    Note that this error may be safely ignored if it is known that all security checks will be performed with PreviousMode == KernelMode.
    Know what you're doing if you choose to ignore this.
*/
{
    return SeCreateAccessStateEx(PsGetCurrentThread(), PsGetCurrentProcess(), AccessState, AuxData, DesiredAccess, GenericMapping);
}
...
Routine Description:
    ...
    - Allocating an Operation ID, which is an LUID that will be used to associate different parts of the access attempt in the audit log.
Arguments:
    Thread - Optional thread to capture impersonation token from. If NULL no impersonation token is captured.
...
*/
{
    ...
    MappedAccessMask = DesiredAccess;// Don't modify what he passed in

    // Map generic access to object specific access iff generic access types are specified and a generic access mapping table is provided.
...
VOID SeDeleteAccessState(__in PACCESS_STATE AccessState)
/*
Routine Description:
    This routine deallocates any memory that may have been allocated as part of constructing the access state
    (normally only for an excessive number of privileges),
    and frees the Subject Context.
Arguments:
    AccessState - a pointer to the ACCESS_STATE structure to be deallocated.
*/
{
...
Routine Description:
    ...
    It must be called before access validation is performed if the GenericMapping is not passed in when the AccessState structure is created.
Arguments:
...
NTSTATUS SeAppendPrivileges(__inout PACCESS_STATE AccessState, __in PPRIVILEGE_SET Privileges)
/*
Routine Description:
...
Return Value:
    STATUS_INSUFFICIENT_RESOURCES - an attempt to allocate pool memory failed.
*/
{
    ...
    if (Privileges->PrivilegeCount + AuxData->PrivilegesUsed->PrivilegeCount > INITIAL_PRIVILEGE_COUNT) {
        ...
        // Note that this will adjust the privilege count in the structure for us.
        ...
        AccessState->PrivilegesAllocated = TRUE;// Mark that we've allocated memory for the privilege set, so we know to free it when we're cleaning up.
    } else {
        // Note that this will adjust the privilege count in the structure for us.
...
Routine Description:
    ...
    There must be enough space left at the end of the first privilege set to contain the second.
Arguments:
    TargetPrivilegeSet - Supplies a buffer containing a privilege set. The buffer must be large enough to contain the second privilege set.
    TargetBufferSize - Supplies the size of the target buffer.
    SourcePrivilegeSet - Supplies the privilege set to be copied into the target buffer.
*/
{
    PVOID Base;
    PVOID Source;
    ...
    ASSERT(((ULONG)SepPrivilegeSetSize(TargetPrivilegeSet) + (ULONG)SepPrivilegeSetSize(SourcePrivilegeSet) - SEP_PRIVILEGE_SET_HEADER_SIZE) <= TargetBufferSize);
