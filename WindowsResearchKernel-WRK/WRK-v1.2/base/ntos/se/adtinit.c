...
*/
{
    ...
    if (!NT_SUCCESS(Status)) {
        return;// Didn't work, take the defaults
    }
...
*/
{
    ...
    Status = SepRegQueryDwordValue(L"\\Registry\\Machine\\System\\CurrentControlSet\\Control\\Lsa", CRASH_ON_AUDIT_FAIL_VALUE, &CrashOnAuditFail);
    if (Status == STATUS_OBJECT_NAME_NOT_FOUND) {// If the key isn't there, don't turn on CrashOnFail.
...
*/
{
    ...
    SepAuditOptions.DoNotAuditCloseObjectEvents = FALSE;// initialize the default value

    // if the value is present and set to 1, set the global auditing option accordingly
    Status = SepRegQueryDwordValue(L"\\Registry\\Machine\\System\\CurrentControlSet\\Control\\Lsa\\AuditingOptions", 
