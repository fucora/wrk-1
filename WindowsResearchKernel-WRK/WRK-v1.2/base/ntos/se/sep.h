...
#define IF_SE_GLOBAL( FlagName )     if (SeGlobalFlag & (SE_DIAG_##FlagName))// Test for enabled diagnostic
...
#define IF_SE_GLOBAL( FlagName ) if (FALSE)// Test for diagnostics enabled
#define SeDiagPrint( FlagName, _Text_ )     ;// Diagnostics print statement (expands to no-op)
#endif // SE_DIAGNOSTICS_ENABLED

// The following flags enable or disable various diagnostic capabilities within SE code.
// These flags are set in SeGlobalFlag (only available within a DBG system).
//      SD_TRACKING - Display information about create/deletion of shared security descriptors
#define SE_DIAG_SD_TRACKING          ((ULONG) 0x00000001L)

// Control flag manipulation macros

//  Macro to query whether or not control flags ALL on or not (ie, returns FALSE if any of the flags are not set)
...
// BOOLEAN SepBadImpersonationLevel(IN SECURITY_IMPERSONATION_LEVEL ImpersonationLevel, IN BOOLEAN ServerIsRemote)
// Routine Description:
//     Determine whether a client is trying to impersonate inappropriately
//     This routine should only be called if a thread requesting impersonation is itself already impersonating a client of its own.
//     This routine indicates whether the client is attempting to violate the level of impersonation granted to it by its client.
// Arguments:
//     ImpersonationLevel - Is the impersonation level of the client's effective token.
//     ServerIsRemote - Is a boolean flag indicating whether the client is requesting impersonation services to a remote system.
//         TRUE indicates the session is a remote session, FALSE indicates the session is a local session.
//         Delegation level is necessary to achieve a remote session.
// Return Value:
//     TRUE - Indicates that the impersonation level of the client's client token is inappropriate for the attempted impersonation.
...
#define IsValidParameterCount(p)            (IsInRange((p), 2, SE_MAX_AUDIT_PARAMETERS))
...
#define SepLockLsaQueue()  KeEnterCriticalRegion();                                            ExAcquireResourceExclusiveLite(&SepLsaQueueLock, TRUE)
#define SepUnlockLsaQueue() ExReleaseResourceLite(&SepLsaQueueLock);                           KeLeaveCriticalRegion()
...
typedef struct _SEP_LSA_WORK_ITEM {
    ...
    SEP_RM_LSA_MEMORY_TYPE          CommandParamsMemoryType;// Command Params Memory type
    SEP_LSA_WORK_ITEM_TAG           Tag;// Tag describing what kind of structure we've got

    // The following union contains the data to be passed to LSA.
    union {
        PVOID                       BaseAddress;
        LUID                        LogonId;
    } CommandParams;
    ...
    // CleanupFunction (if specified) will be called with CleanupParameter as its argument before the SEP_LSA_WORK_ITEM is freed by SepRmCallLsa
...
// Each logon session active in the system has a corresponding record of the following type...
