...
Abstract:
    This module implements the SET function for the executive token object.
*/
...
Arguments:
    ...
    TokenInformation - The buffer containing the new values for the specified class of information.
        ...
        TokenInformation Format By Information Class:
           ...
           TokenGroups => This value is not a valid value for this API.
           The Group IDs may not be replaced.
           However, groups may be enabled and disabled using NtAdjustGroupsToken().

           TokenPrivileges => This value is not a valid value for this API.
           Privilege information may not be replaced.
           However, privileges may be explicitly enabled and disabled using the NtAdjustPrivilegesToken API.

           TokenOwner => TOKEN_OWNER data structure.
           TOKEN_ADJUST_DEFAULT access is needed to replace this information in a token.
           The owner values that may be specified are restricted to the user and group IDs with an attribute indicating they may be assigned as the owner of objects.

           TokenPrimaryGroup => TOKEN_PRIMARY_GROUP data structure.
           TOKEN_ADJUST_DEFAULT access is needed to replace this information in a token.
           The primary group values that may be specified are restricted to be one of the group IDs already in the token.

           TokenDefaultDacl => TOKEN_DEFAULT_DACL data structure.
           TOKEN_ADJUST_DEFAULT access is needed to replace this information in a token.
           The ACL provided as a new default discretionary ACL is not validated for structural correctness or consistency.

           TokenSource => This value is not a valid value for this API.
           The source name and context handle  may not be replaced.

           TokenStatistics => This value is not a valid value for this
           API.  The statistics of a token are read-only.

           TokenSessionId => ULONG to set the token session.  Must have TOKEN_ADJUST_SESSIONID and TCB privilege.

           TokenSessionReference => ULONG.  Must be zero.
           Must have TCB privilege to dereference the logon session.
           This info class will remove a reference for the logon session, and mark the token as not referencing the session.

           TokenAuditPolicy => TOKEN_AUDIT_POLICY structure.  This sets the per
           user policy for the token, and all tokens derived from it.  Requires TCB privilege.
    ...
    TokenInformationLength - Indicates the length, in bytes, of the TokenInformation buffer.
        This is only the length of the primary buffer.
        All extensions of the primary buffer are self describing.

Return Value:
    STATUS_SUCCESS - The operation was successful.
    STATUS_INVALID_OWNER - The ID specified to be an owner (or default owner) is not one the caller may assign as the owner of an object.
    STATUS_INVALID_INFO_CLASS - The specified information class is not one that may be specified in this API.
    STATUS_ALLOTTED_SPACE_EXCEEDED - The space allotted for storage
        of the default discretionary access control and the primary group ID is not large enough to accept the new value of one of these fields.
*/
{
    ...
    if (PreviousMode != KernelMode) {
        try {
            ...
            // Any information class-specific data hung off the primary buffer are self describing and must be probed separately below.
    ...
    case TokenOwner:
        //  Make sure the buffer is large enough to hold the necessary information class data structure.
        ...
        SepAcquireTokenWriteLock(Token);//  Gain write access to the token.
        ...
        //  If it can, then set the index in the token's OwnerIndex field.
    ...
    case TokenPrimaryGroup:
        // Assuming everything works out, the strategy is to move everything in the Dynamic part of the token (exept the primary group) to
        // the beginning of the dynamic part, freeing up the entire end of the dynamic part for the new primary group.
        ...
        // See if there is enough room in the dynamic part of the token to replace the current Primary Group with the one specified.
    ...
    case TokenDefaultDacl:
        // Assuming everything works out, the strategy is to move everything in the Dynamic part of the token (exept the default Dacl) to
        // the beginning of the dynamic part, freeing up the entire end of the dynamic part for the new default Dacl.
        ...
        try {
            ...
            if (ARGUMENT_PRESENT(CapturedDefaultDacl)) {
                Status = SeCaptureAcl(CapturedDefaultDacl, PreviousMode, NULL, 0, PagedPool, TRUE, &CapturedDefaultDacl, &NewLength);
            } else {
                NewLength = 0;
                Status = STATUS_SUCCESS;
            }
        ...
        // See if there is enough room in the dynamic part of the token to replace the current Default Dacl with the one specified.
    ...
    case TokenSessionId:
    {
        ...
        SeSetSessionIdToken((PACCESS_TOKEN)Token, SessionId);// Set SessionId for the token
        ObDereferenceObject(Token);
        return(STATUS_SUCCESS);
    }
    ...
    case TokenAuditPolicy:
    {
        ...
        // If the policy was already set on this token then fail.  We only allow setting a token's policy once.
        ...
        try {
            Status = SeCaptureAuditPolicy(pAuditPolicy, PreviousMode, NULL, 0, PagedPool, TRUE, &pCapturedAuditPolicy);
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            ObDereferenceObject(Token);
            SeReleaseAuditPolicy(pCapturedAuditPolicy, PreviousMode, TRUE);
            return GetExceptionCode();
        }
    ...
    case TokenOrigin:
    {
        ...
        try {
            RtlCopyMemory(&Origin, TokenInformation, sizeof(TOKEN_ORIGIN));
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            ObDereferenceObject(Token);
            return GetExceptionCode();
...
VOID SepModifyTokenPolicyCounter(__in PSEP_AUDIT_POLICY TokenPolicy, __in BOOLEAN bIncrement)
/**
Routine Description:
    This modifies the global SepTokenPolicyCounter hint which records the number of tokens in the system with per user auditing settings.
Arguments:
    TokenPolicy - the policy which should be reflected in the hint.
    bIncrement - boolean indicating if this policy is being added (TRUE) or deleted (FALSE) from the counters.
Return Value:
    None.
*/
{
...
VOID SepFreePrimaryGroup(__in PTOKEN Token)
/*
Routine Description:
    Free up the space in the dynamic part of the token take up by the primary group.
    The token is assumed to be locked for write access before calling this routine.
Arguments:
...
*/
{
    ...
    // beginning of the dynamic part, move it there (remember to update the pointer to it).
...
VOID SepFreeDefaultDacl(__in PTOKEN Token)
/*
Routine Description:
    Free up the space in the dynamic part of the token take up by the default discretionary access control list.
    The token is assumed to be locked for write access before calling this routine.
Arguments:
    Token - Pointer to the token.
Return Value:
...
*/
{
    ...
    // Add the size of the Default Dacl (if there is one) to the DynamicAvailable field.
    if (ARGUMENT_PRESENT(Token->DefaultDacl)) {
...
VOID SepAppendPrimaryGroup(__in PTOKEN Token, __in PSID PSid)
/*
Routine Description:
    Add a primary group SID to the available space at the end of the dynamic part of the token.
    It is the caller's responsibility to ensure that the primary group SID fits within the available space of the dynamic part of the token.
    The token is assumed to be locked for write access before calling this routine.
Arguments:
    Token - Pointer to the token.
...
*/
{
    ...
    // Add the size of the Default Dacl (if there is one) to the address of the Dynamic Part of the token to establish where the primary group should be placed.
    if (ARGUMENT_PRESENT(Token->DefaultDacl)) {
...
VOID SepAppendDefaultDacl(__in PTOKEN Token, __in PACL PAcl)
/*
Routine Description:
    Add a default discretionary ACL to the available space at the end of the dynamic part of the token.
    It is the caller's responsibility to ensure that the default Dacl fits within the available space of the dynamic part of the token.
    The token is assumed to be locked for write access before calling this routine.
Arguments:
    Token - Pointer to the token.
...
*/
{
    ...
    // Add the size of the primary group to the address of the Dynamic Part of the token to establish where the primary group should be placed.
...
NTSTATUS SeSetSessionIdToken(__in PACCESS_TOKEN Token, __in ULONG SessionId)
/*
Routine Description:
