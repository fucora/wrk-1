...
Arguments:
    ...
    DesiredAccess - Is an access mask indicating which access types are desired to the newly created token.
        If specified as zero, the granted access mask of the existing token handle is used as the desired access mask for the new token.
...
*/
{
    ...
    if (PreviousMode != KernelMode) {
        try {
            ...
            ProbeForWriteHandle(NewTokenHandle);//  Make sure we can write the handle
    ...
    //  a pointer to that token.  Pick up the default desired access mask from the handle while we're at it.
    ...
    //  If no impersonation level was specified, pick one up from the source token.
    if (!SecurityQosPresent) {
        SecurityQos.ImpersonationLevel = Token->ImpersonationLevel;
    }

    // If an impersonation token is duplicated into an impersonation token, we need to do checks on the impersonation level.
    // The impersonation level requested for the new token is ignored if the new token type is TokenPrimary.
    if ((Token->TokenType == TokenImpersonation) && (TokenType == TokenImpersonation)) {
        // Make sure a legitimate transformation is being requested:
        //    (1) The impersonation level of a target duplicate must not exceed that of the source token.
...
/*
Routine Description:
    This routine does the bulk of the work to actually duplicate a token.
    This routine assumes all access validation and argument probing (except the ObjectAttributes) has been performed.

    THE CALLER IS RESPONSIBLE FOR CHECKING SUBJECT RIGHTS TO CREATE THE TYPE OF TOKEN BEING CREATED.
...
Arguments:
    ...
    ObjectAttributes - Points to the standard object attributes data structure.
        ...
        This information must be specified using parameters to this routine.

    EffectiveOnly - Is a boolean flag indicating whether the entire source token should be duplicated into the target token or
        just the effective (currently enabled) part of the token.
        This provides a means for a caller of a protected subsystem to limit which privileges and optional groups are made available to the protected subsystem.
        A value of TRUE indicates only the currently enabled parts of the source token are to be duplicated.
        Otherwise, the entire source token is duplicated.

    TokenType - Indicates the type of token to make the duplicate token.

    ImpersonationLevel - This value specifies the impersonation level to assign to the duplicate token.
        If the TokenType of the duplicate is not TokenImpersonation then this parameter is ignored.
        Otherwise, it is must be provided.
...
*/
{
    ...
    SepAcquireTokenReadLock(ExistingToken);//  acquire exclusive access to the source token
    ...
    // Calculate the relative offset between the old and new block.
    // We will use this value to fixup embedded pointers in this block.
    ...
    //  Now go through and change the address of each SID pointer for the user and groups
    ...
    // For the time being, take the easy way to generating an "EffectiveOnly" duplicate. 
    // That is, use the same space required of the original, just eliminate any IDs or privileges not active.

    // Ultimately, if duplication becomes a common operation, then it will be
    // worthwhile to recalculate the actual space needed and copy only the effective IDs/privileges into the new token.
    if (EffectiveOnly) {
        SepMakeTokenEffectiveOnly(NewToken);
    }

    // If the NewToken inherited an active SEP_AUDIT_POLICY from ExistingToken, then increment the counter of tokens with policies.
...
VOID SepMakeTokenEffectiveOnly(__in PTOKEN Token)
/*
Routine Description:
    ...
    default owner ID corresponding to a lapsed group will be changed so that the default owner ID is the user ID.
...
*/
{
    ...
    while (Index < ElementCount) {
        // If this privilege is not enabled, replace it with the one at the end of the array and reduce the size of the array by one.
    ...
    // Walk the UserAndGroups array (except for the first entry, which is the user - and can't be disabled) discarding any lapsed groups.
    ...
    while (Index < ElementCount) {
        // If this group is not enabled, replace it with the one at the end of the array and reduce the size of the array by one.
...
*/
{
    ...
    //  Check the handle's access to the existing token and get a pointer to that token. 
    //  Pick up the default desired access mask from the handle while we're at it.
    Status = ObReferenceObjectByHandle(
        ExistingTokenHandle,    // Handle
        TOKEN_DUPLICATE,        // DesiredAccess
        SeTokenObjectType,     // ObjectType
    ...
    EffectiveDesiredAccess = HandleInformation.GrantedAccess;// Check to see if an alternate desired access mask was provided.
    ...
    ObDereferenceObject((PVOID)Token);//  We no longer need our reference to the source token
...
Routine Description:
    Create a new token that is a subset of an existing token.
Arguments:
    ExistingToken - Is a  token already open for TOKEN_DUPLICATE access.
    Flags - Flags indicating additional filtering. The flags may be:
                DISABLE_MAX_PRIVILEGE - Disable all privileges
                SANDBOX_INERT - Record this SAIFER flag in the token.
    SidsToDisable - Contains a list of sids and attributes. All sids with
        the USE_FOR_DENY_ONLY attribute that also exist in the token will cause the new token to have that sid set with the USE_FOR_DENY_ONLY attribute.
    PrivilegesToDelete - Privileges in this list that are present in the existing token will not exist in the final token.
        This is similar to duplicating a token effective only with these privileges set to disabled.
    RestrictedSids - Contains a list of SIDs and attributes that will be stored in the RestrictedSids field of the new token.
        These SIDs are used after a normal access check to further restrict access.
        The attributes of these groups are always SE_GROUP_MANDATORY | SE_GROUP_ENABLED | SE_GROUP_ENABLED_BY_DEFAULT.
        If there already exist RestrictedSids in the original token, these sids will be appended.
    NewToken - Receives a pointer to the newly created token.
Return Value:
    STATUS_SUCCESS - Indicates the operation was successful.
    STATUS_INVALID_PARAMETER - Indicates one or more of the parameter values was invalid.
         This value is returned if the target token is not an impersonation token.
*/
{
    PTOKEN Token;
    ...
    *NewToken = NULL;//  Probe parameters
...
/*
Routine Description:
    This routine does the bulk of the work to actually filter a token.
    This routine assumes all access validation and argument probing has been performed.
...
Arguments:
    ...
    RestrictedSids - Contains a list of SIDs and attributes that will be stored in the RestrictedSids field of the new token.
        These SIDs are used after a normal access check to further restrict access.
        The attributes of these groups are always SE_GROUP_MANDATORY | SE_GROUP_ENABLED | SE_GROUP_ENABLED_BY_DEFAULT.
        If there already exist RestrictedSids in the original token, the intersection of the two sets will be in the final tokense sids will be.
...
*/
{
    ...
    VariableLength = ExistingToken->VariableLength + SidLength;//  Create a new object
    ...
    // Allocate a new modified Id to distinguish this token from the original token.
    ...
    // Compute the beginning portion of the variable part, which contains the sid & attributes arrays and the privilege set.
    // First copy the privileges. We will later remove the ones that are to be deleted.
    NextFree = (ULONG_PTR)(&NewToken->VariablePart);
    NewToken->Privileges = (PLUID_AND_ATTRIBUTES)NextFree;
    RtlCopyLuidAndAttributesArray(ExistingToken->PrivilegeCount, ExistingToken->Privileges, (PLUID_AND_ATTRIBUTES)NextFree);
...
#if defined(_WIN64)
    // At this point NextFree is 4-byte aligned, so no alignment work is necessary for the 32-bit product. 
    // For Win64, ensure that NextFree is 8-byte aligned.
    ...
    // the number of existing restricted SIDs plus the number of new restricted Sids
...
#define MAX(_x_,_y_) ((_x_) > (_y_) ? (_x_) : (_y_))
    ...
    // Now add all the existing restricted sids. We need to take the intersection of the two sets.
    NewToken->RestrictedSids = (PSID_AND_ATTRIBUTES)NextFree;
    for (Index = 0; Index < SidCount; Index++) {
        if ((ExistingToken->RestrictedSidCount == 0) || SepSidInSidAndAttributes(
    ...
    // If it doesn't, then we've ended up with a token that gives us more access than the original, which we don't want.
    if ((ExistingToken->RestrictedSidCount != 0) && (NewToken->RestrictedSidCount == 0)) {
        SepReleaseTokenReadLock(ExistingToken);
        Status = STATUS_INVALID_PARAMETER;

        // Cleanup.  ObDereferenceObject will cause the logon
        // session to be dereferenced, and will free the proxy data as well as the audit data.

        // See SepTokenDeleteMethod(), which is called by the object manager when the token object is being freed.
        ObDereferenceObject(NewToken);
        return(Status);
    }

    // If there are any restricted sids in the token, turn on the restricted flag
    ...
    // Ultimately, if duplication becomes a common operation, then it will be
    // worthwhile to recalculate the actual space needed and copy only the effective IDs/privileges into the new token.
    SepRemoveDisabledGroupsAndPrivileges(NewToken, Flags, GroupCount, GroupsToDisable, PrivilegeCount, PrivilegesToDelete);
