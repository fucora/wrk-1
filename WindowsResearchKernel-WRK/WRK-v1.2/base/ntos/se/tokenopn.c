...
NTSTATUS SepCreateImpersonationTokenDacl(__in PTOKEN Token, __in PACCESS_TOKEN PrimaryToken, __deref_out PACL *Acl)
/*
Routine Description:
...
Arguments:
    ...
    DesiredAccess - Is an access mask indicating which access types are desired to the token.
        These access types are reconciled with the Discretionary Access Control list of the token to determine whether the accesses will be granted or denied.
...
*/
{
    ...
    //  And decrement the reference count of the token to counter the action performed by PsOpenTokenOfProcess().  If the open
...
Arguments:
    ThreadHandle - Specifies the thread whose token is to be opened.
    DesiredAccess - Is an access mask indicating which access types
        are desired to the token.  These access types are reconciled with the Discretionary Access Control list of the token to determine whether the accesses will be granted or denied.
    OpenAsSelf - Is a boolean value indicating whether the access should
        be made using the calling thread's current security context, which may be that of a client if impersonating, or using the caller's process-level security context.
        A value of FALSE indicates the caller's current context should be used un-modified.
        A value of TRUE indicates the request should be fulfilled using the process level security context.
...
*/
{
    ...
    //  If the CopyOnOpen flag is not set, then the token can be opened directly.  Otherwise, the token must be duplicated,
    ...
    //  And decrement the reference count of the existing token to counter the action performed by PsOpenTokenOfThread.  If the open
    //  was successful, the handle will have caused the token's reference count to have been incremented.
    ObDereferenceObject(Token);
