...
*/
{
    ...
    //    }
        ...
        // Initialize the work queue spinlock, list head, and semaphore for each of the work queues.
...
*/
{
    ...
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Dacl, ACL_REVISION, DIRECTORY_QUERY | DIRECTORY_TRAVERSE | READ_CONTROL, SeAliasAdminsSid);
    ASSERT(NT_SUCCESS(Status));
    Status = RtlAddAccessAllowedAce(Dacl, ACL_REVISION, DIRECTORY_TRAVERSE, SeWorldSid);
    ASSERT(NT_SUCCESS(Status));
    ...
    SepInitProcessAuditSd();// Initialize the default SACL to use for auditing accesses to system processes. This initializes SepProcessSacl
