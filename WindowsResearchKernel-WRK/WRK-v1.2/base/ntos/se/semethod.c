...
Abstract:
    This Module implements the SeDefaultObjectMethod procedure.
    This procedure and SeAssignSecurity are the only two procedures that will place a security descriptor on an object.
    Therefore they must understand and agree on how a descriptor is allocated from pool so that they can deallocate and reallocate pool as necessary.
    Any security descriptor that is attached to an object by these procedures has the following pool allocation plan.

    1. if the objects security descriptor is null then there is no pool allocated
    2. otherwise there is at least one pool allocation for the security descriptor header.
       if its ACL fields are null then there are no other pool allocations (this should never happen).
    3. There is a separate pool allocation for each ACL in the descriptor.
       So a maximum of three pool allocations can occur for each attached security descriptor.
    4  Everytime an acl is replace in a descriptor we see if we can use the old ACL and if so then we try and keep the ACL size as large as possible.

    Note that this is different from the algorithm used to capture a security descriptor (which puts everything in one pool allocation).
    Also note that this can be easily optimized at a later time (if necessary) to use only one allocation.
*/
...
NTSTATUS SepDefaultDeleteMethod(IN OUT PSECURITY_DESCRIPTOR *ObjectsSecurityDescriptor);
...
/*
Routine Description:
    This routine builds an access mask representing the accesses necessary to set the object security information specified in the SecurityInformation parameter.
    While it is not difficult to determine this information, the use of a single routine to generate it will ensure minimal impact
    when the security information associated with an object is extended in the future (to include mandatory access control information).
Arguments:
...
/*
Routine Description:
    This routine builds an access mask representing the accesses necessary to query the object security information specified in the SecurityInformation parameter.
    While it is not difficult to determine this information, the use of a single routine to generate it will ensure
    minimal impact when the security information associated with an object is extended in the future (to include mandatory access control information).
Arguments:
...
/*
Routine Description:
    This is the default security method for objects.
...
*/
{
    ...
    // one that has security information associated with it.  Return an error.
    ...
    case SetSecurityDescriptor:
        ...
        return ObSetSecurityDescriptorInfo(Object, SecurityInformation, SecurityDescriptor, ObjectsSecurityDescriptor, PoolType, GenericMapping);
    ...
    case DeleteSecurityDescriptor:
        return SepDefaultDeleteMethod(ObjectsSecurityDescriptor);//  call the default delete security method
...
*/
{
    ...
    try {
        ...
        if ((((*SecurityInformation) & DACL_SECURITY_INFORMATION)) && (IObjectSecurity.Control & SE_DACL_PRESENT) && (IObjectSecurity.Dacl != NULL)) {
            DaclLength = (ULONG)LongAlignSize((IObjectSecurity.Dacl)->AclSize);
            Size += DaclLength;
        }

        if ((((*SecurityInformation) & SACL_SECURITY_INFORMATION)) && (IObjectSecurity.Control & SE_SACL_PRESENT) && (IObjectSecurity.Sacl != NULL)) {
        ...
        *Length = Size;
        ...
        //  The real addresses of each acl will be filled in later when we copy the ACLs over.

        //  Note that we only set a flag in the descriptor if the information
        //  was requested, which is a simple copy of the requested information input variable

        //  The output buffer has already been probed to the passed size, so we can just write to it.
        RtlCreateSecurityDescriptorRelative(SecurityDescriptor, SECURITY_DESCRIPTOR_REVISION);

        // Mark the returned Security Descriptor as being in self-relative format
        RtlpSetControlBits(ISecurityDescriptor, SE_SELF_RELATIVE);

        //  NextFree is used to point to the next free spot in the returned security descriptor.
        NextFree = LongAlignPtr((PUCHAR)SecurityDescriptor + sizeof(SECURITY_DESCRIPTOR_RELATIVE));

        //  Copy the Owner SID if necessary and update the NextFree pointer, keeping it longword aligned.
        if (((*SecurityInformation) & OWNER_SECURITY_INFORMATION) && ((IObjectSecurity.Owner) != NULL)) {
        ...
        //  Copy the Group SID if necessary and update the NextFree pointer, keeping it longword aligned.
        if (((*SecurityInformation) & GROUP_SECURITY_INFORMATION) && (IObjectSecurity.Group != NULL)) {
        ...
        //  If not set in object's security, then everything is already set properly.
        if ((*SecurityInformation) & DACL_SECURITY_INFORMATION) {
        ...
        //  If not set in object's security, then everything is already set properly.
        if ((*SecurityInformation) & SACL_SECURITY_INFORMATION) {
            RtlpPropagateControlBits(ISecurityDescriptor, &IObjectSecurity, SE_SACL_PRESENT | SE_SACL_DEFAULTED | SE_SACL_PROTECTED | SE_SACL_AUTO_INHERITED);
...
/*
Routine Description:
    This is a private procedure to delete the security descriptor for an object.
    It cleans up any pool allocations that have occured as part of the descriptor.
Arguments:
    ObjectsSecurityDescriptor - Supplies the address of a pointer to the security descriptor being deleted.
Return Value:
