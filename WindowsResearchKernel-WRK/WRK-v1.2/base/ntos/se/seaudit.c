...
#ifdef ALLOC_PRAGMA
VOID SepSetAuditInfoForObjectType(
    IN  UCHAR AceFlags,
...
Arguments:
    Object - Address of the object accessed.  This value will not be used as a pointer (referenced).  It is necessary only to enter into log messages.
    ...
    AccessGranted - Indicates whether the access was granted or denied.
    A value of TRUE indicates the access was allowed.
    A value of FALSE indicates the access was denied.
...
Arguments:
    SubsystemName - Supplies a name string identifying the subsystem calling the routine.
    ServiceName - Supplies a name of the privileged subsystem service.
        For example, "RESET RUNTIME LOCAL SECURITY POLICY" might be specified by a Local Security Authority service used to update the local security policy database.
    ClientToken - A handle to a token object representing the client that requested the operation.
        This handle must be obtained from a communication session layer, such as from an LPC Port or Local Named Pipe, to prevent possible security policy violations.
...
*/
{
    ...
    SepAdtPrivilegedServiceAuditAlarm(&SubjectSecurityContext, CapturedSubsystemName, CapturedServiceName, Token, SubjectSecurityContext.PrimaryToken, CapturedPrivileges, AccessGranted);
...
Arguments:
    ...
    Privileges - Supplies a privilege set containing the privilege(s) required for the access.
...
*/
{
    ...
    SepAdtPrivilegedServiceAuditAlarm(SubjectSecurityContext, (PUNICODE_STRING)&SeSubsystemName, ServiceName, SubjectSecurityContext->ClientToken, SubjectSecurityContext->PrimaryToken, Privileges, AccessGranted);
}
...
Arguments:
    ...
    SecurityDescriptor - A pointer to the Security Descriptor against which access is to be checked.
    DesiredAccess - The desired access mask.  This mask must have been previously mapped to contain no generic accesses.
    AuditType - Specifies the type of audit to be generated.  Valid value is: AuditEventObjectAccess
...
*/
{
    ...
    if (!AvoidAudit) {
        ...
        if (GenerateSuccessAudit ||
            ...
            // Only audit successful privilege use for opens.
...
Arguments:
    ...
    AbsoluteObjectName - Supplies the name of the object being accessed.
        ...
        Otherwise, it must be provided.
    SecurityDescriptor - A pointer to the security descriptor of the object being accessed.
    AccessState - A pointer to an access state structure containing the subject context,
        the remaining desired access types, the granted access types, and optionally a privilege set to indicate which privileges were used to permit the access.
...
*/
{
    ...
    if (SecurityDescriptor != NULL) {
        ...
        if (SepAdtAuditThisEventWithContext(AuditCategoryObjectAccess, AccessGranted, !AccessGranted, &AccessState->SubjectSecurityContext)) {
            ...
            if (GenerateAudit) {// Only generate an audit on close of we're auditing from SACL settings.
                *GenerateOnClose = TRUE;
                if (AccessGranted) {// Construct the audit mask that will be placed into the handle.
                    SeMaximumAuditMask(RtlpSaclAddrSecurityDescriptor((PISECURITY_DESCRIPTOR)SecurityDescriptor), RequestedAccess, Token, &AuxData->MaximumAuditMask);
                }
            }
        }
    }
...
Arguments:
    ...
    Object - Address of the object accessed.  This value will not be used as a pointer (referenced).
        It is necessary only to enter into log messages.
        If the open was not successful, then this argument is ignored.  Otherwise, it must be provided.
    AbsoluteObjectName - Supplies the name of the object being accessed.
        If the object doesn't have a name, then this field is left null.
        Otherwise, it must be provided.
    SecurityDescriptor - A pointer to the security descriptor of the object being accessed.
...
*/
{
    ...
    if (AccessState->GenerateAudit) {
        if (AccessState->AuditPrivileges) {
            // ignore the result of the call below so that we do not incorrectly set the value of AuditPerformed which is later assigned to AccessState->GenerateOnClose
...
VOID SeDeleteObjectAuditAlarm(__in PVOID Object, __in HANDLE Handle)
/*
Routine Description:
...
Arguments:
    ...
    GenerateAlarm - Returns a boolean indicating whether or not we should generate an alarm.
Return Value:
...
*/
{
    ...
    *GenerateAlarm = FALSE;
    ...
    // Generate an audit if the user asked for maximum allowed and we find any allowed or deny ace with a sid that matches one of the sids in the user's token.
    MaximumAllowed = 0;
    if (DesiredAccess & MAXIMUM_ALLOWED) {
        if (AccessGranted) {
    ...
    if (*(PUSHORT)((PTOKEN)Token)->UserAndGroups->Sid == *(PUSHORT)SeAnonymousLogonSid && RtlEqualMemory(((PTOKEN)Token)->UserAndGroups->Sid, SeAnonymousLogonSid, SeLengthSid(SeAnonymousLogonSid))) {
        ...
        if (!(((PACE_HEADER)Ace)->AceFlags & INHERIT_ONLY_ACE)) {
            if ((((PACE_HEADER)Ace)->AceType == SYSTEM_AUDIT_ACE_TYPE)) {
                ...
                if (SepSidInToken((PACCESS_TOKEN)Token, NULL, AceSid, FALSE) ||
                    (WorldSidLength && *(PUSHORT)SeWorldSid == *(PUSHORT)AceSid && RtlEqualMemory(SeWorldSid, AceSid, WorldSidLength))) {
...
Arguments:
    ...
    AccessStatus - Specifies STATUS_SUCCESS or other error code to be propagated back to the caller
...
Arguments:
    ...
    AccessStatus - Specifies STATUS_SUCCESS or other error code to be propagated back to the caller
    PrincipalSelfSid - If the security descriptor is associated with an object that represents a principal (for example, a user object), the PrincipalSelfSid parameter should be the SID of the object.
        When evaluating access, this SID logically replaces the SID in any ACE containing the well-known PRINCIPAL_SELF SID (S-1-5-10).
...
#define INVALID_OBJECT_TYPE_LIST_INDEX 0xFFFFFFFF
    ...
    *GenerateFailureAudit = FALSE;
    ...
    for (i = 0, Ace = FirstAce(Sacl); (i < AceCount) && !((*GenerateSuccessAudit || *GenerateFailureAudit) && ObjectTypeListLength <= 1); i++, Ace = NextAce(Ace)) {
        ...
        if ((((PACE_HEADER)Ace)->AceType == SYSTEM_AUDIT_ACE_TYPE)) {
            ...
            if (SepSidInToken(Token, PrincipalSelfSid, AceSid, (BOOLEAN)((AceFlags & FAILED_ACCESS_ACE_FLAG) != 0)) ||
                ...
                if (ObjectTypeListLength == 0) {
                    if (NT_SUCCESS(AccessStatus[0])) {
                        if ((AceFlags & SUCCESSFUL_ACCESS_ACE_FLAG) && ((AccessMask & GrantedAccess[0]) || (MaximumAllowed == SUCCESSFUL_ACCESS_ACE_FLAG))) {
                            *GenerateSuccessAudit = TRUE;
                        }
                    } else {
                        if ((AceFlags & FAILED_ACCESS_ACE_FLAG) && ((AccessMask & DesiredAccess) || (MaximumAllowed == FAILED_ACCESS_ACE_FLAG))) {
                            *GenerateFailureAudit = TRUE;
                        }
                    }
                } else {
        ...
        } else if ((((PACE_HEADER)Ace)->AceType == SYSTEM_AUDIT_OBJECT_ACE_TYPE)) {// Handle an object specific audit ACE
            ...
            } else {// If an object type is in the ACE, Find it in the LocalTypeList before using the ACE.
                if (SepSidInToken(Token, PrincipalSelfSid, AceSid, (BOOLEAN)((AceFlags & FAILED_ACCESS_ACE_FLAG) != 0)) ||
                    (WorldSidLength && *(PUSHORT)SeWorldSid == *(PUSHORT)AceSid && RtlEqualMemory(SeWorldSid, AceSid, WorldSidLength))) {
...
// This list contains the privileges which are not audited for the service accounts.
// There should be no duplicates between this list and the other filter lists.
...
Arguments:
    Verbose - Whether we want to filter by the short or long privileges list.
    Verbose == TRUE means use the short list.
Return Value:
...
BOOLEAN SepFilterPrivilegeAudits(IN ULONG Flags, IN PPRIVILEGE_SET PrivilegeSet)
/*
Routine Description:
...
*/
{
    ...
    if (Flags & SEP_SERVICES_FILTER) {
        // Additionally, check the privileges against those that are not audited for service accounts.
        for (i = 0; i < PrivilegeSet->PrivilegeCount; i++) {
...
BOOLEAN SeAuditingFileEvents(__in BOOLEAN AccessGranted, __in PSECURITY_DESCRIPTOR SecurityDescriptor)
/*
Routine Description:
    This routine is to be called by a file system to quickly determine if we are auditing file open events.
    This allows the file system to avoid the often considerable setup involved in generating an audit.
...
*/
{
    ...
    return(SepAdtAuditThisEvent(AuditCategoryObjectAccess, &AccessGranted) || SepAdtAuditThisEvent(AuditCategoryPrivilegeUse, &AccessGranted));
}
...
BOOLEAN SeAuditingHardLinkEvents(__in BOOLEAN AccessGranted, __in PSECURITY_DESCRIPTOR SecurityDescriptor)
/*
Routine Description:
...
*/
{
    ...
    if ((NULL != Sacl) && (0 != Sacl->AceCount) && (SepAdtAuditThisEvent(AuditCategoryObjectAccess, &AccessGranted))) {
        return TRUE;
    }
