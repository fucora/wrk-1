...
POLICY_AUDIT_LOG_INFO SepAdtLogInformation;// Audit Log Information
...
extern PKEVENT SepAdtLsaDeadEvent;// Set when LSA has died.
...
extern ULONG SepAdtCountEventsDiscarded;// Number of events discarded
extern ULONG SepAdtCurrentListLength;// Number of events on the queue
extern BOOLEAN SepAdtDiscardingAudits;// Flag to tell us that we're discarding audits
extern BOOLEAN SepCrashOnAuditFail;// Flag to tell us that we should crash if we miss an audit.
...
// NT5 does not provide any facility to enable/disable auditing at audit-event level.
// It only supports it at audit category level.
// This creates problems if one wants to audit only certain specific audit events of a category.
// The current design gives you all or none for each category.

// Post NT5 auditing will provide a better/flexible design that will address this issue.
// For now, to delight some valuable customers, we provide this hack / registry based solution.
// This solution will be removed post NT5.
