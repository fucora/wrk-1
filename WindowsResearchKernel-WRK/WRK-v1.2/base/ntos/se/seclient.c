...
Arguments
   ...
   ThreadEffectiveOnly - if the token is an impersonation token, then this is the thread's ImpersonationInfo->EffectiveOnly field value.
...
Return Value:
    ...
    STATUS_BAD_IMPERSONATION_LEVEL - The client is currently impersonating either an Anonymous or Identification level token, which can not be passed on for use by another server.
        ...
        block is for an inter-system communication session and the client thread is impersonating a client of its own using other than delegation impersonation level.
*/
{
    NTSTATUS Status = STATUS_SUCCESS;
    PACCESS_TOKEN Token;
    ...
    if ((!NT_SUCCESS(Status)) || (ClientSecurityQos->ContextTrackingMode == SECURITY_STATIC_TRACKING)) {// If failed, or if a token was copied internally, then deref our token.
...
VOID SeImpersonateClient(__in PSECURITY_CLIENT_CONTEXT ClientContext, __in_opt PETHREAD ServerThread)
/*
Routine Description:
    This service is used to cause the calling thread to impersonate a client.
    The client security context in ClientContext is assumed to be up to date.
Arguments:
...
NTSTATUS SeImpersonateClientEx(__in PSECURITY_CLIENT_CONTEXT ClientContext, __in_opt PETHREAD ServerThread)
/*
Routine Description:
