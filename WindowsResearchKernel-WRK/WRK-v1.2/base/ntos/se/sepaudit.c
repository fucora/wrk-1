...
Abstract:
    This Module implements the audit and alarm procedures that are private to the security component.
*/
...
BOOLEAN FASTCALL SeDetailedAuditingWithToken(__in_opt PACCESS_TOKEN AccessToken)
/*
Routine Description
    This routine computes whether or not a detailed tracking audit should be generated for a given token or context.
    If no token is passed then the current effective token will be captured.
...
*/
{
    ...
    // If no token was passed in then capture the context.
    ...
    if (Token == NULL) {
        // Take whatever action SepAuditFailed takes and return the auditing option we have from the system audit policy
        SepAuditFailed(STATUS_NO_TOKEN);
        goto Cleanup;
    }

    // Audit if the token specifies success auditing (there is not a detailed tracking failure concept) or if global audit policy specifies detailed tracking auditing and this token is not excluded.
    Mask = Token->AuditPolicy.PolicyElements.DetailedTracking;
    if ((Mask & TOKEN_AUDIT_SUCCESS_INCLUDE) || (AuditThisEvent && (0 == (Mask & TOKEN_AUDIT_SUCCESS_EXCLUDE)))) {
        AuditThisEvent = TRUE;
    } else {
...
Cleanup:
    if (AccessToken == NULL) {
        // if AccessToken is NULL then we had to capture the context.  Release it.
...
Routine Description
    ...
    If no context is passed in then the current context is captured and examined.
Arguments
    Category - the category for which we determine whether or not to generate an audit
...
*/
{
    ...
    if ((SeAuditingState[Category].AuditOnSuccess && AccessGranted) || (SeAuditingState[Category].AuditOnFailure && AccessDenied)) {
        AuditThisEvent = TRUE;
    } else {
        AuditThisEvent = FALSE;
    }
    ...
    // Now we have to check the token audit mask because the token may override the policy and say 'do not audit,' even though the array claims we must (or vice versa)
    switch (Category) {
    ...
    if (Mask) {
        // If granted and the token is marked for success_include OR if not granted and token is marked for failure_include then audit the event.
...
BOOLEAN SepAdtPrivilegeObjectAuditAlarm(
...
/*
Routine Description:
    Implements NtPrivilegeObjectAuditAlarm after parameters have been captured.
...
Arguments:
    ...
    ClientToken - Optionally provides a pointer to the client token (only if the caller is currently impersonating)
    ...
    CapturedPrivileges - The set of privileges required for the requested operation.
...
*/
{
    ...
    if (SepAdtAuditThisEventWithContext(AuditCategoryPrivilegeUse, AccessGranted, !AccessGranted, NULL) && SepFilterPrivilegeAudits(0, CapturedPrivileges)) {
        ...
        // A completely zero'd entry will be interpreted as a "null string" or not supplied parameter.
        // Initializing the entire array up front will allow us to avoid filling in each not supplied entry.
...
VOID SepAdtPrivilegedServiceAuditAlarm(
...
Arguments:
    ...
    ServiceName - Supplies a name of the privileged subsystem service.  For example, "RESET RUNTIME LOCAL SECURITY" might be specified by a Local Security Authority service used to update the local security policy database.
    ClientToken - Optionally provides a pointer to the client token (only if the caller is currently impersonating)
    PrimaryToken - Provides a pointer to the caller's primary token.
...
BOOLEAN SepAdtOpenObjectAuditAlarm(
    IN PUNICODE_STRING CapturedSubsystemName,
...
Arguments:
    ...
    CapturedPrivileges - Optionally points to a set of privileges that were required for the access attempt.
        Those privileges that were held by the subject are marked using the UsedForAccess flag of the attributes associated with each privilege.
    ...
    AuditType - Specifies the type of audit to be generated.  Valid values are: AuditCategoryObjectAccess and AuditCategoryDirectoryServiceAccess.
    ...
    GrantedAccessArray - If non NULL, specifies an array of access mask granted to each object in ObjectTypeList.
Return Value:
...
*/
{
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        Status = STATUS_SUCCESS;// ignore this failure
    }
    ...
    if (ObjectTypeListLength != 0) {
        ...
        // If there are any Guids to audit, copy them into a locally allocated buffer.
        if (GuidCount > 0) {
    ...
    SepAdtLogAuditRecord(&AuditParameters);// Audit it.
...
BOOLEAN SepAdtOpenObjectForDeleteAuditAlarm(
    IN PUNICODE_STRING CapturedSubsystemName,
    IN PVOID *HandleId OPTIONAL,
    IN PUNICODE_STRING CapturedObjectTypeName,
    IN PUNICODE_STRING CapturedObjectName OPTIONAL,
...
/*
    Routine Description:
    Implements SeOpenObjectForDeleteAuditAlarm after parameters have been captured.
...
/*
Routine Description:
    This routine implements NtCloseObjectAuditAlarm after parameters have been captured.
...
/*
Routine Description:
    This routine implements NtDeleteObjectAuditAlarm after parameters have been captured.
...
*/
{
    ...
    //  If the user's SID was not passed, get it out of the current subject context
...
Return Value:
    A pointer to a buffer containing a POBJECT_NAME_INFORMATION structure containing the name of the object.
    The string is allocated out of paged pool and should be freed by the caller.
...
Arguments:
    Object - a pointer to an object.
Return Value:
    A pointer to a UNICODE_STRING that contains the name of the object type.
    The string is allocated out of paged pool and should be freed by the caller.
    NULL may also be returned.
*/
{
    ...
    Status = ObQueryTypeName(Object, &ObjectTypeName, 0, &ReturnLength);
    if (Status == STATUS_INFO_LENGTH_MISMATCH) {
...
/*
Routine Description:
    Audits the creation of a process.
    It is the caller's responsibility to determine if process auditing is in progress.
Arguments:
...
*/
{
    ...
    // Use the primary token here, because that's what's going to show up when the created process exits.
...
Routine Description:
    ...
    It is up to the caller to determine if this routine should be called or not.
Arguments:
...
/*
Routine Description:
    Audits the exit of a process.
    The caller is responsible for determining if this should be called.
Arguments:
...
Routine Description:
    ...
    It allocates memory for the image file name.
    This memory is pointed to by pAuditName.
Arguments:
    FileObject - Supplies a pointer to a file object for the image being executed.
    bIgnoreAuditPolicy - boolean that indicates that the call should proceed without regard to the system's auditing policy.
...
*/
{
    ...
    *pAuditName = NULL;
    ...
    // Check if the caller would like to get the process name, even if auditing does not require it.
    if (FALSE == bIgnoreAuditPolicy) {
        // At the time of process creation, this routine should only proceed when Object Access or Detailed Tracking auditing is enabled.
        // In all other cases, the process name is acquired when it is requested.
        if (!SepAdtAuditThisEventWithContext(AuditCategoryObjectAccess, TRUE, FALSE, NULL) &&
    ...
    // The ObjectNameInformationLength contains only a UNICODE_STRING, so if this call succeeded it would indicate an imagefile name of length 0.
    // That is bad, so all return values except STATUS_BUFFER_OVERFLOW (from NTFS) and STATUS_BUFFER_TOO_SMALL (from DFS).
    // This call gives me the buffer size that I need to store the image name.
    pInternalAuditName = &TempNameInfo;
    ObjectNameInformationLength = sizeof(OBJECT_NAME_INFORMATION);
    Status = ObQueryNameString(FilePointer, pInternalAuditName, ObjectNameInformationLength, &ObjectNameInformationLength);
    if ((Status == STATUS_BUFFER_OVERFLOW) || (Status == STATUS_BUFFER_TOO_SMALL)) {
        // Sanity check ObQueryNameString.
        // Different filesystems may be buggy, so make sure that the return length makes sense (that it has room for a non-NULL Buffer in the UNICODE_STRING).
        if (ObjectNameInformationLength > sizeof(OBJECT_NAME_INFORMATION)) {
            ...
            if (pInternalAuditName != NULL) {
                ...
                if (!NT_SUCCESS(Status)) {
                    ...
                    // If the second call to ObQueryNameString did not succeed, then something is very wrong.
                    // Set the image name to NULL string.

                    // Free the memory that the first call to ObQueryNameString requested, and allocate enough space to store an empty UNICODE_STRING.
                    ExFreePool(pInternalAuditName);
                    ObjectNameInformationLength = sizeof(OBJECT_NAME_INFORMATION);
                    pInternalAuditName = ExAllocatePoolWithTag(NonPagedPool, ObjectNameInformationLength, 'aPeS');
                    if (pInternalAuditName != NULL) {
                        RtlZeroMemory(pInternalAuditName, ObjectNameInformationLength);
                        Status = STATUS_SUCCESS;// Status = STATUS_SUCCESS to allow the process creation to continue.
                    } else {
            ...
            // If this happens, then ObQueryNameString is broken for the FS on which it was called.
#if DBG
            DbgPrint("\n** ObqueryNameString failed with 0x%x.\n", Status);
#endif //DBG
        ...
        // If ObQueryNameString returns some other error code, we cannot be certain of which action to take, or whether it has properly set the ReturnLength.
        // For example, ObQueryNameString has slightly different semantics under DFS than NTFS.
        // Additionally, 3rd party file systems may also behave unpredictably.
        // For these reasons, in the case of an unexpected error code from ObQueryNameString we set AuditName to zero length unicode string and allow process creation to continue.
...
/*
Routine Description
    This routine returns the ImageFileName information from the process, if available.
    This is a "lazy evaluation" wrapper around SeInitializeProcessAuditName.
    If the image file name information has already been computed, then this call simply allocates and returns a UNICODE_STRING with this information.
    Otherwise, the function determines the name, stores the name in the EPROCESS structure, and then allocates and returns a UNICODE_STRING.
    Caller must free the memory returned in pImageFileName.
Arguments
...
*/
{
    ...
    *pImageFileName = NULL;
    if (NULL == Process->SeAuditProcessCreationInfo.ImageFileName) {
        // The name has not been predetermined.
        // We must determine the process name.
        // First, reference the PFILE_OBJECT and lookup the name.
        // Then again check the process image name pointer against NULL.
...
/*
Routine Description:
    This routine generates an assign primary token audit.
    It is up to the caller to determine if this routine should be called or not.
Arguments:
...
*/
{
    ...
    // Get information about the current process, that is, the process that is assigning a new primary token.
...
VOID SeAuditHardLinkCreation(__in PUNICODE_STRING FileName, __in PUNICODE_STRING LinkName, __in BOOLEAN bSuccess)
/*
Routine Description:
...
*/
{
    ...
    case SeAdtParmTypeLogonId:
        Length = sizeof(LUID);
        RtlCopyMemory(&AuditParameters->Parameters[Index].Data[0], Data, Length);
        break;
...
Arguments
    ...
    AuditParameters - pointer to an SE_ADT_PARAMETER_ARRAY.  The following fields of the structure should be specified by the caller:
            ...
            Parameters - To initialize each entry in the array please use the provided helper macros named SeSetParmType*.
                A macro exists for each type enumerated in SE_ADT_PARAMETER_TYPE.
                A caller can specify at most SE_MAX_GENERIC_AUDIT_PARAMETERS in this array.
                The Se component must reserve space in the structure for additional information.
Return Value
    NTSTATUS.
*/
{
    ...
    if (UserSid != NULL && RtlValidSid(UserSid) == FALSE) {
        return STATUS_INVALID_PARAMETER;
    }
#endif

    // Determine which Sid to specify as the User.
    // If the UserSid was provided then use that.
    // Otherwise get the user Sid from the effective token.
    if (UserSid != NULL) {
        AuditSid = UserSid;
    } else {
    ...
    // Build up the CompleteParameters structure.
    // This is the structure we will send to the LSA.
    // The LSA and the eventlog will parse this as user defined audit.
    RtlZeroMemory(&CompleteParameters, sizeof(CompleteParameters));
    ...
    // The first two entries in an audit are always the Sid and the Security source name.
    SepSetParmTypeSid(CompleteParameters, CompleteParameters.ParameterCount, AuditSid);
    CompleteParameters.ParameterCount++;
    SepSetParmTypeString(CompleteParameters, CompleteParameters.ParameterCount, (PUNICODE_STRING)&SeSubsystemName);
    CompleteParameters.ParameterCount++;

    // To generate a non system security event, we actually generate an audit of type SE_AUDITID_GENERIC_AUDIT_EVENT.
    // In this audit, the third parameter is the caller specified Source name.
    // The fourth parameter is the specified Audit Id.
    // The event log knows how to parse this special audit.
    SepSetParmTypeString(CompleteParameters, CompleteParameters.ParameterCount, SourceName);
    CompleteParameters.ParameterCount++;
    SepSetParmTypeUlong(CompleteParameters, CompleteParameters.ParameterCount, AuditParameters->AuditId);
