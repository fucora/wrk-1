...
NTSTATUS SepCleanupLUIDDeviceMapDirectory(PLUID pLogonId);
...
// This macro is used to obtain an index into the logon session tracking array given a logon session ID (a LUID).
...
Arguments:
    CommandMessage - Points to structure containing RM command message
        information consisting of an LPC PORT_MESSAGE structure followed by the command number (RmComponentTestCommand) and a command-specific body.
        The command-specific body of this parameter is a LUID of the logon session to be created.
    ReplyMessage - Pointer to structure containing LSA reply message information consisting of an LPC PORT_MESSAGE structure followed
...
*/
{
    ...
    ASSERT(CommandMessage->CommandNumber == RmCreateLogonSession);// Check that command is expected type    
    LogonId = *((LUID UNALIGNED *) CommandMessage->CommandParams);// Typecast the command parameter to what we expect.    
    Status = SepCreateLogonSessionTrack(&LogonId);// Try to create the logon session tracking record    
    ReplyMessage->ReturnedStatus = Status;// Set the reply status
}
...
*/
{
    ...
    ASSERT(CommandMessage->CommandNumber == RmDeleteLogonSession);// Check that command is expected type    
    LogonId = *((LUID UNALIGNED *) CommandMessage->CommandParams);// Typecast the command parameter to what we expect.    
    Status = SepDeleteLogonSessionTrack(&LogonId);// Try to create the logon session tracking record    
    ReplyMessage->ReturnedStatus = Status;// Set the reply status
}
...
*/
{
    ...
    SepRmAcquireDbWriteLock(SessionArrayIndex);// Protect modification of reference monitor database    
    Current = *Previous;// Now walk the list for our logon session array hash index.
    ...
    // Bad news, someone asked us to increment the reference count of a logon session we didn't know existed.  This might be a new
    // token being created, so return an error status and let the caller decide if it warrants a bugcheck or not.
...
/*
Routine Description:
    Make the contents of the (LUID's device map)'s directory object temporary so that their names go away.
Arguments:
    pLogonId - Pointer to the logon session ID whose device is to be cleaned up
Return Value:
    STATUS_SUCCESS - cleaned up the entire device map
    STATUS_INVALID_PARAMETER - pLogonId is a NULL pointer
    STATUS_NO_MEMORY - could not allocate memory to hold the handle buffer
...
*/
{
    ...
    // Attach to the system process if the current process has a reference count of 0 to avoid recursive process deletion.
...
Restart:
    // Create an array of handles to close with each scan of the directory
    ...
    while (TRUE) {
        ...
        if (!wcscmp(DirInfo->TypeName.Buffer, L"SymbolicLink")) {
            ...
            if (Count >= Size) {
                // empty the handle array by closing all the handles and free the handle array so that we can create a bigger handle array
            ...
            InitializeObjectAttributes(&Attributes, &DirInfo->Name, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, DosDevicesDirectory, NULL);
            Status = ZwOpenSymbolicLinkObject(&LinkHandle, SYMBOLIC_LINK_ALL_ACCESS, &Attributes);
            if (NT_SUCCESS(Status)) {
                // Make the object temporary so that its name goes away from the Object Manager's namespace
...
Routine Description:
    ...
    If the reference count is decremented to zero, then there is no possibility for any more tokens to exist for the logon session.
...
*/
{
    ...
    while (Current != NULL) {
        ...
        if (RtlEqualLuid(LogonId, &Current->LogonId)) {
            ...
            if (Refs == 0) {
                *Previous = Current->Next;// Pull it from the list                
                SepRmReleaseDbWriteLock(SessionArrayIndex);// No longer need to protect our pointer to this record.

                // If the Device Map exist for this LUID, dereference the pointer to the Device Map
                if (Current->pDeviceMap != NULL) {
                    // Dereference our reference on the device map our reference should be the last reference,
                    // thus the system will delete the device map for the LUID
                ...
                ExFreePool((PVOID)Current);// Deallocate the logon session track record.                
                SepInformLsaOfDeletedLogon(LogonId);// Inform the LSA about the deletion of this logon session.
                return;
            }
...
Routine Description:
    ...
    rundown.  Therefore, we must queue the work off to a worker thread which can then make an LPC call to the LSA.
Arguments:
...
/*
Routine Description:
    This is the dual of SeRegisterLogonSessionTerminatedRoutine. A File System *MUST* call this before it is unloaded.
Arguments:
...
/*
Routine Description:
    File systems that have registered for logon-termination notification can mark logon sessions they are interested in for callback by calling this routine.
Arguments:
...
Routine Description:
    ...
    Note that we can not be guaranteed that we are in a whole (or wholesome)
    thread, since we may be in the middle of process deletion and object rundown.
    Therefore, we must queue the work off to a worker thread.
Arguments:
    LogonId - Pointer to the logon session ID which has been deleted.
Return Value:
    None.
*/
{
    ...
    if (FSNotifyContext == NULL) {
        // I don't know what to do here... file systems will loose track of a logon session, but the system isn't really harmed in any way.
...
/*
Routine Description:
    This routine is called by components that want a handle to the Device Map for the specified LUID
Arguments:
...
*/
{
    ...
    while (Current != NULL) {
        ...
        if (RtlEqualLuid(pLogonId, &(Current->LogonId))) {
            ...
            if (Current->pDeviceMap == NULL) {
                ...
                InitializeObjectAttributes(&Obja, &UnicodeString, OBJ_CASE_INSENSITIVE | OBJ_OPENIF | OBJ_KERNEL_HANDLE, NULL, NULL);
                Status = ZwCreateDirectoryObject(&hDevMap, DIRECTORY_ALL_ACCESS, &Obja);
                if (NT_SUCCESS(Status)) {
                    Status = ObSetDirectoryDeviceMap(&pDeviceMap, hDevMap);// Set the DeviceMap for this directory object
                ...
                SepRmReleaseDbWriteLock(SessionArrayIndex);
                SepDeReferenceLogonSessionDirect(Current);// Remove the reference we just added
                return Status;
            } else {
    ...
    // Bad news, someone asked us for a device map of a logon session we didn't know existed.  This might be a new
    // token being created, so return an error status and let the caller decide if it warrants a bugcheck or not.
    return STATUS_NO_SUCH_LOGON_SESSION;
}
...
*/
{
    ...
    while (Current != NULL) {
        // If we found it, increment the reference count and return
        if (RtlEqualLuid(LogonId, &Current->LogonId)) {
            // Stick the token address into the track.  Find the last in the list of tokens for this session.
