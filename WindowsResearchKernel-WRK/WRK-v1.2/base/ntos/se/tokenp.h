...
// capabilities within token code.  These flags are set in TokenGlobalFlag (only available within a DBG system).

//      TOKEN_LOCKS - Display information about acquisition and freeing of token locks.
...
// If the size of these parameters passed upon token creation are larger than this default, then the larger value will be charged.
...
// The number of tokens that have audit policies are tracked in SepTokenPolicyCounter.
// This is done so that when all tokens
// with policies are gone (ie SepTokenPolicyCounter == 0) the routines that decide if an audit should be generated can execute faster.
...
// The fixed part of the body contains the fixed length fields.  These are defined in the TOKEN data type.

// The variable part of the body is variable in length and contains privileges and user/group SIDs.
// This part is variable in length between different token objects, but does not change once established for an individual token.

// The dynamic part is used to house default discretionary ACL information and the primary group ID.
...
//              intented to reflect their actual or even relative positions within the real data structures.
//              The exception to this is that THE USER SID IS THE FIRST SID IN THE UserAndGroups ARRAY.

//             ! ! ! !     IMPORTANT     ! ! ! !

//      The access validation routines assume the SIDs are arranged
//      in a particular order within the variable part of the token.
//      Any changes to the order of the SIDs must be coordinated with corresponding changes to the access validation routines.
...
typedef struct _TOKEN {
    ...
    //  token is still valid.  ReadWrite fields may only be referenced when the TokenLock is held.
    ...
    // following ID is changed.  Fields that cause this field to be updated are marked as (Mod) in their comment field.
...
//    TokenSource - Information provided by the executive component that requested the logon that the token represents.
//    TokenId - Is an LUID value.  Each token object has a uniquely assigned LUID.
//    AuthenticationId - Is the LUID assigned by the domain controller for the logon session.
...
//        and changing default ACLs, et cetera.  Any token which is a duplicate of this token will have the same ModifiedId (until
//        one or the other is changed).  This does not cover changes to non-security semantics fields, like TokenInUse.
...
//        ID with no supplemental group IDs.  A value of 5 indicates a user ID and 4 supplemental group IDs.
//    PrivilegeCount - Indicates how many privileges are included in this token.  May be zero or larger.
...
//        indicates the impersonation level.  For TokenPrimary type tokens, this field is ignored.
//    DynamicCharged - Indicates how much pool quota has been charged for the dynamic portion of this token.
//    DynamicAvailable - Indicates how much of the charged quota is still
//        available for use.  This is modified when  pool associated
//        with the dynamic portion of the token is allocated or freed, such as when the default DACL or primary group is replaced.
//    DefaultOwnerIndex - If non-zero, identifies an ID that has explicitly
//        been established as the default owner for this token.  If it is zero, the standard default (user ID) is used as the default owner.
...
//        elements are those of groups.  The number of entries in this array is one greater than
//    PrimaryGroup - Points to an SID that is to be used as the primary group of the token.
//        There are no value restrictions placed upon what can be used as a primary group.
//        This SID is not one of user or group IDs (although it may have the same value as one of those IDs).
//    Privileges - Points to an array of privileges represented as LUID_AND_ATTRIBUTES.
//        The number of elements in this array is contained in the PrivilegesCount field.
//    TokenInUse - Is a boolean that indicates whether a primary token
//        is already in use by a process.  This field value is only valid for primary tokens.
//    ProxyData - Optionally points to a Proxy data structure, containing the information to be passed to AVR routines by file systems.
//        This field being non-null identifies the token as a proxy token.
//    AuditData - Optionally points to an Audit data structure, containing global auditing data for this subject.
...
#define SepAcquireTokenReadLock(T)  KeEnterCriticalRegion(); ExAcquireResourceSharedLite((T)->TokenLock, TRUE)
#define SepAcquireTokenWriteLock(T) KeEnterCriticalRegion(); ExAcquireResourceExclusiveLite((T)->TokenLock, TRUE)
#define SepReleaseTokenReadLock(T)  ExReleaseResourceLite((T)->TokenLock);  KeLeaveCriticalRegion()
#else  // TOKEN_DIAGNOSTICS_ENABLED
...
NTSTATUS FORCEINLINE SepDuplicateLogonSessionReference(IN PTOKEN NewToken, IN PTOKEN ExistingToken)
{
    ...
    // Obtain the logon session reference.
    // If the existing token has a reference then use that to obtain a new one.
    // Otherwise Look up the session the slow way.
    if ((ExistingToken->TokenFlags & TOKEN_SESSION_NOT_REFERENCED) == 0) {
        ...
        Status = SepReferenceLogonSession (&ExistingToken->AuthenticationId, &NewToken->LogonSession);
...
VOID FORCEINLINE SepDeReferenceLogonSessionDirect(IN PSEP_LOGON_SESSION_REFERENCES LogonSession)
{
    LONG OldValue;
    LUID LogonId;
