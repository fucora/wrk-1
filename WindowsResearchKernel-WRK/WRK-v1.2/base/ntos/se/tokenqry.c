...
Abstract:
    This module implements the QUERY function for the executive token object.
*/
...
*/
{
    ...
    case TokenGroups:
        ...
        SepAcquireTokenReadLock(Token);//  Gain exclusive access to the token.

        // Figure out how much space is needed to return the group SIDs.
        // That's the size of TOKEN_GROUPS (without any array entries) plus the size of an SID_AND_ATTRIBUTES times the number of groups.
        // The number of groups is Token->UserAndGroups-1 (since the count includes the user ID). 
        // Then the lengths of each individual group must be added.
    ...
    case TokenPrimaryGroup:
        ...
        //  Gain exclusive access to the token to prevent changes from occurring to the owner.
        SepAcquireTokenReadLock(Token);

        // Return the length required now in case not enough buffer was provided by the caller and we have to return an error.
    ...
    case TokenSource:
        ...
        // The type of a token can not be changed, so exclusive access to the token is not necessary.
    ...
    case TokenImpersonationLevel:
        ...
        // The impersonation level of a token can not be changed, so exclusive access to the token is not necessary.

        //  Make sure the token is an appropriate type to be retrieving the impersonation level from.
    ...
    case TokenGroupsAndPrivileges:
        ...
        RequiredLength = (ULONG)sizeof(TOKEN_GROUPS_AND_PRIVILEGES) + PrivilegesLength + RestrictedSidsLength + GroupsLength;
        ...
        // Now copy the groups, followed by restricted sids, followed by privileges.
        ...
        try {
            ...
            LocalGroupsAndPrivileges->Sids = (PSID_AND_ATTRIBUTES)((ULONG_PTR)LocalGroupsAndPrivileges + (ULONG)sizeof(TOKEN_GROUPS_AND_PRIVILEGES));
            ...
            // To distinguish between a restricted token with zero sids and a non-restricted token.

            if (SeTokenIsRestricted((PACCESS_TOKEN)Token)) {
                LocalGroupsAndPrivileges->RestrictedSids = (PSID_AND_ATTRIBUTES)((ULONG_PTR)LocalGroupsAndPrivileges->Sids + GroupsLength);
            } else {
    ...
    case TokenAuditPolicy:
    {
        ...
        // Figure out how much space is needed to return the audit policy.  Count the policy elements present in the token.
...
NTSTATUS SeQueryAuthenticationIdToken(__in PACCESS_TOKEN Token, __out PLUID AuthenticationId)
/*
Routine Description:
...
*/
{
    ...
    // Token AuthenticationId is a readonly field. No locks are required to read this as its constant for the life of the token.
...
*/
{
    ...
    case TokenOwner:
    {
        ...
        SepAcquireTokenReadLock(Token);//  Gain exclusive access to the token to prevent changes from occurring to the owner.
...
NTSTATUS SeQuerySessionIdToken(__in PACCESS_TOKEN Token, __out PULONG SessionId)
/*
Routine Description:
