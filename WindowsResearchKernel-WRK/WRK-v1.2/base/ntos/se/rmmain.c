...
/*
Routine Description:
    This function is called by Phase 1 System Initialization to initialize the Security Reference Monitor.
    Note that initialization of the Reference Monitor Global State has already been performed in Phase 0 initialization to allow access validation routines to operate without
    having to check that Reference Monitor Initialization is complete.

    The steps listed below are performed in this routine.
    The remainder of Reference Monitor initialization requires the LSA subsystem to have run,
    so that initialization is performed in a separate thread (the RM Command Server Thread, see below),
    so that the present thread can create the Session Manager which execs the LSA.

    o Create the Reference Monitor Command LPC port.  The LSA subsystem sends commands (e.g. turn on auditing) which change the Reference Monitor Global State.
    o Create an Event for use in synchronizing with the LSA subsystem.
      The LSA will signal the event when the portion of LSA initialization upon with the Reference Monitor depends is complete.
      The Reference Monitor uses another LPC port, called the LSA Command Port to send commands to the LSA,
      so the RM must know that this port has been created before trying to connect to it.
    o Create the Reference Monitor Command Server Thread.
      This thread is a permanent thread of the System Init process that fields the Reference Monitor State Change commands described above.
Return Value:
...
*/
{
    ...
    InitializeObjectAttributes(&LsaInitEventObjectAttributes, &LsaInitEventName, 0, NULL, &LsaInitEventSecurityDescriptor);

    // Create an event for use in synchronizing with the LSA.  
    // The LSA will signal this event when LSA initialization has reached the point where the LSA's Reference Monitor Server Port has been created.
...
/*
Routine Description:
    This function is executed indefinitely by a dedicated permanent thread of the Sysinit Process, called the Reference Monitor Server Thread.
    This thread updates Reference Monitor Global State Data by dispatching commands sent from the LSA through the the Reference Monitor LPC Command Port.
    The following steps are repeated indefinitely:
    ...
    o  Wait for RM command sent from LSA, send reply to previous command (if any)
    o  Validate command
...
*/
{
    ...
    // Perform the rest of the Reference Monitor initialization, involving synchronization with the LSA or dependency on the LSA having run.
    ...
    // Initialize LPC port message header type and length fields for the received command message.
    ...
    CommandMessage.MessageHeader.u1.s1.DataLength = CommandMessage.MessageHeader.u1.s1.TotalLength - (CSHORT) sizeof(PORT_MESSAGE);

    // Initialize the LPC port message header type and data sizes for for the reply message.
    ReplyMessage.MessageHeader.u2.ZeroInit = 0;
    ReplyMessage.MessageHeader.u1.s1.TotalLength = (CSHORT) sizeof(RM_COMMAND_MESSAGE);
    ReplyMessage.MessageHeader.u1.s1.DataLength = ReplyMessage.MessageHeader.u1.s1.TotalLength - (CSHORT) sizeof(PORT_MESSAGE);

    Reply = NULL;// First time through, there is no reply.
    ...
    for (;;) {
        ...
        // Now dispatch to a routine to handle the command.  Allow command errors to occur without bringing system down just now.
        ...
        if (CommandMessage.MessageHeader.u2.s2.Type == LPC_REQUEST) {
            if ((CommandMessage.CommandNumber >= RmAuditSetCommand) && (CommandMessage.CommandNumber <= RmDeleteLogonSession)) {
                ...
                // Initialize the client thread info and message id for the reply message.  First time through, the reply message structure is not used.
                ...
                ASSERT((CommandMessage.CommandNumber >= RmAuditSetCommand) && (CommandMessage.CommandNumber <= RmDeleteLogonSession));
                Reply = NULL;
            }
            ...
            HANDLE tmp;
            Status = ZwAcceptConnectPort(&tmp, NULL, (PPORT_MESSAGE)&CommandMessage, FALSE, NULL, NULL);// Reject extra connection attempts
        } else {
            Reply = NULL;
        }
    }
...
Routine Description:
    ...
    o  Wait on the LSA signaling the event.  When the event is signaled, the LSA has already created the LSA Command Server LPC Port
...
*/
{
    ...
    // Save a pointer to our process so we can get back into this process to send commands to the LSA (using a handle to an LPC port created below).
    ...
    // Set up the security quality of service parameters to use over the Lsa Command LPC port.
    // Use the most efficient (least overhead) - which is dynamic rather than static tracking.
    DynamicQos.ImpersonationLevel = SecurityImpersonation;
    DynamicQos.ContextTrackingMode = SECURITY_DYNAMIC_TRACKING;
    DynamicQos.EffectiveOnly = TRUE;

    // Create the section to be used as unnamed shared memory for communication between the RM and LSA.
    ...
    // This setup includes a description of the port memory section so that the LPC connection logic can make the section visible to both the client and server processes.
    ...
    // Use dynamic tracking so that XACTSRV will impersonate the user that we are impersonating when we call NtRequestWaitReplyPort.
    // If we used static tracking, XACTSRV would impersonate the context when the connection is made.
    DynamicQos.ImpersonationLevel = SecurityImpersonation;
    DynamicQos.ContextTrackingMode = SECURITY_DYNAMIC_TRACKING;
    DynamicQos.EffectiveOnly = TRUE;

    // Connect to the Lsa Command LPC Port.  This port is used to send commands from the RM to the LSA.
    ...
    // Store information about the section so that we can create pointers meaningful to LSA.
    SepRmState.RmViewPortMemory = ClientView.ViewBase;
    SepRmState.LsaCommandPortMemoryDelta = (LONG)((ULONG_PTR)ClientView.ViewRemoteBase - (ULONG_PTR)ClientView.ViewBase);
    SepRmState.LsaViewPortMemory = ClientView.ViewRemoteBase;

RmCommandServerThreadInitFinish:
    // Dont need this section handle any more, even if returning success.
...
/*
Routine Description:
    This function sends a command to the LSA via the LSA Reference Monitor Server Command LPC Port.
    If the command has parameters, they will be copied directly into a message structure and sent via LPC,
    therefore, the supplied parameters may not contain any absolute pointers.
    A caller must remove pointers by "marshalling" them into the buffer CommandParams.

    This function will create a queue of requests.
    This is in order to allow greater throughput for the majority if its callers.
    If a thread enters this routine and finds the queue empty,
    it is the responsibility of that thread to service all requests that come in while it is working until the queue is empty again.
    Other threads that enter will simply hook their work item onto the queue and exit.

    To implement a new LSA command, do the following:
    ================================================
    (1)  If the command takes no parameters, just call this routine directly and provide an LSA worker routine called Lsap<command>Wrkr.
         See file lsa\server\lsarm.c for examples
    (2)  If the command takes parameters,
         provide a routine called SepRmSend<command>Command that takes the parameters in unmarshalled form and calls SepRmCallLsa() with the command id, marshalled
         parameters, length  of marshalled parameters and pointer to optional reply message.
         The marshalled parameters are free format:
         the only restriction is that there must be no absolute address pointers.
         These parameters are all placed in the passed LsaWorkItem structure.
    (3)  In file private\inc\ntrmlsa.h, append a command name  to the enumerated type LSA_COMMAND_NUMBER defined in file private\inc\ntrmlsa.h.
         Change the #define for LsapMaximumCommand to reference the new command.
...
Arguments:
    LsaWorkItem - Supplies a pointer to an SE_LSA_WORK_ITEM containing the information to be passed to LSA.
                  This structure will be freed asynchronously by some invocation of this routine, not necessarily in the current context.
...
*/
{
    ...
    while (WorkQueueItem) {
        // Construct a message for LPC.  
        // First, fill in the message header fields for LPC, specifying the message type and data sizes for the outgoing CommandMessage and the incoming ReplyMessage.
        CommandMessage.MessageHeader.u2.ZeroInit = 0;
        CommandMessage.MessageHeader.u1.s1.TotalLength = ((CSHORT)RM_COMMAND_MESSAGE_HEADER_SIZE + (CSHORT)WorkQueueItem->CommandParamsLength);
        CommandMessage.MessageHeader.u1.s1.DataLength = CommandMessage.MessageHeader.u1.s1.TotalLength - (CSHORT) sizeof(PORT_MESSAGE);

        ReplyMessage.MessageHeader.u2.ZeroInit = 0;
        ReplyMessage.MessageHeader.u1.s1.DataLength = (CSHORT)WorkQueueItem->ReplyBufferLength;
        ReplyMessage.MessageHeader.u1.s1.TotalLength = ReplyMessage.MessageHeader.u1.s1.DataLength + (CSHORT) sizeof(PORT_MESSAGE);
        ...
        // Set up the Command Parameters either in the LPC Command Message itself, in the preallocated Lsa shared memory block, or in a specially allocated block.
        // The parameters are either immediate (i.e. in the WorkQueueItem itself, or are in a buffer pointed to by the address in the WorkQueueItem.
        switch (WorkQueueItem->CommandParamsMemoryType) {
        case SepRmImmediateMemory:
            // The Command Parameters are in the CommandParams buffer in the Work Queue Item.  
            // Just copy them to the corresponding buffer in the CommandMessage buffer.
        ...
        case SepRmUnspecifiedMemory:
            ...
            // Since this memory is is not accessible by the LSA, we must copy of them either to the LPC Command Message Block, or into LSA shared memory.
            if (WorkQueueItem->CommandParamsLength <= LSA_MAXIMUM_COMMAND_PARAM_SIZE) {
                ...
                if (WorkQueueItem->CommandParamsLength <= SEP_RM_LSA_SHARED_MEMORY_SIZE) {
                    ...
                    if (!NT_SUCCESS(Status)) {
                        ...
                        // Later, we may consider generating a warning record indicating some records lost.
                        break;
                    }
                ...
                // Buffer has been successfully copied to a shared Lsa memory buffer.  
                // Place the address of the buffer valid in the LSA's process context in the Command Message.
                *((PVOID *)CommandMessage.CommandParams) = LsaViewCopiedCommandParams;
                CommandMessage.MessageHeader.u1.s1.TotalLength = ((CSHORT)RM_COMMAND_MESSAGE_HEADER_SIZE + (CSHORT) sizeof(LsaViewCopiedCommandParams));
                CommandMessage.MessageHeader.u1.s1.DataLength = CommandMessage.MessageHeader.u1.s1.TotalLength - (CSHORT) sizeof(PORT_MESSAGE);
            }
        ...
        if (NT_SUCCESS(Status)) {
            ...
            if (WorkQueueItem->CommandNumber == LsapLogonSessionDeletedCommand && WorkQueueItem->ReplyBuffer == NULL) {
                // send a datagram.
                Status = ZwRequestPort(SepRmState.LsaCommandPortHandle, (PPORT_MESSAGE)&CommandMessage);
                ReplyMessage.ReturnedStatus = STATUS_SUCCESS;
            } else {
                Status = ZwRequestWaitReplyPort(SepRmState.LsaCommandPortHandle, (PPORT_MESSAGE)&CommandMessage, (PPORT_MESSAGE)&ReplyMessage);
            }

            // If the command was successful, copy the data back to the output buffer.
            if (NT_SUCCESS(Status)) {
                // Move output from command (if any) to buffer.  
                // Note that this is done even if the command returns status, because some status values are not errors.
            ...
            // On return from the LPC call to the LSA, we expect the called LSA worker routine to have copied the Command Parameters buffer (if any).
            // If a custom shared memory buffer was allocated, free it now.
        ...
        // Clean up.  We must call the cleanup functions on its parameter and then free the used WorkQueueItem itself.
...
Routine Description:
    ...
    This includes initializing the reference monitor database to a state which allows access validation routines to operate (always granting
    access) prior to the main init of the Reference Monitor in Phase 1 initialization, without having to check if the RM is initialized.
Return Value:
