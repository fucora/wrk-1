...
Abstract:
   ...
   These workers call functions in the Auditing sub-component to do the real work.
*/
...
/*
Routine Description:
    This function carries out the Reference Monitor Set Audit Event Command.
    This command enables or disables auditing and optionally sets the auditing events.
Arguments:
    CommandMessage - Pointer to structure containing RM command message information consisting of an LPC PORT_MESSAGE structure followed
        by the command number (RmSetAuditStateCommand) and a single command parameter in structure form.
    ReplyMessage - Pointer to structure containing RM reply message information consisting of an LPC PORT_MESSAGE structure followed
...
*/
{
    ...
    // For each element in the passed array, process changes to audit nothing, and then success or failure flags.
