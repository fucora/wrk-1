...
Abstract:
    ...
    Both NtAccessCheck and SeAccessCheck check to is if a user (denoted by an input token) can be granted the desired access rights to object protected by
    a security descriptor and an optional object owner.
...
NTSTATUS SeCaptureObjectTypeList(
...
Routine Description:
    ...
    The object types list is validated to ensure a valid hierarchical relationship is represented.
Arguments:
...
*/
{
    ...
    *CapturedObjectTypeList = NULL;//  Set default return
    ...
    try {
        if (ObjectTypeListLength == 0) {
            ...
            if (!IsValidElementCount(ObjectTypeListLength, IOBJECT_TYPE_LIST)) {
                Status = STATUS_INVALID_PARAMETER;
                leave;// No more to do, get out of the try statement:
            }
            ...
            if (LocalTypeList == NULL) {
                ...
                for (i = 0; i < ObjectTypeListLength; i++) {
                    ...
                    if (i == 0) {
                        ...
                        //  my immediate parent,
                        //  my sibling, or the child (or grandchild, etc.) of my sibling.
                    ...
                    // If the above rules are maintained, then my parent object is the last object seen that has a level one less than mine.
                    if (CurrentLevel == 0) {
                    ...
                    Levels[CurrentLevel] = i;// Save this object as the last object seen at this level.
                }
            }
        } // end_if
...
VOID SeFreeCapturedObjectTypeList(__in PVOID ObjectTypeList)
/*
Routine Description:
...
*/
{
    ...
    // If the target node is at the root, we're all done.
    ...
    for (Index = ParentIndex + 1; Index < ObjectTypeListLength; Index++) {
        ...
        // The list of children (or grandchildren) stops as soon as we reach an entry the has the
        // same level as the target (a sibling) or lower than the target (an uncle).
    ...
    // If we've not changed the access to the parent, we're done.
...
Arguments:
    ...
    AccessMask - Mask of access to grant to the target element and all of its descendents
...
*/
{
    ...
    case UpdateCurrentGranted:
        ...
        if (OldCurrentGranted == ObjectTypeList[StartIndex].CurrentGranted) {
            ...
            // We have to visit our children. 
            // Consider the case where there was a previous deny ACE on a child. 
            // That deny would have propagated up the tree to this entry. 
            // However, this allow ACE needs to be added all of the children that haven't been explictly denied.
            AvoidParent = TRUE;
        }
        break;
    case UpdateCurrentDenied:
        ...
        if (OldCurrentDenied == ObjectTypeList[StartIndex].CurrentDenied) {
            // We must visit the children - consider the case where we are applying an ACCESS_DENIED Ace to the entire tree.
            // Suppose a node of the tree was already DENIED the same bits via an ACCESS_DENIED_OBJECT_ACE. 
            // Those deny bits would have propagated up the tree. 
            // If we are denying the bits to this node then we must visit the children.
            AvoidParent = TRUE;
        }
        break;
    default:
...
*/
{
    ...
    // If Sid is the constant PrincipalSelfSid, replace it with the passed in PrincipalSelfSid.
    if (PrincipalSelfSid != NULL && RtlEqualSid(SePrincipalSelfSid, Sid)) {
        Sid = PrincipalSelfSid;
    }

    SidLength = 8 + (4 * ((PISID)Sid)->SubAuthorityCount);// Get the length of the source SID since this only needs to be computed once.    
    TargetShort = *(USHORT *)&((PISID)Sid)->Revision;// To speed up processing we compare the sub authority count and the revision at the same time.
    ...
    for (i = 0; i < UserAndGroupCount; i += 1) {
        ...
        // If revision and sub authority count matches, then compare the SIDs for equality.
        if (*(USHORT *)&MatchSid->Revision == TargetShort) {
            if (RtlEqualMemory(Sid, MatchSid, SidLength)) {
                ...
                // If this is not the first one, then it represents a group, and we must make sure the group is currently enabled before
...
*/
{
    ...
    // The remaining bits are the granted bits for each object type on a restricted check
    if (Restricted) {
        ...
        if (!(((PACE_HEADER)Ace)->AceFlags & INHERIT_ONLY_ACE)) {
            if ((((PACE_HEADER)Ace)->AceType == ACCESS_ALLOWED_ACE_TYPE)) {
                if (SepSidInTokenEx(EToken, PrincipalSelfSid, &((PACCESS_ALLOWED_ACE)Ace)->SidStart, FALSE, Restricted)) {
                    // Only grant access types from this mask that have not already been denied
               ...
               // If an object type is in the ACE, Find it in the LocalTypeList before using the ACE.
                ...
                //  According to the DSA architecture, if we're asked to evaluate a compound ACE and we're not impersonating,
                //  pretend we are impersonating ourselves.  So we can just use the EToken for the client token, since it's already set to the right thing.
                ...
                if (SepSidInTokenEx(EToken, PrincipalSelfSid, &((PACCESS_DENIED_ACE)Ace)->SidStart, TRUE, Restricted)) {
                    // Only deny access types from this mask that have not already been granted
    ...
    // If this check was performed on the restricted SIDs, then CurrentGranted contains the MAXIMUM_ALLOWED access for the set of SIDs. 
    // A check on the restricted SIDs must be constrained by the check on the normal SIDs, so AND the results together now.
    if (Restricted) {
        for (j = 0; j < LocalTypeListLength; j++) {
...
*/
{
    ...
    for (i = 0, Ace = FirstAce(Dacl); (i < AceCount) && (LocalTypeList->Remaining != 0); i++, Ace = NextAce(Ace)) {
        if (!(((PACE_HEADER)Ace)->AceFlags & INHERIT_ONLY_ACE)) {
            ...
            } else if ((((PACE_HEADER)Ace)->AceType == ACCESS_ALLOWED_OBJECT_ACE_TYPE)) {// Handle an object specific Access Allowed ACE
                GUID *ObjectTypeInAce;
            ...
            } else if ((((PACE_HEADER)Ace)->AceType == ACCESS_ALLOWED_COMPOUND_ACE_TYPE)) {// Handle a compound Access Allowed ACE
            ...
            } else if ((((PACE_HEADER)Ace)->AceType == ACCESS_DENIED_ACE_TYPE)) {// Handle an Access Denied ACE
                if (SepSidInTokenEx(EToken, PrincipalSelfSid, &((PACCESS_DENIED_ACE)Ace)->SidStart, TRUE, Restricted)) {
            ...
            } else if ((((PACE_HEADER)Ace)->AceType == ACCESS_DENIED_OBJECT_ACE_TYPE)) {// Handle an object specific Access Denied ACE
                if (SepSidInTokenEx(EToken, PrincipalSelfSid, RtlObjectAceSid(Ace), TRUE, Restricted)) {
                    GUID *ObjectTypeInAce;
...
/*
Routine Description:
    This routine takes the passed security descriptor and applies the "MAXIMUM_ALLOWED" algorithm to the SACL contained in the security descriptor if one exists.
    This mask represents all the success audits that can occur from the passed subject context accessing the passed security descriptor.

    The code walks the SACL and for each SYSTEM_AUDIT_ACE found that matches the passed subject context, keeps a running total of the access bits in the ACE.
    The resulting mask is then masked by the passed GrantedAccess mask, since we're only interested in the bits that the object is actually being opened for.
Arguments:
...
*/
{
    ...
    *pAuditMask = (ACCESS_MASK)0;// Initialize OUT parameters    

    if (0 == (AceCount = Sacl->AceCount)) {// Determine if there is an SACL in the security descriptor.
        return;// If not, nothing to do.
    }

    // Iterate through the ACEs on the Sacl until either we reach the end or discover that we have to take all possible actions,
...
Routine Description:
    ...
    Whether or not we actually evaluate the DACL is based on the following interaction between the SE_DACL_PRESENT bit in the security descriptor and
    the value of the DACL pointer itself.
...
Arguments:
    ...
    PrincipalSelfSid - If the object being access checked is an object which represents a principal (e.g., a user object),
        this parameter should be the SID of the object.
    ...
    DesiredAccess - Access mask describing the user's desired access to the object.
                    This mask is assumed not to contain generic access types.
    ObjectTypeList - Supplies a list of GUIDs representing the object (and sub-objects) being accessed.
                     If no list is present, AccessCheckByType behaves identically to AccessCheck.
    ...
    Privileges - Optionally supplies a pointer in which will be returned any privileges that were used for the access.
                 If this is null, it will be assumed that privilege checks have been done already.
    AccessStatus - Returns STATUS_SUCCESS or other error code to be propagated back to the caller
    ...
    ReturnSomeAccessDenied - Returns a value of FALSE if some of the requested access was not granted.
                             This will always be an inverse of SomeAccessGranted unless ReturnResultList is TRUE.  In that case,
Return Value:
...
*/
{
    ...
    SeAssertMappedCanonicalAccess(DesiredAccess);// Assert that there are no generic accesses in the DesiredAccess
    Remaining = DesiredAccess;

    // Check for ACCESS_SYSTEM_SECURITY here, fail if he's asking for it and doesn't have the privilege.
    if (Remaining & ACCESS_SYSTEM_SECURITY) {
        // Bugcheck if we weren't given a pointer to return privileges into.  Our caller was supposed to have taken care of this in that case.
    ...
    Dacl = RtlpDaclAddrSecurityDescriptor((PISECURITY_DESCRIPTOR)SecurityDescriptor);// Get pointer to client SID's

    //  If the SE_DACL_PRESENT bit is not set, the object has no security, so all accesses are granted. 
    //  If he's asking for MAXIMUM_ALLOWED, return the GENERIC_ALL field from the generic mapping.
    ...
    if (!RtlpAreControlBitsSet((PISECURITY_DESCRIPTOR)SecurityDescriptor, SE_DACL_PRESENT) || (Dacl == NULL)) {
        ...
        if (SeTokenIsRestricted(EToken)) {
            ...
            // There are ungranted accesses. 
            // Since there is nothing in the DACL, they will not be granted.
            // If, however, the only ungranted access at this point is MAXIMUM_ALLOWED, 
            // and something has been granted in the PreviouslyGranted mask, return what has been granted.
    ...
    if ((AceCount = Dacl->AceCount) == 0) {// If the DACL is empty, deny all access immediately.
        // We know that Remaining != 0 here, because we know it was non-zero coming into this routine, 
        // and we've checked it against 0 every time we've cleared a bit.
        ASSERT(Remaining != 0);
    ...
    if ((DesiredAccess & MAXIMUM_ALLOWED) != 0 || ReturnResultList) {
        ...
        if (SeTokenIsRestricted(EToken)) {// If this is a restricted token, do the additional access check
        ...
        // If the caller wants to know the individual results of each sub-object, sub-object, break it down for him.
        if (ReturnResultList) {
            ...
            for (ResultListIndex = 0; ResultListIndex < LocalTypeListLength; ResultListIndex++) {
                ...
                if (GrantedAccess[ResultListIndex] == 0) {// If absolutely no access was granted, indicate so.
                    AccessStatus[ResultListIndex] = STATUS_ACCESS_DENIED;
                    SomeAccessDenied = TRUE;
                } else {// If some requested access is still missing, the bottom line is that access is denied.
            ...
            return RetVal;
        } else {// If the caller is only interested in the access to the object itself, just summarize.
            // Turn off the MAXIMUM_ALLOWED bit and whatever we found that he was granted. 
            // If the user passed in extra bits in addition to MAXIMUM_ALLOWED, make sure that he was granted those access types. 
            // If not, he didn't get what he wanted, so return failure.
...
*/
{
    ...
    try {
        ...
        // initialize PrivilegeCount in case the caller passed in an uninitialized PrivilegeSet
        if (PrivilegeSet && (LocalPrivilegeSetLength >= sizeof(PRIVILEGE_SET))) {
    ...
    if (Token->TokenType != TokenImpersonation) {// It must be an impersonation token, and at impersonation level of Identification or above.
    ...
    Status = SeCaptureObjectTypeList(ObjectTypeList, ObjectTypeListLength, PreviousMode, &LocalObjectTypeList);// Capture any Object type list
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }

    // Compare the DesiredAccess with the privileges in the passed token, and see if we can either satisfy the requested access with a privilege,
    // or bomb out immediately because we don't have a privilege we need.
    ...
    if (DesiredAccess == 0) {
        try {
            if (ReturnResultList) {
                for (ResultListIndex = 0; ResultListIndex < ObjectTypeListLength; ResultListIndex++) {
                    if (PreviouslyGrantedAccess == 0) {// Do not allow the request to go thru if the granted access evaluated to ZERO.
                        AccessStatus[ResultListIndex] = STATUS_ACCESS_DENIED;
                        GrantedAccess[ResultListIndex] = 0;
                    } else {
                        AccessStatus[ResultListIndex] = STATUS_SUCCESS;
    ...
    // This does not ask for privilege set to be returned so we can ignore the return value of the call.
    (VOID)SepAccessCheck(
...
Cleanup:// Free locally used resources.
    if (Token != NULL) {
...
VOID SeFreePrivileges(__in PPRIVILEGE_SET Privileges)
/*
Routine Description:
...
Arguments:
    ...
    SubjectContextLocked - Supplies a flag indicating whether or not the user's subject context is locked, so that it does not have to be locked again.
...
*/
{
    ...
    if (SecurityDescriptor == NULL) {// If the object doesn't have a security descriptor (and it's supposed to), return access denied.
        *AccessStatus = STATUS_ACCESS_DENIED;
        return(FALSE);
    }
    ...
    if (!SubjectContextLocked) {// If the caller did not lock the subject context for us, lock it here to keep lower level routines from having to lock it.
    ...
    if (DesiredAccess == 0) {
        ...
        *AccessStatus = STATUS_SUCCESS;
        ...
        if (!SubjectContextLocked) {// If we locked it in this routine, unlock it before we leave.
            SeUnlockSubjectContext(SubjectSecurityContext);
        }

        // We return failure if any of the following is TRUE
        //   1. The user was really not granted access.
        //   2. The resource manager asked for the list of privileges used to determine granted access and we failed to allocate memory required to return these.
        return(b && Success);
    }
}
...
Arguments:
    ...
    PreviouslyGrantedAccess - Supplies an access mask describing any accesses that have already been granted.
        Bits may be set in here as a result of privilege checks.
...
/*
Routine Description:
    This routine will determine of the Owner of the passed security descriptor is in the passed token. If the token is restricted it cannot be the owner.
Arguments:
...
BOOLEAN SeFastTraverseCheck(
    ...
    /*
    Routine Description:
        This routine will perform a fast check against the DACL of the passed Security Descriptor to see if Traverse access would be granted.
        If so, no further access checking is necessary.

        Note that the SubjectContext for the client process does not have to be locked to make this call,
        since it does not examine any data structures in the Token.

        The caller has the following responsibilities:
        1. It is the job of the caller to verify AccessMode is not KernelMode!
        2. The *caller* is expected to check AccessState for TOKEN_HAS_TRAVERSE_PRIVILEGE if the override is applicable!

    Arguments:
        SecurityDescriptor - The Security Descriptor protecting the container object being traversed.
        AccessState - Running security access state containing caller token information for operation.
        TraverseAccess - Access mask describing Traverse access for this object type. There must be only one bit specified in the mask.
        AccessMode - Supplies the access mode to be used in the check
    Return Value:
    ...
    */
{
    ...
    // Note that I/O calls this function even if the traverse bypass privilege is set.
    // This is done as I/O doesn't want the performance override to apply to the DeviceObject->FileName boundary, 
    // as not all filesystems supply file-level security.
    ...
    // No DACL, no security, all is granted. Note that this function returns NULL if SE_DACL_PRESENT isn't set.
    Dacl = RtlpDaclAddrSecurityDescriptor((PISECURITY_DESCRIPTOR)SecurityDescriptor);
    if (Dacl == NULL) {// If no DACL is supplied, the object has no security, so all accesses are granted.
        return(TRUE);
    }

    if ((AceCount = Dacl->AceCount) == 0) {// There is security on this object.  If the DACL is empty, deny all access immediately
        return(FALSE);
    }

    // A restricted token is a token with two lists of SIDs.
    // Access checks are done against each list, passing only if both lists grant access.
    // The second "restricting SID" list can contain *any allow* SID.

    // This routine doesn't walk a restricted token's restricting-SID list.
    // As such, restricted tokens require a full access check.
    if (AccessState->Flags & TOKEN_IS_RESTRICTED) {
        return FALSE;
    }

    // There's stuff in the DACL, walk down the list and see if both Everyone and Anonymous have been granted TraverseAccess
    ...
    for (i = 0, Ace = FirstAce(Dacl); i < AceCount; i++, Ace = NextAce(Ace)) {
        ...
        if ((((PACE_HEADER)Ace)->AceType == ACCESS_ALLOWED_ACE_TYPE)) {
            ...
            if ((TraverseAccess & ((PACCESS_DENIED_ACE)Ace)->Mask)) {
                return(FALSE);// This ACE might refer to a group the user belongs to (or could be the user). Force a full access check.
            }
        }
    }
