...
POBJECT_TYPE SeTokenObjectType = NULL;// Address of token object type descriptor.
...
TOKEN_TYPE SeTokenType(__in PACCESS_TOKEN Token)
/*
Routine Description:
...
NTKERNELAPI BOOLEAN SeTokenIsAdmin(__in PACCESS_TOKEN Token)
/*
Routine Description:
...
NTKERNELAPI BOOLEAN SeTokenIsRestricted(__in PACCESS_TOKEN Token)
/*
Routine Description:
...
/*
Routine Description:
    This function returns the impersonation level of a token.
    The token is assumed to be a TokenImpersonation type token.
Arguments:
...
BOOLEAN SepCheckTokenForCoreSystemSids(__in PACCESS_TOKEN Token)
/*
Routine Description:
    Perform an access-check against SepImportantProcessSd to determine if the passed token has at least one of the sids present in the ACEs of SepImportantProcessSd.
Arguments:
    Token - a token
Return Value:
    TRUE if Token has at least one of the required SIDs, FALSE otherwise
*/
{
...
*/
{
    ...
    // quickly return if this feature is disabled (indicated by SeProcessAuditSd == NULL)
    if (SepProcessAuditSd == NULL) {
        return;
    }

    // if the token does not have core system sids then return without adding SACL.
    ...
    if (!NT_SUCCESS(Status)) {
        // STATUS_NO_SECURITY_ON_OBJECT should be returned only once during boot when the initial system process is created.
...
*/
{
    ...
    // If the token being assigned to the child process has any one of the following SIDs, then the process is considered to be a system process:
    // -- SeLocalSystemSid
    // -- SeLocalServiceSid
    // -- SeNetworkServiceSid

    // For such a process, add SACL to its security descriptor if that option is enabled.
    // If the option is disabled, this function returns very quickly.

    // Dereference the old token if there is one.

    // Processes typically already have a token that must be dereferenced.
    // There are two cases where this may not be the situation.
    // First, during phase 0 system initialization, the initial system process starts out without a token.
    // Second, if an error occurs during process creation, we may be cleaning up a process that hasn't yet had a primary token assigned.
...
/*
Routine Description:
    This function causes a process reference to a token to be dropped.
Arguments:
    Process - Points to the process whose primary token is no longer needed.
        This is probably only the case at process deletion or when a primary token is being replaced.
*/
{
    PTOKEN OldToken = (PTOKEN)ObFastReplaceObject(&Process->Token, NULL);
...
Arguments:
    ...
    OldAccessToken - Receives a pointer to the process's current token.
        The caller is responsible for dereferencing this token when it is no longer needed.
        This can't be done while the process security locks are held.
Return Value:
    STATUS_SUCCESS - Everything has been updated.
    STATUS_TOKEN_ALREADY_IN_USE - A primary token can only be used by a single process.
        That is, each process must have its own primary token.
        The token passed to  be assigned as the primary token is already in use as a primary token.
...
*/
{
    ...
    // Return the pointer to the old token.  The caller is responsible for dereferencing it if they don't need it.
...
Routine Description:
    ...
    This routine will also be useful to communications subsystems that need to retrieve client' authentication information from
    the local security authority in order to perform a remote authentication.
Parameters:
...
*/
{
    ...
    Privileges[22].Attributes = (SE_PRIVILEGE_ENABLED_BY_DEFAULT | SE_PRIVILEGE_ENABLED);

    Privileges[23].Luid = SeCreateGlobalPrivilege;
    Privileges[23].Attributes = (SE_PRIVILEGE_ENABLED_BY_DEFAULT | SE_PRIVILEGE_ENABLED);
...
Return Value:
    STATUS_SUCCESS - Indicates the sub-process's token has been created successfully.
    Other status values may be returned from memory allocation or object creation services used and typically indicate insufficient resources or quota on the requestor's part.
*/
{
    PTOKEN NewToken;
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (NT_SUCCESS(Status)) {
            ...
            *ChildToken = NewToken;
            ...
            // ObInsertObject dereferences the passed object if it fails, so we don't have to do any cleanup on NewToken here.
        }
    }
...
/*
Routine Description:
    This function creates the token object type descriptor at system initialization and stores the address of the object type descriptor
    in global storage.  It also created token related global variables.

    Furthermore, some number of pseudo tokens are created during system initialization.
    These tokens are tracked down and replaced with real tokens.
Return Value:
    A value of TRUE is returned if the object type descriptor is successfully initialized. Otherwise a value of FALSE is returned.
*/
{
    ...
    RtlInitUnicodeString(&TypeName, L"Token");// Initialize string descriptor.
    ...
    Status = ObCreateObjectType(&TypeName, &ObjectTypeInitializer, (PSECURITY_DESCRIPTOR)NULL, &SeTokenObjectType);//Create object type descriptor.

    // If the object type descriptor was successfully created, then return a value of TRUE. Otherwise return a value of FALSE.
...
/*
Routine Description:
    Create a token object and return a handle opened for access to that token.
    This API requires SeCreateTokenPrivilege privilege.
Arguments:
    TokenHandle - Receives the handle of the newly created token.

    DesiredAccess - Is an access mask indicating which access types the handle is to provide to the new object.

    ObjectAttributes - Points to the standard object attributes data
        structure.  Refer to the NT Object Management Specification for a description of this data structure.

        If the token type is TokenImpersonation, then this parameter must specify the impersonation level of the token.

    TokenType - Type of token to be created.  Privilege is required to create any type of token.
    ...
    Groups - Are the group SIDs to place in the token. The API assumes that the caller has not supplied duplicate group sids.

    Privileges - Are the privileges to place in the token. The API assumes that the caller has not supplied duplicate privileges.
    ...
    DefaultDacl - (optionally) establishes an ACL to be used as the default discretionary access protection for the token.

    TokenSource - Identifies the token source name string and identifier to be assigned to the token.

Return Value:
    STATUS_SUCCESS - Indicates the operation was successful.
    STATUS_INVALID_OWNER - Indicates the ID provided to be assigned as the default owner of the token does not have an attribute indicating it may be assigned as an owner.
    STATUS_INVALID_PRIMARY_GROUP - Indicates the group ID provided via the PrimaryGroup parameter was not among those assigned to the token in the Groups parameter.
    STATUS_BAD_IMPERSONATION_LEVEL - Indicates no impersonation level was provided when attempting to create a token of type TokenImpersonation.
*/
{
    ...
    try {
        ...
        if (ARGUMENT_PRESENT(DefaultDacl) && NT_SUCCESS(Status)) {
            ...
            if (CapturedAddress != NULL) {
                Status = SeCaptureAcl((PACL)CapturedAddress, PreviousMode, NULL, 0, NonPagedPool, TRUE, &CapturedDefaultDacl, &Ignore);
            }
        }
...
Routine Description:
    ...
    It is needed to ensure that all memory allocated for the token gets deallocated.
Arguments:
...
*/
{
    ...
    // If this token had an active SEP_AUDIT_POLICY then decrement the Token audit counter because this token is going byebye.
...
Routine Description:
    Create a token object and return a handle opened for access to that token.
    This API implements the bulk of the work needed for NtCreateToken.
    ...
    NOTE: This routine is also used to create the initial system token.
          In that case, the SystemToken parameter is TRUE and no handle is established to the token.
          Instead, a pointer to the token is returned via the TokenHandle parameter.
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
Arguments:
    ...
    ObjectAttributes - Points to the standard object attributes data structure.
        Refer to the NT Object Management Specification for a description of this data structure.
    ...
    GroupCount - Indicates the number of groups in the 'Groups' parameter.
        This value may be zero, in which case the 'Groups' parameter is ignored.

    Groups - Are the group SIDs, and their corresponding attributes, to place in the token.
...
Return Value:
    ...
    STATUS_INVALID_PARAMETER - Indicates that a required parameter, such as User or PrimaryGroup, was not provided with a legitimate value.
*/
{
    PTOKEN Token;
    NTSTATUS Status;
    ...
    // Make sure the Enabled and Enabled-by-default bits are set on every mandatory group.
    ...
    // be correctly aligned.  Align the Groups Length so that the SID_AND_ATTR array (which is
    ...
    Status = SepReferenceLogonSession(AuthenticationId, &Token->LogonSession);
    if (!NT_SUCCESS(Status)) {
    ...
    if (!SystemToken) {
        Status = SeCreateAccessState(&AccessState, &AuxData, DesiredAccess, &SeTokenObjectType->TypeInfo.GenericMapping);
        if (NT_SUCCESS(Status)) {
...
/*
Routine Description:
    This routine returns a boolean value indicating whether the user or group ID in the specified token with the specified index is assignable as the owner of an object.

    If the index is 0, which is always the USER ID, then the ID is assignable as owner.
    Otherwise, the ID is that of a group, and it must have the "Owner" attribute set to be assignable.
Arguments:
    Token - Pointer to a locked Token to use.
    Index - Index into the Token's UserAndGroupsArray.  This value is assumed to be valid.
Return Value:
    TRUE  - Indicates the index corresponds to an ID that may be assigned as the owner of objects.
    FALSE - Indicates the index does not correspond to an ID that may be assigned as the owner of objects.
*/
{
    PAGED_CODE();
...
/*
Routine Description:
    This routine returns TRUE if the supplied token is a child of the caller's process token.
    This is done by comparing the ParentTokenId field of the supplied token to the TokenId field of the token from the current subject context.
Arguments:
...
*/
{
    ...
    *IsChild = FALSE;
    ...
    if (NT_SUCCESS(Status)) {
        ...
        // Check to see if the supplied token's parent ID is the ID of the caller.
...
/*
Routine Description:
    This routine returns TRUE if the supplied token is a child of the caller's token.
    This is done by comparing the ParentTokenId field of the supplied token to the TokenId field of the token from the current subject context.
Arguments:
    Token - Token to check for childhood
    IsChild - Contains results of comparison.
...
/*
Routine Description:
    This routine returns TRUE if the supplied token is a sibling of the caller's process token.
    This is done by comparing the ParentTokenId field of the supplied token to the ParentTokenId field of the token from the current subject context.
    The authentication IDs of the tokens are also compared for equality.
Arguments:
...
*/
{
    ...
    *IsSibling = FALSE;
    ...
    if (NT_SUCCESS(Status)) {
        ...
        // identical authentication ids then one of the tokens is a duplicate of the other.
        if (RtlEqualLuid(&SuppliedParentTokenId, &CallerParentTokenId)) {
            if (RtlEqualLuid(&SuppliedAuthId, &CallerAuthId)) {
                *IsSibling = TRUE;
            }
        }
    }
...
Routine Description:
    This routine returns TRUE if the supplied token is a sibling of the caller's process token.
    This is done by comparing the ParentTokenId field of the supplied token to the ParentTokenId field of the token from the current subject context.
    The authentication IDs are also compared for equality.
Arguments:
    Token - Token to check for sibling relationship.
    IsSibling - Contains results of comparison.
        TRUE - The supplied token is a sibling of the caller's token
...
*/
{
    ...
    *IsSibling = FALSE;
    ...
    // If the tokens have identical parent token Id fields then one of the tokens is a duplicate of the other.
    if (RtlEqualLuid(&SuppliedParentTokenId, &CallerParentTokenId)) {
        if (RtlEqualLuid(&SuppliedAuthId, &CallerAuthId)) {
            *IsSibling = TRUE;
        }
...
#define EVERYONE_INCLUDES_ANONYMOUS 1
    ...
    // Reference the impersonation token to make sure we are allowed to impersonate it.
    ...
    // Do the impersonation. We want copy on open so the caller can't actually modify this system's copy of this token.
...
/*
Routine Description:
    This routine decides whether the given two tokens are equivalent from AccessCheck perspective.
    ...
    Two tokens are considered equal if all of the below are true.
      ...
      4. Every privilege present in the one token should be present in the other and vice-versa.

Arguments:
    FirstTokenHandle - Handle to the first token. The caller must have TOKEN_QUERY access to the token.
    SecondTokenHandle - Handle to the second token. The caller must have TOKEN_QUERY access to the token.
    Equal - To return whether the two tokens are equivalent from AccessCheck viewpoint.
Return Value:
    STATUS_SUCCESS - Indicates the operation was successful.
*/
{
    ...
    // Reference the first token handle with TOKEN_QUERY access so that it does not go away.
    Status = ObReferenceObjectByHandle(
    ...
    // Reference the second token handle with TOKEN_QUERY access so that it does not go away.
    Status = ObReferenceObjectByHandle(
    ...
    SepAcquireTokenReadLock(TokenOne);// Acquire read lock on the first token.
    SepAcquireTokenReadLock(TokenTwo);// Acquire read lock on the second token.
