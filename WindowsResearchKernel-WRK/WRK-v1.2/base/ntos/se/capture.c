...
Abstract:
    ...
    There are corresponding Release routines for the data structures that are captured into allocated pool.
*/
...
NTSTATUS SeCaptureSecurityDescriptor(__in PSECURITY_DESCRIPTOR InputSecurityDescriptor, 
...
/*
Routine Description:
    This routine probes and captures a copy of the security descriptor based upon the following tests.
...
Arguments:
    ...
    This parameter is assumed to have been provided by the mode specified in RequestorMode.

    RequestorMode - Specifies the caller's access mode.

    PoolType - Specifies which pool type to allocate the captured descriptor from

    ForceCapture - Specifies whether the input descriptor should always be captured

    OutputSecurityDescriptor - Supplies the address of a pointer to the output security descriptor.
        The captured descriptor will be self-relative format.

Return Value:
    STATUS_SUCCESS if the operation is successful.
    STATUS_INVALID_SID - An SID within the security descriptor is not a valid SID.
    STATUS_INVALID_ACL - An ACL within the security descriptor is not a valid ACL.
    STATUS_UNKNOWN_REVISION - The revision level of the security descriptor is not one known to this revision of the capture routine.
*/
{
#define SEP_USHORT_OVERFLOW ((ULONG) ((USHORT) -1))
    ...
    //  if the security descriptor is null then there is really nothing to capture
    if (InputSecurityDescriptor == NULL) {
        (*OutputSecurityDescriptor) = NULL;
        return STATUS_SUCCESS;
    }

    //  check if the requestors mode is kernel mode and we are not to force a capture
    if ((RequestorMode == KernelMode) && (ForceCapture == FALSE)) {
        //  Yes it is so we don't need to do any work and can simply return a pointer to the input descriptor
        (*OutputSecurityDescriptor) = InputSecurityDescriptor;
        return STATUS_SUCCESS;
    }

    //  We need to probe and capture the descriptor.
    //  To do this we need to probe the main security descriptor record first.
    if (RequestorMode != KernelMode) {
        ...
        try {
            ...
            // Verify the alignment is correct for absolute case. This is only needed when pointer are 64 bits.
    ...
    // captured main record to appear as an absolute form so we can use common code for both cases below.

    // Note that the fields of Captured are left pointing to user space addresses.  Treat them carefully.
    try {
    ...
    //  Indicate the size we are going to need to allocate for the captured acls
    SaclSize = 0;
    DaclSize = 0;
    ...
    //  Probe (if necessary) and capture each of the components of a SECURITY_DESCRIPTOR.
    ...
    if ((Captured.Control & SE_DACL_PRESENT) && (Captured.Dacl != NULL)) {
        ...
        Captured.Dacl = NULL;// Force the DACL to null if it is not present
    }
    ...
    //  If there is a System Acl, copy it over and set the output descriptor's offset to point to the newly captured copy.
    if ((Captured.Control & SE_SACL_PRESENT) && (Captured.Sacl != NULL)) {
        try {
    ...
    //  If there is a Discretionary Acl, copy it over and set the output descriptor's offset to point to the newly captured copy.
    if ((Captured.Control & SE_DACL_PRESENT) && (Captured.Dacl != NULL)) {
        try {
    ...
    //  If there is an Owner SID, copy it over and set the output descriptor's offset to point to the newly captured copy.
    if (Captured.Owner != NULL) {
        try {
    ...
    //  If there is a group SID, copy it over and set the output descriptor's offset to point to the newly captured copy.
    if (Captured.Group != NULL) {
        try {
...
Arguments:
    ...
    RequestorMode - The processor mode specified when the descriptor was captured.
    ForceCapture - The ForceCapture value specified when the descriptor was captured.
Return Value:
    None.
*/
{
    // We only have something to deallocate if the requestor was user mode or kernel mode requesting ForceCapture.
    PAGED_CODE();
...
/*
Routine Description:
    This routine probes and captures the embedded structures in a Security Quality of Service structure.
    This routine assumes that it is being called under an existing try-except clause.
Arguments:
    CapturedSecurityQos - Points to the captured body of a QOS structure.
        The pointers in this structure are presumed not to be probed or captured at this point.
Return Value:
...
*/
{
    ...
    if (ARGUMENT_PRESENT(CapturedAuditData)) {
        ...
        // order so that if the final assignment fails the caller will still be able to free the allocated pool.
...
/*
Routine Description:
    This routine frees the data associated with a captured SecurityQos structure.
    It does not free the body of the structure, just whatever its internal fields point to.
Arguments:
...
Arguments:
    ObjectAttributes - The object attributes from which the QOS information is to be retrieved.
    RequestorMode - Indicates the processor mode by which the access is being requested.
    SecurityQosPresent - Receives a boolean value indicating whether or not the optional security QOS information was available and copied.
...
*/
{
    ...
    CapturedQos = FALSE;
    (*SecurityQosPresent) = FALSE;//  Set default return
        ...
        try {
            if (ARGUMENT_PRESENT(ObjectAttributes)) {
                ...
                if (ARGUMENT_PRESENT(LocalSecurityQos)) {
                    ...
                    // Check the length and see if this is a QOS or Advanced QOS structure.
                    if (LocalQosLength == sizeof(SECURITY_QUALITY_OF_SERVICE)) {
                        ...
                        if (LocalQosLength == sizeof(SECURITY_ADVANCED_QUALITY_OF_SERVICE)) {
                            LocalAdvancedSecurityQos = (PSECURITY_ADVANCED_QUALITY_OF_SERVICE)ObjectAttributes->SecurityQualityOfService;
                            ProbeForReadSmallStructure(LocalAdvancedSecurityQos, sizeof(SECURITY_ADVANCED_QUALITY_OF_SERVICE), sizeof(ULONG));
        ...
        if (ARGUMENT_PRESENT(ObjectAttributes)) {
            if (ARGUMENT_PRESENT(ObjectAttributes->SecurityQualityOfService)) {
                ...
                if (((PSECURITY_QUALITY_OF_SERVICE)(ObjectAttributes->SecurityQualityOfService))->Length == sizeof(SECURITY_QUALITY_OF_SERVICE)) {
                    ...
                    (*CapturedSecurityQos) = (*(SECURITY_ADVANCED_QUALITY_OF_SERVICE *)(ObjectAttributes->SecurityQualityOfService));
                }
            } // end_if
        } // end_if
    } // end_if
...
Routine Description:
    ...
    The SID is either captured into a provided buffer, or pool allocated to receive the SID.
...
Arguments:
    InputSid - Supplies the SID to capture.  This parameter is assumed to have been provided by the mode specified in RequestorMode.
    RequestorMode - Specifies the caller's access mode.
    CaptureBuffer - Specifies a buffer into which the SID is to be captured.
        If this parameter is not provided, pool will be allocated to hold the captured data.
    CaptureBufferLength - Indicates the length, in bytes, of the capture buffer.
    PoolType - Specifies which pool type to allocate to capture the SID into.
        This parameter is ignored if CaptureBuffer is provided.
    ForceCapture - Specifies whether the SID should be captured even if requestor mode is kernel.
    ...
    AlignedSidSize - Supplies the address of a ULONG to receive the length of the SID rounded up to the next longword boundary.
Return Value:
    STATUS_SUCCESS indicates the capture was successful.
    STATUS_BUFFER_TOO_SMALL - indicates the buffer provided to capture the SID into wasn't large enough to hold the SID.
    Any access violations encountered will be returned.
*/
{
    ...
    //  check if the requestors mode is kernel mode and we are not to force a capture.
    if ((RequestorMode == KernelMode) && (ForceCapture == FALSE)) {
        //  We don't need to do any work and can simply return a pointer to the input SID
...
Routine Description:
    ...
    This routine should NOT be called if the SID was captured into a provided CaptureBuffer (see SeCaptureSid).
Arguments:
    CapturedSid - Supplies the SID to release.
    RequestorMode - The processor mode specified when the SID was captured.
    ForceCapture - The ForceCapture value specified when the SID was captured.
Return Value:
    None.
*/
{
...
Routine Description:
    ...
    The ACL is either captured into a provided buffer, or pool allocated to receive the ACL.
...
Arguments:
    InputAcl - Supplies the ACL to capture.  This parameter is assumed to have been provided by the mode specified in RequestorMode.
    RequestorMode - Specifies the caller's access mode.
    CaptureBuffer - Specifies a buffer into which the ACL is to be captured.
        If this parameter is not provided, pool will be allocated to hold the captured data.
    CaptureBufferLength - Indicates the length, in bytes, of the capture buffer.
    PoolType - Specifies which pool type to allocate to capture the ACL into.
        This parameter is ignored if CaptureBuffer is provided.
    ForceCapture - Specifies whether the ACL should be captured even if requestor mode is kernel.
    ...
    AlignedAclSize - Supplies the address of a ULONG to receive the length of the ACL rounded up to the next longword boundary.
Return Value:
    STATUS_SUCCESS indicates the capture was successful.
    STATUS_BUFFER_TOO_SMALL - indicates the buffer provided to capture the ACL into wasn't large enough to hold the ACL.
    Any access violations encountered will be returned.
*/
{
    ULONG AclSize;
    ...
    //  check if the requestors mode is kernel mode and we are not to force a capture.
    if ((RequestorMode == KernelMode) && (ForceCapture == FALSE)) {
        //  We don't need to do any work and can simply return a pointer to the input ACL
    ...
    // If the passed pointer is non-null, it has better at least point to a well formed ACL
...
Routine Description:
    ...
    This routine should NOT be called if the ACL was captured into a provided CaptureBuffer (see SeCaptureAcl).
Arguments:
    CapturedAcl - Supplies the ACL to release.
    RequestorMode - The processor mode specified when the ACL was captured.
    ForceCapture - The ForceCapture value specified when the ACL was captured.
Return Value:
    None.
*/
{
...
/*
Routine Description:
    This routine probes and captures a copy of the specified LUID_AND_ATTRIBUTES array.

    The array is either captured into a provided buffer, or pool allocated to receive the array.
...
Arguments:
    InputArray - Supplies the array to capture.  This parameter is assumed to have been provided by the mode specified in RequestorMode.
    ArrayCount - Indicates the number of elements in the array to capture.
    RequestorMode - Specifies the caller's access mode.
    CaptureBuffer - Specifies a buffer into which the array is to be captured.
        If this parameter is not provided, pool will be allocated to hold the captured data.
    CaptureBufferLength - Indicates the length, in bytes, of the capture buffer.
    PoolType - Specifies which pool type to allocate to capture the array into.
        This parameter is ignored if CaptureBuffer is provided.
    ForceCapture - Specifies whether the array should be captured even if requestor mode is kernel.
    ...
    AlignedArraySize - Supplies the address of a ULONG to receive the length of the array rounded up to the next longword boundary.
Return Value:
    STATUS_SUCCESS indicates the capture was successful.
    STATUS_BUFFER_TOO_SMALL - indicates the buffer provided to capture the array into wasn't large enough to hold the array.
    Any access violations encountered will be returned.
*/
{
    ULONG ArraySize;
    ...
    //  check if the requestors mode is kernel mode and we are not to force a capture.
    if ((RequestorMode == KernelMode) && (ForceCapture == FALSE)) {
        //  We don't need to do any work and can simply return a pointer to the input array
...
Routine Description:
    ...
    This routine should NOT be called if the array was captured into a provided CaptureBuffer (see SeCaptureLuidAndAttributesArray).
Arguments:
    CapturedArray - Supplies the array to release.
    RequestorMode - The processor mode specified when the array was captured.
    ForceCapture - The ForceCapture value specified when the array was captured.
Return Value:
    None.
*/
{
...
/*
Routine Description:
    This routine probes and captures a copy of the specified SID_AND_ATTRIBUTES array, along with the SID values pointed to.

    The array is either captured into a provided buffer, or pool allocated to receive the array.

    The format of the captured information is an array of SID_AND_ATTRIBUTES data structures followed by the SID values.
    THIS MAY NOT BE THE CASE FOR KERNEL MODE UNLESS A FORCE CAPTURE IS SPECIFIED.
...
*/
{
    ...
    //  check if the requestor's mode is kernel mode and we are not to force a capture.
    if ((RequestorMode == KernelMode) && (ForceCapture == FALSE)) {
        //  We don't need to do any work and can simply return a pointer to the input array
        (*CapturedArray) = InputArray;
        return STATUS_SUCCESS;
    ...
    // convoluted to avoid problems that could occur if the data is being changed while being captured.
    ...
    // loop just adds up the length needed.  Kernel mode, thus, avoids having to allocate a temporary buffer.
    ...
    // Now copy everything.
    // This is done by copying all the SID_AND_ATTRIBUTES and then copying each individual SID.

    // All SIDs have already been probed for READ access.  We just need to copy them.
    if (RequestorMode != KernelMode) {
        try {
            //  Copy the SID_AND_ATTRIBUTES array elements
            //  This really only sets the attributes, since we over-write the SID pointer field later on.
        ...
        while (NextIndex < ArrayCount) {
            ...
            RtlCopyMemory(NextBufferLocation, NextElement[NextIndex].Sid, RtlLengthRequiredSid(GetSidSubAuthorityCount));
            SidSize = RtlLengthRequiredSid(GetSidSubAuthorityCount);
...
Routine Description:
    ...
    This routine should NOT be called if the array was captured into a provided CaptureBuffer (see SeCaptureSidAndAttributesArray).
Arguments:
    CapturedArray - Supplies the array to release.
    RequestorMode - The processor mode specified when the array was captured.
    ForceCapture - The ForceCapture value specified when the array was captured.
...
Routine Description
    ...
    This routine probes and captures a copy of the specified TOKEN_AUDIT_POLICY.

    It is either captured into a provided buffer, or pool allocated to receive the policy.
...
Arguments:
    RequestorMode - Specifies the caller's access mode.
    CaptureBuffer - Specifies a buffer into which the policy is to be captured.
        If this parameter is not provided, pool will be allocated to hold the captured data.
    CaptureBufferLength - Indicates the length, in bytes, of the capture buffer.
    PoolType - Specifies which pool type to allocate to capture the input into.
        This parameter is ignored if CaptureBuffer is provided.
    ForceCapture - Specifies whether the policy should be captured even if requestor mode is kernel.
...
Return Value:
    STATUS_SUCCESS indicates the capture was successful.
    STATUS_BUFFER_TOO_SMALL - indicates the buffer provided to capture the policy into wasn't large enough.
    Any access violations encountered will be returned.
*/
{
    ULONG PolicyCount;
    ...
    //  check if the requestors mode is kernel mode and we are not to force a capture.
    if ((RequestorMode == KernelMode) && (ForceCapture == FALSE)) {
        //  We don't need to do any work and can simply return a pointer to the input policy
    ...
    if (RequestorMode != KernelMode) {
        try {
            ...
            ProbeForRead(Policy, PolicySize, sizeof(ULONG));
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            return GetExceptionCode();
        }
...
Routine Description:
    ...
    This routine should NOT be called if the policy was captured into a provided CaptureBuffer (see SeCaptureAuditPolicy).
Arguments:
    CapturedPolicy - Supplies the policy to release.
    RequestorMode - The processor mode specified when the data was captured.
    ForceCapture - The ForceCapture value specified when the data was captured.
Return Value:
    None.
*/
{
    ...
    if (CapturedPolicy && (((RequestorMode == KernelMode) && (ForceCapture == TRUE)) || (RequestorMode == UserMode))) {
        ExFreePool(CapturedPolicy);
    }
}
...
/*
Routine Description:
    This routine computes the size of the Group and DACL for the passed security descriptor.
    This quantity will later be used in calculating the amount of quota to charge for this object.
Arguments:
    SecurityDescriptor - Supplies a pointer to the security descriptor to be examined.
    Size - Returns the size in bytes of the sum of the Group and Dacl fields of the security descriptor.
Return Value:
    STATUS_SUCCESS - The operation was successful.
    STATUS_INVALID_REVISION - The passed security descriptor was of an unknown revision.
*/
{
...
BOOLEAN SeValidSecurityDescriptor(__in ULONG Length, __in_bcount(Length) PSECURITY_DESCRIPTOR SecurityDescriptor)
/*
Routine Description:
...
Arguments:
    ...
    SecurityDescriptor - Points to the Security Descriptor (in kernel memory) to be validatated.
Return Value:
...
*/
{
    ...
    if (ISecurityDescriptor->Group != 0) {
        if (!LongAligned((PVOID)(ULONG_PTR)(ULONG)ISecurityDescriptor->Group)) {// Check alignment
            return(FALSE);
        }
    ...
    // Validate the DACL.  A structurally valid SecurityDescriptor may not necessarily have a DACL.
    if (ISecurityDescriptor->Dacl != 0) {
        if (!LongAligned((PVOID)(ULONG_PTR)(ULONG)ISecurityDescriptor->Dacl)) {// Check alignment
            return(FALSE);
        }
    ...
    // Validate the SACL.  A structurally valid SecurityDescriptor may not have a SACL.
    if (ISecurityDescriptor->Sacl != 0) {
        if (!LongAligned((PVOID)(ULONG_PTR)(ULONG)ISecurityDescriptor->Sacl)) {// Check alignment
            return(FALSE);
        }
