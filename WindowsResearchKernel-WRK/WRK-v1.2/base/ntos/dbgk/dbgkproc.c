...
*/
{
    ...
    // If we are in the process of being deleted don't do this.
...
{
    PAGED_CODE();
    KeThawAllThreads();// Thaw the execution of all threads in the current process, but the calling thread.
}
...
Routine Description:
    ...
    The file is opened such that it can be duplicated all the way to the UI where the UI can either map the file or read the file to get the debug info.
Arguments:
...
*/
{
    ...
    if ((OldFlags&PS_PROCESS_FLAGS_IMAGE_NOTIFY_DONE) == 0 && PsImageNotifyEnabled) {
        ...
        if (FileNameInfo != NULL) {
            PsCallImageNotifyRoutines(&FileNameInfo->Name, Process->UniqueProcessId, &ImageInfo);
            ExFreePool(FileNameInfo);
        } else {
            PsCallImageNotifyRoutines(NULL, Process->UniqueProcessId, &ImageInfo);
        }
    ...
    if ((OldFlags&PS_PROCESS_FLAGS_CREATE_REPORTED) == 0) {
        ...
        try {
            ...
            if (NtHeaders) {
                ...
                // The following fields are safe for Wow64 as the offsets are the same for a PE32+ as a PE32 header.
        ...
        try {
            ...
            // Normaly the ntdll loaded fills in this pointer for the debug API's. 
            // We fake it here as ntdll isn't loaded yet and it can't load itself.
            Teb = Thread->Tcb.Teb;
            if (Teb != NULL) {
        ...
        InitializeObjectAttributes(&Obja,
            (PUNICODE_STRING)&PsNtDllPathName,
                                   ...
                                   NULL,
                                   NULL);
        Status = ZwOpenFile(&LoadDllArgs->FileHandle,
            (ACCESS_MASK)(GENERIC_READ | SYNCHRONIZE),
                            &Obja,
                            &IoStatusBlock,
                            FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
...
*/
{
    ...
    // this ensures that other timed lockers of the process will bail since this call is done while holding the process lock, 
    // and lock duration is controlled by debugger
...
Arguments:
    ...
    SectionOffset - Supplies the offset in the section where the process' mapped view begins.
