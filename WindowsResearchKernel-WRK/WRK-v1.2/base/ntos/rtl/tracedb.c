...
Abstract:
    This module contains the implementation for the trace database module (hash table to store stack trace in USer/Kernel mode).
*/
...
// Magic values that prefix tracedb structures and allow early detection of corruptions.
...
// Amount of memory with each a trace database will be increased if a new trace cannot be stored.
...
BOOLEAN RtlpTraceDatabaseInitializeLock(IN PRTL_TRACE_DATABASE Database);
BOOLEAN RtlpTraceDatabaseUninitializeLock(IN PRTL_TRACE_DATABASE Database);
BOOLEAN RtlpTraceDatabaseAcquireLock(IN PRTL_TRACE_DATABASE Database);
BOOLEAN RtlpTraceDatabaseReleaseLock(IN PRTL_TRACE_DATABASE Database);
...
/*
Routine Description:
    This routine creates a trace database, that is a hash table to store stack traces.
Arguments:
    ...
    HashFunction - hash function to be used. If null passed a standard hash function will be provided by the module.
Return Value:
...
*/
{
    ...
    // Prepare trace database flags. The first segment of the database will be allocated in nonpaged pool
    // no matter what flags are used because it contains
    // kernel synchronization objects that need to be in that pool.
    Flags |= RTL_TRACE_IN_KERNEL_MODE;
    FirstFlags = RTL_TRACE_IN_KERNEL_MODE | RTL_TRACE_USE_NONPAGED_POOL;

    // Allocate first segment of trace database that will contain DATABASE, SEGMENT, buckets of the hash table and later traces.
...
/*
Routine Description:
    This routine destroys a trace database.
    It takes care of deallocating everything, uninitializing synchronization objects, etc.
Arguments:
...
*/
{
    ...
    // has as a side effect elevating the irql (in K mode) and then the function will return with raised irql.
    ...
    // Special attention with the last segment because it contains the database structure itself and we do not want to shoot.
    ...
    for (Current = Database->SegmentList; Current != NULL; Current = NextSegment) {
        ...
        // If this is the last segment we need to offset Current pointer by the size of the database structure.
        if (NextSegment == NULL) {
...
Arguments:
    Database - trace database
Return Value:
    TRUE if the database is ok. For most of the inconsistencies this function will break in the debugger.
Environment:
    Any.
*/
{
...
/*
Routine Description:
    This routine adds a new stack trace to the database.
    If the trace already exists then only the `Count' field for the trace will be incremented.
Arguments:
    Database - trace database
    Count - number of pointers (PVOIDs) in the trace
    Trace - array of PVOIDs (the trace)
    TraceBlock - if not null will contain the address of the block where the trace was stored.
Return Value:
    TRUE if a trace was added to the database.
    TraceBlock will contain the address of the block.
    If the trace was already present in the database a block with `Count' greater than 1 will be returned.
Environment:
    Any.
*/
{
...
/*
Routine Description:
    This routine searches a trace in the database.
    If the trace is found then the address of the block that stores the trace will be returned.
Arguments:
    Database - trace database
    Count - number of pointers (PVOIDs) in the trace
    Trace - array of PVOIDs (the trace)
    TraceBlock - if not null will contain the address of the block where the trace is stored.
Return Value:
    TRUE if the trace was found in the database. TraceBlock will contain the address of the block that stores the trace.
Environment:
    Any.
*/
{
...
BOOLEAN RtlpTraceDatabaseInternalAdd(
    IN PRTL_TRACE_DATABASE Database,
    IN ULONG Count,
    IN PVOID * Trace,
    OUT PRTL_TRACE_BLOCK * TraceBlock OPTIONAL
...
/*
Routine Description:
    This is the internal routine to add a trace. See RtlTraceDatabaseAdd for more details.
Arguments:
    Database - trace database
...
Environment:
    Called from RtlTraceDatabaseAdd. Assumes the database lock is held.
*/
{
    PRTL_TRACE_BLOCK Block;
    PRTL_TRACE_SEGMENT Segment;
    ...
    if (RequestSize > (SIZE_T)(TopSegment->SegmentEnd - TopSegment->SegmentFree)) {
        ...
        // Allocate a new database segment. Fail call if cannot allocate.
...
BOOLEAN RtlpTraceDatabaseInternalFind(
    PRTL_TRACE_DATABASE Database,
    IN ULONG Count,
    IN PVOID * Trace,
    OUT PRTL_TRACE_BLOCK * TraceBlock OPTIONAL
...
*/
{
    ...
    for (Current = Database->Buckets[HashValue]; Current != NULL; Current = Current->Next) {
        // If the size of the trace matches we might have a chance to find an equal trace.
    ...
    // If we traversed the whole list for the hashed bucket and did not find anything we will fail the call.
    if (TraceBlock) {
        *TraceBlock = NULL;
    }
...
ULONG RtlpTraceStandardHashFunction(IN ULONG Count, IN PVOID * Trace)
/*
Routine Description:
    This routine is a simple hash function for stack traces in the case the caller of RtlTraceDatabaseCreate does not provide
    one. The function just xor's together all the pointers in the trace.
Arguments:
    Count - size of trace (in PVOIDs)
    Trace - trace
Return Value:
...
PVOID RtlpTraceDatabaseAllocate(
    IN SIZE_T Size,
    IN ULONG Flags, // OPTIONAL in User mode
    IN ULONG Tag    // OPTIONAL in User mode
)
/*
Routine Description:
    This routine allocates memory and hides all the different details for User vs Kernel mode allocation and paged vs nonpaged pool.
Arguments:
    Size -  size in bytes
...
BOOLEAN RtlpTraceDatabaseFree(
    PVOID Block,
    IN ULONG Tag    // OPTIONAL in User mode
)
/*
Routine Description:
    This routine frees memory and hides all the different details for User vs Kernel mode allocation and paged vs nonpaged pool.
Arguments:
...
Routine Description:
    ...
    The callers needs to acquire the database lock only if a trace block will be modified (UserCount, UserSize fields).
...
Routine Description:
    ...
    The callers needs to acquire/release the database lock only if a trace block will be modified (UserCount, UserSize fields).
    The lock is not needed for Add/Find/Enumerate operations.
Arguments:
    Database - trace database
Return Value:
...
PRTL_TRACE_SEGMENT RtlpTraceSegmentCreate(
    IN SIZE_T Size,
    IN ULONG Flags, // OPTIONAL in User mode
    IN ULONG Tag    // OPTIONAL in User mode
)
...
Routine Description:
    ...
    through which a database can increase in size to accommodata more traces.
Arguments:
    Size - size in bytes
...
/*
Routine Description:
    This function enumerates all traces in the database.
    It requires a RTL_TRACE_ENUMERATE function (zeroed initially) to keep the state of the enumeration.
    Since the trace database does not support delete operations we do not need to keep a lock across multiple calls to
    Enumerate(). However this can change if we add support for deletions.
Arguments:
    Database - trace database pointer
    Enumerate - enumeration opaque structure. Used to keep the state of the enumeration.
    TraceBlock - on each successful return this pointer gets filled with the address of a trace block from the database.
Return Value:
    TRUE if a trace block was found (during enumeration) and FALSE if there are no more blocks in the database.
Environment:
    User/Kernel mode.
*/
{
    ...
    // If this is not the first call try to validate what we have inside the enumerator.
    ...
    // Find out the next trace block in case we are at the end of a bucket or the bucket was empty.
