...
NTSTATUS RtlpGenerateInheritAcl(
    IN PACL Acl,
    IN BOOLEAN IsDirectoryObject,
    IN BOOLEAN AutoInherit,
    IN PSID ClientOwnerSid,
...
NTSTATUS RtlpComputeMergedAcl2(
    IN PACL CurrentAcl,
    IN ULONG CurrentGenericControl,
    IN PACL ModificationAcl,
    IN ULONG ModificationGenericControl,
...
BOOLEAN RtlpIsDuplicateAce(IN PACL Acl, IN PKNOWN_ACE NewAce);
BOOLEAN RtlpGuidPresentInGuidList(IN GUID *InheritedObjectType, IN GUID **pNewObjectType, IN ULONG GuidCount);
...
NTSTATUS RtlpImpersonateSelfEx(IN SECURITY_IMPERSONATION_LEVEL ImpersonationLevel, IN ACCESS_MASK AdditionalAccess, OUT PHANDLE ThreadToken OPTIONAL);
...
*/
{
    ...
    for (i = 1; i < S->Length; i++) {
        ...
        // In order to be compatible with zero terminated unicode strings, this algorithm is designed to not produce a wide character of zero as long a the seed is not zero.
...
Arguments:
    ...
    WasEnabled - points to a boolean to receive an indication of whether the privilege was previously enabled or disabled.
        ...
        FALSE indicates the privilege was previously disabled.
...
*/
{
    ...
    ULONG Length;
    UCHAR Buffer1[sizeof(TOKEN_PRIVILEGES) + ((1 - ANYSIZE_ARRAY) * sizeof(LUID_AND_ATTRIBUTES))], Buffer2[sizeof(TOKEN_PRIVILEGES) + ((1 - ANYSIZE_ARRAY) * sizeof(LUID_AND_ATTRIBUTES))];
...
Arguments:
    Sid1, Sid2 - Supply pointers to the two SID values to compare. The SID structures are assumed to be valid.
Return Value:
...
*/
{
    ...
    // To make this routine faster we make the assumption that the revision and subauthority fields are adjacent.
    ...
    if (ISid1->Revision == ISid2->Revision) {
        ...
        if ((ISid1->IdentifierAuthority.Value[0] == ISid2->IdentifierAuthority.Value[0]) &&
            ...
            if (ISid1->SubAuthorityCount == ISid2->SubAuthorityCount) {
                ...
                return TRUE;// All SubAuthority values are equal.
            }
        }
    }
...
Arguments:
    ...
    TargetSid - Indicates where the target SID values s are to be copied.
        This is assumed to be ULONG aligned.
...
Arguments:
    UnicodeString - Returns a unicode string that is equivalent to the SID. The maximum length field is only set if AllocateDestinationString is TRUE.
    Sid - Supplies the SID that is to be converted to unicode.
...
Return Value:
    SUCCESS - The conversion was successful
    STATUS_INVALID_SID - The sid provided does not have a valid structure, or has too many sub-authorities (more than SID_MAX_SUB_AUTHORITIES).
    STATUS_NO_MEMORY - There was not sufficient memory to allocate the target string.  This is returned only if AllocateDestinationString is specified as TRUE.
...
*/
{
    ...
    if ((iSid->IdentifierAuthority.Value[0] != 0) || (iSid->IdentifierAuthority.Value[1] != 0)) {
        ...
        Auth.LowPart = (ULONG)iSid->IdentifierAuthority.Value[5] + (ULONG)(iSid->IdentifierAuthority.Value[4] << 8) + (ULONG)(iSid->IdentifierAuthority.Value[3] << 16) + (ULONG)(iSid->IdentifierAuthority.Value[2] << 24);
...
/*
Routine Description:
    This routine returns the length, in bytes, necessary to capture a structurally valid SECURITY_DESCRIPTOR.  The length includes the length of all associated data structures (like SIDs and ACLs).
...
Arguments:
    ...
    Sacl - Supplies the system ACL for the security descriptor.
        If this optional parameter is not passed, then a null ACL is assigned to the security descriptor.
        The ACL is referenced by, not copied into, by the security descriptor.
...
Arguments:
    ...
    Owner - Supplies the owner SID for the security descriptor.
        If this optional parameter is not passed, then the owner is cleared (indicating the security descriptor has no owner).
...
Arguments:
    SecurityDescriptor - Supplies the security descriptor.
    Owner - Receives a pointer to the owner SID.
        If the security descriptor does not currently contain an owner, then this value will be returned as null.
...
Arguments:
    SecurityDescriptor - Supplies the security descriptor.
    Group - Receives a pointer to the primary group SID.
        If the security descriptor does not currently contain a primary group, then this value will be returned as null.
        In this case, the remaining OUT parameters are not given valid return values.
        Otherwise, this parameter points to an SID and the remaining OUT parameters are provided valid return values.
...
*/
{
    ...
    *Group = RtlpGroupAddrSecurityDescriptor(ISecurityDescriptor);// Return the Group field value.
    *GroupDefaulted = RtlpAreControlBitsSet(ISecurityDescriptor, SE_GROUP_DEFAULTED);// Return the GroupDefaulted flag value.
    return STATUS_SUCCESS;
}
...
*/
{
    ...
    *AccessMask &= ~(GENERIC_READ | GENERIC_WRITE | GENERIC_EXECUTE | GENERIC_ALL);// Now clear the generic flags
}
...
Routine Description:
    ...
    The caller may optionally request to receive a handle to the token assigned to the thread, obviating the need for a subsequent call solely to open the thread token.
Arguments:
    ImpersonationLevel - The level to make the impersonation token.
    AdditionalAccess - Additional requested access rights to the thread token handle beyond TOKEN_IMPERSONATE.
        If an output handle is not specified, this value must be zero.
    ThreadToken - Optional output pointer which receives a handle to the current thread token upon successful completion of this routine.
*/
{
...
Routine Description:
    ...
    Only known DSA ACEs are mapped.
    Unknown ACE types are ignored.
...
Routine Description:
    ...
    The resultant ACE has the SID mapped from a generic SID to a specific SID (e.g., From "creator owner" to the passed in owner sid).
Arguments:
    ...
    GenericMapping - Specifies the generic mapping to use
    pNewObjectType - List of types of object being inherited to.  If not specified, the object has no object type.
    ...
    NewAceLength - Returns the length (in bytes) needed in NewAcl to copy the specified ACE. This might be zero to indicate that the ACE need not be copied at all.
...
Return Value:
    ...
    FALSE - Indicates something went wrong preventing the ACE from being copied.
...
*/
{
    ...
    *AclOverflowed = FALSE;
    ...
    if (IsMSAceType(OldAce)) {
        ...
        if (IsKnownAceType(OldAce)) {
            ...
            if (RtlEqualPrefixSid(ServerSidToCopy, CreatorSid)) {
                ...
                if (!*EffectiveAceMapped) {// If we don't know what this SID is, just copy the original.
                    AceHeaderToCopyLength += SeLengthSid(ServerSidToCopy);
                    ServerSidToCopy = NULL;
                }
            } else {// We don't know what this SID is, just copy the original.
                AceHeaderToCopyLength += SeLengthSid(ServerSidToCopy);
                ServerSidToCopy = NULL;
            }
        } else {// Handle Object ACEs
            ...
            if (ARGUMENT_PRESENT(ObjectAceInherited) && InheritedObjectType != NULL) {
                ...
                if (pNewObjectType == NULL || !RtlpGuidPresentInGuidList(InheritedObjectType, pNewObjectType, GuidCount)) {
                    LengthRequired = 0;
                } else {// If the object type matches the inherited object type, Inherit an ACE with no inherited object type.
                    *ObjectAceInherited = TRUE;// Tell the caller we inherited an object type specific ACE.

                    // If the caller is not going to generate an inheritable ACE, deleting the inherited object type GUID for the effective ACE.
                    ...
                    if (!WillGenerateInheritAce) {
                        *EffectiveAceMapped = TRUE;
                        ...
                        } else {// If an object type GUID is not present, convert the ACE to non-object type specific.
        ...
        if (LengthRequired != 0) {
            ...
            if (LocalMask == 0) {
                ...
                // See if the SID in the ACE is one of the various CREATOR_* SIDs by comparing identifier authorities.
    ...
    // We have the length of the new ACE, but we've calculated it with a ULONG.  It must fit into a USHORT.  See if it does.
...
/*
Routine Description:
    Copy ACEs from of an ACL and perform generic mapping.  Only ACEs specified by 'AceFilter' are copied.
Arguments:
    ...
    for (i = 0, OldAce = FirstAce(Acl); i < Acl->AceCount; i += 1, OldAce = NextAce(OldAce)) {
        ...
        if (CopyAce) {
            ...
            if (MapSids) {
                ...
                NewAceSize = 0;
                GenerateInheritAce = IsDirectoryObject && ((((PACE_HEADER)OldAce)->AceFlags & (OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE)) != 0);

                // If the original ACE is an effective ACE, create an effective ACE.
                if (!(((PACE_HEADER)OldAce)->AceFlags & INHERIT_ONLY_ACE)) {
                    BOOLEAN LocalAclOverflowed;

                    // If the ace has INHERITED_ACE bit and the caller has requested preservation of the bit, copy the effective ace as an INHERITED_ACE.
                ...
                if (AcePosition == NULL || NewAceSize > (ULONG)NewAcl->AclSize - ((PUCHAR)AcePosition - (PUCHAR)NewAcl)) {
                    ...
                    RtlCopyMemory(AcePosition, OldAce, NewAceSize);// Copy the ACE.
                    ...
                    ((PACE_HEADER)AcePosition)->AceFlags &= ~AceFlagsToReset;// Reset any undesirable AceFlags.
                    NewAcl->AceCount += 1;// Account for the new ACE.
                }
            }
...
Return Value:
    ...
    STATUS_NO_INHERITANCE - An inheritable ACL was not successfully generated.
        ...
        The caller should use the default ACL.
...
*/
{
    ...
    *NewGenericControl = AutoInherit ? SEP_ACL_AUTO_INHERITED : 0;
    ...
    // If the a current child ACL is not defaulted, the non-inherited ACEs from the current child ACL are to be preserved.
    if ((ChildGenericControl & SEP_ACL_DEFAULTED) == 0) {
        // The resultant ACL should be protected if the input ACL is protected.
    ...
    if ((!AutoInherit && (ChildGenericControl & SEP_ACL_PRESENT) == 0 || (ChildGenericControl & SEP_ACL_DEFAULTED) != 0) || (AutoInherit && !AclProtected)) {
        ...
        if (DirectoryAcl != NULL) {
            ...
            if (DefaultDescriptorForObject && ChildNewAclSize != 0 && ObjectAceInherited && !AclOverflowed) {
                ...
                ((PACL)AclBuffer)->AceCount -= ChildAceCount;// Adjust the ACE count to remove the deleted ACEs
                UsedChildNewAclSize = 0;// Save the number of bytes of the Child ACL that were actually used.
            }
        }
    }
    ...
    // The caller has to allocate a buffer large enough for ChildNewAclSize rather than UsedChildNewAclSize.
    // Due to the nature of my algorithm above.
...
Return Value:
    STATUS_SUCCESS - An inheritable ACL was successfully generated.
    STATUS_NO_INHERITANCE - An inheritable ACL was not successfully generated. This is a warning completion status.
    STATUS_BAD_INHERITANCE_ACL - Indicates the acl built was not a valid ACL.
...
Arguments:
    ...
    AutoInherit - Specifies if the inheritance is an "automatic inheritance". As such, the inherited ACEs will be marked as such.
    ClientOwnerSid - Specifies the owner Sid to use
    ClientGroupSid - Specifies the new Group Sid to use
    ServerSid - Optionally specifies the Server Sid to use in compound ACEs.
    ClientSid - Optionally specifies the Client Sid to use in compound ACEs.
...
Return Value:
    ...
    STATUS_BAD_INHERITANCE_ACL - Indicates something went wrong preventing the ACE from being inherited.
        This generally represents a bugcheck situation when returned from this call.
...
*/
{
    ...
    //  This is gross and ugly, but it's better than allocating virtual memory to hold the ClientSid, because that can fail, and propagating the error back is a tremendous pain
    ASSERT(RtlLengthRequiredSid(1) == CREATOR_SID_SIZE);
    *ObjectAceInherited = FALSE;
    ...
    if ((IsDirectoryObject  && ContainerInherit(OldAce)) || (!IsDirectoryObject && ObjectInherit(OldAce))) {
        EffectiveAcePosition = AcePosition;// Remember where the effective ACE will be copied to.
        ...
        if (!AclOverflowed && EffectiveAceSize > 0 && EffectiveAcePosition != NULL && RtlpIsDuplicateAce(NewAcl, EffectiveAcePosition)) {
    ...
    if (GenerateInheritAce) {
        ...
        if (EffectiveAceSize != 0 && !EffectiveAceMapped) {
            ...
            LengthRequired += (ULONG)(((PACE_HEADER)OldAce)->AceSize);// Account for the new ACE being added to the ACL.
            if (LengthRequired > 0xFFFF) {
                return STATUS_BAD_INHERITANCE_ACL;
            }
            ...
            if (((PACE_HEADER)OldAce)->AceSize > NewAcl->AclSize - ((PUCHAR)AcePosition - (PUCHAR)NewAcl)) {
                ...
                RtlCopyMemory(AcePosition, OldAce, ((PACE_HEADER)OldAce)->AceSize);// copy it as is, but make sure the InheritOnly bit is set.
...
*/
{
    ...
    *ObjectAceInherited = FALSE;
    for (i = 0, OldAce = FirstAce(Acl); i < Acl->AceCount; i += 1, OldAce = NextAce(OldAce)) {
        ...
        //  RtlpGenerateInheritedAce() will generate the ACE(s) necessary to inherit a single ACE.
        //  This may be 0, 1, or more ACEs.
        ...
        NewAclSize += NewAceSize;// Make room in the ACL for the new ACE
...
*/
{
    ...
    *NewGenericControl = SEP_ACL_AUTO_INHERITED;
    ...
    if ((ModificationGenericControl & SEP_ACL_PROTECTED) != 0) {
        *NewGenericControl |= SEP_ACL_PROTECTED;// Set the Control bits for the resultant descriptor.
        ...
        if (CurrentAcl != NULL) {
            ...
            // Don't bother mapping the sids in these ACEs.
            // They got mapped during inheritance.
            Status = RtlpCopyAces(
                CurrentAcl,
                GenericMapping,
                CopyInheritedAces,
...
*/
{
    ...
    AclBufferSize = 1024;
    for (i = 0; i < 2; i++) {
        (*NewAcl) = ExAllocatePoolWithTag(PagedPool, AclBufferSize, 'cAeS');// Allocate heap for the new ACL.
        if ((*NewAcl) == NULL) {
            return(STATUS_NO_MEMORY);
        }
...
NTSTATUS RtlpConvertToAutoInheritSecurityObject(
...
*/
{
    ...
    if (CurrentSacl == NULL) {
        ...
        NewSaclControl |= CurrentDescriptor->Control & (SE_SACL_PROTECTED | SE_SACL_PRESENT);// Preserve the Acl Present bit and protected bit from the existing descriptor.
        NewSaclControl |= SE_SACL_AUTO_INHERITED;// Always set the autoinherited bit.
        NewSacl = CurrentSacl;
    ...
    if (CurrentDacl == NULL) {
        ...
        NewDaclControl |= CurrentDescriptor->Control & (SE_DACL_PROTECTED | SE_DACL_PRESENT);// Preserve the Acl Present bit and protected bit from the existing descriptor.
        NewDaclControl |= SE_DACL_AUTO_INHERITED;// Always set the autoinherited bit.
        NewDacl = CurrentDacl;
...
*/
{
    ...
    if (!RtlEqualSid((PSID)&ChildAce->SidStart, (PSID)&InheritedAce->SidStart)) {
        ...
        // If the InheritedAce contains a CreatorOwner/Group SID, do the another comparison of the SID in the child ACE with the real owner/group from the child security descriptor.
...
Arguments:
    ...
    ObjectType - GUID of the object type being created.  If the object being created has no GUID associated with it, then this argument is specified as NULL.
    IsDirectoryObject - Specifies if the object is a directory object.  A value of TRUE indicates the object is a container of other objects.
    OwnerSid - Specifies the owner Sid to use.
    GroupSid - Specifies the group SID to use.
    GenericMapping - Specifies the generic mapping to use.
    NewAcl - Receives a pointer to the new (auto inherited) acl. The ACL must be deallocated using the pool (kernel mode) or heap (user mode) deallocator.
    NewGenericControl - Specifies the control flags for the newly generated ACL.
...
*/
{
    ...
    *NewAcl = NULL;
    ...
    for (InheritedAceIndex = 0, InheritedAce = FirstAce(InheritedAcl); InheritedAceIndex < InheritedAcl->AceCount; InheritedAceIndex += 1, InheritedAce = NextAce(InheritedAce)) {
        ...
        // This ACE is some combination of an effective ACE, a container inherit ACE and an object inherit ACE.  Process each of those
        //  attributes separately since they might be represented separately in the ChildAcl.
        ...
        // Loop through the entire child ACL comparing each inherited ACE with each child ACE.  Don't stop simply because we've matched once.
    ...
    // The resultant ACL is composed of the non-inherited ACEs followed by the inherited ACE. The inherited ACEs are built by running the
...
Arguments:
    Acl - Existing ACL
    NewAce - Ace to determine if it is already in Acl. NewAce is expected to be the last ACE in "Acl".
Return Value:
    ...
    *ServerAclAllocated = TRUE;
    ...
    for (i = 0, Ace = FirstAce(Acl), Target = FirstAce(*ServerAcl); i < Acl->AceCount; i += 1, Ace = NextAce(Ace)) {
        ...
        if (Ace->AceType == ACCESS_ALLOWED_ACE_TYPE || (AclUntrusted && Ace->AceType == ACCESS_ALLOWED_COMPOUND_ACE_TYPE)) {
            ...
            RtlCopyMemory(Target, Ace, FIELD_OFFSET(KNOWN_ACE, SidStart));// Copy up to the access mask.
            ...
            RtlCopyMemory(Target, Ace, Ace->AceSize);// Just copy the ACE as is.
...
Arguments:
    ...
    AutoInheritFlags - Controls automatic inheritance of ACES from the Parent Descriptor.  Valid values are a bits mask of the logical OR of one or more of the following bits:
...
*/
{
    ...
    } else {//  No descriptor passed, make a fake one
    ...
    if ((NewOwner) != NULL) {
        OwnerExplicitlyAssigned = TRUE;// Use the specified owner
    } else {// If the caller said to default the owner from the parent descriptor, grab it now.
        if (AutoInheritFlags & SEF_DEFAULT_OWNER_FROM_PARENT) {
            ...
            // If we've been asked to create a ServerObject, we need to make sure to pick up the new owner from the Primary token,
            // not the client token.  If we're not impersonating, they will end up being the same.
    ...
    if (NewGroup == NULL) {
        if (AutoInheritFlags & SEF_DEFAULT_GROUP_FROM_PARENT) {// If the caller said to default the group from the parent descriptor, grab it now.
            if (!ARGUMENT_PRESENT(ParentDescriptor)) {
    ...
    Status = RtlpInheritAcl(ARGUMENT_PRESENT(ParentDescriptor) ? RtlpSaclAddrSecurityDescriptor(((SECURITY_DESCRIPTOR *)ParentDescriptor)) : NULL,
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (AutoInheritFlags & SEF_SACL_AUTO_INHERIT) {// Always set the auto inherit bit if the caller requested it.
        ...
        if (RtlpAreControlBitsSet(CapturedDescriptor, SE_SACL_PRESENT | SE_SACL_DEFAULTED)) {
            ...
            SaclExplicitlyAssigned = TRUE;// This counts as an explicit assignment.
        }
    } else {
        goto Cleanup;// Some unusual error occured
    }

    // Establish Discretionary Acl
    Status = RtlpInheritAcl(ARGUMENT_PRESENT(ParentDescriptor) ? RtlpDaclAddrSecurityDescriptor(((SECURITY_DESCRIPTOR *)ParentDescriptor)) : NULL,
    ...
    if (NT_SUCCESS(Status)) {
    ...
    } else if (Status == STATUS_NO_INHERITANCE) {// Always set the auto inherit bit if the caller requested it.
        ...
        if (RtlpAreControlBitsSet(CapturedDescriptor, SE_DACL_PRESENT | SE_DACL_DEFAULTED)) {
            ...
            DaclExplicitlyAssigned = TRUE;// This counts as an explicit assignment.
        } else if (ARGUMENT_PRESENT(SubjectContextDacl)) {// Default to the DACL on the token.
            NewDacl = SubjectContextDacl;
            NewControlBits |= SE_DACL_PRESENT;
        }
    } else {// Some unusual error occured
        goto Cleanup;
    }

#ifdef  ASSERT_ON_NULL_DACL

    // Culprit will probably be the caller NtCreate*, or RtlNewSecurityObject.
    // Note that although this will not always occur because of explicit user action it still must be corrected.
    ...
    if (RequestorMode == UserMode) {
        ...
        if (SaclExplicitlyAssigned && (AutoInheritFlags & SEF_AVOID_PRIVILEGE_CHECK) == 0) {
            ...
            // Check for appropriate Privileges Audit/Alarm messages need to be generated due to the attempt to perform a privileged operation.
...
NTSTATUS RtlpSetSecurityObject(
...
Arguments:
    ...
    AutoInheritFlags - Controls automatic inheritance of ACES.
        ...
        SEF_DACL_AUTO_INHERIT - If set, inherited ACEs from the DACL in the ObjectsSecurityDescriptor are preserved and inherited ACEs from the ModificationDescriptor are ignored.
        ...
        SEF_SACL_AUTO_INHERIT - If set, inherited ACEs from the SACL in the ObjectsSecurityDescriptor are preserved and inherited ACEs from the ModificationDescriptor are ignored.
            Inherited ACEs are not supposed to be modified; so preserving them across this call is appropriate.
            If a protected server does not itself implement auto inheritance, it should not set this bit.
            The caller of the protected server may implement auto inheritance and my indeed be modifying inherited ACEs.

         SEF_AVOID_PRIVILEGE_CHECK - If set, the Token in not used to ensure the Owner passed in ModificationDescriptor is valid.
...
Return Value:
    STATUS_SUCCESS - The operation was successful.
    STATUS_INVALID_OWNER - The owner SID provided as the new owner of the target security descriptor is not one the caller is authorized to assign as the owner of an object, or the client did not pass a token at all.
    STATUS_NO_CLIENT_TOKEN - Indicates a client token was not explicitly provided and the caller is not currently impersonating a client.
...
*/
{
...
#ifdef  ASSERT_ON_NULL_DACL
        // Culprit will probably be the caller NtSetSecurityObject, or RtlSetSecurityObject.
        if (RtlpAssertOnNullDacls) {
            ASSERT(("NULL DACLs are NOT allowed!", RtlpDaclAddrSecurityDescriptor(IModificationDescriptor) != NULL));
        }
#endif // ASSERT_ON_NULL_DACL

        // If AutoInherit is requested, build a merged ACL.
        if (AutoInheritFlags & SEF_DACL_AUTO_INHERIT) {
            ...
            // If AutoInherit isn't requested, just grab a copy of the input DACL.
        } else {
            ...
            // If the original caller claims he understands auto inheritance, preserve the AutoInherited flag.
    ...
    if (ARGUMENT_PRESENT(Object)) {
        ...
        if (!NT_SUCCESS(Status)) {// The new information is too big.
    ...
    if ((NewControlBits & SE_SACL_PRESENT) == 0) {
        ...
        RtlpPropagateControlBits(NewDescriptor, *IObjectsSecurityDescriptor, SE_SACL_DEFAULTED | SE_SACL_PRESENT | SE_SACL_PROTECTED | SE_SACL_AUTO_INHERITED);
    }
    ...
    if ((NewControlBits & SE_DACL_PRESENT) == 0) {
        ...
        RtlpPropagateControlBits(NewDescriptor, *IObjectsSecurityDescriptor, SE_DACL_DEFAULTED | SE_DACL_PRESENT | SE_DACL_PROTECTED | SE_DACL_AUTO_INHERITED);
    }
...
Arguments:
    ...
    MaxLength - Returns the maximum length this item can be given by the enclosing structure.
Return Value:
...
*/
{
    ...
    // The owner appears to be a structurally valid SID that lies within the bounds of the security descriptor.
    // Do the same for the Group if there is one.
    ...
    if (!RtlpAreControlBitsSet(SecurityDescriptor, SE_DACL_PRESENT)) {
        ...
        if (MaxDaclLength < Dacl->AclSize) {// Make sure the DACL length fits within the bounds of the security descriptor.
            return FALSE;
        }

        if (!RtlValidAcl(Dacl)) {// Make sure the ACL is structurally valid.
            return FALSE;
        }
    }
    ...
    if (!RtlpAreControlBitsSet(SecurityDescriptor, SE_SACL_PRESENT)) {
        ...
        if (MaxSaclLength < Sacl->AclSize) {// Make sure the SACL length fits within the bounds of the security descriptor.
            return FALSE;
        }

        if (!RtlValidAcl(Sacl)) {// Make sure the ACL is structurally valid.
            return FALSE;
        }
    }
...
Arguments:
    InheritedObjectType - Guid from the ace that will be compared against the object types for the object.
...
NTSTATUS RtlMapSecurityErrorToNtStatus(IN SECURITY_STATUS Error)
/*
Routine Description:
    This procedure maps a security HRESULT to the proper NTSTATUS code.
Arguments:
    Error - a security HRESULT
Return Value:  The NTSTATUS code corresponding to the HRESULT. If no status code can be mapped, the original error is returned.
*/
{
    NTSTATUS Status;
