...
Abstract:
    Include file for NT runtime routines that are callable by both kernel mode code in the executive and user mode code in various NT subsystems, but which are private interfaces.
*/
...
#define ASSERT_WELL_FORMED_UNICODE_STRING(Str)       ASSERT( (!(((Str)->Length&1) || ((Str)->MaximumLength&1) )) && ((Str)->Length <= (Str)->MaximumLength) )
#define ASSERT_WELL_FORMED_UNICODE_STRING_IN(Str)    ASSERT( !((Str)->Length&1) )
#define ASSERT_WELL_FORMED_UNICODE_STRING_OUT(Str)   ASSERT( (!((Str)->MaximumLength&1)) && ((Str)->Length <= (Str)->MaximumLength) )
...
//  Macro that tells how many contiguous bits are set (i.e., 1) in a byte
...
* TRAVERSE844W
*
* Traverses the 8:4:4 translation table for the given wide character.
* It returns the final value of the 8:4:4 table, which is a WORD in length.
*
*   Broken Down Version:
*
...
// We will have this upcase macro quickly shortcircuit out if the value is within the normal ANSI range (i.e., < 127).
// We actually won't bother with the 5 values above 'z' because they won't happen very often and coding it this way lets us get out after 1 compare for value less than 'a' and 2 compares for lowercase a-z.
...
// Every compression format that NT supports will need to supply these set of routines in order to be called by NtRtl.
