...
NTSTATUS RtlCreateEnvironment(IN BOOLEAN CloneCurrentEnvironment OPTIONAL, OUT PVOID *Environment)
{
    ...
    // If not cloning a copy of the current process's environment variable block, just allocate a block of committed memory and return its address.
    ...
    try {
        try {
            // Query the current size of the current process's environment variable block.  Return status if failure.
...
NTSTATUS RtlSetCurrentEnvironment(IN PVOID Environment, OUT PVOID *PreviousEnvironment OPTIONAL)
{
    ...
    // If caller requested it, return the pointer to the previous process environment variable block and set the local variable to NULL so we dont destroy it below.
    if (ARGUMENT_PRESENT(PreviousEnvironment)) {
        *PreviousEnvironment = pOld;
    } else {
        if (pOld != NULL) {// If old environment not returned to caller, destroy it.
...
NTSTATUS RtlQueryEnvironmentVariable_U(IN PVOID Environment OPTIONAL, IN PCUNICODE_STRING Name, IN OUT PUNICODE_STRING Value)
{
    NTSTATUS Status;
    ...
    try {
        if (ARGUMENT_PRESENT(Environment)) {
            ...
            PebLockLocked = TRUE;
            RtlAcquirePebLock();// Acquire the Peb Lock for the duration while we munge the environment variable storage block.
            p = ProcessParameters->Environment;// Capture the pointer to the current process's environment variable block.
        }

        if (RtlpEnvironCacheValid && p == ProcessParameters->Environment) {
            if (RtlEqualUnicodeString(Name, &RtlpEnvironCacheName, TRUE)) {
                // Names are equal.  Always return the length of the value string, excluding the terminating null.
                // If there is room in the caller's buffer, return a copy of the value string and success status.
                // Otherwise return an error status.
                // In the latter case, the caller can examine the length field of their value string so they can determine much memory is needed.
        ...
        if (p != NULL) while (1) {
            // Get the length of the terminated string. This should be in the form 'keyword=value'
            len = wcslen(p);
            if (len == 0) {
                break;
            }
            ...
            // If must be at least one bigger than the name we are searching for since it must contain an '=' sign.
            if (NameChars < len) {
                q = &p[NameChars];
                ...
                if (*q == L'=') {
                    ...
                    // After comparing the strings we want to make sure we are not matching something that we shouldn't.
                    // For example if somebody did a lookup of "FRED=BOB" we don't want this to match "FRED=BOB=ALBERT".
                    // The lookup name is invalid and the environment variable is really FRED here not "FRED=BOB".
                    // To eliminate this case we make sure that the "=" character is at the appropriate place
                    // There are environment variable of the for =C:
                    // In order not to eliminate these we skip the first character in our search.
                    if (RtlEqualUnicodeString(Name, &CurrentName, TRUE) && (wcschr(p + 1, L'=') == q)) {
                        // Names are equal.
                        // Always return the length of the value string, excluding the terminating null.
                        // If there is room in the caller's buffer, return a copy of the value string and success status.
                        // Otherwise return an error status.
                        // In the latter case, the caller can examine the length field of their value string so they can determine much memory is needed.
        ...
        if (Status == STATUS_VARIABLE_NOT_FOUND) {
            ...
            if (RtlEqualUnicodeString(Name, &CurrentWorkingDirectoryPseudoVariable, TRUE)) {
                ...
                Status = RtlFindCharInUnicodeString(RTL_FIND_CHAR_IN_UNICODE_STRING_START_AT_END, &CurrentValue, &RtlDosPathSeparatorsString, &PrefixLength);
                if (NT_SUCCESS(Status)) {
                    CurrentValue.Length = PrefixLength + sizeof(WCHAR);
                } else if (Status == STATUS_NOT_FOUND) {
                    Status = STATUS_SUCCESS;// Use the whole thing; just translate the status to success.
                }
            }
...
NTSTATUS RtlSetEnvironmentVariable(IN OUT PVOID *Environment OPTIONAL, IN PCUNICODE_STRING Name, IN PCUNICODE_STRING Value OPTIONAL)
{
    NTSTATUS Status;
    MEMORY_BASIC_INFORMATION MemoryInformation;
    UNICODE_STRING CurrentName;
    ...
    if (ARGUMENT_PRESENT(Environment)) {
        ...
        // Capture the pointer to the current process's environment variable block.
        pOld = ProcessParameters->Environment;
    }
    ...
    try {
        try {
            ...
            pEnd = NULL;
            if (p != NULL) while (*p) {
                // Determine the size of the name and value portions of the current string of the environment variable block.
                CurrentName.Buffer = p;
                CurrentName.Length = 0;
                CurrentName.MaximumLength = 0;
                while (*p) {
                    // If we see an equal sign, then compute the size of the name portion and scan for the end of the value.
                ...
                if (!(CompareResult = RtlCompareUnicodeString(Name, &CurrentName, TRUE))) {
                    // Names are equal.  Now find the end of the current environment variable block.
                    pEnd = p;
                    while (*pEnd) {
                        while (*pEnd++) {
                        }
                    ...
                    if (!ARGUMENT_PRESENT(Value)) {
                        ...
                        // New value is smaller, so copy new value, then null terminate it, 
                        // and then move up the remainder of the variable block so it is immediately after the new null terminated value.
                        ...
                        *pStart++ = L'\0';
                        ...
                        // New value is larger, so query the current size of the environment variable block.  Return status if failure.
                        Status = ZwQueryVirtualMemory(NtCurrentProcess(), pOld, MemoryBasicInformation, &MemoryInformation, sizeof(MemoryInformation), NULL);
                        if (!NT_SUCCESS(Status)) {
                            leave;
                        }

                        // See if there is room for new, larger value.  If not allocate a new copy of the environment variable block.
                        ...
                        if (NewSize >= MemoryInformation.RegionSize) {
                            ...
                            // Copy the current process's environment to the allocated memory inserting the new value as we do the copy.
                    ...
                    // Requested name is less than the current name.
                    // Save this spot in case the variable is not in a sorted position.
                    // The insertion point for the new variable is before the variable just examined.
            ...
            // If we found an insertion point, reset the string pointer back to it.
            if (InsertionPoint != NULL) {
                p = InsertionPoint;
            }

            // If variable name not found and a new value parameter was specified then insert the new variable name 
            // and its value at the appropriate place in the environment variable block (i.e. where p points to).
            if (pEnd == NULL && ARGUMENT_PRESENT(Value)) {
                if (p != NULL) {
                    // Name not found.  Now find the end of the current environment variable block.
                    pEnd = p;
                    while (*pEnd) {
                        while (*pEnd++) {
                        }
                    ...
                    // New value is present, so query the current size of the environment variable block.  Return status if failure.
                    Status = ZwQueryVirtualMemory(NtCurrentProcess(), pOld, MemoryBasicInformation, &MemoryInformation, sizeof(MemoryInformation), NULL);
                    if (!NT_SUCCESS(Status)) {
                        leave;
                    }
        ...
        {
            Status = STATUS_ACCESS_VIOLATION;// If abnormally terminating, assume access violation.
        }
    } finally{
        if (!ARGUMENT_PRESENT(Environment)) {
...
NTSTATUS NTAPI RtlSetEnvironmentStrings(IN PWCHAR NewEnvironment, IN SIZE_T NewEnvironmentSize)
/*
Routine Description:
...
*/
{
    ...
    // Reacquire the lock. The existing block may have been reallocated and so may now be big enough. Ignore this and use the block we have created anyway.
    RtlAcquirePebLock();
    pOld = ProcessParameters->Environment;
    ProcessParameters->Environment = pNew;
    ...
    OldSize = 0;
    Status1 = ZwFreeVirtualMemory(NtCurrentProcess(), &pOld, &OldSize, MEM_RELEASE);
