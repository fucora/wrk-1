...
Module Name:rtlexec.c
Abstract:User Mode routines for creating user mode processes and threads.
*/
...
VOID RtlpCopyProcString(IN OUT PWSTR *pDst, OUT PUNICODE_STRING DestString, IN PUNICODE_STRING SourceString, IN ULONG DstAlloc OPTIONAL)
{
    ...
    if (SourceString->Buffer != NULL && SourceString->Length != 0) {
        RtlCopyMemory(*pDst, SourceString->Buffer, SourceString->Length);
    }
...
NTSTATUS RtlCreateProcessParameters(
    ...
    Arguments:
        pProcessParameters - Pointer to a variable that will receive the address of the process parameter structure created by this routine.
        ...
        ShellInfo - An optional parameter that is an NT String variable that contains uninterpreted data that is passed as is to the target process.
                    If not specified, the target process will receive a pointer to an empty string.
...
#undef VALIDATE_OPTIONAL_STRING_PARAMETER
        ...
        p = RtlAllocateHeap(RtlProcessHeap(), 0, ByteCount);// Allocate memory for the process parameter record.
        if (p == NULL) {
...
NTSTATUS RtlpOpenImageFile(IN PUNICODE_STRING ImagePathName,
                           IN ULONG Attributes,
                         ...
                         OUT PINITIAL_TEB InitialTeb)
{
    ...
    // Enforce a minimal stack commit if there is a PEB setting for this.
...
{
    SIZE_T Zero = 0;
    NTSTATUS Status = ZwFreeVirtualMemory(Process, &InitialTeb->StackAllocationBase, &Zero, MEM_RELEASE);
    if (!NT_SUCCESS(Status)) {
#if DBG
...
*/
{
    ...
    if (RtlGetNtGlobalFlags() & FLG_ENABLE_CSRDEBUG) {
        if (wcsstr(NtImagePathName->Buffer, L"csrss") || wcsstr(NtImagePathName->Buffer, L"CSRSS")) {
            if (ISTERMINALSERVER()) {// For Hydra we don't name the CSRSS process to avoid name collissions when multiple CSRSS's are started
                InitializeObjectAttributes(&ObjectAttributes, NULL, 0, NULL, ProcessSecurityDescriptor);
            } else {
    ...
    Status = ZwQuerySection(Section, SectionImageInformation,
    ...
    if (ProcessInformation->ImageInformation.SubSystemType == IMAGE_SUBSYSTEM_NATIVE) {
        if (ProcessParameters->Flags & RTL_USER_PROC_RESERVE_1MB) {
            ...
            Status = ZwAllocateVirtualMemory(ProcessInformation->Process, (PVOID *)&Environment, 0, &RegionSize, MEM_RESERVE, PAGE_READWRITE);
            if (!NT_SUCCESS(Status)) {
                ZwClose(ProcessInformation->Process);
                ZwClose(Section);
                return(Status);
    ...
    if (s = (PWCHAR)ProcessParameters->Environment) {
        ...
        Status = ZwWriteVirtualMemory(ProcessInformation->Process, Environment, ProcessParameters->Environment, EnvironmentLength, NULL);
        if (!NT_SUCCESS(Status)) {
            ZwClose(ProcessInformation->Process);
            ZwClose(Section);
            return(Status);
    ...
    Status = ZwAllocateVirtualMemory(ProcessInformation->Process, (PVOID *)&Parameters, 0, &ParameterLength, MEM_COMMIT, PAGE_READWRITE);
    if (!NT_SUCCESS(Status)) {
        ZwClose(ProcessInformation->Process);
        ZwClose(Section);
        return(Status);
    ...
    */
{
    ...
    try {// Create an initial context for the new thread.
    ...
    // The new thread will not have a name and its handle will not be inherited by other processes.
    InitializeObjectAttributes(&ObjectAttributes, NULL, OBJ_KERNEL_HANDLE, NULL, ThreadSecurityDescriptor);
