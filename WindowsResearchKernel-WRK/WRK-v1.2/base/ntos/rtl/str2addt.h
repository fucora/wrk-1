...
NTSTATUS RtlIpv6StringToAddressT(IN LPCTSTR S, OUT LPCTSTR *Terminator, OUT struct in6_addr *Addr)
/*
Routine Description:
    Parses the string S as an IPv6 address. See RFC 1884.
    The basic string representation consists of 8 hex numbers separated by colons, with a couple embellishments:
    - a string of zero numbers (at most one) may be replaced with a double-colon. Double-colons are allowed at beginning/end of the string.
...
Arguments:
    ...
    Terminator - Receives a pointer to the character that terminated the conversion.
...
*/
{
    ...
    // There are a several difficulties here.
    // For one, we don't know when we see a double-colon how many zeroes it represents.
    // So we just remember where we saw it and insert the zeroes at the end.
    // For another, when we see the first digits of a number we don't know if it is hex or decimal.
    // So we remember a pointer to the first character of the number and convert it after we see the following character.
    ...
    while (c = *S) {
        ...
        if ((state != InNumber) && (number != NULL)) {
            // Note either numDots > 0 or numColons > 0, because something terminated the number.
            if (numDots == 0) {
                if (numDigits > 4)
...
Finish:
    *Terminator = S;
    ...
    if (sawDoubleColon) {
        RtlMoveMemory(&Addr->s6_words[sawDoubleColon + 8 - numColons], &Addr->s6_words[sawDoubleColon], (numColons - sawDoubleColon) * sizeof(USHORT));
...
NTSTATUS RtlIpv6StringToAddressExT (IN LPCTSTR AddressString, OUT struct in6_addr *Address, OUT PULONG ScopeId, OUT PUSHORT Port)
/*
Routine Description:
    Parsing a human-readable string to Address, port number and scope id.

    The syntax is address%scope-id or [address%scope-id]:port, where the scope-id and port are optional.
    Note that since the IPv6 address format uses a varying number of ':' characters, the IPv4 convention of address:port cannot be supported without the braces.

Arguments:
    AddressString - Points to the zero-terminated human-readable string.
    Address - Receive address part (in6_addr) of this address.
    ScopeId - Receive scopeid of this address. If there is no scope id in the address string, 0 is returned.
    Port - Receive port number of this address. If there is no port number in the string, 0 is returned. Port is returned in network byte order.
Return Value:
...
*/
{
    ...
    // Quick sanity checks.
    if ((AddressString == NULL) || (Address == NULL) || (ScopeId == NULL) || (Port == NULL)) {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (!NT_SUCCESS(RtlIpv6StringToAddressT(AddressString, &Terminator, Address))) {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (*Terminator == _T('%')) {
        ...
        while ((Ch != 0) && (Ch != _T(']'))) {
            ...
            if (((ULONGLONG)TempScopeId * 10 + Ch - _T('0')) > 0xFFFFFFFF) {
                return STATUS_INVALID_PARAMETER;
            }
    ...
    // When we come here, the current char should either be the end of the string or ']' if expectbrace is true.
    if (*Terminator == _T(']')) {
        if (!ExpectBrace) {
            return STATUS_INVALID_PARAMETER;
        }
        ...
        if (*Terminator == _T(':')) {
            ...
            while (Ch != 0) {
                if (ISDIGIT(Ch) && (Ch - _T('0')) < Base) {
                    ...
                    if ((((ULONG)TempPort << 4) + Ch + 10 - (ISLOWER(Ch)? _T('a') : _T('A'))) > 0xFFFF) {
                        return STATUS_INVALID_PARAMETER;
                    }
                    TempPort = (TempPort << 4);
    ...
    // We finished parsing address, scope id and port number. We are expecting the end of the string.
...
NTSTATUS RtlIpv4StringToAddressT(IN LPCTSTR String, IN BOOLEAN Strict, OUT LPCTSTR *Terminator, OUT struct in_addr *Addr)
/*
Routine Description:
    ...
    address.  Note that when an Internet address is viewed as a 32-bit integer quantity on the Intel architecture, the bytes referred to
    above appear as "d.c.b.a''.  That is, the bytes on an Intel processor are ordered from right to left.
...
Arguments:
    String - A character string representing a number expressed in the Internet standard "." notation.
    Strict - If TRUE, the string must be dotted-decimal with 4 parts.
             Otherwise, any of the four forms are allowed, with decimal, octal, or hex.
    Terminator - Receives a pointer to the character that terminated the conversion.
    Addr - Receives a pointer to the structure to fill in with a suitable binary representation of the Internet address given.
Return Value:
    TRUE if parsing was successful. FALSE otherwise.
*/
{
...
again:
    ...
    if (*String == _T('0')) {
        ...
        if (ISDIGIT(*String)) {
            ...
            // It is still decimal but we saw the digit and it was 0.
            sawDigit = TRUE;
        }
    }
    if (Strict && (base != 10)) {
...
NTSTATUS RtlIpv4StringToAddressExT (IN LPCTSTR AddressString, IN BOOLEAN Strict, OUT struct in_addr *Address, OUT PUSHORT Port)
/*
Routine Description:
...
*/
{
    ...
    if ((AddressString == NULL) || (Address == NULL) || (Port == NULL)) {
        return STATUS_INVALID_PARAMETER;
    }

    if (!NT_SUCCESS(RtlIpv4StringToAddressT(AddressString, Strict, &Terminator, Address))) {
        return STATUS_INVALID_PARAMETER;
    }
