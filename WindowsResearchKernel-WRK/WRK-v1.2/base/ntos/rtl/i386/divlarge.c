...
Abstract:
    This module implements the NT runtime library large integer divide routines.
...
Arguments:
    ...
    Remainder - Supplies an optional pointer to a variable which receives the remainder
Return Value:
...
*/
{
    ...
    Quotient = Dividend;
    do {
        // Shift the next dividend bit into the parital remainder and shift the partial quotient (dividend) left one bit.
        ...
        // then subtract the divisor from the partial remainder and insert a one bit into the quotient.
