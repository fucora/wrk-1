...
; Routine Description:
;    This function raises a software exception by building a context record, and calling the exception dispatcher.
;    If the exception dispatcher finds a handler to process the exception the handler is invoked and execution is resumed.
;    Otherwise the ZwRaiseException system service is invoked to provide default handling.
...
; Invoke the helper routine to capture the caller's context.
; This will capture the segment registers, volatile/nonvolatile registers, and the control registers.
; Note that the status flags have not been perturbed from their state prior to this call.
