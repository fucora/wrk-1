...
EXCEPTION_DISPOSITION RtlpExecuteHandlerForException(IN PEXCEPTION_RECORD ExceptionRecord,
...
EXCEPTION_DISPOSITION RtlpExecuteHandlerForUnwind(IN PEXCEPTION_RECORD ExceptionRecord, 
                                                  IN PVOID EstablisherFrame,
                                                  IN OUT PCONTEXT ContextRecord,
                                                  IN OUT PVOID DispatcherContext,
                                                  IN PEXCEPTION_ROUTINE ExceptionRoutine);
...
VOID RtlInvalidHandlerDetected(PVOID Handler, PULONG FunctionTable, ULONG FunctionTableLength)
{
    return;
}
...
BOOLEAN RtlIsValidHandler(IN PEXCEPTION_ROUTINE Handler)
{
    ...
    if (FunctionTable && FunctionTableLength) {
        ...
        // Bias the handler value down by the image base and see if the result is in the table
...
*/
{
    ...
    while (RegistrationPointer != EXCEPTION_CHAIN_END) {
        ...
        // The handler must be executed by calling another routine that is written in assembler. This is required because
        // up level addressing of the handler information is required when a nested exception is encountered.
...
*/
{
    ...
    while (RegistrationPointer != EXCEPTION_CHAIN_END) {
        ...
        if (((ULONG)RegistrationPointer < LowLimit) || (HighAddress > HighLimit) || (((ULONG)RegistrationPointer & 0x3) != 0)) {
            ...
            // The handler must be executed by calling another routine that is written in assembler. This is required because
            // up level addressing of the handler information is required when a collided unwind is encountered.
    ...
    if (TargetFrame == EXCEPTION_CHAIN_END) {
        ...
        //  Either (1) a real exit unwind was performed, or (2) the specified TargetFrame is not present in the exception handler
        //  list.  In either case, give debugger and subsystem a chance to see the unwind.
