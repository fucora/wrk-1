...
Abstract:
    This module contains routines to get runtime stack traces for the x86 architecture.
*/
...
BOOLEAN RtlpCaptureStackLimits(ULONG_PTR HintAddress, PULONG_PTR StartStack, PULONG_PTR EndStack);
BOOLEAN RtlpStkIsPointerInDllRange(ULONG_PTR Value);
BOOLEAN RtlpStkIsPointerInNtdllRange(ULONG_PTR Value);
...
*/
{
    ...
    try {
        // If we need to get the user mode stack trace from kernel mode figure out the proper limits.
        if (Flags == 1) {
            ...
            // or the irql is greater than dispatch, this code can't log a stack.
        ...
        for (Index = 0; Index < Count; Index += 1) {
            if (Fp >= StackEnd ||
                ((Index == 0) ? (Fp < StackStart) : (Fp <= StackStart)) ||
...
ULONG RtlCaptureStackContext(OUT PULONG_PTR Callers, OUT PRTL_STACK_CONTEXT Context, IN ULONG Limit)
/*
Routine Description:
    ...
    Note. Based on experiments you need to save at least 64 pointers to be sure you get a complete stack.
...
Arguments:
    ...
    Context - if not null the caller wants the stack context to be saved here as opposed to the Callers parameter.
    Limit - # of pointers that can be written into Callers and Offsets.
Return value:
    The number of potential callers detected and written into the `Callers' buffer.
*/
{
    ULONG_PTR Current;
    ULONG_PTR Value;
    ...
    // Synchronize stack traverse pointer to the next word after the first return address.
...
Routine description:
    ...
    RtlpStkDllRanges variable. This global is used within RtlpDetectDllReferences to save a stack context.
Arguments:
...
Routine Description:
    ...
    thread might execute a DPC routine and in this case we use a different stack with different limits.
Arguments:
    HintAddress - Address of a local variable or parameter of the caller of the function that should be the start of the stack.
    ...
    *EndStack = (ULONG_PTR)(KeGetCurrentThread()->StackBase);
    ...
    if (*StartStack <= HintAddress && HintAddress <= *EndStack) {
        ...
        *StartStack = *EndStack - KERNEL_STACK_SIZE;
        ...
        // Check if this is within the DPC stack for the current processor.
        if (*EndStack && *StartStack <= HintAddress && HintAddress <= *EndStack) {
            *StartStack = HintAddress;
        } else {
            // This is not current thread's stack and is not the DPC stack of the current processor.
            // We will return just the rest of the stack page containing HintAddress as a valid stack range.
            *StartStack = HintAddress;
