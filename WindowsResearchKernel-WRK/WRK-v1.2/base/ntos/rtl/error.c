...
*/
{
    ...
    if (Status & 0x20000000) {
        return Status;// The customer bit is set so lets just pass the error code on thru
...
{
    RtlSetLastWin32Error(RtlNtStatusToDosError(Status));// RtlNtStatusToDosError stores into NtCurrentTeb()->LastStatusValue.
}


VOID NTAPI RtlSetLastWin32Error(LONG Win32Error)
{
    // Arguably this should clear or reset the last nt status, but it does not touch it.
