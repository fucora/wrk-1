...
Abstract:
    Creation and Maintenance of the NTFS "System Volume Information" directory.
*/

#include "ntrtlp.h"

PVOID RtlpSysVolAllocate(IN  ULONG   Size);
VOID RtlpSysVolFree(IN  PVOID   Buffer);
NTSTATUS RtlpSysVolCreateSecurityDescriptor(OUT PSECURITY_DESCRIPTOR*   SecurityDescriptor, OUT PACL*                   Acl);
NTSTATUS RtlpSysVolCheckOwnerAndSecurity(IN  HANDLE  Handle, IN  PACL    StandardAcl);
...
NTSTATUS RtlpSysVolCreateSecurityDescriptor(OUT PSECURITY_DESCRIPTOR*   SecurityDescriptor, OUT PACL*                   Acl)
{
    ...
    status = RtlAddAccessAllowedAceEx(acl, ACL_REVISION, OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE, STANDARD_RIGHTS_ALL | SPECIFIC_RIGHTS_ALL, systemSid);
    if (!NT_SUCCESS(status)) {
        RtlpSysVolFree(acl);
        RtlpSysVolFree(sd);
        return status;
...
NTSTATUS RtlpSysVolCheckOwnerAndSecurity(IN  HANDLE  Handle, IN  PACL    StandardAcl)
{
    ...
    status = NtQuerySecurityObject(Handle, OWNER_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION, sd, sdLength, &sdLength);
    if (!NT_SUCCESS(status)) {
        RtlpSysVolFree(sd);
        return status;
    }
...
/*
Routine Description:
    This routine is called when the open for the directory failed.
    This routine will attempt to set the owner of the file to the caller's ownership so that another attempt to open the file can be attempted.
Arguments:
...
*/
{
    ...
    status = NtOpenFile(&fileHandle, WRITE_OWNER | SYNCHRONIZE, &oa, &ioStatus, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_SYNCHRONOUS_IO_NONALERT | FILE_DIRECTORY_FILE);
    if (!NT_SUCCESS(status)) {
        NtClose(tokenHandle);
        return;
    }
...
NTSTATUS RtlCreateSystemVolumeInformationFolder(IN  PUNICODE_STRING VolumeRootPath)
/*
Routine Description:
    This routine verifies the existence of the "System Volume Information" folder on the given volume.
    If the folder is not present, then the folder is created with one ACE indicating full access for SYSTEM.
    The ACE will have the inheritance bits set.
    The folder will be created with the HIDDEN and SYSTEM attributes set.

    If the folder is already present, the ACE that indicates full control for SYSTEM will be checked and if necessary modified to have the inheritance bits set.
Arguments:
...
*/
{
    ...
    // Check for wrapping.
    if (dirName.Length < VolumeRootPath->Length || dirName.Length < sysVolName.Length) {
    ...
    status = NtCreateFile(&h, READ_CONTROL | WRITE_DAC | WRITE_OWNER | SYNCHRONIZE, &oa, &ioStatus, NULL, FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM,
                          FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN_IF, FILE_SYNCHRONOUS_IO_NONALERT | FILE_DIRECTORY_FILE, NULL, 0);
    if (!NT_SUCCESS(status)) {
