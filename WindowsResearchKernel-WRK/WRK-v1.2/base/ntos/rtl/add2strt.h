...
LPTSTR RtlIpv6AddressToStringT(__in const struct in6_addr *Addr, __out_ecount(INET6_ADDRSTRLEN) LPTSTR S)
/*
Routine Description:
...
Arguments:
    S - Receives a pointer to the buffer in which to place the string literal.
...
*/
{
    ...
    if ((Addr->s6_words[0] == 0) && (Addr->s6_words[1] == 0) && (Addr->s6_words[2] == 0) && (Addr->s6_words[3] == 0) && (Addr->s6_words[6] != 0)) {
        if ((Addr->s6_words[4] == 0) && ((Addr->s6_words[5] == 0) || (Addr->s6_words[5] == 0xffff)))
        {
            // compatible or mapped
            S += _stprintf(S, _T("::%hs%u.%u.%u.%u"), Addr->s6_words[5] == 0 ? "" : "ffff:", Addr->s6_bytes[12], Addr->s6_bytes[13], Addr->s6_bytes[14], Addr->s6_bytes[15]);
            return S;
        } else if ((Addr->s6_words[4] == 0xffff) && (Addr->s6_words[5] == 0)) {
            // translated
            S += _stprintf(S, _T("::ffff:0:%u.%u.%u.%u"), Addr->s6_bytes[12], Addr->s6_bytes[13], Addr->s6_bytes[14], Addr->s6_bytes[15]);
            return S;
        }
    }
    ...
    if (endHex < 8) {
        S += _stprintf(S, _T(":%u.%u.%u.%u"), Addr->s6_bytes[12], Addr->s6_bytes[13], Addr->s6_bytes[14], Addr->s6_bytes[15]);
    }
...
NTSTATUS RtlIpv6AddressToStringExT(
...
/*
Routine Description:
    This is the extension routine which handles a full address conversion including address, scopeid and port (scopeid and port are optional).
Arguments:
    ...
    AddressStringLength - For input, it is the length of the input buffer; for output it is the length we actual returned.
Return Value:
...
*/
{
    ...
    if ((Address == NULL) || (AddressString == NULL) || (AddressStringLength == NULL)) {
    ...
    if (*AddressStringLength < Length) {
        // Before return, tell the caller how big the buffer we need.
...
LPTSTR RtlIpv4AddressToStringT(__in const struct in_addr *Addr, __out_ecount(16) LPTSTR S)
/*
Routine Description:
...
NTSTATUS RtlIpv4AddressToStringExT(
...
/*
Routine Description:
    This is the extension routine which handles a full address conversion including address and port (port is optional).
Arguments:
    Address - The address part to translate.
    Port - Port number if there is any, otherwise 0. Port is in network byte order.
    AddressString - Receives the formatted address string.
    AddressStringLength - On input, contains the length of AddressString.
        On output, contains the number of characters actually written to AddressString.
Return Value:
    STATUS_SUCCESS if the operation is successful, error code otherwise.
*/
{
