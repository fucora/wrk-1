...
// Macros to hide the different synchronization routines for user mode and kernel mode runtimes.
// For kernel runtime the OKAY_TO_LOCK macro points to a real function that makes sure current thread is not executing a DPC routine.
...
NTSTATUS KeInitializeSpinLockEx(PKSPIN_LOCK_EX Lock)
{
...
VOID KeAcquireSpinLockEx(PKSPIN_LOCK_EX Lock)
{
...
VOID KeReleaseSpinLockEx(PKSPIN_LOCK_EX Lock)
{
    Lock->Owner = NULL;
...
USHORT RtlpLogStackBackTraceEx(ULONG FramesToSkip);
LOGICAL RtlpCaptureStackTraceForLogging(
    PRTL_STACK_TRACE_ENTRY Trace,
    PULONG Hash,
    ULONG FramesToSkip,
    LOGICAL UserModeStackFromKernelMode
);
USHORT RtlpLogCapturedStackTrace(PRTL_STACK_TRACE_ENTRY Trace, ULONG Hash);
PRTL_STACK_TRACE_ENTRY RtlpExtendStackTraceDataBase(IN PRTL_STACK_TRACE_ENTRY InitialValue, IN SIZE_T Size);

// Global per process (user mode) or system wide (kernel mode) stack trace database.
PSTACK_TRACE_DATABASE RtlpStackTraceDataBase;

// Resource used to control access to stack trace database.
// We opted for this solution so that we kept change in the database structure to an absolute minimal.
// This way the tools that depend on this structure (at least umhd and oh) will not need a new version and will not introduce backcompatibility issues.
...
// This is a problem only on x86 if "fuzzy" stack traces are used.
// The typical function used to log the trace is RtlLogStackBackTrace.
// One of the worst limitations of this package is that traces are referred using a ushort index which means we cannot
// ever store more than 65535 traces (remember we never delete traces).
PSTACK_TRACE_DATABASE RtlpAcquireStackTraceDataBase()
{
...
VOID RtlpReleaseStackTraceDataBase()
{
    PSTACK_TRACE_DATABASE DataBase;
    DataBase = RtlpStackTraceDataBase;
...
NTSTATUS RtlInitializeStackTraceDataBase(IN PVOID CommitBase, IN SIZE_T CommitSize, IN SIZE_T ReserveSize)
{
    ...
    if (CommitSize == 0) {
        // Initially commit enough pages to accommodate the increased number of hash chains (for improved performance we switched from ~100
        // to ~1000 in the hope that the hash chains will decrease ten-fold in length).
...
/*
Routine Description:
    This routine extends the stack trace database in order to accommodate the new stack trace that has to be saved.
Arguments:
    InitialValue - stack trace to be saved.
    Size - size of the stack trace in bytes. Note that this is not the depth of the trace but rather `Depth * sizeof(PVOID)'.
Return Value:
    The address of the just saved stack trace or null in case we have hit the maximum size of the database or we get commit errors.
Environment:
    User mode.

    Note. In order to make all this code work in kernel mode we have to rewrite this function that relies on VirtualAlloc.
*/
{
    NTSTATUS Status;
    ...
    // We will try to find space for one stack trace entry in the upper part of the database.
    ...
    if ((!DataBase->PreCommitted) && ((PCHAR)(pp - 1) < (PCHAR)DataBase->CurrentUpperCommitLimit)) {
        ...
        Status = ZwAllocateVirtualMemory(NtCurrentProcess(), (PVOID *)&DataBase->CurrentUpperCommitLimit, 0, &CommitSize, MEM_COMMIT, PAGE_READWRITE);
        if (!NT_SUCCESS(Status)) {
    ...
    // We managed to make sure we have usable space in the upper part therefore we take out one stack trace entry address.
    ...
    if ((!DataBase->PreCommitted) && (((PCHAR)p + Size) > (PCHAR)DataBase->CurrentLowerCommitLimit)) {
        ...
        CommitSize = Size;
        Status = ZwAllocateVirtualMemory(NtCurrentProcess(), (PVOID *)&DataBase->CurrentLowerCommitLimit, 0, &CommitSize, MEM_COMMIT, PAGE_READWRITE);
        if (!NT_SUCCESS(Status)) {
            DataBase->NextFreeUpperMemory += sizeof(*pp);
            return NULL;
        }
    ...
    // Deal with a precommitted database case. If the lower and upper pointers have crossed each other then rollback and return failure.
    ...
    // Save the address of the new stack trace entry in the upper part of the databse.
    *--pp = p;
...
USHORT RtlLogStackBackTrace(VOID)
/*
Routine Description:
    This routine will capture the current stacktrace (skipping the present function) and will save it in the global (per process) stack trace database.
    It should be noted that we do not save duplicate traces.
Return Value:
    Index of the stack trace saved. The index can be used by tools to access quickly the trace data.
    This is the reason at the end of the database we save downwards a list of pointers to trace entries.
    This index can be used to find this pointer in constant time.

    A zero index will be returned for error conditions (e.g. stack trace database not initialized).
Environment:
    User mode.
*/
{
...
Routine Description:
    ...
    It should be noted that we do not save duplicate traces.
Arguments:
    FramesToSkip - no of frames that are not interesting and should be skipped.
Return Value:
    Index of the stack trace saved. The index can be used by tools to access quickly the trace data.
    This is the reason at the end of the database we save downwards a list of pointers to trace entries.
    This index can be used to find this pointer in constant time.
...
/*
Routine Description:
    This routine will capture the user mode stacktrace and will save it in the global (per system) stack trace database.
    It should be noted that we do not save duplicate traces.
Return Value:
    Index of the stack trace saved. The index can be used by tools to access quickly the trace data.
    This is the reason at the end of the database we save downwards a list of pointers to trace entries.
...
#pragma optimize("y", off) // disable FPO
LOGICAL RtlpCaptureStackTraceForLogging(PRTL_STACK_TRACE_ENTRY Trace, PULONG Hash, ULONG FramesToSkip, LOGICAL UserModeStackFromKernelMode)
{
    if (UserModeStackFromKernelMode == FALSE) {
        // Capture stack trace.
        // The try/except was useful in the old days when the function did not validate the stack frame chain. We keep it just to be defensive.
        try {
            Trace->Depth = RtlCaptureStackBackTrace(FramesToSkip + 1, MAX_STACK_DEPTH, Trace->BackTrace, Hash);
        }
        ...
        Trace->Depth = (USHORT)RtlWalkFrameChain(Trace->BackTrace, MAX_STACK_DEPTH, 1);
        if (Trace->Depth == 0) {
            return FALSE;
        } else {
            *Hash = 0;
...
USHORT RtlpLogCapturedStackTrace(PRTL_STACK_TRACE_ENTRY Trace, ULONG Hash)
{
    ...
    // Update statistics counters. Since they are used only for reference and do not control decisions we increment them without protection even if this means we may
    ...
    if (RtlpAcquireStackTraceDataBase() == NULL) {
        ...
        // This can happen only if there is a dump in progress or we are in an invalid context (process shutdown (Umode) or DPC routine (Kmode).
        return 0;
    }
    ...
    try {
        ...
        if (p == NULL) {
            // If we get here we did not find a similar trace in the database. We need to add it.

            // We got the `*pp' value (address of last chain element) while the database lock was acquired shared so we need to take into consideration
            // the case where another thread managed to acquire database exclusively and add a new trace at the end of the chain.
            // Therefore if `*pp' is no longer null we continue to traverse the chain until we get to the end.
            p = NULL;
            if (*pp != NULL) {
                // Somebody added some traces at the end of the chain while we were trying to convert the lock from shared to exclusive.
                while (p = *pp) {
                    if (p->Depth == Trace->Depth) {
                        if (RtlCompareMemory(&p->BackTrace[0], &Trace->BackTrace[0], DepthSize) == DepthSize) {
                            break;
            ...
            if (p == NULL) {
                // Nobody added the trace and now `*pp' really points to the end of the chain either because we traversed the rest of the chain or it was at the end anyway.
                ...
                // Some other thread managed to add the same trace to the database while we were trying to acquire the lock exclusive.
                // `p' has the address to the stack trace entry.
            }
        }
    }
    except(EXCEPTION_EXECUTE_HANDLER)
    ...
    // At this stage we may return zero (failure) if we did not manage to extend the database with a new trace (e.g. due to out of memory conditions).
    if (p != NULL) {
        p->TraceCount += 1;
        ReturnValue = p->Index;
    } else {
...
PVOID RtlpGetStackTraceAddress(USHORT Index)
{
    if (RtlpStackTraceDataBase == NULL) {
        return NULL;
    }
...
ULONG RtlpWalkFrameChainExceptionFilter(ULONG ExceptionCode, PVOID ExceptionRecord)
/*
Routine Description:
    This routine is the exception filter used by RtlWalkFrameChain function.
    This function is platform specific (different code for x86, amd64, ia64) but the exception filter is common.
Arguments:
...
{
#if DBG
    // We skip STATUS_IN_PAGE because this exception can be raised by MM while resolving a page fault.
    // This can happen in an obscure corner case even if RtlWalkFrameChain called MmCanThreadFault.
