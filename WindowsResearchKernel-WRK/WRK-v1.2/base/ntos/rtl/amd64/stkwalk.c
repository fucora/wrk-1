...
Abstract:
    This module implements the routine to get the callers and the callers caller address.
*/
...
PRUNTIME_FUNCTION RtlpConvertFunctionEntry(IN PRUNTIME_FUNCTION FunctionEntry, IN ULONG64 ImageBase);


PRUNTIME_FUNCTION RtlpLookupFunctionEntryForStackWalks(IN ULONG64 ControlPc, OUT PULONG64 ImageBase)
/*
Routine Description:
    This function searches the currently active function tables for an entry
    that corresponds to the specified control PC. This function does not consider the runtime function table.
Arguments:
...
Arguments:
    ...
    CallersCallersPc - Supplies a pointer to a variable that receives the
        address of the caller of the caller of the caller of this routine (A).
Return Value:
    None.
Note:
...
DECLSPEC_NOINLINE ULONG RtlpWalkFrameChain(OUT PVOID *Callers, IN ULONG Count, IN ULONG Flags, IN ULONG FramesToSkip)
/*
Routine Description:
...
Arguments:
    Callers - Supplies a pointer to an array that is to received the return address values.
...
*/
{
    ...
    } except(RtlpWalkFrameChainExceptionFilter(GetExceptionCode(), GetExceptionInformation()))
    {
        Index = 0;
    }
...
DECLSPEC_NOINLINE ULONG RtlWalkFrameChain(OUT PVOID *Callers, IN ULONG Count, IN ULONG Flags)
/*
Routine Description:
    This is a wrapper function for walk frame chain.
    It's purpose is to prevent entering a function that has a huge stack usage to test some if the current code path can take page faults.
