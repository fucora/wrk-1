...
; EXCEPTION_DISPOSITION RtlpExceptionHandler (IN PEXCEPTION_RECORD ExceptionRecord, IN PVOID EstablisherFrame, IN OUT PCONTEXT ContextRecord, IN OUT PVOID DispatcherContext)

; Routine Description:
;   This function is called when a nested exception occurs. Its function
;   is to retrieve the establisher frame pointer from its establisher's call frame, store this information in the dispatcher context record, and return a disposition value of nested exception.
...
; Return Value:
;   A disposition value nested exception is returned if an unwind is not in progress. Otherwise a value of continue search is returned.
...
; Routine Description:
;   This function allocates a call frame, saves the dispatcher context address, establishes an exception handler, and calls the specified language specific handler routine.
;   If a nested exception occurs, then the exception handler of this function is called and the establisher frame pointer is returned to the exception dispatcher via the dispatcher context parameter.

; Arguments:
;   ExceptionRecord (rcx) - Supplies a pointer to an exception record.
;   EstablisherFrame (rdx) - Supplies the frame pointer of the establisher of the exception handler that is to be called.
;   ContextRecord (r8) - Supplies a pointer to a context record.
;   DispatcherContext (r9) - Supplies a pointer to the dispatcher context record.

; Return Value:
;   The disposition value returned by the specified exception handler is returned as the function value.
...
; EXCEPTION_DISPOSITION RtlpUnwindHandler (IN PEXCEPTION_RECORD ExceptionRecord, IN PVOID EstablisherFrame, IN OUT PCONTEXT ContextRecord, IN OUT PVOID DispatcherContext)

; Routine Description:
;   This function is called when a collided unwind occurs. Its function
;   is to retrieve the establisher dispatcher context, copy it to the current dispatcher context, and return a disposition value of collided unwind.
...
; Return Value:
;   A disposition value collided unwind is returned if an unwind is in progress. Otherwise a value of continue search is returned.
...
; Routine Description:
;   This function allocates a call frame, saves the dispatcher context address, establishes an exception handler, and calls the specified unwind handler.
;   If a collided unwind occurs, then the exception handler of this function is called and the establisher dispatcher context is copied to the current
...
;   ContextRecord (r8) - Supplies a pointer to a context record.
;   DispatcherContext (r9) - Supplies a pointer to the dispatcher context record.
;   ExceptionRoutine (5 * 8[rsp]) - Supplies a pointer to the exception handler that is to be called.
