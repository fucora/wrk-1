...
; Routine Description:
;   This function executes a breakpoint instruction. Useful for entering the debugger under program control.
;   This breakpoint will always go to the kernel debugger if one is installed, otherwise it will go to the debug subsystem.
...
; Routine Description:
;   This function executes a breakpoint instruction. Useful for entering the debug subsystem under program control.
;   The kernel debugger will ignore this breakpoint since it will not find the instruction address its breakpoint table.
...
;   This function executes a breakpoint instruction. Useful for entering the debugger under program control.
;   This breakpoint will always go to the kernel debugger if one is installed, otherwise it will go to the debug subsystem.
;   This function is identical to DbgBreakPoint, except that it takes an argument which the debugger can see.

;   Note: The debugger checks the address of the breakpoint instruction against the address RtlpBreakWithStatusInstruction.
;   If it matches, we have a breakpoint with status.
;   A breakpoint is normally issued with the break_debug_stop macro which generates two instructions.
...
; VOID DebugService2(IN PVOID Param1, IN PVOID Param2, IN ULONG Service)
; Routine Description:
