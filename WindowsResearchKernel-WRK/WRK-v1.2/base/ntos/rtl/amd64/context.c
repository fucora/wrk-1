    ...
    */
{
    ...
    Context->EFlags = EFLAGS_IF_MASK;// Initialize the EFflags field.
    ...
    Context->Rip = (ULONG64)InitialPc;// Initialize the program counter.
    Context->ContextFlags = CONTEXT_FULL;// Set context record flags.
    Context->Rcx = (ULONG64)Parameter;// Set the initial context of the thread in a machine specific way.
    Context->R9 = 0xf0e0d0c0a0908070UI64;// Unique stamp to tell wow64 that this is an RtlCreateUserThread context rather than BaseCreateThread context.
}
...
NTSTATUS RtlRemoteCall(HANDLE Process,
    ...
    /*
    Routine Description:
        This function calls a procedure in another thread/process using the system functions get and set context.
        Parameters are passed to the target procedure via the nonvolatile registers.
    Arguments:
    ...
    */
{
    ...
    // Pass the parameters to the target thread via the nonvolatile registers R11-R15.
    Context.Rsp = NewSp;
    ...
    // Set the address of the target code into RIP and set the thread context to cause the target procedure to be executed.
