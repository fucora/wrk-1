...
// Define lookup table for providing the number of slots used by each unwind code.
    ...
    do {
        ...
        if (FunctionEntry != NULL) {
            ...
            if (RtlpIsFrameInBounds(&LowLimit, EstablisherFrame, &HighLimit) == FALSE) {
                ...
                // A linkage routine written in assembler is used to actually call the actual exception handler. This is required by the
                // exception handler that is associated with the linkage routine so it can have access to two sets of dispatcher context when it is called.
                ...
                do {
                    ...
                    ExceptionFlags |= (ExceptionRecord->ExceptionFlags & EXCEPTION_NONCONTINUABLE);// Propagate noncontinuable exception flag.
                    ...
                    switch (Disposition) {
                        ...
                        // If the exception is not continuable, then raise the exception STATUS_NONCONTINUABLE_EXCEPTION.
                    ...
                    case ExceptionNestedException:
                        ...
                        // A collided unwind occurs when an exception dispatch encounters a previous call to an unwind handler. 
                        // In this case the previous unwound frames must be skipped.
            ...
            // Set the point where control left the current function by obtaining the return address from the top of the stack.
            ContextRecord1.Rip = *(PULONG64)(ContextRecord1.Rsp);
            ContextRecord1.Rsp += 8;
        }

        ControlPc = ContextRecord1.Rip;// Set point at which control left the previous routine.
    } while (RtlpIsFrameInBounds(&LowLimit, (ULONG64)ContextRecord1.Rsp, &HighLimit) == TRUE);

    ExceptionRecord->ExceptionFlags = ExceptionFlags;// Set final exception flags and return exception not handled.
...
*/
{
    ...
    // Call real unwind routine specifying a local context record and history table address as extra arguments.
...
Arguments:
    ...
    ExceptionRecord - Supplies an optional pointer to an exception record.
    ReturnValue - Supplies a value that is to be placed in the integer function return register just before continuing execution.
    OriginalContext - Supplies a pointer to a context record that can be used to store context during the unwind operation.
...
*/
{
    ...
    // Get current stack limits, capture the current context, 
    // virtually unwind to the caller of this routine, get the initial PC value, and set the unwind target address.
    CurrentContext = OriginalContext;
    PreviousContext = &LocalContext;
    RtlpGetStackLimits(&LowLimit, &HighLimit);
    RtlCaptureContext(CurrentContext);

    if (ARGUMENT_PRESENT(HistoryTable)) {// If a history table is specified, then set to search history table.
    ...
    // Scan backward through the call frame hierarchy and call exception handlers until the target frame of the unwind is reached.
    do {
        ...
        if (FunctionEntry != NULL) {
            ...
            // Otherwise, check to determine if the current routine has an exception handler.
            if ((RtlpIsFrameInBounds(&LowLimit, EstablisherFrame, &HighLimit) == FALSE) ||
                ((ARGUMENT_PRESENT(TargetFrame) != FALSE) && ((ULONG64)TargetFrame < EstablisherFrame))) {
                RtlRaiseStatus(STATUS_BAD_STACK);
            } else if (ExceptionRoutine != NULL) {// The frame has a exception handler.
                // A linkage routine written in assembler is used to actually call the actual exception handler.
                // This is required by the exception handler that is associated with the linkage routine so it can have access to two sets of dispatcher context when it is called.

                // Call the language specific handler.
                DispatcherContext.TargetIp = (ULONG64)TargetIp;
                ScopeIndex = 0;
                do {
                    // If the establisher frame is the target of the unwind operation, then set the target unwind flag.
                    if ((ULONG64)TargetFrame == EstablisherFrame) {
                        ExceptionFlags |= EXCEPTION_TARGET_UNWIND;
                    }

                    ExceptionRecord->ExceptionFlags = ExceptionFlags;
                    CurrentContext->Rax = (ULONG64)ReturnValue;// Set the specified return value and target IP in case the exception handler directly continues execution.
                    ...
                    DispatcherContext.HistoryTable = HistoryTable;
                    DispatcherContext.ScopeIndex = ScopeIndex;
                    Disposition = RtlpExecuteHandlerForUnwind(ExceptionRecord, EstablisherFrame, CurrentContext, &DispatcherContext);

                    ExceptionFlags &= ~(EXCEPTION_COLLIDED_UNWIND | EXCEPTION_TARGET_UNWIND);// Clear target unwind and collided unwind flags.

                    // Case on the handler disposition.
                    switch (Disposition) {
                        // The disposition is to continue the search.                        
                    case ExceptionContinueSearch:// If the target frame has not been reached, then swap context pointers.
                        ...
                        // Copy the context of the previous unwind and virtually unwind to the caller of the establisher,
                        // then set the target of the current unwind to the dispatcher context of the previous unwind,
                        // and reexecute the exception handler from the collided frame with the collided unwind flag set in the exception record.
                    case ExceptionCollidedUnwind:
                        ControlPc = DispatcherContext.ControlPc;
                        ImageBase = DispatcherContext.ImageBase;
                        FunctionEntry = DispatcherContext.FunctionEntry;
                ...
                if (EstablisherFrame != (ULONG64)TargetFrame) {// If the target frame has not been reached, then swap context pointers.
                    TempContext = CurrentContext;
                    CurrentContext = PreviousContext;
                    PreviousContext = TempContext;
                }
            }
        } else {// Set the point where control left the current function by obtaining the return address from the top of the stack.            
...
Arguments:
    ...
    FrameBase - Supplies the base of the stack frame subject function stack frame.
    FunctionEntry - Supplies the address of the function table entry for the specified function.
...
*/
{
    ...
    while (Index < UnwindInfo->CountOfCodes) {
        ...
        if (PrologOffset >= UnwindInfo->UnwindCode[Index].CodeOffset) {
            ...
            // Skip this unwind operation by advancing the slot index by the number of slots consumed by this operation.
            Index += RtlpUnwindOpSlotTable[UnwindOp];

            // Special case any unwind operations that can consume a variable number of slots.
            switch (UnwindOp) {
...
Arguments:
    ...
    FunctionEntry - Supplies the address of the function table entry for the specified function.
    ContextRecord - Supplies the address of a context record.
    HandlerData - Supplies a pointer to a variable that receives a pointer the the language handler data.
...
*/
{
    ...
    if ((NextByte[0] == RET_OP) || (NextByte[0] == RET_OP_2) || ((NextByte[0] == REP_PREFIX) && (NextByte[1] == RET_OP))) {
        ...
        // An unconditional branch to a target that is equal to the start of or outside of this routine is logically a call to another function.
        ...
        } else if ((BranchTarget == FunctionEntry->BeginAddress) && ((UnwindInfo->Flags & UNW_FLAG_CHAININFO) == 0)) {
            InEpilogue = TRUE;
        }
    } else if ((NextByte[0] == JMP_IND_OP) && (NextByte[1] == 0x25)) {
        // An unconditional jump indirect.

        // This is a jmp outside of the function, probably a tail call to an import function.
        InEpilogue = TRUE;
    } else if (((NextByte[0] & 0xf8) == SIZE64_PREFIX) && (NextByte[1] == 0xff) && (NextByte[2] & 0x38) == 0x20) {
        // This is an indirect jump opcode: 0x48 0xff /4.  The 64-bit
        // flag (REX.W) is always redundant here, so its presence is overloaded to indicate a branch out of the function - a tail call.
    ...
    if (InEpilogue != FALSE) {
        ...
        // Note: this instruction might in fact be a jmp, however we want to emulate a return regardless.
        ContextRecord->Rip = *(PULONG64)(ContextRecord->Rsp);
        ContextRecord->Rsp += 8;
        return NULL;
    }
...
Arguments:
    LowLimit - Supplies a pointer to a variable that is to receive the low limit of the stack.
    HighLimit - Supplies a pointer to a variable that is to receive the high limit of the stack.
*/
{
...
Arguments:
    LowLimit - Supplies a pointer to a variable that contains the current lower stack limit.
...
/*
Routine Description:
    This function returns the master function table entry for a specified function table entry.
Arguments:
...
Arguments:
    ...
    ImageBase - Supplies the address of a variable that receives the image base if a function table entry contains the specified control PC.
    HistoryTable - Supplies an optional pointer to an unwind history table.
Return Value:
...
*/
{
    ...
    // Attempt to find an image that contains the specified control PC.
    ...
    // There was not a match in either of the unwind history tables so attempt to find a matching entry in the loaded module list.
    FunctionTable = RtlLookupFunctionTable((PVOID)ControlPc, (PVOID *)ImageBase, &SizeOfTable);

    // If a function table is located, then search for a function table entry that contains the specified control PC.
...
Routine Description:
    ...
    If it is a chained function entry, the unwind information associated with the primary function entry is returned.
Arguments:
    FunctionEntry - Supplies a pointer to the function entry for which the associated primary function entry will be located.
    ImageBase - Supplies the base address of the image containing the supplied function entry.
    PrimaryEntry - Supplies the address of a variable that receives a pointer to the primary function entry.
Return Value:
    A pointer to the unwind information for the primary function entry is returned as the function value.
*/
{
    ULONG Index;
    PUNWIND_INFO UnwindInfo;
    ...
    // If the unwind information is chained, then locate the parent function entry and loop again.
    do {
        UnwindInfo = (PUNWIND_INFO)(FunctionEntry->UnwindData + ImageBase);
        if ((UnwindInfo->Flags & UNW_FLAG_CHAININFO) == 0) {
            break;
...
/*
Routine Description:
    This function determines whether the address supplied by control Pc lies anywhere within the function associated with FunctionEntry.
Arguments:
    FunctionEntry - Supplies a pointer to a function entry (primary or chained) associated with the function.
    ImageBase - Supplies the base address of the image containing the supplied function entry.
