...
Revision History:
    ...
    Added ability to get elements out in the order inserted.
    *NOTE* *NOTE* This depends on the implicit ordering of record fields:
        SPLAY_LINKS,
        LIST_ENTRY,
        USER_DATA
*/
...
static TABLE_SEARCH_RESULT FindNodeOrParent(IN PRTL_GENERIC_TABLE Table, IN PVOID Buffer, OUT PRTL_SPLAY_LINKS *NodeOrParent)
/*
Routine Description:
...
*/
{
    if (RtlIsGenericTableEmpty(Table)) {
        ...
        PRTL_SPLAY_LINKS NodeToExamine = Table->TableRoot;// Used as the iteration variable while stepping through the generic table.
        PRTL_SPLAY_LINKS Child;// Just a temporary.  Hopefully a good compiler will get rid of it.
        RTL_GENERIC_COMPARE_RESULTS Result;// Holds the value of the comparasion.

        while (TRUE) {
            Result = Table->CompareRoutine(Table, Buffer, &((PTABLE_ENTRY_HEADER)NodeToExamine)->UserData);// Compare the buffer with the key in the tree element.
            if (Result == GenericLessThan) {
                if (Child = RtlLeftChild(NodeToExamine)) {
                    NodeToExamine = Child;
                } else {// Node is not in the tree.  Set the output parameter to point to what would be its parent and return which child it would be.
            ...
            } else {// Node is in the tree (or it better be because of the assert).  Set the output parameter to point to the node and tell the caller that we found the node.
...
Routine Description:
    ...
    It does this by allocating space for the new element (this includes splay links), inserting the element in the table, and
    then returning to the user a pointer to the new element (which is the first available space after the splay links).
...
Arguments:
    ...
    BufferSize - The amount of space to allocate when the (possible) insertion is made.
                 Note that if we actually do not find the node and we do allocate space then we will add the size of the SPLAY_LINKS to this buffer size.
                 The user should really take care not to depend on anything in the first sizeof(SPLAY_LINKS) bytes of the memory allocated via the memory allocation routine.
...
*/
{
    PRTL_SPLAY_LINKS NodeOrParent;// Holds a pointer to the node in the table or what would be the parent of the node.
    TABLE_SEARCH_RESULT Lookup;// Holds the result of the table lookup.

    Lookup = FindNodeOrParent(Table, Buffer, &NodeOrParent);

    return RtlInsertElementGenericTableFull(Table, Buffer, BufferSize, NewElement, NodeOrParent, Lookup);//  Call the full routine to do the real work.
}
...
*/
{
    PRTL_SPLAY_LINKS NodeToReturn;// Node will point to the splay links of what will be returned to the user.

    if (SearchResult != TableFoundNode) {
        ASSERT(Table->NumberGenericTableElements != (MAXULONG - 1));// We just check that the table isn't getting too big.
        ...
        if (NodeToReturn == NULL) {// If the return is NULL, return NULL from here to indicate that the entry could not be added.
        ...
        InsertTailList(&Table->InsertOrderList, &((PTABLE_ENTRY_HEADER)NodeToReturn)->ListEntry);// Insert the new node at the end of the ordered linked list.
        ...
        RtlCopyMemory(&((PTABLE_ENTRY_HEADER)NodeToReturn)->UserData, Buffer, BufferSize);// Copy the users buffer into the user data area of the table.
    } else {
        NodeToReturn = NodeOrParent;
    }

    Table->TableRoot = RtlSplay(NodeToReturn);// Always splay the (possibly) new node.

    if (ARGUMENT_PRESENT(NewElement)) {
        *NewElement = ((SearchResult == TableFoundNode) ? (FALSE) : (TRUE));
    }

    return &((PTABLE_ENTRY_HEADER)NodeToReturn)->UserData;// Insert the element on the ordered list;
}
...
*/
{
    ...
    if ((Lookup == TableEmptyTree) || (Lookup != TableFoundNode)) {
        ...
        Table->TableRoot = RtlDelete(NodeOrParent);// Delete the node from the splay tree.
        ...
        // NOTE: We are giving the deletion routine a pointer to the splay links rather then the user data.
        // It is assumed that the deallocation is rather bad.
...
*/
{
    ...
    *SearchResult = FindNodeOrParent(Table, Buffer, (PRTL_SPLAY_LINKS *)NodeOrParent);
    if ((*SearchResult == TableEmptyTree) || (*SearchResult != TableFoundNode)) {
        ...
        Table->TableRoot = RtlSplay(*NodeOrParent);// Splay the tree with this node.
        return &((PTABLE_ENTRY_HEADER)*NodeOrParent)->UserData;// Return a pointer to the user data.
    }
}
...
Routine Description:
    ...
    As an example of its use, to enumerate all of the elements in a table the user would write:
        for (ptr = EnumerateGenericTable(Table,TRUE); ptr != NULL; ptr = EnumerateGenericTable(Table, FALSE)) {
                :
        }
Arguments:
    Table - Pointer to the generic table to enumerate.
    Restart - Flag that if true we should start with the least element in the tree otherwise, return we return
...
*/
{
    if (RtlIsGenericTableEmpty(Table)) {
        ...
        if (Restart) {
            ...
            // The assumption here is that the root of the tree is the last node that we returned.
            // We find the real successor to the root and return it as next element of the enumeration.
            // The node that is to be returned is splayed (thereby making it the root of the tree).
            // Note that we need to take care when there are no more elements.
        ...
        // If there actually is a next element in the enumeration then the pointer to return is right after the list links.
...
*/
{
    return ((Table->TableRoot) ? (FALSE) : (TRUE));// Table is empty if the root pointer is null.
}
...
*/
{
    ULONG CurrentLocation = Table->WhichOrderedElement;// Current location in the table.
    ULONG NumberInTable = Table->NumberGenericTableElements;// Hold the number of elements in the table.
    ...
    ULONG ForwardDistance, BackwardDistance;// Will hold distances to travel to the desired node;

    PLIST_ENTRY CurrentNode = Table->OrderedPointer;// Will point to the current element in the linked list.
    ...
    if (CurrentLocation > NormalizedI) {
        // When CurrentLocation is greater than where we want to go, if moving forward gets us there quicker than moving backward
        // then it follows that moving forward from the listhead is
        // going to take fewer steps. (This is because, moving forward in this case must move *through* the listhead.)

        // The work here is to figure out if moving backward would be quicker.

        // Moving backward would be quicker only if the location we wish  to go to is more than half way between the listhead and where we currently are.
        if (NormalizedI > (CurrentLocation / 2)) {
            ...
            // Where we want to go is less than halfway between the start and where we currently are.  Start from the listhead.
    ...
    if (RtlIsGenericTableEmpty(Table)) {
        ...
        if (*RestartKey == NULL) {
            ...
            *RestartKey = NodeToReturn;
            ...
            // We call RtlRealSuccessor to step to the next element in the tree.
