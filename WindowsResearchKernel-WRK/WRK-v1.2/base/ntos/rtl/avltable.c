...
Abstract:
    ...
    its tree support, this module is self-contained in that it implements the balanced binary trees directly.
*/
...
*/
{
    PRTL_BALANCED_LINKS NodeOrParent;//  Holds a pointer to the node in the table or what would be the parent of the node.
    ...
    if (Lookup != TableFoundNode) {
        ...
        //  Make RtlEnumerateGenericTableAvl safe by replacing the RestartKey with its predecessor if it gets deleted.
        //  A NULL means return the first node in the tree.
        //  (The Splay routines do not always correctly resume from predecessor on delete!)
        if (NodeOrParent == Table->RestartKey) {
            Table->RestartKey = RealPredecessor(NodeOrParent);
        }

        Table->DeleteCount += 1;//  Make RtlEnumerateGenericTableLikeADirectory safe by incrementing the DeleteCount.
...
*/
{
    ...
    //  If there was a delete since the last time DeleteCount was captured, then we cannot trust the RestartKey.
    ...
    //  parameters the same in either case, but only return the UserData pointer if we really got a match.
    if (NodeOrParent != NULL) {
