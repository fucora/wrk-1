...
Abstract:
    This module implements function table lookup for platforms with table base exception handling.
*/
...
// Define global unwind history table to hold the constant unwind entries for exception dispatch followed by unwind.
...
Arguments:
    ...
    ControlPc - Supplies a PC value to to use in searching the inverted function table.
    ImageBase - Supplies a pointer to a variable that receives the base address of the corresponding module.
    SizeOfTable - Supplies a pointer to a variable that receives the size of the function table in bytes.
Return Value:
    If a matching entry is located in the specified function table, then the function table address is returned as the function value.
...
*/
{
    ...
    // If there are any entries in the specified inverted function table, then search the table for a matching entry.
    if (InvertedTable->CurrentSize != 0) {
        Low = 0;
        High = InvertedTable->CurrentSize - 1;
        while (High >= Low) {
            // Compute next probe index and test entry.
            // If the specified control PC is greater than of equal to the beginning address and less than the ending address of the inverted function
...
Arguments:
    ...
    FunctionTable - Supplies a pointer to a variable that receives the address of the function table, if any.
    Gp - Supplies a pointer to a variable that receives the GP pointer value, if any.
    TableSize - Supplies a pointer to a variable that receives the size of the function table, if any.
*/
{
#if defined(_X86_)
    ...
    if (NtHeaders->OptionalHeader.DllCharacteristics & IMAGE_DLLCHARACTERISTICS_NO_SEH) {
        ...
        *TableSize = (ULONG)-1;
        ...
        LoadConfig = (PIMAGE_LOAD_CONFIG_DIRECTORY32)RtlImageDirectoryEntryToData(Base, TRUE, IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG, &LoadConfigSize);
        if (LoadConfig &&
            ...
            *TableSize = LoadConfig->SEHandlerCount;
            ...
            Cor20Header = RtlImageDirectoryEntryToData(Base, TRUE, IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR, &Cor20HeaderSize);
...
#else
    *FunctionTable = RtlImageDirectoryEntryToData(Base, TRUE, IMAGE_DIRECTORY_ENTRY_EXCEPTION, TableSize);
#endif
}
...
PRUNTIME_FUNCTION
#endif
RtlLookupFunctionTable(IN PVOID ControlPc, OUT PVOID *ImageBase, OUT PULONG SizeOfTable)
/*
Routine Description:
    This function looks up the control PC in the loaded module list, and returns the image base,
    the size of the function table, and the address of the function table.
Arguments:
    ControlPc - Supplies an address in the module to be looked up.
    ImageBase - Supplies a pointer to a variable that receives the base address of the corresponding module.
    SizeOfTable - Supplies a pointer to a variable that receives the size of the function table in bytes.
Return Value:
    If a module is found that contains the specified control PC value and that module contains a function table,
    then the address of the function table is returned as the function value.
...
#ifndef _X86_
    FunctionTable = RtlpSearchInvertedFunctionTable(&PsInvertedFunctionTable, ControlPc, &Base, SizeOfTable);
...
Arguments:
    InvertedTable - Supplies a pointer to the inverted function table in which the specified entry is to be inserted.
...
*/
{
    ...
    // If the inverted table is not full, then insert the entry in the specified inverted table.
    CurrentSize = InvertedTable->CurrentSize;
    if (CurrentSize != InvertedTable->MaximumSize) {
        // If the inverted table has no entries, then insert the new entry as the first entry.
        // Otherwise, search the inverted table for the proper insert position, shuffle the table, and insert the new entry.
        Index = 0;
        if (CurrentSize != 0) {
...
Routine Description:
    ...
    N.B. It is assumed that appropriate locks are held when this routine is called.
Arguments:
    InvertedTable - Supplies a pointer to the inverted function table from which the specified entry is to be removed.
    ImageBase - Supplies the base address of the containing image.
*/
{
    ULONG CurrentSize;
    ...
    // Search for an entry in the specified inverted table that matches the image base.
    // N.B. It is possible a matching entry is not in the inverted table the table was full when an attempt was made to insert the corresponding entry.
    CurrentSize = InvertedTable->CurrentSize;
    for (Index = 0; Index < CurrentSize; Index += 1) {
        if (ImageBase == InvertedTable->TableEntry[Index].ImageBase) {
            break;
    ...
    if (Index != CurrentSize) {// If the entry was found in the inverted table, then remove the entry and reduce the size of the table.        
        if (CurrentSize != 1) {// If the size of the table is not one, then shuffle the table and remove the specified entry.
        ...
        InvertedTable->CurrentSize -= 1;// Reduce the size of the inverted table.
    }
}
#endif      // !_X86_