...
/*
Routine Description:
    This function locates a VirtualAddress within the image header of a file that is mapped as a file and returns a pointer to the section table entry for that virtual address
Arguments:
...
/*
Routine Description:
    This function locates a VirtualAddress within the image header of a file that is mapped as a file and returns the seek address of the data the Directory describes.
Arguments:
    NtHeaders - Supplies the pointer to the image or data file.
    Base - Supplies the base of the image or data file.
    Address - Supplies the virtual address to locate.
...
/*
Routine Description:
    This function walks the loaded user-mode images and makes present the portions of the data necessary to support a kernel-debugger stack dump of the user-mode stack.
    N.B. The necessary pieces are merely touched to be brought in.  In a low
         memory situation, some of the previously-touched pages may be paged out in order for the new ones to be brought in.
         This routine is not guaranteed to do anything useful, however it does more often than not.
*/
{
    PPEB peb;
    PLIST_ENTRY head;
    ...
    // list corruption.  imageCount is used to bail in finite time in the event of a cyclic image list.
    imageCount = 0;
    try {
        peb = NtCurrentPeb();
        ...
        while (imageCount < 1000) {
            ...
            ProbeForReadSmallStructure(ldrDataTableEntry, sizeof(LDR_DATA_TABLE_ENTRY), PROBE_ALIGNMENT(LDR_DATA_TABLE_ENTRY));
            ...
            // separate try/except block here so that subsequent images will be processed in the event of a failure.
            try {
...
/*
Routine Description:
    This function attempts to make present the portions of an image necessary to support a kernel-debugger stack dump of the user-mode stack.
Arguments:
