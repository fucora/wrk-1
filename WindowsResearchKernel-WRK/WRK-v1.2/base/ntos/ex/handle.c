...
Revision History:
    Support dynamic changes to the number of levels we use. The code performs the best for typical handle table sizes and scales better.

    Make the handle allocate, free and duplicate paths mostly lock free except for the lock entry locks, table expansion and locks to solve the A-B-A problem.
*/
...
//  Define global structures that link all handle tables together except the ones where the user has called RemoveHandleTable
...
#if EXHANDLE_EXTRA_CHECKS
#define FREE_HANDLE_MASK ((MAX_HANDLES<<2) - 1)// Mask for next free value from the free lists.
#else
#define FREE_HANDLE_MASK 0xFFFFFFFF// When no checks compiled in this gets optimized away
#endif

#define FREE_SEQ_MASK (0xFFFFFFFF & ~FREE_HANDLE_MASK)// Mask for the free list sequence number
...
PHANDLE_TABLE ExpAllocateHandleTable(IN PEPROCESS Process OPTIONAL, IN BOOLEAN DoInit);
VOID ExpFreeHandleTable(IN PHANDLE_TABLE HandleTable);
BOOLEAN ExpAllocateHandleTableEntrySlow(IN PHANDLE_TABLE HandleTable, IN BOOLEAN DoInit);
...
PHANDLE_TABLE_ENTRY * ExpAllocateMidLevelTable(IN PHANDLE_TABLE HandleTable, IN BOOLEAN DoInit, OUT PHANDLE_TABLE_ENTRY *pNewLowLevel);
PVOID ExpAllocateTablePagedPool(IN PEPROCESS QuotaProcess OPTIONAL, IN SIZE_T NumberOfBytes);
VOID ExpFreeTablePagedPool(IN PEPROCESS QuotaProcess OPTIONAL, IN PVOID PoolMemory, IN SIZE_T NumberOfBytes);
PHANDLE_TABLE_ENTRY ExpAllocateLowLevelTable(IN PHANDLE_TABLE HandleTable, IN BOOLEAN DoInit);
...
VOID ExpUpdateDebugInfo(PHANDLE_TABLE HandleTable, PETHREAD CurrentThread, HANDLE Handle, ULONG Type);
PVOID ExpAllocateTablePagedPoolNoZero(IN PEPROCESS QuotaProcess OPTIONAL, IN SIZE_T NumberOfBytes);
...
*/
{
    ...
    while (1) {
        ...
        // Swap in the new value, and if the swap occurs successfully, we're done.
...
ULONG ExpMoveFreeHandles(IN PHANDLE_TABLE HandleTable)
{
    ...
    Index = OldValue;
    if (Index == 0) {
        return OldValue;// There are no free handles.  Nothing to do.
    }

    // We are pushing old entries onto the free list.
    // We have the A-B-A problem here as these items may have been moved here because another thread was using them in the pop code.
    ...
    if (!StrictFIFO) {
        // We have a complete chain here. If there is no existing chain we can just push this one without any hassles.
        // If we can't then we can just fall into the reversing code anyway as we need to find the end of the chain to continue it.        
        if (InterlockedCompareExchange((PLONG)&HandleTable->FirstFree, OldValue + GetNextSeq(), 0) == 0) {//This is a push so create a new sequence number
            return OldValue;
        }
    }
    ...
    if (FreeSize < 100 && StrictFIFO) {// If we haven't got a pool of a few handles then force table expansion to keep the free handle size high
        OldValue = 0;
    }
...
PHANDLE_TABLE_ENTRY ExpAllocateHandleTableEntry(IN PHANDLE_TABLE HandleTable, OUT PEXHANDLE pHandle)
/*
Routine Description:
    This routine does a fast allocate of a free handle. It's lock free if possible.
    Only the rare case of handle table expansion is covered by the handle table lock.
Arguments:
    HandleTable - Supplies the handle table being allocated from.
    pHandle - Handle returned
Return Value:
    PHANDLE_TABLE_ENTRY - The allocated handle table entry pointer or NULL on failure.
*/
{
    ...
    while (1) {
        ...
        while (OldValue == 0) {
            ExpLockHandleTableExclusive(HandleTable, CurrentThread);//Lock the handle table for exclusive access as we will be allocating a new table level.

            // If we have multiple threads trying to expand the table at the same time then by just acquiring the table 
            // lock we force those threads to complete their allocations and populate the free list.
            // We must check the free list here so we don't expand the list twice without needing to.
            OldValue = HandleTable->FirstFree;
            if (OldValue != 0) {
            ...
            // This must be the first thread attempting expansion or all the free handles allocated by another thread got used up in the gap.
            RetVal = ExpAllocateHandleTableEntrySlow(HandleTable, TRUE);
            ExpUnlockHandleTableExclusive(HandleTable, CurrentThread);
            OldValue = HandleTable->FirstFree;

            // If ExpAllocateHandleTableEntrySlow had a failed allocation then we want to fail the call.
            // We check for free entries before we exit just in case they got allocated or freed by somebody else in the gap.
            if (!RetVal) {
                if (OldValue == 0) {
        ...
        NewValue1 = InterlockedCompareExchange((PLONG)&HandleTable->FirstFree, NewValue, OldValue);
        ExpUnlockHandleTableShared(HandleTable, CurrentThread, Idx);
        if (NewValue1 == OldValue) {
            EXASSERT((NewValue & FREE_HANDLE_MASK) < HandleTable->NextHandleNeedingPool);
            break;
        } else {// We should have eliminated the A-B-A problem so if only the sequence number has changed we are broken.
...
BOOLEAN FORCEINLINE ExpLockHandleTableEntry(PHANDLE_TABLE HandleTable, PHANDLE_TABLE_ENTRY HandleTableEntry)
/*
Routine Description:
    This routine locks the specified handle table entry.  After the entry is locked the sign bit will be set.
Arguments:
...
Return Value:
    TRUE if the entry is valid and locked, and FALSE if the entry is marked free.
*/
{
    LONG_PTR NewValue;
    LONG_PTR CurrentValue;

    ASSERT((KeGetCurrentThread()->CombinedApcDisable != 0) || (KeGetCurrentIrql() == APC_LEVEL));// We are about to take a lock. Make sure we are protected.

    //  We'll keep on looping reading in the value, making sure it is not null,
    //  and if it is not currently locked we'll try for the lock and return true if we get it.  Otherwise we'll pause a bit and then try again.
    while (TRUE) {
        CurrentValue = ReadForWriteAccess(((volatile LONG_PTR *)&HandleTableEntry->Object));

        //  If the handle value is greater than zero then it is not currently locked and we should try for the lock, 
        //  by setting the lock bit and doing an interlocked exchange.
        if (CurrentValue & EXHANDLE_TABLE_ENTRY_LOCK_BIT) {// Remove the            
            NewValue = CurrentValue - EXHANDLE_TABLE_ENTRY_LOCK_BIT;
            if ((LONG_PTR)(InterlockedCompareExchangePointer(&HandleTableEntry->Object, (PVOID)NewValue, (PVOID)CurrentValue)) == CurrentValue) {
                return TRUE;
            }
        } else {
            if (CurrentValue == 0) {//  Make sure the handle table entry is not freed
                return FALSE;
            }
        }
...
NTKERNELAPI VOID FORCEINLINE ExUnlockHandleTableEntry(__inout PHANDLE_TABLE HandleTable, __inout PHANDLE_TABLE_ENTRY HandleTableEntry)
/*
Routine Description:
    This routine unlocks the specified handle table entry.  After the entry is unlocked the sign bit will be clear.
Arguments:
...
*/
{
    ...
    //  This routine does not need to loop and attempt the unlock operation more than once because by definition the caller has the entry already locked and
    //	no one can be changing the value without the lock.
    ...
    ExUnblockPushLock(&HandleTable->HandleContentionEvent, NULL);// Unblock any waiters waiting for this table entry.
}
...
/*
Routine Description:
    This routine is called once at system initialization to setup the ex handle table package
*/
{
...
Arguments:
    Process - Supplies an optional pointer to the process against which quota will be charged.
Return Value:
    If a handle table is successfully created, then the address of the handle table is returned as the function value. Otherwise, a value NULL is returned.
*/
{
    PKTHREAD CurrentThread;
    ...
    HandleTable = ExpAllocateHandleTable(Process, TRUE);//  Allocate and initialize a handle table descriptor
    if (HandleTable == NULL) {
        return NULL;
    }
    ...
    return HandleTable;//  And return to our caller
}
...
/*
Routine Description:
    This function removes the specified exhandle table from the list of exhandle tables.
    Used by PS and ATOM packages to make sure their handle tables are not in the list enumerated by the
    ExSnapShotHandleTables routine and the !handle debugger extension.
Arguments:
...
*/
{
    ...
    //  This routine is written so that multiple calls to remove a handle table will not corrupt the system.
...
NTKERNELAPI VOID ExDestroyHandleTable(__inout PHANDLE_TABLE HandleTable, __in EX_DESTROY_HANDLE_ROUTINE DestroyHandleProcedure OPTIONAL)
/*
Routine Description:
    This function destroys the specified handle table.
Arguments:
    HandleTable - Supplies a pointer to a handle table
    DestroyHandleProcedure - Supplies a pointer to a function to call for each valid handle entry in the handle table.
*/
{
    EXHANDLE Handle;
    ...
    ExRemoveHandleTable(HandleTable);//  Remove the handle table from the handle table list

    //  Iterate through the handle table and for each handle that is allocated we'll invoke the call back.
    //  Note that this loop exits when we get a null handle table entry.
    //  We know there will be no more possible entries after the first null one is encountered because we allocate memory of the handles in a dense fashion.
    //  But first test that we have call back to use
    if (ARGUMENT_PRESENT(DestroyHandleProcedure)) {
        for (Handle.Value = 0; (HandleTableEntry = ExpLookupHandleTableEntry(HandleTable, Handle)) != NULL; Handle.Value += HANDLE_VALUE_INC) {
            if (ExpIsValidObjectEntry(HandleTableEntry)) {//  Only do the callback if the entry is not free
                (*DestroyHandleProcedure)(Handle.GenericHandleOverlay);
            }
        }
    }

    ExpFreeHandleTable(HandleTable);//  Now free up the handle table memory and return to our caller
}
...
*/
{
    ...
    while ((HandleTableEntry = ExpLookupHandleTableEntry(HandleTable, Handle)) != NULL) {
        do {
            if (ExpLockHandleTableEntry(HandleTable, HandleTableEntry)) {//  Only do the callback if the entry is not free
        ...
        Handle.Value += HANDLE_VALUE_INC;// Skip past the first entry that's not a real entry
    }
}
...
NTKERNELAPI BOOLEAN ExEnumHandleTable(
    ...
    Routine Description:
        ...
        If the enumeration function returns TRUE, then the enumeration is stopped, the current handle is returned to the caller via the optional Handle parameter,
    ...
    */
{
    ...
    CurrentThread = KeGetCurrentThread();
    ResultValue = FALSE;//  Our initial return value is false until the enumeration callback function tells us otherwise

    //  Iterate through the handle table and for each handle that is allocated we'll invoke the call back.
    //  Note that this loop exits when we get a null handle table entry.
    //  We know there will be no more possible entries after the first null one is encountered because we allocate memory for the handles in a dense fashion
    KeEnterCriticalRegionThread(CurrentThread);
    for (LocalHandle.Value = 0; // does essentially the following "LocalHandle.Index = 0, LocalHandle.TagBits = 0;"
        ...
        if (ExpIsValidObjectEntry(HandleTableEntry)) {//  Only do the callback if the entry is not free
            //  Lock the handle table entry because we're about to give it to the callback function, then release the entry right after the call back.
            if (ExpLockHandleTableEntry(HandleTable, HandleTableEntry)) {
                //  Invoke the callback, and if it returns true then set the proper output values and break out of the loop.
...
NTKERNELAPI PHANDLE_TABLE ExDupHandleTable(
    ...
    Arguments:
        ...
        DupHandleProcedure - Supplies an optional pointer to a function to call for each valid handle in the duplicated handle table.
        Mask - Mask applied to the object pointer to work outif we need to duplicate
    Return Value:
        If the specified handle table is successfully duplicated, then the address of the new handle table is returned as the function value.
    ...
    */
{
    ...
    NewHandleTable = ExpAllocateHandleTable(Process, FALSE);//  First allocate a new handle table. 
    if (NewHandleTable == NULL) {
        return NULL;// If this fails then return immediately to our caller
    }

    //  Now we'll build up the new handle table.
    //  We do this by calling allocating new handle table entries, 
    //  and "fooling" the worker routine to allocate keep on allocating until the next free index needing pool are equal
    while (NewHandleTable->NextHandleNeedingPool < OldHandleTable->NextHandleNeedingPool) {
        //  Call the worker routine to grow the new handle table.  
        //  If not successful then free the new table as far as we got, set our output variable and exit out here
    ...
    KeMemoryBarrier();// Make sure any table reads occur after the value we fetched from NextHandleNeedingPool

    //  Now modify the new handle table to think it has zero handles and set its free list to start on the same index as the old free list
    NewHandleTable->HandleCount = 0;
    NewHandleTable->ExtraInfoPages = 0;
    NewHandleTable->FirstFree = 0;

    //  Now for every valid index value we'll copy over the old entry into the new entry
    Handle.Value = HANDLE_VALUE_INC;
    KeEnterCriticalRegionThread(CurrentThread);
    while ((NewHandleTableEntry = ExpLookupHandleTableEntry(NewHandleTable, Handle)) != NULL) {
        OldHandleTableEntry = ExpLookupHandleTableEntry(OldHandleTable, Handle);// Lookup the old entry.
        do {
            //  If the old entry is free then simply copy over the entire old entry to the new entry.  The lock command will tell us if the entry is free.
            if ((OldHandleTableEntry->Value&Mask) == 0 || !ExpLockHandleTableEntry(OldHandleTable, OldHandleTableEntry)) {
                FreeEntry = TRUE;
            } else {
                PHANDLE_TABLE_ENTRY_INFO EntryInfo;

                //  Otherwise we have a non empty entry.  So now copy it over, and unlock the old entry.
                //  In both cases we bump the handle count because either the entry is going into
                //  the new table or we're going to remove it with Exp Free Handle Table Entry which will decrement the handle count
                *NewHandleTableEntry = *OldHandleTableEntry;
                ...
                //  Invoke the callback and if it returns true then we unlock the new entry
                if (NT_SUCCESS(Status)) {
                    if ((*DupHandleProcedure) (Process, OldHandleTable, OldHandleTableEntry, NewHandleTableEntry)) {
                        if (NewHandleTable->DebugInfo != NULL) {
                            ExpUpdateDebugInfo(NewHandleTable, PsGetCurrentThread(), Handle.GenericHandleOverlay, HANDLE_TRACE_DB_OPEN);
                        }

                        // Since there is no route to the new table yet we can just clear the lock bit
                ...
                } else {// Duplicate routine doesn't want this handle duplicated so free it
    ...
    ExReleasePushLockExclusive(&HandleTableListLock);
    KeLeaveCriticalRegionThread(CurrentThread);
    return NewHandleTable;//  lastly return the new handle table to our caller
}
...
NTKERNELAPI NTSTATUS ExSnapShotHandleTables(
    ...
    /*
    Routine Description:
        This function visits and invokes the specified callback for every valid handle that it can find off of the handle table.
    Arguments:
        SnapShotHandleEntry - Supplies a pointer to a function to call for each valid handle we encounter.
        HandleInformation - Supplies a handle information structure to be filled in for each handle table we encounter.
                            This routine fills in the handle count, but relies on a callback to fill in entry info fields.
        Length - Supplies a parameter for the callback.  In reality this is the total size, in bytes, of the Handle Information buffer.
        RequiredLength - Supplies a parameter for the callback.  In reality this is a final size in bytes used to store the requested information.
    Return Value:
    ...
    */
{
    ...
    for (NextEntry = HandleTableListHead.Flink; NextEntry != &HandleTableListHead; NextEntry = NextEntry->Flink) {
        ...
        for (Handle.Value = 0; (HandleTableEntry = ExpLookupHandleTableEntry(HandleTable, Handle)) != NULL; Handle.Value += HANDLE_VALUE_INC) {
            if (ExpIsValidObjectEntry(HandleTableEntry)) {//  Only do the callback if the entry is not free
                HandleInformation->NumberOfHandles += 1;//  Increment the handle count information in the information buffer
...
Arguments:
    HandleTable - Supplies a pointer to a handle table
    HandleEntry - Supplies a pointer to the handle entry for which a handle entry is created.
Return Value:
    If the handle entry is successfully created, then value of the created handle is returned as the function value.  Otherwise, a value of zero is returned.
*/
{
    EXHANDLE Handle;
    ...
    Handle.GenericHandleOverlay = NULL;// Clears Handle.Index and Handle.TagBits    
    NewHandleTableEntry = ExpAllocateHandleTableEntry(HandleTable, &Handle);//Allocate a new handle table entry, and get the handle value    
    if (NewHandleTableEntry != NULL) {//  If we really got a handle then copy over the template and unlock the entry
        ...
        *NewHandleTableEntry = *HandleTableEntry;
        ...
        if (HandleTable->DebugInfo != NULL) {// If we are debugging handle operations then save away the details
...
Arguments:
    ...
    HandleTableEntry - Optionally supplies a pointer to the handle table entry being destroyed.  If supplied the entry is assume to be locked.
Return Value:
    If the specified handle is successfully removed, then a value of TRUE is returned. Otherwise, a value of FALSE is returned.
*/
{
    EXHANDLE LocalHandle;
    PETHREAD CurrentThread;
    ...
    //  If the caller did not supply the optional handle table entry then locate the entry via the supplied handle, make sure it is real, and then lock the entry.
    KeEnterCriticalRegionThread(&CurrentThread->Tcb);
    ...
    //  which does the implicit unlock.  The system will not allocate it again until we add it to the free list which we will do right after we take out the lock
...
/*
Routine Description:
    This function provides the capability to change the contents of the handle entry corrsponding to the specified handle.
Arguments:
    HandleTable - Supplies a pointer to a handle table.
    Handle - Supplies the handle for the handle entry that is changed.
    ChangeRoutine - Supplies a pointer to a function that is called to perform the change.
    Parameter - Supplies an uninterpreted parameter that is passed to the change routine.
Return Value:
    If the operation was successfully performed, then a value of TRUE is returned. Otherwise, a value of FALSE is returned.
*/
{
    EXHANDLE LocalHandle;
    PKTHREAD CurrentThread;
    ...
    //  Translate the input handle to a handle table entry and make sure it is a valid handle.
    HandleTableEntry = ExpLookupHandleTableEntry(HandleTable, LocalHandle);
    if ((HandleTableEntry == NULL) || !ExpIsValidObjectEntry(HandleTableEntry)) {
        return FALSE;
    }

    //  Try and lock the handle table entry,  If this fails then that's because someone freed the handle
...
/*
Routine Description:
    This function maps a handle to a pointer to a handle table entry.
    If the map operation is successful then the handle table entry is locked when we return.
Arguments:
    HandleTable - Supplies a pointer to a handle table.
...
Return Value:
    ...
    then the address of the handle table entry is returned as the function value with the entry locked.
...
*/
{
    ...
    return HandleTableEntry;//  Return the locked valid handle table entry
}
...
*/
{
    ...
    if (((LocalHandle.Index & (LOWLEVEL_COUNT - 1)) == 0) ||
        ...
        if (HandleTable->DebugInfo != NULL) {// If we are debugging handle operations then save away the details
            ...
            // Since we have a non-null DebugInfo for the handle table of this process it means application verifier was enabled for this process.
            if (PreviousMode == UserMode) {
                if (!KeIsAttachedProcess()) {
                    // If the current process is marked for verification then we will raise an exception in user mode.
                    // In case application verifier is enabled system wide we will break first.
                ...
                // We bugcheck for kernel handles only if we have the handle exceptions flag set system-wide.
                // This way a user enabling application verifier for a process will not get bugchecks only user mode errors.
                if ((NtGlobalFlag & FLG_ENABLE_HANDLE_EXCEPTIONS)) {
                    KeBugCheckEx(INVALID_KERNEL_HANDLE, (ULONG_PTR)Handle, (ULONG_PTR)HandleTable, (ULONG_PTR)HandleTableEntry, 0x1);
                }
            }
        }
...
VOID ExDereferenceHandleDebugInfo(IN PHANDLE_TABLE HandleTable, IN PHANDLE_TRACE_DEBUG_INFO DebugInfo)
{
    ...
    if (RetVal == 0) {
        ...
        if (HandleTable->QuotaProcess != NULL) {
            PsReturnProcessNonPagedPoolQuota(HandleTable->QuotaProcess, TraceSize);
        }
...
*/
{
    ...
    // Note that performing math in this manner offers improved efficiency and avoids overflow.
    if (((ULONGLONG)TotalNow) * sizeof(DebugInfo->TraceDb[0]) > ((MmMaximumNonPagedPoolInBytes >> 2) + (MmMaximumNonPagedPoolInBytes >> 4))) {
    ...
    if (Process) {
        Status = PsChargeProcessNonPagedPoolQuota(Process, TraceSize);
        if (!NT_SUCCESS(Status)) {
            goto return_and_exit;
        }
    }
    ...
    DebugInfo = ExAllocatePoolWithTag(NonPagedPool, TraceSize, 'dtbO');
    if (DebugInfo == NULL) {
        Status = STATUS_INSUFFICIENT_RESOURCES;
    ...
    CurrentThread = KeGetCurrentThread();// Put the new table in place releasing any existing table
...
quota_return_and_exit:
    if (Process) {
        PsReturnProcessNonPagedPoolQuota(Process, TraceSize);
    }
...
/*
Routine Description:
    This worker routine will allocate and initialize a new handle table structure.
    The new structure consists of the basic handle table struct plus the first allocation needed to store handles.
    This is really one page divided up into the top level node, the first mid level node, and one bottom level node.
Arguments:
    Process - Optionally supplies the process to charge quota for the handle table
    DoInit - If FALSE then we are being called by duplicate and we don't need the free list built for the caller
Return Value:
    A pointer to the new handle table or NULL if unsuccessful at getting pool.
*/
{
    PHANDLE_TABLE HandleTable;
    ...
    //  If any allocation or quota failures happen we will catch it in the following try-except clause and cleanup after outselves before we return null

    //  First allocate the handle table, make sure we got one, charge quota for it and then zero it out
    ...
    //  Now allocate space of the top level, one mid level and one bottom level table structure.  This will all fit on a page, maybe two.
    HandleTableTable = ExpAllocateTablePagedPoolNoZero(Process, TABLE_PAGE_SIZE);
    ...
    //  We stamp with EX_ADDITIONAL_INFO_SIGNATURE to recognize in the future this is a special information entry
    HandleEntry = &HandleTableTable[0];
    HandleEntry->NextFreeTableEntry = EX_ADDITIONAL_INFO_SIGNATURE;
    HandleEntry->Value = 0;

    // For duplicate calls we skip building the free list as we rebuild it manually as we traverse the old table we are duplicating
    if (DoInit) {
        HandleEntry++;

        //  Now setup the free list.
        //  We do this by chaining together the free entries such that each free entry give the next free index (i.e., like a fat chain). 
        //  The chain is terminated with a 0.
        //  Note that we'll skip handle zero because our callers will get that value confused with null.
    ...
    ExInitializePushLock(&HandleTable->HandleContentionEvent);//  Initialize the blocker for handle entry lock contention.
...
/*
Routine Description:
    This worker routine frees a low-level handle table and the additional info memory, if any.
Arguments:
...
*/
{
    ...
    ExpFreeTablePagedPool(QuotaProcess, TableLevel1, TABLE_PAGE_SIZE);//Now free the low level table and return the quota for the process
...
*/
{
    ...
    //  We need to free all pages. We have 3 cases, depending on the number of levels
    if (TableLevel == 0) {//  There is a single level handle table. We'll simply free the buffer        
        PHANDLE_TABLE_ENTRY TableLevel1 = (PHANDLE_TABLE_ENTRY)CapturedTable;
        ExpFreeLowLevelTable(Process, TableLevel1);
    } else if (TableLevel == 1) {//  We have 2 levels in the handle table        
        PHANDLE_TABLE_ENTRY *TableLevel2 = (PHANDLE_TABLE_ENTRY *)CapturedTable;
        for (i = 0; i < MIDLEVEL_COUNT; i++) {//  loop through the pointers to the low-level tables, and free each one            
        ...
        ExpFreeTablePagedPool(Process, TableLevel2, PAGE_SIZE);//  Free the top level table
    } else {//  Here we handle the case where we have a 3 level handle table        
        PHANDLE_TABLE_ENTRY **TableLevel3 = (PHANDLE_TABLE_ENTRY **)CapturedTable;

        for (i = 0; i < HIGHLEVEL_COUNT; i++) {//  Iterates through the high-level pointers to mid-table
            if (TableLevel3[i] == NULL) {
                break;
            }

            for (j = 0; j < MIDLEVEL_COUNT; j++) {//  Iterate through the mid-level table and free every low-level page
        ...
        ExpFreeTablePagedPool(Process, TableLevel3, HIGHLEVEL_SIZE);//  Free the top-level array
    }

    if (HandleTable->DebugInfo != NULL) {// Free any debug info if we have any.
...
Return Value:
    Returns - a pointer to a low-level table if allocation is successful otherwise the return value is null.
*/
{
    ULONG k;
    ...
    NewLowLevel = ExpAllocateTablePagedPoolNoZero(HandleTable->QuotaProcess, TABLE_PAGE_SIZE);//  Allocate the pool for lower level
    if (NewLowLevel == NULL) {
        return NULL;
    }
    ...
    if (DoInit) {
        ...
        //  Now add the new entries to the free list.  To do this we chain the new free entries together.
        //  We are guaranteed to have at least one new buffer.
        //  The second buffer we need to check for.

        //  We reserve the first entry in the table to the structure with additional info
...
/*
Routine Description:
    This worker routine allocates a mid-level table. This is an array with pointers to low-level tables.
...
/*
Routine Description:
    This worker routine allocates a new handle table entry for the specified handle table.
    Note: The caller must have already locked the handle table
Arguments:
    HandleTable - Supplies the handle table being used
...
*/
{
    ...
    //Initializing NewLowLevel is not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    NewLowLevel = NULL;
    CapturedTable = CapturedTable & ~LEVEL_CODE_MASK;
    if (TableLevel == 0) {//  We have a single level. We need to ad a mid-layer to the process handle table        
        NewMidLevel = ExpAllocateMidLevelTable(HandleTable, DoInit, &NewLowLevel);
        if (NewMidLevel == NULL) {
            return FALSE;
        }

        //  Since ExpAllocateMidLevelTable initialize the first position with a new table, 
        //  we need to move it in the second position, and store in the first position the current one
    ...
    } else if (TableLevel == 1) {//  We have a 2 levels handle table        
        PHANDLE_TABLE_ENTRY *TableLevel2 = (PHANDLE_TABLE_ENTRY *)CapturedTable;

        //  Test whether the index we need to create is still in the range for a 2 layers table
        i = HandleTable->NextHandleNeedingPool / (LOWLEVEL_COUNT * HANDLE_VALUE_INC);
        if (i < MIDLEVEL_COUNT) {
            NewLowLevel = ExpAllocateLowLevelTable(HandleTable, DoInit);//  We just need to allocate a new low-level table
            if (NewLowLevel == NULL) {
                return FALSE;
            }
        ...
        } else {//  We exhausted the 2 level domain. We need to insert a new one            
            ...
            CapturedTable = ((ULONG_PTR)NewHighLevel) | 2;//  Encode the level into the table pointer            
            OldValue = InterlockedExchangePointer((PVOID *)&HandleTable->TableCode, (PVOID)CapturedTable);//Change the handle table pointer with this one
        }
    } else if (TableLevel == 2) {//  we have already a table with 3 levels        
        ULONG RemainingIndex;
        PHANDLE_TABLE_ENTRY **TableLevel3 = (PHANDLE_TABLE_ENTRY **)CapturedTable;

        i = HandleTable->NextHandleNeedingPool / (MIDLEVEL_THRESHOLD * HANDLE_VALUE_INC);
        if (i >= HIGHLEVEL_COUNT) {//  Check whether we exhausted all possible indexes.
            return FALSE;
        }
        ...
        if (TableLevel3[i] == NULL) {
            ...
            EXASSERT(OldValue == NULL);
        } else {//  We have already a mid-level table. We just need to add a new low-level one at the end
    ...
    // This must be done after the table pointers so that new created handles are valid before being freed.
    OldIndex = InterlockedExchangeAdd((PLONG)&HandleTable->NextHandleNeedingPool, LOWLEVEL_COUNT * HANDLE_VALUE_INC);
    if (DoInit) {
        OldIndex += HANDLE_VALUE_INC + GetNextSeq();// Generate a new sequence number since this is a push
        ...
        while (1) {
            ...
            // These are new entries that have never existed before. We can't have an A-B-A problem with these so we don't need to take any locks
            NewFree = InterlockedCompareExchange((PLONG)&HandleTable->FirstFree, OldIndex, OldFree);
            if (NewFree == OldFree) {
                break;
            }
        }
...
/*
Routine Description:
    This routine marks a handle table so that handle allocation is done in a strict FIFO order.
Arguments:
...
//  The following is a global variable only present in the checked builds to help catch apps that reuse handle values after they're closed.
...
/*
Routine Description:
    This worker routine returns the specified handle table entry to the free list for the handle table.
    Note: The caller must have already locked the handle table
Arguments:
...
*/
{
    ...
    if (EntryInfo) {
        EntryInfo->AuditMask = 0;//  Clear the AuditMask flags if these are present into the table
    }

    //  A free is simply a push onto the free table entry stack, 
    //  or in the debug case we'll sometimes just float the entry to catch apps who reuse a recycled handle value.
    ...
    if (ExReuseHandles) {
        ...
        if (!HandleTable->StrictFIFO) {
            ...
            // Prevent the A-B-A problem by shifting to an alternate list read this element has the list head out of the loop.
        ...
        while (1) {
            ...
            if ((ULONG)InterlockedCompareExchange((PLONG)Free, NewFree + SeqInc, OldFree) == OldFree) {
...
Return Value:
    Returns a pointer to the corresponding table entry for the input handle.
    Or NULL if the handle value is invalid (i.e., too large for the tables current allocation.
*/
{
    ULONG_PTR i, j, k;
    ...
    if (Handle.Value >= MaxHandle) {// See if this can be a valid handle given the table levels.
        return NULL;
    }

    CapturedTable = *(volatile ULONG_PTR *)&HandleTable->TableCode;// Now fetch the table address and level bits. We must preserve the ordering here.
    ...
    switch (TableLevel)//  The lookup code depends on number of levels we have
    {
    case 0://We have a simple index into the array, for a single level handle table        
        ...
        break;
    case 1://  we have a 2 level handle table. We need to get the upper index and lower index into the array        
        ...
        break;
    case 2://  We have here a three level handle table.        
...
*/
{
    ...
    if (InfoTable == NULL) {
        ...
        InfoTable = ExpAllocateTablePagedPool(HandleTable->QuotaProcess, LOWLEVEL_COUNT * sizeof(HANDLE_TABLE_ENTRY_INFO));
        if (InfoTable) {// Update the number of pages for extra info. If somebody beat us to it then free the new table
            if (InterlockedCompareExchangePointer(&InfoStructure->InfoTable, InfoTable, NULL) == NULL) {
                ...
                ExpFreeTablePagedPool(HandleTable->QuotaProcess, InfoTable, LOWLEVEL_COUNT * sizeof(HANDLE_TABLE_ENTRY_INFO));
                InfoTable = InfoStructure->InfoTable;
            }
        }
    }

    if (InfoTable != NULL) {//  Calculate the index and copy the structure        
        ExHandle.GenericHandleOverlay = Handle;
...
*/
{
    ...
    //  The info structure is at the first position in each low-level table
    InfoStructure = ExpLookupHandleTableEntry(HandleTable, ExHandle);
    if (InfoStructure == NULL || InfoStructure->NextFreeTableEntry != EX_ADDITIONAL_INFO_SIGNATURE || InfoStructure->InfoTable == NULL) {
        if (TableEntry) {
            ExUnlockHandleTableEntry(HandleTable, TableEntry);
        }
        return NULL;
...
void ExpUpdateDebugInfo(PHANDLE_TABLE HandleTable, PETHREAD CurrentThread, HANDLE Handle, ULONG Type)
{
    ...
    if (g_ulExpUpdateDebugInfoDebugLevel > 10) {
        DbgPrint("ExpUpdateDebugInfo() BitMaskFlags=0x%x, CurrentStackIndex=%d, Handle=0x%p, Type=%d \n",
                 DebugInfo->BitMaskFlags, DebugInfo->CurrentStackIndex, Handle, Type);
    }
#endif
    if (DebugInfo->BitMaskFlags & (HANDLE_TRACE_DEBUG_INFO_CLEAN_DEBUG_INFO | HANDLE_TRACE_DEBUG_INFO_COMPACT_CLOSE_HANDLE)) {
        // we wish to preserve lock-free behavior in the non-compaction path so we lock only in this path
    ...
    if (DebugInfo->BitMaskFlags & HANDLE_TRACE_DEBUG_INFO_CLEAN_DEBUG_INFO) {
        ...
        RtlZeroMemory(DebugInfo->TraceDb, sizeof(*DebugInfo) + DebugInfo->TableSize * sizeof(DebugInfo->TraceDb[0]) - sizeof(DebugInfo->TraceDb));
    }

    if ((DebugInfo->BitMaskFlags & HANDLE_TRACE_DEBUG_INFO_COMPACT_CLOSE_HANDLE) && (Type == HANDLE_TRACE_DB_CLOSE)) {
        // This is assuming that either:
        // 1) this flag was set from the beginning, so there are no close items.
        // 2) this flag was set via KD, in which case HANDLE_TRACE_DEBUG_INFO_CLEAN_DEBUG_INFO must have been set also, so again there are no close items
        ...
        for (uiNextItem = 1; uiNextItem <= uiMaxNumOfItemsInTraceDb; uiNextItem++) {
            // if HANDLE_TRACE_DEBUG_INFO_COMPACT_CLOSE_HANDLE is on there could be no HANDLE_TRACE_DB_CLOSE items
            // This ASSERT can fire if the HANDLE_TRACE_DEBUG_INFO_COMPACT_CLOSE_HANDLE flag is set dynamically,
            // so another thread was not using the locks to add items to the list, and could have added a HANDLE_TRACE_DB_CLOSE item.
            ASSERT(DebugInfo->TraceDb[uiNextItem%DebugInfo->TableSize].Type != HANDLE_TRACE_DB_CLOSE);

            if ((DebugInfo->TraceDb[uiNextItem%DebugInfo->TableSize].Type == HANDLE_TRACE_DB_OPEN) &&
        ...
        if (!(uiNextItem <= uiMaxNumOfItemsInTraceDb)) {
            ...
            // it was created or that a duplicated handle was created before handle tracking was initiated
            // so we cannot ASSERT for that

            /*
            ASSERT((DebugInfo->BitMaskFlags & HANDLE_TRACE_DEBUG_INFO_WAS_SOMETIME_CLEANED) ||
            (DebugInfo->BitMaskFlags & HANDLE_TRACE_DEBUG_INFO_WAS_WRAPPED_AROUND));
                */
        ...
        ULONG Index = ((ULONG)InterlockedIncrement((PLONG)&DebugInfo->CurrentStackIndex)) % DebugInfo->TableSize;
        ASSERT((Type != HANDLE_TRACE_DB_CLOSE) || (!(DebugInfo->BitMaskFlags & HANDLE_TRACE_DEBUG_INFO_COMPACT_CLOSE_HANDLE)));
        if (0 == Index) {
            // this is a wraparound of the db, mark it as such, and if there's a debugger attached break into it
            DebugInfo->BitMaskFlags |= HANDLE_TRACE_DEBUG_INFO_WAS_WRAPPED_AROUND;
...
#define MAX_ALLOCS 20
    ...
    for (i = 0; i < 100000; i++) {
        ...
        for (j = 0; j < MAX_ALLOCS; j++) {
            Handle[j].GenericHandleOverlay = NULL;// Clears Handle.Index and Handle.TagBits
            HandleEntryArray[j] = ExpAllocateHandleTableEntry(HandleTable, &Handle[j]);//Allocate a new handle table entry, and get the handle value
        ...
        for (j = MAX_ALLOCS; j > 0; j--) {
            ...
            if (HandleEntryArray[k] != NULL) {
                ExpFreeHandleTableEntry(HandleTable, Handle[k], HandleEntryArray[k]);// Free the entry
