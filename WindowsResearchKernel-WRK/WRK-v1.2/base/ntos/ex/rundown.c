...
Abstract:
    ...
    The basic principle of these routines is to allow fast protection of a data structure that is torn down by a single thread.
    Threads wishing to access the data structure attempt to obtain rundown protection via calling ExAcquireRundownProtection.
    If this function returns TRUE then accesses are safe until the protected thread calls ExReleaseRundownProtection.
    The single teardown thread calls ExWaitForRundownProtectionRelease to mark the rundown structure as being run down and
    the call will return once all protected threads have released their protection references.
...
//  Checks if 1st argument is aligned on given power of 2 boundary specified by 2nd argument
...
*/
{
    ...
    do {
        ...
        // somebody else changed the variable before we did. Either a protection call came and went or rundown was initiated. 
        // We just repeat the whole loop again.
        Value = NewValue;
    } while (TRUE);
}
...
*/
{
    ...
    do {
        ...
        // somebody else changed the variable before we did. 
        // Either a protection call came and went or rundown was initiated. We just repeat the whole loop again.
        Value = NewValue;
    } while (TRUE);
}
...
*/
{
    ...
    Value = RunRef->Count;
    do {
        // If the block is already marked for rundown then decrement the wait block count and wake the rundown thread if we are the last
        if (Value & EX_RUNDOWN_ACTIVE) {
            PEX_RUNDOWN_WAIT_BLOCK WaitBlock;

            // Rundown is active. since we are one of the threads blocking rundown we have the right to follow the pointer and decrement the active count.
            // If we are the last thread then we have the right to wake up the waiter. After doing this we can't touch the data structures again.
            WaitBlock = (PEX_RUNDOWN_WAIT_BLOCK)(Value & (~EX_RUNDOWN_ACTIVE));

            //  For cache-aware rundown protection, it's possible that the count in the waitblock is zero. We assert weakly on the uniproc case alone
...
#endif
                KeSetEvent(&WaitBlock->WakeEvent, 0, FALSE);// We are the last thread out. Wake up the waiter.
            }
            return;
        } else {
            // Rundown isn't active. Just try and decrement the count. Some other protector thread way come and/or
            // go as we do this or rundown might be initiated. We detect this because the exchange will fail and we have to retry
...
*/
{
    ...
    do {
        ...
        if (Value & EX_RUNDOWN_ACTIVE) {
            ...
            // For cache-aware rundown protection, it's possible the count in the waitblock is actually zero at this point, so this assert is not applicable.
            // We assert weakly only for uni-proc.
            ...
            // Rundown isn't active. Just try and decrement the count.
            // Some other protector thread way come and/or go as we do this or rundown might be initiated.
            // We detect this because the exchange will fail and we have to retry

            // For cache-aware rundown protection, 
            // it's possible that the value on this processor is actually 0 or some integer less than count, so this assert is not applicable.
...
*/
{
    ...
    // been rundown and ExRundownCompleted. 
    // This second case allows for callers that might initiate rundown multiple times (like handle table rundown) to have subsequent rundowns become noops.
    ...
    Event = NULL;
    do {
        WaitCount = (Value >> EX_RUNDOWN_COUNT_SHIFT);// Extract total number of waiters. Its biased by 2 so we can hanve the rundown active bit.
        ...
        // Store the wait count in the wait block. Waiting threads will start to decrement this as they exit if our exchange succeeds.
        // Its possible for accessors to come and go between our initial fetch and the interlocked swap.
        // This doesn't matter so long as there is the same number of outstanding accessors to wait for.
...
PEX_RUNDOWN_REF FORCEINLINE EXP_GET_CURRENT_RUNDOWN_REF(IN PEX_RUNDOWN_REF_CACHE_AWARE RunRefCacheAware)
/*
Routine Description
    Returns the per-processor cache-aligned rundown ref structure for the current processor
Arguments
...
*/
{
    return ((PEX_RUNDOWN_REF)(((PUCHAR)RunRefCacheAware->RunRefs) + (KeGetCurrentProcessorNumber() % RunRefCacheAware->Number) * RunRefCacheAware->RunRefSize));
}


PEX_RUNDOWN_REF FORCEINLINE EXP_GET_PROCESSOR_RUNDOWN_REF(IN PEX_RUNDOWN_REF_CACHE_AWARE RunRefCacheAware, IN ULONG Index)
/*
Routine Description
...
*/
{
    return ((PEX_RUNDOWN_REF)(((PUCHAR)RunRefCacheAware->RunRefs) + (Index % RunRefCacheAware->Number) * RunRefCacheAware->RunRefSize));
}
...
*/
{
    ...
    RunRefCacheAware = ExAllocatePoolWithTag(PoolType, sizeof(EX_RUNDOWN_REF_CACHE_AWARE), PoolTag);
    if (NULL != RunRefCacheAware) {
        RunRefCacheAware->Number = KeNumberProcessors;// Capture number of processors: this will be the # of rundown counts we use
        ...
        RunRefCacheAware->RunRefSize = PaddedSize;//  Remember the size
        RunRefPool = ExAllocatePoolWithTag(PoolType, PaddedSize * RunRefCacheAware->Number, PoolTag);
        if (RunRefPool == NULL) {
        ...
        if ((RunRefCacheAware->Number > 1) && !EXP_IS_ALIGNED_ON_BOUNDARY(RunRefPool, PaddedSize)) {
            ExFreePool(RunRefPool);//  We will allocate a padded size, free the old pool

            //  Allocate enough padding so we can start the refs on an aligned boundary
            RunRefPool = ExAllocatePoolWithTag(PoolType, PaddedSize * RunRefCacheAware->Number + PaddedSize, PoolTag);
            if (RunRefPool == NULL) {
                ExFreePool(RunRefCacheAware);
                return NULL;
            }
            ...
            CurrentRunRef = RunRefPool;//  Already aligned pool
        }
...
Routine Description:
    ...
    This is for drivers who like to allocate this structure as part of their device extension.
    Callers of this routine must have obtained the required size via a call to ExSizeOfCacheAwareRundownProtection and passed a pointer to that structure.

    NOTE:This structure should NOT be freed via ExFreeCacheAwareRundownProtection().
Arguments
...
*/
{
    ...
    //  Reverse engineer the size of each rundown structure based on the size that is passed in
    ...
    //  via ExFreeCacheAwareRundownProtection: if there's a bugcheck with an attempt to access this address & ExFreeCacheAwareRundownProtection on the stack, 
...
NTKERNELAPI VOID FASTCALL ExReleaseRundownProtectionCacheAwareEx(__inout PEX_RUNDOWN_REF_CACHE_AWARE RunRefCacheAware, __in ULONG Count)
/*
Routine Description:
    Dereference a rundown block and wake the rundown thread if we are the last to exit
    This routine is NON-PAGED because it is being called on the paging path.
...
NTKERNELAPI VOID FASTCALL ExWaitForRundownProtectionReleaseCacheAware(__inout PEX_RUNDOWN_REF_CACHE_AWARE RunRefCacheAware)
/*
Routine Description:
    Wait till all outstanding rundown protection calls have exited
Arguments:
...
*/
{
    ...
    // Obtain the outstanding references by totalling up all the counts for all the RunRef structures.
    TotalCount = 0;
    WaitBlock.Count = 0;
    for (Index = 0; Index < RunRefCacheAware->Number; Index++) {
        RunRef = EXP_GET_PROCESSOR_RUNDOWN_REF(RunRefCacheAware, Index);
        Value = RunRef->Count;//  Extract current count &  mark rundown active atomically
        do {
            ASSERT((Value&EX_RUNDOWN_ACTIVE) == 0);
            ...
            NewValue = (ULONG_PTR)InterlockedCompareExchangePointer(&RunRef->Ptr, (PVOID)NewValue, (PVOID)Value);
            if (NewValue == Value) {
                break;//  Succeeded in making rundown active
            }

            Value = NewValue;
        } while (TRUE);

        // Add outstanding references on this processor to the total Ignore overflow: note the rundown active bit will be zero for Value so we will not add up those bits
        TotalCount += Value;
    }

    // If total count was zero there are no outstanding references active at this point - since no refs can creep in after we have set the rundown flag on each processor
    ASSERT((LONG_PTR)TotalCount >= 0);

    //  If total count was zero there are no outstanding references active at this point - since no refs can creep in after we have set the rundown flag on each processor
    if (TotalCount != 0) {
        TotalCount >>= EX_RUNDOWN_COUNT_SHIFT;//  Extract actual number of waiters - count is biased by 2

        //  Initialize the gate - since the dereferencer can decrement the count as soon as we add the per-processor count to the on-stack count
        KeInitializeEvent(&WaitBlock.WakeEvent, SynchronizationEvent, FALSE);

        //  Add the total count to the on-stack count. If the result is zero, there are no more outstanding references. Otherwise wait to be signaled
#if defined(_WIN64)
        if (InterlockedExchangeAdd64((PLONGLONG)&WaitBlock.Count, (LONGLONG)TotalCount) != (-(LONGLONG)TotalCount)) {
#else
        if (InterlockedExchangeAdd((PLONG)&WaitBlock.Count, (LONG)TotalCount) != (-(LONG)TotalCount)) {
#endif
