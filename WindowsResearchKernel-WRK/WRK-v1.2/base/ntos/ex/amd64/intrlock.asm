...
; LARGE_INTEGER ExInterlockedAddLargeInteger (__inout PLARGE_INTEGER Addend, __in LARGE_INTEGER Increment, __inout PKSPIN_LOCK Lock)

; Routine Description:
;   This function performs an interlocked add of an increment value to an addend variable of type unsigned large integer.
;   The initial value of the addend variable is returned as the function value.

;   N.B. The specification of this function requires that the given lock
;        must be used to synchronize the update even though on AMD64 the operation can actually be done atomically without using the lock.

; Arguments:
;   Addend (rcx) - Supplies a pointer to a variable whose value is to be adjusted by the increment value.
;   Increment (rdx) - Supplies the increment value to be added to the addend variable.
;   Lock (r8) - Supplies a pointer to a spin lock to be used to synchronize access to the addend variable.
...
; ULONG ExInterlockedAddUlong (__inout PULONG Addend, __in ULONG Increment, __inout PKSPIN_LOCK Lock)

; Routine Description:
;   This function performs an interlocked add of an increment value to an addend variable of type unsigned long.
;   The initial value of the addend variable is returned as the function value.
...
; Arguments:
;   Addend (rcx) - Supplies a pointer to a variable whose value is to be adjusted by the increment value.
;   Increment (edx) - Supplies the increment value to be added to the addend variable.
;   Lock (r8) - Supplies a pointer to a spin lock to be used to synchronize access to the addend variable.
...
; PLIST_ENTRY ExInterlockedInsertHeadList (__inout PLIST_ENTRY ListHead, __inout PLIST_ENTRY ListEntry, __inout PKSPIN_LOCK Lock)
...
; Arguments:
;   ListHead (rcx) - Supplies a pointer to the head of the doubly linked list into which an entry is to be inserted.
;   ListEntry (rdx) - Supplies a pointer to the entry to be inserted at the head of the list.
;   Lock (r8) - Supplies a pointer to a spin lock to be used to synchronize access to the list.

; Return Value:
;   Pointer to entry that was at the head of the list or NULL if the list was empty.
...
; PLIST_ENTRY ExInterlockedInsertTailList (__inout PLIST_ENTRY ListHead, __inout PLIST_ENTRY ListEntry, __inout PKSPIN_LOCK Lock)
...
; Arguments:
;   ListHead (rcx) - Supplies a pointer to the head of the doubly linked list into which an entry is to be inserted.
;   ListEntry (rdx) - Supplies a pointer to the entry to be inserted at the tail of the list.
;   Lock (r8) - Supplies a pointer to a spin lock to be used to synchronize access to the list.

; Return Value:
;   Pointer to entry that was at the tail of the list or NULL if the list was empty.
...
; PLIST_ENTRY ExInterlockedRemoveHeadList (__inout PLIST_ENTRY ListHead, __inout PKSPIN_LOCK Lock)

; Routine Description:
;   This function removes an entry from the head of a doubly linked list so that access to the list is synchronized in a multiprocessor system.
;   If there are no entries in the list, then a value of NULL is returned.
;   Otherwise, the address of the entry that is removed is returned as the function value.

; Arguments:
;   ListHead (rcx) - Supplies a pointer to the head of the doubly linked list from which an entry is to be removed.
;   Lock (rdx) - Supplies a pointer to a spin lock to be used to synchronize access to the list.
...
; PSINGLE_LIST_ENTRY ExInterlockedPopEntryList (__inout PSINGLE_LIST_ENTRY ListHead, __inout PKSPIN_LOCK Lock)

; Routine Description:
;   This function removes an entry from the front of a singly linked list so that access to the list is synchronized in a multiprocessor system.
;   If there are no entries in the list, then a value of NULL is returned.
;   Otherwise, the address of the entry that is removed is returned as the function value.

; Arguments:
;   ListHead (rcx) - Supplies a pointer to the head of the singly linked list from which an entry is to be removed.
;   Lock (rdx) - Supplies a pointer to a spin lock to be used to synchronize access to the list.
...
; PSINGLE_LIST_ENTRY ExInterlockedPushEntryList (__inout PSINGLE_LIST_ENTRY ListHead, __inout PSINGLE_LIST_ENTRY ListEntry, __inout PKSPIN_LOCK Lock)
...
; Arguments:
;   ListHead (rcx) - Supplies a pointer to the head of the singly linked list into which an entry is to be inserted.
;   ListEntry (rdx) - Supplies a pointer to the entry to be inserted at the head of the list.
;   Lock (r8) - Supplies a pointer to a spin lock to be used to synchronize access to the list.

; Return Value:
;   Previous contents of ListHead. NULL implies list went from empty to not empty.
