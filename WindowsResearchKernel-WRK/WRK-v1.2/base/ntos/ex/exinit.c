...
Abstract:
    The module contains the the initialization code for the executive component.
    It also contains the display string and shutdown system services.
*/
...
/*
Routine Description:
    This function initializes the PRCB lookaside pointers to temporary values that will be updated during phase 1 initialization.
*/
{
    ULONG Index;
    ...
    // Initialize the paged and nonpaged small pool lookaside list pointers in the PRCB to temporarily point to the global pool lookaside lists.
    // During phase 1 initialization per processor lookaside lists are allocated and the pointer to these lists are established in the PRCB of each processor.
...
Return Value:
    A value of TRUE is returned if the initialization is success. Otherwise a value of FALSE is returned.
*/
{
...
/*
Routine Description:
    This function performs Phase 0 initialization of the executive component of the NT system.
Return Value:
...
*/
{
    ...
    // Initialize Resource objects, currently required during SE Phase 0 initialization.
    if (ExpResourceInitialization() == FALSE) {
        Initialized = FALSE;
        KdPrint(("Executive: Resource initialization failed\n"));
    }

    ExInitializeFastMutex(&ExpEnvironmentLock);// Initialize query/set environment variable synchronization fast mutex.
    ExInitializeResourceLite(&ExpKeyManipLock);// Initialize the key manipulation resource.
    InitializeListHead(&ExPoolLookasideListHead);// Initialize the paged and nonpaged small pool lookaside structures,
    for (Index = 0; Index < POOL_SMALL_LISTS; Index += 1) {
        Lookaside = &ExpSmallNPagedPoolLookasideLists[Index];
        ExInitializeSystemLookasideList(Lookaside, NonPagedPool, (Index + 1) * sizeof(POOL_BLOCK), 'looP', 256, &ExPoolLookasideListHead);
        Lookaside = &ExpSmallPagedPoolLookasideLists[Index];
        ExInitializeSystemLookasideList(Lookaside, PagedPool, (Index + 1) * sizeof(POOL_BLOCK), 'looP', 256, &ExPoolLookasideListHead);
    }
    ...
    InitializeListHead(&ExSystemLookasideListHead);// Initialize the system paged and nonpaged lookaside list.
    InitializeListHead(&ExpFirmwareTableProviderListHead);// Initialize the Firmware table provider list
    ExInitializeResourceLite(&ExpFirmwareTableResource);// Initialize the FirmwareTableProvider resource
...
/*
Routine Description:
    This function performs Phase 1 initialization of the executive component of the NT system.
Return Value:
...
*/
{
    ...
    RtlInitializeAtomPackage('motA');// Initialize the ATOM package
    ExpInitializePushLocks();// Initialize pushlocks.

    if (!NT_SUCCESS(ExpWorkerInitialization())) {// Initialize the worker threads.
        Initialized = FALSE;
        KdPrint(("Executive: Worker thread initialization failed\n"));
    }

    if (ExpEventInitialization() == FALSE) {// Initialize the executive objects.
        Initialized = FALSE;
    ...
    for (Index = 0; Index < (ULONG)KeNumberProcessors; Index += 1) {
        ...
        // Allocate all of the lookaside list structures at once so they will be dense and aligned properly.
        Lookaside = ExAllocatePoolWithTag(NonPagedPool, sizeof(GENERAL_LOOKASIDE) * POOL_SMALL_LISTS * 2, 'LooP');

        // If the allocation succeeded, then initialize and fill in the new per processor lookaside structures.
        // Otherwise, use the default structures initialized during phase zero.
        if (Lookaside != NULL) {
            for (List = 0; List < POOL_SMALL_LISTS; List += 1) {
                ExInitializeSystemLookasideList(Lookaside, NonPagedPool, (List + 1) * sizeof(POOL_BLOCK), 'LooP', 256, &ExPoolLookasideListHead);
                Prcb->PPNPagedLookasideList[List].P = Lookaside;
                Lookaside += 1;
                ExInitializeSystemLookasideList(Lookaside, PagedPool, (List + 1) * sizeof(POOL_BLOCK), 'LooP', 256, &ExPoolLookasideListHead);
...
/*
Routine Description:
    This routine computes the tick count multiplier that is used to compute a tick count value.
Arguments:
...
*/
{
    ...
    // The integer part is the whole number of milliseconds between clock interrupts. It is assumed that this value is always less than 128.
    IntegerPart = TimeIncrement / (10 * 1000);

    // Compute the fraction part of the tick count multiplier.

    // The fraction part is the fraction milliseconds between clock interrupts and is computed to an accuracy of 24 bits.
    ...
    // The tick count multiplier is equal to the integer part shifted left by 24 bits and added to the 24 bit fraction.
...
Routine Description:
    ...
    N.B. The caller must have SeShutdownPrivilege to shut down the system.
Arguments:
    Action - Supplies an action that is to be taken after having shutdown.
Return Value:
    !NT_SUCCESS - The operation failed or the caller did not have appropriate privileges.
*/
{
    ...
    case ShutdownNoReboot:
        SystemAction = PowerActionShutdown;
        break;
    case ShutdownReboot:
    ...
    Status = NtSetSystemPowerState(SystemAction, PowerSystemSleeping3, POWER_ACTION_OVERRIDE_APPS | POWER_ACTION_DISABLE_WAKES | POWER_ACTION_CRITICAL);
    return Status;
}
...
Routine Description:
    ...
    This routine is exported from the kernel so that drivers can call it, as they may have to do probing of embedded pointers to user structures on IOCTL calls that the I/O
    system can't probe for them on the FastIo path, which does not pass previous mode via the FastIo parameters.
Return Value:
...
/*
Routine Description:
    This function initializes a system lookaside list structure and inserts the structure in the specified lookaside list.
Arguments:
    ...
    ListHead - Supplies a pointer to the lookaside list into which the lookaside list structure is to be inserted.
*/
{
    // Initialize pool lookaside list structure and insert the structure in the pool lookaside list.
    ExInitializeSListHead(&Lookaside->ListHead);
