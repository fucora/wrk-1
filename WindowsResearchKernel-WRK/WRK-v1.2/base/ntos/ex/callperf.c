...
*/
{
    ...
    if (InitializationPhase != 0) {
        ExAcquireSpinLock(&CallData->SpinLock, &OldIrql);// Acquire the call performance data structure spinlock.

        // Lookup the callers address in call performance data hash table. If the address does not exist in the table, then create a new entry.
