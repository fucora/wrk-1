...
#define MINIMUM_LOOKASIDE_DEPTH 4// Define Minimum lookaside list depth.
#define MINIMUM_ALLOCATION_THRESHOLD 25// Define minimum allocation threshold.
...
*/
{
    ...
    case 1:
        ...
        // N.B. Only one set of pool paged and nonpaged lookaside lists are scanned each scan period.
    case 2:
        ExpScanSystemLookasideList();
        break;
    }

    // Increment the scan count. If a complete cycle has been completed, then zero the scan count and check if any changes occurred during the complete scan.
    ExpScanCount += 1;
    if (ExpScanCount == 3) {
        ExpScanCount = 0;
    }
...
*/
{
    ...
    // Compute the total number of allocations and misses per second for this scan period.
    Allocates = Lookaside->TotalAllocates - Lookaside->LastTotalAllocates;
    Lookaside->LastTotalAllocates = Lookaside->TotalAllocates;

    // If the verifier is enabled, disable lookasides so driver problems can be isolated. Otherwise, compute the target lookaside list depth.
    if (ExMinimumLookasideDepth == 0) {
        Target = 0;
    } else {
        // If the allocate rate is less than the minimum threshold, then lower the maximum depth of the lookaside list.
        // Otherwise, if the miss rate is less than .5%, then lower the maximum depth.
        // Otherwise, raise the maximum depth based on the miss rate.
        ...
        if (Allocates < (ScanPeriod * MINIMUM_ALLOCATION_THRESHOLD)) {
            ...
            // N.B. The number of allocates is guaranteed to be greater than zero because of the above test.

            // N.B. It is possible that the number of misses are greater than the number of allocates, but this won't cause the an incorrect computation of the depth adjustment.
...
*/
{
    ...
    ExAcquireSpinLock(SpinLock, &OldIrql);// Raise IRQL and acquire the specified spinlock.

    // Scan the specified list of lookaside descriptors and adjust the maximum depth as necessary.

    // N.B. All lookaside list descriptors are treated as if they were paged descriptors even though they may be nonpaged descriptors.
    //      This is possible since both structures are identical except for the locking fields which are the last structure fields.
    ...
    ExReleaseSpinLock(SpinLock, OldIrql);// Release spinlock, lower IRQL, and return function value.
}
...
*/
{
    ...
    // Scan the current set of lookaside descriptors and adjust the maximum depth as necessary.
    // Either a set of per processor small pool lookaside lists or the global small pool lookaside lists are scanned during a scan period.
    ...
    if (ExpPoolScanCount == (ULONG)KeNumberProcessors) {
        ...
        // Adjust the maximum depth for the global set of system lookaside descriptors.
        ...
        // Adjust the maximum depth for the global set of small pool lookaside descriptors.
        ...
        // Adjust the maximum depth for the global set of per processor system lookaside descriptors.
        ...
        // Adjust the maximum depth for a set of per processor small pool lookaside descriptors.
        for (Index = 0; Index < POOL_SMALL_LISTS; Index += 1) {
            // Compute target depth of nonpaged lookaside list.
...
*/
{
    ...
    // Insert the lookaside list structure in the system nonpaged lookaside list.
    ExInterlockedInsertTailList(&ExNPagedLookasideListHead, &Lookaside->L.ListEntry, &ExNPagedLookasideLock);
}


VOID ExDeleteNPagedLookasideList(__inout PNPAGED_LOOKASIDE_LIST Lookaside)
/*
Routine Description:
...
*/
{
    ...
    // Acquire the nonpaged system lookaside list lock and remove the specified lookaside list structure from the list.
    ...
    // Remove all pool entries from the specified lookaside structure and free them.
...
*/
{
    ...
    // Insert the lookaside list structure in the system paged lookaside list.
    ExInterlockedInsertTailList(&ExPagedLookasideListHead, &Lookaside->L.ListEntry, &ExPagedLookasideLock);
}


VOID ExDeletePagedLookasideList(__inout PPAGED_LOOKASIDE_LIST Lookaside)
/*
Routine Description:
...
*/
{
    ...
    // Acquire the paged system lookaside list lock and remove the specified lookaside list structure from the list.
