...
typedef struct _VI_DEADLOCK_RESOURCE {
    ...
    PVOID LastReleaseTrace [VI_MAX_STACK_DEPTH];// Stack trace for last release
...
typedef struct _VI_DEADLOCK_THREAD {
    ...
    // The node representing the last resource acquisition made by this thread.
...
typedef struct _VI_DEADLOCK_GLOBALS {
    ...
    PVOID Instigator;// Resource address that caused the deadlock    
    ULONG NumberOfParticipants;// Number of participants in the deadlock    
    PVI_DEADLOCK_NODE Participant [NO_OF_DEADLOCK_PARTICIPANTS];// List of the nodes that participate in the deadlock
