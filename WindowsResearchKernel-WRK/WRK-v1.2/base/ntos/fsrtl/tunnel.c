...
Abstract:
    ...
    The tunnel package provides a set of routines that allow compatibility with applications that rely on filesystems being able to "hold onto"
    file meta-info for a short period of time after deletion/renaming and reinstantiating a new directory entry with that meta-info if a create/rename occurs to cause a file of that name to appear again in a short period of time.

    Note that this violates POSIX rules.
    This package should not be used on POSIX fileobjects, i.e. fileobjects that have case-sensitive names.

    Entries are keyed by directory and one of the short/long names.
    An opaque rock of information is also associated (create time, last write time, etc.).
    This is expected to vary on a per-filesystem basis.

    A TUNNEL variable should be initialized for every volume in the system at mount time.
    Thereafter, each delete/rename-out should add to the tunnel and each create/rename-in should read from the tunnel.
    Each directory deletion should also notify the package so that all associated entries can be flushed.
    The package is responsible for cleaning out aged entries.

    Tunneled information is in the paged pool.

    Concurrent access to the TUNNEL variable is controlled by this package.
    Callers are responsible for synchronizing access to the FsRtlDeleteTunnelCache call.
...
//  We use a lookaside list to manage the common size tunnel entry.
//  The common size is contrived to be 128 bytes by adjusting the size we defer for the long name to 16 characters, which is pretty reasonable.
//  If we ever expect to get more than a ULONGLONG data element or common names are observed to become larger, adjusting this may be required.
...
//  TUNNEL_NODES are then arranged off of the TUNNEL in a splay tree keyed by DirKey ## Name, where Name is whichever of the names was removed from the directory (short or long).
//  Each node is also timestamped and inserted into a timer queue for age expiration.

typedef struct
{
    RTL_SPLAY_LINKS      CacheLinks;//  Splay links in the Cache tree
    LIST_ENTRY           ListLinks;//  List links in the timer queue
    LARGE_INTEGER        CreateTime;//  Time this entry was created (for constant time insert)
    ULONGLONG            DirKey;//  Directory these names are associated with
    ULONG                Flags;//  Flags for the entry
...
*/
{
    return  (Node->DirKey > DirectoryKey ? 1 : (Node->DirKey < DirectoryKey ? -1 : RtlCompareUnicodeString((FlagOn(Node->Flags, TUNNEL_FLAG_KEY_SHORT) ? &Node->ShortName : &Node->LongName), Name, TRUE)));
}
...
/*
Routine Description:
    Performs the common work of deleting a node from a tunnel cache. Pool memory is not deleted immediately but is saved aside on a list for deletion later by the calling routine.
Arguments:
    Cache - the tunnel cache the node is in
    Node - the node being removed
    FreePoolList - an initialized list to take the node if it was allocated from pool
    Splay - an optional flag to indicate whether the tree should be splayed on the delete. Set to FALSE if splaying was performed.
*/
{
    if (Splay && *Splay) {
...
*/
{
    ...
    //  Don't worry about failure in retrieving from the registry. We've gotten this far so fall back on defaults even if there was a problem with resources.
    ...
    if (TunnelMaxAge == 0) {
        //  If the registry has been set so the timeout is zero, we should force the number of entries to zero also.
        //  This preserves expectations and lets us key off of max entries alone in performing the hard disabling of the caching code.
        TunnelMaxEntries = 0;
    }
    ...
    if (TunnelMaxEntries > MAXUSHORT) {
        LookasideDepth = MAX_LOOKASIDE_DEPTH;//  User is hinting a big need to us
    } else {
        LookasideDepth = ((USHORT)TunnelMaxEntries) / 16;
    }

    if (LookasideDepth == 0 && TunnelMaxEntries) {
        LookasideDepth = (USHORT)TunnelMaxEntries + 1;//  Miniscule number of entries allowed. Lookaside 'em all.
    }

    if (LookasideDepth > MAX_LOOKASIDE_DEPTH) {
        LookasideDepth = MAX_LOOKASIDE_DEPTH;//  Finally, restrict the depth to something reasonable.
    }
...
//    FsRtlInitializeTunnelCache will allocate a default cache (resizing policy is common to all file systems) and initialize it to be empty.
//    File systems will store a pointer to this cache in their per-volume structures.

//    Information is retained in the tunnel cache for a fixed period of time.
//    Assume that a value of 10 seconds would satisfy the vast majority of situations.
//    This could be controlled by the registry or could be a compilation constant.

//  Change: W95 times out at 15 seconds. Would be a registry value initialized at tunnel creation time, with a proposed default of 15 seconds.
VOID FsRtlInitializeTunnelCache(__in TUNNEL *Cache)
/*
Routine Description:
...
//    FsRtlAddToTunnelCache is called by file systems when a name disappears from a directory.
//    This typically occurs in both the delete and the rename paths.
//    When a name is deleted, all information needed to be cached is extracted from the file and passed in a single buffer.
//    This information is stored keyed by the directory key (a ULONG that is unique to the directory) and the short-name of the file.
...
//  Change: W95 maintains 10 items in the tunnel cache.
//  Since we are a potential server this should be much higher.
//  The max count would be initialized from the registry with a proposed default of 1024.
//  Adds which run into the limit would cause least recently inserted recycling (i.e., off of the top of the timer queue).

//  Change: Key should be by the name removed, not necessarily the short name.
//  If a long name is removed, it would be incorrect to miss the tunnel. Use KeyByShortName boolean to specify which.
...
Routine Description:
    ...
    ShortName, LongName, and Data are copied and stored in the tunnel. As a side effect, if there are too many entries in the tunnel cache, this routine will initiate expiration in the tunnel cache.
Arguments:
    ...
    DataLength - specifies the length of the opaque data segment (file system specific) which contains the tunneling information for this file
...
*/
{
    ...
    if (NewNode == NULL) {
        ...
        if (NewNode == NULL) {
            return;//  Give up tunneling this entry
        }
    ...
    if (Node) {
        ...
        if (*Links) {
            ...
            if (!RtlIsRoot(*Links)) {
                //  Change over the parent links. Note that we've messed with *Links now since it is pointing at the parent member.
                ...
                Cache->Cache = &NewNode->CacheLinks;//  Set root of the cache
            }
    ...
    //  Initialize the internal UNICODE_STRINGS to point at the buffer segments.
    //  For various reasons (UNICODE APIs are incomplete, we're avoiding calling any allocate routine more than once, UNICODE strings are not guaranteed to be null terminated) we have to do a lot
    //  of this by hand.
...
//    FsRtlFindInTunnelCache consults the cache to see if an entry with the same DirectoryKey and ShortName exist.
//    If so, it returns the data associated with the cache entry.  The entry may or may not be freed from the cache.  Information that is
    ...
    while (Links) {
        ...
        if (Compare > 0) {
            ...
            if (Compare < 0) {
                ...
                break;//  Found tunneling information
            }
        }
    }
...
VOID FsRtlDeleteKeyFromTunnelCache(__in TUNNEL *Cache, __in ULONGLONG DirKey)
/*
Routine Description:
...
*/
{
    ...
    while (Links) {
        ...
        if (Node->DirKey > DirKey) {
            ...
            if (Node->DirKey < DirKey) {
                if (LastLinks) {
                    //  If we have previously seen a candidate node to delete and we have now gone too far left - we know where to start.
                    break;
                }

                Links = RtlRightChild(&Node->CacheLinks);
            } else {
                //  Node is a candidate to be deleted, but we might have more nodes to the left in the tree. Note this location and go on.
                LastLinks = Links;
                Links = RtlLeftChild(&Node->CacheLinks);
            }
        }
    ...
    FsRtlEmptyFreePoolList(&FreePoolList);//  Free delayed pool
}
...
//    Arguments:
//        Cache        a tunnel cache initialized by FsRtlInitializeTunnelCache()
VOID FsRtlDeleteTunnelCache(__in TUNNEL *Cache)
/*
Routine Description:
...
Routine Description:
    ...
    Pool memory is returned on a list for deletion by the calling routine at a time of its choosing.
...
*/
{
    ...
    //  We have to check for future time because the clock may jump as a result of hard clock change.
    //  If we did not do this, a rogue entry with a future time could sit at the top of the queue and prevent entries from going away.
...
/*
Routine Description:
    Given a unicode value name this routine will go into the registry location for the Tunnel parameter information and get the value.
Arguments:
