...
Abstract:
    The file lock package provides a worker thread to handle stack overflow conditions in the file systems.
    When the file system detects that it is near the end of its stack during a paging I/O read request it will post the request to this extra thread.
*/

#include "FsRtlP.h"

// Queue object that is used to hold work queue entries and synchronize worker thread activity.
...
#if defined(_AMD64_)
VOID FsRtlpNopStackOverflowRoutine(IN PVOID Context, IN PKEVENT Event);
#endif
...
typedef struct _STACK_OVERFLOW_ITEM
{
    ...
    PFSRTL_STACK_OVERFLOW_ROUTINE StackOverflowRoutine;//  This is the call back routine
...
NTSTATUS FsRtlInitializeWorkerThread(VOID)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        Status = PsCreateSystemThread(&Thread, THREAD_ALL_ACCESS, &ObjectAttributes, 0L, NULL, FsRtlWorkerThread, ULongToPtr(i));
    ...
    //  Initialize the serial workitem so we can guarantee to post items for paging files to the worker threads.
    KeInitializeEvent(&StackOverflowFallbackSerialEvent, SynchronizationEvent, TRUE);

    // Pass a NOP routine through FsRtlPostStackOverfow() in order to flush out any prefetchw instructions that may lie in that path.
    // Otherwise we'll try to patch the prefetchw instruction when we can ill afford the stack space.
#if defined(_AMD64_)
...
DECLSPEC_NOINLINE VOID FsRtlPostStackOverflow(IN PVOID Context, IN PKEVENT Event, IN PFSRTL_STACK_OVERFLOW_ROUTINE StackOverflowRoutine)
/*
Routine Description:
...
DECLSPEC_NOINLINE VOID FsRtlPostPagingFileStackOverflow(IN PVOID Context, IN PKEVENT Event, IN PFSRTL_STACK_OVERFLOW_ROUTINE StackOverflowRoutine)
/*
Routine Description:
    This routines posts a stack overflow item to the stack overflow thread and returns.
Arguments:
...
*/
{
    ...
    //  Allocate a stack overflow work item it will later be deallocated by the stack overflow thread.  Conserve stack by raising here.
    ...
    //  We can't have a single fallback item for non-pagingfile IO since this could lead to deadlocks if it waits on a thread that itself needs the fallback item.
    ...
    KeInsertQueue(&FsRtlWorkerQueues[PagingFile], &StackOverflowItem->Item.List);//  Safely add it to the overflow queue
...
*/
{
    ...
    //  Since stack overflow reads are always recursive, set the TopLevelIrp field appropriately so that recursive reads from this point will not think they are top level.
    PsGetCurrentThread()->TopLevelIrp = FSRTL_FSP_TOP_LEVEL_IRP;

    //  Get a pointer to the stack overflow item and then call the callback routine to do the work
...
VOID FsRtlWorkerThread(IN PVOID StartContext)
{
    ...
    do {
        ...
        // By specifying a wait mode of KernelMode, the thread's kernel stack is NOT swappable
        ...
        if (KeGetCurrentIrql() != 0) {
            KeBugCheckEx(IRQL_NOT_LESS_OR_EQUAL, (ULONG_PTR)WorkItem->WorkerRoutine, (ULONG_PTR)KeGetCurrentIrql(), (ULONG_PTR)WorkItem->WorkerRoutine, (ULONG_PTR)WorkItem);
        }
    } while (TRUE);
}
...
/*
Routine Description:
    This routine serves as a "dummy" stack overflow routine for the initial "prefetchw flushing" invocation of FsRtlPostStackOverflow().
Arguments:
