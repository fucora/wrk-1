...
// We defer calling the MUP with the registration data until the second redir loads and Dfs is disabled.  This structure holds the data necessary to make that call.
struct
{
    HANDLE MupHandle;
...
NTSTATUS FsRtlRegisterUncProvider(__inout PHANDLE MupHandle, __in PUNICODE_STRING RedirDevName, __in BOOLEAN MailslotsSupported)
/*
Routine Description:
...
*/
{
    ...
    default:
        ...
        status = FsRtlpRegisterProviderWithMUP(mupHandle, RedirDevName, MailslotsSupported);//  Pass the request to the MUP for this redir
        break;
    }
...
VOID FsRtlDeregisterUncProvider(__in HANDLE Handle)
/*
Routine Description:
