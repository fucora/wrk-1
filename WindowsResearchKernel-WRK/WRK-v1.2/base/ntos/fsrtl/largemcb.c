...
Abstract:
    ...
    The MCB routines provide support for maintaining an in-memory copy of the retrieval mapping information for a file.
    The general idea is to have the file system lookup the retrieval mapping for a VBN once from
    the disk, add the mapping to the MCB structure, and then utilize the MCB to retrieve the mapping for subsequent accesses to the file.
    A variable of type MCB is used to store the mapping information.

    The routines provided here allow the user to incrementally store some or all of the retrieval mapping for a file and to do so in any order.
    That is, the mapping can be inserted to the MCB structure all at once starting from the beginning and working to the end of the file, or it
    can be randomly scattered throughout the file.

    The package identifies each contiguous run of sectors mapping VBNs and LBNs independant of the order they are added to the MCB
    structure.  For example a user can define a mapping between VBN sector 0 and LBN sector 107, and between VBN sector 2 and LBN sector
    109.  The mapping now contains two runs each one sector in length.
    Now if the user adds an additional mapping between VBN sector 1 and LBN sector 106 the MCB structure will contain only one run 3 sectors in length.

    Concurrent access to the MCB structure is control by this package.

    The following routines are provided by this package:

      o  FsRtlInitializeMcb - Initialize a new MCB structure.  There should be one MCB for every opened file.
         Each MCB structure must be initialized before it can be used by the system.

      o  FsRtlUninitializeMcb - Uninitialize an MCB structure.  This call is used to cleanup any ancillary structures allocated and maintained by the MCB.
         After being uninitialized the MCB must again be initialized before it can be used by the system.
      ...
      o  FsRtlLookupLastMcbEntryAndIndex - This routine returns the mapping for the largest VBN stored in the structure as well as its index
         Note that calling LookupLastMcbEntry and NumberOfRunsInMcb cannot be synchronized except by the caller.

      o  FsRtlNumberOfRunsInMcb - This routine tells the caller total number of discontiguous sectors runs stored in the MCB structure.
...
//  Retrieval mapping data structures.  The following two structure together are used to map a Vbn to an Lbn.  It is layed out as follows:
...
//  The pairs from 0 to PairCount - 1 are valid.  Given an index between 0 and PairCount - 1 (inclusive) it represents the following Vbn to Lbn mapping information
...
//  To compute the mapping of a Vbn to an Lbn the following algorithm is used

//      1. search through the pairs until we find the slot "i" that contains the Vbn we after.  Report an error if none if found.
//      2. Lbn = StartingLbn + (Vbn - StartingVbn);

//  A hole in the allocation (i.e., a sparse allocation) is represented by an Lbn value of -1 (note that is is different than Mcb.c).
...
BOOLEAN FsRtlFindLargeIndex(IN PBASE_MCB Mcb, IN VBN Vbn, OUT PULONG Index);
VOID FsRtlAddLargeEntry(IN PBASE_MCB Mcb, IN ULONG WhereToAddIndex, IN ULONG AmountToAdd);
VOID FsRtlRemoveLargeEntry(IN PBASE_MCB Mcb, IN ULONG WhereToRemoveIndex, IN ULONG AmountToRemove);
...
#define FsRtlAllocateFastMutex()          ExAllocateFromNPagedLookasideList( &FsRtlFastMutexLookasideList )
...
//  Define a small cache of free mapping pairs structures and also the initial size of the mapping pair

#define INITIAL_MAXIMUM_PAIR_COUNT       (15)

PAGED_LOOKASIDE_LIST FsRtlFirstMappingLookasideList;

//  The following lookaside is used to keep all the Fast Mutexes we will need to boot contiguous.

NPAGED_LOOKASIDE_LIST FsRtlFastMutexLookasideList;

//  The following few routines define the small mcb package which is implemented behind everyones back as large mcbs.  The only funny
//  thing we really need to do here is to make sure that unused Lbns
//  get returned as 0 and not -1.  This is the result of an historical difference between the original Mcb and LargeMcb packages.


VOID FsRtlInitializeMcb(__in PMCB Mcb, __in POOL_TYPE PoolType)
{
    PAGED_CODE();
...
VOID FsRtlUninitializeMcb(__in PMCB Mcb)
{
    PAGED_CODE();
...
VOID FsRtlTruncateMcb(__in PMCB Mcb, __in VBN Vbn)
{
    PAGED_CODE();
...
BOOLEAN FsRtlAddMcbEntry(__in PMCB Mcb, __in VBN Vbn, __in LBN Lbn, __in ULONG SectorCount)
{
    PAGED_CODE();
...
VOID FsRtlRemoveMcbEntry(__in PMCB OpaqueMcb, __in VBN Vbn, __in ULONG SectorCount)
{
...
BOOLEAN FsRtlLookupLastMcbEntry(__in PMCB Mcb, __out PVBN Vbn, __out PLBN Lbn)
{
    BOOLEAN Result;
    LONGLONG LiVbn;
    LONGLONG LiLbn;
...
ULONG FsRtlNumberOfRunsInMcb(__in PMCB Mcb)
{
    PAGED_CODE();
...
BOOLEAN FsRtlGetNextMcbEntry(__in PMCB Mcb, __in ULONG RunIndex, __out PVBN Vbn, __out PLBN Lbn, __out PULONG SectorCount)
{
    BOOLEAN Result;
    LONGLONG LiVbn;
    LONGLONG LiLbn;
...
/*
Routine Description:
    This routine initializes the global portion of the large mcb package at system initialization time.
*/
{
...
VOID FsRtlInitializeBaseMcb(__in PBASE_MCB Mcb, __in POOL_TYPE PoolType)
/*
Routine Description:
    This routine initializes a new Mcb structure.
    The caller must supply the memory for the Mcb structure.
    This call must precede all other calls that set/query the Mcb structure.
...
*/
{
    ...
    //  Allocate a new buffer an initial size is one that will hold 16 runs
...
VOID FsRtlInitializeLargeMcb(__in PLARGE_MCB Mcb, __in POOL_TYPE PoolType)
/*
Routine Description:
    This routine initializes the full large_mcb package by allocating the fast mutex and initializing the base mcb
Arguments:
    OpaqueMcb - Supplies a pointer to the Mcb structure to initialize.
    PoolType - Supplies the pool type to use when allocating additional internal Mcb memory.
*/
{
    ...
    try {
        ...
        //  If this is an abnormal termination then we need to deallocate the GuardedMutex and/or mapping (but once the mapping is allocated, we can't raise).
...
VOID FsRtlUninitializeBaseMcb(__in PBASE_MCB Mcb)
/*
Routine Description:
    This routine uninitializes an Mcb structure.
    After calling this routine the input Mcb structure must be re-initialized before being used again.
Arguments:
...
VOID FsRtlUninitializeLargeMcb(__in PLARGE_MCB Mcb)
/*
Routine Description:
    This routine uninitializes an Mcb structure.
    After calling this routine the input Mcb structure must be re-initialized before being used again.
...
VOID FsRtlTruncateBaseMcb(__in PBASE_MCB Mcb, __in LONGLONG LargeVbn)
/*
Routine Description:
    This routine truncates an Mcb structure to the specified Vbn.
    After calling this routine the Mcb will only contain mappings up to and not including the input vbn.
Arguments:
    OpaqueMcb - Supplies a pointer to the Mcb structure to truncate.
    LargeVbn - Specifies the last Vbn at which is no longer to be mapped.
*/
{
    ...
    if (Vbn == 0) {
        ...
        //  There is nothing to do if the Mcb already ends prior to this point.
        if (FsRtlFindLargeIndex(Mcb, Vbn - 1, &Index)) {
            //  If this entry currently describes a hole then truncate to the previous entry.
            if (StartingLbn(BaseMcb, Index) == UNUSED_LBN) {
                BaseMcb->PairCount = Index;

                //  Otherwise we will truncate the Mcb to this point.  Truncate the number of Vbns of this run if necessary.
            } else {
    ...
    //  We'll shrink the mapping pair buffer if the new pair count will fit within a quarter of the current maximum pair count and the
    //  current maximum is greater than the initial pair count.
    if ((BaseMcb->PairCount < (BaseMcb->MaximumPairCount / 4)) && (BaseMcb->MaximumPairCount > INITIAL_MAXIMUM_PAIR_COUNT)) {
        ULONG NewMax;
        PMAPPING Mapping;

        //  We need to allocate a new mapping so compute a new maximum pair
        //  count.  We'll allocate double the current pair count, but never less than the initial pair count.
        ...
        if (Mapping != NULL) {
            ...
            //  Deallocate the old buffer.  This should never be the size of an initial mapping ...
...
VOID FsRtlTruncateLargeMcb(__in PLARGE_MCB Mcb, __in LONGLONG LargeVbn)
/*
Routine Description:
    This routine truncates an Mcb structure to the specified Vbn.
    After calling this routine the Mcb will only contain mappings up to and not including the input vbn.
...
/*
Routine Description:
    This routine truncates an Mcb structure to contain zero mapping pairs.  It does not shrink the mapping pairs array.
Arguments:
    OpaqueMcb - Supplies a pointer to the Mcb structure to truncate.
*/
{
...
Arguments:
    OpaqueMcb - Supplies a pointer to the Mcb structure to truncate.
    SelfSynchronized - Indicates whether the caller is already synchronized with respect to the Mcb.
Return Value:
    None.
*/
{
    if (SelfSynchronized) {
        //  If we are self-synchronized, then all we do is clear out the current mapping pair count.
        Mcb->BaseMcb.PairCount = 0;
    } else {
        //  Since we are not self-synchronized, we must serialize access to the Mcb before clearing the pair count
        KeAcquireGuardedMutex(Mcb->GuardedMutex);
        Mcb->BaseMcb.PairCount = 0;
        KeReleaseGuardedMutex(Mcb->GuardedMutex);
    }
...
Routine Description:
    ...
    The mapping for the VBNs must not already exist in the Mcb.
    If the mapping continues a previous run, then this routine will actually coalesce them into 1 run.
...
Return Value:
    BOOLEAN - TRUE if the mapping was added successfully (i.e., the new Vbns did not collide with existing Vbns), and FALSE otherwise.
...
*/
{
    ...
    if (FsRtlFindLargeIndex(Mcb, Vbn, &Index)) {
        ...
        //  First check the case where we are adding to an existing mcb run and if so then we will modify the insertion to complete the run
        ...
        if (StartingLbn(BaseMcb, Index) != UNUSED_LBN) {
            ...
            //  Otherwise we will simply trim off the request for the new run to not overlap with the existing run
            ...
            //  At this point the new run start in a hole, now check that if crosses into a non hole and if so then adjust new run to fit in the hole
            ...
            //  Truncate the sector count to go up to but not include the existing run
            SectorCount = StartingVbn(BaseMcb, EndIndex) - Vbn;
        }
    }

    //  Find the index for the starting Vbn of our new run, if there isn't a hole found then index will be set to paircount.

    if (((Index = Mcb->PairCount) == 0) || (PreviousEndingVbn(BaseMcb, Index) + 1 <= Vbn) || !FsRtlFindLargeIndex(Mcb, Vbn, &Index)) {
        //  We didn't find a mapping, therefore this new mapping must go on at the end of the current mapping.

        //  See if we can just grow the last mapping in the current mcb.
        //  We can grow the last entry if (1) the Vbns follow on, and (2)
        //  the Lbns follow on.  We can only grow the last mapping if the index is not 0.
        if ((Index != 0) && (PreviousEndingVbn(BaseMcb, Index) + 1 == Vbn) && (PreviousEndingLbn(BaseMcb, Index) + 1 == Lbn)) {
            //      --LastRun--|---NewRun--|
        ...
        //  We couldn't grow the last mapping, now check to see if this is a continuation of the last Vbn (i.e., there isn't
        //  going to be a hole in the mapping).  Or if this is the first run in the mapping
        if ((Vbn == 0) || (PreviousEndingVbn(BaseMcb, Index) + 1 == Vbn)) {
            //      --LastRun--||---NewRun--|

            //      0:|--NewRun--|

            //  We only need to add one more run to the mcb, so make sure there is enough room for one.
        ...
        //  If we reach this point then there is going to be a hole in the mapping. and the mapping gets appended to the end of the current
    ...
    //  We found an index for the Vbn therefore we must be trying to fill up a hole in the mcb.  So first we need to check to make
    //  sure there really is a hole to be filled
    LastVbn = Vbn + SectorCount - 1;

    if ((StartingLbn(BaseMcb, Index) == UNUSED_LBN) && (StartingVbn(BaseMcb, Index) <= Vbn) && (LastVbn <= EndingVbn(BaseMcb, Index))) {
        //  The mapping fits in this hole, but now here are the following cases we must consider for the new mapping
        if ((StartingVbn(BaseMcb, Index) < Vbn) && (LastVbn < EndingVbn(BaseMcb, Index))) {
            ...
            //  Make room for two more entries.  The NextVbn field of the one we're shifting remains valid.
            FsRtlAddLargeEntry(Mcb, Index, 2);
        ...
        if ((StartingVbn(BaseMcb, Index) == Vbn) && (LastVbn < EndingVbn(BaseMcb, Index))) {
            if (PreviousEndingLbn(BaseMcb, Index) + 1 == Lbn) {
                ...
                //  Leaves a hole at the rear, and does not continue the earlier run.  As occurs if index is zero.
                ...
                //  Make room for one more entry.  The NextVbn field of the one we're shifting remains valid.
                FsRtlAddLargeEntry(Mcb, Index, 1);
                ...
                //  The hole is already set up by the add entry call, because that call just shift over the original hole to that slot
                try_return(Result = TRUE);
            }
        }
        ...
        if ((StartingVbn(BaseMcb, Index) < Vbn) && (LastVbn == EndingVbn(BaseMcb, Index))) {
            if (NextStartingLbn(BaseMcb, Index) == Lbn + SectorCount) {
                ...
                //  Leaves a hole at the front, and does not continue the following run
                ...
                (BaseMcb->Mapping)[Index + 1].Lbn = Lbn;//  Add the new mapping
                try_return(Result = TRUE);
            }
        }

        if ((PreviousEndingLbn(BaseMcb, Index) + 1 == Lbn) && (NextStartingLbn(BaseMcb, Index) == Lbn + SectorCount)) {
            ...
            //  We need to collapse the current index and the following index but first we copy the NextVbn of the follwing run into
            //  the NextVbn field of the previous run to so it all becomes one run
        ...
        if (NextStartingLbn(BaseMcb, Index) == Lbn + SectorCount) {
            ...
            //  This index is going away so we need to stretch the following run to meet up with the previous run
            (BaseMcb->Mapping)[Index + 1].Lbn = Lbn;
        ...
        if (PreviousEndingLbn(BaseMcb, Index) + 1 == Lbn) {
            ...
            //  This index is going away so we need to stretch the previous run to meet up with the following run
    ...
    //  We tried to overwrite an existing mapping so we'll have to tell our caller that it's not possible
    Result = FALSE;
...
BOOLEAN FsRtlAddLargeMcbEntry(__in PLARGE_MCB Mcb, __in LONGLONG LargeVbn, __in LONGLONG LargeLbn, __in LONGLONG LargeSectorCount)
/*
Routine Description:
    This routine is used to add a new mapping of VBNs to LBNs to an existing Mcb. The information added will map
...
VOID FsRtlRemoveBaseMcbEntry(__in PBASE_MCB Mcb, __in LONGLONG Vbn, __in LONGLONG SectorCount)
/*
Routine Description:
    This is the work routine for remove large mcb entry.  It does the work without taking out the mcb GuardedMutex.
Arguments:
...
*/
{
    ...
    //  While there is some more mapping to remove we'll continue with our main loop
    while (SectorCount > 0) {
        ...
        //  Now that we some something to remove the following cases must be considered
        if ((StartingVbn(BaseMcb, Index) == Vbn) && (EndingVbn(BaseMcb, Index) < Vbn + SectorCount)) {
            ULONG i;
            ...
            if (StartingLbn(BaseMcb, Index) == UNUSED_LBN) {
                ...
                if ((PreviousEndingLbn(BaseMcb, Index) != UNUSED_LBN) || (Index == 0)) {
                    ...
                    FsRtlRemoveLargeEntry(Mcb, Index, 1);//  Just remove this entry
                } else {
                    ...
                    FsRtlRemoveLargeEntry(Mcb, Index - 1, 2);//  Just remove this entry, and preceding entry
                }
            } else if (((PreviousEndingLbn(BaseMcb, Index) != UNUSED_LBN) || (Index == 0)) && (NextStartingLbn(BaseMcb, Index) != UNUSED_LBN)) {
                ...
                (BaseMcb->Mapping)[Index].Lbn = (LBN)UNUSED_LBN;//  Make this index a hole
            } else if (((PreviousEndingLbn(BaseMcb, Index) != UNUSED_LBN) || (Index == 0)) && (NextStartingLbn(BaseMcb, Index) == UNUSED_LBN)) {
                //  Following is hole
                ...
                FsRtlRemoveLargeEntry(Mcb, Index, 1);//  Simply remove this entry
            } else if ((PreviousEndingLbn(BaseMcb, Index) == UNUSED_LBN) && (NextStartingLbn(BaseMcb, Index) != UNUSED_LBN)) {
                //  Previous is hole
                ...
                (BaseMcb->Mapping)[Index].Lbn = (LBN)UNUSED_LBN;//  Mark current entry a hole                
                FsRtlRemoveLargeEntry(Mcb, Index - 1, 1);//  Remove previous entry
            } else {
                ...
                FsRtlRemoveLargeEntry(Mcb, Index - 1, 2);//  Remove previous and this entry
            }
            ...
            if (StartingLbn(BaseMcb, Index) == UNUSED_LBN) {
                ...
                (BaseMcb->Mapping)[Index + 1].Lbn += (LBN)SectorCount;//  Set the new Lbn for the remaining run
            } else {
                //  Previous is hole
                ...
                (BaseMcb->Mapping)[Index - 1].NextVbn += (VBN)SectorCount;//  Expand the preceding hole                
                (BaseMcb->Mapping)[Index].Lbn += (LBN)SectorCount;//  Set the new Lbn for the remaining run
            }
            ...
            if (StartingLbn(BaseMcb, Index) == UNUSED_LBN) {
                ...
                (BaseMcb->Mapping)[Index + 2].Lbn += SectorsWithinRun(BaseMcb, Index) + SectorsWithinRun(BaseMcb, Index + 1);
            }
...
VOID FsRtlRemoveLargeMcbEntry(__in PLARGE_MCB Mcb, __in LONGLONG LargeVbn, __in LONGLONG LargeSectorCount)
/*
Routine Description:
    This routine removes a mapping of VBNs to LBNs from an Mcb.  The mappings removed are for
    ...
    The operation works even if the mapping for a Vbn in the specified range does not already exist in the Mcb.
    If the specified range of Vbn includes the last mapped Vbn in the Mcb then the Mcb mapping shrinks accordingly.
...
Arguments:
    ...
    Lbn - Receives the Lbn corresponding to the Vbn.  A value of -1 is returned if the Vbn does not have a corresponding Lbn.
...
Return Value:
    BOOLEAN - TRUE if the Vbn is within the range of VBNs mapped by the
        MCB (even if it corresponds to a hole in the mapping), and FALSE if the Vbn is beyond the range of the MCB's mapping.

        For example, if an MCB has a mapping for VBNs 5 and 7 but not for 6, then a lookup on Vbn 5 or 7 will yield a non zero Lbn and a sector
        count of 1.  A lookup for Vbn 6 will return TRUE with an Lbn value of 0, and lookup for Vbn 8 or above will return FALSE.
*/
{
    PNONOPAQUE_BASE_MCB BaseMcb = (PNONOPAQUE_BASE_MCB)Mcb;
    ULONG LocalIndex;
    ...
    //  Compute the lbn for corresponding to the vbn, the value is the starting lbn of the run plus the number of sectors offset into the
    ...
    //  If there sector count argument is present then we'll return the number of sectors remaing in the run.
    if (ARGUMENT_PRESENT(LargeSectorCount)) {
        *LargeSectorCount = EndingVbn(BaseMcb, LocalIndex) - ((ULONG)LargeVbn) + 1;
    }

    //  Compute the starting lbn for corresponding to the start of the run, the value is the starting lbn of the run.  
    //  But if it's a hole then the sector Lbn is zero.
    ...
    //  If there sector count argument is present then we'll return the number of sectors in the run.
...
Arguments:
    ...
    Index - Receives the index of the run found.
Return Value:
    BOOLEAN - TRUE if the Vbn is within the range of VBNs mapped by the MCB (even if it corresponds to a hole in the mapping), and FALSE
...
BOOLEAN FsRtlLookupLastBaseMcbEntry(__in PBASE_MCB Mcb, __out PLONGLONG LargeVbn, __out PLONGLONG LargeLbn)
/*
Routine Description:
    ...
    The results of this function is useful when extending an existing file and needing to a hint on where to try and allocate sectors on the disk.
Arguments:
...
BOOLEAN FsRtlLookupLastLargeMcbEntry(__in PLARGE_MCB Mcb, __out PLONGLONG LargeVbn, __out PLONGLONG LargeLbn)
/*
Routine Description:
    This routine retrieves the last Vbn to Lbn mapping stored in the Mcb.
    It returns the mapping for the last sector or the last run in the Mcb.
...
BOOLEAN FsRtlLookupLastBaseMcbEntryAndIndex(__in PBASE_MCB Mcb, __out PLONGLONG LargeVbn, __out PLONGLONG LargeLbn, __out PULONG Index)
/*
Routine Description:
    This routine retrieves the last Vbn to Lbn mapping stored in the Mcb.
    It returns the mapping for the last sector or the last run in the Mcb.
...
ULONG FsRtlNumberOfRunsInBaseMcb(__in PBASE_MCB Mcb)
/*
Routine Description:
    This routine returns to the its caller the number of distinct runs mapped by an Mcb.
    Holes (i.e., Vbns that map to Lbn=UNUSED_LBN) are counted as runs.
    For example, an Mcb containing a mapping for only Vbns 0 and 3 will have 3 runs, one for the first mapped sector, a second for the
...
ULONG FsRtlNumberOfRunsInLargeMcb(__in PLARGE_MCB Mcb)
/*
Routine Description:
    This routine returns to the its caller the number of distinct runs mapped by an Mcb.
    Holes (i.e., Vbns that map to Lbn=UNUSED_LBN) are counted as runs.  For example, an Mcb containing a mapping for only Vbns 0 and 3
    will have 3 runs, one for the first mapped sector, a second for the hole covering Vbns 1 and 2, and a third for Vbn 3.
Arguments:
    OpaqueMcb - Supplies the Mcb being examined.
Return Value:
    ULONG - Returns the number of distinct runs mapped by the input Mcb.
...
BOOLEAN FsRtlGetNextBaseMcbEntry(
    __in PBASE_MCB Mcb,
    __in ULONG RunIndex,
    __out PLONGLONG LargeVbn,
    __out PLONGLONG LargeLbn,
...
/*
Routine Description:
    This routine returns to its caller the Vbn, Lbn, and SectorCount for distinct runs mapped by an Mcb.  Holes are counted as runs.
    For example, to construct to print out all of the runs in a a file is:
...
Return Value:
    BOOLEAN - TRUE if the specified run (i.e., RunIndex) exists in the Mcb,
        and FALSE otherwise.  If FALSE is returned then the Vbn, Lbn, and SectorCount parameters receive zero.
*/
{
    PNONOPAQUE_BASE_MCB BaseMcb = (PNONOPAQUE_BASE_MCB)Mcb;
    BOOLEAN Result = FALSE;
...
Arguments:
    ...
    SectorCount - Receives the number of sectors within the returned run, or zero if the run does not exist.
Return Value:
    BOOLEAN - TRUE if the specified run (i.e., RunIndex) exists in the Mcb, and FALSE otherwise.
        If FALSE is returned then the Vbn, Lbn, and SectorCount parameters receive zero.
*/
{
    BOOLEAN Result = FALSE;
...
BOOLEAN FsRtlSplitBaseMcb(__in PBASE_MCB Mcb, __in LONGLONG LargeVbn, __in LONGLONG LargeAmount)
/*
Routine Description:
    This routine is used to create a hole within an MCB, by shifting the mapping of Vbns.
    All mappings above the input vbn are shifted by the amount specified and while keeping their current lbn value.
    Pictorially we have as input the following MCB
    ...
    1. The input Vbn is beyond the last run.  In this case this operation is a noop.
    2. The input Vbn is within or adjacent to a existing run of unused Lbns.
       In this case we simply need to extend the size of the existing hole and shift succeeding runs.
    3. The input Vbn is between two existing runs, including the an input vbn
       value of zero.  In this case we need to add a new entry for the hole and shift succeeding runs.
    4. The input Vbn is within an existing run.  In this case we need to add two new entries to contain the split run and the hole.
...
Return Value:
    BOOLEAN - TRUE if the mapping was successfully shifted, and FALSE otherwise. If FALSE is returned then the Mcb is not changed.
*/
{
    PNONOPAQUE_BASE_MCB BaseMcb = (PNONOPAQUE_BASE_MCB)Mcb;
    VBN Vbn = ((ULONG)LargeVbn);
    ...
    ASSERTMSG("LargeInteger not supported yet ", ((((PLARGE_INTEGER)&LargeVbn)->HighPart == 0) || (Mcb->PairCount == 0)));
    ASSERTMSG("LargeInteger not supported yet ", ((((PLARGE_INTEGER)&LargeAmount)->HighPart == 0) || (Mcb->PairCount == 0)));
    ...
    //  takes care of the case where the input vbn is beyond the last run in the mcb
    if (!FsRtlFindLargeIndex(Mcb, Vbn, &Index)) {
        try_return(Result = FALSE);
    }
    ...
    if (StartingLbn(BaseMcb, Index) == UNUSED_LBN) {
        ...
        //      In this case the vbn is somewhere within the hole and we simply need to added the amount of each existing run beyond the hole.

        //  In this case there is really nothing to do here because the ending code will already shift the runs by proper amount starting at index
        NOTHING;
        ...
        //      In this case the vbn points to the start of the existing run and we need to do the split between the hole and the
        //      existing run by simply adding the amount to each existing run beyond the hole.

        //  In this case we need to decement the index by 1 and then fall to the bottom code which will do the shifting for us
        Index -= 1;
        ...
        //      In this case the vbn points to the start of an existing run and the preceding is either a real run or the start
        //      of mapping pairs We simply add a new entry for the hole and shift succeeding runs.
        FsRtlAddLargeEntry(Mcb, Index, 1);
        ...
        //      In this case the vbn points within an existing run we need to add two new extries for hole and split run and shift succeeding runs
        ...
        (BaseMcb->Mapping)[Index + 2].Lbn = (BaseMcb->Mapping)[Index + 2].Lbn + StartingVbn(BaseMcb, Index + 1) - StartingVbn(BaseMcb, Index);
        Index += 2;
    }

    //  At this point we have completed most of the work we now need to shift existing runs from the index to the end of the mappings by the specified amount
...
BOOLEAN FsRtlSplitLargeMcb(__in PLARGE_MCB Mcb, __in LONGLONG LargeVbn, __in LONGLONG LargeAmount)
/*
Routine Description:
    This routine is used to create a hole within an MCB, by shifting the mapping of Vbns.
    All mappings above the input vbn are shifted by the amount specified and while keeping their current lbn value.
...
/*
Routine Description:
    This is a private routine that locates a mapping for a Vbn in a given mapping array
Arguments:
    ...
    Index - Receives the index within the mapping array of the mapping
        containing the Vbn.  If none if found then the index is set to PairCount.
Return Value:
...
*/
{
    ...
    //  We'll just do a binary search for the mapping entry.  Min and max are our search boundaries

    MinIndex = 0;
    MaxIndex = BaseMcb->PairCount - 1;

    while (MinIndex <= MaxIndex) {
        MidIndex = ((MaxIndex + MinIndex) / 2);//  Compute the middle index to look at

        if (Vbn < StartingVbn(BaseMcb, MidIndex)) {//  check if the Vbn is less than the mapping at the mid index           
            MaxIndex = MidIndex - 1; //  Vbn is less than the middle index so we need to drop the max down        
        } else if (Vbn > EndingVbn(BaseMcb, MidIndex)) {//  check if the Vbn is greater than the mapping at the mid index            
            MinIndex = MidIndex + 1;//  Vbn is greater than the middle index so we need to bring up the min        
        } else {//  Otherwise we've found the index containing the Vbn so set the index and return TRUE.
            *Index = MidIndex;
            return TRUE;
        }
    }

    *Index = BaseMcb->PairCount;//  A match wasn't found so set index to PairCount and return FALSE
    return FALSE;
}
...
/*
Routine Description:
    This routine takes a current Mcb and detemines if there is enough room to add the new mapping entries.
    If there is not enough room it reallocates a new mcb buffer and copies over the current mapping.
    If also will spread out the current mappings to leave the specified index slots in the mapping unfilled.
    For example, if WhereToAddIndex is equal to the current pair count then we don't need to make a hole
    in the mapping, but if the index is less than the current pair count then we'll need to slide some of the mappings down to make room at the specified index.
Arguments:
    Mcb - Supplies the mcb being checked and modified
    WhereToAddIndex - Supplies the index of where the additional entries need to be made
    AmountToAdd - Supplies the number of additional entries needed in the mcb
*/
{
    PNONOPAQUE_BASE_MCB BaseMcb = (PNONOPAQUE_BASE_MCB)Mcb;

    //  Check to see if the current buffer is large enough to hold the additional entries
    if (BaseMcb->PairCount + AmountToAdd > BaseMcb->MaximumPairCount) {
        ULONG NewMax;
        PMAPPING Mapping;

        //  We need to allocate a new mapping so compute a new maximum pair count.  We'll only be asked to grow by at most 2 at a time, so
        //  doubling will definitely make us large enough for the new amount.
        //  But we won't double without bounds we'll stop doubling if the pair count gets too high.
        ...
        RtlCopyMemory(Mapping, BaseMcb->Mapping, sizeof(MAPPING) * BaseMcb->PairCount);//  Now copy over the old mapping to the new buffer
    ...
    //  Now see if we need to shift some entries over according to the WhereToAddIndex value
    ...
    BaseMcb->PairCount += AmountToAdd; //  Now increment the PairCount   
    return;//  And return to our caller
}
...
*/
{
    ...
    //  Check to see if we need to shift everything down because the entries to remove do not include the last entry in the mcb
    if (WhereToRemoveIndex + AmountToRemove < BaseMcb->PairCount) {
        RtlMoveMemory(&((BaseMcb->Mapping)[WhereToRemoveIndex]),
                      ...
                      (BaseMcb->PairCount - (WhereToRemoveIndex + AmountToRemove)) * sizeof(MAPPING));
    }
    ...
    BaseMcb->PairCount -= AmountToRemove;//  Now decrement the PairCount    
    return;//  And return to our caller
}