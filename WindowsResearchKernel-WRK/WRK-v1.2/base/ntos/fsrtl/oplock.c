...
Abstract:
    ...
    The OPLOCK routines provide support to filesystems which implement opportunistic locks.
    The specific actions needed are based on the current oplocked state of the file (maintained in the OPLOCK structure) and the Irp the Io system provides to the file systems.
    ...
    The following routines are provided by this package:
      ...
      o  FsRtlUninitializeOplock - Uninitialize an OPLOCK structure.
         This call is used to cleanup any ancillary structures allocated and maintained by the OPLOCK.
...
//  Each Waiting Irp record corresponds to an Irp that is waiting for an oplock break to be acknowledged and is maintained in a queue off of the Oplock's WaitingIrps list.
typedef struct _WAITING_IRP
{
    ...
    POPLOCK_WAIT_COMPLETE_ROUTINE CompletionRoutine;//  This is the routine to call when we are done with an Irp we held on a waiting queue.  (We originally returned STATUS_PENDING).
...
NTSTATUS FsRtlOplockBreakToNone(IN OUT PNONOPAQUE_OPLOCK Oplock, IN PIO_STACK_LOCATION IrpSp, IN PIRP Irp, IN PVOID Context,
                                IN POPLOCK_WAIT_COMPLETE_ROUTINE CompletionRoutine OPTIONAL, IN POPLOCK_FS_PREPOST_IRP PostIrpRoutine OPTIONAL);
...
VOID FsRtlInitializeOplock(__inout POPLOCK Oplock)
/*
Routine Description:
    This routine initializes a new OPLOCK structure.
    This call must precede any other call to this entry point with this OPLOCK structure.
    In addition, this routine will have exclusive access to the Oplock structure.
Arguments:
...
VOID FsRtlUninitializeOplock(__inout POPLOCK Oplock)
/*
Routine Description:
    This routine uninitializes an OPLOCK structure.
    After calling this routine, the OPLOCK structure must be reinitialized before being used again.
Arguments:
    Oplock - Supplies the address of an opaque OPLOCK structure.
*/
{
    ...
    //  If the Oplock structure has not been allocated, there is no action to take.
    if (*Oplock != NULL) {
        ...
        *Oplock = NULL;
        ...
        //  Grab the waiting lock queue mutex to exclude anyone from messing with the queue while we're using it
        ...
        try {
            ...
            //  No matter how we complete the preceding statements we will now release the waiting lock queue mutex
            ExReleaseFastMutexUnsafe(ThisOplock->FastMutex);
        }

        ExFreePool(ThisOplock->FastMutex);//  Deallocate the mutex.
        ExFreePool(ThisOplock);//  Deallocate the Oplock structure.
    }
...
NTSTATUS FsRtlOplockFsctrl(__in POPLOCK Oplock, __in PIRP Irp, __in ULONG OpenCount)
/*
Routine Description:
    This is the interface with the filesystems for Fsctl calls, it handles oplock requests, break acknowledgement and break notify.
Arguments:
    Oplock - Supplies the address of the opaque OPLOCK structure.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
    OpenCount - This is the number of user handles on the file if we are requesting an exclusive oplock.
        A non-zero value for a level II request indicates that there are locks on the file.
Return Value:
    NTSTATUS - Returns the result of this operation.
               If this is an Oplock request which is granted, then STATUS_PENDING is returned.
               If the Oplock isn't granted then STATUS_OPLOCK_NOT_GRANTED is returned.
               If this is an Oplock I break to no oplock, then STATUS_SUCCESS.
               If this is an Oplock I break to Oplock II then STATUS_PENDING is returned.
               Other error codes returned depend on the nature of the error.

               STATUS_CANCELLED is returned if the Irp is cancelled during this operation.
               STATUS_SUCCESS is returned if this is a create asking for a filter oplock.
*/
{
    NTSTATUS Status;
    ...
    IrpSp = IoGetCurrentIrpStackLocation(Irp);//  Get the current IRP stack location
    ...
    //  Check if this is the create case where the user is requesting a pending filter oplock.
    if (IrpSp->MajorFunction == IRP_MJ_CREATE) {
        ...
        //          This will ensure an asynch open since the SYNCHRONIZE flag can't be set.
    ...
    } else {//  Case on the FsControlFile code control code.
        ...
        case FSCTL_REQUEST_OPLOCK_LEVEL_1:
            SetFlag(OplockState, EXCLUSIVE);//  Set the other flags for an exclusive oplock.

            //  We short circuit the request if this request is treated synchronously or the open count is not 1.
            //  Otherwise the Io system will hold the return code until the Irp is completed.

            //  Also fail this if the flag is set which indicates that the IO system should copy data back to a user's buffer.

            //  If cleanup has occurred on this file, then we refuse the oplock request.
            if ((OpenCount != 1) ||
        ...
        case FSCTL_REQUEST_OPLOCK_LEVEL_2:
            //  We short circuit the request if this request is treated synchronously.
            //  Otherwise the Io system will hold the return code until the Irp is completed.
            ...
            //  A non-zero open count in this case indicates that there are file locks on the file.
            //  We will also fail the request in this case.
            if ((OpenCount != 0) ||
                IoIsOperationSynchronous(Irp) ||
                FlagOn(Irp->Flags, IRP_INPUT_OPERATION) ||
                FlagOn(IrpSp->FileObject->Flags, FO_CLEANUP_COMPLETE)) {
...
NTSTATUS FsRtlCheckOplock(__in POPLOCK Oplock,
                          __in PIRP Irp,
...
Routine Description:
    ...
    It is used to synchronize I/O requests with the current Oplock state of a file.
    If the I/O operation will cause the Oplock to break, that action is initiated.
    If the operation cannot continue until the Oplock break is complete, STATUS_PENDING is returned and the caller supplied routine is called.
Arguments:
    Oplock - Supplies a pointer to the non-opaque oplock structure for this file.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
    Context - This value is passed as a parameter to the completion routine.
    CompletionRoutine - This is the routine which is called if this Irp must wait for an Oplock to break.
                        This is a synchronous operation if not specified and we block in this thread waiting on an event.
    PostIrpRoutine - This is the routine to call before we put anything on our waiting Irp queue.
Return Value:
...
*/
{
    ...
    //  If there is no oplock structure or this is system I/O, we allow the operation to continue.
    //  Otherwise we check the major function code.
    if ((ThisOplock != NULL) && !FlagOn(Irp->Flags, IRP_PAGING_IO)) {
        ...
        //  Capture the file object first and then the oplock state to perform the unsafe checks below.
        //  We capture the file object first in case there is an exclusive oplock break in progress.
        //  Otherwise the oplock state may indicate break in progress but it could complete by the time we snap the file object.
        OplockFileObject = ThisOplock->FileObject;
        OplockState = ThisOplock->OplockState;

        //  Examine the Irp for the appropriate action provided there are current oplocks on the file.
        if (OplockState != NoOplocksHeld) {
            ...
            case IRP_MJ_CREATE:
                //  If we are opening for attribute access only, we return status success.
                //  Always break the oplock if this caller wants a filter oplock.
                if (!FlagOn(IrpSp->Parameters.Create.SecurityContext->DesiredAccess, ~(FILE_READ_ATTRIBUTES | FILE_WRITE_ATTRIBUTES | SYNCHRONIZE)) &&
                    !FlagOn(IrpSp->Parameters.Create.Options, FILE_RESERVE_OPFILTER)) {
                    break;
                }

                //  If there is a filter oplock granted and this create iS reading the file then don't break the oplock as long as we share for reads.
                if (FlagOn(OplockState, FILTER_OPLOCK) && !FlagOn(IrpSp->Parameters.Create.SecurityContext->DesiredAccess, ~FILTER_OPLOCK_VALID_FLAGS) &&
            ...
            case IRP_MJ_READ:
                ...
                //  We will assume the oplock will have been broken if this create needed to do that.
            ...
            case IRP_MJ_SET_INFORMATION:
                //  We are only interested in calls that shrink the file size or breaking batch oplocks for the rename case.
            ...
            if (BreakToII) {
                //  If there are no outstanding oplocks or level II oplocks are held, we can return immediately.
                //  If the first two tests fail then there is an exclusive oplock.
                //  If the file objects match we allow the operation to continue.
                if ((OplockState != OplockIIGranted) && (OplockFileObject != IrpSp->FileObject)) {
                    Status = FsRtlOplockBreakToII((PNONOPAQUE_OPLOCK)*Oplock, IrpSp, Irp, Context, CompletionRoutine, PostIrpRoutine);
                }
            } else if (BreakToNone) {
                //  If there are no oplocks, we can return immediately.
                //  Otherwise if there is no level 2 oplock and this file object matches the owning file object then this write is on behalf of the owner of the oplock.
                if ((OplockState == OplockIIGranted) || (OplockFileObject != IrpSp->FileObject)) {
                    Status = FsRtlOplockBreakToNone((PNONOPAQUE_OPLOCK)*Oplock, IrpSp, Irp, Context, CompletionRoutine, PostIrpRoutine);
                }
            }
        }
    }
...
BOOLEAN FsRtlOplockIsFastIoPossible(__in POPLOCK Oplock)
/*
Routine Description:
    This routine indicates to the caller where there are any outstanding oplocks which prevent fast Io from happening.
Arguments:
...
*/
{
    ...
    //  There are not any current oplocks if the variable is null or the state is no oplocks held.
    //  If an exclusive oplock was granted but no break is in progress then allow the Fast IO.
    if (*Oplock != NULL) {
        OPLOCK_STATE OplockState;
...
BOOLEAN FsRtlCurrentBatchOplock(__in POPLOCK Oplock)
/*
Routine Description:
...
*/
{
    ...
    //  There are not any current oplocks if the variable is null or the state is no oplocks held.
    //  We check whether there are batch oplocks or filter oplocks which have not been broken.
...
/*
Routine Description:
    This routine is called to initialize and allocate an opaque oplock structure.
    After allocation, the two events are set to the signaled state.
    The oplock state is set to NoOplocksHeld and the other fields are filled with zeroes.
...
/*
Routine Description:
    This routine is called whenever a user is requesting either a batch/filter oplock or a level I oplock.
    The request is granted if there are currently no oplocks on the file or we are completing the filter oplock request.

    NOTE - We already know that the open count on this file is exactly one.
        If the caller is requesting a PendingFilter Oplock then the state must be NoOplockHeld.
Arguments:
    Oplock - Supplies a pointer to the non-opaque oplock structure for this file.
    IrpSp - This is the Irp stack location for the current Irp.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
          This is not specified if we are granting a pending filter oplock (during a create).
...
*/
{
    ...
    //  We can grant the oplock if no one else owns a level I or level II oplock on this file.
    //  If the oplock pointer is NULL then there are no oplocks on the file.
    //  Otherwise we need to check the oplock state in an existing oplock structure.
    if (*Oplock == NULL) {
    ...
    //  Use a try-finally to facilitate cleanup.
    try {
        //  If we are requesting a PendingFilter Oplock then it must be safe to grant.
        //  There is only one open handle and we are in the process of opening it.
        if (NextOplockState == OpFilterReqPending) {
            ...
            //  If the current oplock state is no oplocks held then we will grant the oplock to this requestor.
            //  If the state is either of the OpFilter states then also grant the request.
            //  We won't check for a matching file object because there can only be one file object.
            //  Grant the request anyway.

            //  If the current state is OplockII granted then it must be owned by this request.
            //  Break the oplock II and grant the exclusive lock.
            ...
            //  Now if the irp is cancelled then we'll call the cancel routine right now to do away with the irp, otherwise we set the cancel routine
            if (Irp->Cancel) {
                AcquiredMutex = FALSE;
            ...
            //  We'll complete the Irp with the Oplock not granted message and return that value as a status.
...
/*
Routine Description:
    This routine is called when a user is requesting an Oplock II on an open file.
    The request is granted if there are currently no level 1 oplocks on the file and an oplock break is not in progress.
Arguments:
    Oplock - Supplies a pointer to the non-opaque oplock structure for this file.
    IrpSp - This is the Irp stack location for the current Irp.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
...
*/
{
    ...
    //  We can grant the oplock if no one else owns a level I oplock on this file.
    //  If the oplock pointer is NULL then there are no oplocks on the file.
    //  Otherwise we need to check the oplock state in an existing oplock structure.
    if (*Oplock == NULL) {
        DebugTrace(0, Dbg, "Oplock currently not allocated\n", 0);
    ...
    //  Use a try-finally to facilitate cleanup.
    try {
        //  If the current oplock state is no oplocks held or OplockIIGranted then we will grant the oplock to this requestor.
...
/*
Routine Description:
    This routine is called when a user is acknowledging an Oplock I break.
    If the level 1 oplock was being broken to level 2, then a check is made to ensure that the level 2 has not been broken in the meantime.

    If an oplock 1 break is not in progress then this will be treated as an asynchronous break request.
    If this is an asynchronous break request and the file object owns an outstanding level 1 oplock, then the oplock will be broken at this point.

    A spurious break request via a file object which does not (or did not) own the level 1 oplock will generate a warning but will not affect the oplock state.
...
Arguments:
    ...
    IrpSp - This is the Irp stack location for the current Irp.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
    GrantLevelII - Indicates that this caller wants a level II oplock left on the file.
Return Value:
    STATUS_SUCCESS if we can complete the operation on exiting this thread.
    STATUS_CANCELLED if the Irp is cancelled before we return.
*/
{
    ...
    //  If there is no oplock structure, we complete this with invalid oplock protocol.
    if (Oplock == NULL) {
    ...
    try {
        ...
        //  If a break is underway but this is not the owner of the level 1 oplock, we complete the request and return a warning.
        ...
        //  If the user would like a level II and we are breaking to level II then grant the oplock.
        if (GrantLevelII && FlagOn(Oplock->OplockState, BREAK_TO_II)) {
            ...
            //  We need to add this Irp to the oplock II queue, 
            //  change the oplock state to Oplock II granted and set the return value to STATUS_PENDING.
            IoMarkIrpPending(Irp);
            Irp->IoStatus.Status = STATUS_SUCCESS;
            ...
            //  We will break to none since this is the expected case for these cases.
            ...
            //  In this case the user expects to be at level II.
            //  He is expecting this Irp to be completed when the LevelII Oplock is broken.
...
/*
Routine Description:
    This routine is called when a user is acknowledging a batch oplock break or Level I oplock break.
    In this case the user is planning to close the file as well and doesn't need a level II oplock.
Arguments:
    Oplock - Supplies a pointer to the non-opaque oplock structure for this file.
    IrpSp - This is the Irp stack location for the current Irp.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
...
*/
{
    ...
    try {
        ...
        if (Oplock->FileObject != IrpSp->FileObject) {
            ...
            //  If this is an opbatch operation we want to note that a close is pending.
            //  For an exclusive oplock we set the state to no oplocks held.
            //  There must be a break in progress to process however.
            if (FlagOn(Oplock->OplockState, BREAK_TO_II | BREAK_TO_NONE | BREAK_TO_II_TO_NONE)) {
                ...
                } else {//  Set the state to close pending for batch and filter oplocks.
        ...
        FsRtlCompleteRequest(Irp, Status);//  We simply complete this request.
    } finally{
        ExReleaseFastMutexUnsafe(Oplock->FastMutex);//  Release the synchronization object.
...
/*
Routine Description:
    This routine is called when the Irp refers the user request to be notified when there is no level 1 oplock break in progress.
    Under any other condition this routine completes immediately with STATUS_SUCCESS.
    Otherwise we simply add this Irp to the list of Irp's waiting for the break to complete.
Arguments:
    Oplock - Supplies a pointer to the non-opaque oplock structure for this file.
    IrpSp - This is the Irp stack location for the current Irp.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
...
*/
{
    ...
    try {
        ...
        //  Otherwise we need to add this Irp to the list of Irp's waiting for the oplock break to complete.
        AcquiredMutex = FALSE;
...
/*
Routine Description:
    This routine is called to coordinate a cleanup operation with the oplock state for a file.
    If there is no level 1 oplock for the file, then there is no action to take.
    If the file object in this Irp matches the file object used in granting the level 1 oplock, then the close operation will terminate the oplock.
    If this cleanup refers to a file object which has a level II oplock, then that Irp is completed and removed from the list of level II oplocked Irps.
Arguments:
    Oplock - Supplies a pointer to the non-opaque oplock structure for this file.
    IrpSp - This is the Irp stack location for the current Irp.
*/
{
    ...
    try {
        ...
        if (Oplock->OplockState == OplockIIGranted) {
            ...
            for (Link = Oplock->IrpOplocksII.Flink; Link != &Oplock->IrpOplocksII; Link = Link->Flink) {
                ...
                if (IrpSp->FileObject == NextIrpSp->FileObject) {
                    Link = Link->Blink;//  Back up to remember this link.
                    FsRtlRemoveAndCompleteIrp(Link->Flink);
                }
            }

            //  If all the level II oplocks are gone, then the state is no oplocks held.
        ...
        //  If this file object matches that used to request an exclusive oplock, we completely close the oplock break.
        if (IrpSp->FileObject == Oplock->FileObject) {
            DebugTrace(0, Dbg, "Handle owns level 1 oplock\n", 0);

            //  If an oplock break is not in progress, we initiate one and complete the exclusive Irp immediately.
...
/*
Routine Description:
    This routine is a generic worker routine which is called when an operation will cause all oplocks to be broken to level II before the operation can proceed.
Arguments:
    Oplock - Supplies a pointer to the non-opaque oplock structure for this file.
    IrpSp - This is the Irp stack location for the current Irp.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
...
*/
{
    ...
    try {
        ...
        //  If this file object owns that oplock, we allow the operation to continue.
        if (Oplock->FileObject == IrpSp->FileObject) {
            DebugTrace(0, Dbg, "Handle owns level 1 oplock\n", 0);
            try_return(Status = STATUS_SUCCESS);
        }

        //  If there is currently an exclusive oplock held then complete the exclusive irp.
        if (!FlagOn(Oplock->OplockState, PENDING | OPLOCK_BREAK_MASK)) {
            ...
            //  If the Irp has been cancelled, we complete the Irp with status cancelled and break the oplock completely.
        ...
        //  If this is an open operation and the user doesn't want to block, we will complete the operation now.
        if ((IrpSp->MajorFunction == IRP_MJ_CREATE) && FlagOn(IrpSp->Parameters.Create.Options, FILE_COMPLETE_IF_OPLOCKED)) {
            DebugTrace(0, Dbg, "Don't block open\n", 0);
            try_return(Status = STATUS_OPLOCK_BREAK_IN_PROGRESS);
        }

        //  If we get here that means that this operation can't continue until the oplock break is complete.
...
/*
Routine Description:
    This routine is a generic worker routine which is called when an operation will cause all oplocks to be broken before the operation can proceed.
Arguments:
    Oplock - Supplies a pointer to the non-opaque oplock structure for this file.
    IrpSp - This is the Irp stack location for the current Irp.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
...
*/
{
    ...
    try {
        ...
        if (!FlagOn(Oplock->OplockState, LEVEL_II_OPLOCK | PENDING | OPLOCK_BREAK_MASK)) {
            ...
            //  If we are currently breaking to level II then change that to BreakToIIToNone.
...
/*
Routine Description:
    This routine is called to remove an Irp from a list of Irps linked with the Tail.ListEntry field and 
    complete them with STATUS_CANCELLED if the Irp has been cancelled, STATUS_SUCCESS otherwise.
Arguments:
...
*/
{
    ...
    IoAcquireCancelSpinLock(&Irp->CancelIrql);//  Clear the cancel routine in the irp.

    IoSetCancelRoutine(Irp, NULL);
    IoReleaseCancelSpinLock(Irp->CancelIrql);

    RemoveEntryList(Link);// Remove this from the list.
...
/*
Routine Description:
    This routine is called to create a Wait Irp structure and attach it to the current Irp.
    The Irp is then added to the list of Irps waiting for an oplock break.
    We check if the Irp has been cancelled and if so we call our cancel routine to perform the work.

    This routine is holding the Mutex for the oplock on entry and must give it up on exit.
Arguments:
    Oplock - Supplies a pointer to the non-opaque oplock structure for this file.
    Irp - Supplies a pointer to the Irp which declares the requested operation.
    Context - This value is passed as a parameter to the completion routine.
    CompletionRoutine - This is the routine which is called if this Irp must wait for an Oplock to break.
                        This is a synchronous operation if not specified and we block in this thread waiting on an event.
    PostIrpRoutine - This is the routine to call before we put anything on our waiting Irp queue.
    Event - If there is no user completion routine, this thread will block using this event.
Return Value:
    STATUS_SUCCESS if we can complete the operation on exiting this thread.
    STATUS_PENDING if we return here but hold the Irp.
    STATUS_CANCELLED if the Irp is cancelled before we return.
*/
{
    ...
    AcquiredMutex = TRUE;//  Remember that we have the mutex.
    ...
    try {
        ...
        //  Take appropriate action if depending on the value of the completion routine.
        ...
        Irp->IoStatus.Status = STATUS_SUCCESS;//  Initialize the return value to status success.
        InsertTailList(&Oplock->WaitingIrps, &WaitingIrp->Links);//  We put this into the Waiting Irp queue.

        //  We grab the cancel spinlock and store the address of the oplock.
        IoAcquireCancelSpinLock(&Irp->CancelIrql);
        Irp->IoStatus.Information = (ULONG_PTR)Oplock;

        //  If the Irp is cancelled then we'll call the cancel routine right now to do away with the Waiting Irp structure.
        if (Irp->Cancel) {
            ExReleaseFastMutexUnsafe(OplockFastMutex);
            AcquiredMutex = FALSE;
            ...
            //  Otherwise, we set the cancel routine and decide whether we are going to wait on our local event.
        } else {
            IoSetCancelRoutine(Irp, FsRtlCancelWaitIrp);
            IoReleaseCancelSpinLock(Irp->CancelIrql);

            //  If we wait on the event, we pull the return code out of the Irp.
            if (!ARGUMENT_PRESENT(CompletionRoutine)) {
                ExReleaseFastMutexUnsafe(Oplock->FastMutex);
                AcquiredMutex = FALSE;
                KeWaitForSingleObject(Event, Executive, KernelMode, FALSE, NULL);
                Status = Irp->IoStatus.Status;
            } else {//  Otherwise, we return STATUS_PENDING.
                IoMarkIrpPending(Irp);
                Status = STATUS_PENDING;
            }
        }
...
/*
Routine Description:
    This routine is called when an operation must be synchronous with respect to the oplock package.
    This routine will simply set the event in the Signaled state, allowing some other thread to resume execution.
Arguments:
...
/*
Routine Description:
    This routine is called for an Irp that is placed on the waiting Irp queue.
    We remove the Cancel routine from the specified Irp and then call the completion routines for all the cancelled Irps on the queue.
Arguments:
    DeviceObject - Ignored.
    Irp - Supplies the Irp being cancelled.  A pointer to the Oplock structure for the Irp is stored in the information field of the Irp's Iosb.
*/
{
    ...
    try {
        for (Links = Oplock->WaitingIrps.Flink; Links != &Oplock->WaitingIrps; Links = Links->Flink) {
            ...
            if (WaitingIrp->Irp->Cancel) {
                //  Now we need to remove this waiter and call the completion routine.
                //  But we must not mess up our link iteration so we need to back up link one step and then the next iteration will go to our current flink.
                Links = Links->Blink;
                FsRtlRemoveAndCompleteWaitIrp(WaitingIrp);
            }
        }
    } finally{
        ExReleaseFastMutex(Oplock->FastMutex);//  No matter how we exit we release the mutex
...
/*
Routine Description:
    This routine is called for an Irp that is placed in the Oplock II Irp queue.
    We remove the Cancel routine from the specified Irp and then call the completion routines for all the cancelled Irps on the queue.
Arguments:
    DeviceObject - Ignored.
    Irp - Supplies the Irp being cancelled.  A pointer to the Oplock structure for the Irp is stored in the information field of the Irp's Iosb.
*/
{
    ...
    try {
        ...
        //  If the list is now empty, change the oplock status to no oplocks held.
...
*/
{
    ...
    try {
        //  We look for the exclusive Irp, if present and cancelled we complete it.
...
/*
Routine Description:
    This routine is called to remove and perform any necessary cleanup for an Irp stored on the waiting Irp list in an oplock structure.
Arguments:
    WaitingIrp - This is the auxiliary structure attached to the Irp being completed.
*/
{
    PIRP Irp;
    ...
    WaitingIrp->CompletionRoutine(WaitingIrp->Context, Irp);//  Call the completion routine in the Waiting Irp.
    ExFreePool(WaitingIrp);//  And free up pool
...
/*
Routine Description:
    This is the completion routine called when a break notify Irp is to be completed.
    We simply call FsRtlComplete request to dispose of the Irp.
Arguments:
    Context - Ignored.
