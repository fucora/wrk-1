...
#define FsRtlAllocatePool(PoolType, NumberOfBytes )                ExAllocatePoolWithTag((POOL_TYPE)((PoolType) | POOL_RAISE_IF_ALLOCATION_FAILURE), NumberOfBytes, 'trSF')
#define FsRtlAllocatePoolWithQuota(PoolType, NumberOfBytes )       ExAllocatePoolWithQuotaTag((POOL_TYPE)((PoolType) | POOL_RAISE_IF_ALLOCATION_FAILURE), NumberOfBytes, 'trSF')
...
//  This macro returns TRUE if a flag in a set of flags is on and FALSE otherwise
...
//  This macro takes a pointer (or ulong) and returns its rounded up word value

#define WordAlign(Ptr) (                \
    ((((ULONG_PTR)(Ptr)) + 1) & -2) \
    )

//  This macro takes a pointer (or ulong) and returns its rounded up longword value
#define LongAlign(Ptr) (                \
    ((((ULONG_PTR)(Ptr)) + 3) & -4) \
    )

//  This macro takes a pointer (or ulong) and returns its rounded up quadword value
#define QuadAlign(Ptr) (                \
    ((((ULONG_PTR)(Ptr)) + 7) & -8) \
    )

//  This macro takes a ulong and returns its value rounded up to a sector boundary
#define SectorAlign(Ptr) (                \
    ((((ULONG_PTR)(Ptr)) + 511) & -512) \
    )

//  This macro takes a number of bytes and returns the number of sectors required to contain that many bytes, i.e., it sector aligns and divides by the size of a sector.
#define SectorsFromBytes(bytes) ( \
    ((bytes) + 511) / 512         \
    )

//  This macro takes a number of sectors and returns the number of bytes contained in that many sectors.
#define BytesFromSectors(sectors) ( \
    (sectors) * 512                 \
    )

//  The following types and macros are used to help unpack the packed and misaligned fields found in the Bios parameter block
...
//  Macro for validating the FastIo dispatch routines before calling them in the FastIo pass through functions.
