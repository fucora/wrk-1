...
//  Wild characters are not considered legal, they should be checked separately if allowed.
...
BOOLEAN FsRtlInitSystem()
{
    ...
    if (!NT_SUCCESS(FsRtlInitializeWorkerThread())) {// Initialize the FsRtl stack overflow work QueueObject and thread.
        return FALSE;
    }
    
    if (!NT_SUCCESS(FsFilterInit())) {// Initialize the FsFilter component of FsRtl.
        return FALSE;
    }
...
/*
Routine Description:
    Given a unicode value name this routine will go into the registry location for the Chicago compatibility mode information and get the value.
Arguments:
