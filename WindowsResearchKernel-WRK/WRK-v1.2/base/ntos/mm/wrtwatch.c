...
Routine Description:
    ...
    UserAddressArray is filled with the base address of each page that has been written to since the last NtResetWriteWatch call (or if no NtResetWriteWatch calls have been made, then each page written since this address space was created).
Arguments:
    ProcessHandle - Supplies an open handle to a process object.
    Flags - Supplies WRITE_WATCH_FLAG_RESET or nothing.
    BaseAddress - An address within a region of pages to be queried. This value must lie within a private memory region with the write-watch attribute already set.
    RegionSize - The size of the region in bytes beginning at the base address specified.
    UserAddressArray - Supplies a pointer to user memory to store the user addresses modified since the last reset.
    UserAddressArrayEntries - Supplies a pointer to how many user addresses can be returned in this call.
                              This is then filled with the exact number of addresses actually returned.
    Granularity - Supplies a pointer to a variable to receive the size of modified granule in bytes.
Return Value:
...
*/
{
    ...
    SearchResult = MiFindNodeOrParent(Process->PhysicalVadRoot, MI_VA_TO_VPN(BaseAddress), (PMMADDRESS_NODE *)&PhysicalView);
    if ((SearchResult == TableFoundNode) &&
        ...
        // No virtual address is marked for write-watch at the specified base address, return an error.
        UNLOCK_WS(CurrentThread, Process);
        Status = STATUS_INVALID_PARAMETER_1;
        goto ErrorReturn;
    }
    ...
    while (PointerPte <= EndPte) {
        ...
        // If the PTE is marked dirty (or writable) OR the BitMap says it's dirtied, then let the caller know.
        if (RtlCheckBit(BitMap, BitMapIndex) == 1) {
            UserWritten = TRUE;

            // Note that a chunk of bits cannot be cleared at once because the user array may overflow at any time.
            // If the user specifies a bad address and the results cannot be written out, then it's his own fault that he won't know which bits were cleared !
        ...
        ClearPteIfValid:
            // If the page table page is not present, then the dirty bit has already been captured to the write watch bitmap.
            // Unfortunately all the entries in the page cannot be skipped as the write watch bitmap must be checked for each PTE.
#if (_MI_PAGING_LEVELS >= 4)
            if (PointerPxe->u.Hard.Valid == 0) {
                // Skip the entire extended page parent if the bitmap permits.
                // The search starts at BitMapIndex (not BitMapIndex + 1) to avoid wraps.
                ...
                // Compare the bitmap jump with the PTE jump and take the lesser of the two.
                if ((NextBitMapIndex == NO_BITS_FOUND) || ((ULONG)(NextPte - PointerPte) < (NextBitMapIndex - BitMapIndex))) {
            ...
            if ((PteContents.u.Hard.Valid == 1) && (MI_IS_PTE_DIRTY(PteContents))) {
                ...
                if (Flags & WRITE_WATCH_FLAG_RESET) {
                    ...
                    // For all other platforms, the PTE writable bit must be disabled now so future writes trigger write watch updates.
                    PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(&PteContents);
                    Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
                    ASSERT(Pfn1->u3.e1.PrototypePte == 0);
                    MI_MAKE_VALID_USER_PTE(TempPte, PageFrameIndex, Pfn1->OriginalPte.u.Soft.Protection, PointerPte);
                    ...
                    // A page's protection is being changed, on certain hardware the dirty bit should be ORed into the modify bit in the PFN element.
        ...
        if (UserWritten == TRUE) {
            *PoolAreaPointer = BaseAddress;
            ...
            if (PagesWritten == NumberOfPages) {
                // User array isn't big enough to take any more.
                // The API (inherited from Win9x) is defined to return at this point.
                break;
            }
        }
...
Routine Description:
    ...
    This allows callers to "forget" old writes and only see new ones from this point on.
Arguments:
    ProcessHandle - Supplies an open handle to a process object.
    BaseAddress - An address within a region of pages to be reset.  This value must lie within a private memory region with the write-watch attribute already set.
    RegionSize - The size of the region in bytes beginning at the base address specified.
Return Value:
    Various NTSTATUS codes.
*/
{
    ...
    // Clear the write watch status (and PTE writable/dirty bits) for each page in the range.
    // Note if the PTE is not currently valid, then the write watch bit has already been captured to the bitmap.
    // Hence only valid PTEs need adjusting.
    ...
    LOCK_PFN(OldIrql);
    while (PointerPte <= EndPte) {
        // If the page table page is not present, then the dirty bit has already been captured to the write watch bitmap.  So skip it.
...
/*
Routine Description:
    This routine sets the write watch bit corresponding to the argument virtual address.
Arguments:
