...
Abstract:
    This module contains the routines to manipulate pages within the Page Frame Database.
*/

#include "mi.h"


// The following line will generate an error if the number of colored free page lists is not 2, ie ZeroedPageList and FreePageList.
// If this number is changed, the size of the FreeCount array in the kernel node structure (KNODE) must be updated.
...
// This counter is used to determine if standby pages are being cannibalized for use as free pages and therefore more aging should be attempted.
...
PFN_NUMBER FASTCALL MiRemovePageByColor(IN PFN_NUMBER Page, IN ULONG PageColor);
...
*/
{
    ...
    MM_PFN_LOCK_ASSERT();
    ASSERT((PageFrameIndex != 0) && (PageFrameIndex <= MmHighestPhysicalPage) && (PageFrameIndex >= MmLowestPhysicalPage));
    ...
    if ((MiSaveStacks != 0) && (MmFirstReservedMappingPte != NULL)) {
        ...
        // Copy the callstack into the middle of the page (since special pool is using the beginning of the freed page).
        DataPage[PAGE_SIZE / (2 * sizeof(ULONG_PTR))] = StackPointer;

        // For now, don't get fancy with copying more than what's in the current stack page.
        // To do so would require checking the thread stack limits, DPC stack limits, etc.
    ...
    // Inserting the page at the front would make better use of the hardware caches, but it makes debugging much harder because the pages get reused so quickly.
    // For now, continue to insert at the back of the list.
    last = ListHead->Blink;
    ...
    // Update the count of usable pages in the system.
    // If the count transitions from 0 to 1, the event associated with available pages should become true.
    MmAvailablePages += 1;

    // A page has just become available, check to see if the page wait events should be signaled.
    ...
    if (ColorHead->Flink == MM_EMPTY_LIST) {
        // This list is empty, add this as the first and last entry.
    ...
    if ((ListHead->Total >= MmMinimumFreePagesToZero) && (MmZeroingPageThreadActive == FALSE)) {
        // There are enough pages on the free list, start the zeroing page thread.
...
/*
Routine Description:
    This procedure inserts a page at the end of the specified list (standby, bad, zeroed, modified).
Arguments:
    ListHead - Supplies the listhead of the list in which to insert the specified physical page.
    PageFrameIndex - Supplies the physical page number to insert in the list.
Return Value:
    None.
Environment:
...
*/
{
    ...
    if (MmDebug & MM_DBG_PAGE_REF_COUNT) {
        if ((ListName == StandbyPageList) || (ListName == ModifiedPageList)) {
            ...
            if ((Pfn1->u3.e1.PrototypePte == 1) && (MmIsAddressValid(Pfn1->PteAddress))) {
                ...
                // The page containing the prototype PTE is not valid, map the page into hyperspace and reference it that way.
                Process = PsGetCurrentProcess();
            ...
            ASSERT((MI_GET_PAGE_FRAME_FROM_TRANSITION_PTE(PointerPte) == PageFrameIndex) || (MI_GET_PAGE_FRAME_FROM_PTE(PointerPte) == PageFrameIndex));
    ...
    if (ListHead == &MmModifiedPageListHead) {
        // On MIPS R4000 modified pages destined for the paging file are kept on separate lists which group pages of the same color together.
        if (Pfn1->OriginalPte.u.Soft.Prototype == 0) {
            // This page is destined for the paging file (not a mapped file).  Change the list head to the appropriate colored list head.
            ...
            // This page is destined for a mapped file (not the paging file).
            // If there are no other pages currently destined for the mapped file, start our timer so that we can ensure that these pages make it to disk even if we don't pile
            // up enough of them to trigger the modified page writer or need the memory.
            // If we don't do this here, then for this scenario, only an orderly system shutdown will write them out (days, weeks, months or years later) and any power out in between means we'll have lost the data.
    ...
    // Pages destined for the zeroed list go to the front to take advantage of cache locality.
    // All other lists get the page at the rear for LRU longest life.
    ...
    // If the page was placed on the standby or zeroed list, update the count of usable pages in the system.
    // If the count transitions from 0 to 1, the event associated with available pages should become true.
    if (ListName <= StandbyPageList) {
        MmAvailablePages += 1;
        // A page has just become available, check to see if the page wait events should be signaled.
    ...
    if (ListName == ModifiedPageList) {
        ...
        if (MmAvailablePages < MM_PLENTY_FREE_LIMIT) {
            ...
            } else if ((MmAvailablePages < MM_TIGHT_LIMIT) && (MmModifiedPageListHead.Total >= MmModifiedWriteClusterSize)) {
                KeSetEvent(&MmModifiedPageWriterEvent, 0, FALSE);
            }
        }
...
Routine Description:
    ...
    This is only needed at system initialization to keep the higher physically numbered pages at the front of the zeroed list (normally we only put zeroed pages in the front of the list for better cache locality).
Arguments:
    PageFrameIndex - Supplies the physical page number to insert in the list.
Environment:
    Must be holding the PFN database lock.
...
*/
{
    ...
    // If the page was placed on the free, standby or zeroed list, update the count of usable pages in the system.
    // If the count transitions from 0 to 1, the event associated with available pages should become true.
    MmAvailablePages += 1;
...
/*
Routine Description:
    This procedure removes a page from the head of the specified list (free, standby or zeroed).
    This routine clears the flags word in the PFN database, hence the PFN information for this page must be initialized.
Arguments:
    ListHead - Supplies the listhead of the list in which to remove the specified physical page.
Return Value:
...
*/
{
    ...
    // For standby removals, the caller is responsible for specifying which prioritized standby list to remove from.
    ...
    // Decrement the count of pages on the list and remove the first page from the list.
    ListHead->Total -= 1;
    ...
    // If the last page was removed (the ListHead->Flink is now MM_EMPTY_LIST) make the Listhead->Blink MM_EMPTY_LIST as well.
    if (ListHead->Flink != MM_EMPTY_LIST) {
        // Make the PFN element blink point to MM_EMPTY_LIST signifying this is the first page in the list.
    ...
    MmAvailablePages -= 1;
    if (ListName == StandbyPageList) {
        // This page is currently in transition, restore the PTE to its original contents so this page can be reused.
...
/*
Routine Description:
    This procedure removes a page from the middle of a list.
    This is designed for the faulting of transition pages from the standby and modified list and making them active and valid again.
Arguments:
    Pfn - Supplies a pointer to the PFN database element for the physical page to remove from the list.
Environment:
    Must be holding the PFN database lock.
*/
{
    ...
    // Page not on standby or modified list, check to see if the page is currently being written by the modified page writer, if so, just return this page.
    // The reference count for the page will be incremented, so when the modified page write completes, the page will not be put back on the list, rather, it will remain active and valid.
    if (Pfn->u3.e2.ReferenceCount > 0) {
        // The page was not on any "transition lists", check to see if this has I/O in progress.
        if (Pfn->u2.ShareCount == 0) {
            return;
        }
        KdPrint(("MM:attempt to remove page from wrong page list\n"));
        KeBugCheckEx(PFN_LIST_CORRUPT, 2, MI_PFN_ELEMENT_TO_INDEX(Pfn), MmHighestPhysicalPage, Pfn->u3.e2.ReferenceCount);
    }

    ListHead = MmPageLocationList[Pfn->u3.e1.PageLocation];

    // Must not remove pages from free or zeroed without updating the colored lists.
...
Routine Description:
    This procedure removes a page from the middle of a list.
    This is designed for the removing of free or zeroed pages from the middle of their lists.
Arguments:
...
*/
{
    ...
    MmAvailablePages -= 1;
    if (MmAvailablePages < MmMinimumFreePages) {
        MiObtainFreePages();// Obtain free pages.
    }
}
...
VOID MiNoPagesLastChance(IN ULONG Limit)
{
    ULONG i;
    KIRQL OldIrql;
    PMMPFN Pfn1;
    ...
    // A driver has blocked, deadlocking the modified or mapped page writers.
    // Examples of this include mutex deadlocks or accesses to paged out memory in filesystem drivers, filter drivers, etc.  This indicates a driver bug.

    // The storage driver(s) are not processing requests.
    // Examples of this are stranded queues, non-responding drives, etc.  This indicates a driver bug.

    // Not enough pool is available for the storage stack to write out modified pages.  This indicates a driver bug.

    // A high priority realtime thread has starved the balance set manager from trimming pages and/or starved the modified writer from writing them out.
    // This indicates a bug in the component that created this thread.
    BitField = 0;
    ...
    if (MmSystemShutdown != 0) {
        // Because applications are not terminated and drivers are not unloaded, they can continue to access pages even after the modified writer has terminated.
        // This can cause the system to run out of pages since the pagefile(s) cannot be used.
    ...
    if ((KdPitchDebugger == FALSE) && (KdDebuggerNotPresent == FALSE)) {
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "Without a debugger attached, the following bugcheck would have occurred.\n");
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "%4lx ", BugcheckCode);
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "%p %p %p %p\n", MmModifiedPageListHead.Total, PagesOnList, BitField, Status);

        // Pop into the debugger (even on free builds) to determine the cause of the starvation and march on.
        if (Mi4dBreak == TRUE) {
            DbgBreakPoint();
        }
    ...
    // Toss modified mapped pages until the limit is reached allowing this thread to make forward progress.
    // Each toss represents lost data so this is very bad.
    // But if the filesystem is deadlocked then there is nothing else that can be done other than to crash.
    // This tossing provides the marginal benefit that at least a delayed write popup event is queued (with the name of the file) and the system is kept alive - the administrator will then *HAVE* to fix these files.
    // The alternative would be to continue to bugcheck the system which not only impacts availability, but also loses *all* the delayed mapped writes without any event queueing to inform the administrator about which files have been corrupted.
    FilePointer = NULL;
    LdwContext = &MiLastChanceLdwContext;

    LOCK_PFN(OldIrql);

    // If enough free pages have been created then return to give our caller another chance.
    // Note another thread may have already done this for us.
    ...
    if (LdwContext->FileObject != NULL) {
        // Some other thread is tossing pages right now, just return for another delay.
        UNLOCK_PFN(OldIrql);
        return;
    }
    ...
    while (ModifiedPage != MM_EMPTY_LIST) {
        ...
        if (Pfn1->OriginalPte.u.Soft.Prototype == 1) {
            // This page is destined for a file.  Chuck it and any other pages destined for the same file.
            Subsection = MiGetSubsectionAddress(&Pfn1->OriginalPte);
            ControlArea = Subsection->ControlArea;
            ASSERT(ControlArea->FilePointer != NULL);
            if ((!ControlArea->u.Flags.Image) && (!ControlArea->u.Flags.NoModifiedWriting) && ((FilePointer == NULL) || (FilePointer == ControlArea->FilePointer))) {
                ASSERT(ControlArea->NumberOfPfnReferences >= 1);
                MiUnlinkPageFromList(Pfn1);

                // Use the reference count macros so we don't prematurely free the page because the physical page may have references already.
                MI_ADD_LOCKED_PAGE_CHARGE_FOR_MODIFIED_PAGE(Pfn1);

                // Clear the dirty bit in the PFN entry so the page will go to the standby instead of the modified list.
                // If the page is not modified again before it is reused, the data will be lost.
                MI_SET_MODIFIED(Pfn1, 0, 0x1A);

                // Put the page on the standby list if it is the last reference to the page.
                ...
                // Search for any other modified pages in this control area because the entire file needs to be fixed anyway.
            }
        }
    }
    ...
    // If we were able to free up some pages then return to our caller for another attempt/wait if necessary.
    // Otherwise if no debugger is attached, then there's no point in continuing so bugcheck.
    if (FilePointer != NULL) {
    ...
    if (MarchOn == FALSE) {
        KeBugCheckEx(BugcheckCode, MmModifiedPageListHead.Total, PagesOnList, BitField, Status);
    }
}
...
/*
Routine Description:
    This procedure ensures that a physical page is available on the zeroed, free or standby list such that the next call to remove a page absolutely will not block.
    This is necessary as blocking would require a wait which could cause a deadlock condition.

    If a page is available the function returns immediately with a value of FALSE indicating no wait operation was performed.
    If no physical page is available, the thread enters a wait state and the function returns the value TRUE when the wait operation completes.
Arguments:
    Process - Supplies a pointer to the current process if, and only if, the working set mutex is currently held and should be released if a wait operation is issued.
              Supplies the value NULL otherwise.
...
*/
{
    ...
    // If this thread has explicitly disabled APCs (FsRtlEnterFileSystem does this), then it may be holding resources or mutexes that may in turn be blocking memory making threads from making progress.
    // We'd like to detect this but cannot (without changing the FsRtlEnterFileSystem macro) since other components (win32k for example) also enter critical regions and then access paged pool.
    // At least give system threads a free pass as they may be worker threads processing potentially blocking items drivers have queued.
    ...
    // Initializing WsHeldSafe is not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    Ws = NULL;
    WsHeldSafe = FALSE;
    WsHeldShared = FALSE;
    ...
    do {
        ...
        if (Process == HYDRA_PROCESS) {
            ...
            // The working set lock may have been acquired safely or unsafely by our caller.  Handle both cases here and below.
        ...
        // Wait 70 seconds for pages to become available.
        // Note this number was picked because it is larger than both SCSI and redirector timeout values.

        // Unfortunately we are using a notification event and may be waiting in some cases with APCs enabled.
        // Thus inside KeWait, the APC is delivered and then the event gets signaled.
        // The APC is handled, but the available pages are taken and the event above cleared by other thread(s).
        // Then this thread looks at the event and sees it isn't signaled (and thus doesn't realize it ever happened) and so goes back into a wait state.
        // In a pathological case (we have seen this happen), this scenario repeats until the thread's timeout expires and gets returned as such, even though the event has been signaled many times.
        Status = KeWaitForSingleObject(Event, WrFreePage, KernelMode, FALSE, (PLARGE_INTEGER)&MmSeventySeconds);
...
/*
Routine Description:
    This procedure removes a zero page from either the zeroed, free or standby lists (in that order).
    If no pages exist on the zeroed or free list a transition page is removed from the standby list and the PTE (may be a prototype PTE) which refers to this page is changed from transition back to its original contents.
...
Arguments:
    Color - Supplies the page color for which this page is destined.
            This is used for checking virtual address alignments to determine if the D cache needs flushing before the page can be reused.

            The above was true when we were concerned about caches which are virtually indexed (ie MIPS).
            Today we are more concerned that we get a good usage spread across the L2 caches of most machines.
            These caches are physically indexed.
            By gathering pages that would have the same index to the same color, then maximizing the color spread, we maximize the effective use of the caches.

            This has been extended for NUMA machines.
            The high part of the color gives the node color (basically node number).
            If we cannot allocate a page of the requested color, we try to allocate a page on the same node before taking a page from a different node.
Return Value:
    The physical page number removed from the specified list.
Environment:
    Must be holding the PFN database lock.
*/
{
...
#if defined(MI_MULTINODE)
    // Initializing Node is not needed for correctness, but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    Node = NULL;
    ...
    do {
        ...
        // Attempt to remove a page from the zeroed page list.
        // If a page is available, then remove it and return its page frame index.
        // Otherwise, attempt to remove a page from the free page list or the standby list.

        // N.B. It is not necessary to change page colors even if the old color is not equal to the new color.
        //      The zero page thread ensures that all zeroed pages are removed from all caches.
...
#endif
            ASSERT((Pfn1->u3.e1.PageLocation == ZeroedPageList) || ((Pfn1->u3.e1.PageLocation == FreePageList) && (FreePagesByColor == MmFreePagesByColor[FreePageList])));
...
#if defined(MI_MULTINODE)
        // If this is a multinode machine and there are zero pages on this node, select another color on this node in preference to random selection.
        if (KeNumberNodes > 1) {
            ...
            // Since this is a multinode machine, zero an available local free page now instead of allocating a zeroed page from another node below.
    ...
    // No zeroed page of the primary color exists, try a free page of the secondary color.
    // Note in the multinode case this has already been done above.
#if defined(MI_MULTINODE)
    if (KeNumberNodes <= 1) {
#endif
        FreePagesByColor = MmFreePagesByColor[FreePageList];
    ...
    // Remove a page from the standby list and restore the original contents of the PTE to free the last reference to the physical page.
...
PFN_NUMBER FASTCALL MiRemoveAnyPage(IN ULONG Color)
/*
Routine Description:
    This procedure removes a page from either the free, zeroed, or standby lists (in that order).
    If no pages exist on the zeroed or free list a transition page is removed from the standby list and the PTE (may be a prototype PTE) which refers to this page is changed from transition back to its original contents.

    Note pages MUST exist to satisfy this request.
    The caller ensures this by first calling MiEnsureAvailablePageOrWait.
Arguments:
    Color - Supplies the page color for which this page is destined.
            This is used for checking virtual address alignments to determine if the D cache needs flushing before the page can be reused.

            The above was true when we were concerned about caches which are virtually indexed.   (eg MIPS).
            Today we are more concerned that we get a good usage spread across the L2 caches of most machines.
            These caches are physically indexed.
            By gathering pages that would have the same index to the same color, then maximizing the color spread, we maximize the effective use of the caches.
...
*/
{
...
#if defined(MI_MULTINODE)
    // Bias color to memory node.
    // The assumption is that if memory of the correct color is not available on this node, it is better to choose memory of a different color if you can stay on this node.
    ...
    do {
        ...
        // Check the free page list, and if a page is available remove it and return its value.
        ASSERT(Color < MmSecondaryColors);
        ...
        // If this is a multinode machine and there are free pages on this node, select another color on this node in preference to random selection.
#if defined(MI_MULTINODE)
    ...
    // Check the zeroed page list, and if a page is available remove it and return its value.
...
PFN_NUMBER FASTCALL MiRemovePageByColor(IN PFN_NUMBER Page, IN ULONG Color)
/*
Routine Description:
    This procedure removes a page from the front of the free or zeroed page list.
Arguments:
    Page - Supplies the physical page number to unlink from the list.
    Color - Supplies the page color for which this page is destined.
            This is used for checking virtual address alignments to determine if the D cache needs flushing before the page can be reused.
Return Value:
    The page frame number that was unlinked (always equal to the one passed in, but returned so the caller's fastcall sequences save extra register pushes and pops.
Environment:
    Must be holding the PFN database lock.
*/
{
...
VOID MiPurgeTransitionList(VOID)
{
    ...
    // Run the transition list and free all the entries so transition faults are not satisfied for any of the non modified pages that were freed.
    for (ListHead = &MmStandbyPageListByPriority[0]; ListHead < &MmStandbyPageListByPriority[MI_PFN_PRIORITIES]; ListHead += 1) {
        if (ListHead->Total == 0) {
            continue;
        }
