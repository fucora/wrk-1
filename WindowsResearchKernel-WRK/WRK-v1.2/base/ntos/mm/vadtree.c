...
Abstract:
    This module contains the routines to manipulate the virtual address descriptor tree.
...
/*
Routine Description:
    This function charges various quotas for the specified virtual address descriptor.
Arguments:
...
*/
{
    ...
    if (Vad->u.VadFlags.CommitCharge != MM_MAX_COMMIT) {
        ...
        // Charge quota for the nonpaged pool for the VAD.
        ...
        if (PageCharge != 0) {
            // Since the commitment was successful, charge the page table pages.
    ...
    // Note VADs like the PEB & TEB start on page (not 64K) boundaries so for these, the relevant bits may already be set.
    ...
    if ((CurrentProcess->VadFreeHint != NULL) && (((ULONG_PTR)((PMMVAD)CurrentProcess->VadFreeHint)->EndingVpn + MI_VA_TO_VPN(X64K)) >= Vad->StartingVpn)) {
...
Arguments:
    ...
    QuickCheck - Supplies a zero if a quick check for free memory after the VadFreeHint exists, non-zero if checking should start at the lowest address.
...
*/
{
    ...
    if ((QuickCheck == 0) && (Alignment == X64K)) {
        ...
        // Skip the first bit here as we don't generally recommend that applications map virtual address zero.
        ...
        *((PULONG)VAD_BITMAP_SPACE) &= FirstBitValue;
        if (StartPosition != NO_BITS_FOUND) {
            *Base = (PVOID)(((ULONG_PTR)StartPosition) * X64K);
            ...
            if (MiCheckForConflictingVad(CurrentProcess, *Base, (ULONG_PTR)*Base + SizeOfRange - 1) != NULL) {
                DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "MiFindEmptyAddressRange: overlapping VAD %p %p\n", *Base, SizeOfRange);
                DbgBreakPoint();
            }
#endif
            return STATUS_SUCCESS;
        ...
        if (FreeHint != NULL) {
            ...
            if (NextVad == NULL) {
                if (SizeOfRange < (((ULONG_PTR)MM_HIGHEST_USER_ADDRESS + 1) - MI_ROUND_TO_SIZE((ULONG_PTR)EndingVa, Alignment))) {
    ...
    Status = MiFindEmptyAddressRangeInTree(SizeOfRange, Alignment, &CurrentProcess->VadRoot, (PMMADDRESS_NODE *)&CurrentProcess->VadFreeHint, Base);
    return Status;
}
...
PFILE_OBJECT * MmPerfVadTreeWalk(IN PEPROCESS TargetProcess)
/*
Routine Description:
    This routine walks through the VAD tree to find all files mapped into the specified process.
    It returns a pointer to a pool allocation containing the referenced file object pointers.
Arguments:
    TargetProcess - Supplies the process to walk.  Note this is usually NOT the same as the current process.
Return Value:
    Returns a pointer to a NULL terminated pool allocation containing the file object pointers which have been referenced in the process, NULL if the memory could not be allocated.
...
/*
Routine Description:
    This routine compares two virtual addresses (Va1 and Va2) to determine whether they fall within the same VAD in the current process or the target address (Va3) falls inside an image file.
Arguments:
    ...
    PermitStackExecution - Indicates whether the thunk may reside on the stack.
Return Value:
    Returns TRUE if execution of the thunk is permitted, FALSE otherwise.
Environment:
    PASSIVE_LEVEL, arbitrary thread context.
    Address space lock not taken.
*/
{
    PEPROCESS CurrentProcess;
    BOOLEAN RetValue;
    ...
    if (PermitStackExecution == FALSE) {
        // Ensure that the instruction pointer does not refer to the same VAD as the stack pointer.
        // And that the instruction pointer does not reside in an image section.
        InstructionVad = MiLocateAddress(InstructionPointer);
        StackVad = MiLocateAddress(StackPointer);
        if ((InstructionVad == NULL) || (StackVad == NULL) || (InstructionVad == StackVad) || (InstructionVad->u.VadFlags.VadType == VadImageMap)) {
            RetValue = FALSE;
        }
    }
    if (RetValue != FALSE) {
        ...
        if (TargetVad == NULL || TargetVad->u.VadFlags.VadType != VadImageMap) {
            RetValue = FALSE;
        }
    }
