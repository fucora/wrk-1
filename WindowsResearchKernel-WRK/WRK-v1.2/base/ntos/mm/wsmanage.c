...
Abstract:
    This module contains routines which manage the set of active working set lists.

    Working set management is accomplished by a parallel group of actions
        1. Writing modified pages.
        2. Trimming working sets by :
            a) Aging pages by turning off access bits and incrementing age counts for pages which haven't been accessed.
            b) Estimating the number of unused pages in a working set and keeping a global count of that estimate.
            c) When getting tight on memory, replacing rather than adding pages in a working set when a fault occurs in a working set that has a significant proportion of unused pages.
            d) When memory is tight, reducing (trimming) working sets which are above their maximum towards their minimum.
               This is done especially if there are a large number of available pages in it.
    The metrics are set such that writing modified pages is typically accomplished before trimming working sets, however, under certain cases
    where modified pages are being generated at a very high rate, working set trimming will be initiated to free up more pages.

    Once a process has had its working set raised above the minimum specified, the process is put on the Working Set Expanded list and is now eligible for trimming.
    Note that at this time the FLINK field in the WorkingSetExpansionLink has an address value.
*/
...
const LARGE_INTEGER MmClaimParameterAdjustUpTime = {60 * 1000 * 1000 * 10, 0};// Sixty seconds.
const LARGE_INTEGER MmClaimParameterAdjustDownTime = {2 * 1000 * 1000 * 10, 0};// 2 seconds.
...
ULONG MiComputeSystemTrimCriteria(IN OUT PMMWS_TRIM_CRITERIA Criteria);
LOGICAL MiCheckSystemTrimEndCriteria(IN OUT PMMWS_TRIM_CRITERIA Criteria, IN KIRQL OldIrql);
WSLE_NUMBER MiDetermineTrimAmount(IN PMMWS_TRIM_CRITERIA Criteria, IN PMMSUPPORT VmSupport);
VOID MiProcessWorkingSets(IN ULONG WorkingSetRequest, IN PMMWS_TRIM_CRITERIA TrimCriteria);
VOID MiAdjustClaimParameters(IN LOGICAL EnoughPages);
VOID MiRearrangeWorkingSetExpansionList(VOID);
VOID MiCaptureAndResetWorkingSetAccessBits(IN PMMSUPPORT WsInfo, IN ULONG Flags);


VOID MiAdjustWorkingSetManagerParameters(IN LOGICAL WorkStation)
/*
Routine Description:
    This function is called from MmInitSystem to adjust the working set manager trim algorithms based on system type and size.
Arguments:
...
/*
Routine Description:
    This function examines the size of the modified list and the total number of pages in use because of working set increments and obtains pages by writing modified pages and/or reducing working sets.
Environment:
    Kernel mode, APCs disabled, working set and PFN mutexes held.
*/
{
    // Check to see if there are enough modified pages to institute a write.
    if (MmModifiedPageListHead.Total >= MmModifiedWriteClusterSize) {
        KeSetEvent(&MmModifiedPageWriterEvent, 0, FALSE);// Start the modified page writer.
    }

    // See if there are enough working sets above the minimum threshold to make working set trimming worthwhile.
    if ((MmPagesAboveWsMinimum > MmPagesAboveWsThreshold) || (MmAvailablePages < 5)) {
        KeSetEvent(&MmWorkingSetManagerEvent, 0, FALSE);// Start the working set manager to reduce working sets.
    }
}


LOGICAL MmIsMemoryAvailable(IN PFN_NUMBER PagesDesired)
/*
Routine Description:
    This function checks whether there are sufficient available pages based on the caller's request.
    If currently active pages are needed to satisfy this request and non-useful ones can be taken, then trimming is initiated here to do so.
Arguments:
...
*/
{
    ...
    // If twice the pages that the caller asked for are available without trimming anything, return TRUE.
    ...
    // If there are few pages available or claimable, we adjust to do a hard trim.
    if (CurrentAvailablePages + CurrentTotalClaim < PagesDesired) {
        MiHardTrim = TRUE;
    }

    // Active pages must be trimmed to satisfy this request and it is believed that non-useful pages can be taken to accomplish this.
...
LOGICAL MiAttachAndLockWorkingSet(IN PMMSUPPORT VmSupport)
/*
Routine Description:
    This function attaches to the proper address space and acquires the relevant working set mutex for the address space being trimmed.
    If successful, this routine returns with APCs blocked as well.
    On failure, this routine returns without any APCs blocked, no working set mutex acquired and no address space attached to.
Arguments:
...
*/
{
    ...
    if (VmSupport == &MmSystemCacheWs) {
        ...
        if (ExTryAcquirePushLockExclusive(&VmSupport->WorkingSetMutex) == FALSE) {
            // System working set mutex was not granted, don't trim the system cache.
    ...
    if (VmSupport->Flags.SessionSpace == 0) {
        ...
        if (ProcessToTrim != PsInitialSystemProcess) {
            ...
            if (ProcessToTrim->Flags & PS_PROCESS_FLAGS_OUTSWAP_ENABLED) {
                // We have effectively performed an inswap of the process due to the force attach.
                // Mark the process (and session) accordingly.
        ...
        // Attempt to acquire the working set pushlock.
        // If the lock cannot be acquired, skip over this process.
        count = 0;
        do {
            KeEnterGuardedRegionThread(&Thread->Tcb);
            if (ExTryAcquirePushLockExclusive(&VmSupport->WorkingSetMutex) != FALSE) {
    ...
    MiAttachSession(SessionSpace);// Attach directly to the session space to be trimmed.
    KeEnterGuardedRegionThread(&Thread->Tcb);// Try for the session working set mutex.
    if (ExTryAcquirePushLockExclusive(&VmSupport->WorkingSetMutex) == FALSE) {
        // This session space's working set mutex was not granted, don't trim it.
        KeLeaveGuardedRegionThread(&Thread->Tcb);
        MiDetachSession();
        return FALSE;
    }
...
VOID MiDetachAndUnlockWorkingSet(IN PMMSUPPORT VmSupport)
/*
Routine Description:
    This function detaches from the target address space and releases the relevant working set mutex for the address space that was trimmed.
Arguments:
...
/*
Routine Description:
    Implements the NT working set manager thread.
    When the number of free pages becomes critical and ample pages can be obtained by reducing working sets,
    the working set manager's event is set, and this thread becomes active.
Environment:
    Kernel mode.
*/
{
    ...
    // Set the trim criteria: If there are plenty of pages, the existing sets are aged and FALSE is returned to signify no trim is necessary.
    // Otherwise, the working set expansion list is ordered so the best candidates for trimming are placed at the front and TRUE is returned.
    ...
    if (WorkingSetRequestFlags != 0) {
        ...
        if (WorkingSetRequestFlags & (MI_TRIM_ALL_WORKING_SETS | MI_EMPTY_ALL_WORKING_SETS)) {
            MiDeferredUnlockPages(0);// Clear the deferred entry list to free up some pages.
        }

        MiProcessWorkingSets(WorkingSetRequestFlags, &TrimCriteria);
    }

    // If memory is critical then when we trimmed any dirty pages, the modified writer was signaled during the insertion on the modified list.
    // If there are a significant number of modified pages to be written then signal the modified writer now, regardless of the above to avoid generating large backlogs.
...
Arguments:
    Criteria - Supplies a pointer to the trim criteria information.  Various fields in this structure are set as needed by this routine.
Return Value:
...
*/
{
    ...
    // Check the number of pages available to see if any trimming (or aging) is really required.
    ...
    if (StandbyRemoved <= MiLastStandbyRePurposed) {
        ...
        // The value is nonzero, we need to synchronize so we get a coordinated snapshot of both values.
        LOCK_PFN(OldIrql);
        Available = MmAvailablePages;
        StandbyRemoved = MmStandbyRePurposed;
        UNLOCK_PFN(OldIrql);
    ...
    // If we're low on pages, or we've been replacing within a given working set, or we've been cannibalizing a large number of standby pages, then trim now.
    if ((Available <= PlentyFreePages) || (MiReplacing == TRUE) || (StandbyRemoved >= (Available >> 2))) {
        ...
        // Inform our caller to start trimming since we're below plenty pages - order the list so the bigger working sets are in front so our caller trims those first.
        ...
        // If more than 25% of the available pages were recycled standby pages, then trim more aggressively in an attempt to get more of the cold pages into standby for the next pass.
        ...
        // No need to lock synchronize the MiReplacing clearing as it gets set every time a page replacement happens anyway.
        MiReplacing = FALSE;
        ...
        // Don't trim but do age unused pages and estimate the amount available in working sets.
        OutFlags = MI_AGE_ALL_WORKING_SETS;
    } else {
        // There is an overwhelming surplus of memory and this is a big server then don't even bother aging at this point (note the claim and estimated available are not cleared so they may contain stale values, but at this level it doesn't really matter).
        OutFlags = 0;
    }
...
LOGICAL MiCheckSystemTrimEndCriteria(IN PMMWS_TRIM_CRITERIA Criteria, IN KIRQL OldIrql)
/*
Routine Description:
     Check the ending criteria.
     If we're not done, delay for a little bit to let the modified writes catch up.
Arguments:
    Criteria - Supplies the trim criteria information.
    OldIrql - Supplies the old IRQL to lower to if the expansion lock needs to be released.
Return Value:
...
*/
{
    ...
    if ((MmAvailablePages > Criteria->DesiredFreeGoal) || (Criteria->NumPasses >= MI_MAX_TRIM_PASSES)) {
        return TRUE;// We have enough pages or we trimmed as many as we're going to get.
    }
    ...
    // We don't have enough pages - give the modified page writer 10 milliseconds to catch up.
    // The wait is also important because a thread may have the system cache locked but has been preempted by the balance set manager due to its higher priority.
    // We must give this thread a shot at running so it can release the system cache lock (all the trimmable pages may reside in the system cache).
    ...
    if (MmAvailablePages > Criteria->DesiredFreeGoal) {
        FinishedTrimming = TRUE;// Now we have enough pages so break out.
    } else {
        // We don't have enough pages so let's do another pass.
        // Go get the next working set list which is probably the one we put back before we gave up the processor.
...
/*
Routine Description:
    Walk through the sets on the working set expansion list, taking action as specified by the argument request.
Arguments:
    WorkingSetRequestFlags - Supplies the working set request flags to apply to all the working sets.
...
*/
{
    ...
    while (!IsListEmpty(&MmWorkingSetExpansionHead.ListHead)) {
        ...
        // Note that other routines that set this bit must remove the entry from the expansion list first.
        ASSERT(VmSupport->WorkingSetExpansionLinks.Flink != MM_WS_TRIMMING);
        ...
        if (VmSupport->LastTrimTime.QuadPart == CurrentTime.QuadPart) {
            ...
            if ((WorkingSetRequestFlags & MI_TRIM_ALL_WORKING_SETS) && (MiCheckSystemTrimEndCriteria(TrimCriteria, OldIrql)) == FALSE) {
                KeQuerySystemTime(&CurrentTime);// Start a new round of trimming.
                continue;
            }

            // No more pages are needed.
            // If no new requests have been queued, then we're done.
            ...
            if (WorkingSetRequestFlagsDone != MiWorkingSetRequestFlags) {
                // Some other thread has made an additional request(s) so pick up the new bits and process them now before returning.
                WorkingSetRequestFlags = (WorkingSetRequestFlagsDone ^ MiWorkingSetRequestFlags);
            ...
            if (MiWorkingSetRequestFlags != 0) {
                // Clear all the flags and wake any waiters since every request has been processed.
                MiWorkingSetRequestFlags = 0;
        ...
        // Only attach if the working set is worth examining.
        // This is not just an optimization, as care must be taken not to attempt an attach to a process which is a candidate for being currently (or already) swapped out because if we attach to a page directory that is in transition it's all over.
        if ((VmSupport->WorkingSetSize <= MM_PROCESS_COMMIT_CHARGE) && (VmSupport != &MmSystemCacheWs) && (VmSupport->Flags.SessionSpace == 0)) {
        ...
        if (MiAttachAndLockWorkingSet(VmSupport) == TRUE) {
            ...
            if (WorkingSetRequestFlags & MI_TRIM_ALL_WORKING_SETS) {
                ...
                Trim = MiDetermineTrimAmount(TrimCriteria, VmSupport);// Determine how many pages to trim from this working set.

                // If there's something to trim...
                if ((Trim != 0) && ((TrimCriteria->TrimAllPasses > TrimCriteria->NumPasses) || (MmAvailablePages < TrimCriteria->DesiredFreeGoal))) {
                    // We haven't reached our goal, so trim now.
                    Trim = MiTrimWorkingSet(Trim, VmSupport, TrimCriteria->TrimAge);
                }

                // Estimating the current claim is always done here by taking a sample of the working set.
                // Aging is only done if the trim pass warrants it (ie: the first pass only).
                MiAgeWorkingSet(VmSupport, TrimCriteria->DoAging, NULL, &TrimCriteria->NewTotalClaim, &TrimCriteria->NewTotalEstimatedAvailable);
            } else if (WorkingSetRequestFlags & MI_AGE_ALL_WORKING_SETS) {
                MiAgeWorkingSet(VmSupport, TRUE, &WslesScanned, &TrimCriteria->NewTotalClaim, &TrimCriteria->NewTotalEstimatedAvailable);
            }

            if (WorkingSetRequestFlags & MI_CAPTURE_AND_RESET_ALL_ACCESS_BITS) {
                MiCaptureAndResetWorkingSetAccessBits(VmSupport, WorkingSetRequestFlags);
            } else if (WorkingSetRequestFlags & MI_CAPTURE_ALL_ACCESS_BITS) {
                // This mode could be handled by acquiring the working set pushlock shared instead of exclusive.
                // Note that if this is implemented, only do it if this is the only flag bit set (as the other bits require exclusive).
                MiCaptureAndResetWorkingSetAccessBits(VmSupport, WorkingSetRequestFlags);
            }
            ...
            // Unable to attach to the working set presumably because some other thread has it locked.
            // Set the ForceTrim flag so it will be trimmed later by whoever owns it (or whoever tries to insert the next entry).
            LOCK_EXPANSION(OldIrql);
        ...
        if (VmSupport->WorkingSetExpansionLinks.Blink == NULL) {
            ...
            // The process is terminating - the value in the blink is the address of an event to set.
...
WSLE_NUMBER MiDetermineTrimAmount(PMMWS_TRIM_CRITERIA Criteria, PMMSUPPORT VmSupport)
/*
Routine Description:
...
*/
{
    ...
    if (OutswapEnabled == FALSE) {
        // Don't trim the cache or non-swapped sessions or processes below their minimum.
        MaxTrim -= VmSupport->MinimumWorkingSetSize;
    }

    switch (Criteria->NumPasses) {
    case 0:
        Trim = VmSupport->Claim >> ((VmSupport->Flags.MemoryPriority == MEMORY_PRIORITY_FOREGROUND) ? MI_FOREGROUND_CLAIM_AVAILABLE_SHIFT : MI_BACKGROUND_CLAIM_AVAILABLE_SHIFT);
    ...
    if ((MmDebug & MM_DBG_WS_EXPANSION) && (Trim != 0)) {
        if (VmSupport->Flags.SessionSpace == 0) {
            ProcessToTrim = CONTAINING_RECORD(VmSupport, EPROCESS, Vm);
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_TRACE_LEVEL, "           Trimming        Process %16s, WS %6d, Trimming %5d ==> %5d\n",
                       ProcessToTrim ? ProcessToTrim->ImageFileName : (PUCHAR)"System Cache", VmSupport->WorkingSetSize, Trim, VmSupport->WorkingSetSize - Trim);
        } else {
            SessionSpace = CONTAINING_RECORD(VmSupport, MM_SESSION_SPACE, Vm);
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_TRACE_LEVEL, "           Trimming        Session 0x%x (id %d), WS %6d, Trimming %5d ==> %5d\n",
                       SessionSpace, SessionSpace->SessionId, VmSupport->WorkingSetSize, Trim, VmSupport->WorkingSetSize - Trim);
        }
    }
#endif
...
/*
Routine Description:
    This routine is called to reset the accessed bits on all the working set entries in the specified working set.
Arguments:
...
Environment:
    Kernel mode.
    The working set pushlock is held exclusive.
*/
{
        ...
        if (Wsle->u1.e1.Valid) {
            ...
            if (MI_GET_ACCESSED_IN_PTE(PointerPte) == 1) {
                ...
                if (Flags & MI_CAPTURE_AND_RESET_ALL_ACCESS_BITS) {
                    // Clear the accessed bit and the WSLE age so that new references can be easily tracked.
...
/*
Routine Description:
    Age pages (clear the access bit or if the page hasn't been accessed, increment the age) for a portion of the working set.
    Also, walk through a sample of the working set building a set of counts of how old the pages are.
    The counts are used to create a claim of the amount the system can steal from this process if memory becomes tight.
Arguments:
    VmSupport - Supplies the VM support structure to age and estimate.
    DoAging - TRUE if pages are to be aged.  Regardless, the pages will be added to the availability estimation.
    WslesScanned - Total numbers of WSLEs scanned on this sweep, used as a control to prevent excessive aging on large systems with many processes.
    TotalClaim - Supplies a pointer to system wide claim to update.
    TotalEstimatedAvailable - Supplies a pointer to system wide estimate to update.
Environment:
    Kernel mode, APCs disabled, working set mutex.
    PFN lock NOT held.
*/
{
    ...
    if (DoAging == TRUE) {
        // Clear the used bits or increment the age of a portion of the working set.
        // Try to walk the entire working set every 2^MI_AGE_AGING_SHIFT seconds.
        if (VmSupport->WorkingSetSize > WorkingSetList->FirstDynamic) {
            ...
            // Bigger machines can easily have working sets that span terabytes so limit the absolute walk.
            if (NumberToExamine > MI_MAXIMUM_SAMPLE) {
                NumberToExamine = MI_MAXIMUM_SAMPLE;
            }

            // In addition to large working sets, bigger machines may also have huge numbers of processes - checking the aggregate number of working set list entries scanned prevents this situation from triggering excessive scanning.
    ...
    // The working set may have shrunk or the non-paged portion may have grown since the last time.
    // Put the next counter at the FirstDynamic if so.
    CurrentEntry = VmSupport->NextEstimationSlot;
    if (CurrentEntry > LastEntry || CurrentEntry < FirstDynamic) {
        CurrentEntry = FirstDynamic;
    }

    // When aging, walk the entire working set every 2^MiEstimationShift seconds.
    CounterShift = 0;
    SampleSize = 0;
    if (VmSupport->WorkingSetSize > WorkingSetList->FirstDynamic) {
        ...
        // Bigger machines may have huge numbers of processes - checking the aggregate number of working set list entries scanned prevents this situation from triggering excessive scanning.
        if ((WslesScanned != NULL) && (*WslesScanned >= MiMaximumWslesPerSweep)) {
            ...
            // Calculate the necessary counter shift to estimate pages in use.
    ...
    *TotalClaim += Claim >> ((VmSupport->Flags.MemoryPriority == MEMORY_PRIORITY_FOREGROUND) ? MI_FOREGROUND_CLAIM_AVAILABLE_SHIFT : MI_BACKGROUND_CLAIM_AVAILABLE_SHIFT);
...
/*
Routine Description:
    Adjust the rate at which we walk through working sets.
    If we have enough pages (we aren't trimming pages that aren't considered young), then we check to see whether we should decrease the aging rate and vice versa.
    The limits for the aging rate are 1/8 and 1/128 of the working sets.
    This means that the finest age granularities are 8 to 128 seconds in these cases.
    With the current 2 bit counter, at the low end we would start trimming pages > 16 seconds old and at the high end > 4 minutes.
Arguments:
...
*/
{
    ...
    if (EnoughPages == TRUE && ((MmTotalClaim + MmAvailablePages) > MiClaimAdjustmentThreshold[MiAgingShift])) {
        // Don't adjust the rate too frequently, don't go over the limit, and make sure there are enough claimed and/or available.
        if (((CurrentTime.QuadPart - MiLastAdjustmentOfClaimParams.QuadPart) > MmClaimParameterAdjustUpTime.QuadPart) && (MiAgingShift < MI_MAXIMUM_AGING_SHIFT)) {
        ...
        if ((CurrentTime.QuadPart - MiLastAdjustmentOfClaimParams.QuadPart) > MmClaimParameterAdjustDownTime.QuadPart) {
            // Always set the time so we don't adjust up too soon after a 2nd pass trim.
            MiLastAdjustmentOfClaimParams.QuadPart = CurrentTime.QuadPart;
...
/*
Routine Description:
    This function arranges the working set list into different groups based upon the claim.
    This is done so the working set trimming will take place on fat processes first.
...
*/
{
    ...
    while (!IsListEmpty(&MmWorkingSetExpansionHead.ListHead)) {
        ...
        if (VmSupport->Flags.SessionSpace == 1) {
            ...
            if (SessionGlobal->ResidentProcessCount == 0) {
                ...
                if (MmDebug & MM_DBG_SESSIONS) {
                    DbgPrintEx(DPFLTR_MM_ID, DPFLTR_TRACE_LEVEL, "Mm: Session %d heavily trim/aged - all its processes (%d) swapped out %d seconds ago\n",
                               SessionGlobal->SessionId, SessionGlobal->ReferenceCount, (ULONG)(SessionIdleTime.QuadPart / 10000000));
                }
#endif
        ...
        if (VmSupport->Flags.MemoryPriority == MEMORY_PRIORITY_FOREGROUND) {
            // Put the foreground processes at the end of the list, to give them priority.
            Size = 6;
        } else {
...
#if DBG
        if (MmDebug & MM_DBG_WS_EXPANSION) {
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_TRACE_LEVEL, "MM-rearrange: TrimHard = %d, WS Size = 0x%x, Claim 0x%x, Bucket %d\n", VmSupport->Flags.TrimHard, VmSupport->WorkingSetSize, VmSupport->Claim, Size);
        }
#endif //DBG

        // Note: this reverses the bucket order each time we reorganize the lists.
        // This may be good or bad - if you change it you may want to think about it.
    ...
    // Put the head of first non-empty list at the beginning of the MmWorkingSetExpansion list.
...
VOID MiQueueWorkingSetRequest(IN ULONG RequestFlags)
/*
Routine Description:
...
Return Value:
    None.
    On return, the request has been processed.
Environment:
    Kernel mode.
    No locks held.
    APC level or below.
*/
{
    ...
    // For session working sets, we cannot attach directly to the session space to be trimmed because it would result in session space references by other threads in this process to the attached session instead of the (currently) correct one.
    // In fact, we cannot even queue this to a worker thread because the working set manager (who shares the same page directory) may be attaching or detaching from a session (any session).
    // So this must be queued to the working set manager.
    LOCK_EXPANSION(OldIrql);
    if (MiWorkingSetRequestFlags == 0) {
    ...
    KeSetEvent(&MmWorkingSetManagerEvent, 0, FALSE);
    KeWaitForSingleObject(&MiWorkingSetRequestEvent, WrVirtualMemory, KernelMode, FALSE, NULL);
}
...
VOID MmCaptureAllWorkingSetAccessBits(IN LOGICAL ResetAccessed)
/*
Routine Description:
...
// This is deliberately initialized to 1 and only cleared when we have initialized enough of the system working set to support a trim.
LONG MiTrimInProgressCount = 1;
ULONG MiTrimAllPageFaultCount;


LOGICAL MmTrimAllSystemPageableMemory(__in LOGICAL PurgeTransition)
/*
Routine Description:
    This routine unmaps all pageable system memory.
    This does not unmap user memory or locked down kernel memory.
    Thus, the memory being unmapped resides in paged pool, pageable kernel/driver code & data, special pool and the system cache.
    Note that pages with a reference count greater than 1 are skipped (ie: they remain valid, as they are assumed to be locked down).
    This prevents us from unmapping all of the system cache entries, etc.
    Non-locked down kernel stacks must be outpaged by modifying the balance set manager to operate in conjunction with a support routine.
    This is not done here.
Arguments:
    PurgeTransition - Supplies whether to purge all the clean pages from the transition list.
Return Value:
...
*/
{
    ...
    // Just return if it's too early during system initialization or if another thread/processor is racing here to do the work for us.
    ...
    // thread) because this priority is not boosted - so a preemption that occurs after a WS mutex is acquired can result in the thread never running again and then all the other threads will be denied the mutex.
    ...
    // 1.  APCs must be disabled while any resources are held to prevent suspend APCs from deadlocking the system.
    // 2.  Once the working set has been marked MM_WS_TRIMMING, either the thread must not be preempted or the working set mutex must be held throughout.
    //     Otherwise a high priority thread can fault on a system code and data address and the two pages will thrash forever (at high priority) because no system working set expansion is allowed while TRIMMING is set.
