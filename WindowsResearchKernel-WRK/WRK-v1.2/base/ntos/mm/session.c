...
// Note that actually the SUM of the two maximums below is currently MM_ALLOCATION_GRANULARITY / PAGE_SIZE.
...
VOID MiSessionDeletePde(IN PMMPTE Pde, IN PMMPTE SelfMapPde);
...
/*
Routine Description:
    Mark the argument process as having the ability to create or delete session spaces.  This is only granted to the session manager process.
Arguments:
...
VOID MmSessionSetUnloadAddress(IN PDRIVER_OBJECT DriverObject)
/*
Routine Description:
...
*/
{
    ...
    // Once the Session pointer in the EPROCESS is set it can never be cleared because it is accessed lock-free.
...
Routine Description:
    ...
    This may trigger a substantial round of dereferencing and resource freeing if it is also the last process in the session, (holding the last image in the group, etc).
Environment:
...
*/
{
    ...
    if (Process->Vm.Flags.SessionLeader == 1) {
        // smss may transiently have a session space but that's of no interest to our caller.
        return PsDefaultThreadLocaleId;
    }

    // The Session field of the EPROCESS is never cleared once set so these checks can be done lock free.
    SessionGlobal = (PMM_SESSION_SPACE)Process->Session;
    if (SessionGlobal == NULL) {// The system process has no session space.
        return PsDefaultThreadLocaleId;
    }
...
*/
{
    ...
    // If this ever grows beyond the maximum size, the session virtual address space layout will need to be enlarged.
...
#error "no target architecture"
    ...
    if (MiSessionIdBitmap != NULL) {
        ...
        KeBugCheckEx(INSTALL_MORE_MEMORY, MmNumberOfPhysicalPages, MmLowestPhysicalPage, MmHighestPhysicalPage, 0x200);
    }
}


ULONG MiSessionPoolSmallLists(VOID)
{
    return SESSION_POOL_SMALL_LISTS;
}


PGENERAL_LOOKASIDE MiSessionPoolLookaside(VOID)
{
...
PVOID MiSessionPoolTrackTable(VOID)
{
    ASSERT(KeGetCurrentIrql() == PASSIVE_LEVEL);
...
SIZE_T MiSessionPoolTrackTableSize(VOID)
{
    SIZE_T i;
...
PVOID MiSessionPoolBigPageTable(VOID)
{
    ASSERT(KeGetCurrentIrql() == PASSIVE_LEVEL);
...
SIZE_T MiSessionPoolBigPageTableSize(VOID)
{
    SIZE_T i;
    SIZE_T NumberOfBytes;
    SIZE_T NumberOfEntries;
...
Routine Description:
    ...
    In NT32, this initial mapping for the portion of session space mapped by the first PDE will automatically be inherited by all child processes when the system copies the system portion of the page directory for new address spaces.
...
*/
{
    ...
    // Initializing these are not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    PageTablePage = 0;
    PointerPte = NULL;
    ...
    while (TRUE) {
        ...
        // The structure changed beneath us.  Use the return value from the exchange and try it all again.
    ...
    *SessionId = RtlFindClearBitsAndSet(MiSessionIdBitmap, 1, 0);
    if (*SessionId == NO_BITS_FOUND) {
        MiCreateBitMap(&NewBitmap, MiSessionIdBitmap->SizeOfBitMap + MI_SESSION_ID_INCREMENT, PagedPool);
        if (NewBitmap == NULL) {
        ...
        RtlCopyMemory(NewBitmap->Buffer, MiSessionIdBitmap->Buffer, ((MiSessionIdBitmap->SizeOfBitMap + 31) / 32) * sizeof(ULONG));
    ...
    // Note this is done prior to the commitment charges just to simplify error handling.
    ...
    GotPages = TRUE;
    MI_DECREMENT_RESIDENT_AVAILABLE(ResidentPages + MI_SESSION_SPACE_WORKING_SET_MINIMUM, MM_RESAVAIL_ALLOCATE_CREATE_SESSION);

    // Allocate both session space data pages first as on some architectures a region ID will be used immediately for the TB references as the PTE mappings are initialized.
    ...
    for (i = 0; i < MiSessionDataPages; i += 1) {
        ...
        // Map the data pages immediately in global space.
        // Some architectures use a region ID which is used immediately for the TB references after the PTE mappings are initialized.

        // The global bit can be left on for the global mappings (unlike the session space mapping which must have the global bit off since
        // we need to make sure the TB entry is flushed when we switch to a process in a different session space).
    ...
    // Do not reference the top level parent page as it belongs to the current process (SMSS).
...
#if defined (_WIN64)
    // IA64 can reference the top level parent page here because a unique one is allocated per process.
    // AMD64 is also ok because there is another hierarchy level above.
    ...
    // This page frame references itself instead of the current (SMSS.EXE) page directory as its PteFrame.
    // This allows the current process to appear more normal (at least on 32-bit NT).
    // It just means we have to treat this page specially during teardown.
    MiInitializePfnForOtherProcess(PageTablePage, PointerPde, PageTablePage);
#endif

    // This page is never paged, ensure that its WsIndex stays clear so the release of the page is handled correctly.
    ...
    // Allocate pages for session pool tag information and large session pool allocation tracking.
    ...
    // Now that the data page is mapped, it can be freed as full hierarchy teardown in the event of any future failures encountered by this routine.
    UNLOCK_PFN(OldIrql);
    ...
    // Load the session data page table entry so that other processes can fault in the mapping.
    SessionSpace->PageTables[PointerPde - MiGetPdeAddress(MmSessionBase)] = *PointerPde;
#endif

    // This list entry is only referenced while within the session space and has session space (not global) addresses.
    ...
    // Initialize the view mapping support - note this must happen after initializing session pool.
    if (MiInitializeSystemSpaceMap(&SessionGlobal->Session) == FALSE) {
        goto Failure;
    }
    MmUnlockPageableImageSection(ExPageLockHandle);

    // Use the global virtual address rather than the session space virtual address to set up fields that need to be globally accessible.
...
Failure:
    ...
    if (GotPages == TRUE) {
        ...
        // Do not call MiFreeSessionSpaceMap () as the maps cannot have been initialized if we are in this path.

        // Free the initial page table page that was allocated for the paged pool range (if it has been allocated at this point).
        MiFreeSessionPoolBitMaps();

        // Capture all needed session information now as after sharing is disabled below, no references to session space can be made.
        ...
        UNLOCK_PFN(OldIrql);
        MI_INCREMENT_RESIDENT_AVAILABLE(ResidentPages + MI_SESSION_SPACE_WORKING_SET_MINIMUM, MM_RESAVAIL_FREE_CREATE_SESSION);
    }

    if (GlobalMappingPte != NULL) {
        MiReleaseSystemPtes(GlobalMappingPte, MiSessionDataPages, SystemPteSpace);
    }
...
*/
{
    ...
    // No need to go through all this if it does.
    // Creation races are caught below and recovered from regardless.
    if (CurrentProcess->Flags & PS_PROCESS_FLAGS_IN_SESSION) {
        return STATUS_ALREADY_COMMITTED;
    }

    if (CurrentProcess->Vm.Flags.SessionLeader == 0) {
        // Only the session manager can create a session.  Make the current process the session leader if this is the first session creation ever.
    ...
    // Initialize the session paged pool lookaside lists.
    // Note this cannot be done until both the session pool and working set lists are fully initialized because on IA64,
    // the lookaside list initialization does a pool allocation which will trigger a demand zero fault in the session pool.

    // THIS IS THE FIRST POOL ALLOCATION IN THIS SESSION SPACE.
    SessionGlobal = SESSION_GLOBAL(MmSessionSpace);

    for (i = 0; i < SESSION_POOL_SMALL_LISTS; i += 1) {
        ExInitializePagedLookasideList((PPAGED_LOOKASIDE_LIST)&SessionGlobal->Lookaside[i], NULL, NULL, PagedPoolSession, (i + 1) * sizeof(POOL_BLOCK), 'looP', 256);
    }
...
*/
{
    ...
    // See if the calling process has a session space - this must be checked since we can be called via a system service.
    if ((CurrentProcess->Flags & PS_PROCESS_FLAGS_IN_SESSION) == 0) {
#if DBG
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_WARNING_LEVEL, "MmSessionDelete: Process %p not in a session\n", CurrentProcess);
        DbgBreakPoint();
#endif
    ...
    if (CurrentProcess->Vm.Flags.SessionLeader == 0) {
        // Only the session manager can delete a session.
        // This is because it affects the virtual mappings for all threads within the process when this address space is deleted.
        // This is different from normal VAD clearing because win32k and other drivers rely on this space.
        return STATUS_UNABLE_TO_FREE_VM;
    }
...
#if DBG
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_WARNING_LEVEL, "MmSessionDelete: Wrong SessionId! Own %d, Ask %d\n", MmSessionSpace->SessionId, SessionId);
        DbgBreakPoint();
#endif
        return STATUS_UNABLE_TO_FREE_VM;
    }
...
*/
{
    ...
    ASSERT(RtlCompareMemoryUlong(PointerPde, MiSessionSpacePageTables * sizeof(MMPTE), 0) == MiSessionSpacePageTables * sizeof(MMPTE));
...
Environment:
    Kernel mode.
    No locks held.
    Current process must not have a session space to return to - ie: this should be the system process.
*/
{
    PMMPTE PointerPde;
...
/*
Routine Description:
    Decrement this process' session reference.
    The session itself may have already been deleted.
    If this is the last reference to the session, then the session data page and its mapping PTE (if any) will be destroyed upon return.
Arguments:
    SessionGlobal - Supplies the global session space pointer being dereferenced.
                    The caller has already verified that this process is a member of the target session.
Environment:
...
*/
{
    ...
    // Free the datapages & self-map PTE now since this is the last process reference and KeDetach has returned.
#if (_MI_PAGING_LEVELS < 3)
    ...
    MI_INCREMENT_RESIDENT_AVAILABLE(MiSessionDataPages, MM_RESAVAIL_FREE_DEREFERENCE_SESSION);
    InterlockedDecrement(&MmSessionDataPages);

    // Return commitment for the datapages.
    MiReturnCommitment(MiSessionDataPages);
    MM_TRACK_COMMIT(MM_DBG_COMMIT_RETURN_SESSION_DATAPAGE, MiSessionDataPages);
...
*/
{
    ...
    if (InterlockedDecrement((PLONG)&MmSessionSpace->ReferenceCount) != 0) {
        ...
        // Don't delete any non-smss session space mappings here.
        // Let them live on through process death.
        // This handles the case where MmDispatchWin32Callout picks csrss - csrss has exited as it's not the last process (smss is).
        // smss is simultaneously detaching from the session and since it is the last process, it's waiting on the AttachCount below.
        // The dispatch callout ends up in csrss but has no way to synchronize against csrss exiting through this path as the object reference count doesn't stop it.
        // So leave the session space mappings alive so the callout can execute through the remains of csrss.

        // Note that when smss detaches, the address space must get cleared here so that subsequent session creations by smss will succeed.
        if (Process->Vm.Flags.SessionLeader == 1) {
            SessionGlobal = SESSION_GLOBAL(MmSessionSpace);
            ...
            // This process' reference to the session must be NULL as the KeDetach has completed so no swap context referencing the earlier session page can occur from here on.
            // This is also needed because during clean shutdowns, the final dereference
            // of this process (smss) object will trigger an MmDeleteProcessAddressSpace - this routine will dereference the (no-longer existing) session space if this bit is not cleared properly.
            ASSERT(Process->Session == NULL);

            // Another process may have won the race and exited the session as this process is executing here.
            // Hence the reference count is carefully checked here to ensure no leaks occur.
    ...
    // This is the final process in the session so the entire session must be dereferenced now.
...
*/
{
    ...
    // If an unload function has been registered for WIN32K.SYS, call it now before we force an unload on any modules.
    // WIN32K.SYS is responsible for calling any other loaded modules that have unload routines to be run.
    // Another option is to have the other session drivers register a DLL initialize/uninitialize pair on load.
    if (MmSessionSpace->Win32KDriverUnload) {
        // Note win32k does not reference the argument driver object so just pass in NULL.
    ...
    // This allows us to force an unload of any kernel images loaded by the session so we do not have any virtual space and paging file leaks.
    ...
    // Reference the session space structure using its global kernel PTE based address.
    // This is to avoid deleting it out from underneath ourselves.
...
#if (_MI_PAGING_LEVELS >= 3)
            // The PXE and PPE must be valid because all of session space is contained within a single PPE.
            // However, each PDE must be checked for validity.
    ...
    // Account for the session data structure data pages in our tracking structures.
    // The actual data pages and their commitment can only be returned after the last process has been reaped (not just exited).
...
#if (_MI_PAGING_LEVELS >= 3)
    // For NT64, the page directory page and its parent are explicitly accounted for here because only page table pages are checked in the loop after this.
    ...
    // Note the common structure (not the local PDEs) must be examined as any page tables
    // that were dynamically materialized in the context of a different process may not be in the current process' page directory 
    // (ie: the current process has never accessed the materialized VAs) !
#if (_MI_PAGING_LEVELS >= 3)
    ...
    // Note that whenever win32k or drivers loaded by it leak pool, the ASSERT below will be triggered.
    ...
    if (MmSessionSpace->u.Flags.Initialized == 1) {
        ...
        while (PointerPte < EndPte) {
            if (PointerPte->u.Long) {
                ...
                // Don't return the resident available pages charge here as it's going to be returned in one chunk below as part of CountReleased.
                UNLOCK_PFN(OldIrql);
    ...
    // Cut off the pagetable reference as the local page table is going to be freed now.
    // Any needed references must go through the global PTE space or superpages but never through the local session VA.
    // The actual session data pages are not freed until the very last process of the session receives its very last object dereference.
    ...
    for (Index = 0; Index < MiSessionSpacePageTables; Index += 1, PointerPde += 1) {
        ...
        // all the page session space page tables use the session data page's page table as the containing frame (to avoid referencing a top level user page directory page in smss).
        // However, the MiIdentifyPfn logging code gets confused if we delete a page that has an already-freed containing frame, hence the ordering.
    ...
    // The page directory still has a share count with the page directory parent so decrement that now so the parent can be freed.
    MiDecrementShareCount(Pfn1, PageParentFrame);
#else
    StartPde = MiGetPdeAddress((PVOID)MmSessionBase);
    ...
    // Return resident available for the pool tagging & tracking tables as well as page table pages and working set structure pages.
    MI_INCREMENT_RESIDENT_AVAILABLE(CountReleased, MM_RESAVAIL_FREE_DEREFERENCE_SESSION_PAGES);
    MI_INCREMENT_RESIDENT_AVAILABLE(MI_SESSION_SPACE_WORKING_SET_MINIMUM, MM_RESAVAIL_FREE_DEREFERENCE_SESSION_WS);
    ...
    // The session leader must release the reference here since the session leader never exits and thus doesn't trigger this via process delete.
    if (Process->Vm.Flags.SessionLeader == 1) {
        ASSERT(Process->Session == NULL);
        MiReleaseProcessReferenceToSessionDataPage(SessionGlobal);
    }
...
*/
{
    ...
    while (StartPde <= EndPde) {
...
#endif
        {
            ...
            if (Waited != 0) {
                // The session space working set mutex & PFN lock were released and reacquired so recheck the master page directory as another thread may have filled this entry in the interim.
                UNLOCK_PFN(OldIrql);
                continue;
            }
            ...
            WorkingSetIndex = MiAddValidPageToWorkingSet(SessionPte, StartPde, Pfn1, 0);
            if (WorkingSetIndex == 0) {
                // A working set entry could not be allocated.
                // Just delete the page table we just allocated as no one else could be using it (as we have held the session's working set mutex since initializing the PTE) and return a failure.
                ASSERT(Pfn1->u3.e1.PrototypePte == 0);
                LOCK_PFN(OldIrql);
                MI_SET_PFN_DELETED(Pfn1);
                UNLOCK_PFN(OldIrql);
                MiTrimPte(SessionPte, StartPde, Pfn1, HYDRA_PROCESS, ZeroPte);
            ...
            ActualPages += 1;
            ASSERT(WorkingSetIndex == MiLocateWsle(SessionPte, WorkingSetList, Pfn1->u1.WsIndex, FALSE));
            if (WorkingSetIndex >= WorkingSetList->FirstDynamic) {
                ...
                if (WorkingSetIndex != WorkingSetList->FirstDynamic) {
                    ...
                    MiSwapWslEntries(WorkingSetIndex, SwapEntry, &MmSessionSpace->Vm, FALSE);
                }
    ...
    if (ActualPages < SizeInPages) {
        ...
        MI_INCREMENT_RESIDENT_AVAILABLE(SizeInPages - ActualPages, MM_RESAVAIL_FREE_SESSION_PAGE_TABLES_EXCESS);
    }
...
*/
{
    ...
    // If the process has already exited then it is no longer really a member of the session so don't count it in the outswap counts.
    ...
    if (InterlockedDecrement(&SessionGlobal->ResidentProcessCount) == 0) {
        ...
        if (MmDebug & MM_DBG_SESSIONS) {
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_TRACE_LEVEL, "Mm: Last process (%d total) just swapped out for session %d, %d pages\n",
                       SessionGlobal->ReferenceCount, SessionGlobal->SessionId, SessionGlobal->Vm.WorkingSetSize);
        }
        MiSessionInfo[0] += 1;
#endif
...
Arguments:
    ...
    Forced - Supplies TRUE if the process was brought in via KeForceAttach, FALSE otherwise.
Environment:
    Kernel mode.  This routine must not enter a wait state for memory resources or the system will deadlock.
*/
{
    ...
    // If the process has already exited then it is no longer really a member of the session so don't count it in the inswap counts.
    if (Process->Flags & PS_PROCESS_FLAGS_VM_DELETED) {
        return;
    }
    ...
    if (InterlockedIncrement(&SessionGlobal->ResidentProcessCount) == 1) {
        ...
        MiSessionInfo[2] += 1;
        if (MmDebug & MM_DBG_SESSIONS) {
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_TRACE_LEVEL, "Mm: First process (%d total) just swapped back in for session %d, %d pages\n",
                       SessionGlobal->ReferenceCount, SessionGlobal->SessionId, SessionGlobal->Vm.WorkingSetSize);
        }
#endif
...
ULONG MmGetSessionId(IN PEPROCESS Process)
/*
Routine Description:
...
Return Value:
    The session ID.  Note these are recycled when sessions exit, hence the caller must use proper object referencing on the specified process.
Environment:
...
*/
{
    ...
    if (Process->Vm.Flags.SessionLeader == 1) {
        return 0;// smss may transiently have a session space but that's of no interest to our caller.
    }

    // The Session field of the EPROCESS is never cleared once set so these checks can be done lock free.
    SessionGlobal = (PMM_SESSION_SPACE)Process->Session;
    if (SessionGlobal == NULL) {
        return 0;// The system process has no session space.
    }
...
ULONG MmGetSessionIdEx(IN PEPROCESS Process)
/*
Routine Description:
...
*/
{
    ...
    SessionGlobal = (PMM_SESSION_SPACE)Process->Session;
    if (SessionGlobal == NULL) {
        return (ULONG)-1;// The system process has no session space.
    }
...
PVOID MmGetNextSession(IN PVOID OpaqueSession)
/*
Routine Description:
    This function allows code to enumerate all the sessions in the system.
    The first session (if OpaqueSession is NULL) or subsequent session (if session is not NULL) is returned on each call.

    If OpaqueSession is not NULL then this session must have previously been obtained by a call to MmGetNextSession.

    Enumeration may be terminated early by calling MmQuitNextSession on the last non-NULL session returned by MmGetNextSession.
    ...
    for (OpaqueSession = MmGetNextSession (NULL); OpaqueSession != NULL; OpaqueSession = MmGetNextSession (OpaqueSession))
    {
         ...
         if (MmGetSessionId (OpaqueSession) == DesiredId) {
             ...
             if (NT_SUCCESS (MmAttachSession (OpaqueSession, &ApcState))) {
                // Session hasn't exited yet, so call interesting work functions that need session context ...
             ...
             // If the interesting work functions failed and error recovery (ie: walk back through all the sessions already operated on and try to undo the actions), then do this.
             // Note you must add similar checks to the above if the operations were only done to specifically requested session IDs.
             if (ErrorRecoveryNeeded) {
                 for (OpaqueSession = MmGetPreviousSession (OpaqueSession); OpaqueSession != NULL; OpaqueSession = MmGetPreviousSession (OpaqueSession)) {
                      // MmAttachSession/DetachSession as needed to obtain context, etc.
                 }
...
Arguments:
    OpaqueSession - Supplies the session to get the next session from or NULL for the first session.
Return Value:
...
*/
{
    ...
    if (EntryProcess == NULL) {
        ...
        // The Session field of the EPROCESS is never cleared once set so this field can be used lock free.
    ...
    while (NextEntry != &MiSessionWsList) {
        ...
        if ((Session->u.Flags.DeletePending == 0) && (NextProcessEntry != &Session->ProcessList)) {
            ...
            if (Process->Vm.Flags.SessionLeader == 1) {
                // If session manager is still the first process (ie: smss hasn't detached yet), then don't bother delivering to this session this early in its lifetime.
                // And since smss is serialized, it can't be creating another session yet so just bail now as we must be at the end of the list.
                break;
            }

            // If the process has finished rudimentary initialization, then select it as an attach can be performed safely.
            // If this first process has not finished initializing there can be no others in this session, so just march on to the next session.

            // Note the VmWorkingSetList is used instead of the
            // AddressSpaceInitialized field because the VmWorkingSetList is never cleared so we can never see an exiting process (whose AddressSpaceInitialized field gets zeroed) and incorrectly decide the list must be empty.
            if (Process->Vm.VmWorkingSetList != NULL) {
                // Reference any process in the session so that the session cannot be completely deleted once the expansion lock is released (note this does NOT prevent the session from being cleaned).
    ...
    // Regardless of whether a next session is returned, if a starting one was passed in, it must be dereferenced now.
    if (EntryProcess != NULL) {
...
PVOID MmGetPreviousSession(IN PVOID OpaqueSession)
/*
Routine Description:
    This function allows code to reverse-enumerate all the sessions in the system.
    This is typically used for error recovery - ie: to walk backwards undoing work done by MmGetNextSession semantics.
    The first session (if OpaqueSession is NULL) or subsequent session (if session is not NULL) is returned on each call.
    If OpaqueSession is not NULL then this session must have previously been obtained by a call to MmGetNextSession.
Arguments:
    OpaqueSession - Supplies the session to get the next session from or NULL for the first session.
...
NTSTATUS MmQuitNextSession(IN PVOID OpaqueSession)
/*
Routine Description:
    This function is used to prematurely terminate a session enumeration that began using MmGetNextSession.
Arguments:
    OpaqueSession - Supplies a non-NULL session previously obtained by a call to MmGetNextSession.
*/
{
    PEPROCESS EntryProcess;
...
NTSTATUS MmAttachSession(IN PVOID OpaqueSession, OUT PRKAPC_STATE ApcState)
/*
Routine Description:
    This function attaches the calling thread to a referenced session previously obtained via MmGetNextSession.
Arguments:
    OpaqueSession - Supplies a non-NULL session previously obtained by a call to MmGetNextSession.
    ApcState - Supplies APC state storage for the subsequent detach.
Return Value:
    NTSTATUS.  If successful then we are attached on return.  The caller is responsible for calling MmDetachSession when done.
*/
{
    KIRQL OldIrql;
    PEPROCESS EntryProcess;
    ...
    if ((CurrentProcess->Vm.Flags.SessionLeader == 0) && (CurrentSession != NULL)) {
        ...
        if (CurrentSession == EntrySession) {
            ...
            // Call KeStackAttach anyway (this has the overhead of an extra dispatcher lock acquire and release) so that callers can always use MmDetachSession to detach.
            // This is a very infrequent path so the extra lock acquire and release is not significant.

            // Note that by resetting EntryProcess below, an attach will not actually occur.
...
NTSTATUS MmDetachSession(IN PVOID OpaqueSession, IN PRKAPC_STATE ApcState)
/*
Routine Description:
    This function detaches the calling thread from the referenced session previously attached to via MmAttachSession.
Arguments:
    OpaqueSession - Supplies a non-NULL session previously obtained by a call to MmGetNextSession.
    ApcState - Supplies APC state storage information for the detach.
Return Value:
    NTSTATUS.  If successful then we are detached on return.  The caller is responsible for eventually calling MmQuitNextSession on return.
*/
{
    KIRQL OldIrql;
    PEPROCESS EntryProcess;
...
PVOID MmGetSessionById(IN ULONG SessionId)
/*
Routine Description:
    This function allows callers to obtain a reference to a specific session.
    The caller can then MmAttachSession, MmDetachSession & MmQuitNextSession to complete the proper sequence so reference counting and address context operate properly.
Arguments:
    SessionId - Supplies the session ID of the desired session.
Return Value:
    An opaque session token or NULL if the session cannot be found.
Environment:
    Kernel mode, the caller must guarantee the session cannot exit or the ID becomes meaningless as it can be reused.
*/
{
    KIRQL OldIrql;
    PLIST_ENTRY NextEntry;
    ...
    while (NextEntry != &MiSessionWsList) {
        ...
        if (Session->SessionId == SessionId) {
            ...
            Process = CONTAINING_RECORD(NextProcessEntry, EPROCESS, SessionProcessLinks);
            if (Process->Vm.Flags.SessionLeader == 1) {
                // Session manager is still the first process (ie: smss hasn't detached yet), don't bother delivering to this session this early in its lifetime.
                // And since smss is serialized, it can't be creating another session yet so just bail now as we must be at the end of the list.
                break;
            }
