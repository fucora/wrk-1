...
Abstract:
    This module contains the routines which implement mapping views of sections into the system-wide cache.
*/
...
Routine Description:
    ...
    The page protection is identical to that of the prototype PTE.
...
Arguments:
    ...
    BaseAddress - Supplies a pointer to a variable that will receive the base address of the view.
                  If the initial value of this argument is not null, then the view will be allocated starting at the specified virtual address rounded down to the next 64kb address boundary.
                  If the initial value of this argument is null, then the operating system will determine where to allocate the view using the information
                  specified by the ZeroBits argument value and the section allocation attributes (i.e. based and tiled).
    SectionOffset - Supplies the offset from the beginning of the section to the view in bytes. This value must be a multiple of 256k.
    ViewSize - Supplies a pointer to a variable that will receive the actual size in bytes of the view.
               The initial values of this argument specifies the size of the view in bytes and is rounded up to the next host page size boundary and must be less than or equal to 256k.
Environment:
...
*/
{
    ...
    // Assert the view size is less than 256k and the section offset is aligned on a 256k boundary.
    ASSERT(*CapturedViewSize <= X256K);
    ASSERT((SectionOffset->LowPart & (X256K - 1)) == 0);

    // Make sure the section is not an image section or a page file backed section.
    ...
    // An unoccupied address range has been found, put the PTEs in the range into prototype PTEs.
    if (ControlArea->FilePointer != NULL) {
        // Increment the view count for every subsection spanned by this view, creating prototype PTEs if needed.
        ...
        if (!NT_SUCCESS(Status)) {
            // Zero both the next and TB flush stamp PTEs before unmapping so the unmap won't hit entries it can't decode.
            ...
            // Decrement the number of mapped views for the segment and check to see if the segment should be deleted.
            ControlArea->NumberOfMappedViews -= 1;
            ControlArea->NumberOfSystemCacheViews -= 1;
    ...
    // Check to see if the TB needs to be flushed.
    // Note that due to natural TB traffic and the number of system cache views, this is an extremely rare operation.
    ...
    *CapturedBase = MiGetVirtualAddressMappedByPte(PointerPte);
    ...
    while (PointerPte < LastPte) {
        ...
        ASSERT(((ULONG_PTR)PointerPte & (MM_COLOR_MASK << PTE_SHIFT)) == (((ULONG_PTR)ProtoPte & (MM_COLOR_MASK << PTE_SHIFT))));
        PointerPte += 1;
        ProtoPte += 1;
    }
...
/*
Routine Description:
    This function maps a view in the current address space to the specified control area.
    The page protection is identical to that of the prototype PTE.
    This routine assumes the caller has called MiCheckPurgeAndUpMapCount, hence the PFN lock is not needed here.
Arguments:
    FirstPte - Supplies a pointer to the first PTE of the current address space to initialize.
...
Routine Description:
    ...
    NOTE: When this function is called, no pages may be locked in the cache for the specified view.
Arguments:
    BaseAddress - Supplies the base address of the section in the system cache.
    SectionToUnmap - Supplies a pointer to the section which the base address maps.
    AddToFront - Supplies TRUE if the unmapped pages should be added to the front of the standby list (i.e., their value in the cache is low).  FALSE otherwise.
Environment:
    Kernel mode.
*/
{
    ...
    ASSERT((ControlArea->u.Flags.Image == 0) && (ControlArea->u.Flags.PhysicalMemory == 0) && (ControlArea->u.Flags.GlobalOnlyPerSession == 0));
    ...
    do {
        // The cache is organized in chunks of 256k bytes, clear the first chunk then check to see if this is the last chunk.

        // The page table page is always resident for the system cache.
        // Check each PTE: it is in one of three states, either valid or prototype PTE format or zero.
        ...
        if (PteContents.u.Hard.Valid == 1) {
            ...
            // The subsections can only be decremented after all the PTEs have been cleared and PFN sharecounts decremented so no prototype PTEs will be valid if it is indeed the final subsection dereference.
            // This is critical so the dereference segment thread doesn't free pool containing valid prototype PTEs.
            if (FilePointer != NULL) {
            ...
            // The PFN sharecount will be decremented below when the PFN lock is held.
            ...
            if (FilePointer != NULL) {
                ...
                if (Subsection == NULL) {
                    // The prototype PTE may be paged out - but the contents must be accessed to reconstruct the subsection pointer.
                    // Acquire the PFN lock to prevent the prototype PTE from changing as we will potentially reference through it to an actual PFN, etc.
                    ...
                    if (PointerPde->u.Hard.Valid == 0) {
                        ...
                        // Ok to march on even though locks were released and reacquired.
                    }
    ...
    if ((Subsection != NULL) && (Subsection->ControlArea != ControlArea)) {
        KeBugCheckEx(MEMORY_MANAGEMENT, 0x782, (ULONG_PTR)&PteArray, (ULONG_PTR)BaseAddress, (ULONG_PTR)SectionToUnmap);
    }
    ...
    for (; j < i; j += 1) {
        ...
        MI_CAPTURE_DIRTY_BIT_TO_PFN(&PteArray[j], Pfn1);// Capture the state of the modified bit for this PTE.
    ...
    while (Subsection != NULL) {
        ...
        if ((ProtoPte >= Subsection->SubsectionBase) && (ProtoPte < Subsection->SubsectionBase + Subsection->PtesInSubsection)) {
            break;
        }

        Subsection = (PMSUBSECTION)Subsection->NextSubsection;

        // The only way the subsection could be NULL is if we overshoot the subsection list due to a incorrect ending prototype computation.
        // This should only occur is someone has corrupted the PTEs.
        if (Subsection == NULL) {
            KeBugCheckEx(MEMORY_MANAGEMENT, 0x783, (ULONG_PTR)&PteArray, (ULONG_PTR)BaseAddress, (ULONG_PTR)SectionToUnmap);
        }
...
VOID MiRemoveMappedPtes(IN PVOID BaseAddress, IN ULONG NumberOfPtes, IN PCONTROL_AREA ControlArea, IN PMMSUPPORT Ws)
/*
Routine Description:
    This function unmaps a view from the system or session view space.
    NOTE: When this function is called, no pages may be locked in the space for the specified view.
Arguments:
    BaseAddress - Supplies the base address of the section in the system or session view space.
...
*/
{
    ...
    while (NumberOfPtes) {
        // The page table page is always resident for the system space (and for a session space) map.
        // Check each PTE, it is in one of two states, either valid or prototype PTE format.
        PteContents = *PointerPte;
        if (PteContents.u.Hard.Valid == 1) {
            // Lock the working set to prevent races with the trimmer, then re-examine the PTE.
            if (!WsHeld) {
                WsHeld = TRUE;
                LOCK_WORKING_SET(CurrentThread, Ws);
                continue;
            ...
            if ((Pfn1->u3.e1.PrototypePte) && (Pfn1->OriginalPte.u.Soft.Prototype)) {
                if ((LastSubsection != NULL) && (Pfn1->PteAddress >= LastSubsection->SubsectionBase) && (Pfn1->PteAddress < LastSubsection->SubsectionBase + LastSubsection->PtesInSubsection)) {
                    NOTHING;
                } else {
                    ...
                    if (LastSubsection != MappedSubsection) {
                        ...
                        if ((ControlArea->FilePointer != NULL) && (ControlArea->u.Flags.Image == 0) && (ControlArea->u.Flags.PhysicalMemory == 0)) {
            ...
            // The PDE must be carefully checked against the master table because the PDEs are all zeroed in process creation.
            // If this process has never faulted on any address in this range (all references prior and above were filled directly by the TB as the PTEs are global on non-Hydra), then the PDE reference below to determine the page table frame will be zero.

            // Note this cannot happen on NT64 as no master table is used.
            if (PointerPde->u.Long == 0) {
                PMMPTE MasterPde;

                MasterPde = &MmSystemPagePtes[((ULONG_PTR)PointerPde & (PD_PER_SYSTEM * (sizeof(MMPTE) * PDE_PER_PAGE) - 1)) / sizeof(MMPTE)];
            ...
            if (PteContents.u.Soft.Prototype == 1) {
                ...
                // But make sure it's only done once per subsection in a given view.
                ProtoPte = MiPteToProto(&PteContents);
                if ((LastSubsection != NULL) && (ProtoPte >= LastSubsection->SubsectionBase) && (ProtoPte < LastSubsection->SubsectionBase + LastSubsection->PtesInSubsection)) {
                    NOTHING;
                } else {
                    if (WsHeld) {
                    ...
                    if (PointerPde->u.Hard.Valid == 0) {
                        ...
                        UNLOCK_PFN(OldIrql);
#if defined(_X86PAE_)
                        // PAE PTEs are written in 2 separate writes so the working set pushlock must always be held even to examine PTEs in prototype format - because a trim could be happening in parallel (writing the
                        // low half and then the upper half which would cause otherwise cause us to read the PTE contents split in the middle).
                        WsHeld = TRUE;
                        LOCK_WORKING_SET(CurrentThread, Ws);
#endif
                        continue;
    ...
    // Decrement the number of user references as the caller upped them via MiCheckPurgeAndUpMapCount when this was originally mapped.
...
/*
Routine Description:
    This routine initializes the system cache working set and data management structures.
Arguments:
    MinimumWorkingSet - Supplies the minimum working set for the system cache.
    MaximumWorkingSet - Supplies the maximum working set size for the system cache.
Environment:
...
*/
{
    ...
    // Don't use entry 0 as an index of zero in the PFN database means that the page can be assigned to a slot.
    // This is not a problem for process working sets as page 0 is private.
...
#if defined(_X86_)
    // For 32-bit machines, the user and system space is shared in a single 4gb region, so the maximum system working set is 4gb minus the user.
    MaximumSystemWorkingSet = 0x100000 - MM_MAXIMUM_WORKING_SET;
#else
    MaximumSystemWorkingSet = MM_MAXIMUM_WORKING_SET;
#endif

    MmSystemCacheWorkingSetList->HashTableStart = (PVOID)((PCHAR)PAGE_ALIGN(&MmSystemCacheWorkingSetList->Wsle[MaximumSystemWorkingSet]) + PAGE_SIZE);
    ...
    SizeOfSystemCacheInPages = MI_COMPUTE_PAGES_SPANNED(MmSystemCacheStart, (PCHAR)MmSystemCacheEnd - (PCHAR)MmSystemCacheStart + 1);
    ...
    if (MiSystemCacheEndExtra != MmSystemCacheEnd) {
        SizeOfSystemCacheInPages = ADDRESS_AND_SIZE_TO_SPAN_PAGES(MiSystemCacheStartExtra, (PCHAR)MiSystemCacheEndExtra - (PCHAR)MiSystemCacheStartExtra + 1);
        HunksOf256KInCache = SizeOfSystemCacheInPages / (X256K / PAGE_SIZE);
...
BOOLEAN MmCheckCachedPageState(IN PVOID SystemCacheAddress, IN BOOLEAN SetToZero)
/*
Routine Description:
    This routine checks the state of the specified page that is mapped in the system cache.
    If the specified virtual address can be made valid (i.e., the page is already in memory), it is made valid and the value TRUE is returned.
    If the page is not in memory, and SetToZero is FALSE, the value FALSE is returned.
    However, if SetToZero is TRUE, a page of zeroes is materialized for the specified virtual address and the address is made valid and the value TRUE is returned.
    This routine is for usage by the cache manager.
Arguments:
    SystemCacheAddress - Supplies the address of a page mapped in the system cache.
    SetToZero - Supplies TRUE if a page of zeroes should be created in the case where no page is already mapped.
Return Value:
    FALSE if touching this page would cause a page fault resulting in a page read.
...
*/
{
    ...
    if (ProtoPteContents.u.Hard.Valid == 1) {
        PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(&ProtoPteContents);
        Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
        TempPte = ProtoPteContents;// The prototype PTE is valid, make the cache PTE valid and add it to the working set.
    } else if ((ProtoPteContents.u.Soft.Transition == 1) && (ProtoPteContents.u.Soft.Prototype == 0)) {
        // Prototype PTE is in the transition state.
        // Remove the page from the page list and make it valid.
        PageFrameIndex = MI_GET_PAGE_FRAME_FROM_TRANSITION_PTE(&ProtoPteContents);
        Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
        if ((Pfn1->u3.e1.ReadInProgress) || (Pfn1->u4.InPageError)) {
            goto UnlockAndReturnTrue;// Collided page fault, return.
        }
        ...
        if (MmAvailablePages < MM_HIGH_LIMIT) {
            ...
            if ((PsGetCurrentThread()->MemoryMaker == 0) || (MmAvailablePages == 0)) {
                goto UnlockAndReturnTrue;
            }
        }
        ...
        // Ensure the requested attributes do not conflict with the PFN attributes.
        ...
        MI_MAKE_VALID_KERNEL_PTE(TempPte, PageFrameIndex, Protection, MiHighestUserPte + 1);
        MI_WRITE_VALID_PTE(ProtoPte, TempPte);

        // Increment the valid PTE count for the page containing the prototype PTE.
        Pfn2 = MI_PFN_ELEMENT(Pfn1->u4.PteFrame);
    } else {
        // Page is not in memory, if a page of zeroes is requested, get a page of zeroes and make it valid.
        ...
        // Increment the count of Pfn references for the control area corresponding to this file.
        ...
        // This barrier check is needed after zeroing the page and before setting the PTE (not the prototype PTE) valid.
        ...
        MI_MAKE_VALID_KERNEL_PTE(TempPte, PageFrameIndex, Pfn1->OriginalPte.u.Soft.Protection, MiHighestUserPte + 1);
        MI_WRITE_VALID_PTE(ProtoPte, TempPte);
    }

    // Increment the share count since the page is being put into a working set.
    Pfn1->u2.ShareCount += 1;

    // Increment the reference count of the page table page for this PTE.
    PointerPde = MiGetPteAddress(PointerPte);
    ...
    WorkingSetIndex = MiAllocateWsle(&MmSystemCacheWs, PointerPte, Pfn1, WsleMask.u1.Long);
    if (WorkingSetIndex == 0) {
        // No working set entry was available so just trim the page.
        // Note another thread may be writing too so the page must be trimmed instead of just tossed.
    ...
    if ((WorkingSetIndex != 0) && (CCPF_IS_PREFETCHER_ACTIVE()) && (TempPte.u.Soft.Prototype == 1)) {
...
NTSTATUS MmCopyToCachedPage(IN PVOID SystemCacheAddress, IN PVOID UserBuffer, IN ULONG Offset, IN SIZE_T CountInBytes, LOGICAL ExposeZeroPageOk)
/*
Routine Description:
    This routine checks the state of the specified page that is mapped in the system cache.
    If the specified virtual address can be made valid (i.e., the page is already in memory), it is made valid and success is returned.
    This routine is for usage by the cache manager.
Arguments:
    SystemCacheAddress - Supplies the address of a page mapped in the system cache.  This MUST be a page aligned address!
    UserBuffer - Supplies the address of a user buffer to copy into the system cache at the specified address + offset.
    ...
    ExposeZeroPageOk - Supplies TRUE if it is ok to have a page of zeroes exposed (reachable by concurrent mappers) prior to the data getting copied into the page from user space.
                       This can only be done when the copy is being done beyond the valid data length of the file (as this is guaranteed to be zero filled anyway) and the cache manager and filesystems are in the process of extending VDL.
Environment:
...
*/
{
    ...
    // Access the user's buffer to make it resident.
    // This is purely an optimization so that pagefault clustering can kick in for the case where the user address is not resident.
    try {
    ...
    // Acquire the working set mutex now as it is highly likely we will be inserting this system cache address into the working set list.
    // This allows us to safely recover if no WSLEs are available because it prevents any other threads from locking down the address until we are done here.
    LOCK_SYSTEM_WS(Thread);
...
Recheck:
    ...
    ASSERT(PointerPte->u.Soft.Prototype == 1);// Make the PTE valid if possible.
    ...
    ProtoPteContents = *ProtoPte;
    if (ProtoPteContents.u.Hard.Valid == 1) {
        // The prototype PTE is valid, make the cache PTE valid and add it to the working set.
        PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(&ProtoPteContents);
        Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);

        // Increment the share count as this prototype PTE will be mapped into the system cache shortly.
        Pfn1->u2.ShareCount += 1;
        TempPte = ProtoPteContents;
    } else if ((ProtoPteContents.u.Soft.Transition == 1) && (ProtoPteContents.u.Soft.Prototype == 0)) {
        // Prototype PTE is in the transition state.  Remove the page from the page list and make it valid.
        PageFrameIndex = MI_GET_PAGE_FRAME_FROM_TRANSITION_PTE(&ProtoPteContents);
        Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
        if ((Pfn1->u3.e1.ReadInProgress) || (Pfn1->u4.InPageError)) {
            // Collided page fault or in page error, try the copy operation incurring a page fault.
            UNLOCK_PFN(OldIrql);
            UNLOCK_SYSTEM_WS(Thread);
            goto Copy;
        }
        ...
        if (MmAvailablePages < MM_LOW_LIMIT) {
            ...
            if (MiEnsureAvailablePageOrWait(NULL, OldIrql)) {
                // A wait operation occurred which could have changed the state of the PTE.  Recheck the PTE state.
                Pfn1 = NULL;
                goto Recheck;
            }
        }
        ...
        MI_MAKE_VALID_PTE(TempPte, PageFrameIndex, Protection, NULL);
        MI_SET_PTE_DIRTY(TempPte);
        MI_WRITE_VALID_PTE(ProtoPte, TempPte);

        // Do NOT increment the share count for the page containing the prototype PTE because it is already correct (the share count is for both transition & valid PTE entries and this one was transition before we just made it valid).
    } else {
        ...
        if (ExposeZeroPageOk == TRUE) {
            ...
            // Assert both locals are set properly so the copy occurs and that no inpage block gets released.
            ASSERT(Event == NULL);
            ...
            if (Event == NULL) {
                ...
                // A delay has already occurred if the allocation really failed so no need to do another here, just retry immediately.
                goto Recheck;
            }
            ...
            // Remove any page from the list and turn it into a transition page in the cache with read in progress set.
            // This causes any other references to this page to block on the specified event while the copy operation to the cache is on-going.
            ...
            // Transition collisions rely on the entire PFN (including the event field) being initialized, the Event's event being not-signaled, and the Event's thread and waitcount being initialized.
            ...
            // This is needed in case a special kernel APC fires that ends up referencing the same page (this may even be through a different virtual address from the user/system one here).
            ...
            // Map the page with a system PTE and do the copy into the page directly.
            UNLOCK_PFN(OldIrql);

            // APCs must be explicitly disabled to prevent suspend APCs from interrupting this thread before the RtlCopyBytes completes.
            // Otherwise this page can remain in transition indefinitely (until the suspend APC is released) which blocks any other threads that may reference it.
            ...
            if (CopyPte != NULL) {
                MI_MAKE_VALID_KERNEL_PTE(TempPte, PageFrameIndex, MM_READWRITE, CopyPte);
                MI_SET_PTE_DIRTY(TempPte);
                ...
                if (EndFill != 0) {
                    RtlZeroMemory((PVOID)((PCHAR)Buffer + CountInBytes), EndFill);
                }

                // Perform the copy of the user buffer into the page under an exception handler.
                ...
                {
                    // This page will be discarded (due to the error status) before any thread can see it, so the contents don't matter.
                ...
                // No PTEs available for us to take the fast path, just go the long way (fault on the page, read it in and copy the new user data into it.
                // Use NewPage == FALSE along with STATUS_INSUFFICIENT_RESOURCES to signify this.
                ASSERT(NewPage == FALSE);
            ...
            // No need to keep the prototype PTE pool allocation resident any longer so release it here.
            ASSERT(LockedProtoPfn->u3.e2.ReferenceCount >= 1);
...
Bail:
    if (Event != NULL) {
        ...
        // This is only possible for a newly allocated page and so it is already completely filled in.
        if (ApcNeeded == TRUE) {
        ...
        // If the fast path failed for lack of a system PTE, reset the Status and march forward and copy.
...
Copy:
    ...
    // If a virtual address was made directly present (ie: not via the normal fault mechanisms), then log prefetch fault information now that the PFN lock has been released and the PTE has been made valid.
    // This minimizes PFN lock contention, allows CcPfLogPageFault to allocate (and fault on) pool, and allows other threads in this process to execute without faulting on this address.
    if ((WsleMask.u1.e1.Protection == MM_ZERO_ACCESS) && (TempPte2.u.Soft.Prototype == 1)) {
...
/*
Routine Description:
    This routine is a filter for exceptions during copying data from the user buffer to the system cache.
    It stores the status code from the exception record into the status argument.
    In the case of an in page i/o error it returns the actual error code and in the case of an access violation it returns STATUS_INVALID_USER_BUFFER.
Arguments:
...
*/
{
    ...
    // If the exception is STATUS_IN_PAGE_ERROR, get the I/O error code from the exception record.
...
Arguments:
    AddressInCache - Supplies the address where the page was locked in the system cache.  This must be the same address that MmCopyToCachedPage was called with.
*/
{
    PMMPTE PointerPte;
    PMMPFN Pfn1;
    ...
    if (Pfn1->u3.e2.ReferenceCount <= 1) {
        KeBugCheckEx(MEMORY_MANAGEMENT, 0x777, (ULONG_PTR)PointerPte->u.Hard.PageFrameNumber, Pfn1->u3.e2.ReferenceCount, (ULONG_PTR)AddressInCache);
        return;
    }
...
LOGICAL MmIsSystemCacheAddress(IN PVOID VirtualAddress)
{
