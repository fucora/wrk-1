...
// Keep separate counters for the mapped and modified writer threads.  This way they can both be read and updated without locks.
...
SIZE_T MiAttemptPageFileExtension(IN ULONG PageFileNumber, IN SIZE_T SizeNeeded, IN LOGICAL Maximum);
NTSTATUS MiZeroPageFileFirstPage(IN PFILE_OBJECT File);
...
VOID MiGatherMappedPages(IN KIRQL OldIrql);
VOID MiGatherPagefilePages(IN PMMMOD_WRITER_MDL_ENTRY ModWriterEntry, IN PRTL_BITMAP Bitmap);
...
// Log pagefile writes so that scheduling, filesystem and storage stack problems can be tracked down.
...
VOID FORCEINLINE MiSnapPagefileWrite(IN PMMMOD_WRITER_MDL_ENTRY ModWriterEntry, 
                                     ...
                                     IN NTSTATUS Status)
{
    ...
    RtlCopyMemory(Information->MdlHack, &ModWriterEntry->Mdl, sizeof(Information->MdlHack));
}

#if MI_TRACK_PAGEFILE_WRITES
#define MI_PAGEFILE_WRITE(ModWriterEntry,CurrentTime,IrpPriority,Status)  MiSnapPagefileWrite(ModWriterEntry,CurrentTime,IrpPriority,Status)
#else
...
/*
Routine Description:
    Non-pageable wrapper to signal the modified writer when the first pagefile creation has completely finished.
*/
{
    KIRQL OldIrql;
...
/*
Routine Description:
    This routine zeroes the first page of the newly created paging file to ensure no stale crashdump signatures get to live on.
Arguments:
...
NTSTATUS NtCreatePagingFile(__in PUNICODE_STRING PageFileName,
                            __in PLARGE_INTEGER MinimumSize,
                            __in PLARGE_INTEGER MaximumSize,
                            __in ULONG Priority)
/*
Routine Description:
    This routine opens the specified file, attempts to write a page to the specified file,
    and creates the necessary structures to use the file as a paging file.
...
Arguments:
    ...
    MinimumSize - Supplies the starting size of the paging file. This value is rounded up to the host page size.
    MaximumSize - Supplies the maximum number of bytes to write to the file. This value is rounded up to the host page size.
...
*/
{
    ...
    if (PreviousMode != KernelMode) {
        ...
        try {
#if !defined (_WIN64)
            // Note we only probe for byte alignment because early releases of NT did and we don't want to break user apps that had bad alignment if they worked before.
            ...
            CapturedMinimumSize = *MinimumSize; // Capture arguments.
    ...
    if ((CapturedMinimumSize.QuadPart > MI_MAXIMUM_PAGEFILE_SIZE) || (CapturedMinimumSize.LowPart < MINIMUM_PAGE_FILE_SIZE)) {
    ...
    if (PreviousMode != KernelMode) {
        ...
        RtlCopyMemory(CapturedBuffer, CapturedName.Buffer, CapturedName.Length);// Copy the string to the allocated buffer.
    }
    ...
    DaclLength = sizeof(ACL) + sizeof(ACCESS_ALLOWED_ACE) * 2 + RtlLengthSid(SeLocalSystemSid) + RtlLengthSid(SeAliasAdminsSid);
    ...
    InitializeObjectAttributes(&PagingFileAttributes, &CapturedName, (OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE), NULL, &SecurityDescriptor);
...
#define ROUND64_TO_PAGES(Size)  (((ULONG64)(Size) + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1))
    ...
    if (NT_SUCCESS(Status)) {
        // Update the DACL in case there was a pre-existing regular file named pagefile.sys (even supersede above does not do this).
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            if (Status != STATUS_DISK_FULL) {
                DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "MM MODWRITE: unable to open paging file %wZ - status = %X \n", &CapturedName, Status);
            }
#endif
        ...
        Status = ObReferenceObjectByHandle(FileHandle, FILE_READ_DATA | FILE_WRITE_DATA, IoFileObjectType, KernelMode, (PVOID *)&File, NULL);
        if (!NT_SUCCESS(Status)) {
            goto ErrorReturn2;
        }
        ...
        if (NewMaxSizeInPages > FoundExisting->MaximumSize) {
            // Make sure that the pagefile increase doesn't cause the commit limit (in pages) to wrap.
            // Currently this can only happen on PAE systems where 16 pagefiles of 16TB (==256TB) is greater than the 32-bit commit variable (max is 16TB).
            ...
            // We may be low on commitment and/or may have put a temporary stopgate on things.
            // Clear up the logjam now by forcing an extension and immediately returning it.
        ...
        if (NewMinSizeInPages > FoundExisting->MinimumSize) {
            ...
            if (NewMinSizeInPages > FoundExisting->Size) {
                // Queue a message to the segment dereferencing / pagefile extending thread to see if the page file can be extended.
            ...
            if (FoundExisting->Size >= NewMinSizeInPages) {
                ...
                // No easy way to undo any maximum raising that may have been done as the space may have already been used, 
                // so just set Status so our caller knows it didn't all go perfectly.
                Status = STATUS_INSUFFICIENT_RESOURCES;
            }
        }
    ...
    Status = ZwSetInformationFile(FileHandle, &IoStatus, &EndOfFileInformation, sizeof(EndOfFileInformation), FileEndOfFileInformation);
    if (!NT_SUCCESS(Status)) {
    ...
    // Get the address of the target device object and ensure the specified file is of a suitable type.
    ...
    Status = IoQueryVolumeInformation(File, FileFsDeviceInformation, sizeof(FILE_FS_DEVICE_INFORMATION), &FileDeviceInfo, &ReturnedLength);
    if (FILE_FLOPPY_DISKETTE & FileDeviceInfo.Characteristics) {
        Status = STATUS_FLOPPY_VOLUME;
        goto ErrorReturn3;
    }

    // Check with all of the drivers along the path to the file to ensure that they are willing to follow the rules required of them and to give them a chance to lock down code and data that needs to be locked.
    // If any of the drivers along the path refuses to participate, fail the pagefile creation.
    Status = PpPagePathAssign(File);
    if (!NT_SUCCESS(Status)) {
        KdPrint(("PpPagePathAssign(%wZ) FAILED: %x\n", &CapturedName, Status));
        goto ErrorReturn3;// Fail the pagefile creation if the storage stack tells us to.
    }

    NewPagingFile = ExAllocatePoolWithTag(NonPagedPool, sizeof(MMPAGING_FILE), '  mM');
    if (NewPagingFile == NULL) {// Allocate pool failed.
        Status = STATUS_INSUFFICIENT_RESOURCES;
        goto ErrorReturn3;
    }
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        // This means an old crashdump signature could still be around so fail the create.
        for (i = 0; i < MM_PAGING_FILE_MDLS; i += 1) {
            ExFreePool(NewPagingFile->Entry[i]);
        }
        ExFreePool(NewPagingFile);
    ...
    RtlClearBits(NewPagingFile->Bitmap, 1, (ULONG)(NewPagingFile->Size - 1));// Set the first bit as 0 is an invalid page location, clear the following bits.

    // See if this pagefile is on the boot partition, and if so, mark it so we can find it later if someone enables crashdump.
    ...
    if (PageFileNumber == 0) {
        // The first paging file has been created and reservation of any crashdump pages has completed, signal the modified page writer.
        MiReleaseModifiedWriter();
    }
    KeReleaseGuardedMutex(&MmPageFileCreationLock);

    // Note that the file handle (a kernel handle) is not closed during the create path (it IS duped and closed in the pagefile size extending path) to prevent the paging file from being deleted or opened again.
    // It is also kept open so that extensions of existing pagefiles can be detected because successive IoCreateFile calls will fail.
...
HANDLE MmGetSystemPageFile(VOID)
/*
Routine Description:
    Returns a filehandle to the paging file on the system boot partition.
    This is used by crashdump to enable crashdump after the system has already booted.
Return Value:
    A file handle to the paging file on the system boot partition, NULL if no such pagefile exists.
*/
{
    HANDLE FileHandle;
    ULONG PageFileNumber;
...
LOGICAL MmIsFileObjectAPagingFile(IN PFILE_OBJECT FileObject)
/*
Routine Description:
...
Return Value:
    Returns TRUE if the file object refers to a paging file, FALSE if not.
    Note this routine is called both at DISPATCH_LEVEL by drivers in their completion routines and it is called in the path to satisfy pagefile reads,
...
*/
{
    ...
    // The modified writer may be scanning the old map without holding any locks - if so, 
    // he will have reference counted the old bitmap, so only free it if this isn't in progress.
    // If it is in progress, the modified writer will free the old bitmap.
...
Arguments:
    ...
    Maximum - Supplies TRUE if the page file should be extended by the maximum size possible, but not to exceed SizeNeeded.
*/
{
    KIRQL OldIrql;
    PMMPAGING_FILE PagingFile;

    PagingFile = MmPagingFile[PageFileNumber];// Clear bits within the paging file bitmap to allow the extension to take effect.
...
Return Value:
    ...
    Zero if the page file cannot be extended.
*/
{
    NTSTATUS status;
    ...
    status = IoQueryVolumeInformation(MmPagingFile[PageFileNumber]->File, FileFsSizeInformation, sizeof(FileInfo), &FileInfo, &ReturnedLength);
    if (!NT_SUCCESS(status)) {
        // The volume query did not succeed - return 0 indicating the paging file was not extended.
        return 0;
    }

    MinimumExtension = MmPageFileExtension << 4;// Attempt to extend by at least 16 megabytes, if that fails then attempt for at least a megabyte.
...
retry:
    ...
    if (SizeToExtend > PagesAvailable) {
        ...
        if ((SizeToExtend < SizeNeeded) && (Maximum == FALSE)) {// Can't meet the requested (mandatory) requirement.
            return 0;
        }
    }
    ...
    if ((UINT64)BytesAvailable.QuadPart > (UINT64)MmMinimumFreeDiskSpace) {
        ...
        if ((Maximum == FALSE) && (PagesAvailable < SizeNeeded)) {// Can't meet the requested (mandatory) requirement.
            return 0;
        }
    } else {
        return 0;// Not enough space is available period.
    }
    ...
    EndOfFileInformation.EndOfFile.HighPart = 0;// Set high part to zero as paging files are limited to 4GB.
#endif
...
/*
Routine Description:
    This routine attempts to extend the paging files to provide SizeNeeded bytes.
    Note - Page file expansion and page file reduction are synchronized because a single thread is responsible for performing the operation.
           Hence, while expansion is occurring, a reduction request will be queued to the thread.
Arguments:
...
*/
{
    ...
    // Snap the globals into locals so calculations will be consistent from step to step.
    // It is ok to snap the globals unsynchronized with respect to each other as even when pagefile expansion occurs,
    // the expansion space is not reserved for the caller - any process could consume the expansion prior to this routine returning.
    ...
    if (MmNumberOfPagingFiles == 1) {
        // If the attempt didn't succeed for one (not enough disk space free) - don't try to set it to the maximum size.
        InterlockedExchange((PLONG)&PageExpand->InProgress, 0);
        return 0;
    }
...
alldone:
    ...
    InterlockedExchangeAddSizeT(&MmTotalCommitLimit, ExtendedSize);// Increase the systemwide commit limit.

    // Clear the in progress flag - if this is the global cantexpand structure it is possible for it to be immediately reused.
    InterlockedExchange((PLONG)&PageExpand->InProgress, 0);
    return ExtendedSize;
}
...
/*
Routine Description:
    This routine checks to see if ample space is no longer committed and if so, does enough free space exist in any paging file.
    IFF the answer to both these is affirmative, a reduction in the paging file size(s) is attempted.
*/
{
    ULONG i;
    KIRQL OldIrql;
    ...
    // This is an unsynchronized check but that's ok.
    // The real check is made when the packet below is processed by the dereference thread.
    ...
    // See if the page file contraction item is already queued up, if so then nothing more needs to be done.
...
*/
{
    ...
    // Mark the global pagefile contraction item as being processed so other threads will know to reset it if another contraction is desired.
    ExAcquireSpinLock(&MmDereferenceSegmentHeader.Lock, &OldIrql);
    ...
    // Snap the globals into locals so calculations will be consistent from step to step. 
    // It is ok to snap the globals unsynchronized with respect to each other.
    CommittedPages = MmTotalCommittedPages;
    CommitLimit = MmTotalCommitLimit;

    // Make sure the commit limit is significantly greater than the number of committed pages to avoid thrashing.
    ...
    for (i = 0; i < MmNumberOfPagingFiles; i += 1) {
        if (MaxReduce < MmMinimumPageFileReduction) {
            break;// Don't reduce any more paging files.
        }
        ...
        // This unsynchronized check is ok because a synchronized check is made later.
        if (PagingFile->FreeSpace < MmMinimumPageFileReduction) {
            continue;
        }

        // Lock the PFN database and check to see if ample pages are free at the end of the paging file.
        ...
        do {
            ...
            } else {// Reduction has failed.
                break;
            }
        } while (TRUE);

        // Make sure there are no outstanding writes to pages within the start reduction range.
        if (StartReduction != 0) {
            // There is an outstanding write past where the new end of the paging file should be.
            // This is a very rare condition, so just punt shrinking the file.
        ...
        // First try to charge the reduction amount to confirm that the reduction is still a sensible thing to do.
        ...
        // Reduce the systemwide commit limit - note this is carefully done *PRIOR* to returning this commitment so no one else (including a DPC in this very thread) can consume past the limit.
        InterlockedExchangeAddSizeT(&MmTotalCommitLimit, 0 - ReductionSize);

        // Now that the systemwide commit limit has been lowered, the amount we have removed can be safely returned.
        ...
        FileAllocationInfo.AllocationSize.HighPart = 0;// Set high part to zero, paging files are limited to 4gb.
#endif

        // Reduce the allocated size of the paging file thereby actually freeing the space and setting a new end of file.
        ASSERT(KeGetCurrentIrql() < DISPATCH_LEVEL);
        status = IoSetInformation(PagingFile->File, FileAllocationInformation, sizeof(FILE_ALLOCATION_INFORMATION), &FileAllocationInfo);
#if DBG
        // Ignore errors on truncating the paging file as we can always have less space in the bitmap than the pagefile holds.
        if (status != STATUS_SUCCESS) {
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "MM: pagefile truncate status %lx\n", status);
        }
#endif
    }
}
...
/*
Routine Description:
    This routine is the worker routine to send a lost delayed write data popup for a given control area/file.
Arguments:
...
*/
{
    ...
    // If everything fails, the user probably couldn't have figured out what failed either.
...
/*
Routine Description:
    This routine is the APC write completion procedure.
    It is invoked at APC_LEVEL when a page write operation is completed.
Arguments:
...
*/
{
    ...
    // A page write has completed, at this time the pages are not on any lists, 
    // write-in-progress is set in the PFN database, and the reference count was incremented.
    ...
    if (!NT_SUCCESS(status)) {
        // If the file object is over the network, assume that this I/O operation can never complete and mark the pages as clean and indicate in the control area all I/O should fail.
        // Note that the modified bit in the PFN database is not set.

        // If the user changes the protection on the volume containing the file to readonly, this puts us in a problematic situation.
        // We cannot just keep retrying the writes because if there are no other pages that can be written,
        // not writing these can cause the system to run out of pages, ie: bugcheck 4D.
        // So throw away these pages just as if they were on a network that has disappeared.
        if (((status != STATUS_FILE_LOCK_CONFLICT) && (ControlArea != NULL) && (ControlArea->u.Flags.Networked == 1)) ||
            (status == STATUS_FILE_INVALID) ||
            ((status == STATUS_MEDIA_WRITE_PROTECTED) && (ControlArea != NULL))) {
            ...
            // The modified write operation failed, SET the modified bit for each page which was written and free the page file space.

#if DBG
            ULONG i;

            // Save error status information to ease debugging.
            // Since this doesn't need to be exact, don't bother lock-synchronizing.
        ...
        if (Pfn1->OriginalPte.u.Soft.Prototype == 0) {
            ...
            if ((Offset < 8192) && (GET_PAGING_FILE_NUMBER(Pfn1->OriginalPte) == 0)) {
                ...
                if ((!MI_IS_PFN_DELETED(Pfn1)) && 
                    ((MmPagingFileDebug[Offset] & ~0x1f) != ((ULONG_PTR)Pfn1->PteAddress << 3)) && 
                    ...
                    if ((Pfn1->PteAddress < (PMMPTE)PTE_TOP) || 
                        ((Pfn1->OriginalPte.u.Soft.Protection & MM_COPY_ON_WRITE_MASK) == MM_PROTECTION_WRITE_MASK)) {
    ...
    // Choose which list to insert this entry into depending on the amount of free space left in the paging file.
    if ((WriterEntry->PagingFile != NULL) && (WriterEntry->PagingFile->FreeSpace < MM_USABLE_PAGES_FREE)) {
        ...
        if (MmNumberOfActiveMdlEntries == 0) {
            // If we leave this entry on the list, there will be no more paging.
            // Locate all entries which are non zero and pull them from the list.
    ...
    if (ControlArea != NULL) {
        ...
        // A write to a mapped file just completed, check to see if there are any waiters on the completion of this i/o.
        ...
        if (MiDrainingMappedWrites == TRUE) {
            if (MmModifiedPageListHead.Flink != MM_EMPTY_LIST) {
                ...
                // holding the PFN lock - otherwise the other thread (modified or mapped writer) can clear TimerPending before we set the event here.
                MiTimerPending = TRUE;
    ...
    if (!NT_SUCCESS(status)) {
        ...
        if (MmIsRetryIoStatus(status)) {
            // Low resource scenarios are a chicken and egg problem.
            // The mapped and modified writers must make forward progress to alleviate low memory situations.
            // If these threads are unable to write data due to resource problems in the driver stack then temporarily fall back to single page I/Os as the stack guarantees forward progress with those.
            // This causes the low memory situation to persist slightly longer but ensures that it won't become terminal either.
            do {
                OldClusterWritesDisabled = ReadForWriteAccess(&MiClusterWritesDisabled);
                ClusterWritesDisabled = InterlockedCompareExchange(&MiClusterWritesDisabled, MI_SLOW_CLUSTER_WRITES, OldClusterWritesDisabled);
        ...
        do {
            ...
            ClusterWritesDisabled = InterlockedCompareExchange(&MiClusterWritesDisabled, OldClusterWritesDisabled - 1, OldClusterWritesDisabled);
        } while (OldClusterWritesDisabled != ClusterWritesDisabled);
    }
}
...
/*
Routine Description:
    This routine attempts to stop a pending mapped page writer write for the specified PFN.
    Note that if the write can be stopped, any other pages that may be clustered with the write are also stopped.
Arguments:
...
*/
{
    ...
    // Walk the MmMappedPageWriterList looking for an MDL which contains the argument page.  If found, remove it and cancel the write.
...
CancelWrite:
    UNLOCK_PFN(OldIrql);
    // File lock conflict to indicate an error has occurred, but that future I/Os should be allowed.
    // Keep APCs disabled and call the write completion routine.
...
/*
Routine Description:
    Implements the NT modified page writer thread.
    When the modified page threshold is reached, or memory becomes overcommitted the modified page writer event is set, and this thread becomes active.
Arguments:
...
*/
{
    ...
    // Allocate enough MDLs such that 2% of system memory can be pending at any point in time in mapped writes.
    // Even smaller memory systems get 20 MDLs as the minimum.
    ...
    for (i = 0; i < MmNumberOfMappedMdls; i += 1) {
        ModWriteEntry = ExAllocatePoolWithTag(NonPagedPool, sizeof(MMMOD_WRITER_MDL_ENTRY) + MmModifiedWriteClusterSize * sizeof(PFN_NUMBER), 'eWmM');
    ...
    // Start a secondary thread for writing mapped file pages.
    // This is required as the writing of mapped file pages could cause page faults resulting in requests for free pages.
    // But there could be no free pages - hence a dead lock.
    // Rather than deadlock the whole system waiting on the modified page writer, 
    // creating a secondary thread allows that thread to block without affecting on going page file writes.
    KeInitializeEvent(&MmMappedPageWriterEvent, NotificationEvent, FALSE);
    InitializeListHead(&MmMappedPageWriterList);
    InitializeObjectAttributes(&ObjectAttributes, NULL, 0, NULL, NULL);
    Status = PsCreateSystemThread(&ThreadHandle, THREAD_ALL_ACCESS, &ObjectAttributes, 0L, NULL, MiMappedPageWriter, NULL);
    if (!NT_SUCCESS(Status)) {
        KeBugCheckEx(MEMORY_MANAGEMENT, 0x41288, Status, 0, 0);
    }
...
/*
Routine Description:
    This routine is executed whenever modified mapped pages are waiting to be written.
    Its job is to signal the Modified Page Writer to write these out.
Arguments:
...
Routine Description:
    Implements the NT modified page writer thread.
    When the modified page threshold is reached, 
    or memory becomes overcommitted the modified page writer event is set,
    and this thread becomes active.
Environment:
    Kernel mode.
*/
{
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            // If we didn't wake up explicitly to deal with mapped pages, then determine which type of pages are the most popular:
            // page file backed pages, or mapped file backed pages.
            if ((WakeupStatus == MappedPagesNeedWriting) || 
                (MmTotalPagesForPagingFile < MmModifiedPageListHead.Total - MmTotalPagesForPagingFile)) {

                // More pages are destined for mapped files.
                if (MmModifiedPageListHead.Flink != MM_EMPTY_LIST) {
                    if (WakeupStatus == MappedPagesNeedWriting) {
                        // Our mapped pages DPC went off, only deal with those pages.
                        // Write all the mapped pages (ONLY), then clear the flag and come back to the top.
                    ...
                    if (IsListEmpty(&MmMappedFileHeader.ListHead)) {
                        ...
                        if (WakeupStatus != MappedPagesNeedWriting) {
                            // No MDLs are available for writing mapped pages, try for a page destined for a pagefile.
                    ...
                    // No more modified mapped pages (there may still be modified pagefile-destined pages), 
                    // so clear only the mapped pages event and check for directions at the top again.
                    if (WakeupStatus == MappedPagesNeedWriting) {
                        MiTimerPending = FALSE;
                        KeClearEvent(&MiMappedPagesTooOldEvent);
                        UNLOCK_PFN(OldIrql);
                ...
                if (((IsListEmpty(&MmPagingFileHeader.ListHead)) || (MiFirstPageFileCreatedAndReady == FALSE))) {
                    ...
                    if (MmModifiedPageListHead.Flink != MM_EMPTY_LIST) {
                        ...
                        if ((MiFirstPageFileCreatedAndReady == FALSE) && (MmNumberOfPagingFiles != 0)) {
                            // The first paging has been created but the reservation checking for crashdumps has not finished yet.
                            // Delay a bit as this will finish shortly and then restart.
                            UNLOCK_PFN(OldIrql);
            ...
            if (MappedPage == TRUE) {
                if (IsListEmpty(&MmMappedFileHeader.ListHead)) {
                    if (WakeupStatus == MappedPagesNeedWriting) {
                        ...
                        // don't wait for an MDL below because drivers may take an inordinate amount of time processing the outstanding ones.
                        // We might have to wait too long, resulting in the system running out of pages.
                        if (MiTimerPending == TRUE) {
                            // This should be normal case - the reason we must first check timer pending above is for the rare case - when this thread first ran for normal modified page processing and took care of all the pages including the mapped ones.
                    ...
                    // Reset the event indicating no mapped files in the list, 
                    // drop the PFN lock and wait for an I/O operation to complete with a one second timeout.
                    KeClearEvent(&MmMappedFileHeader.Event);
                    UNLOCK_PFN(OldIrql);
                    KeWaitForSingleObject(&MmMappedFileHeader.Event, WrPageOut, KernelMode, FALSE, (PLARGE_INTEGER)&Mm30Milliseconds);

                    // Don't go on as the old PageFrameIndex at the top of the ModifiedList may have changed states.
                    LOCK_PFN(OldIrql);
                } else {
                    MiGatherMappedPages(OldIrql);// This routine returns with the PFN lock released !

                    // Nothing magic here, just give other processors a turn at the PFN lock (and allow DPCs a chance to execute).
                    LOCK_PFN(OldIrql);
                }
            } else {
                if (IsListEmpty(&MmPagingFileHeader.ListHead)) {
                    // Reset the event indicating no paging files MDLs in the list, drop the PFN lock and wait for an I/O operation to complete.
                    KeClearEvent(&MmPagingFileHeader.Event);
                    UNLOCK_PFN(OldIrql);
                    KeWaitForSingleObject(&MmPagingFileHeader.Event, WrPageOut, KernelMode, FALSE, (PLARGE_INTEGER)&Mm30Milliseconds);
            ...
            // If this is a mapped page timer, then keep on writing till there's nothing left.
            if (WakeupStatus == MappedPagesNeedWriting) {
                continue;
            }

            // If this is a request to write all modified pages, then keep on writing.
...
/*
Routine Description:
    This routine processes the specified modified page by examining the prototype PTE for that page and the adjacent prototype PTEs building a cluster of modified pages destined for a mapped file.
    Once the cluster is built, it is sent to the mapped writer thread to be processed.
Arguments:
    OldIrql - Supplies the IRQL the caller acquired the PFN lock at.
Return Value:
    The number of pages in the attempted write.  The PFN lock is released prior to returning.
Environment:
    PFN lock held.
*/
{
    ...
    // This page is destined for a mapped file, 
    // check to see if there are any physically adjacent pages are also in the modified page list and write them out at the same time.
    Subsection = MiGetSubsectionAddress(&Pfn1->OriginalPte);
    ControlArea = Subsection->ControlArea;
    if (ControlArea->u.Flags.NoModifiedWriting) {
        // This page should not be written out, add it to the tail of the modified NO WRITE list and get the next page.
    ...
    if (ControlArea->u.Flags.Image) {
        // This is an image section, writes are not allowed to an image section.

        // Change page contents to look like it's a demand zero page and put it back into the modified list.

        // Decrement the count for PfnReferences to the segment as paging file pages are not counted as "image" references.
        ControlArea->NumberOfPfnReferences -= 1;
        ...
        // Insert the page at the tail of the list and get color update performed.
        MiInsertPageInList(&MmModifiedPageListHead, PageFrameIndex);
        UNLOCK_PFN(OldIrql);
        return;
    }

    // Look at backwards at previous prototype PTEs to see if this can be clustered into a larger write operation.
    ...
    // If the prototype PTEs are not currently mapped, map them via hyperspace.
    // BasePte refers to the prototype PTEs for nonfaulting references.
    ...
    while (PointerPte >= NextPte) {
        ...
        if ((PteContents.u.Hard.Valid == 1) || (PteContents.u.Soft.Transition == 0) || (PteContents.u.Soft.Prototype == 1)) {
            break;
        }
    ...
    *Page = PageFrameIndex;
    ...
    // See if any adjacent pages are also modified and in the transition state and if so, write them out at the same time.
    LastPte = StartingPte + MmModifiedWriteClusterSize;

    // Look at the last PTE, ensuring a page boundary is not crossed.
    // If LastPte is not in the same page as the StartingPte, set LastPte so the cluster will not cross.
    ...
    // Loop until an MDL is filled, the end of a subsection is reached, or a page boundary is reached.
    // Note, PointerPte points to the PTE.
    // NextPte points to where it is mapped in hyperspace (if required).
    ...
    if ((ULONG)ModWriterEntry->Mdl.ByteCount > ((1 + MmModifiedWriteClusterSize)*PAGE_SIZE)) {
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "Mdl %p, MDL End Offset %lx %lx Subsection %p\n",
                   ModWriterEntry->Mdl, ModWriterEntry->u.LastByte.LowPart, ModWriterEntry->u.LastByte.HighPart, Subsection);
        DbgBreakPoint();
    }
#endif
    ...
    // Increment the count of modified page writes outstanding in the control area.
    ControlArea->ModifiedWriteCount += 1;

    // Increment the number of PFN references.  This allows the file system to purge (i.e. call MmPurgeSection) modified writes.
    ControlArea->NumberOfPfnReferences += 1;
...
/*
Routine Description:
    This routine processes the specified modified page by getting that page and gather any other pages on the modified list destined for the paging file in a large write cluster.
    This cluster is then written to the paging file.
Arguments:
    ...
    Bitmap - Supplies the captured bitmap to scan for free pagefile blocks.
             This address was captured under the PFN lock by the caller - this routine must free the bitmap pool if it detects that a new bitmap (ie: the pagefile has been extended) while the lock free scan was underway.
Environment:
    No locks held.
*/
{
    ...
    // As scan durations may be long, first scan for a possible hit without holding the PFN lock.
    do {
        // Attempt to cluster MmModifiedWriteClusterSize pages together.
        // Reduce by one page until we succeed or can't find a single page free in the paging file.
        StartBit = RtlFindClearBits(Bitmap, (ULONG)ThisCluster, 0);
        if (StartBit != NO_BITS_FOUND) {
            LOCK_PFN(OldIrql);

            // Note that the current bitmap may be different from the one that was passed in.
            StartBit = RtlFindClearBitsAndSet(CurrentPagingFile->Bitmap, (ULONG)ThisCluster, StartBit);
            if (StartBit != NO_BITS_FOUND) {
                ...
                if (Bitmap == CurrentPagingFile->Bitmap) {
                    // The paging file has not been extended so don't free the existing bitmap.
                    Bitmap = NULL;
                }
    ...
    if (StartBit == NO_BITS_FOUND) {
        ...
        do {
            // Attempt to cluster MmModifiedWriteClusterSize pages together.
            // Since we hold the PFN lock, 
            // reduce by one half (instead of one page) until we succeed or can't find a single page free in the paging file.
            StartBit = RtlFindClearBitsAndSet(CurrentPagingFile->Bitmap, (ULONG)ThisCluster, 0);
            if (StartBit != NO_BITS_FOUND) {
                break;
            }
    ...
    if (MmTotalPagesForPagingFile == 0) {
        // No modified pages left - other threads may have put them back into working sets, flushed them or deleted them.
    ...
    // Search through the modified page list looking for other pages destined for the paging file and build a cluster.
    while (ClusterSize != ThisCluster) {
        // Is this page destined for a paging file?
        if (Pfn1->OriginalPte.u.Soft.Prototype == 0) {
            *Page = PageFrameIndex;

            // Remove the page from the modified list.
            // Note that write-in-progress marks the state.
            ...
            // Change the original PTE contents to refer to the paging file offset where this was written.
    ...
    bail:
        // A complete cluster could not be located, free the excess page file space that was reserved and adjust the size of the packet.
        ...
        // If there are no pages to write, don't issue a write request and restart the scan loop.
        if (ClusterSize == 0) {
    ...
    InterlockedExchangeAdd(&Prcb->MmDirtyPagesWriteCount, (LONG)ClusterSize);
    ...
    MI_PAGEFILE_WRITE(ModWriterEntry, &ModWriterEntry->IssueTime, IrpPriority, (NTSTATUS)-1);
...
Routine Description:
    ...
    This is required as the writing of mapped file pages could cause page faults resulting in requests for free pages.
    But there could be no free pages - hence a deadlock.
    Rather than deadlock the whole system waiting on the modified page writer, 
    creating a secondary thread allows that thread to block without affecting ongoing page file writes.
Arguments:
    StartContext - not used.
Environment:
    Kernel mode.
*/
{
    ...
    while (TRUE) {
        KeWaitForSingleObject(&MmMappedPageWriterEvent, WrVirtualMemory, KernelMode, FALSE, NULL);
        ...
        if (IsListEmpty(&MmMappedPageWriterList)) {
            ...
            if (ModWriterEntry->ControlArea->u.Flags.FailAllIo == 1) {
                ...
                Status = FsRtlAcquireFileForModWriteEx(ModWriterEntry->File, &ModWriterEntry->u.LastByte, &ModWriterEntry->FileResource);
                if (NT_SUCCESS(Status)) {
                    // Issue the write request.
                    IrpPriority = IoPagingPriorityNormal;
                    ...
                    // Unable to get the file system resources, 
...
BOOLEAN MmDisableModifiedWriteOfSection(__in PSECTION_OBJECT_POINTERS SectionObjectPointer)
/*
Routine Description:
    This function disables page writing by the modified page writer for the section which is mapped by the specified file object pointer.
    This should only be used for files which CANNOT be mapped by user programs, e.g., volume files, directory files, etc.
Arguments:
    SectionObjectPointer - Supplies a pointer to the section objects
Return Value:
    Returns TRUE if the operation was a success, FALSE if either the there is no section or the section already has a view.
*/
{
    KIRQL OldIrql;
    BOOLEAN state;
    ...
    if (ControlArea != NULL) {
        if (ControlArea->NumberOfMappedViews == 0) {
            // There are no views to this section, indicate no modified page writing is allowed.
            ControlArea->u.Flags.NoModifiedWriting = 1;
        } else {
            state = (BOOLEAN)ControlArea->u.Flags.NoModifiedWriting;// Return the current modified page writing state.
        }
    } else {
        state = 0;// This file no longer has an associated segment.
    }
    UNLOCK_PFN(OldIrql);
...
BOOLEAN MmEnableModifiedWriteOfSection(IN PSECTION_OBJECT_POINTERS SectionObjectPointer)
/*
Routine Description:
    This function enables page writing by the modified page writer for the section which is mapped by the specified file object pointer.
...
*/
{
    ...
    if ((ControlArea != NULL) && (ControlArea->u.Flags.NoModifiedWriting)) {
        ...
        // Move any straggling pages on the modnowrite list back onto the modified list otherwise they would be orphaned and this could cause us to run out of pages.
...
*/
{
    ...
    *Length = 0;
    ...
    for (i = 0; i < MmNumberOfPagingFiles; i += 1) {
        ...
        // The PageFileName portion of the UserBuffer must be saved locally to protect against a malicious thread changing the contents.
        // This is because we will reference the contents ourselves when the actual string is copied out carefully below.
        ...
        RtlCopyMemory(UserBufferPageFileName.Buffer, MmPagingFile[i]->PageFileName.Buffer, MmPagingFile[i]->PageFileName.Length);
...
/*
Routine Description:
    Non-pageable routine to check to see if a given file has no sections and therefore is eligible to become a paging file.
Arguments:
...
*/
{
    ...
    if ((File->SectionObjectPointer->DataSectionObject != NULL) || (File->SectionObjectPointer->ImageSectionObject != NULL)) {
        UNLOCK_PFN(OldIrql);
...
/*
Routine Description:
    Non-pageable routine to add a page file into the list of system wide page files.
Arguments:
...
*/
{
    ...
    for (i = 0; i < MM_PAGING_FILE_MDLS; i += 1) {
        InsertTailList(&MmPagingFileHeader.ListHead, &MmPagingFile[MmNumberOfPagingFiles - 1]->Entry[i]->Links);
        MmPagingFile[MmNumberOfPagingFiles - 1]->Entry[i]->CurrentList = &MmPagingFileHeader.ListHead;
    }
    ...
    // Increase the systemwide commit limit maximum first.  Then increase the current limit.
...
Routine Description:
    ...
    It looks through all the paging files to see if ample space is available and if not, tries to expand the paging files.
    If more than 90% of all the paging files are in use, the commitment limit is set to the total and then 100 pages are added.
*/
{
    ULONG i;
    PFN_NUMBER Total;
    ...
    if (((Total >> 5) + (Total >> 4)) >= Free) {
        ...
        // If all the pagefiles are already at their maximums, 
        // then don't make things worse by setting commit to the maximum - this gives systems with lots of memory a longer lease on life when they have small pagefiles.
        i = 0;
        do {
        ...
        if (i == MmNumberOfPagingFiles) {
            // No pagefiles can be extended, display a popup if we haven't before.
        ...
        // IFF the total number of committed pages is less than the limit, or in any event, 
        // no more than 50 pages past the limit, then charge pages against the commitment to trigger pagefile expansion.

        // If the total commit is more than 50 past the limit, then don't bother trying to extend the pagefile.
...
*/
{
    ...
    // If there are no paging files, then no sense in waiting for modified writes to complete.
    if (MmNumberOfPagingFiles == 0) {
        return;
    }
...
Routine Description:
    Queue a message to the segment dereferencing / pagefile extending thread to see if the page file can be extended.
    Extension is done in the context of a system thread due to mutexes which the current thread may be holding.
Arguments:
...
*/
{
    ...
    if (Thread->StartAddress == (PVOID)(ULONG_PTR)MiDereferenceSegmentThread) {
        return FALSE;// The segment dereference thread cannot wait for itself !
    }

    // Avoid repeatedly waking the segment dereference thread just for it to perform a no-op by first checking whether the pagefile(s) are already at their maximum (or don't exist) - if so, just return a failure.
    for (i = 0; i < MmNumberOfPagingFiles; i += 1) {
    ...
    status = KeWaitForSingleObject(&PageExtend->Event, Executive, KernelMode, FALSE, (PageExtend->RequestedExpansionSize < 10) ? (PLARGE_INTEGER)&MmOneSecond : (PLARGE_INTEGER)&MmTwentySeconds);
    if (status == STATUS_TIMEOUT) {
        // The wait has timed out, if this request has not been processed, remove it from the list and check to see if we should allow this request to succeed.
        // This prevents a deadlock between the file system trying to allocate memory in the FSP and the segment dereferencing thread trying to close a file object, and waiting in the file system.
        ...
        while (NextEntry != &MmDereferenceSegmentHeader.ListHead) {
            if (NextEntry == &PageExtend->DereferenceList) {// Check to see if this is the entry we are waiting for.
        ...
        KeWaitForSingleObject(&PageExtend->Event, Executive, KernelMode, FALSE, NULL);// Entry is being processed, wait for completion.
    }
...
Environment:
    Kernel mode.
    No locks held.
    DISPATCH_LEVEL or less.
    Note this routine must be very careful to not use any paged pool as the only reason it is being called is because pool is depleted.
*/
{
    KIRQL OldIrql;
    ...
    if (OriginalInProgress != 0) {
        // An expansion request is already in progress, assume it will help enough (another can always be issued later) and that it will succeed.
        return;
    }
