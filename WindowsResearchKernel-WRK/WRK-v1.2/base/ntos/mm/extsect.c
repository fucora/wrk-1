...
LOGICAL MiAppendSubsectionChain(IN PMSUBSECTION LastSubsection, IN PMSUBSECTION ExtendedSubsectionHead)
/*
Routine Description:
...
Arguments:
    ...
    ExtendedSubsectionHead - Supplies an anchor pointing to the first subsection in the chain to append.
Return Value:
...
*/
{
    ...
    // This subsection may be extending a range that is already mapped by a VAD(s).
    // There is no way to tell how many VADs already map it so if any do, just leave all the new subsections marked as not reclaimable until the control area is deleted.

    // If however, there are no *USER* references to this control area, then the subsections can be marked as dynamic now.
    // Note other portions of code (currently only prefetch) that issue "dereference from this subsection to the end of file" are safe because these
    // portions create user sections first and so the first check below will be FALSE.
    if (LastSubsection->ControlArea->NumberOfUserReferences != 0) {
        ...
        // The prototype PTEs are no longer required (user views went away) even though they were when the caller first built the subsections.
    ...
    // A memory barrier is needed to ensure the writes initializing the subsection fields are visible prior to linking the subsection into the chain.
    // This is because some reads from these fields are done lock free for improved performance.
    KeMemoryBarrier();
...
*/
{
    ...
    // In reality, this is a nonissue as far as maximum section size is concerned because any number of subsections can be chained together
    // and in fact, subsections are allocated to span less to facilitate efficient dynamic prototype PTE trimming and reconstruction.
    ...
    if (IgnoreFileSizeChecking == FALSE) {
        ExReleaseResourceLite(&MmSectionExtendResource);// Release the resource so we don't deadlock with the file system trying to extend this section at the same time.
        ExAcquireResourceExclusiveLite(&MmSectionExtendSetResource, TRUE);// Get a different resource to single thread query/set operations.
    ...
    if (RequiredPtes == 0) {
        ...
        // If there are any user views, then also allocate the prototype PTEs now (because a user view may be for an extended VAD which would already be encompassing this new extension).
        // If there are no user views, then don't allocate the prototype PTEs until the views are actually mapped (system views never pre-extend past the end of the file).
        ...
        do {
            ...
            //  2. later on last control area dereference, each subsection is converted to dynamic and can be pruned/recreated individually without losing (or requiring) contiguous pool.
            ...
            ExtendedSubsection->u.SubsectionFlags.Protection = (unsigned)Segment->SegmentPteTemplate.u.Soft.Protection;
            ...
            // Note that since the next allocation in this loop may fail, the very first previous subsection changes are not rippled to the chained subsection until the loop completes successfully.
            if (LastExtendedSubsection == &ExtendedSubsectionHead) {
                ...
                // If the number of sectors doesn't completely fill the PTEs (only possible when the page size is not MM4K), then fill it now.
            ...
            // This subsection may be extending a range that is already mapped by a VAD(s).
            // There is no way to tell how many VADs already map it so just mark the entire subsection as not reclaimable until the control area is deleted.

            // This also saves other portions of code that issue "dereference from this subsection to the end of file" as these subsections are
            // marked as static not dynamic (at least until segment dereference time).

            // When this chain is appended to the control area at the end of this routine an attempt is made to convert the subsection chain to dynamic if no user mapped views are active.
        ...
        // All the subsections have been allocated, try to append the subsection chain without allocating prototype PTEs.
        // If user views are present, the append will fail, at which point we will attempt to allocate prototype PTEs and retry.
        Appended = MiAppendSubsectionChain((PMSUBSECTION)LastSubsection, &ExtendedSubsectionHead);
        if (Appended == FALSE) {
            RunningSize = 0;
            ...
            do {
                ...
                RunningSize += PartialSize;
                ProtoPtes = (PMMPTE)ExAllocatePoolWithTag(PagedPool | POOL_MM_ALLOCATION, PartialSize, MMSECT);
                if (ProtoPtes == NULL) {
                    goto ExtensionFailed;
                }
            ...
            // Note that any adjustments (NumberOfFullSectors, etc) made to the temp copy of the last subsection in the existing control
            // area must be *CAREFULLY* copied to the real copy in the chain (the entire structure cannot just be block copied) as other fields
            // in the real copy (ie: NumberOfMappedViews may be changed in parallel by another thread).
            Appended = MiAppendSubsectionChain((PMSUBSECTION)LastSubsection, &ExtendedSubsectionHead);
...
ExtensionFailed:
    ...
    // Reset the subsection and control area fields to their original values.
    LastSubsection->PtesInSubsection -= PtesUsed;
    LastSubsection->UnusedPtes += PtesUsed;
    TotalNumberOfPtes -= PtesUsed;
...
Arguments:
    Vad - Supplies a pointer to the virtual address descriptor which encompasses the virtual address.
...
*/
{
    ...
    while ((Subsection->SubsectionBase == NULL) || (Vad->FirstPrototypePte < Subsection->SubsectionBase) || (Vad->FirstPrototypePte >= &Subsection->SubsectionBase[Subsection->PtesInSubsection])) {
        Subsection = Subsection->NextSubsection;// Get the next subsection.
        if (Subsection == NULL) {
            return NULL;
        }
    }
...
*/
{
    ...
    // Locate the subsection which contains the First Prototype PTE for this VAD.
    // Note all the SubsectionBase values must be non-NULL for the subsection range spanned by the VAD because the VAD still exists.
    // Carefully skip over preceding subsections not mapped by this VAD because if no other VADs map them either, their base can be NULL.
    while ((Subsection->SubsectionBase == NULL) || (Vad->FirstPrototypePte < Subsection->SubsectionBase) || (Vad->FirstPrototypePte >= &Subsection->SubsectionBase[Subsection->PtesInSubsection])) {
        Subsection = Subsection->NextSubsection;// Get the next subsection.
        if (Subsection == NULL) {
            return NULL;
