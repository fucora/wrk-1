...
// Registry settable but must always be a page multiple and less than or equal to KERNEL_LARGE_STACK_SIZE.
...
// Ke support is not enabled for all platforms, so key Mm support off the Ke define (MAXIMUM_EXPANSION_SIZE) for the platforms that are supported.
...
PFN_NUMBER MiMakeOutswappedPageResident(
...
VOID MiDeletePteList(IN PMMPTE_DELETE_LIST PteDeleteList, IN PEPROCESS CurrentProcess);
...
VOID MiUpdateSystemPdes(IN PEPROCESS Process);
#endif
...
Arguments:
    MinimumWorkingSetSize - Supplies the minimum working set size for this address space.
                            This value is only used to ensure that ample physical pages exist to create this process.
...
Return Value:
    Returns TRUE if an address space was successfully created, FALSE if ample physical pages do not exist.
Environment:
    Kernel mode.
    APCs Disabled.
*/
{
    ...
    // Charge commitment for the page directory pages, working set page table page, and working set list.
    // If Vad bitmap lookups are enabled, then charge for a page or two for that as well.
    ...
    if (MI_NONPAGEABLE_MEMORY_AVAILABLE() <= (SPFN_NUMBER)MinimumWorkingSetSize) {
        ...
        return FALSE;// Indicate no directory base was allocated.
    }

    MM_TRACK_COMMIT(MM_DBG_COMMIT_PROCESS_CREATE, MM_PROCESS_COMMIT_CHARGE);
    MI_DECREMENT_RESIDENT_AVAILABLE(MinimumWorkingSetSize, MM_RESAVAIL_ALLOCATE_CREATE_PROCESS);
    ...
    if (Pfn1->u3.e1.CacheAttribute != MiCached) {
        MI_FLUSH_TB_FOR_INDIVIDUAL_ATTRIBUTE_CHANGE(PageDirectoryIndex, MiCached);
    ...
    PointerPxe = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageDirectoryIndex);
    ...
    // Now that the top level extended page parent page is initialized, allocate a page parent page.
    if (MmAvailablePages < MM_HIGH_LIMIT) {
        MiEnsureAvailablePageOrWait(NULL, OldIrql);
    }
    ...
    // Map this directory parent page into the top level extended page directory parent page.
    TempPte.u.Hard.PageFrameNumber = PageDirectoryIndex;
    PointerPxe = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageDirectoryParentIndex);
    ...
    if (Pfn1->u3.e1.CacheAttribute != MiCached) {
        MI_FLUSH_TB_FOR_INDIVIDUAL_ATTRIBUTE_CHANGE(HyperDirectoryIndex, MiCached);
        Pfn1->u3.e1.CacheAttribute = MiCached;
    }
...
#if (_MI_PAGING_LEVELS >= 4)
    PointerPpe = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageDirectoryIndex);
#endif
    ...
    if (Pfn1->u3.e1.CacheAttribute != MiCached) {
        MI_FLUSH_TB_FOR_INDIVIDUAL_ATTRIBUTE_CHANGE(HyperSpaceIndex, MiCached);
        Pfn1->u3.e1.CacheAttribute = MiCached;
    }

#if (_AMD64_)
    TempPte.u.Hard.PageFrameNumber = HyperSpaceIndex;
    PointerPde = MiMapPageInHyperSpaceAtDpc(CurrentProcess, HyperDirectoryIndex);
    ...
    if (Pfn1->u3.e1.CacheAttribute != MiCached) {
        MI_FLUSH_TB_FOR_INDIVIDUAL_ATTRIBUTE_CHANGE(VadBitMapPage, MiCached);
        Pfn1->u3.e1.CacheAttribute = MiCached;
    }
    ...
    if (Pfn1->u3.e1.CacheAttribute != MiCached) {
        MI_FLUSH_TB_FOR_INDIVIDUAL_ATTRIBUTE_CHANGE(PageContainingWorkingSet, MiCached);
        Pfn1->u3.e1.CacheAttribute = MiCached;
    }
    ...
    if (MappingPte != NULL) {
        MI_MAKE_VALID_KERNEL_PTE(TempPte2, PageDirectoryParentIndex, MM_READWRITE, MappingPte);
        MI_SET_PTE_DIRTY(TempPte2);
        MI_WRITE_VALID_PTE(MappingPte, TempPte2);
        PointerPte = MiGetVirtualAddressMappedByPte(MappingPte);
    } else {
        PointerPte = MiMapPageInHyperSpace(CurrentProcess, PageDirectoryParentIndex, &OldIrql);
    }

    PointerFillPte = &PointerPte[MiGetPxeOffset(KI_USER_SHARED_DATA)];
    RtlCopyMemory(PointerFillPte, CurrentAddressSpacePde, ((1 + (MiGetPxeAddress(MM_SYSTEM_SPACE_END) - CurrentAddressSpacePde)) * sizeof(MMPTE)));
    if (MappingPte != NULL) {
    ...
    MappingPte = MiReserveSystemPtes(1, SystemPteSpace);
    if (MappingPte != NULL) {
        MI_MAKE_VALID_KERNEL_PTE(TempPte2, HyperSpaceIndex, MM_READWRITE, MappingPte);
        MI_SET_PTE_DIRTY(TempPte2);
        MI_WRITE_VALID_PTE(MappingPte, TempPte2);
        PointerPte = MiGetVirtualAddressMappedByPte(MappingPte);
    } else {
...
/*
Routine Description:
    This routine initializes the working set and mutexes within a newly created address space to support paging.
    No page faults may occur in a new process until this routine has completed.
Arguments:
    ProcessToInitialize - Supplies a pointer to the process to initialize.
    ProcessToClone - Optionally supplies a pointer to the process whose address space should be copied into the ProcessToInitialize address space.
    SectionToMap - Optionally supplies a section to map into the newly initialized address space.

    Only one of ProcessToClone and SectionToMap may be specified.

    CreateFlags - Flags specifying varying parameters pertinent to process creation.
        Currently, only the PROCESS_CREATE_FLAGS_ALL_LARGE_PAGE_FLAGS are relevant to memory management.
        On output, these indicate to the caller whether or not the process image was successfully mapped with large pages.
...
*/
{
...
#if !defined(_WIN64)
    // Check whether our new process needs an update that occurred while we were filling its system PDEs.
    // If so, we must recopy here as the updater isn't able to tell where we were in the middle of our first copy.
    ASSERT(ProcessToInitialize->Pcb.DirectoryTableBase[0] != 0);

    LOCK_EXPANSION(OldIrql);
    if (ProcessToInitialize->PdeUpdateNeeded) {
        // Another thread updated the system PDE range while this process was being created.
        // Update the PDEs now (prior to attaching so if an interrupt occurs that accesses the mapping it will be correct).
        PS_CLEAR_BITS(&ProcessToInitialize->Flags, PS_PROCESS_FLAGS_PDE_UPDATE_NEEDED);
    ...
    // NOTE:  The process block has been zeroed when allocated, so there is no need to zero fields and set pointers to NULL.
    ...
    // Obtain a page to map the working set and initialize the working set.  Get the PFN lock to allocate physical pages.
    LOCK_PFN(OldIrql);

    // Initialize the PFN database for the Page Directory and the PDE which maps hyper space.
#if (_MI_PAGING_LEVELS >= 3)
    ...
    // The VAD bitmap spans one page when booted 2GB and the working set page follows it.
    // If booted 3GB, the VAD bitmap spans 1.5 pages and the working set list uses the last half of the second page.
    NumberOfPages = 2;
    ...
    if (MmTrackLockedPages == TRUE) {
        ...
        if (LockedPagesHeader != NULL) {
            ...
            // Note an explicit memory barrier is not needed here because we must detach from this process before the field can be accessed.
            // And any other processor would need to context swap to this process before the field could be accessed, so
    ...
    // 3gb of user address space and the process is not being cloned, then create a VAD for the shared memory page.

    // Always create a VAD for the shared memory page for 64-bit systems as clearly it always falls into the user address space there.

    // Only x86 booted without /3GB doesn't need the VAD
    // (because the shared memory page lies above the highest VAD the user can allocate so the user can never delete it).
    if ((MM_HIGHEST_VAD_ADDRESS > (PVOID)MM_SHARED_USER_DATA_VA) && (ProcessToClone == NULL)) {
        ...
        VadShare = MiAllocateVad(MM_SHARED_USER_DATA_VA, MM_SHARED_USER_DATA_VA + (X64K)-1, FALSE);
        if (VadShare == NULL) {
            KeDetachProcess();
            return STATUS_NO_MEMORY;
        }

        // If a section is being mapped and the executable is not large address space aware,
        // then create a VAD that reserves the address space between 2gb and the highest user address.
        if (SectionToMap != NULL) {
            ...
            if (ProcessToInitialize->Flags & PS_PROCESS_FLAGS_OVERRIDE_ADDRESS_SPACE) {
                ...
                if ((ImageInfo->ImageCharacteristics & IMAGE_FILE_LARGE_ADDRESS_AWARE) == 0) {
                    ...
                    // Provide 4 gigabytes of address space for wow64 apps that have the large address aware bit set.
            ...
            if (BaseAddress != NULL) {
                // Allocate a VAD to map the address space between 2gb and the highest user address.
                // If a VAD cannot be allocated, then deallocate the shared address space VAD, detach from the target process, and return a failure status.
                // This VAD is marked as not deletable.
                VadReserve = MiAllocateVad((ULONG_PTR)BaseAddress - WowLACompatRes, (ULONG_PTR)MM_HIGHEST_VAD_ADDRESS, FALSE);
                if (VadReserve == NULL) {
                    KeDetachProcess();
                    if (VadShare != NULL) {
                        ExFreePool(VadShare);
                ...
                if (ImageInfo->Machine == IMAGE_FILE_MACHINE_I386) {
                    ...
                    // Since the user can only access a limited amount of virtual address space,
                    // reduce the number of commit bits needed to span his page table pages.
...
#if (_MI_PAGING_LEVELS >= 3)
    // Allocate the commitment tracking bitmaps for page directory and page table pages.
    // This must be done before any non-MM_MAX_COMMIT VAD creations occur.
    ...
    MmWorkingSetList->CommittedPageTables = (PULONG)ExAllocatePoolWithTag(
        MmPagedPoolEnd != NULL ? PagedPool : NonPagedPool,
    ...
    RtlZeroMemory(MmWorkingSetList->CommittedPageDirectories, (MaximumUserPageDirectoryPages + 7) / 8);
#endif

    RtlZeroMemory(MmWorkingSetList->CommittedPageTables, (MaximumUserPageTablePages + 7) / 8);
    ...
    // If the registry indicates all applications should get virtual address ranges from the highest address downwards then enforce it now.
    // This makes it easy to test 3GB-aware apps on 32-bit machines as well as 64-bit apps on NT64.

    // Note this is only done if the image has the large-address-aware bit set because otherwise the compatibility VAD occupies the range from 2gb->3gb
    // and setting top-down by default can cause allocations like the stack trace database to displace kernel32 causing the process launch to fail.
    if ((MmAllocationPreference != 0) && ((VadReserve == NULL) || (BaseAddress > (PVOID)(ULONG_PTR)_2gb))) {
    ...
    if (ProcessToClone == NULL) {
        ...
        // This range (and pieces of it) can be unreserved by user mode code such as WOW64 or csrss.
        // Hence commit must be charged for the page directory and table pages.
        ASSERT(MiCheckForConflictingVad(ProcessToInitialize, WOW64_COMPATIBILITY_AREA_ADDRESS, MM_SHARED_USER_DATA_VA - 1) == NULL);
        VadShare = MiAllocateVad(WOW64_COMPATIBILITY_AREA_ADDRESS, MM_SHARED_USER_DATA_VA - 1, TRUE);
        if (VadShare == NULL) {
            KeDetachProcess();
            return STATUS_NO_MEMORY;
        }

        // Zero the commit charge so inserting the VAD will result in the proper charges being applied.
        // This way when it is split later, the correct commitment will be returned.

        // N.B.  The system process is not allocated with commit because paged pool and quotas don't exist at the point in Phase0 where this is called.
        ...
        // mutex must be held (as calls inside MiInsertVad to support routines to charge commit require it), failures can occur and must be handled.
        Status = MiInsertVadCharges(VadShare, ProcessToInitialize);
        if (!NT_SUCCESS(Status)) {
            // Note that any inserted VAD (ie: the VadReserve and Wow64 allocations) are automatically released on process destruction so there is no need to tear them down here.
            ExFreePool(VadShare);
            KeDetachProcess();
            return Status;
        }
    ...
    if (SectionToMap != NULL) {
        // Map the specified section into the address space of the process but only if it is an image section.
        if (!((PSECTION)SectionToMap)->u.Flags.Image) {
            ...
            *Dst = '\0';
            ...
            // The status of mapping the section is what must be returned unless the system DLL load fails.
            // This is because if the exe section is relocated (ie: STATUS_IMAGE_NOT_AT_BASE),
            // this must be returned (not STATUS_SUCCESS from the system DLL mapping).
            ProcessToInitialize->SectionBaseAddress = BaseAddress;
            if (NT_SUCCESS(Status)) {
                if ((*CreateFlags & PROCESS_CREATE_FLAGS_LARGE_PAGES) != 0) {
                    PMMPTE PointerPde;

                    // Determine whether or not the image was succesfully mapped with large pages.
                    // Note that no locks are required as no other thread can access this process,
                    // and no page in the process can be trimmed (the process is not yet on Mm's list of working sets).
                ...
                SystemDllStatus = PsMapSystemDll(ProcessToInitialize, NULL, FALSE);
    ...
    if (ProcessToClone != NULL) {
        ...
        // As the page directory and page tables are private to each process, the physical pages which map the directory page
        // and the page table usage must be mapped into system space so they can be updated while in the context of the process we are cloning.
#if defined(_WIN64)
        if (ProcessToClone->Wow64Process != NULL) {
            ...
            MappingPte = MiReserveSystemPtes(1, SystemPteSpace);
            if (MappingPte != NULL) {
                MI_MAKE_VALID_KERNEL_PTE(TempPte2, ProcessToClone->WorkingSetPage, MM_READWRITE, MappingPte);
                MI_SET_PTE_DIRTY(TempPte2);
                MI_WRITE_VALID_PTE(MappingPte, TempPte2);
                WorkingSetList = MiGetVirtualAddressMappedByPte(MappingPte);
            } else {
                WorkingSetList = MiMapPageInHyperSpace(ProcessToInitialize, ProcessToClone->WorkingSetPage, &OldIrql);
            }
            ...
            if (MappingPte != NULL) {
                ...
                MiUnmapPageInHyperSpace(ProcessToInitialize, WorkingSetList, OldIrql);
            }
        ...
        KeDetachProcess();
        Status = MiCloneProcessAddressSpace(ProcessToClone, ProcessToInitialize);
        MiAllowWorkingSetExpansion(&ProcessToInitialize->Vm);
    } else {// System Process.
        KeDetachProcess();
    ...
    // If this a cloned process or the system process, then large pages were not used.
    if (NT_SUCCESS(Status)) {
        *CreateFlags &= ~(PROCESS_CREATE_FLAGS_LARGE_PAGES);
    }
...
#if !defined (_WIN64)
VOID MiInsertHandBuiltProcessIntoList(IN PEPROCESS ProcessToInitialize)
/*
Routine Description:
...
NTSTATUS MmInitializeHandBuiltProcess(IN PEPROCESS ProcessToInitialize, OUT PULONG_PTR DirectoryTableBase)
/*
Routine Description:
    This routine initializes the working set mutex and address creation mutex for this "hand built" process.
    Normally the call to MmInitializeAddressSpace initializes the working set mutex.
    However, in this case, we have already initialized the address space and we are now creating a second process using the address space of the idle thread.
Arguments:
    ProcessToInitialize - Supplies a pointer to the process to initialize.
    DirectoryTableBase - Receives the pair of directory table base pointers.
Environment:
    Kernel mode.
    APCs disabled, idle process context.
*/
{
#if !defined(NT_UP)
    // On MP machines the idle & system process do not share a top level page directory because hyperspace mappings are protected by a per-process spinlock.
    // Having two processes share a single hyperspace (by virtue of sharing a top level page directory) would make the spinlock synchronization meaningless.

    // Note that it is completely illegal for the idle process to ever enter a wait state, but the code below should never encounter waits for mutexes, etc.
    return MmCreateProcessAddressSpace(0, ProcessToInitialize, DirectoryTableBase);
#else
    PEPROCESS CurrentProcess;
...
NTSTATUS MmInitializeHandBuiltProcess2(IN PEPROCESS ProcessToInitialize)
/*
Routine Description:
    This routine initializes the shared user VAD.
    This only needs to be done for NT64 (and x86 when booted /3GB) because on all other systems,
    the shared user address is located above the highest user address.
    For NT64 and x86 /3GB,
    this VAD must be allocated so that other random VAD allocations do not overlap this area which would cause the mapping to receive the wrong data.
Arguments:
...
*/
{
    ...
    // Having two processes share a single hyperspace (by virtue of sharing a top level page directory) would make the spinlock synchronization meaningless.
    DummyFlags = 0;
    Status = MmInitializeProcessAddressSpace(ProcessToInitialize, NULL, NULL, &DummyFlags, NULL);

#if defined(_X86_)
    if ((MmVirtualBias != 0) && (PsInitialSystemProcess == NULL) && (NT_SUCCESS(Status))) {
        KIRQL OldIrql;
        PMMPTE PointerPte;
        ...
        // When booted /3GB, the initial system mappings at 8xxxxxxx must be copied because things like the loader block contain pointers to
        // this area and are referenced by the system during early startup despite the fact that the rest of the system is biased correctly.
        CurrentProcess = PsGetCurrentProcess();
        ...
        MappingPte = MiReserveSystemPtes(1, SystemPteSpace);
        if (MappingPte != NULL) {
            MI_MAKE_VALID_KERNEL_PTE(TempPte2, PageFrameIndex, MM_READWRITE, MappingPte);
            MI_SET_PTE_DIRTY(TempPte2);
            MI_WRITE_VALID_PTE(MappingPte, TempPte2);
            PointerPte = MiGetVirtualAddressMappedByPte(MappingPte);
            OldIrql = MM_NOIRQL;
        } else {
            PointerPte = MiMapPageInHyperSpace(CurrentProcess, PageFrameIndex, &OldIrql);
        }

        PointerFillPte = &PointerPte[MiGetPdeOffset(CODE_START)];
        CurrentAddressSpacePde = MiGetPdeAddress((PVOID)CODE_START);
        RtlCopyMemory(PointerFillPte, CurrentAddressSpacePde, (((1 + CODE_END) - CODE_START) / MM_VA_MAPPED_BY_PDE) * sizeof(MMPTE));
    ...
    if ((MmTrackLockedPages == TRUE) && (NT_SUCCESS(Status))) {
        ...
        if (LockedPagesHeader != NULL) {
            ...
            // Note an explicit memory barrier is not needed here because we must detach from this process before the field can be accessed.
            // And any other processor would need to context swap to this process before the field could be accessed,
            // so the implicit memory barriers in context swap are sufficient.
            ProcessToInitialize->LockedPagesList = (PVOID)LockedPagesHeader;
        } else {
            Status = STATUS_NO_MEMORY;
        }
...
*/
{
    ...
    if (Process->AddressSpaceInitialized == 2) {
        ...
        // Map the hyper space page table page from the deleted process so the vad bit map page can be captured.
        PageFrameIndex = MI_GET_HYPER_PAGE_TABLE_FRAME_FROM_PROCESS(Process);
        PointerPte = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex);
        ...
        // Get a pointer to the top-level page directory parent page via its KSEG0 address.

#if (_MI_PAGING_LEVELS >= 4)
        ExtendedPageDirectoryParent = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex);

        // Remove the hyper space page directory parent page from the deleted process.
        ...
        PageDirectoryParent = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex3);
        ...
        // Process initialization never completed, just return the pages to the free list.
...
#if (_MI_PAGING_LEVELS >= 4)
        PageDirectoryParent = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex);
        ...
        PointerPte = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex2);
        VadBitMapPage = MI_GET_PAGE_FRAME_FROM_PTE(PointerPte + MiGetPteOffset(VAD_BITMAP_SPACE));
        MiUnmapPageInHyperSpaceFromDpc(CurrentProcess, PointerPte);
        MiInsertPageInFreeList(VadBitMapPage);
        MiInsertPageInFreeList(PageFrameIndex2);// Free the hyper space page table page.
        ...
        MiInsertPageInFreeList(PageFrameIndex);// Free the topmost page directory page.
    }
    UNLOCK_PFN(OldIrql);

    MI_INCREMENT_RESIDENT_AVAILABLE(MM_PROCESS_CREATE_CHARGE, MM_RESAVAIL_FREE_DELETE_PROCESS);

#if defined (_X86PAE_)
    ASSERT(PaeVa != &MiSystemPaeVa);
    MiPaeFree(PaeVa);// Free the page directory page pointers.
#endif

    if (Process->Session != NULL) {
        // The Terminal Server session space data page and mapping PTE can only be freed when the last process in the session is deleted.
        // This is because IA64 maps session space into region 1 and exited processes
        // maintain their session space mapping as attaches may occur even after process exit that reference win32k, etc.
        // Since the region 1 mapping is being inserted into region registers during swap context,
        // these mappings cannot be torn down until the very last deletion occurs.
        MiReleaseProcessReferenceToSessionDataPage(Process->Session);
    }

    MiContractPagingFiles();// Check to see if the paging files should be contracted.
}
...
/*
Routine Description:
    This routine deletes a range of PTEs and when possible, the PDEs, PPEs and PXEs as well.
    Commit is returned here for the hierarchies here.
Arguments:
    ...
    AddressSpaceDeletion - Supplies TRUE if the address space is in the final stages of deletion, FALSE otherwise.
Environment:
...
*/
{
    ...
    if ((PointerPxe->u.Hard.Valid == 1) && (PointerPpe->u.Hard.Valid == 1) && (PointerPde->u.Hard.Valid == 1) && (PointerPte->u.Hard.Valid == 1))
#else
    if ((PointerPpe->u.Hard.Valid == 1) && (PointerPde->u.Hard.Valid == 1) && (PointerPte->u.Hard.Valid == 1))
#endif
#else
    if ((PointerPde->u.Hard.Valid == 1) && (PointerPte->u.Hard.Valid == 1))
#endif
    {
        do {
            ...
            if (Process != NULL) {
                MiDeletePte(PointerPte, TempVa, AddressSpaceDeletion, Process, NULL, &PteFlushList, OldIrql);
                Process->NumberOfPrivatePages += 1;
            } else {
                MiDeleteValidSystemPte(PointerPte, TempVa, WsInfo, &PteFlushList);
            }

            CommittedPages += 1;
            PointerPte += 1;

            // If all the entries have been removed from the previous page table page, delete the page table page itself.
            // Likewise with the page directory page.
            ...
            if ((Boundary == TRUE) || (FinalPte == TRUE)) {
                if (PteFlushList.Count == 0) {
                    ...
                    MI_FLUSH_MULTIPLE_TB(PteFlushList.Count, &PteFlushList.FlushVa[0], AllProcessors);
                } else {
                    if (AllProcessors) {
                ...
                if ((Pfn1->u2.ShareCount == 1) && (Pfn1->u3.e2.ReferenceCount == 1) && (Pfn1->u1.WsIndex != 0)) {
                    if (Process != NULL) {
                        MiDeletePte(PointerPde, PointerPte - 1, AddressSpaceDeletion, Process, NULL, NULL, OldIrql);
                        Process->NumberOfPrivatePages += 1;
                    } else {
                        MiDeleteValidSystemPte(PointerPde, PointerPte - 1, WsInfo, &PteFlushList);
                    }
                    ...
                    if ((FinalPte == TRUE) || (MiIsPteOnPpeBoundary(PointerPte))) {
                        ...
                        if (Pfn1->u2.ShareCount == 1 && Pfn1->u3.e2.ReferenceCount == 1) {
                            if (Process != NULL) {
                                MiDeletePte(PointerPpe, PointerPde, AddressSpaceDeletion, Process, NULL, NULL, OldIrql);
                                Process->NumberOfPrivatePages += 1;
                            } else {
                                MiDeleteValidSystemPte(PointerPpe, PointerPde, WsInfo, &PteFlushList);
                            }
                            ...
                            if ((FinalPte == TRUE) || (MiIsPteOnPxeBoundary(PointerPte))) {
                                ...
                                if (Pfn1->u2.ShareCount == 1 && Pfn1->u3.e2.ReferenceCount == 1) {
                                    if (Process != NULL) {
                                        MiDeletePte(PointerPxe, PointerPpe, AddressSpaceDeletion, Process, NULL, NULL, OldIrql);
                                        Process->NumberOfPrivatePages += 1;
                                    } else {
                                        MiDeleteValidSystemPte(PointerPxe, PointerPpe, WsInfo, &PteFlushList);
                                    }
                                    CommittedPages += 1;
                                }
                            }
...
VOID MiUnlinkWorkingSet(IN PMMSUPPORT WsInfo)
/*
Routine Description:
    This routine removes the argument working set from the working set manager's linked list.
Arguments:
...
*/
{
    ...
    // If working set expansion for this process is allowed, disable it and remove the process from expanded process list if it is on it.
    LOCK_EXPANSION(OldIrql);

    if (WsInfo->WorkingSetExpansionLinks.Flink == MM_WS_TRIMMING) {
        // Initialize an event and put the event address in the blink field.
        ...
        // This process' working set is in an initialization state and has never been inserted into any lists.
        UNLOCK_EXPANSION(OldIrql);
    } else {
...
/*
Routine Description:
    This routine cleans an address space by deleting all the user and pageable portions of the address space.
    At the completion of this routine, no page faults may occur within the process.
Environment:
    Kernel mode, APCs disabled.
*/
{
    ...
    if ((Process->Flags & PS_PROCESS_FLAGS_VM_DELETED) || (Process->AddressSpaceInitialized == 0)) {
        ...
        // However, this process can still have a session space.  Get rid of it now.
    ...
    if (Process->AddressSpaceInitialized == 1) {
        ...
        MI_INCREMENT_RESIDENT_AVAILABLE(Process->Vm.MinimumWorkingSetSize - MM_PROCESS_CREATE_CHARGE, MM_RESAVAIL_FREE_CLEAN_PROCESS1);

        // Clear the AddressSpaceInitialized flag so we don't over-return resident available as this routine can be called more than once for the same process.
    ...
    // Both mutexes must be owned to synchronize with the bit setting and clearing of VM_DELETED.
    // This is because various callers acquire only one of them (either one) before checking.
    ...
    // At this point page faults are allowed on user space addresses but are generally rare (would have to be an attached thread doing the references).
    // Faults are allowed on page tables for user space, which requires that we keep the working set structure consistent until we finally take it all down.
    MiDeleteAddressesInWorkingSet(Process);

    // Remove hash table pages, if any.
    // This is the first time we do this during the deletion path,
    // but we need to do it again before we finish because we may fault in some page tables during the VAD clearing.
    // We could have maintained the hash table validity during the WorkingSet
    // deletion above in order to avoid freeing the hash table twice, but since we're just deleting it all anyway, it's faster to do it this way.
    // Note that if we don't do this or maintain the validity, we can trap later in MiGrowWsleHash.
    LastPte = MiGetPteAddress(MmWorkingSetList->HighestPermittedHashAddress);
    MiDeletePteRange(&Process->Vm, PointerPte, LastPte, FALSE);

    // Clear the hash fields as a fault may occur below on the page table pages during VAD clearing and resolution of the fault may result in adding a hash table.
    // Thus these fields must be consistent with the clearing just done above.
    MmWorkingSetList->HashTableSize = 0;
    MmWorkingSetList->HashTable = NULL;

    UNLOCK_WS_UNSAFE(Thread, Process)

        // Delete the virtual address descriptors and dereference any section objects.
        while (Process->VadRoot.NumberGenericTableElements != 0) {
            ...
            // If the system is NT64 (or NT32 and has been biased to an alternate base address to allow 3gb of user address space),
            ...
            if (((Vad->u.VadFlags.PrivateMemory == 0) && (Vad->ControlArea != NULL)) || (Vad->u.VadFlags.VadType == VadDevicePhysicalMemory)) {
                // This VAD represents a mapped view or a driver-mapped physical view - delete the view and perform any section related cleanup operations.
                if (Vad->u.VadFlags.VadType == VadRotatePhysical) {
                    MiPhysicalViewRemover(Process, Vad);
                }

                // NOTE: MiRemoveMappedView releases the working set pushlock before returning.
                ...
                if (Vad->u.VadFlags.VadType == VadLargePages) {
                    ...
                    MiDeleteVirtualAddresses(MI_VPN_TO_VA(Vad->StartingVpn), MI_VPN_TO_VA_ENDING(Vad->EndingVpn), Vad);
                }
    ...
    // Note this deliberately compares the highest user address instead of the highest VAD address.
    // This is because we must always delete the link to the physical page even on platforms where the VAD was not allocated.
    // The only exception to this is when we're booted on x86 with /USERVA=1nnn to increase the kernel address space beyond 2GB.
    if (MmHighestUserAddress > (PVOID)MM_SHARED_USER_DATA_VA) {
        // This zeroes the PTE - so you would think after this, it can still be faulted on and re-evaluated directly via MiCheckVirtualAddress
        // even though it has no VAD - however this cannot happen because during this deletion,
        // the containing page table page will be deleted and this cannot be replaced without a VAD, thus short circuiting the re-evaluation case above.
        MiDeleteVirtualAddresses((PVOID)MM_SHARED_USER_DATA_VA, (PVOID)MM_SHARED_USER_DATA_VA, NULL);
    }

    // Adjust the count of pages above working set maximum.
    // This must be done here because the working set list is not updated during this deletion.
    ...
    // Note that for NT64, we allocate the arrays *AFTER* the initial MM_MAX_COMMIT VADs are created
    // because we detect 32 bit processes and allocate smaller arrays for those since they have less accessible user virtual address space.
    ...
    ASSERT(Process->CloneRoot == NULL);// Make sure all the clone descriptors went away.
    ...
    if (Process->NumberOfLockedPages != 0) {
        if (LockedPagesHeader != NULL) {
            if ((LockedPagesHeader->Count != 0) && (LockedPagesHeader->Valid == TRUE)) {
            ...
            if ((KdDebuggerEnabled) && (KdDebuggerNotPresent == FALSE)) {
                DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "A driver has leaked %d bytes of physical memory.\n", Process->NumberOfLockedPages << PAGE_SHIFT);
                DbgBreakPoint();// Pop into the debugger (even on free builds) to determine the cause of the leak and march on.
            }
    ...
    if (LockedPagesHeader != NULL) {
        // No need to acquire the spinlock to traverse here as the pages are (unfortunately) never going to be unlocked.
        // Since this routine is pageable, this removes the need to add a nonpaged stub routine to do the traverses and frees.
        NextEntry = LockedPagesHeader->ListHead.Flink;
    ...
    if ((Process->NumberOfPrivatePages != 0) && (MmDebug & MM_DBG_PRIVATE_PAGES)) {
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "MM: Process contains private pages %ld\n", Process->NumberOfPrivatePages);
        DbgBreakPoint();
    }
#endif
    ...
    // Remove hash table pages, if any.
    // Yes, we've already done this once during the deletion path,
    // but we need to do it again because we may have faulted in some page tables during the VAD clearing.
    PointerPte = MiGetPteAddress(&MmWsle[MM_MAXIMUM_WORKING_SET]) + 1;
    ...
    // The first page is not removed because an attached thread could still reference the shared user data page and since no VAD is required for this translation,
    // the fault code will materialize a page table page(s) and access the working set list to add entries for it (and its page table pages) to the working set list.
    MiRemoveWorkingSetPages(&Process->Vm);

    // Update the count of available resident pages.
    MI_INCREMENT_RESIDENT_AVAILABLE(Process->Vm.MinimumWorkingSetSize - MM_PROCESS_CREATE_CHARGE, MM_RESAVAIL_FREE_CLEAN_PROCESS2);
...
PVOID MmCreateKernelStack(IN BOOLEAN LargeStack, IN UCHAR PreferredNode)
/*
Routine Description:
    This routine allocates a kernel stack and a no-access page within the non-pageable portion of the system address space.
Arguments:
    LargeStack - Supplies the value TRUE if a large stack should be created.  FALSE if a small stack is to be created.
    PreferredNode - Supplies the preferred node to use for the physical page allocations.  MP/NUMA systems only.
Return Value:
    Returns a pointer to the base of the kernel stack.
    Note, that the base address points to the guard page, so space must be allocated on the stack before accessing the stack.
...
*/
{
    ...
    if (MiChargeCommitment(ChargedPtes, NULL) == FALSE) {
        return NULL;// Commitment exceeded, return NULL, indicating no kernel stacks are available.
    }

    // Obtain enough pages to contain the stack plus a guard page from the system PTE pool.
    // The system PTE pool contains nonpaged PTEs which are currently empty.
    ...
    MI_MAKE_VALID_KERNEL_PTE(TempPte, 0, MM_READWRITE, (PointerPte + 1));
    MI_SET_PTE_DIRTY(TempPte);
    ...
    MI_DECREMENT_RESIDENT_AVAILABLE(NumberOfPages, MM_RESAVAIL_ALLOCATE_CREATE_STACK);
...
/*
Routine Description:
    This routine deletes a kernel stack and the no-access page within the non-pageable portion of the system address space.
Arguments:
...
*/
{
    ...
    // PointerPte points to the guard page, point to the previous page before removing physical pages.
    PointerPte -= 1;

    // Check to see if the stack page should be placed on the dead kernel stack page list.
    // The dead kernel stack list is a singly linked list of kernel stacks from terminated threads.
    // The stacks are saved on a linked list up to a maximum number to avoid the overhead of flushing the entire TB on all processors everytime a thread terminates.
    // The TB on all processors must be flushed as kernel stacks reside in the non paged system part of the address space.
    if (!LargeStack) {
#if defined(MI_MULTINODE)
        // Scan the physical page frames and only place this stack on the dead stack list if all the pages are on the same node.
        // Realize if this push goes cross node it may make the interlocked instruction slightly more expensive, but worth it all things considered.
        ULONG NodeNumber;
        ...
        if (ExQueryDepthSList(DeadStackList) < MmMaximumDeadKernelStacks) {
#if defined(MI_MULTINODE)
            // The node could use some more dead stacks - but first make sure all the physical pages are from the same node in a multinode system.
            if (KeNumberNodes > 1) {
                ULONG CheckPtes;
            ...
            InterlockedPushEntrySList(DeadStackList, (PSLIST_ENTRY)&Pfn1->u1.NextStackPfn);
            return;
        }
    } else {
    ...
    FreeStack:
             ...
             // We have exceeded the limit of dead kernel stacks or this is a large stack, delete this kernel stack.
             NumberOfPages = 0;
             ...
             for (i = 0; i < NumberOfPtes; i += 1) {
                 ...
                 if (PteContents.u.Hard.Valid == 1) {
                     ...
                     // Mark the page as deleted so it will be freed when the reference count goes to zero.
                     MI_SET_PFN_DELETED(Pfn1);
                     MiDecrementShareCount(Pfn1, PageFrameIndex);
                     NumberOfPages += 1;
                 }
             ...
             ASSERT(PointerPte->u.Hard.Valid == 0);// Now at the stack guard page, ensure it is still a guard page.
             UNLOCK_PFN(OldIrql);
             ...
             MI_INCREMENT_RESIDENT_AVAILABLE(NumberOfPages, MM_RESAVAIL_FREE_DELETE_STACK);
...
NTSTATUS MmGrowKernelStack(__in PVOID CurrentStack)
/*
Routine Description:
    This function attempts to grows the current thread's kernel stack such that there is always KERNEL_LARGE_STACK_COMMIT bytes below the current stack pointer.
Arguments:
...
NTSTATUS MmGrowKernelStackEx(__in PVOID CurrentStack, __in SIZE_T CommitSize)
/*
Routine Description:
    This function attempts to grows the current thread's kernel stack such that there is the specified number of bytes commited below the specified stack pointer.
Arguments:
    CurrentStack - Supplies a pointer to the current stack pointer.
...
*/
{
    ...
    // If the new stack limit exceeds the reserved region for the kernel stack, then return an error.
    EndStack = MiGetPteAddress((PVOID)((PUCHAR)Thread->StackBase - MI_LARGE_STACK_SIZE));
    if (NewLimit < EndStack) {
        // Don't go into guard page.
        MiStackGrowthFailures[0] += 1;
#if DBG
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "MmGrowKernelStack failed: Thread %p %p %p\n", Thread, NewLimit, EndStack);
#endif
    ...
    // Note MmResidentAvailablePages must be charged before calling MiEnsureAvailablePageOrWait as it may release the PFN lock.
    MI_DECREMENT_RESIDENT_AVAILABLE(NumberOfPages, MM_RESAVAIL_ALLOCATE_GROW_STACK);
    while (StackLimit >= NewLimit) {
        ...
        MI_MAKE_VALID_KERNEL_PTE(TempPte, PageFrameIndex, MM_READWRITE, StackLimit);
        MI_SET_PTE_DIRTY(TempPte);
        *StackLimit = TempPte;
        StackLimit -= 1;
    }
...
VOID MiOutPageSingleKernelStack(IN PKTHREAD Thread, IN PKERNEL_STACK_SEGMENT StackInfo, IN PMMPTE_FLUSH_LIST PteFlushList)
/*
Routine Description:
    This routine makes the specified kernel stack non-resident and puts the pages on the transition list.
    Note that pages below the CurrentStackPointer are not useful and these pages are freed here.
Arguments:
    ...
    StackInfo - Supplies a pointer to the relevant stack information :
                    StackBase - Supplies the highest virtual address of the stack.  This is where the stack begins.
                    KernelStack - Supplies the current stack pointer location.
                    StackLimit - Supplies the lowest committed virtual address in the stack.
                    ActualLimit - Supplies the lowest possible virtual address in the stack.
    PteFlushList - Supplies a flush list to use so the TB flush can be deferred to the caller.
Environment:
    Kernel mode.
*/
{
    ...
    ASSERT(((PCHAR)StackInfo->StackBase - (PCHAR)StackInfo->StackLimit) <= ((LONG)MI_LARGE_STACK_SIZE + PAGE_SIZE));
    ...
    if (MI_STACK_IS_TRIMMABLE(Thread)) {
        ...
        // This is a large stack.  The stack pagein won't necessarily bring back all the pages.
    ...
    *((PULONG_PTR)StackInfo->KernelStack - 1) = (ULONG_PTR)Thread;
    ...
    while (PointerPte != EndOfStackPte) {
        ...
        // Return resident available for pages beyond the guaranteed portion as an explicit call to grow the kernel stack will be needed to get these pages back.
    ...
    if (ResAvailToReturn != 0) {
        MI_INCREMENT_RESIDENT_AVAILABLE(ResAvailToReturn, MM_RESAVAIL_FREE_OUTPAGE_STACK);
        ResAvailToReturn = 0;
    }
...
*/
{
    ...
    if (PteFlushList.Count == 1) {
        ...
        MI_FLUSH_MULTIPLE_TB(PteFlushList.Count, &PteFlushList.FlushVa[0], TRUE);
    } else {
...
VOID MiInPageSingleKernelStack(IN PKTHREAD Thread, IN PKERNEL_STACK_SEGMENT StackInfo)
/*
Routine Description:
...
*/
{
    ...
    if (MI_STACK_IS_TRIMMABLE(Thread)) {
        ...
        // Trim back the stack.  Make sure that the stack does not grow, i.e.  StackLimit remains the limit.
    ...
    LOCK_PFN(OldIrql);
    while (PointerPte >= EndOfStackPte) {
        if (!((PointerPte->u.Long == KernelDemandZeroPte.u.Long) || (PointerPte->u.Soft.Protection == MM_NOACCESS))) {
        ...
        MiMakeOutswappedPageResident(PointerPte, PointerPte, 1, ContainingPage, OldIrql);
        PointerPte -= 1;
        NumberOfPages += 1;
    }
    ...
    if (*((PULONG_PTR)StackInfo->KernelStack - 1) != (ULONG_PTR)Thread) {
        KeBugCheckEx(KERNEL_STACK_INPAGE_ERROR, DiskRead, *((PULONG_PTR)StackInfo->KernelStack - 1), 0, (ULONG_PTR)StackInfo->KernelStack);
    }
    UNLOCK_PFN(OldIrql);
...
*/
{
    ...
    // Construct a local StackInfo structure and make the pertinent stack resident so that any others can be walked to.
    // Note ActualLimit is only set in Current, and InitialStack in Previous.
    // StackLimit is the lowest committed address.
    // KernelStack is the lowest referenced virtual address.
    // StackBase is the highest virtual address.
    // ActualLimit is the lowest possible virtual address but this field in the local StackInfo is not filled in here.
    ...
    // And the StackInfo itself is not resident.  So this field cannot be referenced until *AFTER* the stack has been made resident.
    LocalStackInfo.StackBase = (ULONG_PTR)Thread->StackBase;
    LocalStackInfo.StackLimit = (ULONG_PTR)Thread->StackLimit;
    LocalStackInfo.KernelStack = (ULONG_PTR)Thread->KernelStack;
...
#if defined (_MI_CHAINED_STACKS)
    // Note constructing the local StackInfo (instead of calling KeGetFirstKernelStackSegment) was necessary because the thread's initial stack VA is not resident on entry to this function.
    StackInfo = KeGetFirstKernelStackSegment(Thread);
    ...
    do {
        ...
        // KeGetNextKernelStackSegment (StackInfo) cannot be used because it reaches into the next segment which is currently paged out.
...
#if (_MI_PAGING_LEVELS >= 3)
PFN_NUMBER MiTrimPageParents(IN PEPROCESS OutProcess, IN PFN_NUMBER PdePage, IN PMMPTE OutTempPte)
{
    PFN_NUMBER TopPage;
    KIRQL OldIrql;
    MMPTE TempPte;
    ...
    // OutProcess really is referenced on checked or non-AMD builds, but it's not worth ifdefing this just for the compiler.
    ...
    if (MappingPte > (PMMPTE)1) {
        MI_MAKE_VALID_KERNEL_PTE(TempPte2, PpePage, MM_READWRITE, MappingPte);
        MI_SET_PTE_DIRTY(TempPte2);
        MI_WRITE_VALID_PTE(MappingPte, TempPte2);
    ...
    if (MappingPte == (PMMPTE)1) {
        NOTHING;// Nothing needs to be done if a KSEG3 mapping was used ...
...
#if (_MI_PAGING_LEVELS >= 4)
    // Remove the page directory parent page.  Then remove the top level extended page directory parent page.
    ...
    MappingPte = MiReserveSystemPtes(1, SystemPteSpace);
    if (MappingPte != NULL) {
        MI_MAKE_VALID_KERNEL_PTE(TempPte2, PxePage, MM_READWRITE, MappingPte);
        MI_SET_PTE_DIRTY(TempPte2);
        MI_WRITE_VALID_PTE(MappingPte, TempPte2);
        PageDirectoryMap = MiGetVirtualAddressMappedByPte(MappingPte);
    } else {
...
*/
{
    ...
    // The process cannot be fully swapped out unless its working set has been reduced to the bare minimum.
    // If the working set is larger than this, then just return as the working set manager thread will trim it down.
    // Eventually when it is trimmed to the bare minimum, the working set manager thread will detach from the process and if it is the last
    // thread (ie: all the others are in usermode waits),
    // then another process outswap request will occur for this process at which time we will be able to proceed below.
    if (OutProcess->Vm.WorkingSetSize == MM_PROCESS_COMMIT_CHARGE) {
        ...
        if (OutProcess->Vm.WorkingSetExpansionLinks.Flink == MM_WS_TRIMMING) {
            // An outswap is not allowed at this point because the process has been attached to and is being trimmed.
            UNLOCK_EXPANSION(OldIrql);
            return;
        }

        // Swap the process working set info and page parent/directory/table pages from memory.
        ...
        // Note the PFN lock does not need to be held to put the PTEs for these pages into transition because no one can be referencing these addresses in the context of the process being outswapped.
        HyperSpacePageTable = MI_GET_HYPER_PAGE_TABLE_FRAME_FROM_PROCESS(OutProcess);
        HyperSpacePageTableMap = NULL;
        MappingPte = MiReserveSystemPtes(1, SystemPteSpace);
        if (MappingPte > (PMMPTE)1) {
            MI_MAKE_VALID_KERNEL_PTE(TempPte3, HyperSpacePageTable, MM_READWRITE, MappingPte);
            ...
            HyperSpacePageTableMap = MiMapPageInHyperSpace(CurrentProcess, HyperSpacePageTable, &OldIrql);
        }
        ...
        // Capture all the PFN information and then get the PFN lock and actually move all the pages onto the modified list.
        Pfn1 = MI_PFN_ELEMENT(VadBitMapPage);
        ...
        MappingPte = MiReserveSystemPtes(1, SystemPteSpace);
        if (MappingPte > (PMMPTE)1) {
            MI_MAKE_VALID_KERNEL_PTE(TempPte3, PdePage, MM_READWRITE, MappingPte);
            MI_SET_PTE_DIRTY(TempPte3);
            MI_WRITE_VALID_PTE(MappingPte, TempPte3);
            PageDirectoryMap = MiGetVirtualAddressMappedByPte(MappingPte);
        } else {
        ...
        // Now acquire the PFN lock and free all the pages whose PTEs we put in transition above.
        LOCK_PFN(OldIrql);
        ASSERT(Pfn1->u3.e1.Modified == 1);
        MiDecrementShareCount(Pfn1, VadBitMapPage);
        ...
        // Decrement share count so the top level page directory page gets removed.
        // This can cause the PteCount to equal the sharecount as the page directory page no longer contains itself, yet can have itself as a transition page.
        ...
        Pfn1->u2.ShareCount += 1;// Increment the share count for the process page.
        UNLOCK_PFN(OldIrql);
...
*/
{
    ...
    if (OutProcess->Flags & PS_PROCESS_FLAGS_OUTSWAPPED) {
...
#if (_MI_PAGING_LEVELS >= 4)
        // Only the extended page directory parent page has really been read in above.
        //Read in the page directory parent page now.
        ...
        PageDirectoryPage = MiMakeOutswappedPageResident(MiGetPxeAddress(MmWorkingSetList), &TempPte, 0, PdePage, OldIrql);
...
#if (_MI_PAGING_LEVELS >= 3)
        // Only the page directory parent page has really been read in above (and the extended page directory parent for 4-level architectures).
        // Read in the page directory page now.
        PageDirectoryParentMap = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PdePage);
        TempPte = PageDirectoryParentMap[MiGetPpeOffset(MmWorkingSetList)];
        MiUnmapPageInHyperSpaceFromDpc(CurrentProcess, PageDirectoryParentMap);
        PageDirectoryPage = MiMakeOutswappedPageResident(MiGetPpeAddress(MmWorkingSetList), &TempPte, 0, PdePage, OldIrql);
        ASSERT(PageDirectoryPage == TempPte.u.Hard.PageFrameNumber);
        PageDirectoryParentMap = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PdePage);
        ...
        for (i = 0; i < PD_PER_SYSTEM - 1; i += 1) {
            MiMakeOutswappedPageResident(MiGetPteAddress(PDE_BASE + (i << PAGE_SHIFT)), &PageDirectoryPtes[i], 0, PdePage, OldIrql);
        ...
        TempPte = PageDirectoryMap[MiGetPdeOffset(MmWorkingSetList)];
        MiUnmapPageInHyperSpaceFromDpc(CurrentProcess, PageDirectoryMap);
        HyperSpacePageTable = MiMakeOutswappedPageResident(MiGetPdeAddress(HYPER_SPACE), &TempPte, 0, PdePage, OldIrql);
        ASSERT(Pfn1->u2.ShareCount >= 3);
        PageDirectoryMap = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PdePage);
        ...
        // Map in the hyper space page table page and retrieve the PTEs that map the working set list and VAD bitmap.
        // Note that although both PTEs lie in the same page table page, they must
        // be retrieved separately because: the Vad PTE may indicate its page is in a paging file and the WSL PTE may indicate its PTE is in transition.
        // The VAD page inswap may take the WSL page from the transition list - CHANGING the WSL PTE !  So the WSL PTE cannot be captured until after the VAD inswap completes.
        ...
        VadBitMapPage = MiMakeOutswappedPageResident(VadBitMapPte, &VadBitMapPteContents, 0, HyperSpacePageTable, OldIrql);

        // Read in the working set list page.
        HyperSpacePageTableMap = MiMapPageInHyperSpaceAtDpc(CurrentProcess, HyperSpacePageTable);
        TempPte = HyperSpacePageTableMap[WorkingSetListPteOffset];
        MiUnmapPageInHyperSpaceFromDpc(CurrentProcess, HyperSpacePageTableMap);
        WorkingSetPage = MiMakeOutswappedPageResident(WorkingSetListPte, &TempPte, 0, HyperSpacePageTable, OldIrql);

        // Update the PTEs, this can be done together for PTEs that lie within the same page table page.
        HyperSpacePageTableMap = MiMapPageInHyperSpaceAtDpc(CurrentProcess, HyperSpacePageTable);
        ...
        if (OutProcess->PdeUpdateNeeded) {
            // Another thread updated the system PDE range while this process was outswapped.  Update the PDEs now.
            PS_CLEAR_BITS(&OutProcess->Flags, PS_PROCESS_FLAGS_PDE_UPDATE_NEEDED);
...
Arguments:
    ...
    Base - Supplies a pointer to place the PEB/TEB virtual address on success. This has no meaning if success is not returned.
Environment:
...
*/
{
    ...
    // Allocate and initialize the Vad before acquiring the address space and working set mutexes so as to minimize mutex hold duration.
    ...
    // creating or deleting address space at the same time and get the working set mutex so virtual address descriptors can be inserted and walked.
    ...
    if (Size == sizeof(PEB)
        ...
        if (MiCheckForConflictingVadExistence(TargetProcess, HighestVadAddress, (PVOID)((PCHAR)HighestVadAddress + ROUND_TO_PAGES(Size) - 1)) == FALSE) {
            *Base = HighestVadAddress;
            goto AllocatedAddress;// Got an address ...
        }
    }

    Status = MiFindEmptyAddressRangeDown(&TargetProcess->VadRoot, ROUND_TO_PAGES(Size), ((PCHAR)MM_HIGHEST_VAD_ADDRESS + 1), PAGE_SIZE, Base);
    if (!NT_SUCCESS(Status)) {
...
AllocatedAddress:
    // An unoccupied address range has been found, finish initializing the virtual address descriptor to describe this range.
...
NTSTATUS MmCreateTeb(IN PEPROCESS TargetProcess, IN PINITIAL_TEB InitialTeb, IN PCLIENT_ID ClientId, OUT PTEB *Base)
/*
Routine Description:
    This routine creates a TEB page within the target process and copies the initial TEB values into it.
Arguments:
    ...
    Base - Supplies a location to return the base of the newly created TEB on success.
Environment:
    Kernel mode.
*/
{
    ...
    TebSize = sizeof(TEB);// Get the size of the TEB
    ...
    if (Wow64Process != NULL) {
        ...
        // Capture the 32-bit InitialTeb if the target thread's process is a Wow64 process, and the creating the thread is running inside a Wow64 process as well.
        if (PsGetCurrentProcess()->Wow64Process != NULL) {
            try {
                InitialTeb32Ptr = Wow64GetInitialTeb32();
                if (InitialTeb32Ptr != NULL) {
                    ProbeForReadSmallStructure(InitialTeb32Ptr, sizeof(InitialTeb32), PROBE_ALIGNMENT(INITIAL_TEB));
                    RtlCopyMemory(&InitialTeb32, InitialTeb32Ptr, sizeof(InitialTeb32));
    ...
    KeAttachProcess(&TargetProcess->Pcb);// Attach to the specified process.
    ...
    // if no address space is available for the TEB or the user has exceeded quota (non-paged, pagefile, commit) or
    // the TEB is paged out and an inpage error occurs when fetching it.

    // Note that since the TEB is populated with demand zero pages, only nonzero fields need to be initialized here.
    try {
#if !defined(_WIN64)
        TebBase->NtTib.ExceptionList = EXCEPTION_CHAIN_END;
#endif

        // Although various fields must be zero for the process to launch properly, don't assert them as an ordinary user could provoke these
        // by maliciously writing over random addresses in another thread, hoping to nail a just-being-created TEB.
        ...
        TebBase->ReservedForPerf = BBTBuffer;// Used for BBT of ntdll and kernel32.dll.
    ...
    {
        Status = GetExceptionCode();// An exception has occurred, inform our caller.
    }
...
// This code is built twice on the Win64 build - once for PE32+ and once for PE32 images.
...
#if defined(_WIN64)
NTSTATUS MiInitializeWowPeb(IN PIMAGE_NT_HEADERS NtHeaders, IN PPEB PebBase, IN PEPROCESS TargetProcess)
/*
Routine Description:
    This routine creates a PEB32 page within the target process and copies the initial PEB32 values into it.
Arguments:
...
*/
{
    ...
    // All references to the Peb and NtHeaders must be wrapped in try-except in case the user has exceeded quota (non-paged, pagefile, commit) or any inpage errors happen for the user addresses, etc.
    try {
    ...
    // Inspect the address space to determine if the executable image is mapped with a single copy on write subsection (ie: as data) or
    // if the alignment was such that it was mapped as a full image section.
    LOCK_ADDRESS_SPACE(TargetProcess);
    ...
    if (Vad->u.VadFlags.PrivateMemory == 0) {
        ...
        if ((ControlArea->u.Flags.Image == 1) && (ControlArea->Segment->SegmentFlags.ExtraSharedWowSubsections == 0)) {
            ...
            // Real images always have a subsection for the PE header and then at least one other subsection for the other sections in the image.
    ...
    try {
        ImageConfigData32 = RtlImageDirectoryEntryToData(PebBase->ImageBaseAddress, MappedAsImage, IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG, &ReturnedSize);
    ...
    try {
        ...
        // Leave the AffinityMask in the 32bit PEB as zero and let the 64bit NTDLL set the initial mask.
        // This is to allow the round robin scheduling of non MP safe imaging in the caller to work correctly.

        // Later code will set the affinity mask in the PEB32 if the image actually specifies one.

        // Note that the AffinityMask in the PEB is simply a mechanism to pass affinity information from the image to the loader.

        // Pass the affinity mask up to the 32 bit NTDLL via the PEB32.
        // The 32 bit NTDLL will determine that the affinity is not zero and try to set the affinity mask from user-mode.
        // This call will be intercepted by the wow64 thunks which will convert it into a 64bit affinity mask and call the kernel.
...
NTSTATUS MmCreatePeb(IN PEPROCESS TargetProcess, IN PINITIAL_PEB InitialPeb, OUT PPEB *Base)
/*
Routine Description:
    This routine creates a PEB page within the target process and copies the initial PEB values into it.
Arguments:
    ...
    Base - Supplies a location to return the base of the newly created PEB on success.
Environment:
    Kernel mode.
*/
{
    ...
    // Initialize the Peb.
    // Every reference to the Peb must be wrapped in try-except in case the inpage fails.
    // The inpage can fail for any reason including network failures, disk errors, low resources, etc.
    try {
    ...
    // Every reference to NtHeaders (including the call to RtlImageNtHeader) must be wrapped in try-except in case the inpage fails.
    // The inpage can fail for any reason including network failures, disk errors, low resources, etc.
    try {
    ...
    if (NtHeaders != NULL) {
        ...
        {
            try {
                ImageConfigData = RtlImageDirectoryEntryToData(PebBase->ImageBaseAddress, TRUE, IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG, &ReturnedSize);
                if (ImageConfigData != NULL) {
                    ProbeForReadSmallStructure((PVOID)ImageConfigData, sizeof(*ImageConfigData), PROBE_ALIGNMENT(IMAGE_LOAD_CONFIG_DIRECTORY));
                }
        ...
        try {
            if (Characteristics & IMAGE_FILE_UP_SYSTEM_ONLY) {
                ...
                if (ProcessAffinityMask != 0) {
                    // Pass the affinity mask from the image header to LdrpInitializeProcess via the PEB.
...
*/
{
    ...
    // If someone has secured the TEB (in addition to the standard securing that was done by memory management on creation,
    // then don't delete it now - just leave it around until the entire process is deleted.
    ...
    if (Vad->u2.VadFlags2.OneSecured) {
        ...
        // If there's only one entry, then that's the one we defined when we initially created the TEB.
        // So TEB deletion can take place right now.
        // If there's more than one entry, let the TEB sit around until the process goes away.
    ...
    if (NT_SUCCESS(Status)) {
        ...
        // Return commitment for page table pages and clear VAD bitmaps if possible.
...
*/
{
    ...
    LOCK_EXPANSION(OldIrql);
    InsertTailList(&MmWorkingSetExpansionHead.ListHead, &WsInfo->WorkingSetExpansionLinks);
    UNLOCK_EXPANSION(OldIrql);
}
...
*/
{
    ...
    // Go through the working set and for any user-accessible page which is in it, rip it out of the working set and free the page.
    index = 2;
    Wsle = &MmWsle[index];
    PteDeleteList.Count = 0;
    MmWorkingSetList->HashTable = NULL;

    // Go through the working set list and remove all pages for user space addresses.
    for (; index <= MmWorkingSetList->LastEntry; index += 1, Wsle += 1) {
        ...
        if (PointerPte->u.Hard.Valid == 0) {
            KeBugCheckEx(MEMORY_MANAGEMENT, 0x3452, (ULONG_PTR)Va, (ULONG_PTR)Wsle, (ULONG_PTR)PointerPte->u.Long);
        }

        // This is a user mode address, for each one we remove we must maintain the NonDirectCount.
        // This is because we may fault later for page tables and need to grow the hash table when updating the working set.
        // NonDirectCount needs to be correct at that point.
        ...
        if (index < MmWorkingSetList->FirstDynamic) {
            ...
            if (index != MmWorkingSetList->FirstDynamic) {
                ...
                // We may have reused the WSLE we just deleted to hold a valid entry as a result of the swap, so make sure we check it again.
                index -= 1;
                Wsle -= 1;
            }
        }
...
Environment:
    Kernel mode.
    Working set mutex held.
    Note since this is only called during process teardown, the write watch bits are not updated.
    If this ever called from other places, code will need to be added here to update those bits.
*/
{
    ULONG i;
    ULONG j;
    ...
    for (i = 0; i < PteDeleteList->Count; i += 1) {
        ...
        if (Pfn1->u3.e1.PrototypePte == 1) {
            ...
            MI_CAPTURE_DIRTY_BIT_TO_PFN(PointerPte, Pfn1);// Capture the state of the modified bit for this PTE.
            MiDecrementShareCountInline(Pfn2, PageTableFrameIndex);// Decrement the share and valid counts of the page table page which maps this PTE.
            MiDecrementShareCount(Pfn1, PageFrameIndex);// Decrement the share count for the physical page.

            // Set the pointer to PTE to be a demand zero PTE.
            // This allows the page usage count to be kept properly and handles the case when a page table page has only valid PTEs and needs to be deleted later when the VADs are removed.
            MI_WRITE_INVALID_PTE(PointerPte, DemandZeroWritePte);

            // Check to see if this is a fork prototype PTE and if so update the clone descriptor address.
            ...
            if (CloneDescriptor != NULL) {
                ...
                // note that this could release and reacquire the mutexes hence cannot be done until after the working set index has been removed.

                // Flush the TB as this path could release the PFN lock.
                // Even though the process is terminating, a thread could attach to it and try to reference the user address space.
                // By flushing the TB we ensure no access to free pages is allowed.

                // The PteDeleteList could be overloaded to save stack,
                // but using a separate list lets us flush just the entries since the last call and still retain a pristine PteDeleteList for debugging purposes.
                while (j <= i) {
                ...
                MiDecrementCloneBlockReference(CloneDescriptor, CloneBlock, CurrentProcess, &PteFlushList, OldIrql);

                // Note PteFlushList.Count will be set to 0 if MiDecrementCloneBlockReference did a flush.
            }
        } else {
            ...
            // Set the pointer to PTE to be a demand zero PTE.
            // This allows the page usage count to be kept properly and handles the case when a page table page has only valid PTEs and
            // needs to be deleted later when the VADs are removed.
            MI_WRITE_INVALID_PTE(PointerPte, DemandZeroWritePte);
        }
    }

    // Flush the TB - even though the process is terminating, a thread could attach to it and try to reference the user address space.
    // By flushing the TB we ensure no access to free pages is allowed.
    MI_FLUSH_PROCESS_TB(FALSE);
    UNLOCK_PFN(OldIrql);
}
...
Arguments:
    ActualPteAddress - Supplies the actual address that the PTE will reside at.  This is used for page coloring.
    PointerTempPte - Supplies the PTE to operate on, returns a valid PTE.
    Global - Supplies 1 if the resulting PTE is global.
    ContainingPage - Supplies the physical page number of the page which contains the resulting PTE.
                     If this value is 0, no operations on the containing page are performed.
...
restart:
    ...
    if (PointerTempPte->u.Long == MM_KERNEL_DEMAND_ZERO_PTE) {
        ...
        if (Global) {
            MI_MAKE_VALID_KERNEL_PTE(TempPte, PageFrameIndex, MM_READWRITE, ActualPteAddress);
        } else {
        ...
        if ((MmAvailablePages == 0) || ((Pfn1->u4.InPageError == 1) && (Pfn1->u3.e1.ReadInProgress == 1))) {
            ...
            // This ensures that only a safe amount of the compressed virtual cache is directly mapped so that if the hardware gets into trouble,
            // we can bail it out.
            UNLOCK_PFN(OldIrql);
        ...
        if (Pfn1->u3.e1.PageLocation != ActiveAndValid) {
            ...
            // Even though this routine is only used to bring in special system pages that are separately charged, a modified write
            // may be in progress and if so, will have applied a systemwide charge against the locked pages count.
            // This all works out nicely (with no code needed here) as the write completion will see the nonzero ShareCount and remove the charge.
            InterlockedIncrementPfn((PSHORT)&Pfn1->u3.e2.ReferenceCount);
            Pfn1->u3.e1.PageLocation = ActiveAndValid;
        }

        // Update the PFN database, the share count is now 1 and the reference count is incremented as the share count just went from zero to 1.
        Pfn1->u2.ShareCount += 1;
        MI_SET_MODIFIED(Pfn1, 1, 0x12);
        if (Pfn1->u3.e1.WriteInProgress == 0) {
            MiReleasePageFileSpace(Pfn1->OriginalPte);// Release the page file space for this page.
        ...
        // Initialize the PFN database element, but don't set read in progress as collided page faults cannot occur here.
        MiInitializePfnForOtherProcess(PageFrameIndex, ActualPteAddress, ContainingPage);
        UNLOCK_PFN(OldIrql);
...
UCHAR MiSetMemoryPriorityProcess(IN PEPROCESS Process, IN UCHAR MemoryPriority)
/*
Routine Description:
...
*/
{
    if (MmSystemSize == MmSmallSystem && MmNumberOfPhysicalPages < ((15 * 1024 * 1024) / PAGE_SIZE)) {
        MemoryPriority = MEMORY_PRIORITY_BACKGROUND;// If this is a small system, make every process BACKGROUND.
    }
...
Arguments:
    ...
    Deletable - Supplies TRUE if the VAD is to be marked as deletable, FALSE if deletions of this VAD should be disallowed.
Return Value:
...
PFN_NUMBER MmGetDirectoryFrameFromProcess(IN PEPROCESS Process)
/*
Routine Description:
    This routine retrieves the PFN of the process's top pagetable page.
    It can be used to map physical pages back to a process.
Arguments:
