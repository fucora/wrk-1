...
Abstract:
    This module contains the routines which manipulate the working set list tree.
*/
...
VOID MiRepointWsleHashIndex(IN MMWSLE WsleEntry, IN PMMWSL WorkingSetList, IN WSLE_NUMBER NewWsIndex);
...
/*
Routine Description:
    This routine inserts a Working Set List Entry (WSLE) into the hash list for the specified working set.
Arguments:
...
*/
{
    ...
    // Check hash table size and see if there is enough room to hash or if the table should be grown.
    if ((WorkingSetList->NonDirectCount + 10 + (HashTableSize >> 4)) > HashTableSize) {
        ...
        if ((WorkingSetList->NonDirectCount + (HashTableSize >> 4)) > HashTableSize) {
            // Look for a free entry (or repurpose one if necessary) in the hash table, space is tight.
            j = Hash;
            Tries = 0;
            EntriesExamined = 16;
            ...
            do {
                ...
                // Examine this in-use one.
                // We're looking for the least recently accessed one to use as a victim.
                // Use the WSLE age bits and the PTE access bit to compute this.
                Index = Table[j].Index;
                ...
                if (CurrentVictimHashIndex == WSLE_NULL_INDEX) {
                    ...
                    CurrentVictimAccessed = (ULONG)MI_GET_ACCESSED_IN_PTE(MiGetPteAddress(Table[j].Key));
                } else {
                ...
                if (EntriesExamined == 0) {
                    ...
                    if (CurrentVictimWsleContents.u1.e1.Age >= MI_PASS0_TRIM_AGE) {
                        // This address hasn't been accessed in a while, so take the WSLE and the hash entry.
                        if (MiFreeWsle(Index, WsInfo, MiGetPteAddress(Table[CurrentVictimHashIndex].Key))) {
                            // The previous entry (WSLE & hash) has been eliminated, so take it.
                    ...
                    // Purge the previous entry's hash index (but not WSLE) so it can be reused for the new entry.
                    // This is nice because it preserves both entries in the working set (although it is a bit more costly to remove the original entry later since it won't have a hash entry).
...
GotHash:
    ...
    // Or in the valid bit so subsequent searches below can compare in one instruction for both the virtual address and the valid bit.
    ...
    for (Wsle = WorkingSetList->Wsle; TRUE; Wsle += 1, Index += 1) {
        ...
        if (Wsle->u1.e1.Direct == 0) {
            ...
            if (Pfn1->u1.WsIndex == Index) {
                ...
                // Even though this working set is not the direct owner, it was able to share the direct owner's index.
                // So don't bother hashing it for now.
                // This index will remain valid for us unless the direct owner swaps this WSL entry or our working set gets compressed.
                goto DidOne;
            }
            ...
            while (Table->Key != 0) {
                ...
                if (Table == OriginalTable) {
                    ...
                    // Just bail out, we'll do linear walks to lookup this entry until the hash can be further expanded later.

                    // Mark any remaining entries as having no hash entry.
                    do {
                        ASSERT(Wsle <= WorkingSetList->Wsle + WorkingSetList->LastEntry);
                        if ((Wsle->u1.e1.Valid == 1) && (Wsle->u1.e1.Direct == 0)) {
                            Wsle->u1.e1.Hashed = 0;
                            Count -= 1;
                        }
                        Wsle += 1;
        ...
        DidOne:
            ...
            ASSERT((Wsle->u1.e1.Hashed == 0) || (Wsle->u1.e1.LockedInWs == 1) || (Wsle->u1.e1.LockedInMemory == 1));
        }
    }
...
VOID MiCheckWsleHash(IN PMMWSL WorkingSetList)
{
    ...
    for (i = 0; i < WorkingSetList->HashTableSize; i += 1) {
        if (WorkingSetList->HashTable[i].Key != 0) {
            found += 1;
            ASSERT(WorkingSetList->HashTable[i].Key == MI_GENERATE_VALID_WSLE(&Wsle[WorkingSetList->HashTable[i].Index]));
        }
    }

    if (found > WorkingSetList->NonDirectCount) {
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "MMWSLE: Found %lx, nondirect %lx\n", found, WorkingSetList->NonDirectCount);
        DbgBreakPoint();
    }
}
#endif
...
/*
Routine Description:
    This function locates the specified virtual address within the working set list.
Arguments:
    VirtualAddress - Supplies the virtual address to locate within the working set list.
    ...
    Deletion - Supplies TRUE if the WSLE is going to be deleted by the caller.
               This is used as a hint to remove the WSLE hash so that it does not have to be searched for twice.
Return Value:
...
*/
{
    ...
    // Or in the valid bit so the compares in the loops below can be done in one instruction for both the virtual address and the valid bit.
    VirtualAddress = (PVOID)((ULONG_PTR)VirtualAddress | 0x1);
    if ((WsPfnIndex <= WorkingSetList->LastInitializedWsle) && (MI_GENERATE_VALID_WSLE(&Wsle[WsPfnIndex]) == VirtualAddress)) {
        return WsPfnIndex;
    }
    ...
    if (WsPteIndex != 0) {
        ...
        while (WsPteIndex <= WorkingSetList->LastInitializedWsle) {
            ...
            // No working set index so far for this PTE.  Since the working set may be very large (8TB would mean up to half a million loops) just fall back to the hash instead.
            if (LoopCount == 0) {
                break;
            }
    ...
    do {
        if (MI_GENERATE_VALID_WSLE(Wsle) == VirtualAddress) {
            ASSERT((Wsle->u1.e1.Hashed == 0) || (WorkingSetList->HashTable == NULL));
...
/*
Routine Description:
    This routine removes a Working Set List Entry (WSLE) from the working set.
Arguments:
    ...
    if (WsleContents.u1.e1.Direct == 0) {
...
#if DBG
            if (WorkingSetList->HashTable != NULL) {
                // Or in the valid bit so virtual address 0 is handled properly (instead of matching a free hash entry).
                VirtualAddress = (PVOID)((ULONG_PTR)VirtualAddress | 0x1);
                Table = WorkingSetList->HashTable;
        ...
        if (WorkingSetList->HashTable != NULL) {
            ...
            while (Table[Hash].Key != VirtualAddress) {
                ...
                if (Hash == StartHash) {
                    // The entry could not be found in the hash, it must never have been inserted.
                    // This is ok, we don't need to do anything more in this case.
...
/*
Routine Description:
    This routine swaps the working set list entries Entry and SwapEntry in the specified working set list.
Arguments:
    SwapEntry - Supplies the first entry to swap.  This entry must be valid, i.e. in the working set at the current time.
    Entry - Supplies the other entry to swap.  This entry may be valid or invalid.
    WsInfo - Supplies the working set list.
    EntryNotInHash - Supplies TRUE if the Entry cannot possibly be in the hash table (ie, it is newly allocated), so the hash table search can be skipped.
Environment:
    Kernel mode, Working set lock and PFN lock held (if system cache), APCs disabled.
*/
{
    MMWSLE WsleEntry;
    MMWSLE WsleSwap;
    ...
    if (WsleEntry.u1.e1.Valid == 0) {
        MiRemoveWsleFromFreeList(Entry, Wsle, WorkingSetList);// Entry is not on any list. Remove it from the free list.
        MI_LOG_WSLE_CHANGE(WorkingSetList, Entry, WsleSwap); // Copy the entry to this free one.
        Wsle[Entry] = WsleSwap;
...
#endif
                KeBugCheckEx(MEMORY_MANAGEMENT, 0x41289, (ULONG_PTR)WsleSwap.u1.VirtualAddress, (ULONG_PTR)PointerPte->u.Long, (ULONG_PTR)WorkingSetList);
#if (_MI_PAGING_LEVELS < 3)
            }
#endif
        }
...
#endif
                KeBugCheckEx(MEMORY_MANAGEMENT, 0x4128A, (ULONG_PTR)WsleEntry.u1.VirtualAddress, (ULONG_PTR)PointerPte->u.Long, (ULONG_PTR)WorkingSetList);
#if (_MI_PAGING_LEVELS < 3)
            }
#endif
        }
        ...
        if (PointerPte->u.Hard.Valid == 0) {
            ...
            if (!NT_SUCCESS(MiCheckPdeForPagedPool(WsleSwap.u1.VirtualAddress))) {
#endif
                KeBugCheckEx(MEMORY_MANAGEMENT, 0x4128B, (ULONG_PTR)WsleSwap.u1.VirtualAddress, (ULONG_PTR)PointerPte->u.Long, (ULONG_PTR)WorkingSetList);
#if (_MI_PAGING_LEVELS < 3)
            }
#endif
        }
...
/*
Routine Description:
    This function locates the specified virtual address within the working set list and then removes it, performing any necessary structure updates.
Arguments:
    VirtualAddress - Supplies the virtual address to locate within the working set list.
...
*/
{
    ...
    WorkingSetList = WsInfo->VmWorkingSetList;
    WorkingSetIndex = MiLocateWsle(VirtualAddress, WorkingSetList, WsPfnIndex, TRUE);
    ...
    // If the entry was locked in the working set, then adjust the locked list so it is contiguous by reusing this entry.
    if ((WsleContents.u1.e1.LockedInWs == 1) || (WsleContents.u1.e1.LockedInMemory == 1)) {
        ...
        if (WorkingSetIndex != WorkingSetList->FirstDynamic) {
            ...
            MiSwapWslEntries(WorkingSetList->FirstDynamic, WorkingSetIndex, WsInfo, FALSE);
        }
    } else {
        ASSERT(WorkingSetIndex >= WorkingSetList->FirstDynamic);
    }
...
/*
Routine Description:
    This routine re-points the working set list hash entry for the supplied address so it points at the new working set index.
Arguments:
...
*/
{
    ...
    while (Table[Hash].Key != VirtualAddress) {
        ...
        if (StartHash == Hash) {
            // Didn't find the hash entry, so this virtual address must not have one.
            // That's ok, just return as nothing needs to be done in this case.
            return;
        }
    }
...
Routine Description:
    ...
    It is used when the entry required is not the first element in the free list.
Arguments:
...
*/
{
    ...
    if (Entry == Free) {
        ASSERT(((Wsle[Entry].u1.Long >> MM_FREE_WSLE_SHIFT) <= WorkingSetList->LastInitializedWsle) || ((Wsle[Entry].u1.Long >> MM_FREE_WSLE_SHIFT) == WSLE_NULL_INDEX));
        WorkingSetList->FirstFree = (WSLE_NUMBER)(Wsle[Entry].u1.Long >> MM_FREE_WSLE_SHIFT);
    } else {
        // See if the entry is conveniently pointed to by the previous or next entry to avoid walking the singly linked list when possible.
