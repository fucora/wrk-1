...
Abstract:
    This module contains routines which operate on the working set list structure.
*/
...
VOID MiDoReplacement(IN PMMSUPPORT WsInfo, IN WSLE_ALLOCATION_TYPE Flags);
VOID MiReplaceWorkingSetEntry(IN PMMSUPPORT WsInfo, IN WSLE_ALLOCATION_TYPE Flags);
VOID MiUpdateWsle(IN OUT PWSLE_NUMBER DesiredIndex, IN PVOID VirtualAddress, IN PMMSUPPORT WsInfo, IN PMMPFN Pfn, IN ULONG_PTR WsleMask);
VOID MiCheckWsleHash(IN PMMWSL WorkingSetList);
LOGICAL MiEliminateWorkingSetEntry(IN WSLE_NUMBER WorkingSetIndex, IN PMMPTE PointerPte, IN PMMPFN Pfn, IN PMMSUPPORT WsInfo, IN LOGICAL Force);
...
/*
Routine Description:
    This function examines the working set list for the specified working set and locates an entry to contain a new page.
    If the memory is not tight, the new page is added without removing a page.
    If memory is tight (or this working set is at its limit), a page is removed from the working set and the new page added in its place.
Arguments:
    WsInfo - Supplies the working set list.
    PointerPte - Supplies the PTE of the virtual address to insert.
    Pfn1 - Supplies the PFN entry of the virtual address being inserted.
           If this pointer has the low bit set, no trimming can be done at this time (because it is a WSLE hash table page insertion).
           Strip the low bit for these cases.
    WsleMask - Supplies the mask to logical OR into the new working set entry.
Return Value:
    Returns the working set index which was used to insert the specified entry, or 0 if no index was available.
Environment:
...
*/
{
    ...
    // Determine if a page should be removed from the working set to make room for the new page.  If so, remove it.
...
/*
Routine Description:
    This function determines whether the working set should be grown or if a page should be replaced.
    Replacement is done here if deemed necessary.
Arguments:
    ...
    Flags - Supplies 0 if replacement is not required.
            ...
            Supplies 2 if working set trimming must not be done here - this is only used for inserting new WSLE hash table pages as a trim would not know how to interpret them.
Environment:
...
*/
{
    ...
    // Determine whether the working set can be grown or whether a page needs to be replaced.
recheck:
    if (WsInfo->WorkingSetSize >= WsInfo->MinimumWorkingSetSize) {
        if ((WsInfo->Flags.ForceTrim == 1) && (TrimFlags != WsleAllocationDontTrim)) {
            // The working set manager cannot attach to this process to trim it.
            // Force a trim now and update the working set manager's fields properly to indicate a trim occurred.
            Trim = WsInfo->Claim >> ((WsInfo->Flags.MemoryPriority == MEMORY_PRIORITY_FOREGROUND) ? MI_FOREGROUND_CLAIM_AVAILABLE_SHIFT : MI_BACKGROUND_CLAIM_AVAILABLE_SHIFT);
            ...
            MiAgeWorkingSet(WsInfo, TRUE, NULL, &Dummy1, &Dummy2);
        ...
        if ((WsInfo->Flags.MaximumWorkingSetHard) && (CurrentSize >= WsInfo->MaximumWorkingSetSize)) {
        ...
        //      - or if we are growing too much in this time interval and there isn't much memory available
        MemoryMaker = PsGetCurrentThread()->MemoryMaker;
        if ((Flags == WsleAllocationReplace) ||
            ((MmAvailablePages < MM_VERY_HIGH_LIMIT) && ((MmAvailablePages == 0) ||
            ...
            if ((Flags == WsleAllocationReplace) || (MemoryMaker == 0)) {
                ...
                // Set the must trim flag because this could be a realtime thread where the fault straddles a page boundary.
                // If it's realtime, the balance set manager will never get to run and the thread will endlessly replace one WSL entry with the other half of the straddler.
                // Setting this flag guarantees the next fault will guarantee a forced trim and allow a reasonable available page threshold trim calculation since GrowthSinceLastEstimate will be cleared.
                LOCK_EXPANSION(OldIrql);
                WsInfo->Flags.ForceTrim = 1;
                UNLOCK_EXPANSION(OldIrql);
                GrowthSinceLastEstimate = 0;
            } else {
                // If we've only trimmed a single page, then don't force replacement on the next fault.
                // This prevents a single instruction causing alternating faults on the referenced code & data in a (realtime) thread from looping endlessly.
                if (PagesTrimmed > 1) {
                    LOCK_EXPANSION(OldIrql);
                    WsInfo->Flags.ForceTrim = 1;
                    UNLOCK_EXPANSION(OldIrql);
    ...
    // If there isn't enough memory to allow growth, find a good page to remove and remove it.
    WsInfo->GrowthSinceLastEstimate += GrowthSinceLastEstimate;
}


NTSTATUS MmEnforceWorkingSetLimit(IN PEPROCESS Process, IN ULONG Flags)
/*
Routine Description:
    This function enables/disables hard enforcement of the working set minimums and maximums for the specified WsInfo.
Arguments:
    Process - Supplies the target process.
    Flags - Supplies new flags (MM_WORKING_SET_MAX_HARD_ENABLE, etc).
Environment:
    Kernel mode, APCs disabled.
    The working set mutex must NOT be held.
...
*/
{
    ...
    // If the caller's request will result in hard enforcement of both limits is enabled, then check whether the current minimum and maximum working set values will guarantee forward progress even in pathological scenarios.
    ...
    if ((TempBits.MinimumWorkingSetHard == 1) && (TempBits.MaximumWorkingSetHard == 1)) {
        // Net result is hard enforcement on both limits so check that the two limits cannot result in lack of forward progress.
...
Arguments:
    ...
    Flags - Supplies 0 if replacement is not required.
            Supplies 1 if replacement is desired.  Note replacement cannot be guaranteed (the entire existing working set may
                  be locked down) - if no entry can be released the caller can detect this because MMWSL->FirstFree will not contain any free entries - and so the caller should release the working set mutex and retry the operation.
Environment:
    Kernel mode, APCs disabled, working set lock.
    PFN lock NOT held.
*/
{
    ...
    while (TRUE) {
        // Keep track of the oldest page along the way in case we don't find one that's >= MI_IMMEDIATE_REPLACEMENT_AGE before we've looked at MM_WORKING_SET_LIST_SEARCH entries.
        while (Wsle[WorkingSetIndex].u1.e1.Valid == 0) {
            ...
            if (WorkingSetIndex == TheNextSlot) {
                if (Flags == WsleAllocationAny) {
                    // Entire working set list has been searched, increase the working set size.
                    // Note this may result in exceeding a hard maximum limit on the system cache (only) in the case where all the other entries (paged pool) have been locked (so there isn't one to replace).
        ...
        if (OldestWorkingSetIndex == WSLE_NULL_INDEX) {
            // First time through, so initialize the OldestWorkingSetIndex to the first valid WSLE.
            // As we go along, this will be re-pointed at the oldest candidate we come across.
            OldestWorkingSetIndex = WorkingSetIndex;
            OldestAge = -1;
        }

        PointerPte = MiGetPteAddress(Wsle[WorkingSetIndex].u1.VirtualAddress);
        if ((Flags == WsleAllocationReplace) || ((MI_GET_ACCESSED_IN_PTE(PointerPte) == 0) && (OldestAge < (LONG)MI_GET_WSLE_AGE(PointerPte, &Wsle[WorkingSetIndex])))) {
        ...
        if ((Flags == WsleAllocationReplace) || OldestAge >= MI_IMMEDIATE_REPLACEMENT_AGE || NumberOfCandidates > MM_WORKING_SET_LIST_SEARCH) {
            ...
            // Clear the OldestWorkingSetIndex so that it gets set to the next valid entry above like the first time around.
...
/*
Routine Description:
    This function removes the page mapped by the specified PTE from the process's working set list.
Arguments:
    PointerPte - Supplies a pointer to the PTE mapping the page to be removed from the working set list.
    Pfn1 - Supplies a pointer to the PFN database element referred to by the PointerPte.
Return Value:
    Returns TRUE if the specified page was locked in the working set, FALSE otherwise.
Environment:
...
*/
{
    ...
    Wsle = WorkingSetList->Wsle;
    VirtualAddress = MiGetVirtualAddressMappedByPte(PointerPte);
    WorkingSetIndex = MiLocateWsle(VirtualAddress, WorkingSetList, Pfn1->u1.WsIndex, TRUE);
    ASSERT(WorkingSetIndex != WSLE_NULL_INDEX);
    MiEliminateWorkingSetEntry(WorkingSetIndex, PointerPte, Pfn1, WsInfo, TRUE);
...
/*
Routine Description:
    This function releases a previously reserved working set entry to be reused.
    A release occurs when a page fault is retried due to changes in PTEs and working sets during an I/O operation.
Arguments:
    WorkingSetIndex - Supplies the index of the working set entry to release.
Environment:
...
/*
Routine Description:
    This routine updates a reserved working set entry to place it into the valid state.
Arguments:
    DesiredIndex - Supplies the index of the working set entry to update.
    VirtualAddress - Supplies the virtual address which the working set entry maps.
...
*/
{
    ...
    if (WorkingSetList == MmSystemCacheWorkingSetList) {
        // This assert doesn't hold for NT64 as we can be adding page directories and page tables for the system cache WSLE hash tables.
    ...
    // Make the WSLE valid, referring to the corresponding virtual page number.

#if DBG
    if (Pfn->u1.WsIndex <= WorkingSetList->LastInitializedWsle) {
        ASSERT((PAGE_ALIGN(VirtualAddress) != PAGE_ALIGN(Wsle[Pfn->u1.WsIndex].u1.VirtualAddress)) || (Wsle[Pfn->u1.WsIndex].u1.e1.Valid == 0));
    }
#endif
    ...
    // The working set mutex is a process wide mutex and two threads in different processes could be adding the same physical page to their working sets.
    // Each one could see the WsIndex field in the PFN as 0 and therefore want to set the direct bit.

    // To solve this, the WsIndex field is updated with an interlocked operation.
    // Note for private pages, there can be no race so a simple update is enough.
    if (Pfn->u1.Event == NULL) {
        // Directly index into the WSL for this entry via the PFN database element.
        ...
        // must zero the top 32 bits as well.  So instead of setting the WsIndex field, set the overlaid Event field with appropriate casts.
        if (Pfn->u3.e1.PrototypePte == 0) {
            // This is a private page so this thread is the only one that can be updating the PFN, so no need to use an interlocked update.
            // Note this is true even if the process is being forked because in that case, the working set mutex is held throughout the fork so this thread would have blocked earlier on that mutex first.
            Pfn->u1.Event = (PKEVENT)(ULONG_PTR)WorkingSetIndex;
            ASSERT(Pfn->u1.Event == (PKEVENT)(ULONG_PTR)WorkingSetIndex);
            OldValue = 0;
        } else {
            // This is a sharable page so a thread in another process could be trying to update the PFN at the same time.
            // Use an interlocked update so only one thread gets to set the value.
#if defined (_WIN64)
            OldValue = InterlockedCompareExchange64((PLONGLONG)&Pfn->u1.Event, (LONGLONG)(ULONG_PTR)WorkingSetIndex, 0);
#else
            OldValue = InterlockedCompareExchange((PLONG)&Pfn->u1.Event, WorkingSetIndex, 0);
#endif
        }
    ...
    if ((Index < WorkingSetList->LastInitializedWsle) && (Index > WorkingSetList->FirstDynamic) && (Index != WorkingSetIndex)) {
        if (Wsle[Index].u1.e1.Valid) {
            ...
            // On free list, try to remove quickly without walking all the free pages.
            ...
            if (WorkingSetList->FirstFree == Index) {
                ...
                Wsle[Index] = Temp;
                WorkingSetIndex = Index;
                ASSERT(((Wsle[WorkingSetList->FirstFree].u1.Long >> MM_FREE_WSLE_SHIFT) <= WorkingSetList->LastInitializedWsle) || ((Wsle[WorkingSetList->FirstFree].u1.Long >> MM_FREE_WSLE_SHIFT) == WSLE_NULL_INDEX));
            ...
            if (FreeIndex != 0) {
                ...
                ASSERT(((Wsle[FreeIndex].u1.Long >> MM_FREE_WSLE_SHIFT) <= WorkingSetList->LastInitializedWsle) || ((Wsle[FreeIndex].u1.Long >> MM_FREE_WSLE_SHIFT) == WSLE_NULL_INDEX));
            }
        }
...
/*
Routine Description:
    This routine frees the specified WSLE and decrements the share count for the corresponding page, putting the PTE into a transition state if the share count goes to 0.
Arguments:
...
Return Value:
    Returns TRUE if the WSLE was removed, FALSE if it was not removed.
        Pages with valid PTEs are not removed (i.e. page table pages that contain valid or transition PTEs).
Environment:
    Kernel mode, APCs disabled, working set lock.
    PFN lock NOT held.
*/
{
    ...
    // Note, don't clear the access bit for page table pages with valid PTEs as this could cause an access trap fault which would not be handled (it is only handled for PTEs not PDEs).
    Pfn1 = MI_PFN_ELEMENT(PointerPte->u.Hard.PageFrameNumber);

    // Perform a preliminary check without the PFN lock so that lock contention is avoided for cases that cannot possibly succeed.
    if (WsInfo == &MmSystemCacheWs) {
    ...
    if (MiEliminateWorkingSetEntry(WorkingSetIndex, PointerPte, Pfn1, WsInfo, FALSE) == FALSE) {
        return FALSE;
    }
...
/*
Routine Description:
    This routine initializes a process's working set to the empty state.
Arguments:
...
*/
{
    ...
    // Note when booted /3GB, the second VAD bitmap page is automatically inserted as part of the working set list page as the page is shared by both.
...
/*
Routine Description:
    This function initializes the working set for the session space and adds it to the list of session space working sets.
Return Value:
...
*/
{
    ...
    // Use the global address for pointer references by MmWorkingSetManager before it attaches to the address space.
    ...
    // Build the PDE entry for the working set - note that the global bit must be turned off.
    PointerPde = MiGetPdeAddress(WorkingSetList);

    // The page table page for the working set and its first data page are charged against MmResidentAvailablePages and for commitment.
    if (PointerPde->u.Hard.Valid == 1) {
        // The page directory entry for the working set is the same as for another range in the session space.  Share the PDE.
    ...
    // The data pages needed to map up to maximum working set size are also charged against MmResidentAvailablePages and for commitment.
    NumberOfEntriesMappedByFirstPage = (WSLE_NUMBER)(((PMMWSLE)((ULONG_PTR)WorkingSetList + PAGE_SIZE)) - MmSessionSpace->Wsle);
    ...
    // Use the global address for mutexes since the contained event must be accessible from any address space.
    ...
    MI_DECREMENT_RESIDENT_AVAILABLE(ResidentPages, MM_RESAVAIL_ALLOCATE_INIT_SESSION_WS);
    if (AllocatedPageTable == TRUE) {
        ...
        // The global bit is masked off since we need to make sure the TB entry is flushed when we switch to a process in a different session space.
        ...
        // This page is never paged, ensure that its WsIndex stays clear so the release of the page will be handled correctly.
...
#define MI_INITIALIZE_SESSION_WSLE(_VirtualAddress, _WslEntry) {   \
    ...
    // The first reserved slot is for the page table page mapping the session data page.
    ...
    if (AllocatedPageTable == TRUE) {
        // The next reserved slot is for the page table page mapping the working set page.
        MI_INITIALIZE_SESSION_WSLE(PointerPte, WslEntry);
    }

    // The next reserved slot is for the page table page mapping the first session paged pool page.
    ...
    // Calculate the maximum number of entries dynamically as the size of session space is registry configurable.
    // Then add in page table and page directory overhead.
...
#if defined (_X86PAE_)
    // One less page table page is needed on PAE systems as the session working set structures easily fit within 2MB.
    ...
    InterlockedExchangeAddSizeT(&MmSessionSpace->NonPageablePages, ResidentPages);
    ...
    for (i = CurrentEntry + 1; i < NumberOfEntriesMapped; i += 1) {
        // Build the free list, note that the first working set entries (CurrentEntry) are not on the free list.
        // These entries are reserved for the pages which map the working set and the page which contains the PDE.
        WslEntry->u1.Long = i << MM_FREE_WSLE_SHIFT;
        WslEntry += 1;
    }
...
/*
Routine Description:
    This routine acquires the address space mutex so a consistent snapshot of the argument process' commit charges can be used by Ps when adding this process to a job.
    Note that the working set mutex is not acquired here so the argument process' working set sizes cannot be reliably snapped by Ps, but since Ps doesn't look at that anyway, it's not a problem.
Arguments:
...
Environment:
    Kernel mode, IRQL APC_LEVEL or below.  The caller provides protection from the target process going away.
*/
{
    LOGICAL Attached;
    LOGICAL Status;
    ...
    // Join the job unconditionally.  If the process is over any limits, it will be caught on its next request.
...
NTSTATUS MmQueryWorkingSetInformation(IN PSIZE_T PeakWorkingSetSize, IN PSIZE_T WorkingSetSize, IN PSIZE_T MinimumWorkingSetSize, IN PSIZE_T MaximumWorkingSetSize, IN PULONG HardEnforcementFlags)
/*
Routine Description:
    This routine returns various working set information fields for the current process.
Arguments:
    PeakWorkingSetSize - Supplies an address to receive the peak working set size in bytes.
    WorkingSetSize - Supplies an address to receive the current working set size in bytes.
    MinimumWorkingSetSize - Supplies an address to receive the minimum working set size in bytes.
    MaximumWorkingSetSize - Supplies an address to receive the maximum working set size in bytes.
    HardEnforcementFlags - Supplies an address to receive the current working set enforcement policy.
Environment:
...
VOID MmQuerySystemCacheWorkingSetInformation(OUT PSYSTEM_FILECACHE_INFORMATION Info)
/*
Routine Description:
    This routine returns various working set information fields for the system cache.
Arguments:
    Info - Supplies a pointer to captured (ie, system not user !) space to store the results in.
           Note the fields in this structure are not consistent (some are bytes, some are pages) but since NT4 shipped this way we must maintain it.
Environment:
    Kernel mode, IRQL APC_LEVEL or below.
*/
{
...
NTSTATUS MmAdjustWorkingSetSizeEx(
...
/*
Routine Description:
    This routine adjusts the current size of a process's working set list.
    If the maximum value is above the current maximum, pages are removed from the working set list.

    A failure status is returned if the limit cannot be granted.
    This could occur if too many pages were locked in the process's working set.

    Note: if the minimum and maximum are both (SIZE_T)-1, the working set is purged, but the default sizes are not changed.
Arguments:
    WorkingSetMinimumInBytes - Supplies the new minimum working set size in bytes.
    WorkingSetMaximumInBytes - Supplies the new maximum working set size in bytes.
    SystemCache - Supplies TRUE if the system cache working set is being adjusted, FALSE for all other working sets.
    IncreaseOkay - Supplies TRUE if this routine should allow increases to the working set minimum.
    Flags - Supplies flags (MM_WORKING_SET_MAX_HARD_ENABLE, etc) for enabling/disabling hard enforcement of the working set minimums and maximums for the specified WsInfo.
...
*/
{
    ...
    *IncreaseRequested = FALSE;
    ...
    if (SystemCache) {
        // Initializing CurrentProcess is not needed for correctness, but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
        CurrentProcess = NULL;
    ...
    // Make sure that the number of locked pages will not make the working set not fluid.
    if ((WsInfo->VmWorkingSetList->FirstDynamic + MM_FLUID_WORKING_SET) >= WorkingSetMaximum) {
        ReturnStatus = STATUS_BAD_WORKING_SET_LIMIT;
        goto Returns;
    }

    // If hard working set limits are being enabled (or are already enabled), then make sure the minimum and maximum will not starve this process.
    if ((Flags & MM_WORKING_SET_MIN_HARD_ENABLE) ||
        ((WsInfo->Flags.MinimumWorkingSetHard == 1) && ((Flags & MM_WORKING_SET_MIN_HARD_DISABLE) == 0)) ||
        (Flags & MM_WORKING_SET_MAX_HARD_ENABLE) ||
        ((WsInfo->Flags.MaximumWorkingSetHard == 1) && ((Flags & MM_WORKING_SET_MAX_HARD_DISABLE) == 0))) {
        // Working set maximum is (or will be hard) as well.

        // Check whether the requested minimum and maximum working set values will guarantee forward progress even in pathological scenarios.
        if (WorkingSetMinimum + MM_FLUID_WORKING_SET >= WorkingSetMaximum) {
            ReturnStatus = STATUS_BAD_WORKING_SET_LIMIT;
            goto Returns;
        }

        // Make sure the system cache never gets a hard limit below 16mb or so as this working set does work for all the others.
    ...
    // Check to make sure ample resident physical pages exist for this operation.
    LOCK_PFN(OldIrql);

    if (i > 0) {
        // New minimum working set is greater than the old one.
        // Ensure that increasing is okay, and that we don't allow this process' working set minimum to increase to a point where subsequent nonpaged pool allocations could cause us to run out of pages.
        // Additionally, leave 100 extra pages around so the user can later bring up tlist and kill processes if necessary.
    ...
    // Adjust the number of resident pages up or down dependent on the size of the new minimum working set size versus the previous minimum size.
    ...
    if (WorkingSetMaximum < WorkingSetList->LastInitializedWsle) {
        // The new working set maximum is less than the current working set maximum.
        if (WsInfo->WorkingSetSize > WorkingSetMaximum) {
            ...
            do {
                ...
                MiTrimWorkingSet(WsInfo->WorkingSetSize - WorkingSetMaximum, WsInfo, TrimAge);
                if (WsInfo->WorkingSetSize <= WorkingSetMaximum) {
                    break;
                }
                ...
                if (TrimAge == 0) {
                    ...
                    // Page table pages are not becoming free, give up and return an error.
                    ReturnStatus = STATUS_BAD_WORKING_SET_LIMIT;
                    break;
                }
    ...
    if (ExtraWsFreed == TRUE) {
        ...
        // If the caller's request will result in hard enforcement of both limits, the minimum and maximum working set values must guarantee forward progress even in pathological scenarios.
...
NTSTATUS MmAdjustWorkingSetSize(__in SIZE_T WorkingSetMinimumInBytes, __in SIZE_T WorkingSetMaximumInBytes, __in ULONG SystemCache, __in BOOLEAN IncreaseOkay)
/*
Routine Description:
    This routine adjusts the current size of a process's working set list.
    If the maximum value is above the current maximum, pages are removed from the working set list.
...
*/
{
    ...
    return MmAdjustWorkingSetSizeEx(WorkingSetMinimumInBytes, WorkingSetMaximumInBytes, SystemCache, IncreaseOkay, 0, &IncreasePerformed);
}
...
/*
Routine Description:
    This function grows the working set list above working set maximum during working set adjustment.
    At most one page can be added at a time.
Return Value:
...
*/
{
    ...
    // Initializing OldIrql is not needed for correctness, but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    OldIrql = PASSIVE_LEVEL;
    WorkingSetList = WsInfo->VmWorkingSetList;
    Wsle = WorkingSetList->Wsle;
    MM_WS_LOCK_ASSERT(WsInfo);

    // The maximum size of the working set is being increased, check to ensure the proper number of pages are mapped to cover the complete working set list.
    ...
    if ((PVOID)Va >= WorkingSetList->HashTableStart) {
        // Adding this entry would overrun the hash table.  The caller must replace instead.
        return FALSE;
    }
    ...
    // The PPE is guaranteed to always be resident for architectures using 3 level lookup.
    // This is because the hash table PPE immediately follows the working set PPE.

    // For x86 PAE the same paradigm holds in guaranteeing that the PDE is always resident.

    // x86 non-PAE uses the same PDE and hence it also guarantees PDE residency.

    // Architectures employing 4 level lookup use a single PXE for this, but each PPE must be checked.
    ...
    if (PointerPpe->u.Hard.Valid == 0) {
        ...
        // Continue holding the PFN lock until the entire hierarchy is allocated.
        // This eliminates error recovery which would be needed if the lock was released and then when reacquired it is discovered that one of the pages cannot be allocated.
        PfnHeld = TRUE;
        LOCK_PFN(OldIrql);
        if ((MmAvailablePages < MM_HIGH_LIMIT) || (MI_NONPAGEABLE_MEMORY_AVAILABLE() < MM_HIGH_LIMIT)) {
        ...
        // Apply the resident available charge for the working set page directory table page now before releasing the PFN lock.
    ...
    if (PointerPde->u.Hard.Valid == 0) {
        ...
        // Apply the resident available charge for the working set page table page now before releasing the PFN lock.
        MI_DECREMENT_RESIDENT_AVAILABLE(1, MM_RESAVAIL_ALLOCATE_ADD_WS_PAGE);
    ...
    // Finally allocate and map the actual working set page now.
    // The PFN lock is only held if another page in the hierarchy needed to be allocated.

    // Further down in this routine (once an actual working set page has been allocated) the working set size will be increased by 1 to reflect the working set size entry for the new page directory page.
    // The page directory page will be put in a working set entry which will be locked into the working set.
    if (PfnHeld == FALSE) {
        LOCK_PFN(OldIrql);
        if ((MmAvailablePages < MM_HIGH_LIMIT) || (MI_NONPAGEABLE_MEMORY_AVAILABLE() < MM_HIGH_LIMIT)) {
            // No pages are available, the caller will have to replace.
    ...
    // Apply the resident available charge for the working set page now before releasing the PFN lock.
    MI_DECREMENT_RESIDENT_AVAILABLE(1, MM_RESAVAIL_ALLOCATE_ADD_WS_PAGE);
    ...
    if (WsInfo->Flags.SessionSpace == 1) {
        ...
        InterlockedExchangeAddSizeT(&MmSessionSpace->NonPageablePages, NumberOfPages);
    ...
    if ((WorkingSetList->HashTable == NULL) && (MmAvailablePages > MM_HIGH_LIMIT)) {
        // Add a hash table to support shared pages in the working set to eliminate costly lookups.
...
/*
Routine Description:
    This function adds a page directory, page table or actual mapping page for hash table creation (or expansion) for the current process.
Arguments:
...
*/
{
    ...
    // As we have grown the working set, take the next free WSLE from the list and use it.
    Pfn1 = MI_PFN_ELEMENT(WorkingSetPage);
    Pfn1->u1.Event = NULL;

    // Set the low bit in the PFN pointer to indicate that the working set
    // should not be trimmed during the WSLE allocation as the PTEs for the new hash pages are valid but we are still in the midst of making all the associated fields valid.
    WorkingSetIndex = MiAllocateWsle(WsInfo, PointerPte, (PMMPFN)((ULONG_PTR)Pfn1 | 0x1), 0);
    if (WorkingSetIndex == 0) {
        // No working set index was available, flush the PTE and the page, and decrement the count on the containing page.
        ...
        MiTrimPte(MiGetVirtualAddressMappedByPte(PointerPte), PointerPte, Pfn1, PsGetCurrentProcess(), ZeroPte);
...
/*
Routine Description:
    This function grows (or adds) a hash table to the working set list to allow direct indexing for WSLEs than cannot be located via the PFN database WSINDEX field.
    The hash table is located AFTER the WSLE array and the pages are locked into the working set just like standard WSLEs.
    Note that the hash table is expanded by setting the hash table field in the working set to NULL, but leaving the size as non-zero.
    This indicates that the hash should be expanded and the initial portion of the table zeroed.
Arguments:
    WsInfo - Supplies a pointer to the working set info structure.
Environment:
    Kernel mode, APCs disabled, working set lock held.
*/
{
    ...
    if (Table == NULL) {
        NewSize = PtrToUlong(PAGE_ALIGN(((1 + WorkingSetList->NonDirectCount) * 2 * sizeof(MMWSLE_HASH)) + PAGE_SIZE - 1));

        // Note that the Table may be NULL and the HashTableSize/PTEs nonzero in the case where the hash has been contracted.
        j = First * sizeof(MMWSLE_HASH);

        // Don't try for additional hash pages if we already have the right amount (or too many).
        ...
        // Attempt to increase by 1/4 of the indirect count.
        // If this is less than 4 pages, then grow by 4.
        // Make sure the working set list has enough free entries for the growth size though.
    ...
    // If MiAddWsleHash was unable to allocate memory above, then roll back any extra PPEs & PDEs that may have been created.
    // Note NewSize must be recalculated to handle the fact that memory may have run out.
    if (PointerPte != EndPte) {
        CurrentProcess = PsGetCurrentProcess();

        // Clean up the last allocated PPE/PDE as they are not needed.
        // Note that the system cache and the session space working sets have no current process (which MiDeletePte requires) which is needed for WSLE and PrivatePages adjustments.
        if (AllocatedPde != NULL) {
            ...
            if (WsInfo->VmWorkingSetList == MmWorkingSetList) {
                LOCK_PFN(OldIrql);
                MiDeletePte(AllocatedPde, TempVa, FALSE, CurrentProcess, NULL, NULL, OldIrql);
                UNLOCK_PFN(OldIrql);

                CurrentProcess->NumberOfPrivatePages += 1;// Add back in the private page MiDeletePte subtracted.
            } else {
                LOCK_PFN(OldIrql);
                MiDeleteValidSystemPte(AllocatedPde, TempVa, WsInfo, NULL);
                UNLOCK_PFN(OldIrql);
            }
        ...
        if (AllocatedPpe != NULL) {
            ...
            if (WsInfo->VmWorkingSetList == MmWorkingSetList) {
                LOCK_PFN(OldIrql);
                MiDeletePte(AllocatedPpe, TempVa, FALSE, CurrentProcess, NULL, NULL, OldIrql);
                UNLOCK_PFN(OldIrql);
                ...
                LOCK_PFN(OldIrql);
                MiDeleteValidSystemPte(AllocatedPpe, TempVa, WsInfo, NULL);
                UNLOCK_PFN(OldIrql);
            }
        ...
        if (AllocatedPxe != NULL) {
            ...
            if (WsInfo->VmWorkingSetList == MmWorkingSetList) {
                LOCK_PFN(OldIrql);
                MiDeletePte(AllocatedPxe, TempVa, FALSE, CurrentProcess, NULL, NULL, OldIrql);
                UNLOCK_PFN(OldIrql);

                CurrentProcess->NumberOfPrivatePages += 1;// Add back in the private page MiDeletePte subtracted.
            } else {
                LOCK_PFN(OldIrql);
                MiDeleteValidSystemPte(AllocatedPxe, TempVa, WsInfo, NULL);
                UNLOCK_PFN(OldIrql);
            }
    ...
    VirtualAddress = MiGetVirtualAddressMappedByPte(PointerPte);
    ASSERT((VirtualAddress == WorkingSetList->HighestPermittedHashAddress) || (MiIsAddressValid(VirtualAddress, FALSE) == FALSE));
...
/*
Routine Description:
    This routine frees the specified list of WSLEs decrementing the share count for the corresponding pages, putting each PTE into a transition state if the share count goes to 0.
Arguments:
    WsInfo - Supplies a pointer to the working set structure.
...
*/
{
    ...
    if (Wsle == MmWsle) {
        AllProcessors = FALSE;
    } else {// Must be the system cache or a session space.
        AllProcessors = TRUE;
    }
    ...
    for (i = 0; i < WsleFlushList->Count; i += 1) {
        ...
        // If the PTE is a page table page with non-zero share count or within the system cache with its reference count greater than 1, don't remove it.
        if (WsInfo == &MmSystemCacheWs) {
            if (Pfn1->u3.e2.ReferenceCount > 1) {
            ...
            if ((Pfn1->u2.ShareCount > 1) && (Pfn1->u3.e1.PrototypePte == 0)) {
                ...
                if (WsInfo->Flags.SessionSpace == 1) {
                    ...
                    ASSERT32((Wsle[WorkingSetIndex].u1.VirtualAddress >= (PVOID)PTE_BASE) && (Wsle[WorkingSetIndex].u1.VirtualAddress <= (PVOID)PTE_TOP));
                }
#endif

                // Don't remove page table pages from the working set until all transition pages have exited.
                WsleFlushList->FlushIndex[i] = 0;
                continue;
            }
        }
        ...
        // This page is being removed from the working set, the dirty bit must be ORed into the modify bit in the PFN element.
        ...
        if (Pfn1->u3.e1.PrototypePte) {
            ...
            } else {// The protection is in the prototype PTE.
            ...
            // If the PTE indicates the page has been modified (this is different from the PFN indicating this), then ripple it back to the write watch bitmap now since we are still in the correct process context.
            if ((MI_IS_PTE_DIRTY(TempPte)) && (Wsle == MmWsle)) {
                ...
                if (Process->Flags & PS_PROCESS_FLAGS_USING_WRITE_WATCH) {
                    ...
                    MiCaptureWriteWatchDirtyBit(Process, Wsle[WorkingSetIndex].u1.VirtualAddress);
                }
            }

            // Assert that the share count is 1 for all user mode pages.
            ASSERT((Pfn1->u2.ShareCount == 1) || (Wsle[WorkingSetIndex].u1.VirtualAddress > (PVOID)MM_HIGHEST_USER_ADDRESS));

            // Set the working set index to zero.
            // This allows page table pages to be brought back in with the proper WSINDEX.
            ASSERT(Pfn1->u1.WsIndex != 0);
        ...
        if (PteFlushList.Count < MM_MAXIMUM_FLUSH_COUNT) {
            PteFlushList.FlushVa[PteFlushList.Count] = Wsle[WorkingSetIndex].u1.VirtualAddress;
            PteFlushList.Count += 1;
        }

        // Flush the translation buffer and decrement the number of valid PTEs within the containing page table page.
...
Arguments:
    ...
    TrimAge - Supplies the age value to use - ie: pages of this age or older will be removed.
Return Value:
...
TrimMore:
    while (NumberLeftToRemove != 0) {
        if (Wsle[TryToFree].u1.e1.Valid == 1) {
            ...
            ASSERT(PointerPte->u.Hard.Valid == 1);
            if ((TrimAge == 0) || ((MI_GET_ACCESSED_IN_PTE(PointerPte) == 0) && (MI_GET_WSLE_AGE(PointerPte, &Wsle[TryToFree]) >= TrimAge))) {
...
/*
Routine Description:
    This routine removes the specified working set list entry from the working set, flushes the TB for the page, decrements the share count for the physical page, and, if necessary turns the PTE into a transition PTE.
Arguments:
    WorkingSetIndex - Supplies the working set index to remove.
    PointerPte - Supplies a pointer to the PTE corresponding to the virtual address in the working set.
...
*/
{
    ...
    if (Pfn->u3.e1.PrototypePte) {
        ...
        // The PFN database does not contain the contents of this PTE it contains the contents of the prototype PTE.
        // This PTE must be reconstructed to contain a pointer to the prototype PTE.
        ...
        // Obtain the containing page table page as its share count will need to be decremented (when the PFN lock is held) as the PTE for the removed page is no longer valid or in transition.
        ContainingPageTablePage = MiGetPteAddress(PointerPte);
#if (_MI_PAGING_LEVELS >= 3)
        ASSERT(ContainingPageTablePage->u.Hard.Valid == 1);
#else
        ...
        // Assert that the share count is 1 for all user mode pages.
        ASSERT((Pfn->u2.ShareCount == 1) || (WsleContents.u1.VirtualAddress > (PVOID)MM_HIGHEST_USER_ADDRESS));
        ...
        if (WsInfo == &MmSystemCacheWs) {
            ...
            if ((Pfn->u2.ShareCount > 1) && (Pfn->u3.e1.PrototypePte == 0)) {
                ...
                if (WsInfo->Flags.SessionSpace == 1) {
                    ...
                    ASSERT32((WsleContents.u1.VirtualAddress >= (PVOID)PTE_BASE) && (WsleContents.u1.VirtualAddress <= (PVOID)PTE_TOP));
                }
#endif
    ...
    // A page is being removed from the working set, on certain hardware the dirty bit should be ORed into the modify bit in the PFN element.
...
/*
Routine Description:
    This routine compresses the WSLEs into the front of the working set and frees the pages for unneeded working set entries.
Arguments:
...
Top:
    ...
    if (WorkingSetList->HashTable) {
        ...
        if (WsInfo->WorkingSetSize < 200) {
            ...
            NewSize = PtrToUlong(PAGE_ALIGN((WorkingSetList->NonDirectCount * 2 * sizeof(MMWSLE_HASH)) + PAGE_SIZE - 1));
        ...
        if (NewSize < WorkingSetList->HashTableSize) {
            ...
            // Set the hash table to null indicating that no hashing is going on.
            WorkingSetList->HashTable = NULL;
...
#if (_MI_PAGING_LEVELS >= 4)
        // For NT64, the page tables and page directories are also deleted during contraction.
    ...
    if (WsInfo == &MmSystemCacheWs) {
        // The first entry of the system cache working set list is never used because WSL index 0 in a PFN is treated specially by the fault
        // processing code (and trimming) to mean that the entry should be inserted into the WSL by the current thread.

        // This is not an issue for process or session working sets because for them, entry 0 is the top level page directory which must already
        // be resident in order for the process to even get to run (ie: so the process cannot fault on it).
    ...
    if (DynamicEntries == 0) {
        // If the only pages in the working set are locked pages (that is all pages are BEFORE first dynamic, just reorganize the free list).
        LastIndex = WorkingSetList->FirstDynamic;
        LastEntry = &Wsle[LastIndex];
    } else {
        // Start from the first dynamic and move towards the end looking for free entries.
        // At the same time start from the end and move towards first dynamic looking for valid entries.
        ...
        while (FreeEntry < LastEntry) {
            if (FreeEntry->u1.e1.Valid == 1) {
                ...
                *FreeEntry = WsleContents;
                ...
                if (WsleContents.u1.e1.Direct) {
                    ...
                    // Remove it from there and re-insert it in the hash at the lowest free slot.
            ...
            if (DynamicEntries == 0) {
                // The last dynamic entry has been processed, no need to look at any more - the rest are all invalid.
                LastEntry = FreeEntry;
                LastIndex = FreeIndex;
                break;
            }
    ...
    ASSERT(((LastEntry - 1)->u1.e1.Valid == 1) || (WsInfo->WorkingSetSize == 0) || ((WsInfo == &MmSystemCacheWs) && (LastEntry - 1 == WorkingSetList->Wsle)));
    ...
    // If the working set valid & free entries are already compressed optimally (or fit into a single page) then bail.
    if ((MovedOne == FALSE) && ((WorkingSetList->LastInitializedWsle + 1 == (PAGE_SIZE - BYTE_OFFSET(WorkingSetList->Wsle)) / sizeof(MMWSLE)) ||
        ((WorkingSetList->FirstFree == LastIndex) && ((WorkingSetList->LastEntry == LastIndex - 1) ||
    ...
    // Calculate the start and end of the working set pages at the end that we will delete shortly.
    // Don't delete them until after LastInitializedWsle is reduced so that debug WSL validation code in MiReleaseWsle (called from MiDeletePte) will see a consistent snapshot.
    LastPte = MiGetPteAddress(&Wsle[WorkingSetList->LastInitializedWsle]) + 1;
    PointerPte = MiGetPteAddress(FreeEntry) + 1;

    // Mark the last working set entry in the list as free.
    // Note if the list has no free entries, the marker is in FirstFree (and cannot be put into the list anyway because there is no space).
    if (WorkingSetList->FirstFree == WSLE_NULL_INDEX) {
        FreeEntry -= 1;
        PointerPte -= 1;
    } else {
    ...
    // Note that since these are in the working set themselves, that after this deletion, we must reprocess the working set itself again to look for excess pages.
    ASSERT(WorkingSetList->FirstFree >= WorkingSetList->FirstDynamic);
    ...
    // The working set size will be reduced by the number of PTEs just deleted above.
    // If this enables us to shrink the table size then it is mandatory to do so since this is called from the process exit path to remove everything possible (anything that isn't will be leaked!).
...
Arguments:
    ...
    NeedLock - Supplies TRUE if the caller needs us to acquire mutex synchronization for the working set.
               Supplies FALSE if the caller has already acquired synchronization.
Return Value:
    Status of operation.
Environment:
    Kernel mode. No locks.
    For session operations, the caller is responsible for attaching into the proper session.
*/
{
    PETHREAD Thread;
    PMMPTE PointerPte;
    ...
    // Attempt to remove the pages starting at the top to keep the free list compressed as entries are added to the freelist in FILO order.
    FirstDynamic = WorkingSetList->FirstDynamic;
    ...
    if (NeedLock == TRUE) {
        ...
        if (MiTbDebug) {
            ...
            // Run the transition list and free all the entries so transition faults are not satisfied for any of the modified pages that were just written.
            // This will generally cause faults to be satisfied using a different page from the original one,
            // which is a useful mode to us when we are trying to track down TB flush problems.
