...
Abstract:
    This module contains the routines which implement the section object.
*/
...
/*
Routine Description:
    This routine is called whenever the last reference to a segment object has been removed.
    This routine releases the pool allocated for the prototype PTEs and performs consistency checks on those PTEs.

    For segments which map files, the file object is dereferenced.

    Note, that for a segment which maps a file, no PTEs may be valid or transition.

    A segment which is backed by a paging file may have transition pages,
    but no valid pages (there can be no PTEs which refer to the segment) unless it is a large page segment.

    If it is a large page segment that backs the paging file, then all the prototype PTEs are expected to be valid, but their sharecounts must be exactly 1.
Arguments:
...
*/
{
    ...
    // Capture the segment flags locally so we can check them while the PFN lock is held below (the segment is pageable).
    ...
    if (ControlArea->DereferenceList.Flink != NULL) {
        ...
        // segment thread cannot be processing any subsections from this control area right now because it bumps the NumberOfMappedViews
        // for the control area prior to releasing the PFN lock and it checks for BeingDeleted.
    ...
    if ((ControlArea->u.Flags.Image) || (ControlArea->u.Flags.File)) {
        ...
        if (ControlArea->u.Flags.DebugSymbolsLoaded != 0) {
            ...
            Status = RtlUnicodeStringToAnsiString(&AnsiName, (PUNICODE_STRING)&Segment->ControlArea->FilePointer->FileName, TRUE);
        ...
        // Clear the segment context and dereference the file object for this Segment.
        ...
        // If there have been committed pages in this segment, adjust the total commit count.
    ...
    // The PFN lock is required for deallocating pages from a paging file and for deleting transition PTEs.
    ...
    // Access the first prototype PTE to try and make it resident before acquiring the PFN lock.  This is purely an optimization to reduce
    ...
    *(volatile MMPTE *)PointerPte;
    ...
    while (PointerPte < LastPte) {
        if ((MiIsPteOnPdeBoundary(PointerPte)) && (PointerPte != Subsection->SubsectionBase)) {
        ...
        // Prototype PTEs for segments backed by paging file are either in demand zero, page file format, or transition.
        if (PteContents.u.Hard.Valid == 1) {
            ...
            // to bother updating the prototype PTE (especially since we've just biased the PteAddress in the PFN entry).
            Pfn1->u3.e1.PrototypePte = 0;

            // Delete these one by one in case there is pending I/O.
            MiDecrementShareCountInline(Pfn1, PageFrameIndex);
            MI_INCREMENT_RESIDENT_AVAILABLE(1, MM_RESAVAIL_FREE_LARGE_PAGES_PF_BACKED);
            ...
            if (PteContents.u.Soft.Transition == 1) {
                ...
                // Check the reference count for the page, if the reference count is zero and the page is not on the freelist,
                ...
                // This is not a prototype PTE, if any paging file space has been allocated, release it.
...
ULONG MmDoesFileHaveUserWritableReferences(IN PSECTION_OBJECT_POINTERS SectionPointer)
/*
Routine Description:
    This routine is called by the transaction filesystem to determine if the given transaction is referencing a file which has user writable sections
    or user writable views into it.  If so, the transaction must be aborted as it cannot be guaranteed atomicity.

    The transaction filesystem is responsible for checking and intercepting file object creates that specify write access prior to using this interface.
    Specifically, prior to starting a transaction, the transaction filesystem must ensure that there are no writable file objects that
    currently exist for the given file in the transaction.
    While the transaction is ongoing, requests to create file objects with write access for the transaction files must be refused.

    This Mm routine exists to catch the case where the user has closed the file handles and the section handles, but still has open writable views.

    For this reason, no locks are needed to read the value below.
Arguments:
    SectionPointer - Supplies a pointer to the section object pointers from the file object.
Return Value:
...
/*
Routine Description:
    This routine is called by the object management procedures whenever the last reference to a section object has been removed.
    This routine dereferences the associated segment object and checks to see if the segment object should be deleted by queueing the segment to the segment deletion thread.
Arguments:
...
*/
{
    ...
    if (Section->Segment == NULL) {
        // The section was never initialized, no need to remove any structures.
        return;
    }
    ...
    if (Section->Address.StartingVpn != 0) {
        // This section is based, remove the base address from the tree.
    ...
    // Decrement the number of section references to the segment for this section.  This requires APCs to be blocked and the PFN lock to
    // synchronize upon.
...
/*
Routine Description:
    This routine is the thread for dereferencing segments which have no references from any sections or mapped views AND there are
    no prototype PTEs within the segment which are in the transition state (i.e., no PFN database references to the segment).
...
*/
{
    ...
    for (;;) {
        Status = KeWaitForMultipleObjects(SegMaximumObject, &WaitObjects[0], WaitAny, WrVirtualMemory, UserMode, FALSE, NULL, &WaitBlockArray[0]);
        // Switch on the wait status.
        switch (Status) {
        case SegmentDereference:
            // An entry is available to dereference, acquire the spinlock and remove the entry.
            ...
            if (ControlArea->Segment != NULL) {
                // This is a control area, delete it after indicating this entry is not on any list.
...
/*
Routine Description:
    This function creates the section object type descriptor at system initialization and stores the address of the object type descriptor in global storage.
Return Value:
...
#define TYPE_SECTION L"Section"
    ...
    if (!NT_SUCCESS(ObCreateObjectType(&TypeName, &ObjectTypeInitializer, (PSECURITY_DESCRIPTOR)NULL, &MmSectionObjectType))) {
        return FALSE;
    }

    // Create the Segment dereferencing thread.
    InitializeObjectAttributes(&ObjectAttributes, NULL, 0, NULL, NULL);
    if (!NT_SUCCESS(PsCreateSystemThread(&ThreadHandle, THREAD_ALL_ACCESS, &ObjectAttributes, 0, NULL, MiDereferenceSegmentThread, NULL))) {
        return FALSE;
    }
    ...
    // Now that the segment object is created, create a section object which refers to the segment object.
...
#define DEVICE_PHYSICAL_MEMORY L"\\Device\\PhysicalMemory"
    ...
    InitializeObjectAttributes(&ObjectAttributes, &SectionName, OBJ_PERMANENT | OBJ_KERNEL_EXCLUSIVE, NULL, NULL);
    Status = ObCreateObject(KernelMode, MmSectionObjectType, &ObjectAttributes, KernelMode, NULL, sizeof(SECTION), sizeof(SECTION), 0, (PVOID *)&Section);
    if (!NT_SUCCESS(Status)) {
        ExFreePool(ControlArea);
        ExFreePool(Segment);
        return FALSE;
    ...
    Status = ObInsertObject((PVOID)Section, NULL, SECTION_MAP_READ, 0, NULL, &Handle);
    if (!NT_SUCCESS(Status)) {
        return FALSE;
    }
...
BOOLEAN MmForceSectionClosed(__in PSECTION_OBJECT_POINTERS SectionObjectPointer, __in BOOLEAN DelayClose)
/*
Routine Description:
    This function examines the Section object pointers.  If they are NULL, no further action is taken and the value TRUE is returned.

    If the Section object pointer is not NULL, the section reference count and the map view count are checked.
    If both counts are zero, the segment associated with the file is deleted and the file closed.
    If one of the counts is non-zero, no action is taken and the value FALSE is returned.
Arguments:
    SectionObjectPointer - Supplies a pointer to a section object.
    DelayClose - Supplies the value TRUE if the close operation should occur as soon as possible in the event this section cannot be closed now due to outstanding references.
Return Value:
    TRUE - The segment was deleted and the file closed or no segment was located.
    FALSE - The segment was not deleted and no action was performed OR an I/O error occurred trying to write the pages.
*/
{
    PCONTROL_AREA ControlArea;
    KIRQL OldIrql;
    ...
    // Check the status of the control area, if the control area is in use or the control area is being deleted, this operation cannot continue.
    ...
    do {
        // Set the being deleted flag and up the number of mapped views for the segment.  Upping the number of mapped views prevents
        // the segment from being deleted and passed to the deletion thread while we are forcing a delete.
...
/*
Routine Description:
    This function examines each prototype PTE in the section and takes the appropriate action to "delete" the prototype PTE.

    If the PTE is dirty and is backed by a file (not a paging file), the corresponding page is written to the file.

    At the completion of this service, the section which was operated upon is no longer usable.

    NOTE - ALL I/O ERRORS ARE IGNORED.  IF ANY WRITES FAIL, THE DIRTY PAGES ARE MARKED CLEAN AND THE SECTION IS DELETED.
Arguments:
    ControlArea - Supplies a pointer to the control area for the section.
    DirtyDataPagesOk - Supplies TRUE if dirty data pages are ok.  If FALSE is specified then no dirty data pages are expected (as
                       this is a dereference operation) so any encountered must be due to pool corruption so bugcheck.

                       Note that dirty image pages are always discarded.
                       This should only happen for images that were either read in from floppies or images with shared global subsections.
*/
{
    ...
    if (ControlArea->u.Flags.Image) {
        ...
        // Initializing these are not needed for correctness as they are overwritten below, but without it the compiler cannot compile
    ...
    // Stop the modified page writer from writing pages to this file, and if any paging I/O is in progress, wait for it to complete.
    ControlArea->u.Flags.NoModifiedWriting = 1;
    while (ControlArea->ModifiedWriteCount != 0) {
        // There is modified page writing in progess.  Set the flag in the control area indicating the modified page
        // writer should signal when a write to this control area is complete.  Release the PFN LOCK and wait in an
        // atomic operation.  Once the wait is satisfied, recheck to make sure it was this file's I/O that was written.
        ControlArea->u.Flags.SetMappedFileIoComplete = 1;

        // Keep APCs blocked so no special APCs can be delivered in KeWait
        // which would cause the dispatcher lock to be released opening a window where this thread could miss a pulse.
    ...
    restartchunk:
        ...
        while (PointerPte < LastPte) {
            if ((MiIsPteOnPdeBoundary(PointerPte)) || (First)) {
                First = FALSE;
                if ((ImageSection) || (MiCheckProtoPtePageState(PointerPte, MM_NOIRQL, &DroppedPfnLock))) {
                    MiMakeSystemAddressValidPfn(PointerPte, OldIrql);
                } else {
            ...
            // Prototype PTEs for Segments backed by paging file are either in demand zero, page file format, or transition.
            if (PteContents.u.Hard.Valid == 1) {
                KeBugCheckEx(POOL_CORRUPTION_IN_FILE_AREA, 0x0, (ULONG_PTR)ControlArea, (ULONG_PTR)PointerPte, (ULONG_PTR)PteContents.u.Long);
            }
            ...
            if (PteContents.u.Soft.Prototype == 1) {
                ...
                //  1. The page is part of an image which is sharable and refers to the paging file - dereference page file space and free the physical page.
                ...
                if (Pfn1->u3.e2.ReferenceCount != 0) {
                    if (DelayCount < 20) {
                        // There must be an I/O in progress on this page.  Wait for the I/O operation to complete.
                        UNLOCK_PFN(OldIrql);

                        // Drain the deferred lists as these pages may be sitting in there right now.
                        ...
                        // Redo the loop, if the delay count is greater than 20, assume that this thread is deadlocked and
                        // don't purge this page.  The file system can deal with the write operation in progress.
                    ...
                    // the fact that the I/O is in progress and delete the page.
                ...
                if (Pfn1->OriginalPte.u.Soft.Prototype == 0) {
                    ...
                    MI_SET_PFN_DELETED(Pfn1);
                    if (!ImageSection) {
                        // This is not an image section, it must be a page file backed section, therefore decrement the PFN reference count for the control area.
                        ControlArea->NumberOfPfnReferences -= 1;
                        ASSERT((LONG)ControlArea->NumberOfPfnReferences >= 0);
                    }
#if DBG
                    else {
                        // This should only happen for images with shared global subsections.
                    }
#endif
                    ...
                    // freelist, move the page to the free list, if the reference count is not zero, ignore this page.  When
                    // the reference count goes to zero, it will be placed on the free list.
                    ...
                    // If a cluster of pages to write has been completed, set the WriteNow flag.
                    if (LastWritten != NULL) {
                        WriteNow = TRUE;
                    }
                } else {
                    if ((Pfn1->u3.e1.Modified == 0) || (ImageSection)) {
                        ...
                        // Check the reference count for the page, if the reference count is zero and the page is not on
                        // the freelist, move the page to the free list, if the reference count is not zero, ignore this
                        // page. When the reference count goes to zero, it will be placed on the free list.
                        if ((Pfn1->u3.e2.ReferenceCount == 0) && (Pfn1->u3.e1.PageLocation != FreePageList)) {
                            MiUnlinkPageFromList(Pfn1);
                            MiReleasePageFileSpace(Pfn1->OriginalPte);
                            MiInsertPageInFreeList(MI_GET_PAGE_FRAME_FROM_TRANSITION_PTE(&PteContents));
            ...
            // Write the current cluster if it is complete, full, or the loop is now complete.
            PointerPte += 1;
        WriteItOut:
            DelayCount = 0;
            ...
            if ((WriteNow) || ((PointerPte == LastPte) && (LastWritten != NULL))) {
                ...
                if (DirtyDataPagesOk == FALSE) {
                    KeBugCheckEx(POOL_CORRUPTION_IN_FILE_AREA, 0x1, (ULONG_PTR)ControlArea, (ULONG_PTR)Mdl, ControlArea->u.LongFlags);
                }

                WriteNow = FALSE;

                // Make sure the write does not go past the end of file. (segment size).
                ASSERT(Subsection->ControlArea->u.Flags.Image == 0);
                ...
                while (TRUE) {
                    ...
                    Status = IoSynchronousPageWrite(ControlArea->FilePointer, Mdl, &StartingOffset, &IoEvent, &IoStatus);
                    if (NT_SUCCESS(Status)) {
                ...
                if (MiIsPteOnPdeBoundary(PointerPte) == 0) {
                    // The next PTE is not in a different page, make sure this page did not leave memory when the I/O was in progress.
                ...
                if (!NT_SUCCESS(IoStatus.Status)) {
                    if ((MmIsRetryIoStatus(IoStatus.Status)) && (MaxClusterSize != 1) && (Mdl->ByteCount > PAGE_SIZE)) {
                        // Retried I/O of a cluster have failed, reissue the cluster one page at a time as the
                        // storage stack should always be able to make forward progress this way.
                ...
                while (Page < LastPage) {
                    ...
                    if (!MI_IS_PFN_DELETED(Pfn2)) {
                        // Make sure the prototype PTE is still in the working set.
                        if (MiGetPteAddress(WrittenPte)->u.Hard.Valid == 0) {
                            MiMakeSystemAddressValidPfn(WrittenPte, OldIrql);
                        }

                        if (Pfn2->PteAddress != WrittenPte) {
                            // The PFN lock was released to make the page table page valid, and while it
                            // was released, the physical page was reused.  Go onto the next one.
                            Page += 1;
                            continue;
                        }
                        ...
                        if ((WrittenContents.u.Soft.Prototype == 0) && (WrittenContents.u.Soft.Transition == 1)) {
                            ...
                            // Check the reference count for the page, if the reference count is zero and the
                            // page is not on the freelist, move the page to the free list, if the reference
...
Arguments:
    ...
    FileNameInfo - Returns the Unicode name of the corresponding section.  The caller must free this pool block when success is returned.
Return Value:
    NTSTATUS.
Environment:
    Kernel mode, APC_LEVEL or below, no mutexes held.
*/
{
    ...
    *FileNameInfo = ExAllocatePoolWithTag(PagedPool, NumberOfBytes, '  mM');
    ...
    Status = ObQueryNameString(FileObject, *FileNameInfo, NumberOfBytes, &AdditionalLengthNeeded);
    if (!NT_SUCCESS(Status)) {
        if (Status == STATUS_INFO_LENGTH_MISMATCH) {
            // Our buffer was not large enough, retry just once with a larger one (as specified by ObQuery).  Don't try more than once to
            // prevent broken parse procedures which give back wrong AdditionalLengthNeeded values from causing problems.
...
PFILE_OBJECT MmGetFileObjectForSection(IN PVOID Section)
/*
Routine Description:
...
Environment:
    ...
    The caller must ensure that the section is valid for the duration of the call.
*/
{
    PFILE_OBJECT FileObject;
...
/*
Routine Description:
    This routine checks the reference counts for the specified control area,
    and if the counts are all zero, it marks the control area for deletion and queues it to the deletion thread.
...
#define DEREF_SEGMENT   0x2
    ...
    if ((ControlArea->NumberOfMappedViews == 0) && (ControlArea->NumberOfSectionReferences == 0)) {
        ...
        if (ControlArea->FilePointer != NULL) {
            if (ControlArea->NumberOfPfnReferences == 0) {
                // There are no views and no physical pages referenced by the Segment, dereference the Segment object.
                ControlArea->u.Flags.BeingDeleted = 1;
                ...
                // Insert this segment into the unused segment list (unless it is already on the list).
                if (ControlArea->DereferenceList.Flink == NULL) {
                    MI_INSERT_UNUSED_SEGMENT(ControlArea);
                }

                // Indicate if this section should be deleted now that the reference counts are zero.
                if (ControlArea->u.Flags.DeleteOnClose) {
                    Action |= DELETE_ON_CLOSE;
                }

                // The number of mapped views are zero, the number of section references are zero, but there are some
                // pages of the file still resident.  If this is an image with Global Memory, "purge" the subsections
                // which contain the global memory and reset them to point back to the file.
                if (ControlArea->u.Flags.GlobalMemory == 1) {
                    ...
                    ControlArea->NumberOfMappedViews -= 1;
                    if ((ControlArea->NumberOfMappedViews == 0) && (ControlArea->NumberOfSectionReferences == 0) && (ControlArea->NumberOfPfnReferences == 0)) {
                        ControlArea->u.Flags.BeingDeleted = 1;
                        Action |= DEREF_SEGMENT;
                        ControlArea->u.Flags.FilePointerNull = 1;
                ...
                // If delete on close is set and the segment was not deleted, 
                // up the count of mapped views so the control area will not be deleted when the PFN lock is released.
    ...
    if (Action != 0) {
        ...
        // If any threads are waiting for the segment, indicate the the purge operation has completed.
        if (PurgeEvent != NULL) {
...
/*
Routine Description:
    This routine checks the reference counts for the specified control area, and if the counts are all zero,
    it marks the control area for deletion and queues it to the deletion thread.
Arguments:
    ControlArea - Supplies a pointer to the control area to check.
Return Value:
    None.
...
*/
{
    ...
    MM_PFN_LOCK_ASSERT();
    if ((ControlArea->NumberOfPfnReferences == 0) && (ControlArea->NumberOfMappedViews == 0) && (ControlArea->NumberOfSectionReferences == 0)) {
        // This segment is no longer mapped in any address space nor are there any prototype PTEs within the segment
        // which are valid or in a transition state.  Queue the segment to the segment-dereferencer thread
        // which will dereference the segment object, potentially causing the segment to be deleted.
        ControlArea->u.Flags.BeingDeleted = 1;
        ASSERT(ControlArea->u.Flags.FilePointerNull == 0);
        ControlArea->u.Flags.FilePointerNull = 1;
        ...
        ExAcquireSpinLock(&MmDereferenceSegmentHeader.Lock, &OldIrql);
        if (ControlArea->DereferenceList.Flink != NULL) {
            // Remove the entry from the unused segment list and put it on the dereference list.
            RemoveEntryList(&ControlArea->DereferenceList);
            MI_UNUSED_SEGMENTS_REMOVE_CHARGE(ControlArea);
        }

        // Image sections still have useful header information in their segment even if no pages are valid or transition so put these at the tail.
        // Data sections have nothing of use if all the data pages are gone so we used to put those at the front.  Now both types go to the rear
...
/*
Routine Description:
    This routine checks the status of the control area for the specified SectionObjectPointers.  If the control area is in use, that is, the
    number of section references and the number of mapped views are not both zero, no action is taken and the function returns FALSE.

    If there is no control area associated with the specified SectionObjectPointers or the control area is in the process of being
    created or deleted, no action is taken and the value TRUE is returned.

    If, there are no section objects and the control area is not being created or deleted, the address of the control area is returned
    in the ControlArea argument, the address of a pool block to free is returned in the SegmentEventOut argument and the PFN_LOCK is still held at the return.

Arguments:
    *SegmentEventOut - Returns a pointer to NonPaged Pool which much be freed by the caller when the PFN_LOCK is released.
                       This value is NULL if no pool is allocated and the PFN_LOCK is not held.
    SectionCheckType - Supplies the type of section to check on, one of CheckImageSection, CheckDataSection, CheckBothSection.
    SectionObjectPointers - Supplies the section object pointers through which the control area can be located.
    DelayClose - Supplies a boolean which if TRUE and the control area is being used, the delay on close field should be set in the control area.
    *ControlAreaOut - Returns the address of the control area.
    PreviousIrql - Returns, in the case the PFN_LOCK is held, the previous IRQL so the lock can be released properly.
Return Value:
    FALSE if the control area is in use, TRUE if the control area is gone or in the process or being created or deleted.
Environment:
...
*/
{
    ...
    // Allocate an event to wait on in case the segment is in the process of being deleted.  This event cannot be allocated
    ...
    *ControlAreaOut = NULL;
    ...
    // Acquire the PFN lock and examine the section object pointer value within the file object.
    ...
    //  Depending on the type of section, check for the pertinent reference count being non-zero.
    ...
    if ((SectRef != 0) || (ControlArea->NumberOfMappedViews != 0) || (ControlArea->u.Flags.BeingCreated)) {
        ...
        if (DelayClose) {
            // The section should be deleted when the reference counts are zero, set the delete on close flag.
    ...
    // The segment has no references, delete it.  If the segment is already being deleted, set the event field in the control
    // area and wait on the event.
    if (ControlArea->u.Flags.BeingDeleted) {
        // The segment object is in the process of being deleted.
        // Check to see if another thread is waiting for the deletion, otherwise create and event object to wait upon.
        if (ControlArea->WaitingForDeletion == NULL) {
            ...
            // No interlock is needed for the RefCount increment as no thread can be decrementing it since it is still
        ...
        // WaitingForDeletion field must be cleared (and may be reinitialized to something else), but cannot be reset
        // to our local event.  This allows us to dereference the event count lock free.
    ...
    // NO interlock is needed for the RefCount clearing as the event counter was never pointed to by a control area.

#if DBG
    SegmentEvent->RefCount = 0;
#endif

    InterlockedPushEntrySList(&MmEventCountSListHead, (PSLIST_ENTRY)&SegmentEvent->ListEntry);
...
/*
Routine Description:
    This function maintains a list of "events" to allow waiting on segment operations (deletion, creation, purging).
Arguments:
    None.
Return Value:
    Event to be used for waiting (stored into the control area) or NULL if no event could be allocated.
Environment:
...
*/
{
    ...
    // An interlock is needed for the RefCount decrement as the event counter is no longer pointed to by a control area and thus, any number of
    ...
    if (InterlockedDecrement((PLONG)&Support->RefCount) == 0) {
        if (ExQueryDepthSList(&MmEventCountSListHead) < 4) {
            InterlockedPushEntrySList(&MmEventCountSListHead, &Support->ListEntry);
            return;
        }
        ExFreePool(Support);
    }
...
BOOLEAN MmCanFileBeTruncated(__in PSECTION_OBJECT_POINTERS SectionPointer, __in_opt PLARGE_INTEGER NewFileSize)
/*
Routine Description:
    This routine does the following:
        1.  Checks to see if a image section is in use for the file, if so it returns FALSE.
        2.  Checks to see if a user section exists for the file, if it does,
            it checks to make sure the new file size is greater than the size of the file, if not it returns FALSE.
        3.  If no image section exists, and no user created data section exists or the file's size is greater, then TRUE is returned.
Arguments:
    SectionPointer - Supplies a pointer to the section object pointers from the file object.
...
Routine Description:
    This routine does the following:
        ...
        2.  Checks to see if a user section exists for the file, if it does,
            it checks to make sure the new file size is greater than the size of the file, if not it returns FALSE.
        3.  If no image section exists, and no user created data section exists or the files size is greater, then TRUE is returned.
Arguments:
    SectionPointer - Supplies a pointer to the section object pointers from the file object.
    NewFileSize - Supplies a pointer to the size the file is getting set to.
    BlockNewViews - Supplies TRUE if the caller will block new views while the operation (usually a purge) proceeds.
                    This allows this routine to return TRUE even if the user has section references, provided the user currently has no mapped views.
    PreviousIrql - If returning TRUE, returns Irql to use when unlocking Pfn database.
Return Value:
...
*/
{
    ...
    if (ControlArea != NULL) {
        ...
        // If there are user references and the size is less than the size of the user view, don't allow the truncation.
        if ((ControlArea->NumberOfUserReferences != 0) && ((BlockNewViews == FALSE) || (ControlArea->NumberOfMappedViews != 0))) {
            // You cannot truncate the entire section if there is a user reference.
            if (!ARGUMENT_PRESENT(NewFileSize)) {
                goto UnlockAndReturn;
            }
            ...
            // If there are mapped views, we will skip the last page of the section if the size passed in falls in that page.
...
/*
Routine Description:
    This routine walks the MmUnusedSegmentList and returns a pointer to a pool allocation containing the referenced file object pointers.
Arguments:
    None.
Return Value:
    Returns a pointer to a NULL terminated pool allocation containing the file object pointers from the unused segment list, NULL if the memory could not be allocated.
    It is also the responsibility of the caller to dereference each file object in the list and then free the returned pool.
Environment:
...
#define MI_INSTRUMENT_DEREF_ACTION(i)       ASSERT (i < MI_DEREF_ACTION_SIZE);   MiDerefActions[i] += 1;
...
/*
Routine Description:
    This routine removes unused segments (no section references, no mapped views only PFN references that are in transition state).
Arguments:
    None.
Return Value:
    None.
...
*/
{
    ...
    // If overall system pool usage is acceptable, then don't discard any cache.
    ...
    while ((MI_UNUSED_SEGMENTS_SURPLUS()) || (MmUnusedSegmentForceFree != 0)) {
        ...
        if ((LoopCount & (64 - 1)) == 0) {
            ...
            // Periodically delay so the mapped and modified writers get a shot at writing out the pages this (higher priority) thread is releasing.
            ExAcquireSpinLock(&MmDereferenceSegmentHeader.Lock, &OldIrql);
            while (!IsListEmpty(&MmDereferenceSegmentHeader.ListHead)) {
                MiSubsectionActions |= 0x8000000;

                // The list is not empty, see if the first request is for a commit extension and if so, process it now.
                NextEntry = RemoveHeadList(&MmDereferenceSegmentHeader.ListHead);
                ...
                if (ControlArea->Segment != NULL) {
                    ...
                    // This is a control area deletion that needs to be processed (so the file object can be dereferenced, etc).
                    // Note the section object pointers have already been zeroed.
                    ControlArea->DereferenceList.Flink = NULL;
            ...
            // If we are looping without freeing enough pool then signal the cache manager to start unmapping
            // system cache views in an attempt to get back the paged pool containing its prototype PTEs.
            if (LoopCount >= 128) {
        ...
        // Eliminate some of the unused segments which are only kept in memory because they contain transition pages.
        ...
        if ((IsListEmpty(&MmUnusedSegmentList)) && (IsListEmpty(&MmUnusedSubsectionList))) {
            ...
            // We weren't able to get as many segments or subsections as we wanted.  So signal the cache manager to start unmapping
            // system cache views in an attempt to get back the paged pool containing its prototype PTEs.  If Cc was able to free
        ...
        if (IsListEmpty(&MmUnusedSegmentList)) {
            ...
            ProcessSubsectionsFirst :
                                    ...
                                    // The unused segment list was empty, go for the unused subsection list instead.
                                    ...
                                    // Set the flink to NULL indicating this subsection is not on any lists.
                                    ...
                                    // Up the number of mapped views to prevent other threads from freeing this.  Clear the accessed bit so we'll know
                                    // if another thread opens the subsection while we're flushing and closes it before we finish the flush - the other thread
                                    // may have modified some pages which can then cause our MiCleanSection call (which expects no modified pages in this case) to deadlock with the filesystem.
                                    ...
                                    // Increment the number of mapped views on the control area to prevent threads that are purging the section from deleting it
                                    ...
                                    // it needs to in order to avoid deadlock.  Don't hold this protection any longer than we need to.
                                    ...
                                    // Before checking for any failure codes, see if any other threads accessed the subsection while the flush was ongoing.

                                    // Note that beyond the case of another thread currently using the subsection, the more subtle one is where another
                                    ...
                                    // If any of these cases have occurred, grant this subsection a reprieve.
                                    ASSERT(MappedSubsection->u.SubsectionFlags.SubsectionStatic == 0);
                                    ...
                                    Requeue:
                                        ...
                                        // If the other thread(s) are done with this subsection, it MUST be requeued here - otherwise if there are any
                                        // pages in the subsection, when they are reclaimed, MiCheckForControlAreaDeletion checks for and expects
                                        // the control area to be queued on the unused segment list.

                                        // Note this must be done very carefully because if the other threads are not done with the subsection, 
                                        // it had better not get put on the unused subsection list.
                                    ...
                                    if (!NT_SUCCESS(Status)) {
                                        ...
                                        // If the filesystem told us it had to unroll to avoid deadlock OR we hit a mapped writer collision OR the error occurred on a local file:

                                        // Then requeue this at the end so we can try again later.

                                        // Any other errors for networked files are assumed to be permanent (ie: the link may have gone down for an indefinite period), 
                                        // so these sections are cleaned regardless.
                                        ASSERT((LONG_PTR)MappedSubsection->NumberOfMappedViews >= 1);
                                        MappedSubsection->NumberOfMappedViews -= 1;
                                        ...
                                        // 10 consecutive file locking failures means we need to yield the processor to allow the filesystem to unjam.
                                        // Nothing magic about 10, just a number so it gives the worker threads a chance to run.
                                    ...
                                    // The final check that must be made is whether any faults are currently in progress which are backed by this subsection.
                                    // Note this is the perverse case where one thread in a process has unmapped the relevant VAD even while other threads in the
                                    // same process are faulting on the addresses in that VAD (if the VAD had not been unmapped then the subsection view count would
                                    // have been nonzero and caught above).  Clearly this is a bad process, but nonetheless it must be detected and handled here
                                    // because upon conclusion of the inpage, the thread will compare (unsynchronized) against the prototype PTEs which may in
                                    ...
                                    // Note checking the prototype PTEs must be done carefully as they are pageable and the PFN lock is (and must be) held.
                                    ...
                                    while (ProtoPtes2 < LastProtoPte) {
                                        if ((ProtoPtes2 == ProtoPtes) || (MiIsPteOnPdeBoundary(ProtoPtes2))) {
                                            if (MiCheckProtoPtePageState(ProtoPtes2, OldIrql, &DroppedPfnLock) == FALSE) {
                                                // Skip this chunk as it is paged out and thus, cannot have any valid or transition PTEs within it.
                                                ...
                                                // The prototype PTE page is resident right now - but if the PFN lock was dropped & reacquired to make it
                                                // so, then anything could have changed - so everything must be rechecked.
                                        ...
                                        if (PteContents.u.Soft.Prototype == 1) {
                                            ...
                                            if (Pfn1->u3.e1.Modified == 1) {
                                                // An I/O transfer finished after the last view was unmapped.  MmUnlockPages can set the modified bit
                                                // in this situation so it must be handled properly here - ie: mark the subsection as needing to be reprocessed and march on.
                                            ...
                                            if (Pfn1->u3.e2.ReferenceCount != 0) {
                                                // A fault is being satisfied for deleted address space, so don't eliminate this subsection right now.
                                    ...
                                    // Sever the subsection's tie to the prototype PTEs while still holding the lock and then decrement the counts on any resident prototype pages.
                                    ASSERT(MappedSubsection->NumberOfMappedViews == 1);
                                    ...
                                    while (ProtoPtes2 < LastProtoPte) {
                                        if ((ProtoPtes2 == ProtoPtes) || (MiIsPteOnPdeBoundary(ProtoPtes2))) {
                                            if (MiCheckProtoPtePageState(ProtoPtes2, OldIrql, &DroppedPfnLock) == FALSE) {
                                                ...
                                                // The prototype PTE page is resident right now - but if the PFN lock was dropped & reacquired to make it
                                                // so, then anything could have changed - but notice that the SubsectionBase was zeroed above before
                                                // entering this loop, so even if the PFN lock was dropped & reacquired, nothing needs to be rechecked.
                                            }
                                        ...
                                        if (PteContents.u.Soft.Prototype == 1) {
                                            ...
                                            // If the page is on the standby list, move it to the freelist.  If it's not on the standby list (ie: I/O
                                            // is still in progress), when the Iast I/O completes, the page will be placed on the freelist as the PFN entry
                                    ...
                                    // If all the cached pages for this control area have been removed then delete it.  This will actually insert the control
        ...
        // Set the flink to NULL indicating this control area is not on any lists.
        MI_INSTRUMENT_DEREF_ACTION(38);
        ControlArea->DereferenceList.Flink = NULL;

        MI_LOG_DEREF_INFO(0x30, 0, ControlArea);

        if ((ControlArea->NumberOfMappedViews == 0) && (ControlArea->NumberOfSectionReferences == 0) && (ControlArea->u.Flags.BeingDeleted == 0)) {
            // If there is paging I/O in progress on this segment, just put this at the tail of the list, as the call to MiCleanSegment would block waiting
            // for the I/O to complete.  As this could tie up the thread, don't do it.  
            // Check if these are the only types of segments on the dereference list so we don't
            ...
            // Up the number of mapped views to prevent other threads from freeing this.  Clear the accessed bit so we'll know
            // if another thread opens the control area while we're flushing and closes it before we finish the flush - the other thread
            // may have modified some pages which can then cause our MiCleanSection call (which expects no modified pages in this case) to deadlock with the filesystem.
            ControlArea->NumberOfMappedViews = 1;
            ...
            if (ControlArea->u.Flags.Image == 0) {
                ...
                while (Subsection->SubsectionBase == NULL) {
                    ...
                    if (Subsection == NULL) {
                        ...
                        // All the subsections for this segment have already been trimmed so nothing left to flush.  Just get rid
                        // of the segment carcass provided no other thread accessed it while we weren't holding the PFN lock.
                ...
                // Preacquire the file to prevent deadlocks with other flushers
                // Also mark ourself as a top level IRP so the filesystem knows we are holding no other resources and that it can unroll if
                // it needs to in order to avoid deadlock.  Don't hold this protection any longer than we need to.
            ...
            // Before checking for any failure codes, see if any other threads accessed the control area while the flush was ongoing.

            // Note that beyond the case of another thread currently using the control area, the more subtle one is where another
            // thread accessed the control area and modified some pages.
            // The flush needs to redone (so the clean is guaranteed to work)
            // before another clean can be issued.

            // If any of these cases have occurred, grant this control area a reprieve.
                  ...
                  (ControlArea->u.Flags.BeingDeleted == 0))) 
            {
                ...
                // it MUST be requeued here - otherwise if there are any pages in the control area, when they are reclaimed,
                // MiCheckForControlAreaDeletion checks for and expects the control area to be queued on the unused segment list.

                // Note this must be done very carefully because if the other threads are not done with the control area,
                // it had better not get put on the unused segment list.
            ...
            // Don't let any pages be written by the modified page writer from this point on.
            ControlArea->u.Flags.NoModifiedWriting = 1;
            ASSERT(ControlArea->u.Flags.FilePointerNull == 0);
            UNLOCK_PFN(OldIrql);
            ...
            // The segment was not eligible for deletion.  Just leave it off the unused segment list and continue the loop.
