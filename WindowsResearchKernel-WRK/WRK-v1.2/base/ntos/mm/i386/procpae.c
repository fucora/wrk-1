...
*/
{
    ...
    LOCK_PFN(OldIrql);// Get the PFN lock to get physical pages.
    ...
    if (MmAvailablePages < MM_HIGH_LIMIT) {// Allocate the hyper space page table page.
        MiEnsureAvailablePageOrWait(NULL, OldIrql);
    }
    ...
    if (MmAvailablePages < MM_HIGH_LIMIT) {// Remove page(s) for the VAD bitmap.
        MiEnsureAvailablePageOrWait(NULL, OldIrql);
    }
    ...
    if (MmAvailablePages < MM_HIGH_LIMIT) {// Remove a page for the working set list.
        MiEnsureAvailablePageOrWait(NULL, OldIrql);
    }
    ...
    TempPte.u.Hard.PageFrameNumber = VadBitMapPage;// Initialize the PTEs for hyperspace and the VAD bitmap mapping.
    ...
    // Add the new process to our internal list prior to filling any system PDEs so if a system PDE changes (large page map or unmap)
    ...
    MappingPte = MiReserveSystemPtes(1, SystemPteSpace);
    if (MappingPte != NULL) {
        MI_MAKE_VALID_KERNEL_PTE(TempPte2, PageDirectories[PD_PER_SYSTEM - 1], MM_READWRITE, MappingPte);
        MI_SET_PTE_DIRTY(TempPte2);
        MI_WRITE_VALID_PTE(MappingPte, TempPte2);
        PointerPte = MiGetVirtualAddressMappedByPte(MappingPte);
    } else {
    ...
    // Copy the entire page directory page for the highest GB so all the kernel mappings are inherited.
    ...
    PdeOffset = MiGetPdeOffset(HYPER_SPACE);// Map the working set page table page.

    TempPte.u.Hard.PageFrameNumber = HyperSpaceIndex;
    PointerPte[PdeOffset] = TempPte;

    // Zero the remaining page directory range used to map the working set list and its hash.
    PdeOffset += 1;
    ASSERT(MiGetPdeOffset(MmHyperSpaceEnd) >= PdeOffset);

    MiZeroMemoryPte(&PointerPte[PdeOffset], (MiGetPdeOffset(MmHyperSpaceEnd) - PdeOffset + 1));
    ...
    // This includes kernel/HAL code & data, the PFN database, initial nonpaged pool, any extra system PTE or system cache areas, system views and session space.
    if (MmSystemRangeStart < (PVOID)0xC0000000) {
        PageDirectoryIndex = MI_GET_PAGE_FRAME_FROM_PTE(&PaeVa->PteEntry[PD_PER_SYSTEM - 2]);
        MappingPte = MiReserveSystemPtes(1, SystemPteSpace);
        if (MappingPte != NULL) {
            MI_MAKE_VALID_KERNEL_PTE(TempPte2, PageDirectoryIndex, MM_READWRITE, MappingPte);
            MI_SET_PTE_DIRTY(TempPte2);
            MI_WRITE_VALID_PTE(MappingPte, TempPte2);
            PointerPte = MiGetVirtualAddressMappedByPte(MappingPte);
        } else {
        ...
        RtlCopyMemory(PointerFillPte, CurrentAddressSpacePde, PAGE_SIZE - PdeOffset * sizeof(MMPTE));
        if (MappingPte != NULL) {
            MiReleaseSystemPtes(MappingPte, 1, SystemPteSpace);
        } else {
            MiUnmapPageInHyperSpace(CurrentProcess, PointerPte, OldIrql);
    ...
    MiSessionAddProcess(NewProcess);// Up the session space reference count.
    return TRUE;
}
...
/*
Routine Description:
    This routine updates the system PDEs, typically due to a large page system PTE mapping being created or destroyed.  This is rare.
    Note this is only needed for 32-bit platforms (64-bit platforms share a common top level system page).
Arguments:
    Process - Supplies a pointer to the process to update.
Environment:
    Kernel mode, expansion lock held.
    The caller acquired the expansion lock prior to clearing the update bit from this process.
    We must update the PDEs prior to releasing it so that any new updates can also be rippled.
*/
{
    ULONG PdeOffset;
    ULONG PdeEndOffset;
    ...
    // Map the system process page directory as we know that's always kept up to date.
    PointerPte = MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageDirectoryIndex);
    ...
    for (i = 0; i < MiPteRangeIndex; i += 1) {
        ...
        RtlCopyMemory(TargetAddressSpacePde, PointerPde, (PdeEndOffset - PdeOffset + 1) * sizeof(MMPTE));
    }

    MiUnmapPageInHyperSpaceFromDpc(CurrentProcess, PointerPte);

    // Just invalidate the mapping on the current processor as we cannot have context switched.
