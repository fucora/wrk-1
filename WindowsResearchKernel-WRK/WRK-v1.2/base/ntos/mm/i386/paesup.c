...
Return Value:
    ...
    The page returned must be below physical 4GB as required by the processor.
...
*/
{
    ...
    do {
        ...
        if (MiPaeAllocatePages() == 0) {
            ...
            // Since all the working sets have been trimmed, check whether another thread has replenished our list.
            // If not, then attempt to do so since the working set pain has already been absorbed.
...
*/
{
    ...
    // This is a potentially expensive call so pick up a chunk of pages at once to amortize the cost.
    MemoryDescriptorList = MiAllocatePagesForMdl(LowAddress, HighAddress, SkipBytes, NumberOfPages << PAGE_SHIFT, MiCached, 0);
    ...
    // Map each page individually as they may need to be freed individually later.
    for (i = 0; i < ActualPages; i += 1) {
        ...
        // Put the first chunk into the SLIST if it's still low, and just enqueue all the other entries normally.
        if ((i == 0) && (ExQueryDepthSList(&MiPaeEntrySList) < MINIMUM_PAE_SLIST_THRESHOLD)) {
            ...
            InterlockedPushListSList(&MiPaeEntrySList, (PSLIST_ENTRY)((PPAE_ENTRY)BaseAddress + 1), (PSLIST_ENTRY)Pae, PAES_PER_PAGE - 1);
        } else {
            KeAcquireInStackQueuedSpinLock(&MiPaeLock, &LockHandle);
