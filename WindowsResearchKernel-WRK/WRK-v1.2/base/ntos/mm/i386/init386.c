...
Abstract:
    ...
    It is specifically tailored to the INTEL x86 and PAE machines.
*/
...

{
    ...
    for (i = 0; i < MiLargeVaRangeIndex; i += 1) {
        ...
        if (EntryFound == FALSE) {
            ...
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_WARNING_LEVEL, "MM: Loader/HAL memory block indicates large pages cannot be used for %p->%p\n", MiLargeVaRanges[i].VirtualAddress, MiLargeVaRanges[i].EndVirtualAddress);
            ...
            for (j = 0; j < MiLargeVaRangeIndex; j += 1) {
                ...
                if ((PageFrameIndex2 >= PageFrameIndex) && (PageFrameIndex2 <= LastPageFrameIndex)) {
                    DbgPrintEx(DPFLTR_MM_ID, DPFLTR_WARNING_LEVEL, "MM: Disabling large pages for all ranges due to overlap\n");
...

{
    ...
    RtlCopyMemory((PVOID)&MiSystemPaeVa, (PVOID)(KSEG0_BASE | DirBase), sizeof(MiSystemPaeVa));
    ...
    *(PMEMORY_ALLOCATION_DESCRIPTOR)&MxOldFreeDescriptor = *MxFreeDescriptor;
    ...
    for (i = 0; i < PdeCount; i += 1) {
        ...
        if ((Pde->u.Hard.Valid == 1) && (Pde->u.Hard.LargePage == 0)) {
            ...
            TempPde.u.Long = MiDetermineUserGlobalPteMask(PointerPte) & ~MM_PTE_ACCESS_MASK;// Set global bit.
            ...
            for (j = 0; j < PTE_PER_PAGE; j += 1) {
                if (PointerPte->u.Hard.Valid == 1) {
                    ...
                    if ((MiIsRegularMemory(LoaderBlock, (PFN_NUMBER)PointerPte->u.Hard.PageFrameNumber)) &&
                        ...
                        ((va >= MM_KSEG2_BASE) &&
                        ((va < KSEG0_BASE + MmVirtualBias) || (va >= (KSEG0_BASE + MmVirtualBias + MmBootImageSize)))) ||
                         ((MmVirtualBias == 0) && (va >= (ULONG)MmNonPagedPoolStart) && (va < (ULONG)MmNonPagedPoolStart + MmSizeOfNonPagedPoolInBytes))) {
