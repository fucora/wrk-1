...
Abstract:
    This module contains the routines which implement the quota and commitment charging for memory management.
*/
...
/*
Routine Description:
    This routine checks to ensure the system has sufficient page file space remaining.
    Since this routine is generally used to charge commitment on behalf of usermode or other optional actions, this routine does not allow the
    caller to use up the very last morsels of commit on the premise that the operating system and drivers can put those to better use than any application in order to prevent the appearance of system hangs.
Arguments:
    QuotaCharge - Supplies the quota amount to charge.
    Process - Optionally supplies the current process IF AND ONLY IF the working set mutex is held.
              If the paging file is being extended, the working set mutex is released if this is non-null.
Return Value:
    TRUE if there is sufficient space, FALSE if not.
Environment:
    Kernel mode, APCs disabled, WorkingSetLock and AddressCreation mutexes *MAY* be held.
*/
{
    ...
    do {
        ...
        while (NewCommitValue + MmSystemCommitReserve > MmTotalCommitLimit) {
            // If the pagefiles are already at the maximum, then don't bother trying to extend them, but do trim the cache.
            ...
            // This is done in the context of a system thread due to mutexes which may currently be held.
...
#if defined(_WIN64)
        NewCommitValue = InterlockedCompareExchange64((PLONGLONG)&MmTotalCommittedPages, (LONGLONG)NewCommitValue, (LONGLONG)OldCommitValue);
#else
        NewCommitValue = InterlockedCompareExchange((PLONG)&MmTotalCommittedPages, (LONG)NewCommitValue, (LONG)OldCommitValue);
#endif
    ...
    // Success.
    // If system commit exceeds 90%, attempt a preemptive pagefile increase anyway.
    ...
    if (NewCommitValue > ((CommitLimit / 10) * 9)) {
        if (CommitLimit < MmTotalCommitLimitMaximum) {
            // Attempt to expand the paging file, but don't wait to see if it succeeds.
...
Routine Description:
    ...
    The routine determines if the paging file space is exhausted, and if so, it attempts to ascertain if the paging file space could be expanded.
Arguments:
    QuotaCharge - Supplies the quota amount to charge.
...
/*
Routine Description:
    This routine attempts to charge the specified commitment without expanding the paging file.
...
/*
Routine Description:
    This routine examines the range of pages from the starting address up to and including the ending address and returns the commit charge for the pages within the range.
Arguments:
...
Environment:
    Kernel mode, APCs disabled, WorkingSetLock and AddressCreation mutexes held.
*/
{
    PMMPTE PointerPte;
    PMMPTE LastPte;
        ...
        do {
            ...
            retry:
                 ...
                 while (!MiDoesPxeExistAndMakeValid(PointerPxe, Process, MM_NOIRQL, &Waited)) {
                     // No PXE exists for the starting address, therefore the page is not committed.
                     PointerPxe += 1;
                 ...
                 while (!MiDoesPpeExistAndMakeValid(PointerPpe, Process, MM_NOIRQL, &Waited)) {
                     // No PPE exists for the starting address, therefore the page is not committed.
                     PointerPpe += 1;
                     PointerPde = MiGetVirtualAddressMappedByPte(PointerPpe);
                     PointerPte = MiGetVirtualAddressMappedByPte(PointerPde);
                     if (PointerPte > LastPte) {
                 ...
                 while (!MiDoesPdeExistAndMakeValid(PointerPde, Process, MM_NOIRQL, &Waited)) {
                     // No PDE exists for the starting address, therefore the page is not committed.
                     PointerPde += 1;
                     PointerPte = MiGetVirtualAddressMappedByPte(PointerPde);
                     if (PointerPte > LastPte) {
                         return NumberOfCommittedPages;
    ...
    restart:
        while (PointerPte <= LastPte) {
            if (MiIsPteOnPdeBoundary(PointerPte)) {
                // This is a PDE boundary, check to see if the all the PXE/PPE/PDE pages exist.
                PointerPde = MiGetPteAddress(PointerPte);
                PointerPpe = MiGetPteAddress(PointerPde);
                PointerPxe = MiGetPteAddress(PointerPpe);
                do {
                    if (!MiDoesPxeExistAndMakeValid(PointerPxe, Process, MM_NOIRQL, &Waited)) {
                        // No PDE exists for the starting address, check the VAD to see if the pages are not committed.
                        PointerPxe += 1;
                        PointerPpe = MiGetVirtualAddressMappedByPte(PointerPxe);
                        PointerPde = MiGetVirtualAddressMappedByPte(PointerPpe);
                        PointerPte = MiGetVirtualAddressMappedByPte(PointerPde);
                        goto restart;// Check next page.
                    }
                    ...
                    if (!MiDoesPpeExistAndMakeValid(PointerPpe, Process, MM_NOIRQL, &Waited)) {
                        // No PDE exists for the starting address, check the VAD to see if the pages are not committed.
                        PointerPpe += 1;
                        PointerPxe = MiGetPteAddress(PointerPpe);
                        PointerPde = MiGetVirtualAddressMappedByPte(PointerPpe);
                    ...
                    if (!MiDoesPdeExistAndMakeValid(PointerPde, Process, MM_NOIRQL, &Waited)) {
                        // No PDE exists for the starting address, check the VAD to see if the pages are not committed.
                        PointerPde += 1;
                        PointerPpe = MiGetPteAddress(PointerPde);
                        PointerPxe = MiGetPteAddress(PointerPpe);
            ...
            if (PointerPte->u.Long != 0) {
                ...
                if (MiIsPteDecommittedPage(PointerPte)) {
                    NumberOfCommittedPages -= 1;// This page is decommitted, remove it from the count.
                }
            }
...
restart2:
    while (PointerPte <= LastPte) {
        if (MiIsPteOnPdeBoundary(PointerPte)) {
            // This is a PDE boundary, check to see if the entire PXE/PPE/PDE pages exist.
            PointerPde = MiGetPteAddress(PointerPte);
            PointerPpe = MiGetPteAddress(PointerPde);
            PointerPxe = MiGetPdeAddress(PointerPde);
            do {
                if (!MiDoesPxeExistAndMakeValid(PointerPxe, Process, MM_NOIRQL, &Waited)) {
                    // No PXE exists for the starting address, check the VAD to see if the pages are not committed.
                    PointerPxe += 1;
                    PointerPpe = MiGetVirtualAddressMappedByPte(PointerPxe);
                    PointerPde = MiGetVirtualAddressMappedByPte(PointerPpe);
                    PointerPte = MiGetVirtualAddressMappedByPte(PointerPde);

                    goto restart2;// Check next page.
                }
                ...
                if (!MiDoesPpeExistAndMakeValid(PointerPpe, Process, MM_NOIRQL, &Waited)) {
                    // No PPE exists for the starting address, check the VAD to see if the pages are not committed.
                    PointerPpe += 1;
                    PointerPxe = MiGetPteAddress(PointerPpe);
                    PointerPde = MiGetVirtualAddressMappedByPte(PointerPpe);
                    PointerPte = MiGetVirtualAddressMappedByPte(PointerPde);
        ...
        if ((PointerPte->u.Long != 0) && (!MiIsPteDecommittedPage(PointerPte))) {
            NumberOfCommittedPages += 1;// This page is committed, count it.
        }
...
/*
Routine Description:
    This routine returns commitment for COMPLETE page table pages which span the virtual address range.
    For example (assuming 4k pages), if the StartingAddress =  64k and the EndingAddress = 5mb,
    no page table charges would be freed as a complete page table page is not covered by the range.
    However, if the StartingAddress was 4mb and the EndingAddress was 9mb, 1 page table page would be freed.
Arguments:
    StartingAddress - Supplies the starting address of the range.
    EndingAddress - Supplies the ending address of the range.
...
*/
{
    ...
    if (PreviousPage == FirstPage) {
        FirstPage += 1;// A VAD is within the starting page table page.
    }

    if (NextPage == LastPage) {
        LastPage -= 1;// A VAD is within the ending page table page.
    }
    ...
    if (PreviousPpPage == FirstPpPage) {
        FirstPpPage += 1;// A VAD is within the starting page directory parent page.
    }

    if (NextPpPage == LastPpPage) {
        LastPpPage -= 1;// A VAD is within the ending page directory parent page.
    }
    ...
    if (PreviousPdPage == FirstPdPage) {
        FirstPdPage += 1;// A VAD is within the starting page directory page.
    }

    if (NextPdPage == LastPdPage) {
        LastPdPage -= 1;// A VAD is within the ending page directory page.
    }
...
/*
Routine Description:
    This function causes an over commit popup to occur (if the popup has never been sent before).
*/
{
    LONG PopupNumber;

    // Give the user a meaningful message - either to increase the minimum, maximum, or both.
    if ((MmTotalCommittedPages > MmTotalCommitLimitMaximum - 100) || (MmTotalCommitLimit == MmTotalCommitLimitMaximum)) {
...
/*
Routine Description:
    This function is called (with a spinlock) whenever PS detects a quota limit has been exceeded.
    The purpose of this function is to attempt to increase the specified quota.
Arguments:
...
*/
{
    ...
    if (PoolType == PagedPool) {
        ...
        if (MmSizeOfPagedPoolInPages < (PagedPoolInfo->AllocatedPagedPool + ((MMPAGED_QUOTA_CHECK) >> PAGE_SHIFT))) {
            return FALSE;
        }
        ...
        *NewQuotaLimit = OldQuotaLimit + (MMPAGED_QUOTA_INCREASE);
        ...
        // Make sure 200 pages and 5mb of nonpaged pool expansion available.
        // Raise quota by 64k.
