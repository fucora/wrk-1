...
Abstract:
    This module contains the routines which implement the NtCreateSection and NtOpenSection.
*/
...
// The maximum number of image object (object table entries) is the number which will fit into the MM_MAXIMUM_IMAGE_HEADER with the start of the PE image header in the last word of the first.
#define MM_MAXIMUM_IMAGE_SECTIONS       ((MM_MAXIMUM_IMAGE_HEADER - (PAGE_SIZE + sizeof(IMAGE_NT_HEADERS))) / sizeof(IMAGE_SECTION_HEADER))
...
VOID MiUpdateImageHeaderPage(IN PMMPTE PointerPte, IN PFN_NUMBER PageFrameNumber, IN PCONTROL_AREA ControlArea, IN LOGICAL MarkModified);
...
LOGICAL MiCheckDosCalls(IN PIMAGE_OS2_HEADER Os2Header, IN ULONG HeaderSize);
VOID MiInsertImageSectionObject(IN PFILE_OBJECT File, IN PCONTROL_AREA ControlArea);
LOGICAL MiFlushDataSection(IN PFILE_OBJECT File);
...
/*
Routine Description:
    This function creates a section object and opens a handle to the object with the specified desired access.
Arguments:
    ...
    MaximumSize - Supplies the maximum size of the section in bytes.
                  This value is rounded up to the host page size and specifies the size of the section (page file backed section) or the maximum size to which a file can be extended or mapped (file backed section).
    SectionPageProtection - Supplies the protection to place on each page in the section.
                            One of PAGE_READ, PAGE_READWRITE, PAGE_EXECUTE, or PAGE_WRITECOPY and, optionally, PAGE_NOCACHE or PAGE_WRITECOMBINE may be specified.
    AllocationAttributes - Supplies a set of flags that describe the allocation attributes of the section.

        AllocationAttributes Flags

        SEC_BASED - The section is a based section and will be allocated at the same virtual address in each process address space that receives the section.
                    This does not imply that addresses are reserved for based sections.
                    Rather if the section cannot be mapped at the based address an error is returned.
        ...
        SEC_FILE - The file specified by the file handle is a mapped file.
                   If a file handle is supplied and neither SEC_IMAGE or SEC_FILE is supplied, SEC_FILE is assumed.
        SEC_NO_CHANGE - Once the file is mapped, the protection cannot be changed nor can the view be unmapped.
                        The view is unmapped when the process is deleted.
                        Cannot be used with SEC_IMAGE.
        SEC_LARGE_PAGES - The section will be created using large pages.
                          This physical memory is allocated immediately upon section creation (instead of upon access).
                          If contiguous physical memory for the large page(s) cannot be found, then the section creation will fail (rather than use small pages).

                          Since this physical memory is allocated up front and is nonpaged (to ensure it remains physically contiguous and in a large page), privilege is required.
                          Since section mappers can extend the section life indefinitely, section creators should ensure proper access permissions are used.

                          Note subsequent mapped views must specify a large page aligned start address and length (or must have available virtual space so the system can find one) for them to automatically receive large page(s) for the view.
                          Callers of the map APIs do not need to specify large pages - the system will automatically provide the mapping in large pages (assuming satisfactory alignment) or small pages if not.
    FileHandle - Supplies an optional handle of an open file object.
                 If the value of this handle is NULL, then the section will be backed by a paging file.
...
*/
{
    ...
    if ((AllocationAttributes & ~(SEC_COMMIT | SEC_RESERVE | SEC_BASED | SEC_LARGE_PAGES | SEC_IMAGE | SEC_NOCACHE | SEC_NO_CHANGE)) != 0) {
    ...
    if ((AllocationAttributes & SEC_IMAGE) && (AllocationAttributes & (SEC_COMMIT | SEC_RESERVE | SEC_LARGE_PAGES | SEC_NOCACHE | SEC_NO_CHANGE))) {
        return STATUS_INVALID_PARAMETER_6;
    }
    ...
    if ((SectionPageProtection & PAGE_NOCACHE) || (SectionPageProtection & PAGE_WRITECOMBINE) || (SectionPageProtection & PAGE_GUARD) || (SectionPageProtection & PAGE_NOACCESS)) {
        return STATUS_INVALID_PAGE_PROTECTION;// No cache is only specified through the SEC_NOCACHE option in the allocation attributes.
    }
...
#if !defined (_WIN64)
                // Note we only probe for byte alignment because prior to 2195, we never probed at all!  We don't want to break user apps that had bad alignment if they worked before.
...
retry:
    ...
    Status = MmCreateSection(&Section, DesiredAccess, ObjectAttributes, &CapturedSize, SectionPageProtection, AllocationAttributes, FileHandle, NULL);
    ASSERT(KeGetCurrentIrql() < DISPATCH_LEVEL);
    if (!NT_SUCCESS(Status)) {
        if ((Status == STATUS_FILE_LOCK_CONFLICT) && (RetryCount < 3)) {
            // The file system may have prevented this from working due to log file flushing.  Delay and try again.
            RetryCount += 1;
...
Routine Description:
    This function creates a section object and opens a handle to the object with the specified desired access.
Arguments:
    Section - A pointer to a variable that will receive the section object address.
    DesiredAccess - The desired types of access for the section.
    DesiredAccess Flags
         EXECUTE - Execute access to the section is desired.
         READ - Read access to the section is desired.
    ...
    InputMaximumSize - Supplies the maximum size of the section in bytes.
                       This value is rounded up to the host page size and specifies the size of the section (page file backed section) or
                       the maximum size to which a file can be extended or mapped (file backed section).
    SectionPageProtection - Supplies the protection to place on each page in the section.
                            One of PAGE_READ, PAGE_READWRITE, PAGE_EXECUTE, or PAGE_WRITECOPY and, optionally, PAGE_NOCACHE or PAGE_WRITECOMBINE may be specified.

                            Note the NO_CACHE, write combined and GUARD attributes are mutually exclusive.
    ...
    AllocationAttributes - Supplies a set of flags that describe the allocation attributes of the section.
        ...
        SEC_LARGE_PAGES - The section will be created using large pages.
                          ...
                          Note subsequent mapped views must specify a large page aligned start address and length
                          (or must have available virtual space so the system can find one) for them to automatically receive large page(s) for the view.
                          Callers of the map APIs do not need to specify large pages - the system will automatically provide the mapping in large pages (assuming satisfactory alignment) or small pages if not.
    FileHandle - Supplies an optional handle of an open file object.
                 If the value of this handle is null, then the section will be backed by a paging file.
                 Otherwise the section is backed by the specified data file.
    FileObject - Supplies an optional pointer to the file object.
                 If this value is NULL and the FileHandle is NULL, then there is no file to map (image or mapped file).
                 If this value is specified, then the File is to be mapped as a MAPPED FILE and NO file size checking will be performed.

                 ONLY THE SYSTEM CACHE SHOULD PROVIDE A FILE OBJECT WITH THE CALL!! as this is optimized to not check the size, only do data mapping, no protection check, etc.
...
*/
{
    ...
    ASSERT((AllocationAttributes & ~(SEC_COMMIT | SEC_RESERVE | SEC_BASED | SEC_LARGE_PAGES | SEC_IMAGE | SEC_NOCACHE | SEC_NO_CHANGE)) == 0);
    ASSERT((AllocationAttributes & (SEC_COMMIT | SEC_RESERVE | SEC_IMAGE)) != 0);
    ASSERT(!((AllocationAttributes & SEC_IMAGE) && (AllocationAttributes & (SEC_COMMIT | SEC_RESERVE | SEC_NOCACHE | SEC_NO_CHANGE))));
    ASSERT(!((AllocationAttributes & SEC_COMMIT) && (AllocationAttributes & SEC_RESERVE)));
    ASSERT(!((SectionPageProtection & PAGE_NOCACHE) || (SectionPageProtection & PAGE_WRITECOMBINE) || (SectionPageProtection & PAGE_GUARD) || (SectionPageProtection & PAGE_NOACCESS)));
    ...
    // Initializing Segment is not needed for correctness, but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    Segment = (PSEGMENT)-1;
    if (ARGUMENT_PRESENT(FileHandle) || ARGUMENT_PRESENT(FileObject)) {
        ...
        // If a FileObject is supplied, this must be from the file system and therefore the file's size should not be checked.
        if (ARGUMENT_PRESENT(FileObject)) {
            ...
            if (File->SectionObjectPointer->DataSectionObject) {
                ...
                if ((ControlArea != NULL) && (!ControlArea->u.Flags.BeingDeleted) && (!ControlArea->u.Flags.BeingCreated)) {
                    // Control area exists and is not being deleted, reference it.
                    NewSegment = ControlArea->Segment;
                    if ((ControlArea->NumberOfSectionReferences == 0) && (ControlArea->NumberOfMappedViews == 0) && (ControlArea->ModifiedWriteCount == 0)) {
                        // Dereference the current file object (after releasing the PFN lock) and reference this one.
                    ...
                    // Inform the object manager to defer this deletion by queueing it to another thread to eliminate deadlocks with the redirector.
            ...
            Status = ObReferenceObjectByHandle(FileHandle, FileDesiredAccess, IoFileObjectType, PreviousMode, (PVOID *)&File, NULL);
            if (!NT_SUCCESS(Status)) {
                return Status;
            }

            // If this file doesn't have a section object pointer, return an error.
        ...
        // If not, indicate in the file object's pointer to an FCB that a section is being built.
        // This synchronizes segment creation for the file.
        if (AllocationAttributes & SEC_IMAGE) {
            // This control area is always just a place holder - the real one is allocated in MiCreateImageFileMap and will be allocated with the correct size and this one freed in a short while.

            // This place holder must always be allocated as a large control area so that it can be chained for the per-session case.
            ControlAreaSize = sizeof(LARGE_CONTROL_AREA) + sizeof(SUBSECTION);

            // For image sections, make sure that Cc has released all references to the section due to a previous data section mapping.
            // This will decrease the chance that the image will need to be backed by the pagefile because writeable data sections remain on the file.
            CcWaitForUninitializeCacheMap(File);
        } else {
            // Data files are mapped with larger subsections than images or pagefile-backed shared memory.  Factor that in here.
        ...
        // We only need the file resource if this was a user request, i.e. not a call from the cache manager or file system.
        ...
        // Initializing GlobalNeeded is not needed for correctness, but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
        GlobalNeeded = FALSE;
        SegmentEvent = NULL;
    ...
    RecheckSegment:
        ...
        if (ControlArea != NULL) {
            // A segment already exists for this file.
            // Make sure that it is not in the process of being deleted, or being created.
            if ((ControlArea->u.Flags.BeingDeleted) || (ControlArea->u.Flags.BeingCreated)) {
                // The segment object is in the process of being deleted or created.
                // Check to see if another thread is waiting for the deletion, otherwise create an event object to wait upon.
                if (ControlArea->WaitingForDeletion == NULL) {
                    // Initialize an event and put its address in the control area.
                    if (SegmentEvent == NULL) {
                        // Allocate an event to wait as the segment is in the process of being deleted.
                        // This event cannot be allocated with the PFN database locked as pool expansion would deadlock.
                        UNLOCK_PFN(OldIrql);
            ...
            // There is already a segment for this file, have this section refer to that segment.
            // No need to reference the file object any more.
            if ((ControlArea->u.Flags.ImageMappedInSystemSpace) && (AllocationAttributes & SEC_IMAGE) && (KeGetPreviousMode() != KernelMode)) {
                UNLOCK_PFN(OldIrql);
            ...
            // If this reference was not from the cache manager up the count of user references.
        ...
        if (NewSegment != NULL) {
            ...
            // If we're creating an imagemap, flush the data section if there is one.
            if (AllocationAttributes & SEC_IMAGE) {
            ...
            // The section is in paged pool, this can't be set until the PFN mutex has been released.
            if ((!IgnoreFileSizing) && (ControlArea->u.Flags.Image == 0)) {
                // The file size in the segment may not match the current file size, query the file system and get the file size.
                ...
                if (EndOfFile == 0 && *MaximumSize == 0) {
                    // Can't map a zero length without specifying the maximum size as non-zero.
                ...
                EndOfFile = (UINT64)NewSegment->SizeOfSegment;// The size is okay in the segment.
            }
            ...
            if (*MaximumSize == 0) {
                ...
                // EndOfFile is greater than the MaximumSize, use the specified maximum size.
                Section.SizeOfSection.QuadPart = (LONGLONG)*MaximumSize;
                FileSizeChecked = TRUE;
            } else {
                // Need to extend the section, make sure the file was opened for write access.
            ...
            // The file does not have an associated segment, create a segment object.
            if (AllocationAttributes & SEC_IMAGE) {
                Status = MiCreateImageFileMap(File, &Segment);
            } else {
                Status = MiCreateDataFileMap(File, &Segment, MaximumSize, SectionPageProtection, AllocationAttributes, IgnoreFileSizing);
                ASSERT(PreviousSectionPointer == File->SectionObjectPointer);
            }

            if (!NT_SUCCESS(Status)) {
                // Lock the PFN database and check to see if another thread has tried to create a segment to the file object at the same time.
                LOCK_PFN(OldIrql);
                Event = ControlArea->WaitingForDeletion;
                ...
                ObDereferenceObject(File);
                if (Event != NULL) {
                    KeSetEvent(&Event->Event, 0, FALSE);// Signal any waiters that the segment structure exists.
                }

                return Status;
            }

            // If the size was specified as zero, set the section size from the created segment size.
            // This solves problems with race conditions when multiple sections are created for the same mapped file with varying sizes.
            if (*MaximumSize == 0) {
        ...
        if (AllocationAttributes & SEC_LARGE_PAGES) {
            ...
            if (!SeSinglePrivilegeCheck(SeLockMemoryPrivilege, KeGetPreviousMode())) {
        ...
        // Set the section size from the created segment size.
        // This solves problems with race conditions when multiple sections are created for the same mapped file with varying sizes.
        Section.SizeOfSection.QuadPart = (LONGLONG)NewSegment->SizeOfSegment;
        ControlArea = NewSegment->ControlArea;

        // Set IncrementedRefCount so any failures from this point before the object is created will result in the control area & segment getting torn down - otherwise these could leak.
        // This is because pagefile backed sections are not (and should not be) added to the dereference segment cache.
        IncrementedRefCount = 1;
    }

    if (NewSegment == NULL) {
        // A new segment had to be created.
        // Lock the PFN database and check to see if any other threads also tried to create a new segment for this file object at the same time.
        NewSegment = Segment;
        ...
        if ((AllocationAttributes & SEC_IMAGE) || (SegmentControlArea->u.Flags.Rom == 1)) {
            ExFreePool(NewControlArea);// Deallocate the pool used for the original control area.
        }
    ...
    // Being created has now been cleared allowing other threads to reference the segment.  Release the resource on the file.
    if (FileAcquired) {
        IoSetTopLevelIrp(NULL);
        FsRtlReleaseFile(File);
        FileAcquired = FALSE;
...
ReferenceObject:
    // Now that the segment object is created, make the section object refer to the segment object.
    Section.Segment = NewSegment;
    Section.u.LongFlags = ControlArea->u.LongFlags;

    // Update the count of writable user sections so the transaction semantics can be supported.
    ...
    // So all that needs to be provided is a way for the transaction manager to know that there are lingering user views or created sections still open that have write access.

    // This must be done before creating the object so a rogue user program that suspends this thread cannot subvert a transaction.
    if ((FileObject == NULL) && (SectionPageProtection & (PAGE_READWRITE | PAGE_EXECUTE_READWRITE)) && (ControlArea->u.Flags.Image == 0) && (ControlArea->FilePointer != NULL)) {
        Section.u.Flags.UserWritable = 1;
        InterlockedIncrement((PLONG)&ControlArea->WritableUserReferences);
    }

    // Create the section object now.  The section object is created now so that the error handling when the section object cannot be created is simplified.
    ...
    Status = ObCreateObject(PreviousMode, MmSectionObjectType, ObjectAttributes, PreviousMode, NULL, sizeof(SECTION), PagedPoolCharge, NonPagedPoolCharge, (PVOID *)&NewSection);
    if (!NT_SUCCESS(Status)) {
        if ((FileObject == NULL) && (SectionPageProtection & (PAGE_READWRITE | PAGE_EXECUTE_READWRITE)) && (ControlArea->u.Flags.Image == 0) && (ControlArea->FilePointer != NULL)) {
    ...
    if (!IgnoreFileSizing) {
        // Indicate that the cache manager is not the owner of this section.
        NewSection->u.Flags.UserReference = 1;
        if (AllocationAttributes & SEC_NO_CHANGE) {
            NewSection->u.Flags.NoChange = 1;// Indicate that once the section is mapped, no protection changes or freeing the mapping is allowed.
        }

        if (((SectionPageProtection & PAGE_READWRITE) | (SectionPageProtection & PAGE_EXECUTE_READWRITE)) == 0) {
            NewSection->u.Flags.CopyOnWrite = 1;// This section does not support WRITE access, indicate that changing the protection to WRITE results in COPY_ON_WRITE.
        }
        ...
        if (AllocationAttributes & SEC_BASED) {
            ...
            // Ensure it does not wrap the virtual address space as the SECTION structure would have to widen to accommodate this and
            // it's not worth the performance penalty for the very few isolated cases that would want this.
            // Note that sections larger than the address space can easily be created - it's just that beyond a
            // certain point you shouldn't specify SEC_BASED (anything this big couldn't use a SEC_BASED section for anything anyway).
            ...
            Status2 = MiFindEmptyAddressRangeDownBasedTree(SizeOfSection, MmHighSectionBase, X64K, SectionBasedRoot, (PVOID *)&NewSection->Address.StartingVpn);
    ...
    ControlArea->u.Flags.WasPurged |= IgnoreFileSizing;// If the cache manager is creating the section, set the was purged flag as the file size can change.

    // Check to see if the section is for a data file and the size of the section is greater than the current size of the segment.
    if (((ControlArea->u.Flags.WasPurged == 1) && (!IgnoreFileSizing)) && (!FileSizeChecked) || ((UINT64)NewSection->SizeOfSection.QuadPart > NewSection->Segment->SizeOfSegment)) {
        TempSectionSize = NewSection->SizeOfSection;
        NewSection->SizeOfSection.QuadPart = (LONGLONG)NewSection->Segment->SizeOfSegment;
        // Even if the caller didn't specify extension rights, we enable it here temporarily to make the section correct.
        // Use a temporary section instead of temporarily editing the real section to avoid opening a security window that other concurrent threads could exploit.
        if (((NewSection->InitialPageProtection & PAGE_READWRITE) | (NewSection->InitialPageProtection & PAGE_EXECUTE_READWRITE)) == 0) {
...
UnrefAndReturn:
    // Unreference the control area, if it was referenced and return the error status.
    if (FileAcquired) {
        IoSetTopLevelIrp(NULL);
        FsRtlReleaseFile(File);
    }
...
LOGICAL MiMakeControlAreaRom(IN PFILE_OBJECT File, IN PLARGE_CONTROL_AREA ControlArea, IN PFN_NUMBER PageFrameNumber)
/*
Routine Description:
    This function marks the control area as ROM-backed.
    It can fail if the parallel control area (image vs data) is currently active as ROM-backed as the same PFNs cannot be used for both control areas simultaneously.
Arguments:
...
*/
{
    ...
    // This could be made smarter (ie: throw away the other control area if it's not in use) but for now, keep it simple.
...
/*
Routine Description:
    This function creates the necessary structures to allow the mapping of an image file.
    The image file is opened and verified for correctness, a segment object is created and initialized based on data in the image header.
Arguments:
...
#define InvalidAlignmentAllowed FALSE
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        if (Status == STATUS_FILE_IS_A_DIRECTORY) {
            return STATUS_INVALID_FILE_FOR_SECTION;// Can't map a directory as a section - return an error.
        }
    ...
    if (EndOfFile.HighPart != 0) {
        return STATUS_INVALID_FILE_FOR_SECTION;// File too big. Return error.
    }
    ...
    *Page = PageFrameNumber;
    ...
    // At the same time, capture whether there are any references to the data control area.
    // If so, flip the pages from the image control area into pagefile backings to prevent anyone else's data writes from changing the file after it is validated
    // (this can happen if the pages from the image control area need to be re-paged in later).
    ...
    if (Base == NULL) {
        ...
        if (IoStatus.Information != PAGE_SIZE) {
            // A full page was not read from the file, zero any remaining bytes because this page will be cached if the section creation succeeds.
            ...
            MarkHeaderModified = 1;// Mark the PFN as modified so we don't lose the zeroing if the page gets trimmed.
        }
    } else {
    ...
    // Check to determine if this is an NT image (PE format) or a DOS image, Win-16 image, or OS/2 image.
    // If the image is not NT format, return an error indicating which image it appears to be.
    ...
    if (((ULONG)DosHeader->e_lfanew + sizeof(IMAGE_NT_HEADERS) + (16 * sizeof(IMAGE_SECTION_HEADER))) <= (ULONG)DosHeader->e_lfanew) {
        Status = STATUS_INVALID_IMAGE_PROTECT;
        MI_BAD_IMAGE(6);
        goto BadPeImageSegment;
    }
    ...
    if (((ULONG)DosHeader->e_lfanew + sizeof(IMAGE_NT_HEADERS) + (16 * sizeof(IMAGE_SECTION_HEADER))) > PAGE_SIZE) {
        // The PE header is not within the page already read or the objects are in another page.
        ...
        if (IoStatus.Information != MM_MAXIMUM_IMAGE_HEADER) {
            // 2 full pages were not read from the file, no need to zero any remaining bytes (unless we were to cache it),
            // but do check that the amount read covers the minimum NT header.
        ...
        NtHeaderSize = (ULONG)IoStatus.Information - (ULONG)(BYTE_OFFSET((ULONG)DosHeader->e_lfanew));
    ...
    if (NtHeader->OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR32_MAGIC) {
        ...
        ComPlusDirectoryEntry = &NtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR];// Read in the COM+ directory entry.

#if defined (_WIN64)
    } else {
        // The image is 32-bit.
        // All code below this point must check if NtHeader is NULL.
        // If it is, then the image is PE32 and NtHeader32 must be used.
        ...
        ComPlusDirectoryEntry = &NtHeader32->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR];// Read in the COM+ directory entry.
    }
#endif
    ...
    if ((ImageAlignment >= PAGE_SIZE) || (CheckSplitPages == TRUE)) {
        ...
        ControlArea = ExAllocatePoolWithTag(NonPagedPool, sizeof(CONTROL_AREA) + (sizeof(SUBSECTION) * SubsectionsAllocated), 'iCmM');
        SingleSubsection = FALSE;
    } else {
        // The image alignment is less than the page size, map the image with a single subsection.
        ControlArea = ExAllocatePoolWithTag(NonPagedPool, sizeof(CONTROL_AREA) + sizeof(SUBSECTION), MMCONTROL);
    ...
    Subsection = (PSUBSECTION)(ControlArea + 1);
    SizeOfSegment = sizeof(SEGMENT) + (sizeof(MMPTE) * ((ULONG)NumberOfPtes - 1)) + sizeof(SECTION_IMAGE_INFORMATION);
    NewSegment = ExAllocatePoolWithTag(PagedPool | POOL_MM_ALLOCATION, SizeOfSegment, MMSECT);
    if (NewSegment == NULL) {
        // The requested pool could not be allocated.
        MI_BAD_IMAGE(0xD);
        Status = STATUS_INSUFFICIENT_RESOURCES;
    ...
    *Segment = NewSegment;
    ...
    NewSegment->u2.ImageInformation = (PSECTION_IMAGE_INFORMATION)((PCHAR)NewSegment + sizeof(SEGMENT) + (sizeof(MMPTE) * (NumberOfPtes - 1)));
...
#if DBG
    // Zero fill the prototype PTEs so they can be checked in the error path to make sure no valid entries got left behind.
    if (NumberOfPtes != 0) {
...
#define INIT_IMAGE_INFORMATION(OptHdr) {                            \
    NewSegment->u2.ImageInformation->TransferAddress = (PVOID)((ULONG_PTR)((OptHdr).ImageBase) + (OptHdr).AddressOfEntryPoint);          \
    ...
    NewSegment->u2.ImageInformation->ImageContainsCode = (BOOLEAN)(((OptHdr).SizeOfCode != 0) || ((OptHdr).AddressOfEntryPoint != 0)); \
    }
    ...
    if (NtHeader) {
        ...
        INIT_IMAGE_INFORMATION(NtHeader32->OptionalHeader);// The image is 32-bit so use the 32-bit header
    }
#endif
...
#undef INIT_IMAGE_INFORMATION
    ...
    if ((ActiveDataReferences == TRUE) ||
        ...
        ((FileHeader->Characteristics & IMAGE_FILE_REMOVABLE_RUN_FROM_SWAP) && (FILE_REMOVABLE_MEDIA & File->DeviceObject->Characteristics)) ||
        ((FileHeader->Characteristics & IMAGE_FILE_NET_RUN_FROM_SWAP) && (FILE_REMOTE_DEVICE & File->DeviceObject->Characteristics))) {
        // This file resides on a floppy disk or a removable media or network with flags set indicating it should be copied to the paging file.
    ...
    if (FILE_REMOTE_DEVICE & File->DeviceObject->Characteristics) {
        ControlArea->u.Flags.Networked = 1;// This file resides on a redirected drive.
    }
    ...
    if (NextVa != 0 && NextVa == MiMatchSectionBase) {
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "MM: NewSegment %p being created\n", NewSegment);
        DbgBreakPoint();
    }
#endif
    ...
    if (CheckSplitPages == FALSE) {
        ...
        Subsection->PtesInSubsection = (ULONG)MI_COMPUTE_PAGES_SPANNED(0, MI_ROUND_TO_SIZE(SizeOfHeaders, ImageAlignment));
    }
    ...
    if (SingleSubsection == TRUE) {
...
#if !defined (_WIN64)
        // Note this only needs to be checked for 32-bit systems as NT64 does a much more extensive reallocation of images that are built with alignment less than PAGE_SIZE.
        if ((UINT64)SizeOfImage < (UINT64)EndOfFile.QuadPart) {
            // Images that have a size of image (according to the header) that's smaller than the actual file only get that many prototype PTEs.
            // Initialize the subsection properly so no one can read off the end as that would corrupt the PFN database element's original PTE entry.
        ...
        for (i = 0; i < NumberOfPtes; i += 1) {
            ...
            if (SectorOffset < EndOfFile.LowPart) {
                MI_WRITE_INVALID_PTE(PointerPte, TempPte);// Data resides on the disk, refer to the control section.
            } else {// Data does not reside on the disk, use Demand zero pages.
        ...
        // Alignment is PAGE_SIZE or greater (or the image is a 32-bit 4K aligned image being run with ALTPTE support).
        if (SizeOfHeaders == 0) {
            MI_BAD_IMAGE(0x31);
            Status = STATUS_INVALID_IMAGE_FORMAT;
            goto BadPeImageSegment;
    ...
    PreferredImageBase = ImageBase;// Build the additional subsections.

    // At this point the object table is read in (if it was not already read in) and may displace the image header.
    ...
    if ((BYTE_OFFSET(NtHeader) + OffsetToSectionTable +
...
#endif
         ((NumberOfSubsections + 1) * sizeof(IMAGE_SECTION_HEADER))) <= NtHeaderSize) {
        ...
        // Has an extended header been read in and are the object tables resident?
        if (ExtendedHeader != NULL) {
#if defined(_WIN64)
            if (NtHeader32) {
                SectionTableEntry = (PIMAGE_SECTION_HEADER)((PCHAR)NtHeader32 + OffsetToSectionTable);
            ...
            // Is the whole range of object tables mapped by the extended header?
    ...
    if (SectionTableEntry == NULL) {
        ...
        BytesForSectionTableEntries = BYTE_OFFSET(DosHeader->e_lfanew + OffsetToSectionTable) + (NumberOfSubsections * sizeof(IMAGE_SECTION_HEADER));
        ...
        // The section table entries are not in the same pages as the other data already read in.
        // Read in the object table entries.
        if (ExtendedHeader == NULL) {
            ExtendedHeader = ExAllocatePoolWithTag(NonPagedPool, MM_MAXIMUM_IMAGE_HEADER, MMTEMPORARY);
            if (ExtendedHeader == NULL) {
                Status = STATUS_INSUFFICIENT_RESOURCES;
        ...
        SectionTableEntry = (PIMAGE_SECTION_HEADER)((PCHAR)ExtendedHeader + BYTE_OFFSET((ULONG)DosHeader->e_lfanew + OffsetToSectionTable));
        KeClearEvent(&InPageEvent);
        Status = IoPageRead(File, Mdl, &StartingOffset, &InPageEvent, &IoStatus);
        if (Status == STATUS_PENDING) {
        ...
        // From this point on NtHeader is only valid if it was in the first page of the image, otherwise reading in the object tables wiped it out.
    }

    if ((SingleSubsection == TRUE) && (InvalidAlignmentAllowed == FALSE)) {
        // The image header is no longer valid.

        // Loop through all sections and make sure there is no uninitialized data.
        ...
        while (NumberOfSubsections > 0) {
            ...
            if (SectionTableEntry->PointerToRawData + SectionTableEntry->SizeOfRawData < SectionTableEntry->PointerToRawData) {
            ...
            // If the virtual size and address does not match the rawdata and invalid alignments not allowed return an error.
            if (((SectionTableEntry->PointerToRawData != SectionTableEntry->VirtualAddress)) || (SectionVirtualSize > SectionTableEntry->SizeOfRawData)) {
    ...
    if ((SingleSubsection == TRUE) && (InvalidAlignmentAllowed == TRUE)) {
        ...
        // The image header is no longer valid.
        // Loop through all sections and make sure there is no uninitialized data.
        // Also determine if there are shared data sections and the number of extra PTEs they require.
        ...
        while (TempNumberOfSubsections > 0) {
            ...
            // On x86 this is handled by the subsection mapping.  Note the additional data must be in memory so it can be shuffled around later.
            ...
            if ((TempSectionTableEntry->Characteristics & IMAGE_SCN_MEM_SHARED) &&
                (!(TempSectionTableEntry->Characteristics & IMAGE_SCN_MEM_EXECUTE) || (TempSectionTableEntry->Characteristics & IMAGE_SCN_MEM_WRITE))) {
                ...
                PtesInThisSection = MI_ROUND_TO_SIZE(SectionVirtualSize, RoundingAlignment) >> PAGE_SHIFT;
                if ((PtesInThisSection != 0) && (AdditionalPtes + PtesInThisSection <= AdditionalPtes)) {
                    Status = STATUS_INVALID_IMAGE_FORMAT;
                    MI_BAD_IMAGE(0x2E);
                    goto BadPeImageSegment;
                }
        ...
        if (AdditionalBasePtes == 0 && (AdditionalSubsections == 0 || AdditionalPtes == 0)) {
            goto PeReturnSuccess;// No shared data sections.
        }
        ...
        // Check that the new number of PTEs of PTEs cannot cause the segment prototype PTE allocation size to wrap.
        // Since originally the number of PTEs came from the ULONG SizeOfImage in the OptionalHeader, we know it was ok then.
        // But adding various subsection counts above may have pushed it over the limit.
        ...
        // For shared sections, allocate new PTEs for these sections at the end of the image.
        // The usermode wow loader will change fixups to point to the new pages.

        // First reallocate the control area.
        NewSubsectionsAllocated = SubsectionsAllocated + AdditionalSubsections;
        NewControlArea = ExAllocatePoolWithTag(NonPagedPool, (ULONG)(sizeof(CONTROL_AREA) + (sizeof(SUBSECTION) * NewSubsectionsAllocated)), 'iCmM');
        if (NewControlArea == NULL) {
            MI_BAD_IMAGE(0x17);
            Status = STATUS_INSUFFICIENT_RESOURCES;
            goto BadPeImageSegment;
        ...
        // Now allocate a new segment that has the newly calculated number of PTEs, initialize it from the previously allocated new segment,
        // and overwrite the fields that should be changed.
        OldSegment = NewSegment;
        OrigNumberOfPtes += AdditionalBasePtes;
        PointerPte += AdditionalBasePtes;
        SizeOfSegment = sizeof(SEGMENT) + (sizeof(MMPTE) * (OrigNumberOfPtes + AdditionalPtes - 1)) + sizeof(SECTION_IMAGE_INFORMATION);
        NewSegment = ExAllocatePoolWithTag(PagedPool | POOL_MM_ALLOCATION, SizeOfSegment, MMSECT);
        if (NewSegment == NULL) {
            // The requested pool could not be allocated.
            MI_BAD_IMAGE(0x18);
        ...
        *Segment = NewSegment;
        ...
        // This reallocation code is not zeroing the extra PTEs and this got flagged for invalid PTE states in an error path below.
        // The error path (checked only) now only checks if no extra subsections were allocated.
        NewSegment->SegmentFlags.ExtraSharedWowSubsections = 1;
        NewSegment->u2.ImageInformation = (PSECTION_IMAGE_INFORMATION)((PCHAR)NewSegment + sizeof(SEGMENT) + (sizeof(MMPTE) * (OrigNumberOfPtes + AdditionalPtes - 1)));
        ...
        RtlCopyMemory(NewSegment->u2.ImageInformation, OldSegment->u2.ImageInformation, sizeof(SECTION_IMAGE_INFORMATION));

        // Adjust the prototype PTE pointer of the PE header page to point at the new segment.
        ASSERT(Pfn1->u2.Blink == (PFN_NUMBER)OldSegment->PrototypePte);
        Pfn1->u2.Blink = (PFN_NUMBER)NewSegment->PrototypePte;

        // Now change the fields in the subsections to account for the new control area and the new segment.
        // Also change the PTEs in the newly allocated segment to point to the new subsections.
        ...
        for (i = 0; i < SubsectionsAllocated; i += 1) {
            // Note: SubsectionsAllocated is always 1 (for wx86), so this loop is executed only once.
            NewSubsection->ControlArea = (PCONTROL_AREA)NewControlArea;
            NewSubsection->SubsectionBase = NewSegment->PrototypePte + (Subsection->SubsectionBase - OldSegment->PrototypePte);
            ...
            for (j = 0; j < OldSegment->TotalNumberOfPtes + AdditionalBasePtes; j += 1) {
                if ((OldPointerPte->u.Soft.Prototype == 1) && (MiGetSubsectionAddress(OldPointerPte) == Subsection)) {
                    ...
                    // Since the outer for loop is executed only once, there is no need for the i == 0 above, but it is safer to have it.
                    // If the code changes later and other sections are added, their PTEs will get initialized here as DemandZero and if they are not DemandZero,
                    // they will be overwritten in a later iteration of the outer loop.
                    // For now, this else if clause will be executed only for DemandZero PTEs.
                    OriginalProtection = MI_GET_PROTECTION_FROM_SOFT_PTE(OldPointerPte);
                    TempPteDemandZero.u.Long = 0;
                ...
                // Stop incrementing the OldPointerPte at the last entry and use it for the additional base PTEs.
        ...
        NumberOfPtes = AdditionalPtes;  // # PTEs that haven't yet been used in previous subsections.

        // Additional base PTEs have been added.  Only continue if there are additional subsections to process.
    ...
    while (NumberOfSubsections > 0) {
        if ((InvalidAlignmentAllowed == FALSE) ||
            ((SectionTableEntry->Characteristics & IMAGE_SCN_MEM_SHARED) && (!(SectionTableEntry->Characteristics & IMAGE_SCN_MEM_EXECUTE) ||
            ...
            // Fix for Borland linker problem.
            ...
            if (((NextVa != (PreferredImageBase + SectionTableEntry->VirtualAddress)) && (InvalidAlignmentAllowed == FALSE)) || (SectionVirtualSize == 0)) {
            ...
            EndingAddress = (SectionTableEntry->PointerToRawData + SectionTableEntry->SizeOfRawData + FileAlignment) & ~FileAlignment;
            ...
            // Build both a demand zero PTE and a PTE pointing to the subsection.
            TempPte.u.Long = 0;
            TempPteDemandZero.u.Long = 0;
            TempPte.u.Long = MiGetSubsectionAddressForPte(Subsection);
            TempPte.u.Soft.Prototype = 1;
            ...
            // Assume the subsection will be unwritable and therefore won't be charged for any commitment.
            SectionCommit = FALSE;
            ImageCommit = FALSE;
            ...
            if (TempPte.u.Soft.Protection & MM_PROTECTION_WRITE_MASK) {
                if ((TempPte.u.Soft.Protection & MM_COPY_ON_WRITE_MASK) == MM_COPY_ON_WRITE_MASK) {
                    // This page is copy on write, charge ImageCommitment for all pages in this subsection.
                    ImageCommit = TRUE;
                } else {
                    // This page is write shared, charge commitment when the mapping completes.
                    SectionCommit = TRUE;
            ...
            for (i = 0; i < Subsection->PtesInSubsection; i += 1) {
                ...
                if (SectorOffset < SectionVirtualSize) {
                    ...
                    if (SectorOffset < SizeOfRawData) {
                        ...
                        MI_WRITE_INVALID_PTE(PointerPte, TempPte);
                    } else {// Demand zero pages.
                        MI_WRITE_INVALID_PTE(PointerPte, TempPteDemandZero);
                    }
                } else {
                    // No access pages.
                    // This can only occur for images with section alignment greater than the native PAGE_SIZE.
    ...
    ASSERT((PointerPte > NewSegment->PrototypePte) && (PointerPte <= NewSegment->PrototypePte + NewSegment->TotalNumberOfPtes));
    if (InvalidAlignmentAllowed == FALSE) {
        ...
        // The total number of PTEs was decremented as sections were built, make sure that there are less than 64k's worth at this point.
        ...
        if ((ExtendedHeader == NULL) && (SizeOfHeaders < PAGE_SIZE)) {
            ...
            if (SizeOfHeaders < MaxToZero) {
                RtlZeroMemory((PVOID)((PCHAR)Base + SizeOfHeaders), MaxToZero - SizeOfHeaders);
                MarkHeaderModified = 1;// Mark the PFN as modified so we don't lose the zeroing if the page gets trimmed.
            }
        }
    }
...
PeReturnSuccess:
    // If this image is global per session (or is going to be executed directly from ROM), then allocate a large control area.
    // Note this doesn't need to be done for systemwide global control areas or non-global control areas.
    ...
    if (GlobalPerSession == TRUE) {
        LargeControlArea = ExAllocatePoolWithTag(NonPagedPool, (ULONG)(sizeof(LARGE_CONTROL_AREA) + (sizeof(SUBSECTION) * SubsectionsAllocated)), 'iCmM');
        ...
        // Copy the normal control area into our larger one, fix the linkages, Fill in the additional fields in the new one and free the old one.
    ...
    // Turn the image header and any straddler pages into transition pages within the prototype PTEs.

    // Note this could not be done earlier because otherwise these pages could have been trimmed (and reused) before we even finished initializing the segment.
    // This would be especially bad for the page that contains the PE header because we'd still have it mapped and be looking at its section header entries,
    // etc while the page was being reused !
    PointerPte = NewSegment->PrototypePte;
    Pfn1 = ImagePages;
    do {
        ...
        // Note straddler pages must always get marked modified.
        // The PE header only gets marked modified if it was shared with a straddler.

        // The list is reversed, so the last entry (Pfn2 == NULL) is the PE header.
        if (Pfn2 == NULL) {
        ...
        // Mark straddler pages as modified so they will be written and retrieved from the pagefile (and never from the filesystem again).
        // Put the straddler page on the modified list and set the transition bit in the prototype PTE.
        MiUpdateImageHeaderPage(PointerPte, Pfn1 - MmPfnDatabase, ControlArea, MarkModified);
        Pfn1 = Pfn2;
    } while (Pfn1 != NULL);
    ...
    if (NewSegment != NULL) {
...
#if DBG
        // The wow reallocation code doesn't zero the extra PTEs so skip this check for that case.
        if (NewSegment->SegmentFlags.ExtraSharedWowSubsections == 0) {
            ...
            for (i = 0; i < NewSegment->TotalNumberOfPtes; i += 1) {
                ...
                ASSERT((TempPte.u.Hard.Valid == 0) && ((TempPte.u.Soft.Prototype == 1) || (TempPte.u.Soft.Transition == 0)));
                PointerPte += 1;
            }
        }
#endif
...
*/
{
    ...
    // exe headers are notorious for having junk values for offsets in the import table and module table so the header must be very carefully validated.

    // Find out where the Module ref table is.
    // Mod table has two byte for each entry in import table.
    // These two bytes tell the offset in the import table for that entry.
    ...
    for (i = 0; i < ModuleCount; i += 1) {
        ...
        // Import table has count byte followed by the string where count is the string length.
        ...
        // Make sure the offset is within the valid range.
        // The sizeof(UCHAR) is included in the check because ImportTable was incremented above and is used in the RtlEqualMemory comparison below.
        ...
        ModuleTable = (PUSHORT)((PCHAR)ModuleTable + 2);// Move on to the next module table entry.  Each entry is 2 bytes.
    }
...
*/
{
    ...
    if (NtHeader->Signature != IMAGE_NT_SIGNATURE) {
        if ((USHORT)NtHeader->Signature == (USHORT)IMAGE_OS2_SIGNATURE) {
            ...
            if ((!MiCheckDosCalls((PIMAGE_OS2_HEADER)NtHeader, NtHeaderSize)) && ((((PIMAGE_OS2_HEADER)NtHeader)->ne_exetyp == 2) ||
                ((((PIMAGE_OS2_HEADER)NtHeader)->ne_exetyp == 0) && (((((PIMAGE_OS2_HEADER)NtHeader)->ne_expver & 0xff00) == 0x200) ||
                                                                                  ((((PIMAGE_OS2_HEADER)NtHeader)->ne_expver & 0xff00) == 0x300))))) {
                return STATUS_INVALID_IMAGE_WIN_16;// This is a win-16 image.
            }

            // The following OS/2 headers types go to NTDOS

            // - exetype == 5 means binary is for Dos 4.0. e.g Borland Dos extender type
            ...
            if (((PIMAGE_OS2_HEADER)NtHeader)->ne_exetyp == 5 || ((PIMAGE_OS2_HEADER)NtHeader)->ne_enttab == ((PIMAGE_OS2_HEADER)NtHeader)->ne_imptab) {
            ...
            // The PharLap config block is pointed to by the SizeofHeader field in the DosHdr.
            // The following algorithm for detecting a pharlap exe was recommended by PharLap Software Inc.
            ...
            if ((ULONG_PTR)pb < (ULONG_PTR)DosHeader + PAGE_SIZE - 0x30 - sizeof(USHORT)) {
                pb += *(PUSHORT)(pb + 0x30);
                if ((ULONG_PTR)pb < (ULONG_PTR)DosHeader + PAGE_SIZE - 36 && RtlEqualMemory(pb, "Copyright (C) Rational Systems, Inc.", 36)) {
                    return STATUS_INVALID_IMAGE_PROTECT;
                }
            }

            // Check for lotus 123 family of applications.
            // Starting with 123 3.0 (till recently shipped 123 3.4), every exe header is bound but is meant for DOS.
            // This can be checked via, a string signature in the extended header.
            // <len byte>"1-2-3 Preloader" is the string at ne_nrestab offset.
            ...
            if (pResTableAddress > DosHeader->e_lfanew &&
                ...
                RtlEqualMemory(((PUCHAR)NtHeader + 1 + (ULONG)(pResTableAddress - DosHeader->e_lfanew)), "1-2-3 Preloader", 15)) {
                return STATUS_INVALID_IMAGE_PROTECT;
            }
        ...
        if ((USHORT)NtHeader->Signature == (USHORT)IMAGE_OS2_SIGNATURE_LE) {
            // This is a LE (OS/2) image. We don't support it, so give it to DOS subsystem.
            // There are cases (Rbase.exe) which have a LE header but actually it is suppose to run under DOS.
            // When we do support LE format, some work needs to be done here to decide whether to give it to VDM or OS/2.
            return STATUS_INVALID_IMAGE_PROTECT;
        }
    ...
    if ((NtHeader->FileHeader.Machine == 0) && (NtHeader->FileHeader.SizeOfOptionalHeader == 0)) {
        return STATUS_INVALID_IMAGE_PROTECT;// This is a bogus DOS app which has a 32-bit portion masquerading as a PE image.
    }
...
#define VALIDATE_NTHEADER(Hdr) {                                    \
    ...
    if (((((Hdr)->OptionalHeader).SectionAlignment - 1) & ((Hdr)->OptionalHeader).SectionAlignment) != 0) {         \
        return STATUS_INVALID_IMAGE_FORMAT;                         \
    }                                                               \
                                                                    \
    ...
    if ((Hdr)->FileHeader.NumberOfSections > MM_MAXIMUM_IMAGE_SECTIONS) { \
        return STATUS_INVALID_IMAGE_FORMAT;                         \
                                                                    ...
                                                                    \
    if (((Hdr)->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC) && !((Hdr)->FileHeader.Machine == IMAGE_FILE_MACHINE_I386))  { \
        return STATUS_INVALID_IMAGE_FORMAT;                         \
    }                                                               \
                                                                    \
    if (((Hdr)->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) && \
        !(((Hdr)->FileHeader.Machine == IMAGE_FILE_MACHINE_IA64) || ((Hdr)->FileHeader.Machine == IMAGE_FILE_MACHINE_AMD64))) { \
        return STATUS_INVALID_IMAGE_FORMAT;                         \
    }                                                               \
}
    ...
    if (NtHeader->OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR_MAGIC) {
        ...
        if (NtHeader->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
            return STATUS_INVALID_IMAGE_WIN_64;// 64bit image on a 32bit machine.
        }
#endif
        return STATUS_INVALID_IMAGE_FORMAT;
    }
...
/*
Routine Description:
    This function creates the necessary structures to allow the mapping of a data file.
    The data file is accessed to verify desired access, a segment object is created and initialized.
Arguments:
    File - Supplies the file object for the image file.
    Segment - Returns the segment object.
    ...
    IgnoreFileSizing - Supplies TRUE if the cache manager is specifying the file size and so it does not need to be validated.
*/
{
    NTSTATUS Status;
    ULONG j;
    ...
    if (!IgnoreFileSizing) {
        Status = FsRtlGetFileSize(File, (PLARGE_INTEGER)&EndOfFile);
        if (Status == STATUS_FILE_IS_A_DIRECTORY) {
            return STATUS_INVALID_FILE_FOR_SECTION;// Can't map a directory as a section. Return error.
        }
        ...
        if (EndOfFile == 0 && *MaximumSize == 0) {
            return STATUS_MAPPED_FILE_SIZE_ZERO; // Can't map a zero length without specifying the maximum size as non-zero.
        }

        // Make sure this file is big enough for the section.
        if (*MaximumSize > EndOfFile) {
            // If the maximum size is greater than the end-of-file, and the user did not request page_write or page_execute_readwrite to the section, reject the request.
            if (((SectionPageProtection & PAGE_READWRITE) | (SectionPageProtection & PAGE_EXECUTE_READWRITE)) == 0) {
                return STATUS_SECTION_TOO_BIG;
            }

            // Check to make sure that the allocation size large enough to contain all the data, if not set a new allocation size.
        ...
        EndOfFile = *MaximumSize;// Ignore the file size, this call is from the cache manager.
    }

    // Calculate the number of prototype PTE chunks to build for this section.
    // A subsection is also allocated for each chunk as all the prototype PTEs in any given chunk are initially encoded to point at the same subsection.

    // The maximum total section size is 16PB (2^54).
    // This is because the StartingSector4132 field in each subsection, ie: 2^42-1 bits of file offset where the offset is in 4K (not pagesize) units.
    // Thus, a subsection may describe a *BYTE* file start offset of maximum 2^54 - 4K.

    // Each subsection can span at most 16TB - 64K.
    // This is because the NumberOfFullSectors and various other fields in the subsection are ULONGs.
    // In reality, this is a nonissue as far as maximum section size is concerned because any number of subsections can be chained together and in fact, subsections are allocated to span less to facilitate
    ...
    // Allocate the subsection memory in smaller sizes so the corresponding prototype PTEs can be trimmed later if paged pool virtual address space becomes scarce.
    // Note the size is snapped locally so it can be changed dynamically without locking.
    ...
    // Initializing Last is not needed for correctness, but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    Last = NULL;
    ControlArea = (PCONTROL_AREA)File->SectionObjectPointer->DataSectionObject;
    do {
        ...
        } else {// Control area and first subsection were zeroed at allocation time.
    ...
    *Segment = (PSEGMENT)NewSegment;
    ...
    ControlArea->NumberOfSectionReferences = 1;
    if (IgnoreFileSizing == FALSE) {
        ControlArea->NumberOfUserReferences = 1;// This reference is not from the cache manager.
    } else {
        ControlArea->u.Flags.WasPurged = 1;// Set the was purged flag to indicate that the file size was not explicitly set.
    }
    ...
    do {
        ...
        if (Subsection->NextSubsection == NULL) {
            ...
            // Note this next line restricts the number of bytes mapped by a single subsection to 16TB-4K.
            // Multiple subsections can always be chained together to support an overall file of size 16K TB.
            ...
            Subsection->PtesInSubsection = (ULONG)(NumberOfPtesForEntireFile - (FileOffset >> (PAGE_SHIFT - MM4K_SHIFT)));
        ...
        FileOffset += (((UINT64)Subsection->PtesInSubsection) << (PAGE_SHIFT - MM4K_SHIFT));
...
VOID MiUpdateLargeProtoPfns(IN PMMPTE PointerPte, IN PMMPTE LastPte, IN PMMPFN Pfn1, IN MMPTE OriginalPte)
{
    PMMPFN Pfn2;
    KIRQL OldIrql;
    ...
    while (PointerPte < LastPte) {
        ...
        // Also update the share count on the page containing the prototype PTEs.
        // This prevents the page from getting trimmed which is good because we wouldn't know to update the PteFrame of our physical pages then anyway !
...
NTSTATUS MiCreatePagingFileMap(OUT PSEGMENT *Segment, IN PUINT64 MaximumSize, IN MM_PROTECTION_MASK ProtectionMask, IN ULONG AllocationAttributes)
/*
Routine Description:
    This function creates the necessary structures to allow the mapping of a paging file.
Arguments:
    Segment - Returns the segment object.
    MaximumSize - Supplies the maximum size for the mapping.
...
*/
{
    ...
    // Limit page file backed sections to the amount of pool that could possibly be allocated to hold the prototype PTEs.
    // Note this may be larger than the size of any *single* pagefile.

#if defined (_WIN64)
    // Limit the maximum size to the number of PTEs that can be stored in the NonExtendedPtes field in the segment so that segment deletion which uses this will use the proper value.
    ...
    if (AllocationAttributes & SEC_COMMIT) {
        ...
        if (AllocationAttributes & SEC_LARGE_PAGES) {
            ...
            LargePageListHead = MiAllocateLargeZeroPages(NumberOfPtes, ProtectionMask);
    ...
    *Segment = NewSegment;
    ...
    PointerPte = &NewSegment->ThePtes[0];// Align the prototype PTEs on the proper boundary.
    ...
    // Record the process that created this segment for the performance analysis tools.
    ...
    if (AllocationAttributes & SEC_COMMIT) {
        ...
        if (AllocationAttributes & SEC_LARGE_PAGES) {
            ...
            // Fill all the prototype PTEs with valid entries.
            // Note this is unusual for prototype PTEs because these entries are valid with a nonzero share and and reference count even though no real page table entries are currently mapping them.
            // This is to prevent premature trimming of them.
            MI_MAKE_VALID_PTE(TempValidPte, 0, ProtectionMask, NULL);
            ...
            while (LargePageListHead != NULL) {
                ...
                MiUpdateLargeProtoPfns(PointerPte - LargePageInfo->PageCount, LastPte, Pfn1 - LargePageInfo->PageCount, TempPte);
    ...
    // Set all the prototype PTEs to either no access or demand zero depending on the commit flag.
...
/*
Routine Description:
    This function opens a handle to a section object with the specified desired access.
Arguments:
    Sectionhandle - Supplies a pointer to a variable that will receive the section object handle value.
    DesiredAccess - Supplies the desired types of access  for the section.
    DesiredAccess Flags
         EXECUTE - Execute access to the section is desired.
         READ - Read access to the section is desired.
         WRITE - Write access to the section is desired.
...
*/
{
    ...
    // Open handle to the section object with the specified desired access.
    Status = ObOpenObjectByName(ObjectAttributes, MmSectionObjectType, PreviousMode, NULL, DesiredAccess, NULL, &Handle);
...
/*
Routine Description:
    This function takes a section characteristic mask from the image and converts it to an PTE protection mask.
Arguments:
    SectionCharacteristics - Supplies the characteristics mask from the image.
Return Value:
...
/*
Routine Description:
    This non-pageable function acquires the PFN lock, removes a page and updates the PFN database as though the page was ready to be deleted if the reference count is decremented.
Arguments:
...
*/
{
    ...
    // Increment the reference count for the page so the paging I/O will work, and so this page cannot be stolen from us.
    Pfn1 = MI_PFN_ELEMENT(PageFrameNumber);
...
/*
Routine Description:
    This non-pageable function acquires the PFN lock, and turns the specified prototype PTE into a transition PTE referring to the specified physical page.
    It then decrements the reference count causing the page to be placed on the standby or modified list.
Arguments:
    ...
    MarkModified - Supplies TRUE if the PFN should be marked modified (this will give it pagefile-backing on trim vs filesystem backing).
*/
{
    PMMPTE PointerPde;
    PMMPFN Pfn1;
    ...
    MiDecrementReferenceCount(Pfn1, PageFrameNumber);// Add the page to the standby list.
...
/*
Routine Description:
    This non-pageable function acquires the PFN lock, and decrements the reference count thereby causing the physical page to be deleted.
Arguments:
...
/*
Routine Description:
    This function searches the control area chains (if any) for an existing cache of the specified image file.
    For non-global control areas, there is no chain and the control area is shared for all callers and sessions.
...
Arguments:
    ...
    GlobalNeeded - Supplies a pointer to store whether a global control area is required as a placeholder.
                   This can only be set when there
                   is already some global control area in the list - ie: our caller should only rely on this when this function returns NULL so the caller knows what kind of control area to insert.
Return Value:
...
*/
{
    ...
    *GlobalNeeded = FALSE;
    ...
    // If no control area, or the control area is not session global, then our job is easy.
    // Note, however, that they each require different return values as they represent different states.
...
/*
Routine Description:
    This function inserts the control area into the file's section object pointers.
    For non-global control areas, there is no chain and the control area is shared for all callers and sessions.
    Likewise for systemwide global control areas.
...
*/
{
    ...
    // If this is not a global-per-session control area or just a placeholder control area (with no chain already in place) then just put it in.
    ...
    if (File->SectionObjectPointer->ImageSectionObject == NULL) {
        ...
        // Insert new entry before the current first entry.
        // The control area must be in the midst of creation/deletion or have a valid session ID to be inserted.
        ASSERT(ControlArea->u.Flags.BeingDeleted || ControlArea->u.Flags.BeingCreated || ControlArea->SessionId != (ULONG)-1);
        FirstControlArea = (PLARGE_CONTROL_AREA)(File->SectionObjectPointer->ImageSectionObject);
...
*/
{
    ...
    // Get a pointer to the first control area.
    // If this is not a global-per-session control area, then there is no list, so we're done.
    ...
    if (FirstControlArea == ControlArea) {
        ...
        // If it is the only entry in the list, then the new first entry pointer will be NULL.
    ...
    // Remove the entry, note that the ImageSectionObject need not be updated as the entry is not at the head.
...
/*
Routine Description:
    This routine copies the image header from the data section if there is one and the page is already resident or in transition.
Arguments:
    File - Supplies the file object.
...
*/
{
    ...
    // Take a quick (safely unsynchronized) look to see whether to bother mapping the image header page at all - if there's no data section object, then skip it and just return.
    ...
    // If the prototype PTEs have been tossed (or never created) then we don't have any data to copy.
    ...
    if (MiGetPteAddress(PointerPte)->u.Hard.Valid == 0) {
        // We have no reference to the data section so if we can't do this without relinquishing the PFN lock, then don't bother.
        // ie: the entire control area and everything can be freed while a call to MiMakeSystemAddressValidPfn releases the lock.
        UNLOCK_PFN(OldIrql);
        if (ImagePte != NULL) {
            MiReleaseSystemPtes(ImagePte, 1, SystemPteSpace);
        }
    ...
    if ((PteContents.u.Hard.Valid == 1) || ((PteContents.u.Soft.Prototype == 0) && (PteContents.u.Soft.Transition == 1))) {
        ...
        // If the new page had been used to hold code in a previous life we must ensure the instruction cache is kept coherent.
    ...
    // The data page is not resident, so return NULL and the caller will take the long way.
    UNLOCK_PFN(OldIrql);
