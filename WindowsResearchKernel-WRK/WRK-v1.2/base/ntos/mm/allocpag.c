...
PVOID MiFindContiguousMemoryInPool(IN PFN_NUMBER LowestPfn, IN PFN_NUMBER HighestPfn, IN PFN_NUMBER BoundaryPfn, IN PFN_NUMBER SizeInPages, IN PVOID CallingAddress);
...
// Set this to a nonzero (ie: 10000) value to cause every pool allocation to be checked and an ASSERT fires if the allocation is larger than this value.
ULONG MmCheckRequestInPages = 0;

// Set this to a nonzero (ie: 0x23456789) value to cause this pattern to be written into freed nonpaged pool pages.
...
Arguments:
    ...
    SizeInPages - Supplies the size of the request in pages - zero indicates to keep going until there are no more protected PTEs (ie: the caller doesn't know how many protected PTEs there are).
Return Value:
...
*/
{
    ...
    // Either the flink or the blink may be pointing at protected nonpaged pool.  Unprotect now.
    FreeFlink = (PVOID)0;
    FreeBlink = (PVOID)0;
    ...
    if (FreeFlink) {
        MiProtectFreeNonPagedPool(FreeFlink, 1);// Reprotect the flink.
    }

    if (FreeBlink) {
        MiProtectFreeNonPagedPool(FreeBlink, 1);// Reprotect the blink.
    }
}
    ...
    if (MmUnusedSegmentForceFree == 0) {
        if (!IsListEmpty(&MmUnusedSegmentList)) {
            ...
            if (MiUnusedSubsectionPagedPool < 4 * PAGE_SIZE) {
                // No unused segments and tossable subsection usage is low as well.
                // Start unmapping system cache views in an attempt to get back the paged pool containing its prototype PTEs.
...
Return Value:
    Returns the POOL_TYPE (PagedPool, NonPagedPool, PagedPoolSession or NonPagedPoolSession).
Environment:
    Kernel Mode Only.
*/
{
...
SIZE_T MmAvailablePoolInPages(IN POOL_TYPE PoolType)
/*
Routine Description:
...
/*
Routine Description:
    This function examines various resources to determine if this pool allocation should be allowed to proceed.
Arguments:
    PoolType - Supplies the type of pool to retrieve information about.
    NumberOfBytes - Supplies the number of bytes to allocate.
    Priority - Supplies an indication as to how important it is that this request succeed under low available resource conditions.
Return Value:
...
*/
{
    ...
    // This pool allocation is permitted, but because we're starting to run low, trigger a round of dereferencing in parallel before returning success.
    // Note this is only done on machines with at least 1GB of RAM as smaller configuration machines will already trigger this due to physical page consumption.
    Status = TRUE;

nopool:
    // Running low on pool - if this request is not for session pool, force unused segment trimming when appropriate.
    if ((PoolType & SESSION_POOL_MASK) == 0) {
        if ((PoolType & BASE_POOL_TYPE_MASK) == NonPagedPool) {
            MmPreemptiveTrims[MmPreemptForNonPagedPriority] += 1;
            OldIrql = KeAcquireQueuedSpinLock(LockQueueMmNonPagedPoolLock);

            // Only pulse the event if it is not already set.
            // This saves on dispatcher lock contention, but more importantly since
            // KePulse always clears the event it saves us having to check whether to set it (and potentially do the setting) afterwards.
            ...
            // Only pulse the event if it is not already set.
            // This saves on dispatcher lock contention, but more importantly since KePulse always clears the event it saves us having to check whether to set it (and potentially do the setting) afterwards.
...
Environment:
    These functions are used by the internal Mm page allocation/free routines only and should not be called directly.
    The nonpaged pool spinlock must be held at DISPATCH_LEVEL when calling this function.
*/
{
    PFN_NUMBER i;
    PMMPFN Pfn1;
    ...
    for (i = 0; i < NumberOfPages; i += 1) {
        ...
        // Set the pointer to the PTE as empty so the page is deleted when the reference count goes to zero.
    ...
    // No TB flush is done here - the system PTE management code will lazily flush as needed.
    UNLOCK_PFN_FROM_DPC();

    // Generally there is no need to update resident available pages at this time as it has all been done during initialization.
    // However, only some of the expansion pool was charged at init, so calculate how much (if any) resident available page charge to return.
...
*/
{
    ...
    for (Index = 0; Index < MI_MAX_FREE_LIST_HEADS; Index += 1) {
        ...
        while (Entry != &MmNonPagedPoolFreeListHead[Index]) {
            ...
            // The list is not empty, see if this one is virtually mapped.
...
VOID MiMarkPoolLargeSession(IN PVOID VirtualAddress)
/*
Routine Description:
    This function marks a NONPAGED pool allocation as being of type large session.
Arguments:
    VirtualAddress - Supplies the virtual address of the pool allocation.
Environment:
    This function is used by the general pool allocation routines and should not be called directly.
...
LOGICAL MiIsPoolLargeSession(IN PVOID VirtualAddress)
/*
Routine Description:
    This function determines whether the argument nonpaged allocation was marked as a large session allocation.
Arguments:
    VirtualAddress - Supplies the virtual address of the pool allocation.
Environment:
    This function is used by the general pool allocation routines and should not be called directly.
...
PVOID MiAllocatePoolPages(IN POOL_TYPE PoolType, IN SIZE_T SizeInBytes)
/*
Routine Description:
    This function allocates a set of pages from the specified pool and returns the starting virtual address to the caller.
Arguments:
    PoolType - Supplies the type of pool from which to obtain pages.
    SizeInBytes - Supplies the size of the request in bytes.  The actual size returned is rounded up to a page boundary.
Return Value:
    Returns a pointer to the allocated pool, or NULL if no more pool is available.
Environment:
    These functions are used by the general pool allocation routines and should not be called directly.
...
*/
{
    ...
    if ((PoolType & BASE_POOL_TYPE_MASK) == NonPagedPool) {
        if ((SizeInPages == 1) && (ExQueryDepthSList(&MiNonPagedPoolSListHead) != 0)) {
        ...
        do {
            ...
            while (Entry != ListHead) {
                ...
                if (FreePageInfo->Size >= SizeInPages) {
                    // This entry has sufficient space, remove the pages from the end of the allocation.
                    ...
                    if (FreePoolInPages < MiHighNonPagedPoolThreshold) {
                        ...
                        // KeReadStateEvent since we are holding the nonpaged pool lock and want to keep instructions at a minimum.
        ...
        // No more entries on the list, expand nonpaged pool if possible to satisfy this request.

        // If pool is starting to run low then free some page cache up now.
        // While this can never guarantee pool allocations will succeed, it does give allocators a better chance.
        FreePoolInPages = MmMaximumNonPagedPoolInPages - MmAllocatedNonPagedPool;
        ...
        if (StartingPte == NULL) {
            ...
            // Check to see if there are too many unused segments lying around.  If so, set an event so they get deleted.
            if (MI_UNUSED_SEGMENTS_SURPLUS()) {
                KeSetEvent(&MmUnusedSegmentCleanup, 0, FALSE);
            }

            // If there are any cached expansion PTEs, free them now in an attempt to get enough contiguous VA for our caller.
        ...
        if ((PfnList == NULL) && (MiChargeCommitmentCantExpand(SizeInPages, FALSE) == FALSE)) {
            if ((PsGetCurrentThread()->MemoryMaker == 1) && (!KeIsExecutingDpc())) {
        ...
        if (PfnList != NULL) {
            ...
            // The lock must be acquired prior to filling any PTEs to prevent a free happening in parallel from mistakenly trying to coalesce with this allocation !
            OldIrql = KeAcquireQueuedSpinLock(LockQueueMmNonPagedPoolLock);
            ...
            do {
                ...
                ASSERT(Pfn1->OriginalPte.u.Long == MM_DEMAND_ZERO_WRITE_PTE);
                ASSERT(Pfn1->u4.MustBeCached == 1);
                ASSERT((Pfn1->u3.e1.StartOfAllocation == 0) || (Pfn1 == PfnList));
                ASSERT(Pfn1->u3.e1.EndOfAllocation == 0);
                ASSERT(Pfn1->u3.e1.PageLocation == ActiveAndValid);
                ASSERT(Pfn1->u3.e1.CacheAttribute == MiCached);
                ASSERT(Pfn1->u3.e1.LargeSessionAllocation == 0);
                ASSERT((Pfn1->u4.VerifierAllocation == 0) || ((Pfn1 == PfnList) && (PoolType & POOL_VERIFIER_MASK)));
        ...
        // Make sure we have 1 more than the number of pages requested available.
        ...
        do {
            ...
            // Note we can skip the flush if we've already done it once in this loop already because the PFN lock is held throughout.
            if ((Pfn1->u3.e1.CacheAttribute != MiCached) && (FlushedTb == FALSE)) {
                MI_FLUSH_TB_FOR_INDIVIDUAL_ATTRIBUTE_CHANGE(PageFrameIndex, MiCached);
                FlushedTb = TRUE;
            }
    ...
    AllocationSucceeded:
        FreePoolInPages = MmMaximumNonPagedPoolInPages - MmAllocatedNonPagedPool;
        if (FreePoolInPages < MiHighNonPagedPoolThreshold) {
            // Read the state directly instead of calling KeReadStateEvent since we are holding the nonpaged pool lock and want to keep instructions at a minimum.
    ...
    if ((PoolType & SESSION_POOL_MASK) == 0) {
        ...
        if ((SizeInPages == 1) && (ExQueryDepthSList(&MiPagedPoolSListHead) != 0)) {
    ...
    StartPosition = RtlFindClearBitsAndSet(PagedPoolInfo->PagedPoolAllocationMap, (ULONG)SizeInPages, PagedPoolInfo->PagedPoolHint);
    if (StartPosition == NO_BITS_FOUND) {
        // No room in pool - attempt to expand the paged pool.  If there is a surplus of unused segments (or subsections) get rid of some now.
        if (MI_UNUSED_SEGMENTS_SURPLUS()) {
            KeSetEvent(&MmUnusedSegmentCleanup, 0, FALSE);
        }
        ...
        // Make sure there is enough space to create at least some page table pages.
        // Note if we can create even one it's worth doing as there may be free space in the already existing pool (at the end) - and this can be concatenated with the expanded portion below into one big allocation.
        if (PagedPoolInfo->NextPdeForPagedPoolExpansion > MiGetPteAddress(PagedPoolInfo->LastPteForPagedPool)) {
        ...
        if (((StartPosition - 1) + PagedPoolInfo->NextPdeForPagedPoolExpansion) > MiGetPteAddress(PagedPoolInfo->LastPteForPagedPool)) {
        ...
        EndPosition = (ULONG)((PagedPoolInfo->NextPdeForPagedPoolExpansion - MiGetPteAddress(PagedPoolInfo->FirstPteForPagedPool)) * PTE_PER_PAGE);
        ...
        MI_DECREMENT_RESIDENT_AVAILABLE(PageTableCount, MM_RESAVAIL_ALLOCATE_PAGETABLES_FOR_PAGED_POOL);// Update the count of available resident pages.
        ...
        do {
            ...
            if (SessionSpace) {
                ...
                MiInitializePfnForOtherProcess(PageFrameIndex, PointerPte, SessionSpace->SessionPageDirectoryIndex);
                MM_BUMP_SESS_COUNTER(MM_DBG_SESSION_PAGEDPOOL_PAGETABLE_ALLOC1, 1);
            } else {
                MmSystemPagePtes[((ULONG_PTR)PointerPte & (PD_PER_SYSTEM * (sizeof(MMPTE) * PDE_PER_PAGE) - 1)) / sizeof(MMPTE)] = TempPte;
                MiInitializePfnForOtherProcess(PageFrameIndex, PointerPte, MmSystemPageDirectory[(PointerPte - MiGetPdeAddress(0)) / PDE_PER_PAGE]);
            }
        ...
        // Clear the bitmap locations for the expansion area to indicate it is available for consumption.
        RtlClearBits(PagedPoolInfo->PagedPoolAllocationMap, EndPosition, (ULONG)PageTableCount * PTE_PER_PAGE);

        PagedPoolInfo->NextPdeForPagedPoolExpansion += PageTableCount;// Denote where to start the next pool expansion.

        // Mark the PTEs for the expanded pool no-access.
        MiZeroMemoryPte(VirtualAddressSave, PageTableCount * (PAGE_SIZE / sizeof(MMPTE)));
        if (SessionSpace) {
            InterlockedExchangeAddSizeT(&SessionSpace->CommittedPages, PageTableCount);
            MM_BUMP_SESS_COUNTER(MM_DBG_SESSION_NP_PAGETABLE_ALLOC, PageTableCount);
            InterlockedExchangeAddSizeT(&SessionSpace->NonPageablePages, PageTableCount);
        }

        // Start searching from the beginning of the bitmap as we may be able to coalesce an earlier entry and only use part of the expansion we just did.
        // This is not only important to reduce fragmentation but in fact, required for the case where we could not expand enough to cover the entire allocation and thus, must coalesce backwards in order to satisfy the request.
    ...
    // This is paged pool, the start and end can't be saved in the PFN database as the page isn't always resident in memory.
    // The ideal place to save the start and end would be in the prototype PTE, but there are no free bits.
    // To solve this problem, a bitmap which parallels the allocation bitmap exists which contains set bits in the positions where an allocation ends.
    // This allows pages to be deallocated with only their starting address.

    // For sanity's sake, the starting address can be verified from the 2 bitmaps as well.
    // If the page before the starting address is not allocated (bit is zero in allocation bitmap) then this page is obviously a start of an allocation block.
    // If the page before is allocated and the other bit map does not indicate the previous page is the end of an allocation,
    // then the starting address is wrong and a bugcheck should be issued.
    if (SizeInPages == 1) {
        PagedPoolInfo->PagedPoolHint = StartPosition + (ULONG)SizeInPages;
    }

    // If paged pool has been configured as non-pageable, commitment has already been charged so just set the length and return the address.
    if ((MmDisablePagingExecutive & MM_PAGED_POOL_LOCKED_DOWN) && (SessionSpace == NULL)) {
        ...
        InterlockedExchangeAddSizeT(&PagedPoolInfo->AllocatedPagedPool, SizeInPages);
        FreePoolInPages = MmSizeOfPagedPoolInPages - MmPagedPoolInfo.AllocatedPagedPool;
        if (FreePoolInPages < MiHighPagedPoolThreshold) {
            // Read the state directly instead of calling KeReadStateEvent since we are holding the paged pool mutex and want to keep instructions at a minimum.
    ...
    if (MiChargeCommitmentCantExpand(SizeInPages, FALSE) == FALSE) {
        if ((PsGetCurrentThread()->MemoryMaker == 1) && (!KeIsExecutingDpc())) {
            ...
            // Could not commit the page(s), return NULL indicating no pool was allocated.
            // Note that the lack of commit may be due to unused segments and the MmSharedCommit, prototype PTEs, etc associated with them.
            // So force a reduction now.
            if (SessionSpace == NULL) {
                KeReleaseGuardedMutex(&MmPagedPoolMutex);
                MmPoolFailures[MmPagedPool][MmHighPriority] += 1;
                MmPoolFailureReasons[MmPagedNoCommit] += 1;
    ...
    InterlockedExchangeAddSizeT(&PagedPoolInfo->PagedPoolCommit, SizeInPages);
    ...
    for (i = 0; i < SizeInPages; i += 1, PointerPte += 1) {
        ...
        if (TimeStamp == LastTimeStamp) {
            if (PreviousPteNeededFlush == TRUE) {
                // If this entry is the same as the prior one then it must be treated the same with respect to flushes.
    ...
    // Fill the PTEs inline instead of using MiFillMemoryPte because on most platforms MiFillMemoryPte degrades to a function call and typically only a small number of PTEs are filled here.
    do {
...
/*
Routine Description:
    This function returns a set of pages back to the pool from which they were obtained.
    Once the pages have been deallocated the region provided by the allocation becomes available for allocation to other callers, i.e. any data in the region is now trashed and cannot be referenced.
Arguments:
    StartingAddress - Supplies the starting address which was returned in a previous call to MiAllocatePoolPages.
Return Value:
...
*/
{
    ...
    // Determine pool type based on the virtual address of the block to deallocate.

    // This assumes paged pool is virtually contiguous.
    if ((StartingAddress >= MmPagedPoolStart) && (StartingAddress <= MmPagedPoolEnd)) {
        SessionSpace = NULL;
        PagedPoolInfo = &MmPagedPoolInfo;
        StartPosition = (ULONG)(((PCHAR)StartingAddress - (PCHAR)MmPagedPoolStart) >> PAGE_SHIFT);
        ...
        StartPosition = (ULONG)(((PCHAR)StartingAddress - (PCHAR)MmNonPagedPoolStart) >> PAGE_SHIFT);
        if (MI_IS_PHYSICAL_ADDRESS(StartingAddress)) {// Check to ensure this page is really the start of an allocation.
            ...
            if (((StartingAddress >= MmNonPagedPoolExpansionStart) && (StartingAddress < MmNonPagedPoolEnd)) || ((StartingAddress >= MmNonPagedPoolStart) && (StartingAddress < MmNonPagedPoolEnd0))) {
                NOTHING;
            } else {
        ...
        if (Pfn1->u3.e1.StartOfAllocation == 0) {
            KeBugCheckEx(BAD_POOL_CALLER, 0x41, (ULONG_PTR)StartingAddress, (ULONG_PTR)MI_PFN_ELEMENT_TO_INDEX(Pfn1), MmHighestPhysicalPage);
        }
        ...
        if ((Pfn1->u3.e1.EndOfAllocation == 1) &&
            ...
            InterlockedPushEntrySList(&MiNonPagedPoolSListHead, (PSLIST_ENTRY)StartingAddress);
            return 1;
        }

        // The nonpaged pool being freed may be the target of a delayed unlock.
        // Since these pages may be immediately released, force any pending delayed actions to occur now.
#if !defined(MI_MULTINODE)
        if (MmPfnDeferredList != NULL) {
            MiDeferredUnlockPages(0);
        }
#else
        // Each and every node's deferred list would have to be checked so we might as well go the long way and just call.
        MiDeferredUnlockPages(0);
#endif
        ...
        // ASSERT to ensure the pool being freed is not still in use for an I/O.
        if ((Pfn1->u3.e2.ReferenceCount == 0) || ((Pfn1->u3.e2.ReferenceCount > 1) && (Pfn1->u3.e1.WriteInProgress == 0))) {
            MiBadRefCount(Pfn1);
        }
        ...
        if (PointerPte == NULL) {
            ...
            while (Pfn1->u3.e1.EndOfAllocation == 0) {
                ...
                ASSERT((Pfn1->u3.e2.ReferenceCount == 1) || (Pfn1->u3.e1.WriteInProgress == 1));
            }
        ...
        if (StartingAddress >= MmNonPagedPoolExpansionStart) {
            // This page was from the expanded pool, should it be freed?

            // NOTE: all pages in the expanded pool area have PTEs so no physical address checks need to be performed.
            if (Pfn1->u4.MustBeCached == 1) {
                // This is a slush page that is concurrently mapped (cached) within the kernel or HAL's large page mapping.
                // To prevent a possible TB attribute conflict, these pages are never put on the general purpose page free lists.
                // Instead they are cached here for nonpaged pool usage only.
                PfnList = NULL;
                PointerPte = StartPte;

                // Hold the nonpaged pool lock until all the PTEs are cleared so another thread trying to coalesce will see consistent values.
                ...
                MiReleaseSystemPtes(StartPte, (ULONG)NumberOfPages, NonPagedPoolExpansion);
                KeReleaseQueuedSpinLock(LockQueueMmNonPagedPoolLock, OldIrql);
                return (ULONG)NumberOfPages;// No TB flush is done here - the system PTE management code will lazily flush as needed.
            }
        ...
        // We cannot walk off the end of nonpaged expansion pages as the highest expansion allocation is always virtual and guard-paged.
        i = NumberOfPages;
        ...
        if ((Pfn1 != NULL) && (Pfn1->u3.e1.StartOfAllocation == 0)) {
            // This range of pages is free.  Remove this entry from the list and add these pages to the current range being freed.
        ...
        // If it is not the end of an allocation, it must be free and therefore this allocation can be tagged onto the end of that allocation.

        // We cannot walk off the beginning of expansion pool because it is guard-paged.
        // If the initial pool is superpaged instead, we are also safe as the must succeed pages always have EndOfAllocation set.
        ...
        if (Pfn1 != NULL) {
            if (Pfn1->u3.e1.EndOfAllocation == 0) {
                // This range of pages is free, add these pages to this entry.
                // The owner field points to the address of the list entry which is linked into the free pool pages list.
                ...
                // If this entry became larger than MM_SMALL_ALLOCATIONS pages, move it to the tail of the list.
                // This keeps the small allocations at the front of the list.
                if (Entry->Size < MI_MAX_FREE_LIST_HEADS - 1) {
                    ...
                    if (MmProtectFreedNonPagedPool == FALSE) {
                        ...
                        MiProtectedPoolInsertList(&MmNonPagedPoolFreeListHead[Index], &Entry->List, Entry->Size < MM_SMALL_ALLOCATIONS ? TRUE : FALSE);
                    }
                } else {
                    Entry->Size += i;// Add these pages to the previous entry.
                }
            }
        }
        ...
        if (Entry == (PMMFREE_POOL_ENTRY)StartingAddress) {
            ...
            // This entry was not combined with the previous, insert it into the list.
            Entry->Size = i;
            Index = (ULONG)(Entry->Size - 1);
            if (Index >= MI_MAX_FREE_LIST_HEADS) {
                Index = MI_MAX_FREE_LIST_HEADS - 1;
    ...
    // Paged pool.  Need to verify start of allocation using end of allocation bitmap.
    if (!RtlCheckBit(PagedPoolInfo->PagedPoolAllocationMap, StartPosition)) {
        KeBugCheckEx(BAD_POOL_CALLER, 0x50, (ULONG_PTR)StartingAddress, (ULONG_PTR)StartPosition, MmSizeOfPagedPoolInBytes);
    }
    ...
    if (StartPosition > 0) {
        ...
        if (RtlCheckBit(PagedPoolInfo->PagedPoolAllocationMap, StartPosition - 1)) {
            if (!RtlCheckBit(PagedPoolInfo->EndOfPagedPoolBitmap, StartPosition - 1)) {
                ...
                KeBugCheckEx(MEMORY_MANAGEMENT, 0x41286, (ULONG_PTR)PagedPoolInfo->PagedPoolAllocationMap, (ULONG_PTR)PagedPoolInfo->EndOfPagedPoolBitmap, StartPosition);
            }
        }
    ...
    // Find the last allocated page and check to see if any of the pages being deallocated are in the paging file.
    ...
    if (SessionSpace == NULL) {
        if (VerifierLargePagedPoolMap != NULL) {
            ...
            if (MI_CHECK_BIT(BitMap, StartPosition)) {
                ...
                VerifierFreeTrackedPool(StartingAddress, NumberOfPages << PAGE_SHIFT, PagedPool, FALSE);
            }
        }

        if ((NumberOfPages == 1) && (ExQueryDepthSList(&MiPagedPoolSListHead) < MiPagedPoolSListMaximum)) {
            InterlockedPushEntrySList(&MiPagedPoolSListHead, (PSLIST_ENTRY)StartingAddress);
            return 1;
        }

        // If paged pool has been configured as non-pageable, only virtual address space is released.
        if (MmDisablePagingExecutive & MM_PAGED_POOL_LOCKED_DOWN) {
            KeAcquireGuardedMutex(&MmPagedPoolMutex);
            RtlClearBit(PagedPoolInfo->EndOfPagedPoolBitmap, (ULONG)i);// Clear the end of allocation bit in the bit map.
            RtlClearBits(PagedPoolInfo->PagedPoolAllocationMap, StartPosition, (ULONG)NumberOfPages);// Clear the allocation bits in the bit map.
            if (StartPosition < PagedPoolInfo->PagedPoolHint) {
                PagedPoolInfo->PagedPoolHint = StartPosition;
            }

            InterlockedExchangeAddSizeT(&PagedPoolInfo->AllocatedPagedPool, 0 - NumberOfPages);
            FreePoolInPages = MmSizeOfPagedPoolInPages - MmPagedPoolInfo.AllocatedPagedPool;
            if (FreePoolInPages > MiLowPagedPoolThreshold) {
                // Read the state directly instead of calling KeReadStateEvent since we are holding the paged pool mutex and want to keep instructions at a minimum.
                if (MiLowPagedPoolEvent->Header.SignalState != 0) {
    ...
    PagesFreed = MiDeleteSystemPageableVm(PointerPte, NumberOfPages, 0, NULL);
    ...
    if (SessionSpace) {
        KeReleaseGuardedMutex(PoolMutex);
        InterlockedExchangeAddSizeT(&PagedPoolInfo->AllocatedPagedPool, 0 - NumberOfPages);
        InterlockedExchangeAddSizeT(&SessionSpace->CommittedPages, 0 - NumberOfPages);
        ...
        KeReleaseGuardedMutex(PoolMutex);
        InterlockedExchangeAdd((PLONG)&MmPagedPoolCommit, (LONG)(0 - NumberOfPages));
    }

    MiReturnCommitment(NumberOfPages);
    InterlockedExchangeAddSizeT(&PagedPoolInfo->PagedPoolCommit, 0 - NumberOfPages);
...
*/
{
    ...
    // If the verifier or special pool is enabled, then disable lookasides so driver bugs can be found more quickly.
    if ((MmVerifyDriverBufferLength != (ULONG)-1) || (MmProtectFreedNonPagedPool == TRUE) || ((MmSpecialPoolTag != 0) && (MmSpecialPoolTag != (ULONG)-1))) {
    ...
    // Set the last nonpaged pool PFN so coalescing on free doesn't go past the end of the initial pool.
    ...
    // Note that MmResidentAvailablePages & MmAvailablePages are not initialized yet, but this amount is subtracted when MmResidentAvailablePages is initialized later.

    // Limit the charge to 1/6 of the amount of physical memory at bootup or 256mb, whichever is less.
    // Not the virtual size is not reduced because if memory is plentiful, we want to allow the system to grow to its maximal value.

    // Note also the 256mb is not picked randomly - we have seen sparsely populated NT64 machines that need more than 128mb in order to boot because the PFN bitmap (not database) was 142mb.
    ...
    // A guard PTE is built at the top by our caller.
    // This allows us to freely increment virtual addresses in MiFreePoolPages and just check for a blank PTE.
}
...
/*
Routine Description:
    Ensure that the current session has no pool allocations since it is about to exit.  All session allocations must be freed prior to session exit.
Environment:
    Kernel mode.
*/
{
    ...
    // Note the session pool descriptor TotalPages field is not reliable for leak checking because of the fact that nonpaged session allocations
    // are converted to global session allocations - thus when a small nonpaged session allocation results in splitting a full page, the global
    // nonpaged pool descriptor (not the session pool descriptor) is (and must be because of the remaining fragment) charged.
    ...
    if (PagedBytes != 0) {
        ...
        // Being here means they aren't - this is fatal.
        // Force in any valid pages so that a debugger can show who the guilty party is.
        ...
        while (StartPde <= EndPde) {
            if (StartPde->u.Long != 0) {
                // Hunt through the page table page for valid pages and force them in.
                // Note this also forces in the page table page if it is not already.
                PointerPte = MiGetVirtualAddressMappedByPte(StartPde);
                for (i = 0; i < PTE_PER_PAGE; i += 1) {
                    TempPte = *PointerPte;
                    if ((TempPte.u.Hard.Valid == 0) && (TempPte.u.Soft.Protection != 0) && (TempPte.u.Soft.Protection != MM_NOACCESS)) {
                        VirtualAddress = MiGetVirtualAddressMappedByPte(PointerPte);
    ...
    ASSERT(MmSessionSpace->PagedPool.RunningAllocs == MmSessionSpace->PagedPool.RunningDeAllocs);
#endif
}
...
Arguments:
    ...
    ContainingPageFrame - Supplies the page frame number of the page directory page which contains this PDE.
    SessionAllocation - Supplies TRUE if this allocation is in session space, FALSE otherwise.
Return Value:
...
*/
{
    ...
    if ((MmAvailablePages < MM_MEDIUM_LIMIT) || (MI_NONPAGEABLE_MEMORY_AVAILABLE() <= 1)) {
        UNLOCK_PFN2(OldIrql);
        return STATUS_NO_MEMORY;
    }

    // Ensure no other thread handled this while this one waited.  If one has, then return STATUS_RETRY so the caller knows to try again.
    ...
    *PageFrameIndex = MiRemoveZeroPage(MI_GET_PAGE_COLOR_FROM_PTE(PointerPde));
    ...
    // This page will be locked into working set and assigned an index when the working set is set up on return.
...
*/
{
    ...
    MiZeroMemoryPte(PointerPde, (1 + MiGetPdeAddress(MmSessionSpace->PagedPoolEnd) - PointerPde));
    ...
    // We start with all pages in the virtual address space as "busy", and clear bits to make pages available as we dynamically expand the pool.
    RtlSetAllBits(PagedPoolInfo->PagedPoolAllocationMap);
    RtlClearBits(PagedPoolInfo->PagedPoolAllocationMap, 0, PTE_PER_PAGE);// Indicate first page worth of PTEs are available.
    MiCreateBitMap(&PagedPoolInfo->EndOfPagedPoolBitmap, MmSessionPoolSize >> PAGE_SHIFT, NonPagedPool);// Create the end of allocation range bitmap.
...
/*
Routine Description:
    This function searches nonpaged pool for contiguous pages to satisfy the request.
    Note the pool address returned maps these pages as MmCached.
Arguments:
...
*/
{
    ...
    // Initializing SpanInPages* is not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    SpanInPages = 0;
    SpanInPages2 = 0;
    BaseAddress = NULL;
    ...
    // Grab the pool lock and manually search for a page which meets the requirements.
    MmLockPageableSectionByHandle(ExPageLockHandle);

    // Trace through the page allocator's pool headers for a page which meets the requirements.
    ...
    while (Index < MI_MAX_FREE_LIST_HEADS) {
        ...
        while (Entry != &MmNonPagedPoolFreeListHead[Index]) {
            ...
            // The list is not empty, see if this one meets the physical requirements.
            FreePageInfo = CONTAINING_RECORD(Entry, MMFREE_POOL_ENTRY, List);
            ASSERT(FreePageInfo->Signature == MM_FREE_POOL_SIGNATURE);
            if (FreePageInfo->Size >= SizeInPages) {
                // This entry has sufficient space, check to see if the pages meet the physical requirements.
                Va = MiCheckForContiguousMemory(PAGE_ALIGN(Entry), FreePageInfo->Size, SizeInPages, LowestPfn, HighestPfn, BoundaryPfn, MiCached);
                if (Va != NULL) {
                    // These pages meet the requirements.
                    // The returned address may butt up on the end, the front or be somewhere in the middle.
                    // Split the Entry based on which case it is.
                    ...
                    // Adjust the number of free pages remaining in the pool.
                    // The TotalBigPages calculation appears incorrect for the case where we're splitting a block, 
                    // but it's done this way because ExFreePool corrects it when we free the fragment block below.
                    // Likewise for MmAllocatedNonPagedPool and MmNumberOfFreeNonPagedPool which is corrected by MiFreePoolPages for the fragment.
                    InterlockedExchangeAdd((PLONG)&NonPagedPoolDescriptor.TotalBigPages, (LONG)FreePageInfo->Size);
                    InterlockedExchangeAddSizeT(&NonPagedPoolDescriptor.TotalBytes, FreePageInfo->Size << PAGE_SHIFT);
                    ...
                    ASSERT((LONG)MmNumberOfFreeNonPagedPool >= 0);
                    if (Va == Entry) {
                        AllocationPosition = 0;// Butted against the front.
                    } else if (((PCHAR)Va + (SizeInPages << PAGE_SHIFT)) == ((PCHAR)Entry + (FreePageInfo->Size << PAGE_SHIFT))) {
                        AllocationPosition = 2;// Butted against the end.
                    } else {
                        AllocationPosition = 1;// Somewhere in the middle.
                    }

                    // Pages are being removed from the front of the list entry and the whole list entry will be removed and then the remainder inserted.

                    // Mark start and end for the block at the top of the list.
                    ...
                    // Calculate the ending PFN address, note that since these pages are contiguous, just add to the PFN.
                    Pfn1 += SizeInPages - 1;
                    ASSERT(Pfn1->u4.VerifierAllocation == 0);
                    ASSERT(Pfn1->u3.e1.LargeSessionAllocation == 0);
                    ASSERT(Pfn1->u3.e1.EndOfAllocation == 0);
                    Pfn1->u3.e1.EndOfAllocation = 1;
                    if (SizeInPages == FreePageInfo->Size) {// Unlock the pool and return.
                        KeReleaseQueuedSpinLock(LockQueueMmNonPagedPoolLock, OldIrql);
                    ...
                    if (AllocationPosition != 2) {
                        // The end piece needs to be freed as the removal came from the front or the middle.
                        BaseAddress = (PVOID)((PCHAR)Va + (SizeInPages << PAGE_SHIFT));
                        SpanInPages = FreePageInfo->Size - SizeInPages - (((ULONG_PTR)Va - (ULONG_PTR)Entry) >> PAGE_SHIFT);
                        ...
                        // Calculate the ending PTE's address, can't depend on these pages being physically contiguous.
                        if (PointerPte == NULL) {
                            Pfn1 += (SpanInPages - 1);
                        } else {
                    ...
                    if (AllocationPosition != 0) {
                        // The front piece needs to be freed as the removal came from the middle or the end.
                    ...
                    // Note that we have overcharged the pool - the entire free chunk has been billed.
                    // Here we return the piece we didn't use and correct the momentary overbilling.

                    // The start and end allocation bits of this split entry which we just set up enable ExFreePool and his callees to correctly adjust the billing.
                    if (BaseAddress) {
...
PFN_NUMBER MiFindContiguousPages(IN PFN_NUMBER LowestPfn, IN PFN_NUMBER HighestPfn, IN PFN_NUMBER BoundaryPfn, IN PFN_NUMBER SizeInPages, IN MEMORY_CACHING_TYPE CacheType)
/*
Routine Description:
    This function searches nonpaged pool and the free, zeroed, and standby lists for contiguous pages that satisfy the request.
...
Return Value:
    ...
    Nonzero - Returns the base physical frame number to the allocated physically contiguous memory.
Environment:
    Kernel mode, IRQL of APC_LEVEL or below.

    Note that in addition to being called at normal runtime, this routine is also called during Phase 0 initialization before the loaded module
    list has been initialized - therefore this routine cannot be made PAGELK as we wouldn't know how to find it to ensure it was resident.
*/
{
    PMMPTE DummyPte;
    PKTHREAD Thread;
...
Retry:
    ...
    do {
        ...
        if ((count != 0) && (Page + SizeInPages <= LastPage)) {
            // A fit may be possible in this run, check whether the pages are on the right list.
            found = 0;
            Pfn1 = MI_PFN_ELEMENT(Page);
            for (; Page < LastPage; Page += 1, Pfn1 += 1) {
                if ((Pfn1->u3.e1.PageLocation <= StandbyPageList) && (Pfn1->u1.Flink != 0) && (Pfn1->u2.Blink != 0) && (Pfn1->u3.e2.ReferenceCount == 0) && ((CacheAttribute == MiCached) || (Pfn1->u4.MustBeCached == 0))) {
                    // Before starting a new run, ensure that it can satisfy the boundary requirements (if any).
                    if ((found == 0) && (BoundaryPfn != 0)) {
                        if (((Page ^ (Page + SizeInPages - 1)) & BoundaryMask) != 0) {
                            continue;// This run's physical address does not meet the requirements.
                        }
                    }

                    found += 1;
                    if (found == SizeInPages) {
                        // Lock the PFN database and see if the pages are still available for us.
                        // Note the invariant condition (boundary conformance) does not need to be checked again as it was already checked above.
                        Pfn1 -= (found - 1);
                        Page -= (found - 1);
                        LOCK_PFN(OldIrql);
                        do {
                            ...
                            found -= 1;
                            if (found == 0) {
                                // All the pages matched the criteria, keep the PFN lock, remove them and map them for our caller.
                                goto Success;
                            }
                        ...
                        found = 0;// Restart the search at the first possible page.
                    }
                } else {
                    found = 0;
                }
    ...
    // This is done with gradually increasing aggressiveness so as not to prematurely drain modified writes unless it's truly needed.
    switch (RetryCount) {
    case 4:
    ...
    case 2:
        ...
        break;
    case 1:
        // Purge the transition list as transition pages keep page tables from being taken and we are desperate.
        MiPurgeTransitionList();

        // Empty all the working sets now that the transition list has been purged.
        // This will put page tables on the modified list.
        MmEmptyAllWorkingSets();

        // Write out modified pages (including newly trimmed page table pages).
        MiFlushAllPages();

        // Give the writes a chance to complete so the modified pages can be marked clean and put on the transition list.
        KeDelayExecutionThread(KernelMode, FALSE, (PLARGE_INTEGER)&MmOneSecond);

        // Purge the transition list one last time to get the now-clean page table pages out.
        MiPurgeTransitionList();

        // Finally get any straggling active pages onto the transition lists.
        MmEmptyAllWorkingSets();
        MiFlushAllPages();
        break;
    default:
...
Success:
    ...
    // A match has been found, remove these pages and return.  The PFN lock is held.
    ...
    do {
        ...
        // Initialize PteAddress so an MiIdentifyPfn scan won't crash.  The real value is put in after the loop.
...
/*
Routine Description:
    This function frees the specified physical page range, returning both commitment and resident available.
Arguments:
...
Environment:
    ...
    This is callable from MiReloadBootLoadedDrivers->MiUseDriverLargePages during Phase 0.
    ExPageLockHandle and other variables won't exist at this point, so don't get too fancy here.
*/
{
    KIRQL OldIrql;
    PMMPFN Pfn1;
...
*/
{
    ...
    if (CacheAttribute == MiCached) {
        ...
        // An existing range of nonpaged pool satisfies the requirements so return it now.
        if (BaseAddress != NULL) {
            return BaseAddress;
        }
    }
...
PFN_NUMBER MiFindLargePageMemory(IN PCOLORED_PAGE_INFO ColoredPageInfoBase, IN PFN_NUMBER SizeInPages, IN MM_PROTECTION_MASK ProtectionMask, OUT PPFN_NUMBER OutZeroCount)
/*
Routine Description:
    This function searches the free, zeroed, standby and modified lists for contiguous pages to satisfy the request.
    Note the caller must zero the pages on return if these are made visible to the user.
Arguments:
    ColoredPageInfoBase - Supplies the colored page info structure to hang allocated pages off of.
                          This allows the caller to zero only pages that need zeroing, and to easily do those in parallel.
    SizeInPages - Supplies the number of pages to allocate.
...
Return Value:
    0 - a contiguous range could not be found to satisfy the request.
    NON-0 - Returns the starting page frame number of the allocated physically contiguous memory.
Environment:
    Kernel mode, APCs disabled, AddressCreation mutex held.
    The caller must bring in PAGELK.
    The caller has already charged commitment for the range (typically by virtue of the VAD insert) so no commit is charged here.
*/
{
    ULONG Color;
    PKTHREAD Thread;
...
rescan:
    for (; start != MmPhysicalMemoryBlock->NumberOfRuns; start += 1) {
        ...
        while (Page < LastPage) {
            if ((Pfn1->u3.e1.PageLocation <= StandbyPageList) &&
                ...
                ((CurrentNodeColor == ANY_COLOR_OK) || (Pfn1->u3.e1.PageColor == CurrentNodeColor))) {
                found += 1;
                if (found == SizeInPages) {
                    // Lock the PFN database and see if the pages are still available for us.
                    Pfn1 -= (found - 1);
                    ...
                    LOCK_PFN(OldIrql);
                    do {
                        if ((Pfn1->u3.e1.PageLocation <= StandbyPageList) && (Pfn1->u1.Flink != 0) && (Pfn1->u2.Blink != 0) && (Pfn1->u3.e2.ReferenceCount == 0)) {
                            NOTHING;            // Good page
                        } else {
                            break;
                        }
                    ...
                    if (MiShowStuckPages != 0) {
                        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "MiFindLargePages : could not claim stolen PFN %p\n", Page);
...
#if DBG
                if (MiShowStuckPages != 0) {
                    DbgPrintEx(DPFLTR_MM_ID, DPFLTR_INFO_LEVEL, "MiFindLargePages : could not claim PFN %p %x %x\n", Page, Pfn1->u3.e1, Pfn1->u4.EntireFrame);
                    if (MiShowStuckPages & 0x8) {
                        DbgBreakPoint();
                    }
                }
            ...
            Pfn1 += 1;
            if (found == 0) {
                // The last page interrogated wasn't available so skip ahead to the next acceptable boundary.
                NewPage = MI_ROUND_TO_SIZE(Page, (MM_MINIMUM_VA_FOR_LARGE_PAGE >> PAGE_SHIFT));
    ...
    // If we were trying for local memory (ie: NUMA machine), then expand our search to include any node and try one last time.
...
Done:
    // A match has been found, remove these pages, map them and return.  The PFN lock is held.
    ASSERT(start != MmPhysicalMemoryBlock->NumberOfRuns);
    ...
    do {
        ...
        // Add free and standby pages to the list of pages to be zeroed by our caller.
        if (NeedToZero == TRUE) {
...
/*
Routine Description:
    This function returns a contiguous large page allocation to the free memory lists.
Arguments:
...
*/
{
    ...
    // The actual commitment for this range (and its page table pages, etc) is released when the vad is removed.
    // Because we will release commitment below for each physical page, temporarily increase the charge now so it all balances out.
    // Block user APCs so a suspend can't stop us.
        ...
        do {
            ...
            if (OriginalCount == EntryCount) {
                ...
                if (OriginalCount == 1) {
                    ...
                    // Note this routine returns resident available and commitment for the page.
...
LOGICAL MmIsSessionAddress(IN PVOID VirtualAddress)
/*
Routine Description:
...
ULONG MmGetSizeOfBigPoolAllocation(IN PVOID StartingAddress)
/*
Routine Description:
    This function returns the number of pages consumed by the argument big pool allocation.
    It is assumed that the caller still owns the allocation (and guarantees it cannot be freed from underneath us) so this routine can run lock-free.
Arguments:
    StartingAddress - Supplies the starting address which was returned in a previous call to MiAllocatePoolPages.
Return Value:
    Returns the number of pages allocated.
Environment:
    These functions are used by the general pool free routines and should not be called directly.
*/
{
    PMMPFN StartPfn;
    PMMPFN Pfn1;
    ...
    if ((StartingAddress >= MmPagedPoolStart) && (StartingAddress <= MmPagedPoolEnd)) {
        ...
        PagedPoolInfo = &MmSessionSpace->PagedPoolInfo;
        StartPosition = (ULONG)(((PCHAR)StartingAddress - (PCHAR)MmSessionSpace->PagedPoolStart) >> PAGE_SHIFT);
#if DBG
        SessionSpace = SESSION_GLOBAL(MmSessionSpace);
        PoolMutex = &SessionSpace->PagedPoolMutex;
#endif
        ...
        if (StartingAddress < MM_SYSTEM_RANGE_START) {
            KeBugCheckEx(BAD_POOL_CALLER, 0x44, (ULONG_PTR)StartingAddress, (ULONG_PTR)MM_SYSTEM_RANGE_START, 0);
        }
        ...
        if (MI_IS_PHYSICAL_ADDRESS(StartingAddress)) {
            ...
            ASSERT(StartPosition < (ULONG)BYTES_TO_PAGES(MmSizeOfNonPagedPoolInBytes));
            if ((StartingAddress < MmNonPagedPoolStart) || (StartingAddress >= MmNonPagedPoolEnd0)) {
                KeBugCheckEx(BAD_POOL_CALLER, 0x45, (ULONG_PTR)StartingAddress, 0, 0);
            }
        } else {
            PointerPte = MiGetPteAddress(StartingAddress);
            ...
            if (((StartingAddress >= MmNonPagedPoolExpansionStart) && (StartingAddress < MmNonPagedPoolEnd)) ||
                ...
                KeBugCheckEx(BAD_POOL_CALLER, 0x46, (ULONG_PTR)StartingAddress, 0, 0);
            }
            Pfn1 = MI_PFN_ELEMENT(PointerPte->u.Hard.PageFrameNumber);
        }

        if (Pfn1->u3.e1.StartOfAllocation == 0) {
            KeBugCheckEx(BAD_POOL_CALLER, 0x47, (ULONG_PTR)StartingAddress, (ULONG_PTR)MI_PFN_ELEMENT_TO_INDEX(Pfn1), MmHighestPhysicalPage);
        }
    ...
    // Paged pool.  Need to verify start of allocation using end of allocation bitmap.
    if (!RtlCheckBit(PagedPoolInfo->PagedPoolAllocationMap, StartPosition)) {
        KeBugCheckEx(BAD_POOL_CALLER, 0x48, (ULONG_PTR)StartingAddress, (ULONG_PTR)StartPosition, MmSizeOfPagedPoolInBytes);
    }
...
// The number of large page ranges must always be larger than the number of translation register entries for the target platform.
...
/*
Routine Description:
    This routine checks whether the specified page frame must be mapped fully cached because it is already part of a large page which is fully cached.
    This must be detected otherwise we would be creating an incoherent overlapping TB entry as the same physical page would be mapped by 2 different TB entries with different cache attributes.
Arguments:
...
Environment:
    ...
    PFN lock must be held for the results to relied on, but note callers will sometimes call without it for a preliminary scan and then repeat it with the lock held.
*/
{
    ...
    while (Range < LastValidRange) {
        if ((PageFrameIndex >= Range->StartFrame) && (PageFrameIndex <= Range->LastFrame)) {
            return TRUE;
        }
...
/*
Routine Description:
    This routine searches the cached list for PFN-mapped entries and ripples the must-be-cached bits into each PFN entry.
Environment:
...
/*
Routine Description:
    This routine adds the specified page range to the "must be mapped fully cached" list.

    This is typically called with a range which is about to be mapped with large pages fully cached and so no portion of the range can ever be
    mapped noncached or writecombined otherwise we would be creating an incoherent overlapping TB entry as the same physical page would be mapped by 2 different TB entries with different cache attributes.
Arguments:
...
/*
Routine Description:
    This routine removes the specified page range from the "must be mapped fully cached" list.

    This is typically called with a range which was mapped with large pages fully cached and so no portion of the range can ever be mapped noncached or writecombined otherwise we would be creating an
    incoherent overlapping TB entry as the same physical page would be mapped by 2 different TB entries with different cache attributes.
...
*/
{
    ...
    for (i = 0; i < MiLargePageRangeIndex; i += 1, Range += 1) {
        if ((PageFrameIndex == Range->StartFrame) && (LastPageFrameIndex == Range->LastFrame)) {

            // Found it, slide everything else down to preserve any other non zero ranges.
            // Decrement the last valid entry so that searches don't need to walk the whole thing.
