...
Abstract:
    This module contains the routines which implement the NtUnmapViewOfSection service.
*/
...
NTSTATUS MiUnmapViewOfSection(IN PEPROCESS Process, IN PVOID BaseAddress, IN ULONG Flags)
/*
Routine Description:
    This function unmaps a previously created view to a section.
Arguments:
...
*/
{
    ...
    // Get the address creation mutex to block multiple threads from creating or deleting address space at the same time and get the working set mutex so virtual address descriptors can be removed.
    ...
    // If this Vad is for an image section, then get the base address of the section.
    ...
    if (Vad->u.VadFlags.NoChange == 1) {
        // An attempt is being made to delete a secured VAD, check the whole VAD to see if this deletion is allowed.
        status = MiCheckSecuredVad(Vad, StartingVa, RegionSize, MM_SECURE_DELETE_CHECK);
        if (!NT_SUCCESS(status)) {
            if ((Flags & UNMAP_ADDRESS_SPACE_HELD) == 0) {
                UNLOCK_ADDRESS_SPACE(Process);
            }
    ...
    if (Vad->u.VadFlags.VadType == VadRotatePhysical) {
        if ((Flags & UNMAP_ROTATE_PHYSICAL_OK) == 0) {
            // This caller is treating the VA space via the section APIs, but he's not supposed to know that internally we converted the private memory request into a section.
            // So fail this.
            if ((Flags & UNMAP_ADDRESS_SPACE_HELD) == 0) {
                UNLOCK_ADDRESS_SPACE(Process);
            }
    ...
    MiRemoveMappedView(Process, Vad);// NOTE: MiRemoveMappedView releases the working set pushlock before returning.
    MiReturnPageTablePageCommitment(StartingVa, EndingVa, Process, PreviousVad, NextVad);// Return commitment for page table pages if possible.
...
NTSTATUS MmUnmapViewOfSection(__in PEPROCESS Process, __in PVOID BaseAddress)
/*
Routine Description:
    This function unmaps a previously created view to a section.
Arguments:
...
/*
Routine Description:
    This function decrements the subsections, inserting them on the unused subsection list if they qualify.
Arguments:
    FirstSubsection - Supplies the subsection to start at.
    LastSubsection - Supplies the last subsection to insert.  Supplies NULL to decrement all the subsections in the chain.
Environment:
    PFN lock held.
*/
{
    ...
    ASSERT((FirstSubsection->ControlArea->u.Flags.Image == 0) && (FirstSubsection->ControlArea->FilePointer != NULL) && (FirstSubsection->ControlArea->u.Flags.PhysicalMemory == 0));
    ...
    do {
        ...
        if ((MappedSubsection->NumberOfMappedViews == 0) && (MappedSubsection->u.SubsectionFlags.SubsectionStatic == 0)) {
            InsertTailList(&MmUnusedSubsectionList, &MappedSubsection->DereferenceList);// Insert this subsection into the unused subsection list.
...
/*
Routine Description:
    This function removes the mapping from the current process's address space.
    The physical VAD may be a normal mapping (backed by a control area) or it may have no control area (it was mapped by a driver).
Arguments:
...
*/
{
    ...
    if (Vad->u.VadFlags.VadType == VadDevicePhysicalMemory) {
        ...
        // This is a physical memory view.
        // The pages map physical memory and are not accounted for in the working set list or in the PFN database.
        ...
        if (MI_PDE_MAPS_LARGE_PAGE(PointerPde)) {
            MiFreeLargePages(MI_VPN_TO_VA(Vad->StartingVpn), MI_VPN_TO_VA_ENDING(Vad->EndingVpn), TRUE);
            ...
            while (PointerPte <= LastPte) {
                ...
                MI_DECREMENT_USED_PTES_BY_HANDLE(UsedPageTableHandle);// Decrement the count of non-zero page table entries for this page table.
                ...
                // And if this results in an empty page directory page, then delete that too.
                if (MI_GET_USED_PTES_FROM_HANDLE(UsedPageTableHandle) == 0) {
                    ...
                    MiDeletePte(PointerPde, TempVa, FALSE, CurrentProcess, NULL, &PteFlushList, OldIrql);
                    ...
                    if (MI_GET_USED_PTES_FROM_HANDLE(UsedPageDirectoryHandle) == 0) {
                        ...
                        MiDeletePte(PointerPpe, TempVa, FALSE, CurrentProcess, NULL, &PteFlushList, OldIrql);
                        ...
                        if (MI_GET_USED_PTES_FROM_HANDLE(UsedPageDirectoryParentHandle) == 0) {
                            ...
                            PteFlushList.Count = MM_MAXIMUM_FLUSH_COUNT;
                            MiDeletePte(PointerPxe, TempVa, FALSE, CurrentProcess, NULL, &PteFlushList, OldIrql);
                            CurrentProcess->NumberOfPrivatePages += 1;// Add back in the private page MiDeletePte subtracted.
                        }
#endif
        ...
        if (Vad->u2.VadFlags2.ExtendableFile) {
            ...
            // Release the working set pushlock before referencing the segment since it is pageable.
            UNLOCK_WS_UNSAFE(CurrentThread, CurrentProcess);
            ExtendedInfo = NULL;
        ...
        if (Vad->u.VadFlags.VadType != VadImageMap) {
            if (ControlArea->FilePointer != NULL) {
                if ((Vad->u.VadFlags.Protection == MM_READWRITE) || (Vad->u.VadFlags.Protection == MM_EXECUTE_READWRITE)) {
                    InterlockedDecrement((PLONG)&ControlArea->WritableUserReferences);// Adjust the count of writable user mappings to support transactions.
                }
                ...
                // In this case, all the subsections can be safely decremented.
            ...
            if (
                ...
                MiFreeLargePages(TempVa, MI_VPN_TO_VA_ENDING(Vad->EndingVpn), FALSE);
                UNLOCK_WS_UNSAFE(CurrentThread, CurrentProcess);
                MiReleasePhysicalCharges(Vad->EndingVpn - Vad->StartingVpn + 1, CurrentProcess);

                // Decrement the count of the number of views for the control area.
...
#if DBG
                // Release the working set pushlock before referencing the prototype PTEs since they are pageable.
                UNLOCK_WS_UNSAFE(CurrentThread, CurrentProcess);
...
#if defined (_WIN64)
            MiDeletePageDirectoriesForPhysicalRange(MI_VPN_TO_VA(Vad->StartingVpn), MI_VPN_TO_VA_ENDING(Vad->EndingVpn));
#endif
        } else {
    ...
    if (ControlArea) {
        // Decrement the count of the number of views for the Segment object.
        // This requires the PFN lock to be held (it is already).
        ControlArea->NumberOfMappedViews -= 1;
        ControlArea->NumberOfUserReferences -= 1;
...
/*
Routine Description:
    This function locates subsections within an image section that contain global memory and resets the global memory back to the initial subsection contents.
    Note, that for this routine to be called the section is not referenced nor is it mapped in any process.
Arguments:
    ...
    do {
        if (Subsection->u.SubsectionFlags.GlobalMemory == 1) {
            ...
            if (Subsection->StartingSector != 0) {
                ...
                SizeOfRawData = (Subsection->NumberOfFullSectors << MMSECTOR_SHIFT) | Subsection->u.SubsectionFlags.SectorEndOffset;
            }
            ...
            while (PointerPte < LastPte) {
                ...
                if ((PteContents.u.Soft.Prototype == 0) && (PteContents.u.Soft.Transition == 1)) {
                    ...
                    // If the prototype PTE is no longer pointing to the original image page (not in protopte format), or has been modified, remove it from memory.
                    if ((Pfn1->u3.e1.Modified == 1) || (Pfn1->OriginalPte.u.Soft.Prototype == 0)) {
                        ASSERT(Pfn1->OriginalPte.u.Hard.Valid == 0);

                        // This is a transition PTE which has been modified or is no longer in protopte format.
                        if (Pfn1->u3.e2.ReferenceCount != 0) {
                            ...
                            if ((DelayCount % 1024) == 0) {
                                DbgPrintEx(DPFLTR_MM_ID, DPFLTR_WARNING_LEVEL, "MMFLUSHSEC: waiting for i/o to complete PFN %p\n", Pfn1);
                            }
                            DelayCount += 1;
#endif
                        ...
                        // Only reduce the number of PFN references if the original PTE is still in prototype PTE format.
                        ...
                        // If the reference count for the page is zero, insert it into the free page list, otherwise leave it alone and when the reference count is decremented to zero the page will go to the free list.
                        if (Pfn1->u3.e2.ReferenceCount == 0) {
                    ...
                    if (PteContents.u.Soft.Prototype == 0) {
                        // This refers to a page in the paging file, as it no longer references the image, restore the PTE contents to what they were at the initial image creation.
                ...
                if (OffsetIntoSubsection >= SizeOfRawData) {
                    // There are trailing demand zero pages in this subsection, set the PTE contents to be demand zero for the remainder of the PTEs in this subsection.
