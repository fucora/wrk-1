...
// Note these are deliberately sign-extended so they will always be greater than the highest user address.
...
*/
{
    ...
    for (i = 0; i < MmPhysicalMemoryBlock->NumberOfRuns; i += 1) {
        ...
        for (; Pfn1 < LastPfn; Pfn1 += 1) {
            ...
            if (PfnId.u1.e1.UseDescription == MMPFNUSE_PAGEFILEMAPPED) {
                Master = MM_PAGEFILE_BACKED_SHMEM_MARK;// This page belongs to a pagefile-backed shared memory section.
                ...
                Master = MM_NONPAGED_POOL_MARK;// This is nonpaged pool, put it in the nonpaged pool cell.
            } else if (PfnId.u1.e1.UseDescription == MMPFNUSE_PAGEDPOOL) {
                Master = MM_PAGED_POOL_MARK;// This is paged pool, put it in the paged pool cell.
            } else if (PfnId.u1.e1.UseDescription == MMPFNUSE_SESSIONPRIVATE) {
                Master = MM_PAGED_POOL_MARK;// Call this paged pool for now.
            } else if (PfnId.u1.e1.UseDescription == MMPFNUSE_DRIVERLOCKPAGE) {
                Master = MM_NONPAGED_POOL_MARK;// Call this nonpaged pool for now.
            } else if (PfnId.u1.e1.UseDescription == MMPFNUSE_AWEPAGE) {
                Master = MM_NONPAGED_POOL_MARK;// Call this nonpaged pool for now.
            } else {
                // See if the page is part of the kernel or a driver image.
                // If not but it's in system PTEs, call it a kernel thread stack.

                // If neither of the above, then see if the page belongs to a user address or a session pagetable page.
            ...
            if (PfnId.u1.e1.ListDescription == ActiveAndValid) {
                Info->ValidCount += 1;
            } else if ((PfnId.u1.e1.ListDescription == StandbyPageList) || (PfnId.u1.e1.ListDescription == TransitionPage)) {
                Info->TransitionCount += 1;
            } else if ((PfnId.u1.e1.ListDescription == ModifiedPageList) || (PfnId.u1.e1.ListDescription == ModifiedNoWritePageList)) {
    ...
    for (Info = InfoStart; Info < InfoEnd; Info += 1) {
        ...
        } else if (((PUCHAR)Info->StringOffset >= (PUCHAR)&KernMap[0]) && ((PUCHAR)Info->StringOffset <= (PUCHAR)&KernMap[KernSize])) {
...
// One benefit of using run length maximums of less than 4GB is that even frame numbers above 4GB are handled properly despite the 32-bit limitations of the bitmap routines.
...
*/
{
    ...
    for (i = 0; i < MmPhysicalMemoryBlock->NumberOfRuns; i += 1) {
        ...
        if (Pfn1 == LastPfn) {
            continue;// No valid PFNs in this block, move on to the next block.
        }
        ...
        if (InitialPfn != NULL) {
            ...
            PerfInfoLogBytes(PERFINFO_LOG_TYPE_MEMORYSNAPLITE, MemSnap, sizeof(MMPFN_MEMSNAP_INFORMATION) + (ULONG)((MemSnap->Count + 7) / 8));
            RtlClearAllBits(&BitMap);
        }
    }
...
/*
Routine Description:
    This routine calls the pfn id code for each page.
    Because the logging can't handle very large amounts of data in a burst (limited buffering), the data is broken into page size chunks.
Environment:
    Kernel mode.
    PASSIVE level.
    No locks held.
*/
{
    ...
    // Walk through the ranges and identify pages until the buffer is full or we've run out of pages.
    for (i = 0; i < MmPhysicalMemoryBlock->NumberOfRuns; i += 1) {
        ...
        while (Pfn1 < EndPfn) {
            ...
            if (BufferPointer == BufferLast) {
                UNLOCK_PFN(OldIrql);// Release and reacquire the PFN lock so it's not held so long.
    ...
    // Note that releasing this mutex here means the last entry can be inserted out of order if we are preempted and another thread starts the same operation (or if we're on an MP machine).
    // The PERF module must handle this properly as any synchronization provided by this routine is purely a side effect not deliberate.
    MI_UNLOCK_DYNAMIC_MEMORY_SHARED();
    KeLeaveGuardedRegionThread(Thread);
...
*/
{
    ...
    if (PageFrameIndex == MiIdentifyFrame) {
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_TRACE_LEVEL, "MmIdentifyPfn: requested PFN %p\n", PageFrameIndex);
        DbgBreakPoint();
    }
#endif
    ...
    case BadPageList:
        return;
    case ActiveAndValid:
        // It's too much work to determine if the page is locked in a working set due to cross-process WSL references, etc.
        ...
        } else if (Pfn1->u3.e2.ReferenceCount > 1) {// This page is pinned, presumably for an ongoing I/O.
            PfnIdentity->u1.e1.Pinned = 1;
    ...
    case ModifiedNoWritePageList:
        ...
        if ((Pfn1->u3.e1.PageLocation == ModifiedPageList) && (MI_IS_PFN_DELETED(Pfn1)) && (Pfn1->u2.ShareCount == 0)) {
            // This page may be a modified write completing in the context of the modified writer thread.
            // If the address space was deleted while the I/O was in progress, the frame will be released now.
            // More importantly, the frame's containing frame is meaningless as it may have already been freed and reused.

            // We can't tell what this page was being used for since its address space is gone, so just call it process private for now.
            MI_INCREMENT_IDENTIFY_COUNTER(40);
            PfnIdentity->u1.e1.UseDescription = MMPFNUSE_PROCESSPRIVATE;
            return;
        }

        if ((Pfn1->u3.e1.PageLocation == StandbyPageList) && (MI_IS_PFN_DELETED(Pfn1)) && (Pfn1->u3.e1.ReadInProgress == 1)) {
            // Don't bother trying to get fancy and decode any prototype PTE encoding here in the PFN or original PTE.
            // This is a case where the user deleted the virtual address space while one of its threads was in the middle of reading it in !
        ...
        break;
    case TransitionPage:
        // This page is pinned due to a straggling I/O - the virtual address has been deleted but an I/O referencing it has not completed.
        PfnIdentity->u1.e1.Pinned = 1;
        MI_INCREMENT_IDENTIFY_COUNTER(11);
        PfnIdentity->u1.e1.UseDescription = MMPFNUSE_PROCESSPRIVATE;
        return;
    default:
#if DBG
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "MmIdentifyPfn: unknown PFN %p %x\n", Pfn1, Pfn1->u3.e1.PageLocation);
        DbgBreakPoint();
#endif
        break;
    }
    ...
    // General purpose stress shows 40% of the pages are prototypes so for speed, check for these first.
    if (Pfn1->u3.e1.PrototypePte == 1) {
        MI_INCREMENT_IDENTIFY_COUNTER(12);
        if (Pfn1->OriginalPte.u.Soft.Prototype == 0) {
            // Demand zero or (equivalently) pagefile backed.
            // There are some hard problems here preventing more indepth identification of these pages:
            // 1.  The PFN contains a backpointer to the prototype PTE - but there is no definitive way to get to the SEGMENT or CONTROL_AREA from this.
            // 2.  The prototype PTE pointer itself may be paged out and the PFN lock is held right now.
        ...
        // Put the Flags.Image bit into the FilePointer so we can tell between a file opened as Image/Data.
        ...
        if (Subsection->SubsectionBase != NULL) {
            ...
            // The only time we should be here (a valid PFN with no subsection) is if we are the segment dereference thread putting pages into the freelist.
            // At this point the PFN lock is held and the control area/subsection/PFN structures are not yet consistent so just treat this as an offset of 0 as it should be rare.
            ASSERT(PsGetCurrentThread()->StartAddress == (PVOID)(ULONG_PTR)MiDereferenceSegmentThread);
        }

        // Check for nomodwrite sections - typically this is filesystem metadata although it could also be registry data (which is named).
        if (ControlArea->u.Flags.NoModifiedWriting) {
        ...
        // No name - this file must be in the midst of a purge, but it still *was* a mapped file of some sort.
    ...
    if ((PageFrameIndex >= MiStartOfInitialPoolFrame) && (PageFrameIndex <= MiEndOfInitialPoolFrame)) {
        ...
        VirtualAddress = (PVOID)((ULONG_PTR)MmNonPagedPoolStart + ((PageFrameIndex - MiStartOfInitialPoolFrame) << PAGE_SHIFT));
    ...
    if (MI_IS_SESSION_ADDRESS(VirtualAddress)) {
        // Note session addresses that map images (or views) that haven't undergone a copy-on-write split were already treated as prototype PTEs above.
        // This clause handles session pool and copy-on-written pages.
    ...
    for (i = 0; i < MiPteRangeIndex; i += 1) {
        if ((VirtualAddress >= MiPteRanges[i].StartingVa) && (VirtualAddress <= MiPteRanges[i].EndingVa)) {
            // This is driver space, kernel stack, special pool or other system PTE mappings.
...
#if defined (_WIN64)
    // NT64 has a separate special pool virtual address range (unlike 32 bit that has to share it with system PTEs).
    if ((VirtualAddress >= MmSpecialPoolStart) && (VirtualAddress < MmSpecialPoolEnd) && (MmSpecialPoolStart != NULL)) {
        POOL_TYPE PoolType;
    ...
    if (Pfn1->u4.PteFrame == MI_MAGIC_AWE_PTEFRAME) {
        ...
        if ((Pfn1->u3.e1.StartOfAllocation == 1) && (Pfn1->u3.e1.EndOfAllocation == 1) && (Pfn1->u3.e1.PageLocation == ActiveAndValid)) {
    ...
    // AWE frames get their containing frame decremented when the AWE frame is freed.
    ...
    // Check the PFN working set index carefully here.
    // This must be done before walking back through the containing frames because if this page is not in a working set, the containing frame may not be meaningful and dereferencing it can crash the system and/or yield incorrect walks.
    // This is because if a page will never be trimmable there is no need to have a containing frame initialized.
    // This also covers the case of data pages mapped via large page directory entries as these have no containing page table frame.
    if (Pfn1->u3.e1.PageLocation == ActiveAndValid) {
        if (Pfn1->u1.WsIndex == 0) {
            // Default to calling these allocations nonpaged pool because even when they technically are not, from a usage standpoint they are.
            // Note the default is overridden for specific cases where the usage is not in fact nonpaged.
            PfnIdentity->u1.e1.UseDescription = MMPFNUSE_NONPAGEDPOOL;
    ...
    while (Pfn1->u4.PteFrame != PageFrameIndex) {
        // The only way the PTE address will go out of bounds is if this is a top level page directory page for a process that has been swapped out but is still waiting for the transition/modified page table pages to be reclaimed.
        // ie: until that happens, the page directory is marked Active, but the PteAddress & containing page are pointing at the EPROCESS pool page.
...
#if defined(_X86PAE_)
    // PAE is unique because the 3rd level is not defined as only a mini 4 entry 3rd level is in use.
    // Check for that explicitly, noting that it takes one extra walk to get to the top.
    // Top level PAE pages (the ones that contain only the 4 PDPTE pointers) are treated above as active pinned pages, not as pagetable pages because each one is shared across 127 processes and resides in the system global space.
    ...
    // Must have been a page in the hierarchy (not a data page) as we arrived at the top early.
    MI_INCREMENT_IDENTIFY_COUNTER(30);
    PfnIdentity->u1.e1.UseDescription = MMPFNUSE_PAGETABLE;
}


VOID MiSnapCopyMemory(IN PVOID Destination, IN PFN_NUMBER PageFrameIndex, IN SIZE_T NumberOfBytes)
// nonpaged helper routine since it acquires a spinlock but is used from paged code callers.
{
    PVOID Va;
    KIRQL OldIrql;
    ...
    // Determine if we're already at DISPATCH to avoid "raising" IRQL to the existing level.
    // In addition to avoiding the overhead of a HAL call on some platforms, this also avoids the verifier irql stack trace logging which consumes large amounts of kernel stack (ie, can cause overflow).
    if (KeGetCurrentIrql() == DISPATCH_LEVEL) {
        ...
        // Note the call below can also set OldIrql to MM_NOIRQL (when superpages are used) so a distinct local (AtDpc) must be used.
