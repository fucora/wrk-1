...
// Every time a thread is created, the kernel stack is subtracted from this and every time a process is created, the minimum working set is subtracted from this.
...
// Note the ROM page listhead is deliberately not in the set of MmPageLocationList ranges.
...
// This is set in the registry to the maximum number of gigabytes of RAM that can be added to this machine (ie: #of DIMM slots times maximum supported DIMM size).
// This lets configurations that won't use the absolute maximum indicate that a smaller (virtually) PFN database size can be used thus leaving more virtual address space for things like system PTEs, etc.
...
LOGICAL  MmTrackLockedPages;// Set via the registry to identify which drivers are leaking locked pages.
LOGICAL MmSnapUnloads = TRUE;// Set via the registry to identify drivers which unload without releasing resources or still have active timers, etc.
...
KGUARDED_MUTEX MmSectionCommitMutex;// Section commit mutex.
KGUARDED_MUTEX MmSectionBasedMutex;// Section base address mutex.
...
// If more than this percentage of pool is consumed and pool allocations might fail, then trim unused segments & subsections to get back to this percentage.
...
// Limit on committed pages.  When MmTotalCommittedPages would become greater than or equal to this number the paging files must be expanded.
...
// Cache control stuff.  Note this may be overridden by deficient hardware platforms at startup.
