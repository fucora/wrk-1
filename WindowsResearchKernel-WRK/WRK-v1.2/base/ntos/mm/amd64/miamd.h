...
// PAGE_SIZE for AMD64 is 4k, virtual page is 36 bits with a PAGE_SHIFT byte offset.
...
#define MM_NUMBER_OF_COLORS (1)// AMD64 does not require support for colored pages.
#define MM_COLOR_MASK (0)// Mask for obtaining color from a physical page number.
#define MM_COLOR_ALIGNMENT (0)// Boundary for aligned pages of like color upon.
#define MM_COLOR_MASK_VIRTUAL (0)// Mask for isolating color from virtual address.
...
#define MM_MAXIMUM_WORKING_SET     ((((ULONG_PTR)8 * 1024 * 1024 * 1024 * 1024) - (64 * 1024 * 1024)) >> PAGE_SHIFT) //8Tb-64Mb
...
// Bit fields to or into PTE to make a PTE valid based on the protection field of the invalid PTE.
...
#define MM_ZERO_PTE 0// Zero PTE
#define MM_ZERO_KERNEL_PTE 0// Zero Kernel PTE
...
// VOID MI_MAKE_VALID_USER_PTE (OUT OUTPTE, IN FRAME, IN PMASK, IN PPTE);
// Routine Description:
//    This macro makes a valid *USER* PTE from a page frame number, protection mask, and owner.
//    THIS MUST ONLY BE USED FOR PAGE TABLE ENTRIES (NOT PAGE DIRECTORY ENTRIES), MAPPING USER (NOT KERNEL OR SESSION) VIRTUAL ADDRESSES.
// Arguments
...
// VOID MI_MAKE_VALID_KERNEL_PTE (OUT OUTPTE, IN FRAME, IN PMASK, IN PPTE);
// Routine Description:
//    This macro makes a valid *KERNEL* PTE from a page frame number, protection mask, and owner.
//    THIS MUST ONLY BE USED FOR PAGE TABLE ENTRIES (NOT PAGE DIRECTORY ENTRIES), MAPPING GLOBAL (NOT SESSION) VIRTUAL ADDRESSES.
// Arguments
//    OUTPTE - Supplies the PTE in which to build the valid PTE.
//    FRAME - Supplies the page frame number for the PTE.
//    PMASK - Supplies the protection to set in the valid PTE.
...
// VOID MI_MAKE_VALID_PTE (OUT OUTPTE, IN FRAME, IN PMASK, IN PPTE);
// Routine Description:
//    This macro makes a valid PTE from a page frame number, protection mask, and owner.
// Arguments
...
// VOID MI_MAKE_VALID_PTE_TRANSITION (
//    IN OUT OUTPTE
//    IN PROTECT
//    );
// Routine Description:
...
// Arguments
//    OUTPTE - Supplies the current valid PTE.  This PTE is then modified to become a transition PTE.
...
// VOID MI_MAKE_TRANSITION_PTE (OUT OUTPTE, IN PAGE, IN PROTECT, IN PPTE);
// Routine Description:
//    This macro takes a valid pte and turns it into a transition PTE.
// Arguments
//    OUTPTE - Supplies the PTE in which to build the transition PTE.
...
//    PROTECT - Supplies the protection to set in the transition PTE.
//    PPTE - Supplies a pointer to the PTE, this is used to determine the owner of the PTE.
...
// VOID MI_MAKE_TRANSITION_PTE_VALID (OUT OUTPTE, IN PPTE);
// Routine Description:
...
// VOID MI_MAKE_TRANSITION_KERNELPTE_VALID (OUT OUTPTE, IN PPTE);
// Routine Description:
...
// VOID MI_MAKE_TRANSITION_PROTOPTE_VALID (OUT OUTPTE, IN PPTE);
// Routine Description:
//    This macro takes a transition prototype PTE (in paged pool) and makes it a valid PTE.
//    Because we know this is a prototype PTE and not a pagetable PTE, this can directly or in the global bit.
//    This makes a measurable performance gain since every instruction counts when holding the PFN lock.
// Arguments
//    OUTPTE - Supplies the PTE in which to build the valid PTE.
//    PPTE - Supplies a pointer to the transition PTE.
...
// VOID MI_SET_PTE_IN_WORKING_SET (OUT PMMPTE PTE, IN ULONG WSINDEX);
// Routine Description:
//    This macro inserts the specified working set index into the argument PTE.
//    No TB invalidation is needed for other processors (or this one) even though the entry may already be in a TB - it's just a software field update and doesn't affect miss resolution.
// Arguments
...
// ULONG WsIndex MI_GET_WORKING_SET_FROM_PTE(IN PMMPTE PTE);
// Routine Description:
//    This macro returns the working set index from the argument PTE.
//    Since the AMD64 PTE has no free bits nothing needs to be done on this architecture.
// Arguments
...
// VOID MI_SET_PTE_WRITE_COMBINE (IN MMPTE PTE);
// Routine Description:
//    This macro takes a valid PTE and enables WriteCombining as the caching state.
//    Note that the PTE bits may only be set this way if the Page Attribute Table is present and the PAT has been initialized to provide Write Combining.

//    If either of the above conditions is not satisfied, then the macro enables WEAK UC (PCD = 1, PWT = 0) in the PTE.
// Arguments
...
// VOID MI_PREPARE_FOR_NONCACHED (IN MI_PFN_CACHE_ATTRIBUTE CacheAttribute);
// Routine Description:
//    This macro prepares the system prior to noncached PTEs being created.
// Arguments
//    CacheAttribute - Supplies the cache attribute the PTEs will be filled with.
...
// VOID MI_FLUSH_TB_FOR_INDIVIDUAL_ATTRIBUTE_CHANGE (IN PFN_NUMBER PageFrameIndex, IN MI_PFN_CACHE_ATTRIBUTE CacheAttribute);
// Routine Description:
...
//    Additionally, the cache must be flushed if we are switching from write back to write combined (or noncached) because otherwise the
//    current data may live in the cache while the uc/wc mapping is used and then when the uc/wc mapping is freed, the cache will hold stale data that will be found when a normal write back mapping is reapplied.
// Arguments
//    PageFrameIndex - Supplies the page frame number that is going to be used with the new attribute.
//    CacheAttribute - Supplies the cache attribute the new PTEs will be filled with.
...
// VOID MI_FLUSH_ENTIRE_TB_FOR_ATTRIBUTE_CHANGE (IN MI_PFN_CACHE_ATTRIBUTE CacheAttribute);
// Routine Description:
//    The entire TB must be flushed if we are changing cache attributes.
//    KeFlushSingleTb cannot be used because we don't know what virtual address(es) this physical frame was last mapped at.
...
// VOID MI_FLUSH_TB_FOR_CACHED_ATTRIBUTE (VOID);
// Routine Description:
//    The entire TB must be flushed if we are changing cache attributes.
//    KeFlushSingleTb cannot be used because we don't know what virtual address(es) a physical frame was last mapped at.
//    Note no cache flush is needed because the attribute-changing-pages are going to be mapped fully cached.
#define MI_FLUSH_TB_FOR_CACHED_ATTRIBUTE()                         \
            MiFlushTbForAttributeChange += 1;                      \
            MI_FLUSH_ENTIRE_TB (0x23);


// VOID MI_SET_PTE_DIRTY (IN MMPTE PTE);
// Routine Description:
...
// VOID MI_SET_PTE_CLEAN (IN MMPTE PTE);
// Routine Description:
...
// VOID MI_IS_PTE_DIRTY (IN MMPTE PTE);
// Routine Description:
...
// VOID MI_SET_GLOBAL_STATE (IN MMPTE PTE, IN ULONG STATE);
// Routine Description:
...
// VOID MI_ENABLE_CACHING (IN MMPTE PTE);
// Routine Description:
//    This macro takes a valid PTE and sets the caching state to be enabled.
//    This is performed by clearing the PCD and PWT bits in the PTE.

//    Semantics of the overlap between PCD, PWT, and the USWC memory type in the MTRR are:
...
// VOID MI_DISABLE_CACHING (IN MMPTE PTE);
// Routine Description:
//    This macro takes a valid PTE and sets the caching state to be disabled.  This is performed by setting the PCD and PWT bits in the PTE.
...
//    Since an effective memory type of UC is desired here, the WT bit is set.
// Arguments
...
// BOOLEAN MI_IS_CACHING_DISABLED (IN PMMPTE PPTE);
// Routine Description:
//    This macro takes a valid PTE and returns TRUE if caching is disabled.
// Arguments
...
// VOID MI_SET_PFN_DELETED (IN PMMPFN PPFN);
// Routine Description:
//    This macro takes a pointer to a PFN element and indicates that the PFN is no longer in use.
// Arguments
...
// VOID MI_MARK_PFN_UNDELETED (IN PMMPFN PPFN);
// Routine Description:
//    This macro takes a pointer to a deleted PFN element and mark that the PFN is not deleted.
// Arguments
//    PPTE - Supplies a pointer to the PFN element.
#define MI_MARK_PFN_UNDELETED(PPFN)     PPFN->PteAddress = (PMMPTE)((ULONG_PTR)PPFN->PteAddress & ~0x1);


// BOOLEAN MI_IS_PFN_DELETED (IN PMMPFN PPFN);
// Routine Description:
//    This macro takes a pointer to a PFN element and determines if the PFN is no longer in use.
// Arguments
//    PPTE - Supplies a pointer to the PFN element.
// Return Value:
...
// VOID MI_CHECK_PAGE_ALIGNMENT (IN ULONG PAGE, IN PMMPTE PPTE);
// Routine Description:
//    This macro takes a PFN element number (Page) and checks to see if the virtual alignment for the previous address of the page is compatible with the new address of the page.
//    If they are not compatible, the D cache is flushed.
// Arguments
//    PAGE - Supplies the PFN element.
//    PPTE - Supplies a pointer to the new PTE which will contain the page.
#define MI_CHECK_PAGE_ALIGNMENT(PAGE,PPTE)// does nothing on AMD64.


// VOID MI_INITIALIZE_HYPERSPACE_MAP (VOID);
// Routine Description:
//    This macro initializes the PTEs reserved for double mapping within hyperspace.
#define MI_INITIALIZE_HYPERSPACE_MAP(INDEX)// does nothing on AMD64.


// ULONG MI_GET_NEXT_COLOR (IN ULONG COLOR);
// Routine Description:
...
// ULONG MI_GET_PREVIOUS_COLOR (IN ULONG COLOR);
// Routine Description:
...
// VOID MI_GET_MODIFIED_PAGE_BY_COLOR (OUT ULONG PAGE, IN ULONG COLOR);
// Routine Description:
//    This macro returns the first page destined for a paging file with the desired color.
//    It does NOT remove the page from its list.
// Arguments
//    PAGE - Returns the page located, the value MM_EMPTY_LIST is returned if there is no page of the specified color.
...
// VOID MI_GET_MODIFIED_PAGE_ANY_COLOR (OUT ULONG PAGE, IN OUT ULONG COLOR);
// Routine Description:
//    This macro returns the first page destined for a paging file with the desired color.
//    If not page of the desired color exists, all colored lists are searched for a page.
//    It does NOT remove the page from its list.
// Arguments
//    PAGE - Returns the page located, the value MM_EMPTY_LIST is returned if there is no page of the specified color.
//    COLOR - Supplies the color of page to locate and returns the color of the page located.
...
// VOID MI_MAKE_VALID_PTE_WRITE_COPY (IN OUT PMMPTE PTE);
// Routine Description:
//    This macro checks to see if the PTE indicates that the page is writable and if so it clears the write bit and sets the copy-on-write bit.
// Arguments
...
// Routine Description:
//    This macro examines the virtual address of the PTE and determines if the PTE resides in system space or user space.
// Arguments
//    PTE - Supplies the PTE to operate upon.
// Return Value:
...
// VOID MI_SET_ACCESSED_IN_PTE (IN OUT MMPTE PPTE, IN ULONG ACCESSED);
// Routine Description:
...
// ULONG MI_GET_ACCESSED_IN_PTE (IN OUT MMPTE PPTE);
// Routine Description:
...
// Mask to clear all fields but protection in a PTE to or in paging file location.
#define CLEAR_FOR_PAGE_FILE 0x000003E0


// VOID MI_SET_PAGING_FILE_INFO (OUT MMPTE OUTPTE, IN MMPTE PPTE, IN ULONG FILEINFO, IN ULONG OFFSET);
// Routine Description:
//    This macro sets into the specified PTE the supplied information to indicate where the backing store for the page is located.
// Arguments
...
// PMMPTE MiPteToProto (IN OUT MMPTE PPTE, IN ULONG FILEINFO, IN ULONG OFFSET);
// Routine Description:
//   This macro returns the address of the corresponding prototype which was encoded earlier into the supplied PTE.
// Arguments
...
// ULONG MiProtoAddressForPte (IN PMMPTE proto_va);
// Routine Description:
//    This macro sets into the specified PTE the supplied information to indicate where the backing store for the page is located.
//    MiProtoAddressForPte returns the bit field to OR into the PTE to reference a prototype PTE.
//    And set the protoPTE bit, MM_PTE_PROTOTYPE_MASK.
// Arguments
...
// ULONG MiProtoAddressForKernelPte (IN PMMPTE proto_va);
// Routine Description:
//    This macro sets into the specified PTE the supplied information to indicate where the backing store for the page is located.
//    MiProtoAddressForPte returns the bit field to OR into the PTE to reference a prototype PTE.
//    And set the protoPTE bit, MM_PTE_PROTOTYPE_MASK.

//    This macro also sets any other information (such as global bits) required for kernel mode PTEs.
// Arguments
//    proto_va - Supplies the address of the prototype PTE.
// Return Value:
//    Mask to set into the PTE.
...
// PSUBSECTION MiGetSubsectionAddress (IN PMMPTE lpte);
// Routine Description:
//   This macro takes a PTE and returns the address of the subsection that the PTE refers to.  Subsections are quadword structures allocated from nonpaged pool.
// Arguments
//    lpte - Supplies the PTE to operate upon.
// Return Value:
...
// ULONG MiGetSubsectionAddressForPte (IN PSUBSECTION VA);
// Routine Description:
//    This macro takes the address of a subsection and encodes it for use in a PTE.
// Arguments
//    VA - Supplies a pointer to the subsection to encode.
// Return Value:
//     The mask to set into the PTE to make it reference the supplied subsection.
...
// PMMPTE MiGetPdeAddress (IN PVOID va);
// Routine Description:
//    MiGetPdeAddress returns the address of the PDE which maps the given virtual address.
// Arguments
...
// PMMPTE MiGetPteAddress (IN PVOID va);
// Routine Description:
//    MiGetPteAddress returns the address of the PTE which maps the given virtual address.
// Arguments
...
// ULONG MiGetPxeOffset (IN PVOID va);
// Routine Description:
//    MiGetPxeOffset returns the offset into an extended page directory parent for a given virtual address.
// Arguments
//    Va - Supplies the virtual address to locate the offset for.
// Return Value:
//    The offset into the extended parent page directory table the corresponding PXE is at.
#define MiGetPxeOffset(va) ((ULONG)(((ULONG_PTR)(va) >> PXI_SHIFT) & PXI_MASK))


// ULONG MiGetPxeIndex (IN PVOID va);
// Routine Description:
//    MiGetPxeIndex returns the extended page directory parent index for a given virtual address.
//    N.B. This does not mask off PXE bits.
// Arguments
//    Va - Supplies the virtual address to locate the index for.
// Return Value:
//    The index into the extended page directory parent - ie: the virtual page directory parent number.
//    This is different from the extended page directory parent offset because this spans extended page directory parents on supported platforms.
#define MiGetPxeIndex(va) ((ULONG)((ULONG_PTR)(va) >> PXI_SHIFT))


// ULONG MiGetPpeOffset (IN PVOID va);
// Routine Description:
//    MiGetPpeOffset returns the offset into a page directory parent for a given virtual address.
// Arguments
//    Va - Supplies the virtual address to locate the offset for.
// Return Value:
//    The offset into the parent page directory table the corresponding PPE is at.
#define MiGetPpeOffset(va) ((ULONG)(((ULONG_PTR)(va) >> PPI_SHIFT) & PPI_MASK))


// ULONG MiGetPpeIndex (IN PVOID va);
// Routine Description:
//    MiGetPpeIndex returns the page directory parent index for a given virtual address.
//    N.B. This does not mask off PXE bits.
// Arguments
//    Va - Supplies the virtual address to locate the index for.
// Return Value:
//    The index into the page directory parent - ie: the virtual page directory number.
//    This is different from the page directory parent offset because this spans page directory parents on supported platforms.
#define MiGetPpeIndex(va) ((ULONG)((ULONG_PTR)(va) >> PPI_SHIFT))


// ULONG MiGetPdeOffset (IN PVOID va);
// Routine Description:
//    MiGetPdeOffset returns the offset into a page directory for a given virtual address.
// Arguments
//    Va - Supplies the virtual address to locate the offset for.
// Return Value:
...
// ULONG MiGetPdeIndex (IN PVOID va);
// Routine Description:
//    MiGetPdeIndex returns the page directory index for a given virtual address.
...
//    This is different from the page directory offset because this spans  page directories on supported platforms.
#define MiGetPdeIndex(va) ((ULONG)((ULONG_PTR)(va) >> PDI_SHIFT))


// ULONG MiGetPteOffset (IN PVOID va);
// Routine Description:
//    MiGetPteOffset returns the offset into a page table page for a given virtual address.
// Arguments
//    Va - Supplies the virtual address to locate the offset for.
// Return Value:
...
// PVOID MiGetVirtualAddressMappedByPxe (IN PMMPTE PTE);
// Routine Description:
//    MiGetVirtualAddressMappedByPxe returns the virtual address which is mapped by a given PXE address.
// Arguments
...
// PVOID MiGetVirtualAddressMappedByPpe (IN PMMPTE PTE);
// Routine Description:
//    MiGetVirtualAddressMappedByPpe returns the virtual address which is mapped by a given PPE address.
// Arguments
...
// PVOID MiGetVirtualAddressMappedByPde (IN PMMPTE PTE);
// Routine Description:
//    MiGetVirtualAddressMappedByPde returns the virtual address which is mapped by a given PDE address.
// Arguments
...
// PVOID MiGetVirtualAddressMappedByPte (IN PMMPTE PTE);
// Routine Description:
//    MiGetVirtualAddressMappedByPte returns the virtual address which is mapped by a given PTE address.
// Arguments
...
// LOGICAL MiIsVirtualAddressOnPxeBoundary (IN PVOID VA);
// Routine Description:
//    MiIsVirtualAddressOnPxeBoundary returns TRUE if the virtual address is on an extended page directory parent entry boundary.
// Arguments
...
// LOGICAL MiIsVirtualAddressOnPpeBoundary (IN PVOID VA);
// Routine Description:
//    MiIsVirtualAddressOnPpeBoundary returns TRUE if the virtual address is on a page directory entry boundary.
// Arguments
//    VA - Supplies the virtual address to check.
// Return Value:
//    TRUE if on a boundary, FALSE if not.
...
// LOGICAL MiIsVirtualAddressOnPdeBoundary (IN PVOID VA);
// Routine Description:
//    MiIsVirtualAddressOnPdeBoundary returns TRUE if the virtual address is on a page directory entry boundary.
// Arguments
//    VA - Supplies the virtual address to check.
// Return Value:
...
// LOGICAL MiIsPteOnPpeBoundary (IN PVOID PTE);
// Routine Description:
...
// LOGICAL MiIsPteOnPdeBoundary (IN PVOID PTE);
// Routine Description:
//    MiIsPteOnPdeBoundary returns TRUE if the PTE is on a page directory entry boundary.
// Arguments
//    PTE - Supplies the PTE to check.
// Return Value:
//    TRUE if on a 2MB PDE boundary, FALSE if not.
...
//ULONG GET_PAGING_FILE_NUMBER (IN MMPTE PTE);
// Routine Description:
...
//ULONG GET_PAGING_FILE_OFFSET (IN MMPTE PTE);
// Routine Description:
...
//ULONG IS_PTE_NOT_DEMAND_ZERO (IN PMMPTE PTE);
// Routine Description:
...
// VOID MI_MAKE_PROTECT_WRITE_COPY (IN OUT MMPTE PPTE);
// Routine Description:
...
// VOID MI_SET_PAGE_DIRTY(IN PMMPTE PPTE, IN PVOID VA, IN PVOID PFNHELD);
// Routine Description:
...
// VOID MI_NO_FAULT_FOUND(IN FAULTSTATUS, IN PMMPTE PPTE, IN PVOID VA, IN PVOID PFNHELD);
// Routine Description:
//    This macro handles the case when a page fault is taken and no PTE with the valid bit clear is found.
// Arguments
...
// ULONG MI_CAPTURE_DIRTY_BIT_TO_PFN (IN PMMPTE PPTE, IN PMMPFN PPFN);
// Routine Description:
//    This macro gets captures the state of the dirty bit to the PFN and frees any associated page file space if the PTE has been modified element.
//    NOTE - THE PFN LOCK MUST BE HELD!
// Arguments
//    PPTE - Supplies the PTE to operate upon.
//    PPFN - Supplies a pointer to the PFN database element that corresponds to the page mapped by the PTE.
// Return Value:
//    None.
...
// BOOLEAN MI_IS_PHYSICAL_ADDRESS (IN PVOID VA);
// Routine Description:
//    This macro determines if a given virtual address is really a physical address.
// Arguments
...
// ULONG MI_CONVERT_PHYSICAL_TO_PFN (IN PVOID VA);
// Routine Description:
//    This macro converts a physical address (see MI_IS_PHYSICAL_ADDRESS) to its corresponding physical frame number.
// Arguments
//    VA - Supplies a pointer to the physical address.
// Return Value:
//    Returns the PFN for the page.
#define MI_CONVERT_PHYSICAL_TO_PFN(Va)     ((PFN_NUMBER)(MiGetPdeAddress(Va)->u.Hard.PageFrameNumber) + (MiGetPteOffset((ULONG_PTR)Va)))
...
typedef struct _MMPTE_LIST {
    ...
    // Note the Prototype bit must not be used for lists like freed nonpaged pool because lookaside pops can legitimately reference bogus addresses
    // (since the pop is unsynchronized) and the fault handler must be able to distinguish lists from protos so a retry status can be returned (vs a fatal bugcheck).

    // The same caveat applies to both the Transition and the Protection fields as they are similarly examined in the fault handler and would
...
// VOID MI_WRITE_VALID_PTE (IN PMMPTE PointerPte, IN MMPTE PteContents);
// Routine Description:
//    MI_WRITE_VALID_PTE fills in the specified PTE making it valid with the specified contents.
// Arguments
...
// VOID MI_WRITE_INVALID_PTE (IN PMMPTE PointerPte, IN MMPTE PteContents);
// Routine Description:
//    MI_WRITE_INVALID_PTE fills in the specified PTE making it invalid with the specified contents.
// Arguments
//    PointerPte - Supplies a PTE to fill.
//    PteContents - Supplies the contents to put in the PTE.
...
// VOID MI_WRITE_ZERO_PTE (IN PMMPTE PointerPte);
// Routine Description:
...
// VOID MI_WRITE_VALID_PTE_NEW_PROTECTION (IN PMMPTE PointerPte, IN MMPTE PteContents);
// Routine Description:
//    MI_WRITE_VALID_PTE_NEW_PROTECTION fills in the specified PTE (which was already valid) changing only the protection or the dirty bit.
// Arguments
//    PointerPte - Supplies a PTE to fill.
//    PteContents - Supplies the contents to put in the PTE.
...
// VOID MI_WRITE_VALID_PTE_NEW_PAGE (IN PMMPTE PointerPte, IN MMPTE PteContents);
// Routine Description:
//    MI_WRITE_VALID_PTE_NEW_PAGE fills in the specified PTE (which was already valid) changing the page and the protection.
...
// VOID MiFillMemoryPte (IN PMMPTE Destination, IN ULONG  NumberOfPtes, IN MMPTE  Pattern
//    };
...
#define MiFillMemoryPte(Destination, Length, Pattern)     __stosq((PULONG64)(Destination), Pattern, Length)
#define MiZeroMemoryPte(Destination, Length)              __stosq((PULONG64)(Destination), 0, Length)



__forceinline LOGICAL MI_IS_WRITE_COMBINE_ENABLED (IN PMMPTE PointerPte)
// BOOLEAN MI_IS_WRITE_COMBINE_ENABLED (IN PMMPTE PPTE);
// Routine Description:
//    This macro takes a valid PTE and returns TRUE if write combine is enabled.
// Arguments
//    PPTE - Supplies a pointer to the valid PTE.
// Return Value:
...
{
    if (MiWriteCombiningPtes == TRUE) {
        if ((PointerPte->u.Hard.CacheDisable == 0) && (PointerPte->u.Hard.WriteThrough == 1)) {
            return TRUE;
        }
    } else {
        if ((PointerPte->u.Hard.CacheDisable == 1) && (PointerPte->u.Hard.WriteThrough == 0)) {
            return TRUE;
        }
    }
...
// BOOLEAN MI_IS_PAGE_TABLE_ADDRESS (IN PVOID VA);
// Routine Description:
//    This macro takes a virtual address and determines if it is a page table address.
// Arguments
...
// BOOLEAN MI_IS_PAGE_TABLE_OR_HYPER_ADDRESS (IN PVOID VA);
// Routine Description:
//    This macro takes a virtual address and determines if it is a page table or hyperspace address.
// Arguments
//    VA - Supplies a virtual address.
// Return Value:
...
// BOOLEAN MI_IS_KERNEL_PAGE_TABLE_ADDRESS (IN PVOID VA);
// Routine Description:
//    This macro takes a virtual address and determines if it is a page table address for a kernel address.
// Arguments
//    VA - Supplies a virtual address.
// Return Value:
//    TRUE if the address is a kernel page table address, FALSE if not.
#define MI_IS_KERNEL_PAGE_TABLE_ADDRESS(VA)   ((PVOID)(VA) >= (PVOID)MiGetPteAddress(MM_SYSTEM_RANGE_START) && (PVOID)(VA) <= (PVOID)PTE_TOP)


// BOOLEAN MI_IS_PAGE_DIRECTORY_ADDRESS (IN PVOID VA);
// Routine Description:
//    This macro takes a virtual address and determines if it is a page directory address.
// Arguments
//    VA - Supplies a virtual address.
// Return Value:
...
// BOOLEAN MI_IS_HYPER_SPACE_ADDRESS (IN PVOID VA);
// Routine Description:
//    This macro takes a virtual address and determines if it is a hyper space address.
// Arguments
//    VA - Supplies a virtual address.
// Return Value:
...
// BOOLEAN MI_IS_PROCESS_SPACE_ADDRESS (IN PVOID VA);
// Routine Description:
//    This macro takes a virtual address and determines if it is a process-specific address.
//    This is an address in user space or page table pages or hyper space.
// Arguments
//    VA - Supplies a virtual address.
// Return Value:
...
// BOOLEAN MI_IS_PTE_PROTOTYPE (IN PMMPTE PTE);
// Routine Description:
...
// BOOLEAN MI_IS_SYSTEM_CACHE_ADDRESS (IN PVOID VA);
// Routine Description:
//    This macro takes a virtual address and determines if it is a system cache address.
// Arguments
//    VA - Supplies a virtual address.
// Return Value:
...
// VOID MI_BARRIER_SYNCHRONIZE (IN ULONG TimeStamp);
// Routine Description:
//    MI_BARRIER_SYNCHRONIZE compares the argument timestamp against the current IPI barrier sequence stamp.
//    When equal, all processors will issue memory barriers to ensure that newly created pages remain coherent.

//    When a page is put in the zeroed or free page list the current barrier sequence stamp is read (interlocked - this is necessary to get the correct value - memory barriers won't do the trick) and stored in the pfn entry for the page.
//    The current barrier sequence stamp is maintained by the IPI send logic and is incremented (interlocked) when the target set of an IPI send includes all processors, but the one doing the send.
//    When a page is needed its sequence number is compared against the current barrier sequence number.
//    If it is equal, then the contents of the page may not be coherent on all processors, and an IPI must be sent to all processors to ensure a memory barrier is executed (generic call can be used for this).
//    Sending the IPI automatically updates the barrier sequence number.
//    The compare is for equality as this is the only value that requires the IPI (i.e., the sequence number wraps, values in both directions are older).
//    When a page is removed in this fashion and either found to be coherent or made coherent, it cannot be modified between that time and writing the PTE.
//    If the page is modified between these times, then an IPI must be sent.
// Arguments
//    TimeStamp - Supplies the timestamp at the time when the page was zeroed.
#define MI_BARRIER_SYNCHRONIZE(TimeStamp)// does nothing on AMD64.


// VOID MI_BARRIER_STAMP_ZEROED_PAGE (IN PULONG PointerTimeStamp);
// Routine Description:
//    MI_BARRIER_STAMP_ZEROED_PAGE issues an interlocked read to get the current IPI barrier sequence stamp.
//    This is called AFTER a page is zeroed.
// Arguments
//    PointerTimeStamp - Supplies a timestamp pointer to fill with the current IPI barrier sequence stamp.
#define MI_BARRIER_STAMP_ZEROED_PAGE(PointerTimeStamp)// does nothing on AMD64.


// LOGICAL MI_RESERVED_BITS_CANONICAL (IN PVOID VirtualAddress);
// Routine Description:
//    This routine checks whether all of the reserved bits are correct.
//    The processor implements at 48 bits of VA and memory management uses them all so the VA is checked against 48 bits to prevent reserved bit faults as our caller is not going to be expecting them.
// Arguments
//    VirtualAddress - Supplies the virtual address to check.
// Return Value:
//    TRUE if the address is ok, FALSE if not.
LOGICAL __inline MI_RESERVED_BITS_CANONICAL (IN PVOID VirtualAddress)
{
    // Bits 48-63 of the address must match, i.e., they must be all zeros or all ones.
    return ((ULONG64)(((LONG64)VirtualAddress >> 48) + 1) <= 1);
}


// VOID MI_DISPLAY_TRAP_INFORMATION (IN PVOID TrapInformation);
// Routine Description:
...
VOID MiGetStackPointer (OUT PULONG_PTR StackPointer);