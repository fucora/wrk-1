...
VOID MiMemorySan(IN PLOADER_PARAMETER_BLOCK LoaderBlock);
...
// A temporary unsignaled event for the available pages & pool events to point at early during phase 0.
// This is because the exportable events with DACLs cannot be created until phase 1.
...
// For convenience the number of pages is calculated based on a 4k page size.  Hence 12mb with 4k page is 3072.
...
// Architecture-dependent initialization may re-point it to a HAL routine for NUMA configurations.
...
/*
Routine Description:
    This routine is called during initial freelist population or when physical memory is being hot-added.
    It then determines which node (in a multinode NUMA system) the physical memory resides in, and marks the PFN entry accordingly.
...
*/
{
    ...
    if (Phase == 0) {
        ...
        // Point various event thresholds at a temp unsignaled event since real event creation is not available until phase 1.
...
#define MM_MB_MAPPED_BY_PDE (MM_VA_MAPPED_BY_PDE / (1024*1024))
        ...
        if (MmSystemViewSize < (MM_VA_MAPPED_BY_PDE / (1024 * 1024))) {
            ...
            if (MmVirtualBias == 0) {
                ...
                // NT64 locates system views just after systemwide paged pool, so the size of the system views are not limited by session space.
                // Arbitrarily make the maximum a PPE's worth.

                // NT32 shares system view VA space with session VA space due to the shortage of virtual addresses.
                // Thus increasing the system view size means potentially decreasing the maximum session space size.
                SystemViewMax = (MI_SESSION_SPACE_MAXIMUM_TOTAL_SIZE) / (1024 * 1024);

#if !defined(_WIN64)
                // Ensure at least enough space is left for the standard default session layout.
                SystemViewMax -= (MmSessionSize / (1024 * 1024));
#endif

                // Note a view size of -1 will be rounded to zero.  Treat -1 as requesting the maximum.
        ...
        if (MmVirtualBias == 0) {
            if (MmSessionImageSize < MM_MB_MAPPED_BY_PDE) {
                ...
                // The Hydra image size has been specified (in megabytes) by the registry.
            ...
            // Now initialize the session pool and view ranges which lie virtually below it.
            if (MmSessionViewSize < MM_MB_MAPPED_BY_PDE) {
                MmSessionViewSize = MI_SESSION_DEFAULT_VIEW_SIZE;
            } else {
                // The Hydra view size has been specified (in megabytes) by the registry.  Validate it.
            ...
            // Now initialize the session pool start and size which lies virtually just below it.
            MiSessionPoolEnd = MiSessionViewStart;
            if (MmSessionPoolSize < MM_MB_MAPPED_BY_PDE) {
#if !defined(_WIN64)
                // Professional and below use systemwide paged pool for session allocations (this decision is made in win32k.sys).
                // Server and above use real session pool and 16mb isn't enough to play high end game applications, etc.
                // Since we're not booted /3GB, try for an additional 16mb now.
                ...
                // The Hydra pool size has been specified (in megabytes) by the registry.  Validate it.
...
#if defined (_WIN64)
            // Session special pool immediately follows session regular pool assuming the user has enabled either the verifier or special pool.
            ...
            // When booted /3GB, no size overrides are allowed due to the already severely limited virtual address space.
...
#if defined(_X86PAE_)
            MmSystemCacheStart = (PVOID)((ULONG_PTR)MmSystemCacheStart + MM_SYSTEM_CACHE_WORKING_SET_3GB_DELTA);
#endif
        }

        // Set the highest section base address.

        // N.B. In 32-bit systems this address must be 2gb or less even for systems that run with 3gb enabled.
        //      Otherwise, it would not be possible to map based sections identically in all processes.
        ...
        if (MmSizeOfPagedPoolInBytes == (SIZE_T)-1) {
            // The registry indicates that paged pool should be enlarged to the maximum possible.
            // Steal from the system cache if possible to accomplish this.
            if (MmVirtualBias == 0) {
        ...
        // If boot.ini specified a sane number of MB that the administrator wants to use for user virtual address space then use it.
        UserVaLimit = 0;
        ...
        if (MmVirtualBias != 0) {
            if (MmNumberOfSystemPtes == (ULONG)-1) {
                ...
                // Since we are booted /3GB, take some system cache VA space for this.
                MiRequestedSystemPtes = (ULONG)MmNumberOfSystemPtes;
                MmNumberOfSystemPtes = MM_MAXIMUM_SYSTEM_PTES;
                ReduceSystemCacheForPtes = TRUE;
            }

            // If the size of the boot image (likely due to a large registry) overflows into where paged pool would normally start, then move paged pool up now.
            // This costs virtual address space (ie: performance) but more importantly, allows the system to boot.
            ...
            // The system has been biased to an alternate base address to allow 3gb of user address space, set the user probe address and the maximum system cache size.
            if ((UserVaLimit > 2048) && (UserVaLimit < 3072)) {
                // Use any space between the maximum user virtual address and the system for extra system PTEs.

                // Convert input MB to bytes.
                UserVaLimit -= 2048;
                UserVaLimit *= (1024 * 1024);

                // Don't let the user specify a value which would cause us to prematurely overwrite portions of the kernel & loader block.
            ...
            MmSessionBase = (ULONG_PTR)((ULONG_PTR)MmSystemCacheStart + (MaximumSystemCacheSize << PAGE_SHIFT));
            ...
            // Note there is no protection against users that try to use the USERVA option to reduce user space below 2GB.
            UserVaLimit *= (1024 * 1024);
        ...
        // Some checks to make sure prototype PTEs can be placed in either paged or nonpaged (prototype PTEs for paged pool are here) can be put into PTE format.
        ...
        // ... except these...
        // If you change this list, please make the corresponding changes over in boot\lib\blmemory.c!BlDetermineOSVisibleMemory()
...
#if defined (_WIN64)
            // 64-bit NT is not constrained by virtual address space - but there is physical cost for the preallocated page table pages used by system PTEs.
            // No other tradeoffs between nonpaged pool, paged pool and system PTEs need to be made.
            // So just allocate PTEs on a linear scale as a function of the amount of RAM.
...
#if defined(_X86_)
        // The allocation preference key must be carefully managed.
        // This is because doing every allocation top-down can cause failures if an ntdll process startup allocation 
        // (like the stack trace database) gets a high address which then causes a subsequent system DLL rebase collision.

        // This is circumvented as follows:
        // 1.  For 32-bit machines, the allocation preference key is only useful when booted /3GB as only then can this key help track down apps with high virtual address bit sign extension problems.
        //     In 3GB mode, the system DLLs are based just below 2GB so ntdll would have to allocate more than 1GB of VA space before this becomes a problem.
        //     So really the problem can only occur for machines in 2GB mode and since the key doesn't help these machines anyway, just turn it off in these cases.
        // 2.  For 64-bit machines, there is plenty of VA space above the addresses system DLLs are based at so it is a non-issue.
        //     EXCEPT for wow64 binaries which run in sandboxed 2GB address spaces.
        //     Explicit checks are made to detect a wow64 process in
        //     the Mm APIs which check this key and the key is ignored in this case as it doesn't provide any sign extension help and therefore we don't allow it to burn up any valuable VA space which could cause a collision.
        if (MmVirtualBias == 0) {
        ...
        // Set the initial commit page limit high enough so initial pool allocations (which happen in the machine dependent init) can succeed.
        MmTotalCommitLimit = _2gb / PAGE_SIZE;
        MmTotalCommitLimitMaximum = MmTotalCommitLimit;

        // Pick a reasonable size for the default prototype PTE allocation chunk size.
        // Make sure it's always a PAGE_SIZE multiple.
        // The registry entry is treated as the number of 1K chunks.
        if (MmAllocationFragment == 0) {
            ...
            // Then round it to a PAGE_SIZE multiple.
            // Finally bound it reasonably.
        ...
        // Initialize listhead, spinlock and semaphore for segment dereferencing thread.
...
#if defined(_X86_)
        // Virtual bias indicates the offset that needs to be added to 0x80000000 to get to the start of the loaded images.
        // Update it now to indicate the offset to MmSessionBase as that is the lowest system address that process creation needs to make sure to duplicate.

        // This is not done until after machine dependent initialization runs as that initialization relies on the original meaning of VirtualBias.

        // Note if the system is booted with both /3GB & /USERVA, then system PTEs will be allocated below virtual 3GB and that will end up being the lowest system address the process creation needs to duplicate.
        if (MmVirtualBias != 0) {
            MmVirtualBias = (ULONG_PTR)MmSessionBase - CODE_START;
        }
#endif

        // Create the bitmap which represents valid memory.
        // Note the largest possible size is used because (unlike the MmPhysicalMemoryBlock) we
        // don't want to free and reallocate this bitmap during hotadds because we want callers to be able to reference it lock free.
        ASSERT(MmHighestPossiblePhysicalPage + 1 < _4gb);
        Bitmap = ExAllocatePoolWithTag(NonPagedPool, (((MmHighestPossiblePhysicalPage + 1) + 31) / 32) * 4, '  mM');
        if (Bitmap == NULL) {
            KeBugCheckEx(INSTALL_MORE_MEMORY, MmNumberOfPhysicalPages, MmLowestPhysicalPage, MmHighestPhysicalPage, 0x101);
        }

        RtlInitializeBitMap(&MiPfnBitMap, Bitmap, (ULONG)(MmHighestPossiblePhysicalPage + 1));
        ...
        if ((AutosizingFragment == TRUE) &&
            ...
            // This is a system with at least 1GB of RAM.
            // Presumably it will be used to cache many files.
            // Maybe we should factor in pool size here and adjust it accordingly.
            MmAllocationFragment;
        }
#endif

        // Temporarily initialize resident available pages so large page allocations can succeed if the memory exists.
        MmResidentAvailablePages = MmAvailablePages - MM_FLUID_PHYSICAL_PAGES;
        MiInitializeLargePageSupport();
        MiInitializeDriverLargePageList();

        // Relocate all the drivers so they can be paged (and protected) on a per-page basis.
        ...
        if (MmNumberOfPhysicalPages >= ((32 * 1024 * 1024) / PAGE_SIZE)) {
            if (MmProductType == 0x00690057) {// If we are on a workstation, 32Mb and above are considered large systems.
                MmSystemSize = MmLargeSystem;
            } else {// For servers, 64Mb and greater is a large system
        ...
        if (NT_SUCCESS(MmIsVerifierEnabled(&VerifierFlags))) {
            // The verifier is enabled so don't defer any MDL unlocks because without state, debugging driver bugs in this area is very difficult.
        ...
        if (MmProductType == 0x00690057) {// Determine if we are on an AS system (Winnt is not AS).
        ...
        // Subtract off the size of future nonpaged pool expansion so that nonpaged pool will always be able to expand regardless of prior system load activity.
...
#if DBG
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "system cache working set too big\n");
#endif
            return FALSE;
        }
        ...
        if (StartPxe->u.Hard.Valid == 0) {
            // Map in a page directory parent page for the system cache working set.  Note that we only populate one page table for this.
            LOCK_PFN(OldIrql);
            DirectoryFrameIndex = MiRemoveZeroPage(MI_GET_PAGE_COLOR_FROM_PTE(StartPxe));
...
#if (_MI_PAGING_LEVELS >= 4)
        // The shared user data is already initialized and it shares the page table page with the system cache working set list.
        ASSERT(StartPde->u.Hard.Valid == 1);
#else
        // Map in a page table page.
        ASSERT(StartPde->u.Hard.Valid == 0);
        LOCK_PFN(OldIrql);
        PageFrameIndex = MiRemoveZeroPage(MI_GET_PAGE_COLOR_FROM_PTE(StartPde));
...
#if defined(_X86_)
        // Make the system cache virtual size the maximum since 32 bit address space limits us to far less than we'd want anyway.
        ...
        if (MiMaximumSystemCacheSizeExtra != 0) {
            ...
            MiSystemCacheEndExtra = (PVOID)(((PCHAR)MiSystemCacheStartExtra + (MiMaximumSystemCacheSizeExtra)* PAGE_SIZE) - 1);
        } else {
            MiSystemCacheStartExtra = MmSystemCacheStart;
            MiSystemCacheEndExtra = MmSystemCacheEnd;
        }
#else
        // For NT64, size the system cache based on the amount of physical memory so we don't go too far.
        // Note there is still cost to for the preallocated page table pages so factor that in as well (for smaller memory systems).
        MmSizeOfSystemCacheInPages = (PFN_COUNT)((128 * 1024 * 1024) >> PAGE_SHIFT);
        i = (MmNumberOfPhysicalPages + 65) / 1024;
        if (i > 4) {
            // System has at least 4032 pages.
            // Make the system cache 128mb + 16mb for each additional 1024 pages.
            MmSizeOfSystemCacheInPages += ((i - 4) * ((16 * 1024 * 1024) >> PAGE_SHIFT));
        ...
        if (ReduceSystemCacheForPtes == TRUE) {
            ...
            // Increment the system PTEs (for autoconfiguration purposes) but don't actually add the PTEs till later (to prevent fragmentation).
        ...
        MiInitializeSystemCache((ULONG)MmSystemCacheWsMinimum, (ULONG)MmAvailablePages);
        ...
        // Now that we have booted far enough, replace the temporary commit limits with real ones: set the initial commit page limit to the number of available pages.
        // This value is updated as paging files are created.
        ...
        // Set maximum working set size to 512 pages less than the total available memory.
        ...
        KeInitializeEvent(&MmModifiedPageWriterEvent, NotificationEvent, FALSE);// Create the modified page writer event.
        MiBuildPagedPool();// Build paged pool.

        // Initialize the loaded module list.
        // This cannot be done until paged pool has been built.
        if (MiInitializeLoadedModuleList(LoaderBlock) == FALSE) {
#if DBG
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "Loaded module list initialization failed\n");
#endif
            return FALSE;
        }

        // Initialize the handle for the PAGELK section now that all drivers have been relocated to their final resting place and the loaded module list has been initialized.
        ExPageLockHandle = MmLockPageableCodeSection((PVOID)(ULONG_PTR)MmShutdownSystem);
        MmUnlockPageableImageSection(ExPageLockHandle);

        // Initialize the unused segment threshold.
        // Attempt to keep pool usage below this percentage (by trimming the cache) if pool requests can fail.
        ...
        // Snap a copy of the initial page directory so that when large page system PTE mappings are deleted the proper values can be restored.
        MiInitialSystemPageDirectory = ExAllocatePoolWithTag(NonPagedPool, PD_PER_SYSTEM * PAGE_SIZE, 'dPmM');
        if (MiInitialSystemPageDirectory == NULL) {
#if DBG
            DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "can't snap system page directory\n");
#endif
            return FALSE;
        }

        RtlCopyMemory(MiInitialSystemPageDirectory, (PVOID)PDE_BASE, PD_PER_SYSTEM * PAGE_SIZE);
#endif
    ...
    if (Phase == 1) {
        ...
        // The PTE is deliberately allocated from paged pool so that it will always have a PTE itself instead of being superpaged.
        // This way, checks throughout the fault handler can assume that the PTE can be checked without having to special case this.
        ...
        MI_MAKE_VALID_KERNEL_PTE(TempPte, PageFrameIndex, MM_READONLY, PointerPte);
        *MmSharedUserDataPte = TempPte;
        ...
        if (MmHighestUserAddress < (PVOID)MM_SHARED_USER_DATA_VA) {
            // Install the PTE mapping now as faults will not because the shared user data is in the system portion of the address space.
            // Note the pagetable page has already been allocated and locked down.
        ...
        // Initialize the low and high memory events.  This must be done before starting the working set manager.
        ...
        for (; NextEntry != &PsLoadedModuleList; NextEntry = NextEntry->Flink) {
            ...
            if ((NtHeaders != NULL) && (NtHeaders->OptionalHeader.MajorOperatingSystemVersion >= 5) && (NtHeaders->OptionalHeader.MajorImageVersion >= 5)) {
...
*/
{
    ...
    if (ARGUMENT_PRESENT(InputMemory)) {
        ...
        // The caller wants us to allocate the return result buffer.
        // Size it by allocating the maximum possibly needed as this should not be very big (relatively).
        // It is the caller's responsibility to free this.
        // Obviously this option can only be requested after pool has been initialized.
        NextMd = LoaderBlock->MemoryDescriptorListHead.Flink;
        while (NextMd != &LoaderBlock->MemoryDescriptorListHead) {
            InitialAllocation += 1;
            MemoryDescriptor = CONTAINING_RECORD(NextMd, MEMORY_ALLOCATION_DESCRIPTOR, ListEntry);
    ...
    if (i == 0) {
        // Don't bother shrinking this as the caller will be freeing it shortly as it is just an empty list.
        Memory->Run[i].BasePage = 0;
        Memory->Run[i].PageCount = 0;
    } else if (!ARGUMENT_PRESENT(InputMemory)) {// Shrink the buffer (if possible) now that the final size is known.
...
VOID MmFreeBootRegistry(VOID)
{
    ...
    // Since systemwide commitment was determined early in Phase 0 and excluded the ranges just freed, add them back in now.
        ...
        for (Runs = MiBootRegistryRuns; Runs->BasePage != 0; Runs += 1) {
            ...
            while (NumberOfPages != 0) {
                if (MI_IS_PHYSICAL_ADDRESS(MiGetVirtualAddressMappedByPte(PointerPte))) {
                    ...
                    PagesFreed += MiDeleteSystemPageableVm(PointerPte, 1, MI_DELETE_FLUSH_TB, NULL);
                }
...
#if defined(_X86_)
            // LWFIX - pass bigger sizes to MiAddExtraSystemPtes, then rip out MmKeepBootRegistry (keep 0x1 behavior only)
#endif
        }
...
*/
{
    ...
    if ((MmKeepBootRegistry & 0x1) && (RegistryDescriptors != 0)) {
        MiBootRegistryRuns = ExAllocatePoolWithTag(NonPagedPool, sizeof(REGISTRY_MEMORY_RUN) * (RegistryDescriptors + 1), 'lMmM');
        RegistryRuns = MiBootRegistryRuns;
        if (MiBootRegistryRuns != NULL) {
            // Zero this allocation (and its NULL terminator) so it can be walked later when the registry finally releases it.
            RtlZeroMemory(RegistryRuns, sizeof(REGISTRY_MEMORY_RUN) * (RegistryDescriptors + 1));

            // Walk through the memory descriptors and make the boot-loaded registry ranges pageable.
            NextMd = LoaderBlock->MemoryDescriptorListHead.Flink;
            while (NextMd != &LoaderBlock->MemoryDescriptorListHead) {
                MemoryDescriptor = CONTAINING_RECORD(NextMd, MEMORY_ALLOCATION_DESCRIPTOR, ListEntry);
                if ((MemoryDescriptor->MemoryType == LoaderRegistryData) && (MemoryDescriptor->PageCount != 0)) {
            ...
            do {
                ...
                if (MiNoLowMemory != 0) {
                    if (NextPhysicalPage < MiNoLowMemory) {
                        // Don't make this run pageable as it is below the memory threshold configured for this system.
                ...
                // Stash the starting PTE so the range can be easily released later.
                ...
                while (i != 0) {
                    ...
                    if (Pfn1->PteAddress != ExpectedPte) {
                        // The PTE for this PFN is not contiguous with the prior one so do not attempt to reclaim the VA space or physical pages later.
                        RegistryRuns->PageCount = 0;
                    }

                    // Make the page pageable and put the PTE into transition.
                    // If it is a PFN without a PTE then leave it as nonpaged until MmFreeBootRegistry is called.
    ...
    while (NextMd != &LoaderBlock->MemoryDescriptorListHead) {
        ...
        case LoaderNlsData:
            // Capture the data to temporary storage so we won't free memory we are referencing.
    ...
    if (Runs != RunBase) {
        ...
        do {
            ...
            if (MiNoLowMemory != 0) {
                if (NextPhysicalPage < MiNoLowMemory) {
                    // Don't free this run as it is below the memory threshold configured for this system.
                    Runs -= 1;
                    continue;
                }
            }
            ...
            while (i != 0) {
                if ((MmKeepBootRegistry & 0x1) && (Pfn1->u2.ShareCount > 1)) {
                    // Note LoaderOsloaderHeap is used to describe page table pages used to map the registry so care must be taken not to blindly free those.
                    // Distinguish them via the share count check above.
                    NOTHING;
                    ...
                    if (NextPhysicalPage != 0) {
                        // Remove PTE and insert into the free list.
                        // If it is a physical address within the PFN database, the PTE element does not exist and therefore cannot be updated.
...
#if defined(_X86_)
    if (MmVirtualBias != 0) {
        // If the kernel has been biased to allow for 3gb of user address space, the boot images are doubly mapped at KSEG0_BASE and at ALTERNATE_BASE.
        // Therefore, the KSEG0_BASE entries must be unmapped.
        PMMPTE Pde;
        ULONG NumberOfPdes;
    ...
    // This virtual address space is being deleted and the system cannot be accessing it (or it would be a serious bug) - thus it is safe to flush the TB after releasing the PFN lock.
...
*/
{
...
#if defined (_WIN64)
    // NT64 places system mapped views directly after paged pool.
    // Ensure enough VA space is available.
    ...
    // If paged pool is really nonpageable then limit the size based on how much physical memory is actually present.
    // Disable this feature if not enough physical memory is present to do it.
    if (MmDisablePagingExecutive & MM_PAGED_POOL_LOCKED_DOWN) {
        Size = MmSizeOfPagedPoolInBytes / PAGE_SIZE;
        if ((MI_NONPAGEABLE_MEMORY_AVAILABLE() < 2048) || (MmAvailablePages < 2048)) {
            Size = 0;
        } else {
    ...
    MiZeroMemoryPte(PointerPde, (1 + MiGetPdeAddress(MmPagedPoolEnd) - PointerPde));
    ...
    // Since the commitment return path is lock free, the total committed page count must be atomically incremented.
...
/*
Routine Description:
    This function is called to initialize the default nonpaged pool signaling thresholds.
Environment:
    Kernel Mode Only.  System initialization.
*/
{
...
/*
Routine Description:
    This function locates the start and end of the initialization code for each loaded module list entry.  This code resides in the INIT section of each image.
Arguments:
...
*/
{
    ...
    *StartVa = NULL;
    ...
    // Acquire the load lock to ensure that we don't slice into a load in progress (ie: fixups on INIT code may be ongoing) on a driver already in the list.
    ...
    while (Next != &PsLoadedModuleList) {
        ...
        if (LdrDataTableEntry->Flags & LDRP_MM_LOADED) {// This entry was loaded by MmLoadSystemImage so it's already had its init section removed.
            Next = Next->Flink;
            continue;
        }
        ...
        // From the image header, locate the sections named 'INIT', PAGEVRF* and PAGESPEC.
        // INIT always goes, the others go depending on registry configuration.
        ...
        while (i > 0) {
            ...
            if ((*(PULONG)SectionTableEntry->Name == 'TINI') || ((SectionTableEntry->Characteristics & IMAGE_SCN_MEM_DISCARDABLE) != 0)) {
                ...
                if ((SectionTableEntry->Name[4] == 'V') && (SectionTableEntry->Name[5] == 'R') && (SectionTableEntry->Name[6] == 'F')) {
            ...
            if (DiscardSection == TRUE) {
                ...
                if (i == 1) {
                    ...
                    do {
                        ...
                        if ((SectionTableEntry->Characteristics & IMAGE_SCN_MEM_DISCARDABLE) != 0) {
                            LastDiscard = SectionTableEntry;// Discard this too.
                        } else {
                            break;
                        }
                    } while (i > 1);
                ...
                if (LastDiscard) {
                    ...
                    InitEnd = (PVOID)(((PCHAR)CurrentBase + LastDiscard->VirtualAddress) + (Span - 1));

                    // If this isn't the last section in the driver then the
                    // the next section is not discardable.
                    // So the last section is not rounded down, but all others must be.
                    if (i != 1) {
                        InitEnd = (PVOID)((PCHAR)PAGE_ALIGN((PCHAR)InitEnd + (NtHeader->OptionalHeader.SectionAlignment - 1)) - 1);
                    }
                }
                ...
                if (InitStart <= InitEnd) {
                    if ((MiFindInitializationCodeAddress >= InitStart) && (MiFindInitializationCodeAddress <= InitEnd)) {
                        ...
                        *EndVa = InitEnd;
                        ...
                        // There would also be the issue of the frames getting given out and mapped with a conflicting TB attribute as well ...

                        // Make a special exception for the kernel & HAL since those never unload.
                        if (MI_IS_PHYSICAL_ADDRESS(InitStart)) {
                            ...
                            if (!RtlEqualUnicodeString(&NameString, &LdrDataTableEntry->BaseDllName, TRUE)) {
...
/*
Routine Description:
    This function is called to delete the initialization code for each loaded module list entry.
Arguments:
...
*/
{
...
#if defined (_MI_MORE_THAN_4GB_)
    if (MiNoLowMemory != 0) {
        // Don't free this range as the kernel is always below the memory threshold configured for this system.
        return;
    }
#endif
    ...
    if (MI_IS_PHYSICAL_ADDRESS(StartVa)) {
        // Don't put these frames on the general purpose freelist because they lie inside a range being mapped with a large page (or otherwise locked down TB entry).
        // Thus we must ensure that these are always mapped cached to avoid TB attribute conflict problems.
        PageFrameIndex = MI_CONVERT_PHYSICAL_TO_PFN(StartVa);
        MiAddExpansionNonPagedPool(PageFrameIndex, PagesFreed, FALSE);
    } else {
        PagesFreed = MiDeleteSystemPageableVm(PointerPte, PagesFreed, MI_DELETE_FLUSH_TB, NULL);
    }

    if (PagesFreed != 0) {
        MiReturnCommitment(PagesFreed);
        MI_INCREMENT_RESIDENT_AVAILABLE(PagesFreed, MM_RESAVAIL_FREE_UNLOAD_SYSTEM_IMAGE1);
    }
}
...
/*
Routine Description:
    This function locates the start and end of the pageable code for each loaded module entry.
    This code resides in the PAGE section of each image.
Environment:
    Kernel Mode Only.  End of system initialization.
*/
{
    ...
    CurrentThread = PsGetCurrentThread();// Walk through the loader blocks looking for the base which contains this routine.
    ...
    while (Next != &PsLoadedModuleList) {
        LdrDataTableEntry = CONTAINING_RECORD(Next, KLDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
        if (LdrDataTableEntry->SectionPointer != NULL) {// This entry was loaded by MmLoadSystemImage so it's already paged.
            Next = Next->Flink;
            continue;
        }

        CurrentBase = (PVOID)LdrDataTableEntry->DllBase;
        if ((MI_IS_PHYSICAL_ADDRESS(CurrentBase)) || (MI_PDE_MAPS_LARGE_PAGE(MiGetPdeAddress(CurrentBase)))) {// Mapped physically, can't be paged.
            Next = Next->Flink;
            continue;
        }
    ...
    restart:
        ...
        while (i > 0) {
            ...
            if ((PUCHAR)SectionBaseAddress == ((PUCHAR)CurrentBase + SectionTableEntry->VirtualAddress)) {
                ...
                // This subsection has already been locked down (and possibly unlocked as well) at least once.
                // If it is NOT locked down right now and the pages are not in the system working set then include it in the chunk to be paged.
                ...
                if (*SectionLockCountPointer == 0) {
                    ...
                    SubsectionLastPte = MiGetPteAddress
                    ((PVOID)((ULONG_PTR)CurrentBase + SectionTableEntry->VirtualAddress + (NtHeader->OptionalHeader.SectionAlignment - 1) + Span - PAGE_SIZE));
            ...
            if (*(PULONG)SectionTableEntry->Name == 'EGAP') {
                if ((SectionTableEntry->Name[4] == 'K') && (SectionTableEntry->Name[5] == 'D')) {
                    PageSection = KdPitchDebugger;// Only pageout PAGEKD if KdPitchDebugger is TRUE.
                } else if ((SectionTableEntry->Name[4] == 'V') && (SectionTableEntry->Name[5] == 'R') && (SectionTableEntry->Name[6] == 'F')) {
                    if (MmVerifyDriverBufferLength != (ULONG)-1) {// Pageout PAGEVRF* if no drivers are being instrumented.
                        PageSection = FALSE;
                    }
                } else if (*(PULONG)&SectionTableEntry->Name[4] == 'CEPS') {
                    if (MiSpecialPoolFirstPte != NULL) {// Pageout PAGESPEC special pool code if it's not enabled.
                        PageSection = FALSE;
                    }
                }
            }
            ...
            if (PageSection) {
                if (PointerPte == NULL) {// This section is pageable, save away the start and end.
                    ...
                    PointerPte = MiGetPteAddress((PVOID)(ROUND_TO_PAGES((ULONG_PTR)CurrentBase + SectionTableEntry->VirtualAddress)));
                }
                ...
                LastPte = MiGetPteAddress((PVOID)((ULONG_PTR)CurrentBase + SectionTableEntry->VirtualAddress + (NtHeader->OptionalHeader.SectionAlignment - 1) + Span - PAGE_SIZE));
            } else {// This section is not pageable, if the previous section was pageable, enable it.
                if (PointerPte != NULL) {
                    ASSERT(StartSectionTableEntry != NULL);
                    LOCK_SYSTEM_WS(CurrentThread);
                    LOCK_PFN(OldIrql);
                    ...
                    // Now that we're holding the proper locks, rewalk all the sections to make sure they weren't locked down after we checked above.
                    while (StartSectionTableEntry < SectionTableEntry) {
                        ...
                        if (((PUCHAR)SectionBaseAddress == ((PUCHAR)CurrentBase + StartSectionTableEntry->VirtualAddress)) && (*SectionLockCountPointer != 0)) {
                            // Restart at the top as the section has been explicitly locked by a driver since we first checked above.
                            UNLOCK_PFN(OldIrql);
                            UNLOCK_SYSTEM_WS(CurrentThread);
                            goto restart;
                        }
...
*/
{
    ...
    while (PointerPte <= LastPte) {
        // The PTE must be carefully checked as drivers may call MmPageEntire during their DriverEntry yet faults may occur prior to this routine running which cause pages to already be resident and in the working set at this point.
        // So checks for validity and wsindex must be applied.
        if (PointerPte->u.Hard.Valid == 1) {
            ...
            if (Pfn->u1.WsIndex == 0) {
                ...
                // Flush the TB and decrement the number of valid PTEs within the containing page table page.
...
LOGICAL MiInitializeMemoryEvents(VOID)
{
    ...
    // The thresholds may be set in the registry, if so, they are interpreted in megabytes so convert them to pages now.

    // If the user modifies the registry to introduce his own values, don't
    // bother error checking them as they can't hurt the system regardless (bad values just may result in events not getting signaled or staying signaled when they shouldn't, but that's not fatal).
    if (MmLowMemoryThreshold != 0) {
        MmLowMemoryThreshold *= ((1024 * 1024) / PAGE_SIZE);
    } else {
        // Scale the threshold so on servers the low threshold is approximately 32MB per 4GB, capping it at 64MB.
...
#if DBG
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_ERROR_LEVEL, "MM: Memory event initialization failed %x\n", Status);
#endif
        return FALSE;
    }
    ...
    // Initialize the pool threshold events based on the current system values.
...
NTSTATUS MiCreateMemoryEvent(IN PUNICODE_STRING EventName, OUT PKEVENT *Event)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes, EventName, OBJ_KERNEL_HANDLE | OBJ_PERMANENT, NULL, &SecurityDescriptor);
    Status = ZwCreateEvent(&EventHandle, EVENT_ALL_ACCESS, &ObjectAttributes, NotificationEvent, FALSE);
    ExFreePool(Dacl);
    if (NT_SUCCESS(Status)) {
        Status = ObReferenceObjectByHandle(EventHandle, EVENT_MODIFY_STATE, ExEventObjectType, KernelMode, (PVOID *)Event, NULL);
    }
...
VOID MiInitializeCacheOverrides(VOID)
{
    ...
    // It would be better if the platform returned what the new cachetype should be.
    if (Information.PlatformFlags & HAL_PLATFORM_DISABLE_UC_MAIN_MEMORY) {
        // The attribute table's noncached entries must be overridden so they are equivalent to cached (for main system memory) since that's all the underlying hardware supports.
    ...
    if (Information.PlatformFlags & HAL_PLATFORM_DISABLE_WRITE_COMBINING) {
        // The attribute table's writecombine entries must be overridden so they are equivalent to cached (for main system memory) since that's all the underlying hardware supports.
...
/*
Routine Description:
    This function scans the page directory and page tables for HAL I/O space mappings so they can be added to the page attribute table (to prevent any subsequent mappings from using a conflicting attribute).
    This also lets the debugger automatically apply the correct attribute so !dd on any of these ranges just works.
Environment:
...
*/
{
    ...
    for (i = 0; i < PdeCount; i += 1) {
        if ((PointerPde->u.Hard.Valid == 1) && (PointerPde->u.Hard.LargePage == 0)) {
            ...
            for (j = 0; j < PTE_PER_PAGE; j += 1) {
                ...
                if (PteContents.u.Hard.Valid == 1) {
                    ...
                    if (!MI_IS_PFN(PageFrameIndex)) {
                        ...
                        if ((MI_IS_CACHING_DISABLED(&PteContents)) && (PteContents.u.Hard.WriteThrough == 1)) {
                            CacheAttribute = MiNonCached;
                        } else if ((MiWriteCombiningPtes == TRUE) && (PteContents.u.Hard.CacheDisable == 0) && (PteContents.u.Hard.WriteThrough == 1)) {
                            CacheAttribute = MiWriteCombined;
                        } else if ((MiWriteCombiningPtes == FALSE) && (PteContents.u.Hard.CacheDisable == 1) && (PteContents.u.Hard.WriteThrough == 0)) {
                            CacheAttribute = MiWriteCombined;
                        }

                        MiInsertIoSpaceMap(VirtualAddress, PageFrameIndex, 1, CacheAttribute);
                    }
                }
...
__declspec(noinline) PVOID MiGetInstructionPointer(VOID)
{
