...
Abstract:
    This module contains the Mm support routines for the cache manager to prefetching groups of pages from secondary storage using logical file offsets instead of virtual addresses.
    This saves the cache manager from having to map pages unnecessarily.

    The caller builds a list of various file objects and logical block offsets, passing them to MmPrefetchPagesIntoLockedMdl.
    The code here then examines the internal pages, reading in those that are not already valid or in transition.
    These pages are read with a single read, using a dummy page to bridge gaps of pages that were valid or transition prior to the I/O being issued.

    Upon conclusion of the I/O, control is returned to the calling thread.
    All pages are referenced counted as though they were probed and locked, regardless of whether they are currently valid or transition.
*/
...
NTSTATUS MiCcPrepareReadInfo(IN PMI_READ_INFO MiReadInfo);
NTSTATUS MiCcPutPagesInTransition(IN PMI_READ_INFO MiReadInfo);
NTSTATUS MiCcCompletePrefetchIos(PMI_READ_INFO MiReadInfo);
...
/*
Routine Description:
    This routine fills an MDL with pages described by the file object's offset and length.
    This routine is for cache manager usage only.
Arguments:
    FileObject - Supplies a pointer to the file object for a file which was opened with NO_INTERMEDIATE_BUFFERING clear, i.e., for which CcInitializeCacheMap was called by the file system.
    ...
    MdlOut - On output it returns a pointer to an Mdl describing the desired data.
Environment:
...
*/
{
    ...
    // Prepare for the impending read : allocate MDLs, inpage blocks, reference count subsections, etc.
    ...
    // APCs must be disabled once we put a page in transition.
    // Otherwise a thread suspend will stop us from issuing the I/O - this will hang any other threads that need the same page.
    CurrentThread = PsGetCurrentThread();
    ApcNeeded = FALSE;

    KeEnterCriticalRegionThread(&CurrentThread->Tcb);

    // The nested fault count protects this thread from deadlocks where a special kernel APC fires and references the same user page(s) we are putting in transition.
    ...
    // Allocate physical memory, lock down all the pages and issue any I/O that may be needed.
    // When MiCcPutPagesInTransition returns STATUS_SUCCESS or STATUS_ISSUE_PAGING_IO, it guarantees that the ApiMdl contains reference-counted (locked-down) pages.
    status = MiCcPutPagesInTransition(&MiReadInfo);
    if (NT_SUCCESS(status)) {
        // No I/O was issued because all the pages were already resident and have now been locked down.
        ...
        // Some error occurred (like insufficient memory, etc) so fail the request by falling through.
    }

    // Release acquired resources like pool, subsections, etc.
    MiCcReleasePrefetchResources(&MiReadInfo, status);

    // Only now that the I/O have been completed (not just issued) can APCs be re-enabled.
    // This prevents a user-issued suspend APC from keeping a shared page in transition forever.
    KeEnterGuardedRegionThread(&CurrentThread->Tcb);
...
/*
Routine Description:
    This routine releases all resources consumed to handle a system cache logical offset based prefetch.
Environment:
    Kernel mode, PASSIVE_LEVEL.
*/
{
    ...
    while (FirstReferencedSubsection != LastReferencedSubsection) {
        MiRemoveViewsFromSectionWithPfn((PMSUBSECTION)FirstReferencedSubsection, FirstReferencedSubsection->PtesInSubsection);
...
/*
Routine Description:
    This routine constructs MDLs that describe the pages in the argument read-list.
    The caller will then issue the I/O on return.
Arguments:
...
*/
{
    ...
    // If the section is backed by a ROM, then there's no need to prefetch anything as it would waste RAM.
    if (ControlArea->u.Flags.Rom == 1) {
        return STATUS_NOT_SUPPORTED;
    }

    MiReadInfo->ControlArea = ControlArea;// Initialize the internal Mi readlist.
    ...
    // Allocate and initialize an MDL to return to our caller.
    // The actual frame numbers are filled in when all the pages are reference counted.
    ...
    // Make sure the section is really prefetchable - physical and pagefile-backed sections are not.
    if ((ControlArea->u.Flags.PhysicalMemory) || (ControlArea->u.Flags.Image == 1) || (ControlArea->FilePointer == NULL)) {
    ...
    // Stash a pointer to the start of the prototype PTE array (the values in the array are not contiguous as they may cross subsections) in the inpage block so we can walk it quickly later when the pages are put into transition.
    ProtoPteArray = (PMMPTE *)(Mdl + 1);
    InPageSupport->BasePte = (PMMPTE)ProtoPteArray;

    // Data (but not image) reads use the whole page and the filesystems zero fill any remainder beyond valid data length so we don't bother to handle this here.
    // It is important to specify the entire page where possible so the filesystem won't post this which will hurt perf.  LWFIX: must use CcZero to make this true.
    ...
    for (i = 0; i < NumberOfPages; i += 1) {
        ...
        // A quick check is made first to avoid recalculations and loops where possible.
        if (ProtoPte >= LastProto) {
            // Handle extended subsections.
            // Increment the view count for every subsection spanned by this request, creating prototype PTEs if needed.
            ASSERT(i != 0);
            Subsection = Subsection->NextSubsection;
            Status = MiAddViewsForSectionWithPfn((PMSUBSECTION)Subsection, Subsection->PtesInSubsection);
            if (!NT_SUCCESS(Status)) {
...
/*
Routine Description:
    This routine allocates physical memory for the specified read-list and puts all the pages in transition (so collided faults from other threads for these same pages remain coherent).
    I/O for any pages not already resident are issued here.
    The caller must wait for their completion.
Arguments:
    MiReadInfo - Supplies a pointer to the read-list.
Return Value:
    STATUS_SUCCESS - all the pages were already resident, reference counts have been applied and no I/O needs to be waited for.
...
*/
{
    ...
    if (MdlPages + 1 > MAXUSHORT) {
        // The PFN ReferenceCount for the dummy page could wrap, refuse the request.
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    if (((SPFN_NUMBER)MdlPages > (SPFN_NUMBER)(MmAvailablePages - MM_HIGH_LIMIT)) || (MI_NONPAGEABLE_MEMORY_AVAILABLE() <= (SPFN_NUMBER)MdlPages)) {
        UNLOCK_PFN(OldIrql);
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    // Charge resident available immediately as the PFN lock may get released and reacquired below before all the pages have been locked down.
    ...
    // Always bias the reference count by 1 and charge for this locked page up front so the myriad increments and decrements don't get slowed down with needless checking.
    ...
    for (; ProtoPteArray < EndProtoPteArray; ProtoPteArray += 1, IoPage += 1, ApiPage += 1) {
        ...
        // The PointerPte better be inside a prototype PTE allocation so that subsequent page trims update the correct PTEs.
        ASSERT(((PointerPte >= (PMMPTE)MmPagedPoolStart) && (PointerPte <= (PMMPTE)MmPagedPoolEnd)) || ((PointerPte >= (PMMPTE)MmSpecialPoolStart) && (PointerPte <= (PMMPTE)MmSpecialPoolEnd)));

        // Check the state of this prototype PTE now that the PFN lock is held.
        // If the page is not resident, the PTE must be put in transition with read in progress before the PFN lock is released.

        // Lock page containing prototype PTEs in memory by incrementing the reference count for the page.
        // Unlock any page locked earlier containing prototype PTEs if the containing page is not the same for both.
        if (PfnProto != NULL) {
    ...
    recheck:
        ...
        if ((PteContents.u.Soft.Prototype == 0) && (PteContents.u.Soft.Transition == 1)) {
            ...
            // If there is an inpage still in progress, wait for it to complete.
            // Reference the PFN and then march on.
            PageFrameIndex = MI_GET_PAGE_FRAME_FROM_TRANSITION_PTE(&PteContents);
            Pfn1 = MI_PFN_ELEMENT(PageFrameIndex);
            ASSERT(Pfn1->u3.e1.PrototypePte == 1);
            if (Pfn1->u4.InPageError) {
                // There was an in-page read error and there are other threads colliding for this page, delay to let the other threads complete and then retry.
                UNLOCK_PFN(OldIrql);
        ...
        // LWFIX: need to handle protos that are now pagefile (or dzero) backed - prefetching it from the file here would cause us to lose the contents.
        // Note this can happen for session-space images as we back modified (ie: for relocation fixups or IAT updated) portions from the pagefile.  remove the assert below too.
        ...
        PageColor = MI_PAGE_COLOR_VA_PROCESS(MiGetVirtualAddressMappedByPte(PointerPte), &CurrentProcess->NextPageColor);
        ...
        // These pieces of MiInitializePfn initialization are overridden here as these pages are only going into prototype transition and not into any page tables.
        ...
        // Increment the PFN reference count in the control area for the subsection.
    ...
    // If all the pages were resident, dereference the dummy page references now and notify our caller that I/O is not necessary.
    if (NumberOfPagesNeedingIo == 0) {
        ...
        // Return the upfront resident available charge as the individual charges have all been made at this point.
        MI_INCREMENT_RESIDENT_AVAILABLE(ResidentAvailableCharge, MM_RESAVAIL_FREE_BUILDMDL_EXCESS);
        return STATUS_SUCCESS;
    }
    ...
    // If the MDL is not already embedded in the inpage block, see if its final size qualifies it - if so, embed it now.
    if ((Mdl != &InPageSupport->Mdl) && (Mdl->ByteCount <= (MM_MAXIMUM_READ_CLUSTER_SIZE + 1) * PAGE_SIZE)) {
#if DBG
        RtlFillMemoryUlong(&InPageSupport->Page[0], (MM_MAXIMUM_READ_CLUSTER_SIZE + 1) * sizeof(PFN_NUMBER), 0xf1f1f1f1);
#endif
    ...
    InterlockedExchangeAdd(&Prcb->MmPageReadCount, (LONG)NumberOfPagesNeedingIo);
    ...
    if (MiCcDebug & MI_CC_DELAY) {
        // This delay provides a window to increase the chance of collided faults.
        KeDelayExecutionThread(KernelMode, FALSE, (PLARGE_INTEGER)&MmHalfSecond);
    }
#endif
    ...
    // The API caller expects an MDL containing all the locked pages so it can be used for a transfer.

    // Note that an extra reference count is not taken on each page -
    // rather when the Io MDL completes, its reference counts are not decremented (except for the dummy page).
    // This combined with the reference count already taken on the resident pages keeps the accounting correct.
    // Only if an error occurs will the Io MDL completion decrement the reference counts.
    LocalPrototypePte = InPageSupport->BasePte;// Initialize the inpage support block Pfn field.
    ...
    ASSERT(KeGetCurrentIrql() == PASSIVE_LEVEL);
    status = IoPageRead(InPageSupport->FilePointer, (PMDL)((ULONG_PTR)Mdl | 0x1), &InPageSupport->ReadOffset, &InPageSupport->Event, &InPageSupport->IoStatus);
    if (!NT_SUCCESS(status)) {
        // Set the event as the I/O system doesn't set it on errors.
        // This way our caller will automatically unroll the PFN reference counts, etc, when the MiWaitForInPageComplete returns this status.
...
NTSTATUS MiCcCompletePrefetchIos(IN PMI_READ_INFO MiReadInfo)
/*
Routine Description:
    This routine waits for a series of page reads to complete and completes the requests.
Arguments:
...
*/
{
    ...
    status = MiWaitForInPageComplete(InPageSupport->Pfn, InPageSupport->BasePte, NULL, InPageSupport->BasePte, InPageSupport, PREFETCH_PROCESS);

    // MiWaitForInPageComplete RETURNS WITH THE PFN LOCK HELD!!!
    NumberOfBytes = (LONG)Mdl->ByteCount;
    while (NumberOfBytes > 0) {
        PfnClusterPage = MI_PFN_ELEMENT(*Page);// Only decrement reference counts if an error occurred.

#if DBG
        if (PfnClusterPage->u4.InPageError) {
            // If the page is marked with an error, then the whole transfer must be marked as not successful as well.
            // The only exception is the prefetch dummy page which is used in multiple transfers concurrently and thus may have the inpage error bit set at any time (due to another transaction besides the current one).
        ...
        // Note the reference count for each page is NOT decremented unless the I/O failed, in which case it is done below.
        // This allows the MmPrefetchPagesIntoLockedMdl API to return a locked page MDL.
        Page += 1;
    ...
    if (status != STATUS_SUCCESS) {
        ...
        // All the pages which were just put into transition must be put onto the free list if InPageError is set, and their PTEs restored to the proper contents.
        Page = (PPFN_NUMBER)(Mdl + 1);
        NumberOfBytes = (LONG)Mdl->ByteCount;
        while (NumberOfBytes > 0) {
    ...
    // All the relevant prototype PTEs should be in the transition or valid states and all page frames should be referenced.
