...
Abstract:
    This module contains the routines which map physical pages into reserved PTEs within hyper space.
*/
...
PVOID MiMapPageInHyperSpace(IN PEPROCESS Process, IN PFN_NUMBER PageFrameIndex, IN PKIRQL OldIrql)
/*
Routine Description:
    This procedure maps the specified physical page into hyper space and returns the virtual address which maps the page.
...
*/
{
    ...
    offset = MI_GET_PAGE_FRAME_FROM_PTE(PointerPte);// Get offset to first free PTE.
    if (offset == 0) {
        ...
        offset = NUMBER_OF_MAPPING_PTES;// Use the page frame number field of the first PTE as an offset into the available mapping PTEs.
        MI_FLUSH_PROCESS_TB(FALSE);// Flush entire TB only on processors executing this process.
    }

    PointerPte->u.Hard.PageFrameNumber = offset - 1;// Change offset for next time through.
    ...
    return MiGetVirtualAddressMappedByPte(PointerPte);// Return the VA that maps the page.
}
...
PVOID MiMapPagesToZeroInHyperSpace(IN PMMPFN Pfn1, IN PFN_COUNT NumberOfPages)
/*
Routine Description:
    This procedure maps the specified physical pages for the zero page thread and returns the virtual address which maps them.
...
Return Value:
    Returns the virtual address where the specified physical pages were mapped.
Environment:
    PASSIVE_LEVEL.
*/
{
    ...
    if (NumberOfPages > offset) {
        ...
        MI_FLUSH_ENTIRE_TB(0x25);// Flush entire TB on all processors as this thread may have migrated.
    }

    PointerPte->u.Hard.PageFrameNumber = offset - NumberOfPages;// Change offset for next time through.

    // Point to free entries and make them valid.
    // Note that the frames are mapped in reverse order but our caller doesn't care anyway.
    PointerPte += (offset + 1);
