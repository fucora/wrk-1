...
Abstract:
    This module contains the routines which reserve and release system wide PTEs reserved within the non paged portion of the system space.
    These PTEs are used for mapping I/O devices and mapping kernel stacks for threads.
*/
...
LOGICAL MiRecoverExtraPtes(ULONG NumberOfPtes);
...
// Define inline functions to pack and unpack pointers in the platform specific non-blocking queue pointer structure.
...
PMMPTE MiReserveSystemPtes(IN ULONG NumberOfPtes, IN MMSYSTEM_PTE_POOL_TYPE SystemPtePoolType)
/*
Routine Description:
    This function locates the specified number of unused PTEs within the non paged portion of system space.
Arguments:
    NumberOfPtes - Supplies the number of PTEs to locate.
    SystemPtePoolType - Supplies the PTE type of the pool to expand, one of SystemPteSpace or NonPagedPoolExpansion.
Return Value:
...
*/
{
    ...
    // phase 1 initialization - this means that the Ex worker package is ready to go (it is initialized in phase 1 just before Mm), 
    // so that MiReleaseSystemPtes can queue SLIST expansion if needed.
    ...
    NumberOfPtes = MmSysPteIndex[Index];
    for (i = 0; i < 10; i += 1) {
        PointerPte = MiReserveAlignedSystemPtes(NumberOfPtes, SystemPteSpace, 0);
        if (PointerPte == NULL) {
            return;
        }
...
/*
Routine Description:
    This function locates the specified number of unused PTEs to locate within the non paged portion of system space.
Arguments:
    NumberOfPtes - Supplies the number of PTEs to locate.
    SystemPtePoolType - Supplies the PTE type of the pool to expand, one of SystemPteSpace or NonPagedPoolExpansion.
    Alignment - Supplies the virtual address alignment for the address the returned PTE maps.
                For example, if the value is 64K, the returned PTE will map an address on a 64K boundary.
...
*/
{
    ...
    restart:
           ...
           // The nonpaged PTE pool uses the invalid PTEs to define the pool structure.
           // A global pointer points to the first free set in the list, each free set contains the number free and a pointer to the next free set.
           // The free sets are kept in an ordered list such that the pointer to the next free set is always greater than the address of the current free set.

           // As to not limit the size of this pool, two PTEs are used to define a free region.
           // If the region is a single PTE, the prototype field within the PTE is set indicating the set consists of a single PTE.

           // The page frame number field is used to define the next set and the number free.
           // The two flavors are:
           //                           o          V
           //                           n          l
           //                           e          d
...
#if defined (_X86_)
               if ((SystemPtePoolType == SystemPteSpace) && (MiRecoverExtraPtes(NumberOfPtes) == TRUE)) {
                   goto restart;
               }
#endif
           ...
           if (Alignment <= PAGE_SIZE) {
               ...
               while (TRUE) {
                   ...
                   if (NumberOfPtes < SizeInSet) {
                       // Get the PTEs from this set and reduce the size of the set.
                       // Note that the size of the current set cannot be 1.
                       if ((SizeInSet - NumberOfPtes) == 1) {
                           PointerPte->u.List.OneEntry = 1;// Collapse to the single PTE format.
                       } else {
                   ...
                   if (NumberOfPtes == SizeInSet) {
                   ExactFit:
                       // Satisfy the request with this complete set and change the list to reflect the fact that this set is gone.
                       ...
                       MiUnlockSystemSpace(OldIrql);// Release the lock and flush the TB.
                       break;
                   }
               ...
               while (TRUE) {
                   ...
                   if (NumberOfRequiredPtes < SizeInSet) {
                       ...
                       // This current block will be slit into 2 blocks if the PointerPte does not match the alignment.

                       // Check to see if the first PTE is on the proper alignment, if so, eliminate this block.
                       ...
                       if (PtesToObtainAlignment == 0) {
                           ...
                           PointerPte->u.List.NextEntry = PteOffset;// Point to the new set at the end of the block we are giving away.

                           // Update the size of the current set.
                           if (PtesToObtainAlignment == 1) {
                               PointerPte->u.List.OneEntry = 1;// Collapse to the single PTE format.
                           } else {
                               PointerFollowingPte->u.List.NextEntry = PtesToObtainAlignment; // Set the set size in the next PTE.
                           }
                       }
           ...
           // Capture the global timestamp into a local so the compare will use the same value throughout.
           ...
           if (MiCompareTbFlushTimeStamp(TimeStamp, SYSPTES_FLUSH_COUNTER_MASK) == FALSE) {
               // The TB has been flushed since the last release so nothing needs to be done here.
               NOTHING;
...
*/
{
    ...
    if (SystemPtePoolType == SystemPteSpace) {
        ...
        if (HighConsumer != NULL) {
            KeBugCheckEx(DRIVER_USED_EXCESSIVE_PTES, (ULONG_PTR)HighConsumer, HighPteUse, MmTotalFreeSystemPtes[SystemPtePoolType], MmNumberOfSystemPtes);
        }
    }

    KeBugCheckEx(NO_MORE_SYSTEM_PTES, (ULONG_PTR)SystemPtePoolType, NumberOfPtes, MmTotalFreeSystemPtes[SystemPtePoolType], MmNumberOfSystemPtes);
}
...
/*
Routine Description:
    This routine is the worker routine to add additional SLISTs for the system PTE nonblocking queues.
Arguments:
...
*/
{
    ...
    if (Expansion->SListPages < MI_MAXIMUM_SLIST_PTE_PAGES) {
        SListChunks = (PPTE_SLIST)ExAllocatePoolWithTag(NonPagedPool, PAGE_SIZE, 'PSmM');// Allocate another page of SLIST entries for the nonblocking PTE queues.
        if (SListChunks != NULL) {
            ...
            for (i = 0; i < SListEntries; i += 1) {
                InterlockedPushEntrySList(&MiSystemPteSListHead, (PSLIST_ENTRY)SListChunks);
                SListChunks += 1;
            }
        }
    }
...
PVOID MmMapLockedPagesSpecifyCache(__in PMDL MemoryDescriptorList,
...
/*
Routine Description:
    This function maps physical pages described by a memory descriptor list into the system virtual address space or the user portion of the virtual address space.
Arguments:
    ...
    RequestedAddress - Supplies the base user address of the view.
                       ...
                       This is only treated as an address if the AccessMode is UserMode.
                       ...
                       If the AccessMode is KernelMode, then this argument is treated as a bit field of attributes.
    BugCheckOnFailure - Supplies whether to bugcheck if the mapping cannot be obtained.
                        This flag is only checked if the MDL's MDL_MAPPING_CAN_FAIL is zero, which implies that the default MDL behavior is to bugcheck.
                        This flag then provides an additional avenue to avoid the bugcheck.
...
*/
{
    ...
    // If this assert fires, the MiPlatformCacheAttributes array initialization needs to be checked.
    ...
    if (AccessMode == KernelMode) {
        ...
        // Map the pages into the system part of the address space as kernel read/write.
...
#pragma prefast(suppress: 2000, "SAL 1.2 needed for accurate MDL struct annotation.")
                ...
                if (MI_PAGE_FRAME_INDEX_MUST_BE_CACHED(PageFrameIndex)) {
                    ...
                    if (((MemoryDescriptorList->MdlFlags & MDL_MAPPING_CAN_FAIL) == 0) && (BugCheckOnFailure)) {
                        KeBugCheckEx(MEMORY_MANAGEMENT, 0x1000, (ULONG_PTR)MemoryDescriptorList, (ULONG_PTR)PageFrameIndex, (ULONG_PTR)CacheAttribute);
                    }
                    return NULL;
                }
        ...
        if (MmTrackPtes & 0x1) {
            ...
            MiInsertPteTracker(MemoryDescriptorList, 0, IoMapping, CacheAttribute, CallingAddress, CallersCaller);
        }
...
VOID MmUnmapLockedPages(__in PVOID BaseAddress, __in PMDL MemoryDescriptorList)
/*
Routine Description:
    This routine unmaps locked pages which were previously mapped via an MmMapLockedPages call.
Arguments:
    BaseAddress - Supplies the base address where the pages were previously mapped.
    MemoryDescriptorList - Supplies a valid Memory Descriptor List which has been updated by MmProbeAndLockPages.
Environment:
    Kernel mode.  DISPATCH_LEVEL or below if base address is within system space; APC_LEVEL or below if base address is user space.
...
*/
{
    ...
    if (BaseAddress > MM_HIGHEST_USER_ADDRESS) {
        ...
        if (MmTrackPtes != 0) {
            if (MmTrackPtes & 0x1) {
                MiRemovePteTracker(MemoryDescriptorList, BaseAddress, NumberOfPages);
            }
            if (MmTrackPtes & 0x2) {
        ...
        MemoryDescriptorList->MdlFlags &= ~(MDL_MAPPED_TO_SYSTEM_VA | MDL_PARTIAL_HAS_BEEN_MAPPED | MDL_FREE_EXTRA_PTES);
        ...
        if (NumberOfPages <= MM_PTE_TABLE_LIMIT) {
            ...
            if (MmTotalFreeSystemPtes[SystemPteSpace] >= MM_MIN_SYSPTE_FREE) {
                ...
                if (MmSysPteListBySizeCount[Index] <= i) {
                    // Zero PTEs, then encode the PTE pointer and the TB flush counter into Value.
        ...
        if (MmTrackPtes & 0x2) {
            // This release has already been updated in the tracking bitmaps so mark it so that MiReleaseSystemPtes doesn't attempt to do it also.
...
/*
Routine Description:
    This function releases the specified number of PTEs within the non paged portion of system space.
    Note that the PTEs must be invalid and the page frame number must have been set to zero.
Arguments:
    StartingPte - Supplies the address of the first PTE to release.
    NumberOfPtes - Supplies the number of PTEs to release.
    SystemPtePoolType - Supplies the PTE type of the pool to release PTEs to, one of SystemPteSpace or NonPagedPoolExpansion.
Environment:
    Kernel mode.
*/
{
    ...
    if ((MmTrackPtes & 0x2) && (SystemPtePoolType == SystemPteSpace)) {
        // If the low bit is set, this range was never reserved and therefore should not be validated during the release.
    ...
    if ((SystemPtePoolType == SystemPteSpace) && (NumberOfPtes <= MM_PTE_TABLE_LIMIT)) {
        ...
        if (MmTotalFreeSystemPtes[SystemPteSpace] >= MM_MIN_SYSPTE_FREE) {
            ...
            if (MmSysPteListBySizeCount[Index] <= i) {
                ...
                // No lookasides are left for inserting this PTE allocation into the nonblocking queues.
                // Queue an extension to a worker thread so it can be done in a deadlock-free manner.
                if (MiPteSListExpand.SListPages < MI_MAXIMUM_SLIST_PTE_PAGES) {
                    ...
                    if (ExtensionInProgress == 0) {
                        ExInitializeWorkItem(&MiPteSListExpand.WorkItem, MiPteSListExpansionWorker, (PVOID)&MiPteSListExpand);
        ...
        // The insert failed - our lookaside list must be empty or we are low on PTEs systemwide or we already had plenty on our list and didn't try to insert.
        // Fall through to queue this in the long way.
        NumberOfPtes = MmSysPteIndex[Index];
    }
    ...
    // Since the PTEs have already been zeroed, snap the TB flush time stamp now (lock-free).

    // Note this can only be set after the system space spin lock is acquired.
    // This prevents a first-thread in which becomes the last thread out from setting a too-old timestamp which would cause PTEs inserted by other threads in the gap to not get TB-flushed on reuse.

    // This lock synchronization is necessary because one timestamp is being used for all the system PTEs (instead of per-chunk) because the different chunks get coalesced below.
    ...
    while (TRUE) {
        ...
        if (PteOffset < PointerPte->u.List.NextEntry) {
            // Insert in the list at this point.
            // The previous one should point to the new freed set and the new freed set should point to the place the previous set points to.

            // Attempt to combine the clusters before we insert.
            ...
            if ((PointerPte + Size) == StartingPte) {
                ...
                // Point the starting PTE to the beginning of the new free set and try to combine with the following free cluster.
                StartingPte = PointerPte;
            } else {
                // Can't combine with previous. Make this PTE the start of a cluster.

                StartingPte->u.List.NextEntry = PointerPte->u.List.NextEntry;// Point this cluster to the next cluster.
                PointerPte->u.List.NextEntry = PteOffset;// Point the current cluster to this cluster.
            ...
            // Attempt to combine the newly created cluster with the following cluster.
            if ((StartingPte + NumberOfPtes) == NextPte) {
                // Combine with following cluster.

                // Set the next cluster to the value contained in the cluster we are merging into this one.
        ...
        PointerPte = NextPte;// Point to next freed cluster.
    }
}
...
Routine Description:
    This function releases the specified number of PTEs within the non paged portion of system space.
    Note that the PTEs must be invalid and the page frame number must have been set to zero.
    This portion is a split portion from a larger allocation so careful updating of the tracking bitmaps must be done here.
Arguments:
    StartingPte - Supplies the address of the first PTE to release.
    NumberOfPtes - Supplies the number of PTEs to release.
    SystemPtePoolType - Supplies the PTE type of the pool to release PTEs to, one of SystemPteSpace or NonPagedPoolExpansion.
...
*/
{
    ...
    if ((MmTrackPtes & 0x2) && (SystemPtePoolType == SystemPteSpace)) {
        ...
        if (StartBit != 0) {
            if (RtlCheckBit(MiPteStartBitmap, StartBit - 1)) {
                if (!RtlCheckBit(MiPteEndBitmap, StartBit - 1)) {
                    // In the middle of an allocation - update the previous so it ends here.
        ...
        // Unconditionally set the end bit (and clear any others) in case the split chunk crosses multiple allocations.
...
VOID MiInitializeSystemPtes(IN PMMPTE StartingPte, IN PFN_NUMBER NumberOfPtes, IN MMSYSTEM_PTE_POOL_TYPE SystemPtePoolType)
/*
Routine Description:
...
Arguments:
    ...
    SystemPtePoolType - Supplies the PTE type of the pool to initialize, one of SystemPteSpace or NonPagedPoolExpansion.
Environment:
    Kernel mode.
*/
{
    ...
    // This takes into account that systems may have additional PTE pools below the PTE_BASE.
    ...
    // If there are no PTEs specified, then make a valid chain by indicating that the list is empty.
    if (NumberOfPtes == 0) {
    ...
    // The page frame field points to the next cluster.
    // As we only have one cluster at initialization time, mark it as the last cluster.
    ...
    // If there is only one PTE in the pool, then mark it as a one entry PTE. Otherwise, store the cluster size in the following PTE.
    if (NumberOfPtes == 1) {
    ...
    if (SystemPtePoolType == SystemPteSpace) {
        ...
        Chunk = SListChunks;
        for (i = 0; i < SListEntries; i += 1) {
            InterlockedPushEntrySList(&MiSystemPteSListHead, (PSLIST_ENTRY)Chunk);
            Chunk += 1;
        }
        ...
        // Turn this on after the multiple releases of the binned PTEs (that came from a single reservation) above.
        if (MiPteStartBitmap != NULL) {
            MmTrackPtes |= 0x2;
        }
    }
...
/*
Routine Description:
    This routine increments the total number of PTEs.
    This is done separately from actually adding the PTEs to the pool so that autoconfiguration can use the high number in advance of the PTEs actually getting added.
Arguments:
...
*/
{
    ...
    // Set the low bit to signify this range was never reserved and therefore should not be validated during the release.
    if (MmTrackPtes & 0x2) {
        StartingPte = (PMMPTE)((ULONG_PTR)StartingPte | 0x1);
    }
...
/*
Routine Description:
    This routine returns the number of free entries of the list which covers the specified size.
    The size must be less than or equal to the largest list index.
Arguments:
...
/*
Routine Description:
    This routine checks how many SystemPteSpace PTEs are available for the requested size.
    If plenty are available then TRUE is returned.
    If we are reaching a low resource situation, then the request is evaluated based on the argument priority.
Arguments:
...
/*
Routine Description:
    This function checks the reserve of the specified number of system space PTEs.
Arguments:
...
*/
{
    ...
    for (; i < StartBit + NumberOfPtes; i += 1) {
        if (MI_CHECK_BIT(StartBitMapBuffer, i)) {
            KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x201, (ULONG_PTR)VirtualAddress, (ULONG_PTR)VirtualAddress + ((i - StartBit) << PAGE_SHIFT), NumberOfPtes);
        }
    }
    ...
    for (i = StartBit; i < StartBit + NumberOfPtes; i += 1) {
        if (MI_CHECK_BIT(EndBitMapBuffer, i)) {
            KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x202, (ULONG_PTR)VirtualAddress, (ULONG_PTR)VirtualAddress + ((i - StartBit) << PAGE_SHIFT), NumberOfPtes);
        }
    }
...
/*
Routine Description:
    This function checks the release of the specified number of system space PTEs.
Arguments:
    StartingPte - Supplies the address of the first PTE to release.
    NumberOfPtes - Supplies the number of PTEs to release.
Environment:
...
*/
{
    ...
    if (NumberOfPtes == 0) {
        KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x300, (ULONG_PTR)VirtualAddress, (ULONG_PTR)LowestVirtualAddress, (ULONG_PTR)HighestVirtualAddress);
    }

    if (StartingPte < MmSystemPtesStart[SystemPteSpace]) {
        KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x301, (ULONG_PTR)VirtualAddress, (ULONG_PTR)LowestVirtualAddress, (ULONG_PTR)HighestVirtualAddress);
    }

    if (StartingPte > MmSystemPtesEnd[SystemPteSpace]) {
        KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x302, (ULONG_PTR)VirtualAddress, (ULONG_PTR)LowestVirtualAddress, (ULONG_PTR)HighestVirtualAddress);
    }
    ...
    for (i = StartBit; i < StartBit + CalculatedPtes; i += 1) {
        if (MI_CHECK_BIT(StartBitMapBuffer, i) == 0) {
            KeBugCheckEx(SYSTEM_PTE_MISUSE, 0x306, (ULONG_PTR)VirtualAddress, (ULONG_PTR)VirtualAddress + ((i - StartBit) << PAGE_SHIFT), CalculatedPtes);
        }
    }
...
VOID MiDumpSystemPtes(IN MMSYSTEM_PTE_POOL_TYPE SystemPtePoolType)
{
    ...
    for (;;) {
        ...
        DbgPrintEx(DPFLTR_MM_ID, DPFLTR_TRACE_LEVEL, "System Pte at %p for %p entries (%p)\n", PointerPte, ClusterSize, EndOfCluster);
        if (PointerPte->u.List.NextEntry == MM_EMPTY_PTE_LIST) {
            break;
        }
...
VOID MiAddExtraSystemPteRanges(IN PMMPTE PointerPte, IN ULONG NumberOfPtes)
{
    PVOID StartingVa;
    KIRQL OldIrql;
    ULONG_PTR PteOffset;
    ...
    // Since the next PTE is used to hold the count, don't bother with any single PTE additions (there shouldn't be any of those anyway).
    if (NumberOfPtes == 1) {
        return;
    }
    ...
    // This provides a crude way to keep the larger allocations contiguous for a longer period of time.
    MiLockSystemSpace(OldIrql);
...
LOGICAL MiRecoverExtraPtes(IN ULONG NumberOfPtes)
/*
Routine Description:
    This routine is called to recover extra PTEs for the system PTE pool.
    These are not just added in earlier in Phase 0 because the system PTE allocator uses the low addresses first which would fragment these bigger ranges.
Return Value:
...
*/
{
    ...
    // Quickly do an unsynchronized check so we avoid the spinlock if we have no chance.
    if (ThisPte->u.List.NextEntry == MM_EMPTY_PTE_LIST) {
        return FALSE;
    }

    // Search for an entry large enough to satisfy the request, if one cannot be found, 
    // then remove them all in hopes that adjacent allocations can be coalesced by our caller to satisfy the request.
    MiLockSystemSpace(OldIrql);
    if (ThisPte->u.List.NextEntry == MM_EMPTY_PTE_LIST) {
        MiUnlockSystemSpace(OldIrql);
        return FALSE;
    ...
    do {
        ...
        // Just free the whole entry as splitting it into the exact amount can trigger race conditions where another thread allocates a PTE from the front before our caller retries (causing our caller to fail the API).
        if (NumberOfPtes <= (NextPte + 1)->u.List.NextEntry) {
            // The request can only be satisfied by using the entire entry so delink it now.
