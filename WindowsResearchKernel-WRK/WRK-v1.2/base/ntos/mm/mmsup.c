...
LOGICAL FASTCALL MiIsProtectionCompatible(IN WIN32_PROTECTION_MASK OldWin32Protect, IN WIN32_PROTECTION_MASK NewWin32Protect)
/*
Routine Description:
    ...
    ExecuteRead       NoAccess, ReadOnly, ReadWriteCopy, Execute, ExecuteRead, ExecuteWriteCopy
    ExecuteWrite      NoAccess, ReadOnly, ReadWriteCopy, Execute, ExecuteRead, ExecuteWriteCopy, ReadWrite, ExecuteWrite
    ExecuteWriteCopy  NoAccess, ReadOnly, ReadWriteCopy, Execute, ExecuteRead, ExecuteWriteCopy
...
LOGICAL FASTCALL MiIsPteProtectionCompatible(IN MM_PROTECTION_MASK PteProtection, IN WIN32_PROTECTION_MASK NewProtect)
{
    MM_PROTECTION_MASK Mask;
    WIN32_PROTECTION_MASK Win32ProtectMask;
...
Return Value:
    ...
    Note that MM_INVALID_PROTECTION (-1) is returned for an invalid protection request.
    Since valid PTE protections fit in 5 bits and are zero-extended, it's easy for callers to distinguish this.
Environment:
    Kernel Mode.
*/
{
    ...
    if (Win32Protect & PAGE_GUARD) {
        if ((ProtectCode == MM_NOACCESS) || (Win32Protect & (PAGE_NOCACHE | PAGE_WRITECOMBINE))) {
            // Invalid protection -
    ...
    if (Win32Protect & PAGE_NOCACHE) {
        ...
        if ((ProtectCode == MM_NOACCESS) || (Win32Protect & PAGE_WRITECOMBINE)) {
            // Invalid protection -
...
/*
Routine Description:
    This routine examines the specified Page Directory Parent Entry to determine if the page directory page mapped by the PPE exists.
    If it does, then it examines the specified Page Directory Entry to determine if the page table page mapped by the PDE exists.
...
*/
{
    ...
    // Page directory parent (or extended parent) entry not valid, make it valid.
    PointerPte = MiGetVirtualAddressMappedByPte(PointerPde);

    do {
        ASSERT(KeAreAllApcsDisabled() == TRUE);

        // Fault it in, this must be done one level at a time because the fault handler checks for the preceding level already being valid for system space addresses.
...
ULONG FASTCALL MiMakeSystemAddressValid(IN PVOID PageTableVirtualAddress, IN PEPROCESS CurrentProcess)
/*
Routine Description:
    This routine checks to see if the virtual address is valid, and if not makes it valid.
Arguments:
    PageTableVirtualAddress - Supplies the virtual address to make valid - this MUST be a page table hierarchy address because the process working set pushlock is used to synchronize the validity check below.
    CurrentProcess - Supplies a pointer to the current process.
Return Value:
    Returns TRUE if the working set pushlock was released and wait performed, FALSE otherwise.
Environment:
    Kernel mode, APCs disabled, working set pushlock held.
*/
{
    ...
    ASSERT((PageTableVirtualAddress < MM_PAGED_POOL_START) || (PageTableVirtualAddress > MmPagedPoolEnd));
    ASSERT(KeAreAllApcsDisabled() == TRUE);

    while (!MiIsAddressValid(PageTableVirtualAddress, TRUE)) {
        // The virtual address is not present.
        // Release the working set pushlock and fault it in.

        // The working set pushlock may have been acquired safely or unsafely by our caller.
        ...
        if (!NT_SUCCESS(status)) {
            KeBugCheckEx(KERNEL_DATA_INPAGE_ERROR, 1, (ULONG)status, (ULONG_PTR)CurrentProcess, (ULONG_PTR)PageTableVirtualAddress);
        }
...
Arguments:
    ...
    CurrentProcess - Supplies a pointer to the current process (whose working set pushlock is held).
...
*/
{
    ...
    while (!MiIsAddressValid(VirtualAddress, FALSE)) {
        // The virtual address is not present.
        // Release the working set pushlock and PFN lock and fault it in.
        ...
        if (!NT_SUCCESS(status)) {
            KeBugCheckEx(KERNEL_DATA_INPAGE_ERROR, 2, (ULONG)status, (ULONG_PTR)CurrentProcess, (ULONG_PTR)VirtualAddress);
        }
...
*/
{
    ...
    // The virtual address is not present.
    // Release the PFN lock and fault it in.
    UNLOCK_PFN(OldIrql);
...
/*
Routine Description:
    This procedure restores the original contents into the PTE (which could be a prototype PTE) referred to by the PFN database for the specified physical page.
    It also updates all necessary data structures to reflect the fact that the referenced PTE is no longer in transition.
...
*/
{
    ...
    if (Pfn1->u3.e1.PrototypePte) {
        ...
        // This page is referenced by a prototype PTE.
        // The segment structures need to be updated when the page is removed from the transition state.
        if (Pfn1->OriginalPte.u.Soft.Prototype) {
            // The prototype PTE is in subsection format, calculate the address of the control area for the subsection and decrement the number of PFN references to the control area.
        ...
        // The page points to a page or page table page which may not be for the current process.
        // Map the page into hyperspace and reference it through hyperspace.
        // If the page resides in system space (but not session space), it does not need to be mapped as all PTEs for system space must be resident.
        // Session space PTEs are only mapped per session so access to them must also go through hyperspace.
        PointerPte = Pfn1->PteAddress;
    ...
    // The PTE has been restored to its original contents and is no longer in transition.
    // Decrement the share count on the page table page which contains the PTE.
...
PSUBSECTION MiGetSubsectionAndProtoFromPte(IN PMMPTE PointerPte, OUT PMMPTE *ProtoPte)
/*
Routine Description:
...
Environment:
    Kernel mode - Must be holding the PFN lock and working set pushlock (acquired safely) with APCs disabled.
*/
{
    PMMPTE PointerProto;
    PMMPFN Pfn1;
...
BOOLEAN MmIsNonPagedSystemAddressValid(__in PVOID VirtualAddress)
/*
Routine Description:
    For a given virtual address this function returns TRUE if the address is within the non-pageable portion of the system's address space, FALSE otherwise.
Arguments:
    VirtualAddress - Supplies the virtual address to check.
Return Value:
    TRUE if the address is within the non-pageable portion of the system address space, FALSE otherwise.
Environment:
    Kernel mode.
*/
{
    // Return TRUE if address is within the non-pageable portion of the system.
    // Check limits for paged pool and if not within those limits, return TRUE.
    if ((VirtualAddress >= MmPagedPoolStart) && (VirtualAddress <= MmPagedPoolEnd)) {
        return FALSE;
    }

    // Check special pool before checking session space because on NT64 nonpaged session pool exists in session space (on NT32, nonpaged session requests are satisfied from systemwide nonpaged pool instead).
...
VOID MmHibernateInformation(IN PVOID    MemoryMap, OUT PULONG_PTR  HiberVa, OUT PPHYSICAL_ADDRESS HiberPte)
{
    ...
    // Return the dump PTEs to the loader (as it needs to use them to map it's relocation code into the kernel space on the final bit of restoring memory).
