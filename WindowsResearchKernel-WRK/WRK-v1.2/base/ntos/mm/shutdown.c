...
BOOLEAN MiShutdownSystem(VOID);
VOID MiZeroPageFile(IN PVOID Context);
LOGICAL MiZeroAllPageFiles(VOID);
...
/*
Routine Description:
    This routine zeroes all inactive pagefile blocks in the specified paging file.
Arguments:
    Context - Supplies the information on which pagefile to zero and a zeroed page to use for the I/O.
Return Value:
...
*/
{
    ...
    for (i = 1; i < PagingFile->Size; i += 1) {
        if (RtlCheckBit(PagingFile->Bitmap, (ULONG)i) == 0) {
            // Claim the pagefile location as the modified writer may already be scanning.
        ...
        if (write) {
            ...
            // Ignore all I/O failures - there is nothing that can be done at this point.
            if (!NT_SUCCESS(Status)) {
                KeSetEvent(&IoEvent, 0, FALSE);
            }

            Status = KeWaitForSingleObject(&IoEvent, WrPageOut, KernelMode, FALSE, (PLARGE_INTEGER)&MmTwentySeconds);
            if (Status == STATUS_TIMEOUT) {
                // The write did not complete in 20 seconds, assume that the file systems are hung and return an error.

                // Note the zero page (and any MDL system virtual address a driver may have created) is leaked because we don't know what the filesystem or storage stack might (still) be doing to them.
                Pfn1 = MI_PFN_ELEMENT(ZeroedPageFrame);

                LOCK_PFN(OldIrql);

                // Increment the reference count on the zeroed page to ensure it is never freed.
...
*/
{
    ...
    for (i = NumberOfPagingFiles; i != 0; i -= 1) {
        ...
        ZeroContext = ExAllocatePoolWithTag(NonPagedPool, sizeof(MM_ZERO_PAGEFILE_CONTEXT), 'wZmM');
        if (ZeroContext == NULL) {
        ...
        if (i != 1) {
            ExInitializeWorkItem(&ZeroContext->WorkItem, MiZeroPageFile, (PVOID)ZeroContext);
            ExQueueWorkItem(&ZeroContext->WorkItem, CriticalWorkQueue);
        } else {
            // Zero the first pagefile ourself, then wait for any others to finish.
            KeSetEvent(WaitObjects[i - 1], 0, FALSE);
    ...
    if (NumberOfPagingFiles > 1) {
        KeWaitForMultipleObjects(NumberOfPagingFiles, &WaitObjects[0], WaitAll, Executive, KernelMode, FALSE, NULL, &WaitBlockArray[0]);
    }
...
/*
Routine Description:
    This function performs the shutdown of memory management.
    This is accomplished by writing out all modified pages which are destined for files other than the paging file.

    All processes have already been killed, the registry shutdown and shutdown IRPs already sent.
    On return from this phase all mapped file data must be flushed and the unused segment list emptied.
    This releases all the Mm references to file objects, allowing many drivers (especially the network) to unload.
Return Value:
...
*/
{
    ...
    if (MmSystemShutdown == 0) {
        ...
        while (ModifiedPage != MM_EMPTY_LIST) {
            ...
            if (Pfn1->OriginalPte.u.Soft.Prototype == 1) {
                ...
                if ((!ControlArea->u.Flags.Image) && (!ControlArea->u.Flags.NoModifiedWriting)) {
                    ...
                    MI_SET_MODIFIED(Pfn1, 0, 0x28);// Issue the write.
                    MI_ADD_LOCKED_PAGE_CHARGE_FOR_MODIFIED_PAGE(Pfn1);// Up the reference count for the physical page as there is I/O in progress.
                    *Page = ModifiedPage;
                ...
                retry:
                    ...
                    if (NT_SUCCESS(Status)) {
                        ...
                        FsRtlReleaseFileForCcFlush(FilePointer);// Release the file we acquired.
                    }

                    if (!NT_SUCCESS(Status)) {

                        // Only try the request more than once if the filesystem said it had a deadlock.
        ...
        MmSystemShutdown = 1;// Indicate to the modified page writer that the system has shutdown.
    ...
    if (PoCleanShutdownEnabled()) {
        ...
        // Give it 5 seconds to empty otherwise assume the filesystems are hung and march on.
...
#if DBG
            if (count == 400) {
                // Everything should have been flushed by now.
                // Give the filesystem team a chance to debug this on checked builds.
                ASSERT(FALSE);
            }
#endif

            // Resignal if needed in case more closed file objects triggered additional entries.
        ...
        // Get rid of any paged pool references as they will be illegal by the time MmShutdownSystem is called again since the filesystems will have shutdown.
        ...
        while (NextEntry != &PsLoadedModuleList) {
            ...
            if ((ImportList != (PVOID)LOADED_AT_BOOT) && (ImportList != (PVOID)NO_IMPORTS_USED) && (!SINGLE_ENTRY(ImportList))) {
        ...
        // Close all the pagefile handles, note we still have an object reference to each keeping the underlying object resident.
        // At the end of Phase1 shutdown we'll release those references to trigger the storage stack unload.
        // The handle close must be done here however as it will reference pageable structures.
        for (i = 0; i < MmNumberOfPagingFiles; i += 1) {
            // Free each pagefile name now as it resides in paged pool and may need to be inpaged to be freed.
            // Since the paging files are going to be shutdown shortly, now is the time to access pageable stuff and get rid of it.
            // Zeroing the buffer pointer is sufficient as the only accesses to this are from the try-except-wrapped GetSystemInformation APIs and all the user processes are gone already.
...
BOOLEAN MmShutdownSystem(IN ULONG Phase)
/*
Routine Description:
    This function performs the shutdown of memory management.
    This is accomplished by writing out all modified pages which are destined for files other than the paging file.
Arguments:
    Phase - Supplies 0 on the initiation of shutdown.
            All processes have already been killed, the registry shutdown and shutdown IRPs already sent.
            On return from this phase all mapped file data must be flushed and the unused segment list emptied.
            This releases all the Mm references to file objects, allowing many drivers (especially the network) to unload.

            Supplies 1 on the initiation of shutdown.
            The filesystem stack has received its shutdown IRPs (the stack must free its paged pool allocations here and lock down any pageable code it intends to call) as no more references to pageable code or data are allowed on return.
            ie: Any IoPageRead at this point is illegal.
            Close the pagefile handles here so the filesystem stack will be dereferenced causing those drivers to unload as well.

            Supplies 2 on final shutdown of the system.
            Any resources not freed by this point are treated as leaks and cause a bugcheck.
Return Value:
    TRUE if the pages were successfully written, FALSE otherwise.
*/
{
    ...
    if (Phase == 1) {
        // The filesystem has shutdown.  References to pageable code or data is no longer allowed at this point.

        // Close the pagefile handles here so the filesystem stack will be dereferenced causing those drivers to unload as well.
        if (MmSystemShutdown < 2) {
            MmSystemShutdown = 2;
            if (PoCleanShutdownEnabled() & PO_CLEAN_SHUTDOWN_PAGING) {
                // Make any IoPageRead at this point illegal.
                // Detect this by purging all system pageable memory.
                MmTrimAllSystemPageableMemory(TRUE);

                // There should be no dirty pages destined for the filesystem.
                // Give the filesystem team a shot to debug this on checked builds.
                ASSERT(MmModifiedPageListHead.Total == MmTotalPagesForPagingFile);

                // Dereference all the pagefile objects to trigger a cascading unload of the storage stack as this should be the last reference to their driver objects.
                for (i = 0; i < MmNumberOfPagingFiles; i += 1) {
...
*/
{
    ...
    // Free the pagefile structures - note the PageFileName buffer was freed earlier as it resided in paged pool and may have needed an inpage to be freed.
    for (i = 0; i < MmNumberOfPagingFiles; i += 1) {
        ASSERT(MmPagingFile[i]->PageFileName.Buffer == NULL);
    ...
    // Free the verifier list last because it must be consulted to debug any bugchecks.
