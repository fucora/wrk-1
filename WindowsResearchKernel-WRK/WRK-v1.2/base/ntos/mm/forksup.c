...
ULONG MiLeaveThisPageGetAnother(OUT PPFN_NUMBER PageFrameIndex, IN PMMPTE PointerPde, IN PEPROCESS CurrentProcess);
...
VOID MiBuildForkPageTable(IN PFN_NUMBER PageFrameIndex, IN PMMPTE PointerPde, IN PMMPTE PointerNewPde, IN PFN_NUMBER PdePhysicalPage, IN PMMPFN PfnPdPage, IN LOGICAL MakeValid);
...
/*
Routine Description:
    This routine stands on its head to produce a copy of the specified process's address space in the process to initialize.
    This is done by examining each virtual address descriptor's inherit attributes.
    If the pages described by the VAD should be inherited, each PTE is examined and copied into the new address space.
    For private pages, fork prototype PTEs are constructed and the pages become shared, copy-on-write, between the two processes.
Arguments:
    ProcessToClone - Supplies the process whose address space should be cloned.
    ProcessToInitialize - Supplies the process whose address space is to be created.
Environment:
    Kernel mode, APCs disabled.  No VADs exist for the new process on entry and the new process is not on the working set expansion links list so it cannot be trimmed (or outswapped).
    Hence none of the pages in the new process need to be locked down.
*/
{
    ...
    // Get the working set mutex and the address creation mutex of the process to clone.
    // This prevents page faults while we are examining the address map and prevents virtual address space from being created or deleted.
    ...
    // Attempt to acquire the needed pool before starting the clone operation, this allows an easier failure path in the case of insufficient system resources.
    // The working set mutex must be acquired (and held throughout) to protect against modifications to the NumberOfPrivatePages field in the EPROCESS.

    // In addition since paged pool cannot be allocated or accessed while holding the working set pushlock (unless the pool is locked down), allocate it and lock it down now.
RetryPrivatePageAllocations:
    ...
    // Charge the current process the quota for the paged and nonpaged global structures.
    // This consists of the array of clone blocks in paged pool and the clone header in non-paged pool.
    status = PsChargeProcessPagedPoolQuota(CurrentProcess, sizeof(MMCLONE_BLOCK) * NumberOfPrivatePages);
    if (!NT_SUCCESS(status)) {// Unable to charge quota for the clone blocks.
        goto ErrorReturn1;
    }

    status = PsChargeProcessNonPagedPoolQuota(CurrentProcess, sizeof(MMCLONE_HEADER));
    if (!NT_SUCCESS(status)) {// Unable to charge quota for the clone header.
    ...
    // Lock down the paged pool since we will be holding the working set pushlock and cannot fault on it when this is held without potentially deadlocking.
    ...
    // Check for AWE, rotate physical, write watch and large page regions as they are not duplicated so fork is not allowed.
    if (CurrentProcess->PhysicalVadRoot != NULL) {
        RestartKey = NULL;

        // Don't allow cloning of any processes with physical VADs of any sort.
        // This is to prevent drivers that map nonpaged pool into user space from creating security/corruption holes if
        // the application clones because drivers wouldn't know how to clean up (destroy) all the views prior to freeing the pool.
    ...
    // Indicate to the pager that the current process is being forked.
    // This blocks other threads in that process from modifying clone block counts and contents as well as alternate PTEs.
    CurrentProcess->ForkInProgress = PsGetCurrentThread();
    TargetCloneRoot = ExAllocatePoolWithTag(NonPagedPool, sizeof(MM_AVL_TABLE), 'rCmM');
    ...
    // Only allocate a clone root for the calling process if he doesn't already have one (ie: this is his first fork call).
    // Note that if the allocation succeeds, the root table remains until the process exits regardless of whether the fork succeeds.
    if (CurrentProcess->CloneRoot == NULL) {
        CloneRoot = ExAllocatePoolWithTag(NonPagedPool, sizeof(MM_AVL_TABLE), 'rCmM');
        if (CloneRoot == NULL) {
            status = STATUS_INSUFFICIENT_RESOURCES;
            goto ErrorReturn2;
        }
    ...
    if (PhysicalViewCount != 0) {
        // The working set mutex synchronizes the allocation of the EPROCESS PhysicalVadRoot but is not needed here because no one
        // can be manipulating the target process except this thread.
        // This table root is not deleted until the process exits.
        if ((ProcessToInitialize->PhysicalVadRoot == NULL) && (MiCreatePhysicalVadRoot(ProcessToInitialize, TRUE) == NULL)) {
            status = STATUS_INSUFFICIENT_RESOURCES;
            goto ErrorReturn2;
        }
    ...
    while (Vad != NULL) {
        ...
        if ((Vad->u.VadFlags.VadType != VadAwe) &&
            ...
            if (NewVad == NULL) {
                // Unable to allocate pool for all the VADs.
                // Deallocate all VADs and other pool obtained so far.
                *VadList = NULL;
                status = STATUS_INSUFFICIENT_RESOURCES;
                goto ErrorReturn2;
    ...
    *VadList = NULL;
    ...
    // Initializing UsedPageTableEntries is not needed for correctness, but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    UsedPageTableEntries = NULL;
    PdeBase = NULL;

#if (_MI_PAGING_LEVELS >= 3)
    // Initializing these is not needed for correctness, but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    PageDirFrameIndex = 0;
    ...
    // Map the (extended) page directory parent page into the system address space.
...
#if (_MI_PAGING_LEVELS >= 4)
    // The PxeBase is going to map the real top-level.
    // For 4 level architectures, the PpeBase above is mapping the wrong page, but
    // it doesn't matter because the initial value is never used - it just serves as a way to obtain a mapping PTE and will be re-pointed correctly before it is ever used.
    ...
    // Initialize a page directory map so it can be unlocked in the loop and the end of the loop without any testing to see if has a valid value the first time through.
...
#else
    // All 4 page directory pages need to be mapped for PAE so the heavyweight mapping must be used.
    ...
    RtlCopyMemory(MdlPageDirectory + 1, PageDirectoryFrames, PD_PER_SYSTEM * sizeof(PFN_NUMBER));
    PdePhysicalPage = RootPhysicalPage;
    PdeBase = (PMMPTE)MmMapLockedPagesSpecifyCache(MdlPageDirectory, KernelMode, MmCached, NULL, FALSE, HighPagePriority);
#endif
    ...
    // MmWorkingSetList is not page aligned when booted /3GB so account for that here when established the used page table entry pointer.
    HyperWsl = (PMMWSL)((PCHAR)HyperBase + BYTE_OFFSET(MmWorkingSetList));
#endif

    // Map the hyperspace page (any cached page would serve equally well) to acquire a mapping PTE now (prior to starting through the loop) -
    // this way we guarantee forward progress regardless of resources once the loop is entered.
    NewPteMappedAddress = (PMMPTE)MiMapSinglePage(NULL, HyperPhysicalPage, MmCached, HighPagePriority);
    ...
    // Build a new clone prototype PTE block and descriptor, note that each prototype PTE has a reference count following it.
    ...
    // Insert the clone descriptor for this fork operation into the process which was cloned.
    MiInsertClone(CurrentProcess, CloneDescriptor);
    TempCloneMapping = NULL;

    // Examine each virtual address descriptor and create the proper structures for the new process.
    Vad = MiGetFirstVad(CurrentProcess);
    NewVad = FirstNewVad;
    while (Vad != NULL) {
        // Examine the VAD to determine its type and inheritance attribute.
        if ((Vad->u.VadFlags.VadType != VadAwe) &&
            (Vad->u.VadFlags.VadType != VadLargePages) &&
            (Vad->u.VadFlags.VadType != VadRotatePhysical) &&
            (Vad->u.VadFlags.VadType != VadLargePageSection) &&
            ((Vad->u.VadFlags.PrivateMemory == 1) ||
            (Vad->u2.VadFlags2.Inherit == MM_VIEW_SHARE))) {
            // The virtual address descriptor should be shared in the forked process.

            // Make a copy of the VAD for the new process, the new VADs are preallocated and linked together through the parent field.
            ...
            if (NewVad->u.VadFlags.NoChange) {
                if ((NewVad->u2.VadFlags2.OneSecured) || (NewVad->u2.VadFlags2.MultipleSecured)) {
                    // Eliminate these as the memory was secured only in this process, not in the new one.
            ...
            // If the VAD refers to a section, up the view count for that section.
            // This requires the PFN lock to be held.
            if ((Vad->u.VadFlags.PrivateMemory == 0) && (Vad->ControlArea != NULL)) {
                if (((Vad->u.VadFlags.Protection == MM_READWRITE) || (Vad->u.VadFlags.Protection == MM_EXECUTE_READWRITE)) && (Vad->ControlArea->FilePointer != NULL) && (Vad->ControlArea->u.Flags.Image == 0)) {
            ...
            if (Vad->u.VadFlags.VadType == VadDevicePhysicalMemory) {
                ...
                MiInsertNode((PMMADDRESS_NODE)PhysicalView, ProcessToInitialize->PhysicalVadRoot);
            }

            // Examine each PTE and create the appropriate PTE for the new process.
            ...
            do {
                // For each PTE contained in the VAD check the page table page, and if non-zero, make the appropriate modifications to copy the PTE to the new process.
                if ((FirstTime) || MiIsPteOnPdeBoundary(PointerPte)) {
                    ...
                    do {
#if (_MI_PAGING_LEVELS >= 4)
                        while (!MiDoesPxeExistAndMakeValid(PointerPxe, CurrentProcess, MM_NOIRQL, &Waited)) {
                            // Extended page directory parent is empty, go to the next one.
                            PointerPxe += 1;
                            PointerPpe = MiGetVirtualAddressMappedByPte(PointerPxe);
                            PointerPde = MiGetVirtualAddressMappedByPte(PointerPpe);
                            PointerPte = MiGetVirtualAddressMappedByPte(PointerPde);
                            if (PointerPte > LastPte) {
                                goto AllDone;// All done with this VAD, exit loop.
                            }
                        }
#endif
                        Waited = 0;
                        while (!MiDoesPpeExistAndMakeValid(PointerPpe, CurrentProcess, MM_NOIRQL, &Waited)) {
                        ...
                        while (!MiDoesPdeExistAndMakeValid(PointerPde, CurrentProcess, MM_NOIRQL, &Waited)) {
...
#if (_MI_PAGING_LEVELS >= 4)
                    // Calculate the address of the PXE in the new process's extended page directory parent page.
                    PointerNewPxe = &PxeBase[MiGetPpeOffset(PointerPte)];
                    if (PointerNewPxe->u.Long == 0) {
                        // No physical page has been allocated yet, get a page and map it in as a valid page.
                        // This will become a page directory parent page for the new process.

                        // Note that unlike page table pages which are left in transition, page directory parent pages (and page directory pages) are left valid and hence no share count decrement is done.
                        ReleasedWorkingSetMutex = MiLeaveThisPageGetAnother(&PageParentFrameIndex, PointerPxe, CurrentProcess);
                        MI_ZERO_USED_PAGETABLE_ENTRIES(MI_PFN_ELEMENT(PageParentFrameIndex));
                        if (ReleasedWorkingSetMutex) {// Ensure the PDE (and any table above it) are still resident.
                            MiMakePdeExistAndMakeValid(PointerPde, CurrentProcess, MM_NOIRQL);
                        }

                        // Hand initialize this PFN as normal initialization would do it for the process whose context we are attached to.

                        // The PFN lock must be held while initializing the frame to prevent those scanning the database for free frames from taking it after we fill in the u2 field.
                        MiBuildForkPageTable(PageParentFrameIndex, PointerPxe, PointerNewPxe, RootPhysicalPage, PfnPxPage, TRUE);

                        // Map the new page directory page into the system portion of the address space.
                        // Note that hyperspace cannot be used as other operations (allocating nonpaged pool at DPC level) could cause the hyperspace page being used to be reused.
                        ...
                        // If we are switching from one page directory parent frame to another and the last one is one that we
                        // freshly allocated, the last one's reference count must be decremented now that we're done with it.

                        // Note that at least one target PXE has already been initialized for this codepath to execute.
...
#if (_MI_PAGING_LEVELS >= 3)
                    // Calculate the address of the PPE in the new process's page directory parent page.
                    PointerNewPpe = &PpeBase[MiGetPdeOffset(PointerPte)];
                    if (PointerNewPpe->u.Long == 0) {
                        // No physical page has been allocated yet, get a page and map it in as a valid page.
                        // This will become a page directory page for the new process.

                        // Note that unlike page table pages which are left in transition, page directory pages are left valid and hence no share count decrement is done.
                        ReleasedWorkingSetMutex = MiLeaveThisPageGetAnother(&PageDirFrameIndex, PointerPpe, CurrentProcess);
                        ...
                        // If we are switching from one page directory frame to another and the last one is one that we freshly allocated, the last one's reference count must be decremented now that we're done with it.

                        // Note that at least one target PPE has already been initialized for this codepath to execute.
                    ...
                    // Calculate the address of the PDE in the new process's page directory page.

#if defined (_X86PAE_)
                    // All four PAE page directory frames are mapped virtually contiguous and so the PpePdeOffset can (and must) be safely used here.
                    ...
                    if (PointerNewPde->u.Long == 0) {
                        // No physical page has been allocated yet, get a page and map it in as a transition page.
                        // This will become a page table page for the new process.
                        ReleasedWorkingSetMutex = MiDoneWithThisPageGetAnother(&PageFrameIndex, PointerPde, CurrentProcess);
                        ...
                        MiBuildForkPageTable(PageFrameIndex, PointerPde, PointerNewPde, PdePhysicalPage, PfnPdPage, FALSE);

#if (_MI_PAGING_LEVELS >= 3)
                        MI_INCREMENT_USED_PTES_BY_HANDLE(UsedPageDirectoryEntries);
#endif

                        // Map the new page table page into the system portion of the address space.
                        // Note that hyperspace cannot be used as other operations (allocating nonpaged pool at DPC level) could cause the hyperspace page being used to be reused.
                        ASSERT(NewPteMappedAddress != NULL);
                        PointerNewPte = (PMMPTE)MiMapSinglePage(NewPteMappedAddress, PageFrameIndex, MmCached, HighPagePriority);
                        ASSERT(PointerNewPte != NULL);
                    }

                    // Calculate the address of the new PTE to build.
                    // Note that FirstTime could be true, yet the page table page already built.
...
#else
                    UsedPageTableEntries = (PVOID)&HyperWsl->UsedPageTableEntries[MiGetPdeIndex(MiGetVirtualAddressMappedByPte(PointerPte))];
#endif
#endif
                }
                ...
                if (PteContents.u.Long == 0) {
                    ...
                    if (Vad->u.VadFlags.VadType == VadDevicePhysicalMemory) {
                        ...
                        // The share count and valid count must be updated in the new page table page which contains this PTE.
                    ...
                    WorkingSetIndex = MiLocateWsle(VirtualAddress, MmWorkingSetList, Pfn2->u1.WsIndex, FALSE);
                    ...
                    if (Pfn2->u3.e1.PrototypePte == 1) {
                        ...
                        // A PTE is now non-zero, increment the used page table entries counter.
                        MI_INCREMENT_USED_PTES_BY_HANDLE(UsedPageTableEntries);

                        // Check to see if this is a fork prototype PTE, and if it is increment the reference count which is in the longword following the PTE.
                        if (MiLocateCloneAddress(CurrentProcess, (PVOID)Pfn2->PteAddress) != NULL) {
                            // The reference count field, or the prototype PTE for that matter may not be in the working set.
                            ...
                            if (PAGE_ALIGN(TempCloneMapping) == PAGE_ALIGN(CloneProto)) {
                                ...
                                // Release the working set pushlock before accessing paged pool to prevent potential deadlock as we may need a page for the pool
                                // reference and this process' working set may contain all the trimmable pages.

                                // Undo the actions taken above since the PTE may not be valid when we reprocess it.
                                ...
                                // Process this PTE again as the working set pushlock was released and reacquired.
                                continue;
                            }
                        }
                    } else {
                        // This is a private page, create a fork prototype PTE which becomes the "prototype" PTE for this page.
                        // The protection is the same as that in the prototype PTE so the WSLE does not need to be updated.
                        ...
                        // Transform the PFN element to reference this new fork prototype PTE.
                        ...
                        // Increment the share count for the page containing the fork prototype PTEs as we have just placed a valid PTE into the page.
                        PfnForkPtePage = MI_PFN_ELEMENT(ContainingPte->u.Hard.PageFrameNumber);
                        MiUpForkPageShareCount(PfnForkPtePage);

                        // Change the protection in the PFN database to COPY on write, if writable.
                        MI_MAKE_PROTECT_WRITE_COPY(Pfn2->OriginalPte);

                        // Clear the protection field in the WSLE as the protection is obtained from the prototype PTE and we want to ensure the hash deleted bit is cleared.
                        MmWsle[WorkingSetIndex].u1.e1.Protection = MM_ZERO_ACCESS;
                        TempPte.u.Long = MiProtoAddressForPte(Pfn2->PteAddress);
                        TempPte.u.Proto.Prototype = 1;
                        MI_WRITE_INVALID_PTE(PointerNewPte, TempPte);

                        MI_INCREMENT_USED_PTES_BY_HANDLE(UsedPageTableEntries);// A PTE is now non-zero, increment the used page table entries counter.
                        CurrentProcess->NumberOfPrivatePages -= 1;// One less private page (it's now shared).

                        ForkProtoPte += 1;
                        NumberOfForkPtes += 1;
                    }
                } else if (PteContents.u.Soft.Prototype == 1) {
                    // Prototype PTE, check to see if this is a fork prototype PTE already.
                    // Note that if COW is set, the PTE can just be copied (fork compatible format).
                    ...
                    if (MiLocateCloneAddress(CurrentProcess, (PVOID)CloneProto) != NULL) {
                        ...
                        if (PAGE_ALIGN(TempCloneMapping) == PAGE_ALIGN(CloneProto)) {
                            ...
                            // Release the working set pushlock before accessing paged pool to prevent potential
                            // deadlock as we may need a page for the pool reference and this process' working set may contain all the trimmable pages.

                            // Undo the used PTE increment above since it will be done again when we reprocess it.
                            MI_DECREMENT_USED_PTES_BY_HANDLE(UsedPageTableEntries);
                            UNLOCK_WS_UNSAFE(CurrentThread, CurrentProcess);
                            if (TempCloneMapping != NULL) {
                                MiUnlockPagedAddress(TempCloneMapping);
                    ...
                    if (MiHandleForkTransitionPte(PointerPte, PointerNewPte, ForkProtoPte)) {
                        continue;// PTE is no longer transition, try again.
                    }
                    ...
                    ForkProtoPte += 1;
                    NumberOfForkPtes += 1;
                } else {// Page file format (may be demand zero).
                    if (IS_PTE_NOT_DEMAND_ZERO(PteContents)) {
                        if (PteContents.u.Soft.Protection == MM_DECOMMIT) {
                            // This is a decommitted PTE, just move it over to the new process.
                            // Don't increment the count of private pages.
                            MI_WRITE_INVALID_PTE(PointerNewPte, PteContents);
                        } else {
                            // The PTE is not demand zero, move the PTE to a fork prototype PTE and make this PTE and the new processes PTE refer to the fork prototype PTE.
                        ...
                        // The page is demand zero, make the new process's page demand zero.
                        MI_WRITE_INVALID_PTE(PointerNewPte, PteContents);
                    }
    ...
    if (NumberOfForkPtes != 0) {
        // The number of fork PTEs is non-zero, set the values into the structures.
        ...
        // There were no fork PTEs created.
        // Remove the clone descriptor from this process and clean up the related structures.
        ...
        // Return the pool for the global structures referenced by the clone descriptor.
    ...
    // As we have updated many PTEs to clear dirty bits, flush the TB cache.
    // Note that this was not done every time we changed a valid PTE so other threads could be modifying the address space without causing copy on writes.
    // Too bad, because an app that is not synchronizing itself is going to have coherency problems anyway.
    // Note that this cannot cause any system page corruption because the address space mutex was (and is) still held throughout and is not released until after we flush the TB now.
    ...
    while (Clone != NULL) {
        ...
        do {
            ...
            // There are insufficient resources to continue this operation, however, to properly clean up at this point, all the clone headers must be allocated, so when the cloned process is deleted, the clone headers will be found.
            // If the pool is not readily available, loop periodically trying for it.

            // Release the working set mutex so this process can be trimmed and reacquire after the delay.
            UNLOCK_WS_UNSAFE(CurrentThread, CurrentProcess);
        ...
        *NewClone = *Clone;
        ...
        // Carefully update the FinalReferenceCount as this forking thread may have begun while a faulting thread is waiting in MiDecrementCloneBlockReference for the clone PTEs to inpage.
        // In this case, the ReferenceCount has been decremented but the FinalReferenceCount hasn't been yet.
        // When the faulter awakes, he will automatically take care of this process, but we must fix up the child process now.
        // Otherwise the clone descriptor, clone header and clone PTE pool allocations will leak and so will the charged quota.
    ...
    *CloneList = NULL;
    ...
    // Release the working set mutex and the address creation mutex from the current process as all the necessary information is now captured.
    UNLOCK_WS_UNSAFE(CurrentThread, CurrentProcess);
    ...
    if (CloneProtos != NULL) {
        for (i = 0; i < sizeof(MMCLONE_BLOCK) * NumberOfPrivatePages; i += PAGE_SIZE) {
            MiUnlockPagedAddress((PVOID)((PCHAR)CloneProtos + i));// Unlock the paged pool.
        }
    }

    // Attach to the process to initialize and insert the vad and clone descriptors into the tree.
    if (Attached) {
    ...
    // We are now in the context of the new process, build the VAD list and the clone list.
    Vad = FirstNewVad;
    ...
    while (PointerPpe <= PointerPpeLast) {
        ...
        // Update the WSLE for this page directory parent page (if we haven't already done it in this loop).
        ...
        if (PfnPdPage->u1.Event == NULL) {
            do {
                WorkingSetIndex = MiAddValidPageToWorkingSet(PointerPpe, PointerPxe, PfnPdPage, 0);
        ...
        if (PointerPpe->u.Long != 0) {
            ...
            do {
                WorkingSetIndex = MiAddValidPageToWorkingSet(MiGetVirtualAddressMappedByPte(PointerPpe), PointerPpe, PfnPdPage, 0);
                if (WorkingSetIndex != 0) {
                    break;
                }
    ...
    WslesFinished:
                 ...
                 if (CurrentProcess->PhysicalVadRoot != NULL) {
                     ...
                     do {
                         ...
                         do {
                             // The PDE entry must still be in transition (they all are), but we guaranteed that the page table page itself is still
                             // resident because we've incremented the sharecount by the number of physical mappings the page table page contains.

                             // The only reason the PDE would be valid here is if we've already processed a different physical view that shares the same page table page.

                             // Increment it once more here (to account for itself), make the PDE valid and insert it into the working set list.
                             if (PointerPde->u.Hard.Valid == 0) {
                                 PageFrameIndex = MI_GET_PAGE_FRAME_FROM_PTE(PointerPde);
                                 Pfn2 = MI_PFN_ELEMENT(PageFrameIndex);
                                 // There can be no I/O in progress on this page, it cannot be trimmed and no one else can be accessing it, so the PFN lock is not needed to increment the share count here.
                                 Pfn2->u2.ShareCount += 1;
                                 ...
                                 do {
                                     WorkingSetIndex = MiAddValidPageToWorkingSet(MiGetVirtualAddressMappedByPte(PointerPde), PointerPde, Pfn2, 0);
                                     if (WorkingSetIndex != 0) {
                                         break;
                                     }
                 ...
                 while (Vad != NULL) {
                     ...
                     if (!NT_SUCCESS(status2)) {
                         // Charging quota for the VAD failed, set the remaining quota fields in this VAD and all subsequent VADs to zero so the VADs can be inserted and later deleted.
                         status = status2;
                         continue;// Do the loop again for this VAD.
                     }
                 ...
                 while (Clone != NULL) {
                     ...
                     // Calculate the paged pool and non-paged pool to charge for these operations.
...
ULONG MiDecrementCloneBlockReference(
...
/*
Routine Description:
    This routine decrements the reference count field of a "fork prototype PTE" (clone-block).
    If the reference count becomes zero, the reference count for the clone-descriptor is decremented and if that becomes zero,
    it is deallocated and the number of processes count for the clone header is decremented.
    If the number of processes count becomes zero, the clone header is deallocated.
Arguments:
    CloneDescriptor - Supplies the clone descriptor which describes the clone block.
    CloneBlock - Supplies the clone block to decrement the reference count of. If NULL is supplied then just the clone descriptor is decremented.
    CurrentProcess - Supplies the current process.
    PteFlushList - Supplies a PTE list to TB flush if the PFN lock is going to be released here.
...
*/
{
    ...
    // Note carefully : the clone descriptor count is decremented *BEFORE* dereferencing the pageable clone PTEs.
    // This is because the working set mutex is released and reacquired if the clone PTEs need to be made resident for the dereference.
    // And this opens a window where a fork could begin.
    // This thread will wait for the fork to finish, but the fork will copy the clone descriptors (including this one) and get a stale descriptor reference count (too high by one) as our decrement will only occur in our descriptor and not the forked one.

    // Decrementing the clone descriptor count *BEFORE* potentially releasing the working set mutex solves this entire problem.

    // Note that after the decrement, the clone descriptor can only be referenced here if the count dropped to exactly zero.
    // (If it was nonzero, some other thread may drive it to zero and free it in the gap where we release locks to inpage the clone block).
    CloneDescriptor->NumberOfReferences -= 1;
    ASSERT(CloneDescriptor->NumberOfReferences >= 0);
    if (CloneDescriptor->NumberOfReferences == 0) {
        // There are no longer any PTEs in this process which refer to the fork prototype PTEs for this clone descriptor.
        ...
        // MiRemoveClone and its callees are pageable so release the PFN lock now.
        MiRemoveClone(CurrentProcess, CloneDescriptor);
        MutexReleased = TRUE;
        LOCK_PFN(OldIrql);
    }

    // Now process the clone PTE block and any other descriptor cleanup that may be needed.
    ...
    while ((CurrentProcess->ForkInProgress != NULL) && (CurrentProcess->ForkInProgress != PsGetCurrentThread())) {
        // Flush TBs prior to releasing the PFN lock.
        if ((ARGUMENT_PRESENT(PteFlushList)) && (PteFlushList->Count != 0)) {
            MiFlushPteList(PteFlushList);
        }
    ...
    if (ARGUMENT_PRESENT(CloneBlock)) {
        ...
        if (NewCount == 0) {
            ...
            if (CloneContents.u.Long != 0) {
                ...
                // Assert that the PTE is not in subsection format (doesn't point to a file).
                ...
                if (CloneContents.u.Soft.Transition == 1) {
                    ...
                    // Check the reference count for the page, if the reference count is zero and the page is not on the freelist, move the page to the free list, if the reference count is not zero, ignore this page.
    ...
    // Decrement the final number of references to the clone descriptor.
    // The decrement of the NumberOfReferences above serves to decide whether to remove the clone descriptor from the process tree so that
    // a wait on a paged out clone PTE block doesn't let a fork copy the descriptor while it is half-changed.

    // The FinalNumberOfReferences serves as a way to distinguish which thread (multiple threads may have collided waiting for the inpage
    // of the clone PTE block) is the last one to wake up from the wait as only this one (it may not be the same one who drove NumberOfReferences to zero) can finally free the pool safely.
    ...
    if (CloneDescriptor->FinalNumberOfReferences == 0) {
        ...
        // The working set lock may have been acquired safely or unsafely by our caller.
        // Handle both cases here and below.
        CurrentThread = PsGetCurrentThread();
        ...
        // If the count is zero, there are no more processes pointing to this fork header so blow it away.
        if (NewCount == 0) {
#if DBG
            ULONG i;
        ...
        // The working set lock may have been acquired safely or unsafely by our caller.
        // Reacquire it in the same manner our caller did.
    ...
    ASSERT((MutexReleased == FALSE) || (!ARGUMENT_PRESENT(PteFlushList)) || (PteFlushList->Count == 0));
...
*/
{
    ...
    // A fork operation is in progress and the count of clone-blocks and other structures may not be changed.
    // Release the working set mutex and wait for the address creation mutex which governs the fork operation.
    ...
    // The working set mutex may have been acquired safely or unsafely by our caller.
    // Handle both cases here and below, carefully making sure that the OldIrql left in the WS mutex on return is the same as on entry.

    // Note it is ok to drop to PASSIVE or APC level here as long as it is not lower than our caller was at.
    // Using the WorkingSetMutex whose irql field was initialized by our caller ensures that the proper irql environment is maintained (ie: the caller may be blocking APCs deliberately).
    UNLOCK_WS_REGARDLESS(CurrentThread, CurrentProcess, WsHeldSafe, WsHeldShared);

    // Acquire the address creation mutex as this can only succeed when the forking thread is done in MiCloneProcessAddressSpace.
    // Thus, acquiring this mutex doesn't stop another thread from starting another fork, but it does serve as a way to know the current fork is done (enough).
    LOCK_ADDRESS_SPACE(CurrentProcess);
    UNLOCK_ADDRESS_SPACE(CurrentProcess);
...
VOID MiUpControlAreaRefs(IN PMMVAD Vad)
{
    ...
    if ((ControlArea->u.Flags.Image == 0) && (ControlArea->FilePointer != NULL) && (ControlArea->u.Flags.PhysicalMemory == 0)) {
        FirstSubsection = MiLocateSubsection(Vad, Vad->StartingVpn);

        // Note LastSubsection may be NULL for extendable VADs when the EndingVpn is past the end of the section.
        // In this case, all the subsections can be safely incremented.

        // Note also that the reference must succeed because each subsection's prototype PTEs are guaranteed to already
        // exist by virtue of the fact that the creating process already has this VAD currently mapping them.
...
ULONG MiDoneWithThisPageGetAnother(IN PPFN_NUMBER PageFrameIndex, IN PMMPTE PointerPde, IN PEPROCESS CurrentProcess)
{
    ...
    if (*PageFrameIndex != (PFN_NUMBER)-1) {
        Pfn1 = MI_PFN_ELEMENT(*PageFrameIndex);// Decrement the share count of the last page which we operated on.
    ...
    *PageFrameIndex = MiRemoveZeroPage(MI_PAGE_COLOR_PTE_PROCESS(PointerPde, &CurrentProcess->NextPageColor));
    ...
    // Temporarily mark the page as bad so that contiguous memory allocators won't steal it when we release the PFN lock below.
    // This also prevents the MiIdentifyPfn code from trying to identify it as we haven't filled in all the fields yet.
...
ULONG MiHandleForkTransitionPte(IN PMMPTE PointerPte, IN PMMPTE PointerNewPte, IN PMMCLONE_BLOCK ForkProtoPte)
{
    ...
    // Now that we have the PFN lock which prevents pages from leaving the transition state, examine the PTE again to ensure that it is still transition.
    PteContents = *PointerPte;
    ...
    Pfn2 = MI_PFN_ELEMENT(PteContents.u.Trans.PageFrameNumber);// The PTE is still in transition, handle like a valid PTE.
    ASSERT(Pfn2->u3.e1.PrototypePte != 1);// Assertion that PTE is not in prototype PTE format.

    // This is a private page in transition state, create a fork prototype PTE which becomes the "prototype" PTE for this page.
    ForkProtoPte->ProtoPte = PteContents;
    ...
    // Transform the PFN element to reference this new fork prototype PTE.
    // Decrement the share count for the page table page which contains the PTE as it is no longer valid or in transition.
    Pfn2->PteAddress = &ForkProtoPte->ProtoPte;
    Pfn2->u3.e1.PrototypePte = 1;
    ...
    // Increment the share count for the page containing the fork prototype PTEs as we have just placed a transition PTE into the page.
    PfnForkPtePage = MI_PFN_ELEMENT(ContainingPte->u.Hard.PageFrameNumber);
...
VOID MiBuildForkPageTable(IN PFN_NUMBER PageFrameIndex, IN PMMPTE PointerPde, IN PMMPTE PointerNewPde, IN PFN_NUMBER PdePhysicalPage, IN PMMPFN PfnPdPage, IN LOGICAL MakeValid)
{
    ...
    PfnPdPage->u2.ShareCount += 1;// Increment the share count for the page containing this PTE as the PTE is in transition.
    UNLOCK_PFN(OldIrql);

    if (MakeValid == TRUE) {
#if (_MI_PAGING_LEVELS >= 3)
        // Put the PPE into the valid state as it will point at a page directory page that is valid (not transition) when the fork is complete.
        // All the page table pages will be in transition, but the page directories cannot be as they contain the PTEs for the page tables.
        TempPpe = ValidPdePde;
        ...
        // Put the PDE into the transition state as it is not really mapped and decrement share count does not put private pages into transition, only prototypes.
