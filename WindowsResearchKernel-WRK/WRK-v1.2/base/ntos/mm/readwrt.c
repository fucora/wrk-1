...
Abstract:
    This module contains the routines which implement the capability to read and write the virtual memory of a target process.
*/

#include "mi.h"


#define MAX_LOCK_SIZE ((ULONG)(14 * PAGE_SIZE))// The maximum amount to try to Probe and Lock is 14 pages, this way it always fits in a 16 page allocation.
...
ULONG MiGetExceptionInfo(IN PEXCEPTION_POINTERS ExceptionPointers, IN PLOGICAL ExceptionAddressConfirmed, IN PULONG_PTR BadVa);
...
*/
{
    ...
    if (BufferSize != 0) {// If the buffer size is not zero, then attempt to read data from the specified process address space into the current process address space.
        Status = ObReferenceObjectByHandle(ProcessHandle, PROCESS_VM_READ, PsProcessType, PreviousMode, (PVOID *)&Process, NULL);// Reference the target process.
        if (Status == STATUS_SUCCESS) {// If the process was successfully referenced, then attempt to read the specified memory either by direct mapping or copying through nonpaged pool.
            Status = MmCopyVirtualMemory(Process, BaseAddress, PsGetCurrentProcessByThread(CurrentThread), Buffer, BufferSize, PreviousMode, &BytesCopied);
...
*/
{
    ...
    BytesCopied = 0;
    Status = STATUS_SUCCESS;
    if (BufferSize != 0) {// If the buffer size is not zero, then attempt to write data from the current process address space into the target process address space.
        Status = ObReferenceObjectByHandle(ProcessHandle, PROCESS_VM_WRITE, PsProcessType, PreviousMode, (PVOID *)&Process, NULL);// Reference the target process.
        if (Status == STATUS_SUCCESS) {// If the process was successfully referenced, then attempt to write the specified memory either by direct mapping or copying through nonpaged pool.
            Status = MmCopyVirtualMemory(PsGetCurrentProcessByThread(CurrentThread), Buffer, Process, BaseAddress, BufferSize, PreviousMode, &BytesCopied);
            ObDereferenceObject(Process);// Dereference the target process.
        }
    }
...
)
{
    ...
    if (BufferSize > POOL_MOVE_THRESHOLD) {
        ...
        // If the completion status is not a working quota problem, then finish the service.
        // Otherwise, attempt to write the memory through nonpaged pool.
    ...
    // There was not enough working set quota to write the memory via direct mapping or the size of the write was below the pool move threshold.
    // Attempt to write the specified memory through nonpaged pool.
...
CompleteService:
    ExReleaseRundownProtection(&ProcessToLock->RundownProtect);// Indicate that the vm operation is complete.
    return Status;
}
...
*/
{
    ...
    // If the exception code is an access violation, guard page violation, or an in-page read error, then return the faulting address.
    // Otherwise. return a special address value.
    *ExceptionAddressConfirmed = FALSE;
    ExceptionRecord = ExceptionPointers->ExceptionRecord;
    if ((ExceptionRecord->ExceptionCode == STATUS_ACCESS_VIOLATION) ||
        (ExceptionRecord->ExceptionCode == STATUS_GUARD_PAGE_VIOLATION) ||
        (ExceptionRecord->ExceptionCode == STATUS_IN_PAGE_ERROR)) {
        // The virtual address which caused the exception is the 2nd parameter in the exception information array.

        // The number of parameters will be zero if an exception handler above us (like the one in MmProbeAndLockPages) caught the original exception and subsequently just raised status.
...
*/
{
    ...
    // Initializing BadVa & ExceptionAddressConfirmed is not needed for correctness but without it the compiler cannot compile this code W4 to check for use of uninitialized variables.
    BadVa = 0;
    ExceptionAddressConfirmed = FALSE;
    while (LeftToMove > 0) {
        ...
        } except(MiGetExceptionInfo(GetExceptionInformation(), &ExceptionAddressConfirmed, &BadVa))
        {
            ...
            // If the failure occurred during the move operation, determine which move failed, and calculate the number of bytes actually moved.
