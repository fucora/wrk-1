...
LONG Abort(HWND hWnd, char * Msg, DWORD Error)
{
    ...
    if ((Error == ERROR_INVALID_HANDLE || Error == ERROR_ACCESS_DENIED || Error == ERROR_FILE_NOT_FOUND))
...
LRESULT APIENTRY MainDialog(HWND hDlg, UINT message, UINT wParam, LONG lParam)
{
    char	label[MAX_PATH];
    SYSTEM_INFO sysInfo;
    DWORD	i, nb, ioctl;
